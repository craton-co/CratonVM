...
> **STATUS: APPLIED (2026-09-21, H0-1 N4).** §4's one-line nomination landed
> in `vm/src/runtime/interpreter/typecheck.rs`: the `synthetic_implements`
> guard now excludes `java/util/ImmutableCollections$Map*` and
> `$AbstractImmutableMap` alongside the pre-existing `java/util/Collections$`
> exclusion, citing this record by path in the comment. `RImmutableFactoryTypes`
> is registered in `regression-suite/run.sh`'s `CORE_CLASSES`.
>
> **MEASURED 2026-09-21**, binary built from `origin/dev` @ `16ab9ea3f`:
> `RImmutableFactoryTypes PASS` in all three arms (`--jdk-only` 140/140,
> `SUITE=all` 140/140, `SUITE=core` 97/97). §4's predicted table holds,
> row for row — `Map.of(...)`/`copyOf` correctly refuse `Collection` and
> `Iterable` under `--jdk-only`, and `List.of`/`Set.of`/`Arrays.asList` are
> unmoved.
>
> **The live residual §4 named up front — the separate `--real-jdk` face
> (`Map.of(k,v) instanceof AbstractMap`) — is now ALSO closed, by a different
> mechanism this record never proposed.** `H15-2` (2026-08-20) diagnosed why
> that face stayed wrong (the opcode never consulted the `getClass()` display
> alias `Class.isInstance` already used) and its patch, landed by `H18-1`/
> `H18-2` (2026-08-21), is what the same 2026-09-21 run confirms: this
> record's own §4 table predicted `Map.of(k,v) instanceof AbstractMap` under
> `SUITE=all` would stay "false — NOT fixed by this", and it is now `true`,
> without anything in *this* record's patch changing. See `H0-2`/`H15-2` for
> that mechanism.
...
```rust
    if obj_name.starts_with("java/util/Collections$")
        ...
        // docs/jdk-only/W8-C4-2-map-of-instanceof-collection.md
