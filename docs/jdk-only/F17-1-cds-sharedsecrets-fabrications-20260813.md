...
**Correction (F23-1, 2026-08-13): this section under-reported its own
retraction.** `dumpClassList(Ljava/lang/String;)V` and
`dumpDynamicArchive(Ljava/lang/String;)V` were also on F8's original list of
five off-surface registrations, and both are real `private,static,native`
methods on JDK 25's `CDS` — the same public-only-baseline blind spot as
`logLambdaFormInvoker`, not a second and third fabrication. This lane kept all
three registrations, so nothing was lost, but the retraction above is **three
registrations, not one**: `logLambdaFormInvoker`, `dumpClassList` and
`dumpDynamicArchive` are all real natives a public-only baseline could not see.
See `F23-1-the-guard-that-could-not-see-a-private-native-20260813.md` §3.1.
...
* **`scripts/jdk-baseline/generate.py`'s public-only filter** — §2.1 is a defect
  ...
  `javap -p` instead. **DONE — F23-1 (2026-08-13).** The filter was actually in
  the Java oracle (`JdkBaseline.java:221`/`:237`), not `generate.py`; all 32
  baselines were regenerated (format version 1 → 2), 528 previously-invisible
  rows are now visible, and `native_surface()` gives a two-way census a
  registrar can be checked against.
