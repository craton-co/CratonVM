...
| `-Xverify:all` (`XverifyMode::All` → `ClassManager::strict_verification`) | off | Strengthens, never weakens, in three places at once: forces strict mode for *every* class including the trusted JDK image (`verify_bytecode_strict`); withdraws the `CRATONVM_LOADER_AWARE_RESOLUTION` Pass-3 deferral two rows below; and stops `verifier_skip_eligible` skipping link-time Pass 2 for bootstrap classes. Wired 2026-08-10 — before that it parsed and was read by nothing, so `all` and `remote` behaved identically. | `ClassManager::strict_verification()`; a bootstrap-trusted class with a frameless branch target is rejected (`bytecode_verifier::tests::xverify_all_makes_a_trusted_class_reject_what_remote_accepts`) |
