...
| `-Xverify:all` (`verify_bytecode_strict`) | off | Forces strict mode for *every* class including the trusted JDK image. Strengthens, never weakens. | — |
...
| `CRATONVM_LOADER_AWARE_RESOLUTION` | **on** | When on, user-defined-loader classes skip the Pass-3 *type-state verdict* (gap 1 above). Structural bytecode verification still runs. | `debug!` at `class_manager.rs:5078`; per-method `FastPathVeto::IncompleteWalk` in the published type maps |
