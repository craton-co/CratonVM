...
|---|---|---|
| **Availability.** Applications that currently proceed through a compatibility substitution now fail. | An optional-dependency probe that used to return "absent" via an honest path now aborts the application if the class was load-bearing. | `--compatible` (deprecated alias `--real-jdk`) is the documented fallback, selecting the pre-2026-09-20 behaviour byte for byte; it was the launcher default until then and is still the default for every embedding entry point. See [`../jdk-only-migration.md`](../jdk-only-migration.md). |
...
| **False assurance in CI.** A strict job that falls back to `--compatible` on failure reports green while proving nothing. | Silent loss of the entire signal. | Every strict job passes `--jdk-only`, asserts the mode it ran in, and never retries under the fallback. |
