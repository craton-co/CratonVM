...
Round 4 MED #11 unresolved — `CpKey::Utf8(s.to_string())` still happens on every hit. **Fix:** change `CpKey::Utf8(Arc<str>)`, intern once via `cratonvm_types::intern_arc` (returns the same Arc on second call → free dedup); or use `HashMap::raw_entry_mut().from_key(s)` for probe-by-`&str`.
