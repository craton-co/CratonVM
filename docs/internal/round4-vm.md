...
```rust
if std::env::var_os("CRATONVM_DISABLE_JIT").map(|v| { ... }).unwrap_or(false) {
    ...
    return 0;
}
if std::env::var_os("CRATONVM_DBG_JIT_DISPATCH").is_some() { ... }
```
...
call. The `CRATONVM_DBG_JIT_DISPATCH` lookup happens unconditionally even
...
## 3. [CRIT] `CRATONVM_FRAME_TRACE` and friends called per frame push/pop

**File:** `vm/src/runtime/interpreter.rs:2239`, `:2379`, `:4606`, `:4911`,
`:10042`, `:10100`, `:10595`, `:12828`, `:12954` (and a dozen more)

```rust
if std::env::var_os("CRATONVM_FRAME_TRACE").is_some() { ... }
if std::env::var("CRATONVM_TRACE_SB_FILTER").is_ok() { ... }
```
...
each `CRATONVM_*` flag once at VM init into a `static AtomicBool` and
