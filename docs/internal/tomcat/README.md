...
| [c1-c2-single-arm-timing-sensitive-flakes-RESOLVED-20260912.md](c1-c2-single-arm-timing-sensitive-flakes-RESOLVED-20260912.md) | 5 | ✅ Two of the five "flakes" were real CratonVM defects, both fixed: `TestAccessLogValve` (native `Stream.distinct()` / `Set.of` duplicate checks were all-pairs `equals` scans, costing the first `Calendar.getInstance` per locale 1.5 s) and `TestRateLimitFilter` (a fresh `ConcurrentHashMap` lost a concurrent first insert, `computeIfAbsent` returned null). `TestFormAuthenticatorB`/`C` 10/10 standalone. `TestMapperPerformance` is a real throughput gap, split out to `../../known-issues/tomcat/mapper-performance-native-field-access-gap-20260912.md` |
