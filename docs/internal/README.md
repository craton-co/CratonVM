# `docs/internal/` — non-public engineering archive
...
Public `docs/` holds **current state only**. If a fact in here is still true of
the VM today, it belongs in a public document *as a plain present-tense
statement*, not as a pointer into this tree.

## Two hard rules

1. **No public document may link to a path under `docs/internal/`.** Those
...
| `fixed-suite-bugs/` | The same, for application/test-suite bugs. |
