...
The focused regression uses `Stream<String>.reduce(Integer, BiFunction,
BinaryOperator)`, so the emitted bytecode contains this exact descriptor and
the accumulator type differs from the stream element type.
