...
| `OpenTelemetryTracingAutoConfigurationTests` | `SdkTracerProvider` repeated shutdown lines, then silence | Not deadlocked. Completed in 1161.6s post-fix (1/36 failed, a `StackOverflowError` — see [`opentelemetry-contextstorage-early-init-recursion-cluster.md`](../../known-issues/springboot/opentelemetry-contextstorage-early-init-recursion-cluster.md)). This one needed the longest of the 5 — **still exceeds even a 900s carve-out**, needed 1400s in the runner's per-class override table. |
| `OpenTelemetryEventPublishingContextWrapperBeansTestExecutionListenerIntegrationTests` | Weakest-confidence member; hangs almost immediately, not after many cycles | **Confirmed different bug** — genuine unbounded recursion, not resource-cycle-related at all. See [`opentelemetry-contextstorage-early-init-recursion-cluster.md`](../../known-issues/springboot/opentelemetry-contextstorage-early-init-recursion-cluster.md). |
