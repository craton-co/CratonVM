...
## Three further bugs found closing out the same test (stacked, in handshake order)
...
`ServerHttpsRequestIntegrationTests::checkUri()` still fails, now on a
`NoSuchMethodError: java/lang/String.getOption(I)Ljava/lang/Object;` inside
real `java.net.Socket.getLocalAddress()` bytecode — a VM method-dispatch bug
unrelated to TLS/SSLEngine. Tracked separately in
[`../known-issues/socket-getlocaladdress-getoption-nsme.md`](../known-issues/socket-getlocaladdress-getoption-nsme.md).
