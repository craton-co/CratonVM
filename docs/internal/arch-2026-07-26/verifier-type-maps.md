...
(`classloading/src/bytecode_verifier.rs`) walks every instruction of every
...
returned unit**. Verification is ON by default (`vm/src/config.rs:419`,
`skip_verification: false`), so the VM was paying for the analysis on every
class load and throwing away the answer.

Four subsystems re-derive that same information at runtime, badly:

| subsystem | what it does instead | file |
|---|---|---|
| value representation | every slot carries a runtime tag; `Value` is 16 bytes | `types/src/value.rs` |
| operand stack | a parallel `kinds: Vec<u8>` because a 64-bit `long` cannot be NaN-boxed | `vm/src/runtime/value_stack.rs:211` |
| GC root scan | scans runtime tags, and tags get lost — a JIT callee's object return reaching an interpreter local under a non-object tag — so it falls back to conservative scanning, which forces a non-moving collector | `vm/src/memory/roots.rs:118` |
| interpreter fast path | cannot be proven safe per method, so it is gated on a **package-name deny-list** whose own TODO names the fix: *"a per-method `unsafe_for_fast_path` flag computed at verification time"* | `vm/src/runtime/frame.rs:553` |
| JIT | re-infers types it was already handed | — |

## 2. What landed

A new module, `classloading/src/type_maps.rs`, plus ~50 lines of wiring in
`classloading/src/bytecode_verifier.rs` and a module declaration +
re-export in `classloading/src/lib.rs`.

**On by default.** No cargo feature. No `CRATONVM_*` env var. No opt-in and no
opt-out. `verify_bytecode` builds and publishes the maps for every class it
verifies, on the default build path. The only thing that stops maps being
produced is the pre-existing `--noverify` escape hatch, which stops
verification itself — and that case is explicitly representable (see §6).

### Files touched

| file | change |
|---|---|
| `classloading/src/type_maps.rs` | NEW — the whole module (~1080 lines incl. tests) |
| `classloading/src/bytecode_verifier.rs` | `verify_method` now returns `Option<MethodTypeMaps>`; `verify_by_inference` returns `MethodTypeMaps`; both walks feed a builder; `verify_bytecode_inner` publishes |
| `classloading/src/lib.rs` | `pub mod type_maps;` + re-export block |
| `classloading/src/verify_insn.rs` | **unchanged** — no change was needed |
...
freed, which is what makes `&'static MethodTypeMaps` sound (see §8).
...
consumer that would want it (the JIT) is already walking the bytecode and can
re-derive it; keeping it would have doubled the depth array.
...
2. Every `*load` / `*store` / `iinc` / `ret` operand — **and the `+1` upper
   half of every category-2 access** (`lstore n` writes `n` and `n+1`) — is
   `< max_locals` (`LocalIndexOutOfRange`). This is the precise property the
   H7 comment in `interpreter.rs` relies on: the fast-path handlers index
...
existing deny-list in `frame.rs` also excludes `java/lang/invoke/*` and the
...
|---|---|---|
| pc | 2 | `u16` while `code_length < 65536` (JVMS §4.7.3 mandates it); 4 only for out-of-spec synthetic bytecode |
...
### 7.1 GC root scan (`vm/src/memory/roots.rs`)

Today, per frame:

```rust
frame.scan_local_objects(&mut roots, &shared.heap);        // tag-driven
frame.stack.scan_object_refs(&mut roots, &shared.heap);    // tag-driven
if conservative_locals { frame.scan_locals_conservative(..); }
```

The precise replacement, per frame:

1. Resolve the map once:
   `type_maps_for(frame.class_id, frame.method_index)`, or
   `type_maps_for_named(frame.class_id, &frame.method_name, &frame.method_descriptor)`
   until the index is cached (see 7.3).
...
3. Otherwise, with `let m = maps.frame_map_at(pc)?`:
   ...
   * stack: `for i in m.stack_for_runtime_depth(frame.stack.len()).iter_set()`
     — slot `i` is a proven object reference.
   * Slots `frame.locals.len() > max_locals` tail: still conservative.
...
* **`frame.pc` semantics at a GC point.** The map is keyed on
  *instruction-start* pcs and describes the state *before* that instruction
  ...
  the correct query is the *previous* instruction start.
...
### 7.2 Interpreter fast path (`vm/src/runtime/frame.rs:553`, `interpreter.rs:7800`)
...
* **Step 1 (safe now).** Cache `safe_for_fast_path` on `Frame` at
  `Frame::new` and use
  `!frame.is_jdk_class && frame.verifier_fast_path_ok && !skip_verification`.
...
* **Step 2 (the actual win).** Drop `is_jdk_class` from the condition and rely
  ...
  chain) are handled elsewhere — the TODO at `frame.rs:501` describes them as
...
* Note `verifier_fast_path_ok` must default to **`false`** when the map is
  absent (`type_maps_for` → `None`), which subsumes the
  `skip_verification` check.

### 7.3 Frame representation

`Frame` should cache, at `Frame::new`:

* `method_index: u16` — the position in `Class::methods`, so the root scan uses
  the `O(1)` `type_maps_for` instead of the name lookup.
* `type_maps: Option<&'static MethodTypeMaps>` — a single `&'static`, resolved
  once per frame instead of per GC. Sound because the store leaks entries.
* `verifier_fast_path_ok: bool` — see 7.2.

That is three fields and ~6 lines in `Frame::new`; nothing else in the frame
needs to change for the first wave. The eventual payoff (dropping the 16-byte
tagged `Value` and the parallel `kinds: Vec<u8>`) is a later wave that depends
...
2. **Redefinition leaks.** `replace_class_type_maps` swaps the pointer and
   ...
   or deferring the free to a safepoint where no scan is in flight.
...
orchestrator builds after merging).
