...
| `jit/src/ir_lower.rs` | 8 | 3 | 5 | 0 | 0 | 0 | 4 | 2 |
...
**2026-08-03 update — cov-01.** The `ir_lower.rs` row moved from
`7 | 3 | 4 | 0 | 0 | 0 | 3 | 0` to the values above. Two separate changes are
folded into that:

* the eighth `HEADER_SIZE` is the guarded inline compact `getfield` added
  2026-07-31 (already in the site list above; the row here had not been updated
  with it, and the in-code inventory had);
* `emit_inline_getstatic` (cov-01) adds the fifth `SLOT_SIZE`, the fourth
  `FIELD_CELL_PAYLOAD32_OFFSET` and **both** `FIELD_CELL_PAYLOAD64_OFFSET`s —
  the `use` list and the site.
