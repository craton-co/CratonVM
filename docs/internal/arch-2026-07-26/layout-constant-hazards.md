...
| `jit/src/lib.rs` `StringBuilderFieldLayout::new` (new 2026-09-11) | `(HEADER_SIZE + body_off) as i32` — compact `StringBuilder` `count`/`value`/`coder` **payload** addresses | disp32 |
| `jit/src/lib.rs` `StringBuilderFieldLayout::new` (new 2026-09-11) | `(HEADER_SIZE + idx * SLOT_SIZE) as i32 + FIELD_CELL_PAYLOAD{32,64}_OFFSET` — the legacy cells for the same three fields; `value` is a reference (payload64), `count` and `coder` are int-category (payload32) | disp32 |
| `jit/src/x64/objects.rs` `emit_inline_tlab_newarray` (new 2026-09-11) | the array header it stamps: `class_id` at 0, `shape` at `ARRAY_LENGTH_OFFSET`, both mark-word halves at `MARK_WORD_OFFSET`, and the object end at `ARRAY_DATA_OFFSET` | **disp8**, SCREENED — the emitter refuses the site and keeps the helper when any of the three exceeds 127 |
| `jit/src/x64/objects.rs` `emit_sb_append_char_body` (new 2026-09-11) | `MOV R9D,[RDX+ARRAY_LENGTH_OFFSET]` capacity load and `MOV [RDX+R8+ARRAY_DATA_OFFSET],CL` element store on a `StringBuilder`'s `byte[]` | **disp8**, reachable only behind the screen above |
