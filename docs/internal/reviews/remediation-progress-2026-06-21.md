...
- **JNI cross-file HIGH** (`fix/jni-localref-arraypin` then `fix/jni-arraypin-forcecopy`, build-green): implicit JNI local-ref frame (`JniImplicitFrameGuard` bracketing the native dispatch, alongside the existing TLS `JniContextGuard`) so native-created local refs are GC roots; + `GetPrimitiveArrayCritical`/`Get<Type>ArrayElements` now **force-copy** — they hand native code a detached, native-endian copy (`is_copy=JNI_TRUE`), never a raw pointer into the live array body, so a moving GC can never dangle the native pointer. This is collector-agnostic (Generational + G1) and closes the UAF with **zero GC-internals surgery**. The refcounted **GC pin set** (`gc/src/pinned.rs`, `pin`/`unpin`) is **retained as a keep-alive primitive** (roots the source array until copy-back at Release; belt-and-suspenders, since the array is already kept alive for the call by `native_pin_roots` / the implicit local frame). **Residual RESOLVED:** the moving-collector no-relocation *consult* is no longer needed — and the earlier "one-line TODO at `gc::try_forward_object`" was in fact **unsafe** for a copying collector (`return old_ptr` leaves the object in from-space, clobbered after the space swap). Verified: `cargo build --workspace` green; jni.rs/pinned.rs/heap.rs fmt+clippy clean; `gc` pinned tests pass.
...
**Remediation status: COMPLETE** — 2/2 critical, all highs (incl. full GC-root cluster + JNI), all ~71 mediums, perf, S/M features (implemented), L/XL features (design+scaffold). Full workspace `cargo build` is 0-errors green (verified via forced full recompile). Remaining genuine follow-ups: StrictMath fdlibm bit-reproducibility (documented, not implemented); the workspace-wide pre-existing clippy/fmt debt (separate project); and executing the L/XL design docs. (The former "moving-GC pin consult one-liner" follow-up is **resolved** — JNI now force-copies, so no in-place pinning is needed; see the JNI bullet above.)
