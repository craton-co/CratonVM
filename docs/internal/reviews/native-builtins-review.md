...
| `src/phases_late.rs:12837-12885` | **HIGH** | `p98_read_jar_manifest` does NOT verify signed-JAR signatures — no `META-INF/*.SF` digest check, no signature-block verification against the manifest. Combined with `Class.getSigners()` (`src/lang_class.rs:9968-9994`) returning bytes from a `CodeSource` that was never validated, downstream code that trusts `Class.getSigners() != null` is misled. |
