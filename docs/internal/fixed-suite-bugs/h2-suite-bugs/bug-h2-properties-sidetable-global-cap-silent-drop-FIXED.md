...
`docs/internal/h2-suite-bugs/` to `docs/internal/fixed-suite-bugs/h2-suite-bugs/`;
every other file in that directory was carried over except this one, and the
gap then propagated silently through later merges since only one side of
each merge ever touched the path. Restored here, at the post-refactor
location, with its original content intact.)*
