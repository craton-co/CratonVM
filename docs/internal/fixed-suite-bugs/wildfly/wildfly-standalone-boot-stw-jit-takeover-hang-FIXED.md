...
clear evidence of continued progress afterward. Moving to `..`.
...
the same diagnostic signature) -- moved back to `../../../known-issues` accordingly. The historical
...
post-fix, versus ~80-90% before. Moved to `..` accordingly; see the bottom
...
`wildfly-gc-barrier-boot-hang-and-harness-fixes.md` (dev `bee86ff0`,
...
`wildfly-gc-barrier-boot-hang-and-harness-fixes.md` — the earlier,
...
- `CCE_CRASH` (6/15, 40%) — `ClassCastException: Object cannot be cast to
  ...
  *original* site of this exact signature was root-caused and fixed
  ...
  `..`) — that fix IS present in the binary used for this batch
...
`..`; the `CCE_CRASH` bucket above is a recurrence of the same bug class at
