...
which includes the `70154861` fix as an ancestor. Moved back to `../../../known-issues` accordingly.
...
- **Four separate, real, narrow bugs were found and fixed while chasing this residual — none of them close
  ...
  3. **A genuinely unrelated regression, not this bug family at all**: `d8092acb` (2026-07-14) accidentally
     ...
     `wildfly-standalone-boot-objectname-ca-array-npe-FIXED.md` (commit
  ...
  4. `invoke_virtual`'s lambda-dispatch decision point (`../../../../vm/src/vm/vm_exec.rs`) read `receiver`/lambda
     ...
     across it — `wildfly-invoke-virtual-lambda-sam-compat-stale-locals-FIXED.md`
...
- **Still OPEN.** Every fix above was verified not to change the residual's reproduction rate (matched
  ...
  precise-oop-map/shadow-stack infrastructure (`../../../feature-designs/precise-jit-maps-default.md`) and/or
...
**A4 cross-thread-STW-JIT-root-scan gap** (`../../../feature-designs/precise-jit-maps-default.md`, "Step 8 —
...
(`../fork6-fjp-multithread-jit-root-reclamation-FIXED.md`), but that fix's
