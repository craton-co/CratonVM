...
`query.hql.FunctionTests`, also on the affected list, has its own prior REFUTED history (`hib-query-hql-functiontests-translation-cluster-NOT-A-BUG.md`) — further evidence the affected-class list is a mixed bag of unrelated symptoms, not a single temp-table bug.
