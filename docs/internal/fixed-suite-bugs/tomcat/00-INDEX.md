...
| [ws-async-seq2](websocket-async-send-interframe-latency-CLOSED-20260803.md) | The gap between two async WebSocket messages, opened as "root cause not found". It is not in the WebSocket code: `clearHandler` hands the `SendHandler` to the container `ThreadPoolExecutor`, so SEQ2's 500 us budget contains **two** AQS handoffs — and every AQS-mediated handoff in the VM is 13-26x HotSpot (`Condition.signal`→`await` 96.9 us vs 7.4; `execute`→task 167 us vs 7.9), while `park`/`unpark` and monitor `wait`/`notify` are within 3-4x. The doc's own probes measured Semaphore/LockSupport/Object.wait — none of which is the primitive on the path. One real defect fell out and is FIXED: **the invokestatic inline cache had been globally suppressed since the 2026-07-04 `loader_aware_resolution` default flip** (`loader_specific_dispatch` set on *attempting* loader-aware resolution, not on selecting an owner), so `Thread.onSpinWait()` — which AQS calls up to 255 times per handoff — cost 807 ns instead of 109 ns | TestAsyncMessagesPerformance | perf | ✅ **CLOSED** (2026-08-03) — **the test still FAILS**; the residue is the per-call floor and was carried by [aqs-thread-handoff-latency](../../aqs-thread-handoff-latency-RETIRED-20260805.md) — RETIRED 2026-08-05, funnel half fixed (leaf natives, 3.9-10.5x), test still blocked by the two thirds of an uncontended lock pair nobody has attributed: [uncontended-reentrantlock-pair-mostly-unattributed](../../uncontended-reentrantlock-pair-mostly-unattributed-RETIRED-20260805.md) — ATTRIBUTED 2026-08-05, pair now ~1,200 ns; residual is [native-funnel-fixed-cost-is-the-remaining-wall](../../../known-issues/vm/native-funnel-fixed-cost-is-the-remaining-wall-20260805.md) |
| [ecj-opstack](../ecj-operandstack-corruption-jsp-compilation-500s-FIXED-20260821.md) | ECJ's own `OperandStack.pop()` threw `AssertionError: Unexpected operand at stack top` on every JSP compiled after that method tiered up, rendered by Tomcat as an HTTP 500. Not GC, not ECJ, not `java.util.Stack`: a direct-call site that also carries `invoke_info` reserves a cold-deopt argument copy ABOVE the popped argument slots, and the x64 single-pass backend pushed the call's return value from that moved spill cursor — `n` operand-stack slots too deep. Invisible inside a basic block; wrong code at the first branch target after the call, which re-derives depth from the bytecode. The dispatch-helper arm below it already did this reclaim; the two direct-call arms never did | TestEncodingDetector, TestJspDocumentParser, TestParser, TestJspCompilationContext, TestStandardContextResources, TestGenerator, TestJspConfig | FAIL/HANG | ✅ **FIXED 2026-08-21** (twice — see the page). The 2026-08-06 direct-call fix stands; the same SYMPTOM returned through the INLINER after `0f55466d0` made branchy callees spliceable, and `tableswitch`/`lookupswitch` were the one branch shape that did not canonicalise the shift away. 15/15 classes pass, 216 tests |
| [https-delegate](../httpsurlconnection-delegate-null-ssl-ocsp-cluster-FIXED-20260821.md) | `URL.openConnection()` ALLOCATES the concrete `sun.net.www.protocol.https.HttpsURLConnectionImpl` rather than constructing it, so the `DelegateHttpsURLConnection` its constructor would create is null — and every method that class declares is `getfield delegate; invokevirtual …`. `TomcatBaseTest.methodUrl` calls `setUseCaches(false)` before anything else, so the whole SSL/TLS + OCSP cluster died on the first line of the shared helper. Fixed by re-pointing each override at the SUPERCLASS body it overrides (`invoke_special_bytecode_only`), not by re-implementing 20 JDK methods. Same page: the `util.CookieFilter` classpath gap, and a `SharedSecrets` stand-in that was minted as `AnonymousObject$1` — an empty method table — making all five `JavaUtilJarAccess` natives inert and every webapp class load out of a JAR throw `NoSuchMethodError` | TestSsl, TestClientCert, TestSSLHostConfigCompat, TestOcsp*, +12 | FAIL | ✅ **FIXED 2026-08-21** — `delegate` NPE count 0 in all 16; 12/16 fully green, 274 tests. Three unrelated TLS defects it was masking are OPEN in `known-issues/tomcat/ssl-client-cert-renegotiation-and-ocsp-hang-20260821.md` |
| [my-fallback](gc-moving-young-persistent-nonmoving-fallback-regression-CLOSED.md) | "Throughput-wall recurrence #3": the `TestHostConfigAutomaticDeployment*` family and `TestNonBlockingAPI` HANG standalone because moving-young keeps falling back to the non-moving sweep. Neither half holds. All ten classes PASS (up to 32x HotSpot, no hang), and the GC-side work measures **0.4%** of the run: 101 minor collections, 0 major, 1.41 s of card refinement in 352 s, and `CRATONVM_NO_MOVING_YOUNG=1` changes nothing. The blocking obligation is not either condition the report guessed but `XT_HELPER_WINDOW` (83-91%) - a peer parked in a syscall with JIT frames below it. The real residual is BCEL class parsing (`ConstantPool.getConstant` 55% of interpreted time; 48.2 M native invocations per class run, two thirds of them `DataInputStream`), handed to the annotation-scan doc with its profile | 9x TestHostConfigAutomaticDeployment*, TestNonBlockingAPI | HANG (claimed) | ✅ **CLOSED** (2026-08-07) |
