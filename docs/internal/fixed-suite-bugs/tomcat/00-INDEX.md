...
| [30](30-hot-loop-jit-admission-bans-testmethodperformance-CLOSED.md) | Hot path fully interpreted — **re-derived and closed 2026-07-31**. All three named bans are settled: RBC.7's premise removed, the `osr_dead_mask` gate hidden behind it lifted, RBC.6 deliberately still narrow (re-widening costs a Spring wrong-locals defect and buys nothing here), the ctor `putfield` ban default-lifted. Loop control now runs at HotSpot parity; the residual is per-frame / native-call cost in the JDK charset chain, re-homed to the VM-wide baseline. Turned up a **pre-existing silent wrong-result bug** on the way — a javac slot reused across type categories | TestMethodPerformance | perf | ✅ **CLOSED** (residual of 24) |
