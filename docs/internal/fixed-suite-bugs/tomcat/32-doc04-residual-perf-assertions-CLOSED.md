...
| item | disposition |
|---|---|
| 32.1 | Moved to [jit-optimizing-tier-disabled-by-moving-young-default](../../../known-issues/jit-optimizing-tier-disabled-by-moving-young-default.md). Cause 1 fixed 07-30; the entire residual is the raw JIT-to-JIT direct-call edge that document re-gated on 07-31. No Tomcat work. |
...
prediction. Reopening that edge is not a Tomcat decision: its owning document
states the precondition (an unguarded callee frame must first be describable to
the root scan) and the acceptance gate (that Spring class, 14 consecutive runs
on default flags). 32.1 closes when that happens, and is tracked there.
