...
**Status:** FIXED 2026-07-09. The real-FJP `Fork6`/`Fork6Hard` A4 repro is retired on dev after fixing live-blocked STW accounting, `Unsafe.compareAndExchange*` witness semantics, and the real-FJP non-moving reference-local snapshot. The broader `GC_STRESS` residual document stays open for remaining inconsistent-header warning noise, but this run no longer produces the stale receiver / `ClassCastException` / `NoSuchMethodError` / underflow / crash / timeout signatures that kept A4 open. See [`docs/known-issues/gcstress-residual-corruption-faces.md`](../../known-issues/gcstress-residual-corruption-faces.md).
