...
| `keycloak-crypto-fips1402-cryptoprovider-serviceloader.md` | `Provider.containsKey(Object)`, `Provider.get(Object)`, and `Provider.getServices()` now mirror the provider side tables populated by `Provider.put`; Keycloak FIPS `ServiceLoader` resolves `org.keycloak.crypto.fips.FIPS1402Provider` (`providerCount=1`) and representative suite classes no longer fail at `CryptoInitRule.before`. |
