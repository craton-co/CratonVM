...
cargo build --release -p cratonvm-cli          # ~5 min cold, ~2 min incremental
...
2. Run `CRATONVM_DBG_LETSGO=1 CRATONVM_DISABLE_JIT=1 …` on both letsgo-eureka and WildFly. Capture the dispatch-trace dump on SEGV. Save dumps under `applogs/segv-traces/`.
...
3. Run demo with `CRATONVM_DBG_PBE=1`. The `[PBE]` lines name the actual failing setter (suspected: `setMetadataReaderFactory(null)` or a primitive `setOrder(int)` coercion).
...
5. Run insurance with `CRATONVM_DBG_MAIN_RETURN=1`. The `[MAIN-RETURN]` line tells us if main returned `Ok` / `Err::Internal(msg)` / silently. From there, decide between (a) JIT dispatch sink fix vs (b) Spring `ApplicationFailedEvent` intercept vs (c) `refreshContext` deep audit.
...
- Generic ByteBuddy dispatcher wraps any reflection-target failure. Get full stack trace under `RUST_BACKTRACE=full RUST_LOG=cratonvm_vm=trace` and identify the underlying cause.
...
timeout 25 "$CRATONVM" --java-home "$JDK" --Xmx 512m -c "$CP" kafka.Kafka
...
# Keycloak
timeout 30 "$CRATONVM" --java-home "$JDK" --Xmx 1g \
  --jar "$APPS_BASE/keycloak-26.2.4/lib/quarkus-run.jar" show-config

# Probes
"$CRATONVM" --java-home "$JDK" \
...
"$CRATONVM" --java-home "$JDK" -c "$APPS_BASE/cleaner_probe" CleanerProbe
```
