...
design — see `CRATONVM_SOFT_EXIT=1` to soft-return past it).

KC16 boot output today (commit `88d3f71`):

```
DEFAULT MODE:
  [cratonvm] stack-dump watchdog armed: will dump + abort after 45s
  INFO Block 2C: synthetic JBoss LogManager shim active
  [cratonvm] System.exit(1) called -- process terminating
  rc=0

WITH CRATONVM_SOFT_EXIT=1:
  [cratonvm] stack-dump watchdog armed: will dump + abort after 45s
  INFO Block 2C: synthetic JBoss LogManager shim active
  WARN [cratonvm] System.exit(1) soft-returned (CRATONVM_SOFT_EXIT=1)
  rc=0
```
...
with `CRATONVM_SOFT_EXIT=1` set — investigation for Session 102+.
...
Root cause (per agent's `CRATONVM_STRICT_SWALLOWS=1` trace): JDK 25's
...
- Session 100: ServerLogger CCE on `_$logger_en_US` (locale-pattern
  ...
  `CRATONVM_SOFT_EXIT=1` soft-return, RBIGDEC.1 partial.
...
1. **First post-soft-exit failure** — visible only with
   `CRATONVM_SOFT_EXIT=1` set; needs investigation. Likely the next
...
3. **RFJP.1** — JIT regalloc clobber on deeply-recursive
   `RecursiveTask<Long>.compute()`. Workaround: `CRATONVM_DISABLE_JIT=1`.
...
eliminated. With `CRATONVM_SOFT_EXIT=1`, main reaches normal completion
...
- System.exit(1) source identified + soft-return env shipped. Caller
  ...
  `CRATONVM_DBG_EXIT=1` (caller-chain dump) and `CRATONVM_SOFT_EXIT=1`
...
- Agent A (JDKModuleLogger NPE) — empty worktree, agent concluded "all
  ...
  Lesson: future prompts must require `CRATONVM_STRICT_SWALLOWS=1`
...
- RFJP.1 first attempt (Session 96 Agent 4) — diagnosed JIT regalloc
  clobber but couldn't ship fix. Workaround `CRATONVM_DISABLE_JIT=1`
...
2. **First post-soft-exit failure** — visible only with
   `CRATONVM_SOFT_EXIT=1` set; needs investigation, then a fix per the
...
`CRATONVM_STRICT_SWALLOWS=1` to capture the failing native by name; the
...
- **RFJP.1** — `pool.invoke(RecursiveTask)` divide-and-conquer at depth ≥10
  ...
  `CRATONVM_DISABLE_JIT=1` confirmed working in Session 96.
...
- **`BigDecimal.<clinit>` NPE on `signum`** — RESOLVED (boot path; arithmetic
  ...
  `CRATONVM_STRICT_SWALLOWS=1` still escalates).

Reproducer (unchanged):
```
target/release/cratonvm.exe --java-home "C:/Program Files/Eclipse Adoptium/jdk-25.0.2.10-hotspot" \
...
`compute()` Long-arithmetic at depth ≥10. Workaround: `CRATONVM_DISABLE_JIT=1`.
...
- `native-builtins/src/jboss_module_loader.rs::build_module_object`
  ...
  `CRATONVM_DBG_MCL=1` trace (now permanent, gated). Patched to
...
- **RKC16N.9** — `org.jboss.modules.Module.<clinit>` NPE. The original
  ...
  - new `obj_arg` backtrace diagnostic in `native-builtins/src/lib.rs`,
    gated on `CRATONVM_DBG_NULL_NATIVE`, for future "which native got
    null" investigations.
...
```
target/release/cratonvm.exe --java-home "C:/.../jdk-25.0.2.10-hotspot" \
   --Xmx 2g --jar /tmp/keycloak/keycloak-16.1.1/jboss-modules.jar -- \
   -mp /tmp/keycloak/keycloak-16.1.1/modules org.jboss.as.standalone \
   "-Djboss.home.dir=/tmp/keycloak/keycloak-16.1.1"
```
...
- `CRATONVM_STRICT_SWALLOWS=1` panics inside `common_superclass` →
...
Keycloak 16.1.1 (WildFly / JBoss-Modules) under current `target/release/cratonvm.exe`
...
`RUST_LOG=cratonvm_vm=trace` → `/tmp/kc16_d2_stderr.txt` (4.0 GB, 20.1 M lines, killed at 206 s).
