...
Status: RESOLVED (2026-07-10) — see "Fixed" update below. Moved to `docs/internal/`.
...
## 2026-07-10 update (later same day) — Fixed, via this doc's own suggested fix sketch

A separate, parallel session (working the gating `ThreadPoolExecutor.execute()` NPE from
`docs/known-issues/wildfly-domain-heap-corrupt-value-timeout.md`) independently converged on the
same root cause and, rather than the narrower per-method registry exemption the companion
`execute()` fix used, removed the `native-api/src/registry.rs` `class_name ==
"java/util/concurrent/ThreadPoolExecutor"` drop clause **entirely** (all methods, not just
`execute`) — see `docs/internal/threadpoolexecutor-execute-npe-on-ctl-regression-FIXED.md` for the
full writeup and the reconciliation with the companion `execute()`-only fix already on `dev`.

This doc's own fix sketch step 2 was correct: `native_es_shutdown`/the `shutdown` closures already
called `executor_has_real_workers` internally, so once the registration-time drop stopped hiding
them, they worked correctly out of the box — the only change needed there was swapping the
real-instance branch from a silent no-op to actually forwarding to real bytecode (via the new
`NativeContext::invoke_virtual_bytecode_only`, so a genuinely-real executor's `shutdown()` runs its
own real state-machine transition instead of being ignored). `submit(Runnable)`/`submit(Callable)`
got the same per-instance `executor_has_real_workers` treatment added to `native_es_submit_runnable`/
`native_es_submit_callable`. `invokeAll`/`invokeAny`/`submit(Runnable, T)` were not independently
reproduced or touched — flagging as still-unverified if anyone hits them on a synthetic receiver.

**Verified**: this doc's own repro (`Executors.newFixedThreadPool(2)`, `execute(() -> {})`,
`shutdown()`) — no NPE, clean exit — plus the ES-suite doc's `submit()`+`shutdown()` repro and a
genuinely-real `new ThreadPoolExecutor(...)` (`execute()`/`shutdown()`/`awaitTermination()` all run
real bytecode correctly, confirming `shutdownNow()`'s real worker-interruption semantics — the
original 2026-06-17 intent — still holds).
