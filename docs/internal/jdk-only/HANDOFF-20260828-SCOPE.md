...
> **RETIRED 2026-09-01 — campaign record, not an operating page.** Its three
> stated blockers are discharged (§0). Anything you need in order to *work* has
> moved to
> [`../../contributing/jdk-only-lane-operations.md`](../../contributing/jdk-only-lane-operations.md);
> what remains here is what was measured, by whom, and when. The known-red lists
> below are dated and were true on the day they were written — re-derive rather
> than trust one.
...
| --- | --- | --- | --- |
| **L5 reflection & class metadata** | **COMPLETE 2026-08-29; a residual round on 2026-08-30 closed its last residual and found nine more defects — see `the-cast-that-asType-performs-…`, `the-fix-that-changed-nothing-…` and the still-OPEN `varhandle-checks-neither-…` in `known-issues/jdk-only/`.** Original: **COMPLETE 2026-08-29** — dispatch worklist 483 rows / 20 fixed, PLUS its two recorded-open items and five more the probe found: 125 rows, **28 defects over 608 rows**, 1 residual (`invoke`'s reference-argument cast). Records: `L5-reflection-lane-complete-20260828.md` and `L5-residuals-module-packages-and-invokeexact-20260828.md` | `C:\craton\cratonvm\.claude\worktrees\h2-known-issues-206dee` | `claude/jdk-only-mode-handoff-09b48c` |
...
| **L4 `java.io` / `java.nio`** | **COMPLETE 2026-08-28** — 199 native-won triples, **1616 probe rows, 1615 identical in both modes**; 52 defects fixed and 8 shadows retired; 1 recorded residual (`FileInputStream.skip`). **RESIDUALS CLOSED 2026-09-10** — it was not a resolution finding: `native_fis_is_regular_file0` read its RECEIVER as its `FileDescriptor` parameter and answered false for every file, so `skip`'s own `if (isRegularFile())` chose the superclass branch. **4397 rows across thirteen probes, 0-diff in BOTH modes**, and the retirement dial's floor moved from `DIFF 2` to **0** on sixteen of eighteen families. The last two are ONE nomination with its order measured: `java/nio/file/Path` has to be minted as the concrete class before `FileSystemProvider` can be. Records are `L4-the-io-and-nio-worklist-49-defects-and-a-bounds-check-that-killed-the-vm-20260828.md` and `L4-residuals-the-dispatch-finding-was-a-boolean-20260910.md` | `/data/cvm-l4io-20260828` (Linux build host) | `claude/l4-io-nio-20260828` |
...
| **L3 `java.util` collections** | **CLOSED 2026-08-30** — 586 owning rows, 2530 probe rows in 23 probes, 0-diff in BOTH modes on 22 of them; the 23rd differs only on class NAMES. All 8 residuals and the companion record's 13 are fixed, including `ArrayDeque` fail-fast (closed by RETIRING the iterator, not by adding a check) and a `--jdk-only` crash on deserializing any `List.of`. Records retired to `internal/jdk-only/l3-java-util-collections-1879-rows-and-69-defects-20260828.md` and `internal/jdk-only/a-bound-method-reference-is-a-different-dispatch-door-20260828.md`; what the lane found and does not own is `l3-followups-the-carrier-identity-and-the-dead-registrations-20260830.md` | `/data/cvm-l3u-20260828` (Linux build host) | `claude/l3-util-collections-20260828` |
...
**RETIRED 2026-09-01. All three of its stated blockers are discharged, and this
records how, because the page named its own criterion and it is fair to hold the
retirement to it.** The criterion was: *"the two lanes above finishing and §5
being rehomed somewhere it stays visible — in that order."*

* **Phase 2 is adjudicated** (2026-08-30). Every one of the 270 classes on the
  shadow surface was armed individually — 34 load-bearing, 236 not — and the
  answer is that **the corpus cannot decide a retirement**: arming those 236
  together fails 54 of 118 vectors and breaks 35 of 78 probe families, with
  14 123 530 dispatches reaching the dial and **zero leaks**, so the retirement
  was complete and it still failed. One triple of 1477 survived the evidence and
  is retired. Record:
  `phase-2-adjudicated-the-corpus-cannot-decide-a-retirement-20260830.md`.
* **§4's two OPEN items are closed** (2026-08-29/30). The FFM carrier question
  was settled as a CONTRACT — the carrier is this VM's own allocation shape, so
  its class name is not comparable — and then confirmed a second time from the
  other side, when retiring the FFM natives to get the JDK's real class name
  took `FfmSegmentSweep` from 40 diffs to 181 and killed it at row 18.
  `KeyStore.getInstance("JCEKS")` is implemented, with the PKCS12-writes-JKS
  interop defect its own guard caught on the way in.
* **§5 is rehomed and stays visible.** The method, the traps, the landing
  protocol and what "done" means are in
  [`../../contributing/jdk-only-lane-operations.md`](../../contributing/jdk-only-lane-operations.md),
  which is permanent and linked from `INDEX.md` and `docs/jdk-only-migration.md`.
  What is left below it is dated by construction — which vectors were red on
  which day — and belongs with the campaign that measured it rather than on a
  permanent page. **Every item in it is now marked FIXED or CLOSED.**

**What this page is now.** A campaign record: eight lanes, the sizing that split
them, and the dated operational history of the fortnight they ran. Read
`../../contributing/jdk-only-lane-operations.md` for anything you intend to
*do*; read this for what was measured and when.

**What is NOT closed is in §2** — the 1477-triple surface is adjudicated, not
retired, and §2's four preconditions are what a retirement wave needs. That is a
question about the VM, not about this page, and it now lives on the Phase 2
record and in §7 of the operations page.
...
### OPEN, owned, do not duplicate — ALL CLOSED as of 2026-09-01

**Every row below is now struck through or marked closed.** The two that
survived longest were this page's §4 retirement blocker: the FFM carrier
contract and `KeyStore.getInstance("JCEKS")`. Kept in full rather than deleted,
because each row records who closed it and how, and a table of closed items is
how the next campaign avoids re-opening them.
...
| `Arena`/`MemorySegment` report an INTERFACE as an instance's class | **MEASURED TWICE, INDEPENDENTLY, AND THE TWO AGREE.** Behaviour is FIXED (9 defects, 199 rows) -- see `ffm-segment-surface-nine-behavioural-defects-and-the-interface-classed-family-20260829.md`, which is the record of this defect and sizes the identity residual across FIVE class families. The MECHANISM and the contract question are in `fixed-suite-bugs/jdk-only/arena-and-memorysegment-hand-out-an-interface-FIXED-20260901.md`: the carrier is minted through `try_ensure_synthetic_class`, the door `--jdk-only` refuses by design, so strict falls back to the interface. **CLOSED 2026-08-29/30.** The contract question was DECIDED — the carrier is the VM's own allocation shape, so its class NAME is not comparable and must not be 'fixed' by mimicking the JDK's — and the decision was applied: `FfmCarrierProbe` 104 rows 0-diff in both modes, asking `getClass().isInterface()` rather than the class name. Record: `the-ffm-carrier-is-the-vms-own-allocation-shape-20260829.md`. **Then confirmed from the opposite direction on 2026-08-30**: retiring the FFM natives DOES make `Arena.allocate()` hand back the real `jdk.internal.foreign.NativeMemorySegmentImpl` — and takes `FfmSegmentSweep` from 40 diffs to 181, dead at row 18 of 199, because the real implementation needs real state this VM does not keep. So the JDK's class name is not reachable by fabrication OR by retirement, which is the decision, measured twice from opposite sides. |
...
| ~~`KeyStore.getInstance("JCEKS")` unsupported~~ | **DONE 2026-08-29.** Implemented — provider row, advertised type, `write_jks_with_magic` for the `0xCECECECE` magic, and format selection by SPI class name. `KeyStoreTypeProbe` 50 rows 0-diff in both modes across four store types. It also surfaced an interop defect nobody had asked about: a PKCS12 store was WRITING JKS bytes. Worth noting the probe did NOT catch the missing engine surface — `every_advertised_keystore_class_has_an_engine_surface` did, because the probe's operations reach the `java/security/KeyStore` shim rather than the SPI. A differential probe and a registration guard ask different questions. |
| ~~`java/lang/StringBuilder` cluster~~ | **CLOSED by L2, 2026-08-29** — see the L2 row in §0. It closes `WORKER-3-NOTE-3` N1 and N2 and refutes its §5; the retirement itself is declined WITH A NUMBER (2.0x-3.4x), which is the outcome that note asked for. |
