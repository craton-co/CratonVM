...
| **Companions** | [`../jdk-only-migration.md`](../jdk-only-migration.md) · [`../jdk-only-native-review.md`](../jdk-only-native-review.md) · [`../known-issues/jdk-only/INDEX.md`](../known-issues/jdk-only/INDEX.md) · [`jdk-only-lanes/`](../known-issues/jdk-only-lanes/) · **§2b** for the parallel-campaign rules |
...
queue, and the merge order -- is **§2b**, rehomed here on 2026-09-12 from the
lane-0 page that held it for the nine-lane split of 2026-09-10. The
measurements of that campaign are in
[`../internal/jdk-only/`](../internal/jdk-only/); the rules are here, because
they outlived it.
...
`native-collections/src/lib.rs` is ~81 500 lines and hosts many families;
`native-builtins/src/lib.rs` is ~50 500 (measured 2026-09-12; both grow weekly,
so re-measure rather than quoting these). Several lanes will touch them at once.
...
## 2b. Running lanes in parallel: ownership, shared cells, the build queue

Rehomed here 2026-09-12 from `lane-0-integration-and-gates.md` §2-§6 when that
page retired. It is the same move, for the same reason, that this page records
in its own preamble: the coordination rules outlived the campaign that wrote
them, and leaving them in a lane page meant the lane page could not retire
without taking them out of circulation. The campaign's *measurements* went to
[`../internal/jdk-only/lane-0-integration-and-gates-RETIRED-20260912.md`](../internal/jdk-only/lane-0-integration-and-gates-RETIRED-20260912.md);
what a future parallel campaign needs is below.

**One lane integrates.** Call it L0 by convention. It owns what cannot be
parallelised: the shared cells, the merge order, and *what the number is*.
Every other lane reports a delta and never edits a constant.

### Ownership is a prefix set, and the sets must be disjoint

Assign each lane a set of class-name prefixes, derive the row counts from one
`--dump-native-registry --explain-jdk-only`, and publish the table with a
TOTAL that reconciles to the campaign population. A lane that wants an unowned
row **claims it by amending that table in its own commit**, which serialises
the claim through one file.

Three rules, each of which cost something:

* **A prefix is a string, and `java/lang/Class` is a prefix of
  `java/lang/ClassLoader`.** The 2026-09-10 table was computed that way and
  silently handed the `Class` lane all 27 `ClassLoader` rows, plus
  `ClassNotFoundException` and `ClassCastException`. A year's worth of readers
  would have inherited it. Write the explicit set, and when you add a prefix
  anywhere, check what else it is a prefix of. The same trap then recurred in
  a *survivors* script written by the person who wrote this warning — which is
  why the rule is here and not only in the retired page.
* **The unit of work for a cross-cutting registrar is the registrar.** One
  function looping a class table produces hundreds of rows spanning many
  lanes' prefixes: `register_throwable_subclass_natives` was 1,100 rows from 57
  call sites over 61 classes in seven lanes. Splitting it would have had seven
  lanes editing one `for` loop, each measuring a seventh of one behaviour
  change. Give such a registrar to one lane whole, and while it holds it, no
  prefix lane may retire a triple that registrar produces — *even one inside
  its own prefix set*. The carve-out test is "spans more than one LANE", not
  "more than one CLASS": applying the class rule instead moved 212 rows.
* **Unowned rows are frozen, not unassigned-by-accident.** Do not retire one.

### Shared cells: who may edit what
...
| same file — `RETIRED_SHADOW_TABLES` | the lane | append **your** table, same commit as the array |
| same file — `triple_is_retired_shadow()` | **L0** | never edit; it iterates the const |
| same file — `RETIRED_SHADOW_PREFIXES` | the lane | append **your** prefix in the same commit as your first entry |
| same file — the N-way disjointness test | **L0** | never edit; it must fail if you collide |
| `native-builtins/tests/stub_ratchet.rs` — the `BASELINE_*` constants | **L0** | **never edit.** Report your measured delta in your commit message |
...
| this page | **L0** | propose via your lane page |
| `apps/probes/L<N>*.java` | the lane | namespace your probes; `apps/` is gitignored, so `git add -f` |

### Adding your table: three appends, and the const is what keeps them honest

```rust
// 1. your sorted table, doc comment carrying its measurement
static RETIRED_SHADOW_L<N>_TRIPLES: &[(&str, &str, &str)] = &[ /* ... */ ];

// 2. one line in the const the predicate walks
static RETIRED_SHADOW_TABLES: &[&[(&str, &str, &str)]] = &[ /* ..., */ RETIRED_SHADOW_L<N>_TRIPLES ];

// 3. your prefix, if it is not already admitted
```

then your own sortedness, reachability and held-families tests. **You do not
touch the predicate or the disjointness test**, and that is a 2026-09-11
change worth understanding rather than just obeying.

The predicate used to be a chain of `||` arms, one per table, and the
disjointness test a hand-unrolled pairwise cascade. Both grew with the table
count, and both were *sources of truth that could disagree with each other*.
`origin/dev` shipped `RETIRED_SHADOW_TABLES` short **twice** — once at 7
listed against 9 consulted, missing `L1_HM` and `L1_JT` — meaning a table's
rows were retired by the predicate while a whole-population gate walked a list
that had never heard of them. The repair was to make the predicate *iterate*
the const:

```rust
RETIRED_SHADOW_TABLES.iter().any(|t| t.binary_search(&key).is_ok())
```

so the two lists cannot disagree, plus `no_triple_is_claimed_by_two_tables`
(one `BTreeMap` over the same const) and
`the_tables_const_lists_every_table_the_predicate_consults`. An O(n²) source
cascade paid one line at a time by each lane sounds like the honest cost; it
was the mechanism by which two lanes' tables went unwalked.

### Prefixes stay narrow, because the list is a second guard

`RETIRED_SHADOW_PREFIXES` is an early-out for the binary search, so a wider
list plus an empty table answers `false` for the same inputs — the inertness
claim is true, and the conclusion "L0 should pre-add every lane's prefixes" was
still wrong. The narrowness is a *deliberately redundant* guard:
...
`java/lang/ref/` is absent on purpose; `sun/nio/` is admitted only as
`sun/nio/fs/` and `sun/nio/ch/` because a package-scoped dial sweep scored
34/36 against retiring it. Pre-adding a broad `java/lang/` would keep every
test green **and** delete that guard for the next reader. So: append the
narrowest prefix that covers your entry, in the same commit as the entry; and
if you widen one that carries a note, beat that note's measurement in the same
commit. Forgetting is caught loudly — an entry outside every prefix makes the
predicate answer `false` for a row that is present, which the registry-driven
reachability test fails on. A triple outside every prefix is *invisible in a
workload*, which is the same thing a wrong row looks like.

### Why the ratchet constants belong to the integrating lane alone
...
above reality — three of them drifted by 3/3/12 that way before 2026-09-09,
and a sibling `BASELINE_INTRINSICS` sat one row stale for two weeks with the
gate green. **The constant is read off the merged tree, never computed.**
Report `stubs before -> after` from your own run and let L0 re-measure after
the merge; a re-measure is also a free consistency check, because the same
number has to land from both decompositions (dev's + yours, and yours + the
sibling lane's).

### Build queue — the binding constraint on parallelism

A release build is **17-61 minutes** on this host and the host is shared.
...
* **Batch your candidates.** Most of a lane's work needs no build at all: the
  candidate funnel runs off a dump from *any* recent binary, probe authoring
  and HotSpot oracle capture need no cratonvm build, and a source-scanning gate
  can be scored at two revisions with one binary. Prepare a whole wave, build
  once.
* **Prefer a runtime switch to a second build.** `CRATONVM_UNRETIRE_NATIVE_SHADOW`
  turns named retirement rows back off *by editing the table*, so a wave's A/B
  is one binary and two runs, with codegen, JDK and host load held constant.
  The dial (`CRATONVM_ENFORCE_NATIVE_SHADOW`) is **not** a substitute: it
  declines at a dispatch door and runs the native anyway where the receiver has
  no concrete body, which leaks — `reached=1027 yielded=901 leaked=126`,
  byte-identical on two binaries five weeks apart. A leaky dial can *reject* a
  candidate and can never *promote* one, because `armed == base` is exactly
  what a promotion looks like.
* **Never take a timing measurement.** This host's noise floor is worse than
  any effect you would claim: the same binary read 146 s and then 334 s on
  `RMapGcStress` in one session. Correctness deltas only, and run A/B arms
  concurrently rather than sequentially.
* **`RMapGcStress` needs `TIMEOUT=600`.** Under concurrent load it times out
  and the corpus silently reports one vector fewer.
...
2. Merge `origin/dev` before the acceptance measurement, not after. A
   measurement on an unmerged tree is not a measurement of what you will land.
3. Re-price every lever after merging — a lane that lands first can make your
   candidate already-retired or already-broken.
4. Verify a merge with **the whole gate set** (§5), not the quick subset. Merging
   commits that touch none of your files can still move your numbers.
...
7. Stage conflicted files **by name**. `git add -A` before a merge commit sweeps
   in unaccepted work — the stub ratchet's "total DOWN" column is what caught 22
   deletions that arrived that way.
