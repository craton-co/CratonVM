...
## A `<=` ratchet sitting BELOW its constant reports nothing, and the sibling did it for two weeks

`intrinsic_count_does_not_regress` asserts `intrinsic <= BASELINE_INTRINSICS`,
exactly like the stub ratchet, and it was measured on 2026-09-11 by reading the
line it prints on a **passing** run:
...
The stub side is tight. The intrinsic side is one row below its constant and
the gate is green, because a DECREASE passes silently. `BASELINE_INTRINSICS`
was frozen at 1387 on 2026-08-24 (`458ca46a7`, the `-11` that proved a
retirement) and nobody had read the printed number since.

**Read the live value off a passing run, for every `<=` ratchet you own.** That
is the whole detection mechanism: the assertion cannot tell you, the CI log
can. A green `<=` gate means "not above", and "not above" includes "drifted
below, and the constant now describes no tree".

### Why one missing `Intrinsic` is worth a look rather than a lowered constant

A missing `Intrinsic` has two readings and they want opposite responses:

* a reviewed exemption was **removed on purpose** — lower the baseline, name
  the row and say why; or
* a reviewed exemption was **lost by accident**, which is a regression with a
  blast radius bigger than the tag. `Intrinsic` is exempt from the `--jdk-only`
  shadow census *and* from the retirement retag in
  `NativeMethodRegistry::register`, so a row that reverts to `Bridge` re-enters
  the population a retirement wave can act on. A later wave may then retire a
  row that was exempt for a measured reason — `Class.getName` answering the
  internal form is the worked example of what that costs.

Re-freezing without knowing which of the two happened is what this page's own
doctrine forbids: the constant is a before-number, and the account is the point.

### Attributing a decrease: the per-file table, and what a source diff cannot see

The test prints `intrinsic-ratchet: <n>  <file>` per registering file on every
run, for the reason `synthetic_by_file` was added. **That table, not the total,
is the thing to keep**: a total says the population moved and never which
subsystem moved it.

A source diff is not a substitute, and the 1387 -> 1386 case shows why. Between
the freeze and the reading, `git diff` over `native-builtins/src` and
`native-api/src` finds **two added `NativeKind::Intrinsic` lines and zero
removed ones** — the count fell anyway. Three reasons a registration can leave
this census without any line mentioning `Intrinsic` changing:

* the rows come from **tables**, not from the kind argument. A site is
  `for (c, m, d) in [...] { register_with_kind(..., Intrinsic) }`, so removing
  one tuple removes one intrinsic and touches no line a kind-grep matches.
* a triple registered **twice** keeps one winner. A later `Bridge` registration
  of the same triple takes the slot, and the census counts slots —
  `overwrote` is the field that says so.
* a registrar call can drop out of `register_boot_path`, or move behind a
  `cfg`/feature, taking its whole file's rows with it.

So: compare the per-file tables from two runs, not two source trees. If you
only have one run, the honest move is to record the reading and leave the
constant alone — which is what lane 0 did here, deliberately, rather than
freeze 1386 and lose the question.
