...
**Which class library** — `JdkMode`, selected by `--compatible` (deprecated
alias `--real-jdk`) / `--synthetic-jdk`.
...
**Which substitutions are permitted** — `CompatibilityMode`. `compatible`
allows Bridges, Intrinsics *and* synthetic stubs; `jdk-only` makes the real
class bytes authoritative: no fabricated compatibility class, no
`SyntheticStub` registered or invoked, and a structured error instead of a
silent substitution. Strictness is a runtime policy, never inferred from a
Cargo feature or an environment variable.

**The `cratonvm` launcher has defaulted to `jdk-only` since 2026-09-20**, with
`--compatible` as the opt-out; every embedding entry point still defaults to
`compatible`. So the launcher's shipped default is no longer the hybrid it
was. The ratchet census measures both configurations:

| configuration | total registrations | `SyntheticStub` |
|---|---|---|
| `real-jdk`, `compatible` (`--compatible`; the launcher default until 2026-09-20, and still the embedding default) | 11,639 | **685** |
| `real-jdk`, `jdk-only` (the launcher default now) | 10,954 | **0** (737 registrations refused) |
