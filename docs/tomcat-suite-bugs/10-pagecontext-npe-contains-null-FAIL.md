...
3. **Boot-class resource loading (FIXED, this branch, `net_phase_e.rs`).**
   ...
   single-level `jar:file:` branch of `URL.openStream`, try the
   `classes/`-prefixed entry first for `.jmod` containers; also added a `jrt:`
   scheme arm (for jimage-based boot classpaths). Verified vs HotSpot:
