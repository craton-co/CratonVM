...
[completed follow-ups record](../internal/fixed-suite-bugs/gpu-offload-followups-20260711.md),
...
| e. (optional) `GpuProbe` `DOT_CHECKSUM` | Matches HotSpot, only when `GATE_REDUCTION=1` | Off by default. Reduction kernels (e.g. `dotReduce`) are fully analyzed and lowered to PTX today, but `try_dispatch`'s void-return gate means they always fall through to the CPU (follow-ups doc, item 1) — so this gate would trivially pass on a CPU-computed checksum and prove nothing. Flip it on once dispatch-side reduction read-back ships. |
...
| `GATE_REDUCTION` | `0` | Set to `1` to enable gate e once reduction dispatch ships. |
