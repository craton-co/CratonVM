...
Four of these have been found by reading, one at a time, years apart:
`ldc`/`ldc_w`/`ldc2_w` (AUDIT C31, 2026-05-24), `invokestatic` under
`ALLOW_INTRINSIC_CALLS` (2026-07-11), and — on one day, 2026-09-21 —
`newarray` under `ALLOW_ALLOCATION` and `areturn` under no annotation
at all. Each was admitted by the analyzer with no arm in `emit_op`.
The first two were closed by writing the missing arm; the last two
could not be, and that is the more interesting outcome.

**Finding one found the next.** `areturn` (0xB0) was not looked for.
It fell out of writing down *why* `newarray` could not be lowered —
the allocated array is the return value, so `areturn` must lower too,
so does it? It does not, and it never did. The difference is that
`newarray` needed `@GpuKernel(admit = ALLOW_ALLOCATION)` to be
reachable and `areturn` needs nothing: `ParamKind::from_field` maps
`[I` to `I32Array` for a return type exactly as for a parameter, and
0xB0 sits in the permitted `0xAC..=0xB1` band, so
...
was analyzed eligible and lowering-rejected, for anyone, at any time.
The annotated hole had a test pinning it and a doc page advertising
it; the unannotated one had neither, and was the wider of the two.
Both now reject from the analyzer — the array-return check sits with
the parameter-type check, from the descriptor, before the body scan.

**`newarray` is the case where the emitter is the wrong place to
look.** The analyzer's rule was that the allocated array is the
kernel's output buffer sized from a parameter — which means lowering
it is a *host* problem: allocate a device buffer from a runtime
argument, copy it back, materialise a Java array for the caller.
`KernelSignature::return_kind` and `lowering::build_param_list`
already express an array return (`ret_ptr` + `ret_len`); the VM
marshaller does not, and `SerializedResult::PrimitiveArray*` is never
populated on that path. So the third layer of Symptom 1b is not just
another list to keep in sync — it can be the layer that decides
whether an opcode is lowerable at all. `ALLOW_ALLOCATION` is now
accepted-and-inert, the reject is unconditional, and
`docs/gpu/annotations.md` says so to users. One IR would have made the
disagreement impossible to write; it would not have made this shape
lowerable.
