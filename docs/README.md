...
**The `cratonvm` launcher default since 2026-09-20.** `--jdk-only` asks the VM
to treat real JDK class bytes as authoritative — no fabricated compatibility
class, no synthetic-stub native registered or invoked — so that every
compatibility substitution CratonVM performs becomes a counted, attributed
event.

Enforcement is still at stage 1 of 4, so a program that depends on a
substitution may fail under the default and pass under **`--compatible`**
(deprecated alias `--real-jdk`); that difference is the measurement, not a bug
in either mode. Every *embedding* entry point — `libcratonvm`,
`cratonvm-embed`, `VmConfig::default` — still defaults to `compatible`, on
purpose: a host that passed no policy argument has not asked for a stricter
one and has no flag of its own to opt back out with.
