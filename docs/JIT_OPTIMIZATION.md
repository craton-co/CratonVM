...
### Status after JIT review round 9 (2026-09-19)

Round 9 ran eleven waves of parallel review-and-fix lanes on 2026-09-18..19 and was
merged into `dev`. It fixed about 520 defects and moved 91 pages to
`docs/internal/fixed-bugs/*-FIXED-20260918.md`. Each lane's report is in
`docs/internal/jit-review-r9/NOTES-w<N>-<lane>.md`. What is still open, with evidence and
next steps, is indexed in
[`docs/known-issues/jit/jit-review-round9-remaining-work-20260918.md`](known-issues/jit/jit-review-round9-remaining-work-20260918.md).

**Wrong-code fixes that had reached users** include:
- floating-point spills across calls;
- an OSR inner-loop bounds-check guard;
- `synchronized` fast-door lost updates;
- private `invokeinterface` bound to a receiver's namesake.

**Defaults that changed in round 9**, all measured interleaved and all with kill switches:

| Flag | Default | What it does | Measured |
|---|---|---|---|
| `CRATONVM_JIT_CYCLE_EDGE_CELL` | ON | mutual-recursion cycle edges call through a cell instead of dispatch | `MutualRec` 970 -> 163 ms |
| `CRATONVM_JIT_RETIRE_CELL` | ON | an in-loop direct call goes through a cell that eviction clears | `TryLoop throw` 8 500 -> 2 400 ms |
| `CRATONVM_JIT_OSR_GUARD_EXIT_CHARGE` | ON | a guard exit out of an OSR body is charged to the guard's real reason, so a failing guard is withdrawn | `TryLoop throw` 2 400 -> 80 ms, `TryLoopRefl` 8 800 -> 89 ms |
| `CRATONVM_JIT_OSR_REENTRY_MEMO` | ON | a validated per-thread memo for OSR re-entry after a caught exception | -0.25-0.3 us per caught exception |
| `CRATONVM_JIT_INSTANCE_SELF_CALL` | ON | a statically bound private instance self-call is a direct `CALL` | `MutPriv self` 2 800 -> 190 ms |
| `CRATONVM_JIT_DIV_GUARD_RETIER` | ON | a hot IR division guard that keeps deopting re-tiers the method to single-pass | `ExcLoop callee` 4.4 -> 2.9 us |
| `CRATONVM_NO_IR_BOX_UNBOX_FOLD` (kill switch) | fold ON | `Integer.valueOf` read back only by `intValue`/`longValue` is deleted | `BoxProbe bu` 660 -> 4 ms |
| `CRATONVM_JIT_IR_CARRY_RCX_TWIN` | ON | a loop-carried value computes straight into RCX | neutral on ArithProbe |
| `CRATONVM_JIT_IR_VOLATILE_FIELDS` | ON | the IR tier admits volatile fields and fences a volatile store (`MFENCE`); since wave 10 also in spliced callees | `VolSplice vol` 295 -> 65 ms (the wave-10 spliced-callee half) |
| `CRATONVM_ZGC_READABLE_GIVE_BACK` | ON | ZGC returns pages readable, so the JIT's announce path stays on | Binary Trees 5.2 -> 2.0 s |
| `CRATONVM_ZGC_PRISTINE_CHUNKS` | ON | a never-written TLAB chunk skips its `memset` | helps only before the first GC |
| `CRATONVM_OMIT_STACK_TRACE_IN_FAST_THROW` | **OFF** | HotSpot's `OmitStackTraceInFastThrow` | 2x on caught implicit exceptions; breaks the `RExceptions` vector (owner's decision) |
| `CRATONVM_ZGC_LAZY_TLAB_ZERO` | OFF | zero a TLAB chunk a step ahead instead of at carve time | neutral |

**Biggest remaining gaps vs HotSpot** (details on the pages indexed above):
- a caught implicit exception costs about 3 us (HotSpot under 1 us);
- `ConcurrentHashMap.get` is about 180x slower (the generic native call per `get`);
- an `Integer` box that escapes pays a helper allocation;
- the optimizing OSR body of a long counted loop is slower than the single-pass body;
- the ZGC TLAB chunk `memset`;
- the AArch64 backend still refuses field and array access.

### Module size

> **Current, 2026-09-19** (`wc -l`, after round 9 and the merge with `dev`):
> `jit/src/**/*.rs` is 354,496 lines (244,308 in the 52 top-level files). The
> largest files are `ir_lower.rs` 41,553, `lib.rs` 34,685, `ir.rs` 20,533,
> `ir_optimize.rs` 13,666, `aarch64_backend.rs` 11,588 and `regalloc.rs` 11,608.
> `jit/tests/` is 29,999 lines in 50 files. `vm/src/jit/` is 53,432, and
> `vm/src/runtime/interpreter/jit_bridge.rs` is 16,634.
>
...
### Register residency in the optimizing tier — BUILT AND DEFAULT-ON

> **Corrected 2026-09-16.** The heading read "built, verified, still opt-in",
> and the section below still argues from that. It is wrong in the direction
> that matters: `linear_scan_enabled` (`jit/src/ir_lower.rs`) has been
> **DEFAULT ON since 2026-09-02**, with `CRATONVM_JIT_IR_LINEAR_SCAN=0` as the
> kill switch, and its own comment gives the reason — the enumeration
> disagreement that declined the file on every array-touching method was
> reconciled the same day.
>
> The closing paragraph of this section, *"a loop counter and a loop
> accumulator are `Op::Phi` at the loop header, and phis are excluded"*, is
> wrong for the same reason. **Phis are admitted**, behind
> `ir_phi_residency_enabled` (`CRATONVM_JIT_IR_PHI_RESIDENCY`, default on
> *within* the outer flag so the two can be A/B'd apart);
> `regalloc::ir_op_defines_value` and `ir_lower::op_defines_result_slot` both
> list `Op::Phi`, `LiveModel::release_phi_pins` exists, and `emit_phi_copies`
> publishes into the phi's register at every incoming edge as well as writing
> the home word.
>
> The **measured table below therefore describes a configuration that is no
> longer the default**, and re-taking it is the open work. Read it as the
> problem statement it was, not as the current state.
>
> What IS still refused, and by type rather than by heuristic: a `Ref`-typed
> phi. A GC root walk reads frame slots only (`OopMapEntry` carries
> `frame_slot_offsets` and nothing else), so no reference may be
> register-resident at a safepoint, and a phi is published on the *edge* —
> outside the per-node audit that `invalidate_ref_residency` performs. The
> refusal is unconditional, including under `CRATONVM_JIT_IR_REF_RESIDENCY=1`.

### The section as written when residency was opt-in
...
residency off. And the assertion that would catch an OSR trampoline entering
past a prologue that saves registers now covers the GP band too.

> **Corrected 2026-09-16.** That last sentence used to open "And the IR tier
> still publishes no OSR entry table". It does publish one, and had for five
> days when the sentence was written: `ir_lower::emit_osr_entry_stubs`
> (`CRATONVM_JIT_IR_OSR_ENTRY`, default ON since 2026-09-05) fills
> `CompiledMethod::ir_osr_entries`, and `ir_osr_enter` is its ABI. The
> correction matters here specifically because the sentence is load-bearing for
> the register-residency argument: "no OSR entry exists" and "an OSR entry
> exists and must not enter past the prologue that saves RBX/R12–R15" are
> different obligations, and only the second is real. It is discharged by
> `emit_osr_entry_stubs` building the frame itself rather than being entered
> through the single-pass `osr_trampoline` — see `ir_osr_entry_addr`'s doc for
> the two ABIs side by side.

### The optimizing tier's OSR reach — wired, and narrower than the gate

Also corrected 2026-09-16: `ir_lower`'s own `ir_osr_entry_enabled` doc said
"nothing in the VM calls these yet". The consumer is
`vm::runtime::interpreter::jit_bridge::try_osr`, gated by
`CRATONVM_JIT_OSR_OPTIMIZING` and **default ON since 2026-09-05**. It asks
`compile_optimizing_artifact` for a body, caches it per `(class, method,
entry_pc)`, and enters it with `ir_osr_enter`; `compile_osr_artifact` — the
single-pass OSR compile — is the fallback beneath it, not the only route.

The real limit is the door's admission, which is
`ir_osr_entry_addr(pc).is_some() && ir_osr_sentinel_free` — **both**.
`ir_osr_sentinel_free` means the body emits no deopt stub and no
call-exception stub, so **any method containing a call fails it**. That is
most methods, which is why `jit_bridge` keeps a negative cache
(`OSR_OPTIMIZING_REFUSED`) to stop the door paying a full optimizing compile
per OSR attempt for a body it will always refuse.

Since 2026-09-16 `lower_inner` also *skips emitting* the entry stubs for a
body that is not sentinel-free, since the only door will refuse it: that is
bytes, a `JMP` patch, a published triple and a `build_live_model` pass saved
on the common case. `ir_osr_entry_skipped_census()` counts the skips and
`CRATONVM_JIT_IR_OSR_ENTRY_ALWAYS=1` restores unconditional emission for an
A/B.

So the honest statement of the gap is not "OSR never reaches the optimizing
tier" but **"OSR reaches the optimizing tier only for call-free loop
bodies"**, and widening it means giving an entered optimizing body a way to
survive its own deopt sentinel — the `OsrEntryPlan::resume_after_exit` proof
the single-pass path has and this one does not.
...
| ZGC VM-thread TLAB (the chunk the inline bump bumps) | on since 2026-09-18 (JIT review round 9 wave 5, measured: bintrees ~6x, ZgcTlabStress faster, PutScale level) | `CRATONVM_ZGC_JIT_TLAB=0` turns it off |
...
## Built, tested, parked on a measurement — the list, so it stops being invisible

Two optimizations in this backend are complete, soaked at the unit level, and
**default OFF for want of a number rather than for want of code**. They are
collected here because a capability that is off by default and mentioned only in
a doc comment is indistinguishable, from outside, from one that does not exist.

> This table said "three" and listed `CRATONVM_JIT_IR_LINEAR_SCAN` until
> 2026-09-16. That entry was wrong — the flag has been default ON since
> 2026-09-02 — and it was wrong because the table was written from the
> register-residency section above, which was itself stale. The lesson is the
> one this document keeps re-learning: a flag's default is a fact about
> `*_enabled()`, and reading it out of prose reproduces the prose's age.

| flag | default | what it turns on | what the flip needs |
|---|---|---|---|
| `CRATONVM_JIT_REASSOC` | off | affine strength reduction (`ir_optimize::reassociate_affine`) — collapses an unrolled affine recurrence `x = x*c1 + c2` (×N) into one `k*root + c` | **a quiet-host A/B — correctness is no longer the open question.** See below |
| `CRATONVM_SCALAR_DEOPT` | off | a scalar-replaced object survives a guard failure by materialising from a `FrameValue::VirtualObject` descriptor, instead of forcing a whole-method re-run | **not a measurement — see below.** Its soak RAN, and found the flag green and *inert* |


### The `CRATONVM_JIT_REASSOC` correctness soak ran on 2026-09-16

Added 2026-09-16. The row above used to say the flip needed "a soak"; that
conflated two different soaks, and only one of them is still outstanding.

`cratonvm-difftest fuzz-jit --seed 300 --count 150 --modes nojit,direct-emit,ir-jit`
with `CRATONVM_JIT_REASSOC=1` ran 150 programs and reported 3 failures. All
three (seeds 335, 345, 350) were the **interpreter's** `dup2_x2` bug fixed the
same day — each panicked at its own `dup2_x2` with the oracle unusable, and
none was a reassociation miscompile. See
`internal/fixed-bugs/interpreter-dup2_x2-missed-jvms-form-3-FIXED-20260916.md`.

So: reassociation is differentially clean over 150 programs against the
interpreter. What the flip still needs is the **performance** number — the
quiet-host A/B on compute kernels — not a correctness argument. That is a
materially smaller ask than the row implied, and worth writing down, because
"needs a soak" is the kind of phrase that keeps a finished feature parked
indefinitely once nobody remembers which soak was meant.


### `CRATONVM_SCALAR_DEOPT` is a different case from the other two, and saying
### "it needs a measurement" understates it

Corrected 2026-09-16, after reading the soak instead of the flag. The
2026-08-27 gauntlet soak (`internal/performance/scalar-deopt-gauntlet-soak-20260827.md`,
summarised in `feature-designs/activate-ir-optimizer.md`) did run, and its
finding was not "we could not tell". It was:

> the flag green and **inert**: 30,392 allocation-bearing IR compiles across
> netty and hibernate produced ZERO scalar replacements, so it emitted no
> descriptor anywhere — including on the probe written to exercise it. Flipping
> it on that evidence would record a soak that never ran the feature.

So the blocker is one level down. The question is not "is guard-surviving
materialisation worth it" but **"why does escape analysis scalar-replace
nothing on real workloads"**, and until that has an answer this flag cannot be
soaked at all — there is nothing for it to do.

That question is answerable with the instrument already in the tree:
`ir_evidence`'s `Transform::ScalarReplacement` is noted whenever the optimizer
replaces one, so a run over netty/hibernate with the evidence census on says
directly whether the 2026-08-27 reading still holds. Note that `26c880f88`
("allocation elision fires by default") landed after that soak, so the reading
may have moved; nobody has re-taken it.

**The order of work this implies:** re-take the census; if EA still fires zero
times on those workloads, that is the defect and this flag is downstream of it.
Do not flip this flag first.

**The rule this table is written under.** This project does not flip a default
on the strength of a reading; every flip above the line got a soak
(`IR_CALL`/`IR_LONG`/`IR_FP`/`LICM`/`UNROLL` all name theirs). Nothing here
argues for changing that. What it argues for is that "parked" is a state
somebody has to be able to *see*, with the specific missing evidence named, so
that the next person with a quiet box knows exactly which three runs to do.

**How to tell a parked flag from a dead one.** A parked flag has a soak
condition in this table. A flag with no line here and no consumer is dead code —
`jit/src/pgo.rs` is the worked example, and `docs/jit/pgo-inlining.md` §1 is how
that gets written down.

---

## Escape analysis offers no scalar-replacement candidates — measured, 2026-09-16

This section replaces an inference this document and its neighbours had been
repeating: that the 2026-08-27 gauntlet soak showed escape analysis
"scalar-replaces nothing on real workloads", and that `CRATONVM_SCALAR_DEOPT`
was therefore blocked on a workload property. The finding is sharper than that
and sits one stage earlier in the pipeline.

**First, two instrument defects had to be fixed before the question could be
asked at all**, and both are worth recording because both would have produced a
confident wrong answer:

1. `ir_evidence::transform_census()` had **no caller**. The counter was
   correct and `pub` and nothing read it, so on a release binary the counts
   accumulated and were dropped at exit. Fixed with `exit_summary`, on both VM
   exit arms, under `CRATONVM_JIT_TRANSFORM_CENSUS`.
2. The `Transform::ScalarReplacement` note was attached to
   `escape_analysis::apply_scalar_replacement`, **which has no production
   caller** — every call site is a unit test in that module. The shipping path
   plans on the EA graph and applies on the IR graph (`lib.rs`,
   `plan_scalar_replacement` plus the victim splice). A counter on an
   unreachable applier reads zero whether or not the optimization is running,
   which is worse than no counter: it answers wrongly rather than not at all.

With both fixed, and a probe (`ScalarReplProbe`) written as the deliberate
mirror of `IrEscapeProbe` — every allocation consumed entirely within the
method that made it, no arrays, no statics, nothing that would refuse IR
admission — forced to the optimizing tier with `CRATONVM_JIT_FORCE_C2=1`:

```text
[jit-transform-census] scalar-replacement funnel: candidates=0 planned=0
[jit-transform-census]   scalar-replacement = 0
```

`candidates=0` is the reading that matters. **Escape analysis offers nothing
to replace.** The planner's refusal arms are not implicated, the applier is
not implicated, and no workload property is implicated — on the friendliest
input that can be constructed, the analysis produces an empty
`scalar_replaceable`. That is where anyone trying to flip
`CRATONVM_SCALAR_DEOPT` should start, and it is a different place from where
the parked-flag table has been pointing.

The `candidates` / `planned` pair exists so this stays answerable: a zero in
the first is a statement about the analysis, a non-zero first with a zero
second is a statement about the planner.

### Three successive refutations, and where the question now sits

Added 2026-09-16, after building two more instruments. Each stage refuted the
explanation the previous stage suggested, which is the only reason this is not
still confidently wrong.

**Stage 1 — `candidates=0`.** Established that escape analysis offers nothing.
Suggested the planner was innocent, which was right.

**Stage 2 — all thirteen refusal rows are ZERO.** The refusal census
(`escape_analysis::scalar_refusal_census`) counts every `ScalarRefusal` reason
at the single exit of `find_scalar_replacements`. Every row reads zero. That
**refutes both** explanations available from reading the module:

* *"Every `new` escapes to its own `<init>`"* — `Op::Call` does arg-escape every
  reference operand (`escape_analysis.rs`, the `Op::Call` arm), and a Java
  allocation is always `new / dup / invokespecial <init>`, so the reasoning is
  sound. It is not what is happening: `refused[escapes:arg-escape] = 0`.
* *"The refusal a hot loop hits"* — `ScalarRefusal::LoadNotAnswerable`'s own doc
  comment says so. Also not happening: that row is zero too.

Escape analysis is not rejecting these allocations for any reason. It is not
seeing an allocation to reject.

**Stage 3 — `analyses=2, allocation-nodes-seen=0`.** The scope census
(`scalar_scope_census`) separates "never ran" from "ran on a graph with no
allocation node". The analysis RUNS, on graphs containing zero
`Op::New`/`Op::NewArray`. Two further candidates are eliminated by direct
check:

* the IR→EA bridge maps allocations correctly (`ea_ir_bridge.rs`,
  `ir::Op::New => EaOp::New`, `ir::Op::NewArray => EaOp::NewArray`);
* the allocation sites are not taking the deferred/unresolved path —
  `[c2-supersede] deferred-new retries: held=0 spent=0` and `ir deferred
  carries: candidates=0`, so `new_info` is populated and `ir.rs`'s `0xbb` arm
  emits a real `Op::New` rather than planting `TrapCause::UnresolvedNew`.

And the allocating method IS admitted: `[ir] admission
ScalarReplProbe.hot(II)I: admitted to the optimizing pipeline`.

**What is left, stated precisely.** Five methods are admitted to the optimizing
pipeline and escape analysis runs one or two times, on graphs with no
allocation in them — i.e. not on `hot`. So the open question is no longer about
escape analysis at all: it is *why the EA block does not run for the admitted
method that allocates*. The block is guarded by `cached.exception_table
.is_empty()` (`lib.rs`), and admission is documented elsewhere in this file as
a PREDICTION — a method the chain admits can still fall back to the
single-pass backend inside the pipeline, in which case no IR-tier pass runs for
it at all. Either would produce exactly this reading, and distinguishing them
needs one more counter (which method each analysis ran on), not more reasoning.

That is a deliberate stopping point. Every explanation offered so far has been
refuted by the next measurement, and the honest move is to note that the next
step is a counter rather than to offer a fourth story.

### An open lead, stated as a lead and not as a finding

While measuring the above, `ScalarReplProbe` — whose hot method is called
100,000 times — did not promote past C1, and the optimizing tier engaged only
under `CRATONVM_JIT_FORCE_C2=1`. `CRATONVM_DBG=jit-method-stats` reports:

```text
3 distinct methods tracked, 2 ever invoked, 5032 total invocations
  still-interpreted=1 c1=2 full-profile=0 c2=0
  compiles: c1=2 c2=1 osr=1   c1_threshold=500   hot_but_stuck_in_interpreter=0
```

Two things in that line are unexplained. The counted invocations (≈5,000) are
far below the real call count (100,000), and the methods sit at `c1=2 / c2=0`
even when `CRATONVM_TIER_C2_THRESHOLD` is lowered to 1,000 — below the count
that was actually reached. `tiered.rs` does have a C1 -> C2 arm guarded on
`invocation_count`, and it also has a "settling" mechanism that lets a call
site stop asking once a method looks stable.

The obvious hypothesis is that invocations stop being counted once a method is
C1-compiled, which would make the C1 -> C2 arm unreachable in practice and
leave OSR as the only route into the optimizing tier. **That is a hypothesis
from reading the code plus two runs, not a measured conclusion**, and it is
deliberately not written up as one. If it holds it would suppress the whole
optimizing tier rather than escape analysis alone, so it deserves its own
investigation rather than a paragraph here.

## Computed but never read — the other half of the same census

A parked flag is a capability with a switch nobody has flipped. These are
capabilities with **no switch at all**: an analysis produces the answer and
nothing consumes it. They cost compile time on every compile that reaches them
and buy nothing, and — more to the point — they are invisible from outside,
because there is no flag to look up and no counter to read.

| produced by | what it is | consumer |
|---|---|---|
| `escape_analysis::EscapeResult::lock_coarsening` | plans to merge two adjacent monitor regions on the same object into one | ~~none~~ **WIRED 2026-09-16** in `apply_ea_to_ir_pinned`, behind `CRATONVM_JIT_LOCK_COARSEN` (default off), with `LOCK_COARSENING_PLANS_OFFERED` / `LOCK_COARSENINGS_APPLIED` so the two numbers can be read apart |
| `escape_analysis::EscapeResult::stack_allocatable` | every `ArgEscape` allocation — the population a stack-allocating tier would take | **none.** The module header says so: "computed, never read", and calls a non-empty result "a measurement, not an optimisation" |
| ~~`ir_optimize::licm_scev_corroborates`~~ | the bytecode-side counted-loop corroboration of an SSA hoist | ~~none as a gate~~ **DELETED round 9 wave 3**, with the legacy `scev::InductionVar` / `loop_analysis::{detect_loops, find_invariant_loads}` API it was the only caller of. A bytecode-threaded gate, if one is ever wanted, should be built on `loop_analysis::analyze_counted_loop_at` + `body_is_heap_stable` |

Lock coarsening **was** the one of the three that was finished, and it is now
wired (see the row). The remaining two are not finished and should not be
described as if they were: `stack_allocatable` has no allocator to hand its
population to, and the SCEV bridge needs bytecode threaded into a function that
does not take it.

### The same pattern is easy to re-create, and this document did it

Added 2026-09-16. The round-3 work added `ir_evidence::TRANSFORM_CENSUS` — the
per-transform counter whose absence had forced the 2026-08-27 scalar-deopt soak
to build a bespoke instrumented binary — and its doc comment ended with the
words "this is a reader." It was not. `transform_census()` was `pub`, it was
correct, and **nothing in the tree called it**, so on a release binary the
counts accumulated and were then dropped on exit. That is the same failure as
the three rows above, committed by the change that was documenting the failure.

The general lesson is worth more than the fix: `pub fn` is not a consumer. A
counter is read when a *caller* exists, and on this codebase a caller that is
only a `debug!` does not count either, because the workspace pins
`release_max_level_info` and compiles those away. `exit_summary` is now that
caller, on both VM exit arms under `CRATONVM_JIT_TRANSFORM_CENSUS`, and it
prints the zeroes — a zero is what separates "the pass ran and was neutral"
from "the pass never ran", which is the entire question the census exists to
answer.

<!-- superseded paragraph retained below for the audit trail -->
Of the three, **lock coarsening is the one that is finished**. Its applier is
written, it re-verifies rather than trusting the plan (an elision that already
removed the pair is a no-op; one that removed the *outer* enter or exit is a
refusal, so coarsening can never compound an unbalanced sequence it did not
create), and the only thing between it and production is a call in
`apply_ea_to_ir_pinned` beside the lock-elision loop, plus a default-off flag
and a soak. The other two are not finished and should not be described as if
they were: `stack_allocatable` has no allocator to hand its population to, and
the SCEV bridge needs bytecode threaded into a function that does not take it.
