...
### WP6.5 — BouncyCastle compatibility  [L, 3d]  ⚠️ PARTIAL — Service.&lt;init&gt; NPE closed (session 101)
...
- **Slice closed (session 101)**: `Provider$Service.<init>` NPE at pc=29 (the EJBCA-bootstrap first-failure proven by `bench/wildfly/bcprobe.stderr.log`) is fixed. `native-builtins/src/jca/provider_chain.rs` now registers a native `<init>` shim that copies the six argument refs straight into the receiver fields (real-JDK by name + synthetic-mode slot mirror at 0/1/2), bypassing the `knownEngines.get(type)` bytecode read; companion `<clinit>` no-ops on `Provider$ServiceKey` / `Provider$EngineDescription` keep the inner-class init chain quiet. Tests: `vm/tests/wp6_5_provider_service_init.rs` (4/4 PASS) + `native-builtins/src/jca/provider_chain.rs::tests` (5 new pins: descriptor, slot population, null-aux args, null-`this` NPE, inner-class clinits — all PASS). Regression guard: `wp2_5_proxy` 21/21 still PASS, `wp8_11_ejbca_bootstrap_smoke` 8/8 still PASS.
- **Remaining for full closure**: BC's `addAlgorithm` chain now constructs `Provider$Service` instances cleanly, but downstream `Cipher.getInstance("AES/GCM/NoPadding", "BC")` lookups still need BC's populated `serviceMap` to be reachable via `Provider.getService(type, algo)` once the provider is registered. The existing `phases_early.rs` synthetic getters at slot 0/1/2 keep working for the synthetic-mode case; real-JDK `Provider.getService` still routes through the JDK bytecode that reads its own internal `serviceMap`. End-to-end BC algorithm round-trip (e.g. `Cipher.getInstance(..., "BC").doFinal(...)`) is **not** validated here — that requires `Provider.parseLegacyPut` / `Provider.put` to put the freshly-constructed Service into the right map, which is downstream of this fix.
...
- **Subtasks (actionable)**:
  ...
  - **WP8.10.7** Register `Throwable.getMessage` / `printStackTrace(PrintStream)` on synthetic-stub Throwable subclasses  [S, ~30-50 LoC]. Surfaces secondary `NoSuchMethodError` in catch-block code paths. See `bench/wildfly-boot/diagnostic.md` §WP8.10.7.
  - **WP8.10.8** `RUSTJVM_JBOSS_MP_ROOT` env-var fallback for `-mp`  [XS, applied session 100]. Lets external integration tests inject the modules root without owning argv. See `native-builtins/src/jboss_module_loader.rs:188-200` and tests at line 1825-1853.
  - **WP8.10.9** Register `String.contains(Ljava/lang/CharSequence;)Z` (and `startsWith`/`endsWith` if missing) on synthetic-jdk `String` stub  [XS, ~10 LoC]. Surfaced session 101 as the new first-failure inside `vm/tests/wp8_10_jboss_modules_smoke.rs::probe0` once WP8.10.5 unblocked the synthetic-stub fallback. The probe Java calls `name.contains("Module")` and trips `NoSuchMethodError` for `String.contains`. Probes 1-6 already pass — this is purely a probe0 flush, but the same NSME will hit any WildFly boot code that uses `String.contains` on a class name. Trivial native shim that delegates to `String.indexOf(CharSequence)`.
