# RustJVM — Complete Path to 100% JDK 25 Conformance
...
RustJVM from where it is today to a JDK 25 implementation that runs
...
> | T1.1.g skip list empty-body | ✅ **NEW** | Remaining blanket bans are now `InitComplexity::Complex` plus `java/util/*` / `rustjvm/*` (conservative-only, overridable via env). Trivial inits clear under Aggressive. |
...
> | `rustjvm-jit` lib | **561/561** |
> | `rustjvm-classloading` lib | **287/287** |
> | `rustjvm-vm` lib (jit + new17) | **41/41** |
> | `rustjvm-vm` interpreter_tests (single-thread) | **903/903** |
> | `rustjvm-native-builtins` panama new18 | **5/5** |
> | `rustjvm-vm` bench-gate bin | **20/20** |
...
> | T1.1.16 TckLang under aggressive | **95/96** s46 TCK-lang tests pass under `RUSTJVM_JIT_ALLOW_PACKAGES=java/util,rustjvm/`. The one failure (`test_s46_exc_hierarchy`) is the NEW-1.3 hash-table loop miscompile, outside T1 scope. Default path (conservative) passes 96/96. |
...
> | T1.4.6 JDK 25 jimage round-trip | `t1_jimage_loads_real_jdk_modules` opens `$JAVA_HOME/lib/modules` via `rustjvm_reader::jimage::JImageReader` and verifies `java.base/java/lang/Object` round-trips. Skipped at runtime when JAVA_HOME is unset. |
...
> | `rustjvm-jit` lib | **569/569** (was 561 — 8 new regalloc-invariant tests) |
> | `rustjvm-classloading` lib | **287/287** |
> | `rustjvm-gc` lib | **595/595** |
> | `rustjvm-vm` interpreter_tests (single-thread) | **903/903** |
>
...
> | Item | Delivery |
> |---|---|
> | **T1.1.38** CI gate test | 3 new tests: `tier1_skip_list_no_blanket_java_util_ban`, `tier1_skip_list_no_blanket_rustjvm_ban`, `tier1_skip_list_targeted_entries_are_only_known_miscompiles`. Build fails if blanket ban reintroduced. 28/28 skip-list tests green. |
...
This tier is *the* hard milestone: get RustJVM running real OpenJDK 25
...
- **T2.1.1** Stand up CI job `no-default-natives` that runs
  `cargo test -p rustjvm-vm --no-default-features --features
...
- **T2.7.17** Test: serve TLS 1.3 from RustJVM and connect from a
  HotSpot client.
...
- **T2.10.1** Run `cargo test -p rustjvm-vm --no-default-features
...
- **T2.11.4** Run `javac HelloWorld.java` from RustJVM (yes, javac
  itself runs on the JVM) producing a real class file.
- **T2.11.5** Run JShell from RustJVM, type `1+1`, observe `2`.
...
- **T3.5.9** Run Hibernate-core 6 against H2 on RustJVM (smoke test
  only — boot the SessionFactory).
- **T3.5.10** Run Liquibase migrations against H2 on RustJVM.
...
- **T3.12.1** Boot `com.sun.tools.javac.Main` on RustJVM.
- **T3.12.2** `javac HelloWorld.java` compiles to a valid class file
  that RustJVM can then run.
- **T3.12.3** Self-host: compile RustJVM's own Java test sources via
  RustJVM-hosted javac.

## T3.13 — `jdk.jshell`

- **T3.13.1** Boot JShell on RustJVM.
- **T3.13.2** Evaluate `1 + 1` returning `2`.
- **T3.13.3** Define a class in the REPL, instantiate it, call a method.

## T3.14 — `jdk.jpackage`

- **T3.14.1** Boot jpackage on RustJVM.
...
- **T4.9.12** Boot OpenJDK's own javac on RustJVM and use it to
  compile RustJVM's own Java test sources.
...
- **T4.10.1** Build a differential test harness:
  `differential::run(class, method, args) → (rustjvm_result, hotspot_result)`.
- **T4.10.2** Run on every JCK test that passed individually.
- **T4.10.3** Capture and document any HotSpot-vs-RustJVM divergence.
...
By tier 4 RustJVM is correct. Tier 5 makes it competitive.
...
- **T5.1.2** Compute geomean ratio RustJVM/HotSpot per metric.
...
- **T5.7.1** AOT-load a curated set of `java.base` classes. — Done
  ...
  profiles + raw JIT bytecodes to `~/.rustjvm/aot-cache/<hash>.profile` at
...
- **T5.7.2** Class data sharing (CDS) via `jsa` archive. — Done
  ...
  dump/memory-map `~/.rustjvm/cds/core.jsa` with a JDK build-ID header
...
- **T6.2.1** Every JDK 25 JFR event has a real producer in RustJVM.
...
- **T6.5.1** Run a Maven test build against RustJVM as the JVM.
...
- **T6.6.1** IntelliJ IDEA "Add JDK..." accepts RustJVM's `release`
  + `lib/modules` layout.
- **T6.6.2** VS Code Java extension recognizes RustJVM as a JDK 25.
- **T6.6.3** Eclipse JDT.
...
- **T8.5.4** ✅ `register_missing_deprecated_shims` — any API not explicitly
  implemented gets an `UnsupportedOperationException("RustJVM T8.5.4: ...")` shim.
...
> capturing HotSpot numbers** — the RustJVM-side optimizations are
> all landed and live.
...
- **T10.2.1** Add `rustc-hash` crate dependency to `rustjvm-types`.
...
- **T10.3.1** Add `rustc-hash` dependency to `rustjvm-vm`, `rustjvm-classloading`, `rustjvm-jit`.
...
- **T10.4.1** Add `dashmap` dependency to `rustjvm-vm`.
...
- **T10.8.1** Re-run full benchmark suite. ⚠️ still gated on fixing
  ...
  RustJVM-side optimization wire-ups are complete.
- **T10.8.2** Compute geomean ratio vs HotSpot C2 — must be ≤ 1.5×.
  ...
  currently reports every row as `RJ_BOOT` because the rustjvm side
...
- **T10.9.D.4** Guard: `cargo test -p rustjvm-vm --features synthetic-jdk --lib`
...
# TIER 15 — Real-app bootstrap: Keycloak on RustJVM ✅ DELIVERED (Session 65)

The final tier: boot Keycloak 26.2.4 on RustJVM and measure startup
...
- **T15.2.1** Run `rustjvm --java-home JDK25 --Xmx 1g --jar keycloak/lib/quarkus-run.jar`.
...
- **T15.2.4** Target: Keycloak boots within 60 seconds on RustJVM (6× HotSpot).
...
- **T15.4.1** Keycloak boot + health check passes on RustJVM.
...
> `cargo test -p rustjvm-vm --features synthetic-jdk --lib` and found 74
...
- **T16.V3** ✅ `cargo test -p rustjvm-vm --features synthetic-jdk --lib`
...
test -p rustjvm-vm --features synthetic-jdk --lib`: **baseline 74 → 0
...
`cargo check --workspace` clean. `rustjvm-classloading --lib` 335/335.
`rustjvm-native-builtins` / `rustjvm-native-io` per-crate suites green.
...
**Reproduce locally:**
```
rustjvm.exe --stack-dump-on-timeout=15 \
...
11. **Keycloak 26.2.4 boots and serves health check on RustJVM (T15.4.1).**
12. **Zero missing JDK 25 ACC_NATIVE methods (T12+T13+T14 complete).**

At that point RustJVM can replace HotSpot as the JVM for any Java 25
