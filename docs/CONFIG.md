...
| `--jdk-only` | Real JDK **and** real class bytes are authoritative: no class is fabricated without real bytes, and no synthetic-stub native is registered or invoked. **This is the default**; the flag states it explicitly. Conflicts with `--compatible` and `--synthetic-jdk`. See [JDK-only mode](#jdk-only-mode) below. | **on** (`LAUNCHER_DEFAULT_COMPATIBILITY_MODE` = `jdk-only`) |
| `--compatible` | Real JDK class files from `jmods/` or `lib/modules` — roughly **2,700** native registrations in Rust (`REAL_JDK_NATIVE_REGISTRATIONS` in [`vm/src/config.rs`](../vm/src/config.rs)) — **plus** the pre-2026-09-20 compatibility substitutions. The opt-out from strict mode. Fails loudly if no usable JDK is found. | off |
| `--real-jdk` | Deprecated alias for `--compatible`. It only ever selected the real library, which was already the default, so every caller that passed it was asking for the compatible policy. Prints a one-line deprecation note. | off |
| `--synthetic-jdk` | Use the synthetic Rust standard library (~5,200 native stubs, no JDK needed) instead. Implies `--compatible`; mutually exclusive with `--compatible`/`--real-jdk` as a library choice and with `--jdk-only` as a policy choice. Requires a build with the `synthetic-jdk` Cargo feature — otherwise the launch fails rather than starting a VM with no class library at all. | off |
...
**The `cratonvm` launcher default since 2026-09-20.** The opt-out is
`--compatible` (deprecated alias: `--real-jdk`), which selects the
pre-2026-09-20 behaviour and is unchanged by anything in this section. Every
embedding entry point still defaults to `compatible`; see
...
recorded and counted rather than made fatal. A program that depends on a
substitution may therefore fail under the default and pass under
`--compatible`; that difference is the measurement, not a bug in either mode. Normative semantics live in
...
|---|---|---|
| *(none)* | `real-jdk` | `jdk-only` |
| `--jdk-only` | `real-jdk` | `jdk-only` |
| `--compatible` | `real-jdk` | `compatible` |
| `--real-jdk` | `real-jdk` | `compatible` |
| `--synthetic-jdk` | `synthetic-jdk` | `compatible` (implied) |
| `--jdk-only --compatible`, `--jdk-only --real-jdk` | **rejected at startup** | |
| `--jdk-only --synthetic-jdk` | **rejected at startup** | |
| `--synthetic-jdk --compatible`, `--synthetic-jdk --real-jdk` | **rejected at startup** | |

`--synthetic-jdk` *implies* `compatible` rather than inheriting the default.
The synthetic library is by construction the set of substitutions strict mode
forbids, so there is no strict synthetic mode to resolve to; inheriting the
default would turn a bare `--synthetic-jdk` into a startup error caused by a
change it has nothing to do with. The pair is only rejected when the argv
*states* both policies.

The rejected rows are configuration errors, not preferences. `jdk-only` forbids
...
|---|---|---|
| `cratonvm` launcher (`VmConfig::for_launcher`) | `real-jdk` (`LAUNCHER_DEFAULT_JDK_MODE`) | **`jdk-only`** (`LAUNCHER_DEFAULT_COMPATIBILITY_MODE`) |
...
| C API / Rust facade (`VmConfig::with_host_jdk_default`) | `real-jdk` (`LAUNCHER_DEFAULT_JDK_MODE`) | `compatible` (`EMBEDDED_HOST_JDK_DEFAULT_COMPATIBILITY_MODE`) |

Both asymmetries are deliberate, and they split along different lines — which
is the point of declaring five constants rather than two.

The `JdkMode` pair differs because "which class library loads" is a
hermeticity question: an in-tree test must not start resolving JMODs from
whatever JDK the build machine has.

The `CompatibilityMode` values differ because of **who asked**. The launcher
is a person at a prompt: they are asking the VM to run Java, the conforming
answer is the right one, and a refusal they disagree with is one `--compatible`
away. An embedding host has no command line of its own, did not ask for
anything, and cannot be handed a policy that rejects work its previous build
accepted — `libcratonvm::DEFAULT_COMPATIBILITY_MODE` is ABI. That is also why
`VmConfig::with_host_jdk_default` stopped being an alias for
`VmConfig::for_launcher` on 2026-09-20: re-aliasing it is an ABI break.

What did **not** change: strictness is never *inferred*. It comes from a
declared default constant, an explicit `--jdk-only`, or an explicit
`VmConfig::with_compatibility_mode(CompatibilityMode::JdkOnly)` — never from a
Cargo feature, an environment variable, or the other entry point's default.
...
|---|---|
| launcher | nothing — it is the default. `--jdk-only` states it explicitly. |
...
third of the contract. Since jdk-only became the launcher default the variable
is largely redundant; the launcher prints a one-time note saying so when it
sees the variable on a run that opted out with `--compatible`.
...
| `use_synthetic_jdk` | true (library) / **false — real-JDK, always** (launcher) | **Never host-detected.** Library/embedding default (`VmConfig::default()`, `EMBEDDED_DEFAULT_JDK_MODE`) stays `true` so the in-tree suite is hermetic. The `cratonvm` launcher uses `VmConfig::for_launcher()` and the C embedding API uses `VmConfig::with_host_jdk_default()`; both take `LAUNCHER_DEFAULT_JDK_MODE`, which is **real-JDK unconditionally** (the two constructors differ only in compatibility mode — see the row below). Select the other class library with `--synthetic-jdk`, which is mutually exclusive with `--compatible` / `--real-jdk`. If the selected mode is unavailable — no usable JDK for `--real-jdk`, or a build without the `synthetic-jdk` Cargo feature for `--synthetic-jdk` — the launch is a **hard error** naming everything searched; there is no silent fallback to the other class library. |
| `compatibility_mode` | `JdkOnly` launcher · `Compatible` embedded | Which substitutions are permitted, orthogonal to `use_synthetic_jdk`. Like the JDK-mode pair above, `LAUNCHER_DEFAULT_COMPATIBILITY_MODE` (`JdkOnly` since 2026-09-20) and `EMBEDDED_DEFAULT_COMPATIBILITY_MODE` / `EMBEDDED_HOST_JDK_DEFAULT_COMPATIBILITY_MODE` (`Compatible`) deliberately **differ** — but on a different axis: *who asked*. Strict mode rejects work that `Compatible` accepts, so an embedding host that passed no policy argument must never be handed it, while a person at a prompt can read the refusal and pass `--compatible`. It is still never inferred from a Cargo feature and never read from the environment. Set it with `--jdk-only` / `--compatible` or `VmConfig::with_compatibility_mode`; see [JDK-only mode](#jdk-only-mode). |
