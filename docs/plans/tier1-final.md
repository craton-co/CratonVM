...
The test fails specifically when `rustjvm/*` methods become
...
3. Fix the bug OR, if rooted in a larger JIT issue that needs more
   ...
   keep the blanket `rustjvm/*` ban. (The roadmap explicitly
...
- `rustjvm/*`: convert to a per-method list including the
