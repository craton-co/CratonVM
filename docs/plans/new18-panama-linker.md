...
Replace RustJVM's hand-rolled FFI dispatch (max 8 args, integer-only
...
- A pure-Rust helper module `panama_libffi` translates RustJVM's
...
RustJVM already exposes `NativeContext::invoke_virtual`. We can stash
...
**Verify:** `cargo check -p rustjvm-native-builtins`.
...
fixed-arg count. RustJVM's `FunctionDescriptor` synthetic doesn't
