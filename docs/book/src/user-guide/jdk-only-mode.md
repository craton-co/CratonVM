...
> **Status: the launcher default since 2026-09-20; enforcement at stage 1 of
> 4.** Stage 1 is instrumentation and measurement; enforcement is partial. A
> program that runs fine under `--compatible` may fail under the default — that
> is the signal the mode exists to produce, not a bug in the program. Strict
> mode is not yet a compatibility guarantee, which is why `--compatible` exists
> and why every *embedding* entry point still defaults to it.
...
`--jdk-only` implies the **real class library**: it tightens which
substitutions are permitted, it does not choose a library, but there is nothing
for it to be strict about except real bytes. Passing it is optional — it is the
default — and it exists so a script, CI lane or bug report can state the policy
it ran under.

It **conflicts with `--synthetic-jdk`**, and the launcher says so specifically
rather than raising a generic library-selection conflict — the synthetic
library is built entirely out of the substitutions this mode exists to forbid,
so a strict run would have no class library left to execute. It also conflicts
with `--compatible` (and its deprecated alias `--real-jdk`), which names the
opposite policy: an argv that states both has no reading, so the launcher
refuses rather than letting flag order decide.
...
`--compatible` is the fallback: same binary, same JDK, compatibility
substitutions permitted again. It is what every strict-mode violation message
points at, and `--real-jdk` is a deprecated alias for it. It was the launcher
default until 2026-09-20 and is still the default for `libcratonvm`,
`cratonvm-embed` and `VmConfig::default`.

`--synthetic-jdk` is not a fallback — it is a different class library, and it
