...
| `--java-home <PATH>` | JDK to use for boot/ext classpath discovery and JMOD loading. It does not *select* real-JDK mode — that is already the launcher default; it points the selected mode at one installation. It also does not select a compatibility policy. | `CRATONVM_JAVA_HOME`, then `JAVA_HOME` |
| `--jdk-only` | Real JDK class library with compatibility substitutions **rejected**: real class bytes are authoritative. **The launcher default**; the flag states it explicitly. Conflicts with `--compatible` and `--synthetic-jdk`. See [JDK-Only Mode](jdk-only-mode.md). | On |
| `--compatible` | Real JDK class library **with** the pre-2026-09-20 compatibility substitutions — the opt-out from strict mode. Conflicts with `--jdk-only` and `--synthetic-jdk`. | Off |
| `--real-jdk` | Deprecated alias for `--compatible`. It only ever selected the real library, which was already the default, so every caller that passed it was asking for the compatible policy. | Off |
| `--synthetic-jdk` | Select the synthetic (Rust) standard library. Requires a build with the `synthetic-jdk` Cargo feature. Implies `--compatible`; conflicts with `--compatible`/`--real-jdk` (library) and `--jdk-only` (policy). | Off |

The launcher default is real-JDK **plus the jdk-only policy**, both fixed at
compile time — neither is derived from whether a JDK happens to be installed,
and a selected mode that is unavailable is a hard error rather than a silent
switch to the other library. See
...
| `--Xms <SIZE>` | Initial heap size — the bytes committed at startup. **Honoured on all three collectors** since 2026-09-21 (G1 since F-16); clamped to `-Xmx` rather than refused. | — |
...
and forbids the compatibility substitutions the VM would otherwise make. It is
the launcher default; `--compatible` is the opt-out. The four diagnostic flags
below work in **either** policy — under `--compatible` they record what a
strict run *would* have rejected, which is what makes them a measurement of
the distance to strict mode rather than a post-mortem of a failed one.

| Flag | Description |
|------|-------------|
| `--jdk-only` | Real JDK, compatibility substitutions rejected. The default; conflicts with `--compatible` and `--synthetic-jdk`. |
