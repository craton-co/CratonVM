...
None of these require `--jdk-only` — which is the launcher default anyway; the
point is that they also work under `--compatible`, where they record what a
strict run *would* have rejected, making them a measurement rather than a
