...
Set `-Xmx` explicitly. CratonVM commits substantial backing memory at startup,
so an unconstrained or incorrectly detected host limit can turn into an
avoidable container eviction.

**The default collector is ZGC, not the generational collector** — since
2026-08-10, in any build with the `zgc` Cargo feature, which is on by default.
*(This section said "the generational collector with a non-moving young sweep
and selective promotion" until 2026-09-21. `vm/src/config.rs:23`–`:28` records
why that mistake matters operationally: while root-causing a GC bug, three
measurement arms recorded as "generational" were ZGC runs.)* ZGC here is a
stop-the-world mark-sweep over one two-ended arena that **compacts by
default**; it is not OpenJDK ZGC, and nothing about its pause behaviour should
be inferred from the name.

| Mode | Operational position |
|------|----------------------|
| Default (ZGC) | What you get if you pin nothing. Size it as you would any compacting collector; see [memory and GC](../user-guide/memory-and-gc.md) for the headroom caveat when a cycle declines to compact. |
| `-XX:+UseGenerationalGC` | The former default, and the fallback in a `--no-default-features` build. The escape hatch if ZGC misbehaves on your workload. |
| `-XX:+UseG1GC` | Experimental; validate on the exact workload before rollout. |
| Moving young generation (Generational backend) | Opt-in and fail-closed when root coverage is not proven; use for controlled evaluation. |
