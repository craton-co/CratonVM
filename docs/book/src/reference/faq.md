...
CratonVM runs the resulting bytecode. See [JDK
Modes](../getting-started/jdk-modes.md).
...
security-sensitive environments. See the [Security
Overview](../security/overview.md) and [Known
Limitations](../java-support/limitations.md).

### How fast is it?

On compute-bound code, the JIT is within roughly **1.3×–1.9×** of HotSpot C2
(about **1.5× overall** on the QuickBench micro-suite). Allocation-heavy
workloads (Binary Trees) are currently much slower and are a tracked roadmap
item. See [Benchmarks](../performance/benchmarks.md).
...
raise `-Xmx`; (2) GC throughput on short-lived allocation is a known weak spot.
Inside a container, set `-Xmx` explicitly because the ergonomic default is based
on host RAM. See [Memory & GC](../user-guide/memory-and-gc.md) and
...
Your program uses a standard-library method CratonVM hasn't implemented yet.
Audit exactly what's missing with `--XX:AuditMissingNatives` (or dump it to JSON
with `--dump-missing-natives`). See [Debugging &
...
x86-64 only (the interpreter runs everywhere). See the [Platform Support
...
`--nojit` (or `CRATONVM_DISABLE_JIT=1`). This is the fastest way to tell whether
a wrong result or hang is an interpreter or a JIT issue.
