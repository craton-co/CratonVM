...
Since 2026-09-20 the `cratonvm` launcher additionally defaults to the
**jdk-only** compatibility policy: those real class bytes are treated as
authoritative, so no class is fabricated without them and no synthetic-stub
native is registered or invoked. That is a separate axis from which class
library boots — see [JDK-Only Mode](../user-guide/jdk-only-mode.md) — and
`--compatible` opts back out of it while staying in real-JDK mode.
