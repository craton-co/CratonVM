...
| Token | Description |
|-------|-------------|
| `-stubs` | Drop every synthetic-stub native so calls fall through to real bytecode (or a clear `NoSuchMethodError`). This is the native-registry third of [JDK-only mode](../user-guide/jdk-only-mode.md); it cannot express the class-loading or dispatch half. Since jdk-only became the launcher default this token is largely redundant; setting it on a run that opted OUT with `--compatible` prints a one-time note saying so. |
