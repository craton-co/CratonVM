...
|------|----------------|------------------------------------|
| **Real JDK** | The `cratonvm` launcher default, or `--compatible` / `--real-jdk` | Real `java.base` bytecode loaded from the JDK's `java.base.jmod` (or `lib/modules` on a jlink image), backed by CratonVM's native methods |
...
The mode comes from an explicit flag (`--compatible` / `--real-jdk` /
`--synthetic-jdk`) or from a **fixed default** — never from what the host
happens to have installed. The `cratonvm` launcher defaults to real-JDK; the
in-process library default (`VmConfig::default()`) stays synthetic so hermetic
tests and embedders that ship no JDK do not start resolving JMODs from the
build machine's JDK.

This page is about *which class library boots*. The orthogonal question — which
compatibility substitutions are permitted once it has — is
[JDK-Only Mode](../user-guide/jdk-only-mode.md). Note that `--compatible` and
its deprecated alias `--real-jdk` answer **both**: real library, permissive
policy. `--synthetic-jdk` implies the permissive policy too, because the
synthetic library is made of exactly the substitutions strict mode forbids.
...
`--synthetic-jdk` wins over `--java-home`, and conflicts with `--compatible`, `--real-jdk`
and `--jdk-only`.
...
- **For maximum compatibility**, run in real-JDK mode (the launcher default) and add `--compatible` if a program trips the default strict policy.
