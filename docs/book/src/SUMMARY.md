# Summary

[Introduction](introduction.md)

# Getting Started

- [Installation](getting-started/installation.md)
- [Your First Program](getting-started/first-program.md)
- [JDK Modes: Real vs. Synthetic](getting-started/jdk-modes.md)

# User Guide

- [Running Programs](user-guide/running-programs.md)
- [Command-Line Reference](user-guide/cli-reference.md)
- [Configuration](user-guide/configuration.md)
- [Memory & Garbage Collection](user-guide/memory-and-gc.md)
- [The JIT Compiler](user-guide/jit-compiler.md)
- [Modules (JPMS)](user-guide/modules.md)
- [Containers & cgroups](user-guide/containers.md)
- [Debugging & Diagnostics](user-guide/debugging.md)
- [Troubleshooting](user-guide/troubleshooting.md)

# Java Platform Support

- [Language Features](java-support/language-features.md)
- [Java Version Support](java-support/version-support.md)
- [Standard Library Coverage](java-support/standard-library.md)
- [Known Limitations](java-support/limitations.md)

# Security

- [Security Overview](security/overview.md)
- [Sandboxing & Hardening](security/sandboxing.md)
- [Cryptography](security/cryptography.md)

# Performance

- [Benchmarks](performance/benchmarks.md)
- [Performance Tuning](performance/tuning.md)
- [How the JIT Got Fast](performance/jit-internals.md)
- [Profiling](performance/profiling.md)

# Operations

- [Deployment and Operations](operations/deployment.md)
- [Observability](operations/observability.md)
- [Incident Response](operations/incident-response.md)

# GPU Offload

- [GPU Offload](gpu/overview.md)
- [Benchmarks](gpu/benchmarks.md)

# Embedding CratonVM

- [Embedding Overview](embedding/overview.md)
- [The C ABI (libcratonvm)](embedding/c-abi.md)
- [The Rust Facade (cratonvm-embed)](embedding/rust-facade.md)

# Architecture & Internals

- [Architecture Overview](internals/architecture.md)
- [Runtime Lifecycle](internals/runtime-lifecycle.md)
- [Runtime Contracts](internals/runtime-contracts.md)
- [The Interpreter](internals/interpreter.md)
- [The JIT Compiler](internals/jit.md)
- [The Garbage Collector](internals/garbage-collector.md)
- [Class Loading & Verification](internals/class-loading.md)
- [Threading & Concurrency](internals/threading.md)
- [Native Methods](internals/native-methods.md)

# Contributing

- [Building from Source](contributing/building.md)
- [Testing](contributing/testing.md)
- [Contributing Guide](contributing/contributing.md)
- [Documentation Guide](contributing/documentation.md)
- [Roadmap](contributing/roadmap.md)

# Reference

- [Environment Variables](reference/environment-variables.md)
- [Platform Support Matrix](reference/platform-support.md)
- [Compatibility and Support Policy](reference/compatibility-policy.md)
- [FAQ](reference/faq.md)
- [Glossary](reference/glossary.md)
