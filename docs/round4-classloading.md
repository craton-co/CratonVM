...
**Fix:** Change `ObjectRef(String)`/`ArrayRef(String)` to `Arc<str>`. Constructors (`from_verification_type_info`, `from_field_descriptor`, `initial_frame`, `compact_initial_frame`) already hand out short-lived owned values; switch to `intern_arc()` from `rustjvm_types`. Clone becomes a refcount bump.
