...
- `org/jboss/as/` (WildFly boot, part of the SPB.8b/8c family) — **BAN LIFTED
  ...
  leak-detector `Cleaner` tore the boot down mid-flight. Root-caused and fixed;
  see `docs/internal/fixed-suite-bugs/wildfly/modeltypevalidator-validtypes-npe.md`. WILDFLY-CONTROLLER-JIT.1 was only *transitively* implicated
  by that same finding and is therefore no longer backed by it either — it is
  kept in place pending its own re-test.
