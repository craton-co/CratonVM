...
| [`brave-baggagefields-classcast-summary-printing.md`](brave-baggagefields-classcast-summary-printing.md) | 1 (`BraveAutoConfigurationTests`) | LOW (not memory-unsafe) | OPEN, small — `ClassCastException` in JUnit summary printing (`Formatter`/CLDR `Locale` bootstrap), surfaced by the `OnClassCondition` fix changing this class's bean/condition counts |
