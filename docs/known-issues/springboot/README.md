...
| [`charbuffer-getarray-scopedmemoryaccess-copymemory-aioobe.md`](charbuffer-getarray-scopedmemoryaccess-copymemory-aioobe.md) | blocks all 7 classes above | HIGH | OPEN — found 2026-07-14 while verifying the `CharBuffer.order()` fix. `CharBuffer.getArray`'s always-taken `ScopedMemoryAccess.copyMemory` fast path (`isAddressable()` is unconditionally `true` in real JDK bytecode) throws `ArrayIndexOutOfBoundsException` for a heap-backed view built by `ByteBuffer.asCharBuffer()` — not yet root-caused |
