...
| [`testengine-getid-abstractmethoderror-young-gc-forwarding-gap.md`](testengine-getid-abstractmethoderror-young-gc-forwarding-gap.md) | 2 (fatal CRASH) | CRITICAL | OPEN — found 2026-07-16 (`rerun-20260716`, shard4). **New regression from the same-day GC merge**: a brand-new young-GC pre-forwarding walk (`gc/src/gen_heap.rs`, commits `1c4aaa069`/`fb15be63f`) only special-cases the `GAP_FILLER_CLASS_ID` sentinel, unlike the established `exact_cursor` walk which also consults the free-list via `skip_free_blocks`. On a free/TLAB-remnant range it doesn't recognize, it aborts the whole walk early, dropping ~99.9998% of live young-gen objects from the forwarding set; the semispace flip then reclaims them, and any surviving reference reads back an all-zero header. Manifests here as `AbstractMethodError: TestEngine.getId()` when a corrupted receiver happens to be a JUnit test-engine instance |
| [`basiccontroller-stale-pointer-invokevirtual-aqs-conditionnode-crash.md`](basiccontroller-stale-pointer-invokevirtual-aqs-conditionnode-crash.md) | 1 (fatal CRASH) | CRITICAL | OPEN — found 2026-07-16 (`rerun-20260716`, shard6). Independent confirmation of the same young-GC forwarding-walk truncation bug as the `TestEngine.getId()` entry above (cross-referenced against a same-day WildFly finding) — here manifesting as a stale-pointer storm against `AbstractQueuedSynchronizer$ConditionNode`/`Unsafe` receivers, ending in an uncaught `ClassCastException` inside `SbRunner.main`. **An unmerged fix branch already exists**: `fix/wildfly-cce0079-close-20260716` (commit `92ebad642`) adds the free-list/TLAB skip + walk-completeness flag this worktree is missing — not yet merged here |
...
above, dated 2026-07-16). Two are the **same newly-discovered, critical GC
regression** (`testengine-getid...`/`basiccontroller-stale-pointer...`) — a
brand-new young-GC pre-forwarding walk added the same day silently drops
live objects from the forwarding set under certain free-list/TLAB-remnant
shapes; **a fix for this already exists on an unmerged branch**
(`fix/wildfly-cce0079-close-20260716`). One (`wrong-receiver-virtual-dispatch...`)
