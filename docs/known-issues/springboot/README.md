...
| [`propertieslauncher-jarloading-cluster-broad-regression-jit-halfgap-suspected.md`](propertieslauncher-jarloading-cluster-broad-regression-jit-halfgap-suspected.md) | OPEN, partially fixed 2026-07-21 — root cause found: a botched merge (`fix/springboot-cloudfoundry-integrate-20260720` → dev at `b90ecea19`) silently reverted a large chunk of `vm_exec.rs`'s force-native override chain (NOT the suspected JIT halfgap branch). Restored `ExplodedArchive.getClassPathUrls` + `URLClassLoader.getResourceAsStream`: 15/32 → 12/32 `PropertiesLauncherTests` fail, plus `ExplodedArchiveTests`/`RepackagerTests` fixed as a side effect, zero regressions (73-class sibling sweep). Residual 12 failures split into 2 clusters, root cause NOT found for either despite deep interpreter-dispatch tracing; several other reverted-but-unfixed overrides catalogued (ZipFile, Path.toString, Netty/Mongo/DatagramChannel, JMX ThreadInfo, TLAB GC-safe alloc, monitor-release-on-thread-death) |
