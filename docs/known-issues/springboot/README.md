...
| `JdbcSessionAutoConfigurationTests` HSQLDB `RangeGroupEmpty` loop | 1 (`JdbcSessionAutoConfigurationTests`) | MEDIUM | **FIXED 2026-07-15** — moved to [`../../internal/springboot/jdbcsession-hsqldb-rangegroupempty-hang-FIXED.md`](../../internal/springboot/jdbcsession-hsqldb-rangegroupempty-hang-FIXED.md); `java.lang.reflect.Array.set` now validates primitive-array values before unboxing, so HSQLDB's non-wrapper `RangeGroupEmpty` never reaches a speculative field-0 read |
