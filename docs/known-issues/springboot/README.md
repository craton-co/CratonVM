...
| [`charbuffer-order-missing-native-idn-clinit-cluster.md`](charbuffer-order-missing-native-idn-clinit-cluster.md) | 6 FAIL + 1 fatal CRASH | HIGH | OPEN — found 2026-07-14 (`crashfail-20260714`). `CharBuffer.order()` has no native registration anywhere in CratonVM; the first process-wide reference to `java.net.IDN` (via ICU4X/`StringPrep`) hits it, poisoning `IDN`'s `<clinit>` for the rest of the process (`NoClassDefFoundError` on every later reference; a fatal `linkage error` crash if it's the very first thing touched) |
| [`logback-loggercontext-listenerlist-final-field-corruption.md`](logback-loggercontext-listenerlist-final-field-corruption.md) | 20+ | HIGH | OPEN — found 2026-07-14. Logback's `LoggerContext.loggerContextListenerList` (a `private final` field) reads as null after N successful resets of the same process-wide singleton — a `final` field cannot legally go null via any Java bytecode path, so this is heap/GC corruption hitting an already-valid object, not a construction bug. Two specific JIT constructor-inlining mechanisms were inspected and ruled out; likely belongs to the existing GC-field-loss bug family |
| [`structured-logging-map-entry-getkey-lambda-dispatch-precedence.md`](structured-logging-map-entry-getkey-lambda-dispatch-precedence.md) | 5 | HIGH | OPEN — found 2026-07-14, confirmed with a standalone (non-Spring-Boot) repro. `Map.Entry::getKey`/`getValue` invoked as method references over a synthetic `UnmodifiableMapEntry` wrapper resolve to a generic interface-level native (`java/util/Map$Entry`) instead of the wrapper's own correct delegating native, returning the whole entry instead of the key/value |
