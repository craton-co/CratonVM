...
| [`mockresolver-dynamicclassloader-classnotfound-forked-testcontext.md`](mockresolver-dynamicclassloader-classnotfound-forked-testcontext.md) | OPEN — found 2026-07-20 while re-verifying `repeatablecontainers-method-cache-classcastexception`. `ChildManagementContextInitializerAotTests` (`@CompileWithForkedClassLoader`) now fails later, when Mockito's `MockResolver` plugin loading throws `ClassNotFoundException: SpringMockResolver` — confirmed the whole `DynamicClassLoader`/`CompileWithForkedClassLoaderClassLoader` delegation chain runs as real bytecode with ZERO CratonVM classloader-native involvement (ruled out the obvious `check_override` hypothesis), root cause not yet found |
...
| [`pemcertificates-clientauth-rustls-decrypterror.md`](pemcertificates-clientauth-rustls-decrypterror.md) | OPEN — found 2026-07-20, residual of the reactor-netty hang fix. Client PKCS12 key material and alias selection both verified correct; leading (unconfirmed) hypothesis is a process-wide TLS-identity singleton racing across the 36-test-per-process suite |
