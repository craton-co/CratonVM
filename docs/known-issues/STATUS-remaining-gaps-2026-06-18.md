...
| 4 | ~~`spring-bug-04-junit-timeout-interceptor-double-proceed`~~ | JUnit "interceptor invoked twice" was a MASK — synthetic compare raised `NoSuchMethodError` not `ClassCastException` for a non-`Comparable` element | ✅ **FIXED** `fc20e970` (merged to `dev`) — `compare_via_compare_to` guards with `implements_comparable`; archived in `docs/internal/fixed-suite-bugs/` | DONE | `ValueCodeGeneratorTests` 41/41; not threading/MH (deterministic, single-thread); other 2 doc classes were mis-attributed |
...
| 12 | `→ docs/internal/hibernate-jta-narayana-xa-completion-and-socket-loopback` | JTA Narayana cluster | ✅ **RESOLVED 2026-06-20** — L0 fixed on dev; L1 REFUTED (XA completion works); L2 = `accept()` deadlock + `getLocalPort==0`, fixed on `fix/hib-jta-xa-loopback` (`e0426050`) | — | full begin/persist/commit/truncate passes in default mode; doc moved to internal |
| 13 | `→ docs/internal/hibernate-jaxb-classloading-bytebuddy-bootstrap-slow` | XML/JAXB + ByteBuddy `MethodGraph` bootstrap | ✅ **RESOLVED / no-repro 2026-06-20** — storm fixed on dev; JoinedSubclass `MethodGraph` boots ~16s `--nojit` | — | doc moved to internal; original JUnit repros gated by `@ExtendWith` meta-anno gap |
