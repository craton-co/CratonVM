...
1. **`jdk/internal/misc/PreviewFeatures.isPreviewEnabled()Z` missing** — blocked
   every class's JUnit launcher/discovery on JDK 21 before a single test method
   ran. See `keycloak-previewfeatures-ispreviewenabled-native.md` (now marked
   fixed). native-builtins/src/lib.rs.
