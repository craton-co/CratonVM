...
Status: FIXED — merged to dev 2026-07-11 (commit 289ec7e5, reviewed and approved by the user). See "Verification" and "Second bug found during verification" below: this fix resolves its own targeted mechanism completely (0/30 hangs post-fix vs 8/15 pre-fix); a separate, pre-existing, already-tracked bug (gc-audit finding 1(b)) remains open and unrelated.
...
This fix was initially parked (not merged) per this investigation's explicit "no hang in the repro, or do not merge" bar -- at the time, `AqsContentionProbe.java` still hung intermittently (~13%) due to the second, unrelated bug below, so the letter of that bar was not met for the *combined* state of the codebase even though this patch was not the cause of the remaining failures. The doc was written recommending merge anyway, since the two bugs are independent and this fix's own liveness argument holds regardless of the second bug's status.

The user reviewed the diff and this doc directly, agreed with that recommendation, and approved merging on that basis. **Merged to `dev` 2026-07-11, commit `289ec7e5`** (rebased from the original `c12b94fb` on `fix/gc-barrier-stwready-alivecount-20260711-135958`; content identical, only the hash changed). No shadowing commits on `alive_count_blocked_and_os_tids`/`alive_count_and_os_tids`/`gc_barrier.rs` were found on `dev` between the fix being written and merged (checked via `git log -S<symbol>` immediately before push).

## Recommended next steps

1. ~~Review and merge~~ -- **done**, see Merge history above.
2. Chase the second bug (heap corruption / stale `AbstractQueuedSynchronizer` pointer under GC pressure) using `AqsContentionProbe.java` as the repro -- it is dramatically cheaper to iterate on than MTChurn/BinaryTrees/a full WildFly boot, and directly continues `gc-audit-2026-07-10-open-findings.md` finding 1(b) (already tracked there; no new doc needed for it specifically).
3. Once the second bug is also resolved, `AqsContentionProbe8` should pass 100% reliably across many runs and multiple host-load conditions -- re-run this doc's own verification loop (30+ iterations) as the final confirmation. In the meantime, `WFLYHC0053` investigation can resume now that this doc's fix is merged (the STW/AQS freeze should no longer block reaching that bug's own failure point), even though the second bug may still cause occasional unrelated flakiness.
