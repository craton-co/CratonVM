...
* **The call frames themselves.** `probes/CallArgCostProbe.java` prices a
  ...
  **Elided 2026-08-26, then narrowed and de-duplicated 2026-08-27 — all
  measured NOT to help this class.** The per-call 14-store full-GPR blind
  spill is now elided where the caller frame is provably oop-clean, worth
  1.4–2.2x on every shape of compiled call in `CallArgCostProbe`.
  `HttpStatusClassLoopRate` does not move (106.5–121.2 vs 105.8–112.1 ns/iter,
  three interleaved rounds each), and the counter says why: `elided=1 ...
  ref-local-in-reg=46`, every refusal the same clause. The two follow-on
  levers on a refusal — narrowing the spill to the registers that can hold an
  oop, then dropping the register copy of arguments already staged to the
  frame — both shipped default-ON 2026-08-27 and neither separates from
  `control` on this class either; there is no fourth lever of this shape. See
