...
|---|---|---|---|
| ~~`io.netty.channel.NativeImageHandlerMetadataTest`~~ **and the 16 rows below it — RESOLVED 2026-08-30, all 17** | ~~Resource path computed from Maven metadata this non-Maven harness never populates~~ — the harness DOES model it (`module-scoped-classes.tsv` + `module-args/`), the machinery was just inert on Windows: `run-netty-suite.sh` hard-coded `USE_MODSCOPE=0` and `gen-module-args.sh` assumed `mvn` on PATH, Linux paths and `:` as the classpath separator | **`USE_MODSCOPE=1` with the argfiles generated: `status: PASS=17`, 16 s for all seventeen.** Not an environment gap after all — the third red cluster on this fixture to be one line of harness configuration, after the missing `-ea` and `-Dio.netty.bootstrap.extensions=serviceload` | `compatible-fail-triage-20260830.md` |
...
| `io.netty.test.udt.nio.NioUdtByteRendezvousChannelTest` | `basicEcho()` fails identically on stock HotSpot JDK 25 with a byte-count mismatch (`expected: <1188976> but was: <1141280>`) | Direct HotSpot cross-check, quiet host, one class per process: `ok=1 failed=1` on both | `compatible-fail-triage-20260830.md` |
