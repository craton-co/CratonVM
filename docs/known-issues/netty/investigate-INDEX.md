...
- [batch 08](investigate-batch-08.md) — **RESOLVED 2026-08-12**, all 15 match HotSpot test-for-test. Only ONE was a CratonVM defect: a JDK-generated EC identity was rejected by the rustls/ring TLS stack, so `Http2MultiplexTransportTest` HUNG ([record](ec-server-identity-rejected-jdk-pkcs8-v1-20260812.md)) — that one affects every EC server/client identity, well beyond netty. Nine were already fixed by batch 07's `StackWalker$Option` repair; five fail identically on HotSpot and are harness gaps. **Take the harness gaps seriously — one of them was masking a second VM defect** ([`-ea` is parsed and discarded](ea-flag-ignored-so-assert-never-fires-20260812.md), OPEN): with `-ea` HotSpot goes 34/34 and CratonVM stays 28/34, but without it both fail equally and the class reads as environment noise.
