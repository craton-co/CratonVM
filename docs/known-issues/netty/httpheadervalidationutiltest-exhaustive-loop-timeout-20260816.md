...
4. **The call chain and the per-iteration floor.** Unchanged and now the whole
   ...
   **Elided, narrowed, and de-duplicated 2026-08-26/27 — all three measured NOT
   to help this class.** The 14-store full-GPR blind spill
   `emit_pre_safepoint_spill` emits at every GC-capable call is now elided
   where the caller frame is provably oop-clean (`CRATONVM_JIT_CALL_SPILL_ELISION`,
   default on), which is worth **1.4-2.2x on every shape of compiled call** in
   `CallArgCostProbe`. On THIS class it does nothing — three interleaved rounds
   each, quiet host, `652-685` vs `684-718` ns/iter on the value loop — and the
   counter says why in one line: `elided=1 ... ref-local-in-reg=116`, every
   refusal the same clause. A frame that keeps a receiver in a register-homed
   local cannot use the elision. The two follow-on levers on a refusal —
   narrowing the spill to just the registers that can hold an oop
   (`CRATONVM_JIT_SPILL_NARROW`, default ON) and then dropping the register
   copy of arguments already staged to the frame
   (`CRATONVM_JIT_SPILL_ARGS_PUBLISHED`, default ON) — both shipped 2026-08-27
   and neither separates from `control` on this class either. There is no
