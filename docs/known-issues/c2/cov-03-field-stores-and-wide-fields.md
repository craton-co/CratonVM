...
> builder — which is the code that writes reference fields. Measured on the
> merged tree, the `putfield` row went **37 → 60** and this lane now owns
> **66 of the 67** remaining builder refusals: every other structural refusal
> in the corpus is gone. It is the largest single refusal anywhere below
> `ir_compatible`. The line numbers moved as well (`0xb4` at `ir.rs:5216`,
> `0xb5` at `ir.rs:5260` on the merged tree); re-derive both. See
