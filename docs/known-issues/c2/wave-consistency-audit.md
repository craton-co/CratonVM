...
| **10** | **Dangling doc reference introduced by the JVMTI lane.** `vm/src/runtime/jvmti.rs:2975` points readers at `docs/known-issues/c2/jvmti-delivery-threading.md` for "the current census" of unattributed hooks. That file does not exist. It is the census that would have surfaced finding 1. | `8ebe8e604` | `ls docs/known-issues/c2/jvmti-delivery-threading.md` → no such file. The census content is in `docs/jvmti-vm-scoping.md` §"left unattributed". | **LOW** | Repoint to `jvmti-vm-scoping.md`, or write the census file — and make it list the six `vm_init.rs` sites from finding 1 by line. |
