...
| `hir-02` | increment 3 (real registers) — the prologue save area is tractable; the change that would give it a consumer is on the design doc's own "what to refuse" list | same | nobody |
...
| `osr-02` | the differential is a Java-level oracle, not a frame comparator: it observes the resumed frame through per-execution side effects and a per-iteration digest of the loop-carried state, so a divergence in a local the rest of the loop never reads would not be seen | same | nobody |
...
|---|---|---|
| HIR/LIR/MIR | ~~`hir-01`~~ ~~`hir-02`~~ **both closed 2026-08-03; `hir-02`'s residuals retired 2026-08-04** | Consolidated into `docs/feature-designs/jit-machine-level-and-instruction-selection.md`. Four levels, not three; the report's "HIR" is the bytecode. Increment 0 (shadow selection, emits nothing) measured **15.7–19.0%** coverage with `Rule::Lea`/`AluImm` firing **zero** times. Both row gaps are now closed and the selector has a **production caller** — `CRATONVM_JIT=ir-isel-emit`, default off, fail-closed, with a byte-equality oracle that ran over a real workload and found 0 disagreements. `Rule::Lea` fires 26 tiles where it fired none. Increment 3 (registers) is deliberately unbuilt and says why. See `docs/internal/hir-02-mir-regalloc-handoff-RETIRED-20260804.md`. |
...
| OSR | ~~`osr-01`~~ ~~`osr-02`~~ **both closed 2026-08-03; `osr-02` fully retired 2026-08-04** | The metadata contract is executable and enforced — `docs/feature-designs/jit-osr-entry-metadata.md`. Two findings: there are **three** coordinate spaces, not the two the brief names, and the second compile door (`compile_osr_artifact` calling `x64::compile` directly) is still open and is `osr-01`'s whole remaining item. `osr-02` is **done**: the per-pc livelock memo was already built, the lifecycle counters landed 2026-08-03, and the exit-state differential — the increment the lane called its point — landed 2026-08-04 with the historical defect injected and caught. Two things the writing changed: `osr_exit_points` conflates the loop-boundary map with the `invokedynamic` trap (one emitter writes both), and refusing an ambiguous resume bci on `(semantics, reason)` cost `CratonBench.matrixKernel` its OSR permanently — the predicate is `semantics` alone. See `osr-02-exit-and-recompile-RETIRED-20260804.md`. |
