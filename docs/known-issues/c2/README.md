...
| Loop transforms | `loop-01` **both increments landed 2026-08-03**, `loop-02` | Peeling is reachable (the bypassable-header arm) and guarded versioning exists — a pre-header check from `scev::PreheaderGuard`, the transform on the guarded path, an untouched copy on the fallback. Unswitching, interchange and fusion are still unbuilt, and the whole lane is *unreachable*: `deopt_real` is default-on and is the first of `loop-02`'s four whole-compile refusals, so no armed compile ever reaches a loop. `loop-02` is now the blocking item for this lane, not a parallel one. |
