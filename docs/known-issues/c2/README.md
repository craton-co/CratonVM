...
|---|---|---|
| HIR/LIR/MIR | ~~`hir-01`~~ ~~`hir-02`~~ **both closed 2026-08-03; `hir-02`'s residuals retired 2026-08-04** | Consolidated into `docs/feature-designs/jit-machine-level-and-instruction-selection.md`. Four levels, not three; the report's "HIR" is the bytecode. Increment 0 (shadow selection, emits nothing) measured **15.7–19.0%** coverage with `Rule::Lea`/`AluImm` firing **zero** times. Both row gaps are now closed and the selector has a **production caller** — `CRATONVM_JIT=ir-isel-emit`, default off, fail-closed, with a byte-equality oracle that ran over a real workload and found 0 disagreements. `Rule::Lea` fires 26 tiles where it fired none. Increment 3 (registers) is deliberately unbuilt and says why. See `hir-02-mir-regalloc-handoff-RETIRED-20260804.md`. |
