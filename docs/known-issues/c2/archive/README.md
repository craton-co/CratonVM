...
| ~~`osr-02-exit-and-recompile.md`~~ **RETIRED 2026-08-04** | `4bec5efca`, then `fix/c2-osr02-exit-differential-20260804` | all four items. The per-pc memo already existed; lifecycle counters landed 2026-08-03; the **exit-state differential** — the increment the doc called the point of the lane — landed with the historical defect injected and caught (200 000 requested, 200 006 executed, while the sum stayed identical); `osr_exit_points` is cross-checked against where exits are actually taken | the differential is a Java-level oracle rather than a frame comparator, and the de-speculation reason lookup is still by bci. Both stated with their reasons in `osr-02-exit-and-recompile-RETIRED-20260804.md` (internal) |
