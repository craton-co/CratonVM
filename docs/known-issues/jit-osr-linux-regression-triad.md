...
**CORRECTION (found independently, investigating the wider Linux FAIL
bucket, same day):** the wrong-vtable-dispatch signature originally cited for
#1 (`NoSuchMethodError: java/lang/Object.<method>`) is **NOT exclusive to
OSR** either way — two more classes hit the exact same pattern with
`CRATONVM_JIT_OSR` OFF: `jpa.criteria.InPredicateTest`
(`Object.removeEldestEntry`) and `lob.JpaLargeBlobTest` (`Object.read()I`).
This corroborates the isolation finding above: it's a **broader, pre-existing
JIT wrong-receiver-type bug**, not an OSR-exclusive defect — OSR likely just
makes it MORE likely to trigger for some classes (it compiles hot loops
mid-execution that wouldn't otherwise reach the normal JIT threshold), but
...
OSR regression. This lines up with the CORRECTION above: the same
`NoSuchMethodError: java/lang/Object.<method>` signature also fires with OSR
completely OFF on two unrelated classes — so even if this exact class
resurfaces it, it's evidence for the general pre-existing JIT bug, not a
reason to reopen the OSR-exclusive framing.
