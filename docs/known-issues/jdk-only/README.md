...
| `W7-65-stream-reuse-throws.md` | Six residuals, each left on purpose with its cost stated: primitive streams (25 call sites swallow the error via `.unwrap_or_default()`); the deferred intermediate op in Compatible mode; `close()`; `onClose()`; the `GathererOp` stage-replacement constructor; and every stream minted with fewer than five slots. |
