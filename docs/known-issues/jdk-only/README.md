...
(W5-1, W6-6) — **40 since 2026-08-12, W7-79** — `RJdkModule` 44 checks
(W2-3, W4-2, W6-2). `RJdkModule` needs
...
| `W5-1-loadlibrary-allowlist-too-wide.md` · `W6-6-nativelibraries-load-fabricated-success.md` | **Rewritten 2026-08-12 (W7-79) — this row was stale, and W5-1's claim that it had already been discharged was wrong.** Loader-scoped since 2026-08-11 under `--jdk-only` (`LOADED_LIBRARIES` in `lang_system.rs`), and **verified firing** on 2026-08-12: two `URLClassLoader`s loading one library give HotSpot's `already loaded in another classloader` on the strict arm, plain success on `Compatible`. What is left: (a) `Compatible` deliberately not scoped — it would turn successes into errors in exactly the servlet-container shape, decision recorded in W5-1; (b) the boot-loader case, unarmed on both roads because `BootLoader.loadLibrary` is a no-op; (c) the key is the spelling, not the file. |
...
<!-- merge: both lanes' index rows kept -->
<!-- merge: both sides kept; separate lanes' index rows are complementary -->
...
| `W7-61-sslengine-layout-and-tls-blocking.md` | Two, both stated with the reason. (1) `javax/net/ssl/SSLEngine` carries a 7-slot map from `register_p68_ssl` and a 14-slot map from `tls.rs::register_ssl_engine` on ONE object in **synthetic-jdk** mode: p68 wins its 21 triples, and the 8 tls.rs triples it does not re-register keep the wider map and index slots 7–13 past the end of a 7-wide allocation. Giving the surface one owner moves two files plus a `vm/src/vm/tests.rs` fixture that pins the map by hand. The Compatible-mode half of W7-49's "LIVE, 7 vs 2" row is a **false positive**, and the record shows the registration ordering that makes it one. (2) The **Windows** half of the TLS read wakeup: Winsock has no `shutdown` that aborts a pending blocking call, so a reader already parked in `s2_tls_read_direct` / `rustls_stream_read` still cannot be woken. Expected reading: `AsyncCloseProbe`'s `tlsRead` row reports `TIMEOUT` on Windows CratonVM. |
...
| ~~`W4-1-publiclookup-allowedmodes-never-checked.md`~~ | **CLOSED 2026-08-12 (W7-62).** Dead block deleted (332 lines: `enforce_lookup_access`, `lk_member_access_flags`, eleven `lk_find_*` — the code was SUPERSEDED, `lang_invoke::register_p63_method_handles_lookup` owns all ten triples). SIX tests, not four, re-pointed at `lk_enforce_find_access`, plus four arms they could not express. **`lk_public_lookup` was NOT deleted** — this record's list is wrong about it, it IS registered. |
...
| `W5-1-loadlibrary-allowlist-too-wide.md` | Arm `BootLoader.loadLibrary` (`native-builtins/src/lib.rs:14050`; `record_boot_loader_library` at `lang_system.rs:3198` has zero callers); return the resolved path from `load_native_library`. ~~fix the Compatible-mode `Runtime.load0`/`loadLibrary0` argument index~~ — **done 2026-08-12 (W7-79), with five new `RJdkJni` checks that fail on the pre-fix binary.** |
...
| `W7-20-refusal-laundered-into-wrong-answer.md` | **Half settled 2026-08-12 (W7-62).** The kind map is re-frozen — TWELVE rows, not nine, and the nine carrier rows went to `bridge 1 1` with a source edit rather than to the `bridge 0 1` the tree produced, because that dropped an adjudication the gate refuses one-way. `jdk-only-bridge-ratchet.json` still fires and is deliberately left firing with its derived movement in its `note`; it needs a census. **And a THIRD stale ratchet nobody had recorded:** `native-builtins/tests/stub_ratchet.rs`, +6, also firing. |
...
| `W6-6-nativelibraries-load-fabricated-success.md` | The boot-loader case cannot fire on either road, because `BootLoader.loadLibrary` is a no-op. W5-1 owns the arming. **And the headline fix itself is INERT on this host, measured 2026-08-12 (W7-79): `CRATONVM_DBG_NATIVELIBRARIES_LOAD_OK=1` changes nothing in `RJdkJni` in either mode, because `vm_exec.rs` forces the native override for all six `System`/`Runtime` load triples so real `ClassLoader.loadLibrary` bytecode never runs.** |
...
| ~~`W7-16-arraydeque-and-linkedlist-residuals.md`~~ | **CLOSED IN SOURCE 2026-08-12 (W7-62), not rebuilt.** The `jdk_interfaces` arm is applied. It CLOSES the `ClassCastException` rather than moving it — `fabricate_class` is the shared body of all three `ensure_*_class` doors so the arm reaches the VM-internal mint; `is_assignable_to_name_inner` walks interfaces transitively; `ListIterator`'s nine abstract methods are exactly the nine natives on the carrier. Instrument: `probes/ListItrInterfaceProbe.java` + a HotSpot 25 control transcript. |
...
| `W7-36-differential-view-families.md` | `stream.reuseThrows` is **closed** by `W7-65-stream-reuse-throws.md` — the "~99 call sites" were 99 substring matches over three functions that all bottom out in one funnel, so the flag needed one check-and-set, not 99; that record names the residuals it left on purpose (primitive streams, the deferred intermediate op in Compatible mode, `close`/`onClose`). Still open here: the synthetic-mode `EmptyStackException` follow-up; `native_tm_get_or_default`; five TreeMap/TreeSet null-and-bound type checks that return where the JDK refuses. |
...
| Are W7-20's baselines correct now? (W7-62) | `bash regression-suite/bridge-ratchet.sh` — one census, both gates. Expect the kind map to PASS and the bridge ratchet to fire by exactly +10 `without_acc_native`, +1 `shadows_bytecode_anywhere`, −2 `kind_disagreements`. Anything else is a finding, not a re-freeze. |
| Is `stub_ratchet` at 1269/1259? (W7-62) | `cargo test -p cratonvm-native-builtins --test stub_ratchet -- --nocapture`. Expected to FIRE; the rise is a §1.4 shadow retirement, the motion the file's own `939 -> 1038` section documents. **Paste what it prints, not the 1269.** |
| Does `probes/ListItrInterfaceProbe` go green in both modes? (W7-62, W7-16) | `javac -d /tmp/probeout probes/ListItrInterfaceProbe.java`, then both arms with `--java-home`. Expect 16/16 in each; `al.*` red means broken instrument, not finding. |
