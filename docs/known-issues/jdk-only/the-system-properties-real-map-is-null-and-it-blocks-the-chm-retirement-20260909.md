...
* Nothing about `--real-jdk`'s BEHAVIOUR, which this change cannot reach: the
  ...
  **Wrong, closed 2026-09-18.** `resolve_native_dispatch_wave1` is not the
  only consumer of the re-tagged kind: `../../../vm/src/vm/vm_exec.rs`'s
  `invoke_or_native` reads `native_kind == SyntheticStub` directly, for the
  `CRATONVM_REAL` differential switch, in `Compatible` mode, independently of
  `--jdk-only` and of the §7 write-back. `CRATONVM_REAL=all` therefore DOES
  reach the retired `java/util/Properties` triples from plain `--real-jdk`,
  landing on the exact NPE quoted in §1
  (`jdk/internal/util/StaticProperty.<clinit>`, reached from VM BOOT — before
  any application class loads, via `cargo test -p cratonvm-vm --test
  synthetic_diff synthetic_vs_real`). Fixed by extending the §3/§7 "populate
  the real map" mechanism to fire under `CRATONVM_REAL` too, not only
  `--jdk-only`: a new `NativeContext::prefers_real_bytecode(class_name)` seam
  (`../../../native-api/src/registry.rs`, implemented in `../../../vm/src/vm/vm_exec.rs` as a
  thin wrapper over `real_bytecode_selector().prefers_real(..)`, since
  `native-builtins` cannot depend on `vm` to call that function directly), a
  per-call check in `native_system_get_properties` (mirrors what
  `native_system_get_properties_jdk_only` always does), and a new
  `native_system_init_phase1_real_selected` wrapper (mirrors
  `native_system_init_phase1_jdk_only`'s four-field publish cluster —
  `System.props`, `SharedSecrets.javaLangAccess`, `VM.savedProps`,
  `ClassLoader.scl` — needed because `VM.savedProps` is read EAGERLY by
  `jdk/internal/loader/ClassLoaders.<clinit>`, before any per-call publish
  could reach it). Default `Compatible` mode with `CRATONVM_REAL` unset is
  unchanged: `prefers_real_bytecode` returns `false` and every new call is a
  no-op.

  Fixing this exposed the NEXT divergence in the same test, unrelated and not
  fixed here: `new TreeMap<>(collectorsToMapResult)` returns empty under
  `CRATONVM_REAL=all`, most likely the same species of bug one class over
  (`java/util/HashMap`'s real `table` field, never populated, the way
  `Properties.map` wasn't) — flagged separately rather than folded in, since
  this page's fix is scoped to the `Properties`/`System`-properties cluster
  and HashMap's blast radius has not been measured.
