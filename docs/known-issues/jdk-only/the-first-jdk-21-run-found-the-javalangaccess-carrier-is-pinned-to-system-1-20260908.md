...
**Status: FIXED 2026-09-09 — all four defects this page found are closed.**

| # | defect | closed by |
|---|---|---|
| 1 | `JavaLangAccess` pinned to `System$1` — `parkVirtualThread` | the carrier is resolved from the image, 2026-09-09 |
| 2 | the same pin — `encodeASCII` | same change |
| 3 | `serialization` round-trip returns the wrong class | `jdk-21-serialization-round-trip-returns-the-wrong-class-FIXED-20260909.md` |
| 4 | `textformat` separators are US-style for every locale | `jdk-21-strict-mode-answers-a-non-root-locale-with-root-resources-FIXED-20260909.md` |

**Measured on the jdk-21 Linux strict corpus after the carrier fix:** 8 of the
10 baselined `21-linux` rows report NO LONGER DIVERGING, including every
`NoSuchMethodError` row and all three `JdkOnlyCensusLoadProbe/strict` rows. The
two that remain are `JdkOnlyPlatformProbe/{real,strict}/vthreads`, and they are
**one cell** — `handoffs=64` on HotSpot against `handoffs=57` on both CratonVM
arms, every other field on the line identical. That cell is the load-fragile
counter §7 of
...
measured at 64 on eight of eight runs on BOTH VMs on an idle host, and ruled
unusable as an oracle. The host these runs were taken on had a load average of
3.7. **It is not a carrier residual and it is not a JDK 21 story** — the same
cell is what the `25-linux` and `25-windows` keys' vthreads rows are.

**This page stays at this path** rather than moving to `docs/internal/retired/`
like the two pages named in the table above. `scripts/baselines/jdk-only-strict-corpus-21-windows.txt`
...
* **No fix is proposed.** §1 names the mechanism and the 33 call sites; it does
  ...
  > **SUPERSEDED 2026-09-09 — that lane ran, and the shape guessed here is the
  > shape that landed.** `JLA_CARRIER_CANDIDATES` lists both names that have
  > carried the interface; `resolve_jla_carrier` asks the loaded class whether
  > it is assignable to `jdk/internal/access/JavaLangAccess` and mints the
  > singleton under whichever answers yes. The four static sites (registrar,
  > `defineClass`, the `force_native_over_real_jdk_bytecode` list, the
  > synthetic-stub layout table, and the strict-mode allowlist in `vm_exec.rs`)
  > take BOTH names instead: `System$2` does not exist on 25 and `System$1` is
  > not the carrier on 21, so registering under both is a superset that is
  > inert on the image where it does not apply, and only the MINT has to
  > choose. What this page could not have said, and the run did: the repair is
  > worth 8 of the 10 rows in the `21-linux` key.
