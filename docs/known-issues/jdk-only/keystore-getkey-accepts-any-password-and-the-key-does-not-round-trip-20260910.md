...
## 9. KS-7 is FIXED, 2026-09-17, and KS-4 alongside it

`recover_key_any_scheme` gained the third scheme §6 named:
`recover_jceks_pbe_key` (`../../../native-builtins/src/keystore.rs`), reverse-engineered
from the JDK 25 `src.zip`'s `com.sun.crypto.provider.{JceKeyStore,KeyProtector,
PBES1Core}` (not a guess — the source was read, not decompiled blind). A real
`JceKeyStore` private-key entry is a PKCS#8 `EncryptedPrivateKeyInfo` under OID
`1.3.6.1.4.1.42.2.19.1` (`JAVASOFT_JCEKeyProtector`), whose params are a PBE
`SEQUENCE { salt OCTET STRING, iterationCount INTEGER }` and whose
`encryptedData` is `PBEWithMD5AndTripleDES` (CBC, PKCS5Padding) ciphertext of
the plaintext `PrivateKeyInfo` — a DIFFERENT envelope from both the JKS
`KeyProtector` this VM's own JCEKS writer uses and PKCS#12's PBES2/legacy bags,
confirming the cause §6 named. Key derivation is `PBES1Core.deriveCipherKey`'s
`DESede` branch transcribed directly (salt-half MD5 iteration, `des::TdesEde3`
+ `cbc` for the cipher — same RustCrypto family as the `aes`/`cbc` crates
already in the tree).

MEASURED on `cratonvm` (release), `../../../apps/probes/KSInteropWriteRead.java`, JDK
25.0.4+7 on Linux — every one of the 12 writer×reader×store-type×password
cells now matches HotSpot exactly (up from 8 of 9 in §8, using the finer
12-cell matrix that also exercises `writer=cratonvm reader=hs`):

```text
   writer=hs  JCEKS  reader=cratonvm   ENTRY pw   RSA/PKCS#8/usable   <- KS-7, was NOT-A-KEY/THREW
                                       STORE pw   THREW UnrecoverableKeyException
                                       WRONG pw   THREW UnrecoverableKeyException
```

KS-4 (cosmetic, §1) closed alongside it: `KeyStoreError::JksMacMismatch`'s
message is now `"Keystore was tampered with, or password was incorrect"`,
verbatim from `JavaKeyStore.engineLoad`/`JceKeyStore.engineLoad` (both throw
identical text), replacing `"JKS HMAC integrity check failed"`. Verified for
JKS and JCEKS with a small reload-with-wrong-password probe; PKCS#12's own
message text (`"PKCS#12 MAC verification failed (wrong password?)"` vs
HotSpot's `"keystore password was incorrect"`) is a separate, undocumented
difference this record never claimed and is untouched.

**What remains open, and is NOT closed by this**: KS-2/KS-3 (the four-slot
mirror `getKey` returns instead of a real key object, so `equals`/round-trip
still read `false`) and §3's registrar-retirement blocker (the TLS identity
side table). Both need "the class's state has to become real before its
shadow can be retired," the same larger project every other family in this
campaign hit — `LTKeyStoreEngineSweep` therefore stays unpromoted; its
round-trip and `setEntry`-equality rows are still red for that reason, unarmed.
