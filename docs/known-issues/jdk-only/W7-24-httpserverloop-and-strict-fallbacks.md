...
> **RESOLVED 2026-09-18** (spring `--jdk-only` lane,
> `docs/internal/fixed-suite-bugs/spring/spring-jdkonly-673-abends-and-failures-FIXED-20260918.md`).
> The second of the two options named at the end of this section landed: the
> subscription is a **real class defined from bytes the VM ships**
> (`native-builtins/src/class_bytes/HttpBodyReplaySubscription.java`, compiled
> with `javac --release 17`, embedded with `include_bytes!`), defined once per VM
> by `re5_new_real_replay_subscription` and used only when `ctx.is_jdk_only()`;
> `--real-jdk` keeps the fabricated carrier unchanged. Its `request(n)`/`cancel()`
> are the Java form of the two natives below, with the same claim-before-deliver
> ordering, so delivery stays synchronous on the requesting thread and the
> `ofPublisher()` case is not touched. `HttpClient.send` with a custom
> `BodyHandler` under `--jdk-only` now returns the body (probe: local
> `HttpServer` + `HttpClient.send(req, info -> BodySubscribers.ofString(..))`,
> plus `ofInputStream()` and `ofString()`). The text below is kept as the record of
