...
> **FIFTH PASS — 2026-09-17, doc-retirement groundwork. This file is now
> HISTORICAL and frozen; do not rebuild it.** Every pass below this one
> predates the campaign's own close: lanes L0, L1, L2, L3, L5, L6, L7 and LT —
> the "remove all synthetic bridges shadowing real bytecode" effort this file
> was tracking — retired between 2026-09-10 and 2026-09-16
> (`docs/internal/jdk-only/lane-0-integration-and-gates-RETIRED-20260916.md`).
> Lane L4 is the one exception and is still active; its own page,
> [`jdk-only-lanes/lane-4-io-nio-foreign.md`](lane-4-io-nio-foreign.md),
> is authoritative for it, not this file.
>
> **For current status, use
> [`the-six-acceptance-criteria-measured-20260910.md`](the-six-acceptance-criteria-measured-20260910.md)**,
> not this index and not `README.md` §1's older normative summary. **For the
> backlog that followed the campaign's close, use
> [`lane-backlog-baseline-and-diagnostics-20260917.md`](lane-backlog-baseline-and-diagnostics-20260917.md)**,
> which opens six new lanes over the residual defects the closed campaign's
> own retirement pages named as open. Rebuilding this file's per-record table
> against the ~350 records added since the FOURTH PASS is explicitly **not**
> attempted here — most of that growth is now-closed defect history from a
> finished campaign, better served by a retirement sweep (moving closed
> records to `docs/internal/jdk-only/`, the way this directory's own earlier
> `RETIREMENT-*.md` audits did) than by one more snapshot table that rots on
> the same schedule as the four before it. That sweep is real, sizeable work
> (~350 records to read and classify) and is intentionally left as a follow-up
...
| record | subject | status | prov | notes |
|---|---|---|---|---|
| C7-1-map-values-is-an-abstractcollection-not-a-list | `Map.values()` returns an `ArrayList` | **RETIRED to `internal/jdk-only/` 2026-09-17** | PRED | resolved — see `internal/jdk-only/C7-1-map-values-is-an-abstractcollection-not-a-list-RETIRED-20260917.md`. Was: partial. **Banner**: the inherited-method hazard does not exist; the rewrite costs +17 registrations |
...
| C13-1-the-interface-doors-never-open-for-a-values-view | the doors never open; routing layer landed | **RETIRED to `internal/jdk-only/` 2026-09-17** | MIXED | resolved differently than proposed — see `internal/jdk-only/C13-1-the-interface-doors-never-open-for-a-values-view-RETIRED-20260917.md` |
| C13-2-the-five-values-view-classes-are-not-one-family | per-class native needs for the five view classes | **RETIRED to `internal/jdk-only/` 2026-09-17** | MIXED | resolved differently than proposed — see `internal/jdk-only/C13-2-the-five-values-view-classes-are-not-one-family-RETIRED-20260917.md` |
...
- [G86-1](../../internal/jdk-only/G86-1-the-two-lists-are-one-and-already-dead-20260819-RETIRED-20260917.md) — **RETIRED to `internal/jdk-only/` 2026-09-17** (joining its already-archived `G79-1`-`G85-1`/`G88-1`/`G89-1` siblings). The P0 *Duplicate dispatch* row's "two lists that disagree" were centralised into ONE predicate and the StringJoiner exception retired on 2026-08-04; and under `--jdk-only` the list cannot fire at all (its guard needs a SyntheticStub, of which strict mode registers zero). Does NOT close the row — the JIT is a third location and is untouched.
- [G87-1](../../internal/jdk-only/G87-1-the-jit-third-location-measured-20260819-RETIRED-20260917.md) — **RETIRED to `internal/jdk-only/` 2026-09-17**. The P0 *Duplicate dispatch* row's JIT "third location", measured under strict: 0 direct native binds, 0 inline-cache natives, and 38 fast-path admissions REFUSED (the field name reads as the opposite of what it counts). Live in `--real-jdk`, neutralised in `--jdk-only`. Does NOT close the row.
...
- HANDOFF-20260819 (retired: `HANDOFF-20260819`) — where the goal stood at wave G, the four-step method that produced G90-1, the instruments and what each lies about, how to run the arms without losing an hour, and the standing traps. Delinked 2026-09-06: the page is not in the tree and is not in the internal tree either, so the link was the only thing pointing at it — and it was the single issue `tools/check_markdown_links.py` reported on the whole repository, which is a gate the next reader has to get past. Fifteen rows in this file already use this `retired:` form for exactly this reason; the summary is kept because it is the only surviving description of what that handoff said.
...
- [H1-1](../../jdk-only/H1-1-the-sink-that-capped-every-count-20260820.md) — `FIXED-UNVERIFIED` at writing, **since MEASURED** (census live, strict arm 104/104). Two truncations, not one: the producer and the consumer were gated on the same `FULL` flag, so once the sink filled the VM stopped *walking* for shadows and `interpreter_shadow_unenforced` froze — a drop counter alone would have counted nothing. Cap 256→4096, drop counter, `truncated`, `CRATONVM_NATIVE_SHADOW_SINK_CAP`, and the census now prints on every strict arm. **Also: the P0 *Native-first dispatch* row's premise expired 2026-08-04** — `COPY 1 OF 2` is gone (`b90f9a614`), there is ONE predicate with twelve entries, and `real_protected_stub_class` is not in the file the row names. The two-column table the row asks for would have had two provably identical columns; source-witness tests freeze the one-predicate property instead.
