...
| --- | --- | --- | --- |
| **L5 reflection & class metadata** | **COMPLETE 2026-08-29; its residual CLOSED 2026-08-30, and the follow-on round found nine more.** Original: dispatch worklist 483 rows / 20 fixed, plus its two recorded-open items and five more the probe found — **28 defects over 608 rows**, 1 residual (`invoke`'s reference-argument cast). Since: the Groovy 18-class corpus cluster root-caused to one null `Set` (`groovy-died-on-a-null-set-…`); **11 refusal-type defects on the three `Lookup.define*Class*` doors**, whose first fix landed on a shadowed registrar and changed nothing (`the-fix-that-changed-nothing-…`); and **the residual itself plus `bindTo`, `invokeWithArguments` and `Method.invoke`'s interface formals** (`the-cast-that-asType-performs-…`). `L5ModuleInvokeSweep` and `L8InvokeLookupSweep` are now 0-diff. One item OPEN and owned: `VarHandle` checks neither its receiver nor its value (`varhandle-checks-neither-…`). Records: `L5-reflection-lane-complete-20260828.md`, `L5-residuals-module-packages-and-invokeexact-20260828.md`, and the four dated pages above | `C:\craton\cratonvm\.claude\worktrees\h2-known-issues-206dee` | `claude/jdk-only-mode-handoff-09b48c` |
...
| `VarHandle` checks neither its receiver nor its value — an `Integer` STORED into a `String`-declared field with no error, and a read through a receiver of the wrong class that RETURNED a value | **L5 (mine), MEASURED 2026-08-30, OPEN.** 5 rows of `probes/ReflectArgTypeSweep.java`, both modes, so an ordinary defect. Not a one-line fix: the six `VH_KIND_INSTANCE` arms are the CAS-dominated paths the file has been tuned for twice, so it wants the fast-path design (`meta.class_id` integer compare first) and its own kill switch before it is profiled. `varhandle-checks-neither-its-receiver-nor-its-value-20260830.md` has the design |
| One rule has THREE spellings: `lang_class::coerce_arg_strict_msg`, `lang_invoke::reference_arg_admitted` and `typecheck::aastore_element_assignable` all decide reference assignability | **PARTLY CONSOLIDATED 2026-08-30.** `NativeContext::class_assignable_to_name` now exposes the loader-blind by-name walk the `checkcast`/`aastore` path already used, and the two reflective spellings call it for interface formals — which is what closed `m.interfaceWrong` and `x.interfaceArgWrong`. They are still three functions. Anyone touching reference assignability should collapse them rather than add a fourth |
