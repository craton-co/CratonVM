...
> **Deleted and restored — 2026-08-13.** This page was removed from the tree by
> `ad3393f7c` ("new netty bug docs"), which touched no other GC file and left
> six pages — including `docs/gc-tuning.md` and `docs/GC.md`, which cite it as
> the justification for making ZGC the default collector — pointing at a path
> that no longer existed. Restored unmodified from `ad3393f7c^`. It is read as
> current evidence by Phase 1 of
> [`zgc-maturity-assessment-and-plan-20260813.md`](../../feature-designs/zgc-maturity-assessment-and-plan-20260813.md),
> whose own finding is that **the 08-11 rows below supersede the 08-10 rows that
> those two pages actually quote.**
