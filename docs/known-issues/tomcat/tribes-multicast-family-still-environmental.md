...
## Update 2026-09-17 — the outbound-policy symptom is superseded, not re-verified here

`link-local-outbound-policy-blocks-legitimate-local-services-RETIRED-20260917.md`
(now in `docs/internal/jdk-only/`) narrowed the outbound SSRF guard from the
entire `169.254.0.0/16`/`fe80::/10` link-local ranges down to the specific
known cloud-metadata addresses. `TestRemoteProcessException`'s
`fe80::67b0:99e:5a9b:287e` is not one of those addresses, so the exact
`connect denied by outbound policy` exception quoted below will no longer
fire for that reason — CratonVM will now attempt the connect, same as
HotSpot. **Not re-verified**: the full Tomcat/Tribes suite was not re-run
after that fix (it needs a Tomcat checkout this session did not have), so
whether `TestRemoteProcessException` now fails with a *different* symptom, or
passes, or still fails the same JUnit assertion for the underlying
multicast-does-not-work-on-this-host reason, is unconfirmed. This page's core
diagnosis is unaffected either way: the family's other three classes
(`TestDataIntegrity`, `TestMulticastPackages`, `TestUdpPackages`) never
touched the outbound policy and remain environmental for the reason below.
