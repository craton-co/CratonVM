...
| | |
|---|---|
| **Status** | Regression from `fb4a333d` FIXED (2026-07-07) by reverting the unconditional reason-8 (`UnreachedCode`, the invokedynamic uncommon trap) routing through the precise frame-deopt trampoline, restoring the pre-`fb4a333d` "safe reject" behavior for THAT one trap reason only. **This reopens the original silent-data-corruption risk `fb4a333d` fixed** (see "Tradeoff" below) — a real fix that closes both bugs at once is still needed. |
