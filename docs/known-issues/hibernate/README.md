...
- [hib-boot-models-xml-qname-jandex-cluster.md](hib-boot-models-xml-qname-jandex-cluster.md) — 9 classes (4 QName CCE + 5 Jandex indexing, co-occurring). Boot-time XML/annotation model-building gap.
- [hib-misc-singleton-failures.md](hib-misc-singleton-failures.md) — ~16 classes across several small independent clusters (`EOFException` serialization, `InvalidMappingException` XML-mapping parse, `SyntaxException` HQL boolean-negation, `SQLGrammarException` ×2 including a possible in-process-javac regression, `UnknownNamedQueryException`, `CannotContainSubGraphException`, `FailureExpectedExtension$ExpectedFailureDidNotFail` — the latter NOT a defect).
- [hib-connections-proxy-serializationexception-cluster.md](hib-connections-proxy-serializationexception-cluster.md) — 5 classes. `could not deserialize` on connection-provider/proxy serialization round-trip (likely related to the `EOFException` cluster above).
