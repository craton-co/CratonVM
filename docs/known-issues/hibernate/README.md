...
- [serializationhelpertest-null-classloader-forname-scope-leak.md](serializationhelpertest-null-classloader-forname-scope-leak.md) — `SerializationHelperTest.testSerializeDeserialize` FAILs deterministically: `Class.forName(name, false, null)` (explicit bootstrap-only loader) incorrectly resolves application-classpath classes via CratonVM's flat global class store instead of throwing CNFE, so `SerializationHelper`'s loader-fallback chain never reaches the correct (already-loaded) custom-classloader `Class` object. Root-caused to `native-builtins/src/lang_class.rs::native_class_for_name`'s `effective_loader` match collapsing "explicit null loader" and "no loader arg" into the same branch. Distinct from the 2026-06-22 `getResourceAsStream` parent-delegation bug on this same test class (already fixed, see below) — this is a different, previously undocumented residual. OPEN, not fixed.
