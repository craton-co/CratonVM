...
2. ~~`TestClientCert`/`TestCustomSslTrustManager`'s issuer-check gap
   ...
   **IMPLEMENTED and verified via direct repro** (see "Residual #2
   implementation" above). (a) confirm end-to-end on the actual Tomcat
   suite once the classloading/vtable-install deadlock (item #7, shared
   with the cipher-restriction session) is fixed — **DONE**: that deadlock
   was fixed on dev (`60e2b20d`/`48b3c2d2`) before the 2026-07-07 follow-up
   session below ran; no hang reproduced. (b) root-cause why Tomcat's
   connector never sets `need`/`want` client auth to `true` — **DONE, and
   RECLASSIFIED**: root-caused precisely (`engine_begin`'s
   `state.conn.is_some()` short-circuit silently discards Tomcat's
   `SSLEngine.setNeedClientAuth(true)` + rehandshake), but the underlying
   operation Tomcat needs — real TLS renegotiation on an established
   connection — is something rustls 0.23 categorically refuses to perform
   on either side (confirmed directly from its own source/docs). This is
   **not a CratonVM configuration bug** to fix in a future session; it is a
   **permanent, upstream-dependency limitation**, the same class as the TLS
   1.2 DHE cipher case in item 3 below. See "Residual #2 follow-up
   (2026-07-07...)" below for the full trace, the workaround that was tried
   and its measured (non-improving) tradeoff, and two independent, real
   bugs found and fixed along the way (`KeyManagerFactory.getKeyManagers()`/
   `TrustManagerFactory.getTrustManagers()` returning non-functional
   bare-interface-stamped objects — a general-purpose fix, not specific to
   this doc's scenario).
...
This section previously tracked a "severe regression affecting ALL native HttpsURLConnection connections" and recommended a bisect between `40df7946` and dev. Both the framing and the bisect plan turned out wrong; kept here corrected because the failure signature is distinctive and likely to be searched for again:

- **The bisect endpoints were invalid.** Plain `40df7946` does NOT pass `TlsRepro3` (it fails as `SocketTimeoutException: Read timed out`); what the earlier session verified passing was `40df7946` **plus** its own `chooseClientAlias` branch — and `5db12fe5` (that fix as merged into dev history) also fails. A `git bisect run` across the recommended range converges vacuously on the first commit in range (`037672d5`, the StringBuilder count fix — unrelated). The failure *shape* changed over time, but `TlsRepro3` never passed on dev mainline in this environment.
- **Scope was narrower than "all HTTPS":** the client side (`http_url_connection.rs` rustls path) was fine — the stall was on the **server** side of any real-JDK `com.sun.net.httpserver.HttpsServer` exchange (and by extension anything driving the server-side `SSLEngine` through `result.getStatus()`/`result.getHandshakeStatus()` identity comparisons). Tomcat's NIO driver mostly consults `engine.getHandshakeStatus()` (which already returned real singletons), which is why the Tomcat TLS suites kept passing.

**Root cause 1 (the stall):** `register_p68_ssl` (`phases_late.rs`) registers SyntheticStub accessors for `javax/net/ssl/SSLEngineResult` — `getStatus()`/`getHandshakeStatus()` returned a FRESH 2-slot fake enum (name@0, ordinal@1) on every call, and `bytesConsumed()`/`bytesProduced()` misread real objects. Stub-dropping is opt-in (`CRATONVM_NO_STUBS`), and since the httpserver-pkcs12 fix `register_p68_ssl` runs on the real-JDK path too (registration is last-wins, shadowing both the real bytecode and an older duplicate in `tls.rs`) — this is when the stall started. A real result's enum-reference field read through `.as_int().unwrap_or(0)` produced "OK"/"NOT_HANDSHAKING"-named fakes that `==`-match no real singleton, so `sun.net.httpserver.SSLStreams.recvData` never saw `NEED_WRAP`, never called `doHandshake`, never sent a ServerHello — the server busy-spun in `recvAndUnwrap`'s `chan.read()==0` loop (zero syscalls; caught via `--stack-dump-on-timeout` at `recvAndUnwrap` pc=102 + strace showing ClientHello received, nothing sent) while the client timed out with the EAGAIN. Same stub-shadows-real-bytecode family as the `SymbolLookup` and `KeyManagerFactory` fixes above. Fix: both accessor copies (phases_late.rs the live one, tls.rs the shadowed one, now cross-referenced) are value-aware — real enum references pass through (by-name first), int codes resolve the REAL enum singleton via `valueOf`, byte counts read the real fields.
...
This session picked up exactly where "Residual #2 implementation" point 2 and "Recommendation for follow-up sessions" point 2(b) left off: root-cause why Tomcat's connector never sets `need`/`want` client auth to `true` on the native `SSLEngine`. Re-verified the symptom first against a fresh dev-tip baseline (commit `33bef88d`, includes `f94c13a9`/`33bef88d` from the task brief) — **identical to the doc's prior description**: `CRATONVM_DBG_TLS_AUTH=1` across a full `TestClientCert` run showed `setSSLParameters id=N need=false want=false` and `engine_begin request=false client_ca_none=true` for all 11 engines. Baseline: `TestClientCert` 13/18 pass (5 fail), `TestCustomSslTrustManager` 7/9 pass (2 fail) — exactly matching the doc's prior numbers.

### Root cause 1 (confirmed, real, but NOT the final blocker): `engine_begin`'s `state.conn.is_some()` early-return silently discards Tomcat's rehandshake

Tomcat's `SSLHostConfig.certificateVerification` stays `NONE` throughout every test in this cluster (never explicitly set) — client-cert auth is driven **entirely** by a mid-connection mechanism: `NioEndpoint$NioSocketWrapper.doClientAuth` (`java/org/apache/tomcat/util/net/NioEndpoint.java`) calls `engine.setNeedClientAuth(true)` directly on the live `SSLEngine`, then `SecureNioChannel.rehandshake()`, which calls `sslEngine.beginHandshake()` again on an engine that already completed its first handshake (to receive the HTTP request line before the servlet layer can decide preemptive-vs-protected). Traced via temporary `eprintln!` instrumentation (`DIRECT setNeedClientAuth id=N v=true conn_already_realized=true` / `engine_begin SHORT-CIRCUIT`): the direct `setNeedClientAuth(true)` call **does** fire and **does** correctly update `state.need_client_auth` — but `engine_begin` (`t27_tls.rs`) returns immediately whenever `state.conn` is already `Some(...)` (set by the first handshake), without ever re-consulting the updated flag or rebuilding the rustls connection. This confirms the doc's own framing was accurate as far as it went, but the "why" was previously unverified — it's now traced end-to-end.

**This turned out not to be fixable as a CratonVM bug**, because the underlying operation Tomcat is asking for — a real TLS 1.2/1.3 renegotiation on an already-established connection — is something **rustls 0.23 categorically refuses to do on either side**, confirmed directly from the dependency's own source and documentation (`rustls-0.23.38/src/common_state.rs::process_msg`: any post-handshake `ClientHello` (server side) or `HelloRequest` (client side) is unconditionally rejected with a `no_renegotiation` alert once `may_receive_application_data` is true, which happens the instant the first handshake completes — there is no window in which a genuine renegotiation attempt would be processed instead of rejected; `rustls-0.23.38/src/manual/tlsvulns.rs`: *"rustls does not support renegotiation in TLSv1.2. TLSv1.3 also no longer supports renegotiation."*, a deliberate upstream security decision, not an oversight). This is the same class of gap as this doc's already-documented, permanently-unfixable TLS 1.2 DHE cipher case (residual #3 closeout) — closing it would require swapping to a different TLS backend (e.g. a direct OpenSSL binding) capable of driving real renegotiation, not a bug fix within the current dependency set.

**A workaround (speculative optional CertificateRequest on the first handshake) was implemented, measured, and reverted** — see the `t27_tls.rs::engine_begin` server-identity branch's own "INVESTIGATED, NOT APPLIED" doc comment for the mechanism and full measurement. Summary: it made the server always offer an OPTIONAL client-cert request on the very first handshake whenever the SSLContext carried trust roots, sidestepping the need for renegotiation entirely (a client with a KeyManager installed volunteers its cert immediately; one without behaves identically to no request being sent at all, via `WebPkiClientVerifier::allow_unauthenticated()`). It genuinely worked end-to-end — `TestCustomSslTrustManager` improved 2/9 → 1/9 failing (remaining failure = the doc's already-known `testCustomTrustManagerNone` order-dependent flake) — but `TestClientCert` stayed at 5/18 failing with a **different, not-strictly-better** failure set: `testClientCertGetWithPreemptive` newly passed, but `testClientCertPostLarger` newly failed, because `TestClientCert.doTestClientCertGet`/`doTestClientCertPost` both contain an explicit `Assert.assertEquals(0, TesterSupport.getLastClientAuthRequestedIssuerCount())` after the **first** (unprotected-resource) request — the suite is deliberately verifying that NO CertificateRequest is sent until a protected resource actually needs one. A speculative upfront request can only ever satisfy the "preemptive" half of this suite's intent while breaking the (larger) "non-preemptive" half's own explicit assertion; it cannot net-improve `TestClientCert` without implementing the actual deferred/mid-connection request the suite wants, which circles back to requiring real renegotiation. Reverted (kept as dead code behind a hardcoded `false`, not deleted, since the surrounding wiring is correct and reusable if a future session finds a renegotiation-capable path, e.g. a direct OpenSSL-backed engine variant).

### Root cause 2 (found, fixed, real, and independently valuable): `KeyManagerFactory.getKeyManagers()` / `TrustManagerFactory.getTrustManagers()` returned non-functional objects — `AbstractMethodError` on direct use

While root-causing #1, moving `chooseClientAlias` earlier (during the speculative-auth experiment) surfaced a **second, completely independent, pre-existing bug** that had never been exercised before: `phases_late.rs`'s `KeyManagerFactory.getInstance()`/`getKeyManagers()` (the ACTIVE registration — allocates its own synthetic KMF object rather than going through the real SPI `factorySpi.engineGetKeyManagers()` delegation chain `x509_manager.rs`'s `kmf_engine_get_key_managers` correctly backs) returned `alloc_concurrent_synthetic(ctx, "javax/net/ssl/X509KeyManager", 0)` — an object stamped with the **bare interface's own class id** (confirmed via `CRATONVM_DBG_NOCODE=1` tracing: `recv_cid=1122 recv_class=javax/net/ssl/X509KeyManager`). All 6 `X509KeyManager` methods are `abstract` with no Code, so any caller invoking one **directly** (rather than through this crate's own `t27_tls::JavaKeyManagerResolver`, which never calls `getKeyManagers()` at all — it captures the KeyManager array straight off `SSLContext.init()`'s arguments) hit `AbstractMethodError` deterministically, 100% of the time, on every attempt — confirmed NOT a timing/vtable-visibility race (an incorrect hypothesis this session initially chased: `ensure_class_initialized` immediately before the call never helped, and a bounded retry-with-backoff failed identically on every attempt, every time). Real-world trigger: Tomcat's own `TesterSupport.TrackingKeyManager`/`TrackingExtendedKeyManager` test wrappers (`test/org/apache/tomcat/util/net/TesterSupport.java`) delegate to exactly this object (`manager.chooseClientAlias(keyType, issuers, socket)`), and `TrustManagerFactory.getTrustManagers()` had the **identical** bug (`alloc_concurrent_synthetic(ctx, "javax/net/ssl/X509TrustManager", 2)`) — `getAcceptedIssuers()` on that shape was already known and fixed via a narrow per-interface native (`FIX (x509-trustmanager-abstractmethod-20260707)`, `t27_tls::register_accepted_issuers`), but `checkClientTrusted`/`checkServerTrusted` need real per-instance trust state that a single interface-wide native can't provide, so they'd never been given an equivalent fix and kept throwing for Tomcat's post-handshake `engine_run_trust_check` the moment a caller (any caller, not just this session's changes) invoked one directly.

**Fix:** both handlers now build the SAME real, natively-backed objects `x509_manager.rs` already produces for the SPI-delegation path (`FQN_SUN_X509_KM` = `sun/security/ssl/SunX509KeyManagerImpl`, `FQN_X509_TM` = `sun/security/ssl/X509TrustManagerImpl`), registered into the same `km_registry`/`tm_registry` id spaces `chooseClientAlias`/`getPrivateKey`/`checkClientTrusted`/`checkServerTrusted` already consult correctly. `KeyManagerFactory.init(KeyStore, char[])` and `TrustManagerFactory.init(KeyStore)` stash the resolved keystore/tm id via a new identity-hash side table (`kmf_keystore_id_by_identity`/`tmf_tm_id_by_identity` in `phases_late.rs` — a real bytecode `KeyManagerFactory`/`TrustManagerFactory` object has no room for a `cratonvm$...` pseudo-field and its existing field slots are already in active use for provider/algorithm/keystore-ref, so `set_field_by_name`/reusing `set_km_id`'s own field-0 write would either silently no-op or corrupt an unrelated field — same trap this file's other helpers were built to route around). `TrustManagerFactory.init(ManagerFactoryParameters)` — the overload Tomcat's own `SSLUtilBase.getTrustManagers()` actually calls whenever `sslHostConfig.getTruststoreAlgorithm()` is `"PKIX"` (the default; confirmed via tracing this is the overload the **server** side of every mTLS test in this suite uses, distinct from the test harness's client-side `init(KeyStore)`) — previously did nothing at all for this purpose; a new shared helper (`x509_manager::build_and_register_tm_state_from_mfp`, extracted from the existing correct `tmf_engine_init_params`) now walks `CertPathTrustManagerParameters.getParameters().getTrustAnchors()` the same way and registers a real `TrustManagerState` for it too.

Promoted from private to `pub(crate)` in `x509_manager.rs` (mirroring this doc's own established precedent for `jca::provider_chain::find`/`make_provider`): `km_registry`, `next_km_id`, `set_km_id`, `FQN_SUN_X509_KM`, `set_tm_id`, `FQN_X509_TM`, plus the new `build_and_register_tm_state_from_mfp`.

### Verified (own freshly-measured baseline, same host/binary lineage as the numbers above)

| Class | Baseline (this session, dev-tip `33bef88d`) | After this session's fix (`29163e3f`) |
|---|---|---|
| `TestClientCert` (JSSE) | 13/18 pass, 5 fail: `GetWithPreemptive`, `PostSame`, `PostZero`, `GetWithoutPreemptive`, `PostSmaller` | **Identical** — 13/18 pass, same 5 failing (root cause 1 above; unfixable without renegotiation) |
| `TestCustomSslTrustManager` (JSSE) | 7/9 pass, 2 fail: `testCustomTrustManagerCA`, `testCustomTrustManagerAll` | **Identical** — 7/9 pass, same 2 failing |
| `TestClientCertTls13` (JSSE) | not re-measured this session; doc's last recorded number was 2/6 fail | **6/6 pass** — no regression (improvement likely from unrelated intervening fixes, not this session's changes) |
| `TestSslHandshakeFailure` | — | **1/1 pass** |
| `TestSSLHostConfigCipher` | — | **11/12 pass** — only `testTls12CipherNotAvailable[JSSE]` fails (the permanently-unfixable DHE case), unchanged |
| `TlsRepro3`-shaped plain HTTPS smoke test | — | **RC=200**, no regression |
| `cargo test -p cratonvm-native-builtins` | 2802 passed / 13 failed (pre-existing: `lang_class::tests::*` ×6, `security_manager::policy::tests::wp68_substitution_dollar_escape_preserves_literal`, `unsafe_jdk25::tests::c39_array_index_scale_derives_from_class_mirror`, `lang_class::jspecify_type_use_parameter_annotations_use_parameter_index`) | **Identical** — same 13 pre-existing failures, zero new. `x509_manager::`/`t27_tls::`/`tls::` module tests: 48/48, 22/22, 81/81 all pass |

**Net honest outcome:** `TestClientCert`/`TestCustomSslTrustManager` counts are unchanged from baseline — this session did **not** move those numbers, despite two real, independently-valuable bugs being found and fixed (the `KeyManagerFactory`/`TrustManagerFactory` bare-interface-stub `AbstractMethodError`, which is a **general-purpose fix benefiting any current or future caller of `getKeyManagers()`/`getTrustManagers()` that invokes a returned manager's methods directly**, not specific to this doc's mTLS scenario) and one genuine, dependency-verified root cause for the remaining gap (rustls's categorical refusal to support TLS renegotiation). The residual #2(b) gap — Tomcat's deferred/mid-connection client-cert request — is **not a CratonVM configuration bug to fix**; it is a **permanent, upstream-dependency limitation**, exactly like the TLS 1.2 DHE cipher case, and should be reclassified as such rather than left as an open action item for a future session to re-attempt without this context.

**Commits:** `93074b21` (debug tracing), `a9370570` (speculative-auth attempt, later disabled), `c9f3b8af`/`e2c6f63f`/`4186cfcd`/`6f400966` (debug tracing for the AbstractMethodError investigation), `a954d8db` (`KeyManagerFactory.getKeyManagers()` fix), `51876208` (`TrustManagerFactory.getTrustManagers()` fix), `627ccd06` (debug tracing), `e7e5e3f7` (`TrustManagerFactory.init(ManagerFactoryParameters)` fix — the overload Tomcat's server side actually uses), `21fc38dc` (A/B test: disable speculative-auth), `29163e3f` (clean up: correct comments, finalize revert). Branch `fix/tomcat-clientauth-engine-config-0707b`, merged to `dev`.
