...
## 2026-07-06 Update — heap-guard mechanism fixed; new proximate blocker found
...
is now **root-caused and fixed on `dev`**: it was the plain-field 16-byte `Value`-slot
tearing bug closed by commits `2dfdfddc` and `5198fccd` (2026-07-06, see that doc for the
full analysis). That fix landed the day after this doc's own evidence was captured, so it
was not yet present when either symptom (the corrupt-cell spam, or this doc's timeout) was
first observed.

This session tried to reconfirm the fix end-to-end but no Maven/`wildfly-core` testsuite
...
**Updated diagnosis:** the corrupt-`Value`-cell diagnostic was very likely a genuine but
secondary symptom of ordinary field-tearing under WildFly's heavy `java.util.concurrent`
usage during the 120-second retry loop — now fixed. The **actual gating blocker** for
"managed servers reach started state" is the MSC real-service-start completeness gap
...
3. Diagnose the domain-mode-specific hang under `CRATONVM_MSC_REAL_START=1` (no exception,
   ...
   services follow-ups.
4. Once boot reaches real sustained concurrent execution again, re-verify this doc's
   timeout is gone and the heap-guard fix from
   `wildfly-domain-heap-corrupt-value-timeout.md` holds under load.
