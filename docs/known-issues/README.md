...
- [`springboot/onclasscondition-fix-blocked-by-heap-corruption.md`](springboot/onclasscondition-fix-blocked-by-heap-corruption.md) — **the largest single cluster** (75 classes, 348 occurrences): `OnClassCondition.addAll` received a live `NullPointerException` object where a `String[]` annotation attribute belongs (`ClassCastException: java.lang.NullPointerException cannot be cast to [Ljava.lang.String;`), breaking `@ConditionalOnClass` broadly. **Root-caused and fixed** (verified against all 75/75 classes), but the fix is BLOCKED from merging — it exposes a separate, unrelated heap-corruption bug (crash/hang) in 3 other test classes that isn't yet understood.
