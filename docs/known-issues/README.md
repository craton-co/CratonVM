...
| # | Bug | Status | Doc |
|---|---|---|---|
| **C** | Deep JIT→JIT recursion overruns the **native** stack (ANTLR `closure()`); same-method recursive edges now route through guarded dispatch except static tail self-jumps, but the full overflow path still needs stack-banging + a fault-recovery handler. Only arises with an *unmerged* cold-path-throughput experiment. | ⚪ **LATENT / PARTIALLY CONTAINED** (not a current blocker; 2026-07-01 recursive-edge routing landed) | [springrepos-extension-hang-jit-throughput-and-deep-recursion.md](springrepos-extension-hang-jit-throughput-and-deep-recursion.md) §6–7 |
