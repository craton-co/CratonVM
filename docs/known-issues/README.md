...
[`bug-h2-charset-cp500-unsupported-FIXED.md`](../internal/fixed-suite-bugs/h2-suite-bugs/bug-h2-charset-cp500-unsupported-FIXED.md)
...
[`dohead-jit-heap-corruption-register-invisibility-FIXED.md`](../internal/fixed-suite-bugs/tomcat/dohead-jit-heap-corruption-register-invisibility-FIXED.md)).
...
- FIXED/RETIRED: [`wildfly-standalone-managed-server-boot-fails-under-surefire-fork.md`](../internal/fixed-suite-bugs/wildfly/wildfly-standalone-managed-server-boot-fails-under-surefire-fork.md) вЂ”
...
- FIXED/RETIRED: [`threadpoolexecutor-execute-dispatch-degrades-to-synchronous-FIXED.md`](../internal/fixed-suite-bugs/threadpoolexecutor-execute-dispatch-degrades-to-synchronous-FIXED.md) вЂ” root cause was the fourth, unpatched dispatch path this doc's own analysis suspected: `try_stackless_invoke`'s direct native lookup. Fixed the same day by generalizing the registry-drop-removal approach (see the `ThreadPoolExecutor` regression entry below) вЂ” verified with this doc's own `ExecProbe3.java`, now printing 3 distinct worker threads instead of one.
...
- FIXED/RETIRED: [`threadpoolexecutor-execute-npe-on-ctl-regression-FIXED.md`](../internal/fixed-suite-bugs/threadpoolexecutor-execute-npe-on-ctl-regression-FIXED.md) (execute) + [`threadpoolexecutor-shutdown-npe-on-mainlock-synthetic-executor-FIXED.md`](../internal/fixed-suite-bugs/threadpoolexecutor-shutdown-npe-on-mainlock-synthetic-executor-FIXED.md) (submit/shutdown) + [`threadpoolexecutor-execute-dispatch-degrades-to-synchronous-FIXED.md`](../internal/fixed-suite-bugs/threadpoolexecutor-execute-dispatch-degrades-to-synchronous-FIXED.md) (real-executor async semantics) вЂ” `Executors.newSingleThreadExecutor()`/`newFixedThreadPool()`/`newCachedThreadPool()` return objects whose `.execute(Runnable)`/`.submit(...)`/`.shutdown()` used to NPE on `ThreadPoolExecutor`'s uninitialized `ctl`/`mainLock` fields, and separately any real `ThreadPoolExecutor.execute()` had degraded to synchronous dispatch вЂ” root cause was a registration-time drop in `native-api/src/registry.rs` (not the originally-bisected `f28d6ae6`; that was a red herring) that starved these synthetic objects' native overrides, compounded by a fourth, unpatched dispatch path (`try_stackless_invoke`) once a narrower fix tried to make the drop receiver-aware. Three overlapping fixes landed the same day from parallel sessions (`fix/tpe-npe-dispatch-20260710` for `execute()`'s NPE, this session's own Executors-real-init fix for `submit`/`shutdown`, then `fix/wildfly-hib32-gate-20260710` generalizing the dispatch fix and closing the async-semantics gap); see the FIXED docs for the reconciliation.
...
- FIXED/RETIRED: [`ES-CRASH-20260709-libs-tdigest-org-elasticsearch-tdigest-sortingdigesttests-0249530511-FIXED.md`](../internal/fixed-suite-bugs/elasticsearch-suite/ES-CRASH-20260709-libs-tdigest-org-elasticsearch-tdigest-sortingdigesttests-0249530511-FIXED.md) вЂ” `JavaLangAccess.findNative(ClassLoader, String)J` is already registered on `java/lang/System$1`/`JavaLangAccess` with the exact crashing descriptor; rebuilding `dev` and rerunning the doc's exact repro confirms the `NoSuchMethodError` and `rc=139` crash no longer occur (now `rc=1`, all 20 tests parsed).
...
- FIXED/RETIRED: [`keycloak-model-realmmodeltest-post-infinispan-liquibase-timeout-FIXED.md`](../internal/fixed-suite-bugs/keycloak/keycloak-model-realmmodeltest-post-infinispan-liquibase-timeout-FIXED.md) - the broader post-Infinispan tracker is closed by the same full-class no-JIT validation.
...
- FIXED/RETIRED: [`expression.spel.*` SpEL `EL1040E` double-literal-suffix parse failures](../internal/fixed-suite-bugs/spring/spring-expression-double-literal-suffix-FIXED.md) - Java `Float.parseFloat`/`Double.parseDouble` type suffixes (`d/D/f/F`) now parse in native-builtins, closing the 33 suffix parse failures.
...
- вњ… RETIRED: [hib-local-windows-rerun-20260707.md](../internal/fixed-suite-bugs/hibernate/hib-local-windows-rerun-20260707.md) вЂ” the 4-shard local rerun remains historical evidence for 15 confirmed fixes, `JpaLargeBlobTest`'s non-hang slow path, the superseded temporal-skew observation, and the harness status-computation false-positive. A 2026-07-08 Azure `dev@22d79b10` recheck with unique binary `/data/data/cratonvm-binaries/cvhiblocalcurrentdev-20260708-160310-rebased` passed `InPredicateTest` (`ok=1`, 60.643s) plus the fixed-bucket sample (`LocalXmlResourceResolverTest` 23/23, `ConfigurationTest` 1/1, `OrmXmlEnumTypeTest` 1/1), so this aggregate note no longer owns an open task.
- вњ… FIXED same day: the SQL-placeholder duplication that rerun re-confirmed (plus 2 more affected classes outside `type.temporal.*` вЂ” `ExtendedEnhancementNonStandardAccessTest`, `FunctionTests`, proving it a general SQL-generation-path defect) was the reopened JIT reason-8 imprecise-resume corruption; see [`../internal/fixed-suite-bugs/hibernate/hib-temporal-sql-parameter-placeholder-duplication-FIXED.md`](../internal/fixed-suite-bugs/hibernate/hib-temporal-sql-parameter-placeholder-duplication-FIXED.md) and the identity-sound precise-resume entry below.
...
- `elasticsearch-lucene-binary-docvalues-range-hangs.md` retired to
  [`docs/internal/`](../internal/fixed-suite-bugs/elasticsearch-suite/elasticsearch-lucene-binary-docvalues-range-hangs.md): its three
...
- вњ… FIXED: `fb4a333d`'s precise-resume routing for the invokedynamic
  ...
  [`../internal/fixed-suite-bugs/jit-invokedynamic-uncommon-trap-precise-resume-groovy-regression-FIXED.md`](../internal/fixed-suite-bugs/jit-invokedynamic-uncommon-trap-precise-resume-groovy-regression-FIXED.md).
...
- FIXED: the post-oldgen Fork6Hard `GC_STRESS` residual is closed by keeping
  ...
  [gcstress-residual-corruption-faces-FIXED.md](../internal/fixed-suite-bugs/gcstress-residual-corruption-faces-FIXED.md).
...
- вњ… FIXED (branch `fix/hib-temporal-placeholder-dup-20260707`, 2026-07-07): the 5
  ...
  [`../internal/fixed-suite-bugs/hibernate/hib-temporal-sql-parameter-placeholder-duplication-FIXED.md`](../internal/fixed-suite-bugs/hibernate/hib-temporal-sql-parameter-placeholder-duplication-FIXED.md);
  the reason-8 saga's remaining follow-ups stay tracked in
  [jit-invokedynamic-uncommon-trap-precise-resume-groovy-regression-FIXED.md](../internal/fixed-suite-bugs/jit-invokedynamic-uncommon-trap-precise-resume-groovy-regression-FIXED.md);
...
- вњ… FIXED (branch `fix/tls-identity-singleton-clobber-20260707`, 2026-07-07):
  ...
  [`../internal/fixed-suite-bugs/http-server-sslengine-identity-singleton-clobber-FIXED.md`](../internal/fixed-suite-bugs/http-server-sslengine-identity-singleton-clobber-FIXED.md).
  ...
  [`../internal/fixed-suite-bugs/reactive-netty-https-sslengine-handshake-underflow-FIXED.md`](../internal/fixed-suite-bugs/reactive-netty-https-sslengine-handshake-underflow-FIXED.md).
  ...
  [`../internal/fixed-suite-bugs/netty-client-socket-write-after-close-nsme-FIXED.md`](../internal/fixed-suite-bugs/netty-client-socket-write-after-close-nsme-FIXED.md).
...
- [crypto/fips1402 CryptoProvider ServiceLoader bootstrap](../internal/fixed-suite-bugs/keycloak/keycloak-crypto-fips1402-cryptoprovider-serviceloader.md) - `ServiceLoader` now sees the FIPS `CryptoProvider`; representative classes no longer fail at `CryptoInitRule.before` with `containersFailed=1`. Residual: the module can still fail later after bootstrap (`CryptoIntegration.getProvider(): init first`).
...
- [IgnoredArtifactsTest.multipleDatasources datasource properties missing](../internal/fixed-suite-bugs/keycloak/quarkus-runtime-ignoredartifacts-multipledatasources-boolean.md) - module-scoped runner working directories now match Keycloak's relative `src/test/resources` setup, and Windows `File.toPath().toUri()` no longer emits `%5C` backslash URIs.
- [LoggingConfigurationTest getPropertyNames stale log-category key](../internal/fixed-suite-bugs/keycloak/quarkus-runtime-logging-getpropertynames-garbage-key.md) - `System.setProperties(Properties)` now replaces the VM-wide properties state, so stale fixture keys do not leak into SmallRye property-name iteration.
- [LoggingConfigurationTest wildcard DEBUG level resolves null](../internal/fixed-suite-bugs/keycloak/quarkus-runtime-logging-wildcard-debug-level-null.md) - fixed by the same System-properties replacement semantics as the stale log-category key.
- [TelemetryConfigurationTest telemetry-service-name wrong value](../internal/fixed-suite-bugs/keycloak/quarkus-runtime-telemetry-service-name-wrong-value.md) - fixed by System-properties replacement/reset semantics.

Already-tracked, not re-documented: the 37 `testsuite/model` CRASHes were the
[Infinispan GlobalConfigurationBuilder.isClustered() NoSuchMethodError](../internal/fixed-suite-bugs/keycloak/keycloak-model-infinispan-globalconfiguration-isclustered-nosuchmethod-FIXED.md),
...
[Infinispan ConfigurationBuilder.build() ClassCastException](../internal/fixed-suite-bugs/keycloak/keycloak-model-infinispan-configurationbuilder-classcastexception.md),
...
[Infinispan JGroupsTransport.start() never invoked](../internal/fixed-suite-bugs/keycloak/keycloak-model-jgroupstransport-start-never-invoked-FIXED.md)
...
[Infinispan Cache.config null after real DefaultCacheManager.start()](../internal/fixed-suite-bugs/keycloak/keycloak-model-infinispan-cache-config-null-after-real-start-FIXED.md),
...
bootstrap configuration. See the [retired record](../internal/fixed-suite-bugs/keycloak/keycloak-arquillian-auth-server-undertow-container-not-found-FIXED.md).
...
- The XT takeover activation corruption note moved to
  ...
  [`../internal/fixed-suite-bugs/tomcat/dohead-jit-heap-corruption-register-invisibility-FIXED.md`](../internal/fixed-suite-bugs/tomcat/dohead-jit-heap-corruption-register-invisibility-FIXED.md).
...
14. **Spring-suite sweep (2026-06-19 в†’ re-verified 2026-06-20).** Status after running the suite
    ...
    - вњ… **toArray-recursion FIXED** (commit `8795b88d`, в†’ dev) вЂ” `ReferencePipeline.toArray(IntFunction)`
      ...
      [`../internal/fixed-suite-bugs/spring/springsuite-0620-toarray-referencepipeline-recursion.md`](../internal/fixed-suite-bugs/spring/springsuite-0620-toarray-referencepipeline-recursion.md).
    - вњ… **Unsafe off-heap DirectBuffer (bug-A + bug-A2) FULLY RESOLVED** вЂ” `PooledDataBufferTests`
      **10/10**, `LeakAwareDataBufferFactoryTests` 2/2. The bug-A2 Netty `refCnt` AIOOBE no longer
      reproduces. Archived в†’ [`../internal/fixed-suite-bugs/spring/springsuite-0619-unsafe-offheap-directbuffer.md`](../internal/fixed-suite-bugs/spring/springsuite-0619-unsafe-offheap-directbuffer.md).
...
15. **[GC: moving-collector lost-tag missed root](../internal/fixed-suite-bugs/gc-moving-interpreter-lost-tag-missed-root.md)** вЂ”
...
16. **[GC: rs_cache-presence reactor-shutdown timing race](../internal/fixed-suite-bugs/gc-rscache-reactor-shutdown-timing-race.md)** вЂ”
...
18. **[GC: live young `Thread` mirror in a blocked thread's frame reclaimed (Tomcat real-net/real-AQS HARD CRASHES)](gc-blocked-thread-frame-stale-thread-mirror.md)** вЂ”
    ...
    [`../internal/fixed-suite-bugs/repros/gc-concurrent-spawn-reclamation/`](../internal/fixed-suite-bugs/repros/gc-concurrent-spawn-reclamation/).
...
19. **[Hibernate `type.temporal.*` вЂ” moving GC strands lambda refs in native stream/collection intrinsics](../internal/fixed-suite-bugs/hibernate/hib-temporal-gc-lambda-native-stale-local.md)** вЂ”
...
[`../internal/fixed-suite-bugs/jit-regalloc-callee-saved-clobber-family.md`](../internal/fixed-suite-bugs/jit-regalloc-callee-saved-clobber-family.md).
...
> [gcstress-concurrent-oldgen-races-FIXED.md](../internal/fixed-suite-bugs/gcstress-concurrent-oldgen-races-FIXED.md)
> and [gcstress-residual-corruption-faces-FIXED.md](../internal/fixed-suite-bugs/gcstress-residual-corruption-faces-FIXED.md).
...
> and tripping JUnit's chain detector. See [`../internal/fixed-suite-bugs/spring/spring-bug-04-junit-timeout-interceptor-double-proceed.md`](../internal/fixed-suite-bugs/spring/spring-bug-04-junit-timeout-interceptor-double-proceed.md).
...
| `WeakHashMap` stream infinite hang | VM-HANG в†’ JIT codegen | вњ… **FIXED on `dev`** (`1cd0ab26`, JIT ban; verified; doc removed). The broader callee-saved GPR local-home family is now retired by the default-off fix and archived in the [JIT regalloc family doc](../internal/fixed-suite-bugs/jit-regalloc-callee-saved-clobber-family.md). | _(removed)_ |
...
[`springrepos-extension-hang-jit-throughput-and-deep-recursion.md`](../internal/fixed-suite-bugs/spring/springrepos-extension-hang-jit-throughput-and-deep-recursion.md)
...
[`../internal/fixed-suite-bugs/hql-antlr-chained-operator-syntax-error-FIXED.md`](../internal/fixed-suite-bugs/hql-antlr-chained-operator-syntax-error-FIXED.md).
`a + b + c`
...
- [hib-proxyclassreuse-loader-blind-class-resolution-FIXED.md](../internal/fixed-suite-bugs/hibernate/hib-proxyclassreuse-loader-blind-class-resolution-FIXED.md) -
...
- вњ… **Mixed JUnit 5.10.3/6.0.3 runtime on `kc-universal-cp.txt`** вЂ” FIXED (local
  ...
  [../internal/keycloak-junit-namespaceawarestore-classpath-crashes.md](../internal/fixed-suite-bugs/keycloak/keycloak-junit-namespaceawarestore-classpath-crashes.md).
- вњ… **`Assert.assertNotNull` linkage crash (37 `CRASH` rows, `testsuite/model`)** вЂ”
  ...
  [../internal/keycloak-smallrye-assertnotnull-linkage-crashes.md](../internal/fixed-suite-bugs/keycloak/keycloak-smallrye-assertnotnull-linkage-crashes.md).
- вњ… **`SmallRyeConfigBuilder.addDefaultSources` linkage crash (3 `CRASH` rows,
  ...
  [../internal/keycloak-smallrye-configbuilder-defaultsources-linkage-crashes.md](../internal/fixed-suite-bugs/keycloak/keycloak-smallrye-configbuilder-defaultsources-linkage-crashes.md).
- вњ… **`org.keycloak.testframework.config.Config` Quarkus classpath gap** вЂ” FIXED
  ...
  [../internal/fixed-suite-bugs/keycloak-testframework-quarkus-config-classpath-gap.md](../internal/fixed-suite-bugs/keycloak/keycloak-testframework-quarkus-config-classpath-gap.md).
- **`EnterpriseDbDatabaseSupplier` supplier discovery classpath gap** -
  ...
  [../internal/fixed-suite-bugs/keycloak-testframework-enterprisedb-supplier-noclassdef.md](../internal/fixed-suite-bugs/keycloak/keycloak-testframework-enterprisedb-supplier-noclassdef.md).
- **Keycloak universal classpath post-EnterpriseDB residuals** -
  ...
  [../internal/fixed-suite-bugs/keycloak-universal-classpath-post-enterprisedb-residuals.md](../internal/fixed-suite-bugs/keycloak/keycloak-universal-classpath-post-enterprisedb-residuals.md).
...
- ~~keycloak-model-infinispan-globalconfiguration-isclustered-nosuchmethod.md~~ - FIXED
  ...
  `../internal/fixed-suite-bugs/keycloak/keycloak-model-infinispan-globalconfiguration-isclustered-nosuchmethod-FIXED.md`.
- ~~keycloak-model-infinispan-configurationbuilder-classcastexception.md~~ -
  ...
  `../internal/fixed-suite-bugs/keycloak/keycloak-model-infinispan-configurationbuilder-classcastexception.md`.
  ...
  [keycloak-model-stw-takeover-hang-eventloopgroup-shutdown-FIXED.md](../internal/fixed-suite-bugs/keycloak/keycloak-model-stw-takeover-hang-eventloopgroup-shutdown-FIXED.md).
...
**h2-suite-bugs/**
- [bug-h2-charset-cp500-unsupported.md](h2/bug-h2-charset-cp500-unsupported.md)
- [bug-h2-inprocess-javac-resource-bundle.md](h2/bug-h2-inprocess-javac-resource-bundle.md)
- [bug-h2-mvstore-insert-loop-perf-hang.md](h2/bug-h2-mvstore-insert-loop-perf-hang.md)
- [bug-h2-netutils-missing-pbe-algparams.md](h2/bug-h2-netutils-missing-pbe-algparams.md)
- [bug-h2-timezone-dst-offset.md](h2/bug-h2-timezone-dst-offset.md)
