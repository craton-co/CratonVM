...
- OPEN: [`keycloak-model-liquibase-checksum-status-nojit-timeout.md`](keycloak-model-liquibase-checksum-status-nojit-timeout.md) - after the Xerces parse fix and the `AbstractChange$1.include` checksum-filter shortcut, `RealmModelTest` no longer hits the 900s CratonVM no-JIT watchdog. It now fails in 851.630s at `liquibase.snapshot.SnapshotGeneratorFactory.getGenerators` with `NoSuchMethodError: java/lang/Object.compare(Object,Object)I`, so the active residual is a Liquibase comparator lambda/SAM dispatch bug. HotSpot `-Xint` passed the same class/list in 142.649s.
