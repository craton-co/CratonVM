...
## 2026-07-08/09 Keycloak RealmModelTest protobuf metadata residual fixed; Liquibase no-JIT residual advanced
...
- FIXED: [`keycloak-model-liquibase-xerces-xml-parse-nojit-timeout-FIXED.md`](../internal/fixed-suite-bugs/keycloak-model-liquibase-xerces-xml-parse-nojit-timeout-FIXED.md) - the no-JIT Liquibase/Xerces XML parse watchdog point is gone after native fast paths for XMLChar, XMLLimitAnalyzer, XSSimpleTypeDecl normalization, XSDKey identity, XMLEntityScanner content/space handling, opti DOM getters, and RangeToken sorting.
- OPEN: [`keycloak-model-liquibase-checksum-status-nojit-timeout.md`](keycloak-model-liquibase-checksum-status-nojit-timeout.md) - after the Xerces parse fix, `RealmModelTest` still exceeds the 900s CratonVM no-JIT watchdog, but the main thread has advanced to Liquibase checksum/status serialization (`ChangeSet.generateCheckSum -> StringChangeLogSerializer.serializeObject -> AbstractChange$1.include`). HotSpot `-Xint` passed the same class/list in 142.649s.
...
- OPEN: [Keycloak `RealmModelTest` timeout after Infinispan bootstrap reaches Liquibase](keycloak-model-realmmodeltest-post-infinispan-liquibase-timeout.md) - with those layers fixed, CratonVM reaches Liquibase changelog work but still times out well after HotSpot. The active specific follow-up is now the [Liquibase checksum/status no-JIT timeout](keycloak-model-liquibase-checksum-status-nojit-timeout.md), not the old ProtoStream `fullName` decode failure or the fixed Xerces XML parse hotspot.
...
> A later residual corruption survived that fix on the same aggressive lane,
> but is now **FIXED** too; see
> [gcstress-residual-corruption-faces-FIXED.md](../internal/gcstress-residual-corruption-faces-FIXED.md). Separately, the
