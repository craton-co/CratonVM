...
## 2026-07-10 Hibernate remote rerun: 5/33 pass, 20 non-deterministic SIGSEGV/HANG cluster found

- [hib-global-temptable-nondeterministic-sigsegv-20260710.md](hib-global-temptable-nondeterministic-sigsegv-20260710.md) — 🔴 OPEN. Rerunning the shrunk 33-class Hibernate non-passed list on dev (post the 2026-07-09/10 merge wave) found 20/33 classes crashing with SIGSEGV. Confirmed non-deterministic (same class alone: PASS, PASS, HANG, CRASH across 4 attempts) rather than host contention or a clean deterministic repro — narrowed to a likely race in global-temp-table DDL lifecycle (create/drop across repeated @Test methods), not yet root-caused to an exact native code path.
...
## 2026-07-10 zero-byte HTTP request blocker RULED OUT (host-load artifact, confirmed on idle host); FileDescriptor NPE was already fixed concurrently; new unrelated segfault found post-merge
...
- OPEN (new, unrelated, found after merging the above + 24 other concurrent commits): after that merge, all three `TestFormAuthenticatorA/B/C` classes now **segfault** (SIGSEGV) partway through the run (after ~6/9, 6/6, 6/7 methods complete with no errors) — a regression almost certainly introduced by one of the merged GC/JIT/native-builtins changes (`gc/src/zgc.rs`, `jit/src/x64.rs`, `jit/src/ir_lower.rs`, `native-builtins/src/lang_string.rs`, `vm/src/vm/vm_object.rs` all changed substantially in that merge window), not by anything in this investigation. Not root-caused (out of scope, no retrievable core dump on this host). See the closing section of the retired doc above for full detail; possibly related to the non-deterministic SIGSEGV cluster in `hib-global-temptable-nondeterministic-sigsegv-20260710.md` above (same "post 2026-07-09/10 merge wave" timing, same repeated-lifecycle-churn shape) — flagged for a follow-up session, not investigated further here.
