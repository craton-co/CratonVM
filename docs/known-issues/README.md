...
- OPEN (new): [`threadpoolexecutor-execute-dispatch-degrades-to-synchronous.md`](threadpoolexecutor-execute-dispatch-degrades-to-synchronous.md) — a plain, directly-constructed `new ThreadPoolExecutor(...).execute()` (nothing to do with `Executors.*`) no longer dispatches asynchronously: on dev `aa21e334` alone the task is silently never run at all; on current dev tip (`aa21e334` + `306cd352` + later) it runs, but synchronously on the calling thread instead of a real worker thread. `306cd352`'s receiver-aware "is this real" checks don't appear to be preventing the forced-native/inline-fallback path for genuinely real receivers in some case; not root-caused (unfamiliar dispatch code from a different, concurrently-landed fix). Confirmed not JIT-specific.
...
- OPEN (new, partially fixed): [`ES-FAIL-20260710-executors-factory-synthetic-mainlock-npe.md`](elasticsearch-suite/ES-FAIL-20260710-executors-factory-synthetic-mainlock-npe.md) — all 3 classes' `testConcurrentSerialization`/`testConcurrentHashCode`/`testConcurrentEquals`/`testConcurrentToXContent` (from `AbstractWireTestCase`) fail with `NullPointerException: ... because "mainLock" is null`. Root cause, layer 1 (FIXED this session): `Executors.newFixedThreadPool`/`newCachedThreadPool`(x2)/`newSingleThreadExecutor` in `native-builtins/src/phases_early.rs` synthesized objects mistagged `ScheduledThreadPoolExecutor` (copy-paste from the scheduled-executor registrations above them) instead of plain `ThreadPoolExecutor` — fixed the class tags. Root cause, layer 2 (OPEN): even correctly tagged, these factory methods hand back a `ThreadPoolExecutor`-labeled object that never ran the real constructor, so `mainLock`/`ctl`/`workQueue`/`workers` are never initialized; `native-api/src/registry.rs`'s `drop_real_layout_synthetic` gate unconditionally drops the synthetic-aware `shutdown()`/`submit()` natives for any `ThreadPoolExecutor`-tagged class (assuming it is always a genuinely-real object), so real bytecode always runs against the uninitialized fields and NPEs. Needs either real-constructor-based construction of these factory objects or a per-object (not per-class) real-vs-synthetic marker; not attempted given the blast radius of `Executors.*` across the whole suite.
