...
| # | Bug | Status | Doc |
|---|---|---|---|
| **C** | Deep JIT→JIT recursion overruns the **native** stack (ANTLR `closure()`); same-method recursive edges and compile-time-discovered mutual direct-call cycles now route through guarded dispatch except static tail self-jumps, and the ANTLR cold-path validation lift keeps the known-bad `PredictionContext` cluster interpreted, but the full overflow path still needs stack-banging + a fault-recovery handler. | ⚪ **LATENT / PARTIALLY CONTAINED** (not a current blocker; 2026-07-01 recursive-edge + compile-cycle routing, native-shadow gate narrowing, and guarded ANTLR validation lift landed) | [springrepos-extension-hang-jit-throughput-and-deep-recursion.md](springrepos-extension-hang-jit-throughput-and-deep-recursion.md) §6–7 |
