...
- OPEN: [`keycloak-model-liquibase-checksum-status-nojit-timeout.md`](keycloak-model-liquibase-checksum-status-nojit-timeout.md) - after the Xerces parse fix, `RealmModelTest` still exceeds the 900s CratonVM no-JIT watchdog, but the run has advanced into Liquibase update/checksum work while applying schema changes around `META-INF/jpa-changelog-authz-3.4.0.CR1`. HotSpot `-Xint` passed the same class/list in 142.649s.
...
- OPEN: [Keycloak `RealmModelTest` timeout after Infinispan bootstrap reaches Liquibase](keycloak-model-realmmodeltest-post-infinispan-liquibase-timeout.md) - with those layers fixed, CratonVM reaches Liquibase changelog work but still times out well after HotSpot. The active specific follow-up is now the [Liquibase update/checksum no-JIT timeout](keycloak-model-liquibase-checksum-status-nojit-timeout.md), not the old ProtoStream `fullName` decode failure or the fixed Xerces XML parse hotspot.
