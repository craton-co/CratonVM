...
| `test.context.aot.AotIntegrationTests` | TIMEOUT | **Changed shape, not yet closed.** It used to fail at ~589 s on `endToEndTestsForBeanOverrides` with `IllegalArgumentException: Unable to adapt value of type ContextConfiguration[] to …` — the array-identity bug now fixed. It gets past that and runs out of the 1500 s ceiling instead; `endToEndTestsForBeanOverrides` drives 175 test classes through a forked loader, so slow-but-finite is plausible and is being re-measured at 3600 s. Decide slow-vs-hung before treating this as a defect |
