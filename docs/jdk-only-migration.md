...
| | |
|---|---|
| **Status** | **The `cratonvm` launcher default since 2026-09-20**; enforcement at stage 1 of 4. A program may fail under the default and pass under `--compatible`; that is the intended signal, not a bug in your program. Every *embedding* entry point still defaults to `compatible`. |
...
|---|---|---|
| *(no flag)* | `Real` | `JdkOnly` |
| `--jdk-only` | `Real` | `JdkOnly` |
| `--compatible` | `Real` | `Compatible` |
| `--real-jdk` *(deprecated alias)* | `Real` | `Compatible` |
| `--synthetic-jdk` | `Synthetic` | `Compatible` *(implied)* |
| `--jdk-only --compatible`, `--jdk-only --real-jdk` | — | **configuration error** |
| `--jdk-only --synthetic-jdk` | — | **configuration error** |
| `--synthetic-jdk --compatible`, `--synthetic-jdk --real-jdk` | — | **configuration error** |
...
Two notes on the rows that changed on 2026-09-20. `--real-jdk` used to name
only the class library, which was already the default, so it was silent about
policy; every caller that passed it was in practice asking for the compatible
policy, which is what it now means. And `--synthetic-jdk` *implies*
`Compatible` rather than inheriting the new default, because the synthetic
library is by construction the set of substitutions strict mode forbids —
inheriting would have turned a bare `--synthetic-jdk` into a startup error.
...
Remediation:
  ...
  Re-run with --compatible to use compatibility mode, or collect:
...
target/release/cratonvm --compatible --java-home "$JAVA_HOME" -cp app.jar com.example.Main
...
`--compatible` is the supported opt-out. It is not going away during this
rollout; `--real-jdk` is a deprecated alias for it and also keeps working.

```bash
# Strict — the default, stated explicitly.
cratonvm --jdk-only    --java-home "$JAVA_HOME" -cp app.jar com.example.Main

# Opt out, still a real JDK. This is the fallback.
cratonvm --compatible  --java-home "$JAVA_HOME" -cp app.jar com.example.Main
...
it actually ran in, and must **never** silently retry under `--compatible`. An
...
# interface. Prints a one-time note, now only on a run that opted OUT of the
# full policy and set the token anyway.
CRATONVM_REAL=-stubs cratonvm --compatible -cp app.jar com.example.Main

# Recommended: the full policy, which is simply the default.
cratonvm -cp app.jar com.example.Main
```
...
5. **The `--compatible` result** — did the same command pass under the fallback?
...
**The launcher default changed on 2026-09-20**, between stages 1 and 2 of the
table below, and ahead of the schedule this section originally described. What
did not change is the contract that makes the change safe: `--compatible` is a
supported, documented opt-out that selects the previous behaviour byte for
byte, and every embedding entry point still defaults to it. The stage numbers
below continue to describe *enforcement* maturity, not which mode is default.
...
Between stages, three things stay constant: `--compatible` remains a supported
opt-out that selects the previous behaviour byte for byte, the `synthetic-jdk`
build configuration keeps its blocking compile/test job so the fallback code
does not rot, and strict CI never retries under the fallback.
