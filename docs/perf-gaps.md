# RustJVM Performance Gap Analysis

**Updated:** 2026-04-16 (T10.1 baseline capture + T10 optimization plan)

## Methodology

1. **RustJVM benchmarks** are captured via `cargo bench --bench vm_benchmarks`.
...
3. Baselines are stored in `bench/hotspot-baseline.json` and
   `bench/baseline.json`.
4. Per-metric ratio = RustJVM_ns / HotSpot_ns.
...
every row as `RJ_BOOT` (rustjvm side is still the all-zeros bootstrap
baseline in `bench/baseline.json`); geomean prints `0.000×` and the
process exits `3` ("bootstrap; nothing to compare"). This is the
expected state — the HotSpot side is now real; the rustjvm side will
