# RustJVM vs HotSpot Divergence Log

This document tracks known behavioral divergences between RustJVM and HotSpot
...
(RustJVM) behavior, root cause, and resolution status.
...
```
cargo test -p rustjvm-vm --test differential -- --ignored --nocapture
```
...
| Field     | Value |
|-----------|-------|
| Class     | `rustjvm/DiffString` |
| Method    | `toUpperCase()` |
| HotSpot   | Uses default locale rules (e.g. Turkish dotted-I) |
| RustJVM   | ASCII-only uppercasing in synthetic String stub |
| Status    | Open |
...
| RustJVM   | Uses Rust `format!("{}")` (may differ in trailing zeros) |
| Status    | Open |
...
| RustJVM   | Generic or simplified exception messages |
| Status    | Open |
| Severity  | Low (does not affect control flow, only message text) |

**Root cause:** HotSpot (since JDK 14, JEP 358) generates helpful NPE
messages like "Cannot invoke method X on null reference." RustJVM generates
...
1. For each test method, invoke it under RustJVM via `Vm::invoke()` and
