...
Wave 2 left the `std::env::var_os("RUSTJVM_DBG_JIT_GEN").is_some()` calls in the per-invoke emission body. Compile-time only, but a 1k-method megamorphic dispatch still acquires the env mutex 1k+ times. **Fix:** cache `let dbg = std::env::var_os(…).is_some();` once at the top of `compile_bytecode`.
