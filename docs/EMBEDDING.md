...
|---|---|---|---|
| Compatible | `CRATONVM_COMPATIBILITY_COMPATIBLE` (`0`) | `CompatibilityMode::Compatible` | The pre-2026-09-20 behaviour: bridges, intrinsics **and** compatibility shims. **The default on every embedding entry point** — the flat API, `JNI_CreateJavaVM` and the Rust facade — and deliberately *not* on the `cratonvm` launcher, which defaults to JDK-only. |
...
> **Maturity — enforcement is at stage 1 of 4.** The `cratonvm` launcher
> defaults to JDK-only as of 2026-09-20; **this library does not, and that is
> deliberate** (see the rules below). JDK-only and wave 1 is instrumentation and
...
> expect failures on programs that run fine under `Compatible` — that is the
...
- **The default here is `Compatible`, and you reach it by doing nothing.**
  Strict mode is never inferred from a Cargo feature, from `CRATONVM_REAL` /
  `CRATONVM_NO_STUBS`, from what the host machine has installed, or from the
  launcher's default: those select a native-registry filter only, or belong to
  a different entry point, and none can express the class-loading or dispatch
  half of the contract. Promoting one would produce a run enforcing rules its
  embedder never asked for.
- **The launcher's default is NOT this library's default, and re-aligning them
  is an ABI break.** Since 2026-09-20 `cratonvm` defaults to JDK-only, with
  `--compatible` as the opt-out. `libcratonvm::DEFAULT_COMPATIBILITY_MODE`
  stayed `Compatible`: these constants are ABI, a compiled host does not
  re-read the header, and it has no command line to pass `--compatible` on.
  `VmConfig::with_host_jdk_default()` stopped being an alias for
  `VmConfig::for_launcher()` on the same day for exactly this reason — build
  on the former, not the latter.
- **Both `JdkMode` and compatibility mode are asymmetric between entry points,
  and they split along different lines.** `VmConfig::default()` (the embedding
  / in-tree-test path) is `JdkMode::Synthetic`, while
  `with_host_jdk_default()` — the base config both C entry points build on — is
  `JdkMode::Real`, because the two paths genuinely want different class
  libraries. Every entry point reachable from this document agrees on
  `Compatible`; only the launcher differs. Do not "align" either split.
...
// with_host_jdk_default() is the embedding base: real JDK, deterministic,
// and `Compatible` — unlike for_launcher(), which is strict.
