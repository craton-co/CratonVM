...
| [04](bug-04-surefire-jvm-dup-xmx.md) | CratonVM as Surefire `-Djvm=` fork (Gap A): dup-`-Xmx` **FIXED**; ForkedBooter+JUnit+Arquillian run; `Path.toAbsolutePath()` canonicalize-mangle that broke WildFly `validateWildFlyDir` **FIXED**; next blocker = `ServerSocket.getImpl` NPE | Med | dup-Xmx + toAbsolutePath FIXED; ServerSocket NPE open |
