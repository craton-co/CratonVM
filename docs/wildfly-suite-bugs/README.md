...
| [03](bug-03-regex-perf-deployment-build.md) | Char-by-char loops ~400–1000× slower (Gap C). **(A)** hot String/CharSequence accessors (`charAt`/`length`) were native-bridged in compiled code — **FIXED** (call-site intrinsics activated + CharSequence receiver guard; static charAt loop 18 000→195 ms, ~90×). **(B)** regex still slow: instance methods never invocation-tier-up (only static do), so `Pattern$*.match` never JIT-compiles — **open** (large JIT change, deferred) | Med | **(A) FIXED**; (B) open — instance-method tier-up |
