...
| [04](bug-04-surefire-jvm-dup-xmx.md) | CratonVM as Surefire `-Djvm=` fork (Gap A) — managed-container startup, fixed layer by layer: dup-`-Xmx` **FIXED**, ForkedBooter+JUnit+Arquillian run, `Path.toAbsolutePath()` mangle (validateWildFlyDir) **FIXED**, `ServerSocket/DatagramSocket.setReuseAddress` NPE **FIXED**; next blocker = `ProcessBuilder.start` quotes the java path (os error 123) | Med | 4 layers FIXED; ProcessBuilder quoting open |
