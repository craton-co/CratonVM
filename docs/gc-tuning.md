...
| Should I turn generational on? | If your workload has a **large, stable live set and a high allocation rate** — the shape a young cycle skips most of. `CRATONVM_ZGC_GEN_PROMOTION_AGE` (default 3, clamped to `1..=15` because the header field is 4 bits) decides how many collections an object must survive; a lower value promotes sooner, so young cycles skip more and old garbage accumulates faster. Measure with `gen=` and `old_retained`, and note that a young cycle's **sweep** cost does not fall with the split — the sweep walks every registered object whatever the generation says, which is what a real young space would fix | the plan's §3 |
