...
| Is it concurrent, generational or compacting? | **Concurrent: YES since 2026-08-16.** The strong closure is traced by a worker pool while every mutator runs; the pause replays the SATB ingress, re-scans the roots and sweeps. Kill switch `CRATONVM_ZGC_CONC_START=0`. **Compacting: YES, on by default since 2026-08-13** (`CRATONVM_ZGC_RELOCATE=0` is the kill switch). **Generational: NO — not built**; page ages, a card barrier and a young scope are computed, but there is no young-only collection | [the concurrent+generational plan](feature-designs/zgc-concurrent-and-generational-plan-20260813.md) |
...
| When does a concurrent cycle open? | When allocation crosses `CRATONVM_ZGC_CONC_START`% (default 60) of the collection threshold, on an allocation that reaches `maybe_gc`. **A `System.gc()`-driven workload never opens one** — a forced collection is meant to collect now, not to start marking — so a benchmark built out of `System.gc()` calls measures the stop-the-world path however this is configured | `ZgcRealHeap::should_start_concurrent_mark` |
