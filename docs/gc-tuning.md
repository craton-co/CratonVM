...
| Does ZGC have TLABs? | **Yes for the backend buffer (default-on); the VM-thread buffer is OPT-IN.** `CRATONVM_ZGC_JIT_TLAB=1` makes `VmHeap::refill_tlab` hand the VM thread's own buffer a chunk, so the interpreter's `new` and the JIT's inline bump both hit -- correct, but measured SLOWER than the helper path (see `zgc_vm_tlab_enabled_by_default`); the backend keeps its own buffer under that for the helper and native paths (`CRATONVM_ZGC_TLAB=0`, which switches both off — they carve from one source). Engagement: `vm_tlab_refills` on the `[GC] zgc-features:` line | `gc/src/zgc/vm_tlab.rs`, `ZgcRealHeap::alloc_raw_tlab` |
