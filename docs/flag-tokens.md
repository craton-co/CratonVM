...
## Which collector a token binds

A token's name says what it does; it does not say which backend reads it. Most
of the surface is unambiguous because the name carries the collector
(`zgc-*`, `g1-*`). Two GC tokens are not, and both reach a collector the reader
would not guess. This section is hand-written and lists only the cases where the
name is actively misleading — it is not a second inventory.

| Token (legacy name) | Generational | G1 | ZGC |
| --- | --- | --- | --- |
| `-moving-young` (`CRATONVM_NO_MOVING_YOUNG=1`) | **binds** — turns off young evacuation | **binds** — the same process-wide switch gates G1's moving decision | **binds, with a caveat below** |
| `zgc-relocate-proven-jit=0` (`CRATONVM_ZGC_RELOCATE_UNDER_PROVEN_JIT=0`) | no | no | **binds, with the same caveat** |
| `zgc-relocate=0` (`CRATONVM_ZGC_RELOCATE=0`) | no | no | binds unconditionally — the slide never runs |

**The caveat, and it is the surprising part.** ZGC's relocation refusal is a
first-failing-term chain, and the whole chain is applied only on a cycle where
a **compiled frame is live**
(`gc_quiescence::is_active() || unregistered_jit_frame_on_stack()`). Four of its
six terms — including both rows above — are computed on every cycle and
discarded when no compiled frame is live, because every term in the chain is an
obligation *about compiled frames* and with none live the obligations are
discharged vacuously.

So on a JIT-cold cycle, `CRATONVM_NO_MOVING_YOUNG=1` and
`CRATONVM_ZGC_RELOCATE_UNDER_PROVEN_JIT=0` **do not stop ZGC compacting**. Set
either on a warm workload and ZGC refuses on every JIT-warm cycle and compacts
on every cold one.

`CRATONVM_NO_MOVING_YOUNG` binding ZGC at all is deliberate, not an accident of
a shared predicate: it is the process-wide "do not move objects out from under
compiled frames" switch, and the whole rewritable-root contract ZGC's chain
leans on (`moving_young_coverage_incomplete`, the `MOVABLE_BOUNDS` publication,
the conservative-scan suppression) stops being computed when it is off. A ZGC
that kept sliding then would be relocating on a proof nothing is producing.
The reasoning is written down at
`zgc::relocation_skip_reason::MOVING_YOUNG_DISABLED`.

**Reading it on a run.** `--verbose:gc` prints two censuses over the same index
space:

* `[GC] zgc-relocation-refusal-term: <term>=N` — the term was violated on N
  cycles, counted where the verdict is computed;
* `[GC] zgc-relocation-skip-reason: <term>=N` — the term actually stopped a
  slide on N cycles.

The gap between them is the bypass. A term that appears on the first line with
no matching second line fired on every cycle it was asked about and stopped none
of them. If you need the refusal unconditionally, use `zgc-relocate=0`, which
has no such gate.
...
| `afc-pending-future` | `CRATONVM_DBG_AFC_PENDING_FUTURE` |
...
| `cmctx` | `CRATONVM_DBG_CMCTX` |
| `findclass` | `CRATONVM_DBG_FINDCLASS` |
...
| `g1-verify-stale-forward` | `CRATONVM_G1_VERIFY_STALE_FORWARD` |
...
| `scoped-memory-direct` | `CRATONVM_JIT_SCOPED_MEMORY_DIRECT` |
...
| `code-cache-sweep` | `CRATONVM_JIT_CODE_CACHE_SWEEP` |
...
| `inline-fp-arith` | `CRATONVM_JIT_NO_INLINE_FP_ARITH` |
...
| `ir-licm-guard-attribution` | `CRATONVM_JIT_IR_LICM_GUARD_ATTRIBUTION` |
| `ir-guard-token` | `CRATONVM_JIT_IR_GUARD_TOKEN` |
...
| `ir-osr-entry-always` | `CRATONVM_JIT_IR_OSR_ENTRY_ALWAYS` |
| `code-arena` | `CRATONVM_JIT_CODE_ARENA` |
| `backend-parity` | `CRATONVM_JIT_BACKEND_PARITY` |
| `profile-save` | `CRATONVM_JIT_PROFILE_SAVE` |
| `profile-load` | `CRATONVM_JIT_PROFILE_LOAD` |
| `print-compilation` | `CRATONVM_JIT_PRINT_COMPILATION` |
| `perf-map-dir` | `CRATONVM_JIT_PERF_MAP_DIR` |
| `lock-coarsen` | `CRATONVM_JIT_LOCK_COARSEN` |
...
| `volatile-load-fence` | `CRATONVM_JIT_VOLATILE_LOAD_FENCE` |
| `ir-zero-ref-slots` | `CRATONVM_JIT_IR_ZERO_REF_SLOTS` |
| `ea-cross-block-loads` | `CRATONVM_JIT_EA_CROSS_BLOCK_LOADS` |
| `inline-prim-putfield` | `CRATONVM_JIT_INLINE_PRIM_PUTFIELD` |
| `ir-snapshot-liveness` | `CRATONVM_JIT_IR_SNAPSHOT_LIVENESS` |
| `ir-volatile-fields` | `CRATONVM_JIT_IR_VOLATILE_FIELDS` |
| `rotated-preheader` | `CRATONVM_JIT_ROTATED_PREHEADER` |
| `cycle-direct-bind` | `CRATONVM_JIT_CYCLE_DIRECT_BIND` |
| `cycle-edge-cell` | `CRATONVM_JIT_CYCLE_EDGE_CELL` |
| `instance-self-call` | `CRATONVM_JIT_INSTANCE_SELF_CALL` |
| `div-guard-retier` | `CRATONVM_JIT_DIV_GUARD_RETIER` |
| `osr-guard-exit-charge` | `CRATONVM_JIT_OSR_GUARD_EXIT_CHARGE` |
| `osr-reentry-memo` | `CRATONVM_JIT_OSR_REENTRY_MEMO` |
| `ir-carry-rcx-twin` | `CRATONVM_JIT_IR_CARRY_RCX_TWIN` |
| `ir-box-unbox-fold` | `CRATONVM_NO_IR_BOX_UNBOX_FOLD` |
| `retire-cell` | `CRATONVM_JIT_RETIRE_CELL` |
| `arraylist-pin` | `CRATONVM_JIT_NO_ARRAYLIST_PIN` |
| `omit-stack-trace-in-fast-throw` | `CRATONVM_OMIT_STACK_TRACE_IN_FAST_THROW` |
| `ir-const-div` | `CRATONVM_JIT_IR_CONST_DIV` |
| `object-hashcode-string` | `CRATONVM_JIT_OBJECT_HASHCODE_STRING` |
| `osr-prefer-single-pass` | `CRATONVM_JIT_OSR_PREFER_SINGLE_PASS` |
| `site-cache-special` | `CRATONVM_JIT_SITE_CACHE_SPECIAL` |
| `ir-ls-call-last-use` | `CRATONVM_JIT_IR_LS_CALL_LAST_USE` |
| `ir-ls-edge-resolution` | `CRATONVM_JIT_IR_LS_EDGE_RESOLUTION` |
| `instanceof-final-miss` | `CRATONVM_JIT_INSTANCEOF_FINAL_MISS` |
| `operand-fold` | `CRATONVM_JIT_NO_OPERAND_FOLD` |
| `ir-recursive-inline` | `CRATONVM_JIT_IR_RECURSIVE_INLINE` |
...
| `ir-this-nonnull` | `CRATONVM_JIT_IR_THIS_NONNULL` |
...
| `ir-frame-lines` | `CRATONVM_JIT_NO_IR_FRAME_LINES` |
| `npe-trap-lines` | `CRATONVM_JIT_NO_NPE_TRAP_LINES` |
| `inline-caller-frames` | `CRATONVM_JIT_NO_INLINE_CALLER_FRAMES` |
...
| `ir-range-edges` | `CRATONVM_JIT_IR_RANGE_EDGES` |
...
| `ir-ls-splits` | `CRATONVM_JIT_IR_LS_SPLITS` |
...
| `native-cf-complete` | `CRATONVM_NATIVE_CF_COMPLETE` |
...
| `implicit-local-handlers` | `CRATONVM_JIT_IMPLICIT_LOCAL_HANDLERS` |
...
| `transform-census` | `CRATONVM_JIT_TRANSFORM_CENSUS` |
| `entry-counter` | `CRATONVM_JIT_ENTRY_COUNTER` |
...
| `scalar-under-precise-frames` | `CRATONVM_JIT_SCALAR_UNDER_PRECISE_FRAMES` |
...
| `ir-fused-fcmp` | `CRATONVM_JIT_IR_FUSED_FCMP` |
| `ir-zgc-announce` | `CRATONVM_JIT_IR_ZGC_ANNOUNCE` |
| `arraylist-intrinsics` | `CRATONVM_NO_JIT_ARRAYLIST_INTRINSICS` |
...
| `verify-branches` | `CRATONVM_JIT_VERIFY_BRANCHES` |
...
| `helper-panics-fatal` | `CRATONVM_JIT_HELPER_PANICS_FATAL` |
| `assert-code-free-audit` | `CRATONVM_JIT_ASSERT_CODE_FREE_AUDIT` |
...
| `g1-ihop-gross-growth` | `CRATONVM_G1_IHOP_GROSS_GROWTH` |
| `g1-ihop-backoff` | `CRATONVM_G1_IHOP_BACKOFF` |
| `g1-mark-side-tables` | `CRATONVM_G1_MARK_SIDE_TABLES` |
| `g1-cleanup-satb-discard` | `CRATONVM_G1_CLEANUP_SATB_DISCARD` |
| `g1-cleanup-reset-tams` | `CRATONVM_G1_CLEANUP_RESET_TAMS` |
| `g1-mark-convergence-epoch` | `CRATONVM_G1_MARK_CONVERGENCE_EPOCH` |
...
| `g1-serial-evac-holder-screen` | `CRATONVM_G1_SERIAL_EVAC_HOLDER_SCREEN` |
...
| `zgc-par-relocate` | `CRATONVM_ZGC_PAR_RELOCATE` |
| `zgc-jit-inline-announce` | `CRATONVM_ZGC_JIT_INLINE_ANNOUNCE` |
| `zgc-bitmap-sweep` | `CRATONVM_ZGC_BITMAP_SWEEP` |
| `zgc-mark-root-filter` | `CRATONVM_ZGC_MARK_ROOT_FILTER` |
| `zgc-tlab-owned-starts` | `CRATONVM_ZGC_TLAB_OWNED_STARTS` |
...
| `g1-tlab-clamp` | `CRATONVM_G1_TLAB_CLAMP` |
| `g1-humongous-best-fit` | `CRATONVM_G1_HUMONGOUS_BEST_FIT` |
...
| `g1-verify-sweep-pauses` | `CRATONVM_G1_VERIFY_SWEEP_PAUSES` |
| `g1-free-census-on-reclaim` | `CRATONVM_G1_FREE_CENSUS_ON_RECLAIM` |
| `g1-narrow-drain-fixup` | `CRATONVM_G1_NARROW_DRAIN_FIXUP` |
| `g1-card-cursor` | `CRATONVM_G1_CARD_CURSOR` |
| `g1-rset-prune-on-scan` | `CRATONVM_G1_RSET_PRUNE_ON_SCAN` |
| `g1-block-offsets` | `CRATONVM_G1_BLOCK_OFFSETS` |
| `g1-evac-local-queue` | `CRATONVM_G1_EVAC_LOCAL_QUEUE` |
| `g1-evac-census` | `CRATONVM_G1_EVAC_CENSUS` |
| `g1-evac-share-census` | `CRATONVM_G1_EVAC_SHARE_CENSUS` |
| `g1-evac-dest-latch` | `CRATONVM_G1_EVAC_DEST_LATCH` |
| `g1-parallel-seed` | `CRATONVM_G1_PARALLEL_SEED` |
| `g1-parallel-mixed` | `CRATONVM_G1_PARALLEL_MIXED` |
| `g1-mixed-futile-guard` | `CRATONVM_G1_MIXED_FUTILE_GUARD` |
| `g1-heap-resize` | `CRATONVM_G1_HEAP_RESIZE` |
| `g1-pause-cost-model` | `CRATONVM_G1_PAUSE_COST_MODEL` |
| `g1-humongous-run-guard` | `CRATONVM_G1_HUMONGOUS_RUN_GUARD` |
| `g1-drain-not-a-pause` | `CRATONVM_G1_DRAIN_NOT_A_PAUSE` |
| `g1-evac-cost-from-young` | `CRATONVM_G1_EVAC_COST_FROM_YOUNG` |
| `g1-mark-backoff-deadline` | `CRATONVM_G1_MARK_BACKOFF_DEADLINE` |
| `g1-ihop-poll-gate` | `CRATONVM_G1_IHOP_POLL_GATE` |
| `g1-alloc-mark-drive` | `CRATONVM_G1_ALLOC_MARK_DRIVE` |
| `g1-evac-cost-real` | `CRATONVM_G1_EVAC_COST_REAL` |
| `g1-block-offset-audit` | `CRATONVM_G1_BLOCK_OFFSET_AUDIT` |
...
| `zgc-relocate-cost-gate` | `CRATONVM_ZGC_RELOCATE_COST_GATE` |
| `zgc-readable-give-back` | `CRATONVM_ZGC_READABLE_GIVE_BACK` |
| `zgc-pristine-chunks` | `CRATONVM_ZGC_PRISTINE_CHUNKS` |
...
| `zgc-vm-tlab-share` | `CRATONVM_ZGC_VM_TLAB_SHARE` |
| `zgc-headroom-bypasses-rearm` | `CRATONVM_ZGC_HEADROOM_BYPASSES_REARM` |
| `zgc-census` | `CRATONVM_ZGC_CENSUS` |
| `zgc-census-access` | `CRATONVM_ZGC_CENSUS_ACCESS` |
| `zgc-pause-target-includes-relocate` | `CRATONVM_ZGC_PAUSE_TARGET_INCLUDES_RELOCATE` |
| `zgc-metrics` | `CRATONVM_ZGC_METRICS` |
| `zgc-trigger-shadow` | `CRATONVM_ZGC_TRIGGER_SHADOW` |
| `zgc-tlab-reserved-bytes` | `CRATONVM_ZGC_TLAB_RESERVED_BYTES` |
| `zgc-mark-ref-chunk` | `CRATONVM_ZGC_MARK_REF_CHUNK` |
| `zgc-page-evac` | `CRATONVM_ZGC_PAGE_EVAC` |
| `zgc-page-survey` | `CRATONVM_ZGC_PAGE_SURVEY` |
| `zgc-sweep-page-census` | `CRATONVM_ZGC_SWEEP_PAGE_CENSUS` |
| `zgc-mark-root-filter-conc` | `CRATONVM_ZGC_MARK_ROOT_FILTER_CONC` |
| `zgc-relocate-budget-mb` | `CRATONVM_ZGC_RELOCATE_BUDGET_MB` |
| `zgc-metrics-tsv` | `CRATONVM_ZGC_METRICS_TSV` |
| `zgc-metrics-run` | `CRATONVM_ZGC_METRICS_RUN` |
...
| `jar-archive-cache-cap` | `CRATONVM_JAR_ARCHIVE_CACHE_CAP` |
