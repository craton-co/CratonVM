...
**Status:** Partial — `--jdk-only` exists, boots, enforces the core refusals,
and **is the `cratonvm` launcher default as of 2026-09-20**; the hard-coded
dispatch lists it was meant to retire are still there, bypassed rather than
removed.
...
`CompatibilityMode::JdkOnly`, and conflicts with `--synthetic-jdk`.

**The launcher defaults to `JdkOnly`; every embedding entry point still
defaults to `Compatible`.** The opt-out is `--compatible`, and `--real-jdk` is
a deprecated alias for it. The split is declared in `vm/src/config.rs` as
`LAUNCHER_DEFAULT_COMPATIBILITY_MODE` (JdkOnly) versus
`EMBEDDED_DEFAULT_COMPATIBILITY_MODE` and
`EMBEDDED_HOST_JDK_DEFAULT_COMPATIBILITY_MODE` (both Compatible), and
`VmConfig::with_host_jdk_default` — which `libcratonvm` and `cratonvm-embed`
build on — stopped being an alias for `VmConfig::for_launcher` on the same
day. A person at a prompt can read a refusal and pass `--compatible`; a host
program that linked the C API cannot, and `libcratonvm`'s
`DEFAULT_COMPATIBILITY_MODE` is ABI.
...
**Renaming, as it actually landed.** This section used to plan an end state of
"`--jdk-only` becomes `--real-jdk`, and today's `--real-jdk` (`Compatible`)
becomes `--synthetic-jdk`". That is **superseded** and was never done: reusing
`--synthetic-jdk` for a real-JDK mode would have collided with the flag that
selects the synthetic class library, and re-pointing `--real-jdk` would have
silently changed the meaning of every committed difftest ledger, CI arm and
`scripts/jdk-only-*.sh` baseline that passes it.

What landed instead, on 2026-09-20, is a default flip with no renaming:
strict becomes the launcher default, the old default gains the explicit name
`--compatible`, and `--real-jdk` is kept as a deprecated alias for
`--compatible` so that every existing caller keeps its exact meaning.
`--jdk-only` is still accepted and now restates the default.

**No synthetic method used by either surviving mode
...
`--compatible` keeps the pre-2026-09-20 behaviour exactly; `--real-jdk` is its
deprecated alias. `--synthetic-jdk` is unchanged, implies `--compatible`, and
conflicts with `--jdk-only`.
...
- Under `JdkOnly`, a `--java-home` that does not EXIST must carry the same
  ...
  `internal/jdk-only/G82-1-the-run-that-closed-a-p0-row-20260819.md` §4 N1.
- Do not edit `native-builtins/src/lib.rs`; the stub reclassification is a
  ...
  **1328** still refused at VM init. The P0 *Residual synthetic native set* row
  ...
  `internal/jdk-only/G83-1-the-ratchet-was-already-red-20260819.md` §3a.
...
```text
--jdk-only                    Real JDK, reject compatibility stubs and fabricated classes (default).
--compatible                  Real JDK with the pre-2026-09-20 compatibility behaviour.
--real-jdk                    Deprecated alias for --compatible.
--synthetic-jdk               Standalone synthetic library; implies --compatible, conflicts with --jdk-only.
...
Resolution table, implemented once in `resolve_compatibility_mode`
(`vm-cli/src/main.rs`) and mirrored for the `-version` banner by
`scan_requested_compatibility_mode`:

| argv | `JdkMode` | `CompatibilityMode` |
|---|---|---|
| *(nothing)* | `Real` | `JdkOnly` |
| `--jdk-only` | `Real` | `JdkOnly` |
| `--compatible` | `Real` | `Compatible` |
| `--real-jdk` | `Real` | `Compatible` |
| `--synthetic-jdk` | `Synthetic` | `Compatible` (implied) |
| `--jdk-only --compatible` / `--jdk-only --real-jdk` | — | error |
| `--jdk-only --synthetic-jdk` | — | error |
| `--synthetic-jdk --compatible` / `--synthetic-jdk --real-jdk` | — | error (library conflict) |

`--synthetic-jdk` *implies* `Compatible` rather than erroring under the new
default, because the synthetic library is by construction the set of
substitutions strict mode forbids — there is no strict synthetic mode to
resolve to. Reading the default into it would turn a flag that worked for a
year into a `validate_compatibility` failure caused by a change it has nothing
to do with. The pair is only an error when the argv *states* both.

`CRATONVM_REAL=-stubs` keeps working as a native-registry filter. Its
one-time note now fires only on a run that resolved to `Compatible` and set
the token anyway — under the default the run already has the full policy, and
advising it to ask for what it has would be noise.
