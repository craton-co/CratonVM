...
rebuild: **`CRATONVM_ZGC_SWEEP_HEADER_ZERO=0`** restores the whole-body memset,
**`CRATONVM_ZGC_SWEEP_DEAD_RUNS=0`** the per-object calls. *(Spelled
`CRATONVM_ZGC_GEN_*` here until 2026-09-20/21. Those spellings match no variable
in the tree, so they fail OPEN and silently: an A/B run with them measures the
default twice. The `GEN_` names were renamed to `SWEEP_` when the optimisations
stopped being scoped to young cycles, and this paragraph was not updated —
contradicting this same document's own rename note below.*

*Two independent review rounds found this in the same week and corrected this
one paragraph twice, which is why both records are cited here: the ZGC round's
`docs/internal/zgc-round-20260920/ROUND-SUMMARY.md` §1 item 10, and the
generational round's
`docs/internal/gaps/gengc-plumbing2-legacy-flag-typos-fail-silently-20260920.md`.
Two reviewers reaching the same defect from different directions is the
strongest evidence in this file that it was real.)* Engagement counters on the
