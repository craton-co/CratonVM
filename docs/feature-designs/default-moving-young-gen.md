...
moving_cycles_under_live_jit=… coverage_fallbacks=…` plus the per-reason
histogram before treating the collector as actively compacting in a given run.
A correct checksum proves safety; only a non-zero moving-cycle count proves
copying.

The field was spelled `cycles=…` until 2026-09-20. Do not trust that older
spelling if you meet it in a log or an older page: it was structurally pinned
at 0 *while the collector was copying*, so it asserted the opposite of the
truth on exactly the runs this paragraph tells you to check. A measured run
after the rename reads `moving_cycles_total=1510` where the old field read 0
