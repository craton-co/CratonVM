...
**Status:** Partial — the **default** collector is ZGC, whose marking *can* be
concurrent (opt-in since 2026-08-16) and which *can* be generational (opt-in
since 2026-08-17); the Generational backend's concurrent old-gen marking ships
and runs whenever that backend is selected; G1 is opt-in and concurrent.

*This page's status line and the table below said "Generational is the default"
and gave ZGC's concurrency as "None" until 2026-09-21, contradicting this same
page's own "What is not built yet" section three paragraphs down. Both were
written before 2026-08-10 and 2026-08-16. The default-attribution error is the
one `vm/src/config.rs:23`–`:28` records as load-bearing: three arms recorded as
"generational" while root-causing
`bug-g1-evacuates-live-jit-reference-20260819-FIXED.md` were ZGC runs.*

## What is built

Three collector backends exist (`GcBackend` in `gc/src/vm_heap.rs`,
`GcAlgorithm` in `vm/src/config.rs`). **ZGC is the default** in any build with
the `zgc` feature, which is on by default (`vm/src/config.rs:947`); Generational
is the default only in a `--no-default-features` build, and is otherwise reached
through `-XX:+UseGenerationalGC`. The others are selected at startup and are not
tried automatically.

| Backend | How it is selected | Concurrency it actually has |
|---|---|---|
| Generational | `-XX:+UseGenerationalGC`, or the default in a `--no-default-features` build | STW initial mark → concurrent mark → STW remark → concurrent sweep, for the **old gen**. No flag; triggered by `old_gen_needs_gc()`. The young gen is a Cheney copy under STW. |
...
| ZGC | **the default**; also `-XX:+UseZGC` | **Opt-in, since 2026-08-16.** `CRATONVM_ZGC_CONC_START=<percent>` traces the strong closure over `Arc<ZgcRealHeap>` while every mutator runs (`ZMarkCoordinator`, `gc/src/zgc_concurrent.rs`); the default is `0`, so the shipping cycle is stop-the-world end to end. The sweep and the compacting slide are STW in **every** arm. `ZgcRealHeap` **does move objects** — compaction is default-on, `CRATONVM_ZGC_RELOCATE=0` is the kill switch — and no load barrier is ever armed. |

Concurrency on the **Generational** collector is not gated by any environment
variable. There is no `CRATONVM_*CONCURRENT*` or `CRATONVM_SATB*` flag; its
phases are unconditional. ZGC's is gated, by `CRATONVM_ZGC_CONC_START`.

- **The Generational cycle** is driven from `maybe_concurrent_gc`
  (`vm/src/runtime/interpreter/gc_and_alloc.rs`) over `ConcurrentMarker`
  (`gc/src/concurrent_mark.rs`). Read every "the default cycle" below this line
  as *the Generational cycle*: this section and the ones that follow were
  written while Generational was the default and have not been re-argued for
  ZGC.
- **SATB is wired by default.** `enable_concurrent_gc(satb, state)` runs at VM
  ...
  (`gc/src/gen_heap.rs`) gates on `is_marking_active()`. ZGC has its own,
  separate SATB path (a **pre-write** barrier, `gc/src/zgc/barrier.rs`), which
  is what makes its concurrent marking snapshot-at-the-beginning.
...
- **The Generational collector has no marker thread.** The "concurrent" mark and
...
*(Written before 2026-08-10, when `GcBackend::Generational` was the default. It
is not: see "What is built" above. The motivation below survives the flip; the
premise sentence does not.)*
...
- **ZGC** (`gc/src/zgc.rs`) is `ZgcRealHeap`, wired as `GcBackend::Zgc` /
  `VmHeap::Zgc`, **the default**, and also selectable with `-XX:+UseZGC`. It is
  stop-the-world and **compacting by default** (since 2026-08-13); the
  low-latency colored-pointer / load-barrier implementation remains future work
  — the encoding exists in `gc/src/zgc/vaddr.rs` and no slot ever holds one of
  its words. *(This bullet described ZGC as "split into an honest simulation
  (`ZgcCollector`/`ZgcHeap`, metadata-only … `warn_zgc_simulation_selected()`)
  and a real STW mark-sweep", and as "deliberately non-moving", until
  2026-09-21. The simulation was **deleted** in `78c20bd3d`; compaction went
  default-on on 2026-08-13.)*
...
- ZGC-real is not merely selectable, it is **the default** (since 2026-08-10),
  which more than closes the backend reachability gap this argument assumed —
  **re-read this section's premise before acting on it.** It is a
  stop-the-world collector that *does* compact (since 2026-08-13). Maturing
  production ZGC still means real colored pointers, an atomic load barrier, and
  **concurrent** relocation; compaction itself is no longer the missing piece.
