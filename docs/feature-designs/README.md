...
| [`real-cdi-bean-container.md`](real-cdi-bean-container.md) | Retire the per-framework shim cluster (ArC/Spring/WildFly/MSC/Infinispan/Agroal) with real bytecode. | XL | general `<clinit>`/classloading fixes |
...
| [`proxy-real-classfile.md`](proxy-real-classfile.md) | Generate a real `$ProxyN` class file (vs. name-lookup synthetic shim). | M | runtime defineClass (WP2.3) |
