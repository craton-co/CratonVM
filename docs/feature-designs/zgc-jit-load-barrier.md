...
**Status (corrected 2026-09-20):** **Stage (a) landed on 2026-08-13**, and this
header said "Designed, not built. No barrier emission exists in the JIT — a
search of `jit/src` and `jit-api/src` for ZGC finds only two incidental test
comments" for three months afterwards. That is no longer true and it was
load-bearing: it is the sentence a reader reaches before deciding whether a
compacting default run is possible at all.

What exists today: `jit/src/x64/licm.rs` carries
`zgc_read_barrier_blocks_inline_fields()` and
`zgc_codegen_honours_read_barrier()`, ZGC appears across twenty-odd files in
`jit/src`, and `vm/src/vm/vm_init.rs`'s `zgc_relocation_permitted` returns
`true` for a JIT run on the strength of that capability check — which is why a
**default JIT run compacts**. The routing is the compressed-oops mechanism:
every compact-field access goes through `jit_getfield` / `jit_putfield_object`
whenever the read barrier is armed, and those use the heap's own barriering
accessors.

What is still designed and not built is **stages (b) onwards**: inline barrier
emission, so a compiled reference load does not have to leave the fast path. The
cost model and site inventory below are for that, and remain current.
...
> **Discharged 2026-08-13.** Stage (a) landed, `zgc_codegen_honours_read_barrier()`
> answers `true`, and `zgc_relocation_permitted` therefore admits a JIT run.
> The verdict above is kept because it is the argument for the gate, and the
> gate is still there and still load-bearing — it is the predicate that would
> refuse again if the codegen ever stopped honouring the barrier. Read the rest
> of this section as the case that was made, not as a description of a refusal
> that fires today.
...
* **ZGC + relocation + JIT, before stage (a) lands** → refuse at VM init.
  ...
  the non-moving mark-sweep it was before 2026-08-13 — which is exactly what
  `CRATONVM_ZGC_RELOCATE=0` still selects today — rather than degrading the
  whole VM to the interpreter.
