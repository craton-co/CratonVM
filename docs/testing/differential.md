...
* The pre-existing `moving-gc` mode sets `CRATONVM_NO_SELECTIVE_PROMOTE`, which
  ...
  baseline and were deliberately not touched. (One exception, 2026-09-20: they
  now pass `--compatible` explicitly. The launcher default flipped to
  `jdk-only` that day, so an empty argv stopped reproducing the policy those
  baselines were captured under — freezing the argv would have unfrozen the
  measurement.)
