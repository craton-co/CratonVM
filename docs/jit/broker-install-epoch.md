...
| **In flight** | A worker records the dispatch epoch -- the epoch `dequeue_fresh` just passed the request at, not a second read -- in its own slot (`TieredCompilationManager::inflight_install_epoch()` reports the OLDEST running dispatch; `0` when every worker is idle) and compares it again on return. If it moved, the completion carries `epoch_moved`, and an unpublished outcome is then NOT charged to `tier_fail_count` (see *Fail-closed* below). |
...
The same rule covers a request the gate could not catch: one dispatched at the
current epoch whose epoch then moved **while it compiled**. Its body is refused
by the flush barrier, the VM reports `published = false`, and `finish` used to
charge that as a failed compile. It now sees `Completion::epoch_moved` and
releases the slot without spending a retry (test:
`a_compile_overtaken_by_an_epoch_move_spends_no_retry`). A compile that genuinely
failed in the same window is also uncharged; that is the fail-open direction,
bounded by the epoch having to move, and a real failure is charged on the next
attempt.
...
| `queue_refused_ineligible` | A request refused by `CompilerCore::admit` because its method is `ineligible` or has exhausted `MAX_TIER_FAIL_RETRIES`. Not a drop -- nothing was queued. Expected zero: the policy doors ask first, so a count names a door that did not (today, the VM's deopt re-queue). |

Read them with `metrics::scheduling_counts()` (all rows),
`metrics::scheduling_count(name)` (one row), or
`metrics::scheduling_dropped_total()`, which sums the four drop rows -- stale,
invalidated, shutdown and deoptimized -- and neither refusal row nor the
in-flight or panic rows. (Until 2026-09-18 it omitted `queue_dropped_deoptimized`
while its per-manager mirror `dropped_requests()` counted it.) Per-manager mirrors, for a caller that must not read a
process-wide table: `TieredCompilationManager::dropped_requests()`,
`::deduplicated_requests()`, `::ineligible_refusals()`, `::cap_refusals()`,
`::worker_panics()`, `::install_epoch()` and `::inflight_install_epoch()`.
...
| `a_compile_overtaken_by_an_epoch_move_spends_no_retry` | An unpublished compile during which the epoch moved releases its slot and spends no retry |
...
- **`inflight_epoch_moved` does not cancel.** Nothing cancels an in-flight
  compile; the artifact is built and then refused at publication. The flag's
  only effect on scheduling is that the refusal is not charged as a failure.
