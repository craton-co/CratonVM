...
> **Corrected 2026-09-18 (r9).** The encoder half of that is stale since R14
> (2026-09-17): `fp_load`, `fp_store`, `fp_binop` **and** `emit_xmm_frame_move`
> (the fourth no-REX encoder, fed by `IR_PROLOGUE_SAVED`) all emit REX now,
> pinned by `every_xmm_encoder_reaches_xmm8_through_rex`. The ceiling is an
> **ownership** question — XMM8–XMM15 belong to `xmm_roles::VECTOR_REGION_MAX`
> — plus 16 frame bytes per saved register on Win64. See the R14 note on
> `regalloc::xmm_roles::IR_LINEAR_SCAN`.
...
| `Guard` | yes | **no** |
| `Rem` (`Float`/`Double`), `Load(_)`, `Store(_)` | **no** | yes |
| `Rem` (`Int`/`Long`) | **no** | **no** (r9 — see below) |
...
* **`Rem`, `Load`, `Store` are calls but not oop-map publication sites.**
  ...
  `Op::Rem` without a type test (it sees only the op), so until 2026-09-18 an
  integer remainder was also treated as a clobber — and every `double` live
  across an `i % k` in a loop was split there, which on this path means not
  promoted at all. **r9:** `MachineModel::for_graph` now exempts an integral
  `Rem` (`regalloc::is_inline_integer_rem`): its lowering is `CDQ/CQO ; IDIV`
  between an inline `MIN / -1` guard and a zero-divisor deopt jump, and calls
  nothing. Test: `an_integer_remainder_is_not_a_call`.
...
4. **Splits and reloads are planned but not emitted.** Any value the allocator
   ...
   **Since R4, in the default-off arm:** `CRATONVM_JIT_IR_LS_SPLITS=1` follows
   the timeline position by position (`ls_split_transitions`). **r9 found it
   unsound on non-fall-through CFG edges** — the timeline is in layout order and
   the allocator does no edge resolution, so a value evicted inside a loop body
   is still "in r0" at the loop top on the back edge. Do not turn that flag on
   until `regalloc::split_edge_conflicts` is consulted (cross-lane request in
   `docs/internal/jit-review-r9/NOTES-regalloc.md`); see
   `docs/internal/fixed-bugs/linear-scan-split-timeline-has-no-edge-resolution-FIXED-20260918.md`.

   **r9 wave 2:** the allocator half of the real fix exists —
   `regalloc::split_edge_resolution` returns, per non-fall-through edge, the
   parallel copy (`Reg ← Reg` or `Reg ← home`) that makes the edge deliver
   every flagged value where the target block's code reads it, plus a
   placement (`PredEnd` / `SuccStart` / `Critical`) and an `unresolvable` set.
   The gate can be relaxed for a flagged value once this file emits those
   copies at its edge site — after the back-edge poll, sequenced together with
   `emit_phi_copies`' own copies — and refuses `Critical` edges it cannot split.
   **Last use at a call (r9 wave 2).** `MachineModel::reads_precede_clobber`
   (empty by default) lets a value whose last use is a call operand keep a
   caller-saved register through the call instead of splitting — which, with
   the whole-range consumer here, is the difference between a register and
   none. **Since r9 wave 4** this file opts in behind
   `CRATONVM_JIT_IR_LS_CALL_LAST_USE=1` (default off):
   `ir_lower_machine_model` calls
   `mark_operand_reads_first_where(.., ir_call_reads_operands_first)`, which
   lists only the audited ops (`Op::Store`, FP `Op::Rem`; `Op::Call` is refused
   because its safepoint map calls out before the arguments are marshalled),
   and both second opinions in `plan_register_residency` go through
   `clobbered_within_for_value`, which applies the same
   `MachineModel::read_before_clobber` exemption. See
