...
> **Status, verified 2026-09-18 (r9):** the EA copy is resolved.
> `ea_memory_token_slot` now lives in `jit/src/ea_ir_bridge.rs` (moved out of
> `lib.rs` on 2026-09-16) and is a one-line delegation to
> `ir::memory_token_slot`, so it cannot disagree with `Op::memory_shape`. The
> text below is the state this page was written against.
...
> **Status, verified 2026-09-18 (r9): landed.** `ir_op_to_ea_op` and the
> second-pass operand re-packing in `jit/src/ea_ir_bridge.rs` map
> `MonitorEnter`/`MonitorExit` (locked reference re-packed from slot 2 to EA
> input 0), and `apply_ea_to_ir_pinned` applies the per-object plans
> all-or-nothing. The builder emits both ops. The prescription below is kept
> for the record.
