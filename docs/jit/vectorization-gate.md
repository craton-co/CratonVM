...
### Unit step means `scale == 1` and `stride == 1`

The dependence arithmetic above only needs `step = scale * stride`, and until
round 9 the gate admitted any access with `step == 1`. That includes `a[-i]`
walked by `i--` (scale −1, stride −1): a contiguous ascending access, but not
one anything downstream can emit. `VecPlan` records neither sign, and
`vec_emit` addresses `[base + iv*size + disp]` with an *increasing* `iv` and an
`iv + lanes <= bound` head test. The gate now requires both factors to be `+1`
(`a_reversed_unit_step_is_refused`); a reversal-aware plan is future work, not
something to admit by accident.

## One element type per plan

`VecPlan` has one `elem`, and `vec_emit` chooses every opcode from it. So the
gate refuses (`MixedElementWidths`):

* accesses of two different element **types**, even at the same width — an
  `int[]` and a `float[]` in one body would have the float lanes added with
  `VPADDD`;
* an arithmetic node whose type differs from the element type — `long s +=
  a[i]` over `int[]` needs a widening the plan cannot describe, and handed to
  the emitter it would be folded as a 32-bit reduction (the exact truncation the
  single-pass `long` sum shipped once: eight `Integer.MAX_VALUE` summed to −8).
  `int` arithmetic over `byte`/`char`/`short` elements is the same refusal.

(`mixed_element_types_and_widening_arithmetic_are_refused`.)

## The element type comes from the memory model

`AliasClass::ArrayElem` now carries the element `MemKind`, read off the
accessing op by `ir::access_location`. The gate takes the element type from the
alias class, not from the producer's `VecBodyOp::elem`:

* a reference element on **either** side is `GcReferenceAccess` — a producer
  that labels an `Object[]` store as `int` can no longer talk the gate into a
  barrier-free vector store;
* a producer label that disagrees with the model is
  `UnstructuredMemoryAccess` — the producer mapped the wrong node, and its
  subscript for that node cannot be trusted either.

(`the_element_type_comes_from_the_memory_model_not_the_producer`.)

## Second exits

`CountedLoop::has_other_exit` (a `break`, `return` or `athrow` in the body) is
an `IrreducibleControl` refusal in its own right. It was already refused
indirectly — the trip-count minimum is 0 and the witness proof refuses — but the
gate no longer relies on the producer's `reducible` flag, or on that indirect
path, for a fact `scev` states directly.
...
`ARRAY_DATA_OFFSET + first_index * element_size` and asks whether it is a
multiple of the vector width, given a provable object-base alignment.
(`ARRAY_DATA_OFFSET`, not `HEADER_SIZE`: equal today at 16, but the planned
array-length prefix in `heap_types.rs` moves array data to 24.)
...
compile-time assert and nothing pins it higher. So the data offset is not the
obstacle — the base is. **16-byte alignment of an array's element 0 is not
provable in this VM**, and 32-byte alignment certainly is not.
...
| `IrreducibleControl` | widening assumes every iteration runs the same straight-line trace; also any second loop exit (`has_other_exit`) |
| `VariableStride` | a runtime step has no compile-time dependence distance |
| `NonUnitStep` | `scale != 1` or `stride != 1`; only the ascending unit-stride shape is emittable |
...
| `MixedElementWidths` | one plan `elem`: two element types, or arithmetic in a type other than the element's |
...
* ~~`jit/src/ir.rs` — `AliasClass::ArrayElem` carries no element type~~ —
  it does now, and since round 9 the gate reads it: see
  [The element type comes from the memory model](#the-element-type-comes-from-the-memory-model).
