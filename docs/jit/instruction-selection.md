...
**Status:** landed, compiled, and **wired behind flags** (all default off).
`ir_lower::build_mir_plan` runs `select_block`; under `CRATONVM_JIT=ir-isel-emit`
its `Rule::AluReg` tiles are encoded through this table and ARE the emitted
bytes, and under `ir-isel-verify` they are compared byte-for-byte against the
per-opcode arm (a mismatch refuses the compile). §6 items 2, 5 and 9 are
closed. A wrong row is therefore wrong code behind `ir-isel-emit`, not only a
failing test — see §6 item 11 for the one that was. See [Wiring](#wiring) and
`docs/feature-designs/jit-machine-level-and-instruction-selection.md` for the
increment order the rest of the wiring should follow.
...
## 0. Read this first: compiled; wired only behind flags
...
* ~~Anything at `Ty::I32`.~~ Admitted since §6 item 5 (`lea_r32_m`).
...
11. ~~**`Int` bitwise ops were selected at 64 bits.**~~ **Fixed (round 9).**
   `alu_op_of` mapped `And`/`Or`/`Xor` to `Ty::I64` for an `Int` node, on the
   reasoning that the low half of a 64-bit bitwise op is the 32-bit result.
   It is — but `ir_lower`'s `Int` arms had since moved to the 32-bit form
   because the *high* half matters: an int's home word must be the
   zero-extension of its low half, and a REX.W `AND`/`OR`/`XOR` over one
   sign-extended and one zero-extended operand yields a word that is neither
   (an out-of-bounds element address that passes a low-half bounds check;
   `x | -1` as `OR r64, imm8` sets all 64 bits). So under `ir-isel-emit` an
   `Int` `&`/`|`/`^` tile re-introduced that bug, and under `ir-isel-verify`
   every one was a byte mismatch that refused the compile. The ops now take
   the node's own width, backed by three new rows (`and/or/xor_r32_r32`,
   anchored to `ir_lower`'s `[0x21|0x09|0x31, 0xC8]`), and
   `an_int_bitwise_node_selects_the_32bit_form` pins it.
12. **An RSP SIB index is now refused structurally** (`Pattern::check`), not
   only by rows that remembered `Constraint::IndexNotRsp`. The two `LEA` rows
   — the ones an `IrAddr` lowering feeds — did not declare it, so an index
   allocated to RSP would have encoded `[base + rsp]` as `[base]`.

### The register-form ALU rows

`add_r64_r64`, `add_r32_r32`, `sub_r32_r32`, `and_r64_r64`, `or_r64_r64`,
`xor_r64_r64`, `imul_r64_r64`, and (round 9) `and_r32_r32`, `or_r32_r32`,
`xor_r32_r32`. Each is anchored to a byte literal
