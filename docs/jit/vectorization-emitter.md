...
"this value is only ever narrowed", so all sub-word bodies are refused. Adding
that fact (a `NarrowedOnly` value class, plus `VPMOVSX`/`VPMOVZX` widening loads
for the reduction case) is the cheapest route to `byte[]`/`char[]` coverage.
