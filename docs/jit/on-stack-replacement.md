...
## 8. Round 9 changes to the OSR exit and re-entry path (2026-09-19)

These three changes live in `vm/src/runtime/interpreter/jit_bridge.rs`. The pages behind them
are `docs/internal/fixed-bugs/evicted-callee-stays-reachable-through-a-compiled-callers-binding-FIXED-20260918.md`
and `../internal/fixed-bugs/perf-implicit-exceptions-in-compiled-code-cost-microseconds-each-FIXED-20260920.md`.

* **An OSR guard exit is charged to the guard** (`osr_guard_exit_reason`, kill switch
  `CRATONVM_JIT_OSR_GUARD_EXIT_CHARGE=0`).
  - Before: the OSR-exit transfer in `try_osr` recorded no deopt. An OSR body whose
    loop-header speculative bounds-check guard failed on every entry was reused forever,
    and the rest of the loop ran interpreted: `TryLoop throw` took 2 550 reuses and 0 deopts.
  - Now: a bounds-check or receiver-type guard exit is charged under the guard's own reason,
    with the same per-bci de-spec policy the normal compiled body uses. After 4 exits the
    guard is withdrawn, and the next OSR and normal compiles are guard-free.
  - The `OsrExit` map itself, the indy trap, and exits forced by the OSR-exit test flags are
    not charged.
* **Traps are attributed to the right point** (`guard_reason_for_trapped_frame`).
  - The loop-header `OsrExit` map is the first deopt point at its bci. The first-call
    tier-up sink in `interpreter.rs` and three deopt paths in `jit_bridge.rs` used to take
    the first point at the trapping bci, so they charged a guard trap as `OsrExit`. The
    method was then never evicted, and it ended not-compilable (`TryLoopRefl`).
  - Now the other points at the bci decide. When they do not agree, the answer falls back
    to `deopt_resume::despec_reason_for`.
* **A stale cached OSR body is recompiled** (`cached_osr_body_is_despec_stale`): a cached OSR
  body that still carries a guard the per-bci de-spec has since withdrawn is not reused.
  `CRATONVM_DBG_JITC=1` prints the reason as `cached-keeps-a-despeculated-guard`.
* **OSR re-entry after a caught exception** (`osr_reentry_memo_get`, kill switch
  `CRATONVM_JIT_OSR_REENTRY_MEMO=0`).
  - The handler runs in the interpreter, and the loop re-enters OSR at the same pc.
    Instead of the whole `compile_osr_artifact` gate chain, a per-thread memo remembers the
    last OSR body entered at that loop.
  - The memo is thrown away when the code cache or the deopt log changes. The body's
    `retired` flag and the OSR-denied check are re-read on every hit.

Measured on one host, interleaved:
- `TryLoop throw`: 2 415 -> 80 ms (HotSpot 16 ms).
- `TryLoopRefl`: 8 800 -> 89 ms.
- `ExcLoop`: 3.3 -> 3.05 us per caught exception.

Still open: running the handler in the compiled body instead of exiting to the interpreter.
That needs JIT exception-dispatch codegen; see the implicit-exceptions page.
