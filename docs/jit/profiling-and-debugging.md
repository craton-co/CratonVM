...
| `CRATONVM_JIT_PRINT_COMPILATION` | `CRATONVM_JIT=print-compilation` | one line per compilation on stderr | all |
...
```bash
CRATONVM_JIT=print-compilation cratonvm -cp app.jar Main 2>compiles.txt
```

The `-XX:+PrintCompilation` analogue: one line per compilation on stderr, in
the order the compiler finished them.

```text
      0      0 c1 java/lang/String.hashCode()I (184 bytes)
     12      1 c1 pkg/Cls.loop([I)J (512 bytes)
     14      2 c2 pkg/Cls.loop([I)J bailout:register pressure @lower
     31      3 c2 pkg/Cls.loop([I)J (1184 bytes)
     33      4 c2 pkg/Cls.cold()V (96 bytes) (c1-body)
```

| Column | Meaning |
|---|---|
| 1 | milliseconds since the FIRST compilation of the process, not since process start — the origin is fixed so consecutive lines are comparable with each other |
| 2 | a monotonic id, so two lines in the same millisecond are still orderable |
| 3 | the tier that was REQUESTED: `c1` single-pass, `c2` optimizing |
| 4 | `class.method(descriptor)` |
| 5 | `(<n> bytes)` for an installed body, `bailout:<reason> @<phase>` for a refusal, `abandoned` for a compile that produced neither |

`(c1-body)` marks a `c2` request that fell through to the single-pass backend.
That event is the reason the tier column reports the request rather than the
outcome: a fall-through is exactly what an operator reading this stream is
looking for, and a column that reported only the outcome would hide it.

Only the FIRST bailout reason is printed. A bail cascades — the phase that
refuses reports it and so does each caller unwinding past it — and the line
wants the cause, not the last frame to notice. The full list, with every phase
that reported, is in `CRATONVM_JIT_METRICS`'s structured report.

This flag is **independent of `CRATONVM_JIT_METRICS`**: the stream prints a
line and retains nothing, while the metrics ring allocates a report per compile
and keeps a window of them. Turning on one does not turn on the other.

Implementation: `jit/src/metrics.rs`, `PrintCompilation` and the three call
sites on `CompileRecorder` (`begin`, `installed`, `note_bailout_reason`, plus
`Drop` for the no-body case).

> The grouped spelling is parsed by `print_compilation_enabled` itself, and
> `print-compilation` **is** a row in `types/src/flag_groups.rs`'s registry
> (since 2026-09-17), so the flag-group resolver no longer reports it as an
> unknown token. This paragraph used to say the row was missing; that was true
