...
| C4 | Every node strictly between `first.exit` and `second.enter` is on the gap allowlist — by id, and (r9) every load whose recorded memory token lies in that range, whatever its id |
...
#### Which nodes are "in the gap" (r9, 2026-09-18)

"Between" was decided by node id alone, which is creation order. A node a
later pass creates does not have to respect it: a full unroll
(`ir_optimize::unroll`) clones a loop's loads with fresh, high ids into the
preheader, so after an unroll that left the graph branch-free (C2 holds) a
cloned load can execute between the two regions while its id sits after both.
A clone keeps its original memory token, and a load whose token lies in
`[first.exit, second.enter)` executes before the next memory operation, i.e.
before `second.enter`. So C4 now also runs `gap_node_refusal` over every load in
`Graph::load_mem_tokens` whose token is in that range but whose id is not.
Coarsening is opt-in (`CRATONVM_JIT_LOCK_COARSEN=1`), so this closed a hole in
an off-by-default transform. Test:
`r9_ea_tests::a_cloned_load_in_the_gap_is_seen_by_its_memory_token`.
