...
* **External** — `1 - largest_free_extent / free_bytes`. Answers "can a 40 KiB
  ...
  macOS/ARM64) and keeps its own census (`jit_code_arena_census`), but nothing
  feeds that census into this report yet, so even with the arena on the
  gauge reads `0.0`. A real limitation, not a measured zero — see
  *Reconciliation* 6 below.
...
`external_fragmentation` reads `0.0` until something does. In the DEFAULT
...
have free space and already computes it (`JitCodeArenaCensus`, logged by
`log_jit_code_arena_census`). The remaining wiring is one call: copy that
census's free-list shape into `record_free_space` from the vm-side report.
Until then the gauge understates fragmentation in exactly the configuration
where it can be non-zero.
