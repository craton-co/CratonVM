...
**The reader side of publication (AArch64).** The writer's flush and flip
cannot give *other* cores the context-synchronisation event ARM ARM B2.4.4
requires before they execute new instructions; on Linux and Windows the
protection change's TLB-shootdown IPI supplies it by accident, and on
macOS/ARM64 (`MAP_JIT`, no protection change) nothing does. So
`validate_code_ptr` — on every compiled entry — issues an `ISB` at most once
per thread per value of the code-region epoch
(`platform::synchronize_instruction_stream_for_epoch`). That epoch must move
AFTER the bytes exist and BEFORE the entry is observable; it is bumped at
region registration and again in `ExecutableBuffer::finalize`
(`note_code_stream_published`). Before round 9 only the registration bump
existed, which happens before a single byte is emitted, so a thread that
dispatched anything while a body was still being written recorded that epoch
as synchronised and then entered the finished body with no barrier. Compiled
...
| `failed_allocations`, `failed_allocation_bytes`, `failed_allocations_by_reason[4]` | `jit_code_alloc_failure_stats()` — the four sites that can actually refuse |
| `free_extents`, `free_bytes`, `largest_free_extent` | `platform::jit_code_arena_census()`, when `CRATONVM_JIT_CODE_ARENA` is on; structurally zero when it is off, because without the arena every body is its own mapping and there is no free list to describe |

The last two rows landed in round 10 wave 5. This page previously grouped the
allocation failures and free extents with the unfed fields below, which stopped
being true then.

**The seven fields that used to have no producer on the process path, and what
each one reads now.** Only `CodeCacheLifecycle::install`, `::retire` and
`::sweep` write them, and no production path calls any of the three on the
process instance — so on every real run all seven were `0`. Three rounds of
waves closed five of them; the split is per FIELD, because
`ModelledFieldSources`' flags are per printed fragment:

| Field group | State | Producer, or why not |
|---|---|---|
| `peak_live_bytes` | **FED** (wave 8) | `installed_bytes - reclaimed_bytes` sampled into a `fetch_max` at each publish. A LOWER bound by construction, so `code_cache_lifecycle_raw` clamps it with `.max(live)` |
| `retirements_by_reason[6]` | **FED** (wave 8) | every withdrawal site in `cratonvm_jit` passes a `retire_reason` chosen by its named entry point |
| the deferral AGE | **FED** (wave 8), in a new field | `jit_oldest_retirement_age()`, in retirement GENERATIONS, into `oldest_deferral_generations` — not into `max_deferral_sweeps`, which is a different unit |
| `recompilations`, `methods_compiled`, `max_versions_for_one_method` | **FED** (wave 9) | `jit_compilation_census()`: one publication count per method key, filled at `JitCache::put`/`put_osr` and gated on the same `mark_published` call that bumps `installed_bodies`. Bounded; past its bound it answers `None` and the group is suppressed again |
| `deferrals`, `max_deferral_sweeps` | still SUPPRESSED | both counted in SWEEPS. The queue advances a generation per WITHDRAWAL and keeps no per-owner drain count, so there is nothing in that unit to pull |

Round 10 wave 7 made that absence **visible rather than zero**, which is what
made the print site safe to add at all.
`CodeCacheLifecycleRaw::modelled_sources` (a `ModelledFieldSources`, four
independent flags — one per printed fragment) records which groups a snapshot has
a producer for: `ModelledFieldSources::ALL` from an instance of the model,
`NONE` from `code_cache_lifecycle_raw()`'s pull, which then flips back the ones
it can feed. `Display` prints a group's figures only when its flag is set, and a
named `n/a` with the reason when it is not.

That mattered because two of the readings were not merely zero but
arithmetically impossible, which is how the defect was found by inspection:

* `peak_live_bytes` at `0` beside a large `live_bytes` — a high-water mark
  cannot be below the current value;
* `versions_per_method` at `0.0000` beside a positive `installs` — a method
  with a compiled body has at least one version, and `0.0` is what the module's
  `ratio` helper answers for a zero denominator rather than `NaN`.

and a third was worse for being silent: `retire_reason_breakdown()` filters out
zero buckets, so the "retirement by reason" line simply did not appear —
absence that reads as "nothing was retired", beside a `retired=` saying
otherwise.

`docs/feature-designs/jit-r10-report-proposals.md` specifies the four
`cratonvm_jit` accessors that turned four of the five groups back into a figure.
Each one flips exactly one flag, at the pull that feeds it.
...
Three of these were computed on every call, asserted by the module's tests and
printed by nothing until round 10 wave 7: `reachable_bytes`,
`sweep_success_rate` and `mean_body_bytes`. All three derive entirely from
pulled inputs, so printing them needed no new producer. `reachable` sits on the
first line beside `live` for the reason given above — a retired body is still
mapped, and `live` against `reachable` is the difference between a full cache
and a wedged one. `success_rate` sits beside `sweeps=` / `(deferred N)` because
it is exactly their quotient.

`recompilation_ratio` and `versions_per_method` were the two ratios NOT printed
on the process path, because their denominators were among the seven unsourced
fields. Round 10 wave 9 fed the compilation census and they print now.
`versions_per_method` is at or above `1.0` by construction: the census and
`installed_bodies` are bumped from the same `ExecutableBuffer::mark_published`
call, and `code_cache_lifecycle_raw` samples the census BEFORE the install
counter so the stale side is the denominator.

Two arms still keep the line off. The census answering `None` — its bounded table
filled — prints the `n/a`, because a truncated denominator beside a climbing
`installs` reads exactly like the thrash the ratio exists to detect. And
`methods_compiled == 0` prints "no method has a compiled body yet" rather than
the two ratios, because `ratio(_, 0)` is `0.0` and the literal string
`versions_per_method=0.0000` is the reading this whole story taught readers to
treat as a broken counter. It must not be printed even on the one snapshot where
it is arithmetically defensible.
...
* **External** — `1 - largest_free_extent / free_bytes`. Answers "can a 40 KiB
  body be installed even though 400 KiB is free?". By default CratonVM
  allocates each body as its own OS mapping, so there is no shared free space
  and this reads `0.0`. A shared arena exists (`platform::JitCodeArena`,
  opt-in with `CRATONVM_JIT_CODE_ARENA`, default OFF and inert on
  macOS/ARM64) and keeps its own census (`jit_code_arena_census`). Round 10
  wave 5 wired that census in: `code_cache_lifecycle_raw()` calls
  `arena_free_space(&platform::jit_code_arena_census())` when
  `platform::jit_code_arena_enabled()`, so with the arena ON this is a real
  ratio. With the arena OFF the `0.0` is structural (no free list exists to be
  fragmented), not a missing measurement. This page previously said nothing fed
  the census; that stopped being true in wave 5.
...
`cratonvm_types::flags().jit.method_stats`, beside the
`[cratonvm] implicit null-check table health:` line. Its lines carry their own
`[JIT]` tag, the way `collector_decision_report`'s carry `[GC]`.

Before round 10 wave 7 there was no print site at all: every reference to
`code_cache_lifecycle_report()` in the tree was its own declaration, a doc
comment or `#[cfg(test)] mod tests`, so the whole pull — and the poison-free
warning and keep-alive census the `Display` also carries, both must-be-zero
correctness signals — ran only under `cargo test`
(`docs/internal/retired/r10-wrappers-process-report-has-no-reader-and-seven-unfed-fields-20260921-RETIRED-20260922.md`).
The suppression described under *Counter inventory* had to land first and in the
same change, because printing an impossible number is worse than printing
nothing.

Printed unconditionally on that switch rather than only when something moved:
`installed=0 bodies` says the JIT published nothing on this workload, which is
the first thing to rule out when a perf claim does not reproduce, and it is not
distinguishable from a missing line.
...
// Per-process report: one `Display`, five lines always plus up to three
// conditional ones (the poison-free warning, the keep-alive census, and the
// per-reason retirement line or its `n/a`). `vm-cli` already prints this at
// shutdown under `CRATONVM_DBG_JIT_METHOD_STATS=1` (or `CRATONVM_DBG` set to
// `jit-method-stats`); call it directly only from a harness.
...
body itself is withdrawn once, from the cache.

Both halves are closed now. Round 10 wave 8 threaded a `retire_reason` through
every withdrawal site — `retire_withdrawn_body` takes the code and bumps
`withdrawn_bodies` and one bucket together, so the histogram sums to the total by
construction — and wave 9 added the per-method publication census that
`recompilations` needs, filled at `put` and `put_osr`. Note that the two count
different populations on purpose: `withdrawn_by_reason[SUPERSEDED]` counts
publications that displaced a body still in the map, and `recompilations` counts
every publication that was not a method's first, so a body withdrawn by a class
unload and later recompiled is the second and not the first.

### 5. `vm/src/runtime/jit_integration.rs` — the model cache

`CodeCache::install` returns `CodeCacheError::Full { available, needed }`:
that is a `CAP_EXCEEDED` allocation failure verbatim. It is not *recorded* by
calling into this module, though — round 10 established that `jit` cannot depend
on `vm`, so the refusal is counted on the JIT side and the report PULLS it
(`cratonvm_jit::jit_code_alloc_failure_stats()`). The three `record_*` push
entry points this paragraph used to name are deleted; see
`docs/known-issues/jit/r10-gcs2-code-cache-alloc-failure-gauges-never-fed-20260921.md`.
This model cache's own `Full` refusal is a separate thing from the real cap gate
and is still not counted anywhere. `CodeCache::sweep` drops every `is_valid == false` entry
...
### 6. ~~Nothing publishes a `FreeSpace` gauge~~ — DONE by PULL, and `0.0` is structural without the arena

The heading above described the state before round 10 and contradicted the
paragraphs under it, which is the shape this round spent itself removing one
level down: a reader greps headings.

`external_fragmentation` reads `0.0` with no arena. In the DEFAULT
configuration there is nothing to publish: `ExecutableBuffer::new` takes one OS
mapping per body. That is itself the finding — a per-body `mmap` has no
external fragmentation but pays a page of internal fragmentation per compiled
method plus one VMA per method, which the `internal_fragmentation` gauge now
measures.

With `CRATONVM_JIT_CODE_ARENA=1` the bodies allocated through
`ExecutableBuffer::new_in` come out of `platform::JitCodeArena`, which does
have free space and already computes it (`JitCodeArenaCensus`). Round 10 wired
that half: the report now PULLS the census when the arena is enabled, and the
`record_free_space` push entry point this paragraph used to prescribe is
deleted, because `jit` cannot depend on `vm` and so no JIT-side site could ever
have called it. Outside the arena configuration the gauge still reads `0.0`, for
the reason given above — there is genuinely nothing to publish.
