...
| `peak_live_values` | **real** (optimizing path) | `ir_lower`'s frame-slot planner (`SlotPlan::peak_live`) through the ambient `metrics::note_current_peak_live_values`. The single-pass backend computes no live ranges and leaves it `null`. |
| `spills`, `reloads` | **real only under `CRATONVM_JIT_IR_LINEAR_SCAN`** | The EMITTED counts, from `ir_lower` through `metrics::note_current_spills` / `note_current_reloads`. `null` on every path that allocates no registers (the default build, and the single-pass backend). |
| `planned_spills`, `planned_reloads` | **not measured** | The allocation's PLAN, deliberately a separate pair so plan-vs-emission is itself the measurement. `CompileRecorder::set_planned_spills` / `set_planned_reloads` exist and have no production caller; see `CompilationReport::planned_spills`. |
...
* **Disabled** (the default): `CompileRecorder::begin` is one relaxed atomic
  load (plus the latched `print_compilation_enabled` read, which is its own
  independent switch) and returns a handle holding `None`. No allocation, no clock read, no
...
`bailout_categories` is the exception, and so are the four tables beside it --
`scheduling` (`scheduling_counts()`), `osr` (`osr_counts()`), `loop_xform` and
`deopt_stash`: each is its process-wide counter table verbatim, unbounded, and
fed whether or not metrics are enabled. `scheduling` is the one to read before
concluding a method was never hot, since a request the scheduler dropped
produces no report at all; `metrics::scheduling_dropped_total()` sums its four
drop rows (stale epoch, class invalidated, shutdown, deoptimized).

**Scheduler-side numbers live on the tier manager, not here.** Compile counts
by tier, deopts, `c2_bailouts` and the per-lane compile-queue latency
(`CompilationStats::dispatched` / `queue_wait_ms_total` / `queue_wait_ms_max`)
are printed by `tiered::dump_method_stats_to_stderr`
(`CRATONVM_DBG_JIT_METHOD_STATS=1`) as the `JIT method stats` and
`JIT compile queue` lines. Their `osr=` counts published OSR artifacts (it
counted queued OSR *requests* until 2026-09-18), and `c2_bailouts=` counts
methods that became bailed out, once each. The queue line's
`osr_refused_in_flight=` counts `request_osr` calls refused because the
method's own OSR request was still queued or compiling (a method has separate
method-entry and OSR in-flight slots since round 9 wave 2). A third line,
`JIT time-to-tier`, splits time-to-peak into its two waits, once per method:
`nominated->first-body` (first method-entry admission to first published
method-entry body) and `first-body->c2` (first body to first method-entry C2
body; `0` for a method whose first body was C2). Fields:
`CompilationStats::{nominate_to_first_body, first_body_to_c2}` and their
`*_ms_total` / `*_ms_max`.
