...
|---|---|
| One request per method at a time (`RequestKey` dedup) | `CompilerCore::admit`: two in-flight slots per method — one for a method-entry request, one for an OSR request (any bci) — each held from admission until completion, stale drop, deopt drop, unload or shutdown. A second request for a held slot — identical or not — is refused and counted as `queue_deduplicated` (`request_osr` checks its slot first and counts that refusal in `CompilationStats::osr_refused_in_flight` instead). The slots were one until round 9 wave 2, which kept a hot loop from OSR for the whole of its own method's C1 and C2-supersede pipeline; an OSR artifact lives in a separate cache, so the two never compile one artifact twice. `enqueue_compilation`, the VM's deopt re-queue door, goes through it. This matters more now that each compile lane has its own workers: a duplicate would be the same method compiled concurrently into two code buffers. |
...
| Code-cache pressure as an admission input | Ported (2026-09-18, the RT-8 soak). `CompilerCore::admit` asks `jit_code_cache_at_capacity` itself, marks the method `deferred_at_cap`, counts `cap_refusals()`, and charges nothing to `tier_fail_count`; the method stays settled until `note_body_withdrawn` or `note_code_cache_pressure` moves the settled generation. Before this, the cap came back from the worker as an unpublished compile and three of them settled the method for good. |
...
- **An in-flight compile cannot be cancelled.** A dispatched compile runs to
  ...
  wasted, and `inflight_epoch_moved` counts it. The refusal is not charged to
  the method's `tier_fail_count` (`Completion::epoch_moved`), so the waste is
  the compile time only, not one of the method's retries.
