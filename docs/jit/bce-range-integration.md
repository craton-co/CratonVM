...
* **A limit local that is a skippable-store PARAMETER** (round 9,
  2026-09-18). `find_bound_arraylength_provenance` used to accept any local
  with exactly one store, that store being `aload A; arraylength; istore`,
  and argued the store dominated every read by definite assignment. A
  parameter is definitely assigned at entry, so `void fill(int[] a, int n,
  boolean all) { if (all) n = a.length; for (i = 0; i < n; i++) a[i] = 0; }`
  was rewritten to limit `a.length`, `a[i]` was discharged **statically**,
  and `fill(new int[2], 100, false)` stored 98 ints past the array. The proof
  now checks the dominance on the CFG (`store_dominates_every_read`, which
  also roots every pc no normal edge reaches, as a possible handler entry).
  Such a loop falls back to the per-array speculative guard, which checks
  `a.length >= n` at run time. `find_iv_nonneg_start` had the same hole for a
  parameter IV and takes the same check; `loop_analysis::constant_iv_init`
