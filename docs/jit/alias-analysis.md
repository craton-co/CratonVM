...
* `System.identityHashCode`, and an `Object.hashCode` that is not overridden — a
  stable per-object value derived from the header (`Op::IdentityHash`). The
  bridge recognises only the first: an `invokespecial Object.hashCode()I` can
  select an override under `ACC_SUPER` (JVMS §6.5), so since r9 it stays an
  ordinary escaping `Op::Call` (`docs/jit/escape-analysis.md` §4).
...
5. **`Op::Safepoint` is unproduced in the EA graph.** `escape_analysis_from_ir`
   emits `RefCompare` and `IdentityHash` for the shapes it can prove, and
   `Op::Throw` since cov-07 (`ir::Op::Throw`, forwarded with its token dropped
   since r9), but nothing emits `Op::Safepoint` (the IR keeps safepoints in a
   side table, not as nodes). `LockRefusal::SafepointInGap` is therefore
   unreachable from production IR, and lock coarsening relies on the gap
   allowlist alone. The allowlist admits no node that can be a safepoint, so
   the outcome is right; the *reason* is incidental. *(Corrected 2026-09-18:
   this item used to say nothing emits `Op::Throw`.)*
