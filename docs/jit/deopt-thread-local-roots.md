...
| `deopt.rs` `DespecRegistry` (was the process-global `DESPEC_SET` until 2026-09-12; now one per VM, `JitRealm::despec_registry`) | `RwLock<FxHashSet<(String, u32)>>` — `(method_key, bci)` | `DespecRegistry::insert` | VM lifetime | n/a | **none** — no heap ref. Strings and bcis |
