...
`rbc6-getfield-putfield-npe-escape-FIXED-20260818.md`. A status
...
`jit-signals-root-gap.md`, which moved the JIT's pending
...
| the addresses exist **only** at runtime, never in metadata | `DeoptVerifier` already rejects a non-zero `FrameValue::Object` in an emitted `FrameState` as `DeoptMetadataError::BakedObjectAddress`. So `DeoptimizationPoint` and the leaked deopt-point boxes are address-free by construction — the two thread-locals are the whole exposure. (`FrameStateInterner` was listed here too; it was retired 2026-09-21, `deopt-frame-state-interning.md` §7, which strictly shrinks this surface) |
...
| `deopt.rs` `DespecRegistry` (was the process-global `DESPEC_SET` until 2026-09-12; now one per VM, `JitRealm::despec_registry`) | `RwLock<FxHashMap<String, FxHashMap<u32, Vec<u32>>>>` — `(method_key, bci, speculation_id)` (flat `FxHashSet` until r9) | `DespecRegistry::insert` | VM lifetime | n/a | **none** — no heap ref. Strings and bcis |
...
| `deopt.rs` `DeoptimizationPoint` / `FrameState` / `DeoptimizationLog` / `InvalidationManager` | method keys, bcis, slot *descriptions* (`StackSlotRef(off)`, `RegisterRef(r)`), class ids | the compiler frontends | process lifetime (boxes are leaked; see `CompiledMethod`'s Drop) | n/a | **none** — a baked address here is already a hard error (`BakedObjectAddress`), and no non-test producer emits one. (`FrameStateInterner` and its `Arc<[FrameValue]>` chunks were in this row until 2026-09-21; retired, `deopt-frame-state-interning.md` §7) |
