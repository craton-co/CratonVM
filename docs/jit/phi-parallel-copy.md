...
> **Both items below are done (checked 2026-09-18, r9).** Kept for the record.

~~`docs/jit/linear-scan-regalloc.md` carries a block titled *"Reconcile — this is
a live bug in `ir_lower.rs`"*~~ — gone; that file's *Phi resolution is a
parallel copy* section now says "Two callers now, not one" and points at the
consumer.

~~`regalloc::ValueLoc::Slot`'s doc says the payload is "an 8-byte word index, as
`SlotPlan::node_color` numbers them"~~ — relaxed in R7: the doc now says whose
numbering a `Slot` is in (the `Allocation`'s, or the consumer's after
`Allocation::adopt_home_colouring`) and that a backend building `ValueLoc`s
itself may use any injective per-word token, which is what §3's byte offset is.

## 8. What the same sequencer must NOT be asked to do (r9)

`resolve_parallel_copy` orders ONE simultaneous copy. It is not an edge
resolver: the linear-scan allocation it is fed from is a timeline in block
LAYOUT order, and when the scan splits a value the location at the top of a
block is only right for the fall-through edge. The phi copies are safe from
that because every phi source is read at the predecessor's own edge position
(`phi_edge_copies`, `ir_lower::emit_phi_copies`). Split residency's
per-position transitions are not — see
`docs/internal/fixed-bugs/linear-scan-split-timeline-has-no-edge-resolution-FIXED-20260918.md`
and `regalloc::split_edge_conflicts`.

**r9 wave 2:** the edge resolver now exists as its own function,
`regalloc::split_edge_resolution`, and it FEEDS this sequencer rather than
replacing it: its per-edge copies use the same `(destination, source)`
convention as `phi_edge_copies`, so a consumer appends them to the edge's phi
copies and sequences both through one `resolve_parallel_copy` call — which is
required, not optional, because a phi copy can read a register a resolution
copy writes (and the reverse). The duplicate-destination refusal still holds
across the union: a resolution destination is always a register the target
block's code reads a live-in value from, and a phi is never live into its own
block, so a collision there is a malformed plan, not a sequencing problem.
