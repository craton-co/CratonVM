...
| **Implicit null check / signal contract** | field and array accesses fault on a low address; `vm/src/runtime/crash_handler.rs` is the contract, plus explicit `emit_null_check_*` stubs for the cases where a fault would be uninformative | **unreachable** — no dereference of a Java object ever occurs | N/A by construction. Note the corollary: an AArch64 compiled body has *no* recognised faulting PCs, so any SIGSEGV inside one is a genuine backend bug, never an implicit exception. |
