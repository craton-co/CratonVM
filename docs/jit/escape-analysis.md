...
**Still open.** A box that escapes pays a helper allocation per `valueOf`: `BoxProbe box`
takes about 765 ms, against 26 ms on HotSpot. Fixing it needs inline allocation of
`Integer.valueOf` in compiled code, see
`../known-issues/perf/perf-collection-natives-cost-a-generic-native-dispatch-per-call-20260918.md`.
