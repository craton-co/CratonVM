...
| 34 | **The producer's functions match the `DirectHelperFn*` aliases** | the `const _` block after `direct_helper_table` in `vm/src/jit/helpers.rs` (`let _: DirectHelperFnIntegerValueOf = jit_integer_value_of_direct;` …), covering 34 of the 34 `DirectHelperTable` words that are ever called | CT | Changing a `jit_*_direct` helper's Rust signature without changing its `direct_helper_fn_slots!` row |
...
comment says so. It is the **only** one missing — 59 assertions for 60 aliases.

(An earlier revision of this paragraph also named `HelperFnFfmSegmentGet`,
`HelperFnFfmSegmentSet` and `HelperFnLocalHandlerLookup` as absent "with no
comment saying why". That was stale when written: all three are asserted in the
block, at `vm/src/jit/helpers.rs`'s `local_handler_lookup` / `ffm_segment_*`
lines. Corrected here so the next reader does not go looking for three gaps
that are not there — and misses the one that is.)
...
| Invariant | Status |
|---|---|
| **Each call site's hand-written argument setup matches the helper's arity and return-ness** | **Partly asserted.** `helpers_abi::helper_sig` / `helper_sig_of` are the `const fn` lookup that did not exist, and `assert_helper_call_shape!` is the call-site macro built on them. Every hand-written site in `jit/src/runtime_lowering.rs` now carries one (`new_object`, `newarray`, `anewarray_object`, `new_object_cp`, `ldc_class_cp`, `monitor_enter`, `monitor_exit`, `frame_record`, `service_callee_deopt`, `tlab_post_init`). **Now also asserted** at every helper-slot call in `jit/src/x64/*.rs` — with `assert_direct_helper_call_shape!` at the two savebase watchpoints, and hand-written `const _` blocks at the four sites where the macro would assert the WRONG shape (`jit_frem`/`jit_drem` and `math_fma_float`/`math_fma_double` travel in XMM registers, so `float_args == 0` is false of them; `invoke_virtual_mic` hand-writes two Win64 overflow slots, so `win64_stack_args() == 0` is false of it). `jit/src/x64.rs` itself has no helper-slot call to guard — its only `emit_call_absolute` mention is in a doc comment. `jit/src/ir_lower.rs` likewise has none, and its equivalent hazard is covered by the `CALL_ARG_REGS` assertion in the next row. |
| **`CALL_ARG_REGS` is wide enough** | **Asserted.** `jit/src/ir_lower.rs` caps `CALL_ARG_REGS` at 4 on both platforms; the `const _` block immediately below the declaration now ties that to the helper census — `invoke_dispatch` must still be arity 4 with no float arguments and no Win64 stack arguments, `CALL_ARG_REGS.len() >= 4`, and `HELPERS_NEEDING_WIN64_STACK_ARGS == 1`. The last is the load-bearing one: this tier stages every helper argument into `CALL_ARG_REGS[0..=3]` and writes no overflow slot at all, so a SECOND helper acquiring stack arguments would be a silently truncated call here. The 6-argument `invoke_virtual_mic` site is the one member of that census, and it is a single-pass site that hand-writes its fifth and sixth arguments (§4). |
| **`HelperFnGetCurrentThread`'s POINTEE type** | **Everything else about it is now pinned** (ABI-4, `vm/src/jit/helpers.rs`): the producer's full shape by `const _: unsafe extern "C" fn() -> *mut JvmThread = jit_get_current_thread;`, and its arity, float-argument count and return-ness through `helper_sig_of("get_current_thread")`, whose row `helper_fn_slots!` derives from the alias. The alias says `*mut c_void`; nothing ties that to `JvmThread`, and nothing can while the alias is spelled in a crate that cannot name the type. The one ABI-relevant part of the residue IS asserted — that `*mut JvmThread` is a THIN pointer, since an unsized `JvmThread` would return a two-word value in RAX:RDX while the inline-TLAB fast path reads RAX alone. Making the producer return `*mut c_void` would restore the plain coercion and a 60-of-60 count, and was declined: it weakens the producer assertion in exchange for a number. |
...
The check is only as good as the return tag the call site passes it. Every
site must pass the CALLEE's real return type: a site that passes `b'I'` for a
`long`/`double`/`float` callee takes the plain arm and reads a legitimate
`Long.MIN_VALUE` — or `-0.0`, whose bits are exactly `i64::MIN` — as a throw.
The self-recursive `invokestatic` arm did exactly that until round 9 (it
claimed the descriptor was unavailable; it is in `method_key`); it now reads
the tag out of `method_key`, and only an empty key (the legacy test wrapper)
still falls back to `b'I'`.

`int` results must also arrive SIGN-EXTENDED in RAX (the single-pass value
convention every `int` producer ends in `MOVSXD` for). A 32-bit instruction
that leaves `0x00000000_FFFFFFFF` for `-1` is not a sentinel hazard — the check
compares all 64 bits — but it is wrong for every 64-bit consumer after it,
including the inline mini-emitter's no-op `i2l`. Round 9 fixed seven
`invokestatic` intrinsics that ended that way; see
`docs/internal/jit-review-r9/NOTES-invoke.md`.
...
## 10. The second helper table: `DirectHelperTable`

`JitRuntimeHelpers` is not the only table of addresses the backends bake into a
`CALL`. `jit/src/direct_helpers.rs`'s `DirectHelperTable` — built per compile by
`vm/src/jit/helpers.rs::direct_helper_table()` and handed to each compile through
`CompileRequest` — carries 28 further scalar callable slots, three `VarHandle`
arrays (40 read + 36 write + 9 CAS monomorphisations), and four words the
compiler itself calls. Until this section existed it was not mentioned anywhere
under `docs/jit/`, and it had none of §2's machinery: no signature aliases, no
derived signature table, no arity assertions, no version, and — for two of its
slots — no `extern "C"`.
...
| Piece | Where | Kind |
|---|---|---|
| `DirectHelperFn*` aliases, one per slot | `jit/src/direct_helpers.rs`, `direct_helper_fn_slots!` | CT |
| `DIRECT_HELPER_FN_SIGS`, derived from those aliases | same | CT |
| Machine-word rule on every argument | per-row `const _` | CT |
| Hand-written sites fit the Win64 register file | `const _` over `DirectHelperCallPath::HandWritten` | CT |
| `DIRECT_HELPERS_NEEDING_WIN64_STACK_ARGS == 9`, `..._SYSV_... == 4` | `const _` | CT |
| Slot census (struct size ↔ row count) | `const _` | CT |
| `direct_helper_sig` / `direct_helper_sig_of` / `assert_direct_helper_call_shape!` | same | CT |
| Distinct field names, the hand-written set, the marshalled overflow set | `direct_helper_abi_tests` | RT |
| `DIRECT_HELPER_ABI_VERSION` | same | doc |

### The three call paths

The single most important thing this table needs that `JitRuntimeHelpers` does
not is a distinction between *how* a slot's address reaches a `CALL`. The flat
`HELPERS_NEEDING_WIN64_STACK_ARGS == 1` pin cannot be transplanted, because
nine of these slots legitimately exceed four arguments:

* **`HandWritten`** — the emitter loads `ENTRY_ABI_REGS` itself and emits an
  absolute `CALL`. `monitor_enter`, `monitor_exit`
  (`runtime_lowering::emit_monitor_stub`), `arm_savebase_watch`,
  `disarm_savebase_watch` (`x64/frames.rs`). **No stack-argument path exists on
  this route**, so arity must stay ≤ 4.
* **`JavaArgMarshalled`** — the address is installed as a `JitDirectCall` and
  reached through the backends' general Java-call marshalling, whose
  `emit_stack_arg_setup` stages arguments past the register file. This is why
  `varhandle_cas` (5 words) and the eight `ScopedMemoryAccess` accessors (6 for
  the reads, **7** for the writes) are admissible. The four write accessors are
  the only helpers in either table that spill on SysV as well as Win64.
* **`CompilerOnly`** — `static_base_resolver` (+ its `SharedVm` context word)
  and the two served-class predicates. Never in generated code; called from
  Rust while emitting.

So the answer to "does `varhandle_cas` belong in the Win64 stack-args set?" is:
it belongs in *this table's* set, which it is in, and not in
`HELPERS_NEEDING_WIN64_STACK_ARGS`, which describes a different call path with a
different marshalling story. `varhandle_cas_overflows_win64_and_is_marshalled_not_hand_written`
pins exactly that.

### `extern "C"`, not `extern "Rust"`

`buffer_session_served_class` and `nio_byte_element_served_class` were
transmuted to bare `fn(u32) -> bool` / `fn(u32, bool) -> bool` pointers — the
**Rust** ABI, which is explicitly unspecified and carries no guarantee between
separately compiled crates or across codegen flags. The entire reason these
travel as an erased `usize` is that the two ends compile separately. They are
now transmuted through `DirectHelperFnBufferSessionServedClass` /
`DirectHelperFnNioByteElementServedClass`, which are `unsafe extern "C"`.

**Both halves have landed.** The producers were declared `extern "C"` on
`fix/jitr4-abi`, and the two ends are tied together by the producer-side
coercion block described next — invariant 34, the twin of invariant 33 — whose
last three lines coerce `jit_resolve_static_base` and the two served-class
predicates to their `DirectHelperFn*` aliases. That is where the two spellings
finally meet: each producer also carries a local `const _` pin in its own crate,
because an `extern "C" fn` item is called by name identically to a plain one, so
the `extern "C"` would otherwise be deletable without a single error there.

### The producer-side coercion block

`direct_helper_table()` writes `jit_foo as *const () as usize` for every slot, a
cast that type-checks against nothing. The aliases above are only half the link;
the other half has to be compiled where `jit_*_direct` is nameable, i.e. beside
`direct_helper_table()` in `vm/src/jit/helpers.rs`. **That block exists** — it is
invariant 34, and it covers 34 of the 34 `DirectHelperTable` words that are ever
called (the 31 `direct_helper_fn_slots!` rows, plus `static_base_resolver` and
the two served-class predicates). The only word it does not cover is
`static_base_resolver_ctx`, which is a `SharedVm` address rather than a function
and so has no signature to pin. Its shape is the one invariant 33 uses for the
first table:

```rust
const _: () = {
    use cratonvm_jit::direct_helpers::*;
    let _: DirectHelperFnIntegerValueOf = jit_integer_value_of_direct;
    // … one line per slot …
    let _: DirectHelperFnVarHandleCas = jit_varhandle_cas_direct::<0>;
};
```

### Procedure for adding a direct helper slot

1. Append the `pub <name>: usize,` field to `DirectHelperTable` and to `EMPTY`.
2. Append a `direct_helper_fn_slots!` row with the real C signature **and** the
   `DirectHelperCallPath` the emitter will use. If an argument or return type is
   not yet classified, add the `HelperArgAbi` / `HelperRetAbi` impl in
   `jit-api/src/helpers_abi.rs` — deliberately fail-closed, so you cannot skip
   this.
3. Update the two literals in the census `const _` (117 words, 31 rows). The
   assertion's message names this obligation; it is the tripwire for step 2.
4. If the new slot exceeds four arguments, update
   `DIRECT_HELPERS_NEEDING_WIN64_STACK_ARGS` (and `..._SYSV_...` above six) —
   and make sure its path really is `JavaArgMarshalled`, because
   `every_slot_that_overflows_the_register_file_is_marshalled` will say so.
5. Wire it in `direct_helper_table()` and add its line to the producer-side
   coercion block.
6. At a hand-written call site, add
   `assert_direct_helper_call_shape!("<name>", int_args = N, returns_value = …)`.
