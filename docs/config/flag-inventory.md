...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,360 |
| exact string literals (i.e. actually named by code, not prose) | 1,285 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1279** |
...
1290 rows: 1279 declared, 11 allowlisted. Generated by
...
| `CRATONVM_JIT_IR_REG_AUTHORITATIVE` | JIT | `CRATONVM_JIT=ir-reg-authoritative` | opt-in | off | behaviour | snapshot | jit |
