...
The table in [Full inventory](#full-inventory) is generated, not maintained. Its
three inputs are all in the tree:
...
| `CRATONVM_DBG` | DBG | `CRATONVM_DBG=…` | group | unset | — | snapshot | types |
...
| `CRATONVM_DBG_COMPACT_INLINE` | DBG | `CRATONVM_DBG=compact-inline` | opt-in | off | diag | snapshot | jit |
...
| `CRATONVM_DBG_JIT_CODE` | DBG | `CRATONVM_DBG=jit-code` | opt-in | off | diag | snapshot | jit |
...
| `CRATONVM_DBG_JIT_METHOD_STATS` | DBG | `CRATONVM_DBG=jit-method-stats` | opt-in | off | diag | snapshot | types |
...
| `CRATONVM_DBG_OVERLAY` | DBG | `CRATONVM_DBG=overlay` | opt-in | off | diag | snapshot | vm |
| `CRATONVM_DBG_OVERLAY_ALL` | DBG | `CRATONVM_DBG=overlay-all` | opt-in | off | diag | snapshot | vm |
...
| `CRATONVM_DISABLE_INTRINSICS` | JIT | `CRATONVM_JIT=intrinsics` | opt-out | on | behaviour | snapshot | difftest, vm |
| `CRATONVM_DISABLE_JAR_MMAP` | LOADER | `CRATONVM_LOADER=jar-mmap` | opt-out | on | behaviour | snapshot | types |
| `CRATONVM_DISABLE_JIT` | — | `CRATONVM_DISABLE_JIT` | scalar | unset | behaviour | snapshot | difftest, types, vm-cli |
...
| `CRATONVM_ENABLE_ASSERTIONS` | — | `CRATONVM_ENABLE_ASSERTIONS` | scalar | unset | behaviour | snapshot | types |
...
| `CRATONVM_G1_PARALLEL_EVAC` | GC | `CRATONVM_GC=g1-parallel-evac` | opt-in | off | behaviour | snapshot | types |
...
| `CRATONVM_JIT_ALLOW_PACKAGES` | JIT | `CRATONVM_JIT=allow-packages` | opt-in | off | behaviour | snapshot | jit, vm |
| `CRATONVM_JIT_BISECT_ONLY` | DBG | `CRATONVM_DBG=jit-bisect-only` | opt-in | off | diag | snapshot | vm |
| `CRATONVM_JIT_BISECT_SKIP` | DBG | `CRATONVM_DBG=jit-bisect-skip` | opt-in | off | diag | snapshot | vm |
...
| `CRATONVM_JIT_UNBAN_JUNITCORE` | JIT | `CRATONVM_JIT=unban-junitcore` | opt-in | off | behaviour | snapshot | — |
...
| `CRATONVM_SYNTHETIC_BUFFERED_WRITER` | REAL | `CRATONVM_REAL=buffered-writer` | opt-out | on | behaviour | snapshot | types |
...
| `CRATONVM_SYNTHETIC_NET_SOCKETS` | REAL | `CRATONVM_REAL=net-sockets` | both | off | behaviour | snapshot | types |
