...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,496 |
| exact string literals (i.e. actually named by code, not prose) | 1,419 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1411** |
...
1422 rows: 1411 declared, 11 allowlisted. Generated by
...
| `CRATONVM_JIT_IR_RANGE_EDGES` | JIT | `CRATONVM_JIT=ir-range-edges` | opt-in | off | behaviour | snapshot | jit |
| `CRATONVM_JIT_IR_RECEIVER_GUARD_CSE` | JIT | `CRATONVM_JIT=ir-receiver-guard-cse` | default-on | on | behaviour | snapshot | jit |
| `CRATONVM_JIT_IR_RECURSIVE_INLINE` | JIT | `CRATONVM_JIT=ir-recursive-inline` | opt-in | off | behaviour | snapshot | jit |
...
| `CRATONVM_ZGC_LAZY_TLAB_ZERO` | GC | `CRATONVM_GC=zgc-lazy-tlab-zero` | opt-in | off | behaviour | snapshot | gc |
