...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,135 |
| exact string literals (i.e. actually named by code, not prose) | 1,068 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1062** |
...
1073 rows: 1062 declared, 11 allowlisted. Generated by
...
| `CRATONVM_ZGC_MUTATOR_TLAB` | GC | `CRATONVM_GC=zgc-mutator-tlab` | default-on | on | behaviour | snapshot | gc |
