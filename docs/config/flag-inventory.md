...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,133 |
| exact string literals (i.e. actually named by code, not prose) | 1,066 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1060** |
...
1071 rows: 1060 declared, 11 allowlisted. Generated by
...
| `CRATONVM_GPU_BLOCK_REDUCE` | GC | `CRATONVM_GC=gpu-block-reduce` | default-on | on | behaviour | snapshot | jit-cuda |
