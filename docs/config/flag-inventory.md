...
|---|---|
<<<<<<< HEAD
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,168 |
| exact string literals (i.e. actually named by code, not prose) | 1,100 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1094** |
=======
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,177 |
| exact string literals (i.e. actually named by code, not prose) | 1,108 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1102** |
>>>>>>> origin/dev
...
<<<<<<< HEAD
1105 rows: 1094 declared, 11 allowlisted. Generated by
=======
1113 rows: 1102 declared, 11 allowlisted. Generated by
>>>>>>> origin/dev
...
| `CRATONVM_JIT_IR_GATED_REF_STORE` | JIT | `CRATONVM_JIT=ir-gated-ref-store` | default-on | on | behaviour | snapshot | — |
...
| `CRATONVM_JIT_NO_BOX_UNBOX_INTRINSIC` | JIT | `CRATONVM_JIT=box-unbox-intrinsic` | both | off | behaviour | snapshot | jit |
