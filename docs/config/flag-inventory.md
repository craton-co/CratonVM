...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,269 |
| exact string literals (i.e. actually named by code, not prose) | 1,195 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1189** |
...
1200 rows: 1189 declared, 11 allowlisted. Generated by
...
| `CRATONVM_FORCE_CONSERVATIVE_LOCALS` | GC | `CRATONVM_GC=force-conservative-locals` | opt-in | off | behaviour | snapshot | — |
