...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,287 |
| exact string literals (i.e. actually named by code, not prose) | 1,212 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1206** |
...
1217 rows: 1206 declared, 11 allowlisted. Generated by
...
| `CRATONVM_G1_ACCEPT_IMPLAUSIBLE_SLOT` | GC | `CRATONVM_GC=g1-accept-implausible-slot` | opt-in | off | behaviour | snapshot | types |
