...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,386 |
| exact string literals (i.e. actually named by code, not prose) | 1,311 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1305** |
...
1316 rows: 1305 declared, 11 allowlisted. Generated by
...
| `CRATONVM_DBG_DEADREF_STORE` | DBG | `CRATONVM_DBG=deadref-store` | opt-in | off | diag | snapshot | types |
...
| `CRATONVM_JIT_IR_CARRY_2ND` | JIT | `CRATONVM_JIT=ir-carry-2nd` | opt-in | off | behaviour | snapshot | jit |
| `CRATONVM_JIT_IR_CARRY_RCX_FOLDED` | JIT | `CRATONVM_JIT=ir-carry-rcx-folded` | opt-in | off | behaviour | snapshot | jit |
...
| `CRATONVM_JIT_IR_PAIR_OPERANDS` | JIT | `CRATONVM_JIT=ir-pair-operands` | opt-in | off | behaviour | snapshot | jit |
