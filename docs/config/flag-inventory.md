...
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **904** |
...
915 rows: 904 declared, 11 allowlisted. Generated by
...
| `CRATONVM_DBG_JIT_STALE_AFTER_REMAP` | DBG | `CRATONVM_DBG=jit-stale-after-remap` | opt-in | off | diag | snapshot | vm |
...
| `CRATONVM_JIT_SPECIAL_BYTECODE_CALLEE` | JIT | `CRATONVM_JIT=special-bytecode-callee` | default-on | on | behaviour | snapshot | vm |
