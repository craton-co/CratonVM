...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,362 |
| exact string literals (i.e. actually named by code, not prose) | 1,287 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1281** |
...
1292 rows: 1281 declared, 11 allowlisted. Generated by
...
| `CRATONVM_JIT_IR_SPLICE_TYPECHECK` | JIT | `CRATONVM_JIT=ir-splice-typecheck` | opt-in | off | behaviour | snapshot | jit |
