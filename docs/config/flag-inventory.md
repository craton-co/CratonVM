...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,116 |
| exact string literals (i.e. actually named by code, not prose) | 1,049 |
...
| `CRATONVM_GC_PAR_EVAC` | GC | `CRATONVM_GC=par-evac` | default-on | on | behaviour | snapshot | types |
...
| `CRATONVM_GC_SYNC_YOUNG_WIPE` | GC | `CRATONVM_GC=sync-young-wipe` | opt-in | off | behaviour | snapshot | types |
