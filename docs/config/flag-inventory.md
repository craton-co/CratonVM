...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,334 |
| exact string literals (i.e. actually named by code, not prose) | 1,259 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1253** |
...
1264 rows: 1253 declared, 11 allowlisted. Generated by
...
| `CRATONVM_GC_HOLD_HELPER_PEERS` | GC | `CRATONVM_GC=hold-helper-peers` | opt-in | off | behaviour | snapshot | vm |
...
| `CRATONVM_GC_REMAP_FROZEN_PEER_STACKS` | GC | `CRATONVM_GC=remap-frozen-peer-stacks` | opt-in | off | behaviour | snapshot | vm |
