...
| | count |
|---|---|
| distinct `CRATONVM_*` identifiers appearing anywhere in Rust source | 1,323 |
| exact string literals (i.e. actually named by code, not prose) | 1,248 |
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **1242** |
...
1253 rows: 1242 declared, 11 allowlisted. Generated by
...
| `CRATONVM_DBG_PEER_REG_PAIRING` | DBG | `CRATONVM_DBG=peer-reg-pairing` | opt-in | off | diag | snapshot | vm |
