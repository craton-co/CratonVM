...
| **declared** in `flag_groups::INVENTORY` + scalars + group variables | **759** |
...
770 rows: 759 declared, 11 allowlisted. Generated by
...
| `CRATONVM_DBG_ANN_PROXY_PROF` | DBG | `CRATONVM_DBG=ann-proxy-prof` | opt-in | off | diag | snapshot | vm |
...
| `CRATONVM_ZGC_STARTBITS` | GC | `CRATONVM_GC=zgc-startbits` | opt-in | off | behaviour | snapshot | gc |
| `CRATONVM_ZGC_TLAB` | GC | `CRATONVM_GC=zgc-tlab` | opt-in | off | behaviour | snapshot | gc |
