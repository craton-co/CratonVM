...
///
/// Lowered 220 -> 200 -> 150 on 2026-09-20: a coordinated retirement sweep
/// landed the same day across many concurrent lanes (F1, F5, F6, F9, F10,
/// F12, F18, F19, F21, F34, F35, F37, E31, E36, E39, E40 among them), each
/// moving its page out of `docs/known-issues/**` with its own citation and
/// INDEX updates -- the walk is measurably NOT broken (`find docs/known-issues
/// -name '*.md' | wc -l` read 197 on this tree, matching this test's own
/// count), the tree is legitimately shrinking. 150 keeps the floor's actual
/// job -- catching a walk that reads a handful of pages or zero -- without
/// this test blocking the same kind of legitimate retirement tomorrow. If
/// this fires again, re-run that same command before assuming the walk broke.
///
/// Two lanes reached 150 independently on the same day, which is the other
/// half of the reading above: this floor was set flush against the count, and
/// a floor flush against a number that retirement is DESIGNED to lower goes
/// red on whichever lane retires next rather than on whoever set it.
///
/// The failure the floor exists to catch is a MISTYPED PATH, and that yields
/// zero, not 197. [`the_known_issues_root_exists`] now asks that question
/// structurally instead, so this number only has to sit far enough below the
/// real population to stay quiet through ordinary retirement. Do not re-freeze
/// it against the current count.
///
/// # Lowered 150 -> 40 on 2026-09-21, and the paragraph above called it
///
/// 150 went red the next day, on exactly the terms it predicted. The sweep did
/// not stop on 2026-09-20: measured on 2026-09-21, `origin/dev` holds **115**
/// pages under `docs/known-issues/**` and was at **128** twelve commits
/// earlier, so the floor had been red on `dev` for a day before the lane that
/// found it (lane 4's retirement, itself worth 3 of the shortfall's 35)
/// arrived. `find docs/known-issues -name '*.md' | wc -l` reads 115, matching
/// this test's own count: the walk is not broken, the tree is a third smaller
/// than when 150 was chosen.
///
/// 40 is deliberately NOT flush. The population would have to lose another
/// two thirds before this fires again, which is several more months of the
/// same sweep, while a broken walk still yields zero or a handful and is still
/// caught. Setting it flush a third time would just hand the same red to
/// whichever lane retires next -- which is the mistake the paragraph above
/// names and the reason this one exists.
const MIN_PAGES_SCANNED: usize = 40;
...
/// The structural half of [`MIN_PAGES_SCANNED`]: a mistyped path, a moved
/// directory or a walk that cannot read the tree all produce the same thing —
/// an empty scan — and none of them is distinguishable from a clean tree by a
/// count alone. This asserts the root is there and has pages directly under
/// the jdk-only lane, which is the shape the walk assumes. It is deliberately
/// not a count: a count is what the floor above already is, and one of the two
/// has to be free to move when pages retire.
#[test]
fn the_known_issues_root_exists() {
    let root = workspace_root();
    let known = root.join("docs").join("known-issues");
    assert!(
        known.is_dir(),
        "{} is not a directory — the walk in this file is reading nothing, and          every assertion built on it is vacuous",
        known.display()
    );
    let lane = known.join("jdk-only");
    assert!(
        lane.is_dir(),
        "{} is not a directory — the jdk-only lane is where this gate's          population lives",
        lane.display()
    );
    let mut found = Vec::new();
    pages(&lane, &root, &mut found);
    assert!(
        !found.is_empty(),
        "the jdk-only lane holds no readable .md page; the walk is broken, not          the tree clean"
    );
}
