...
const ALLOWED: &[(&str, &str)] = &[
    (
        "CRATONVM_TEST_NPE_OPTIMIZING_BODY_CHILD",
        "kind 3: a TEST HARNESS switch, set on its own re-exec child by          `vm/tests/jit_npe_message_from_an_optimizing_body.rs` and read there          with a raw `std::env::var_os` to decide whether this process IS that          child (line 237) rather than the parent spawning it (line 274). It          never reaches a running VM under test — the child process it marks          runs the JIT body directly instead of spawning again — so there is no          `VmFlags` snapshot to serve it and no `CRATONVM_<GROUP>=token` spelling          an operator could set that would mean anything.",
    ),
    (
        "CRATONVM_ZGC_MARKEND_LOG",
        "kind 3: a TEST HARNESS input, read by `gc/tests/zgc_r_conc_window_fit.rs` to point the concurrent-window analysis at a `--verbose:gc` log captured from a previous run. It names a FILE PATH, not a VM behaviour: unset, the test is an ordinary unit test of its own arithmetic and its parser; set, it parses a log this process did not produce. It never reaches a running VM, so there is no `VmFlags` snapshot to serve it and no `CRATONVM_<GROUP>=token` spelling an operator could set that would mean anything.",
    ),
    (
