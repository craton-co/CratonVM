...
/// loops of all four collectors (`gc/src/{gc,g1,gen_heap,zgc,concurrent_mark}.rs`;
/// since round 9 wave 4 it is also the per-field-access lookup behind the
/// JIT's compact field helpers, `jit_compact_field_slot`), and
...
pub fn with_class_layout<R>(
    ...
    let generation = layout_generation();
    // Ids the dense table declines to index (`SCAN_CACHE_MAX` and up) go to
    // the MRU arm, which caches them: on the dense arm every such lookup was an
    // uncached registry probe (a reader lock plus an `Arc` clone/drop), and
    // since round 9 wave 4 this function is also the per-field-access lookup
    // behind the JIT's compact field helpers. Same registry, same answer.
    if scan_cache_enabled() && (class_id as usize) < SCAN_CACHE_MAX {
...
/// [`with_class_layout`] for MUTATOR per-access paths: the same borrow, served
/// from the small fixed-size `VERSION_CACHE` that [`class_layout_for_fields`]
/// uses, whatever `CRATONVM_GC_LAYOUT_SCAN_CACHE` says.
///
/// # Why a separate entry point
///
/// [`with_class_layout`]'s default arm is the collectors' dense `ScanCache`,
/// a per-thread table indexed by `class_id` that grows to the highest id the
/// thread has asked about (up to `SCAN_CACHE_MAX` entries, 16 bytes each). That
/// is the right trade for a few GC workers scanning every class in the heap and
/// the wrong one for every mutator thread of a server with hundreds of them.
/// A field access touches a handful of classes in a loop, which is what the
/// 8-entry MRU array is sized for.
///
/// Callers that resolve one field per access — every heap's compact
/// `get_field`/`set_field` arm through [`compact_object_field_storage`], and
/// the JIT's compact field helpers — should use this instead of
/// [`class_layout_for_fields`], whose owned `Arc` is a clone/drop pair of
/// atomic RMWs on the class's one shared layout per access: the multi-thread
/// collapse in
/// `docs/internal/fixed-bugs/perf-compact-field-helpers-contend-on-the-shared-layout-arc-FIXED-20260918.md`.
/// Round 9 wave 4, lane `gc4`.
#[inline]
pub fn with_class_layout_versioned<R>(
    class_id: u32,
    field_count: u32,
    f: impl FnOnce(&CompactLayout) -> R,
) -> Option<R> {
    with_class_layout_mru(class_id, field_count, layout_generation(), f)
}
...
pub fn compact_tlab_body_size(class_id: u32, field_count: usize) -> Option<usize> {
    ...
    // Per ALLOCATION: borrow the current recipe from this thread's
    // generation-validated cache instead of `class_layout`, which took the
    // registry `RwLock` and cloned/dropped the shared `Arc` for every object
    // (round 9 wave 4, `gc4`). Same answer: both read `CLASS_LAYOUTS[class_id]`,
    // and `compact_object_body_size` below already resolves this way.
    with_current_class_layout(class_id, |l| {
        (l.field_count() == field_count).then_some(l.body_size as usize)
    })
    .flatten()
}
...
pub fn compact_object_field_storage(
    ...
    // BORROW, never clone (round 9 wave 4, `gc4`). This is every heap's
    // `get_field`/`set_field` compact arm (gen_heap, g1, the interpreter's
    // compact paths), i.e. once per field access. `class_layout_for_fields`
    // returned an owned `Arc`, so every access paid a clone/drop pair of atomic
    // RMWs on the class's ONE shared layout allocation, and eight threads
    // storing into objects of the same class turned that refcount into a
    // coherence storm (`PutScale 8`: 50-90x its single-thread time; see
    // `docs/internal/fixed-bugs/perf-compact-field-helpers-contend-on-the-shared-layout-arc-FIXED-20260918.md`).
    //
    // `with_class_layout_versioned` borrows from the SAME 8-entry
    // `VERSION_CACHE` that `class_layout_for_fields` used, so the hit rate, the
    // per-thread footprint and the answer are exactly what they were; only the
    // refcount traffic is gone. See that function for why it is not
    // `with_class_layout` (the collectors' dense table).
    with_class_layout_versioned(header.class_id.as_u32(), header.num_slots(), |layout| {
        Some((
            layout.field_offset(index)? as usize,
            layout.field_storage(index)?,
        ))
...
mod tests {
    ...
    /// Round 9 wave 4 (`gc4`): the per-field-access lookup
    /// ([`compact_object_field_storage`]) and the TLAB sizing lookup
    /// ([`compact_tlab_body_size`]) moved from OWNING forms
    /// (`class_layout_for_fields` / `class_layout`, an `Arc` clone/drop per
    /// call) to BORROWING ones. They must answer exactly what the owning forms
    /// answer: a hit, every index in and out of range, a field-count mismatch,
    /// a legacy header, a redefine and an unregister.
    #[test]
    fn borrowing_access_lookups_agree_with_the_owning_ones() {
        let _guard = REGISTRY_TEST_LOCK.lock().unwrap();
        clear_class_layouts();
        let two = |body_size: u32| {
            Arc::new(CompactLayout {
                field_offsets: vec![0, 8],
                is_ref: vec![true, false],
                field_kinds: vec![FieldStorageKind::Reference, FieldStorageKind::Long],
                ref_offsets: vec![0],
                body_size,
            })
        };
        const CID: u32 = 93;
        register_class_layout(FIRST_LAYOUT_DOMAIN, CID, two(16));
        let header = |fields: u32, compact: bool| {
            ...
            if compact {
                h.add_gc_flags(GC_FLAG_COMPACT);
            }
            h
        };
        // The pre-wave-4 body of `compact_object_field_storage`, verbatim.
        let owning = |h: &ObjectHeader, index: usize| -> Option<(usize, FieldStorageKind)> {
            if !is_compact_object(h) {
                return None;
            }
            let layout = class_layout_for_fields(h.class_id.as_u32(), h.num_slots())?;
            Some((
                layout.field_offset(index)? as usize,
                layout.field_storage(index)?,
            ))
        };
        // The pre-wave-4 body of `compact_tlab_body_size`, minus its gates.
        let owning_tlab = |fc: usize| {
            class_layout(CID)
                .filter(|l| l.field_count() == fc)
                .map(|l| l.body_size as usize)
        };
        let tlab_gated = compact_ref_fields_enabled() && single_layout_domain();
        let check = |what: &str| {
            for (fields, compact) in [(2u32, true), (1, true), (3, true), (2, false)] {
                let h = header(fields, compact);
                for index in 0..4 {
                    // Twice: the second call is a cache hit.
                    for _ in 0..2 {
                        assert_eq!(
                            compact_object_field_storage(&h, index),
                            owning(&h, index),
                            "{what}: fields {fields}, compact {compact}, index {index}"
                        );
                    }
                }
                assert_eq!(
                    with_class_layout_versioned(CID, fields, |l| l.body_size),
                    class_layout_for_fields(CID, fields).map(|l| l.body_size),
                    "{what}: versioned borrow, fields {fields}"
                );
                if tlab_gated {
                    assert_eq!(
                        compact_tlab_body_size(CID, fields as usize),
                        owning_tlab(fields as usize),
                        "{what}: tlab size, fields {fields}"
                    );
                }
            }
        };
        check("registered");
        assert_eq!(
            compact_object_field_storage(&header(2, true), 1),
            Some((8, FieldStorageKind::Long))
        );
        register_class_layout(FIRST_LAYOUT_DOMAIN, CID, two(24));
        check("redefined");
        if tlab_gated {
            assert_eq!(
                compact_tlab_body_size(CID, 2),
                Some(24),
                "a redefine must not serve the old size"
            );
        }
        unregister_class_layout(CID);
        check("unregistered");
        assert_eq!(compact_object_field_storage(&header(2, true), 0), None);
        clear_class_layouts();
    }
