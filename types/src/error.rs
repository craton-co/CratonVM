...
const REMEDIATION_FALLBACK: &str =
    "re-run with --compatible to restore the pre-2026-09-20 compatibility behaviour";
...
pub enum JdkOnlyViolation {
    ...
    CompatibilityClassRequested {
        ...
        reason: String,
        /// [`ClassOrigin::as_str`]'s tag for the class this row names, and the
        /// last field §1.7 asks for (see [`Self::class_origin`]).
        ///
        /// On THIS variant it is a constant by construction — the refusal fires
        /// while a `CompatibilityStub` is being minted, so the origin is
        /// `"compatibility-stub"` and `reason` carries the part that varies.
        /// It is still recorded rather than left implicit, because a reader of
        /// the JSON should not have to know which variants imply which origin
        /// to read the column.
    ...
    NativeShadowsBytecode {
        ...
        /// Where the class whose bytecode this native stands in front of came
        /// from — see [`Self::class_origin`].
        ///
        /// This is the variant the field was added FOR. "A bridge shadows
        /// `java/io/File.isDirectory`" reads very differently depending on
        /// whether those bytes are the boot image's or an application jar's,
        /// and the row could not say which.
        ///
        /// `None` on the producers that hold only a name triple — the JIT's
        /// thin-helper refusal and the wave-1 dispatch adapter both say so in
        /// their own comments. [`Self::with_class_origin`] is how a consumer
        /// that DOES hold the class store fills it in afterwards.
        class_origin: Option<String>,
    },
...
impl JdkOnlyViolation {
    ...
    /// Provenance of the class this row names: `ClassOrigin::as_str()`'s tag
    /// (`boot-image`, `application-class-path`, `generated-proxy`,
    /// `compatibility-stub`, …), optionally followed by ` from <source>`.
    ///
    /// # Why only two variants have it
    ///
    /// `docs/feature-designs/jdk-only-mode.md` §1.7 lists class origin among
    /// the fields an actionable refusal carries, and until 2026-09-20 it was
    /// the one field NEITHER surface had —
    /// `jdk-only/the-six-acceptance-criteria-measured-20260910-RETIRED-20260921.md`
    /// §5 measured exactly that and called it "the one unambiguous gap".
    ///
    /// It is deliberately NOT universal. Six of the construction sites build
    /// [`Self::SyntheticNativeRegistered`], which fires at REGISTRATION —
    /// before any class is loaded, so there is no origin in existence to
    /// report. Adding `class_origin: null` to those rows would make the
    /// criterion read as met while telling a reader nothing, which is the
    /// failure that record names in advance. The field exists on the two
    /// CLASS-FACING variants and nowhere else, and [`Self::render`] prints the
    /// line only for them.
    pub fn class_origin(&self) -> Option<&str> {
        match self {
            Self::CompatibilityClassRequested { class_origin, .. }
            | Self::NativeShadowsBytecode { class_origin, .. } => class_origin.as_deref(),
            _ => None,
        }
    }

    /// Does this variant HAVE a class origin to report, whether or not one is
    /// known?
    ///
    /// The distinction [`Self::class_origin`] alone cannot make, and the one
    /// that keeps the rendered report honest: on a class-facing variant a
    /// missing origin is a gap in what the producer could see and is printed as
    /// `<unknown>`; on a registration-time variant there is nothing to know and
    /// the line is omitted entirely. A single `Option` would spell both `None`.
    pub fn has_class_origin_field(&self) -> bool {
        matches!(
            self,
            Self::CompatibilityClassRequested { .. } | Self::NativeShadowsBytecode { .. }
        )
    }

    /// Attach a class origin discovered after the fact, and return the row.
    ///
    /// The producers of [`Self::NativeShadowsBytecode`] are dispatch-path
    /// helpers that hold a name triple and nothing else — looking a class up
    /// from there would mean a class-manager read lock on the hottest path in
    /// strict mode, which is the exact cost those adapters exist to avoid. The
    /// consumers (`--jdk-only-report`, `--trace-jdk-only`) run once, cold, and
    /// hold the class store, so the lookup happens there.
    ///
    /// A no-op on every variant without the field, so a caller may apply it to
    /// a whole `violations[]` without matching first. Never overwrites an
    /// origin a producer already knew.
    #[must_use]
    pub fn with_class_origin(mut self, origin: Option<String>) -> Self {
        match &mut self {
            Self::CompatibilityClassRequested { class_origin, .. }
            | Self::NativeShadowsBytecode { class_origin, .. } => {
                if class_origin.is_none() {
                    *class_origin = origin;
                }
            }
            _ => {}
    ...
    /// fixed** — the `--compatible` fallback, then the `--jdk-only-report`
    /// capture hint.
    ///
    ...
    pub fn render(&self, jdk_feature: Option<u32>, verbose: bool) -> String {
        ...
        // §1.7's class-origin field. Printed only on the variants that HAVE
        // one (see `has_class_origin_field`), and as `<unknown>` rather than
        // omitted when such a variant did not learn it — an omitted line and an
        // unanswerable question must not look the same, which is the whole
        // reason the two are different predicates.
        if self.has_class_origin_field() {
            out.push_str(&format!(
                "  class origin:      {}\n",
                match self.class_origin() {
                    Some(origin) => redact_paths(origin, verbose),
    ...
    pub fn to_json(&self) -> String {
        ...
        match self {
            JdkOnlyViolation::CompatibilityClassRequested {
                ...
                // Emitted on the two class-facing variants and on no others, so
                // the column's ABSENCE from a `synthetic-native-registered` row
                // is itself the statement that the question does not apply
                // there. A `null` on every row would say the opposite.
                json_field(
                    &mut out,
                    &mut first,
                    "class_origin",
                    class_origin.as_deref(),
                );
            }
            JdkOnlyViolation::SyntheticNativeRegistered {
                class,
            ...
            JdkOnlyViolation::NativeShadowsBytecode {
                ...
                // See the same field on `CompatibilityClassRequested`. `null`
                // here is meaningful and different from absent: it means this
                // row's producer held a name triple only, and no consumer
                // resolved the class afterwards.
                json_field(
                    &mut out,
                    &mut first,
                    "class_origin",
...
mod tests {
    ...
    fn all_variants() -> Vec<JdkOnlyViolation> {
        vec![
            JdkOnlyViolation::CompatibilityClassRequested {
                ...
                class_origin: Some("compatibility-stub".into()),
            },
            JdkOnlyViolation::SyntheticNativeRegistered {
            ...
            JdkOnlyViolation::NativeShadowsBytecode {
                ...
                class_origin: Some("boot-image".into()),
            },
            JdkOnlyViolation::MissingBootClass {
    ...
    /// §1.7's class-origin field is on the two CLASS-FACING variants and on
    /// NO others — which is the whole content of the criterion, not an
    /// implementation detail.
    ///
    /// `jdk-only/the-six-acceptance-criteria-measured-20260910-RETIRED-20260921.md`
    /// §5 measured class origin as the one field neither surface carried, and
    /// said in the same breath why adding it everywhere would be worse than
    /// leaving it out: six construction sites build
    /// `SyntheticNativeRegistered`, which fires at REGISTRATION, before any
    /// class is loaded — "a change that adds `origin: null` to the latter to
    /// tick the box would make the criterion read as met without telling
    /// anyone anything."
    ///
    /// So the gate is two-sided on purpose. Adding the field to a
    /// registration-time variant fails here just as loudly as removing it from
    /// a class-facing one.
    #[test]
    fn class_origin_is_on_the_class_facing_variants_and_no_others() {
        let expected_with: std::collections::BTreeSet<&str> =
            ["compatibility-class-requested", "native-shadows-bytecode"]
                .into_iter()
                .collect();
        let actual_with: std::collections::BTreeSet<&str> = all_variants()
            .iter()
            .filter(|v| v.has_class_origin_field())
            .map(|v| v.kind())
            .collect();
        assert_eq!(
            actual_with, expected_with,
            "the set of variants carrying a class origin moved. Adding one is \
             only right if the variant NAMES A LOADED CLASS at the moment it \
             fires; a registration-time refusal does not, and `origin: null` on \
             it ticks the criterion without answering it."
        );

        for v in all_variants() {
            let rendered = v.render(Some(25), true);
            let json = v.to_json();
            if v.has_class_origin_field() {
                assert!(
                    rendered.contains("class origin:"),
                    "{} renders no class-origin line:\n{rendered}",
                    v.kind()
                );
                assert!(
                    json.contains("\"class_origin\""),
                    "{} has no class_origin JSON field: {json}",
                    v.kind()
                );
            } else {
                assert!(
                    !rendered.contains("class origin:"),
                    "{} renders a class-origin line it cannot answer:\n{rendered}",
                    v.kind()
                );
                assert!(
                    !json.contains("\"class_origin\""),
                    "{} emits a vacuous class_origin JSON field: {json}",
                    v.kind()
                );
            }
        }
    ...
    /// An applicable variant whose producer did not learn the origin renders
    /// `<unknown>` rather than dropping the line.
    ///
    /// The distinction the criterion turns on. A dropped line is
    /// indistinguishable from a variant that has no origin at all, so a reader
    /// cannot tell "nobody looked" from "there is nothing to look at" — and the
    /// three dispatch-path producers of `NativeShadowsBytecode` all start out
    /// in the first state, by design, because resolving a class from there
    /// costs a store read lock on the hottest path in strict mode.
    #[test]
    fn an_unknown_class_origin_is_printed_not_omitted() {
        let v = JdkOnlyViolation::NativeShadowsBytecode {
            class: "java/util/HashMap".into(),
            method: "put".into(),
            descriptor: "(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;".into(),
            native_kind: "jit-thin-direct-helper",
        ...
        let rendered = v.render(Some(25), true);
        assert!(
            rendered.contains("class origin:") && rendered.contains(UNKNOWN),
            "an unlearned origin must render as {UNKNOWN}, not vanish:\n{rendered}"
        );
        assert!(
            v.to_json().contains("\"class_origin\":null"),
            "and must be an explicit null on the wire: {}",
            v.to_json()
        );
    }

    /// `with_class_origin` fills a gap and never overwrites an answer.
    ///
    /// The consumers apply it blindly across a whole `violations[]`, so it has
    /// to be a no-op on the variants without the field and on rows whose
    /// producer already knew — otherwise a report writer holding a coarse
    /// class-name lookup could overwrite the precise origin a producer
    /// recorded at the moment of the refusal.
    #[test]
    fn with_class_origin_fills_only_a_gap() {
        let known = JdkOnlyViolation::NativeShadowsBytecode {
            class: "java/util/HashMap".into(),
            method: "put".into(),
            descriptor: "()V".into(),
            native_kind: "bridge",
            class_origin: Some("application-class-path".into()),
        };
        assert_eq!(
            known
                .clone()
                .with_class_origin(Some("boot-image".into()))
                .class_origin(),
            Some("application-class-path"),
            "an origin the producer knew must not be overwritten"
        );

        let unknown = JdkOnlyViolation::NativeShadowsBytecode {
            class: "java/util/HashMap".into(),
            method: "put".into(),
            descriptor: "()V".into(),
            native_kind: "bridge",
        ...
        // And a variant without the field absorbs it silently.
        let registration = JdkOnlyViolation::SyntheticNativeRegistered {
            class: "com/example/Strict".into(),
            method: "fake".into(),
            descriptor: "()V".into(),
            registered_by: None,
        ...
        let after = registration.with_class_origin(Some("boot-image".into()));
        assert_eq!(after.class_origin(), None);
        assert!(!after.to_json().contains("class_origin"));
    }
