...
pub use field_layout::{
    ...
    unregister_class_layout, with_class_layout, with_class_layout_versioned, write_compact_field,
    CompactLayout, FieldStorageKind, FIRST_LAYOUT_DOMAIN,
};
pub use flags::{
...
/// The duplicate-class-name latch, as a BYTE compiled code can test in place
/// (`cmp byte [addr], 0`): `0` while every loaded NON-ARRAY binary name maps
/// to one `ClassId` and no class has been redefined under a fresh id; `1`
/// forever after either has happened anywhere in the process.
///
/// Raised by `classloading::class_manager` — by `note_duplicate_class_name`
/// (the mirror of its `ANY_DUPLICATE_CLASS_NAME`, minus array class names,
/// which no plain object's class chain can carry) and by the `allow_redefine`
/// define path when it mints a new id for a name whose old id's instances stay
/// alive. Both raise it on the defining thread BEFORE the new class is
/// published, so before any instance of it can exist. Never lowered.
///
/// The reader is the JIT's inline `instanceof` miss for a `final` target
/// (`jit/src/x64/op_object.rs`, `CRATONVM_JIT_INSTANCEOF_FINAL_MISS`): the
/// runtime helper's only way to admit a receiver of a different class id than
/// a `final` target's is its by-NAME loader-duplication fallback, which needs
/// exactly the situation this latch records. A raised latch sends every such
/// miss back to the helper, which is the pre-optimisation behaviour.
///
/// Process-wide rather than per-VM, deliberately: compiled code tests a fixed
/// absolute address with no VM pointer chase, the latch only ever makes the
/// JIT MORE conservative (a duplicate in one VM costs the other VMs a helper
/// call, never a wrong answer), and the state it mirrors,
/// `ANY_DUPLICATE_CLASS_NAME`, is already process-wide. It is a monotone
/// performance gate, not compatibility state.
static DUPLICATE_CLASS_NAME_SEEN: std::sync::atomic::AtomicU8 = std::sync::atomic::AtomicU8::new(0);

/// Address of the duplicate-class-name latch byte, for an inline
/// `cmp byte [addr], 0`. See [`duplicate_class_name_seen`].
#[inline]
pub fn duplicate_class_name_seen_addr() -> usize {
    // Cast: address of a process-lifetime static
    std::ptr::addr_of!(DUPLICATE_CLASS_NAME_SEEN) as usize
}

/// Whether the duplicate-class-name latch is raised. See
/// [`duplicate_class_name_seen_addr`] for what raises it and who reads it.
#[inline]
pub fn duplicate_class_name_seen() -> bool {
    DUPLICATE_CLASS_NAME_SEEN.load(std::sync::atomic::Ordering::Acquire) != 0
}

/// Raise the duplicate-class-name latch. Idempotent; never undone. `Release`
/// so that the class publication which follows on the defining thread cannot
/// be reordered before it.
#[inline]
pub fn raise_duplicate_class_name_seen() {
    DUPLICATE_CLASS_NAME_SEEN.store(1, std::sync::atomic::Ordering::Release);
}
...
pub mod gc_entry_census {
    ...
    /// ZGC `refill_tlab` hands out chunks only while the VM TLAB is on
    /// (`CRATONVM_ZGC_JIT_TLAB`; default is `ZGC_VM_TLAB_DEFAULT_ON` in
    /// `gc/src/zgc/vm_tlab.rs`), so with it off the expectation is zero
    /// successes and the interesting number is how many bytes flow past it.
    #[inline]
    ...
    ///
    /// Counted apart from the first attempt because, while the ZGC VM TLAB
    /// is off (`CRATONVM_ZGC_JIT_TLAB`), it is the only refill that can
    /// succeed on this backend: the first one is asking a feature that is
    /// off, while the retry runs after a coalescing collection. If it
    /// succeeds, the TLAB it seeds serves
