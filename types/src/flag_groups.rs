...
pub const INVENTORY: &[E] = &[
    ...
    // `ClassLoader.getResource` stopping at the first matching classpath
    // entry, default-ON. `0` restores the whole-list walk that built every
    // matching URL and returned element 0. The two are required to answer
    // identically, so the flag can only change how much of the classpath was
    // touched — which makes it the same-binary A/B for that cost.
    E { group: Group::COMPAT, token: "getresource-first-hit", on_key: Some("CRATONVM_GETRESOURCE_FIRST_HIT"), off_key: None, off_word: Some("0") },
