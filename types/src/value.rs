...
// A diagnostic can only discover a second OS thread on that thread's first
// ObjectRef construction. Remember that observation locally so later ObjectRef
// constructions do not reload the process-wide enable cell or retry the CAS.
thread_local! {
    static SINGLE_THREAD_GUARD_CHECKED: Cell<bool> = const { Cell::new(false) };
}
// ---------------------------------------------------------------------------
...
/// Enforce the single-OS-thread diagnostic once for this OS thread. Later
/// `ObjectRef` constructions on the same thread cannot reveal a new owner, so
/// they avoid the process-global enable load entirely.
#[inline(always)]
fn enforce_single_os_thread() {
    SINGLE_THREAD_GUARD_CHECKED.with(|checked| {
        if !checked.replace(true) && single_thread_guard_enabled() {
