...
pub unsafe extern "C" fn jit_aaload(array_ptr: i64, index: i64) -> i64 {
    ...
    let elem_ptr = ptr.add(HEADER_SIZE + index as usize * cratonvm_types::narrow_oop::ref_element_size());
    ...
    let raw = cratonvm_types::narrow_oop::read_ref_slot(elem_ptr);
...
pub unsafe extern "C" fn jit_aastore(vm_ptr: i64, array_ptr: i64, index: i64, val: i64) {
    ...
    let elem_ptr = ptr.add(HEADER_SIZE + index as usize * cratonvm_types::narrow_oop::ref_element_size()) as *mut u8;
    ...
    let old_raw = cratonvm_types::narrow_oop::read_ref_slot(elem_ptr);
    if old_raw != 0 {
    ...
    cratonvm_types::narrow_oop::write_ref_slot(elem_ptr, val as u64);
...
pub unsafe extern "C" fn jit_getfield(vm_ptr: i64, obj_ptr: i64, field_index: i64) -> i64 {
    ...
    if let Some((off, storage)) = jit_compact_field_slot(obj_ptr, field_index) {
        ...
        if storage.is_reference() {
            ...
            let raw = cratonvm_types::narrow_oop::read_ref_slot(ptr);
...
pub unsafe extern "C" fn jit_putfield_object(
    ...
    if let Some((off, storage)) = jit_compact_field_slot(obj_ptr, field_index) {
        ...
        let old_raw: u64 =
            cratonvm_types::narrow_oop::read_ref_slot((obj_ptr as *const u8).add(HEADER_SIZE + off));
        if old_raw != 0 {
