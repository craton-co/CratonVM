...
impl JitAllocClassCache {
    ...
    ///
    /// The only caller is `jit_invalidate_adapter`
    /// (`vm/src/vm/vm_init.rs`), the `ClassInfoHook` the GC fires on an
    /// **in-place layout change**: a class redefinition, a subclass re-layout,
    /// or a synthetic stub being replaced by real bytecode with a different
    /// field count or order. It fans out over every live VM, because the hook
    /// carries no VM identity.
    ///
