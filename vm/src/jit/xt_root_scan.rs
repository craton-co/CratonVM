...
/// and every readable word from `rsp` up -- and `classify_parked_snapshot`
...
mod imp {
    ...
    ///
    /// In `HELPER_MODE` the handler parks a peer at whatever instruction the
    /// signal found — inside `malloc`, holding a `parking_lot` lock in
    ...
    fn snapshot_parked_slot(
        ...
        // (`band` holds `MAX_STACK_SCAN` bytes and `end <= rsp +
        // MAX_STACK_SCAN`, so the `words < band.len()` bound never cuts a read
        // short of `end`.)
        snap.band_complete = true;
        snap
    }
        ...
        for &tid in blocked_os_tids {
            ...
            match answer {
                STATE_PARKED => {
                    ...
                    // classify over the copy. HELPER_MODE parks a peer at
