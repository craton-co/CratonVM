...
pub fn note_plain_bytecode_tail(
    ...
    WITNESS.with(|w| {
        ...
        {
            if receiver_class.map(ClassId::as_u32) == Some(armed_receiver)
                && name_ptr == method_name.as_ptr() as usize
                && name_len == method_name.len()
                && desc_ptr == descriptor.as_ptr() as usize
                && desc_len == descriptor.len()
...
mod tests {
    ...
    fn the_witness_is_only_claimed_by_its_own_triple() {
        ...
        // not claim it.
        note_plain_bytecode_tail(ClassId::new(99), None, "<clinit>", "()V", true);
        // ...nor may an arrival with the right strings but a non-plain callee.
        note_plain_bytecode_tail(ClassId::new(99), Some(ClassId::new(42)), name, desc, false);
        note_plain_bytecode_tail(ClassId::new(77), Some(ClassId::new(42)), name, desc, true);
    ...
    fn a_nested_arm_restores_the_outer_candidate() {
        ...
        note_plain_bytecode_tail(ClassId::new(42), Some(ClassId::new(42)), name, desc, true);
    ...
    fn a_tail_on_another_receiver_with_the_same_strings_does_not_claim() {
        ...
        let name = "size";
        let desc = "()I";
        let save = arm(ClassId::new(10), name, desc);
        note_plain_bytecode_tail(ClassId::new(20), Some(ClassId::new(20)), name, desc, true);
        assert_eq!(disarm(save), None);
    }
}
