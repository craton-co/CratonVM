...
fn run_transformer_chain(
    ...
    //
    // The funnel, not just the `retransformClasses0` caller: every path that
    // reaches a Java transformer goes through here, so the invariant holds even
    // if a future caller forgets it.
    if initial_bytes.is_empty() {
        tracing::debug!(
            "transformer chain: no class file bytes to transform for class id \
             {class_id:?}; skipping the chain rather than presenting an empty buffer"
        );
        return Vec::new();
    }
