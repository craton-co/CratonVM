...
fn install_signal_handlers() {
    ...
    /// The registered handler, and deliberately SMALL.
    ///
    /// # Why this is split from [`crash_report_handler`]
    ///
    /// rustc reserves a function's WHOLE frame in its prologue, including
    /// locals only the cold path uses, and it does that before the first
    /// statement runs. While the report path lived in this function, the
    /// prologue reserved its buffers too:
    ///
    /// | build   | prologue                            | frame   |
    /// |---------|-------------------------------------|---------|
    /// | debug   | `sub $0x1000` (probe) + `sub $0xbb8` | 7 096 B |
    /// | release | `sub $0x788` + six pushes           | 1 976 B |
    ///
    /// A handler registered with `SA_ONSTACK` runs on the sigaltstack Rust
    /// installs per thread, which is the COMPILE-TIME `SIGSTKSZ` the `libc`
    /// crate carries for linux-gnu: 8 192 bytes. The kernel has already spent
    /// part of that on the signal frame -- `sysconf(_SC_MINSIGSTKSZ)` is 3 376
    /// on this host's CPU and `_SC_SIGSTKSZ` recommends 13 504 -- so roughly
    /// 4 816 bytes reach the handler. Release fitted; **debug did not**, and
    /// the prologue's own stack-probe store landed below the altstack. A fault
    /// while handling SIGSEGV on the altstack makes the kernel reset to
    /// `SIG_DFL` and kill the process, so a debug build produced no banner, no
    /// `hs_err_pid<pid>.log` and no recovered null check -- while the JIT went
    /// on ELIDING receiver null checks, because `note_fault_handler_installed`
    /// had been told the handler existed.
    ///
    /// That made every null receiver in compiled code a silent SIGSEGV instead
    /// of a `NullPointerException`, on exactly the binary CI drives (`ci.yml`
    /// runs the regression suite with `CV=target/debug/cratonvm`), which is
    /// what `RJitUnrollImplicitNpe` had been failing on.
    ///
    /// Keeping the recovery arm here, and nothing else, bounds this frame to a
    /// few hundred bytes in either profile, so the arm that decides
    /// NullPointerException-versus-process-death cannot be starved by the size
    /// of the report path next door. [`ensure_crash_altstack`] separately buys
    /// the report path room to run; this split is what makes CORRECTNESS not
    /// depend on that having been called on this thread.
    extern "C" fn crash_signal_handler(
        ...
        crash_report_handler(sig, info, ucontext);
    }

    /// The crash REPORT, out of line so its locals are not reserved on the
    /// recovery path. See [`crash_signal_handler`] for the measurement that
    /// forced the split.
    ///
    /// `#[inline(never)]` is load-bearing, not a hint: inlining this back into
    /// the registered handler restores the 7 KB prologue and with it the
    /// silent-death-instead-of-NullPointerException defect. `#[cold]` is the
    /// honest annotation for a path that runs at most once per process.
    #[inline(never)]
    #[cold]
    extern "C" fn crash_report_handler(
        sig: std::ffi::c_int,
        info: *mut libc::siginfo_t,
        ucontext: *mut std::ffi::c_void,
    ) {
...
",
    ...
    // faulting.
    // This thread first; `ensure_crash_altstack` is idempotent and every other
    // thread that can execute compiled code calls it for itself.
    ensure_crash_altstack();
    ...
    for &sig in CRASH_SIGNALS {
        unsafe {
            ...
            // `crash_signal_handler` recovers implicit null checks, so the JIT
            // may elide them from now on (hazard 4 in
            // `cratonvm_jit::implicit_null`).
            //
            // READ THE DISPOSITION BACK rather than trusting the return value.
            // What the JIT is told here is not "a syscall succeeded", it is
            // "a null receiver in compiled code will become a
            // NullPointerException instead of killing the process" -- and that
            // is a claim about what the kernel will DO, so the weakest check
            // worth making is that the kernel now names our function. It costs
            // one more syscall, once per process.
            //
            // This does not prove the handler will RUN: for two months it did
            // not, on every debug build, because its prologue overflowed the
            // sigaltstack (see `crash_signal_handler`). A stronger proof would
            // be a deliberate fault at a registered address at start-up; the
            // reason that is not done here is that it would have to synthesise
            // executable code to fault AT, which is a bigger machine than the
            // check deserves. `ensure_crash_altstack` removes the failure that
            // was actually observed, and the split handler makes recovery not
            // depend on it.
            let verified = installed
                && unsafe {
                    let mut back: libc::sigaction = std::mem::zeroed();
                    libc::sigaction(sig, std::ptr::null(), &mut back) == 0
                        && back.sa_sigaction == crash_signal_handler as usize
                };
            if verified && sig == libc::SIGSEGV {
                cratonvm_jit::implicit_null::note_fault_handler_installed();
            }
        }
    }
...
/// Bytes of sigaltstack this VM wants on any thread that can fault in
/// compiled code.
///
/// Rust installs `SIGSTKSZ` (the `libc` crate's compile-time 8 192 on
/// linux-gnu) and the kernel spends part of that on the signal frame before
/// the handler's first instruction -- `sysconf(_SC_MINSIGSTKSZ)` measured
/// 3 376 on the x86-64 build host, and `_SC_SIGSTKSZ` recommends 13 504, i.e.
/// glibc itself says 8 192 is no longer enough. 256 KiB is far more than
/// `crash_report_handler` needs (about 7 KB at its worst, in a debug build)
/// and is cheap: it is lazily faulted, so a thread that never crashes never
/// touches more than a page of it.
#[cfg(unix)]
const CRASH_ALTSTACK_BYTES: usize = 256 * 1024;

#[cfg(unix)]
struct CrashAltStack {
    ptr: *mut libc::c_void,
    len: usize,
}

#[cfg(unix)]
impl Drop for CrashAltStack {
    fn drop(&mut self) {
        if self.ptr.is_null() {
            return;
        }
        // Disable before unmapping, or a signal delivered between the two
        // would be told to run on memory that is no longer mapped -- which is
        // the very failure this type exists to remove, with a worse address.
        // SAFETY: `ss_sp`/`ss_size` are ignored when `SS_DISABLE` is set, and
        // `self.ptr` is this thread's own mapping from `install`.
        unsafe {
            let ss = libc::stack_t {
...
#[cfg(unix)]
thread_local! {
    /// Installed on first [`ensure_crash_altstack`] call on this thread, and
    /// unmapped when the thread ends.
    static CRASH_ALTSTACK: CrashAltStack = CrashAltStack::install();
}

#[cfg(unix)]
impl CrashAltStack {
    fn install() -> CrashAltStack {
        let none = CrashAltStack {
            ptr: std::ptr::null_mut(),
            len: 0,
        };
        // SAFETY: plain queries and a mapping this thread then owns. The
        // `sigaltstack` calls name only local `stack_t`s.
        unsafe {
            let mut old: libc::stack_t = std::mem::zeroed();
            if libc::sigaltstack(std::ptr::null(), &mut old) == 0
                && (old.ss_flags & libc::SS_DISABLE) == 0
                && old.ss_size >= CRASH_ALTSTACK_BYTES
            {
                // Somebody already gave this thread a big one. Leave it: two
                // owners of one thread's altstack is a worse bug than a small
                // altstack.
                return none;
            }
            let ptr = libc::mmap(
                std::ptr::null_mut(),
                CRASH_ALTSTACK_BYTES,
            ...
            if ptr == libc::MAP_FAILED {
                // Keep the small one. The split handler means recovery still
                // works; only the report is at risk.
                return none;
            }
            let ss = libc::stack_t {
                ss_sp: ptr,
                ss_flags: 0,
                ss_size: CRASH_ALTSTACK_BYTES,
            };
            if libc::sigaltstack(&ss, std::ptr::null_mut()) != 0 {
                libc::munmap(ptr, CRASH_ALTSTACK_BYTES);
                return none;
            }
            CrashAltStack {
                ptr,
                len: CRASH_ALTSTACK_BYTES,
            }
        }
    }
}

/// Give THIS thread a sigaltstack big enough for [`crash_report_handler`].
///
/// Idempotent, and safe to call from anywhere: the first call on a thread
/// installs the stack, later ones are a thread-local read. Call it from a
/// thread's start, not from a hot path.
///
/// Rust gives every thread an 8 KiB altstack and `SA_ONSTACK` makes the
/// handler run on it. On a debug build `crash_report_handler` does not fit,
/// and a handler that faults on the altstack while handling SIGSEGV is a
/// silent kill -- no banner, no `hs_err_pid<pid>.log`. That is what this
/// removes.
///
/// A thread that never calls it is not left unsafe, only undiagnosable:
/// `crash_signal_handler` is split so the implicit-null RECOVERY arm fits in
/// the small altstack on its own.
#[cfg(unix)]
pub fn ensure_crash_altstack() {
    let _ = CRASH_ALTSTACK.try_with(|_| ());
}

/// No-op on Windows, where faults arrive through a vectored exception handler
/// on the faulting thread's own stack and there is no alternate signal stack.
#[cfg(not(unix))]
pub fn ensure_crash_altstack() {}
...
/// The sigaltstack guard, which is what makes a crash on a debug build
/// REPORTABLE.
///
/// These are `#[cfg(unix)]` because `ensure_crash_altstack` is a no-op
/// elsewhere: Windows delivers faults to a vectored exception handler on the
/// faulting thread's own stack, so there is no alternate stack to size.
#[cfg(all(test, unix))]
mod crash_altstack_tests {
    use super::*;

    /// What the RUNNING kernel says a signal frame needs, which is the number
    /// the compile-time `SIGSTKSZ` stopped tracking.
    ///
    /// `_SC_SIGSTKSZ` is not in the `libc` crate, and its value is
    /// glibc-specific (250 there), so this asks and falls back rather than
    /// baking it in: a platform that does not answer simply does not
    /// strengthen the assertion. Measured on the x86-64 build host:
    /// `sysconf(_SC_SIGSTKSZ)` = 13 504 and `sysconf(_SC_MINSIGSTKSZ)` =
    /// 3 376, against a compile-time `SIGSTKSZ` of 8 192.
    fn platform_recommended() -> usize {
        #[cfg(target_env = "gnu")]
        const SC_SIGSTKSZ: libc::c_int = 250;
        #[cfg(target_env = "gnu")]
        // SAFETY: a plain `sysconf` query; it writes nothing and returns -1
        // for an unknown name rather than failing.
        let queried = unsafe { libc::sysconf(SC_SIGSTKSZ) };
    ...
    /// The defect this whole change is about, stated as a test: after the
    /// guard runs, this thread's alternate signal stack must be big enough
    /// for the crash report to run on.
    ///
    /// Before 2026-09-21 the only altstack was the one Rust installs, the
    /// `libc` crate's compile-time `SIGSTKSZ` of 8 192 -- and on this CPU the
    /// kernel's own signal frame takes `_SC_MINSIGSTKSZ` = 3 376 of it before
    /// the handler's first instruction. A debug `crash_signal_handler`
    /// reserved 7 096 bytes in its prologue, so it faulted ON the altstack
    /// while handling SIGSEGV, and the kernel killed the process with no
    /// banner, no `hs_err_pid<pid>.log` and no recovered null check.
    #[test]
    fn the_guard_gives_this_thread_a_stack_the_report_fits_on() {
        ensure_crash_altstack();
        // SAFETY: querying this thread's own alternate stack; writes only the
        // local `stack_t`.
        let mut ss: libc::stack_t = unsafe { std::mem::zeroed() };
        let rc = unsafe { libc::sigaltstack(std::ptr::null(), &mut ss) };
        assert_eq!(rc, 0, "sigaltstack query failed");
        assert_eq!(
            ss.ss_flags & libc::SS_DISABLE,
            0,
            "the alternate stack is disabled, so SA_ONSTACK has nowhere to run"
        );
        assert!(
            ss.ss_size >= CRASH_ALTSTACK_BYTES,
            "alternate stack is {} bytes; the guard promises at least {}. \
             A debug `crash_report_handler` reserves about 7 KB in its \
             prologue and the kernel spends {} more on the signal frame \
             before it runs.",
            ss.ss_size,
            CRASH_ALTSTACK_BYTES,
    ...
    /// It has to be callable from a thread's start without anyone tracking
    /// whether it already ran, so a second call must not install a second
    /// stack (which would leak the first and, worse, leave two owners of one
    /// thread's altstack).
    #[test]
    fn the_guard_is_idempotent_and_does_not_reinstall() {
        ensure_crash_altstack();
        let mut first: libc::stack_t = unsafe { std::mem::zeroed() };
        assert_eq!(
            unsafe { libc::sigaltstack(std::ptr::null(), &mut first) },
            0
        );
        ensure_crash_altstack();
        ensure_crash_altstack();
        let mut again: libc::stack_t = unsafe { std::mem::zeroed() };
        assert_eq!(
            unsafe { libc::sigaltstack(std::ptr::null(), &mut again) },
            0
        );
        assert_eq!(
            first.ss_sp, again.ss_sp,
            "a repeat call moved the alternate stack"
        );
        assert_eq!(first.ss_size, again.ss_size, "a repeat call resized it");
    }

    /// Each thread owns its own: `sigaltstack` is per-thread, so a guard that
    /// only ever ran on the installing thread would leave every Java thread
    /// on the 8 KiB one.
    #[test]
    fn a_second_thread_gets_its_own_stack() {
        ensure_crash_altstack();
        let mut here: libc::stack_t = unsafe { std::mem::zeroed() };
        assert_eq!(unsafe { libc::sigaltstack(std::ptr::null(), &mut here) }, 0);
        let here_sp = here.ss_sp as usize;
        let there_sp = std::thread::spawn(|| {
            ensure_crash_altstack();
            let mut ss: libc::stack_t = unsafe { std::mem::zeroed() };
            assert_eq!(unsafe { libc::sigaltstack(std::ptr::null(), &mut ss) }, 0);
            assert!(
                ss.ss_size >= CRASH_ALTSTACK_BYTES,
                "a spawned thread kept the small alternate stack"
            );
            ss.ss_sp as usize
        })
        .join()
        ...
        assert_ne!(
            here_sp, there_sp,
            "two threads were handed the same alternate stack"
        );
    }

    /// The promise has to beat what the platform asks for, or the number is
    /// decoration. `_SC_SIGSTKSZ` was 13 504 on the x86-64 build host, which
    /// is already above the 8 192 Rust installs.
    #[test]
    fn the_promise_clears_what_the_platform_asks_for() {
        assert!(
            CRASH_ALTSTACK_BYTES > platform_recommended(),
            "CRASH_ALTSTACK_BYTES ({}) must exceed sysconf(_SC_SIGSTKSZ) ({})",
            CRASH_ALTSTACK_BYTES,
            platform_recommended(),
        );
        assert!(
            CRASH_ALTSTACK_BYTES > libc::SIGSTKSZ,
            "and the compile-time SIGSTKSZ ({}) Rust's own altstack uses",
            libc::SIGSTKSZ,
        );
    }
}
