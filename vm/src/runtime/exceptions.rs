...
/// docs/jdk-only/W7-37-differential-throwable-and-vm.md.
...
fn klass_origin(shared: &SharedVm, thread: &JvmThread, display_name: &str) -> Option<KlassOrigin> {
    ...
    // docs/jdk-only/W8-C4-1-array-cast-klass-origin.md §1):
...
// (`perf-implicit-exceptions-in-compiled-code-cost-microseconds-each-20260918.md`),
...
/// `(vm, method code address, pc, kind)`.
...
fn create_stackless_exception_object(shared: &SharedVm, class_name: &str) -> Option<ObjectRef> {
    let (class_id, num_fields) = {
        let cm = shared.classes.class_manager.read();
        let class_id = cm.find_bootstrap_class_by_name(class_name)?;
        let class = cm.get_class(class_id)?;
    ...
    // bare `obj` cannot go stale here.
    mirror_throwable_field_initialisers(shared, obj);
    Some(obj)
}
...
struct ClassInitView {
    ...
    initialized: bool,
    /// Never initialised, not being initialised, not erroneous.
    untouched: bool,
    /// Declares a `<clinit>`.
    has_clinit: bool,
...
mod tests {
    ...
    /// `docs/jdk-only/W8-C4-1-array-cast-klass-origin.md`
    ...
    fn stackless_exception_has_the_class_and_no_stored_trace() {
        ...
        let hash = vm.shared.mem.heap.identity_hash_code(fast);
        assert!(
            vm.shared.throwable_stack_trace(hash).is_none(),
