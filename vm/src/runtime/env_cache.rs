...
/// `java/util/TreeMap`'s family carriers, when `CRATONVM_REAL` has handed the
/// map itself to real JDK bytecode.
///
/// L1 wave 8 (2026-09-12) retired the WHOLE `java/util/TreeMap` family together
/// -- the map, its view and iterator carriers, and their producers -- precisely
/// so that a producer and its consumer are never split across native and real.
/// The force-native gate did not know that: it forced the family's carriers
/// onto their natives unconditionally, which under `CRATONVM_REAL`
/// reintroduces exactly the split the retirement exists to prevent.
///
/// The retired `the-treemap-iterator-force-native-gate-outlived-wave-8s-
/// retirement` write-up repaired the three iterator carriers'
/// `hasNext`/`next`/`remove` arm alone and left the rest of the family open.
/// **Repairing one member of a family that was retired as a family is not a
/// repair**, and the measurement says so: with that narrow guard in place,
/// `apps/probes/TmRealViewSweep.java` under `CRATONVM_REAL=all` against
/// HotSpot 25.0.4+7 still diverged on **27 of 63 rows**, and the iterator rows
/// were among them. `TreeMap$EntrySet.iterator()` was still forced native, so
/// it minted a carrier whose SNAPSHOT slots the native fills and whose real
/// `next` field it does not; the now-yielding real `hasNext()` read that null
/// and reported every `TreeMap` in the process empty. `size()` and
/// `firstKey()` -- real bytecode over the real root -- were right the whole
/// time, which is what makes the failure quiet.
///
/// The question is asked of the owning MAP as well as of the carrier, because
/// `CRATONVM_REAL=java/util/TreeMap` names the class a caller thinks in and the
/// carriers have to move with it. `Off`/unset answers `false` for every input,
/// so an ordinary run does not change by one dispatch.
#[inline]
pub fn tree_map_family_yields_to_real_bytecode(class_name: &str) -> bool {
    if !matches!(
        class_name,
    ...
    let sel = real_bytecode_selector();
    sel.prefers_real(class_name) || sel.prefers_real("java/util/TreeMap")
}
...
cached_is_set!(dbg_sttrace, "CRATONVM_DBG_STTRACE");
...
/// Guarded monomorphic virtual/interface inlining. **DEFAULT ON since N8**; opt
/// OUT with `CRATONVM_JIT_GUARDED_VIRTUAL_INLINE=0`. See
/// `docs/feature-designs/profile-guided-inlining.md` for the lowering.
///
/// # History, which is why the flip below is stated carefully
///
/// It defaulted ON once before, from `f697d618e` -- a commit whose own subject
/// calls it "wip(pgo-02): plumbing ... (checkpoint, no codegen yet)" -- while
/// THREE places in `jit/src/lib.rs` described this same flag as "default-off,
/// unsoaked" and reasoned about correctness on that basis. The documented
/// contract and the code disagreed, and the code was the one nobody read. That
/// is an argument about how a default is moved, not about which default is
/// right, and this time the move is the deliberate one those comments were
/// waiting for. (They are now stale in the other direction; `jit/src/lib.rs`
/// is owned by another lane this cycle, so the correction is written up in
/// `NOTES-deopt2.md` rather than made here.)
///
/// That earlier accidental default also produced wrong answers, and the defect
/// is the reason this doc is as long as it is.
/// `probes/TreeTailIterProbe.java`: a compiled
...
///
/// # Why the default is now ON -- N8
///
/// The cost of OFF is not "one optimisation missing": `plan_inline` refuses
/// every speculative plan with `InlineRefusal::GuardNotEmittable` when the
/// resolver is absent, so with this flag off no Java method whose hot work is
/// behind an interface -- streams, collections, Netty handlers, Spring
/// proxies -- ever gets its callee inlined.
///
/// **The parenthesis this paragraph used to carry is now FALSE and is removed
/// rather than softened.** It read "this flag is the ONLY devirtualization in
/// the tree: there is no class-hierarchy analysis anywhere", with a `rg` recipe
/// offered as proof. That recipe now hits real analysis code:
/// `ClassRealm::unique_implementor` (M5, review round 7) surveys the loaded
/// hierarchy for a single concrete provider, and `CRATONVM_JIT_CHA` is its own
/// lever -- see [`jit_cha`], which also explains why the two are ANDed rather
/// than independent.
///
/// What survives the correction is the conclusion, and by a shorter route than
/// the old one: CHA's verdict is lowered as the same exact class-id compare
/// this flag gates, so `plan_inline` refuses it too when the resolver is
/// absent. Turning this off still withdraws every devirtualization in the
/// tree -- not because there is only one analysis, but because both analyses
/// need the one guard.
///
/// The default flip itself (N8) was measured against a tree that had no CHA.
/// Nobody has re-measured it since M5 landed, and this comment is not the place
/// to claim otherwise.
///
/// The guard itself is sound, and the review that flipped this re-confirmed it
/// from the emitter: `jit/src/x64/op_invoke.rs` emits an EXACT class-id compare
/// (`CMP DWORD [RAX+0], guard_class_id`), never a subtype test, and the miss
/// edge falls through to unchanged dispatch rather than to a deopt -- so a
/// newly loaded subclass cannot take the wrong body, and retiring the artifact
/// on a conflicting class load is a PERFORMANCE obligation rather than a
/// correctness one. Class-load invalidation is wired over the full supertype
/// closure (`vm/src/vm/vm_init.rs`).
///
/// **The obligation this flip carries is a soak, not an argument.** Read
/// `nested-splice-guarded` against `nested-splice-guarded-refused` on the
/// workloads the guard was built for (netty `HttpHeaderValidationUtil`,
/// hibernate-reactive, the stream benchmarks); the second counter only started
/// counting top-level refusals recently, so a baseline taken before that is not
/// comparable. `CRATONVM_JIT_GUARDED_VIRTUAL_INLINE=0` is the one-variable
/// bisect if anything regresses -- which is why this is an opt-OUT rather than
/// the resolver simply being passed unconditionally.
///
/// `vm/tests/jit_guarded_inline_native_shadow.rs` still sets `=1` explicitly.
/// That is now redundant and harmless, and it is left alone on purpose: a test
/// that names the state it needs keeps working whichever way the default goes.
#[inline]
pub fn jit_guarded_virtual_inline() -> bool {
    static CACHE: MemoSlot = MemoSlot::new();
    slot_bool(&CACHE, || {
        let raw = cratonvm_types::flags::runtime_var("CRATONVM_JIT_GUARDED_VIRTUAL_INLINE");
        guarded_virtual_inline_from_env(raw.as_deref().ok())
    })
}

/// [`jit_guarded_virtual_inline`]'s rule, as a pure function.
///
/// Split out so the DEFAULT is asserted by a test rather than read off a
/// `map_or`'s first argument. This flag's default has been wrong before — see
/// the History section above, where it was ON by accident while three places
/// reasoned from OFF — and a one-character edit to the memoised reader is not
/// the kind of change a reviewer catches.
fn guarded_virtual_inline_from_env(v: Option<&str>) -> bool {
    match v {
        None => true,
...
/// Class-hierarchy analysis (M5) — `CRATONVM_JIT_CHA`, default ON, `=0`
/// disables.
///
/// # Why this is a separate lever from [`jit_guarded_virtual_inline`]
///
/// CHA used to ride that flag alone: the VM passes
/// `CompileRequest::unique_concrete_resolver` only where it also passes
/// `class_id_name_resolver`, so `CRATONVM_JIT_GUARDED_VIRTUAL_INLINE=0` took
/// class-hierarchy analysis away together with the rest of devirtualization.
/// That is a correct one-variable lever for "is devirtualization the cause"
/// and the WRONG one for "is CHA the cause". The two answer at different
/// sites — the receiver profile speaks for a site it has samples for, CHA for
/// a site whose hierarchy has exactly one concrete body, loaded or not — so a
/// regression bisect could not separate them with one flag.
///
/// # Why the callers AND it with `jit_guarded_virtual_inline`
///
/// CHA's verdict is a HINT, not a binding: it is lowered as an exact receiver
/// class-id compare (`jit/src/x64/op_invoke.rs`) with the miss edge falling
/// through to ordinary dispatch, and it registers an
/// `InlineDependency::UniqueConcreteMethod` so a later class load retires the
/// artifact. Both halves need the class-id namer. So `=1` here with
/// `CRATONVM_JIT_GUARDED_VIRTUAL_INLINE=0` is not a configuration that can
/// exist: `plan_inline` would refuse every plan with
/// `InlineRefusal::GuardNotEmittable` anyway. The three `CompileRequest` sites
/// in `vm/src/runtime/interpreter/jit_bridge.rs` spell that as
/// `jit_cha() && jit_guarded_virtual_inline()` so the impossible combination
/// is refused where a reader can see it, rather than downstream by a refusal
/// counter.
///
/// # What `=0` costs, and what it does not
///
/// Not correctness in either direction. With the resolver withheld, every site
/// CHA would have answered for keeps the receiver-profile path it had before
/// M5 — a missed devirtualization, never a wrong target. See
/// `ClassRealm::unique_implementor` for the survey and `CHA_MAX_SUBTYPES` for
/// its bound.
#[inline]
pub fn jit_cha() -> bool {
    static CACHE: MemoSlot = MemoSlot::new();
    slot_bool(&CACHE, || {
        let raw = cratonvm_types::flags::runtime_var("CRATONVM_JIT_CHA");
        cha_from_env(raw.as_deref().ok())
    })
}

/// [`jit_cha`]'s rule, as a pure function.
///
/// Split out for the same reason [`guarded_virtual_inline_from_env`] is: the
/// DEFAULT is a claim a test can assert, and it is default-ON, so a
/// one-character edit that inverted it would silently withdraw an
/// optimisation from every shipped run. Accepts the same two spellings of
/// "off" that flag does, so the pair behaves identically to an operator.
fn cha_from_env(v: Option<&str>) -> bool {
    match v {
        None => true,
        Some(v) => v != "0" && !v.eq_ignore_ascii_case("false"),
    }
...
mod tests {
    ...
    /// The guarded-inline default, pinned — N8.
    ///
    /// This is the tree's ONLY devirtualization: with it off, `plan_inline`
    /// answers `InlineRefusal::GuardNotEmittable` for every speculative plan
    /// and no method whose hot work is behind an interface ever gets its
    /// callee inlined. The default has been wrong before, in the other
    /// direction and by accident, and nothing failed when it was.
    ///
    /// `=0` is the one-variable bisect and must keep working; so must
    /// `=false`, because `vm/tests/pgo02_guarded_virtual_inline.rs` and
    /// `jit_guarded_inline_native_shadow.rs` set this variable by name and a
    /// reader who knows one spelling will try the other.
    #[test]
    fn guarded_virtual_inlining_is_on_unless_explicitly_switched_off() {
        assert!(
            super::guarded_virtual_inline_from_env(None),
            "the shipped default must perform devirtualization"
        );
        assert!(super::guarded_virtual_inline_from_env(Some("1")));
        assert!(super::guarded_virtual_inline_from_env(Some("true")));
        assert!(!super::guarded_virtual_inline_from_env(Some("0")));
        assert!(!super::guarded_virtual_inline_from_env(Some("false")));
        assert!(!super::guarded_virtual_inline_from_env(Some("FALSE")));
    }

    /// The CHA default, pinned, and pinned to the SAME spellings — C3.
    ///
    /// `CRATONVM_JIT_CHA` exists so a bisect can ask "is class-hierarchy
    /// analysis the cause" without also withdrawing the receiver-profile
    /// devirtualization that `guarded-virtual-inline` controls. That is only
    /// useful if it is default-ON — an opt-OUT bisects the shipped
    /// configuration, an opt-IN bisects one nobody runs — and only usable if
    /// `=0` and `=false` both work, because an operator who learned one
    /// spelling from the flag above will try it here.
    #[test]
    fn cha_is_on_unless_explicitly_switched_off() {
        assert!(
            super::cha_from_env(None),
            "the shipped default must perform class-hierarchy analysis"
        );
        assert!(super::cha_from_env(Some("1")));
        assert!(super::cha_from_env(Some("true")));
        assert!(!super::cha_from_env(Some("0")));
        assert!(!super::cha_from_env(Some("false")));
        assert!(!super::cha_from_env(Some("FALSE")));
    }
