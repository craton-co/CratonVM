...
pub fn capture_full_trace_without_store(frames: &[Frame]) -> Vec<StackTraceEntry> {
    ...
    let (jit, osr_bci) = drop_osr_continuations(frames, jit);
    interleave_compiled_frames_without_store(frames, &jit, &osr_bci)
}
...
/// Whether a throwable trace could contain the construction machinery that
/// [`trim_throwable_fill_frames`] removes.  This deliberately only looks at
/// the innermost frame: the common JDK-native constructor case has neither a
/// Java `<init>` frame nor `fillInStackTrace`, so it can avoid acquiring the
/// class-manager lock solely to discover that there is nothing to trim.
pub fn may_need_throwable_fill_frame_trim(trace: &[StackTraceEntry]) -> bool {
    trace.last().is_some_and(|entry| {
        matches!(&*entry.method_name, "fillInStackTrace" | "<init>")
    })
}
...
fn interleave_compiled_frames_without_store(
    ...
    for (i, frame) in frames.iter().enumerate() {
        ...
        let mut entry = StackTraceEntry {
            ...
            byte_code_index: frame.last_instr_pc.min(i32::MAX as usize) as i32,
            class_id: Some(frame.class_id),
            method_index: None,
        };
...
mod tests {
    ...
    #[test]
    fn only_constructor_or_fill_tail_can_need_throwable_trim() {
        assert!(!may_need_throwable_fill_frame_trim(&[]));
        assert!(!may_need_throwable_fill_frame_trim(&[point("P", "throwSite", 1)]));
        assert!(may_need_throwable_fill_frame_trim(&[point("P", "<init>", 1)]));
        assert!(may_need_throwable_fill_frame_trim(&[point(
            "P",
            "fillInStackTrace",
            1
        )]));
