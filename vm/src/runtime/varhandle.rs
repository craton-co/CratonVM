...
// (`note_resolved` / `resolved_count` and the `RESOLVED_COUNT` static behind
// them were DELETED in round 10. The doc said "called by the native side after
// `findVarHandle` completes successfully"; no such call existed, and neither
// does any construction of `ResolvedVarHandle` outside this file -- the whole
// module is currently unreferenced by production code -- so the counter could
// not have been fed even in principle. The question it answered, "did the VH
// cache path fire?", is answered by `native-builtins`' own lang_invoke
// counters, which are on the path that actually runs. See
