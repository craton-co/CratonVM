...
pub(super) fn maybe_gc_forced_at(shared: &SharedVm, thread: &mut JvmThread, site: &'static str) {
    ...
    // finished and (on the multi-threaded arm) `complete_gc` has reopened the
    // world, exactly as in `maybe_gc`'s epilogue. `CRATONVM_G1_ALLOC_MARK_DRIVE=0`
...
pub(super) fn process_references_after_gc(
    ...
    // Read here for the same reason `free_mb` is: it asks the heap a question,
    // and there is no reason to do that under the reference-processor lock.
    // See the call site below for what it gates and why it is backend-scoped.
    let refproc_prepass = shared.mem.heap.is_generational() && !no_refproc_prepass();
    ...
    // `gengc-mark-dead-softref-object-still-roots-weak-closure-20260920.md`.
    //
    ...
    //
    // WHY GENERATIONAL ONLY. This function is backend-agnostic, but the
    // reordering is this wave's generational lane and `gc/src/zgc.rs` — which
    // runs `remove_collected` at its own `process_references` tail — is owned
    // elsewhere. Gating keeps G1 and ZGC byte-for-byte on the previous
    // sequence; `remove_collected` itself is unchanged, so neither backend's
    // contract moved. `CRATONVM_GC_NO_REFPROC_PREPASS=1` restores the old
    // order on the generational path too.
    if refproc_prepass {
...
fn dbg_compact_tlab_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| cratonvm_types::flags::runtime_var_os("CRATONVM_DBG_COMPACT_TLAB").is_some())
}
...
pub(crate) fn no_refproc_prepass() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        cratonvm_types::flags::runtime_var_os("CRATONVM_GC_NO_REFPROC_PREPASS").is_some()
    })
}
...
pub(super) fn maybe_concurrent_gc_at(shared: &SharedVm, thread: &mut JvmThread, door: MarkDoor) {
    ...
    let mut counted_os_tids: Vec<u32> = Vec::new();
    // Set if the STW closure ran but the remark inside it did not; see the
    // `old_gen_lock()` comment below.
    let mut remark_incomplete = false;
    let remark_done =
        shared
            .mem
            .gc_barrier
    ...
    if remark_done {
        ...
        // The remark is NOT optional (see the `!remark_done` abort below), but
        ...
        // the cycle only gets here when `old_gen_needs_gc()` said yes, which is
        // a hard `false` on G1 and ZGC, and `old_gen_lock()` is unconditionally
        ...
        // files away.
        let remarked = if let Some(guard) = shared.mem.heap.old_gen_lock() {
            // SAFETY: this closure body runs inside
            // `request_stw_counted_with_live_blocked`, after
            ...
            let stw = unsafe { cratonvm_gc::collector::StopTheWorldToken::new() };
            marker.remark(&stw, &root_ptrs, &*guard);
            true
        } else {
            false
        };
        if !remarked {
            remark_incomplete = true;
        }
        // Clear TLAB skip regions + resume frozen peers BEFORE reopening
        // the world (same race rationale as maybe_gc's epilogue).
        retire_skip_spans_and_resume(shared, taken);
    ...
    if !remark_done || remark_incomplete {
        marker.abort_cycle();
        return;
    }
...
pub(crate) fn g1_force_full_cycle(shared: &SharedVm, thread: &mut JvmThread) {
    ...
    // and it counts ONE outcome per CALL rather than one per loop turn.
    //
    // Its contract is different from every other door's: it SPINS to a
    // deadline instead of returning on `waiting`, because its caller is about
    // to throw `OutOfMemoryError` and a deferred cycle is worth nothing to it.
    // Routing it through the shared funnel would either make the funnel spin
    // for everyone or make this path give up early — the census is the part
    // that has to be shared, not the policy.
    //
    // One per call, because `visits` has to mean the same thing in every row
    // of the report or the table cannot be read across doors: a yield-spin
    // that turned a thousand times would otherwise put a thousand `waiting`s
    // against one pause and dominate the column. `waiting` here means "this
    // call had to wait at all".
    ...
    loop {
        if shared.mem.heap.g1_is_marking_active() {
            ...
            if shared.mem.heap.g1_concurrent_mark_finished() {
                ...
                let ran = g1_final_remark_cleanup(shared, thread);
                cratonvm_gc::g1::note_mark_door(
                    cratonvm_gc::g1::MarkDoor::ForceFull,
                    if ran {
                        cratonvm_gc::g1::MarkDoorOutcome::Finished
                    } else {
                        cratonvm_gc::g1::MarkDoorOutcome::LostStw
                    },
                );
            } else {
                if !waited {
                    ...
                    cratonvm_gc::g1::note_mark_door(
                        cratonvm_gc::g1::MarkDoor::ForceFull,
                        cratonvm_gc::g1::MarkDoorOutcome::Waiting,
                    );
                }
            ...
            if shared.mem.heap.g1_is_marking_active() {
                cratonvm_gc::g1::note_mark_door(
                    cratonvm_gc::g1::MarkDoor::ForceFull,
                    cratonvm_gc::g1::MarkDoorOutcome::Started,
                );
            }
        }
