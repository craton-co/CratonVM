...
fn an_unsupported_caller_local_refuses_where_the_in_place_transfer_tolerates_it() {
    ...
    let scope =
        |locals: Vec<FrameValue>, stack: Vec<FrameValue>| cratonvm_jit::deopt::ReconstructedFrame {
            ...
            stack,
            monitors: Vec::new(),
            // A caller scope: its bci names an `invoke` in progress.
...
/// Does this source line carry a real configuration attribute that gates the item
/// after it to the TEST configuration?
///
/// Round 10 found this rule too narrow, in the loudest possible way: it matched
/// only the literal `#[cfg(test)]`, so when `jit/src/x64/objects.rs` gained three
/// `#[cfg(all(test, target_arch = "x86_64"))]` test modules, the whole of each was
/// scanned as PRODUCTION and their `.expect("the test buffer must be flippable to
/// RX")` assertions were reported as production panic sites. The gate then failed
/// on a tree with no defect in it, which is how a gate gets switched off. The JIT
/// crate's own `panic_free_compile_ratchet.rs` had a sibling bug — it tested the
/// boundary before dropping comment lines — and both are the same mistake about
/// what "opens test code" means.
///
/// `not(test)` is excluded deliberately: `#[cfg(not(test))]` gates PRODUCTION
/// code, and treating it as a test boundary would hide the very lines this scan
/// exists to count. The `test` match must be a whole identifier so `latest` or
/// `test_util` in some other predicate does not qualify.
fn opens_test_gated_item(trimmed: &str) -> bool {
    ...
    rest.match_indices("test").any(|(i, _)| {
        let before_ok = i == 0 || !rest[..i].chars().next_back().is_some_and(ident);
        let after_ok = !rest[i + 4..].chars().next().is_some_and(ident);
        before_ok && after_ok
    })
}
...
fn scan_production_section(path: &str, needles: &[&str]) -> (usize, usize) {
    ...
    for line in src.lines() {
        ...
        if !is_comment && skip_depth == 0 && !pending && opens_test_gated_item(trimmed) {
            pending = true;
...
fn hot_files_have_no_production_panics() {
    ...
    for (path, max_allowed) in &targets {
        ...
        let production_prefix = src
            ...
            .position(|l| {
                let t = l.trim_start();
                !(t.starts_with("//") || t.starts_with('*')) && opens_test_gated_item(t)
            })
            .unwrap_or(total);
        assert!(
...
// E40-1 N4 — a real multi-catch `find_exception_handler` test.
//
// `multi_catch_exception_handler` (G37, `vm/src/vm/tests.rs`) used to stand
// in for this and had an EMPTY body: fifteen lines of comment, no statement,
// so no mutation of `find_exception_handler` could turn it red. Its own text
// said the reason was "we can't easily construct bytecode with multi-catch in
// a unit test" — false even when it was written (`register_test_class` in
// that file already accepts a `CodeAttribute` with a real `exception_table`)
// and the function it named has since moved to this module, which the
// deleted test's own citation never noticed rotting.
//
// This builds one `exception_table` with TWO entries over the SAME
// `[start_pc, end_pc)` range and the SAME `handler_pc` but two DIFFERENT
// `catch_type`s — the actual bytecode shape `try { … } catch (A | B e) { … }`
// compiles to (JVMS §3.13's multi-catch note: one handler, N table rows).
// Asserts the handler is found for EITHER exception type, and — the negative
// arm the deleted test's own premise said this needed — that a THIRD,
// unrelated exception type is NOT matched. Without that last assertion an
// implementation that returns the first table entry unconditionally (or
// treats every receiver as a match) would pass.
#[test]
fn multi_catch_two_catch_types_same_handler_third_type_refused() {
    ...
    // The two exception types the multi-catch names, and a third that is
    // neither — three ordinary synthetic classes, no inheritance relation
    // between them needed: `is_subclass_of`/`is_subclass_of_by_name` both
    // treat "is itself" as the base case, so an EXACT-type throw is enough
    // to tell "matched" from "not matched" apart.
    let (exc_a, exc_b, exc_c) = {
        let mut cm = shared.classes.class_manager.write();
        (
            cm.try_ensure_synthetic_class("cratonvm/test/MultiCatchExcA", 0)
                .expect("Compatible mode fabricates; this fixture never runs under --jdk-only"),
            cm.try_ensure_synthetic_class("cratonvm/test/MultiCatchExcB", 0)
                .expect("Compatible mode fabricates; this fixture never runs under --jdk-only"),
            cm.try_ensure_synthetic_class("cratonvm/test/MultiCatchExcC", 0)
                .expect("Compatible mode fabricates; this fixture never runs under --jdk-only"),
        )
    };

    // The frame's OWNING class needs a real constant pool: `catch_type` is a
    // CONSTANT_Class index into it, resolved by `find_exception_handler_impl`
    // via `constant_pool.get_class_name`. Index 0 is the mandatory tombstone.
    let owner_id = {
        let mut cm = shared.classes.class_manager.write();
        let id = cm.class_store.next_id();
        let pool = ConstantPool::new(vec![
            ConstantPoolEntry::Tombstone,
            ConstantPoolEntry::Utf8(std::sync::Arc::from("cratonvm/test/MultiCatchExcA")), // 1
            ConstantPoolEntry::ClassReference { name_index: 1 },                           // 2
            ConstantPoolEntry::Utf8(std::sync::Arc::from("cratonvm/test/MultiCatchExcB")), // 3
            ConstantPoolEntry::ClassReference { name_index: 3 },                           // 4
        ]);
        cm.class_store.add(Class {
            id,
            loader_id: ClassLoaderId::Application,
            name: cratonvm_types::intern_arc("cratonvm/test/MultiCatchOwner"),
            source_file: None,
        ...
        cm.register_class_name(ClassLoaderId::Application, "cratonvm/test/MultiCatchOwner", id);
        id
    };

    // `try { risky(); } catch (MultiCatchExcA | MultiCatchExcB e) { … }` —
    // one PC range, one handler, two table rows (JVMS §3.13).
    let exception_table = vec![
        ExceptionTableEntry {
            start_pc: 0,
            end_pc: 10,
            handler_pc: 20,
            catch_type: 2, // -> MultiCatchExcA
        },
        ExceptionTableEntry {
            start_pc: 0,
            end_pc: 10,
            handler_pc: 20, // SAME handler as the row above
            catch_type: 4,  // -> MultiCatchExcB
        },
    ];
    let frame = Frame::new(
        owner_id,
        "cratonvm/test/MultiCatchOwner".to_string(),
        "risky".to_string(),
        "()V".to_string(),
        None,
        vec![0xb1],
    ...
    let obj_a = shared.mem.heap.alloc_object(exc_a, 0);
    let obj_b = shared.mem.heap.alloc_object(exc_b, 0);
    let obj_c = shared.mem.heap.alloc_object(exc_c, 0);
    let pc_in_range = 5usize;

    assert_eq!(
        find_exception_handler(&shared, &frame, pc_in_range, obj_a),
        Some((20, obj_a)),
        "the FIRST catch_type (MultiCatchExcA) must reach the shared handler"
    );
    assert_eq!(
        find_exception_handler(&shared, &frame, pc_in_range, obj_b),
        Some((20, obj_b)),
        "the SECOND catch_type (MultiCatchExcB) must reach the SAME handler \
         — this is the row a search that stops at the first table entry \
         regardless of match would fail"
    );
    assert_eq!(
        find_exception_handler(&shared, &frame, pc_in_range, obj_c),
        None,
        "a third, unrelated exception type must NOT be matched — without this \
         negative arm, an implementation that returns the first entry \
         unconditionally passes the two assertions above"
    );
}
...
fn multianewarray_arm_guards_the_component_bracket_subtraction() {
    ...
    // the JIT's `jit_multianewarray_n` helper could stop carrying a second,
    ...
    assert!(
        // 2000 -> 3500 on 2026-09-22: the arm grew the outermost-first
        // negative-dimension pre-pass and its account, which pushed the call
        // past the old window. The window is a cheap way to say "in this arm
        // and not somewhere else in the file", so widening it with the arm is
        // the maintenance, not a weakening.
        opcodes[opcode_arm..opcode_arm + 3500].contains("multianewarray_alloc("),
...
/// `multianewarray` must reject the OUTERMOST negative dimension, and must
/// check every dimension before it allocates anything.
///
/// MEASURED on HotSpot 25.0.3+9: `new byte[-1][4][-2]` reports `-1`, and
/// `new byte[2][0][-1]` reports `-1` even though a zero outer length means no
/// inner array would ever be allocated. So the check is a full pre-pass over
/// the dimensions in outermost-first order, not something the allocation walk
/// discovers on its way down.
///
/// This arm used to check inside the POP loop, i.e. innermost first, and
/// answered `-2` and "no throw" for those two. The JIT's helper always read the
/// dimensions outermost-first, so the two tiers disagreed with each other as
/// well as with HotSpot — which is why this is pinned here and not only in
/// `regression-suite/src/RJitMultiArrayDims.java`, whose e-rows cover the same
/// ground on both tiers end to end.
///
/// A source witness because the alternative is a full `Vm` plus a hand-built
/// classfile for what is an ORDERING claim about six lines; it is anchored on
/// code text, not line numbers.
#[test]
fn multianewarray_checks_the_outermost_negative_dimension_first() {
    let opcodes = std::fs::read_to_string(format!(
        "{}/src/runtime/interpreter/opcodes.rs",
        env!("CARGO_MANIFEST_DIR")
    ))
    ...
    let arm = opcodes
        .find("Instruction::Multianewarray { index, dimensions } =>")
        .expect("the multianewarray arm must still exist");
    let body = &opcodes[arm..arm + 3000];

    let reverse = body
        .find(".reverse();")
        .expect("the popped dimensions must still be reversed to outermost-first");
    let check = body
        .find("NegativeArraySizeException")
        .expect("the arm must still raise NegativeArraySizeException");
    assert!(
        check > reverse,
        "the negative-dimension check must come AFTER the reverse, or it reads \
         the dimensions innermost-first: `new byte[-1][4][-2]` then reports -2 \
         where HotSpot and this VM's own JIT helper both report -1"
    );

    let alloc = body
        .find("multianewarray_alloc(")
        .expect("the arm must still call the shared allocator");
    assert!(
        check < alloc,
        "every dimension must be checked BEFORE anything is allocated: HotSpot \
         rejects `new byte[2][0][-1]` although a zero outer length means the \
         inner allocation is never reached"
    );
}
