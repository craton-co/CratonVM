...
pub(super) fn resolve_string_field_layout(
    ...
    // The `java/util/ArrayList` half (round 9 wave 8, `arraylist8`), from the
    // same read lock, for the ARRAYLIST_ACCESS family: an exact-`ArrayList`
    // `get(int)`/`size()` answered inline, declining into the site's ordinary
    // dispatch on anything else. The indices are the ones `native-collections`'
    // `al_slots_resolved` gets from `resolve_field_index` -- `elementData` and
    // `size` are both declared on `ArrayList` itself -- asked through the
    // hierarchy with their declared descriptors, so a synthetic image with
    // another shape resolves nothing.
    //
    // Refused under `--jdk-only`: `native_al_get`/`native_al_size` are
    // registered as `Bridge`, not `Intrinsic`, so the inline answer standing in
    // front of the real bytecode is refused there exactly like the HashMap thin
    // helpers (`direct_native_helper_for_impl`). No redefinition screen is
    // needed: `ArrayList` is in `redefine_immune_synthetic_collection_native`,
    // so a retransformed `ArrayList` keeps its native today, and the inline
    // answer is that native's fast path.
    let array_list = if crate::vm::dispatch_policy(shared).is_jdk_only() {
        None
    } else {
        (|| {
            let al_id = cm.find_bootstrap_class_by_name("java/util/ArrayList")?;
            let store = cm.class_store();
            let field = |name: &str, descriptor: &str| {
                crate::classloading::find_field_recursive_by_descriptor(
                    al_id, name, descriptor, store,
                )
                .map(|(idx, _, _)| idx)
            };
            cratonvm_jit::ArrayListFieldLayout::new(
