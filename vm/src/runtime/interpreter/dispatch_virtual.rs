...
pub(super) fn execute_invokevirtual_cached(
    ...
    match target {
        ...
        CachedInvokeTarget::Bytecode {
            ...
            // known-issues/spring/parallelexecutionspringextensiontests-is-a-10000-test-throughput-bench-and-parallelism-is-not-the-gap-20260910.md.
            //
...
///
/// Recorded in
/// `docs/jdk-only/G42-1-the-intrinsic-cache-and-the-1999-20260817.md`.
#[cfg(test)]
...
thread_local! {
    ...
    /// `(class_id, method_name, descriptor, handle)`.
    ///
...
fn record_receiver_memoized(
    ...
    DOOR_RECV_MEMO.with(|cell| {
        ...
        if let Some((c, n, d, rec)) = slot.as_ref() {
            // Three compares, the same as the address form: `Arc::ptr_eq` is a
            // pointer comparison, not a string comparison. What the owned
            // clones buy is that the pointers still MEAN something.
            if *c == class_id
