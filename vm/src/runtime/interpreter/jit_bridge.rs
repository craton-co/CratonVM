...
/// lane `osr11`; `perf-implicit-exceptions-in-compiled-code-cost-microseconds-each-20260918.md`).
///
/// # Why
///
...
/// One entry per thread -- the hot loop -- keyed by the VM's address, the
...
fn osr_reentry_memo_get(
    ...
    let vm = shared as *const SharedVm as usize;
...
fn osr_reentry_memo_put(
    ...
    let entry = if artifact.compiled_via_osr && artifact.can_osr_enter(entry_pc) {
        Some(OsrReentryMemo {
            vm: shared as *const SharedVm as usize,
            code,
            class_id,
        ...
        for &(pc, cp_idx, opcode) in &scan.invoke_ops {
            ...
            if invoke_kind == 0 && cratonvm_jit::scoped_memory_direct_enabled() {
                if let Some(op) = cratonvm_jit::is_scoped_memory_op(target_class, mn, desc) {
                    let entry = match op {
                        cratonvm_jit::ScopedMemoryOp::GetIntUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_get_int_direct as *const ()
                                as usize
                        }
                        cratonvm_jit::ScopedMemoryOp::GetShortUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_get_short_direct as *const ()
                                as usize
                        }
                        cratonvm_jit::ScopedMemoryOp::GetLongUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_get_long_direct as *const ()
                                as usize
                        }
                        cratonvm_jit::ScopedMemoryOp::GetCharUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_get_char_direct as *const ()
                                as usize
                        }
                        cratonvm_jit::ScopedMemoryOp::PutIntUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_put_int_direct as *const ()
                                as usize
                        }
                        cratonvm_jit::ScopedMemoryOp::PutShortUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_put_short_direct as *const ()
                                as usize
                        }
                        cratonvm_jit::ScopedMemoryOp::PutLongUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_put_long_direct as *const ()
                                as usize
                        }
                        cratonvm_jit::ScopedMemoryOp::PutCharUnaligned => {
                            crate::jit::helpers::jit_scoped_memory_put_char_direct as *const ()
                                as usize
                        }
                    };
...
/// `docs/known-issues/jit/perf-long-counted-loops-lose-the-optimizing-osr-body-20260918.md`),
