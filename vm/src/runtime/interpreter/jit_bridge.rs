...
pub(super) fn dbg_osr_recompile_reason(
    ...
    let why = match cached_osr {
        ...
        Some(_) if despec_stale => "cached-keeps-a-despeculated-guard",
...
/// Whether a cached OSR artifact must be recompiled rather than reused
/// because it still carries a speculative guard this VM has de-speculated
/// since the artifact was compiled (round 9 wave 10, lane `call10`;
/// `evicted-callee-stays-reachable-through-a-compiled-callers-binding-FIXED-20260918.md`).
///
/// `compile_osr_artifact` reuses a cached OSR body whenever it can enter at
/// the pc. The per-bci de-spec (`DespecRegistry`) is honoured by every NEW
/// compile, but an OSR body compiled before the insert kept its failing guard
/// for good: `TryLoop.sumThrow`'s loop-header speculative-BCE guard OSR-exited
/// at once on every entry, each exit was charged, and the method reached
/// `MakeNotCompilable` long before anything recompiled it.
///
/// All of these must hold, so the answer is `false` (reuse, as before) in
/// every ordinary case:
///
/// * `deopt_real_enabled()` -- it stamps `compilation_epoch` on the artifact;
/// * the artifact is OLDER than the method's live compilation epoch (a deopt
///   was recorded since it was compiled). A recompile is stamped with the live
///   epoch, so a guard the recompile still emits does not recompile again
///   until the next charged deopt: the loop is bounded by the deopt ledger;
/// * one of its speculative deopt points (anything but an `OsrExit` map, which
///   every OSR body carries at its loop boundaries and no de-spec removes) is
///   now de-speculated -- `contains_speculation`, so a wildcard entry matches;
/// * the method is not on the not-compilable skip list: the recompile would be
///   refused there, and a stale body is still better than none.
#[inline(never)]
fn cached_osr_body_is_despec_stale(
    shared: &SharedVm,
    cached: &crate::jit::CompiledMethod,
    class_name: &Arc<str>,
    method_name: &Arc<str>,
    descriptor: &Arc<str>,
) -> bool {
    if !cratonvm_jit::deopt_real_enabled() || cached.deopt_points.is_empty() {
        return false;
    }
    let method_key = format!("{class_name}.{method_name}:{descriptor}");
    if cached.compilation_epoch >= shared.compilation_epoch_for(&method_key) {
        return false;
    }
    let registry = &shared.jit.despec_registry;
    if registry.count_for(&method_key) == 0 {
        return false;
    }
    let keeps_withdrawn_guard = cached.deopt_points.iter().any(|p| {
        p.reason != cratonvm_jit::deopt::DeoptReason::OsrExit
            && registry.contains_speculation(&method_key, p.bci, p.speculation_id)
    });
    if !keeps_withdrawn_guard {
        return false;
    }
    let skip_key = (
    ...
    !shared.jit.jit_skip_set.read().contains(&skip_key)
}

/// `CRATONVM_JIT_OSR_GUARD_EXIT_CHARGE`, **default ON** (`=0` opts out): charge
/// an OSR body's speculative-guard exit to the deopt ledger, as every other
/// guard trap is (round 9 wave 11, lane `osr11`). See
/// [`charge_osr_guard_exit`].
fn osr_guard_exit_charge_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_OSR_GUARD_EXIT_CHARGE")
    })
}

/// The speculative guard an OSR body left through at `bci`, when the exit can
/// be attributed to exactly one (round 9 wave 11, lane `osr11`).
///
/// The reconstructed frame carries a bci and no reason, and a loop header
/// carries TWO points: the speculative-BCE guard (`BoundsCheck`) and the
/// OSR-exit map (`OsrExit`). `osr_exit::deopt_reason_at_bci` sees both and
/// answers `Ambiguous`; the first-in-list pick the first-call tier-up sink
/// uses answers `OsrExit`. Neither is what fired. An `OsrExit` map is never an
/// exit source in production: its stub (reason 7) is jumped to only by the
/// test triggers (`CRATONVM_OSR_EXIT_TEST` / `CRATONVM_OSR_EXIT_AFTER`), which
/// [`charge_osr_guard_exit`] refuses to run under. So the `OsrExit` maps are
/// skipped here, as are the exceptional (`RETHROW`) frames, which go to a
/// different stash.
///
/// Only the speculations the single-pass emitter drops once de-spec'd are
/// answered: `BoundsCheck` (the loop-header BCE guard, `x64/driver.rs`) and
/// `ReceiverTypeChanged` (the call-site type guards, `x64/op_invoke.rs`). A
/// guard the recompile would re-emit whatever the registry says must not be
/// charged per exit, or the charges alone would retire the method. Anything
/// else -- the invokedynamic trap's `UnreachedCode` above all, whose policy is
/// "give up on the first" -- keeps the old uncharged behaviour.
fn osr_guard_exit_reason(
    deopt_points: &[cratonvm_jit::deopt::DeoptimizationPoint],
    bci: u32,
) -> Option<cratonvm_jit::deopt::DeoptReason> {
    use cratonvm_jit::deopt::DeoptReason;
    unique_guard_reason_at_bci(deopt_points, bci).filter(|r| {
        matches!(
            r,
            DeoptReason::BoundsCheck | DeoptReason::ReceiverTypeChanged
        )
    })
}

/// The one reason the non-`OsrExit`, non-`RETHROW` points at `bci` agree on,
/// or `None` when there is none or they disagree. See
/// [`osr_guard_exit_reason`] for why the `OsrExit` maps are skipped.
fn unique_guard_reason_at_bci(
    deopt_points: &[cratonvm_jit::deopt::DeoptimizationPoint],
    bci: u32,
) -> Option<cratonvm_jit::deopt::DeoptReason> {
    use cratonvm_jit::deopt::DeoptReason;
    ...
    for p in deopt_points {
        if p.bci != bci || p.semantics.rethrow_exception || p.reason == DeoptReason::OsrExit {
            continue;
        }
        match found {
...
/// The reason to charge for an ordinary stashed frame from a compiled body,
/// with the loop-header `OsrExit` map discounted (round 9 wave 11, lane
/// `osr11`).
///
/// For the sinks that still spell this as a first-in-list
/// `deopt_points.iter().find(|dp| dp.bci == rframe.bci)` -- the first-call
/// tier-up sink in `interpreter.rs` -- a loop-header BCE guard's trap was
/// charged as `OsrExit`, because the exit map at that bci is recorded first.
/// That is the wrong policy twice over: `OsrExit` + `Reinterpret` does not
/// evict the trapping body, so every later call re-entered the same guard
/// until `MakeNotCompilable` (`TryLoopRefl`: 11 such traps, then 1 490 fully
/// interpreted calls), and the per-bci de-spec recorded the `OsrExit`
/// speculation id, which the OSR door's staleness check
/// ([`cached_osr_body_is_despec_stale`]) never matches against the guard.
///
/// Outside the OSR-exit test triggers an `OsrExit` map is never an exit source
/// (see [`osr_guard_exit_reason`]), so the other points at the bci decide;
/// when they do not (none, or disagreeing), this is exactly
/// `deopt_resume::despec_reason_for`.
pub(crate) fn guard_reason_for_trapped_frame(
    compiled: &crate::jit::CompiledMethod,
    rframe: &cratonvm_jit::deopt::ReconstructedFrame,
) -> cratonvm_jit::deopt::DeoptReason {
    if !cratonvm_jit::osr_exit_test_enabled() && cratonvm_jit::osr_exit_after().is_none() {
        if let Some(r) = unique_guard_reason_at_bci(&compiled.deopt_points, rframe.bci) {
            return r;
        }
    }
    crate::runtime::interpreter::despec_reason_for(compiled, rframe)
}

/// Charge a speculative-guard exit out of an OSR body, and let the per-bci
/// de-spec withdraw the guard (round 9 wave 11, lane `osr11`;
/// `evicted-callee-stays-reachable-through-a-compiled-callers-binding-FIXED-20260918.md`).
///
/// The OSR-exit TRANSFER in [`try_osr`] used to charge NOTHING. So an OSR body
/// whose loop-header speculative-BCE guard fails on every entry
/// (`TryLoop.sumThrow(a, a.length + 5)`: `n > a.length`, so the guard exits at
/// once) was reused forever: 5 entries per call, each an immediate exit, and
/// the rest of the 65 541-iteration loop interpreted -- 2 550 `OSR-reuse` +
/// `OSR-exit TRANSFER` pairs on `TryLoopRefl` (8.5 s against HotSpot's 16 ms),
/// and the same ~500 calls before `TryLoop throw` finally de-spec'd through its
/// method-entry trap (the remaining 2.4 s). Wave 10's
/// [`cached_osr_body_is_despec_stale`] could never fire: the ledger never
/// heard of these exits, so neither the epoch nor the registry moved.
///
/// Now the exit goes through the same policy a trapped method-entry body gets
/// (`helpers::despeculate_trapped_method`): charged under the guard's own
/// reason at its bci (evicting the body on the count-based schedule), and
/// after `PER_BCI_DESPEC_LIMIT` (4) charges at that bci the speculation is
/// recorded in the de-spec registry, so the next OSR compile (and the next
/// method-entry compile) drops the guard and keeps per-access bounds checks.
///
/// Called only AFTER a successful transfer: the frame is already parked at its
/// resume bci, and nothing here touches it. Not run for an inlined caller
/// chain (the bci would be a callee's), the re-run sentinel, or under the
/// OSR-exit / eager-deopt TEST triggers, whose exits come from `OsrExit` maps
/// or synthetic reason-2 jumps rather than from a failed speculation.
#[inline(never)]
fn charge_osr_guard_exit(
    shared: &SharedVm,
    class_id: ClassId,
    class_name: &str,
    method_name: &str,
    ...
    if !osr_guard_exit_charge_enabled()
        || rframe.bci == u32::MAX
        || !rframe.caller_frames.is_empty()
        || cratonvm_jit::osr_exit_test_enabled()
        || cratonvm_jit::osr_exit_after().is_some()
        || cratonvm_jit::deopt_eager_enabled()
        || cratonvm_jit::deopt_eager_bci_override().is_some()
    {
        return;
    }
    let Some(reason) = osr_guard_exit_reason(&compiled.deopt_points, rframe.bci) else {
        return;
    };
    if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_DEOPT") {
        eprintln!(
            "[cratonvm-deopt] OSR guard exit charged: {class_name}.{method_name}{descriptor} \
             reason={reason:?} bci={}",
            rframe.bci
        );
    }
...
/// `CRATONVM_JIT_OSR_REENTRY_MEMO`, **default ON** (`=0` opts out): let a
/// re-entry at the pc this thread last OSR-entered skip the door's gate chain
/// while nothing it depends on has moved. See [`OsrReentryMemo`].
fn osr_reentry_memo_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_OSR_REENTRY_MEMO")
    })
}

/// The last single-pass OSR artifact this thread obtained from
/// [`compile_osr_artifact`], and what its answer depended on (round 9 wave 11,
/// lane `osr11`; `perf-implicit-exceptions-in-compiled-code-cost-microseconds-each-FIXED-20260920.md`).
///
/// # Why
///
/// A caught exception in an OSR'd loop is an exception-exit to the
/// interpreter's handler, then a back edge, then a fresh `try_osr` -- one
/// full trip through the door per exception (`ExcLoop.local`: 198 502 OSR
/// entries for 198 500 exceptions). The trip redid, every time, three
/// `String` and three `Arc<str>` allocations, the optimizing tier's
/// mutex-guarded refusal probe, the synchronized-method scan under the
/// class-manager lock, the native-registry probes, the OSR denial probe,
/// `JitCache::get_osr`, and the de-spec staleness check (a `format!` and two
/// map reads) -- to arrive at the artifact it had just left. Sampled on the
/// w10 binary (`ExcLocal`, fast throw on): `try_osr` self 9.7 %,
/// `compile_osr_artifact` 4.1 %, `HashMap<str>::contains_key` 3.4 %,
/// `get_osr` 2.2 %, `compilation_epoch_for` 1.2 %, `mi_malloc` 5.2 %.
///
/// # Why it is exact
///
/// The door's answer for `(this VM, this bytecode, this pc)` is a function of:
///
/// * static facts -- `CRATONVM_DISABLE_JIT` / the bisect levers, the method's
///   `ACC_SYNCHRONIZED` bit, a registered native shadow, the gpu-offload gate:
///   none can change for a method whose loop is running as bytecode;
/// * the JIT cache's contents (`get_osr`): every `put`, `put_osr` and removing
///   invalidation advances [`cratonvm_jit::JitCache::generation`], which is
///   stamped here, and the artifact's own `retired` flag is asked too;
/// * the deopt ledger (the de-spec staleness check reads the method's epoch,
///   the de-spec registry and the not-compilable set): all three move only
///   inside `DeoptimizationController::deoptimize`, which records a deopt
///   first, so the ledger's lifetime `total_deopts` is stamped;
/// * the OSR denial set: asked again on every hit (one read-locked probe);
/// * the optimizing tier's door in `try_osr`: an entry is memoised only when
///   that door answered `None`, which it records as a permanent refusal for
///   the pc (or the tier is off), so it would answer `None` again.
///
/// A hit therefore returns the artifact the full trip would have returned.
/// Held as a `Weak`: the memo must not keep a withdrawn body's code alive.
/// One entry per thread -- the hot loop -- keyed by the VM's identity, the
/// frame's bytecode identity, the class id, the method name/descriptor and the
/// pc.
///
/// `vm` is `SharedVm::vm_identity` (a monotonic counter), not the `SharedVm`'s
/// raw address. A destroyed VM's allocation can be reused by a later one on
/// the same thread -- `vm_init.rs`'s own history names the shape exactly:
/// "the two VMs traded heap objects and the reader segfaulted" was an
/// address-keyed side table, fixed by moving it to `vm_identity`. This memo
/// would have the same failure with the address: a fresh VM B started on a
/// thread that had memoised an entry for a destroyed VM A, running the same
/// class (very plausible -- a host that destroys and recreates an identical
/// embedded VM), could match `code`/`class_id`/`method_name`/`descriptor`/
/// `entry_pc` on coincidence and `generation`/`deopts` on coincidence (both
/// start near zero for a fresh VM), and hand B's OSR entry a `CompiledMethod`
/// built against A's heap and class layout. `Weak::upgrade` rules out a
/// dangling pointer, not a live artifact that answers for the wrong VM.
struct OsrReentryMemo {
    vm: usize,
    code: usize,
    class_id: ClassId,
    method_name: Arc<str>,
    ...
    generation: u64,
    deopts: u64,
    artifact: std::sync::Weak<cratonvm_jit::CompiledMethod>,
}

thread_local! {
    /// See [`OsrReentryMemo`].
    static OSR_REENTRY_MEMO: std::cell::RefCell<Option<OsrReentryMemo>> =
        const { std::cell::RefCell::new(None) };
}

/// What an [`OsrReentryMemo`] entry is stamped with: the JIT cache's
/// generation and the deopt ledger's lifetime count, read BEFORE the lookup the
/// entry memoises, so a change racing the lookup can only cause a miss.
/// `None` when the memo is off.
fn osr_reentry_stamp(shared: &SharedVm) -> Option<(u64, u64)> {
    if !osr_reentry_memo_enabled() {
        return None;
    }
    let generation = shared.jit.jit_cache.generation();
    let deopts = shared.jit.deopt_log.lock().total_deopts();
    Some((generation, deopts))
}

/// The memoised artifact for this entry, when every stamp still holds.
#[allow(clippy::too_many_arguments)]
#[inline(never)]
fn osr_reentry_memo_get(
    shared: &SharedVm,
    stamp: (u64, u64),
    code: usize,
    class_id: ClassId,
    ...
    let artifact = OSR_REENTRY_MEMO.with(|m| -> Option<Arc<cratonvm_jit::CompiledMethod>> {
        let guard = m.try_borrow().ok()?;
        let memo = guard.as_ref()?;
        if memo.vm != vm
            || memo.code != code
            || memo.class_id != class_id
            || memo.entry_pc != entry_pc
            || (memo.generation, memo.deopts) != stamp
            || *memo.method_name != **method_name
            || *memo.descriptor != **descriptor
        {
            return None;
        }
        memo.artifact.upgrade()
    })?;
    if artifact.retired.load(std::sync::atomic::Ordering::Acquire)
        || !artifact.can_osr_enter(entry_pc)
    {
        return None;
    }
...
/// Remember the single-pass artifact the door just answered with. Only a body
/// the door would REUSE next time is kept (an OSR-produced body that can enter
/// here); anything else empties the memo.
#[allow(clippy::too_many_arguments)]
#[inline(never)]
fn osr_reentry_memo_put(
    shared: &SharedVm,
    stamp: (u64, u64),
    code: usize,
    class_id: ClassId,
    ...
    let entry = if artifact.compiled_via_osr && artifact.can_osr_enter(entry_pc) {
        Some(OsrReentryMemo {
...
/// `CRATONVM_DBG_JITC`: name a memo hit (the full trip prints `OSR-reuse`).
/// Out of line and cold, like `dbg_osr_recompile_reason`: `try_osr`'s frame
/// must not carry the formatting temporaries.
#[inline(never)]
#[cold]
fn dbg_osr_reentry_memo_hit(
    c: &cratonvm_jit::CompiledMethod,
    class_name: &str,
    method_name: &str,
    method_descriptor: &str,
    entry_pc: usize,
    ...
    eprintln!(
        "[cratonvm-jitc] OSR-reuse(memo) {class_name}.{method_name}{method_descriptor} \
         entry_pc={entry_pc} entry={:p} len={}",
        c.entry_ptr(),
        c.code_bytes().len()
    );
}

/// Forget this thread's [`OsrReentryMemo`] entry -- the optimizing tier's door
/// answered instead, and a later hit must not route around it.
fn osr_reentry_memo_clear() {
    OSR_REENTRY_MEMO.with(|m| {
        if let Ok(mut slot) = m.try_borrow_mut() {
            *slot = None;
        }
...
pub(super) fn compile_osr_artifact(
    ...
    // reuse kicks in.
    //
    // Round 9 wave 10 (`call10`): NOT a cached body that still carries a
    // speculative guard this VM has since de-speculated
    // ([`cached_osr_body_is_despec_stale`]). Reusing one re-entered the
    // failing guard on every OSR trigger -- `TryLoop.sumThrow`'s loop-header
    // BCE guard, an immediate OSR exit each time, until the aggregate count
    // made the method not-compilable and every later call interpreted its
    // 65 541-iteration loop (8 s against HotSpot's 16 ms).
    let despec_stale = matches!(&cached_osr, Some(c) if c.compiled_via_osr
    && c.can_osr_enter(entry_pc)
    && cached_osr_body_is_despec_stale(
        shared,
        c,
        &class_name_arc,
        &method_name_arc,
    ...
    let compiled = if osr_reused {
        ...
        // Stage: compile the OSR body. See `compile_osr_body`.
        compile_osr_body(
            shared,
            class_id,
            &class_name,
...
/// The compile half of [`compile_osr_artifact`]: resolve the OSR body's
/// per-site tables, run the single-pass backend and publish the artifact.
///
/// Entered only after the door's gates passed and no reusable OSR artifact was
/// cached. `None` refuses the OSR compile; every refusal inside records its own
/// reason, exactly as when this was the door's inline closure. The inputs are
/// the door's own parameters plus what its gates derived, so the stage sees
/// nothing else.
...
fn compile_osr_body(
    ...
    max_locals: usize,
    entry_pc: usize,
    class_name_arc: &Arc<str>,
    method_name_arc: &Arc<str>,
    descriptor_arc: &Arc<str>,
    method_name_check: &str,
    osr_key: &crate::jit::tiered::MethodKey,
) -> Option<Arc<crate::jit::CompiledMethod>> {
    // ── The admission gate ────────────────────────────────────────
    //
    ...
    let admission = match cratonvm_jit::compile_gate::admit(
        ...
        // M7 — handed over even though the OSR door never raises
        // `RuntimeDespeculated`. Which doors honour the verdict is the GATE's
        // decision and is argued in one place; a caller that withheld the
        // registry here would be making that decision again, silently, and the
        // two copies would drift the way every other hand-copied check in this
        // list did.
        Some(&shared.jit.runtime_despec),
    ) {
        Ok(a) => a,
        Err(reason) => {
    ...
    // `H20-1`'s other half, landed with `H12-1` O1/O2 on
    // 2026-09-22: say which policy this door's direct-call
    // ladder obeys, so `undeclared_direct_bind_rows` can tell a
    // door that refused from a door that never asked. Before
    // this, `--jdk-only` and `--real-jdk` measured an IDENTICAL
    // per-door bind profile and nothing in the tree could say
    // why (`H12-1` §3b).
    //
    // Declaring is not deciding. The decision is
    // `helpers::admit_direct_native_entry`'s, at each bind site,
    // because that is the only place a refusal can fall through
    // to the `invoke_info` build instead of stranding the pc.
    admission.declare_direct_call_policy(
        cratonvm_jit::compile_gate::DirectCallPolicy::from_jdk_only(
    ...
    //
    // INERT *in this function*, and deliberately so:
    // `compile_osr_body` reaches `x64::compile_with_param_slots` and has
    // no promotion to refuse. Read the scope carefully — it is the
    // FUNCTION that is single-pass only, not the OSR door. `try_osr`,
    // the caller two frames up, asks `compile_optimizing_artifact`
    // FIRST and enters an optimizing body through
    // `CompiledMethod::ir_osr_enter` when one carries a stub for this
    // bci and is `ir_osr_sentinel_free`; this function is what runs
    // when that declines. (This comment read "this door ... is
    // single-pass only" until 2026-09-16, which a reader takes as a
    // statement about OSR rather than about one stage of it.)
    //
    ...
    if crate::runtime::env_cache::dbg_jitc() {
        eprintln!(
            "[cratonvm-jitc] osr ir-eligibility: {} for {}.{}{} -- INERT in this STAGE, which is the single-pass fallback; the optimizing route is tried by try_osr before this runs",
    ...
    if !scan.typecheck_ops.is_empty() {
        ...
        for &(pc, cp_idx) in &scan.typecheck_ops {
            ...
            let target_id = cm_lock
                ...
                .map(|id| id.as_u32());
            // Round 9 wave 8 (`arraylist8`): the same finality verdict the
            // method-entry door asks through
            // `CompileRequest::class_is_final_resolver`, from the lock already
            // held, so the inline `instanceof` final-class miss fires at this
            // door too (a bind at one door is not a bind).
            let target_is_final = target_id
                .is_some_and(|id| jit_class_is_final_for_typecheck_in(shared, &cm_lock, id));
            let (ptr, len) =
                cratonvm_jit::intern_typecheck_target_with_finality(cn, target_id, target_is_final);
    ...
    // Volatile `getfield`/`putfield` pcs: the backend fences every volatile
    // store (`BackendRequest::volatile_field_pcs`, staged below). This door
    // calls the backend directly, so it has to collect them itself, as the
    // method-entry doors' `CompileRequest::cp_field_volatile_resolver` does
    // (`volatile-instance-fields-are-plain-accesses-in-compiled-code`).
    let mut osr_volatile_field_pcs: std::collections::HashSet<usize> =
    ...
    if !scan.field_ops.is_empty() {
        for &(pc, cp_idx) in &scan.field_ops {
            ...
            if field.is_volatile {
                osr_volatile_field_pcs.insert(pc);
            }
            if compact_fields {
    ...
    //
    // FIX (jdk-only-string-indexof-int-int-miscompiles-in-the-single-pass-tier):
    // `(entry, artifact id AT BAKE TIME)`, not a bare entry. Keeping the
    // callee mapped is only half of what publication has to check. The other
    // half is IDENTITY: `prepare_for_publication` compares this recorded id
    // against whoever owns the address when the body is finally published, and
    // refuses when they differ, because an executable buffer freed after emit
    // is handed straight back out to the next compile. With no id recorded,
    // `expected_of` answered `None` for every OSR-baked call and that check
    // degraded to the liveness-only test a recycled buffer passes: the body
    // published a raw CALL into a method it never resolved, entered with the
    // ABI of the one it did. `String.indexOf(II)I` calling
    // `StringLatin1.indexOf([BIII)I` context-free and landing in a
    // context-taking tenant is how it was caught.
    let mut osr_direct_callee_entries: Vec<(usize, u64)> = Vec::new();
    ...
    if !scan.invoke_ops.is_empty() {
        ...
        for &(pc, cp_idx, opcode) in &scan.invoke_ops {
            ...
            //
            // An `invokeinterface` (kind 2) whose resolved method is a PRIVATE
            // interface method is pinned the same way (JVMS §6.5 selection
            // step 1): it selects that method, never the receiver's public
            // namesake.
            ...
            if matches!(invoke_kind, 0 | 2) {
                if let Some((entry, num_params, ret, guard_class_id)) =
                    ...
                    .filter(|&(entry, _, _, guard_class_id)| {
                        ...
                        // M4 — ask about THIS speculation, not about the
                        // bytecode index. The guard this decides on takes the
                        // `ReceiverTypeChanged` deopt edge
                        // (`x64/op_invoke.rs`), and a protected invoke also
                        // publishes a `PendingException` frame at the SAME bci.
                        // Under the old per-bci key, four traps on the
                        // exceptional frame withdrew this guard too — a
                        // speculation that had never failed, on evidence about
                        // a different one. A wildcard entry
                        ...
                        let by_despec = shared.jit.despec_registry.contains_speculation(
                            &osr_despec_key,
                            pc as u32,
                            cratonvm_jit::deopt::speculation_id(
                                pc as u32,
            ...
            {
                // `H12-1` O1 (2026-09-22): the entry address comes from
                // `admit_direct_native_entry`, which asks the registry
                // what `vm_exec::invoke_or_native` would do with this
                // triple and hands one out only if the answer is "run
                // the native". A `None` FALLS THROUGH -- it must NOT
                // `continue`, or the pc loses its `invoke_info` and the
                // backend compiles the site as a call to the enclosing
                // method. See the H20-1 module block in `jit::helpers`
                // for the backend's own statement of that rule, and for
                // why the policy token rides on the address.
                //
                // That address is still the ITEM's, not the
                // `*_DIRECT_FN` cell's, which is what every arm at this
                // door used to say for itself: `build_helpers` registers
                // those cells only AFTER this construction block, so a
                // cell read here yields 0 on the first OSR compile in a
                // process and the bind is silently skipped. The reason
                // now lives once, on `DirectNativeShadow::entry_addr`.
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::ThreadCurrentThread,
                ) {
            ...
            //
            // Through `admit_direct_native_entry` since `H12-1` O1, like
            // every other shadowing bind at this door.
            if invoke_kind == 3
                ...
                && desc == "(IILjava/util/function/BiFunction;)I"
            {
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::PreconditionsCheckIndex,
                ) {
            ...
            // every other shadowing bind at this door.
            //
            // No `jdk_only` TERM here, and there never was one, because
            // the bind now carries the question instead of spelling it:
            // `admit_direct_native_entry` asks both the policy this door
            // declared and the one `shared` is actually executing under,
            // so a declaration that drifts cannot widen the gate. Belt
            // and braces with
            // the callee side, which is the half that used to carry it
            // alone — each helper asks
            // `jit_direct_helper_refused` on its own fast path, with its
            // own gate, and a refusal declines to the generic dispatcher,
            // which is policy-checked. Since 2026-09-22 that guard asks
            // whether the generic dispatcher would ACTUALLY refuse, rather
            // than refusing every strict-mode call and routing it to a tail
            // that ran the same native anyway — see
            // `helpers::JIT_DIRECT_HELPER_JDK_ONLY_REFUSALS`.
            //
            ...
            {
                let is_put = mn == "put";
                // `for_write` is the choice between two registered triples, so
                // it picks the shadow as well as the helper -- the same split
                // `NIO_BYTEBUFFER_{PUT,GET}_BYTE_GATE` makes on the callee side.
                let shadow = if is_put {
                    crate::jit::helpers::DirectNativeShadow::ByteBufferPutByte
                } else {
                    crate::jit::helpers::DirectNativeShadow::ByteBufferGetByte
                };
                if let Some(entry) =
                    crate::jit::helpers::admit_direct_native_entry(shared, &admission, shadow)
                {
                ...
                && cratonvm_jit::is_buffer_session_site(mn, desc)
            {
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::BufferSession,
                ) {
            ...
            if invoke_kind == 0 && cratonvm_jit::scoped_memory_direct_enabled() {
                if let Some(op) = cratonvm_jit::is_scoped_memory_op(target_class, mn, desc) {
                    // Sixteen hand-written `as *const () as usize` arms until
                    // 2026-09-22 (`H12-1` O1). The op-to-shadow map lives beside
                    // the enumeration and is round-tripped through
                    // `is_scoped_memory_op`, so the jit crate's recognition and
                    // the VM crate's policy identity cannot drift.
                    let shadow =
                        crate::jit::helpers::DirectNativeShadow::for_scoped_memory_op(op);
                    if let Some(entry) =
                        crate::jit::helpers::admit_direct_native_entry(shared, &admission, shadow)
                    {
                        cratonvm_jit::SCOPED_MEMORY_SITES_OSR
                            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        direct_calls2.push((
                            pc,
                            crate::jit::JitDirectCall {
            ...
            // Plain `Unsafe.get/put{Byte,Short,Int,Long}(Object,long)` at
            // this door too. `jdk/internal/misc/Unsafe`'s natives have no
            // bytecode, so §1.4 and §7 step 3 have nothing to say about
            // these eight on the native-shadow policy question -- but
            // `capability::classify_native` maps the whole class to
            // `RawMemory`, and the eight now carry that on their own
            // `DirectHelperGate`s (`jit_direct_helper_refused` refuses them
            // unconditionally of `--jdk-only` / `--compatible`), so binding
            // through the same enumerated path as everything else below
            // costs nothing this door was not already paying and closes the
            // last raw address-takes `direct_bind_source_witness` could not
            // see. See
            ...
            if invoke_kind == 0 && cratonvm_jit::unsafe_accessor_direct_enabled() {
                if let Some(op) = cratonvm_jit::is_unsafe_accessor_op(target_class, mn, desc) {
                    let shadow = crate::jit::helpers::DirectNativeShadow::for_unsafe_accessor_op(op);
                    if let Some(entry) =
                        crate::jit::helpers::admit_direct_native_entry(shared, &admission, shadow)
                    {
                        cratonvm_jit::UNSAFE_ACCESSOR_SITES_OSR
                            .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        direct_calls2.push((
                            pc,
                            crate::jit::JitDirectCall {
                ...
                && desc == "(B)V"
            {
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::MessageDigestUpdateByte,
                ) {
                ...
                && desc == "(Ljava/lang/Object;)V"
            {
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::ReachabilityFence,
                ) {
            ...
            {
                // `H12-1` O1. `Integer.valueOf` is `intrinsic`, so §1.4's
                // reviewed exception admits it in both modes and this bind
                // does not move — which is the point of asking rather than
                // assuming: the answer is now a measured `Some`, not an
                // omission that happens to be right.
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::IntegerValueOf,
                ) {
                    direct_calls2.push((
                        pc,
                        crate::jit::JitDirectCall {
                ...
                && desc == "(J)Ljava/lang/Long;"
            {
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::LongValueOf,
                ) {
                ...
                && desc == "()I"
            {
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::IntegerIntValue,
                ) {
                ...
                && desc == "()J"
            {
                if let Some(entry) = crate::jit::helpers::admit_direct_native_entry(
                    shared,
                    &admission,
                    crate::jit::helpers::DirectNativeShadow::LongLongValue,
                ) {
                    cratonvm_jit::LONG_LONG_VALUE_SITES
                        .fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    // The BIND-vs-EXECUTION triple's half, which
                    // `vm/src/runtime/interp_census.rs` prints as `sites_bound=`
                    // under `CRATONVM_DBG=direct-binds`. Two counters for one bind
                    // is not redundancy — `LONG_LONG_VALUE_SITES` belongs to the
                    // BOXING pair `tiered.rs` prints beside `Long.valueOf`, and
                    // this one belongs to the bind/served/declined triple — and
                    // until 2026-09-22 only the first was fed from this door, so
                    // `sites_bound` read LOWER than the `tiered.rs` figure for the
                    // same quantity. Exactly as the `Integer.intValue` arm above
                    // calls `note_integer_int_value_direct_site()`.
                    // (`docs/internal/fixed-bugs/r10-readers-orphaned-jit-accessors-FIXED-20260922.md`)
                    cratonvm_jit::note_long_long_value_direct_site();
                    direct_calls2.push((
                        pc,
                        crate::jit::JitDirectCall {
                            entry,
            ...
            if invoke_kind == 0 && target_class == "java/util/HashMap" {
                ...
                {
                    Some((
                        crate::jit::helpers::DirectNativeShadow::HashMapPut,
                        2usize,
                        &cratonvm_jit::HASHMAP_PUT_DIRECT_SITES_OSR,
                    ))
                } else if mn == "get" && desc == "(Ljava/lang/Object;)Ljava/lang/Object;" {
                    Some((
                        crate::jit::helpers::DirectNativeShadow::HashMapGet,
                        1usize,
                        &cratonvm_jit::HASHMAP_GET_DIRECT_SITES_OSR,
                    ))
                } else {
                    None
                };
                if let Some((shadow, num_params, sites)) = recognized {
                    if let Some(entry) =
                        crate::jit::helpers::admit_direct_native_entry(shared, &admission, shadow)
                    {
                        // `H12-1` N2: these two binds had no counter at all, and
                        // that is why `H7-1` §6c's prediction about
                        // `collection_direct_helper_sites()` was unfalsifiable --
                        // the counters it named belong to the single-pass door and
                        // cannot see this one. One relaxed add per compile.
                        sites.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                        direct_calls2.push((
                            pc,
                            crate::jit::JitDirectCall {
                                entry,
    ...
    // Round 9 wave 9 (`evict9`): retire cells planned by `retirable_direct_call`;
    // the artifact owns them (moved into `cm._jit_cycle_cells` below).
    let mut osr_retire_cells: Vec<std::sync::Arc<cratonvm_jit::JitCycleEdgeCell>> = Vec::new();
        ...
        pending_callee_compiles
    {
        ...
        if let Some((callee_pin, entry, needs_ctx)) = compiled_callee {
            ...
            if !refuse_dispatch && refuse_handlers.is_none() && !refuse_indy {
                ...
                // Round 9 wave 9 (`evict9`): an in-loop `invokestatic` bind
                // goes through a retire cell under `CRATONVM_JIT_RETIRE_CELL=1`,
                // so a withdrawn callee is not re-entered by this (long-running)
                // OSR frame; otherwise this is the plain row, unchanged.
                // `num_params` is receiver-EXCLUDED, as before.
                direct_calls2.push((
                    ipc,
                    cratonvm_jit::retirable_direct_call(
                        code,
                        code_len,
                        ipc,
                        site_invoke_kind,
                        ...
                        needs_ctx,
                        param_count,
                        crate::jit::return_type(&callee_desc),
                        &callee_class,
                        &callee_method,
                        &callee_desc,
                        &mut osr_retire_cells,
                    ),
                ));
                ...
                // `baked_callee_pins` covers bind-to-publication (it is
                // declared above the bind loop and dropped after
                // `put_osr`), which is what keeps THIS tier's baked
                // addresses meaning the same method at publication as
                // they did at bind. The method-entry tiers had no such
                // pin until 2026-09-21 — their bind closure returned a
                // raw address and dropped its `Arc` on the way out — and
                // that is the whole of
                // `jdk-only-string-indexof-int-int-miscompiles-in-the-single-pass-tier`.
                // Do not narrow this to the emit window.
                osr_direct_callee_entries.push((entry, cratonvm_jit::jit_entry_artifact_id(entry)));
    ...
    // GC-maintained `ldc` constant slots (ldc7 request H, wired round 9
    // wave 7b). No class-manager guard is held here (the ldc scan's ended
    // with its block), which `compiled_ldc_slot_for` requires. A `0` answer
    // (kill switch, redefinition, unresolved site) leaves the site on its
    // helper CALL.
    for &(_, holder, cp_idx) in ldc_string_info2.iter().chain(ldc_class_info2.iter()) {
        let slot_addr =
            crate::jit::helpers::compiled_ldc_slot_for(shared, ClassId::new(holder), cp_idx);
        if slot_addr != 0 {
            backend.ldc_slots.insert((holder, cp_idx), slot_addr);
        }
    }
    ...
    backend.exception_ranges = osr_exception_table
        ...
        .collect();
    // The StoreLoad fence after each volatile `putfield`; collected with
    // `field_info` above.
    backend.volatile_field_pcs = osr_volatile_field_pcs;
    ...
    let Some(mut cm) = cm else {
        ...
        // r10 lane `obs`: the ONLY site that can raise this row. `OSR_EVENTS`
        // has carried `osr_compile_declined` — with a doc comment describing
        // precisely this arm, "the OSR compile produced no artifact at all" —
        // while nothing in the tree incremented it, so `osr_counts()` reported
        // zero declines forever and read as "this never happens". That is the
        // failure mode a mute instrument has: it does not look broken, it looks
        // like evidence.
        cratonvm_jit::metrics::record_osr_event("osr_compile_declined");
        return None;
    };
    ...
    // By entry, and deduped by entry alone: the same address baked twice in
    // one compile is one pin and one identity. Two DIFFERENT ids for one
    // address would mean the buffer was recycled mid-compile, and keeping the
    // first makes publication refuse — which is the outcome we want.
    osr_direct_callee_entries.sort_unstable_by_key(|(entry, _)| *entry);
    osr_direct_callee_entries.dedup_by_key(|(entry, _)| *entry);
    cm._direct_callee_entries = osr_direct_callee_entries.iter().map(|(e, _)| *e).collect();
    cm._direct_callee_expected = osr_direct_callee_entries;
    ...
    cm._jit_cycle_cells.extend(osr_retire_cells);
...
/// `CRATONVM_JIT_OSR_PREFER_SINGLE_PASS`, **default ON** again since round 9
/// wave 6 (`=0` opts out): let the OSR door keep the single-pass body for a
/// method whose optimizing OSR body is not known to be faster (see
/// [`osr_optimizing_not_known_better`]).
///
/// # Why it went back on (round 9 wave 6, lane irlower6)
///
/// Wave 5 made the SINGLE-PASS OSR body register-resident and unrolled
/// (`ArithOsr.mulOnly`: 13 register-only instructions per iteration, two
/// iterations per trip) and the optimizing body did not move, so wave 3's
/// comparison below reversed. Measured on the wave-5 binary
/// (`cratonvm-jitr9-w5b.exe`), `=0` (optimizing) vs `=1` (single-pass), 1e9
/// iterations, two runs each, interleaved:
///
/// | `ArithOsr` loop | optimizing | single-pass |
/// |---|---:|---:|
/// | `mul` (`lcmp`) | 1 709 / 1 720 ms | 790 / 762 ms |
/// | `div` | 1 241 / 1 166 ms | 868 / 844 ms |
/// | `rem` | 1 428 / 1 440 ms | 1 160 / 1 128 ms |
/// | `full` | 1 908 / 1 784 ms | 1 526 / 1 560 ms |
/// | `int` (`idiv`/`irem`, no `lcmp`) | 1 942 / 1 982 ms | 2 032 / 2 038 ms |
///
/// and `CratonBench arithmetic` 4 420 / 4 029 / 3 970 ms optimizing against
/// 3 400 / 3 056 / 3 197 single-pass; `matrix` and `sieve` (no gated opcode)
/// were a wash (2 651/2 833, 2 660/2 638, 2 708/2 712; 7 148/7 258,
/// 7 458/7 445, 7 471/7 416). The `int` row is why the predicate no longer
/// names `idiv` / `irem`: there the optimizing body is the (slightly) faster
/// one.
///
/// # Re-measured round 9 wave 12 (2026-09-20), and the default stands
///
/// The wave-5 table above is kept for the history; it is four binaries old and
/// its absolute numbers are no longer the ones this flag is chosen on. On a
/// `dev` build (`00cc38bea`), 1e9 iterations, interleaved ABC-CBA, three
/// rounds, minimum of six runs per cell (the host carried load 4-7):
///
/// | `ArithOsr` loop | `=1`, single-pass (default) | `=0`, optimizing | HotSpot 21 |
/// |---|---:|---:|---:|
/// | `mul` (`lcmp`) | **393 ms** | 418 ms | 350 ms |
/// | `div` | **511 ms** | 904 ms | 450 ms |
/// | `rem` | **816 ms** | 920 ms | 772 ms |
/// | `full` | **1 066 ms** | 1 237 ms | 964 ms |
/// | `int` (no `lcmp`) | 1 384 ms | **1 401 ms** (a wash) | 1 095 ms |
///
/// and `ArithProbe` (`CratonBench.benchArithmetic`'s loop, 2e9) 3 007 / 3 304 /
/// 3 393 ms on the default against HotSpot's 2 394 / 2 518 / 2 518 and
/// 4 019 / 3 758 / 3 755 with `=0`. The ordering the wave-6 table established
/// holds on every row, so the default stays ON. See
/// `docs/internal/fixed-bugs/perf-long-counted-loops-lose-the-optimizing-osr-body-FIXED-20260920.md`
/// for what the remaining difference is (the single-pass OSR tier's 2x unroll
/// and its in-place carried-value update, not instruction selection: a loop as
/// simple as `s += i * 3` measures 289 ms single-pass against 451 optimizing).
///
/// `mul` was also the one shape whose optimizing body never fused its `lcmp`
/// (the compare's clobber test refused it; fixed this wave in
/// `ir_lower::prepare_fusion_tables`). Re-measure `mul` with `=0` after that
/// build; the other rows already had the fused compare and are unaffected.
///
/// # History
///
/// It landed default-ON in round 9 wave 3 as the stop-gap for the table below,
/// and went default-OFF at that wave's integration once the optimizing tier
/// strength-reduced constant division and fused `lcmp` (`ir_lower.rs`,
/// `emit_div_by_const` / the fused-lcmp branch). Re-measured on the wave-3
/// binary, single-pass (flag on) vs optimizing (flag off), 5e8 iterations:
/// `ArithOsr div` 496 vs 449 ms, `rem` 758 vs 593, `int` 1 094 vs 780, `full`
/// 903 vs 738, `mul` 648 vs 679 (noise); `CratonBench arithmetic` 3 341 vs
/// 2 997 ms; `matrix` three pairs 2 411/2 470, 2 606/2 415, 2 501/2 425 (noise).
/// The optimizing body is now the faster one on every shape the gate names.
fn osr_prefer_single_pass_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_OSR_PREFER_SINGLE_PASS")
    })
}

/// Whether `code` contains a shape whose optimizing OSR body is measured
/// SLOWER than the single-pass one, so the door should not trade the
/// single-pass body for it: a long division / remainder (`ldiv`, `lrem`) or a
/// long compare (`lcmp`) -- i.e. every `long`-counted loop.
///
/// Round 9 wave 6 dropped `idiv` / `irem`: on the wave-5 binary an `int`
/// induction with both measured 1 942 / 1 982 ms optimizing against
/// 2 032 / 2 038 ms single-pass (see [`osr_prefer_single_pass_enabled`]), so
/// the optimizing body is known to be at least as good there.
///
/// # Confirmed round 9 wave 12 (2026-09-20)
///
/// The predicate's SHAPE, not just its default, was re-measured on a `dev`
/// build: the four `long` rows (`ldiv`/`lrem`/`lcmp`) have the single-pass
/// body winning by 6 % (`mul`), 77 % (`div`), 13 % (`rem`) and 16 % (`full`),
/// and the `int` row -- which this predicate deliberately does not name -- is a
/// wash with the optimizing body marginally ahead. The table is in
/// [`osr_prefer_single_pass_enabled`]. So the membership below is exactly
/// right and nothing about it changed; what changed is that it now rests on a
/// current binary instead of a two-year-old one.
///
/// # The measurement (round 9 wave 3, wave-2 binary, 1e9 iterations)
///
/// `diag/src/ArithOsr.java`, `CRATONVM_JIT_OSR_OPTIMIZING=1` vs `=0` (the
/// optimizing body admitted -- `stub=true entries=[5] sentinel_free=true` --
/// vs the single-pass body), two or three runs each:
///
/// | loop | optimizing | single-pass |
/// |---|---:|---:|
/// | `s += i*3 + (i>>1)` (`lcmp`) | 1 309 / 1 323 ms | 1 026 / 1 025 ms |
/// | `s += i*3 - i/2` | 3 601 / 3 377 ms | 1 607 / 1 357 ms |
/// | `s += i*3 + i%7` | 3 740 / 3 512 ms | 2 102 / 2 204 ms |
/// | `/` and `%` | 6 679 / 6 391 ms | 2 280 / 2 512 ms |
/// | same, `int` induction | 3 289 / 3 116 ms | 2 261 / 2 377 ms |
///
/// and `CratonBench arithmetic`: 8 650 ms vs 3 411 ms. Once wave 2 made a
/// constant divisor guard-free, these bodies became sentinel-free and the door
/// admitted them -- a 2.5x regression on the benchmark the admission was meant
/// to help. The causes are in the optimizing tier (`IDIV` for a constant
/// divisor where single-pass multiplies by a magic number; `lcmp` materialised
/// to {-1,0,1} and spilled instead of fused into the branch --
/// `docs/internal/fixed-bugs/perf-long-counted-loops-lose-the-optimizing-osr-body-FIXED-20260920.md`),
/// so this is a policy for "not known to be better", not a correctness gate:
/// flip it off (`CRATONVM_JIT_OSR_PREFER_SINGLE_PASS=0`) and re-measure once
/// the optimizing tier strength-reduces constant division and fuses `lcmp`.
///
/// Loops without these shapes keep the optimizing body: `CratonBench matrix`
/// measured 2 099 ms optimizing vs 2 344 ms single-pass, `sieve` 4 352 vs
/// 4 295 (a wash).
///
/// The whole method is scanned, not only the loop: an admitted optimizing OSR
/// body is call-free by construction (a call makes it non-sentinel-free), so
/// these methods are small, and the scan runs once per `(method, pc)` -- its
/// verdict is kept in the refusal memo.
fn osr_optimizing_not_known_better(code: &[u8]) -> bool {
    const LDIV: u8 = 0x6d;
    const LREM: u8 = 0x71;
    const LCMP: u8 = 0x94;
    let mut pc = 0usize;
    while pc < code.len() {
        if matches!(code[pc], LDIV | LREM | LCMP) {
            return true;
        }
        // `max(1)`: the decoder steps an undecodable byte by one, but a walk
        // that must terminate does not lean on that.
...
pub(super) fn try_osr(
    ...
    // Round 9 wave 11 (`osr11`): a re-entry at the pc this thread last entered
    // -- the next back edge after a caught exception's handler -- takes the
    // artifact straight from the memo while nothing the door's answer depends
    // on has moved. See `OsrReentryMemo` for why a hit is exact. The stamp is
    // read BEFORE either lookup, so a change racing them can only cause a miss.
    // Cast: the bytecode's address is its identity.
    let code_identity = Arc::as_ptr(&code) as *const u8 as usize;
    let reentry_stamp = osr_reentry_stamp(shared);
    let memo_hit = reentry_stamp.and_then(|stamp| {
        osr_reentry_memo_get(
            shared,
            stamp,
            code_identity,
            class_id,
    ...
    let ir_osr: Option<Arc<cratonvm_jit::CompiledMethod>> = if memo_hit.is_some()
        || !osr_optimizing_tier_enabled()
    {
        None
        ...
        // Already built and refused, or already judged not worth building
        // (the arm below): the single-pass body, with no compile.
        None
    } else if osr_prefer_single_pass_enabled() && osr_optimizing_not_known_better(&code) {
        // Admission prefers the single-pass body where the optimizing one is
        // measured slower (see `osr_optimizing_not_known_better`). Kept in the
        // refusal memo, so the scan runs once per `(method, pc)` and the next
        // entry skips straight to the single-pass artifact -- no optimizing
        // compile is paid for a body that would not be entered.
        note_osr_optimizing_refusal(osr_opt_key);
        if crate::runtime::env_cache::dbg_jitc() {
            eprintln!(
                "[cratonvm-jitc] osr optimizing {class_name}.{method_name} pc={entry_pc}: not attempted -- single-pass preferred (ldiv/lrem/lcmp; CRATONVM_JIT_OSR_PREFER_SINGLE_PASS=0 opts out)"
            );
        }
        None
    } else {
        ...
        cached_for_compile
            ...
            .and_then(|c| {
                ...
                // WHOSE body the three fields below describe.
                //
                // Round 9 wave 12. `compile_optimizing_artifact` returns the
                // SINGLE-PASS body when `ir_evidence::accept` refuses the
                // optimizing one, and a single-pass body carries no OSR
                // entries and `ir_osr_sentinel_free == false`. So the line
                // read `entries=[] sentinel_free=false` for two opposite
                // events -- "the optimizing body has a stub the door cannot
                // enter" and "the optimizing body was thrown away by the
                // acceptance gate" -- and nothing distinguished them.
                //
                // That cost a wave a wrong conclusion: wave 11 of
                // `perf-long-counted-loops-lose-the-optimizing-osr-body` read
                // a partial-unroll A/B as "the unrolled optimizing body beats
                // single-pass" when the gate had refused the unrolled body
                // (its only evidence bit is `Unrolled`, which
                // `is_worth_publishing` deliberately excludes) and BOTH arms
                // were running the single-pass body.
                //
                // The verdict is a thread-local take-and-clear published by
                // the compile that just ran on THIS thread, so this reads its
                // own compile's answer. Taken unconditionally rather than
                // inside the `dbg_jitc()` guard, so the slot is left in the
                // same state whether or not the diagnostic is on.
                let accept = match cratonvm_jit::ir_evidence::take_last_verdict() {
                    Some(true) => "accepted",
                    Some(false) => "REFUSED-by-acceptance-gate",
                    None => "not-judged",
                };
                if crate::runtime::env_cache::dbg_jitc() {
                    match &cm {
                        ...
                        Some(cm) => eprintln!(
                            "[cratonvm-jitc] osr optimizing {class_name}.{method_name} pc={entry_pc}: stub={} entries={:?} sentinel_free={} accept={accept}",
    ...
    let compiled = match ir_osr {
        Some(c) => {
            ...
            if reentry_stamp.is_some() {
                osr_reentry_memo_clear();
            }
            c
        }
        None => match memo_hit {
            ...
            None => {
                ...
                match (c, reentry_stamp) {
                    (Some(c), Some(stamp)) => {
                        osr_reentry_memo_put(
                            shared,
                            stamp,
                            code_identity,
                            class_id,
    ...
    if crate::jit::helpers::take_jit_pending_npe() {
        ...
        //
        // r9w9 exc9: through the `OmitStackTraceInFastThrow` door (opt-in; off,
        // this is exactly the `throw_runtime_error` call it wraps).
        match crate::runtime::exceptions::throw_implicit_runtime_error(
            shared,
            thread,
            crate::runtime::exceptions::FastThrowKind::NullPointer,
    ...
    if let Some((index, length)) = crate::jit::helpers::take_jit_pending_aioobe() {
        ...
        match crate::runtime::exceptions::create_implicit_exception_object(
            shared,
            thread,
            crate::runtime::exceptions::FastThrowKind::ArrayIndexOutOfBounds,
            || cratonvm_types::error::out_of_bounds_message::check_index(index, length),
        ) {
            Ok(exc_obj) => {
                match route_osr_exception_out_of_artifact(
                    shared,
    ...
    if crate::jit::helpers::take_jit_pending_arithmetic() {
        match crate::runtime::exceptions::throw_implicit_runtime_error(
            shared,
            thread,
            crate::runtime::exceptions::FastThrowKind::Arithmetic,
    ...
    if result_i64 == i64::MIN {
        ...
        if let Some(rframe) = cratonvm_jit::deopt::take_last_deopt() {
                ...
                })
            {
                ...
                // Round 9 wave 11 (`osr11`): a failed speculation is charged
                // here too, so a guard that fails on every entry is withdrawn
                // instead of being re-entered forever. See the function.
                charge_osr_guard_exit(
                    shared,
                    class_id,
                    class_name,
            ...
            let despec_reason = guard_reason_for_trapped_frame(&compiled, &rframe);
...
/// Round 9 wave 8 (`arraylist8`; `NOTES-w5-typecheck5.md` cross-lane request
/// 2): is class `cid` a `final`, non-interface, non-array class that nothing can
/// subclass in this VM, so that a type check against it may answer a definite
/// MISS for any receiver of another class?
///
/// That needs two things, and both are asked:
///
/// * the class's own `ACC_FINAL && !ACC_INTERFACE` (an array class's
///   synthetic flags are not a statement about subclassing, and the inline
///   `instanceof` arm never takes an array target anyway);
/// * that the "cannot extend a final class" rule is actually ENFORCED:
///   `verify_final_class_constraint` runs in link-time Pass 2, which
///   `-Xverify:none` / `skip_verification` switches off, and a subclass of a
///   final class could then load. Under it the answer is `false`.
///
/// A class whose loader is later unloaded takes its `ClassId` with it, and the
/// sites interned under that id with it; a redefinition cannot change class
/// modifiers. The helper's non-hierarchy admissions (lambda proxies and the
/// autobox wrapper at ids `>= 0x8000_0000`, the by-name loader-duplication
/// fallback behind the duplicate-class-name latch) are screened by the emitted
/// code itself, exactly as for the final boot classes.
pub(super) fn jit_class_is_final_for_typecheck_in(
    shared: &SharedVm,
    cm: &crate::classloading::ClassManager,
    cid: u32,
) -> bool {
    ...
    cm.get_class(ClassId::new(cid))
        .is_some_and(|c| c.is_final() && !c.is_interface() && !c.name.starts_with('['))
}

/// [`jit_class_is_final_for_typecheck_in`] under its own class-manager read
/// lock: the `CompileRequest::class_is_final_resolver` every compile door
/// passes.
pub(super) fn jit_class_is_final_for_typecheck(shared: &SharedVm, cid: u32) -> bool {
    let cm = shared.classes.class_manager.read();
    jit_class_is_final_for_typecheck_in(shared, &cm, cid)
}
...
pub(super) fn jit_invoke_targets_native_shadow(
    ...
    // chain, because `Class::interfaces` points only upwards; the reverse edge
    // exists in `ClassStore`'s direct-implementor index (added for
    // class-hierarchy analysis), but this walk does not consult it and it
    // would answer "which classes could implement this", not "which class the
    // receiver here actually is". So
...
/// Attach the VM's GC-maintained constant slot to a String/Class `ldc` site
/// (`crate::jit::helpers::compiled_ldc_slot_for`), so the single-pass backend
/// can read the resolved constant instead of calling its helper on every
/// execution. The helper stays the miss arm. A `0` slot (kill switch
/// `CRATONVM_JIT_COMPILED_LDC_CONST_CACHE=0`, a class redefinition, an
/// unresolved site, the background worker with no slot yet) leaves the
/// plain variant, i.e. today's helper CALL.
///
/// Call with NO class-manager lock held: the slot lookup takes the
/// resolution-cache, slot-table and JNI global-ref locks.
fn with_ldc_slot(
    shared: &SharedVm,
    c: Option<cratonvm_jit::JitLdcConstant>,
) -> Option<cratonvm_jit::JitLdcConstant> {
    use cratonvm_jit::JitLdcConstant as L;
    match c {
        Some(L::String {
            holder_class_id,
            cp_idx,
        }) => {
            let slot_addr = crate::jit::helpers::compiled_ldc_slot_for(
                shared,
                ClassId::new(holder_class_id),
                cp_idx,
            );
            Some(if slot_addr != 0 {
                L::StringSlot {
...
pub(super) fn compile_optimizing_artifact(
    ...
    // `ACC_VOLATILE` of a `getfield`/`putfield` site, the instance twin of the
    // static resolver's fourth element. Without it the JIT compiled a volatile
    // store with no StoreLoad fence and let the optimizing tier hoist a
    // volatile read (`volatile-instance-fields-are-plain-accesses-in-compiled-
    // code`). An unresolvable site answers `false`; `field_resolver` refuses
    // it anyway.
    let field_volatile_resolver = |cp_idx: u16| -> bool {
        {
            // Through `MemberResolver`, the entry point the resolve guard enforces.
            let resolver = crate::runtime::resolve::MemberResolver::new(shared);
            resolver
                .field_ref(resolver.scope(class_id), cp_idx)
                .and_then(|field| resolver.adopt(field))
                .is_ok_and(|field| field.is_volatile)
        }
    };
    ...
    let invokespecial_owner_resolver = |cp_idx: u16, opcode: u8| -> Option<String> {
        ...
        // JVMS §6.5 `invokeinterface`, selection step 1: an `0xb9` site whose
        // resolved method is a PRIVATE interface method (a default method
        // calling its interface's own private helper) selects exactly that
        // method. Answering with the declaring interface reclassifies the site
        // as a direct bind, exactly as the `0xb6` arm above does for a private
        // `invokevirtual`; without it the MIC/PIC helper resolved the call from
        // the RECEIVER and ran its unrelated public namesake. `None` for every
        // non-private `0xb9` site, which stays ordinary interface dispatch.
        if opcode == 0xb9 {
            let cp_class_id = cm.find_class_by_name_for_class(target_class, class_id)?;
            let store = cm.class_store();
            return crate::classloading::invokevirtual_private_declaring_class(
                cp_class_id,
                method_name,
                descriptor,
                store,
            )
            .and_then(|declaring_id| store.get(declaring_id).map(|c| c.name.to_string()));
        }
        if opcode != 0xb7 {
            return None;
        }
    ...
    // CHA (NOTES-deopt2.md M5): the single loaded concrete implementor of this
    // site, which is what lets an UNPROFILED compile devirtualize at all.
    let unique_concrete_resolver = |cp_class: &str, name: &str, desc: &str| {
        resolve_unique_concrete_bind(shared, cached.declaring_class_id, cp_class, name, desc)
    };
    ...
    let ldc_resolver = |cp_idx: u16| -> Option<cratonvm_jit::JitLdcConstant> {
        // The class-manager guard ends before the slot lookup (`with_ldc_slot`).
        let c = {
            let cm = shared.classes.class_manager.read();
            jit_ldc_constant_for(&cm, class_id, cp_idx)
        };
        with_ldc_slot(shared, c)
    };
    ...
    // FIX (jdk-only-string-indexof-int-int-miscompiles-in-the-single-pass-tier):
    // every callee address this compile binds stays pinned until the compile
    // publishes. See the wrapper below for why the bind, and not publication,
    // is where the keep-alive has to start.
    let bind_pins: std::cell::RefCell<Vec<std::sync::Arc<cratonvm_jit::CompiledMethod>>> =
        std::cell::RefCell::new(Vec::new());
    let callee_compiler_bound = |callee_class: &str,
                                 callee_method: &str,
                                 callee_desc: &str|
     -> Option<(usize, bool)> {
        ...
        // Look up the callee class and method first, so exception table,
        // native shadows, and JIT cache are checked against the resolved callee.
        let (callee_class_id, method, declaring_id) = {
            let cm = shared.classes.class_manager.read();
            let callee_cid =
                cm.find_class_by_name_for_class(callee_class, cached.declaring_class_id)?;
            let store = cm.class_store();
            let (m, d_id) = crate::classloading::find_method_recursive(
                callee_cid,
                callee_method,
                callee_desc,
                store,
            )?;
            (callee_cid, m.clone(), d_id)
        };
        ...
        // `ACC_VOLATILE` of a `getfield`/`putfield` site. See the twin in
        // `compile_optimizing_artifact`.
        let c_field_volatile_resolver = |cp_idx: u16| -> bool {
            {
                // Through `MemberResolver`, the entry point the resolve guard enforces.
                let resolver = crate::runtime::resolve::MemberResolver::new(shared);
                resolver
                    .field_ref(resolver.scope(callee_cid), cp_idx)
                    .and_then(|field| resolver.adopt(field))
                    .is_ok_and(|field| field.is_volatile)
            }
        };
        ...
        // CHA (NOTES-deopt2.md M5), resolved in the CALLEE's loader: this door
        // compiles `callee_cid`'s method, so its sites' constant-pool names
        // belong to that class, not to whatever asked for the compile.
        let c_unique_concrete_resolver = |cp_class: &str, name: &str, desc: &str| {
            resolve_unique_concrete_bind(shared, callee_cid, cp_class, name, desc)
        };
        ...
        // JDK-ONLY-WAVE2 §4. Answers "would the generic tail admit this
        // triple's registered native?" — which since 2026-09-22 is a wider
        // question than "is it a reviewed `NativeKind::Intrinsic`?", because a
        // refused bind never stopped the call: the site keeps its `invoke_info`
        // and `vm_exec::invoke_or_native` ran the same `Bridge` anyway. Still
        // fail-closed for a `SyntheticStub`, for anything unregistered and for
        // an armed `CRATONVM_ENFORCE_NATIVE_SHADOW`. The one predicate is
        // `helpers::direct_native_bind_admitted`, shared with the callee-side
        // gate; this closure used to be a third hand-written copy of a question
        // that had already been hoisted there.
        // Cheap: only the strict arm of `direct_native_helper` calls it, and only
        // for a triple whose helper cell is already non-zero.
        let direct_bind_admission = |class: &str, method: &str, descriptor: &str| -> bool {
            crate::jit::helpers::direct_native_bind_admitted(shared, class, method, descriptor)
        };

        // Round 9 wave 7 (`mutrec7`): see `cycle_edge_target_admitted`.
        let c_class_is_final_resolver = |cid: u32| jit_class_is_final_for_typecheck(shared, cid);
        let c_cycle_edge_admission = |callee_class: &str,
                                      callee_method: &str,
                                      callee_desc: &str| {
            cycle_edge_target_admitted(shared, callee_cid, callee_class, callee_method, callee_desc)
        };
        let mut compiled = crate::jit::try_compile_request(&crate::jit::CompileRequest {
            ...
            cp_field_volatile_resolver: Some(&c_field_volatile_resolver),
            ...
            // CHA has its own lever (`CRATONVM_JIT_CHA=0`) AND rides the guard
            // it feeds. The conjunction is the point: `jit_cha` makes "is
            // class-hierarchy analysis the cause" a one-variable bisect that
            // leaves the receiver-profile devirtualization in place, while
            // `jit_guarded_virtual_inline` stays load-bearing because CHA's
            // verdict is lowered as a receiver class-id compare — with no
            // namer the plan would be refused for want of an invalidation
            // dependency anyway. See `env_cache::jit_cha`.
            unique_concrete_resolver: if crate::runtime::env_cache::jit_cha()
                && crate::runtime::env_cache::jit_guarded_virtual_inline()
            {
                Some(&c_unique_concrete_resolver)
            } else {
                None
            },
            ...
            direct_bind_admission: Some(&direct_bind_admission),
            // This VM's per-bci de-spec registry.
            despec: Some(&shared.jit.despec_registry),
            // M7 — this VM's runtime de-speculation verdicts.
            runtime_despec: Some(&shared.jit.runtime_despec),
            cp_invoke_declaring_class_resolver: Some(&c_invoke_declaring_class_resolver),
            self_call_identity_stable: self_call_identity,
            direct_helpers: &crate::jit::helpers::direct_helper_table_for(shared),
            cycle_edge_admission: Some(&c_cycle_edge_admission),
            // Round 9 wave 8 (`arraylist8`): final-class verdicts for the
            // inline `instanceof` miss. See `jit_class_is_final_for_typecheck`.
            class_is_final_resolver: Some(&c_class_is_final_resolver),
        })?;
    ...
    // The pinning wrapper. `callee_compiler_bound` decides WHETHER to bind and to what;
    // this holds the answer alive. A raw entry address is only as meaningful
    // as the artifact that owns it, and between this bind and
    // `prepare_for_publication` there is a whole compilation during which that
    // artifact can be superseded, retired, freed and its buffer re-handed to
    // an unrelated method. `pin_jit_entry` upgrades the owner's `Weak` here,
    // while it is still the callee, so the address cannot change meaning
    // underneath the code being emitted against it.
    let callee_compiler =
        |callee_class: &str, callee_method: &str, callee_desc: &str| -> Option<(usize, bool)> {
            let bound = callee_compiler_bound(callee_class, callee_method, callee_desc);
            if let Some((entry, _)) = bound {
                if let Some(owner) = cratonvm_jit::pin_jit_entry(entry) {
                    bind_pins.borrow_mut().push(owner);
                }
            }
            bound
        };
    ...
    let self_call_identity = self_call_identity_stable(shared, cached.declaring_class_id);
    // JDK-ONLY-WAVE2 §4. Answers "would the generic tail admit this
    // triple's registered native?" — which since 2026-09-22 is a wider
    // question than "is it a reviewed `NativeKind::Intrinsic`?", because a
    // refused bind never stopped the call: the site keeps its `invoke_info`
    ...
    // Round 9 wave 7 (`mutrec7`): see `cycle_edge_target_admitted`.
    let class_is_final_resolver = |cid: u32| jit_class_is_final_for_typecheck(shared, cid);
    let cycle_edge_admission = |callee_class: &str, callee_method: &str, callee_desc: &str| {
        cycle_edge_target_admitted(shared, class_id, callee_class, callee_method, callee_desc)
    };
    let compiled = crate::jit::try_compile_request(&crate::jit::CompileRequest {
        ...
        cp_field_volatile_resolver: Some(&field_volatile_resolver),
        ...
        // CHA has its own lever (`CRATONVM_JIT_CHA=0`) AND rides the guard it
        // feeds. The conjunction is the point: `jit_cha` makes "is
        // class-hierarchy analysis the cause" a one-variable bisect that leaves
        // the receiver-profile devirtualization in place, while
        // `jit_guarded_virtual_inline` stays load-bearing because CHA's verdict
        // is lowered as a receiver class-id compare — with no namer the plan
        // would be refused for want of an invalidation dependency anyway. See
        // `env_cache::jit_cha`.
        unique_concrete_resolver: if crate::runtime::env_cache::jit_cha()
            && crate::runtime::env_cache::jit_guarded_virtual_inline()
        {
            Some(&unique_concrete_resolver)
        } else {
            None
        },
        // IR-tier inlining. Behind its own gate; `None` splices nothing.
        ...
        self_call_identity_stable: self_call_identity,
        direct_helpers: &crate::jit::helpers::direct_helper_table_for(shared),
        cycle_edge_admission: Some(&cycle_edge_admission),
        // Round 9 wave 8 (`arraylist8`): final-class verdicts for the inline
        // `instanceof` miss. See `jit_class_is_final_for_typecheck`.
        class_is_final_resolver: Some(&class_is_final_resolver),
    })?;
    Some(compiled)
}
...
fn try_jit_upgrade_with_gate_uncontained(
    ...
    // N3 — the same short-circuit for the OTHER sticky verdict. This is the
    // callee-dispatch door, and it is the door the tomcat `Response.toAbsolute()`
    // fix needed a runtime `MakeNotCompilable` to reach: `jit_skip_set` gates
    // only the interpreter's own hotness path, so without a check here a
    // give-up decision reached through a JIT-compiled caller never stuck.
    //
    // `try_compile_request` below asks `compile_gate::admit`, which asks this
    // too, so this check is about COST and not about correctness — exactly the
    // argument RBC.4 makes for the bail-list check above it. The OSR door in
    // `compile_osr_artifact` deliberately does NOT get a copy: a loop in the
    // method may be perfectly compilable while one call site mis-speculates.
    if shared.jit.runtime_despec.is_not_compilable(
        cached.declaring_class_id,
        &cached.class_name,
        &cached.method_name,
        &cached.method_descriptor,
...
/// Round 9 wave 7 (`mutrec7`): the direct-bind gates of the bg door's
/// `direct_callee_lookup`, asked for a compile-time recursion cycle's CLOSING
/// edge (`CompileRequest::cycle_edge_admission`).
///
/// That edge's target is open on the compile stack, so it never reaches a
/// `callee_compiler` and none of these gates ran for it. A
/// `cratonvm_jit::JitCycleEdgeCell` makes it a raw CALL once the target
/// publishes, so the same refusals apply, in the same order and at the same
/// moment (bake time): the FJP blocklist, a Rust native shadow, a
/// `synchronized` callee, a callee declaring its own exception table (unless
/// `direct_call_exc_table_publish_enabled`), and a static callee whose class is
/// not initialized yet (the raw CALL bypasses every init check). Cells are
/// planned for `invokestatic` only, so a non-static target is refused too.
/// `caller_class_id` is the class whose constant pool names the target.
fn cycle_edge_target_admitted(
    shared: &SharedVm,
    caller_class_id: ClassId,
    callee_class: &str,
    callee_method: &str,
    ...
    if is_fjp_subclass_blocklisted(shared, callee_class, Some(caller_class_id)) {
        return false;
    }
    if shared
        .natives
    ...
    // Through `MemberResolver` (the one resolution entry point the resolve
    // guard enforces), not a raw `find_method_recursive` walk.
    let resolver = crate::runtime::resolve::MemberResolver::new(shared);
    let Ok(found) =
        resolver.declared_method(&cm, resolver.scope(callee_cid), callee_method, callee_desc)
    else {
        return false;
    };
    let Ok((declaring_id, method_index)) = resolver.adopt(found) else {
        return false;
    };
    let store = cm.class_store();
    let Some(method) = store
        .get(declaring_id)
        .and_then(|decl| decl.methods.get(method_index as usize))
    else {
        return false;
    };
    ...
    let code_bars_direct_call = match method.code() {
        None => true,
        Some(c) => {
            !c.exception_table.is_empty() && !cratonvm_jit::direct_call_exc_table_publish_enabled()
        }
    };
    if code_bars_direct_call {
        return false;
    }
    store
        .get(declaring_id)
...
pub(super) fn try_jit_compile_callee_slow(
    ...
    let field_volatile_resolver = |cp_idx: u16| -> bool {
        {
            ...
            let resolver = crate::runtime::resolve::MemberResolver::new(shared);
            resolver
                .field_ref(resolver.scope(cid), cp_idx)
                .and_then(|field| resolver.adopt(field))
                .is_ok_and(|field| field.is_volatile)
        }
    };
    ...
    // CHA (NOTES-deopt2.md M5) — see the sibling resolver in `try_jit_compile`.
    let unique_concrete_resolver = |cp_class: &str, name: &str, desc: &str| {
        resolve_unique_concrete_bind(shared, cached.declaring_class_id, cp_class, name, desc)
    };
    // Elidable-`<init>` resolver for `new` scalar replacement, default-ON
    ...
    let bind_pins: std::cell::RefCell<Vec<std::sync::Arc<cratonvm_jit::CompiledMethod>>> =
        std::cell::RefCell::new(Vec::new());
    let direct_callee_lookup_bound = |callee_class: &str,
                                      callee_method: &str,
                                      callee_desc: &str|
     -> Option<(usize, bool)> {
        ...
        let compiled = match cached_body {
            ...
            None => {
                ...
                match try_jit_compile_callee(
                    ...
                    Some((body, _entry, _needs_ctx)) => body,
                    None => {
                        // A refused supersede answers `None` BECAUSE a baseline
                        // body was published while it ran (`refused_supersede`
                        // in `try_jit_compile_callee_slow`). Bind to that body
                        // rather than leave this site on the blind dispatch
                        // helper for the life of the caller's artifact
                        // (`docs/internal/fixed-bugs/perf-splicecast-bimodal-a-racing-eager-callee-compile-leaves-a-blind-dispatch-FIXED-20260918.md`).
                        let raced = {
                            let jit_cache = shared.jit.jit_cache.read();
                            jit_cache.get(
                                &callee_class_arc,
    ...
    // The pinning wrapper. `direct_callee_lookup_bound` decides WHETHER to bind and to what;
    // this holds the answer alive. A raw entry address is only as meaningful
    // as the artifact that owns it, and between this bind and
    // `prepare_for_publication` there is a whole compilation during which that
    // artifact can be superseded, retired, freed and its buffer re-handed to
    ...
    let direct_callee_lookup =
        |callee_class: &str, callee_method: &str, callee_desc: &str| -> Option<(usize, bool)> {
            let bound = direct_callee_lookup_bound(callee_class, callee_method, callee_desc);
            if let Some((entry, _)) = bound {
                if let Some(owner) = cratonvm_jit::pin_jit_entry(entry) {
                    bind_pins.borrow_mut().push(owner);
                }
    ...
    let class_is_final_resolver = |cid: u32| jit_class_is_final_for_typecheck(shared, cid);
    let cycle_edge_admission = |callee_class: &str, callee_method: &str, callee_desc: &str| {
        cycle_edge_target_admitted(shared, cid, callee_class, callee_method, callee_desc)
    };
    let mut compiled = crate::jit::try_compile_request(&crate::jit::CompileRequest {
        cached: &cached,
        cp_class_name_resolver: Some(&resolver),
...
/// Release every withdrawn compiled body this thread's dispatch caches still
/// own: the invoke cache's `Jit` entries, the raw-entry dispatch memos and the
/// lambda direct-call sites.
///
/// # Why a sweep freed nothing without this (RT-8 soak, 2026-09-18)
///
/// Each of those caches OWNS its artifact (`RetainedCode`) and lets go of it
/// only when the call site that filled it runs again. The code-cache sweeper
/// withdraws exactly the bodies whose callers have stopped running. So every
/// withdrawn body stayed mapped behind a dead call site's cache entry: on a
/// phased workload at a 1 MiB cap a sweep withdrew 1,040,800 bytes, the
/// retirement queue released none of them, and the cap never cleared.
///
/// # Cost and schedule
///
/// One relaxed load and a compare when nothing was withdrawn since this
/// thread last looked, which is the steady state. A scan only after a
/// withdrawal -- a supersede, an invalidation or a sweep -- and it removes
/// only entries whose body is `retired`, so a live body keeps its cached
/// dispatch. Called by the owning thread (these caches are thread-owned), from
/// the interpreter's tier-up strides -- the same clock that drives the sweeper
/// -- and from `safepoint_check`, which reaches every thread at a GC pause.
///
/// Dropping an entry here is safe even with a frame of that body on this
/// thread's stack: `RetainedCode` releases through the retirement queue, which
/// frees nothing a thread can still return into.
pub(crate) fn release_withdrawn_code_owners(thread: &mut JvmThread) {
    let withdrawn = cratonvm_jit::jit_withdrawn_bodies();
    if withdrawn == thread.jit_withdrawals_seen {
        return;
    }
    thread.jit_withdrawals_seen = withdrawn;
    thread
        .invoke_cache
        .retain_jit(|code| !code.retired.load(std::sync::atomic::Ordering::Acquire));
    crate::jit::helpers::flush_raw_entry_dispatch_caches();
    super::lambda::release_withdrawn_lambda_code();
}
...
pub(super) fn offer_invocation_to_tiered_manager(
    ...
    // Piggyback the compiled-entry counter drain on this door, BEFORE the
    // settled short-circuit. This door is the interpreter's tier-up stride, so
    // it ticks whenever anything in the process is still being interpreted —
    // which is the clock the drain needs and cannot get from the compiled path
    // itself. Placed ahead of the `tiering_settled` return because that stamp is
    // about the method this call names, and the drain is about every OTHER
    // method; a settled caller is still a perfectly good clock. Strided and
    // flag-gated, so a default build pays one memoised boolean here.
    maybe_drain_jit_entry_counters(shared);
    ...
    // The per-method branch-profile arm: a compile was just queued for a method
    // that is, by construction of this door, still interpreted. Recording its
    // branches from here until its first body publishes is the stretch the
    // optimizing compile's scheduler reads (`branch_counts`), and it is the
    // stretch the global C2 window never covers -- that window opens at the
    // C1 -> C2 supersede, after the method left the interpreter. See
    // `docs/known-issues/jit/c2-branch-window-opens-after-the-method-leaves-the-interpreter-20260918.md`.
    if verdict.recommended.is_some() && crate::jit::profile::c2_branch_window_enabled() {
        let _ = arm_method_branch_profile(&shared.jit.profile_store, cached);
    }
    verdict.recommended
}

/// Arm `cached`'s method for per-method branch recording: its lock-free
/// counters are created and armed in `store`, and the entry's
/// `branch_profile_armed` flag -- what the interpreter's conditional-branch
/// arms test -- is set. Returns whether this call did the arming (`false`:
/// the entry was already armed, and nothing was touched).
///
/// The code length passed is `cached.code.len()`, the same length
/// `interpreter::record_branch_for_frame` sizes the counters with, so the arm
/// and the recorder agree on the counters' shape (a mismatch would only
/// regrow them -- `flat_branches_for` carries the arm across a regrow -- but
/// there is no reason to pay it).
///
/// The flag is per ENTRY, and a method can have several
/// `CachedBytecodeMethod`s (one per call site that resolved it). Only the entry
/// the nomination came through is armed; the other entries of the same method
/// keep recording nothing unless the process-wide gate is on. That loses
/// observations, never correctness, and the nominating entry is the one whose
/// calls made the method hot.
pub(crate) fn arm_method_branch_profile(
    store: &crate::jit::profile::ProfileStore,
    cached: &cratonvm_jit_api::CachedBytecodeMethod,
) -> bool {
    use std::sync::atomic::Ordering::Relaxed;
    if cached.branch_profile_armed.load(Relaxed) {
        return false;
    }
    let _ = store.arm_branch_counters_borrowed(
        cached.declaring_class_id.as_u32(),
        &cached.method_name,
        &cached.method_descriptor,
        cached.code.len(),
    );
    // Stored after the counters exist, so an interpreter thread that sees the
    // flag finds armed counters (it would create them itself otherwise; the
    // order only saves that thread the slow path).
    !cached.branch_profile_armed.swap(true, Relaxed)
}

/// Calls of [`maybe_drain_jit_entry_counters`] between actual drains.
///
/// The drain walks every published body once, so it must not run per
/// invocation; equally it must not run so rarely that a method sits above the
/// C2 bar for seconds. 256 tier-up strides is the compromise, and the stride is
/// itself already a throttle: `offer_invocation_to_tiered_manager` is reached
/// once per `JIT_RETRY_STRIDE` (64) interpreted invocations past warmup, so this
/// is one cache walk per ~16 000 interpreted invocations of not-yet-settled
/// methods. Nothing depends on the exact number — it trades drain latency
/// against walk cost and both are far from any cliff.
const ENTRY_COUNTER_DRAIN_STRIDE: u64 = 256;

/// Tick source for [`ENTRY_COUNTER_DRAIN_STRIDE`].
///
/// Relaxed and shared: an exact modulus is not wanted here, only a bounded
/// average rate, and two threads racing to the same tick simply means one extra
/// drain or one skipped one.
static ENTRY_COUNTER_DRAIN_TICKS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);

/// Drain the compiled-entry invocation counters, at most once per
/// [`ENTRY_COUNTER_DRAIN_STRIDE`] calls.
///
/// Costs one relaxed `fetch_add` and a compare when the counter is armed,
/// which is the default since round 9 wave 3 (`CRATONVM_JIT_ENTRY_COUNTER`
/// unset = ON, priced in `NOTES-w3-tiering3.md`); one memoised boolean when it
/// is disarmed with `CRATONVM_JIT_ENTRY_COUNTER=0` / `CRATONVM_JIT=-entry-counter`.
pub(super) fn maybe_drain_jit_entry_counters(shared: &SharedVm) {
    if !crate::jit::entry_counter::entry_counter_enabled() {
        return;
    }
    let tick = ENTRY_COUNTER_DRAIN_TICKS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    if tick % ENTRY_COUNTER_DRAIN_STRIDE != 0 {
        return;
    }
    drain_jit_entry_counters(shared);
    maybe_sweep_cold_jit_bodies(shared, tick);
}

/// Retire this VM's cold compiled bodies when the process-wide code-cache cap
/// has been reached — RT-8.
///
/// Driven from here, and not from `jit_code_cache_at_capacity`, because the
/// decision is per-VM and that function is not: eviction needs a `JitCache`,
/// the bodies in it and this VM's own entry counters, none of which a
/// process-wide free function in the jit crate can reach. There is no registry
/// of live `JitCache`s and `AGENTS.md` forbids adding a process global to carry
/// per-VM state, so the driver sits on the side that already holds a
/// `SharedVm`.
///
/// This is also the one place in the VM that already runs on the right
/// schedule. It is downstream of the same `ENTRY_COUNTER_DRAIN_STRIDE` throttle
/// and of the same `entry_counter_enabled()` gate that
/// `JitCache::sweep_cold_bodies` itself requires — a sweeper without that
/// counter has no per-artifact usage signal and would find every body equally
/// cold. The counts it acts on are the ones `drain_jit_entry_counters` has just
/// carried back to the tiered manager, so a method the manager is about to
/// promote is one whose count moved, and is therefore not a candidate.
///
/// Costs one relaxed load of `COMMITTED_JIT_CODE_BYTES` per drain while the
/// sweeper is disarmed, which is the default.
///
/// `progress` is the drain's own tick, handed to the sweeper as the clock that
/// tells "no compiled body was entered" apart from "nothing ran" -- see
/// `JitCache::sweep_cold_bodies`.
///
/// Every call also reports the cap's pressure to the tiered manager, before
/// the early return, because the return is the moment that matters: pressure
/// having just CLEARED is what releases the methods the manager deferred at
/// the cap. See `TieredCompilationManager::note_code_cache_pressure`.
fn maybe_sweep_cold_jit_bodies(shared: &SharedVm, progress: u64) {
    let at_cap = cratonvm_jit::jit_code_cache_under_pressure();
    shared.jit.tiered_manager.note_code_cache_pressure(at_cap);
    if !at_cap {
        return;
    }
    let report = shared.jit.jit_cache.read().sweep_cold_bodies(progress);

    // The half a sweeper is not allowed to skip. `current_tier` only ever
    // advances, and it advances at PUBLICATION; nothing lowers it when a body
    // is removed. A method swept while the manager still believes it is at C1
    // falls back to the interpreter and can never be offered again — its only
    // remaining door is `should_compile`'s C1 -> C2 arm, which asks ~20 000
    // invocations of `MethodState::invocation_count`, a counter with no writer
    // once a C1 body has been published. Without this loop the sweeper would
    // permanently deoptimise every method it touched, and nothing would count
    // it. See `TieredCompilationManager::note_body_withdrawn`.
    //
    // Taken outside the cache walk, per row, because it takes the manager's
    // `methods` mutex — the same two-phase shape and the same reason as
    // `drain_jit_entry_counters` above.
    for (class_name, method_name, descriptor, class_id) in &report.withdrawn {
        shared.jit.tiered_manager.note_body_withdrawn(
            &crate::jit::tiered::MethodKey::with_class_id(
                *class_id,
                Arc::clone(class_name),
                Arc::clone(method_name),
    ...
    if report.swept != 0 && cratonvm_jit::metrics::print_compilation_enabled() {
        eprintln!(
            "[cratonvm-jit] code-cache sweep: withdrew {} of {} published bodies, \
             {} bytes ({} cold candidates, {} demoted to the interpreter). The bytes \
             are an upper bound: they return to the OS only once the retirement \
             queue proves no thread can re-enter them and no surviving caller \
             still pins them.",
            report.swept,
            report.observed,
            report.withdrawn_bytes,
            report.candidates,
...
/// Carry every compiled body's own entry count back to the tiered manager.
///
/// This is the second half of the compiled-entry invocation counter; the first
/// is two instructions in `x64::frames::emit_prologue`. See
/// `cratonvm_jit::entry_counter` for why it is needed at all — in one line,
/// `MethodState::invocation_count` has no writer once a C1 body is published,
/// so `should_compile`'s C1→C2 hotness arm is fed a count frozen at roughly the
/// C1 threshold and can never reach the 20 000 it asks for.
///
/// `observed_count` is the parameter this feeds, and it is the parameter that
/// was added for exactly this: it fast-forwards the manager's count to an
/// externally observed one and "degrades to the historical `+= 1` behaviour"
/// when the value is stale or smaller. That degradation is what makes this safe
/// to call unconditionally — a method whose compiled count is below what the
/// manager already knows is left exactly as it was.
///
/// ## What acts on the result
///
/// Nothing here, deliberately. `on_method_invocation_settling` reaches
/// `should_compile_inner`, which ENQUEUES the recommended task itself (through
/// `CompilerCore::admit`) before returning the tier; the returned tier is
/// informational for the interpreter's own logging and worker-start path. So a
/// method this drain pushes over the C2 bar is queued by the same code, at the
/// same priority, through the same idempotence gate, as one the interpreter
/// pushed over it — which is the property that keeps this from being a second
/// tiering policy.
///
/// ## Two known imprecisions, both in the safe direction
///
///  * Each drain also charges every counted method the `+= 1`
///    `on_method_invocation_settling` applies before consulting
///    `observed_count`. At this cadence that is single-digit counts per second
///    against a bar of 20 000, and it can only make a method look hotter than
///    the compiled counter says — never colder.
///  * The compiled counter starts at zero when the body is published, so the
///    method's interpreted history is not in it. The manager still holds that
///    history in its own (frozen) count and takes the maximum, so the union is
///    never worse than either half alone.
///
/// ## The clock, honestly
///
/// This has no clock of its own. It runs off the interpreter's tier-up stride
/// and off every background compile, so it ticks while anything is being
/// interpreted or compiled — which covers every warmup and every steady state
/// that still has cold code entering. A process that reached a state where
/// NOTHING is interpreted and NOTHING compiles would stop draining. Whether
/// that state is reachable in practice is one of the things the default-off A/B
/// exists to find out; the design note in
/// `docs/internal/fixed-bugs/c1-to-c2-live-count-INVESTIGATION-20260916.md`
/// records what the fix would be if it is (an inline threshold compare and a
/// one-shot helper call in the prologue itself).
pub(super) fn drain_jit_entry_counters(shared: &SharedVm) {
    if !crate::jit::entry_counter::entry_counter_enabled() {
        return;
    }
    // Lock-free `arc_swap` walk; empty unless some compiled prologue has
    // actually run. See `JitCache::snapshot_entry_counts` for why the rows are
    // materialised before the manager's mutex is taken rather than during.
    let rows = shared.jit.jit_cache.read().snapshot_entry_counts();
    if rows.is_empty() {
        return;
    }
    // A recommendation is worth nothing without a worker to serve it, and this
    // door can be the first one reached on a `CRATONVM_BG_COMPILE` run whose
    // interpreted methods all settled early. One atomic load once the workers
    // are up.
    ensure_bg_compiler_started(shared);
    let manager = &shared.jit.tiered_manager;
    for (class_name, method_name, descriptor, class_id, count) in rows {
        let key = crate::jit::tiered::MethodKey::with_class_id(
            class_id,
            class_name,
            method_name,
        ...
        // The verdict is dropped on purpose — see the doc comment: the enqueue
        // has already happened inside the manager by the time this returns, and
        // the settled stamp belongs to a `CachedBytecodeMethod` this walk does
        // not have and must not guess at.
        //
        // r9w2-vm2: `on_observed_invocations`, not `on_method_invocation_settling`
        // — a drain REPORTS invocations the compiled prologue already counted;
        // it is not itself an invocation, so it must not charge the `+1`.
        let _ = manager.on_observed_invocations(&key, count);
    }
}
...
pub(super) fn background_compile_task(
    ...
    // The compiled-entry counter's second clock. The interpreter's tier-up
    // stride is the first, and it stops ticking for a method once that method
    // settles — but every background compile is also evidence that the process
    // is still warming up, and it arrives on a worker thread that holds no VM
    // lock and is about to spend milliseconds in codegen. A full drain here is
    // free by comparison, and it is the tick that keeps working when the last
    // interpreted method has settled but compiles are still landing.
    //
    // Unstrided on purpose: compiles are rare events by construction (each one
    // costs milliseconds), so there is nothing here to throttle. Returns before
    // any lock unless `CRATONVM_JIT_ENTRY_COUNTER` is armed.
    drain_jit_entry_counters(&shared);
    ...
    //
    // Held as `RetainedCode`, NOT a bare `Arc`. That keep-alive is exactly what
    // made this local the body's LAST owner: the publish below withdraws the
    // C1 body into the retirement queue, the queue's drop is then not the last
    // one, and this local used to unmap a published body at the end of the
    // function with no quiescence proof — ~27 times per Spring test process,
    // and once under a thread still executing it
    // (`jit-code-uaf-outside-retirement-queue-FIXED-20260918.md`).
    // `RetainedCode`'s drop routes a last reference through the queue.
    let superseded_body: Option<cratonvm_jit::RetainedCode> = optimized
        .then(|| {
    ...
    if published && optimized {
        let replacement = {
            ...
            // Same reason as `superseded_body`: a later publish can withdraw
            // this body before the block ends, leaving this the last owner.
            .map(cratonvm_jit::RetainedCode::new)
        };
...
/// Class-hierarchy analysis for one call site — the VM half of
/// `NOTES-deopt2.md` M5.
///
/// Answers, for a site declared `(cp_class, name, descriptor)` in a method of
/// `requesting_class_id`, the single loaded CONCRETE class below `cp_class`
/// that provides that method. `plan_inline` turns a `Some` into an
/// [`cratonvm_jit::InlineVerdict::UniqueConcreteMethod`] — a body bound behind
/// the exact receiver class-id guard — which is the ONLY way a virtual site
/// gets inlined on a compile with no receiver profile at all. Without it
/// `ReceiverShape::Unprofiled` refuses every one of them.
///
/// Three properties are load-bearing, and each is established here rather than
/// assumed by the planner:
///
///  * **The class name is resolved in the REQUESTING class's loader**, through
///    `find_class_by_name_for_class`, exactly as every other resolver on this
///    path does. Two same-named classes in different loaders are the ByteBuddy
///    / servlet-container shape, and a hierarchy answer about the wrong one
///    would guard on a class id no receiver at this site can have.
///  * **The class-manager lock is released before the analysis runs.**
///    `ClassRealm::unique_implementor` takes the L10 read lock itself, and
///    `OrderedPlRwLock` is `parking_lot`-backed: a second `read()` on a thread
///    that already holds one deadlocks as soon as a writer queues between them.
///  * **The answer is a hint, never a proof.** The bind it enables is guarded
///    and the guard's miss edge is unchanged dispatch, so an answer made stale
///    by a later class define costs a compare that always fails and never a
///    wrong method. See `cratonvm_jit::InlineDependency::UniqueConcreteMethod`
///    for what would have to change before an unguarded bind could be sound.
///
/// No separate kill switch: the caller passes this resolver only when it also
/// passes `class_id_name_resolver`, so `CRATONVM_JIT_GUARDED_VIRTUAL_INLINE=0`
/// takes CHA away with the rest of the devirtualization it feeds.
pub(super) fn resolve_unique_concrete_bind(
    shared: &SharedVm,
    requesting_class_id: ClassId,
    cp_class: &str,
    name: &str,
    descriptor: &str,
) -> Option<cratonvm_jit::UniqueConcreteBind> {
    let static_type = {
        let cm = shared.classes.class_manager.read();
        let static_type = cm.find_class_by_name_for_class(cp_class, requesting_class_id)?;
        // JVMS 11 §6.5 invokeinterface/invokevirtual: a PRIVATE resolved method is
        // the selected method, so no receiver class can "implement" it. See
        // cha-binds-a-private-interface-method-to-the-receivers-public-namesake-20260918.md.
        if crate::classloading::invokevirtual_private_declaring_class(
            static_type,
            name,
            descriptor,
            cm.class_store(),
    ...
    let implementor = shared
        .classes
        .unique_implementor(static_type, name, descriptor)?;
    if crate::runtime::env_cache::dbg_jitc() {
        eprintln!(
            "[cratonvm-jitc] cha unique-implementor {cp_class}.{name}{descriptor} -> class id {}",
            implementor.as_u32()
        );
    }
    Some(cratonvm_jit::UniqueConcreteBind {
        implementor_class_id: implementor.as_u32(),
        static_type_class_id: static_type.as_u32(),
        // The invalidation key. Deliberately the name the SITE carries: that is
        // the string the class-define listener reports through the supertype
        // closure when a second implementor arrives, and therefore the only one
        // `JitCache::invalidate_for_class_change` can match this dependency on.
        static_type_name: cp_class.to_string(),
    })
}
...
fn resolve_inline_site_from(
    ...
    if let Some(receiver_id) = receiver_class_id {
        if shared.classes.is_lambda_proxy_class(receiver_id) {
    ...
    let store = cm.class_store();
    // JVMS 11 §6.5 invokeinterface/invokevirtual: when the method the constant
    // pool RESOLVES to is private, it is the selected method, and no walk from
    // the receiver may pick a same-named method of the receiver's class. See
    // cha-binds-a-private-interface-method-to-the-receivers-public-namesake-20260918.md.
    if receiver_class_id.is_some()
        && crate::classloading::invokevirtual_private_declaring_class(
            cp_class_id,
            callee_method,
            callee_desc,
        ...
        .is_some()
    {
        no!("cp-resolved-method-is-private");
    }
    ...
    // The bisect levers (`CRATONVM_JIT_DENY` / `CRATONVM_JIT_BISECT_ONLY`),
    // asked of the body ACTUALLY being spliced — N8.
    //
    // `jit/src/lib.rs`'s `plan_inline` ladder already asks them, and says in
    // its own comment that it asks the CONSTANT-POOL name: *"guarded
    // virtual/interface speculation resolves its body per receiver class …
    // so a deny aimed at a speculated receiver's own class name is NOT covered
    // by this and would need the receiver's name, which this site does not
    // have."* This site has it. While guarded speculation was default-off that
    // gap was latent; turning it on makes it live, and a lever that cannot stop
    // a splice makes a bisect LIE — which `compile_gate`'s module doc calls out
    // as worse than a missing feature, having paid for it once already (every
    // bisect step on the annotation-scan SIGSEGV read "no effect" while eleven
    // methods were still compiling).
    //
    // Asked of both names a reader would type. The DECLARING class is the one
    // whose source the spliced bytecode came from, and the RECEIVER class is
    // the one a bisect would name after reading a stack trace or a profile —
    // for `TreeMap$EntryIterator.hasNext` those are two different classes, and
    // denying either has to work.
    if cratonvm_jit::jit_force_interpret(declaring_class_name, callee_method) {
        no!("force-interpreted (declaring class)");
    }
    if let Some(receiver_id) = receiver_class_id {
        if let Some(receiver_name) = store.get(receiver_id).map(|c| &*c.name) {
            if receiver_name != declaring_class_name
                && cratonvm_jit::jit_force_interpret(receiver_name, callee_method)
            {
                no!("force-interpreted (receiver class)");
            }
        }
    }
    ...
    for &(ipc, cp_idx, opcode) in &invoke_sites {
        ...
        let (target_name, target_desc) =
            callee_class_info.constant_pool.get_name_and_type(nat_idx)?;
        let mut invoke_kind: u8 = match opcode {
            0xb6 => 0,
            0xb7 => 1,
            0xb9 => 2,
            0xb8 => 3,
        ...
        // JVMS 5.4.6 / §6.5: an `invokevirtual` or `invokeinterface` whose
        // RESOLVED method is private selects exactly that method. A call left
        // at kind 0/2 here is dispatched from the receiver by
        // `jit_invoke_dispatch`, which for a private interface method `I.m`
        // lands on the receiver's unrelated public `m` (and for a private
        // `invokevirtual` on the most-derived same-named private method). Pin
        // it as a direct bind at the declaring class, as every top-level
        // compile door already does. See
        // cha-binds-a-private-interface-method-to-the-receivers-public-namesake-20260918.md.
        let mut private_owner: Option<String> = None;
        if matches!(invoke_kind, 0 | 2) {
            if let Some(cp_class_id) = cm.find_class_by_name_for_class(target_class, declaring_id) {
                let store = cm.class_store();
                if let Some(owner_id) = crate::classloading::invokevirtual_private_declaring_class(
                    cp_class_id,
                    target_name,
                    target_desc,
                    store,
                ) {
                    private_owner = store.get(owner_id).map(|c| c.name.to_string());
        ...
        invoke_targets.push((
            ...
            cratonvm_jit::InlineInvokeTarget {
                class_name: private_owner.unwrap_or_else(|| target_class.to_string()),
    ...
    // Callee pcs of the volatile `getfield`s (round 9 wave 10, irl10; landed
    // by wave 10b, wire10b). See `InlineSite::volatile_field_pcs`.
    let mut volatile_field_pcs: Vec<usize> = Vec::new();
    for (fpc, cp_idx, type_tag) in field_sites {
        if let Ok(resolved) = resolve_field_ref(shared, declaring_id, cp_idx) {
            // A VOLATILE instance field's `field_info` row has no volatility of
            // its own. A volatile STORE refuses the body in both modes: the
            // single-pass splice would store with no StoreLoad fence and the
            // optimizing tier has no fenced spliced store. A volatile LOAD is
            // admitted and its pc recorded in `volatile_field_pcs`: the IR tier
            // builds it as an ordered `Op::Load` (Acquire, never hoisted or
            // merged), and a single-pass splice reads it with one plain load,
            // which x86-64 orders as an acquire. With the IR's volatile
            // modelling switched off (`CRATONVM_JIT_IR_VOLATILE_FIELDS=0`) an IR
            // splice is still refused, as before
            // (`volatile-instance-fields-are-plain-accesses-in-compiled-code`).
            if resolved.is_volatile {
                if code_bytes.get(fpc).copied() == Some(0xb5) {
                    no!("splice-volatile-field-store");
                }
                if ir_mode && !cratonvm_jit::ir::ir_volatile_fields_enabled() {
                    no!("ir-splice-volatile-field");
                }
                // Recorded in BOTH modes, so a site resolved for one tier and
                // read by the other still names its volatile reads.
                volatile_field_pcs.push(fpc);
            }
...
/// The [`JitArgPinGuard`] window for the two doors that receive their
/// arguments already decoded (`execute_jit_call_decoded`,
/// `execute_jit_call_oneshot`).
///
/// Pushes every non-null reference in `args` onto `native_pin_roots`, records
/// its pin index in `arg_pins[i]` (`usize::MAX` for anything else), and
/// returns the guard that releases the window. Read the arguments back through
/// [`pinned_decoded_args`].
///
/// Those doors used to hand the pre-call `args` straight to the exception
/// routers as the handler frame's locals. `args` is a Rust-side copy the
/// collector cannot see, and the compiled call between the decode and that
/// read can run a moving collection, so the handler frame was built with
/// pre-move addresses — the defect `JitArgPinGuard` already closed for
/// `execute_jit_call`, which is the one door that popped its own arguments.
pub(super) fn pin_decoded_jit_args(
    thread: &mut JvmThread,
    args: &[Value],
    arg_pins: &mut [usize],
) -> JitArgPinGuard {
    ...
    for (i, v) in args.iter().enumerate() {
        if let Value::Object(Some(obj)) = v {
            if let Some(pin) = arg_pins.get_mut(i) {
                *pin = thread.native_pin_roots.len();
...
/// `args` with every reference [`pin_decoded_jit_args`] pinned re-read at its
/// current (possibly relocated) address. Non-references, nulls, and any slot
/// without a live pin come back unchanged.
pub(super) fn pinned_decoded_args(
    thread: &JvmThread,
    args: &[Value],
    arg_pins: &[usize],
) -> Vec<Value> {
    args.iter()
        ...
        .map(|(i, v)| {
            let pinned = arg_pins
                ...
                .and_then(|&pin| thread.native_pin_roots.get(pin));
            match (v, pinned) {
                (Value::Object(Some(_)), Some(obj)) => Value::Object(Some(*obj)),
                _ => *v,
            }
        })
        .collect()
...
/// Write the pinned (possibly relocated) reference arguments back into the
/// CALLER's decoded argument buffer — the in-place sibling of
/// [`pinned_decoded_args`].
///
/// r9w2-vm2: the decoded doors return `Ok(None)` ("declined") on two paths
/// that follow the compiled call — a deopt with no resumable frame, and the
/// `deopt_signaled` sentinel — and every caller then builds the interpreted
/// frame from the SAME buffer it passed in. That buffer is a Rust-side copy
/// the collector cannot see, so a moving collection inside the compiled body
/// left it naming from-space addresses. Called immediately before each such
/// post-call `return Ok(None)`, while the [`JitArgPinGuard`] window is still
/// open. Non-references, nulls and slots without a live pin are untouched.
pub(super) fn write_back_pinned_decoded_args(
    thread: &JvmThread,
    dst: &mut [Value],
    arg_pins: &[usize],
) {
    for (i, v) in dst.iter_mut().enumerate() {
        let pinned = arg_pins
            .get(i)
            .and_then(|&pin| thread.native_pin_roots.get(pin));
        if let (Value::Object(Some(_)), Some(obj)) = (*v, pinned) {
            *v = Value::Object(Some(*obj));
        }
    }
}

/// Dispatch-free entries that came back with a throwable stashed.
///
/// A body with `has_dispatch == false` is entered through a door that skips
/// `catch_unwind`, and that door used to ignore `sig.exception` altogether.
/// `take_all_jit_signals` TAKES it, so a throwable stashed by a helper the
/// compiler's `has_dispatch` list did not anticipate was dropped on the floor:
/// the method returned its sentinel or its last value as though it had
/// completed normally. That list (`jit/src/x64/driver.rs`, and the IR tier's
/// own arms in `jit/src/lib.rs`) has been wrong at least six times — the
/// `Character.getType`, `<clinit>`, indy, athrow, checkcast and IR `Op::Throw`
/// entries each record one — so every door now routes the exception whatever
/// the flag says, and counts the event, because a nonzero count names a
/// `has_dispatch` omission for the JIT crate to fix.
static FAST_ENTRY_STRANDED_EXCEPTIONS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);

/// Record that a dispatch-free entry returned with a pending throwable (which
/// the caller then routes). Warns once per process, naming the method, so the
/// missing `has_dispatch` trigger can be found without a debugger.
fn note_fast_entry_stranded_exception(cached: &CachedBytecodeMethod) {
    let prior = FAST_ENTRY_STRANDED_EXCEPTIONS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    if prior == 0 {
        tracing::warn!(
            "JIT: dispatch-free compiled method {}.{}{} returned with a pending \
             exception; routed it, but the compiler's has_dispatch list is missing \
             whatever stashed it",
            cached.class_name,
            cached.method_name,
            cached.method_descriptor,
...
pub(super) fn execute_jit_call(
    ...
    let (result, mut sig) = if !compiled.has_dispatch {
        ...
        // THE TLS THREAD POINTER IS INSTALLED HERE TOO, and its absence was a
        // GC-safety hole rather than an optimization.
        //
        // `jit_safepoint_slow_path_body` is `if let Some((thread, _guard)) =
        // jit_thread_mut() { ... }` — and `jit_thread_mut` reads the
        // `JIT_THREAD` TLS that only `set_jit_thread` writes. This arm did not
        // write it, so for the whole duration of a dispatch-free compiled
        // method that TLS was null and the poll's slow path was a silent
        // no-op: a pure hot loop reached its back-edge poll, saw the
        // stop-the-world flag, called the helper, and returned WITHOUT
        // retiring its TLAB, draining SATB, publishing its roots or arriving
        // at the barrier. The helper's own doc claims the opposite ("lets pure
        // compiled methods use the same poll sequence as context methods"),
        // and the only reason this was survivable is `xt_root_scan`'s forcible
        // `SuspendThread`, which exists on Windows and Linux-x86_64 and
        // nowhere else. (The stale note in `xt_root_scan.rs` saying "JIT code
        // does not poll the STW flag" was accidentally TRUE because of this
        // bug, and is corrected with it.)
        //
        // Only `set_jit_thread` is added, NOT `catch_unwind`: the comment
        // above justifies skipping the unwind shim on a dispatch-free body,
        // and that argument still holds. `restore_jit_thread` additionally
        // resets the shadow-stack `top` watermark, healing any push an
        // abnormal JIT exit left unbalanced — a property this arm previously
        // did not have at all.
        let saved_jit_thread = crate::jit::helpers::set_jit_thread(thread);
        ...
        // r9-vmside: the drain above TAKES the pending throwable, so this arm
        // must route it or it is gone. See `note_fast_entry_stranded_exception`.
        if let Some(exc) = sig.exception.take() {
            note_fast_entry_stranded_exception(cached);
            let exc_locals =
    ...
    // catch and surface it in the caller. The signal's stamped BCI is
    // translated to `JitThrowPc`: a throw outside every range propagates,
    // while a throw inside a protected range reaches only that range's handler.
    if sig.npe {
        ...
        match crate::runtime::exceptions::throw_implicit_runtime_error(
            ...
            MethodCallFailed::ExceptionThrown(exc) => {
                ...
                return route_jit_signal_exception(
                    ...
                    frame_idx,
                    cached,
                    jit_local_athrow_pc_kind(cached, sig.athrow_bci),
                    exc,
                    &exc_locals,
                );
            }
    ...
    // observes the throw. The signal's stamped BCI gives this drain the same
    // location-aware handler selection as the NPE path.
    ...
    // divergence). The stub stamps its BCI before this drain routes the
    // exception, so an outside-range throw cannot match an unrelated handler.
    if sig.arithmetic {
        // These three arms synthesize a FRESH exception and route it; none of
        // them consumes the stashed deopt frame. Draining it here bounds the
        // window in which that frame holds raw heap addresses nothing scans —
...
pub(super) fn execute_jit_call_decoded(
    ...
    const JIT_ABI_MAX_JAVA_ARGS: usize = 8;
    // r9w2-vm2: `caller_args` is `&mut` only so the post-call declines can
    // write the pinned (relocated) references back before the caller re-runs
    // the method interpreted from this buffer; everything else reads it.
    let args_slice: &[Value] = &*caller_args;
    ...
    let vm_ptr = shared as *const _ as i64; // Cast: JIT ABI -- pointer to i64 register
                                            // r9-vmside: root the reference arguments for the whole compiled
                                            // activation — `execute_jit_call`'s `JitArgPinGuard` window, which this
                                            // door never had. `args_slice` is a Rust-side copy the collector cannot
                                            // see, and it is read AFTER the compiled call as the handler frame's
                                            // locals, so a moving collection inside the body handed those locals
                                            // pre-move addresses. Declared after `_synchronized_monitor`, so it drops
                                            // first and its window (above the monitor's pin) unwinds in stack order.
    let mut arg_pins = [usize::MAX; JIT_ABI_MAX_JAVA_ARGS];
    let _arg_pin_guard = pin_decoded_jit_args(thread, &args_slice[..np], &mut arg_pins);
    ...
    let (result, mut sig) = if !compiled.has_dispatch {
        // Installs the `JIT_THREAD` TLS for the same reason as the first
        // dispatch-free door above: without it `jit_safepoint_slow_path_body`
        // finds no thread and every safepoint poll inside this compiled method
        // is a silent no-op. `catch_unwind` is still skipped, deliberately.
        let saved_jit_thread = crate::jit::helpers::set_jit_thread(thread);
        ...
        crate::jit::helpers::restore_jit_thread(saved_jit_thread);
        let mut sig = crate::jit::helpers::take_all_jit_signals(thread);
        // r9-vmside: see `note_fast_entry_stranded_exception`.
        if let Some(exc) = sig.exception.take() {
            note_fast_entry_stranded_exception(cached);
            let throw_pc = jit_local_athrow_pc_kind(cached, sig.athrow_bci);
            let exc_locals = pinned_decoded_args(thread, &args_slice[..np], &arg_pins);
            return route_jit_signal_exception(
                shared,
                thread,
                frame_idx,
        ...
        if let Some(exc) = sig.exception.take() {
            ...
            let throw_pc = jit_local_athrow_pc_kind(cached, sig.athrow_bci);
            // Through the argument pins (r9-vmside): the compiled call may
            // have moved a reference argument.
            let exc_locals = pinned_decoded_args(thread, &args_slice[..np], &arg_pins);
            return route_jit_signal_exception(
                shared,
                thread,
    ...
    if sig.npe {
        ...
        match crate::runtime::exceptions::throw_implicit_runtime_error(
            ...
            MethodCallFailed::ExceptionThrown(exc) => {
                ...
                // Through the argument pins (r9-vmside); see the signal-exception arm.
                let exc_locals = pinned_decoded_args(thread, &args_slice[..np], &arg_pins);
                return route_jit_signal_exception(
                    shared,
                    thread,
    ...
    if result == i64::MIN {
        if let Some(rframe) = cratonvm_jit::deopt::take_last_deopt() {
            ...
            // r9w2-vm2: the caller re-runs from `caller_args`; hand it the
            // references at their current addresses.
            write_back_pinned_decoded_args(thread, &mut caller_args[..np], &arg_pins);
            return Ok(None);
        }
    }
    ...
    if result == i64::MIN && deopt_signaled {
        ...
        // r9w2-vm2: see the stashed-frame arm above.
        write_back_pinned_decoded_args(thread, &mut caller_args[..np], &arg_pins);
        return Ok(None);
    }
...
pub(super) fn execute_jit_call_oneshot(
    ...
    const JIT_ABI_MAX_JAVA_ARGS: usize = 8;
    // r9w2-vm2: `&mut` only for the post-call write-back of the pinned
    // references (see `write_back_pinned_decoded_args`).
    let args_slice: &[Value] = &*caller_args;
    ...
    // r9-vmside: root the reference arguments across the compiled call, for
    // the same reason as `execute_jit_call_decoded` — `args_slice` is read
    // again afterwards as the handler frame's locals.
    let mut arg_pins = [usize::MAX; JIT_ABI_MAX_JAVA_ARGS];
    let _arg_pin_guard = pin_decoded_jit_args(thread, &args_slice[..np], &mut arg_pins);
    ...
    let (result, mut sig) = if !compiled.has_dispatch {
        // Installs the `JIT_THREAD` TLS for the same reason as the other two
        // dispatch-free doors: without it `jit_safepoint_slow_path_body` finds
        // no thread and every safepoint poll inside this compiled method is a
        // silent no-op. `catch_unwind` is still skipped, deliberately.
        let saved_jit_thread = crate::jit::helpers::set_jit_thread(thread);
        // SAFETY: compiled is a finalized JIT CompiledMethod with a validated entry; args match its JVM descriptor (receiver-aware).
        let fast_result: Result<i64, cratonvm_jit::CompileError> = {
            let _jit_root_guard =
        ...
        if let Some(exc) = sig.exception.take() {
            let throw_pc = jit_local_athrow_pc_kind(cached, sig.athrow_bci);
            // Through the argument pins (r9-vmside).
            let exc_locals = pinned_decoded_args(thread, &args_slice[..np], &arg_pins);
            return oneshot_route_exception(
                shared,
                thread,
    ...
    if result == i64::MIN {
        if let Some(rframe) = cratonvm_jit::deopt::take_last_deopt() {
            let resumed = resume_deopted_body(
                shared,
                thread,
                compiled,
                cached,
            ...
            if matches!(resumed, Ok(None)) {
                // r9w2-vm2: declined — the caller re-runs from `caller_args`,
                // so hand it the references at their current addresses.
                write_back_pinned_decoded_args(thread, &mut caller_args[..np], &arg_pins);
            }
            return resumed;
        }
...
/// The class-definition epoch the profile-replay hook last swept at, or
/// `u64::MAX` for "never" / "the last sweep could not take the class manager
/// and must be retried". The epoch is process-wide, so this latch is too; the
/// per-VM progress is [`PROFILE_REPLAY_WATERMARKS`].
static LAST_PROFILE_REPLAY_SWEEP_EPOCH: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(u64::MAX);

/// Per VM: the first `ClassId` slot the profile-replay hook has not looked at
/// yet. Keyed by a `Weak` so the key's address cannot be reused by a later VM
/// while the entry exists (a `Weak` keeps the allocation, not the VM); entries
/// of dropped VMs are pruned on each visit.
///
/// Why a watermark rather than resolving every waiting name at every
/// definition (`profile_store::seed_pending_replay`): that is O(waiting class
/// names) resolver calls per class definition, and a real replay names
/// hundreds to thousands of classes against an application that defines
/// thousands -- millions of lookups during start-up, which is exactly the
/// phase replay exists to shorten. `ClassId`s are allocated monotonically and
/// never reused (`ClassStore::next_id`), so the classes defined since the last
/// visit are the slots from the watermark to `slot_count()`, and each defined
/// class costs one by-name probe of the pending map over the life of the VM.
///
/// Only touched while a replay has records waiting
/// ([`cratonvm_jit::profile_store::any_replay_pending`]), so a run without
/// `CRATONVM_JIT_PROFILE_LOAD` never takes this lock.
static PROFILE_REPLAY_WATERMARKS: std::sync::Mutex<Vec<(std::sync::Weak<SharedVm>, u32)>> =
    std::sync::Mutex::new(Vec::new());

/// Seed profile-replay records whose class has just been defined.
///
/// Called from the class manager's write-guard release
/// (`ClassManagerWriteGuard::drop`, beside
/// [`resweep_deferred_new_after_class_definition`]), after the write lock is
/// gone, so the resolver may take a read lock. The eager load in
/// `SharedVm::new` runs before any application class exists and parks the
/// records it cannot resolve (`profile_store::load_into_with_policy`); this is
/// the door that seeds them when the program defines their class. It loads
/// nothing: it only reacts to definitions the program made. See
/// `docs/known-issues/jit/profile-replay-resolves-classes-before-the-application-loads-them-20260918.md`.
///
/// Cost: one relaxed load when no replay is waiting (every default run); with
/// one, one epoch swap per write-guard release and, per class definition
/// epoch, one read-locked walk of the newly allocated class slots plus one
/// pending-map probe per new class.
pub fn seed_profile_replay_after_class_definition() {
    if !cratonvm_jit::profile_store::any_replay_pending() {
        return;
    }
    let epoch = crate::classloading::class_definition_epoch();
    if LAST_PROFILE_REPLAY_SWEEP_EPOCH.swap(epoch, std::sync::atomic::Ordering::AcqRel) == epoch {
        return;
    }
    for shared in crate::vm::vm_init::live_hook_vms_for_jit() {
        if !seed_profile_replay_for_new_classes(&shared) {
            // The class manager was busy: forget the latch so the next
            // write-guard release retries. The watermark did not move, so
            // nothing defined in between is skipped.
            LAST_PROFILE_REPLAY_SWEEP_EPOCH.store(u64::MAX, std::sync::atomic::Ordering::Release);
        }
    }
}

/// One VM's half of [`seed_profile_replay_after_class_definition`]. Returns
/// `false` when the class manager could not be read without blocking (the
/// watermark is then left where it was).
fn seed_profile_replay_for_new_classes(shared: &Arc<SharedVm>) -> bool {
    let store = &shared.jit.profile_store;
    let from = {
        let marks = PROFILE_REPLAY_WATERMARKS
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        marks
            .iter()
            .find(|(vm, _)| std::ptr::eq(vm.as_ptr(), Arc::as_ptr(shared)))
            .map_or(0, |&(_, mark)| mark)
    };
    // Names of the classes allocated since the last visit, collected under
    // one non-blocking read lock and released before any seeding: the
    // resolver below takes the class manager again, and `ReplayPending`'s
    // mutex must never be taken while this lock is held.
    let (names, upto) = {
        // `try_read_untracked`: this runs from a write-guard `Drop`, on
        // whatever thread defined a class, and a non-blocking acquisition
        // adds no wait-for edge (the lock's own doc gives the argument).
        let Some(cm) = shared.classes.class_manager.try_read_untracked() else {
            return false;
        };
        let class_store = cm.class_store();
        // Cast: `ClassId`s are `u32` by construction (`ClassStore::next_id`).
        let upto = class_store.slot_count() as u32;
        let names: Vec<Arc<str>> = (from..upto)
            .filter_map(|slot| class_store.get(crate::classloading::ClassId::new(slot)))
            .map(|class| Arc::clone(&class.name))
            .collect();
        (names, upto)
    };
    // A resolver that could not take the lock answers `None`, which
    // `seed_pending_replay_for_class` reads as "leave the records waiting" --
    // correct, but the watermark must then not move past that class, or its
    // records would wait forever. `busy` records it.
    let busy = std::cell::Cell::new(false);
    let resolver = |name: &str| -> Option<u32> {
        let Some(cm) = shared.classes.class_manager.try_read_untracked() else {
            busy.set(true);
            return None;
        };
        cm.find_unique_class_by_name(name).map(|cid| cid.as_u32())
    };
    for name in &names {
        let _ = cratonvm_jit::profile_store::seed_pending_replay_for_class(store, name, &resolver);
    }
    if busy.get() {
        return false;
    }
    let mut marks = PROFILE_REPLAY_WATERMARKS
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    marks.retain(|(vm, _)| vm.strong_count() > 0);
    match marks
        .iter_mut()
        .find(|(vm, _)| std::ptr::eq(vm.as_ptr(), Arc::as_ptr(shared)))
    {
        // `max`: a racing visit that walked further must not be rolled back.
        Some((_, mark)) => *mark = (*mark).max(upto),
        None => marks.push((Arc::downgrade(shared), upto)),
    }
    true
}

#[cfg(test)]
mod decoded_arg_pin_tests {
    use super::{pin_decoded_jit_args, pinned_decoded_args, write_back_pinned_decoded_args};
    use crate::threading::jvm_thread::{JvmThread, ThreadId};
    use crate::types::{ObjectRef, Value};

    fn fake_ref(word: &mut [u64; 2]) -> ObjectRef {
        // SAFETY: non-null and 8-aligned; never dereferenced — the pin window
        // only stores the address and hands it back.
        unsafe { ObjectRef::from_raw(word.as_mut_ptr() as *mut u8) }
    }

    /// The decoded doors' handler-frame locals must come from the pin window,
    /// not from the pre-call copy: a collection that relocated a reference
    /// argument during the compiled call rewrites the pin, and the locals must
    /// follow it. The guard must release exactly its own window and leave a
    /// pin that was already below it alone (the synchronized-method monitor
    /// pin sits there).
    #[test]
    fn pinned_decoded_args_follow_a_relocated_reference() {
        let mut thread = JvmThread::new(ThreadId(0), "r9-vmside-pin");
        let mut a = [0u64; 2];
        let mut b = [0u64; 2];
        let mut c = [0u64; 2];
        let before = fake_ref(&mut a);
        let after = fake_ref(&mut b);
        let monitor = fake_ref(&mut c);
        thread.native_pin_roots.push(monitor);
        let base = thread.native_pin_roots.len();
        let args = [
            Value::Int(7),
            Value::Object(Some(before)),
            Value::Object(None),
            Value::Long(-1),
        ];
        let mut pins = [usize::MAX; 8];
        {
            let _guard = pin_decoded_jit_args(&mut thread, &args, &mut pins);
            assert_eq!(
                pins[..4],
                [usize::MAX, base, usize::MAX, usize::MAX],
                "only the non-null reference is pinned"
            );
            // Stand-in for the collector's remap of `native_pin_roots`.
            thread.native_pin_roots[base] = after;
            let locals = pinned_decoded_args(&thread, &args, &pins);
            assert_eq!(locals[0], Value::Int(7));
            assert_eq!(locals[1], Value::Object(Some(after)));
            assert_eq!(locals[2], Value::Object(None));
            assert_eq!(locals[3], Value::Long(-1));
        }
        assert_eq!(
            thread.native_pin_roots.len(),
            base,
            "the guard releases exactly the window it opened"
        );
        assert_eq!(thread.native_pin_roots[0], monitor);
    }

    /// r9w2-vm2: a post-call decline writes the pinned (relocated) references
    /// back into the caller's buffer, which the caller then re-runs the method
    /// interpreted from. Non-references and nulls are left alone.
    #[test]
    fn declined_decoded_call_writes_relocated_args_back() {
        let mut thread = JvmThread::new(ThreadId(0), "r9w2-vm2-writeback");
        let mut a = [0u64; 2];
        let mut b = [0u64; 2];
        let before = fake_ref(&mut a);
        let after = fake_ref(&mut b);
        ...
        let mut pins = [usize::MAX; 8];
        let guard = pin_decoded_jit_args(&mut thread, &caller, &mut pins);
        let pin = pins[0];
        // Stand-in for the collector's remap of `native_pin_roots`.
        thread.native_pin_roots[pin] = after;
        write_back_pinned_decoded_args(&thread, &mut caller, &pins);
        assert_eq!(caller[0], Value::Object(Some(after)));
        assert_eq!(caller[1], Value::Int(3));
        assert_eq!(caller[2], Value::Object(None));
        drop(guard);
        assert!(thread.native_pin_roots.is_empty());
    }

    /// More arguments than pin slots must not panic: the extra reference is
    /// simply not pinned and comes back unchanged.
    #[test]
    fn pin_slots_shorter_than_args_do_not_panic() {
        let mut thread = JvmThread::new(ThreadId(0), "r9-vmside-pin-short");
        let mut a = [0u64; 2];
        let obj = fake_ref(&mut a);
        let args = [Value::Object(Some(obj)), Value::Object(Some(obj))];
        let mut pins = [usize::MAX; 1];
        let guard = pin_decoded_jit_args(&mut thread, &args, &mut pins);
        assert_eq!(thread.native_pin_roots.len(), 1);
        let locals = pinned_decoded_args(&thread, &args, &pins);
        assert_eq!(
            locals,
            vec![Value::Object(Some(obj)), Value::Object(Some(obj))]
        );
        drop(guard);
        assert!(thread.native_pin_roots.is_empty());
    }
...
#[cfg(test)]
mod r9w3_branch_profile_arm_tests {
    use super::arm_method_branch_profile;
    use std::sync::atomic::Ordering::Relaxed;
    use std::sync::Arc;

    fn entry(code_len: usize) -> cratonvm_jit_api::CachedBytecodeMethod {
        cratonvm_jit_api::CachedBytecodeMethod {
            declaring_class_id: cratonvm_types::ClassId::new(77),
            class_name: Arc::from("r9w3/Armed"),
            method_name: Arc::from("hot"),
            method_descriptor: Arc::from("(I)I"),
            source_file: None,
            code: Arc::from(vec![0u8; code_len].as_slice()),
    ...
    /// The nomination door's arm sets the entry's flag AND arms the method's
    /// own counters, sized with the length the interpreter's recorder uses,
    /// and a second call is a no-op.
    #[test]
    fn arming_sets_the_entry_flag_and_the_methods_counters_once() {
        let store = crate::jit::profile::ProfileStore::new();
        let cached = entry(16);
        assert!(!cached.branch_profile_armed.load(Relaxed));
        assert!(arm_method_branch_profile(&store, &cached));
        assert!(cached.branch_profile_armed.load(Relaxed));
        // The same handle the interpreter's recorder gets for this frame.
        let counters = store.branch_counters_borrowed(
            cached.declaring_class_id.as_u32(),
            &cached.method_name,
            &cached.method_descriptor,
            cached.code.len(),
        ...
        assert_eq!(counters.code_len(), 16);
        // Sticky and idempotent: the second call touches nothing.
        assert!(!arm_method_branch_profile(&store, &cached));
        assert!(cached.branch_profile_armed.load(Relaxed));
    }

    /// A clone of an armed entry is armed: it names the same method.
    #[test]
    fn a_clone_of_an_armed_entry_stays_armed() {
        let store = crate::jit::profile::ProfileStore::new();
        let cached = entry(8);
        assert!(arm_method_branch_profile(&store, &cached));
        let copy = cached.clone();
        assert!(copy.branch_profile_armed.load(Relaxed));
        assert!(!entry(8).branch_profile_armed.load(Relaxed));
    }
}

#[cfg(test)]
mod r9w3_profile_replay_hook_tests {
    use super::seed_profile_replay_for_new_classes;
    use cratonvm_jit::profile_store as ps;
    use std::sync::Arc;

    const LATER: &str = "cratonvm/test/ReplayDefinedLater";

    /// A replay record whose class does not exist at load is parked, the
    /// definition hook leaves it parked while the class is still undefined,
    /// and seeds it -- under the class's real id -- once the class is defined.
    /// The page's "How to verify" unit test, driven through the VM's own
    /// hook rather than the JIT-side door.
    #[test]
    fn a_record_parked_at_load_is_seeded_when_its_class_is_defined() {
        let shared = Arc::new(crate::vm::SharedVm::new(crate::config::VmConfig::default()));
        let store = &shared.jit.profile_store;

        // A saved profile naming one method of `LATER`.
        let source = crate::jit::profile::ProfileStore::new();
        let saved_key = crate::jit::profile::MethodKey {
            class_id: 7,
            method_name: Arc::from("run"),
            descriptor: Arc::from("()I"),
        };
        source.with_method_profile_mut(&saved_key, |p| {
            p.record_branch(3, true);
            p.record_branch(3, true);
            p.record_branch(3, false);
        });
        let (bytes, _) = ps::serialize(&source, &|id: u32| -> Option<String> {
            (id == 7).then(|| LATER.to_string())
        });

        // Load time: the class is not defined yet.
        let census = ps::load_into_with_policy(
            store,
            &bytes,
            &|_name: &str| -> Option<u32> { None },
            &ps::ReplaySeedPolicy::identity(),
        )
        .expect("the file decodes");
        assert_eq!(census.methods_seeded, 0);
        assert_eq!(census.methods_unresolved_class, 1);
        assert_eq!(ps::replay_pending_census(store).0, 1);

        // A hook visit before the definition seeds nothing and keeps it waiting.
        assert!(seed_profile_replay_for_new_classes(&shared));
        assert_eq!(ps::replay_pending_census(store).0, 1);

        // The program defines the class.
        let defined = shared
            .classes
            .class_manager
            .write()
            .try_ensure_synthetic_class(LATER, 0)
            .expect("Compatible mode fabricates; this fixture never runs under --jdk-only");

        assert!(seed_profile_replay_for_new_classes(&shared));
        let (classes, methods, seeded, _) = ps::replay_pending_census(store);
        assert_eq!((classes, methods, seeded), (0, 0, 1));
        let live_key = crate::jit::profile::MethodKey {
            class_id: defined.as_u32(),
            method_name: Arc::from("run"),
            descriptor: Arc::from("()I"),
        };
        let profile = store
            .get_profile(&live_key)
            .expect("seeded under the defined class's id");
        assert!(!profile.branches.is_empty());

        // A later visit has nothing new to do and must not re-seed.
        assert!(seed_profile_replay_for_new_classes(&shared));
        assert_eq!(ps::replay_pending_census(store).2, 1);
    }
}

#[cfg(test)]
mod r9w3_osr_single_pass_preference_tests {
    use super::osr_optimizing_not_known_better;

    #[test]
    fn division_remainder_and_long_compare_prefer_single_pass() {
        // lload_0; lconst_1; ldiv; lreturn
        assert!(osr_optimizing_not_known_better(&[0x1e, 0x0a, 0x6d, 0xad]));
        // lload_0; lconst_1; lrem; lreturn
        assert!(osr_optimizing_not_known_better(&[0x1e, 0x0a, 0x71, 0xad]));
        // lload_0; lload_0; lcmp; ireturn
        assert!(osr_optimizing_not_known_better(&[0x1e, 0x1e, 0x94, 0xac]));
    }

    /// The opcode bytes are recognised only at instruction boundaries: an
    /// operand byte that happens to equal `idiv` / `lcmp` is not a division.
    #[test]
    fn operand_bytes_are_not_opcodes() {
        // bipush 0x6c; ireturn
        assert!(!osr_optimizing_not_known_better(&[0x10, 0x6c, 0xac]));
        // sipush 0x9494; ireturn
        assert!(!osr_optimizing_not_known_better(&[0x11, 0x94, 0x94, 0xac]));
        // iload_0; iconst_1; imul; ireturn -- plain arithmetic keeps the
        // optimizing body.
        assert!(!osr_optimizing_not_known_better(&[0x1a, 0x04, 0x68, 0xac]));
        // Round 9 wave 6: `int` division and remainder keep it too (measured
        // at least as fast there on the wave-5 binary).
        // iload_0; iconst_2; idiv; ireturn
        assert!(!osr_optimizing_not_known_better(&[0x1a, 0x05, 0x6c, 0xac]));
        // iload_0; iconst_2; irem; ireturn
        assert!(!osr_optimizing_not_known_better(&[0x1a, 0x05, 0x70, 0xac]));
        assert!(!osr_optimizing_not_known_better(&[]));
    }
}

#[cfg(test)]
mod r9w10_call10_osr_despec_stale_tests {
    use super::cached_osr_body_is_despec_stale;
    use crate::config::VmConfig;
    use crate::vm::SharedVm;
    use cratonvm_jit::deopt::{DeoptReason, SPECULATION_ID_ANY};
    use std::sync::Arc;

    const CLASS: &str = "r9w10/TryLoop";
    const METHOD: &str = "sumThrow";
    const DESC: &str = "([II)J";
    const KEY: &str = "r9w10/TryLoop.sumThrow:([II)J";

    /// An OSR artifact stamped with `epoch`, carrying one deopt point of
    /// `reason` at `bci` (the metadata only; the `ret` body never runs).
    fn osr_body(epoch: u64, bci: u32, reason: DeoptReason) -> cratonvm_jit::CompiledMethod {
        ...
        cm.deopt_points
            .push(cratonvm_jit::deopt::DeoptimizationPoint {
                ...
                speculation_id: cratonvm_jit::deopt::speculation_id(bci, reason),
    ...
    fn stale(shared: &SharedVm, cm: &cratonvm_jit::CompiledMethod) -> bool {
        cached_osr_body_is_despec_stale(
            shared,
            cm,
            &Arc::from(CLASS),
    ...
    /// `TryLoop throw`'s shape: an OSR body compiled before its loop-header
    /// guard was de-speculated is recompiled, not reused -- and only then.
    #[test]
    fn a_cached_osr_body_keeping_a_despeculated_guard_is_stale() {
        if !cratonvm_jit::deopt_real_enabled() {
            return; // no epochs are stamped; the predicate answers `false`
        }
        let shared = Arc::new(SharedVm::new(VmConfig::default()));
        let body = osr_body(0, 5, DeoptReason::BoundsCheck);
        assert!(
            !stale(&shared, &body),
            "nothing de-speculated, epoch current"
        );

        shared.bump_compilation_epoch(KEY);
        assert!(
            !stale(&shared, &body),
            "a deopt alone does not refuse reuse"
        );

        shared
            .jit
            .despec_registry
            .insert(KEY, 5, SPECULATION_ID_ANY);
        assert!(stale(&shared, &body), "the guard at bci 5 was withdrawn");

        // A recompile is stamped with the live epoch: it is reused even if it
        // still carried the guard, until the next charged deopt.
        let fresh = osr_body(
            shared.compilation_epoch_for(KEY),
            5,
            DeoptReason::BoundsCheck,
        );
        assert!(!stale(&shared, &fresh));

        // A de-spec at another bci does not touch this body's guard.
        let other = osr_body(0, 9, DeoptReason::BoundsCheck);
        assert!(!stale(&shared, &other));

        // An OSR-exit map is not a speculation: the recompile keeps it.
        let exit_only = osr_body(0, 5, DeoptReason::OsrExit);
        assert!(!stale(&shared, &exit_only));

        // A not-compilable method keeps its stale body: the recompile would be
        // refused, and no OSR body at all is worse.
        shared.jit.jit_skip_set.write().insert((
            Arc::from(CLASS),
            Arc::from(METHOD),
            Arc::from(DESC),
        ));
        assert!(!stale(&shared, &body));
    }
}

#[cfg(test)]
mod r9w11_osr11_tests {
    use super::{
        cached_osr_body_is_despec_stale, charge_osr_guard_exit, guard_reason_for_trapped_frame,
        osr_guard_exit_reason, osr_reentry_memo_get, osr_reentry_memo_put, osr_reentry_stamp,
    };
    use crate::config::VmConfig;
    use crate::vm::SharedVm;
    ...
    const CLASS: &str = "r9w11/TryLoopRefl";
    const METHOD: &str = "sumThrow";
    const DESC: &str = "([II)J";
    const KEY: &str = "r9w11/TryLoopRefl.sumThrow:([II)J";

    fn point(bci: u32, reason: DeoptReason, semantics: ResumeSemantics) -> DeoptimizationPoint {
        DeoptimizationPoint {
            native_offset: 0,
            bci,
            reason,
    ...
    /// The loop header of `sumThrow`: the speculative-BCE guard and the OSR-exit
    /// map at the same bci, recorded map-first as the single-pass backend does.
    fn loop_header_points() -> Vec<DeoptimizationPoint> {
        vec![
            point(5, DeoptReason::OsrExit, ResumeSemantics::REEXECUTE),
            point(5, DeoptReason::BoundsCheck, ResumeSemantics::REEXECUTE),
            point(15, DeoptReason::PendingException, ResumeSemantics::RETHROW),
        ]
    }

    /// An artifact carrying `points`, OSR-enterable at every pc below 32, with
    /// the metadata only (the `ret` body never runs).
    fn body(epoch: u64, points: Vec<DeoptimizationPoint>) -> cratonvm_jit::CompiledMethod {
        let mut buf = cratonvm_jit::ExecutableBuffer::new(64).unwrap();
        buf.emit(&[0xC3]); // ret
        let mut cm = cratonvm_jit::CompiledMethod::new(buf);
        cm.compilation_epoch = epoch;
        cm.deopt_points = points;
        cm.compiled_via_osr = true;
        cm.osr_pc_to_native = Some(vec![0; 32]);
        cm
    }

    fn frame(bci: u32) -> ReconstructedFrame {
        ReconstructedFrame {
    ...
    /// The OSR-exit test triggers make an `OsrExit` map a real exit source,
    /// and the charge refuses to run under them.
    fn test_triggers_armed() -> bool {
        cratonvm_jit::osr_exit_test_enabled()
            || cratonvm_jit::osr_exit_after().is_some()
            || cratonvm_jit::deopt_eager_enabled()
            || cratonvm_jit::deopt_eager_bci_override().is_some()
    ...
    #[test]
    fn a_loop_header_exit_is_the_guard_not_the_exit_map() {
        let points = loop_header_points();
        assert_eq!(
            osr_guard_exit_reason(&points, 5),
            Some(DeoptReason::BoundsCheck)
        );
        // The exceptional frame is not an ordinary exit.
        assert_eq!(osr_guard_exit_reason(&points, 15), None);
        // An exit map alone is no speculation.
        let exit_only = vec![point(5, DeoptReason::OsrExit, ResumeSemantics::REEXECUTE)];
        assert_eq!(osr_guard_exit_reason(&exit_only, 5), None);
        // The invokedynamic trap keeps its give-up-at-once policy elsewhere.
        let indy = vec![point(
            7,
            DeoptReason::UnreachedCode,
            ResumeSemantics::REEXECUTE,
        )];
        assert_eq!(osr_guard_exit_reason(&indy, 7), None);
        // Two different guards at one bci cannot be told apart.
        let two = vec![
            point(9, DeoptReason::BoundsCheck, ResumeSemantics::REEXECUTE),
            point(
                9,
        ...
        assert_eq!(osr_guard_exit_reason(&two, 9), None);
        let receiver = vec![point(
            9,
            DeoptReason::ReceiverTypeChanged,
            ResumeSemantics::REEXECUTE,
        )];
        assert_eq!(
            osr_guard_exit_reason(&receiver, 9),
            Some(DeoptReason::ReceiverTypeChanged)
        );
    }

    #[test]
    fn a_trapped_frame_at_a_loop_header_is_charged_as_its_guard() {
        if cratonvm_jit::osr_exit_test_enabled() || cratonvm_jit::osr_exit_after().is_some() {
            return;
        }
        let cm = body(0, loop_header_points());
        assert_eq!(
            guard_reason_for_trapped_frame(&cm, &frame(5)),
            DeoptReason::BoundsCheck,
            "not the first-in-list OsrExit map"
        );
        // Nothing at the bci: the old answer (`despec_reason_for`).
        assert_eq!(
            guard_reason_for_trapped_frame(&cm, &frame(40)),
            DeoptReason::UncommonTrap
        );
    }

    /// `TryLoopRefl`'s shape: an OSR body whose loop-header guard exits on
    /// every entry. Four charged exits withdraw exactly that guard, and the
    /// cached body is then stale for the OSR door.
    #[test]
    fn repeated_osr_guard_exits_despec_the_guard_and_stale_the_cached_body() {
        if test_triggers_armed() || !super::osr_guard_exit_charge_enabled() {
            return;
        }
        let shared = SharedVm::new(VmConfig::default());
        let cm = body(0, loop_header_points());
        let spec = cratonvm_jit::deopt::speculation_id(5, DeoptReason::BoundsCheck);
        for n in 1..=4 {
            charge_osr_guard_exit(
                &shared,
                ClassId::new(0),
            ...
            let now = shared
                .jit
                .despec_registry
                .contains_speculation(KEY, 5, spec);
            assert_eq!(now, n >= 4, "per-bci de-spec after 4 charged exits (n={n})");
        }
        if cratonvm_jit::deopt_real_enabled() {
            assert!(
                cached_osr_body_is_despec_stale(
                ...
                "the body compiled before the de-spec must be recompiled, not reused"
            );
        }
    }

    #[test]
    fn exits_that_are_not_a_failed_speculation_are_not_charged() {
        if test_triggers_armed() {
            return;
        }
        let shared = SharedVm::new(VmConfig::default());
        ...
        let exit_only = body(
            0,
            vec![point(5, DeoptReason::OsrExit, ResumeSemantics::REEXECUTE)],
        );
        let mut inlined = frame(5);
        inlined.caller_frames.push(frame(5));
        let guarded = body(0, loop_header_points());
        for _ in 0..6 {
            charge_osr_guard_exit(
                &shared,
                ClassId::new(0),
            ...
            // An inlined chain's bci is a callee's.
            charge_osr_guard_exit(
                &shared,
                ClassId::new(0),
                CLASS,
            ...
            // The identity-less re-run sentinel.
            charge_osr_guard_exit(
                &shared,
                ClassId::new(0),
                CLASS,
        ...
        assert!(!shared.jit.despec_registry.contains(KEY, 5));
    }

    /// The re-entry memo answers with the artifact it was given while every
    /// stamp holds, and with nothing as soon as one moves.
    #[test]
    fn the_reentry_memo_hits_only_while_its_stamps_hold() {
        let shared = SharedVm::new(VmConfig::default());
        let Some(stamp) = osr_reentry_stamp(&shared) else {
            return; // CRATONVM_JIT_OSR_REENTRY_MEMO=0
        };
        let class: Arc<str> = Arc::from(CLASS);
        let method: Arc<str> = Arc::from(METHOD);
        let desc: Arc<str> = Arc::from(DESC);
        let cid = ClassId::new(7);
        let code = 0x1000usize;
        let cm = Arc::new(body(0, loop_header_points()));
        osr_reentry_memo_put(&shared, stamp, code, cid, &method, &desc, 5, &cm);
        let get = |stamp: (u64, u64), code: usize, pc: usize| {
            osr_reentry_memo_get(&shared, stamp, code, cid, &class, &method, &desc, pc)
        };
        let hit = get(stamp, code, 5).expect("same entry, same stamps");
        assert!(Arc::ptr_eq(&hit, &cm));
        drop(hit);
        assert!(get(stamp, code, 6).is_none(), "another pc");
        assert!(get(stamp, 0x2000, 5).is_none(), "another bytecode");
        assert!(
            get((stamp.0 + 1, stamp.1), code, 5).is_none(),
            "a publication"
        );
        assert!(get((stamp.0, stamp.1 + 1), code, 5).is_none(), "a deopt");

        // A real deopt moves the stamp.
        crate::jit::helpers::despeculate_trapped_method(
            &shared,
            CLASS,
            METHOD,
        ...
        let moved = osr_reentry_stamp(&shared).expect("memo on");
        assert_ne!(moved, stamp);
        assert!(get(moved, code, 5).is_none());

        // A withdrawn body is never answered, even with fresh stamps.
        osr_reentry_memo_put(&shared, moved, code, cid, &method, &desc, 5, &cm);
        assert!(get(moved, code, 5).is_some());
        cm.retired.store(true, std::sync::atomic::Ordering::SeqCst);
        assert!(get(moved, code, 5).is_none(), "retired");
        cm.retired.store(false, std::sync::atomic::Ordering::SeqCst);

        // An OSR denial is asked on every hit.
        shared
            .jit
            .tiered_manager
            .mark_osr_denied(crate::jit::tiered::MethodKey::with_class_id(
                cid,
                Arc::clone(&class),
                Arc::clone(&method),
                Arc::clone(&desc),
            ));
        assert!(get(moved, code, 5).is_none(), "denied");
        shared.jit.tiered_manager.clear_osr_denials();
        assert!(get(moved, code, 5).is_some());

        // The memo holds no strong reference.
        drop(cm);
        assert!(get(moved, code, 5).is_none(), "dropped");

        // A body the door would not reuse empties the memo.
        let mut not_osr = body(0, Vec::new());
        not_osr.compiled_via_osr = false;
        let not_osr = Arc::new(not_osr);
        osr_reentry_memo_put(&shared, moved, code, cid, &method, &desc, 5, &not_osr);
        assert!(get(moved, code, 5).is_none());
    }
}
