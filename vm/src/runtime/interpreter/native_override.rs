...
pub(super) fn force_native_over_real_jdk_bytecode(
    ...
    // `next` are forced through the empty native side store instead. Scoped
    // to just these three classes and just this one arm: `EntrySet`/`KeySet`/
    // `Values`'s own methods and `TreeMap$Entry.setValue` are untouched, and
    // widening this guard to them reproduced a boot-time
    // `jdk/internal/loader/ClassLoaders.<clinit>` failure
    // (`IllegalStateException: Not yet initialized` from
    // `BootLoader.getServicesCatalog`), so this stays narrow until that is
    // understood. See
    // `docs/known-issues/jdk-only/the-treemap-iterator-force-native-gate-outlived-wave-8s-retirement-20260918.md`.
    let tree_map_iterator_carrier_prefers_real = matches!(
        class_name,
    ...
    if tree_map_iterator_carrier_prefers_real {
        return false;
    }
