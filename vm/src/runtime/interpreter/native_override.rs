...
/// FFM `java.lang.foreign.FunctionDescriptor` — the THREE `java.lang.Object`
/// names, and nothing else.
///
/// G19-1 NOM-2. Every other method on the interface is declared abstract with
/// no `Object` fallback, so `native-builtins`'
/// `register_p67_foreign_memory` registration already wins for them; these
/// three resolve to `java.lang.Object`'s CONCRETE bodies, which beat any
/// registration, and so **no registration in `foreign_ffm.rs` or `panama.rs`
/// could ever serve them**. MEASURED on 25.0.3+9-LTS against `--jdk-only`:
///
/// ```text
///   FunctionDescriptor.of(JAVA_LONG, ADDRESS).toString()
///       HotSpot   (a8)j8
///       was       java.lang.foreign.FunctionDescriptor@31e3
///   of(JAVA_INT,JAVA_INT).equals(of(JAVA_INT,JAVA_INT))
///       HotSpot   true            was  false
///   ... .hashCode() agreement across equal descriptors
///       HotSpot   true            was  false
/// ```
///
/// The identity hash is a cross-VM **diff hazard** as well as a divergence: it
/// is nondeterministic, so a vector that prints a descriptor can never be
/// compared against the oracle.
///
/// **`MemoryLayout` and `ValueLayout` are deliberately NOT here.** The same
/// measurement shows their three are already right on this VM
/// (`ValueLayout.JAVA_LONG.toString()` is `j8`,
/// `JAVA_INT.withName("a").equals(JAVA_INT.withName("a"))` is true), so adding
/// them would be a change with a blast radius across every layout in the tree
/// buying nothing. The nomination named four class families; the measurement
/// narrowed it to one.
pub(crate) fn is_ffm_function_descriptor_native_override(
    class_name: &str,
    method_name: &str,
    descriptor: &str,
) -> bool {
    class_name == "java/lang/foreign/FunctionDescriptor"
        && matches!(
            (method_name, descriptor),
            ("toString", "()Ljava/lang/String;")
                | ("equals", "(Ljava/lang/Object;)Z")
...
/// `docs/jdk-only/G34-1-who-wins-native-or-bytecode-20260817.md`.
pub(super) fn force_native_over_real_jdk_bytecode(
    ...
    // L1 wave 8 (2026-09-12, `retired_shadow.rs`'s `RETIRED_SHADOW_L1_TM_TRIPLES`)
    // retired `java/util/TreeMap`'s WHOLE family together -- the map itself,
    // its view/iterator carriers, and their producers -- specifically so a
    // producer and its consumer are never split across native and real: real
    // `TreeMap` bytecode maintains a real `root` of real `TreeMap$Entry` nodes,
    // and `tm_pairs_from_real_root`/`tm_publish_real_root` are the mirror that
    // makes the two stores agree.
    //
    // The `hasNext`/`next`/`remove` arm of the iterator-carrier block below
    // (joined 2026-08-22/29, three weeks before wave 8) was never revisited
    // for that retirement. Left unconditional for the three TreeMap carriers,
    // it forces `$EntryIterator`/`$KeyIterator`/`$ValueIterator` back onto the
    // native cursor even when `CRATONVM_REAL` has made `TreeMap.put`/
    // `entrySet`/etc. yield to real bytecode -- reintroducing exactly the
    // split wave 8's retirement was built to avoid: real `put` (yielded)
    // writes only `root`/`Entry` fields, the native cursor's own
    // `tm_array_table`/fast-mode side storage is never populated, and an
    // iterator that `iterator()` (still forced native, and still correct --
    // it seeds itself from `tm_pairs_from_real_root`, wave 8's read mirror)
    // built over the real entries reads as exhausted the moment `hasNext`/
    // `next` are forced through the empty native side store instead.
    //
    // THAT REPAIR WAS ONE MEMBER OF A FAMILY THAT WAS RETIRED AS A FAMILY, and
    // it was not enough. It skipped the force only for the three iterator
    // carriers' `hasNext`/`next`/`remove`, leaving `EntrySet`/`KeySet`/`Values`
    // and `TreeMap$Entry.setValue` forced -- so `entrySet().iterator()` still
    // minted a carrier with the native's SNAPSHOT slots filled and the real
    // `next` field null, and the now-yielding real `hasNext()` read that null
    // and called every TreeMap in the process empty. Measured on
    // `apps/probes/TmRealViewSweep.java`, `CRATONVM_REAL=all`, HotSpot
    // 25.0.4+7 as oracle: 27 of 63 rows still diverged, including the very row
    // the narrow repair was written for. Widening it was blocked at the time
    // by a boot-time `jdk/internal/loader/ClassLoaders.<clinit>` failure
    // (`IllegalStateException: Not yet initialized` from
    // `BootLoader.getServicesCatalog`) that was diagnosed as separate and
    // pre-existing, and has since been fixed at the reader in
    // `jboss_jdkspecific::build_boot_layer`.
    //
    // `tree_map_family_yields_to_real_bytecode` carries the whole family and
    // its measurement; it answers `false` for every input on an unset
    // `CRATONVM_REAL`, so an ordinary run does not change by one dispatch.
    if crate::runtime::env_cache::tree_map_family_yields_to_real_bytecode(class_name) {
        return false;
    }
    ...
    if matches!(
        ...
        "java/util/HashMap$KeyIterator"
            ...
            // The values-side twin, and it was MISSING until 2026-09-21 while
            // every other row of `VALUES_ITR_CARRIERS` had its gate entry --
            // `HashMap$ValueIterator`, `LinkedHashMap$LinkedValueIterator`,
            // `TreeMap$ValueIterator`, `TreeMap$EntryIterator`,
            // `PriorityQueue$Itr` and the two CHM rows above. Exactly the
            // omission the note at the end of this block warns about: a
            // registration and its gate row are one edit.
            //
            // Silent until `CRATONVM_REAL` is armed, because nothing else
            // promoted this class into the `SyntheticStub` yield arbitration;
            // under `CRATONVM_REAL=all` it yields, real `hasNext()` walks the
            // `tab`/`next` fields the snapshot mint never wrote, and
            // `ConcurrentHashMap.values()` iterates empty while `size()` and
            // `keySet()` stay right (`apps/probes/ChmMin.java`: `vs.size=3`,
            // `hasNext=false`, `sum=0` against HotSpot's `sum=6`).
            //
            // Single-producer, which is the condition this block's other rows
            // are held to: the second producer -- `elements()` building a real
            // cursor over the published mirror table -- was REMOVED on
            // 2026-08-28, which is what let `ConcurrentHashMap$ValuesView` join
            // `MAP_VIEW_CARRIERS` in the first place. See `native_chm_elements`
            // and the `VALUES_ITR_CARRIERS` doc comment for that measurement.
    ...
    // docs/jdk-only/H3-1-the-ratchet-that-did-not-compile-20260820.md
    ...
    if is_ffm_function_descriptor_native_override(class_name, method_name, method_descriptor) {
        return true;
    }
    ...
    matches!(
        ...
        ("java/lang/ClassLoader", "setDefaultAssertionStatus", "(Z)V")
            ...
            // H2 census 20260922: `sun.nio.ch.Util$BufferCache` is a real,
            // widely-used JDK class with real bytecode, so without a force
            // entry the receiver-has-own-bytecode rule picks the real
            // `getTemporaryDirectBuffer`/`releaseTemporaryDirectBuffer`/
            // `offerFirst`/`offerLastTemporaryDirectBuffer` bodies over the
            // registered natives (`temporary_direct_buffer_get` /
            // `temporary_direct_buffer_release` in native-io's
            // direct_buffer.rs) that exist SPECIFICALLY to avoid the real
            // bytecode's documented bug: several VM worker threads can
            // observe the same `BufferCache` instance, and its unsynchronised
            // `count`/`start`/ring-array bytecode then hands out a `count`
            // that claims a null array slot, so the caller's next
            // `buf.capacity()` NPEs with "buf is null" (or, depending on
            // timing, a `BufferUnderflowException` from a buffer that was
            // reused while still logically in flight). Confirmed empirically:
            // instrumenting `temporary_direct_buffer_get` with a debug
            // eprintln showed it was NEVER called while `TestReadOnly`,
            // `TestCluster`, `TestUrlJavaObjectSerializer`,
            // `TestAutoReconnect`, `TestJakartaServlet`,
            // `TestPageStoreCoverage`, `TestPgServer`, and `TestServlet` hit
            // exactly this crash shape under `--jdk-only` (default since
            // 2026-09-20). Force all four so the native pool — thread-local,
            // lock-free by construction — is what actually runs.
            | (
                "sun/nio/ch/Util",
...
pub(super) fn synthetic_stub_yields_with_cm(
    ...
    // Triples explicitly designated in `force_native_over_real_jdk_bytecode`
    // (such as carrier iterators that stash snapshot fields outside declared
    // layout) must not yield to real bytecode — real bytecode would walk fields
    // nothing populated.
    if force_native_over_real_jdk_bytecode(class_name, method_name, descriptor) {
        return false;
    }
