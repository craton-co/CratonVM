...
pub(super) fn op_monitorexit(
    ...
    crate::vm::vm_exec::monitor_exit_and_retract_jmx(shared, obj_ref, thread.thread_id)?;
    Ok(())
}
    ...
    match val {
        ...
        Value::Object(Some(mut obj_ref)) => {
            ...
            let result = if let Some(src_desc) = array_descriptor_of(shared, obj_ref) {
                ...
                obj_ref = thread.native_pin_roots.get(pin).copied().unwrap_or(obj_ref);
                thread.native_pin_roots.truncate(pin);
                let target_class_id = resolved
                    .map_err(|e| convert_class_not_found(shared, thread, &target_class_name, e))?;
                ...
                if let Some(epochs_at_entry) = cast_epochs {
                    if target_class_name.starts_with('[')
                        || referencing_class_has_loader_namespace(shared, referencing_class_id)
                    {
        ...
        Value::Object(Some(mut obj_ref)) => {
            ...
            if !cast_ok {
                ...
                if source_class_parser_boundary {
                    if let Value::Object(Some(strings)) = crate::vm::convert_class_values_to_strings(
                        shared,
                        Value::Object(Some(obj_ref)),
                    ) {
                        thread.frames[frame_idx]
                ...
                let is_classloader_target = target_class_name == "java/lang/ClassLoader"
                    ...
                    || target_class_name == "jdk/internal/loader/ClassLoaders$AppClassLoader"
                    || target_class_name == "jdk/internal/loader/ClassLoaders$PlatformClassLoader";
                let obj_is_bare_object =
                    actual_class_id == ClassId::new(0) || obj_class_name == "java/lang/Object";
                if is_classloader_target && obj_is_bare_object {
                    if let Some(loader_obj) =
                        cratonvm_native_builtins::classloader::peek_app_loader(shared.vm_identity)
                    {
                        tracing::debug!(
