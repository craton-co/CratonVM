...
pub(super) fn execute_invokestatic_fast_door(
    ...
    // Static synchronized is declined BEFORE the tier-up bookkeeping below,
    // not at `door_monitor_acquire` after it. It is a constant of the callee, so
    // the late refusal declined every call -- after `note_invocation_for_tierup`
    // had already counted it, and the general path then counted it again: a
    // static synchronized method reached its compile threshold at half the
    // calls, and could be offered to the tiered manager twice per stride.
    // (`door_monitor_acquire` still refuses a receiver-less monitor; this is
    // the same answer, asked before any side effect.)
    if cached.is_synchronized {
