...
pub(super) fn resume_from_ir_deopt(
    ...
    if !rframe.caller_frames.is_empty() {
        return match materialise_inlined_chain(shared, cached, rframe) {
            Ok(chain) => push_inlined_chain(shared, thread, DeoptFrameChain(chain), trace),
...
static DEOPT_FRAME_BAILS: [std::sync::atomic::AtomicU64; 11] =
...
pub(crate) fn build_deopt_frame_chain(
    ...
    // materialiser. `materialise_inner_scopes` parks the innermost scope at
    // `rframe.bci` verbatim, with no bound against that method's code — the
    // caller scopes go through `caller_resume_pc`, which does bound them, but
    // the trapping scope does not — so `u32::MAX` would become a resume pc past
    // the end of the callee's bytecode. The single-frame builder has the same
    // check for the same reason.
    if rframe.bci == u32::MAX {
        note_deopt_frame_bail(DeoptFrameBail::SupersededSentinel, rframe);
        return None;
    }
    match materialise_inlined_chain(shared, cached, rframe) {
        Ok(chain) => Some(DeoptFrameChain(chain)),
        Err(why) => {
            if cratonvm_types::flags::runtime_var_os("CRATONVM_DBG_DEOPT").is_some() {
                eprintln!(
...
pub(crate) struct DeoptFrameChain(Vec<InlinedChainFrame>);
...
pub(crate) fn run_deopt_frame_chain_to_completion(
    ...
    // "nothing happened" — see its own comment there.
    push_inlined_chain(shared, thread, chain, trace)?;
    Some(crate::runtime::interpreter::run_pushed_frame_to_completion(
        shared,
        thread,
        depth_before,
    ))
...
pub(crate) fn build_deopt_frame_inner(
    ...
    //     helper (optimizing tier only). The thread still holds it; the resumed
    ...
    let rframe: &cratonvm_jit::deopt::ReconstructedFrame = if has_virtual {
        ...
        //
        // What stays refused is an `ACC_SYNCHRONIZED` method: its method monitor
        // is not a frame-state entry at all (the invoke path takes it, not a
        // `monitorenter`), so an elision of it under scalar replacement of the
        // receiver would leave no trace here.
    ...
    // the monitor helper. This thread still holds it — the MonitorTable IS the
    // interpreter's record of block monitors, there is no per-frame list — so
...
///   for elided locks), but the SINGLE-PASS backend still describes only the
///   monitors it scalar-replaced: a non-scalar elision sets
///   `has_elided_monitor` and leaves no trace in the frame. This arm cannot
///   tell the two backends' artifacts apart, and eliding is a codegen
///   decision, not a bytecode rewrite, so the ops are still there to see —
///   which is why the refusal stays. An optimizing-tier body with monitors
///   resumes precisely through the `can_deopt_resume` arm instead;
...
pub(super) fn resume_real_ir_deopt(
    ...
    // Taken before the pin watermark below because nothing on this path pins:
    // `build_deopt_frame_chain` materialises without pushing and without
    // pinning, and `push_inlined_chain` pushes each frame's oops directly into
    // the frame that roots them. A refusal still falls through to the
    // whole-method re-run, so the worst case is exactly the old behaviour.
    if !rframe.caller_frames.is_empty() {
        let trace = cratonvm_types::flags::runtime_var_os("CRATONVM_DBG_DEOPT").is_some();
        let chain = build_deopt_frame_chain(shared, cached, rframe)?;
        return push_inlined_chain(shared, thread, chain, trace);
    }
    let pin_base = thread.native_pin_roots.len();
...
fn transfer_osr_exit_chain_into_live_frame(
    ...
    push_inlined_chain(shared, thread, DeoptFrameChain(inner), trace)
        .map(|_| ())
        .ok_or_else(|| {
...
/// The deopt reason to charge for an ordinary stashed frame, disambiguated.
///
/// Four sinks used to spell this inline as
/// `deopt_points.iter().find(|dp| dp.bci == rframe.bci)` — a
/// first-in-list-order pick over a bci that carries more than one point. After
/// the RBC.6b lift a protected invoke carries exactly two: a
/// `ReceiverTypeChanged` guard (re-execute) and a `PendingException` frame
/// (rethrow), and `jit/src/osr_exit.rs` documents that as the ordinary shape
/// for `try { foo(x); } catch`, not an exotic one.
///
/// Picking wrongly is not a cosmetic mis-label. If the guard fired but the
/// exceptional point was recorded first, the sink reports `PendingException`,
/// which is never charged, never escalates and never de-speculates — while the
/// artifact is evicted anyway. The method recompiles with the same wrong
/// speculation, traps, is evicted again, forever, with no counter anywhere
/// recording it. Symmetrically, an exceptional frame at a bci that also carries
/// a guard escalates a method that never mis-speculated.
///
/// `osr_exit::deopt_reason_at_bci` skips the `rethrow_exception` points (an
/// ordinary stash from `take_last_deopt` cannot have come from one) and refuses
/// rather than guessing when what remains disagrees.
///
/// `UncommonTrap` is the answer for both refusals. It is CHARGED, so an
/// ambiguous bci still makes the trap rate visible and the method eventually
/// backs off, and it escalates on the ordinary count-based schedule rather than
/// one-shot — so an attribution nobody could make cannot blacklist a method by
/// itself. The three `jit_bridge` sites previously fell back to
/// `UnreachedCode`, whose `recommend_action` is `MakeNotCompilable` on the
/// FIRST occurrence: "no point carries this bci" blacklisted the method.
pub(crate) fn despec_reason_for(
    ...
    match cratonvm_jit::osr_exit::deopt_reason_at_bci(&compiled.deopt_points, rframe.bci) {
        cratonvm_jit::osr_exit::ReasonAtBci::Unique(r) => r,
...
pub(super) fn despeculate_stashed_frame_method(
    ...
    // we are in. `despec_reason_for` is the disambiguating form and it needs
...
pub(super) fn real_frame_deopt_resume_and_despeculate(
    ...
    let reason = despec_reason_for(compiled, rframe);
    crate::jit::helpers::DeoptimizationController::deoptimize(
        shared,
        &cached.class_name,
        &cached.method_name,
    ...
    if rframe.bci != u32::MAX {
        ...
        if bci_deopts >= PER_BCI_DESPEC_LIMIT {
            ...
            // BOTH. `deopt_reason_at_bci` is asked directly rather than through
            // `despec_reason_for`, because that helper folds `Ambiguous` and
            // `None` into `UncommonTrap` — a real reason, and one whose derived
            // id would withdraw a speculation nothing had said anything about.
            // The two undecidable cases record `SPECULATION_ID_ANY`, the
            // wildcard, which is exactly the coarse behaviour this site had
            // before.
            let speculation = match cratonvm_jit::osr_exit::deopt_reason_at_bci(
                &compiled.deopt_points,
                rframe.bci,
            ) {
                cratonvm_jit::osr_exit::ReasonAtBci::Unique(r) => {
                    cratonvm_jit::deopt::speculation_id(rframe.bci, r)
                }
                cratonvm_jit::osr_exit::ReasonAtBci::Ambiguous
                | cratonvm_jit::osr_exit::ReasonAtBci::None => {
            ...
            shared
                .jit
                .despec_registry
                .insert(&method_key, rframe.bci, speculation);
            if cratonvm_types::flags::runtime_var_os("CRATONVM_DBG_DEOPT").is_some() {
                eprintln!(
