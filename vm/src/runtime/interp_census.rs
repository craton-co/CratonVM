...
/// The deopt stash's residual counters, printed when any is non-zero.
///
/// All three exist to read zero, and before this nothing printed them: the
/// 2026-09-18 soak that measured them needed a one-off print that was not
/// committed, so a large application run could not answer the question without
/// a rebuild. Unconditional for the same reason as
/// [`report_unrebuildable_frames`] — a residual that has to be armed is one
/// nobody reads.
///
/// * `nested` / `dropped_unclaimed` — a trap's frame was left in the ordinary
///   stash by a consumer that handled the trap without taking it, and a later
///   trap on the same thread found it there. The take discards it (it is
///   stale, never a live callee; see
///   `cratonvm_jit::deopt::deopt_stash_nesting_counts`), but while it sat there
///   `has_last_deopt()` answered "yes" for a trap that had already been
///   handled — which is what `jit_dispatch_threw` asks to tell a deopt from a
///   legitimate `Long.MIN_VALUE` return. Non-zero names a consumer to fix.
/// * `non_reexecute_refused` — a producer tried to stash a point that is not a
///   re-execute point, and the stash took the re-run sentinel instead. A
///   producer defect; the trap was handled conservatively.
fn report_deopt_stash_residual() {
    let (nested, dropped) = cratonvm_jit::deopt::deopt_stash_nesting_counts();
    let refused = cratonvm_jit::deopt::deopt_stash_non_reexecute_refused();
    if nested == 0 && dropped == 0 && refused == 0 {
        return;
    }
    eprintln!(
        "[cratonvm] WARNING: deopt stash residual: nested={nested} \
         dropped_unclaimed={dropped} non_reexecute_refused={refused}. A trap's frame was \
         left unclaimed, or a producer offered a non-re-execute point; rerun with \
         CRATONVM_DBG_DEOPT=1 to name the site. See internal/retired/\
         nested-deopt-chain-resume-m3-RETIRED-20260918.md."
    );
}

pub fn report_at_exit() {
    report_unrebuildable_frames();
    report_deopt_stash_residual();
    ...
    if crate::runtime::env_cache::dbg_jitc() && first_publish + unchanged + changed > 0 {
        ...
        // N6 -- exceptional frames a dispatch sink declined to drop because
        // they were not its own. Printed unconditionally, including the zero,
        // for the reason the line above gives: "the rule was never broken" and
        // "the check is not wired up" are the same silence otherwise. A
        // non-zero means a helper abandoned its compiled attempt while a
        // DIFFERENT method's pending-exception frame was stashed; before the
        // ownership check those were dropped silently.
        eprintln!(
            "[c2-supersede] foreign exceptional frames kept: {}",
            crate::jit::helpers::foreign_exceptional_frames_kept()
        );
        ...
        // The population behind that gate. `acceptance` above judges only the
        // bodies that LOWERED; this counts every optimizing request once, at
        // whichever of the four admission exits it left by, so a refusal the
        // gate above never saw is still a number.
        eprintln!(
            "[c2-supersede] admission funnel: {}",
            cratonvm_jit::ir_evidence::admission_funnel_line()
        );
        ...
        // Snapshot liveness (`CRATONVM_JIT_IR_SNAPSHOT_LIVENESS`). The first two
        // counts run in both arms, so an A/B of the flag reads the reclamation
        // as a difference; the snapshot counts are recorded under the flag only.
        let lv = cratonvm_jit::ir_optimize::ir_snapshot_liveness_census();
        eprintln!(
            "[c2-supersede] ir snapshot liveness: dce_killed={} write_only_stores_removed={} graphs={} unattributable={} snapshots={} unconsultable={}",
            lv.dce_nodes_killed,
            lv.write_only_stores_removed,
            lv.graphs,
            lv.unattributable,
            lv.snapshots,
            lv.unconsultable,
    ...
    if crate::runtime::env_cache::dbg_jitc() {
        ...
        eprintln!(
            "{}",
            pair_census_line(&cratonvm_jit::ir_schedule::ir_pair_census()),
        );
    }
...
/// The `[c2-supersede] ir operand pairing:` line, as a string.
///
/// # Why this is a function and not an inline `eprintln!`
///
/// It was one, and it was short a bucket. `PairCensus` gained `memory_order`
/// when the scheduler's step 5b became `check_memory_order`'s business, and
/// BOTH printers that enumerate the fields by hand silently omitted it — this
/// one and the `[ir-ls]` copy in `jit/src/ir_lower.rs`. The identity
/// `PairCensus::closes` asserts went on holding in debug builds the whole
/// time, because the VALUE was right and only the DISPLAY was missing a
/// bucket. So the line read low, and a census that reads low retires levers
/// that are engaged.
///
/// Extracting it makes the line testable — see
/// `the_pairing_line_accounts_for_every_candidate`, which fails if a bucket is
/// added to `PairCensus` and not to this format string.
///
/// **Field order matches the `[ir-ls]` copy** so the two lines can be diffed;
/// that is the only reason `no_node` is last here and third in the struct.
fn pair_census_line(pc: &cratonvm_jit::ir_schedule::PairCensus) -> String {
    format!(
        "[c2-supersede] ir operand pairing: candidates={} paired={} | declined: \
         multi_use={} producer_arm={} other_block={} after_consumer={} \
         already_adjacent={} deopt_between={} memory_order={} no_node={}",
        pc.candidates,
        pc.paired,
        pc.multi_use,
        pc.producer_arm,
...
mod tests {
    use super::*;
    use cratonvm_jit::ir_schedule::PairCensus;

    /// **Every bucket `PairCensus` counts must appear in the printed line.**
    ///
    /// Asked as arithmetic rather than as a list of field names, because a list
    /// of field names in a test is a third hand-maintained enumeration and
    /// would have been forgotten alongside the other two. `closes()` is the
    /// struct's own statement that the buckets partition `candidates`; this
    /// re-derives the same sum from the TEXT, so a bucket that exists in the
    /// struct and not in the format string makes the printed numbers add up to
    /// less than the printed denominator.
    ///
    /// That is exactly the defect this line shipped with: `memory_order` was
    /// counted, asserted and not displayed. Deleting `memory_order={}` (and its
    /// argument) from `pair_census_line` fails this test with `48 != 65`.
    #[test]
    fn the_pairing_line_accounts_for_every_candidate() {
        // Distinct primes so that a transposed pair of arguments — the other
        // way a hand-enumerated format string goes wrong — cannot cancel out.
        let declined = [2usize, 3, 5, 7, 11, 13, 17];
        let paired = 7usize;
        let pc = PairCensus {
            candidates: paired + declined.iter().sum::<usize>(),
            paired,
            multi_use: declined[0],
            producer_arm: declined[1],
            other_block: declined[2],
            after_consumer: declined[3],
            already_adjacent: declined[4],
            deopt_between: declined[5],
            memory_order: declined[6],
            no_node: 0,
        };
        assert!(
            pc.closes(),
            "the fixture itself must satisfy the identity the line reports"
        );

        let line = pair_census_line(&pc);
        // `name=NUMBER`, every occurrence, in the order printed.
        let printed: Vec<(String, usize)> = line
            .split_whitespace()
        ...
        let get = |name: &str| -> usize {
            printed
                .iter()
                .find(|(k, _)| k == name)
                .unwrap_or_else(|| panic!("`{name}` is not printed by `pair_census_line`: {line}"))
                .1
        };
        let denominator = get("candidates");
        let sum: usize = printed
            .iter()
            .filter(|(k, _)| k != "candidates")
            .map(|(_, n)| n)
            .sum();
        assert_eq!(
            sum, denominator,
            "the printed buckets do not add up to the printed denominator, so \
             the line omits one that `PairCensus::closes` counts: {line}"
        );
        // And the values landed against the right names, which the sum alone
        // cannot show.
        assert_eq!(get("memory_order"), declined[6], "{line}");
        assert_eq!(get("no_node"), 0, "{line}");
        assert_eq!(get("paired"), paired, "{line}");
    }
}
