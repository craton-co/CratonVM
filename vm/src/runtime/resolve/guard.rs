...
const ALLOWED: &[(&str, &str, usize, &str)] = &[
    ...
    (
        "vm/src/vm/realms/class_realm.rs",
        "find_method_recursive(",
        1,
        "not a resolution: `ClassRealm::unique_implementor` SURVEYS a          hierarchy, it does not resolve a member to call. Class-hierarchy          analysis asks how many loaded concrete subtypes declare a          non-abstract body for one (name, descriptor); the answer is a HINT          that the JIT lowers behind a receiver class-id compare          (`InlineVerdict::UniqueConcreteMethod`), never a binding. Routing it          through `MemberResolver` would be wrong in both directions: the          survey must answer silently for a subtype that declares nothing,          where the resolver's contract is to distinguish NoSuchMethod from          `not cached`, and it must NOT apply access control, because a          `private` or `native` selection still counts as a provider —          deliberately, so a hierarchy with one ordinary body and one such          oddity reports `many` and binds nothing. The walk is bounded by          `CHA_MAX_SUBTYPES`. A wrong answer here costs a compare that always          fails or a missed devirtualization, never a wrong target.",
    ),
    // ---------------------------------------------------------------
    ...
    (
        ...
        "migration step 4: `NativeContext` helper walks (virtual dispatch \
         ...
         The ELEVENTH (counted 2026-09-16) is the second of the two \
         `original_is_static` retarget guards, in \
         `invoke_on_class_shared_inner`'s refreshed-receiver arm - the \
         exact sibling of the one already inside the count. It resolves \
         on the ORIGINAL `class_id` to keep a static interface method \
         (`Iface::staticMethod`, e.g. JUnit \
         `TestDescriptor::containsTests`) from being retargeted onto the \
         concrete class of what is a plain SAM argument rather than a \
         dispatch receiver, which mis-resolves and recurses into the \
         lambda SAM until the stack overflows. It landed without this \
         number moving and left the guard red on `dev`; both sites \
         migrate together when this row does.",
    ),
];
