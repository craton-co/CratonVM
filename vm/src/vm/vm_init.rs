...
///
/// It had a second caller for one day: a `native_encoding_from_locale_name`
/// that read the codeset out of this string. That was replaced on 2026-09-01 by
/// `cratonvm_native_api::os_encoding`, which asks libc instead — a locale NAME
/// cannot tell an installed locale from a merely requested one, and HotSpot
/// reports what `setlocale` actually gave it.
...
/// * **It does not move `sun.jnu.encoding`.** That key decides how FILE NAMES
///   are encoded, so moving it changes class loading rather than printing —
///   a different blast radius, and still deferred.
///
/// Two bullets that stood here for one day are gone, both closed by
/// `cratonvm_native_api::os_encoding` on 2026-09-01: this DOES read a Windows
/// code page now (`GetACP`, and the attached console's code page for a stream
/// that has one), and it DOES call `setlocale`/`nl_langinfo` rather than
/// parsing the locale NAME — for `LC_CTYPE` only, on one thread, restoring the
/// previous locale, which leaves the process-global state `derive_host_locale`
/// declines to touch exactly as it was found. The name-parse it replaced could
/// not tell an installed locale from a requested one: measured on the audit
/// host, `LANG=en_US.ISO-8859-1` makes HotSpot answer `ANSI_X3.4-1968`
/// (`setlocale` failed, so the process stayed in `C`) where the parse answered
/// `ISO-8859-1`.
...
fn platform_native_encoding() -> String {
    // Delegated to `cratonvm_native_api::os_encoding`, which asks libc
    // (`setlocale(LC_CTYPE, "")` then `nl_langinfo(CODESET)`) on Unix and
    // `GetACP` on Windows, since 2026-09-01.
    //
    // It replaces a locale-NAME parse that lived here for one day and whose own
    // doc named both of its gaps: a name cannot tell a locale that is INSTALLED
    // from one that is merely requested (measured: `LANG=en_US.ISO-8859-1`
    // makes HotSpot answer `ANSI_X3.4-1968`, because `setlocale` failed and the
    // process stayed in `C`, where the parse answers `ISO-8859-1`), and the
    // Windows leg answered a constant `UTF-8` because no code page was read
    // anywhere in this tree. Both are closed there.
    //
    // The `setlocale` this was written to avoid is called once, on one thread,
    // for `LC_CTYPE` only, and the previous locale is restored — so the
    // process-global state `derive_host_locale` declines to touch is left
    ...
    match crate::config::require_real_jdk(config.java_home.as_deref()) {
        ...
        Err(detail) => Err(VmError::InvalidConfiguration(format!(
            "--jdk-only requires a real JDK runtime image, and none was found.\n\
             ...
             behaviour instead, re-run with --compatible.\n\
             \n\
             {detail}"
        ))),
    }
...
pub struct SharedVm {
    ...
    /// RT-7 — did the compiled-tier GPU array barrier arm *for this VM*?
    ///
    /// `cratonvm_jit::gpu_barrier`'s two addresses are process-global and have
    /// to be: the dirty table is drained by bucket across every VM. The
    /// DECISION that reads them is not. Whether a given method may be compiled
    /// while the input-residency cache is live depends on whether the barrier
    /// was armed before *this* VM compiled anything, and
    /// `offload_jit_gate::array_writer_policy` used to answer that from the
    /// process-wide `gpu_barrier::is_armed()` — so a second VM's arming could
    /// flip the policy underneath a first VM whose array writers had already
    /// been compiled with no barrier in them.
    ///
    /// This is that answer, taken once, from this VM's own `--gpu` state and
    /// from `gpu_barrier::arm`'s return value, which nothing used to read.
    ///
    /// `false` on every CPU-only build, on every non-x86_64 target, and under
    /// `CRATONVM_JIT_GPU_ARRAY_BARRIER=0` -- and `false` selects
    /// `ArrayWriterPolicy::KeepCache`, the historical behaviour and the safe
    /// direction.
    ///
    /// It used to be `false` also "when `arm` refused": a `--gpu` VM created
    /// after another VM had compiled code. That refusal was removed on
    /// 2026-09-17 (RT-7), with the proof at `gpu_barrier::arm`; every `--gpu`
    /// VM on x86_64 now records `true` unless the kill switch is set.
    pub gpu_array_barrier_armed: bool,
...
/// Choose one retained-trace shard from a Throwable's raw (GC-remapped)
/// heap address. Heap objects are at least 8-byte aligned, so the low 3
/// bits carry no entropy -- drop them before masking down to the shard
/// count so adjacent allocations still spread across shards.
#[inline]
fn throwable_stack_shard(address: usize) -> usize {
    let shards = crate::vm::realms::thread_realm::THROWABLE_STACK_SHARDS;
...
impl SharedVm {
    ...
    pub fn store_throwable_stack_trace(
        &self,
        throwable: ObjectRef,
        frames: Arc<[StackTraceEntry]>,
    ) {
        let address = throwable.as_ptr() as usize;
        self.threads
            .throwable_stacks
            [throwable_stack_shard(address)]
            .write()
            .insert(address, ThrowableStackTrace { throwable, frames });
    }

    /// Return an owned, source-line-resolved snapshot.
    ///
    /// Throwable construction deliberately stores exact frame identities with
    /// unresolved lines. Copy under the registry lock, then resolve after
    /// releasing it so a reader gets the eager-capture result without making
    /// caught-and-discarded exceptions pay the line-table walk.
    pub fn throwable_stack_trace_for(&self, throwable: ObjectRef) -> Option<Vec<StackTraceEntry>> {
        let address = throwable.as_ptr() as usize;
        let mut frames = self
            .threads
            .throwable_stacks
            ...
            .read()
            .get(&address)
            .map(|trace| trace.frames.as_ref().to_vec())?;
        let cm = self.classes.class_manager.read();
        crate::runtime::stackwalker::resolve_line_numbers_in_place(&cm.class_store, &mut frames);
        Some(frames)
    }

    /// Compatibility lookup for callers which hold only a Java identity hash.
    /// Construction never uses this slow path: a retained trace is keyed by the
    /// object's remapped address, so creating a Throwable does not have to mint
    /// a mark-word hash merely to populate this VM-private registry.
    pub fn throwable_stack_trace(&self, hash: i32) -> Option<Vec<StackTraceEntry>> {
        for shard in &self.threads.throwable_stacks {
            let Some(throwable) = shard
                .read()
                .values()
                .find(|trace| self.mem.heap.identity_hash_code(trace.throwable) == hash)
                .map(|trace| trace.throwable)
            else {
                continue;
            };
            if let Some(frames) = self.throwable_stack_trace_for(throwable) {
                return Some(frames);
            }
        }
        None
    ...
    ///
    /// SINGLE-THREADED INVARIANT: this loop takes a write lock on the
    /// CURRENT shard and, when a remap lands an entry in a *different*
    /// shard, ALSO takes a write lock on that destination shard while the
    /// first is still held. That nested-lock pattern is only deadlock-safe
    /// because this function is never re-entered concurrently with itself:
    /// its one call site (`update_all_roots`, `memory/gc.rs`) runs only
    /// after every other mutator is parked at a safepoint (see the
    /// `StopTheWorldToken` sites in `runtime/interpreter/gc_and_alloc.rs`
    /// that call it) — one thread, one collection cycle, no other GC worker
    /// sweeping a different shard at the same time. If root fixup is ever
    /// parallelized across worker threads, this nested lock becomes a real
    /// deadlock risk (worker A holds shard 3 wanting shard 7 while worker B
    /// holds shard 7 wanting shard 3) and must switch to a two-phase
    /// collect-then-apply pass, or take every shard lock in a fixed index
    /// order.
    pub fn remap_and_sweep_throwable_stack_traces(&self, pointer_map: &cratonvm_types::PointerMap) {
        for shard in &self.threads.throwable_stacks {
            ...
            for (_, mut trace) in old {
                let old_addr = trace.throwable.as_ptr() as usize;
                if let Some(&new_addr) = pointer_map.get(&old_addr) {
                    trace.throwable = unsafe { ObjectRef::from_raw(new_addr as *mut u8) };
                    let new_addr = trace.throwable.as_ptr() as usize;
                    let destination = throwable_stack_shard(new_addr);
                    if destination == throwable_stack_shard(old_addr) {
                        traces.insert(new_addr, trace);
                    } else {
                        self.threads.throwable_stacks[destination]
                            .write()
                            .insert(new_addr, trace);
                    }
                } else {
                    if self.mem.heap.is_object_address(old_addr).is_some() {
                        traces.insert(old_addr, trace);
                    }
                }
            }
        }
    ...
    pub fn new(mut config: VmConfig) -> Self {
        ...
        // `recompute_subclass_layouts` goes blind to this class. The interface
        // list goes through `set_interfaces` for the mirror-image reason: the
        // store's direct-IMPLEMENTOR index is what class-hierarchy analysis
        // walks (`ClassStore::subtypes_of`), and a raw `cls.interfaces.push`
        // would leave `Enumeration`/`Iterator` looking as though this class did
        // not implement them.
        if let Some(enum_impl_id) = enum_impl_id {
            class_manager.set_superclass(enum_impl_id, Some(object_id));
            let ifaces = class_manager
                .get_class(enum_impl_id)
                .map(|cls| cls.interfaces.clone());
            if let Some(mut ifaces) = ifaces {
                for add in [enumeration_id, iterator_id] {
                    if !ifaces.contains(&add) {
                        ifaces.push(add);
                    }
                }
                class_manager.set_interfaces(enum_impl_id, ifaces);
            }
        }
        ...
        if let Some(cmp_native_id) = cmp_native_id {
            ...
            // `set_interfaces` rather than a raw push, as above: the
            // direct-implementor index CHA walks is only maintained there.
            let ifaces = class_manager
                .get_class(cmp_native_id)
                .map(|cls| cls.interfaces.clone());
            if let Some(mut ifaces) = ifaces {
                if !ifaces.contains(&comparator_id) {
                    ifaces.push(comparator_id);
                    class_manager.set_interfaces(cmp_native_id, ifaces);
                }
            }
        }
        ...
        // natives are bound.
        {
            ...
            for (name, ifaces) in unmod_specs {
                ...
                // `set_interfaces` rather than a raw push: only that call keeps
                // the store's direct-implementor index — the one
                // `ClassStore::subtypes_of` and therefore class-hierarchy
                // analysis walk — in step with the field.
                let existing = class_manager
                    .get_class(cid)
                    .map(|cls| cls.interfaces.clone());
                if let Some(mut existing) = existing {
                    ...
                    class_manager.set_interfaces(cid, existing);
                }
            }
        }
        ...
        // `-Xms` is NOT a G1 override any more (2026-09-20). It used to ride on
        // `G1ConfigOverrides::initial_heap_size`, and the struct's name is why
        // only the G1 arm ever read it: on Generational and on ZGC — the
        // DEFAULT — the flag was accepted and dropped with no diagnostic. It is
        // now a parameter every arm sees, and `XmsDisposition` is each arm's
        // answer. The warning for a dropped `-Xms` lives in the launcher
        // (`vm-cli`), which is the only layer that can tell an explicit flag
        // from `VmConfig::initial_heap_size`'s 16 MiB default; the disposition
        // is bound here so the value is not silently discarded a second time.
        let (mut heap, _xms_disposition) = VmHeap::new_with_heap_sizing(
            gc_backend,
            config.max_heap_size,
            Some(config.initial_heap_size),
            g1_overrides,
        );
        ...
        //
        // RT-7 -- the RESULT is kept, on this VM, and it is the whole per-VM
        // half of the barrier: `array_writer_policy` reads this field rather
        // than the process-wide `gpu_barrier::is_armed()`. `arm` no longer
        // refuses a late arming (a VM created after another had compiled
        // code), so here it answers `true`; the proof that this is safe is at
        // `gpu_barrier::arm`, and it depends on THIS function building its own
        // heap -- a constructor that shared a heap between VM identities would
        // be the change that re-opens it.
        #[allow(unused_mut)]
        let mut gpu_array_barrier_armed = false;
        ...
        if config.gpu_offload_enabled
            && cratonvm_types::flags::runtime_var("CRATONVM_JIT_GPU_ARRAY_BARRIER")
                ...
                != Some("0")
        {
            gpu_array_barrier_armed = cratonvm_jit::gpu_barrier::arm(
        ...
        #[cfg(feature = "synthetic-jdk")]
        {
            if config.use_synthetic_jdk {
                ...
                // wildfly-standalone-boot-stw-jit-takeover-hang-FIXED.md's
                ...
                // See filehandler-noarg-ctor-handler-field-layout-gap-FIXED.md.
                ...
                // See `bug-h2-suite-residual-fail-triage-FIXED.md`'s
        ...
        // Encodings. **Read the next three paragraphs before adding a key here.**
        //
        ...
        // Both of `system_bootstrap.rs`'s copies read this map back through
        // `NativeContext::get_system_property`, so the derivation below is the
        // single source of truth in every mode and the three tables cannot
        // disagree.
        //
        // THE STREAM KEYS ARE DERIVED HERE TOO, since 2026-09-01. That is the
        // half the page above staged separately and recommended (§9, option A
        // with C as its kill switch): `stdout.encoding` decides the `Charset`
        // `install_charset` stamps on `System.out`, and therefore the BYTES —
        // under `LANG=C` HotSpot writes `?` for a character US-ASCII cannot
        // map and this VM wrote the UTF-8 sequence, ten rows of a 273-assertion
        // differential. `CRATONVM_STDOUT_ENCODING=UTF-8` restores the old
        // behaviour in one binary.
        //
        // `sun.jnu.encoding` is NOT derived, deliberately: it decides how FILE
        // NAMES are encoded, so moving it changes class loading rather than
        // printing. Different blast radius, different change.
        //
        ...
        // `startup-and-diagnostics.md` §2.5).
        ...
        let vm = Self {
            ...
            #[cfg(feature = "gpu-offload")]
            offload_registry,
            // RT-7. Taken at the arming site near the top of this function, so
            // it describes the state of the barrier BEFORE this VM could
            // compile anything — which is the only moment at which the
            // question has an answer.
            gpu_array_barrier_armed,
            ...
            threads: crate::vm::realms::ThreadRealm {
                throwable_stacks: (0..crate::vm::realms::thread_realm::THROWABLE_STACK_SHARDS)
                    .map(|_| RwLock::new(FxHashMap::default()))
                    .collect(),
                throwable_layout_cache: crate::vm::realms::thread_realm::ThrowableLayoutCache::new(),
            ...
            jit: crate::vm::realms::JitRealm {
                ...
                // M7 — this VM's runtime de-speculation verdicts. `new_registered`
                // is the only constructor: it weakly registers the registry so
                // the two VM-blind free functions in `compile_gate` (the class
                // unload / redefine forget, which the tiered manager drives)
                // can still reach it.
                runtime_despec: cratonvm_jit::compile_gate::RuntimeDespecRegistry::new_registered(),
        ...
        // ── Replay a previous run's profile, if one was named ─────────
        //
        // `CRATONVM_JIT_PROFILE_LOAD=<path>`, default unset. Here and not
        // earlier because the store must exist and profiling must already be
        // configured; here and not later because the seeds are only useful
        // before the first method gets hot.
        //
        // The resolver is `find_unique_class_by_name`, deliberately NOT the
        // deprecated loader-blind `find_class_by_name`. A class is `(name,
        // defining loader)` and the file stores only the name, so a name two
        // loaders define is ambiguous and must resolve to `None` — replay
        // simply does less under per-module loaders rather than seeding the
        // wrong class's shape.
        //
        // Never fails: a corrupt, truncated or missing file is a `warn!`/`info!`
        // and the VM continues with an empty profile. A profile is an
        // optimization hint, and **a bad profile must never be able to change a
        // program's answer** — refusing to start is a way of changing it.
        {
            let resolver = |name: &str| -> Option<u32> {
                vm.classes
                    .class_manager
                    .try_read()
                    .and_then(|cm| cm.find_unique_class_by_name(name))
                    .map(|cid| cid.as_u32())
            };
            cratonvm_jit::profile_store::load_if_configured(&vm.jit.profile_store, &resolver);
        }
...
fn jit_invalidate_adapter(class_id: u32) {
    ...
    for shared in live_hook_vms() {
        // An in-place layout change (stub upgrade, subclass re-layout,
        // redefine) moves the field indices the allocation-init recipe baked
        // (`jit_post_alloc_init`); unpublish it so the next slow-path
        // allocation rebuilds it from the new layout. Takes only the cache's
        // own `retired` mutex, never the class manager (round 9 wave 8b,
        // review8b request 1).
        let _ = shared.jit.jit_alloc_class_cache.invalidate(class_id);
...
impl SharedVm {
    ...
    pub fn jdk_only_refusal_counts(&self) -> JdkOnlyRefusalCounts {
        ...
        JdkOnlyRefusalCounts {
            ...
            jit_direct_helper_fastpaths: crate::jit::helpers::jit_direct_helper_jdk_only_refusals(),
            jit_direct_helper_admissions: crate::jit::helpers::jit_direct_helper_jdk_only_admissions(),
    ...
    ///     "jit_direct_helper_fastpaths": 0, "jit_direct_helper_admissions": 0,
    ///     "interpreter_bytecode_preferred": 0,
    ...
    ///   //
    ///   // 2026-09-22 — AND A REFUSAL IS NOT ALWAYS A BLOCK. Until this date
    ///   // `jdk_only_admit_jit_fast_native` handed §7 a step-3 input
    ///   // `vm_exec::invoke_or_native` — the tail every refusal routes to —
    ///   // does not compute for a `Bridge`, so a Bridge over concrete
    ///   // bytecode was counted here as refused and then RUN by the tail.
    ///   // Every pre-2026-09-22 reading of this column over-states what
    ///   // strict mode prevented by however many of its refusals were
    ///   // Bridges; the fix is in `jit_fast_native_bytecode_terms`, and
    ///   // `docs/internal/jdk-only/jdkonly-native-dispatch-loses-its-invoke-cache-under-strict-policy-20260922-FIXED-20260922.md`
    ///   // has the measurement. A refusal counted from here now means the
    ///   // tail refuses too.
    ///   //
    ...
    /// ```
    ///
    /// # `intrinsic_invocations_by_triple` (W7-95 N8, additive, not in the
    /// example above)
    ///
    /// ```json
    ///   "intrinsic_invocations_by_triple": [
    ///     {"class": "java/lang/Math", "name": "floorDiv", "descriptor": "(II)I",
    ///      "invocations": 5000, "invocations_complete": false}
    ///   ]
    /// ```
    ///
    /// `NativeKind::Intrinsic` is exempt from `--jdk-only` shadow enforcement,
    /// so before this field the report could say `counts.intrinsic_invocations
    /// = 5000` and name none of the calls behind it — the reproducer in
    /// `W7-95-intrinsic-semantics-census.md` ran
    /// `--jdk-only-report` and `--dump-native-registry` side by side and
    /// cross-referenced them by hand because this file had nothing to search.
    /// One row per Intrinsic triple this run actually dispatched
    /// (`owns_slot && invocations > 0`), sorted by `(class, name, descriptor)`
    /// — the same per-row `invocations`/`invocations_complete` pair
    /// `dump_native_census_json` already writes, joined into this file instead
    /// of a second one. A triple with zero invocations is not listed: silence,
    /// not a claim that it is untested — `--dump-native-registry` is still the
    /// place to ask "what is registered but never named here".
    ///
    ...
    /// so — `G60-1-what-jdk-only-still-overrides-RESOLVED-20260817.md`
    ...
    pub(crate) fn dump_jdk_only_report_json_with(
        ...
        // W7-95 N8: the same census `--dump-native-registry` reads, captured
        // once here so `intrinsic_invocations_by_triple` below can join
        // per-row invocation counts into THIS report instead of requiring a
        // second file and a hand cross-reference. See that field for why.
        let census_rows = registry.census();
        ...
        // §1.7's class-origin field, resolved HERE and nowhere on the dispatch
        // path. The three sinks above are filled by helpers that hold a name
        // triple and no class store — deliberately, because a store read lock
        // per observation is the cost their `#[cold]` split exists to avoid —
        // so every `native-shadows-bytecode` row arrives with `class_origin:
        // null` and gets it filled in once, cold, from the census that was
        // already taken three statements up.
        //
        // The SAME `dump_class_origins()` snapshot the report's `classes` block
        // is built from, not a second lookup: a row's origin and the census row
        // for that class must not be able to disagree, and re-asking would let
        // a still-running thread load a class in the gap — the reason the
        // comment above takes one L10 acquisition for both questions.
        //
        // A class the census does not name leaves the field `null`, which is
        // the honest answer: it means the triple was observed but its class is
        // not in the store under that name (an unloaded receiver, or a JIT row
        // naming a class that has since been unloaded). `<unknown>` in the
        // rendered form, absent from the registration-time variants entirely.
        let class_origins: std::collections::HashMap<&str, &str> = origins
            .as_deref()
            .unwrap_or(&[])
            .iter()
        ...
        for sink in &process_sinks {
            violations.extend(sink.iter().map(|v| {
                let resolved = v
                    .clone()
                    .with_class_origin(class_origins.get(v.class()).map(|o| (*o).to_string()));
                (
        ...
        // Additive fields, 2026-09-22. A reader that does not know them
        // still parses the object; one that does gets the door the page
        // `jdkonly-thin-jit-direct-helpers-refuse-what-the-tail-then-runs-20260922`
        // is about, which until then had a `pub fn` nobody called and so
        // reported nothing anywhere.
        out.push_str(&format!(
            "    \"jit_direct_helper_fastpaths\": {},\n",
            refusals.jit_direct_helper_fastpaths
        ));
        out.push_str(&format!(
            "    \"jit_direct_helper_admissions\": {},\n",
            refusals.jit_direct_helper_admissions
        ));
        out.push_str(&format!(
        ...
        out.push_str("    }\n");
        // THE CENSUS EXEMPTION, published beside the figure it qualifies.
        //
        // Every `native-shadows-bytecode` count in this report is a floor for a
        // reason `observation_sink` cannot express: `NativeKind::Intrinsic` is
        // §1.4's reviewed exception, so neither recorder fires for it and the
        // exempted rows were never dropped for want of room — they were never
        // OFFERED. `WORKER-3-NOTE-5` (2026-08-22) measured the static size of
        // that at 398 registry rows, +29% on the then-published 1387, and asked
        // for it to be published beside any quoted figure; nothing had. This is
        // the dispatch-time half of the same fact, from the same run that
        // produced the rows above.
        //
        // A SIBLING object rather than rows in `violations[]`, because
        // `difftest/src/census.rs` tallies that array by `kind` and moving a
        // published number by redefining its population is the one thing a
        // measurement record must not do. `events` is per dispatch and
        // `recorded` per distinct triple — the same polarity as
        // `refusals.interpreter_shadow_unenforced` against
        // `observation_sink.recorded`, and to be read the same way.
        //
        // Written unconditionally, including in `Compatible` where it reads
        // zero, for the reason `observation_sink` gives about itself: an absent
        // block makes "nothing was exempt" and "this binary cannot answer the
        // question" the same JSON.
        out.push_str("  },\n  \"census_exemptions\": {\n");
        out.push_str("    \"intrinsic_shadows\": {\n");
        out.push_str(&format!(
            "      \"events\": {},\n",
            crate::vm::jdk_only_intrinsic_shadow_exempt_events()
        ));
        let exempt = crate::vm::jdk_only_intrinsic_shadow_exempt_observations();
        ...
        out.push_str(&format!(
            "      \"truncated\": {},\n",
            crate::vm::jdk_only_intrinsic_shadow_exempt_saturated()
        ));
        out.push_str(&format!(
            "      \"dropped\": {},\n",
            crate::vm::jdk_only_intrinsic_shadow_exempt_dropped()
        ));
        out.push_str("      \"triples\": [");
        for (i, (class, method, descriptor)) in exempt.iter().enumerate() {
            if i > 0 {
                out.push(',');
            }
            out.push_str(&format!(
        ...
        out.push_str("\n    ]\n");
        out.push_str("  },\n");
        // W7-95 N8 (`W7-95-intrinsic-semantics-census.md`
        // §"make the instrument able to see this category at all"): this
        // report could say "4301 intrinsic invocations"
        // (`counts.intrinsic_invocations` above) and could name none of them.
        // `NativeKind::Intrinsic` is deliberately exempt from `--jdk-only`'s
        // shadow enforcement (it is "a correct fast path for a hot method",
        // not a shadow to refuse), so nothing else in this report ever
        // mentions an Intrinsic triple — an operator auditing one had to run
        // a SEPARATE `--dump-native-registry` and cross-reference the two
        // files by hand, which is exactly what produced the reproducer at the
        // top of that record.
        //
        // The per-triple invocation count already exists on every census row
        // (`NativeCensusEntry::invocations`, the same field
        // `dump_native_census_json` writes); this array joins it into THIS
        // report rather than duplicating the whole census. A SIBLING object,
        // like `observation_sink` and `enforcement_dial` above: additive, so a
        // reader that does not know about it still parses the file.
        //
        // Filtered to `owns_slot` (a row that lost its slot to a
        // last-write-wins re-registration was never dispatched — counting it
        // here would double-book the triple against whichever row now owns
        // it) and to `invocations > 0` (an unexercised triple is not "named",
        // it is silence; the FULL registered set, exercised or not, is what
        // `--dump-native-registry` is for). `invocations_complete` rides
        // along per row for the same reason schema 5 added it to the census:
        // a floor and a total must not render identically.
        let mut intrinsic_rows: Vec<&cratonvm_native_api::NativeCensusEntry> = census_rows
            .iter()
            .filter(|r| r.kind == NativeKind::Intrinsic && r.owns_slot && r.invocations > 0)
            .collect();
        intrinsic_rows.sort_by(|a, b| {
            (&a.class, &a.name, &a.descriptor).cmp(&(&b.class, &b.name, &b.descriptor))
        });
        out.push_str("  \"intrinsic_invocations_by_triple\": [");
        for (i, r) in intrinsic_rows.iter().enumerate() {
            if i > 0 {
                out.push(',');
            }
            out.push_str(&format!(
                "\n    {{\"class\": {}, \"name\": {}, \"descriptor\": {}, \
                 \"invocations\": {}, \"invocations_complete\": {}}}",
                json_escape(&r.class),
                json_escape(&r.name),
                json_escape(&r.descriptor),
                r.invocations,
                r.invocations_complete
            ));
        }
        if !intrinsic_rows.is_empty() {
            out.push_str("\n  ");
        }
        out.push_str("]\n}\n");
...
pub struct JdkOnlyRefusalCounts {
    ...
    /// Thin JIT direct-call helper fast paths refused at CALL time, because
    /// the generic tail they decline to would refuse as well.
    ///
    /// The callee-side twin of `jit_direct_native_binds`, and the counter
    /// `docs/internal/jdk-only/jdkonly-thin-jit-direct-helpers-refuse-what-the-tail-then-runs-20260922-FIXED-20260922.md`
    /// is about. Counter-only, like `jit_inline_cache_natives`: a guarded
    /// helper holds a constant triple and the bind-time half already recorded
    /// a row for it, so emitting a second one per call would duplicate an
    /// identity `violations[]` already carries.
    ///
    /// **A pre-2026-09-22 binary's reading of this is not comparable.** Until
    /// that date the guard refused every call in strict mode without asking
    /// whether the tail would, and the tail — `vm_exec::invoke_or_native` —
    /// ran the same `Bridge` anyway. An old number counts detours; this one
    /// counts blocks.
    pub jit_direct_helper_fastpaths: u64,
    /// Thin JIT direct-call helper fast paths ADMITTED under strict policy.
    ///
    /// Not a refusal, and deliberately excluded from [`Self::total`]. It is
    /// here because its sibling above is otherwise unreadable: zero refusals
    /// means "strict mode blocked nothing" only if something reached the
    /// guard, and this is the number that says so.
    pub jit_direct_helper_admissions: u64,
...
impl JdkOnlyRefusalCounts {
    /// Total refusal events across every refusal source. Not a violation
    /// count — see the type docs. `jit_direct_helper_admissions` and
    /// `interpreter_shadow_unenforced` are excluded: neither counts something
    /// strict policy stopped.
    pub fn total(self) -> u64 {
        self.jit_direct_native_binds
            + self.jit_inline_cache_natives
            + self.jit_fastpath_admissions
            + self.jit_direct_helper_fastpaths
...
impl Drop for Vm {
    fn drop(&mut self) {
        // ── Persist the profile, if a path was named ──────────────────
        //
        // `CRATONVM_JIT_PROFILE_SAVE=<path>`, default unset. This `Drop` is the
        // only VM-wide shutdown hook, and the save runs FIRST in it, before the
        // AOT flush and before finalizers, so a panic or a hang in either
        // cannot cost the profile.
        //
        // `report_replay_outcome` first: it is what says whether the shapes a
        // previous run handed this one were CONFIRMED or CONTRADICTED by live
        // observation. Without it a save/load pair is unfalsifiable — the next
        // run gets hints and nobody can tell whether the last run's hints were
        // any good.
        //
        // The namer is `class_name_adapter`'s `try_read` discipline for the
        // reason that function gives: blocking on the class-manager lock inside
        // a `Drop` would convert a diagnostic into a hang.
        cratonvm_jit::profile_store::report_replay_outcome(&self.shared.jit.profile_store);
        cratonvm_jit::profile_store::save_if_configured(
...
mod tests {
    ...
    fn t19_storm_counters_monotonic_nondecreasing() {
        ...
        // Simulate N allocations publishing one coalesced TLAB batch.
        for _ in 0..32 {
            shared
                .mem
        ...
        assert!(after.1 >= before.1, "gc_cycle_count went backwards");
        assert!(after.2 >= before.2, "bytes_allocated_total went backwards");
        assert_eq!(after.2 - before.2, 32 * 24);
    ...
    /// Round 9 wave 8b (review8b request 1): an in-place layout change (the
    /// JIT-invalidate hook) must unpublish the class's allocation-init recipe,
    /// whose baked field indices the change moved.
    #[test]
    fn jit_invalidate_hook_drops_the_alloc_class_recipe() {
        use crate::jit::alloc_class_cache::{ClassAllocInfo, PrimKind};
        let _guard = HOOK_REGISTRY_TEST_LOCK.lock();
        ...
        let other = 4243u32;
        for cid in [id, other] {
            let _ = shared.jit.jit_alloc_class_cache.insert(
                cid,
                ClassAllocInfo {
                    has_finalizer: false,
                    prim_inits: vec![(3, PrimKind::Int)].into_boxed_slice(),
                },
            );
        }
        assert!(shared.jit.jit_alloc_class_cache.get(id).is_some());

        jit_invalidate_adapter(id);

        assert!(
            shared.jit.jit_alloc_class_cache.get(id).is_none(),
            "a layout change must drop the recipe that baked the old indices"
        );
        assert!(
            shared.jit.jit_alloc_class_cache.get(other).is_some(),
            "another class's recipe is untouched"
        );
    }
...
//
// Census record:
// `docs/jdk-only/F30-1-the-registrar-call-graph-and-the-drifted-arm-20260813.md`.
//
