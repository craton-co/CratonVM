...
///
/// Called by the per-thread exec-depth ceiling in `interpreter::execute` (H6),
/// which is what the S-bytebuddy r2 hard cap this was written for became. That
/// rewrite dropped the call and nothing restored it until 2026-09-22, so the
/// count read zero on every run that had ever existed.
