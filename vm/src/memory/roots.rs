...
moving_young={moving_young} osr_fallback={moving_young_osr_fallback} incomplete={}",
    ...
    // 22. Reference processor: Weak/Soft/Phantom `Reference` OBJECTS the
    // processor has discovered but not yet cleared. See
    // `ReferenceProcessor::pending_reference_object_addresses` — without
    // this, a `Reference` whose last Java-visible use has already passed
    // (the common `WeakReference<T> r = new WeakReference<>(x, q);` pattern,
    // never reading `r` again) can be judged dead by the very cycle that is
    // deciding whether to clear it, silently dropping the enqueue.
    for addr in shared
        .mem
        .ref_processor
        .lock()
    ...
    {
        // SAFETY: every address here is `ReferenceEntry::reference_obj`,
        // recorded at `discover_reference` time from a live heap object
        // header and never mutated to anything else.
