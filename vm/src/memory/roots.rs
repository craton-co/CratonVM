...
pub fn collect_roots(shared: &SharedVm, thread: &JvmThread) -> Vec<ObjectRef> {
    ...
    // The coverage bit is a PRESENCE test (every safepoint recorded *an* oop
    // map), and the contract written above its assignment in
    // `jit/src/x64/driver.rs` says so explicitly: it is the NECESSARY codegen
    // precondition, and the runtime `CRATONVM_DBG_VERIFY_OOP_MAPS` oracle is
    // "the SUFFICIENT proof that must gate the actual backstop suppression".
    //
    // That gate did not exist, and it could not have: the oracle runs inside
    // `scan_one_frame_precise` <- `scan_active_jit_frames`, which is the very
    // call the line below skips. On every cycle that SPENT the bit, the
    // instrument meant to check it was not running — so no `while_covered=0`
    // measurement on this page has ever described a suppressed cycle.
    //
    // Verify first, then suppress. When the proof holds the roots collected to
