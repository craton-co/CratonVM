    ...
    if require_e2e() {
        panic!(
            "[{tag}] {REQUIRE_VAR} is set, but {what} is MISSING, so this test would have skipped \
             ...
             checkout. `tools/probes/` is the tracked home -- top-level `probes/` was \n             folded into `tools/` by `0621ddbba` and no longer exists. Unset {REQUIRE_VAR} \n             to go back to skipping."
        );
    }
    eprintln!(
        "[{tag}] SKIPPING: {what} is MISSING. Searched:\n  {searched}\nThis test asserts NOTHING \
         ...
         a fixture written there is untracked — `tools/probes/` is the tracked home \n         (top-level `probes/` no longer exists; `0621ddbba` folded it into `tools/`)."
    );
    None
}
