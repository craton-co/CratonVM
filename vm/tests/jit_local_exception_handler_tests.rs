...
/// The other half of the same stamp's ambiguity: a self-recursive method with
/// BOTH a covered and an uncovered self-call site, entered through the
/// UNCOVERED one, must let the throw escape.
///
/// The sibling above is the shape where an "outside every protected range"
/// stamp must NOT be read as a refusal. This is the shape where it must be, and
/// the 2026-08-20 remedy for the sibling broke it: routing such a stamp to the
/// pc-unknown search matches a TYPED handler on exception class alone,
/// IGNORING its protected range, so `catch (IllegalStateException)` over
/// `[27,37)` swallowed a throw at bci 23 and `recDeep(4, 4)` returned a number.
///
/// Measured on `dev` 5a6d5073b: 19,930 of 20,000 calls swallowed, in all three
/// JIT arms, 0 under `--nojit` — so this returns **20,407** rather than 40,000
/// and fails on a WRONG VALUE, not a panic. (Run the fixture's own `main` under
/// the two binaries to see it: the sibling checksum is 80,000 on both.)
///
/// **These two tests must be green together.** Each of the last two rounds
/// fixed one of them and measured only that one. See
/// `docs/internal/retired/r10-selfrec-deep-handler-leaks-once-per-million-20260922-RETIRED-20260922.md`.
#[test]
fn test_jit_self_recursive_uncovered_entry_does_not_swallow_its_throw() {
    require_class_files!();
    require_class_library!();
    assert_eq!(
        invoke_self_rec_checksum("selfRecursiveUncoveredEntryChecksum"),
        40_000
    );
}
...
#[test]
fn test_compiled_callee_whose_handler_does_not_match_propagates_without_a_rerun() {
    if !precise_handler_frame_class_files_available() {
        eprintln!("Skipping: .class files not available (javac not on PATH?)");
        return;
    }
    require_class_library!();
    // `uncaughtStep` compiles with precise exception frames (its handler reads
    // the non-parameter local `tag`), and its `catch (Boom)` does not cover the
    // `Other` its callee throws. The KCFULL-13 sink looked that up against the
    // published frame's bci, got a definite "not caught", and discarded it
    // because the athrow stamp -- which a precise stub never writes -- was
    // unknown; the method was then re-run from entry, counting `n++` twice on
    // every throw. Measured before the fix: 6667 mismatches (every throw),
    // all through `route_implicit_exc_through_callee KCFULL-13
    // bail_to_interpreter`.
    assert_eq!(precise_handler_mismatches("uncaughtMismatches"), 0);
}
