...
#[test]
fn test_jit_exception_outside_try_does_not_match_an_unrelated_handler() {
    require_class_files!();
    let verdict = jit_divzero_delta("outsideTryExceptionPropagates");
    assert_eq!(
        verdict, 1,
        "a compiled divide outside the protected range must propagate to its caller; \
         0 means no exception and -7 would mean the unrelated in-method handler caught it"
    );
}
