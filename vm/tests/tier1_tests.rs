...
// T1.9.3 — RETIRED 2026-09-21.
//
// `t1_reference_queue_remove_honors_timeout` constructed
// `cratonvm_gc::reference::ReferenceQueue` and asserted that
// `remove_timeout(50)` on an EMPTY queue blocks for at least 30 ms before
// returning `None`. That elapsed time was the defect, not the contract:
// `remove_timeout` took `&mut self`, so for the entire wait the caller held
// the only handle to the queue and nothing in the process could `enqueue`
// into it. The wait could not succeed, by construction, for any input. An
// honest implementation returns immediately, which is what made this
// assertion unportable — and it is why the type was retired rather than
// repaired (see the note above `ReferenceProcessor` in
// `gc/src/reference.rs`).
//
// T1.9.3's Java-visible subject is NOT lost. `ReferenceQueue.remove(long)` as
// a Java program calls it goes through `native-builtins/src/reference.rs`, and
// `vm/src/vm/tests.rs::reference_remove_timeout_empty` /
// `::s28_reference_queue_remove_timeout_empty` cover it there — both via
// `call_native(.., "java/lang/ref/ReferenceQueue", "remove", "(J)Ljava/lang/ref/Reference;", ..)`,
// both asserting `Value::Object(None)`, neither asserting an elapsed duration.
