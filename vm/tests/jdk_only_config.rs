...
//! the JDK feature version, and the `--compatible` fallback — and it must not
...
use cratonvm_vm::config::{
    ...
    EMBEDDED_HOST_JDK_DEFAULT_COMPATIBILITY_MODE, LAUNCHER_DEFAULT_COMPATIBILITY_MODE,
};
...
#[test]
fn launcher_default_is_real_jdk_plus_jdk_only() {
    ...
    assert_eq!(
        ...
        "…and, since 2026-09-20, treats those bytes as authoritative. The \
         opt-out is --compatible; a run that passed nothing is still \
         reproducible because the default is a declared constant, not a probe"
    );
...
#[test]
fn embedded_default_is_compatible_and_the_launcher_is_not() {
    // The embedded default differs from the launcher on BOTH axes now.
    // `JdkMode`: synthetic, so an embedder needs no JDK. `CompatibilityMode`:
    // compatible, because flipping it to strict would change the semantics of
    // every in-tree test and every embedder that never asked for it.
    ...
    // The real-JDK embedding base — what `libcratonvm` and `cratonvm-embed`
    // build on — takes the launcher's JDK mode and the embedder's policy.
    let host = VmConfig::with_host_jdk_default();
    assert_eq!(host.jdk_mode(), JdkMode::Real);
    assert_eq!(host.compatibility_mode, CompatibilityMode::Compatible);
    assert_eq!(
        EMBEDDED_HOST_JDK_DEFAULT_COMPATIBILITY_MODE,
        CompatibilityMode::Compatible
    );
}
...
#[test]
fn compatible_is_real_jdk_without_the_strict_policy() {
    // What the CLI's `--compatible` branch (and its `--real-jdk` alias)
    // builds: the real class library with the pre-2026-09-20 substitutions.
    // This is what the strict-mode error messages point at as the fallback.
    //
    // It is built from `for_launcher()` and must override, not inherit:
    // selecting the JDK mode alone leaves the policy at the launcher default,
    // which is now strict.
    let cfg = VmConfig::for_launcher()
        .with_jdk_mode(JdkMode::Real)
    ...
    assert!(cfg.validate_compatibility().is_ok());

    // And the half that pins WHY the override is needed: `with_jdk_mode`
    // carries no policy, so a bare `for_launcher().with_jdk_mode(Real)` is
    // still strict. A reader who assumes otherwise writes a "compatible"
    // config that is not one.
...
fn sample_violations() -> Vec<JdkOnlyViolation> {
    vec![
        JdkOnlyViolation::CompatibilityClassRequested {
            ...
            class_origin: Some("compatibility-stub".to_string()),
        },
        ...
        JdkOnlyViolation::NativeShadowsBytecode {
            ...
            native_kind: "bridge",
            class_origin: Some("boot-image".to_string()),
        },
...
#[test]
fn render_always_ends_with_the_compatible_fallback_and_the_report_hint() {
    // Contract §3: the long form ends with the `--compatible` fallback. This
    // is
    ...
    for v in sample_violations() {
        ...
        assert!(
            last_two[0].contains("--compatible"),
