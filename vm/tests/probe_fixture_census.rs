...
/// wins — several tests accept a `tools/probes/` copy as well as the `apps/`
/// one),
...
const FIXTURES: &[Fixture] = &[
    ...
    (
        "fjp_probe",
        // `tools/probes/` is where `0621ddbba` (consolidate bench/probes/
        // test_classes/scripts under tools/) moved this fixture; the candidate
        // list was not updated with it, so the census has been red on dev ever
        // since, and `fjp_recursive.rs` / `rfjp1_recursive.rs` have been
        // returning early and reporting `ok` while asserting nothing -- the
        // exact "dark assertions" failure `ce0561c6b` fixed once before.
        // The pre-reorg path stays listed so an older checkout still resolves.
        &[
            "apps/fjp_probe/FjpProbe.java",
            "tools/probes/FjpProbe.java",
            "probes/FjpProbe.java",
        ],
...
fn every_referenced_probe_fixture_is_present_or_baselined() {
    ...
    assert!(
        ...
        "[probe-fixture-census] {} checked-in probe fixture(s) are MISSING and are not in \
         ...
         `tools/probes/` (tracked -- top-level `probes/` was folded into `tools/` by \n         `0621ddbba` and no longer exists) and add that \
