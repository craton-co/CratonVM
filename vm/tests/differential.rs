...
fn run_cratonvm(class: &str, method: &str, descriptor: &str) -> Outcome {
    ...
    //
    // The compatibility mode is pinned rather than inherited. On 2026-09-20
    // `for_launcher()` also started carrying `CompatibilityMode::JdkOnly`, and
    // every fixture below was captured under `Compatible`. Inheriting the new
    // default would make a strict refusal read as a HotSpot divergence in a
    // harness whose whole job is to attribute divergences correctly. Moving
    // this harness to strict is a worthwhile change and a separate, measured
    // one: it needs each fixture re-read, not a constant re-pointed.
    let config = VmConfig::for_launcher()
        .with_compatibility_mode(cratonvm_vm::config::CompatibilityMode::Compatible)
        .with_classpath(vec![classpath]);
