...
fn proxy_invoke_handler_shared_is_exported() {
    ...
    // test binary.
    // (This used to reference `lambda_proxy::dispatch_count()` /
    // `bootstrap_count()`. Those were deleted in round 10: their feeders
    // `note_dispatch`/`note_bootstrap` had no caller anywhere in production —
    // the `invokedynamic.rs` caller their doc comments claimed does not exist —
    // so both counters read zero on every run that had ever existed, and a
    // linkage check is not a reader. `ImplKind` is a real part of the module
    // and serves the same purpose here.)
    assert!(cratonvm_vm::runtime::lambda_proxy::ImplKind::Virtual.is_instance());
}
