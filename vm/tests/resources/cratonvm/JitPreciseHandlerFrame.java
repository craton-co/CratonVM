...
public class JitPreciseHandlerFrame {
    ...
    // ------------------------------------------------------------------
    // uncaughtStep -- the compiled callee's own handler does NOT cover the
    // exception, and the callee is reached through the dispatch helper.
    //
    // `tag` is a non-parameter local read inside the handler, so the callee
    // compiles with precise exception frames: its protected invoke exits
    // through the reason-9 stub, which publishes a frame and stamps no athrow
    // bci. The KCFULL-13 sink (`route_implicit_exc_through_callee`) looked the
    // throw up against that frame's bci, correctly found no handler for
    // `Other`, and then threw the answer away because its own (always
    // unknown) stamp said "cannot tell" -- re-running the callee from entry
    // instead of propagating. `n++` is the non-idempotent prefix that makes the
    // re-run visible: it is counted twice on every throw.
    //
    // Two things route the call through the helper, as in the real case
    // (netty `HttpPostMultipartRequestDecoder.decodeMultipart` ->
    // `findMultipartDisposition`, where the re-run swallowed the exception):
    // the caller and the callee are mutually recursive (the `i < 0` edge is
    // never taken), and `uncaughtMismatches` warms the caller alone first, so
    // it is compiled while its call site cannot be bound directly.
    // ------------------------------------------------------------------
    ...
    static void maybeThrowOther(boolean fail) {
        if (fail) {
    ...
    static final class Uncaught {
        ...
        private int uncaughtStep(int i, boolean fail) {
            if (i < 0) {
                return run(0, 0);
            }
            n++;
    ...
    public static int uncaughtMismatches() {
        Uncaught u = new Uncaught();
        int bad = 0;
        for (int k = 0; k < 20000; k++) {
            bad += u.run(0, 0);
        }
        for (int k = 0; k < 20000; k += 50) {
            bad += u.run(k, k + 50);
        }
        return bad;
    }
