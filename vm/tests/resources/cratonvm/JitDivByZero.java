...
public class JitDivByZero {
    ...
    /**
     * The exception table protects only the array read. A divide-by-zero after
     * that try must escape to the caller below; routing it to this unrelated
     * RuntimeException handler would return -7 instead.
     */
    private static int outsideTryStep(int divisor) {
        try {
            int ignored = c[0];
        } catch (RuntimeException wrongHandler) {
            return -7;
        }
        return 1 / divisor;
    }
    ...
    /**
     * Warm the exact single-pass exception shape, then prove that the caller,
     * rather than {@link #outsideTryStep(int)}'s unrelated handler, catches the
     * final ArithmeticException.
     */
    public static int outsideTryExceptionPropagates() {
        int sink = 0;
        for (int i = 0; i < 20000; i++) {
            sink += outsideTryStep(1);
        }
        if (sink == -1) return -2;
        try {
            outsideTryStep(0);
