...
//!
//! "Structurally" held only while the single-pass body was the one installed.
//! There is a FIFTH drain, `execute`'s normal-return arm in `interpreter.rs`.
//! An optimizing body that superseded the single-pass one stores through the
//! void `jit_iastore` helper and returns into that drain, and until 2026-09-18
//! it built no message. That made `storeToNull` below fail about one run in
//! fifteen. See `jit_npe_message_from_an_optimizing_body.rs`, which takes that
//! path every time.
//!
