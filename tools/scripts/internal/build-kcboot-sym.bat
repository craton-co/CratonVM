@echo off
REM Full-symbol release build for cdb backtraces (Gap 9 wedge diagnosis).
REM Same codegen as release (lto=fat, opt3, cgu=1) but keep the PDB: debug=1
REM (per-function symbols + line tables) and strip=none. Separate target dir
REM so the working stripped binary in target-kcboot is preserved.
cd /d "%~dp0.."
...
set CARGO_PROFILE_RELEASE_DEBUG=1
set CARGO_PROFILE_RELEASE_STRIP=none
"C:\Users\Victor\.rustup\toolchains\stable-x86_64-pc-windows-msvc\bin\kccargo.exe" build --release -p cratonvm-cli --target-dir target-kcboot-sym %*
