...
| `RejectAllocation.java`   | `new int[]` AND an `[I` return → rejects on the return type (the descriptor check runs first) |
| `RejectScratchAllocation.java` | Allocates a scratch array, returns a scalar → isolates `Reason::Allocation` |
| `RejectArrayReturn.java`  | Returns a PARAMETER array, allocates nothing → `areturn` has no lowering either |
