...
w('## 10. Finding: four disagreeing boolean truth tables')
w('')
w('There is no single answer to "what does `CRATONVM_FOO=false` mean". The tree')
w('contains at least these four:')
w('')
w('| Parser | `unset` | `""` | `"0"` | `"false"` | `"off"` | `"no"` | anything else |')
w('| --- | --- | --- | --- | --- | --- | --- | --- |')
w('| `var_os(..).is_some()` (the ~600-site majority) | false | **true** | **true** | true | true | true | true |')
w('| `env_cache::disable_jit` (`env_cache.rs:70`) | false | **false** | **false** | true | true | true | true |')
w('| `native_io::env_flag_enabled` (`lib.rs:168`) | false | false | false | **false** | **false** | **false** | true |')
w('| `TieredParams` `tiered_enabled` (`tiered.rs:223`) | true | true | **false** | **false** | true | true | true |')
w('')
w('So `CRATONVM_X=0` *enables* the feature at ~600 sites and *disables* it at three')
w('others. This is a genuine footgun and the single strongest argument for one typed')
w('config: the parse happens once, in one place, with one documented truth table.')
w('')
w('**This branch does not unify the truth tables.** Each migrated flag keeps its own')
w('parse function byte-for-byte, because changing `X=0` from "on" to "off" for 600')
w('flags is a behaviour change, not a plumbing change. The typed config makes the')
w('divergence *visible* (each field records which parser it uses) so it can be')
w('retired deliberately, flag by flag, with benchmarks.')
w('')
