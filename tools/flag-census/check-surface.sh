...
# The last filter drops a match on a WHOLE-LINE `//` comment. Prose that names
# the anti-pattern -- `types/src/flag_groups.rs` explains why presence-only
# reads were converted, in a sentence containing the very call it warns about --
# is not a bypass, and failing on it leaves two bad options: reword accurate
# documentation to dodge a grep, or learn to ignore this check. It is the rule
# `jit/tests/no_presence_only_flag_reads.rs` already applies to its own scan. A
# match AFTER code on the same line is still reported, which is the
# conservative direction: `foo(); // std::env::var(..)` is ambiguous, a line
# that begins `//` is not.
