# Copies rustjvm's java launcher into a JDK-shaped tree so Maven Surefire 3.x
...
if (-not (Test-Path $Src)) {
    Write-Error "Missing $Src - run: cargo build --release -p rustjvm-cli"
}
$Bin = Join-Path $Root 'target\rustjvm-mavenjdk\bin'
