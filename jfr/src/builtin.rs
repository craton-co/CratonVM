...
///
/// # What the one production caller actually passes (audited 2026-09-20)
///
/// There is exactly one production emission site for this event, and for
/// [`emit_young_gc_event`] and [`emit_gc_heap_summary_event`] beside it:
/// `vm/src/runtime/interpreter/gc_and_alloc.rs:1450-1476`. It passes:
///
/// * `gc_id` — the **literal `1`**, on every collection. JFR's whole event
///   model is keyed on `gcId`; with every event carrying `1`, a recording
///   cannot separate two collections and any JMC view that groups by GC
///   collapses the entire run into one. `duration` is the only field on the
///   event that is real.
/// * `name` — always `"YoungGC"`, `cause` — always `"Allocation Failure"`,
///   whatever actually triggered the cycle. A `System.gc()`, an old-gen
///   threshold crossing and an allocation failure are indistinguishable.
///
/// It is also **one door, not the collection path**: collections reached
/// through `force_gc_from_native` (`System.gc()`), the allocation-failure
/// escalation, and the old-gen concurrent cycle emit nothing at all, so a
/// recording undercounts by an unknown amount as well as carrying constants.
///
/// Nothing here can fix that — this function faithfully emits what it is
/// given, and the `jfr` crate is not a dependency of `gc`. The repair and the
/// chokepoint it should move to are in
/// `docs/internal/gaps/gengc-plumbing-jfr-gc-events-are-constants-20260920.md`.
/// **Until it lands, no claim about GC frequency, cause or generation may be
/// drawn from a CratonVM JFR recording.** Use `--verbose:gc` and the `[GC] …`
/// shutdown census instead.
...
///
/// # The one production caller invents two of its three numbers (audited 2026-09-20)
///
/// `vm/src/runtime/interpreter/gc_and_alloc.rs:1468` passes `heap_used` from
/// `VmHeap::allocated_bytes()` — real — and then `heap_committed = heap_used`
/// (`// committed ≈ used for our simple heap`) and `heap_max = heap_used * 2`
/// (`// max ≈ 2x used estimate`). Neither is a measurement, and the second is
/// not even a bound: `-Xmx` is known at the call site. `gc_id` is the literal
/// `1` and `when` is always `"After GC"`, so no before/after pair exists.
/// A JMC heap-occupancy chart built from these is a drawing, not data. See
/// `docs/internal/gaps/gengc-plumbing-jfr-gc-events-are-constants-20260920.md`.
...
///
/// # NO PRODUCTION CALLER (audited 2026-09-20)
///
/// `jdk.GCPhasePause` is declared in this crate's event metadata, so a
/// recording opens cleanly in JMC and simply shows the row empty — which reads
/// as a capability rather than as its absence. `rg -n 'emit_gc_phase_pause_event'`
/// returns hits only inside this file and its own tests. **There is no
/// per-phase GC breakdown in JFR at all.**
///
/// Wiring it used to be blocked on more than a call site: the Generational
/// backend's `[gcpause]` phase rows did not partition the pause (no residual
/// term, every figure quantised to whole milliseconds), and emitting those into
/// a format people analyse with tools would have propagated that defect into
/// JMC. **That block was lifted on 2026-09-20** — the marks are microseconds,
/// there is an unconditional `other=` residual, and a `debug_assert` guards
/// against double-counting
/// (`docs/internal/gaps/gengc-plumbing-gcpause-phases-do-not-sum-20260920.md`).
/// So this is now the most tractable of the outstanding JFR steps: the data
/// behind the event is true. What remains is a call site with a real `gc_id`,
/// which is step 1 of
/// `docs/internal/gaps/gengc-plumbing-jfr-gc-events-are-constants-20260920.md`
/// and lives in `vm/`.
...
///
/// # The one production caller passes a constant tenuring threshold, and it is
/// the wrong backend's (audited 2026-09-20)
///
/// `vm/src/runtime/interpreter/gc_and_alloc.rs:1458` passes the literal `15`
/// with the comment "default tenuring threshold". That is **G1's**
/// `promotion_age` (`gc/src/g1.rs:3453`); the generational collector's
/// `PROMOTION_AGE` is **3** (`gc/src/gen_heap.rs:171`), and ZGC's is its own
/// `CRATONVM_ZGC_GEN_PROMOTION_AGE`. So the field is wrong on the backend the
/// event is emitted for, and `gc_id` is the literal `1` — see [`emit_gc_event`]
/// for the rest of the audit and
/// `docs/internal/gaps/gengc-plumbing-jfr-gc-events-are-constants-20260920.md`
/// for the repair.
...
/// # NO PRODUCTION CALLER (audited 2026-09-20)
///
/// Like [`emit_gc_phase_pause_event`], `jdk.OldGarbageCollection` is declared
/// in the metadata and emitted by nothing outside this crate's own tests, so a
/// recording shows the row empty rather than absent. The generational old-gen
/// concurrent cycle and G1's mixed collections are therefore invisible to JFR.
/// See
/// `docs/internal/gaps/gengc-plumbing-jfr-gc-events-are-constants-20260920.md`.
