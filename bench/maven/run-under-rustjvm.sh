#!/usr/bin/env bash
# bench/maven/run-under-rustjvm.sh
# WP8.7 — run the staged maven fixture under rust-jvm and capture
# stdout/stderr/rc deterministically. Compared against bench-baseline.json
# by diff-baseline.sh.
#
# Usage: bash bench/maven/run-under-rustjvm.sh
# Env:
...
if [[ ! -d "$CLASSES_DIR" ]]; then
    echo "run-maven: ERROR staged classes missing; run stage.sh first" >&2
    exit 2
fi
if [[ ! -f "$MAIN_CLASS_FILE" ]]; then
    echo "run-maven: ERROR main-class.txt missing; staging incomplete" >&2
    exit 2
fi
MAIN_CLASS="$(head -n 1 "$MAIN_CLASS_FILE" | tr -d '[:space:]')"
if [[ -z "$MAIN_CLASS" ]]; then
    echo "run-maven: ERROR main-class.txt is empty" >&2
    exit 2
fi
...
if [[ -n "${RUSTJVM_BIN:-}" && -x "$RUSTJVM_BIN" ]]; then
    ...
    echo "run-maven: ERROR rustjvm binary not found; build with 'cargo build --release -p rustjvm-cli'" >&2
    exit 3
fi
...
echo "run-maven: binary=$RUSTJVM" >&2
echo "run-maven: cp=$CP" >&2
echo "run-maven: main=$MAIN_CLASS" >&2
echo "run-maven: timeout=${TIMEOUT_SEC}s" >&2
...
echo "run-maven: rc=$RC" >&2
echo "run-maven: stdout -> $STDOUT_LOG" >&2
echo "run-maven: stderr -> $STDERR_LOG" >&2
exit 0
