...
$Fails = 0
function Report-Fail([string]$msg) { Write-Error "diff-baseline(maven): FAIL $msg"; $script:Fails++ }
...
if ($Fails -eq 0) {
    Write-Output "diff-baseline(maven): baseline match (rc=$ActualRc)"
    exit 0
}
Write-Output "diff-baseline(maven): baseline DRIFT ($Fails check(s) failed)"
exit 1
