#!/usr/bin/env bash
# bench/maven/run-under-cratonvm.sh
# WP8.7 — run the staged maven fixture under cratonvm and capture
# stdout/stderr/rc deterministically. Compared against bench-baseline.json
# by diff-baseline.sh.
#
# Usage: bash bench/maven/run-under-cratonvm.sh
# Env:
...
if [[ -n "${CRATONVM_BIN:-}" && -x "$CRATONVM_BIN" ]]; then
    ...
    echo "run-maven: ERROR cratonvm binary not found; build with 'cargo build --release -p cratonvm-cli'" >&2
    exit 3
fi
...
echo "run-maven: binary=$CRATONVM" >&2
