# bench/tomcat10/stage.ps1
# WP8.7 — stage the tomcat10 forcing-function fixture (Windows PowerShell 5.1).
#
# Network requirement: NONE today (placeholder fixture is in-tree).
# Real-distribution slot (not yet wired): if $env:TOMCAT10_HOME points at an
...
if (-not $Javac) { Write-Error "stage-tomcat10: javac not found"; exit 10 }
Add-Content -Path $CompileLog -Value "stage-tomcat10: using javac at $Javac" -Encoding utf8

$SrcFile = Join-Path $SrcDir 'Main.java'
if (-not (Test-Path $SrcFile)) {
    Write-Error "stage-tomcat10: $SrcFile missing"
    exit 11
}

Add-Content -Path $CompileLog -Value "stage-tomcat10: compiling fixture/Main.java" -Encoding utf8
$rc = Invoke-Javac -Javac $Javac -ArgList @('--release','21','-d',$ClassesDir,$SrcFile) -LogPath $CompileLog
if ($rc -ne 0) {
    Write-Error "stage-tomcat10: javac failed (rc=$rc); see $CompileLog"
    exit 12
}

Set-Content -Path $MainFile -Value 'Main' -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-tomcat10: main class = Main" -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-tomcat10: staged classes at $ClassesDir" -Encoding utf8
Write-Output "stage-tomcat10: OK"
exit 0
