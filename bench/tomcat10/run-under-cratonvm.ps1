# bench/tomcat10/run-under-cratonvm.ps1
# WP8.7 — run the staged tomcat10 fixture under cratonvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-tomcat10: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-tomcat10: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-tomcat10: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-tomcat10: cratonvm.exe not found; cargo build --release -p cratonvm-cli"; exit 3 }
...
Write-Output "run-tomcat10: binary=$Rustjvm"
Write-Output "run-tomcat10: cp=$Cp"
Write-Output "run-tomcat10: main=$MainClass"
Write-Output "run-tomcat10: timeout=${TimeoutSec}s"
...
Write-Output "run-tomcat10: rc=$Rc"
Write-Output "run-tomcat10: stdout -> $StdoutLog"
Write-Output "run-tomcat10: stderr -> $StderrLog"
exit 0
