...
# WP8.7 — compare the last rust-jvm run for tomcat10 against bench-baseline.json.
...
for f in "$STDOUT_LOG" "$STDERR_LOG" "$RC_FILE"; do
    if [[ ! -f "$f" ]]; then
        echo "diff-baseline(tomcat10): ERROR $f missing; run run-under-rustjvm.sh first" >&2
        exit 2
    fi
done
