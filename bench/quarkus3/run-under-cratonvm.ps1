# bench/quarkus3/run-under-cratonvm.ps1
# WP8.7 — run the staged quarkus3 fixture under cratonvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-quarkus3: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-quarkus3: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-quarkus3: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-quarkus3: cratonvm.exe not found; cargo build --release -p cratonvm-cli"; exit 3 }
...
Write-Output "run-quarkus3: binary=$Rustjvm"
Write-Output "run-quarkus3: cp=$Cp"
Write-Output "run-quarkus3: main=$MainClass"
Write-Output "run-quarkus3: timeout=${TimeoutSec}s"
...
Write-Output "run-quarkus3: rc=$Rc"
Write-Output "run-quarkus3: stdout -> $StdoutLog"
Write-Output "run-quarkus3: stderr -> $StderrLog"
exit 0
