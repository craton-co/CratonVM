# bench/quarkus3/diff-baseline.ps1
...
if (-not (Test-Path $Baseline))  { Write-Error "diff-baseline(quarkus3): $Baseline missing"; exit 2 }
foreach ($f in @($StdoutLog, $StderrLog, $RcFile)) {
    if (-not (Test-Path $f)) { Write-Error "diff-baseline(quarkus3): $f missing; run run-under-cratonvm.ps1 first"; exit 2 }
}
...
$Fails = 0
function Report-Fail([string]$msg) { Write-Error "diff-baseline(quarkus3): FAIL $msg"; $script:Fails++ }
...
if ($Fails -eq 0) {
    Write-Output "diff-baseline(quarkus3): baseline match (rc=$ActualRc)"
    exit 0
}
Write-Output "diff-baseline(quarkus3): baseline DRIFT ($Fails check(s) failed)"
exit 1
