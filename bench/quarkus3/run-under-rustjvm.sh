#!/usr/bin/env bash
# bench/quarkus3/run-under-rustjvm.sh
# WP8.7 — run the staged quarkus3 fixture under rust-jvm and capture
# stdout/stderr/rc deterministically. Compared against bench-baseline.json
# by diff-baseline.sh.
#
# Usage: bash bench/quarkus3/run-under-rustjvm.sh
# Env:
...
if [[ -n "${RUSTJVM_BIN:-}" && -x "$RUSTJVM_BIN" ]]; then
    ...
    echo "run-quarkus3: ERROR rustjvm binary not found; build with 'cargo build --release -p rustjvm-cli'" >&2
    exit 3
fi
...
echo "run-quarkus3: binary=$RUSTJVM" >&2
