#!/usr/bin/env bash
# bench/quarkus3/run-under-cratonvm.sh
# WP8.7 — run the staged quarkus3 fixture under cratonvm and capture
# stdout/stderr/rc deterministically. Compared against bench-baseline.json
# by diff-baseline.sh.
#
# Usage: bash bench/quarkus3/run-under-cratonvm.sh
# Env:
...
if [[ ! -d "$CLASSES_DIR" ]]; then
    echo "run-quarkus3: ERROR staged classes missing; run stage.sh first" >&2
    exit 2
fi
if [[ ! -f "$MAIN_CLASS_FILE" ]]; then
    echo "run-quarkus3: ERROR main-class.txt missing; staging incomplete" >&2
    exit 2
fi
MAIN_CLASS="$(head -n 1 "$MAIN_CLASS_FILE" | tr -d '[:space:]')"
if [[ -z "$MAIN_CLASS" ]]; then
    echo "run-quarkus3: ERROR main-class.txt is empty" >&2
    exit 2
fi
...
if [[ -n "${CRATONVM_BIN:-}" && -x "$CRATONVM_BIN" ]]; then
    ...
    echo "run-quarkus3: ERROR cratonvm binary not found; build with 'cargo build --release -p cratonvm-cli'" >&2
    exit 3
fi
...
echo "run-quarkus3: binary=$CRATONVM" >&2
echo "run-quarkus3: cp=$CP" >&2
echo "run-quarkus3: main=$MAIN_CLASS" >&2
echo "run-quarkus3: timeout=${TIMEOUT_SEC}s" >&2
...
echo "run-quarkus3: rc=$RC" >&2
echo "run-quarkus3: stdout -> $STDOUT_LOG" >&2
echo "run-quarkus3: stderr -> $STDERR_LOG" >&2
exit 0
