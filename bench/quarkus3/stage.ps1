# bench/quarkus3/stage.ps1
# WP8.7 — stage the quarkus3 forcing-function fixture (Windows PowerShell 5.1).
#
# Network requirement: NONE today (placeholder fixture is in-tree).
# Real-distribution slot (not yet wired): if $env:QUARKUS3_PROJECT points at an
...
if (-not $Javac) { Write-Error "stage-quarkus3: javac not found"; exit 10 }
Add-Content -Path $CompileLog -Value "stage-quarkus3: using javac at $Javac" -Encoding utf8

$SrcFile = Join-Path $SrcDir 'Main.java'
if (-not (Test-Path $SrcFile)) {
    Write-Error "stage-quarkus3: $SrcFile missing"
    exit 11
}

Add-Content -Path $CompileLog -Value "stage-quarkus3: compiling fixture/Main.java" -Encoding utf8
$rc = Invoke-Javac -Javac $Javac -ArgList @('--release','21','-d',$ClassesDir,$SrcFile) -LogPath $CompileLog
if ($rc -ne 0) {
    Write-Error "stage-quarkus3: javac failed (rc=$rc); see $CompileLog"
    exit 12
}

Set-Content -Path $MainFile -Value 'Main' -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-quarkus3: main class = Main" -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-quarkus3: staged classes at $ClassesDir" -Encoding utf8
Write-Output "stage-quarkus3: OK"
exit 0
