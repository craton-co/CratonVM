# bench/run-jar-under-cratonvm.ps1
# Run a JAR under cratonvm.exe with async stdout/stderr reads before materializing exit code.
...
#   CRATONVM_BIN       - path to cratonvm.exe (overrides -RustJvmBin)
...
#   CRATONVM_DISABLE_JIT, CRATONVM_* - passed through like any env var
#   CRATONVM_CLI_EXTRA - optional extra CLI tokens (space-separated) inserted before --jar
...
if (-not $JavaHome) {
    ...
    else { Write-Error 'run-jar-under-cratonvm: set JAVA_HOME or pass -JavaHome' }
}
if (-not (Test-Path (Join-Path $JavaHome 'bin\java.exe'))) {
    Write-Error "run-jar-under-cratonvm: JAVA_HOME invalid (no bin\java.exe): $JavaHome"
}

if (-not $RustJvmBin) {
    if ($env:CRATONVM_BIN -and (Test-Path $env:CRATONVM_BIN)) {
        $RustJvmBin = $env:CRATONVM_BIN
    }
    elseif (Test-Path (Join-Path $RepoRoot 'target\release\cratonvm.exe')) {
        $RustJvmBin = Join-Path $RepoRoot 'target\release\cratonvm.exe'
    }
    elseif (Test-Path (Join-Path $RepoRoot 'target\wf-build2\release\cratonvm.exe')) {
        $RustJvmBin = Join-Path $RepoRoot 'target\wf-build2\release\cratonvm.exe'
    }
}
if (-not $RustJvmBin -or -not (Test-Path $RustJvmBin)) {
    Write-Error 'run-jar-under-cratonvm: cratonvm.exe not found; set CRATONVM_BIN or build cratonvm-cli'
}
...
$AllArgs.Add('--Xmx')
$AllArgs.Add($Xmx)
if ($env:CRATONVM_CLI_EXTRA) {
    foreach ($tok in ($env:CRATONVM_CLI_EXTRA -split '\s+')) {
...
Write-Output "run-jar-under-cratonvm: binary=$RustJvmBin"
Write-Output "run-jar-under-cratonvm: argv=$argLine"
Write-Output "run-jar-under-cratonvm: timeout_sec=$TimeoutSec (0 = infinite)"
...
if (-not $p) { Write-Error 'run-jar-under-cratonvm: Process::Start returned null' }
...
Write-Output "run-jar-under-cratonvm: rc=$Rc"
Write-Output "run-jar-under-cratonvm: stdout -> $OutLog"
Write-Output "run-jar-under-cratonvm: stderr -> $ErrLog"
Write-Output "run-jar-under-cratonvm: rc file -> $RcFile"

if ($Stderr -and $Stderr.Length -gt 0) {
    Write-Output "run-jar-under-cratonvm: --- last $TailN stderr lines ---"
    $Stderr -split "`r?`n" | Select-Object -Last $TailN | ForEach-Object { Write-Output $_ }
    Write-Output "run-jar-under-cratonvm: --- end stderr tail ---"
}
