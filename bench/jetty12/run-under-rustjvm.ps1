# bench/jetty12/run-under-rustjvm.ps1
# WP8.7 — run the staged jetty12 fixture under rust-jvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-jetty12: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-jetty12: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-jetty12: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-jetty12: rustjvm.exe not found; cargo build --release -p rustjvm-cli"; exit 3 }
...
Write-Output "run-jetty12: binary=$Rustjvm"
Write-Output "run-jetty12: cp=$Cp"
Write-Output "run-jetty12: main=$MainClass"
Write-Output "run-jetty12: timeout=${TimeoutSec}s"
...
Write-Output "run-jetty12: rc=$Rc"
Write-Output "run-jetty12: stdout -> $StdoutLog"
Write-Output "run-jetty12: stderr -> $StderrLog"
exit 0
