...
# WP8.7 — compare the last cratonvm run for jetty12 against bench-baseline.json.
...
for f in "$STDOUT_LOG" "$STDERR_LOG" "$RC_FILE"; do
    if [[ ! -f "$f" ]]; then
        echo "diff-baseline(jetty12): ERROR $f missing; run run-under-cratonvm.sh first" >&2
        exit 2
    fi
done
