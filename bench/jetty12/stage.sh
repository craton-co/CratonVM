#!/usr/bin/env bash
# bench/jetty12/stage.sh
# WP8.7 — stage the jetty12 forcing-function fixture.
#
...
#   * If $JETTY12_HOME points at an unpacked distribution, prefer it.
...
if [[ -n "${JAVA_HOME:-}" && -x "$JAVA_HOME/bin/javac" ]]; then
    ...
    echo "stage-jetty12: ERROR javac not found (JAVA_HOME unset and javac not on PATH)" >&2
    exit 10
fi
echo "stage-jetty12: using javac at $JAVAC" | tee -a "$COMPILE_LOG"

if [[ ! -f "$SRC_DIR/Main.java" ]]; then
    echo "stage-jetty12: ERROR $SRC_DIR/Main.java missing" >&2
    exit 11
fi

echo "stage-jetty12: compiling fixture/Main.java" | tee -a "$COMPILE_LOG"
if ! "$JAVAC" --release 21 -d "$CLASSES_DIR" "$SRC_DIR/Main.java" >> "$COMPILE_LOG" 2>&1; then
    echo "stage-jetty12: ERROR javac failed; see $COMPILE_LOG" >&2
    tail -n 40 "$COMPILE_LOG" >&2 || true
    exit 12
fi

echo "Main" > "$MAIN_CLASS_FILE"
echo "stage-jetty12: main class = Main" | tee -a "$COMPILE_LOG"
echo "stage-jetty12: staged classes at $CLASSES_DIR" | tee -a "$COMPILE_LOG"
echo "stage-jetty12: OK"
exit 0
