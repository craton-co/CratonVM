# bench/gradle/run-under-cratonvm.ps1
# WP8.7 — run the staged gradle fixture under cratonvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-gradle: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-gradle: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-gradle: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-gradle: cratonvm.exe not found; cargo build --release -p cratonvm-cli"; exit 3 }
...
Write-Output "run-gradle: binary=$Rustjvm"
Write-Output "run-gradle: cp=$Cp"
Write-Output "run-gradle: main=$MainClass"
Write-Output "run-gradle: timeout=${TimeoutSec}s"
...
Write-Output "run-gradle: rc=$Rc"
Write-Output "run-gradle: stdout -> $StdoutLog"
Write-Output "run-gradle: stderr -> $StderrLog"
exit 0
