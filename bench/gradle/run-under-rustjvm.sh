#!/usr/bin/env bash
# bench/gradle/run-under-rustjvm.sh
# WP8.7 — run the staged gradle fixture under rust-jvm and capture
# stdout/stderr/rc deterministically. Compared against bench-baseline.json
# by diff-baseline.sh.
#
# Usage: bash bench/gradle/run-under-rustjvm.sh
# Env:
...
if [[ ! -d "$CLASSES_DIR" ]]; then
    echo "run-gradle: ERROR staged classes missing; run stage.sh first" >&2
    exit 2
fi
if [[ ! -f "$MAIN_CLASS_FILE" ]]; then
    echo "run-gradle: ERROR main-class.txt missing; staging incomplete" >&2
    exit 2
fi
MAIN_CLASS="$(head -n 1 "$MAIN_CLASS_FILE" | tr -d '[:space:]')"
if [[ -z "$MAIN_CLASS" ]]; then
    echo "run-gradle: ERROR main-class.txt is empty" >&2
    exit 2
fi
...
if [[ -n "${RUSTJVM_BIN:-}" && -x "$RUSTJVM_BIN" ]]; then
    ...
    echo "run-gradle: ERROR rustjvm binary not found; build with 'cargo build --release -p rustjvm-cli'" >&2
    exit 3
fi
...
echo "run-gradle: binary=$RUSTJVM" >&2
echo "run-gradle: cp=$CP" >&2
echo "run-gradle: main=$MAIN_CLASS" >&2
echo "run-gradle: timeout=${TIMEOUT_SEC}s" >&2
...
echo "run-gradle: rc=$RC" >&2
echo "run-gradle: stdout -> $STDOUT_LOG" >&2
echo "run-gradle: stderr -> $STDERR_LOG" >&2
exit 0
