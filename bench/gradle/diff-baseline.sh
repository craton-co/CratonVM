#!/usr/bin/env bash
# bench/gradle/diff-baseline.sh
# WP8.7 — compare the last cratonvm run for gradle against bench-baseline.json.
...
if [[ ! -f "$BASELINE" ]]; then
    echo "diff-baseline(gradle): ERROR $BASELINE missing" >&2
    exit 2
fi
for f in "$STDOUT_LOG" "$STDERR_LOG" "$RC_FILE"; do
    if [[ ! -f "$f" ]]; then
        echo "diff-baseline(gradle): ERROR $f missing; run run-under-cratonvm.sh first" >&2
        exit 2
    fi
done
...
PY
        ...
        echo "diff-baseline(gradle): WARN no jq/python3 — array fields cannot be parsed" >&2
...
FAILS=0
report_fail() { echo "diff-baseline(gradle): FAIL $*" >&2; FAILS=$((FAILS + 1)); }
...
if (( FAILS == 0 )); then
    echo "diff-baseline(gradle): baseline match (rc=$ACTUAL_RC)"
    exit 0
fi
echo "diff-baseline(gradle): baseline DRIFT ($FAILS check(s) failed) — update bench-baseline.json if intentional, else investigate as a regression"
exit 1
