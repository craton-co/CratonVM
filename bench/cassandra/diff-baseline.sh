...
# WP8.7 — compare the last rust-jvm run for cassandra against bench-baseline.json.
...
for f in "$STDOUT_LOG" "$STDERR_LOG" "$RC_FILE"; do
    if [[ ! -f "$f" ]]; then
        echo "diff-baseline(cassandra): ERROR $f missing; run run-under-rustjvm.sh first" >&2
        exit 2
    fi
done
