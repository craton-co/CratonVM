#!/usr/bin/env bash
# bench/cassandra/run-under-rustjvm.sh
# WP8.7 — run the staged cassandra fixture under rust-jvm and capture
# stdout/stderr/rc deterministically. Compared against bench-baseline.json
# by diff-baseline.sh.
#
# Usage: bash bench/cassandra/run-under-rustjvm.sh
# Env:
...
if [[ -n "${RUSTJVM_BIN:-}" && -x "$RUSTJVM_BIN" ]]; then
    ...
    echo "run-cassandra: ERROR rustjvm binary not found; build with 'cargo build --release -p rustjvm-cli'" >&2
    exit 3
fi
...
echo "run-cassandra: binary=$RUSTJVM" >&2
