# bench/cassandra/run-under-cratonvm.ps1
# WP8.7 — run the staged cassandra fixture under cratonvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-cassandra: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-cassandra: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-cassandra: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-cassandra: cratonvm.exe not found; cargo build --release -p cratonvm-cli"; exit 3 }
...
Write-Output "run-cassandra: binary=$Rustjvm"
Write-Output "run-cassandra: cp=$Cp"
Write-Output "run-cassandra: main=$MainClass"
Write-Output "run-cassandra: timeout=${TimeoutSec}s"
...
Write-Output "run-cassandra: rc=$Rc"
Write-Output "run-cassandra: stdout -> $StdoutLog"
Write-Output "run-cassandra: stderr -> $StderrLog"
exit 0
