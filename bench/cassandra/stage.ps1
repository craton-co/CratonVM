# bench/cassandra/stage.ps1
# WP8.7 — stage the cassandra forcing-function fixture (Windows PowerShell 5.1).
#
# Network requirement: NONE today (placeholder fixture is in-tree).
# Real-distribution slot (not yet wired): if $env:CASSANDRA_HOME points at an
...
if (-not $Javac) { Write-Error "stage-cassandra: javac not found"; exit 10 }
Add-Content -Path $CompileLog -Value "stage-cassandra: using javac at $Javac" -Encoding utf8

$SrcFile = Join-Path $SrcDir 'Main.java'
if (-not (Test-Path $SrcFile)) {
    Write-Error "stage-cassandra: $SrcFile missing"
    exit 11
}

Add-Content -Path $CompileLog -Value "stage-cassandra: compiling fixture/Main.java" -Encoding utf8
$rc = Invoke-Javac -Javac $Javac -ArgList @('--release','21','-d',$ClassesDir,$SrcFile) -LogPath $CompileLog
if ($rc -ne 0) {
    Write-Error "stage-cassandra: javac failed (rc=$rc); see $CompileLog"
    exit 12
}

Set-Content -Path $MainFile -Value 'Main' -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-cassandra: main class = Main" -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-cassandra: staged classes at $ClassesDir" -Encoding utf8
Write-Output "stage-cassandra: OK"
exit 0
