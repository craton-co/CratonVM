# bench/wildfly — EJBCA-vs-rust-jvm smoke harness

Infrastructure built by **WP0.5** of `docs/wildfly-ejbca-roadmap.md`. Pins today's (2026-04-24) EJBCA-on-rust-jvm reality as a repeatable baseline so future waves can detect regressions.
...
stage-ejbca-min.{sh,ps1}  ->  compile a tiny EJBCA/cesecore-shaped Java fixture
          |
          v
run-under-rustjvm.{sh,ps1}  ->  execute under target/release/rustjvm[.exe],
...
| `run-under-rustjvm.sh` / `.ps1` | Run the staged fixture under `target/release/rustjvm[.exe]` with the classpath set to `staged/classes/` plus any staged JARs. Writes `last-run.{stdout,stderr}.log`, `last-run.rc`, `last-run.meta.json`. |
...
powershell -ExecutionPolicy Bypass -File bench\wildfly\run-under-rustjvm.ps1
...
- `RUSTJVM_BIN` — override the default `target/release/rustjvm[.exe]` path.
...
- `run`   exits **rc=1** — rust-jvm throws `NullPointerException: Cannot invoke write on null` on the 2nd `System.out.println`. This is the **WP0.1** blocker documented in `memory/finding_println_regression.md`. The fixture deliberately emits two prints so the harness will notice the day WP0.1 lands.
...
1. Build rustjvm once: `cargo build --release -p rustjvm-cli`.
