# bench/springboot3/run-under-cratonvm.ps1
# WP8.7 — run the staged springboot3 fixture under cratonvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-springboot3: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-springboot3: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-springboot3: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-springboot3: cratonvm.exe not found; cargo build --release -p cratonvm-cli"; exit 3 }
...
Write-Output "run-springboot3: binary=$Rustjvm"
Write-Output "run-springboot3: cp=$Cp"
Write-Output "run-springboot3: main=$MainClass"
Write-Output "run-springboot3: timeout=${TimeoutSec}s"
...
Write-Output "run-springboot3: rc=$Rc"
Write-Output "run-springboot3: stdout -> $StdoutLog"
Write-Output "run-springboot3: stderr -> $StderrLog"
exit 0
