...
if [[ ! -f "$BASELINE" ]]; then
    echo "diff-baseline(springboot3): ERROR $BASELINE missing" >&2
    exit 2
fi
...
PY
        ...
        echo "diff-baseline(springboot3): WARN no jq/python3 — array fields cannot be parsed" >&2
...
FAILS=0
report_fail() { echo "diff-baseline(springboot3): FAIL $*" >&2; FAILS=$((FAILS + 1)); }
...
if (( FAILS == 0 )); then
    echo "diff-baseline(springboot3): baseline match (rc=$ACTUAL_RC)"
    exit 0
fi
echo "diff-baseline(springboot3): baseline DRIFT ($FAILS check(s) failed) — update bench-baseline.json if intentional, else investigate as a regression"
exit 1
