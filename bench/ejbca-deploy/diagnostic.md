...
**Blocking dep:** WP8.10 (real WildFly boot under rust-jvm) — until that
...
HTTP byte, cross-reference against existing rust-jvm code paths.

Companion smoke test: `vm/tests/wp8_11_ejbca_bootstrap_smoke.rs` (8
pins, all pass). Re-run with:
```
cargo test --release -p rustjvm-vm --test wp8_11_ejbca_bootstrap_smoke
```
...
which rust-jvm code path handles it today, (c) what specifically will
