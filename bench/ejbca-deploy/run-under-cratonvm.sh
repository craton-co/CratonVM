#!/usr/bin/env bash
# bench/ejbca-deploy/run-under-cratonvm.sh
# WP8.11 — Boot WildFly+EJBCA under target/release/cratonvm and capture
# the first failure (or the install-wizard 200 OK).
#
# Delegates to bench/wildfly-boot/run-under-cratonvm.sh and passes
...
if [[ ! -x "$WILDFLY_FIXTURE/run-under-cratonvm.sh" ]]; then
    fail "WP8.10 prereq missing: $WILDFLY_FIXTURE/run-under-cratonvm.sh" 11
fi
...
log "delegating boot to $WILDFLY_FIXTURE/run-under-cratonvm.sh"
...
( bash "$WILDFLY_FIXTURE/run-under-cratonvm.sh" > "$RUN_STDOUT" 2> "$RUN_STDERR" ) &
BOOT_PID=$!
log "cratonvm boot PID=$BOOT_PID"
