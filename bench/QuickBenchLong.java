/**
 * QuickBenchLong — extended QuickBench tuned so the whole suite runs
 * > 5 s on HotSpot (and proportionally longer on slower VMs).
 *
 * Same four kernels as QuickBench (Arithmetic, Fibonacci, Sieve, Matrix)
 * plus a fifth: the classic GC-bound "binary-trees" benchmark at depth 18.
 * Arithmetic is scaled 300M -> 1.5B so the total clears the 5 s floor
 * even on a fully warmed-up VM.
 *
 * Output is one "<n>. <name> : <ms> ms  [<checksum>]" line per kernel,
 * then a TOTAL line — identical shape to QuickBench so results diff cleanly.
 */
public class QuickBenchLong {
    ...
    public static void main(String[] args) {
        System.out.println("=== QuickBenchLong ===");
        long total = 0;

        {
            long t0 = System.currentTimeMillis();
            long r = benchArithmetic(1_500_000_000L);
            long elapsed = System.currentTimeMillis() - t0;
            total += elapsed;
            System.out.println("1. Arithmetic (1.5B)    : " + elapsed + " ms  [" + r + "]");
        }
        {
            long t0 = System.currentTimeMillis();
            int r = fib(42);
