...
Ranked in the order they will fire when `rustjvm.exe --jar
...
which rust-jvm code path handles it today, (c) what specifically
fails.
...
### WP8.10.8 — `RUSTJVM_JBOSS_MP_ROOT` env-var fallback for `-mp` [XS, applied]

**Status**: ✅ applied.

Augmented `find_mp_argument` in
`native-builtins/src/jboss_module_loader.rs:188` to honor the
`RUSTJVM_JBOSS_MP_ROOT` env var as a fallback before scanning argv.
...
3. **Then re-run `bench/wildfly-boot/run-under-rustjvm.sh`** with a
