# bench/wave2-4/run-under-cratonvm.ps1
# WP2.4-D - Windows PowerShell 5.1 runner for the WP2.4 instrument /
# jacoco / mockito probes. Mirrors run-under-cratonvm.sh.
#
...
function Run-Probe {
    ...
    if ($skipped -eq 'yes') {
        Write-Output "run-under-cratonvm: probe=$Probe SKIPPED (jar absent at stage time)"
    ...
    Write-Output "run-under-cratonvm: probe=$Probe javaagent=$javaagentArg"
    Write-Output "run-under-cratonvm: probe=$Probe cp=$cp"
    Write-Output "run-under-cratonvm: probe=$Probe main=$mainClass"
...
if ($ProbeFilter -ne 'instrument' -and $ProbeFilter -ne 'jacoco' -and $ProbeFilter -ne 'mockito' -and $ProbeFilter -ne 'all') {
    Write-Error "run-under-cratonvm: ERROR PROBE must be instrument | jacoco | mockito | all"
    exit 4
}
