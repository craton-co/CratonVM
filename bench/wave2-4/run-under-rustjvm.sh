#!/usr/bin/env bash
# bench/wave2-4/run-under-rustjvm.sh
# WP2.4-D — execute the staged instrument / jacoco / mockito probes
# under rust-jvm and capture stdout/stderr/rc deterministically.
#
...
#   - the -javaagent: spec to pass to rustjvm
# ---------------------------------------------------------------------------
run_probe() {
    local probe="$1"
    ...
    if [[ ! -d "$classes_dir" ]] || [[ ! -f "$main_file" ]]; then
        echo "run-under-rustjvm: probe=$probe NOT-STAGED (run stage-$probe-probe.sh first)" >&2
        echo "127" > "$rc_file"
        cat > "$meta_file" <<JSON
{
  "probe":        "$probe",
...
JSON
    ...
    # Skipped probes: no run, no rustjvm invocation. summary.json records
    # rc=-2 so drift tools can tell skip from failure.
    if [[ "$skipped" == "yes" ]]; then
        echo "run-under-rustjvm: probe=$probe SKIPPED (jar absent at stage time)"
        echo "-2" > "$rc_file"
        cat > "$meta_file" <<JSON
{
  "probe":        "$probe",
...
JSON
    ...
    echo "run-under-rustjvm: probe=$probe binary=$RUSTJVM"
    echo "run-under-rustjvm: probe=$probe javaagent=$javaagent_arg"
    echo "run-under-rustjvm: probe=$probe cp=$cp"
    echo "run-under-rustjvm: probe=$probe main=$main_class"
    echo "run-under-rustjvm: probe=$probe timeout=${TIMEOUT_SEC}s"

    # Argv layout: rust-jvm CLI takes -javaagent: BEFORE -c so it's
    # captured by the hotspot-flag pass; -c <cp> <main> follows.
    set +e
    if command -v timeout >/dev/null 2>&1; then
        timeout --kill-after=5 "${TIMEOUT_SEC}" \
            "$RUSTJVM" "$javaagent_arg" -c "$cp" "$main_class" \
...
JSON
    echo "run-under-rustjvm: probe=$probe rc=$rc"
}

PROBE_FILTER="${PROBE:-all}"
case "$PROBE_FILTER" in
    instrument|jacoco|mockito|all) ;;
    *) echo "run-under-rustjvm: ERROR PROBE must be instrument | jacoco | mockito | all" >&2; exit 4 ;;
esac
...
echo "run-under-rustjvm: summary -> $HERE/summary.json"
exit 0
