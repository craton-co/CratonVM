...
run-under-rustjvm.{sh,ps1}        ->  execute each probe under
                                       target/release/rustjvm.exe with
...
| `run-under-rustjvm.sh/.ps1` | Run the three probes; capture per-probe `last-run-*.{stdout,stderr,rc,meta.json}` + `last-run-jacoco.jacoco.exec` + `summary.json`. |
...
bash bench/wave2-4/run-under-rustjvm.sh
...
powershell -ExecutionPolicy Bypass -File bench\wave2-4\run-under-rustjvm.ps1
