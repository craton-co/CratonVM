# bench/ejbca/run-under-cratonvm.ps1
# WP8.7 — run the staged ejbca fixture under cratonvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-ejbca: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-ejbca: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-ejbca: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-ejbca: cratonvm.exe not found; cargo build --release -p cratonvm-cli"; exit 3 }
...
Write-Output "run-ejbca: binary=$Rustjvm"
Write-Output "run-ejbca: cp=$Cp"
Write-Output "run-ejbca: main=$MainClass"
Write-Output "run-ejbca: timeout=${TimeoutSec}s"
...
Write-Output "run-ejbca: rc=$Rc"
Write-Output "run-ejbca: stdout -> $StdoutLog"
Write-Output "run-ejbca: stderr -> $StderrLog"
exit 0
