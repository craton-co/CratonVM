# bench/ejbca/stage.ps1
# WP8.7 — stage the ejbca forcing-function fixture (Windows PowerShell 5.1).
#
# Network requirement: NONE today (placeholder fixture is in-tree).
# Real-distribution slot (not yet wired): if $env:EJBCA_HOME points at an
...
if (-not $Javac) { Write-Error "stage-ejbca: javac not found"; exit 10 }
Add-Content -Path $CompileLog -Value "stage-ejbca: using javac at $Javac" -Encoding utf8

$SrcFile = Join-Path $SrcDir 'Main.java'
if (-not (Test-Path $SrcFile)) {
    Write-Error "stage-ejbca: $SrcFile missing"
    exit 11
}

Add-Content -Path $CompileLog -Value "stage-ejbca: compiling fixture/Main.java" -Encoding utf8
$rc = Invoke-Javac -Javac $Javac -ArgList @('--release','21','-d',$ClassesDir,$SrcFile) -LogPath $CompileLog
if ($rc -ne 0) {
    Write-Error "stage-ejbca: javac failed (rc=$rc); see $CompileLog"
    exit 12
}

Set-Content -Path $MainFile -Value 'Main' -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-ejbca: main class = Main" -Encoding utf8
Add-Content -Path $CompileLog -Value "stage-ejbca: staged classes at $ClassesDir" -Encoding utf8
Write-Output "stage-ejbca: OK"
exit 0
