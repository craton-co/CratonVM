#!/usr/bin/env bash
# bench/wave2-3/run-under-rustjvm.sh
# WP2.3-D — execute the staged CGLIB and ByteBuddy probes under
# rust-jvm and capture stdout/stderr/rc deterministically. Mirrors
...
# Convert /c/foo/bar to C:\foo\bar so the Windows-built rustjvm.exe can read
...
JSON
    ...
    if [[ -z "$main_class" ]]; then
        echo "run-under-rustjvm: probe=$probe ERROR main-class.txt empty" >&2
        echo "126" > "$rc_file"
        return 0
    fi
    ...
    echo "run-under-rustjvm: probe=$probe main=$main_class skipped-real-jar=$skipped"
...
case "$PROBE_FILTER" in
    ...
    *) echo "run-under-rustjvm: ERROR PROBE must be cglib | bytebuddy | both" >&2; exit 4 ;;
esac
