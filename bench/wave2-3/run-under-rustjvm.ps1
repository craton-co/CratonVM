# bench/wave2-3/run-under-rustjvm.ps1
# WP2.3-D - Windows PowerShell 5.1 runner for the staged CGLIB +
# ByteBuddy probes. Mirrors run-under-rustjvm.sh.
#
...
function Run-Probe {
    ...
    if (-not $mainClass) {
        Write-Output "run-under-rustjvm: probe=$Probe ERROR main-class.txt empty"
    ...
    Write-Output "run-under-rustjvm: probe=$Probe main=$mainClass skipped-real-jar=$skipped"
...
if ($ProbeFilter -ne 'cglib' -and $ProbeFilter -ne 'bytebuddy' -and $ProbeFilter -ne 'both') {
    Write-Error "run-under-rustjvm: ERROR PROBE must be cglib | bytebuddy | both"
    exit 4
}
