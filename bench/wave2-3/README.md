...
under rust-jvm so future waves can detect drift.
...
stage-bytebuddy-probe.{sh,ps1}        compile probes into staged-*/classes
          |
          v
run-under-rustjvm.{sh,ps1}        ->  execute each probe under
                                      target/release/rustjvm[.exe], capture
...
| `run-under-rustjvm.sh/.ps1` | Run both probes; capture per-probe `last-run-<probe>.{stdout,stderr,rc,meta.json}` and `summary.json`. |
...
powershell -ExecutionPolicy Bypass -File bench\wave2-3\run-under-rustjvm.ps1
...
- `RUSTJVM_BIN` — override `target/release/rustjvm[.exe]`.
...
- `run-under-rustjvm`: each probe rc=1.
