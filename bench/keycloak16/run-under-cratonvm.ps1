# bench/keycloak16/run-under-cratonvm.ps1
# WP8.7 — run the staged keycloak16 fixture under cratonvm (Windows PS 5.1).
...
if (-not (Test-Path $Classes))  { Write-Error "run-keycloak16: staged classes missing; run stage.ps1 first"; exit 2 }
if (-not (Test-Path $MainFile)) { Write-Error "run-keycloak16: main-class.txt missing"; exit 2 }
$MainClass = (Get-Content $MainFile -Raw).Trim()
if (-not $MainClass) { Write-Error "run-keycloak16: main-class.txt empty"; exit 2 }
...
if (-not $Rustjvm) { Write-Error "run-keycloak16: cratonvm.exe not found; cargo build --release -p cratonvm-cli"; exit 3 }
...
Write-Output "run-keycloak16: binary=$Rustjvm"
Write-Output "run-keycloak16: cp=$Cp"
Write-Output "run-keycloak16: main=$MainClass"
Write-Output "run-keycloak16: timeout=${TimeoutSec}s"
...
Write-Output "run-keycloak16: rc=$Rc"
Write-Output "run-keycloak16: stdout -> $StdoutLog"
Write-Output "run-keycloak16: stderr -> $StderrLog"
exit 0
