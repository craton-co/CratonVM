...
fn every_backend_door_goes_through_the_admission_gate() {
    ...
    // H20-1 N3, and it is an `assert_eq!(.., 0)` as of 2026-09-22.
    //
    // It was an `eprintln!` for a day, because when N3 landed the number was
    // NOT zero — measured on this exact probe, `method-entry=7`, `osr=6`,
    // `eager-first-call=0` — and printing it was the honest thing to do rather
    // than assert a false invariant or break every build. The nomination said
    // what would change that: *"N3 asked for the assertion once the count is
    // observed at zero."*
    //
    // `H12-1` O1/O2 put it at zero. The OSR and eager first-call ladders
    // obtain every shadowing helper address from `admit_direct_native_entry`
    // and both declare their policy; `try_compile_inner` always asked and now
    // says so as well, which is what took `method-entry` off 7.
    //
    // A non-zero here is a door that builds `direct_calls` and never declared
    // — a fourth door, or a regression in one of these three. That is the
    // event this gate exists for, and it is now loud.
    for (door, label) in [
        (DOOR_METHOD_ENTRY, "method-entry"),
        (DOOR_EAGER_FIRST_CALL, "eager-first-call"),
        (DOOR_OSR, "osr"),
    ] {
        assert_eq!(
            nth(&fields, "undeclared-direct-bind-rows", door),
            0,
            "[jit_compile_gate_doors] the {label} door bound direct-call rows \
             without declaring a `DirectCallPolicy`. Every door that builds \
             `direct_calls` must call `declare_direct_call_policy`, and every \
             bind onto a helper that shadows a registered native must come \
             from `admit_direct_native_entry`. Was method-entry=7/osr=6 before \
             H12-1 O1/O2 (2026-09-22).\nstderr:\n{stderr}"
        );
    }
    assert_eq!(
...
{stderr}"
    ...
    // H20-1 N3, this door's own arm — and the one that matters for it, since
    // arm 1 cannot wake this door at all (`bg-compile` is default-ON). Arm 1's
    // `eager-first-call=0` was a zero because the door never ran; this one is
    // a zero because it declared. See the longer note in arm 1.
    assert_eq!(
        nth(&fields, "undeclared-direct-bind-rows", DOOR_EAGER_FIRST_CALL),
        0,
        "[jit_compile_gate_doors] the eager first-call door bound direct-call \
         rows without declaring a `DirectCallPolicy`, with the door awake \
         (`bg-compile=0`).\nstderr:\n{stderr}"
    );
}
