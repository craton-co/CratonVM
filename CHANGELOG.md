...
### 2026-09-22 two staged deopt subsystems retired, and every resume sink now reads the field that was landed for it

Round 10 wave 9 (`deoptretire`) closed the last three `deoptverify` pages and
everything they carried forward. Two of the three residuals were deletions that
earlier waves had said were right and could not reach, because the files
belonged to other lanes; this one owned all of them, and — unlike waves 6–8 —
**built the tree and ran the suite**.

**`CompiledMethod::find_deopt_point` is gone.** It was an exact-offset binary
search over `deopt_points` with no production caller: both backends bake the
ADDRESS of the element into the stub that needs it, so nothing ever resolved a
point by offset. Keeping it was not free — `native_offset` is not a unique key
(`build_and_record_deopt_point` emits no machine code, and the three recorder
families that call it de-duplicate in three maps that never consult each other),
so a same-offset pair would have resolved arbitrarily, and two comments in the
tree had already been caught recommending it on the strength of a uniqueness it
never had.

Its only remaining users were three assertions in `ir_lower.rs`'s own tests.
Their replacement is the interesting part: it asserts that bci 2's native offset
names **exactly one** point, which is a stronger claim than the search it
replaced could make. A deleted oracle left a better test behind.

**`ir::InlineScopeTable` is gone**, with `ir_lower::Lowerer::caller_chain_for`.
This was round 10's defect class — staged machinery with no producer — in its
worst form, and worse than the interned frame-state representation retired the
day before. The interner had no production reference at all, so a `rg` told the
truth about it. This table's READ side was wired to the production lowering
path, so a `rg` found production hits and reported inlined-scope support the
optimizing tier has never had, on any compile: `push_scope`, `bind_snapshot` and
`bind_snapshot_range` had no caller outside `#[cfg(test)]` anywhere in the
workspace. **An orphan whose reference list lies is worse than one with no
references.**

It was not kept as a head start, because it was never the hard half.
`IrBuilder::build` walks one method's bytecode, and when it splices it
deliberately carries the caller's `invoke` bci with re-execute semantics instead
of a scope chain — a chain needs the innermost frame's identity, which
`resolve_frame_state` leaves for the VM to fill from the running
`CompiledMethod`, i.e. it would name the caller. That reasoning now sits where
the table was. `resolve_frame_state` sets `caller: None` outright and says why.

**Every routing and resume decision now reads `ResumeSemantics`**, which was
landed a month ago and read by almost nobody. Both trampoline entries route an
exceptional frame on `semantics.rethrow_exception` rather than on
`reason == PendingException`; so does the OSR exception-exit provenance check;
both rebuild sinks ask `rframe.semantics.reexecute` before parking anything at
the frame's bci instead of trusting a gate two crates away; and a new
`scope_resume_pc` decides where each scope of an inlined chain parks from that
scope's own semantics rather than from its position in the chain.

The swap is behaviour-preserving on any installed artifact — `DeoptVerifier`'s
exception-state-agreement lane refuses a point whose `reason` and
`semantics.rethrow_exception` disagree, on both backends' install paths, in
release builds — and it is not cosmetic. A flagged point carrying some other
reason used to fall through the ordinary stash gate and be replaced by the
identity-less re-run sentinel, losing the precise handler locals. That is the
same defect the tree already recorded for `ir_deopt_entry`'s old shape; it is
now closed for the other direction too, and a test asserts both halves.

`ReconstructedFrame` also gained per-scope `semantics`: the reconstructors stamp
the trapping frame with the point's and **every linked caller with
`for_caller_scope()`**, which is what the retired interned form did per scope and
what the owned `FrameState` cannot say. `reason` survives as what it always
was — the charging key for the deopt log, `recommend_action` and the
de-speculation lookups — and nothing decides re-execute / resume / rethrow from
it any more.

What was deliberately **not** done, each with the argument against it recorded
in `docs/feature-designs/jit-r10-deoptretire-proposals.md`: honouring a `RESUME`
point in the ordinary sink (the stash's nesting argument — why a frame under the
top can only be a stale leftover — depends on every stashed frame being a
re-execute point, and that has to be answered first); the callee-local width
analysis for splice-published frames (no publisher reads it, so it would be this
lane adding an orphan while retiring two); and teaching the orphan-instrument
gate about `&self` readers, which now has a second and sharper data point but
belongs to whoever owns the gate.

### 2026-09-21 writing down why `newarray` could not be lowered found `areturn`, which needed no annotation at all

Closing the `ALLOW_ALLOCATION` hole (above) meant writing down why
lowering it is a host problem rather than an emitter one: the
allocated array is the kernel's *return value*. Which raises the
question nobody had asked — if the array is returned, `areturn` has to
lower too. Does it?

It does not, and it never did. `lowering/emit.rs` has arms for
`ireturn`/`lreturn`/`freturn`/`dreturn` (0xAC..0xAF) and for `return`
(0xB1). 0xB0 falls through the gap to the catch-all. Meanwhile
`ParamKind::from_field` maps `[I` to `I32Array` for a RETURN type
exactly as it does for a parameter, and 0xB0 sits inside `classify`'s
permitted `0xAC..=0xB1` band. So:

```java
static int[] doubleInPlace(int[] a) {   // plain STRICT, no annotation
    for (int i = 0; i < a.length; i++) { a[i] = a[i] * 2; }
    return a;                           // allocates nothing
}
```

analyzed `Eligible(KernelSignature { param_kinds: [I32Array],
return_kind: I32Array, .. })` and then died in the emitter with
`opcode 0xb0 not implemented in lowering`. **This is the same defect
as the allocation one with the annotation taken away.** `newarray`
needed `@GpuKernel(admit = ALLOW_ALLOCATION)` to be reachable at all;
this needed nothing. Any method that returned a primitive array and
otherwise fit every rule paid an analyze→lower round-trip and a
blacklisting, for anyone, at any time. The annotated hole had a test
pinning it and a doc page advertising it. The wider one had neither.

`analyze` now rejects any primitive-array return kind with
`Reason::UnsupportedReturnType`, from the descriptor — next to the
parameter-kind check it belongs with, before the body is scanned.
...
back) but it still needs an `areturn` lowering and a
`SerializedResult` that can name an existing argument, and neither
exists. The out-parameter form every GPU fixture already uses — take
the output array as a parameter, return `void` — needs none of it.

Two fixtures follow from the ordering. `RejectAllocation.build(I)[I`
trips both rules, and the descriptor wins, so it can no longer pin the
allocation band: `RejectScratchAllocation.sumSquares(I)I` allocates
the same `iload_0; newarray int` shape and returns a scalar, which
isolates `Reason::Allocation` and nothing else.
`RejectArrayReturn.doubleInPlace([I)[I` is the shape above, and
`array_return_rejected_before_the_body_is_scanned` pins both the new
reject and the order the two rules fire in.

The same ordering moves two verdicts in the annotation fixtures:
`AdmitAllocation` and `StrictRejectsAllocation` are both
`int[] mapSquare(int[])`, so both now report `UnsupportedReturnType`
rather than `Allocation`. They still report the *same* thing as each
other, which is what the `ALLOW_ALLOCATION` tests exist to pin.
`docs/gpu/annotations.md` says which reason a user will actually see,
and why the doc's own `ALLOW_ALLOCATION` example is rejected twice
over.

### 2026-09-21 the GPU annotation end-to-end tests were all `#[ignore]`d, and all four had rotted under it

`vm/tests/annotations_end_to_end.rs` was written ahead of its
implementation — the file is still dotted with `// PHASE1-GUESS:` — and
every test in it carried `#[ignore = "depends on Items 3/4 ..."]`. The
Items landed; the `#[ignore]`s did not come off. Run with `--ignored`,
four of the five failed, each for a different reason, and none of the
reasons was the one the attribute gave:

* `load_methods` never force-decoded the lazy `Code` attribute, and
  `ClassFileMethod::code()` only returns an ALREADY-decoded one, so
  every verdict assertion in the file answered `Rejected(NoCode)` —
  for any fixture, under any hint. Fixed on 2026-09-21 with the
  `ALLOW_ALLOCATION` change above;
* two tests looked up a method named `doubled` and two more one named
  `add`. No fixture has ever declared either; the names came from the
  doc's example and from thin air respectively, against fixtures that
  declare `mapSquare` and `vectorAdd`. The first pair was corrected
  with the `ALLOW_ALLOCATION` change; this one corrects the second;
* one asserted `Eligible` for a shape the emitter cannot lower, which
  was the bug it existed to catch.

The two `@GpuExclude` tests do not become green on the rename alone,
and the reason is worth stating because it is not a defect:
`OffloadCache::lookup_or_compile` returns `Skip` on `ctx.is_none()`
**before** it decodes any annotation, so on a no-driver box the
`@GpuExclude` short-circuit is unreachable and `Blacklisted` is not an
outcome the cache can produce. The unconditional `expected
Blacklisted` could only ever fail here. The in-module tests in
`vm/src/runtime/offload.rs` already gate on `has_device()` for exactly
this reason; these now do too, split into the half that needs a device
and the half that does not:

* device-free, and unconditional: the fixture really carries
  `@GpuExclude`, with the expected `reason`, and
  `read_method_annotations` — the exact input the short-circuit
  consumes — really sees it. For `ExcludedAndKernel`, that `@GpuKernel`
  is present too, since precedence is only a question when both are.
  This is what catches a fixture or annotation-parser regression, and
  what the old test only looked like it was checking;
* device-only: the outcome is `Blacklisted`. Without a device it is
  `Skip`, and what must hold on every box is that an excluded method is
  never a `Hit`.

Four of the five tests now run by default under the `gpu-offload`
feature. `warmup_class_compiles_two` keeps its `#[ignore]`, and is the
one place the original reason still holds: it is a structural no-op
without a device and an explicit "API not yet available" panic with
one, so `warmup_class` / `entry_count_for_class` really do have to land
first.

### 2026-09-21 `@GpuKernel(admit = ALLOW_ALLOCATION)` promised a GPU and delivered a blacklisting

`jit-cuda/src/analyzer.rs` admitted `newarray` (0xBC) whenever
`AdmissionFlags::allocation` was set and the size operand came from an
`iload`. `jit-cuda/src/lowering/emit.rs` has never had a dispatch arm
for 0xBC — nor for the `areturn` (0xB0) that necessarily follows it,
since the allocated array is the method's return value. So every
annotated method was analyzed `Eligible`, handed to the emitter,
refused with `unsupported IR node: opcode 0xbc not implemented in
lowering`, and blacklisted. `docs/gpu/annotations.md` documented the
hint as a working feature the whole time.

This is the third instance of the same shape — `ldc`/`ldc_w`/`ldc2_w`
(AUDIT C31, 2026-05-24) and `invokestatic` under
`ALLOW_INTRINSIC_CALLS` (2026-07-11) were the first two — and the
analyzer's own contract names it: "the analyzer and the lowering
emitter MUST agree on what they accept: a method admitted here only to
be rejected by `walk` wastes an analyze→lower round-trip and pollutes
the per-method blacklist".

**The first two were closed by writing the missing emitter arm. This
one cannot be**, and that is the part worth recording. The analyzer's
rule was that the allocated array is the kernel's *output buffer*
sized from a parameter, which makes lowering it a host problem, not an
emitter one: allocate a device buffer from a runtime argument, copy it
back after the launch, and materialise a new Java array for the
caller. Half of that is already expressible —
`KernelSignature::return_kind` carries `I32Array` &c. and
`lowering::build_param_list` already emits `ret_ptr` + `ret_len` for an
array return kind — and half is not: `dispatch_method_from_native`
pushes a return slot only for the four scalar `ParamKind`s, its array
arms are explicitly "not yet wired", and
`SerializedResult::PrimitiveArray*` is never populated on this path.

So the reject is unconditional now. `classify`'s allocation band
(`new` / `newarray` / `anewarray` / `multianewarray`) rejects with
`Reason::Allocation` under every hint, and `classify` lost the
`prev_op` parameter that existed for this one arm (along with
`is_iload_family`, its only consumer). A method that allocates goes to
the CPU, where it is correct — the same call already made for
`Reason::UnsupportedGridShape` and `Reason::UnsupportedLaunchGeometry`.

The annotation surface is unchanged: `ALLOW_ALLOCATION` still parses
and still sets `AdmissionFlags::allocation`, so already-annotated
sources keep compiling and a future round that wires the array-return
path has a flag waiting for it. The bit is read by nothing today, and
two tests pin exactly that —
`allow_allocation_hint_is_still_parsed_into_flags` (set) and
`allow_allocation_hint_does_not_admit_newarray` (inert), the latter
replacing `admit_allocation_loosens_primitive_newarray`, which pinned
the bug. `vm/tests/annotations_end_to_end.rs` likewise asserted
`Eligible` for the `AdmitAllocation` fixture and now asserts the
reject; while there, it was calling `method_by_name(.., "doubled")` on
both allocation fixtures, whose method is `mapSquare` — a name taken
from the doc's example, never from the fixture, which `#[ignore]` had
hidden since the file was written.

`docs/gpu/annotations.md` now states the hint is accepted but not yet
lowered, with the layer-by-layer table of what making it real would
take and the out-parameter workaround; `docs/gpu/lowering-architecture.md`
records under "Symptom 1" that the third layer of Symptom 1b can be the
layer that decides whether an opcode is lowerable at all, which one IR
would not have fixed.

### 2026-09-21 W7-2 / W7-15 / W7-19 retired: one real fix, one JIT-warm completeness edit, two already closed

Three more `docs/known-issues/jdk-only/` records retired to `docs/internal/jdk-only/`:

* **W7-15** (`Cipher.getInstance("ChaCha20")` returned AES-256-ECB) was already
  fully fixed and verified by execution per its own 2026-08-12 disposition;
  re-confirmed against current `dev` source with no code change needed.
* **W7-19** (`asCollector`'s wrong container, `bindTo`'s missing refusal) had
  its combinator and binding fixes verified against a real binary on
  2026-09-01 (`RJdkHandles`, 331/331 checks, byte-identical to HotSpot in both
  modes). This pass applies the one remaining out-of-file line the record
  named: `"isVarargsCollector"` added to `vm/src/vm/vm_exec.rs`'s JIT-warm
  dispatch-preference mirror, closing a cold/warm split where a JIT-warm
  caller could still read the real base class's hardcoded `false`. §5.1
  (`bindTo` raising `ClassCastException` for a wrong reference type) stays
  open — it needs a new never-false-refuse assignability predicate on
  `NativeContext` that does not exist yet, not a missed patch.
* **W7-2** (`IntStream.summaryStatistics()` compiled out of the default
  binary) had every registration and wiring fix already measured across three
  earlier passes. This pass fixes the one real residual those passes left:
  `DoubleStream.min()`/`max()` (`native-collections/src/lib.rs`) were still
  folding with Rust's `f64::min`/`f64::max`, which return the non-NaN operand
  where Java's `Math.min`/`Math.max` propagate it. Fixed via two new shared
  functions, `cratonvm_types::jfp::double_min`/`double_max` — the same
  NaN-propagating, signed-zero-pinning rule `native-builtins::lang_math`
  already used for `Math.min`/`Math.max`, now available to
  `native-collections` too without a cross-crate dependency the two crates
  don't have. Measured: `DoubleStream.of(1.0, NaN, 3.0).min()` now answers
  `OptionalDouble[NaN]`, byte-identical to HotSpot, in both Compatible and
  `--jdk-only`. `spliterator()`'s refusal and two `--synthetic-jdk`-only gaps
  (locale rendering, varargs `of(...)` factories) stay open, exactly as the
  record already scoped them.

Verified with a fresh release build off `dev`: `cargo test -p cratonvm-types
jfp::` (7/7 including the new `double_min`/`double_max` test), and
`regression-suite/run.sh ONLY="RJdkHandles RJdkViews RCrypto RJdkSecurity
RJdkFailure"` in both Compatible and `--jdk-only` (5/5 each).

### 2026-09-21 primitive-class-had-a-loader retired: both open findings were already closed on dev

`docs/known-issues/jdk-only/primitive-class-had-a-loader-and-two-deeper-gaps-20260827.md`
left two findings OPEN — `java.lang.Module.getPackages()` short for
`java.base` (§3), and `MethodHandle.invokeExact` not enforcing its exact
signature (§4). Both were already fixed on `dev` the day after the page was
written (`c5db44794`, `a751958a4`) and fully measured in a since-retired
follow-up (`L5-residuals-module-packages-and-invokeexact-20260828.md`); this
page's own paper trail had simply never been updated. Re-measured on a fresh
release build with a new probe, `tools/probes/PrimClassResidualSweep.java`:
5/5 rows byte-identical to HotSpot 25.0.4+7 in both Compatible and
`--jdk-only`, including `java.base.getPackages().size()` matching HotSpot's
197 exactly and `invokeExact` on a mismatched call site now throwing
`WrongMethodTypeException`. No code change; the page is retired to
`docs/internal/jdk-only/`.

### 2026-09-20 the `cratonvm` launcher now defaults to jdk-only; `--compatible` is the opt-out

**BREAKING for the launcher; unchanged for every embedding API.** A bare
`cratonvm Foo` now runs under `CompatibilityMode::JdkOnly`: real JDK class
bytes are authoritative, no class is fabricated without real bytes, and no
`NativeKind::SyntheticStub` is registered or invoked. Enforcement is still at
wave 1, so a program that depends on a substitution can fail here and pass
under the opt-out. That difference is the measurement, not a bug in either
mode.

- **New flag `--compatible`** selects the pre-2026-09-20 behaviour (real class
  library *plus* the compatibility substitutions). It is what every
  strict-mode violation message now points at as the fallback, replacing
  `--real-jdk` in `JdkOnlyViolation::render`'s pinned penultimate line.
- **`--real-jdk` is a deprecated alias for `--compatible`**, not a
  library-axis-only flag. It only ever selected the real library, which was
  already the default, so every caller that passed it was in practice asking
  for the compatible policy — which is exactly what it now means. This is what
  lets `difftest`'s `RealCompatible*` arms, `scripts/jdk-only-census.sh`'s
  `run_policy real --real-jdk`, the CI three-arm control and the three
  duplicated script trees keep the meaning their committed baselines were
  captured with, with no edit. Re-pointing it at the library axis would have
  silently flipped all of them to strict. The flag prints a one-line
  deprecation note.
- **`--synthetic-jdk` now implies `--compatible`.** The synthetic library is by
  construction the set of substitutions strict mode forbids, so inheriting the
  new default would have turned a bare `--synthetic-jdk` into a
  `validate_compatibility` startup error caused by a change to a flag it has
  nothing to do with. The pair is still rejected when the argv *states* both.
- **New conflicts:** `--jdk-only` with `--compatible` or `--real-jdk` (opposite
  policies), and `--synthetic-jdk` with `--compatible` (opposite libraries).
- **The strict banner is now printed only on an explicit `--jdk-only`.** It is
  the default; four lines of preamble on every run is a banner nobody reads.
  `--verbose:class` / `--verbose:gc` still report the resolved mode.

Every **embedding** entry point deliberately stays `Compatible`:
`VmConfig::default`, `libcratonvm::DEFAULT_COMPATIBILITY_MODE` and
`cratonvm-embed`. A host that linked the C API passed no policy argument, has
no command line to opt out on, and must not start getting refusals from a
library upgrade — those constants are ABI. The split is declared as
`LAUNCHER_DEFAULT_COMPATIBILITY_MODE` (now `JdkOnly`) versus
`EMBEDDED_DEFAULT_COMPATIBILITY_MODE` and the new
`EMBEDDED_HOST_JDK_DEFAULT_COMPATIBILITY_MODE` (both `Compatible`), and
**`VmConfig::with_host_jdk_default` is no longer an alias for
`VmConfig::for_launcher`** — re-aliasing it is an ABI break.
`CompatibilityMode::default()` also stays `Compatible`, so the ~5,000 in-tree
tests are untouched.

**Five harnesses now state their policy instead of inheriting it**, because
an empty argv stopped reproducing the runs their committed baselines hold:

- `difftest`'s historical modes (`jit-on`, `nojit`, `no-intrinsics`,
  `moving-gc`, `low-jit-threshold`, `interp-decoded`) pass `--compatible`.
  Their rule used to be "pass no launcher flag, so the argv matches the
  committed gate baseline exactly"; the flip inverted it, since an empty argv
  now selects the *other* policy while `Mode::jdk_profile()` kept labelling
  those rows `compatible`. The frozen thing is the policy, so it is stated.
  The `RealCompatible*` pair keeps `--real-jdk`.
- `vm/tests/differential.rs` pins `Compatible` on top of `for_launcher()`.
  Moving that harness to strict is worth doing and is a separate, measured
  change: it needs each fixture re-read, not a constant re-pointed.
- `regression-suite/run.sh` passes `--compatible` explicitly whenever
  `CRATONVM_ARGS` names no policy of its own.
- `apps/hib-suite-runner/run-hib.sh` and
  `apps/netty-suite-runner/run-netty-suite.sh` pass `--compatible` on their
  real-JDK arm (their synthetic arm already implies it). Moving these
  gauntlets to strict is worth doing and needs the whole class list re-run,
  not a flag dropped.

In each case the frozen thing is the *policy*, not the argv. Every number in
`regression-suite/`, in `jdk-only-coverage.txt`, in `difftest/ledger.json` and
in the HANDOFF documents they cite was measured with a bare invocation meaning
compatible; freezing the argv is exactly what would have unfrozen the
measurement. The CORE regression list is the sharpest case — its own header
says it asserts compatibility behaviour and is deliberately not the strict
corpus.

This supersedes the end state recorded in
`docs/feature-designs/jdk-only-mode.md` ("`--jdk-only` becomes `--real-jdk`,
and today's `--real-jdk` becomes `--synthetic-jdk`"), which was never done:
reusing `--synthetic-jdk` for a real-JDK mode would have collided with the flag
that selects the synthetic class library.

### 2026-09-20 F1-1 / F5-1 / F6-1 retired: measured, on a binary this pass actually built and ran

Three `docs/known-issues/jdk-only/` records — the boxing-cache identity family
(F1-1), `CharBuffer`'s three-way `hasArray`/`array`/`arrayOffset` split (F5-1),
and the SSL null-session width arm (F6-1) — were landed by lanes that could not
build or run CratonVM, so every one of their "after" claims was PREDICTED, not
MEASURED. This pass built a release binary from `dev` and ran the regression
suite against it under `--jdk-only`: `RJdkIntrinsics2`, `RJdkIntrinsics3`,
`RSslNullSession` and `RSslLiveSession` all **PASS** cross-VM against
`openjdk 25.0.3+9-LTS`, closing the open verification gap all three records and
their reconciliation notes (`F14-1`, `G40-1`) had been carrying since 2026-08.

The one nomination still genuinely open across the three records —
`RJdkIntrinsics3`'s `boxid` family had no identity-false rows for `Float`/
`Double`, the only boxed pair that caches nothing (F1-1 NOMINATION 2) — is
landed here: two rows next to the other cache rows in
`regression-suite/src/RJdkIntrinsics3.java`, `boxid`'s denominator moving
69 → 71.

All three records are retired to `docs/internal/jdk-only/` with a banner
summarizing which nomination landed where and citing the measured run; F1-1's
NOMINATIONS 1 and 3, and F5-1's NOMINATIONS 1, 3 and 4, and F6-1's NOMINATIONS
1, 2 and 3 had already landed via `F10-1`/`F14-1`/`F25-1`/`G51-1` and needed no
further code change, only confirmation. F6-1's NOMINATION 4 (linking the
five-record SSL null-session chain in `INDEX.md`) is left open — those four
sibling records were out of this pass's scope.
...
[`docs/internal/fixed-suite-bugs/g1-pins-a-region-for-an-interior-array-cursor-20260912.md`](docs/internal/fixed-suite-bugs/g1-pins-a-region-for-an-interior-array-cursor-20260912.md).
...
- The wave-2 backlog is ranked by danger in `docs/jdk-only/README.md`;
