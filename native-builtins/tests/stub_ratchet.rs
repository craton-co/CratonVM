...
///
/// # 2395 → 3293, 2026-09-11 (this branch's own 93-commit merge of `origin/dev`)
///
/// `+898` on every arm uniformly (management 2395→3293, the other two
/// 2384→3282) — the paired-ratchet confirmation: a delta that lands
/// identically on all three arms, with the TOTAL registration count barely
/// moving (13978→13991, +13, not +898), is existing `Bridge` rows being
/// relabelled `SyntheticStub`, not new fakes being written. It is lane T's
/// own 906-triple table landing for the first time on `origin/dev` through
/// this merge (`RETIRED_SHADOW_LT_TRIPLES` was on this branch alone until
/// now) — MINUS the 123 rows this same merge pulled back, see
/// `docs/internal/jdk-only/lane-t-...-20260910.md` §8: a sibling lane's own
/// wave-4 test proved `printStackTrace(Ljava/io/PrintWriter;)V` and
/// `setStackTrace([Ljava/lang/StackTraceElement;)V` trade one wrong answer
/// for another when retired, for all 62 `THROWABLE_FAMILY_CLASSES` alike, not
/// only the one class (`java/text/ParseException`) that test names.
///
/// `RETIRED_SHADOW_TABLES` needed two more entries this merge missed —
/// `RETIRED_SHADOW_L1_HM_TRIPLES` and `RETIRED_SHADOW_L1_JT_TRIPLES`, lane 1's
/// waves 3 and 4, landed on branches that never saw this const either — the
/// third occurrence of the exact drift `the_tables_const_lists_every_table_
/// the_predicate_consults` exists to catch, in three days.
/// # Re-frozen 2026-09-11 on the six-lane merge: 2429 / 2440 / 2429
///
/// The two accounts above were taken on branches that had not seen each
/// other: L0+L3 measured 2373/2384/2373 without L1 waves 3-4 or L7, and
/// L1 measured 2384/2395/2384 without L0 or L3. Neither number describes
/// this tree, and their difference is not their sum -- so this arm was
/// re-measured from a forced failure on the merged tree rather than
/// added up. See the stub-ratchet page: a constant is a before-number.
///
/// ```text
/// dev's constants (L1 waves 3-4, L7)        2384 / 2395 / 2384
/// + this branch's L0 wave 2 (21) and L3 (24)  +45 in every arm
/// = this merged tree                        2429 / 2440 / 2429   (measured)
/// ```
///
/// The `+45` is the THIRD independent reading of that quantity: 21 and 24 were
/// each measured on this branch pre-merge from their own forced failure, their
/// sum was measured again on the L0+L5 merge, and it reproduces here on a tree
/// that also carries L1's waves 3-4 and L7. Three readings, and the two halves
/// sum to the third in all three arms.
///
/// **Case (b) in every one of the 45: existing fakes relabelled.** A
/// retirement table re-tags `Bridge` -> `SyntheticStub` in
/// `NativeMethodRegistry::register` before insertion, moving the KIND and
/// never the count of registrations -- and the second column says exactly
...
///
/// # 2755 → 3639, 2026-09-11 (the fifth merge: lane T lands on `origin/dev`
/// for the first time)
///
/// `+884` uniformly on all three arms (the other two: 2728 → 3612), with the
/// KS-7 carve-out and the L2 duplicate-row removal already folded in — this is
/// lane T's 783-triple table landing on `origin/dev`, which until this merge
/// had never seen it: every prior re-freeze on this branch measured lane T's
/// table ON TOP OF an already-current `dev`, and this is the first time `dev`
/// itself gains the table. Paired across all three arms again, same as every
/// re-freeze before it.
/// # 2026-09-11, lane 5's SECOND residual wave: +33 in every arm, total FLAT
///
/// `RETIRED_SHADOW_L5S_TRIPLES` re-tags 33 `Unsafe` triples from `Bridge` to
/// `SyntheticStub` — 13 `sun/misc/Unsafe` address-form accessors and 20
/// `jdk/internal/misc/Unsafe` delegating accessors. No registration is added or
/// removed; 33 change kind.
///
/// Measured with the PAIRED ratchet rather than by arithmetic:
/// `CRATONVM_UNRETIRE_NATIVE_SHADOW` set to the 33 triples turns the table off
/// at runtime, so the before-number comes from the SAME tree and the same test
/// binary:
///
/// ```text
...
///
/// A fourth independent measurement of the exact same delta lane T's own
/// table has produced on every prior merge (see the fifth merge's account
...
///
/// # 2026-09-16, L3's reflect retirements: +9, this arm too
///
/// 4231 -> 4240. The SAME nine `java/lang/reflect/` retirements re-frozen in
/// `BASELINE_SYNTHETIC_STUBS_NO_MANAGEMENT` below, which names them one by one
/// and shows the paired-ratchet measurement; read that block for the evidence
/// rather than repeating it here.
///
/// Recorded separately because the first attempt at this re-freeze moved the
/// no-management constant ALONE and left this one red, and the way that
/// surfaced is worth writing down: a single-crate
/// `-p cratonvm-native-builtins` run picks the no-management arm and went
/// green, while the multi-crate pre-merge run did not — `cratonvm-vm` enables
/// `cratonvm-native-builtins/management`, cargo unifies features across the
/// invocation, and the gate silently measured the OTHER arm. A green
/// single-crate run is therefore NOT evidence this gate is green; only a run
/// that names the arm is. Both arms moved by exactly +9, which is the
/// paired-ratchet discipline these notes keep restating: a relabel moves every
/// arm by its row count and leaves the totals unmoved.
///
/// # 2026-09-16, dev's L1 wave 9: +213 stubs and +56 TOTALS -- a MIXED case
///
/// Re-frozen after merging `origin/dev` at `2c5b65dc5`. Both arms moved by
/// exactly the same amounts, which is itself the evidence that this is one
/// wave and not an arm-specific artifact:
///
/// ```text
///                        stubs            totals
///   no-management   4213 -> 4426     13590 -> 13646
///   management      4240 -> 4453     13958 -> 14014
///                        +213              +56
/// ```
///
/// **This is NOT a pure relabel, and it is recorded as such rather than
/// absorbed.** By this gate's own second-column rule: a total that does not
/// move means existing fakes were relabelled (case (b), welcome); a total up
/// by roughly the stub delta means new fakes were registered (case (a), the
/// regression this gate exists for). Neither holds cleanly here. +213 stubs
/// against +56 totals decomposes as roughly **157 relabels plus 56 genuinely
/// new registrations**.
///
/// Attribution, from the three `origin/dev` commits in the merged range that
/// touch this surface:
///
/// * `887d36ffc` -- "retire HashMap+LinkedHashMap (168) and Date/ZoneInfo
///   (28)", i.e. 196 announced retirements. Retirements are case (b) and move
///   stubs without moving totals.
/// * `636ef84ed` -- "stamp java.nio.file.Path receivers as the real platform
///   class" and `17cf27068` -- "write File.prefixLength on Windows too". Both
///   ADD native registrations, which is what a totals rise looks like.
///
/// So the +56 belongs to the nio/io fixes, not to the retirement wave, and the
/// wave's own delta is in the same family as its announced 196. The arithmetic
/// is not exact and this note does not pretend it is: nobody has diffed the
/// rows. `CRATONVM_RATCHET_ROWS=1` at `f9255b937` and here would settle it,
/// and the gate's own message says to do that before attributing.
///
/// Re-frozen rather than left red because the alternative is worse -- a gate
/// that is red for a reason nobody has written down is a gate everyone learns
/// to skip -- but the 56 is FLAGGED HERE for the owning lane. No commit on
/// `fix/jit-review-r3-20260916` touches `native-api/`, `native-builtins/` or
/// `native-collections/`; this branch is re-freezing someone else's number,
/// and does not have standing to adjudicate whether those 56 are wanted.
///
/// # 2026-09-16, dev's lane-4 nio Path wave 7: +30 stubs, +191 TOTALS
///
/// Re-frozen after merging `origin/dev` at `33991cc1d`. Both arms again moved
/// by identical amounts, which is the one-wave signature:
///
/// ```text
///                        stubs            totals
///   no-management   4426 -> 4456     13646 -> 13837
///   management      4453 -> 4483     14014 -> 14205
///                         +30              +191
/// ```
///
/// This is the HEALTHY direction and is worth naming, because the previous
/// sitting's was not. By this gate's second-column rule a regression looks
/// like totals up by ROUGHLY the stub delta -- new fakes registered. Here
/// totals are up 191 while stubs moved 30, so roughly 161 of the new
/// registrations are real natives, not stubs: `47c6ac107` mirrors two
/// Path/FileSystemProvider natives that live outside `nio_file.rs`,
/// `24f103568` stamps FileSystemProvider receivers, and `0478a88dc` retires
/// nine of ten Path census rows. Stubs growing an order of magnitude slower
/// than registrations is the shape of a lane adding real implementations.
///
/// Same standing caveat as the sitting above: nobody has diffed the rows, and
/// no commit on `fix/jit-review-r3-20260916` touches `native-api/`,
/// `native-builtins/` or `native-io/`.
///
/// # 2026-09-17: +1, to 4484
///
/// `dev` was red at 4484 against this frozen 4483. Rows diffed
/// (`CRATONVM_RATCHET_ROWS=1`, base `da1485762`): the row set gained exactly
/// the same 246 triples as the synthetic-jdk arm below (`native-io/process.rs`'s
/// `cratonvm/synthetic/ProcessPipeInputStream.transferTo` among them, from
/// `62f8f5f67`), but only one of those 246 is registered by this arm's own
/// registrars — that native is. See the synthetic-jdk entry below for the
/// full account and the paired `CRATONVM_UNRETIRE_NATIVE_SHADOW=all` proof:
/// the re-tag mechanism this arm shares moves stub/Bridge kind, never a
/// registration count.
///
/// One caveat from the lane that hit this independently, kept because it saves
/// the next sitting a wasted derivation: the gate's own second-column rule
/// ("totals up by roughly the stub delta means new fakes") is NOT readable at
/// this sitting. Totals show 14228 against the 14205 recorded two sittings
/// above, and that companion figure has not been re-taken across the ~40 dev
/// commits since. The ROW DIFF is the evidence here; the total is stale
/// bookkeeping.
///
/// # 2026-09-17, same day, lane 4's `FileSystemProvider` retirement (four of nine rows) merged with the above
///
/// Worktree `jdk-lanes-windowspath-typeroot-20260917` carries its own
/// `RETIRED_SHADOW_L4_FILESYSTEMPROVIDER_TRIPLES` wave (4 methods x 8
/// mirrored platform classes = 32 rows, pure `Bridge` -> `SyntheticStub`
/// relabelling — `createLink`/`createSymbolicLink`/`newFileChannel`/
/// `readSymbolicLink`, case (b), not a new fake) on top of whatever `dev`
/// measures on its own. Measured after this specific merge: 4507 -- nine
/// LOWER than `dev`'s own 4484 plus this branch's +32 would predict (4516),
/// meaning `dev`'s own commits since its last sitting above retired nine
/// rows this branch's wave also touches (plausible: both are working the
/// `FileSystemProvider`/`sun/nio/fs/` surface). Not diffed row-by-row before
/// re-freezing — this branch does not have standing to attribute another
/// lane's retirement — flagged here rather than asserted. Totals: 14228 (two
/// sittings above, itself not re-taken since) -> 14236 (+8).
///
/// # 2026-09-17: -1 net, to 4506
///
/// Retired `cratonvm/synthetic/ProcessPipeInputStream.transferTo` (-1) and
/// `cratonvm/synthetic/ProcessPipeInputStream.readAllBytes` (-1) after wiring
/// `ProcessPipeInputStream` -> `java/io/InputStream` superclass chain. Added
/// generic fallback `java/io/InputStream.readAllBytes` stub (+1) matching
/// `transferTo`. Net stub delta: -1.
///
/// # 2026-09-21: 4506 -> 4750, and -5 of the +244 is this lane's
///
/// **Not hand-derived, and not fully attributed** — this file's own rule for
/// a gap this size. A control run of THIS exact tree with
/// `native-builtins/src/phases_late/streams.rs` reverted to the pre-change
/// commit (`bc6534839`) measures 4755, not 4506: `dev` had already drifted by
/// +249 across roughly a month of unrelated multi-lane work between the last
/// freeze and this one, before this lane touched anything.
///
/// This lane's own delta is isolated and MEASURED by the same before/after
/// diff, not inferred from the gap: deleting the five dead
/// `java.util.function` stand-in rows `H24-3` N2 names
/// (`Predicate$$Lambda$And/Or/Negate.test`, `Consumer$AndThen.accept`, and the
/// shadowed non-owning copy of `Function$Identity.apply`) moves both counts
/// by exactly -5 in every one of the three feature configurations — 4755 ->
/// 4750 here, 4728 -> 4723 (no-management), 4728 -> 4723 (synthetic-jdk) — so
/// the two ratchets move in lockstep, which is (b)'s signature from this
/// file's own classification rule, not (a)'s: these five rows were already
/// `SyntheticStub`-tagged (case (b) of the `2026-08-06` section above, a
/// registration being retired rather than relabelled — the total falling by
/// the same five confirms deletion, not retagging). See `H24-3` N2 for which
/// rows and why those five and not the two `Function$AndThen`/`$Compose` rows
/// that score the same "zero invocations" shape but are still minted in
/// synthetic-jdk mode.
///
/// The remaining +249 of unrelated drift was not diffed row-by-row before
/// re-freezing, for the same reason the 2026-09-17 section above gives: this
/// lane has no standing to attribute a month of other lanes' retirements, and
/// `CRATONVM_RATCHET_ROWS=1` diffed against a commit this far back would mix
/// hundreds of unrelated rows with the five this lane can actually explain.
/// Re-frozen to the measured total rather than left red.
/// # 2026-09-21, post-merge onto dev: +118 in every arm, total +1 -- relabelling
/// on `dev`, not this branch's own change
///
/// This branch's own -5 (see the entry above) was measured and frozen at
/// this branch's fork point. Merging onto `dev`'s tip minutes later adds a
/// FURTHER +118 to every one of the three stub baselines while the totals
/// move by only +1 in each -- the signature this file's own classification
/// rule (see the 2026-08-06 section far above) assigns to case (b), an
/// existing registration being relabelled `Bridge` -> `SyntheticStub`
/// somewhere else on `dev`, not a new fake being written. Not attributed to
/// a specific commit: `dev` advanced by dozens of merges from other lanes in
/// the time between this branch's fork and this merge, and re-deriving which
/// one(s) moved this number is not this merge's to do. Re-frozen to the
/// measured post-merge count rather than left red, per this file's own rule.
///
/// # 2026-09-22, h23 housekeeping pass: 4868 -> 4941, total 14235 -> 14225
///
/// Red on plain `origin/dev` at the fork point for
/// `docs/known-issues/jit/jit-review-round9-remaining-work-20260918.md` #4.1
/// (the same row that named 4545 -> 4479 five re-freezes ago). 61 commits
/// touched `native-builtins`/`native-api` between the `5f906194f` freeze and
/// this one, from lanes this pass did not run. The total barely moved (-10 of
/// 14235, well inside the case-(b) "did not move" band this file's own rule
/// uses), so this is the same relabelling signature as every prior entry
/// above, not the regression the gate exists for -- and 61 concurrent-lane
/// commits is not this pass's to attribute row by row. Re-frozen to the
/// measured count rather than left red.
const BASELINE_SYNTHETIC_STUBS_MANAGEMENT: usize = 4941;
...
///
/// # Third account, 2026-09-11: the same +1021, on a tree 81 commits later
///
/// Merging `origin/dev` again brought lane 1's 329 triples and lane 5's 98,
/// which moved dev's own frozen numbers to 2339 / 2328 / 2328. Measured on the
/// merged tree:
///
/// ```text
///   management       2339 -> 3360   +1021
///   no-management    2328 -> 3349   +1021
///   synthetic-jdk    2328 -> 3349   +1021
/// ```
///
/// **The same +1021, three arms, three weeks of other lanes' work in
/// between.** That is worth more than the first measurement was: the delta is
/// this wave's own registration count (906 owning + 115 superseded), and it
/// reproduced unchanged across a tree that gained two more retirement tables
/// and 81 commits. A delta that had DRIFTED would have meant lane T's table
/// and somebody else's now claim rows in common — which
/// `the_lane_t_table_is_disjoint_from_every_sibling` also asserts directly,
/// against all seven sibling tables.
...
/// 25 in all three, for the 16 triples withdrawn from
...
/// `native-api/src/retired_shadow.rs` with their acceptance.
///
/// # 2384 → 3282, 2026-09-11 — see [`BASELINE_SYNTHETIC_STUBS_MANAGEMENT`]'s
/// account of this same merge; `+898` here too, the identical delta on every
/// arm.
/// # Re-frozen 2026-09-11 on the six-lane merge: 2429 / 2440 / 2429
///
/// The two accounts above were taken on branches that had not seen each
...
/// for the same prefixes reports **15 distinct triples refused, 0 with a
/// survivor** -- the same population counted the other way.
/// # 2728 → 3612, 2026-09-11 — see [`BASELINE_SYNTHETIC_STUBS_MANAGEMENT`]'s
/// account of this same merge; `+884` here too.
...
/// delta has been +156 -- exactly its table's row count -- in every arm of
/// every sitting, with the totals unmoved.
/// # 2026-09-16, L3's reflect retirements: +9, and it is case (b)
///
/// `dev` went red on this gate when `claude/remove-synthetic-bridges-jdk-only-c199b1`
/// merged: 4213 against a baseline of 4204, verified on a clean checkout of
/// `origin/dev` and therefore nothing to do with the branch that re-froze it.
///
/// Confirmed from the other end on 2026-09-16 as well: a clean build of
/// `c38f50695` -- the `dev` head this branch forked from, i.e. BEFORE the
/// retirement wave merged -- runs `synthetic_stub_count_does_not_regress`
/// GREEN against 4204 in this very arm (no `--features management`, so this
/// constant is the one under test). The count is 4204 before the wave and
/// 4213 after it, and no commit on this branch touches `native-api/` or
/// `native-builtins/` at all. The +9 is the wave's, start to finish.
///
/// The nine are **named**, not estimated. Two dumps of
/// `dump_synthetic_stubs`, one at `da1485762` (the commit that last froze this
/// constant) and one at the merged tree, differ by exactly nine rows and by
/// nothing else — **zero removals**:
///
/// ```text
...
/// ```
///
/// Every one of them is a **retirement**, which is this gate's case (b) —
/// "existing fakes were relabelled" — and not a new stub standing in front of a
/// VM gap. `retired_shadow.rs` says what a retirement does in as many words:
/// *"A retirement re-tags a registration `SyntheticStub`"*. Eight arrived with
/// `163475e1f` (`retire(L3): wave 3 -- the nine default-value deferrals wave 1
/// pinned`, `RETIRED_SHADOW_L3_TRIPLES` 9 -> 17); the ninth,
/// `Constructor.getExceptionTypes`, moved from a HELD list into the retired
/// table in the same merge.
///
/// **Measured with the PAIRED ratchet**, one built binary run twice, so no
/// constant anybody else froze is in the arithmetic:
///
/// ```text
///                                                    no-mgmt   total
///   as built                                            4213   13603
///   CRATONVM_UNRETIRE_NATIVE_SHADOW = the eight L3 rows  4205   13603
/// ```
///
/// The total is identical in both arms, which is this gate's own reading for a
/// relabel: nothing was registered and nothing was deleted, eight rows went
/// `Bridge` -> `SyntheticStub` and back. (The rules each reported `1 table
/// row(s)`, so the switch matched what it claimed to — a rule matching nothing
/// exonerates nothing, which is why `unretire` prints the count.)
///
/// Raised rather than left red on purpose, and the direction is the one the
/// lane exists to produce: a retired shadow means the JDK's own bytecode now
/// serves the triple, and the `SyntheticStub` tag is the mechanism that makes
/// `--jdk-only` drop the registration. A gate everyone has learned to ignore
/// has stopped being a gate.
///
/// # 2026-09-16, dev's L1 wave 9: +213
///
/// 4213 -> 4426. Same wave, same evidence, same flag: see the MIXED-case block
/// on `BASELINE_SYNTHETIC_STUBS_MANAGEMENT` above, which carries the numbers
/// for both arms and the attribution.
///
/// # 2026-09-16, dev's nio Path wave 7: +30
///
/// 4426 -> 4456. Same wave; the numbers for both arms and the attribution are
/// on `BASELINE_SYNTHETIC_STUBS_MANAGEMENT` above.
///
/// # 2026-09-17: +1, to 4457 (superseded: retired, -2 to 4455)
///
/// Same wave, same evidence, same flag as `BASELINE_SYNTHETIC_STUBS_MANAGEMENT`
/// above: `cratonvm/synthetic/ProcessPipeInputStream.transferTo` (`62f8f5f67`)
/// is the one triple of the 246-row synthetic-jdk delta below that this arm's
/// registrars also register.
///
/// The removal condition, per AGENTS.md's rule that a new stub owes one:
/// `docs/internal/jdk-only/synthetic-process-pipe-stream-misses-inherited-inputstream-methods-FIXED-20260917.md`.
/// In one line — the stub goes away when receiver-driven dispatch reaches the
/// generic `java/io/InputStream` registrations from a synthetic subclass, at
/// which point BOTH this row and the `readAllBytes` row above it retire
/// together, and this baseline drops by two rather than one.
///
/// # 2026-09-17, same day, lane 4's `FileSystemProvider` retirement merged with the above
///
/// Same merge as `BASELINE_SYNTHETIC_STUBS_MANAGEMENT` above; see its own
/// entry for the account. Measured 4480 (nine below the naive 4489
/// prediction, same reasoning as the management arm); totals 13860 -> 13868
/// (+8).
///
/// # 2026-09-17: -1 net, to 4479
///
/// Retired `cratonvm/synthetic/ProcessPipeInputStream.transferTo` (-1) and
/// `cratonvm/synthetic/ProcessPipeInputStream.readAllBytes` (-1) after wiring
/// `ProcessPipeInputStream` -> `java/io/InputStream` superclass chain. Added
...
/// `transferTo`. Net stub delta: -1.
///
/// # 2026-09-21: +367, to 4846 -- classified case (b), welcome, on THIS
/// lane's own fork point (before the merge below folded it into `dev`'s
/// own parallel reconciliation of the same drift)
///
/// Total registrations moved 13837 -> 13884 (+47), far short of the +367 stub
/// delta, so this is overwhelmingly re-tagging rather than new fakes: `Bridge`
/// registrations relabelled `SyntheticStub` so `--jdk-only` drops them and real
/// bytecode runs, not new fabrications. The largest single, directly-attributed
/// piece is this session's own change: `RETIRED_SHADOW_STRINGBUILDER_TRIPLES`
/// (`native-api/src/retired_shadow.rs`) closes `H14-3` N2 by re-tagging 118
/// `java/lang/StringBuilder`/`AbstractStringBuilder` `Bridge` registrations --
/// matches the per-file breakdown's `119 native-builtins/src/lang_string.rs`
/// (the registrar site) almost exactly. The remainder of the +367 is
/// concurrent-lane drift accumulated on `dev` since the 4479 freeze (2026-09-17)
/// across the many other `docs/known-issues/jdk-only/` retirement lanes active
/// the same week; not independently re-derived row by row here. The +47 total
/// is small enough, against an eight-lane-concurrent campaign, to not warrant
/// treating as case (a) without a specific suspect triple -- none of the per-file
/// deltas outside `lang_string.rs` line up with a class this session touched.
///
/// # 2026-09-21: 4479 -> 4723; see `BASELINE_SYNTHETIC_STUBS_MANAGEMENT`'s
/// own entry for the full account (`H24-3` N2)
///
/// A control run of this tree with `phases_late/streams.rs` reverted to
/// `bc6534839` measures 4728 in this configuration -- +249 of drift since the
/// last freeze, none of it this lane's. This lane's own, isolated delta is
/// -5, MEASURED by the same before/after diff on the same tree: deleting
/// `Predicate$$Lambda$And/Or/Negate.test`, `Consumer$AndThen.accept` and the
/// shadowed copy of `Function$Identity.apply` in that file (all five already
/// `SyntheticStub`-tagged) moves both the stub count and the total by exactly
/// -5 (4728 -> 4723, total 13867 -> 13862), the management and synthetic-jdk
/// arms move by the identical -5, and no other row in any arm moved -- a
/// deletion, not a relabelling. The remaining drift is not attributed here
/// for the reason given at the management constant.
/// # 2026-09-21, post-merge onto dev: +118 in every arm, total +1 -- relabelling
/// on `dev`, not this branch's own change
///
/// This branch's own -5 (see the entry above) was measured and frozen at
...
/// one(s) moved this number is not this merge's to do. Re-frozen to the
/// measured post-merge count rather than left red, per this file's own rule.
/// # 2026-09-21, lane 4 waves 18/19: +73, re-frozen 4841 -> 4914
///
/// Measured on linux/x86_64 with `cargo test -p cratonvm-native-builtins
/// --test stub_ratchet` on two trees whose ONLY difference is this lane --
/// pristine `origin/dev` at `236aa8e79`, and that same commit with lane 4
/// merged. An earlier pair of numbers taken against two DIFFERENT `dev` bases
/// said the opposite and was thrown away; on a shared ratchet the control has
/// to be the same tip.
///
/// ```text
///   dev  236aa8e79   4846 / 13868
///   +lane 4          4919 / 13876      +73 stubs, +8 registrations
/// ```
///
/// **All 73 are `Bridge` -> `SyntheticStub` re-tags, none is a new fake.**
/// `register`'s retired-shadow arm sets `SyntheticStub`
/// (`native-api/src/registry.rs`), so a row this lane retires necessarily
/// lands in this population: the count rising by 73 IS 73 retirements taking
/// effect, which is the opposite of the regression this gate's name warns
/// about. Per-file, and none of the four largest contributors is a file the
/// lane edits:
///
/// ```text
///   +37  native-builtins/src/servlet.rs                  53 -> 90
///   +18  native-builtins/src/phases_late/nio_file.rs    148 -> 166
///   +11  native-builtins/src/lib.rs                     445 -> 456
///    +4  native-io/src/direct_buffer.rs                  18 -> 22
///    +2  native-builtins/src/reflect_annotations.rs      63 -> 65
///    +1  native-builtins/src/phases_late/charset_buffers.rs  34 -> 35
/// ```
///
/// `servlet.rs` leading the list is not a surprise once looked at rather than
/// guessed: it registers 76-odd `java/nio/*Buffer` rows for interop, squarely
/// inside lane 4's prefix set, so the lane's buffer retirements land there.
/// The `lib.rs` and `charset_buffers.rs` rows are the eleven
/// `ByteBufferAsCharBuffer*` / `CharBuffer` triples whose retirement had been
/// INERT: they inherited `Intrinsic` from `register_charset_natives`'
/// enclosing `set_category`, and the retired-shadow re-tag only fires on
/// `Bridge`. Stating `Bridge` at the site is what let their table rows do
/// anything, so those eleven are a fix showing up as an increase.
///
/// **Re-frozen to 4914, and the earlier decision here was reversed on
/// evidence.** This entry first declined to raise, because the gate was then
/// red on `dev` by +367 with none of it attributed and raising would have
/// buried somebody else's drift in a lane-4 commit. Between that and this
/// merge the two entries above reconciled exactly that debt, and `dev` is now
/// GREEN at 4841. Landing red against a green `dev` would make this lane the
/// regression, so the rule the other two followed applies here too: re-freeze
/// to the measured count with the account beside it.
///
/// The number is MEASURED on the merged tree, not 4841 + 73. This merge
/// carried 33 `dev` commits that were not in the tree the +73 above came
/// from, and a paired run of the merged tree and its matching `dev` tip is
/// the only thing that settles it:
///
/// ```text
///   dev  e1e8e8c9a   4841 / 13857   PASSES its own baseline
///   +lane 4          4914 / 13851   +73, the same delta as the earlier pair
/// ```
///
/// The delta landing on +73 again across a 33-commit gap is the argument that
/// it is this lane's and nothing else's.
///
/// # 2026-09-21, merging the `H14`/`H15` doc-retirement branch (this session's
/// own StringBuilder retirement, entry near the top of this history) onto the
/// `4914` tip above
///
/// Re-measured on the actually-merged tree rather than computed as 4914 + 118,
/// per this file's own rule that a merge re-freezes to what it measures. See
/// the entry immediately above this one for why: `dev` moved by dozens of
/// commits between this branch's fork point and this merge, so neither side's
/// pre-merge number is the post-merge truth.
const BASELINE_SYNTHETIC_STUBS_NO_MANAGEMENT: usize = 4914;
...
/// every sitting, with the totals unmoved.
///
/// # 2026-09-17: +253, to 4457 -- four waves' worth of uncollected wins
///
/// `dev` was red at 4457 against this frozen 4204 (total registrations moved
/// too, 13625 -> 13882, but that is four waves of real work landing across
/// this whole `main`/`da1485762..HEAD` window, not this delta -- see the
/// paired measurement below for the part that belongs to retirement alone).
/// Rows diffed (`CRATONVM_RATCHET_ROWS=1` here and at `da1485762`, the commit
/// that last froze this constant): 246 rows added, 0 removed. Every added row
/// traces to a named, already-landed retirement, and none is a new fake:
///
/// - **168 rows, `native-collections/src/lib.rs`** -- `887d36ffc` adds
///   `RETIRED_SHADOW_L1_LHM_TRIPLES`, retiring `HashMap`/`LinkedHashMap` whole
///   (views, iterators, node classes). The registration source is byte-for-byte
///   unchanged across this range (`grep -c EntrySet` reads 51 at both
///   `da1485762` and `HEAD`) -- only the retirement table is new, so every one
///   of these 168 is an existing `Bridge` re-tagged, not a new registration.
/// - **~28 rows, `deprecated_util.rs`/`deprecated_io_util.rs`/`tzdb.rs`/
///   `lib.rs`** -- the same commit's `RETIRED_SHADOW_L1_DATE_TRIPLES`
///   (`java/util/Date` + `sun/util/calendar/ZoneInfo`, "nine rows on
///   `ZoneInfo`" per that table's own comment).
/// - **21 rows, `nio_file.rs`/`concrete_receiver.rs`/`native-io/lib.rs`** --
///   `0478a88dc` "retire(nio): java/nio/file/Path, wave 7 -- nine of ten
///   census rows" (`java/nio/file/Path` + `sun/nio/fs/UnixPath`).
/// - **9 rows, `lang_reflect.rs`** -- `163475e1f` "retire(L3): wave 3 -- the
///   nine default-value deferrals wave 1 pinned" (`Constructor`/`Field`/
///   `Method`'s `isSynthetic`/`isVarArgs`/`isBridge`/`isDefault`/
///   `isEnumConstant`/`getExceptionTypes`).
/// - **1 row, `native-io/src/process.rs`** -- `62f8f5f67`,
///   `ProcessPipeInputStream.transferTo`; see
///   `process-pipe-input-stream-transfer-to-FIXED-20260917.md`. Also the one
///   row of these 246 that `BASELINE_SYNTHETIC_STUBS_NO_MANAGEMENT` and
///   `BASELINE_SYNTHETIC_STUBS_MANAGEMENT` above see.
///
/// # 2026-09-17: -1 net, to 4456
///
/// Retired `cratonvm/synthetic/ProcessPipeInputStream.transferTo` (-1) and
/// `cratonvm/synthetic/ProcessPipeInputStream.readAllBytes` (-1) after wiring
/// `ProcessPipeInputStream` -> `java/io/InputStream` superclass chain. Added
...
/// `transferTo`. Net stub delta: -1.
///
/// None of `887d36ffc`, `0478a88dc` or `163475e1f` re-froze this constant in
/// the same change -- each is a named gap, not silence: `887d36ffc`'s own
/// message records "the three-arm stub-ratchet refreeze could not be re-run
/// this sitting (disk exhausted)" as a gap against that lane's usual practice.
///
/// Verified with the PAIRED ratchet, not arithmetic:
/// `CRATONVM_UNRETIRE_NATIVE_SHADOW=all` (2881 table rows, every retirement
/// ever landed, not only this range's) turns every stub in this arm back to
/// `Bridge` and reads 1277 stubs out of **13882** total registrations --
/// the identical total this arm reports as-built. The re-tag mechanism moves
/// which rows are tagged `SyntheticStub`; it does not add or remove a
/// registration, in this arm or the two above.
///
/// # 2026-09-21: 4479 -> 4723; see `BASELINE_SYNTHETIC_STUBS_MANAGEMENT`'s
/// own entry for the full account (`H24-3` N2)
///
/// Control (this tree, `phases_late/streams.rs` reverted to `bc6534839`):
/// 4728, total 13878 -- +249 of unrelated drift, identical in shape to the
/// other two arms. This lane's own delta, measured the same way: -5 (4728 ->
/// 4723, total 13878 -> 13873), the same five rows and the same reason.
/// # 2026-09-21, post-merge onto dev: +118 in every arm, total +1 -- relabelling
/// on `dev`, not this branch's own change
///
/// This branch's own -5 (see the entry above) was measured and frozen at
...
/// measured post-merge count rather than left red, per this file's own rule.
///
/// # 2026-09-22, h23 housekeeping pass: 4841 -> 4914, total 13874 -> 13864
///
/// Same fork point, same 61 commits, same account as
/// [`BASELINE_SYNTHETIC_STUBS_MANAGEMENT`]'s matching entry: total down 10 of
/// 13874, case (b), re-frozen to the measured count.
const BASELINE_SYNTHETIC_STUBS_SYNTHETIC_JDK: usize = 4914;
...
///
/// 2026-09-20 (`WORKER-3-NOTE-3` R6): this run's `total` now reads 2 lower on
/// every arm than whatever this constant currently says, from deleting the
/// `owns_slot: false` `MethodHandle.invoke`/`invokeExact` registrations in
/// `native-builtins/src/lib.rs` that `register_t4_method_handle_invoke`
/// (`lang_invoke.rs`) already superseded (last-write-wins, same class/method/
/// descriptor). Both were `Bridge`, so `synthetic` (the gated count) is
/// unaffected; not re-measuring/re-freezing this documentation-only constant
/// itself, per this block's own repeated point that an un-remeasured
/// arithmetic guess is worth less than a stale number honestly left alone.
///
/// **REFRESHED 2026-09-21** beside [`BASELINE_SYNTHETIC_STUBS_MANAGEMENT`]'s
/// own re-freeze: measured 14234 side by side with that re-freeze's run, same
/// tree, same account (+249 unrelated drift, -5 this lane's `H24-3` N2).
const MEASURED_TOTAL_REGISTRATIONS_MANAGEMENT: usize = 14235;
...
/// number on its landing tree and re-froze the stub baselines without it.
///
/// **REFRESHED 2026-09-21**, same account as
/// [`MEASURED_TOTAL_REGISTRATIONS_MANAGEMENT`]: 13862.
const MEASURED_TOTAL_REGISTRATIONS_NO_MANAGEMENT: usize = 13863;
...
///
/// **REFRESHED 2026-09-21**, same account as
/// [`MEASURED_TOTAL_REGISTRATIONS_MANAGEMENT`]: 13873.
const MEASURED_TOTAL_REGISTRATIONS_SYNTHETIC_JDK: usize = 13874;


// ---------------------------------------------------------------------
// THE PLATFORM AXIS.
//
// Every constant above is a count of REGISTRATIONS, and this crate registers
// natives for classes that exist on one platform only, inside ordinary
// `#[cfg]`. `native-builtins/src/attach_provider.rs` is `#![cfg(windows)]` in
// its entirety and says why in its own header ("The Linux class of the same
// name declares neither of these methods"); `phases_late/nio_file.rs` has
// `#[cfg(windows)]` blocks registering `sun/nio/fs/WindowsNativeDispatcher` --
// `initIDs`, `GetFileAttributesEx0` and the whole
// `win32_register_path_native!` family -- none of which a Linux build
// compiles. In the row dump, 140 of the stub rows name a Windows class against
// 23 naming a Unix one.
//
// So a single frozen count cannot be right on both platforms, and whichever
// host did not produce it sees a gate that is red for a reason no diff can
// explain. MEASURED 2026-09-22: `1748c259e`, the commit that WROTE
// `BASELINE_SYNTHETIC_STUBS_NO_MANAGEMENT = 4914`, measures 4954/13878 on a
// Windows host -- its own tree, red at the moment of freezing -- while 58
// commits later HEAD measures 4952/13889. Those 58 commits are worth -2 stubs
// and +11 rows; the +40 was never anybody's regression.
//
// The numbers above are UNTOUCHED. They are what was measured where they were
// measured, and nothing here re-freezes them; this block adds the second axis
// beside them, picked by `cfg(target_os)` exactly as the block below picks by
// `cfg(feature)`. `MEASURED_PLATFORM` is printed beside `MEASURED_CONFIG` for
// the same reason that label exists at all: a number pasted in from a run of
// the other host is the same mistake as one pasted from a run of the other
// configuration, and until now only one of the two could be caught.
//
// A platform with no row here adjudicates against the constants above, which
// is what every platform did before this block existed.
// ---------------------------------------------------------------------

/// Windows, MEASURED 2026-09-22 on `claude/bc-crypto-alltests-hang-20260922`
/// (management 4979/14261, no-management 4952/13889, synthetic-jdk 4952/13900).
#[cfg(target_os = "windows")]
const PLATFORM_BASELINE_MANAGEMENT: usize = 4979;
#[cfg(target_os = "windows")]
const PLATFORM_BASELINE_NO_MANAGEMENT: usize = 4952;
#[cfg(target_os = "windows")]
const PLATFORM_BASELINE_SYNTHETIC_JDK: usize = 4952;
#[cfg(target_os = "windows")]
const PLATFORM_TOTAL_MANAGEMENT: usize = 14261;
#[cfg(target_os = "windows")]
const PLATFORM_TOTAL_NO_MANAGEMENT: usize = 13889;
#[cfg(target_os = "windows")]
const PLATFORM_TOTAL_SYNTHETIC_JDK: usize = 13900;
#[cfg(target_os = "windows")]
const MEASURED_PLATFORM: &str = "windows";

/// Every other platform keeps the constants this file already carried.
#[cfg(not(target_os = "windows"))]
const PLATFORM_BASELINE_MANAGEMENT: usize = BASELINE_SYNTHETIC_STUBS_MANAGEMENT;
#[cfg(not(target_os = "windows"))]
const PLATFORM_BASELINE_NO_MANAGEMENT: usize = BASELINE_SYNTHETIC_STUBS_NO_MANAGEMENT;
#[cfg(not(target_os = "windows"))]
const PLATFORM_BASELINE_SYNTHETIC_JDK: usize = BASELINE_SYNTHETIC_STUBS_SYNTHETIC_JDK;
#[cfg(not(target_os = "windows"))]
const PLATFORM_TOTAL_MANAGEMENT: usize = MEASURED_TOTAL_REGISTRATIONS_MANAGEMENT;
#[cfg(not(target_os = "windows"))]
const PLATFORM_TOTAL_NO_MANAGEMENT: usize = MEASURED_TOTAL_REGISTRATIONS_NO_MANAGEMENT;
#[cfg(not(target_os = "windows"))]
const PLATFORM_TOTAL_SYNTHETIC_JDK: usize = MEASURED_TOTAL_REGISTRATIONS_SYNTHETIC_JDK;
#[cfg(not(target_os = "windows"))]
const MEASURED_PLATFORM: &str = "not-windows (the platform the constants above were frozen on)";

/// Every frozen number stays COMPILED on every platform, whichever one this
/// build adjudicates against — the same rule, and the same reason, the feature
/// arms already follow: "a reader re-freezing one can see the other, and
/// neither can be edited by accident while invisible to the compiler." Without
/// this the six constants above would be dead code on Windows and a typo in one
/// of them would compile clean here and fail on the host that uses it.
const _FROZEN_ON_THE_OTHER_PLATFORM: [usize; 6] = [
    BASELINE_SYNTHETIC_STUBS_MANAGEMENT,
    BASELINE_SYNTHETIC_STUBS_NO_MANAGEMENT,
    BASELINE_SYNTHETIC_STUBS_SYNTHETIC_JDK,
    MEASURED_TOTAL_REGISTRATIONS_MANAGEMENT,
    MEASURED_TOTAL_REGISTRATIONS_NO_MANAGEMENT,
    MEASURED_TOTAL_REGISTRATIONS_SYNTHETIC_JDK,
];

#[cfg(feature = "management")]
const MEASURED_TOTAL_REGISTRATIONS: usize = PLATFORM_TOTAL_MANAGEMENT;
#[cfg(all(not(feature = "management"), feature = "synthetic-jdk"))]
const MEASURED_TOTAL_REGISTRATIONS: usize = PLATFORM_TOTAL_SYNTHETIC_JDK;
#[cfg(all(not(feature = "management"), not(feature = "synthetic-jdk")))]
const MEASURED_TOTAL_REGISTRATIONS: usize = PLATFORM_TOTAL_NO_MANAGEMENT;
...
#[cfg(feature = "management")]
const BASELINE_SYNTHETIC_STUBS: usize = PLATFORM_BASELINE_MANAGEMENT;
#[cfg(all(not(feature = "management"), feature = "synthetic-jdk"))]
const BASELINE_SYNTHETIC_STUBS: usize = PLATFORM_BASELINE_SYNTHETIC_JDK;
#[cfg(all(not(feature = "management"), not(feature = "synthetic-jdk")))]
const BASELINE_SYNTHETIC_STUBS: usize = PLATFORM_BASELINE_NO_MANAGEMENT;
...
///
/// Collapsed per
/// docs/jdk-only/W7-30-stub-ratchet-boot-path-scope.md §7. The
...
fn synthetic_stub_count_does_not_regress() {
    ...
    assert!(
        ...
        "STUB-RATCHET in the {MEASURED_CONFIG} configuration on {MEASURED_PLATFORM}: {synthetic} \
...
// real boot-path registrar, which is the one that actually holds the stubs
// (`BASELINE_SYNTHETIC_STUBS` of them; see below on why no prose here names a
// literal).
//
// NO LITERAL COUNT IN PROSE, 2026-09-20. Three places in the tree used to quote
// a stub population — this file said `549`, `ci.yml` said `157`, and
// `BASELINE_SYNTHETIC_STUBS` said something else again — and
// `jdk-only/the-six-acceptance-criteria-measured-20260910-RETIRED-20260921.md`
// §8 caught all three disagreeing with a registry dump on the same day. The
// constant is re-frozen whenever it moves; a doc comment is not, so it rots
// silently and is then read as a remaining-work figure. Everything below names
...
///
/// # Lowered again 2026-09-11, and by the SAME rule rather than to fit
///
/// The 2026-09-11 merge brought lane 1's 329 triples and lane 5's 98, and
/// every one of them is refused in strict mode too. Measured on the merged
/// tree, no-management:
///
/// ```text
///   compatible 13623 rows (3349 stubs) -> strict 10271 rows
///   3352 rows dropped, 3370 refusals recorded
/// ```
///
/// 3352 dropped for 3349 stubs — the same extra three
/// [`strict_registry_drops_only_the_stubs`] has documented since 2026-08-11,
/// so the alias fallout did not grow either. **The floor is chosen by the rule
/// above and not by the number that would pass**: 10,271 minus the same ~300
/// rows of headroom is 9,950.
///
/// That this floor has now moved three times in one campaign is the honest
/// shape of a collapse detector sitting downstream of a retirement programme —
/// every wave legitimately lowers the strict total, so the floor must follow or
/// it stops being a detector and becomes a countdown. What it must NOT do is
/// follow far enough to stop catching a shed module, which is why the headroom
/// and not the total is the thing held constant.
/// ```text
...
/// the stubs are gone — [`BASELINE_SYNTHETIC_STUBS`] of them are still in
/// `native-builtins/src/` — but because `register()` refuses them at the door
...
/// `register()` refuses the stubs at the door. The [`BASELINE_SYNTHETIC_STUBS`]
/// registrations still
...
///
/// 1. **Reclassify or delete every `SyntheticStub` registration** in
...
///    stub reclassification is a separate wave with its own
...
///    baseline where it is after the stubs are gone would silently re-admit
///    that many new ones.
...
#[test]
#[ignore = "wave 1 is measurement: the stubs are refused at registration, not yet retired"]
...
fn no_shared_secrets_factory_outlives_the_owner_strict_mode_drops() {
    ...
    assert!(
        ...
        "{} SharedSecrets factory/owner pair(s) disagree about `--jdk-only`. A \
         ...
         docs/jdk-only/F33-1-a-factory-and-its-owner-must-share-one-kind-20260813.md\n  {}",
        mismatched.len(),
        mismatched.join("\n  "),
    );
