...
/// **RE-TAKEN 2026-09-10, in the same commit as `registrar_drift.rs`'s own
/// re-take, and that is the whole reason it moved.** Six families gained
/// exposure — `register_classloader_natives` 82 -> 85,
...
/// `register_unsafe_define_class` 1 -> 2 — because the drift scanner's
/// enclosing-fn sweep learned to read `register_with_kind(`, which it had been
/// skipping. Every added triple is a shipping registration that was always
/// there and could not be seen. This table's own panic text names the rule:
/// pasting is the whole fix when the sibling gate was re-taken in the same
