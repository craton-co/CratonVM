...
//!
//! The *census* is not new. `docs/jdk-only/W7-5-registrars-that-
...
//! [`scan::rs_files`] and `docs/internal/jdk-only/G54-1-…-20260817.md` §3.
//!
...
const DELIBERATE_SYNTHETIC_ONLY_FAMILIES: &[(&str, &str)] = &[
    ...
    ("register_cds_natives",
     "REAL-JDK BYTECODE: all 1 exclusive classes exist in JDK 25 and declare no native method; drift exposure in FAMILY_DRIFT_EXPOSURE"),
    ("register_character_classification_natives",
     "SHIPPING TWIN: no class exclusive to it; drift exposure in FAMILY_DRIFT_EXPOSURE"),
    ...
    ("register_stack_walker_option_clinit_synthetic",
     "REAL-JDK BYTECODE: all 1 exclusive classes exist in JDK 25 and declare no native method; drift exposure in FAMILY_DRIFT_EXPOSURE"),
...
///
/// **Re-taken 2026-09-17, one row: `register_bigdecimal_natives` 18 -> 20.**
/// Paired with `registrar_drift.rs`'s `BASELINE_TOTAL_DRIFT`/`BASELINE_TOTAL_PAIRS`
/// re-take the same day: `register_bigdecimal_arithmetic_overrides` gained
/// `BigDecimal.compareTo`/`BigDecimal.divide(BigDecimal)`, landed by a merge
/// from origin/dev, not by this branch. `registrar_drift.rs` was re-taken in
/// the same commit, so pasting is the whole fix here too.
///
/// **Re-taken 2026-09-17 (origin/dev, independent), one row:
/// `register_biginteger_natives` 19 -> 16.** Paired with `registrar_drift.rs`'s
/// `BASELINE_TOTAL_DRIFT`/`BASELINE_TOTAL_PAIRS` -3 and its three new
/// `FIXED_NOT_DRIFTING` rows: the synthetic-only `divide`/`mod`/`remainder`
/// trio in `math_bignum.rs`'s `register_biginteger_natives` was deleted (it
/// always won the slot in synthetic-JDK mode over
/// `register_p71_biginteger_extras`, and mis-registered `remainder` as `mod`),
/// so that pass no longer shares those three triples with a shipping pass.
/// Both re-takes apply to the merged tree — `registrar_drift.rs`'s 2026-09-17
/// reconciliation note has the combined-constant account.
///
/// **Re-taken 2026-09-21 (G54-1 NOMINATION 1 & 2, landed), three rows:
/// `register_http2_natives` 48 -> 53, `register_phase53_natives` 45 -> 83,
/// `register_phase60_natives` 28 -> 30.** Paired with `registrar_drift.rs`'s
/// `BASELINE_TOTAL_DRIFT`/`BASELINE_TOTAL_PAIRS` re-take the same day: that
/// scanner's `rs_files` no longer treats a plain `use` import or
/// `vm/src/vm/tests.rs` as a shipping root, so it no longer under-counts
/// `register_p60_callsite`, `register_p60_record`,
/// `register_p65_method_handles_extra`, `register_phase52_string_buffer`,
/// `register_phase53_record` and `register_synthetic_socket_stubs`'s shared
/// triples — all six are inside the subtree of one of the three families
/// above. `registrar_drift.rs` was re-taken in the same commit, so pasting is
/// the whole fix here too.
///
/// **Re-taken 2026-09-21 (G54-1 NOMINATION 5, landed): `register_byte_array_output_stream`
/// removed.** Its two call sites (one in `lib.rs`, one in
/// `serialization.rs`, both inside `register_synthetic_overrides`) were
/// deleted — the function registers nothing (TOMBSTONE-verdicted, see
/// `DELIBERATE_SYNTHETIC_ONLY_FAMILIES`'s retired entry), so removing the
/// calls changes nothing about what either registry contains, but it does
/// remove the pass from `register_synthetic_overrides`'s direct-child set.
/// Also removed from `DELIBERATE_SYNTHETIC_ONLY_FAMILIES` and
/// `SYNTHETIC_ONLY_CLOSURE` in the same commit.
const FAMILY_DRIFT_EXPOSURE: &[(&str, usize)] = &[
    ("register_aot_natives", 1),
    ("register_atomic_boolean_natives", 8),
    ("register_bigdecimal_natives", 20),
    ("register_biginteger_natives", 16),
    ("register_cds_natives", 0),
    ("register_character_classification_natives", 0),
    ...
    // 12 -> 13 on 2026-09-20, G19-1 NOM-1, and the row moved because the
    // drift was REPAIRED. `register_pe_function_descriptor` (in
    // `register_pe_panama`'s subtree) registered `argumentLayouts` under the
    // pre-JDK-22 preview signature `()[Ljava/lang/foreign/ValueLayout;`, so
    // the triple did not collide with the shipping
    // `register_p67_foreign_memory` row and neither gate saw a pair.
    // Correcting it to the measured `()Ljava/util/List;` makes the two passes
    // share one more triple. `registrar_drift.rs` was re-taken in the same
    // commit with the reasoning at the row.
    ("register_pe_panama", 13),
    ("register_phase50_natives", 1),
    ("register_phase51_natives", 44),
    ("register_phase52_natives", 35),
    ("register_phase53_natives", 83),
    ...
    ("register_phase59_natives", 56),
    ("register_phase60_natives", 30),
    ...
    ("register_stack_walker_option_clinit_synthetic", 0),
    ...
    pub fn rs_files(dir: &Path, out: &mut Vec<PathBuf>) {
        ...
        for e in rd.flatten() {
            ...
            if p.is_dir() {
                ...
                // `docs/internal/jdk-only/G54-1-…-20260817.md` §3.
                if matches!(
                    name.as_ref(),
...
fn no_new_synthetic_only_family() {
    ...
    assert!(
        ...
        "{} registrar family/families are reachable ONLY through \
         ...
         bodies at all.\n\
         See docs/jdk-only/W7-5-registrars-that-never-shipped.md §3 for the four \
