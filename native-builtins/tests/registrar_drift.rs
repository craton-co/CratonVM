...
///
/// **1273 -> 1275, 2026-09-17.** `register_bigdecimal_arithmetic_overrides`
/// (`native-builtins/src/math_bignum.rs`, RBIGDEC.1's real-JDK-mode shipping
/// pass) gained `compareTo(BigDecimal)I` and `divide(BigDecimal)BigDecimal`
/// (`native_bd_compare_to_exact` / `native_bd_divide_exact`, pinned against
/// real `openjdk 25.0.3` per that file's own 2026-09-17 comment) -- both
/// already registered by the older synthetic-only `register_bigdecimal_natives`
/// (`native_bd_compare_to` / `native_bd_divide`). Landed by a merge from
/// origin/dev during the lane-4 WindowsPath.type/.root session, not by this
/// branch; whether the two bodies agree was not diffed row-by-row here, so
/// that question is flagged for the owning BigDecimal lane rather than
/// asserted. Two new distinct triples, two new pairs (neither triple drifted
/// under any other pass before).
///
/// **1273 -> 1270, 2026-09-17 (origin/dev, independent).** Three rows left `register_biginteger_natives`'s
/// share of [`DRIFT_TRIPLES`]: `java/math/BigInteger` `divide`/`mod`/`remainder`.
/// The synthetic-only trio in `math_bignum.rs` was deleted (it always won the
/// slot in synthetic-JDK mode by running after `register_p71_biginteger_extras`,
/// and mis-registered the same body for `mod` and `remainder`), so
/// `register_p71_biginteger_extras` is now the sole registrant in every mode.
/// These ARE candidates for [`FIXED_NOT_DRIFTING`] — unlike the two rows above,
/// the triples still have a surviving registrant — and are recorded there.
///
/// **Reconciled 2026-09-17.** This branch's BigDecimal re-take (1273 -> 1275,
/// above) and `origin/dev`'s independent BigInteger cut (1273 -> 1270, this
/// entry) both apply to the merged tree — the BigInteger deletion is real
/// source code that merged cleanly, and the BigDecimal overlap this branch
/// documented is still present (recorded, not fixed, by either side). Neither
/// number alone describes the tree that contains both changes, so the merge
/// re-measures fresh rather than combining by arithmetic: BigInteger's -3
/// against this branch's 1275 lands the merged tree at 1272 — re-verify
/// against this file's own `cargo test` output, not this comment, if the two
/// ever disagree again.
///
/// **Update, later on 2026-09-17: the BigDecimal body defect is FIXED; the rows
/// stay.** Both synthetic bodies now parse their value string exactly
/// (`bd_parse_decimal_str`) and run through the same cores as the shipping ones
/// (`bd_compare_unscaled`, `bd_exact_divide_core`), so the two modes agree in
/// value. The two registrations still exist, so these remain drift rows: this
/// gate counts registrations, not whether their bodies agree. See
/// `docs/internal/jdk-only/synthetic-bigdecimal-comparison-keeps-the-f64-body-20260917.md`.
///
/// **Re-taken 2026-09-21 (G54-1 NOMINATION 1 & 2, landed): 1273 -> 1313, +40.**
/// This scanner's `rs_files` used to (a) treat a plain `use` import as a
/// shipping root and (b) read `vm/src/vm/tests.rs` as source, both of which
/// wrongly excluded six passes from the synthetic-only set —
/// `register_p60_callsite`, `register_p60_record`,
/// `register_p65_method_handles_extra`, `register_phase52_string_buffer`,
/// `register_phase53_record`, `register_synthetic_socket_stubs` — and hid
/// whatever they shared with a shipping pass from this table entirely (see
/// `KNOWN_POPULATION_DIVERGENCE`'s retirement note, and G54-1 §4.3, which
/// found and pinned the six but deliberately deferred the repair because it
/// moves the census). Fixing both closes the gap `registrar_reachability.rs`
/// already closed for its own equivalent scan on 2026-08-17. The +40 is
/// `register_p60_callsite` sharing 2 triples with
/// `register_method_handle_combinator_extras_bridge`, and
/// `register_synthetic_socket_stubs` sharing 38 with `register_re1_socket` /
/// `register_re2_server_socket` / `register_socket_channel_real` /
/// `register_phase52_server_socket_factory` — real, pre-existing drift this
/// scanner was simply blind to, not new drift introduced by the fix. Also
/// absorbed in the same run: 5 rows for `register_http_response` /
/// `register_re5_http_client` sharing `HttpResponse.{uri,request,version,
/// previousResponse,sslSession}` — a real, separately fixed and already-
/// documented drift (the shipping body now answers these; the synthetic-only
/// one is inert outside `--features synthetic-jdk`), unrelated to the scanner
/// repair above and simply never taken by a previous re-take.
///
/// **1313 -> 1316, 2026-09-21.** Three new triples on `java/net/MulticastSocket`
/// — `isClosed()Z`, `joinGroup(Ljava/net/InetAddress;)V`,
/// `leaveGroup(Ljava/net/InetAddress;)V` — drift between the synthetic-only
/// `register_p72_datagram` (`native-builtins/src/phases_late/net_channels.rs`)
/// and the shipping `register_multicast_socket_overrides`
/// (`native-io/src/net.rs`). Not new drift this pass introduced: the shipping
/// body's own comment already explains it — the real bytecode for these three
/// methods calls `this.delegate()`, which this VM never populates, so
/// `register_p72_datagram`'s registrations are measured (a debug print inside
/// that native never fired) to never win ordinary dispatch for
/// `MulticastSocket`; only the shipping registrar, which runs after phase-72
/// and is documented to win for the signatures it implements, reliably serves
/// these triples. Both registrations stay: `register_p72_datagram` still owns
/// the other `MulticastSocket` triples in this table for classes/modes it does
/// serve, and this gate counts registrations, not which one dispatch reaches.
const BASELINE_TOTAL_DRIFT: usize = 1316;
...
/// requires a surviving registrant, and these triples have none.
///
/// **1409 -> 1411, 2026-09-17.** Same account as [`BASELINE_TOTAL_DRIFT`]'s
/// 2026-09-17 entry: `register_bigdecimal_arithmetic_overrides` gaining
/// `BigDecimal.compareTo`/`BigDecimal.divide(BigDecimal)` against the existing
/// `register_bigdecimal_natives` registrations of the same two triples.
///
/// **1409 -> 1406, 2026-09-17 (origin/dev, independent).** Same three-row move
/// as [`BASELINE_TOTAL_DRIFT`]'s matching entry: `register_biginteger_natives`
/// registered `divide`/`mod`/`remainder` as one pass each (no extra synthetic-
/// only pass shared them), so the pair count and the distinct-triple count
/// both drop by exactly three.
///
/// **Reconciled 2026-09-17.** Same reconciliation as [`BASELINE_TOTAL_DRIFT`]:
/// both changes apply to the merged tree, so BigInteger's -3 against this
/// branch's 1411 lands the merged tree at 1408.
///
/// **Update, later on 2026-09-17: the BigDecimal body defect is FIXED; the rows
/// stay.** Both synthetic bodies now parse their value string exactly
/// (`bd_parse_decimal_str`) and run through the same cores as the shipping ones
...
/// `docs/internal/jdk-only/synthetic-bigdecimal-comparison-keeps-the-f64-body-20260917.md`.
///
/// **Re-taken 2026-09-21, 1409 -> 1454, +45.** Same re-take as
/// [`BASELINE_TOTAL_DRIFT`]'s matching entry.
///
/// **1454 -> 1457, 2026-09-21 (later the same day).** Same MulticastSocket
/// account as [`BASELINE_TOTAL_DRIFT`]'s matching entry: three new triples,
/// each shared by exactly one synthetic-only pass and one shipping pass, so
/// the pair count moves by the same +3 as the distinct-triple count.
const BASELINE_TOTAL_PAIRS: usize = 1457;
...
/// `docs/internal/jdk-only/G54-1-…-20260817.md` §2.
///
...
const FIXED_NOT_DRIFTING: &[(&str, &str, &str)] = &[
    ...
    // **BigInteger `divide`/`mod`/`remainder` (3), 2026-09-17.** The synthetic
    // twin in `math_bignum.rs`'s `register_biginteger_natives` was deleted:
    // it ran LAST in synthetic-JDK mode (`register_builtins` ->
    // `register_essential_natives` THEN `register_synthetic_overrides`), so
    // it always won the slot there over the word-based
    // `register_p71_biginteger_extras` twin in `phases_late.rs` — and it was
    // not merely slower, it registered the SAME callback for `mod` and
    // `remainder`, so `remainder()` on mixed-sign operands answered `mod()`'s
    // always-non-negative result instead of following the dividend's sign.
    // `register_p71_biginteger_extras` is the sole remaining registrar for
    // all three and runs in every mode (`register_essential_natives_with_shims`),
    // so the fast, correct body now serves both modes instead of the slow,
    // partly-wrong one owning synthetic-JDK mode by registration order alone.
    (
...
const DRIFT_TRIPLES: &[(&str, &[(&str, &str, &str)])] = &[
    ...
    (
        ...
        &[
            ...
            ("java/net/http/HttpResponse", "previousResponse", "()Ljava/util/Optional;"),
            ("java/net/http/HttpResponse", "request", "()Ljava/net/http/HttpRequest;"),
            ("java/net/http/HttpResponse", "sslSession", "()Ljava/util/Optional;"),
            ("java/net/http/HttpResponse", "statusCode", "()I"),
            ("java/net/http/HttpResponse", "uri", "()Ljava/net/URI;"),
            ("java/net/http/HttpResponse", "version", "()Ljava/net/http/HttpClient$Version;"),
        ],
    ),
    (
    ...
    (
        ...
        &[
            ("java/lang/invoke/MutableCallSite", "setTarget", "(Ljava/lang/invoke/MethodHandle;)V"),
            ("java/lang/invoke/VolatileCallSite", "setTarget", "(Ljava/lang/invoke/MethodHandle;)V"),
        ],
    ),
    (
        "register_p60_flow",
    ...
    (
        ...
        &[
            ("java/lang/foreign/FunctionDescriptor", "argumentLayouts", "()Ljava/util/List;"),
    ...
    pub fn rs_files(dir: &std::path::Path, out: &mut Vec<std::path::PathBuf>) {
        ...
        for p in entries {
            ...
            if p.is_dir() {
                ...
                // actually fired (G54-1 §3.1, 2026-08-17): both are untracked,
                // created and deleted by whichever lanes happen to be
                // running, and once held a stray copy of `phases_late.rs`
                // that injected hundreds of spurious shipping-root
                // references. `registrar_reachability.rs` already excludes
                // both; this scanner did not.
                if matches!(
                    name.as_str(),
                    ".git"
                        | "target"
            ...
            {
                // G54-1 NOMINATION 2: the directory-segment skip above only
                // catches test DIRECTORIES. A test module that lives in a
                // FILE — `vm/src/vm/tests.rs`, `#[cfg(all(test, feature =
                // "synthetic-jdk"))] mod tests;` at `vm/src/vm.rs` — walked
                // straight through and contributed thousands of pass-name
                // references to the shipping roots: doubly not a shipping
                // call site, since it is both a test and compiled only under
                // the feature this gate exists to say the shipping binary
                // does not have. `registrar_reachability.rs` already excludes
                // these by filename; this scanner did not.
                out.push(p);
            }
        }
    }
...
fn build_analysis() -> Analysis {
    ...
    // Module-level references (a pass named in a `static` table) count as
    // non-pass references too. A plain `use path::register_foo;` does NOT:
    // it is not a call, and the genuine call sites it enables are already
    // counted by the in-function scan above, so treating it as a shipping
    // root manufactures false drift (G54-1 NOMINATION 1: five passes,
    // reachable only from `register_synthetic_overrides`, were wrongly
    // excluded from the synthetic-only set this way). A `pub use` is kept —
    // it genuinely re-exports the name to other crates, which IS a real
    // reference this scan should not blind itself to.
    for (fi, f) in files.iter().enumerate() {
        ...
        for (a, b) in segments {
            ...
            while i < b {
                ...
                // Is this occurrence inside a plain (non-`pub`) `use` item?
                // Find the start of the enclosing statement — the character
                // after the nearest preceding `;` in this segment, or the
                // segment start if there is none — and check whether it
                // begins with `use` (not `pub use`).
                let stmt_start = {
                    let mut p = s;
                    while p > a && t[p - 1] != b';' {
                        p -= 1;
                ...
                let mut ps = stmt_start;
                while ps < s && (t[ps] as char).is_whitespace() {
                    ps += 1;
                }
                let rest = &t[ps..s.min(t.len())];
                let is_plain_use = rest.starts_with(b"use ")
                    || rest.starts_with(b"use\t")
                    || rest.starts_with(b"use\n")
                    || rest.starts_with(b"use\r");
                if is_plain_use {
                    continue;
                }
                let id = String::from_utf8_lossy(&t[s..i]).into_owned();
...
///
/// **The first six rows this table used to carry are GONE, not merely fixed
/// in place — see the retirement note at the end of this comment block.**
/// `KNOWN_POPULATION_DIVERGENCE` now carries only the one row that is
...
// G54-1 NOMINATION 1 & 2, RETIRED: this scanner's `rs_files` no longer treats
// a plain `use` import or `vm/src/vm/tests.rs` as a shipping root (see the
// module-level scan above and `rs_files`'s file-name filter), so the six
// names this table used to carry for that reason —
// `register_p60_callsite`, `register_p60_record`,
// `register_p65_method_handles_extra`, `register_phase52_string_buffer`,
// `register_phase53_record`, `register_synthetic_socket_stubs` — now agree
// with `registrar_reachability.rs` and were removed by the same re-take that
// moved `DRIFT_TRIPLES`, `BASELINE_TOTAL_DRIFT` and `BASELINE_TOTAL_PAIRS`.
