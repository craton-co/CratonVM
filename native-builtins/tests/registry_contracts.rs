...
",
    ...
    assert!(
        ...
        "{BODY} no longer calls {PUBLISH_SAVED}, so VM.getSavedProperty throws \
         IllegalStateException(\"Not yet initialized\") for the first class \
         whose <clinit> reads a saved property."
    );
}

/// BigInteger divide, mod, and remainder must be registered exclusively by
/// `phases_late::register_p71_biginteger_extras` as `Bridge` natives, and
/// never shadowed by `math_bignum::register_biginteger_natives` in synthetic
/// mode (which previously registered a decimal-string twin and aliased remainder to mod).
/// See `docs/internal/jdk-only/biginteger-divide-mod-remainder-shadow-confined-to-synthetic-mode-20260917-FIXED-20260917.md`.
#[test]
fn biginteger_divide_mod_remainder_owned_by_phases_late_in_all_modes() {
    let mut reg = NativeMethodRegistry::new();
    cratonvm_native_builtins::register_essential_natives(&mut reg);
    #[cfg(feature = "synthetic-jdk")]
    cratonvm_native_builtins::register_synthetic_overrides(&mut reg);

    for method in ["divide", "mod", "remainder"] {
        // The winning registration (owns_slot == true) must be from phases_late.
        let winner = reg
            .census()
            .into_iter()
            .find(|e| e.class == "java/math/BigInteger" && e.name == method && e.owns_slot)
            .unwrap_or_else(|| panic!("missing winning registration for BigInteger.{method}"));

        // Demoted from Bridge to SyntheticStub because the triple is in
        // RETIRED_SHADOW_L2_TRIPLES (retired shadow policy).
        assert_eq!(winner.kind, NativeKind::SyntheticStub);
        let registered_by = winner.registered_by.as_deref().unwrap_or("");
        assert!(
            registered_by.contains("phases_late"),
            "BigInteger.{method} must be owned by phases_late, but was registered by {registered_by}"
        );

        // math_bignum must NOT register BigInteger.{method} at all.
        let math_bignum_entries: Vec<_> = reg
            .census()
            .into_iter()
            .filter(|e| {
                e.class == "java/math/BigInteger"
                    && e.name == method
                    && e.registered_by
                        .as_deref()
                        .unwrap_or("")
                        .contains("math_bignum")
            })
            .collect();
        assert!(
            math_bignum_entries.is_empty(),
            "math_bignum must not register BigInteger.{method}, found: {math_bignum_entries:?}"
        );
    }
}
