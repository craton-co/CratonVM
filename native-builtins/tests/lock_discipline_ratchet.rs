...
/// figure.
///
/// Lowered 428 -> 427 on 2026-09-17. `dev` was red at 429: `jar_manifest`'s new
/// `ARCHIVE_ORDER` (the bounded jar-archive cache's FIFO eviction queue) and
/// `net_phase_e`'s new `ServerState::conns` (the embedded HttpServer's
/// kept-alive connection table) each landed as a raw `Mutex` with no
/// same-change re-freeze. Both are leaf caches — neither is ever held across a
/// `NativeContext` callback — so both convert to `OrderedPlMutex` at
/// `LockLevel::Scratch`, paying back the +2 regression. That lands one BELOW
/// the frozen figure rather than back at it, because `dev` also carries an
/// unbooked conversion of its own: `classloader.rs`'s `cached_class_path_for_paths`
/// converted its `CACHE` to `OrderedPlMutex` at `LockLevel::Scratch` in
/// b58670306 without lowering this baseline, so that win was never collected.
/// This change collects it.
///
/// Raised again on the same day, merging THIS `dev` state into
/// `jdk-lanes-windowspath-typeroot-20260917` (a `java/nio/file/` lane, no
/// standing to adjudicate locks in unrelated crates): measured 427 after the
/// merge, matching `dev`'s own freeze above exactly — no net delta from
/// combining this branch (whose own diff adds no raw lock) with `dev`'s 140
/// commits since this worktree branched. Recorded rather than left silent,
/// same as every entry above, even though the number does not move.
const BASELINE_RAW_LOCKS: usize = 427;
