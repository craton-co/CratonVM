...
pub struct KeyBlockShape {
    /// How long the separate bulk-MAC secret is, for classic MAC-then-encrypt
    /// suites (e.g. CBC-mode) that don't derive their integrity key from the
    /// AEAD key itself. `0` for AEAD suites (GCM, ChaCha20-Poly1305), which
    /// have no such secret -- this is the case rustls originally shipped
    /// (see `tls12::ConnectionSecrets::make_cipher_pair`'s "we don't
    /// implement any ciphersuites with nonzero mac_key_len" comment prior to
    /// T-CBC.1). `client_write_MAC_secret`/`server_write_MAC_secret`
...
impl AeadKey {
    ...
    ///
    /// T-CBC.1: bumped from 32 to 96 so a CBC `Tls12AeadAlgorithm` can pack a
    /// separate MAC secret ahead of the encryption key into one `AeadKey`
    /// (up to HMAC-SHA384's 48-byte key + AES-256's 32-byte key = 80,
    /// rounded up for headroom) -- see `KeyBlockShape::mac_key_len` and
    /// `tls12::ConnectionSecrets::make_cipher_pair`.
    pub(crate) const MAX_LEN: usize = 96;
}
