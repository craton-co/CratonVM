...
impl ConnectionSecrets {
    ...
    pub(crate) fn make_cipher_pair(&self, side: Side) -> MessageCipherPair {
        ...
        //
        // T-CBC.1: this used to assume mac_key_len is always 0 (true for the
        // AEAD-only suites rustls originally shipped). Classic CBC+HMAC
        // suites need a separate MAC secret ahead of the encryption key, in
        // client-mac/server-mac/client-key/server-key/client-iv/server-iv
        // order per RFC5246 A.6 -- so we slice that out first, then
        // concatenate each side's mac+key into the single `AeadKey` the
        // `Tls12AeadAlgorithm` factory methods accept. For mac_key_len == 0
        // (every pre-existing suite) the concatenation is a no-op (empty +
        // key == key), so this is fully backward compatible.
        let key_block = self.make_key_block();
        let shape = self.suite.aead_alg.key_block_shape();

        let (client_write_mac, key_block) = key_block.split_at(shape.mac_key_len);
        let (server_write_mac, key_block) = key_block.split_at(shape.mac_key_len);
        ...
        let (write_mac, write_key, write_iv, read_mac, read_key, read_iv) = match side {
            Side::Client => (
                client_write_mac,
                client_write_key,
                client_write_iv,
        ...
        let mut read_key_material = Vec::with_capacity(read_mac.len() + read_key.len());
        read_key_material.extend_from_slice(read_mac);
        read_key_material.extend_from_slice(read_key);

        let mut write_key_material = Vec::with_capacity(write_mac.len() + write_key.len());
        write_key_material.extend_from_slice(write_mac);
        write_key_material.extend_from_slice(write_key);

        (
            self.suite
                .aead_alg
                .decrypter(AeadKey::new(&read_key_material), read_iv),
            self.suite
                .aead_alg
                .encrypter(AeadKey::new(&write_key_material), write_iv, extra),
        )
    }
    ...
    fn make_key_block(&self) -> Vec<u8> {
        ...
        let len =
            (shape.mac_key_len + shape.enc_key_len + shape.fixed_iv_len) * 2 + shape.explicit_nonce_len;
    ...
    pub(crate) fn extract_secrets(&self, side: Side) -> Result<PartiallyExtractedSecrets, Error> {
        ...
        // T-CBC.1: skip the (possibly nonzero) MAC secrets first -- see
        // `make_cipher_pair`. Key export for CBC suites isn't implemented
        // (their `extract_keys` returns `UnsupportedOperationError`), but the
        // offsets must still stay aligned for the AEAD suites that are.
        let (_client_mac, key_block) = key_block.split_at(shape.mac_key_len);
        let (_server_mac, key_block) = key_block.split_at(shape.mac_key_len);
