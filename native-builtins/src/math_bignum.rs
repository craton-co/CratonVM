...
fn bd_round_to_precision(
    ...
    // exactly, never anything past it.
...
fn bd_mc_precision_and_mode(ctx: &dyn NativeContext, mc: ObjectRef) -> (i32, i32) {
    let precision = match ctx.get_field(mc, 0) {
        Value::Int(p) => p,
        _ => 0,
    };
    let mode = match ctx.get_field(mc, 1) {
        Value::Object(Some(rm)) => match ctx.get_field(rm, 0) {
            Value::Int(m) => m,
            _ => BD_ROUND_HALF_UP,
        },
        _ => BD_ROUND_HALF_UP,
...
fn native_bd_compare_to_exact(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let cmp = bd_compare_unscaled(&ua, sa, &ub, sb);
...
fn native_bd_divide_exact(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let (ua, sa) = bd_unscaled_bigint(ctx, this);
    let (ub, sb) = bd_unscaled_bigint(ctx, other);
    let (q, scale) = bd_exact_divide_core(&ua, sa, &ub, sb)?;
...
/// `docs/jdk-only/F31-1-three-roads-out-of-one-scale-and-the-zero-operand-that-is-exempt-20260813.md`
