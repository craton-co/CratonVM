...
pub fn context_names_bind_info_for(absolute: &str) -> Result<BindInfo, String> {
    ...
    let mut name = if let Some(rest) = trimmed.strip_prefix("java:") {
        rest
    } else if trimmed.starts_with("jboss")
    ...
    if name.starts_with("/exported/") {
        name = &name[1..];
    }

    let (parent_context_service_name, bind_name) =
        if let Some(rest) = name.strip_prefix("jboss/exported/") {
            (ServiceName::of(["java", "jboss", "exported"]), rest)
        } else if let Some(rest) = name.strip_prefix("jboss/") {
            (ServiceName::of(["java", "jboss"]), rest)
        } else if let Some(rest) = name.strip_prefix("global/") {
            (ServiceName::of(["java", "global"]), rest)
        } else if let Some(rest) = name.strip_prefix('/') {
            (java_context_service_name(), rest)
        } else {
            return Err(format!("illegal java: context in `{trimmed}`"));
        };
...
const BIND_INFO_FIELD_PARENT: usize = 0;
const BIND_INFO_FIELD_BINDER: usize = 1;
const BIND_INFO_FIELD_BINDING_NAME: usize = 2;
const BIND_INFO_FIELD_ABSOLUTE_NAME: usize = 3;
...
fn native_context_names_bind_info_for(
    ...
    let parent = crate::jboss_msc::alloc_java_service_name(ctx, &info.parent_context_service_name);
    let binder = crate::jboss_msc::alloc_java_service_name(ctx, &info.binder_service_name);
    let binding_s = ctx.create_string(info.binding_name.as_ref());
    let absolute_s = ctx.create_string(info.absolute_name.as_ref());
    ctx.set_field(obj, BIND_INFO_FIELD_PARENT, Value::Object(Some(parent)));
    ctx.set_field(obj, BIND_INFO_FIELD_BINDER, Value::Object(Some(binder)));
    ctx.set_field(
        obj,
