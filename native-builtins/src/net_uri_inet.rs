...
// G14-1 N8 — EIGHT DEAD BODIES DELETED (2026-09-20).
//
// `native_uri_init_3` / `_4` / `_5` / `_7`, `native_uri_create`,
// `native_uri_get_scheme_specific_part`, `uri_resolve_path` and
// `uri_normalize_path` lived here and were NEVER REGISTERED AND NEVER
// CALLED — `--dump-native-registry` listed exactly one `java/net/URI.<init>`
// row, the `(Ljava/lang/String;)V` one from `lib.rs`, and `create` is owned by
// `net_phase_e.rs`. The multi-argument constructors are served by REAL JDK
// BYTECODE and every accessor on the objects they build is byte-identical to
// HotSpot, so there was nothing for these to do.
//
// They are deleted rather than left because this file is the one commit
// `8c72d23ca` named as "a fix that landed in dead code": a body that looks
// like the implementation, is reachable by grep, and cannot run is a trap for
// the next reader with a measured row to fix. `#![allow(dead_code)]` on the
// crate root is why the compiler never said so.
//
// `uri_resolve_path`/`uri_normalize_path` went with them: they were called
// only by each other, and `net_phase_e::uri_resolve_ref` /
// `uri_remove_dot_segments` are the bodies `resolve`/`normalize` actually
// reach.
...
pub(crate) fn uri_ipv6_authority_fail(s: &str) -> Option<UriParseFail> {
    let b = s.as_bytes();
    let (open_auth, auth_end) = uri_authority_span(s)?;
...
// ---------------------------------------------------------------------------
// G14-1 §6 — `Parser.parseAuthority`, transcribed
// ---------------------------------------------------------------------------

/// `[start, end)` of the `//`-introduced authority, or `None` when there is
/// none. The authority stops at the first `/`, `?` or `#`, which is the JDK's
/// `scan(p, n, "", "/?#")`.
fn uri_authority_span(s: &str) -> Option<(usize, usize)> {
    let start = s.find("://")? + 3;
    let end = s[start..]
        .find(['/', '?', '#'])
        .map(|r| start + r)
...
/// "*" | "(" | ")"`.
fn uri_is_unreserved(c: u8) -> bool {
    c.is_ascii_alphanumeric() || b"-_.!~*'()".contains(&c)
}

/// `reg_name = 1*( unreserved | escaped | "$" | "," | ";" | ":" | "@" | "&" |
/// "=" | "+" )`.
fn uri_is_reg_name(c: u8) -> bool {
    uri_is_unreserved(c) || b"$,;:@&=+".contains(&c)
}

/// `userinfo = *( unreserved | escaped | ";" | ":" | "&" | "=" | "+" | "$" |
/// "," )`. Differs from [`uri_is_reg_name`] by exactly one character, `@` —
/// which is why the JDK's own `checkChars(p, q, L_USERINFO, …)` can only ever
/// fire on a `[` or a `]`, the two characters `L_SERVER` admits and neither of
/// these does.
fn uri_is_userinfo(c: u8) -> bool {
    uri_is_unreserved(c) || b";:&=+$,".contains(&c)
}

/// `L_SERVER = L_USERINFO | L_ALPHANUM | L_DASH | ".:@[]"`, and
/// `L_SERVER_PERCENT = L_SERVER | "%"` — the set `parseAuthority` uses when the
/// authority contains a `]`, where a `%` is a scope-id separator rather than
/// the start of an escape pair.
fn uri_is_server(c: u8, percent: bool) -> bool {
    uri_is_userinfo(c) || b".:@[]".contains(&c) || (percent && c == b'%')
}

/// The index of the first byte in `[start, end)` that `class_of` rejects, with
/// `%XX` skipped as one legal ESCAPED unit.
///
/// The escape is not a special case bolted on: `L_ESCAPED` is bit zero of
/// every one of these masks, and the JDK's `scan` consults it through
/// `scanEscape`. A malformed escape has already been refused by
/// [`uri_first_char_fault`] before any of this runs, so a `%` reaching here is
/// always well formed.
fn uri_scan_set(
    b: &[u8],
    start: usize,
    end: usize,
    class_of: impl Fn(u8) -> bool,
) -> Option<usize> {
    let mut i = start;
    while i < end {
        let c = b[i];
        if c == b'%' && i + 2 < end + 1 && i + 2 < b.len() {
...
/// The `Illegal character in authority` index — `qreg`, NOT the offending
/// character's own position.
///
/// MEASURED on HotSpot 25.0.3+9-LTS, and it is the row that makes the
/// difference visible:
///
/// ```text
///   http://ho st/       Illegal character in authority at index  9
///   http://[::1]: /p    Illegal character in authority at index  7
///   http://u@[::1]: /p  Illegal character in authority at index  9
/// ```
///
/// The first looks like "the offending character", and the second is the space
/// at 13 while the oracle answers 7. `fail("Illegal character in authority",
/// serverChars ? q : qreg)` settles it: `qreg` is where the REG_NAME scan
/// stopped, so it is the space in the first row and the `[` in the other two.
/// `serverChars` is false whenever this branch is reached from a character
/// fault (an authority that scans clean as a server authority does not reach
/// `fail` here at all), so `qreg` is the whole answer.
pub(crate) fn uri_authority_illegal_index(s: &str, fallback: usize) -> usize {
    let Some((start, end)) = uri_authority_span(s) else {
        return fallback;
    };
    if fallback < start || fallback >= end {
        return fallback;
    }
    uri_scan_set(s.as_bytes(), start, end, uri_is_reg_name).unwrap_or(fallback)
}

/// Does `parseAuthority` demote this authority to registry-based rather than
/// refuse it?
///
/// **This is the whole blast-radius answer for G14-1 §6's "it did not police
/// non-bracketed hostnames".** `L_REG_NAME` and `L_SERVER` differ by exactly
/// two characters, `[` and `]` (plus a bare `%` when a `]` is present), so
/// `regChars` implies `serverChars` and the JDK's ladder collapses to:
///
/// * the authority scans clean as a reg-name  ⇒  **never refused**; a failed
///   server parse silently demotes it (`host`/`userInfo`/`port` become
///   `null`/`null`/`-1`), which is why `http://h:-5/p`, `http://h_x/p` and
///   `http://a.9b/p` are all legal URIs;
/// * it does not, and does not scan clean as a server authority either ⇒
///   `Illegal character in authority` at [`uri_authority_illegal_index`];
/// * it does not, but IS a clean server authority ⇒ `parseServer`'s own
///   exception is re-thrown, and that is the only door the hostname, user-info
///   and IPv6 refusals come through.
///
/// So policing the server parse costs NOTHING for an ordinary reg-name
/// authority: it cannot reach the refusal. MEASURED — every one of
/// `http://h_x/p`, `http://h~x/p`, `http://h!x/p`, `http://h$x/p`,
/// `http://h&x/p`, `http://h'x/p`, `http://h(x)/p`, `http://h*x/p`,
/// `http://h+x/p`, `http://h,x/p`, `http://h;x/p`, `http://h=x/p`,
/// `http://h%41x/p`, `http://h:80x/p`, `http://-h/p`, `http://h-/p`,
/// `http://a..b/p`, `http://.a/p` and `http://a.9b/p` is accepted by the
/// oracle with a null host, and every one of them scans clean as a reg-name.
fn uri_authority_is_reg_name(s: &str, start: usize, end: usize) -> bool {
    uri_scan_set(s.as_bytes(), start, end, uri_is_reg_name).is_none()
}

/// The `checkChars(p, q, L_USERINFO, H_USERINFO, "user info")` refusal.
///
/// MEASURED on HotSpot 25.0.3+9-LTS:
///
/// ```text
///   http://[::1]@h/p       Illegal character in user info at index 7
///   http://[::1]@h:80/p    Illegal character in user info at index 7
///   http://u[v]@h/p        Illegal character in user info at index 8
///   http://u]@h/p          Illegal character in user info at index 8
///   http://[::1]@[::2]/p   Illegal character in user info at index 7
/// ```
///
/// The index is the first offending BYTE, not the user info's start — the two
/// coincide in the first row and do not in the third, which is why one row
/// would have pinned the wrong rule.
fn uri_user_info_fail(s: &str, start: usize, end: usize) -> Option<UriParseFail> {
    // `q = scan(p, n, "/?#", "@")` — the FIRST `@`, and the authority contains
    // no `/`, `?` or `#` by construction.
    let at = s[start..end].find('@').map(|i| start + i)?;
    uri_scan_set(s.as_bytes(), start, at, uri_is_userinfo)
        .map(|i| UriParseFail::at("Illegal character in user info", i))
}

/// `Parser.parseHostname`, refusal half.
///
/// The accessor half is `net_phase_e::uri_parse_hostname`, which answers
/// `None` for the same inputs; this one says WHY and WHERE, which that one
/// cannot carry. MEASURED on HotSpot 25.0.3+9-LTS:
///
/// ```text
///   http://a[b]/p      Illegal character in hostname at index  8
///   http://a]b/p       Illegal character in hostname at index  8
///   http://a[/p        Illegal character in hostname at index  8
///   http://ab]/p       Illegal character in hostname at index  9
///   http://]/p         Illegal character in hostname at index  7
///   http://x[y]z/p     Illegal character in hostname at index  8
///   http://u@a[b]/p    Illegal character in hostname at index 10
///   http://a[b]:80/p   Illegal character in hostname at index  8
/// ```
///
/// The last row is the one that says the scan stops at the bracket and not at
/// the colon: a `:` ENDS the hostname legally, so `p < n && !at(p, ':')` is the
/// test and `http://a[b]:80/p` still reports the `[`.
fn uri_hostname_fail(s: &str, start: usize, end: usize) -> Option<UriParseFail> {
    let b = s.as_bytes();
    let mut p = start;
    let mut last_label: Option<usize> = None;
    loop {
        // domainlabel = alphanum [ *( alphanum | "-" ) alphanum ]
        let q = uri_scan_while(b, p, end, |c| c.is_ascii_alphanumeric());
        if q <= p {
            break;
        }
        last_label = Some(p);
        p = q;
        let q = uri_scan_while(b, p, end, |c| c.is_ascii_alphanumeric() || c == b'-');
        if q > p {
            // A label may not END in `-`, and the JDK reports the dash itself.
            if b[q - 1] == b'-' {
                return Some(UriParseFail::at("Illegal character in hostname", q - 1));
            }
            p = q;
        }
        if p >= end || b[p] != b'.' {
    ...
    // A `:` legally ends the hostname (the port follows); anything else here is
    // a character the hostname grammar has no place for. This test comes FIRST
    // — before the "did we parse any label at all" one — which is why
    // `http://]/p` reports index 7 and not `Expected hostname`.
    if p < end && b[p] != b':' {
        return Some(UriParseFail::at("Illegal character in hostname", p));
    }
    let Some(l) = last_label else {
        return Some(UriParseFail::at("Expected hostname", start));
    };
    // For a multi-label name the RIGHTMOST label must start with a letter.
    if l > start && !b[l].is_ascii_alphabetic() {
        return Some(UriParseFail::at("Illegal character in hostname", l));
    }
    // The port, which only a non-bracketed host reaches through here; the
    // bracketed path has its own copy in `uri_ipv6_authority_fail`.
    if p < end && b[p] == b':' {
        p += 1;
        if end > p {
            if let Some(bad) = (p..end).find(|i| !b[*i].is_ascii_digit()) {
                return Some(UriParseFail::at("Illegal character in port number", bad));
            }
            if s[p..end].parse::<i32>().is_err() {
                return Some(UriParseFail::at("Malformed port number", p));
            }
        }
        p = end;
...
/// `scan(p, n, lowMask, highMask)` for a predicate: the first index at or after
/// `p` whose byte the predicate rejects, clamped to `end`.
fn uri_scan_while(b: &[u8], start: usize, end: usize, ok: impl Fn(u8) -> bool) -> usize {
    let mut i = start;
    while i < end && ok(b[i]) {
        i += 1;
    }
...
/// `parseAuthority`'s decision, whole: demote, refuse the authority, or hand
/// back `parseServer`'s own exception.
///
/// The one entry point [`uri_parse_fail`] calls. See
/// [`uri_authority_is_reg_name`] for why this cannot fire on an ordinary
/// reg-name authority, and [`uri_ipv6_authority_fail`] for the thirty
/// bracketed-host rows it delegates.
pub(crate) fn uri_authority_fail(s: &str) -> Option<UriParseFail> {
    let (start, end) = uri_authority_span(s)?;
    if start >= end {
        return None;
    }
    // `regChars` implies `serverChars`, so a reg-name-clean authority is never
    // refused — a failed server parse demotes it instead.
    if uri_authority_is_reg_name(s, start, end) {
        return None;
    }
    let percent = s[start..end].contains(']');
    if uri_scan_set(s.as_bytes(), start, end, move |c| uri_is_server(c, percent)).is_some() {
        // Neither set accepts it: `fail("Illegal character in authority", qreg)`.
        return Some(UriParseFail::at(
            "Illegal character in authority",
            uri_scan_set(s.as_bytes(), start, end, uri_is_reg_name).unwrap_or(start),
        ));
    }
    // A clean server authority that is not a legal reg-name: `parseServer`'s
    // exception is re-thrown rather than swallowed.
    if let Some(fail) = uri_user_info_fail(s, start, end) {
        return Some(fail);
    }
    let host_start = s[start..end]
        .find('@')
        .map(|i| start + i + 1)
        .unwrap_or(start);
    if s.as_bytes().get(host_start) == Some(&b'[') {
        return uri_ipv6_authority_fail(s);
    }
    uri_hostname_fail(s, host_start, end)
}
...
pub(crate) fn uri_parse_fail(s: &str) -> Option<UriParseFail> {
    ...
    match illegal {
        ...
        Some((pos, UriCharFault::Illegal)) => {
            let reason = uri_illegal_component(s, pos);
            // G14-1 §6: THE AUTHORITY'S INDEX IS `qreg`, NOT THE CHARACTER'S.
            //
            // Every other component reports where the scan stopped, and the
            // authority looks like it does too until a bracketed one is
            // measured. `fail("Illegal character in authority", serverChars ?
            // q : qreg)` — and `serverChars` is false on every input that
            // reaches here, so the index is always where the REG_NAME scan
            // stopped. MEASURED: `http://[::1]: /p` is index **7** on the
            // oracle (the `[`) and was 13 here (the space).
            let pos = if strict && reason == "Illegal character in authority" {
                uri_authority_illegal_index(s, pos)
            } else {
                pos
            };
            return Some(UriParseFail::at(reason, pos));
    ...
    // `parseServer`. See `uri_authority_fail` for the ladder and
    // `uri_ipv6_authority_fail` for the thirty bracketed-host rows.
    if strict {
        if let Some(fail) = uri_authority_fail(s) {
            return Some(fail);
        }
    }
...
pub(crate) fn uri_closing_bracket_fail_index(s: &str) -> Option<usize> {
    let (open, auth_end) = uri_authority_span(s)?;
    // THE `[` MUST BE THE HOST'S FIRST BYTE, not merely somewhere in the
    // authority. `parseServer` reaches `failExpecting("closing bracket for
    // IPv6 address", q)` only under `if (at(p, n, '['))`, where `p` is the
    // host's start — after the user info, if there is one. A `[` anywhere else
    // is an ordinary illegal character in whatever component it lands in.
    // MEASURED on HotSpot 25.0.3+9-LTS, and the two rows disagree:
    //
    //   http://[b/p   Expected closing bracket for IPv6 address at index 9
    //   http://a[/p   Illegal character in hostname             at index 8
    //
    // This used to find the FIRST `[` in the authority, so the second row
    // answered the first row's reason.
    let host_start = s[open..auth_end]
        .find('@')
        .map(|i| open + i + 1)
        .unwrap_or(open);
    if s.as_bytes().get(host_start) != Some(&b'[') {
...
// G14-1 N8: the four multi-argument `java.net.URI` constructors and
// `native_uri_create` stood here, unregistered and uncalled, until they were
// deleted on 2026-09-20. The reasoning is at the top of this file, beside the
// other three bodies that went with them.
...
mod new2_net_tests {
    ...
    // -----------------------------------------------------------------
    // G14-1 §6 — `parseAuthority`'s ladder
    // -----------------------------------------------------------------

    #[track_caller]
    fn assert_authority_fail(uri: &str, reason: &str, index: usize) {
        let got = uri_authority_fail(uri).map(|f| (f.reason, f.index));
        assert_eq!(
            got,
            Some((reason, Some(index))),
            "`{uri}` must be refused with `{reason}` at {index}"
        );
    }

    #[test]
    fn a_bracket_outside_a_bracketed_host_is_an_illegal_hostname() {
        // MEASURED on HotSpot 25.0.3+9-LTS. Every row's index is the `[` or
        // `]` itself, EXCEPT `http://ab]/p`, where the label `ab` is consumed
        // first — which is what says the scan runs the hostname grammar
        // rather than looking for a bracket.
        assert_authority_fail("http://a[b]/p", "Illegal character in hostname", 8);
        assert_authority_fail("http://a]b/p", "Illegal character in hostname", 8);
        assert_authority_fail("http://a[/p", "Illegal character in hostname", 8);
        assert_authority_fail("http://ab]/p", "Illegal character in hostname", 9);
        assert_authority_fail("http://]/p", "Illegal character in hostname", 7);
        assert_authority_fail("http://x[y]z/p", "Illegal character in hostname", 8);
        assert_authority_fail("http://u@a[b]/p", "Illegal character in hostname", 10);
        // A `:` legally ENDS a hostname, so the port does not shift the index.
        assert_authority_fail("http://a[b]:80/p", "Illegal character in hostname", 8);
        assert_authority_fail("http://a[b]", "Illegal character in hostname", 8);
        assert_authority_fail("http://a[b]?q", "Illegal character in hostname", 8);
        assert_authority_fail("http://a[b]#f", "Illegal character in hostname", 8);
    }

    #[test]
    fn a_bracket_in_the_user_info_is_its_own_reason() {
        // MEASURED. The index is the first offending BYTE, not the user
        // info's start — rows one and three disagree about that, which is why
        // one row would have pinned the wrong rule.
        assert_authority_fail("http://[::1]@h/p", "Illegal character in user info", 7);
        assert_authority_fail("http://[::1]@h:80/p", "Illegal character in user info", 7);
        assert_authority_fail("http://u[v]@h/p", "Illegal character in user info", 8);
        assert_authority_fail("http://u]@h/p", "Illegal character in user info", 8);
        assert_authority_fail("http://[::1]@[::2]/p", "Illegal character in user info", 7);
    }

    #[test]
    fn a_reg_name_clean_authority_is_never_refused() {
        // THE BLAST-RADIUS TEST. `L_REG_NAME` and `L_SERVER` differ by exactly
        // `[` and `]`, so a failed server parse over any other authority
        // DEMOTES instead of throwing. MEASURED — the oracle accepts every one
        // of these with a null host.
        for uri in [
            "http://h_x/p",
            "http://h~x/p",
            "http://h!x/p",
            ...
            assert_eq!(uri_authority_fail(uri), None, "`{uri}` must not be refused");
        }
    }

    #[test]
    fn the_illegal_authority_index_is_qreg_not_the_character() {
        // `fail("Illegal character in authority", serverChars ? q : qreg)`.
        // MEASURED: the first row's offending character and `qreg` coincide,
        // the rest disagree — 13 versus 7 for the second, which is the row
        // G14-1 §6 left open.
        assert_eq!(uri_authority_illegal_index("http://ho st/", 9), 9);
        assert_eq!(uri_authority_illegal_index("http://[::1]: /p", 13), 7);
        assert_eq!(uri_authority_illegal_index("http://[::1] /p", 12), 7);
        assert_eq!(uri_authority_illegal_index("http://[::1]<x/p", 12), 7);
        assert_eq!(uri_authority_illegal_index("http://u@[::1]: /p", 15), 9);
        assert_eq!(uri_authority_illegal_index("http://h:8 0/", 10), 10);
        assert_eq!(uri_authority_illegal_index("http://u v@h/p", 8), 8);
        // A fault OUTSIDE the authority is handed back untouched — the rule is
        // the authority's, and a path or query keeps reporting its own scan.
        assert_eq!(uri_authority_illegal_index("http://h/a b", 10), 10);
        assert_eq!(uri_authority_illegal_index("/a b", 2), 2);
    }

    #[test]
    fn the_closing_bracket_refusal_needs_the_bracket_at_the_hosts_start() {
        // MEASURED, and the two rows disagree: a `[` that opens the HOST wants
        // a closing bracket, and a `[` anywhere else is just an illegal
        // hostname character.
        assert_eq!(uri_closing_bracket_fail_index("http://[b/p"), Some(9));
        assert_eq!(uri_closing_bracket_fail_index("http://[]/p"), Some(8));
        assert_eq!(uri_closing_bracket_fail_index("http://[::1/p"), Some(11));
        assert_eq!(uri_closing_bracket_fail_index("http://[/"), Some(8));
        assert_eq!(uri_closing_bracket_fail_index("http://u@[/p"), Some(10));
        assert_eq!(uri_closing_bracket_fail_index("http://a[/p"), None);
        assert_eq!(uri_closing_bracket_fail_index("http://a[b]/p"), None);
        assert_eq!(uri_closing_bracket_fail_index("http://[::1]/p"), None);
    }
