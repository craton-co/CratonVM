...
// `cldr_locale_display_name` was REMOVED 2026-09-20 together with its only
// caller, the `java.util.Locale` display-name overrides in
// `locale_bootstrap.rs`. It read a language or region subtag out of the image's
// CLDR `LocaleNames` bundles, which was the right SOURCE; what it could not do
// was compose a display NAME, and the JDK's own `Locale.getDisplayString` /
// `getDisplayName` now do that on this VM in both modes. See the removal note
// at `locale_bootstrap.rs`'s "Display-name overrides: REMOVED" comment for the
// measurement and for the `LocaleNameProvider` SPI arm that made the bytecode
// path answer.
...
fn rb_get_object(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    if !is_synthetic {
        ...
        if this_cname.as_deref() == Some("java/util/PropertyResourceBundle") {
            if let Value::Object(Some(lookup)) = ctx.get_field_by_name(this, "lookup") {
                if let Ok(Some(val @ Value::Object(Some(_)))) =
            ...
            let msg = ctx.create_string(&format!(
                ...
                key_str.as_deref().unwrap_or_default()
            ));
            ctx.set_field_by_name(exc, "detailMessage", Value::Object(Some(msg)));
            let cname = ctx.create_string("java.util.PropertyResourceBundle");
            ctx.set_field_by_name(exc, "className", Value::Object(Some(cname)));
            ctx.set_field_by_name(exc, "key", Value::Object(Some(key)));
...
pub fn register(registry: &mut NativeMethodRegistry) {
    ...
    registry.register(rb, "keySet", "()Ljava/util/Set;", |ctx, args| {
        ...
        if !is_synthetic {
            // Real PropertyResourceBundle keeps its entries in the `lookup` Map
            // field (populated by its constructor from the .properties stream).
            if this_cname.as_deref() == Some("java/util/PropertyResourceBundle") {
                if let Value::Object(Some(lookup)) = ctx.get_field_by_name(this, "lookup") {
                    return cratonvm_native_collections::native_map_key_set_pub(
                        ctx,
                        &[Value::Object(Some(lookup))],
                    );
                }
            ...
            if let Ok(res) = ctx.invoke_special_bytecode_only(
