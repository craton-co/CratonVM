...
pub(crate) fn register_unsafe_wp1_2(registry: &mut NativeMethodRegistry) {
    ...
    // `acquire`/`release` are `VarHandle` ACCESS-MODE names rather than methods
    // here. Four registrations no caller on any supported image could name.
    ...
    // `Z` rather than the witness value and is registered (and retired) apart.
    for name in [
    ...
    // dead one from a single `name`. A per-NAME loop that registers on two
    // classes cannot express a per-CLASS fact, which is why the sun.misc
    // registration is gone rather than filtered inside the loop.
    for name in [
