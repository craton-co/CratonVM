...
fn jre_adapter_get_locale_service_provider(
    ...
    if spi.as_deref() == Some("java.util.spi.LocaleNameProvider") {
        // 2026-09-20: the FOURTH arm, and it is the one the OPEN section of
        // `uri-resolve-folded-a-reference-into-the-host-and-the-locale-display-name-gap-20260826-RETIRED-20260920.md`
        // was waiting on (RETIRED 2026-09-20 by this arm and the deletion below). That page recorded `Locale`'s display accessors
        // answering the CODE where the JDK answers a localized name, said the
        // fix "needs a data source ... the JDK's own `sun.util.locale.provider`
        // bundles routed through", and it is this null that stops them being
        // routed.
        //
        // `Locale.getDisplayScript`/`getDisplayVariant`/`getDisplayLanguage`/
        // `getDisplayCountry` all funnel into `Locale.getDisplayString`, which
        // asks `LocaleServiceProviderPool.getPool(LocaleNameProvider.class)
        // .getLocalizedObject(...)`. The pool reaches every candidate adapter
        // through THIS method; a null means no adapter is ever accepted, the
        // pool answers null, and `getDisplayString`'s own
        // `result != null ? result : code` tail hands the caller the subtag.
        // That tail is why the defect is silent: no exception, no warning, a
        // language tag rendered into a UI where a name belongs.
        //
        // MEASURED, `apps/probes/UulrLocaleDisplaySweep`, HotSpot 25.0.4+7 as
        // the oracle. The arm is needed in COMPATIBLE mode only — strict mode
        // refuses this whole native, so real `JRELocaleProviderAdapter`
        // bytecode runs there and the pool has always worked:
        //
        //   zh-Hans-CN getDisplayScript(ROOT)   HotSpot Simplified
        //                                       strict  Simplified   (bytecode)
        //                                       compat  Hans         <- this arm
        //   en-US-POSIX getDisplayVariant(US)   HotSpot Computer
        //                                       strict  Computer
        //                                       compat  POSIX        <- this arm
        //
        // 17 rows, every one of them a display name answering a code, in the
        // mode that is not the lane's own. Same DEGRADE contract as the three
        // arms above.
        match ctx.invoke_virtual(
            this,
            ...
            Err(e) => {
                tracing::warn!(
                    error = ?e,
                    "getLocaleNameProvider threw; falling back to the legacy \
                     null. Locale display names then come from \
                     Locale.getDisplayString's own tail, which answers the \
                     language/country/script SUBTAG rather than a name."
                );
                return Ok(Some(Value::Object(None)));
            }
        }
...
pub fn register(registry: &mut NativeMethodRegistry) {
    ...
    // Display-name overrides: REMOVED 2026-09-20, and the reason is the whole
    // point of the change.
    //
    // Six registrations lived here -- `getDisplayLanguage`, `getDisplayCountry`
    // and `getDisplayName`, each in a no-arg and a `(Locale)` form -- plus the
    // `display_lookup`/`display_name` helpers behind them. They existed because
    // the JDK's own path answered the language/country CODE, and they replaced
    // it with a direct read of the image's CLDR `LocaleNames` bundles. That was
    // right about where the data lives and wrong about who should compose it.
    //
    // What the override could not do, measured against HotSpot 25.0.4+7 with
    // `apps/probes/UulrLocaleDisplaySweep` (49 differing rows in strict mode,
    // 66 in compatible):
    //
    //   * `Locale.ROOT` as the display locale read as "no locale at all". It
    //     asked `getLanguage()`/`getCountry()` of the argument, got two empty
    //     strings, and the empty language walked `cldr_suffixes("", "")` to the
    //     ROOT bundle alone -- so every `getDisplayLanguage(Locale.ROOT)`
    //     answered the subtag. 33 rows.
    //   * `getDisplayName` was `Language (Country)` and nothing else. The JDK
    //     composes language, SCRIPT, country and VARIANT, joins the qualifiers
    //     with the display locale's own `ListCompositionPattern` (`, ` in
    //     English, U+3001 in Japanese) and formats the result through
    //     `DisplayNamePattern`. `Chinese (Simplified, China)` came back
    //     `Chinese (China)`; `English (United States, Computer)` lost its
    //     variant. 16 rows.
    //   * A null display locale answered `English` where the JDK throws
    //     `NullPointerException`. 2 rows.
    //
    // Each of those is a re-implementation of `Locale.getDisplayString` and
    // `getDisplayName`'s composition, and re-implementing them correctly means
    // reproducing `LocaleResources.getLocaleName`, the CLDR parent chain, two
    // resource patterns and a `MessageFormat` -- all of which are already in
    // the image, already correct, and already what strict mode runs.
    //
    // The reason bytecode could not be left to answer BEFORE this commit is the
    // `LocaleNameProvider` arm added to `jre_adapter_get_locale_service_provider`
    // above: without it `LocaleServiceProviderPool` accepted no adapter, and
    // `Locale.getDisplayString`'s `result != null ? result : code` tail handed
    // back the subtag. That is the defect the override was papering over, one
    // layer too late and for four of the six accessors only -- `getDisplayScript`
    // and `getDisplayVariant` were never overridden and so were WRONG in
    // compatible mode and RIGHT in strict, which is the asymmetry that named
    // the real cause.
    //
    // The H2 case that motivated the override is unaffected and is what this
    // must not regress: `org.h2.value.CompareMode.getName` asks every locale
    // `Collator` offers for its ENGLISH display name and matches the answer
    // against the requested collation, so `SET COLLATION TURKISH` needs
    // `Locale.of("tr").getDisplayLanguage(Locale.ENGLISH)` to be `Turkish` and
    // not `tr`. It is `Turkish` on the bytecode path too -- that is the path
    // HotSpot runs -- and `apps/probes/UulrLocaleDisplaySweep` pins the row.
