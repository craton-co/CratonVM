...
/// Resolve trust roots DIRECTLY from the actual `TrustManager[]` this
/// `SSLContext.init` call received, mirroring
/// `resolved_identity_pem_for_key_manager_array` above for the trust side of
/// the same bug class: the thread-local `PENDING_TM_TRUST_ROOTS` staged by
/// `TrustManagerFactory.init` is only "whichever `init` ran most recently on
/// this thread", which is wrong the moment a caller builds more than one
/// `TrustManagerFactory`/`SSLContext` pair before using either — the
/// `SSLContext.init` that happens to run FIRST drains whatever is pending,
/// regardless of whether it was that call's OWN factory that staged it
/// (`TrustOrderProbe`: `defaulting.init(null, defaultingTmf.getTrustManagers(),
/// null)` running before `trusting.init(...)` steals the roots
/// `trustingTmf.init(trustKs)` staged, leaving the `trusting` context with
/// none and the `defaulting` context with roots it never asked for —
/// P4A-TOMCAT-20260812.md defect 4).
///
/// Every `TrustManager` `tmf_engine_get_trust_managers` (and its
/// `ManagerFactoryParameters` sibling) returns already carries its
/// originating `tm_registry` id via `set_tm_id` — the same
/// identity-hash-keyed mechanism `get_tm_id` reads here — so reading it back
/// off the actual array element is immune to ordering: it names the exact
/// `TrustManagerState` that factory built, not whatever happens to be
/// sitting in a shared slot. Returns `None` when the array is absent/empty or
/// every element resolves to id 0 (a fully custom, non-JDK-default
/// `X509TrustManager` that never went through `register_trust_manager_state`
/// — those are driven through the Java-callback path instead, per
/// `attach_trust_managers_to_ctx`, and have no roots to resolve here), so the
/// caller can fall back to the thread-local staging exactly as before.
pub(crate) fn resolved_trust_roots_for_trust_manager_array(
    ctx: &mut dyn NativeContext,
    tms_arr: Option<ObjectRef>,
) -> Option<(Vec<Vec<u8>>, Option<RevocationConfig>)> {
    let arr = tms_arr?;
    let len = ctx.array_length(arr);
    for i in 0..len {
        if let Value::Object(Some(tm)) = ctx.get_array_element(arr, i) {
            let id = get_tm_id(ctx, tm);
            if id == 0 {
                continue;
            }
            if let Some(state) = tm_registry().read().get(&id).cloned() {
                return Some((state.anchor_ders, state.revocation));
            }
        }
    }
    None
