...
//! 4. `readConfiguration()` — a DELIBERATE no-op: a JUL properties file may
//!    name a `config` class that real `LogManager` instantiates, so parsing a
//!    filesystem-/environment-selected configuration is an arbitrary-code
//!    -execution vector. `readConfiguration(InputStream)` (and
//!    `updateConfiguration(InputStream, Function)`) DO parse and apply, because
//!    there the bytes came from the caller's own Java code.
...
/// `LogManager.readConfiguration()` / `LogManager.updateConfiguration(Function)`
/// — DELIBERATELY a no-op. Do not "fix" this into a real parser.
///
/// Security: we deliberately do NOT parse untrusted logging config — any path
/// that would load a properties file is suppressed. A JUL properties file may
/// name a `config` class, which real `LogManager` INSTANTIATES, so parsing a
/// filesystem- or environment-selected configuration
/// (`java.util.logging.config.file`, `java.util.logging.config.class`) is a
/// direct arbitrary-code-execution vector, not just a parsing concern. The
/// process-wide tracing subscriber already governs effective levels, and the
/// `InputStream` overloads below still honour a stream the caller's own Java
/// code produced.
