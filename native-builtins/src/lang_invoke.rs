...
const DESC_MEMORY_SEGMENT: &str = "Ljava/lang/foreign/MemorySegment;";
...
pub struct SegmentVhShape {
    ...
    /// `b'S'`, `b'I'`, `b'J'`, `b'F'`, `b'D'`, or [`FFM_ADDRESS_CARRIER`] for
    /// `ADDRESS` (whose real carrier is `MemorySegment`, not an integer).
    pub carrier: u8,
    ...
    /// no index coordinate.
    pub stride: i64,
    /// The leaf layout's `byteAlignment()`. HotSpot's `VarHandle`s for a
    /// `MemoryLayout` refuse an access whose resolved offset does not satisfy
    /// this against the segment's own `maxByteAlignment()` — see
    /// `docs/internal/jdk-only/
    /// ffm-varhandle-address-carrier-and-missing-alignment-check-20260921-RETIRED-20260921.md`.
    /// `1` means "never refuses", which is what every byte-carrier layout and
    /// every pre-existing construction site that predates this field means.
    pub alignment: i64,
}
...
/// The layout carrier byte for `ADDRESS` — `ValueLayout.ADDRESS` and
/// `AddressLayout`, whose real carrier is `MemorySegment`.
///
/// Before this existed, `p67_carrier_descriptor_byte` folded `ADDRESS` into
/// the same fallback as any other non-primitive carrier of its width (`b'J'`,
/// since `ADDRESS` is 8 bytes), so `varType()` answered `long` and `get`/`set`
/// moved a raw pointer value with no `MemorySegment` on either end. That is
/// the `varType()` half of
/// `docs/internal/jdk-only/
/// ffm-varhandle-address-carrier-and-missing-alignment-check-20260921-RETIRED-20260921.md`'s
/// §1: fixing the DESCRIPTION alone (answering `MemorySegment` from
/// `varType()` while `get`/`set` still moved a `long`) would make the handle
/// describe itself as something its own accessors do not produce — worse than
/// today's consistent-but-wrong answer. This carrier code is therefore never
/// read by `layout_vh_decode`/`layout_vh_encode` directly (those only ever
/// see it filtered through `layout_vh_get`/`layout_vh_set`, which special-case
/// it into a real `MemorySegment` mint/unwrap around the same raw-pointer
/// bytes every other 8-byte carrier already moves) — see those two functions.
///
/// Not a JVM descriptor byte (`'Z'`/`'B'`/… are all uppercase ASCII letters
/// the classfile format assigns meaning to); deliberately lowercase so it can
/// never collide with one if a ninth primitive-shaped carrier is ever added.
pub(crate) const FFM_ADDRESS_CARRIER: u8 = b'a';
...
fn layout_vh_carrier_desc(carrier: u8) -> &'static str {
    match carrier {
        ...
        b'F' => DESC_FLOAT,
        b'D' => DESC_DOUBLE,
        FFM_ADDRESS_CARRIER => DESC_MEMORY_SEGMENT,
        _ => DESC_INT,
    }
}
...
/// Where an FFM layout `VarHandle` access lands: a real address for a
/// native/off-heap segment, or an array object plus a byte offset into it for
/// a heap segment (`MemorySegment.ofArray(...)`), which has no native
/// pointer at all. Read/write have to branch on this because a heap array is
/// GC-managed — there is no raw pointer to dereference, ever, not even
/// transiently.
enum LayoutVhAddr {
    Native(usize),
    Heap {
        ...
        /// Absolute byte offset into `base`'s data, i.e. `view.start + offset`.
        byte_offset: i64,
    },
}

/// Resolve an FFM layout handle's access into a validated address, and say
...
fn layout_vh_access(
    ...
    shape: SegmentVhShape,
    args: &[Value],
) -> Result<Option<(LayoutVhAddr, usize)>, MethodCallFailed> {
    ...
    let out_of_bounds = || {
        MethodCallFailed::from(RuntimeError::IndexOutOfBoundsException {
            message: Some(format!(
    ...
    if offset < 0 || offset.saturating_add(width) > size {
        return Err(out_of_bounds());
    }
    // `segment_address` returns 0 for a HEAP segment (`MemorySegment.ofArray`
    // etc.) — a heap segment has no native pointer, only a Java array
    // reference, which is not a defect in `segment_address` and must not be
    // read as "this segment is invalid". It used to be, here: every
    // layout-derived VarHandle access on a heap segment was refused with a
    // spurious IndexOutOfBoundsException, where HotSpot supports it fine. See
    // `docs/internal/jdk-only/
    // ffm-varhandle-address-carrier-and-missing-alignment-check-20260921-RETIRED-20260921.md`.
    let addr = if base != 0 {
        LayoutVhAddr::Native((base as usize).wrapping_add(offset as usize))
    } else {
        let view =
            crate::panama::heap_segment_view(ctx, seg).ok_or_else(out_of_bounds)?;
        LayoutVhAddr::Heap {
            base: view.base,
            elem_width: view.elem_width,
            elem_type: view.elem_type,
            byte_offset: view.start.saturating_add(offset),
        }
    };
    // The bounds check above says the access fits; this says it is aligned.
    // Same predicate as every other FFM alignment gate in `panama.rs`
    // (`asSlice`, `elements`/`spliterator`, the heap `get`/`set` accessor),
    // and the same one `heap_segment_view` above resolves from, so the two
    // cannot disagree about which segment this is:
    // the layout's required alignment must not exceed what the segment
    // promises AT THIS OFFSET (`seg.asSlice(offset).maxByteAlignment()`), not
    // merely what it promises at offset 0 — a `MemorySegment.ofArray(new
    // int[2])` is 4-byte alignable throughout, so a `JAVA_LONG` (alignment 8)
    // var handle access anywhere in it must be refused, exactly the "no
    // access checks alignment" defect this field was added to close.
    if shape.alignment > 1
        && shape.alignment > crate::panama::pe_segment_max_byte_alignment_at(ctx, seg, offset)
    {
        return Err(RuntimeError::IllegalArgumentException {
            message: format!(
                "Target offset {offset} is incompatible with alignment constraint {} for \
                 segment",
                shape.alignment
            ),
        }
    ...
    Ok(Some((addr, value_index)))
}

/// Decode a raw bit pattern into the `Value` `shape.carrier` calls for.
///
/// `raw`'s low `width` bytes hold the value already assembled in the
/// "natural" orientation both addressing modes produce it in — see
/// [`layout_vh_raw_native`] and [`layout_vh_read_heap`], which handle
/// `little_endian` themselves before calling this. Shared by both so the
/// carrier switch — the part a heap/native split has no reason to duplicate —
/// exists exactly once.
fn layout_vh_decode(shape: SegmentVhShape, raw: u64) -> Value {
    match shape.carrier {
        b'Z' => Value::Int(i32::from((raw as u8) != 0)),
        b'B' => Value::Int(raw as u8 as i8 as i32),
        b'C' => Value::Int(raw as u16 as i32),
        b'S' => Value::Int(raw as u16 as i16 as i32),
        // `FFM_ADDRESS_CARRIER` decodes to the same raw 8-byte `Long` `b'J'`
        // does -- `layout_vh_get` is what turns this into a `MemorySegment`,
        // not this function. See `FFM_ADDRESS_CARRIER`'s own doc.
        b'J' | FFM_ADDRESS_CARRIER => Value::Long(raw as i64),
...
/// Encode a `Value` into the raw bit pattern [`layout_vh_decode`] reads back —
/// the inverse, and shared for the same reason.
///
/// No `FFM_ADDRESS_CARRIER` arm: `layout_vh_set` unwraps that carrier's
/// `MemorySegment` value coordinate to its raw pointer `Long` before this
/// runs, so it reaches here exactly like `b'J'` and needs no arm of its own.
fn layout_vh_encode(shape: SegmentVhShape, value: Value) -> u64 {
    match (shape.carrier, value) {
...
/// Assemble the raw bit pattern [`layout_vh_decode`] expects by reading
/// `shape.width` bytes at a native address, honouring `shape.little_endian`.
fn layout_vh_raw_native(shape: SegmentVhShape, addr: usize) -> u64 {
...
/// Read an FFM layout handle's variable out of a native (off-heap) segment.
fn layout_vh_read_native(shape: SegmentVhShape, addr: usize) -> Value {
    layout_vh_decode(shape, layout_vh_raw_native(shape, addr))
}

/// Write an FFM layout handle's variable into a native (off-heap) segment.
fn layout_vh_write_native(shape: SegmentVhShape, addr: usize, value: Value) {
    let w = shape.width.clamp(1, 8) as usize;
    let raw = layout_vh_encode(shape, value);
...
/// Read an FFM layout handle's variable out of a HEAP segment
/// (`MemorySegment.ofArray(...)`), which has no native pointer to
/// dereference. `crate::panama::heap_bytes_read` always assembles its `u64`
/// as little-endian (memory's lowest-address byte is the value's LSB); a
/// `!little_endian` layout needs that reversed WITHIN `width` bytes, which is
/// what `seg_swap_bytes` does and is already proven equivalent to the native
/// path's own big-endian assembly (both walk the same memory bytes, just in
/// opposite shift directions).
fn layout_vh_read_heap(
    ctx: &dyn NativeContext,
    shape: SegmentVhShape,
    base: ObjectRef,
    elem_width: usize,
    ...
    let w = shape.width.clamp(1, 8) as usize;
    let raw_le = crate::panama::heap_bytes_read(ctx, base, elem_width, byte_offset, w);
    let raw = if shape.little_endian {
        raw_le
    } else {
        seg_swap_bytes(raw_le, w as i64)
    };
    layout_vh_decode(shape, raw)
}

/// Write an FFM layout handle's variable into a HEAP segment — see
/// [`layout_vh_read_heap`], its read-side twin, for the endianness argument.
fn layout_vh_write_heap(
    ctx: &mut dyn NativeContext,
    shape: SegmentVhShape,
    base: ObjectRef,
    elem_type: cratonvm_types::ArrayElementType,
    ...
    let w = shape.width.clamp(1, 8) as usize;
    let raw = layout_vh_encode(shape, value);
    let raw_le = if shape.little_endian {
        raw
    } else {
        seg_swap_bytes(raw, w as i64)
    };
    crate::panama::heap_bytes_write(ctx, base, elem_type, elem_width, byte_offset, w, raw_le);
}
...
fn layout_vh_get(
    ...
    let value = match addr {
        LayoutVhAddr::Native(a) => layout_vh_read_native(shape, a),
        LayoutVhAddr::Heap {
            base,
            elem_width,
            byte_offset,
            ..
        } => layout_vh_read_heap(ctx, shape, base, elem_width, byte_offset),
    };
    if shape.carrier == FFM_ADDRESS_CARRIER {
        // `value` is the raw pointer bits as a `Long` (`layout_vh_decode`'s
        // `FFM_ADDRESS_CARRIER` arm) -- box_value_canonical cannot turn that
        // into a `MemorySegment`, it only boxes primitives, so mint the
        // segment here instead of falling through to it. Zero-length and
        // two-slot, the same shape `MemorySegment.NULL` mints
        // (`foreign_ffm.rs`): an `ADDRESS`-carrier read has no byte size to
        // report, only the address, exactly what HotSpot's own
        // `ADDRESS.varHandle().get(...)` answers.
        let ptr = match value {
            Value::Long(v) => v,
            _ => 0,
        };
        ...
        return Ok(Some(Value::Object(Some(seg))));
    }
    // CANONICAL — measured `ffm.layoutInt` / `layoutChar` / `layoutLong` /
    // `layoutByte` = true. `layout_vh_decode` and `layout_vh_carrier_desc`
    // both switch on `shape.carrier`, so the `Value` variant and the
    // descriptor always agree here and the helper's variant guard is never
    // the arm taken.
...
fn layout_vh_set(
    ...
    let mut value = args.get(value_index).copied().unwrap_or(Value::Int(0));
    if shape.carrier == FFM_ADDRESS_CARRIER {
        // The value coordinate is a `MemorySegment`, not a `long` -- unwrap it
        // to the raw pointer `layout_vh_encode`'s `b'J'`-shaped fallback arm
        // already knows how to write.
        //
        // MEASURED on Temurin 25.0.4 (`AddressVhProbe`): a literal Java
        // `null` value coordinate raises `NullPointerException` -- it is NOT
        // the same as writing `MemorySegment.NULL`, which is a real,
        // non-null object whose `.address()` happens to be 0 and which this
        // arm's `Value::Object(Some(seg))` case already handles correctly by
        // reading through `segment_address`.
        let ptr = match value {
            Value::Object(Some(seg)) => crate::panama_libffi::segment_address(ctx, seg),
            Value::Object(None) => {
                return Err(RuntimeError::NullPointerException {
                    message: Some(
                        "VarHandle access with a null value coordinate".to_string(),
                    ),
                }
                .into());
            }
        ...
        value = Value::Long(ptr);
    }
    match addr {
        LayoutVhAddr::Native(a) => layout_vh_write_native(shape, a, value),
        LayoutVhAddr::Heap {
            base,
            elem_type,
            elem_width,
            byte_offset,
        } => layout_vh_write_heap(ctx, shape, base, elem_type, elem_width, byte_offset, value),
    }
    Ok(None)
}
...
/// The call site's own static parameter type for one collected argument,
/// resolved to the primitive descriptor it names.
///
/// F29-1 §2.3 / NOMINATION 2: an `Object[]`-component collector settles
/// nothing at the array, and the wrapper class HotSpot produces comes from
/// what the CALLER wrote at the call site, not from the collector or the
/// runtime `Value` variant (`char` and `int` are the same variant carrying
/// the same bits, and HotSpot boxes them differently). `entry_call_site` is
/// the descriptor `MethodHandle.invoke`'s native armed via
/// `cratonvm_native_api::poly_call_site` / `take_entry_call_site` before
/// calling into `mh_dispatch` — the one channel a native has to that type at
/// all. A miss (no call site armed, index out of range, or a reference
/// parameter) answers `None` and the caller falls back to the `Value`
/// variant, exactly as before this existed.
fn collector_call_site_prim(entry_call_site: Option<&str>, index: usize) -> Option<&'static str> {
    let cs = entry_call_site?;
    let (ptypes, _) = crate::lang_class::parse_descriptor_param_and_return(cs);
    match ptypes.get(index).map(String::as_str) {
        Some("C") => Some(DESC_CHAR),
        Some("Z") => Some(DESC_BOOLEAN),
        Some("B") => Some(DESC_BYTE),
        Some("S") => Some(DESC_SHORT),
        Some("I") => Some(DESC_INT),
        _ => None,
    }
}
...
///
/// `call_site_prim` is [`collector_call_site_prim`]'s answer for this element
/// — `Some` only when the component itself settled nothing AND the call site
/// is known — and is consulted BEFORE the `Value`-variant fallback below, per
/// F29-1 NOMINATION 2.
fn box_collector_element(
    ctx: &mut dyn NativeContext,
    v: Value,
    component: &str,
    call_site_prim: Option<&'static str>,
) -> Value {
    ...
    // case). The call site the caller wrote might still settle it — see
    // [`collector_call_site_prim`] — and takes priority over the runtime
    // `Value` variant fallback below, which is a KNOWN wrong answer for
    // `char`/`boolean`/`byte`/`short` (all `Value::Int`) when it is not.
    if let (Value::Int(_), Some(desc)) = (v, call_site_prim) {
        return crate::lang_class::box_value_canonical(ctx, v, desc);
    }
    // No component and no call site: the only type information left is the
    // runtime `Value` variant. It is a wrong CLASS, not a wrong identity, and
    // caching it does not make it wronger: the same object graph, one
    // allocation cheaper.
    match v {
...
pub(crate) fn alloc_static_var_handle(
    ...
    let owner_raw = owner.map_or(0, |cid| cid.as_u32());
    let vh = try_alloc_concurrent_synthetic(ctx, "java/lang/invoke/VarHandle", VH_FIELD_COUNT)?;
    ...
    if vh_has_synthetic_layout(ctx, vh) {
        ...
        ctx.set_field(vh, VH_CLASS_ID, Value::Int(owner_raw as i32));
    }
...
fn vh_static_slot(
    ...
    // Strict mode records the exact class the handle was minted against; two
    // loaders that define one name are two classes, and by-name resolution
    // reads and initialises the wrong one (see `mh_static_owner_of_mirror`).
    let cid = match owner {
        Some(cid) => {
            let _ = ctx.ensure_class_initialized_with_class_id(cid);
            cid
        }
        None => {
            let _ = ctx.ensure_class_initialized(class);
            ctx.class_id_by_name(class)?
        }
    };
...
/// The class a static `VarHandle` was minted against, when strict mode recorded it
/// (`alloc_static_var_handle`); `None` means resolve the name, as before.
fn vh_static_owner(meta: Option<&VarHandleMeta>) -> Option<ClassId> {
    meta.filter(|m| m.class_id != 0)
        .map(|m| ClassId::new(m.class_id))
}
...
fn segment_raw_access(
    ...
    let (base, base_off) = if class_name.contains("Heap") {
        // `HeapMemorySegmentImpl.offset` is `Unsafe`-style and must be
        // un-biased before use — see `panama_libffi::heap_segment_byte_start`,
        // the one shared reader for this rule (NOM F35-2; it used to be a
        // second, independent `const ABASE: i64 = 16` here). Skipping it
        // lands every heap-segment access 16 bytes past its intended target
        // (silently corrupting data when still in-bounds, throwing when it
        // overflows the backing array).
        ...
        let off = match off_idx {
            Some(i) => match ctx.get_field(seg, i) {
                Value::Long(n) => crate::panama_libffi::heap_segment_byte_start(n),
                _ => 0,
            },
            None => 0,
        };
...
/// Heap-segment access for `SegmentVarHandle`. `addr` is the Unsafe-style
/// absolute byte offset into `base` as returned by `unsafeGetOffset()`; real
/// `byte[]` segments carry `ARRAY_BYTE_BASE_OFFSET` baked into that value so
/// it lands directly on `base`'s own element indices.
///
/// NOM F35-3 (was a real defect, not just a comment): this used to read `base`
/// one BYTE per `get_array_element` call regardless of its actual element
/// type, which is right only for `byte[]`. A `VarHandle` over an
/// `int[]`-backed `MemorySegment` (`MemorySegment.ofArray(new int[8])`) read
/// element `addr..addr+width` of a `byte[]` view that does not exist, i.e. it
/// read INT elements at BYTE granularity — wrong data, not a crash, so nothing
/// short of comparing against the oracle would show it. `heap_bytes_read` is
/// `panama.rs`'s own general reader (`heap_segment_read`'s core), shared here
/// rather than re-derived, per that file's own doc comment asking for exactly
/// this ("make those two `pub(crate)` ... or move both into one module").
fn segment_heap_get(
    ctx: &mut dyn NativeContext,
    base: ObjectRef,
    addr: i64,
    ...
    let elem_width = crate::panama::heap_element_width(elem_type)? as i64;
    let byte_len = (ctx.array_length(base) as i64).checked_mul(elem_width)?;
    if addr < 0 || addr.checked_add(width)? > byte_len {
        return None;
    }
    Some(crate::panama::heap_bytes_read(
        ctx,
        base,
        elem_width as usize,
        addr,
...
/// Write-side twin of [`segment_heap_get`] — see its doc comment (NOM F35-3).
fn segment_heap_set(
    ctx: &mut dyn NativeContext,
    base: ObjectRef,
    addr: i64,
    ...
    let elem_type = ctx.heap_element_type_of(base);
    let Some(elem_width) = crate::panama::heap_element_width(elem_type) else {
        return false;
    };
    let elem_width = elem_width as i64;
    let Some(byte_len) = (ctx.array_length(base) as i64).checked_mul(elem_width) else {
        return false;
    };
    match addr.checked_add(width) {
        Some(end) if addr >= 0 && end <= byte_len => {}
        _ => return false,
    }
    crate::panama::heap_bytes_write(
        ctx,
        base,
        elem_type,
        elem_width as usize,
...
fn varhandle_get(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    match kind {
        ...
        VH_KIND_STATIC => {
            ...
            let raw = match vh_static_slot(ctx, &class, &field, vh_static_owner(meta.as_deref())) {
...
fn varhandle_set(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    match kind {
        ...
        VH_KIND_STATIC => {
            ...
            if let Some((cid, sidx)) =
                vh_static_slot(ctx, &class, &field, vh_static_owner(meta.as_deref()))
            {
    ...
    r.register(
        ...
        |ctx, _args| {
            ...
            if crate::nbflags().dbg_lookup {
                eprintln!(
                    "[DBG_LOOKUP] lookup(): {} frame(s); innermost = {:?}",
                    frames.len(),
                    frames.last().map(|f| (f.class_name.clone(), f.method_name.clone(), f.class_id)),
                );
            }
            let caller_class = if let Some(frame) = frames.last() {
                ...
                // lookup (`ensure_class_initialized`) is loader-blind and
                // collapses to whichever class of that name is globally
                // registered first, which is wrong whenever the actual caller
                // was loaded by a distinct ClassLoader from a same-named class
                // elsewhere on the classpath — the caller then gets handed a
                // `Lookup` whose `lookupClass` is a DIFFERENT class object
                // (defined fresh under the fallback/application namespace)
                // than the one actually executing, so `lookupClass().getClassLoader()`
                // and everything keyed on it (`Lookup.findClass`, access
                // checks, ...) observe the wrong loader identity. See
                // `StackTraceEntry::class_id`'s doc comment and
                // `deprecated_internal.rs`'s `Reflection.getCallerClass(int)`
...
fn lookup_find_static(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let name = ctx.read_string(name_obj).unwrap_or_default();
    let desc = descriptor_from_method_type(ctx, mt_obj);
    let owner = mh_static_owner_of_mirror(&*ctx, Some(class_obj));
    let loaded = ctx.ensure_class_initialized(&class).is_ok();
    lookup_require_method(ctx, loaded, &class, &name, &desc)?;
    ...
    mh_record_static_owner(ctx, mh, owner);
    Ok(Some(Value::Object(Some(mh))))
}
...
fn lookup_find_static_var_handle(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let owner = mh_static_owner_of_mirror(&*ctx, Some(class_obj));
    let vh = alloc_static_var_handle(ctx, &class, &field_name, &field_desc, owner)?;
    ...
    r.register(
        ...
        |ctx, args| {
            let arr_desc: String = match args.first() {
                ...
                {
                    Some(name) => class_name_to_descriptor(&name).into_owned(),
                    None => format!("[{DESC_OBJECT}"),
                },
                _ => format!("[{DESC_OBJECT}"),
            };
            let desc = format!("({arr_desc})I");
            let handle = alloc_method_handle(ctx, &arr_desc, "arrayLength", &desc, MH_KIND_ARRAY_LENGTH)?;
...
pub(crate) fn register_method_handles_constant_bridge(r: &mut NativeMethodRegistry) {
    ...
    r.register(
        ...
        |ctx, args| {
            ...
            // A primitive return type cannot hold `null` — javadoc: "if the
            // type is a primitive and the value is null a NullPointerException
            // is thrown". Measured (`k.constantNullPrimitive`,
            // `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`): CratonVM
            // accepted `constant(int.class, null)` where HotSpot 25 refuses at
            // this call. Only a genuine `null` (not merely an unreadable
            // value) trips this — anything else is an accept.
            if matches!(ret_desc.as_str(), "Z" | "B" | "C" | "S" | "I" | "J" | "F" | "D")
                && matches!(value, Value::Object(None))
            {
                return Err(cratonvm_types::error::RuntimeError::NullPointerException {
                    message: Some(format!(
                        "Primitive type {ret_desc} cannot be null-valued"
                    )),
                }
                .into());
            }
...
pub(crate) fn register_method_handle_combinator_extras_bridge(r: &mut NativeMethodRegistry) {
    ...
    r.register(
        ...
        |ctx, args| {
            ...
            // `insertArguments` refuses when `pos` + `values.length` exceeds
            // the target's own parameter count — javadoc's `0 <= pos <= pos +
            // values.length <= target.type().parameterCount()`. Measured
            // (`p.insertTooMany`, `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`): three values inserted at position 1 into a
            // 3-parameter target were silently accepted where HotSpot 25
            // throws `IllegalArgumentException` at this call. Every way of
            // not resolving the target's descriptor is an ACCEPT — this only
            // refuses on a positive over-supply reading.
            if let (Value::Object(Some(arr)), Some(tdesc)) =
                (values, mh_type_descriptor(ctx, target).or_else(|| mh_read_desc(ctx, target)))
            {
                if let Some((params, _)) = split_descriptor_params(&tdesc) {
                    let nvalues = ctx.array_length(arr);
                    if (pos.max(0) as usize) + nvalues > params.len() {
                        return Err(
                            cratonvm_types::error::RuntimeError::IllegalArgumentException {
                                message: format!(
                                    "too many values to insert: pos={pos}, nvalues={nvalues}, parameterCount={}",
                                    params.len()
                                ),
                            }
                            .into(),
...
/// Derive a loop adapter's `type()` from its `body` handle's descriptor,
/// dropping `drop_leading` leading parameters (the loop-local accumulator
/// plus, for `countedLoop`/`iteratedLoop`, the counter/item slot) and
/// optionally prefixing one leading parameter with no counterpart in
/// `body`'s own signature (`iteratedLoop`'s `Iterable`). Not exercised by any
/// `.type()` assertion in the sweep this fixes
/// (`lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md` names none), so a
/// descriptor `split_descriptor_params` can't parse degrades to a safe,

/// arity-erased `()Ljava/lang/Object;` rather than failing the factory call.
fn make_loop_type_desc(
    ctx: &mut dyn NativeContext,
    body: ObjectRef,
    drop_leading: usize,
    prefix_param: Option<&str>,
) -> String {
    let bdesc = mh_type_descriptor(ctx, body)
        .or_else(|| mh_read_desc(ctx, body))
        .unwrap_or_default();
    match split_descriptor_params(&bdesc) {
        Some((params, ret)) => {
            let extra: String = params.into_iter().skip(drop_leading).collect();
            let prefix = prefix_param.unwrap_or("");
            format!("({prefix}{extra}){ret}")
        }
        None => "()Ljava/lang/Object;".to_string(),
    }
}

/// Shared body of `countedLoop`/`whileLoop`/`doWhileLoop`'s factories — all
/// three take exactly 3 `MethodHandle` arguments and store them in a 3-slot
/// carrier VERBATIM, in their own call's argument order (the dispatch side
/// for each `kind` knows which slot means what — see `MH_KIND_LOOP_COUNTED`/
/// `_WHILE`/`_DOWHILE`'s doc comments). `body_slot` is which of `args[0..3]`
/// is the `body` handle, for `type()` derivation.
fn make_loop3_adapter(
    ctx: &mut dyn NativeContext,
    args: &[Value],
    kind: i32,
    body_slot: usize,
    ...
    let a0 = args.first().copied().unwrap_or(Value::Object(None));
    let a1 = args.get(1).copied().unwrap_or(Value::Object(None));
    let a2 = args.get(2).copied().unwrap_or(Value::Object(None));
    let slots = [a0, a1, a2];
    let body_mh = match slots.get(body_slot).copied() {
        Some(Value::Object(Some(m))) => m,
        _ => return Ok(Some(Value::Object(None))),
    };
    let wrapper = alloc_mh_carrier(ctx, "__mh_loop3_wrapper__", 3);
    ctx.set_field(wrapper, 0, a0);
    ctx.set_field(wrapper, 1, a1);
    ctx.set_field(wrapper, 2, a2);
    let desc = make_loop_type_desc(ctx, body_mh, drop_leading, None);
    let adapter = alloc_method_handle(ctx, "__adapter__", "loop", &desc, kind)?;
    ctx.set_field(adapter, MH_BOUND, Value::Object(Some(wrapper)));
    Ok(Some(Value::Object(Some(adapter))))
}

/// Functional `MethodHandles.countedLoop`/`whileLoop`/`doWhileLoop`/
/// `iteratedLoop`/`tryFinally` — the whole "a JDK 9 API absent" bucket from
/// `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`.
/// Real JDK bytecode for all five builds `LambdaForm`/

/// `BoundMethodHandle` machinery this VM's `MH_KIND_*` shim model does not
/// have (the same species of gap `constant`/`identity`/`insertArguments`
/// already work around) — without a native every one of them either NPEs,
/// throws `NoSuchMethodError`, or, once an earlier failure has left
/// `BoundMethodHandle`'s `<clinit>` in the JVMS 5.5 ERRONEOUS state, throws
/// `NoClassDefFoundError`. Promoted to the real-JDK essentials path (shared
/// with the synthetic-jdk registration) and `check_override`-allow-listed in
/// `vm/src/vm/vm_exec.rs`, mirroring every other combinator bridge in this
/// file.
pub(crate) fn register_method_handle_loop_bridge(r: &mut NativeMethodRegistry) {
    let __prev_cat = r.current_category();
    r.set_category(cratonvm_native_api::NativeKind::Bridge);
    let mh = "java/lang/invoke/MethodHandles";
    let desc3 = "(Ljava/lang/invoke/MethodHandle;Ljava/lang/invoke/MethodHandle;Ljava/lang/invoke/MethodHandle;)Ljava/lang/invoke/MethodHandle;";

    // countedLoop(iterations, init, body): body is args[2], dropping its
    // leading (v, i) pair for type().
    r.register(mh, "countedLoop", desc3, |ctx, args| {
        make_loop3_adapter(ctx, args, MH_KIND_LOOP_COUNTED, 2, 2)
    });
    // whileLoop(init, pred, body): body is args[2], dropping its leading v.
    r.register(mh, "whileLoop", desc3, |ctx, args| {
        make_loop3_adapter(ctx, args, MH_KIND_LOOP_WHILE, 2, 1)
    });
    // doWhileLoop(init, body, pred): body is args[1] — the ARGUMENT ORDER
    // that makes `MH_KIND_LOOP_DOWHILE` read a different wrapper layout than
    // `MH_KIND_LOOP_WHILE`, though the carrier is stored verbatim either way.
    r.register(mh, "doWhileLoop", desc3, |ctx, args| {
        make_loop3_adapter(ctx, args, MH_KIND_LOOP_DOWHILE, 1, 1)
    });
    // iteratedLoop(iterator, init, body): only the `iterator == null` form is
    // implemented (see `MH_KIND_LOOP_ITERATE`'s doc comment); the carrier
    // holds (init, body) only, not the unused `iterator` slot.
    r.register(mh, "iteratedLoop", desc3, |ctx, args| {
        let init = args.get(1).copied().unwrap_or(Value::Object(None));
        let body_mh = match args.get(2) {
            Some(Value::Object(Some(m))) => *m,
            _ => return Ok(Some(Value::Object(None))),
        };
        let wrapper = alloc_mh_carrier(ctx, "__mh_iterate_wrapper__", 2);
        ctx.set_field(wrapper, 0, init);
        ctx.set_field(wrapper, 1, Value::Object(Some(body_mh)));
        let desc = make_loop_type_desc(ctx, body_mh, 2, Some("Ljava/lang/Iterable;"));
        let adapter =
            alloc_method_handle(ctx, "__adapter__", "iterate", &desc, MH_KIND_LOOP_ITERATE)?;
        ctx.set_field(adapter, MH_BOUND, Value::Object(Some(wrapper)));
        Ok(Some(Value::Object(Some(adapter))))
    });
    r.register(
        ...
        |ctx, args| {
            ...
            let wrapper = alloc_mh_carrier(ctx, "__mh_tryfinally_wrapper__", 2);
            ctx.set_field(wrapper, 0, Value::Object(Some(target)));
            ctx.set_field(wrapper, 1, cleanup);
            // type(): tryFinally does not change the caller-visible signature
            // at all — the adapter's type is the target's, verbatim.
...
pub(crate) fn register_p65_method_handles_extra(r: &mut NativeMethodRegistry) {
    ...
    // Functional loop family + `tryFinally` (shared with the real-JDK
    // essentials path).
    register_method_handle_loop_bridge(r);
    r.register(
        mh,
        "dropArguments",
    ...
    // Functional `empty`/`zero` shims (shared with the real-JDK essentials
    // path).
    register_method_handles_zero_empty_bridge(r);
    r.set_category(__prev_cat);
}

/// Zero/default `Value` for a primitive return descriptor, or `Object(None)`
/// (null) for anything else — shared by `zero`/`empty` below.
fn mh_zero_value_for_descriptor(desc: &str) -> Value {
    match desc.as_bytes().first() {
        Some(b'J') => Value::Long(0),
        Some(b'F') => Value::Float(0.0),
        Some(b'D') => Value::Double(0.0),
        Some(b'Z' | b'B' | b'C' | b'S' | b'I') => Value::Int(0),
        _ => Value::Object(None),
    }
}

/// `MethodHandles.zero(Class)` / `MethodHandles.empty(MethodType)` — functional
/// `MH_KIND_CONSTANT` shims (`zero` is exactly `constant(type, <the type's
/// default value>)`; `empty` takes an arbitrary `MethodType` rather than a
/// bare return `Class`, but `MH_KIND_CONSTANT`'s dispatch already ignores its
/// incoming arguments entirely, so the SAME kind serves both).
///
/// Previously both were INERT: a bare 17-slot `MethodHandle` with a `type`
/// field and no `MH_KIND`/`MH_BOUND` at all — allocated directly rather than
/// through `alloc_method_handle`, the exact species of bug
/// `MH_KIND_ARRAY_GET`'s doc comment records for the array accessors. That
/// happened to read back `null` for a REFERENCE `zero`, which is the right
/// answer by accident (source doc's "one row that was passing by accident"),
/// and wrong for every primitive one — `k.zeroInt` was measured
/// `NullPointerException` where HotSpot answers `0`.
///
/// Worse, and the reason this pair is fixed alongside the loop family rather
/// than left alone: this registrar was ONLY ever reachable from
/// `register_p65_method_handles_extra`, itself only called from
/// `register_phase65_natives`, itself only called from the `#[cfg(feature =
/// "synthetic-jdk")]`-gated `register_synthetic_overrides` — the exact
/// wiring bug `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md` §"Why the
/// first attempt failed" describes for the ORIGINAL loop-family attempt, but
/// pre-existing here rather than newly introduced. A real-JDK CLI build
/// (the default; no `synthetic-jdk` feature) never registered `zero`/`empty`
/// at all, so real `MethodHandles.zero`/`empty` bytecode ran instead — both
/// call `MethodHandleImpl.makeConstantReturning` DIRECTLY (not through the
/// public `MethodHandles.constant`, so the already-shimmed `constant` native
/// never intercepts it), which spins the `BoundMethodHandle`/
/// `ClassSpecializer` species machinery this VM's shim model can't run:
/// measured `InternalError: Failed to link speciesData to speciesCode`
/// isolated, and `NoClassDefFoundError` once an earlier call has left
/// `BoundMethodHandle`'s `<clinit>` in the JVMS 5.5 ERRONEOUS state (which is
/// why `k.zeroRef`/`k.emptyVoid` — reference-typed, "passing by accident"
/// under the old inert stub — broke too, not only `k.zeroInt`).
pub(crate) fn register_method_handles_zero_empty_bridge(r: &mut NativeMethodRegistry) {
    let __prev_cat = r.current_category();
    r.set_category(cratonvm_native_api::NativeKind::Bridge);
    let mh = "java/lang/invoke/MethodHandles";
    r.register(
        ...
        |ctx, args| {
            ...
            let desc = format!("(){ret_desc}");
            let handle = alloc_method_handle(ctx, "", "", &desc, MH_KIND_CONSTANT)?;
            ctx.set_field(handle, MH_BOUND, mh_zero_value_for_descriptor(&ret_desc));
            Ok(Some(Value::Object(Some(handle))))
        },
    );
    r.register(
        ...
        |ctx, args| {
            ...
            let desc = mt
                .and_then(|m| methodtype_to_descriptor(ctx, m))
                .unwrap_or_else(|| "()V".to_string());
            let ret_desc = return_type_desc(&desc).to_string();
            let handle = alloc_method_handle(ctx, "", "", &desc, MH_KIND_CONSTANT)?;
            ctx.set_field(handle, MH_BOUND, mh_zero_value_for_descriptor(&ret_desc));
            Ok(Some(Value::Object(Some(handle))))
        },
...
/// Strict mode: the identity of the class a STATIC member handle was minted
/// against, taken from the caller's own `Class` object BEFORE the handle is
/// allocated (allocation can move the object the mirror was read from).
///
/// A handle records its owner as a binary-name string, and dispatch resolves
/// that string to "the" class of that name. Two loaders that define one binary
/// name are two classes (JVMS 5.3.4), so `findStaticGetter(forkCopy, "INSTANCE",
/// forkCopy).invoke()` read the APPLICATION copy's static -- and initialised it,
/// running a `<clinit>` the caller never asked for. `findConstructor` has kept
/// the id in `MH_BOUND` for the same reason since the Groovy work; the static
/// kinds ignore an `Int` there (only an `Object` there is a `bindTo` capture).
///
/// Only under `--jdk-only`, where class bytes are authoritative; the default
/// mode keeps the by-name resolution it has always had.
fn mh_static_owner_of_mirror(
    ctx: &dyn NativeContext,
    owner_mirror: Option<ObjectRef>,
) -> Option<ClassId> {
    if !ctx.is_jdk_only() {
        return None;
    }
    ctx.class_id_from_mirror(owner_mirror?)
}

/// The `Class` object in a reflected `Method`/`Field`'s `clazz` field.
fn member_clazz_mirror(ctx: &mut dyn NativeContext, member: ObjectRef) -> Option<ObjectRef> {
    match ctx.get_field_by_name(member, "clazz") {
        Value::Object(Some(mirror)) => Some(mirror),
        _ => None,
    }
}

/// Record [`mh_static_owner_of_mirror`]'s answer on a freshly allocated handle.
fn mh_record_static_owner(ctx: &mut dyn NativeContext, mh: ObjectRef, owner: Option<ClassId>) {
    if let Some(cid) = owner {
        ctx.set_field(mh, MH_BOUND, Value::Int(cid.as_u32() as i32));
    }
}

/// The owner recorded by [`mh_record_static_owner`], read back from `MH_BOUND`.
fn mh_static_owner(bound: Value) -> Option<ClassId> {
    match bound {
...
fn mh_entry_adapt(
    ...
    if is_collector && trailing_array {
        ...
        let arr = build_varargs_array(ctx, &component, &params[n - 1..], Some(call_site), n - 1);
...
/// `MethodHandles.countedLoop(iterations, init, body)` —
/// `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`'s "whole JDK 9

/// API absent" bucket: the whole loop family plus `tryFinally` had no native
/// at all, so real JDK bytecode ran and NPE'd/AbstractMethodError'd building
/// `LambdaForm`/`MethodHandleImpl$LoopClauses` machinery this VM's `MH_KIND_*`
/// shim model doesn't have. `MH_BOUND` holds a 3-slot carrier `(iterations,
/// init, body)`; dispatch runs `v = init.invoke(A); for i in 0..iterations
/// .invoke(A) { v = body.invoke(v, i, A); }`, measured against HotSpot 25
/// (`l.countedLoop` = 10 for a 5-iteration `value+i` accumulator).
pub(crate) const MH_KIND_LOOP_COUNTED: i32 = 26;

/// `MethodHandles.whileLoop(init, pred, body)` — sibling of
/// `MH_KIND_LOOP_COUNTED`. `MH_BOUND` holds `(init, pred, body)`; dispatch
/// runs `v = init.invoke(A); while (pred.invoke(v, A)) { v = body.invoke(v,
/// A); }`.
pub(crate) const MH_KIND_LOOP_WHILE: i32 = 27;

/// `MethodHandles.doWhileLoop(init, body, pred)` — note the ARGUMENT ORDER
/// differs from `whileLoop`: body before pred. `MH_BOUND` holds `(init, body,
/// pred)`; dispatch runs the body once unconditionally before the first test
/// (`do { v = body.invoke(v, A); } while (pred.invoke(v, A));`), so a
/// predicate that is false on entry still yields one iteration.
pub(crate) const MH_KIND_LOOP_DOWHILE: i32 = 28;

/// `MethodHandles.iteratedLoop(iterator, init, body)` — this VM only
/// implements the `iterator == null` form (the common one: the adapter's own
/// leading parameter becomes the `Iterable` to walk). `MH_BOUND` holds
/// `(init, body)`; dispatch calls `Iterable.iterator()`/`Iterator.hasNext()`/
/// `Iterator.next()` via `NativeContext::invoke_virtual` and folds
/// `v = body.invoke(v, item, A)` over each element.
pub(crate) const MH_KIND_LOOP_ITERATE: i32 = 29;

/// `MethodHandles.tryFinally(target, cleanup)` — `MH_BOUND` holds `(target,
/// cleanup)`. Dispatch runs `target`; on normal return, `cleanup.invoke(null,
/// result, A)` runs and ITS result is returned. On an exception, `cleanup
/// .invoke(thrown, defaultResult, A)` runs and then the ORIGINAL exception is
/// rethrown — unless cleanup itself throws, in which case that new exception
/// replaces it (JDK contract; measured `g.tryFinallyThrows` = the target's
/// `IllegalStateException` propagates even though cleanup ran).
pub(crate) const MH_KIND_TRY_FINALLY: i32 = 30;

/// `MethodHandles.arrayLength(arrayClass)` — a handle of type `(T[])int`
/// that reads `array.length`. Sibling of `MH_KIND_ARRAY_GET`/`_SET` (same
/// factory family, same inert-handle species of bug): this one was never
/// bridged at all, so real JDK 9 bytecode ran and hit the exact
/// `AbstractMethodError` `MH_KIND_ARRAY_GET`'s doc comment predicts for a
/// primitive-array `viewAsType`/`copyWith` path — measured
/// (`k.arrayLength`, `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`)
/// unambiguously: "one factory, one dispatch arm, no loop semantics", the
/// cheapest row in the whole 16-row list to confirm the wiring on.
pub(crate) const MH_KIND_ARRAY_LENGTH: i32 = 31;
...
fn make_fold_adapter(
    ...
    ctx.unpin_native_roots(target_pin);
    ctx.set_field(adapter, MH_BOUND, Value::Object(Some(wrapper)));
    // type(): foldArguments REMOVES ONE PARAMETER, at `pos`, from the
    // target's type — the slot the combiner's non-void result fills is
    // synthesized at dispatch, not supplied by the caller (a void combiner
    // contributes nothing, so the type is unchanged). Measured
    // (`p.foldPrefix.type`, `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`): `foldArguments((String,String,String)String,
    // (String)String)` reported `(String,String,String)String` — the
    // target's raw, un-reduced arity — where HotSpot 25 says
    // `(String,String)String`. This already cost a real caller once: SpEL's
    // `FunctionReference` reads a handle's arity to decide whether to
    // re-wrap its arguments, and the stale type made it double-wrap. Chain
    // off the target's ADAPTED type (not `MH_DESC`) so a stack of adapters
    // stays arity-correct, matching every other combinator's `type()` fix
    // in this file.
    let combiner_is_void = mh_type_descriptor(ctx, combiner_ref)
        .or_else(|| mh_read_desc(ctx, combiner_ref))
        .and_then(|d| split_descriptor_params(&d))
        .map(|(_, ret)| ret == "V")
        .unwrap_or(false);
    if !combiner_is_void {
        if let Some(tdesc) = mh_type_descriptor(ctx, target) {
...
/// `MethodHandles.catchException(target, exType, handler)` refuses when the
/// handler's declared leading parameter is not a supertype of (or equal to)
/// `exType` — javadoc: "the handler must accept a leading parameter of type
/// exType or a supertype". Measured (`g.catchWrongType`,
/// `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`): a handler declared
/// `(IllegalStateException, String)` registered against `ClassCastException`
/// — an unrelated sibling exception type — was silently accepted where
/// HotSpot 25 throws `IllegalArgumentException` at this call. Every way of
/// not resolving both types to a real class hierarchy is an ACCEPT: this only
/// refuses on a positive negative-hierarchy reading, never on an unknown one.
fn mh_catch_exception_type_refusal(
    ctx: &mut dyn NativeContext,
    catch_type: Value,
    handler: Value,
) -> Option<MethodCallFailed> {
    let catch_mirror = match catch_type {
        Value::Object(Some(m)) => m,
        _ => return None,
    };
    let catch_cid = mirror_class_id(ctx, catch_mirror)?;
    let handler_obj = match handler {
        Value::Object(Some(h)) => h,
        _ => return None,
    };
    let hdesc = mh_type_descriptor(ctx, handler_obj).or_else(|| mh_read_desc(ctx, handler_obj))?;
    let (hparams, _) = split_descriptor_params(&hdesc)?;
    let first = hparams.first()?;
    let handler_name = first.strip_prefix('L')?.strip_suffix(';')?;
    let handler_cid = ctx.class_id_by_name(handler_name)?;
    if catch_cid == handler_cid || ctx.is_subclass(catch_cid, handler_cid) {
        return None;
    }
    let catch_name = ctx.class_name_of_id(catch_cid).unwrap_or_default();
    Some(
        cratonvm_types::error::RuntimeError::IllegalArgumentException {
            message: format!("handler should adapt to {catch_name}: (L{handler_name};...)..."),
        }
        .into(),
    )
}
...
/// `MethodHandles.countedLoop` dispatch (`MH_KIND_LOOP_COUNTED`). `bound` is
/// the 3-field wrapper `(iterations, init, body)`. See that constant's doc
/// comment for the semantics and the measurement that pins the argument
/// order (`body(v, i)`, not `body(i, v)`).
fn mh_dispatch_counted_loop(
    ctx: &mut dyn NativeContext,
    bound: Value,
    extra_args: &[Value],
) -> MethodCallResult {
    ...
    let iterations = match ctx.get_field(wrapper, 0) {
        Value::Object(Some(m)) => Some(m),
        _ => None,
    };
    let init = match ctx.get_field(wrapper, 1) {
        Value::Object(Some(m)) => Some(m),
        _ => None,
    };
    let body = match ctx.get_field(wrapper, 2) {
        Value::Object(Some(m)) => Some(m),
        _ => None,
    };
    let count = match iterations {
        Some(m) => match mh_dispatch(ctx, m, extra_args)? {
            Some(Value::Int(n)) => n,
    ...
    let mut v: Option<Value> = match init {
        Some(m) => mh_dispatch(ctx, m, extra_args)?,
        None => None,
    };
    let Some(body_mh) = body else {
        return Ok(v);
    };
    // GC-safety: `body_mh` is dispatched repeatedly across a loop whose body
    // (an arbitrary user method) can allocate and trigger a collection on any
    // iteration; pin it once and re-read the forwarded reference each time,
    // the same idiom `MH_KIND_COLLECT`'s per-element loop above uses.
    let body_pin = ctx.pin_native_root(body_mh);
    for i in 0..count.max(0) {
        let mut bargs: Vec<Value> = Vec::with_capacity(2 + extra_args.len());
        bargs.push(v.unwrap_or(Value::Object(None)));
        bargs.push(Value::Int(i));
        bargs.extend_from_slice(extra_args);
        let body_mh = ctx.read_native_pin(body_pin, body_mh);
        let result = mh_dispatch(ctx, body_mh, &bargs)?;
        if result.is_some() {
            v = result;
        }
    }
...
/// `MethodHandles.whileLoop`/`doWhileLoop` dispatch (`MH_KIND_LOOP_WHILE`/
/// `MH_KIND_LOOP_DOWHILE`). `bound` is the 3-field wrapper; for `whileLoop`
/// it is `(init, pred, body)`, for `doWhileLoop` it is `(init, body, pred)` —
/// the factories store them in their OWN call's argument order rather than
/// normalising, so `do_while` picks which slot order this dispatch reads.
/// `whileLoop` tests BEFORE each iteration; `doWhileLoop` runs the body once
/// unconditionally first (see `MH_KIND_LOOP_DOWHILE`'s doc comment) and then
/// the two are the same loop with the test moved.
fn mh_dispatch_while_loop(
    ctx: &mut dyn NativeContext,
    bound: Value,
    extra_args: &[Value],
    do_while: bool,
    ...
    let init = match ctx.get_field(wrapper, 0) {
        Value::Object(Some(m)) => Some(m),
        _ => None,
    };
    let (body, pred) = if do_while {
        (
            match ctx.get_field(wrapper, 1) {
                Value::Object(Some(m)) => Some(m),
                _ => None,
            },
            match ctx.get_field(wrapper, 2) {
                Value::Object(Some(m)) => Some(m),
                _ => None,
            },
        )
    ...
    let (Some(body_mh), Some(pred_mh)) = (body, pred) else {
        return Ok(v);
    };
    // GC-safety: both handles are dispatched repeatedly across a loop whose
    // body/predicate (arbitrary user methods) can allocate; pin once and
    // re-read the forwarded reference on every use, as above.
    let body_pin = ctx.pin_native_root(body_mh);
    let pred_pin = ctx.pin_native_root(pred_mh);
    loop {
        if do_while {
            let mut bargs: Vec<Value> = Vec::with_capacity(1 + extra_args.len());
            bargs.push(v.unwrap_or(Value::Object(None)));
            bargs.extend_from_slice(extra_args);
            let body_mh = ctx.read_native_pin(body_pin, body_mh);
            v = mh_dispatch(ctx, body_mh, &bargs)?;
        }
        let mut pargs: Vec<Value> = Vec::with_capacity(1 + extra_args.len());
        pargs.push(v.unwrap_or(Value::Object(None)));
        pargs.extend_from_slice(extra_args);
        let pred_mh = ctx.read_native_pin(pred_pin, pred_mh);
        let pred_result = mh_dispatch(ctx, pred_mh, &pargs)?;
        let truthy = mh_guard_truthy(ctx, pred_result);
        if !truthy {
            break;
        }
        if !do_while {
...
/// `MethodHandles.iteratedLoop` dispatch (`MH_KIND_LOOP_ITERATE`), for the
/// `iterator == null` form this VM implements: the adapter's own leading
/// argument IS the `Iterable` to walk (see that constant's doc comment).
/// `bound` is the 2-field wrapper `(init, body)`. Walks the JDK's own
/// `Iterable`/`Iterator` protocol via `NativeContext::invoke_virtual` rather
/// than assuming any particular collection shape, so it works on any real
/// `Iterable`, not just `List`.
fn mh_dispatch_iterated_loop(
    ctx: &mut dyn NativeContext,
    bound: Value,
    extra_args: &[Value],
) -> MethodCallResult {
    ...
    let body = match ctx.get_field(wrapper, 1) {
        Value::Object(Some(m)) => Some(m),
        _ => None,
    };
    let iterable = match extra_args.first() {
        Some(Value::Object(Some(o))) => *o,
        _ => {
            return Err(cratonvm_types::error::RuntimeError::NullPointerException {
                message: Some("iteratedLoop: iterable is null".to_string()),
            }
            .into());
        }
    };
    let trailing: &[Value] = extra_args.get(1..).unwrap_or(&[]);
    let mut v: Option<Value> = match init {
        Some(m) => mh_dispatch(ctx, m, trailing)?,
        None => None,
    };
    let Some(body_mh) = body else {
        return Ok(v);
    };
    let iter_obj = match ctx.invoke_virtual(iterable, "iterator", "()Ljava/util/Iterator;", &[])? {
        Some(Value::Object(Some(it))) => it,
        _ => return Ok(v),
    };
    // GC-safety: both handles are re-used across a loop whose bodies
    // (`Iterator.next`, the user's `body` handle) can allocate; pin once and
    // re-read the forwarded reference on every use, as the other loop
    // dispatch functions above do.
    let body_pin = ctx.pin_native_root(body_mh);
    let iter_pin = ctx.pin_native_root(iter_obj);
    loop {
        let iter_obj = ctx.read_native_pin(iter_pin, iter_obj);
        let has_next = match ctx.invoke_virtual(iter_obj, "hasNext", "()Z", &[])? {
            Some(Value::Int(b)) => b != 0,
            Some(Value::Object(Some(o))) => {
                matches!(crate::lang_class::unbox_value(ctx, o), Value::Int(b) if b != 0)
            }
            _ => false,
        };
        if !has_next {
        ...
        let iter_obj = ctx.read_native_pin(iter_pin, iter_obj);
        let item = ctx
            .invoke_virtual(iter_obj, "next", "()Ljava/lang/Object;", &[])?
            .unwrap_or(Value::Object(None));
        let mut bargs: Vec<Value> = Vec::with_capacity(2 + trailing.len());
        bargs.push(v.unwrap_or(Value::Object(None)));
        bargs.push(item);
        bargs.extend_from_slice(trailing);
        let body_mh = ctx.read_native_pin(body_pin, body_mh);
        v = mh_dispatch(ctx, body_mh, &bargs)?;
    }
    ctx.unpin_native_roots(body_pin);
...
/// `MethodHandles.tryFinally` dispatch (`MH_KIND_TRY_FINALLY`). `bound` is
/// the 2-field wrapper `(target, cleanup)`. See that constant's doc comment
/// for the contract; the exceptional arm mirrors `mh_dispatch_catch`'s use of
/// `MethodCallFailed::ExceptionThrown` to recover the actual thrown object so
/// `cleanup` can read it as its leading parameter.
fn mh_dispatch_try_finally(
    ...
    let target_void = mh_type_descriptor(ctx, target)
        .or_else(|| mh_read_desc(ctx, target))
        .map(|d| return_type_desc(&d) == "V")
        .unwrap_or(false);
    // GC-safety: `mh_dispatch(ctx, target, ...)` below can trigger a
    // collection that relocates `cleanup` (captured above, read again in
    // both arms below).
    let cleanup_pin = ctx.pin_native_root(cleanup);
    match mh_dispatch(ctx, target, extra_args) {
        Ok(result) => {
            let cleanup = ctx.read_native_pin(cleanup_pin, cleanup);
            let mut cargs: Vec<Value> = Vec::with_capacity(2 + extra_args.len());
            cargs.push(Value::Object(None));
            if !target_void {
                cargs.push(result.unwrap_or(Value::Object(None)));
            }
            cargs.extend_from_slice(extra_args);
            mh_dispatch(ctx, cleanup, &cargs)
        }
        Err(MethodCallFailed::ExceptionThrown(thrown)) => {
            let cleanup = ctx.read_native_pin(cleanup_pin, cleanup);
            // GC-safety: `mh_dispatch(ctx, cleanup, ...)` below can trigger a
            // collection that relocates `thrown` (captured above, re-read
            // after cleanup runs and re-thrown below).
            let thrown_pin = ctx.pin_native_root(thrown);
            let mut cargs: Vec<Value> = Vec::with_capacity(2 + extra_args.len());
            ...
            cargs.extend_from_slice(extra_args);
            let cleanup_result = mh_dispatch(ctx, cleanup, &cargs);
            let thrown = ctx.read_native_pin(thrown_pin, thrown);
            match cleanup_result {
                // Cleanup ran normally: the ORIGINAL exception propagates,
                // exactly as if `finally` had no `throw` in it.
                Ok(_) => Err(MethodCallFailed::ExceptionThrown(thrown)),
                // Cleanup itself threw: that exception replaces the original,
                // exactly as a `throw` inside a real `finally` block would.
...
pub(crate) fn mh_dispatch(
    ...
    match kind {
        MH_KIND_STATIC => {
            ...
            let result = match mh_static_owner(bound) {
                Some(owner) => ctx.invoke_by_class_id(owner, &class, &name, &desc, &adapted),
                None => ctx.invoke(&class, &name, &desc, &adapted),
            };
        ...
        MH_KIND_GETTER => {
            ...
            if is_static {
                ...
                let class_id = match mh_static_owner(bound) {
                    Some(owner) => match ctx.ensure_class_initialized_with_class_id(owner) {
                        Ok(()) => owner,
                        Err(_) => return Ok(Some(Value::Object(None))),
                    },
                    None => match ctx.ensure_class_initialized(&class) {
        ...
        MH_KIND_COLLECT => {
            ...
            for i in 0..count {
                ...
                let elem = if elem_type == cratonvm_types::ArrayElementType::Reference {
                    ...
                    // collector); otherwise from the CALL SITE the caller
                    // wrote (F29-1 NOMINATION 2, closed via `entry_call_site`)
                    // when one is armed; otherwise from the `Value` variant,
                    // which is a KNOWN wrong answer for `char`/`boolean`/
                    // `byte`/`short` in the ordinary `Object[]` case. See
                    // `collector_element_box_desc` and
                    // `collector_call_site_prim`.
                    box_collector_element(
                        ctx,
                        v,
                        &comp,
                        collector_call_site_prim(entry_call_site.as_deref(), leading + i),
                    )
                } else {
        ...
        MH_KIND_ARRAY_LENGTH => {
            // `MethodHandles.arrayLength`: (T[])int, reads `array.length`.
            // After `bindTo(array)` the array is pre-captured in MH_BOUND,
            // exactly as MH_KIND_ARRAY_GET/_SET.
            let arr = match bound {
                Value::Object(Some(a)) => Some(a),
                _ => match extra_args.first() {
                    Some(Value::Object(Some(a))) => Some(*a),
            ...
            match arr {
                Some(a) => Ok(Some(Value::Int(ctx.array_length(a) as i32))),
                None => Err(cratonvm_types::error::RuntimeError::NullPointerException {
        ...
        MH_KIND_SPREAD => {
            ...
            // whose length MUST equal `count`; spread its elements into
            // positional args, then dispatch target.
            //
            // G-MHCOMB1: this arm used to spread WHATEVER length array showed
            // up (or, for a null array, spread nothing) instead of checking it
            // against `count` — measured `c.asSpreaderWrongLen` (a 2-element
            // array into a 3-parameter spreader) fabricating `a|b|null` where
            // HotSpot 25 throws `IllegalArgumentException`, and
            // `c.asSpreaderNullArray` fabricating `null|null|null` where
            // HotSpot throws `NullPointerException`. Both are silent-wrong-
            // answer defects: a caller cannot tell a fabricated argument from
            // a real one without an oracle.
            let wrapper = match bound {
                Value::Object(Some(w)) => w,
                _ => return Ok(Some(Value::Object(None))),
            };
            ...
            if count == 0 || extra_args.is_empty() {
                // No trailing array parameter to spread at all (or nothing to
                // spread it from) — dispatch unchanged, exactly as before.
                return mh_dispatch(ctx, target, extra_args);
            }
            let last = extra_args.len() - 1;
            let arr = match extra_args[last] {
                Value::Object(Some(a)) => a,
                _ => {
                    return Err(cratonvm_types::error::RuntimeError::NullPointerException {
                        message: Some(format!("{name}: array is null")),
            ...
            if n != count {
                return Err(
                    cratonvm_types::error::RuntimeError::IllegalArgumentException {
                        message: format!("array length does not match count: {n} != {count}"),
                    }
                    .into(),
                );
            }
            let mut full: Vec<Value> = Vec::with_capacity(last + n);
        ...
        MH_KIND_LOOP_COUNTED => mh_dispatch_counted_loop(ctx, bound, extra_args),
        MH_KIND_LOOP_WHILE => mh_dispatch_while_loop(ctx, bound, extra_args, false),
        MH_KIND_LOOP_DOWHILE => mh_dispatch_while_loop(ctx, bound, extra_args, true),
        MH_KIND_LOOP_ITERATE => mh_dispatch_iterated_loop(ctx, bound, extra_args),
...
fn collect_trailing_varargs(
    ...
    let array = build_varargs_array(ctx, component, &collected, None, 0);
...
///
/// `call_site` and `start_index` are [`collector_call_site_prim`]'s inputs for
/// each element of `vals` (element `i` sits at call-site position
/// `start_index + i`) — `None`/`0` from [`collect_trailing_varargs`]'s
/// permissive path, which has no call site to offer; `Some`/the collected
/// region's own offset from [`mh_entry_adapt`], which does. F29-1 NOMINATION
/// 2: `var.invoke(aChar)` on an `Object[]`-collecting varargs handle is
/// boxed by the CALL SITE the caller wrote, same as `MH_KIND_COLLECT`
/// (MEASURED `mhvar.char` = `java.lang.Character` on HotSpot 25).
    ...
    match component {
        ...
        _ => {
            ...
            for (i, v) in vals.iter().enumerate() {
                ...
                let ov = box_collector_element(
                    ctx,
                    *v,
                    component,
                    collector_call_site_prim(call_site, start_index + i),
                );
...
///
/// **Do not widen this one to the combinator surface.** This predicate also
/// gates [`invoke_reference_cast_refusal`], which reads `MH_DESC` -- the RAW
/// leaf descriptor, by that function's own comment a few lines below -- not
/// `type`. An adapter's `MH_DESC` is its LEAF member's descriptor, not its
/// adapted one (`filterArguments(cat, 1, intToString)` keeps `cat`'s `MH_DESC`
/// while presenting a different parameter list), so adding adapter kinds HERE
/// would make that function compare a caller's argument against the wrong
/// signature and refuse correct calls. [`kind_has_authoritative_adapted_type`]
/// is the measured, `type`-reading counterpart for the two checks that can
/// safely include them.
...
/// [`kind_has_authoritative_type`], widened to the `MethodHandles` combinator
/// surface, for the two checks that read `type` (via [`mh_declared_descriptor`])
/// rather than `MH_DESC`: `exact_call_site_refusal`'s strict `invokeExact`
/// signature check and `invoke_narrowing_arg_refusal`'s primitive-narrowing
/// refusal. `invoke_reference_cast_refusal` stays on the narrower predicate --
/// see its comment there for why `MH_DESC` makes the adapters unsafe for THAT
/// check specifically.
///
/// **The `MethodHandles` combinator surface is IN, now that it has been run.**
/// `MhCombinatorSweep.java` swept 56 rows of exactly this surface through
/// `invoke` and came back byte-identical to HotSpot in both modes
/// (`methodhandles-combinator-surface-16-of-56-RETIRED-20260917.md`), which
/// establishes the adapted `type()` VALUE is right for every kind below.
/// `MhAdapterInvokeExactSweep.java` is the measurement `kind_has_authoritative_
/// type`'s comment said nobody had run: the same combinator shapes, called
/// through `invokeExact` instead of `invoke`, both on a matching call site
/// (must still dispatch and answer the same value) and a deliberately
/// mismatched one (must now throw `WrongMethodTypeException`, byte-identical
/// to HotSpot). Clean in both modes, plus `CodegenFrameworkSmoke.java`'s
/// Groovy rows (closures compile through `IndyInterface`, which stacks
/// `insert` / `asType` / `asCollector` on real call sites), the Netty and
/// Spring corpora, and the three `regression-suite` arms, all unaffected --
/// see the retired L5-residuals write-up for the full run.
///
/// What is added: the eleven kinds `register_method_handle_combinator_extras_
/// bridge` and `MethodHandles.permuteArguments`/`collectArguments` build, each
/// of which either computes its `type` from the target's own ADAPTED
/// descriptor before `alloc_method_handle` (`FILTER`, `RETURN_FILTER`,
/// `COLLECT_ARGS`, `DROP`, `GUARD`, `PERMUTE`) or overwrites `type` right after
/// allocation with a freshly built adapted `MethodType` (`INSERT`, `COLLECT`,
/// `SPREAD`, `FOLD`, `CATCH`) -- never the leaf's raw `MH_DESC`, which is what
/// made the original ARITY check unsafe. `TRY_FINALLY` and the `LOOP_*` family
/// are deliberately still OUT: neither sweep above exercises them through
/// `invokeExact`, so there is no measurement backing them yet, and
/// `MhAdapterInvokeExactSweep`'s two `[CratonVM-only]` rows exist to catch this
/// list growing to include them by accident. Both checks that use this
/// predicate stay behind `CRATONVM_MH_STRICT_INVOKEEXACT`, so a wrong row in
/// either reverts with the existing kill switch -- no new one was added for
/// this widening.
fn kind_has_authoritative_adapted_type(kind: i32) -> bool {
    kind_has_authoritative_type(kind)
        || matches!(
            kind,
            MH_KIND_INSERT
                | MH_KIND_COLLECT
...
fn exact_call_site_refusal(
    ...
    if !mh_strict_exact_signature() || !kind_has_authoritative_adapted_type(kind) {
        return None;
    }
...
fn invoke_reference_cast_refusal(
    ...
    //
    // `kind_has_authoritative_type` stays on the narrow, direct-kinds-only
    // list for exactly the reason above -- this function reads `MH_DESC`, and
    // an adapter's `MH_DESC` is its leaf's, not its own. `exact_call_site_
    // refusal` and `invoke_narrowing_arg_refusal` read `type` instead (via
    // `mh_declared_descriptor`), which IS adapted for the combinator surface,
    // so they use the wider `kind_has_authoritative_adapted_type`. Widening
    // THIS gate to match would reintroduce the false refusal described above.
...
/// Narrow `value` from the return type dispatch produced to the return type
/// the handle DECLARES — the JLS 5.1.3 primitive narrowing conversions,
/// which is the piece [`widen_return_value`] deliberately excludes.
///
/// **Why this is safe beside a widening-only sibling.** A stamped `type`
/// NARROWER than `MH_DESC`'s return is only ever reachable through
/// `explicitCastArguments`: `asType` refuses a narrowing conversion eagerly,
/// at `mh_astype_refusal` (before any handle carrying one is ever produced),
/// so a handle that reaches `box_return_against_target` with a narrowing
/// return cannot have come from `asType`. `explicitCastArguments`'s own
/// contract is exactly JLS 5.1.3-style narrowing (an inserted cast), not
/// `asType`'s widening-only rule — see `mh_explicit_cast_refusal`'s doc
/// comment. Measured (`x.explicitCastReturn`,
/// `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`):
/// `explicitCastArguments(len, (String)byte)` on a 6-char string answered
/// `0` — the un-narrowed `int` boxed against the LEAF descriptor, which the
/// call site's own unboxing (expecting a `Byte`) then read as a different
/// wrapper's layout — where HotSpot 25 answers `6`.
///
/// Uses Rust's `as` casts throughout: for the integral truncations that
/// matches the JVM's `i2b`/`i2s`/`i2c`/`l2i` bytecode exactly, and for the
/// float/double-to-integral conversions (`f2i`/`f2l`/`d2i`/`d2l`) Rust's
/// saturating-cast semantics (NaN → 0, out-of-range → the bound) match the
/// JVM's specified `f2i`/`d2i`-family behaviour exactly too — this is not a
/// coincidence, both are defined the same way.
fn narrow_return_value(value: Value, from_ret: u8, to_ret: u8) -> Option<Value> {
    ...
    Some(match (from_ret, to_ret) {
        (b'I' | b'S' | b'C' | b'J' | b'F' | b'D', b'B') => {
            Value::Int(as_i32(value)? as i8 as i32)
        }
        (b'I' | b'C' | b'J' | b'F' | b'D', b'S') => Value::Int(as_i32(value)? as i16 as i32),
        (b'I' | b'S' | b'B' | b'J' | b'F' | b'D', b'C') => {
            Value::Int(as_i32(value)? as u16 as i32)
        }
        (b'J' | b'F' | b'D', b'I') => Value::Int(as_i32(value)?),
        (b'F', b'J') => match value {
            Value::Float(f) => Value::Long(f as i64),
            _ => return None,
        },
        (b'D', b'J') => match value {
            Value::Double(d) => Value::Long(d as i64),
            _ => return None,
        },
        (b'D', b'F') => match value {
...
/// Apply [`widen_return_value`]/[`narrow_return_value`] to a dispatch result
/// and re-box it against the TARGET descriptor rather than the leaf one.
///
...
/// boxing whenever the target is absent, unparseable, or not a widening or
/// narrowing this pair of functions covers.
fn box_return_against_target(
    ...
    match result {
        Ok(Some(value)) => {
            match widen_return_value(value, from, to).or_else(|| narrow_return_value(value, from, to)) {
...
pub fn register_t4_method_handle_invoke(r: &mut NativeMethodRegistry) {
    ...
    r.register_with_kind(
        ...
        |ctx, args| {
            ...
            // `kind_has_authoritative_adapted_type` for exactly which kinds it
            // is allowed to speak about -- the `MethodHandles` combinator
            // surface is in, MEASURED against Groovy/Netty/Spring
            // (the retired L5-residuals write-up); `TRY_FINALLY` and the
            // `LOOP_*` family stay out, deliberately, still unmeasured.
            let kind = match ctx.get_field(this, MH_KIND) {
                Value::Int(k) => k,
                _ => MH_KIND_VIRTUAL,
            };
...
pub fn register_t28_method_handle_completeness(r: &mut NativeMethodRegistry) {
    ...
    r.register(
        ...
        |ctx, args| {
            ...
            if let Some(refusal) = mh_catch_exception_type_refusal(ctx, catch_type, handler) {
                return Err(refusal);
            }
...
fn lookup_unreflect(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let owner = if is_static {
        {
            let m = member_clazz_mirror(ctx, method_obj);
            mh_static_owner_of_mirror(&*ctx, m)
        }
    } else {
        None
    };
...
fn lookup_unreflect_getter(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let owner = if (modifiers & ACC_STATIC) != 0 {
        let m = member_clazz_mirror(ctx, field_obj);
        mh_static_owner_of_mirror(&*ctx, m)
    } else {
        None
    };
...
fn lookup_unreflect_var_handle(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    if (modifiers & ACC_STATIC) != 0 {
        let owner = mh_static_owner_of_mirror(&*ctx, class_mirror);
        let vh = alloc_static_var_handle(ctx, &class_name, &field_name, &field_desc, owner);
...
fn mhs_permute_arguments(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // Every element of `reorder` must be a nonnegative index less than
    // `newType.parameterCount()` — javadoc's own words. Measured
    // (`p.permuteBadIndex`, `lane-methodhandles-combinator-sweep-20260917-RETIRED-20260921.md`'s
    // residual list): a 2-parameter `newType` with a `5` in the reorder array
    // was silently accepted where HotSpot 25 throws `IllegalArgumentException`
    // at THIS call, before any invocation — an out-of-range index would
    // otherwise read past the target's real argument list at dispatch time.
    if let Some((new_params, _)) = split_descriptor_params(&new_desc) {
        let n = new_params.len() as i32;
        let len = ctx.array_length(reorder_arr);
        for i in 0..len {
            if let Value::Int(idx) = ctx.get_array_element(reorder_arr, i) {
                if idx < 0 || idx >= n {
                    return Err(cratonvm_types::error::RuntimeError::IllegalArgumentException {
                        message: format!("index out of bounds for {n} parameters: {idx}"),
                    }
                    .into());
                }
            }
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // F29-1 NOMINATION 2, closed — the CALL SITE settles an `Object[]`
    // collector's wrapper class once the component cannot.
    //
    // MEASURED (`CollBox.java`): `coll.type()` is `(Object)Object` for every
    // one of the six rows below, so nothing about the collector or the
    // target's `MethodType` can separate them — only the descriptor the
    // caller actually wrote at the call site does.

    #[test]
    fn the_call_site_settles_a_generic_collectors_wrapper_class() {
        // `coll.invoke(aChar)` — call site `(C)Ljava/lang/Object;`.
        assert_eq!(
            collector_call_site_prim(Some("(C)Ljava/lang/Object;"), 0),
            Some("C")
        );
        // `coll.invoke(aBoolean)`.
        assert_eq!(
            collector_call_site_prim(Some("(Z)Ljava/lang/Object;"), 0),
            Some("Z")
        );
        // `coll.invoke(aByte)` / `coll.invoke(aShort)` / `coll.invoke(anInt)`.
        assert_eq!(
            collector_call_site_prim(Some("(B)Ljava/lang/Object;"), 0),
            Some("B")
        );
        assert_eq!(
            collector_call_site_prim(Some("(S)Ljava/lang/Object;"), 0),
            Some("S")
        );
        assert_eq!(
            collector_call_site_prim(Some("(I)Ljava/lang/Object;"), 0),
            Some("I")
        );
    }

    /// No armed call site (a miss on `poly_call_site`, or a dispatch door that
    /// never arms one — `invokeWithArguments`, an adapter re-entry) must fall
    /// back to `None`, exactly the pre-NOMINATION-2 behaviour, never a wrong
    /// guess.
    #[test]
    fn a_missing_call_site_settles_nothing() {
        assert_eq!(collector_call_site_prim(None, 0), None);
        // Out of range: fewer call-site params than the index asks for.
        assert_eq!(collector_call_site_prim(Some("()Ljava/lang/Object;"), 0), None);
        // A reference parameter at the call site carries no primitive either.
        assert_eq!(
            collector_call_site_prim(Some("(Ljava/lang/String;)Ljava/lang/Object;"), 0),
            None
        );
    }

    /// Multiple leading (non-collected) parameters: the index into the call
    /// site must be the ARGUMENT's own position, not always 0.
    #[test]
    fn the_call_site_index_tracks_the_collected_arguments_position() {
        assert_eq!(
            collector_call_site_prim(Some("(ILjava/lang/Object;C)Ljava/lang/Object;"), 0),
            Some("I")
        );
        assert_eq!(
            collector_call_site_prim(Some("(ILjava/lang/Object;C)Ljava/lang/Object;"), 2),
            Some("C")
        );
        assert_eq!(
            collector_call_site_prim(Some("(ILjava/lang/Object;C)Ljava/lang/Object;"), 1),
            None
        );
    }

    /// `box_collector_element`'s three-way priority, end to end: component
    /// first, call site second, `Value` variant last. `char`/`int` share a
    /// `Value` variant and must still separate once either of the first two
    /// is available — this is the same defect shape as
    /// `char_and_int_separate_on_the_component_though_the_variant_cannot`,
    /// one door over.
    #[test]
    fn box_collector_element_prefers_component_then_call_site_then_variant() {
        let mut ctx = MockNativeContext::new();
        let same_bits = Value::Int(97);

        // Component settles it: call-site value must not override a
        // wrapper-typed array (`Character[]` still wins over an `int` call
        // site, which cannot arise on HotSpot anyway but must not corrupt the
        // component's own answer if it did).
        let boxed = box_collector_element(&mut ctx, same_bits, "Ljava/lang/Character;", Some("I"));
        assert_eq!(
            ctx.class_name_of_id(ctx.class_id_of_object(match boxed {
                Value::Object(Some(o)) => o,
                _ => panic!("expected a boxed object"),
            })),
            Some("java/lang/Character".to_string())
        );

        // No component (`Object[]`), call site says `char`.
        let boxed = box_collector_element(&mut ctx, same_bits, "Ljava/lang/Object;", Some("C"));
        assert_eq!(
            ctx.class_name_of_id(ctx.class_id_of_object(match boxed {
                Value::Object(Some(o)) => o,
                _ => panic!("expected a boxed object"),
        ...
        // No component, no call site: falls back to the `Value` variant,
        // exactly the pre-NOMINATION-2 answer.
        let boxed = box_collector_element(&mut ctx, same_bits, "Ljava/lang/Object;", None);
        assert_eq!(
            ctx.class_name_of_id(ctx.class_id_of_object(match boxed {
                Value::Object(Some(o)) => o,
                _ => panic!("expected a boxed object"),
    ...
    #[test]
    fn varhandle_instance_refusal_type_checks() {
        let mut ctx = MockNativeContext::new();
        let recv = match ctx.new_object("Box").unwrap() {
            Some(Value::Object(Some(o))) => o,
            _ => unreachable!(),
        };
        let cid = ctx.class_id_of_object(recv).as_u32();
        let bad_str = match ctx.new_object("java/lang/String").unwrap() {
            Some(Value::Object(Some(o))) => o,
            _ => unreachable!(),
        };
        let int_meta = VarHandleMeta {
            kind: VH_KIND_INSTANCE,
        ...
        // 1. Primitive field receiving non-wrapper reference must refuse with WrongMethodTypeException
        let bad_prim = vh_instance_refusal(
            &mut ctx,
            Some(&int_meta),
            recv,
            Some(Value::Object(Some(bad_str))),
        );
        assert!(
            bad_prim.is_some(),
            "non-wrapper reference into primitive field must refuse"
        );

        // 2. Matching receiver with valid primitive value must pass
        let good_prim = vh_instance_refusal(
            &mut ctx,
            Some(&int_meta),
            recv,
            Some(Value::Int(42)),
        ...
        assert!(
            good_prim.is_none(),
            "valid primitive value into matching receiver must not refuse"
        );

        // 3. None meta or empty class name must not refuse
        assert!(
            vh_instance_refusal(&mut ctx, None, recv, Some(Value::Int(42))).is_none(),
            "None meta must not refuse"
        );
        let unclassed_meta = VarHandleMeta {
            kind: VH_KIND_INSTANCE,
            class_name: String::new(),
            field_name: "i".to_string(),
            field_desc: "I".to_string(),
        ...
        assert!(
            ...
            .is_none(),
            "unclassed meta must not refuse"
        );
    }
...
mod segment_heap_tests {
    ...
    // -----------------------------------------------------------------------
    // NOM F35-3 — `SegmentVarHandle` heap access must not assume `byte[]`.
    //
    // `segment_heap_get`/`segment_heap_set` used to read/write `base` one BYTE
    // per `get_array_element`/`set_array_element` call regardless of its real
    // element type, which is correct only for `byte[]`. A `VarHandle` bound to
    // `MemorySegment.ofArray(new int[4])` read/wrote INT elements at BYTE
    // granularity: `get_array_element(base, addr)` on an `int[]` reads whole
    // element `addr`, not byte `addr` of element `addr / 4`. Wrong data with
    // no crash, so nothing short of comparing against the oracle showed it.
    // `heap_bytes_read`/`heap_bytes_write` (`panama.rs`, shared per that
    // file's own doc comment) fix this by deriving the element width from
    // `ctx.heap_element_type_of(base)` and addressing byte-within-element.
    // -----------------------------------------------------------------------

    /// A 4-byte write at byte offset 4 of an `int[4]` must land entirely in
    /// element 1, leaving elements 0 and 2 untouched. Pre-fix, `addr=4` would
    /// have been read as element INDEX 4 — out of bounds on a 4-element array
    /// — and the write would have silently done nothing at all (the old bounds
    /// check compared `addr` against the ELEMENT count, not the byte length).
    #[test]
    fn segment_heap_set_writes_a_whole_element_not_a_byte_index() {
        let mut ctx = mock_ctx();
        let arr = ctx.new_array(cratonvm_types::ArrayElementType::Int, 4);
        assert!(segment_heap_set(&mut ctx, arr, 4, 4, 0x1122_3344));
        assert_eq!(ctx.get_array_element(arr, 0), Value::Int(0));
        assert_eq!(ctx.get_array_element(arr, 1), Value::Int(0x1122_3344u32 as i32));
        assert_eq!(ctx.get_array_element(arr, 2), Value::Int(0));
        assert_eq!(ctx.get_array_element(arr, 3), Value::Int(0));
    }

    /// Round-trip through `segment_heap_get`, and — the part the old
    /// byte-indexed body could not do at all — a SUB-ELEMENT read: byte 1 of a
    /// little-endian `int` is its second-lowest byte, not `get_array_element`
    /// applied to index 1.
    #[test]
    fn segment_heap_get_reads_a_byte_within_an_element_little_endian() {
        let mut ctx = mock_ctx();
        let arr = ctx.new_array(cratonvm_types::ArrayElementType::Int, 4);
        ctx.set_array_element(arr, 0, Value::Int(0x0102_0304));
        // Pre-fix this called `get_array_element(arr, 1)` — a DIFFERENT
        // element, still zero here — and truncated it to a byte: 0, not 0x03.
        assert_eq!(segment_heap_get(&mut ctx, arr, 1, 1), Some(0x03));
        assert_eq!(segment_heap_get(&mut ctx, arr, 0, 4), Some(0x0102_0304));
    }

    /// The bounds check is in BYTES, matching the general reader: a 4-element
    /// `int[]` is 16 bytes, so a 4-byte access at byte offset 16 is out of
    /// range and must refuse rather than silently succeed against a
    /// mismeasured (element-count) bound.
    #[test]
    fn segment_heap_access_bounds_check_is_in_bytes_not_elements() {
        let mut ctx = mock_ctx();
        let arr = ctx.new_array(cratonvm_types::ArrayElementType::Int, 4);
        assert_eq!(segment_heap_get(&mut ctx, arr, 12, 4), Some(0));
        assert_eq!(segment_heap_get(&mut ctx, arr, 16, 4), None);
        assert!(segment_heap_set(&mut ctx, arr, 12, 4, 7));
        assert!(!segment_heap_set(&mut ctx, arr, 16, 4, 7));
    }

    /// The pre-existing `byte[]` case is unchanged — the negative control that
    /// keeps this fix from being a regression for the common case it replaced.
    #[test]
    fn segment_heap_access_still_works_for_a_plain_byte_array() {
        let mut ctx = mock_ctx();
        let arr = ctx.new_array(cratonvm_types::ArrayElementType::Byte, 8);
        assert!(segment_heap_set(&mut ctx, arr, 0, 4, 0x0102_0304));
        // Little-endian: `ofArray(new byte[8]).set(JAVA_INT_UNALIGNED, 0,
        // 0x01020304)` leaves `[4, 3, 2, 1, 0, 0, 0, 0]` (F35-1 §4.3, measured).
        assert_eq!(ctx.get_array_element(arr, 0), Value::Int(4));
        assert_eq!(ctx.get_array_element(arr, 1), Value::Int(3));
        assert_eq!(ctx.get_array_element(arr, 2), Value::Int(2));
        assert_eq!(ctx.get_array_element(arr, 3), Value::Int(1));
        assert_eq!(segment_heap_get(&mut ctx, arr, 0, 4), Some(0x0102_0304));
    }
}
