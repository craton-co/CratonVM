...
// `docs/jdk-only/G31-1-astype-and-the-verifier-that-was-never-asked-20260817.md`).
//
...
pub(crate) fn register_method_handles_constant_bridge(r: &mut NativeMethodRegistry) {
    ...
    r.register(
        ...
        |ctx, args| {
            ...
            // `lane-methodhandles-combinator-sweep-20260917.md`): CratonVM
...
pub(crate) fn register_method_handle_combinator_extras_bridge(r: &mut NativeMethodRegistry) {
    ...
    r.register(
        ...
        |ctx, args| {
            ...
            // (`p.insertTooMany`, `lane-methodhandles-combinator-sweep-
            // 20260917.md`): three values inserted at position 1 into a
...
/// `.type()` assertion in the sweep this fixes (`docs/jdk-only/
/// lane-methodhandles-combinator-sweep-20260917.md` names none), so a
...
/// `docs/jdk-only/lane-methodhandles-combinator-sweep-
/// 20260917.md`. Real JDK bytecode for all five builds `LambdaForm`/
...
/// wiring bug `lane-methodhandles-combinator-sweep-20260917.md` §"Why the
...
/// `MethodHandles.countedLoop(iterations, init, body)` — `docs/jdk-only/
/// lane-methodhandles-combinator-sweep-20260917.md`'s "whole JDK 9
...
/// (`k.arrayLength`, `lane-methodhandles-combinator-sweep-20260917.md`)
...
fn make_fold_adapter(
    ...
    // (`p.foldPrefix.type`, `lane-methodhandles-combinator-sweep-
    // 20260917.md`): `foldArguments((String,String,String)String,
...
/// `lane-methodhandles-combinator-sweep-20260917.md`): a handler declared
...
/// `lane-methodhandles-combinator-sweep-20260917.md`):
...
fn mhs_permute_arguments(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // (`p.permuteBadIndex`, `lane-methodhandles-combinator-sweep-20260917.md`'s
