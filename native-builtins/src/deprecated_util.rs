...
///
/// `WORKER-3-NOTE-6` (2026-08-22) measured a SEPARATE dispatch defect where a
/// `(AbstractStringBuilder,Void)V` call landed on this `(StringBuilder)V` body
/// instead of `native_string_init_abstract_string_builder`. RE-MEASURED
/// 2026-09-20 (`--dump-native-registry`, `--nojit
/// CRATONVM_DISABLE_INTRINSICS=1`, builder registrations skipped): the
/// `(AbstractStringBuilder,Void)V` triple now shows `invocations: 2` and this
/// `(StringBuilder)V` triple shows `invocations: 0` for the same
/// `sb.toString()` call — the dispatch now lands on the RIGHT body,
/// whatever fixed it did so incidentally. This function is kept correct for
/// either overload regardless, since nothing here depends on which one runs.
...
///
/// docs/jdk-only/H19-2-the-empty-container-was-the-only-one-still-fabricating-20260821.md
...
fn native_locale_get_iso3_language(
    ...
    // `java.util.Locale.getISO3Language`, reproduced end to end. The tail used
    // to be `if the table has no row, hand back the 2-letter code`, and that is
    // the one shape the real method REFUSES:
    //
    // ```java
    //   String lang = baseLocale.getLanguage();
    //   if (lang.length() == 3) return lang;          // already ISO 639-2
    //   else if (lang.isEmpty()) return "";           // Locale.ROOT
    //   String language3 = getISO3Code(lang, LocaleISOData.isoLanguageTable);
    //   if (language3 == null) throw new MissingResourceException(...);
    // ```
    //
    // MEASURED, HotSpot 25.0.4+7 (`apps/probes/UulrLocaleDisplaySweep`):
    //
    // ```text
    //   Locale.of("qq","ZZ").getISO3Language()   MissingResourceException
    //                                            "Couldn't find 3-letter
    //                                             language code for qq"
    //   CratonVM                                 "qq"
    // ```
    //
    // The old answer is the quiet direction: a caller that writes the result
    // into an ISO 639-2 field gets a two-letter string that every consumer of
    // that field will mis-parse, with nothing to say where it came from. The
    // 3-letter pass-through above it is the other arm the old tail was missing
    // -- `Locale.of("eng")` is a legal locale and its ISO3 language is itself,
    // which the 2->3 table cannot express.
    if lang2.is_empty() {
        let s = ctx.create_string("");
        return Ok(Some(Value::Object(Some(s))));
    }
    if lang2.len() == 3 {
        let s = ctx.create_string(&lang2);
        return Ok(Some(Value::Object(Some(s))));
    }
    let iso3 = iso2_to_iso3(&lang2);
    if iso3.is_empty() {
        let exc =
            crate::try_alloc_concurrent_synthetic(ctx, "java/util/MissingResourceException", 3)?;
        let msg = ctx.create_string(&format!(
            "Couldn't find 3-letter language code for {lang2}"
        ));
...
pub(crate) fn register_deprecated_util_natives(r: &mut NativeMethodRegistry) {
    ...
    // `docs/jdk-only/E23-1-synthetic-jdk-nosuchmethoderror-census.md` §6):
    //
...
mod tests {
    ...
    fn test_date_fields_to_millis_julian_before_cutover_matches_proleptic_gregorian_equivalent() {
        // Regression test for `bug-h2-suite-residual-fail-triage-FIXED.md`'s
    ...
    /// An unmapped 2-letter code is a REFUSAL, not a pass-through.
    ///
    /// This test asserted `"xx"` until 2026-09-20, with the comment "iso2_to_iso3
    /// returns "" so we fall back to the 2-letter code" — a test written from the
    /// implementation, which therefore could not see that the implementation was
    /// wrong. `java.util.Locale.getISO3Language` throws `MissingResourceException`
    /// for a code its table does not carry; measured on HotSpot 25.0.4+7 in
    /// `apps/probes/UulrLocaleDisplaySweep`, `Locale.of("qq","ZZ")` and an
    /// 8-letter tag both refuse.
    #[test]
    fn test_locale_get_iso3_language_unknown_refuses() {
        let reg = make_registry();
        let mut ctx = MockNativeContext::new();
        let locale = try_alloc_concurrent_synthetic(&mut ctx, "java/util/Locale", 4).unwrap();
        ...
        assert!(
            ...
            "an unmapped language code must refuse, got {res:?}"
        );
    }

    /// A code that is ALREADY three letters is its own answer — the arm the
    /// 2→3 table cannot express, and the one the old tail turned into a
    /// pass-through by accident rather than on purpose.
    #[test]
    fn test_locale_get_iso3_language_three_letter_passthrough() {
        let reg = make_registry();
        let mut ctx = MockNativeContext::new();
        let locale = try_alloc_concurrent_synthetic(&mut ctx, "java/util/Locale", 4).unwrap();
        let lang = ctx.create_string("haw");
        ctx.set_field(locale, 0, Value::Object(Some(lang)));
