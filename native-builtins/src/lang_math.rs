...
pub(crate) fn register_math_natives(registry: &mut NativeMethodRegistry, class: &str) {
    ...
    // W7-54-strictmath-fdlibm-family-RETIRED-20260917.md for the rest.
    //
...
pub(crate) fn register_wrapper_natives(registry: &mut NativeMethodRegistry) {
    ...
    // W7-98 N2: isDigit/isLetter/isWhitespace/isUpperCase/isLowerCase/
    // toUpperCase/toLowerCase/isLetterOrDigit are Unicode-table classifiers
    // (plus, historically, the S111r15 JIT workaround on the (I)I forms —
    // dead now that `jit/src/x64/driver.rs`'s `has_dispatch` covers
    // direct-call-only methods, see that module's own comment). Real-JDK
    // mode has real `CharacterData` bytecode for every one of these and
    // needs no native; moved to
    // `register_character_classification_natives`, called ONLY from
    ...
    registry.register(
        ...
        // docs/jdk-only/W7-95a-string-code-point-family.md
    ...
    // -----------------------------------------------------------------------
    // `digit(CI)I` and `getNumericValue(C)I` are Unicode-table classifiers —
    // moved to `register_character_classification_natives` (W7-98 N2, see
    // the comment above the Character block near the top of this function).
    // `forDigit` stays: closed-form, no Unicode table, correct in both modes.
    registry.register(
        "java/lang/Character",
        "forDigit",
        "(II)C",
    ...
    // Java 21: Character emoji detection methods — moved to
    // `register_character_classification_natives` (W7-98 N2): these are JDK
    // 21+ methods with real bytecode and real `CharacterData` backing, so
    // real-JDK mode needs no native for them either. See that function.
...
/// `java.lang.Character`'s Unicode-table classifiers — SYNTHETIC-JDK ONLY.
///
/// W7-98 N2 (`W7-98-character-unicode.md`). Every
/// method registered here answers a Unicode classification or case mapping
/// that the real JDK implements as plain bytecode over `CharacterData`'s
/// generated tables — no native required. `Character.getType`,
/// `isSpaceChar`, `toTitleCase`, `isAlphabetic`, `isDefined`,
/// `getDirectionality`, `isMirrored` and the identifier predicates are
/// never registered as natives and match HotSpot exactly on real-JDK mode
/// (measured over all 65,536 BMP code points, hot and cold); this function
/// used to shadow the same chain for ten more methods with a Rust
/// reimplementation of the Unicode database — a DIFFERENT database, wrong on
/// between 3 and 2,153 BMP code points per method before the generated-table
/// fixes in this file closed most of that gap.
///
/// The generated tables (`JAVA_LETTER_RUNS`, `JAVA_DIGIT_RUNS`,
/// `JAVA_TO_UPPER_RUNS`, the emoji `*_RUNS` tables, …) are correct — verified
/// against HotSpot 25 over every code point — so keeping this registration in
/// synthetic mode (which fabricates `java/lang/Character` with no
/// `CharacterData` bytecode at all, per `class_manager.rs`) is not a
/// regression there. It is real-JDK mode that gains: this native block no
/// longer registers here, so `register_essential_natives_with_shims` (shared
/// by both modes) does not touch `java/lang/Character`'s classification
/// triples, and the real, loaded, runnable bytecode answers them directly —
/// which also closes residuals no BMP table can, e.g. supplementary-plane
/// `isUpperCase`/`isLowerCase`/`toUpperCase`/`toLowerCase`.
///
/// The (I)I `toLowerCase`/`toUpperCase` registrations used to also serve as a
/// JIT workaround (S111r15: a direct-call-only method skipped
/// `set_jit_thread`, so `Character.toLowerCase`'s compiled code returned 0
/// after warm-up). That root cause is fixed in `jit/src/x64/driver.rs`
/// (`has_dispatch` now accounts for `direct_calls`), re-measured with
/// `CharWarm.java` at 6,000,000 invocations per probe character with zero
/// cold→hot mismatches, so the workaround is not needed to keep this split
/// correct.
pub(crate) fn register_character_classification_natives(registry: &mut NativeMethodRegistry) {
    let __prev_cat = registry.current_category();
    registry.set_category(cratonvm_native_api::NativeKind::Intrinsic);
    registry.register(
        "java/lang/Character",
    ...
    // Java 21 emoji detection methods. W7-95(C1): all five bodies used to be
    // hand-written coarse ranges ("basic emoji ranges") wrong on 2,746 code
    // points measured against HotSpot 25 over `0..=0x10FFFF`; the tables
    // below are the JDK's own answers, generated the same way as the other
    // classifiers in this function.
...
pub(crate) fn native_math_ieee_remainder(
    ...
    // that was not a last-ULP story. W7-54-strictmath-fdlibm-family-RETIRED-20260917.md.
...
///
/// Round 9 wave 11 (collect11): the per-VM store is an [`IntegerCacheTable`]
/// of ATOMIC slots, leaked once per VM identity, so a hit is read without a
/// lock or a hash (see [`integer_cache_table`]). The map below is now only the
/// directory the tables are filed in; it is taken on a VM switch or a cold VM,
/// never on the steady-state `valueOf` path.
type ScopedIntegerCache = std::collections::HashMap<usize, &'static IntegerCacheTable>;

/// The slot array behind both atomic box tables below.
///
/// A slot holds the raw address of the canonical box (`ObjectRef::as_ptr`),
/// `0` while unfilled. Fills are a `compare_exchange(0, obj)` with `Release`
/// (the box's header and `value` were written before it), reads are
/// `Acquire`, and the GC remap is a `compare_exchange(old, new)`, so a fill and
/// a remap can never lose each other. Two threads that miss together still
/// agree on ONE canonical instance: the CAS loser returns the winner's box.
///
/// Sized once and never resized, and the tables holding one are leaked
/// (`&'static`) and never removed from their directory — that pair of
/// properties is what lets [`INTEGER_CACHE_LAST`] / [`LONG_CACHE_LAST`] hand
/// out a reference with no lock held, and what makes an index computed against
/// a latched bound always in range.
///
/// Round 9 wave 15 (heapapi): extracted from `IntegerCacheTable` so the `Long`
/// twin could have the same treatment without a second copy of the unsafe
/// address round-trip. The `low` offset and the bound live in the TABLES, which
/// differ; everything below this line is identical for both and so is written
/// once. Indices arrive as `i64` because every caller computes `value - low`
/// from a `value` that is unbounded on at least one side: `i32::MAX - (-128)`
/// overflows an `i32`, and a negative index must MISS, not panic a debug build.
struct AtomicBoxSlots {
    slots: Box<[std::sync::atomic::AtomicUsize]>,
}

impl AtomicBoxSlots {
    /// `len` empty slots, or `None` when the backing store cannot be reserved
    /// (the caller falls back to leaving the values uncached).
    fn try_new(len: usize) -> Option<Self> {
        let mut slots: Vec<std::sync::atomic::AtomicUsize> = Vec::new();
        slots.try_reserve_exact(len).ok()?;
        slots.resize_with(len, || std::sync::atomic::AtomicUsize::new(0));
        Some(Self {
            slots: slots.into_boxed_slice(),
        })
    }

    /// How many slots the table has — the latched bound, for the tables whose
    /// LENGTH is the bound (`IntegerCache.high`).
    fn len(&self) -> usize {
        self.slots.len()
    }

    /// The slot at `idx`, or `None` outside the table.
    #[inline]
    fn at(&self, idx: i64) -> Option<&std::sync::atomic::AtomicUsize> {
        if idx < 0 {
            return None;
        }
    ...
    /// The canonical box at `idx` if its slot is filled. Never populates.
    #[inline]
    fn get(&self, idx: i64) -> Option<cratonvm_types::ObjectRef> {
        let raw = self.at(idx)?.load(std::sync::atomic::Ordering::Acquire);
        if raw == 0 {
            return None;
        }
        // SAFETY: a non-zero slot is only ever written from a live
        // `ObjectRef::as_ptr()` (`install`) or a GC pointer-map target
        // (`remap`); both are non-null, aligned heap object addresses, and the
        // slot is a GC root, so the object is live.
        Some(unsafe { cratonvm_types::ObjectRef::from_raw(raw as *mut u8) })
    }

    /// Publish `obj` at `idx` unless another thread won the race; returns
    /// whichever box is canonical. An index outside the table (impossible for a
    /// caller that checked the bound) returns `obj` uncached, as the pre-table
    /// code did for an out-of-range index.
    fn install(&self, idx: i64, obj: cratonvm_types::ObjectRef) -> cratonvm_types::ObjectRef {
        let Some(slot) = self.at(idx) else {
            return obj;
        };
        match slot.compare_exchange(
            ...
            Ok(_) => obj,
            // SAFETY: as in `get` -- a non-zero slot is a live box address.
            Err(existing) => unsafe { cratonvm_types::ObjectRef::from_raw(existing as *mut u8) },
        }
    }

    /// Every filled slot, for the GC root scan. Walks the WHOLE table, so a
    /// widened `IntegerCache.high` is reported too.
    fn scan(&self, out: &mut Vec<cratonvm_types::ObjectRef>) {
        for slot in self.slots.iter() {
            let raw = slot.load(std::sync::atomic::Ordering::Acquire);
            if raw != 0 {
                // SAFETY: a non-zero slot is a live box address (see `get`).
                out.push(unsafe { cratonvm_types::ObjectRef::from_raw(raw as *mut u8) });
            }
        }
    }

    /// Remap every filled slot through `pointer_map`. A
    /// `compare_exchange(old, new)` per slot, so a concurrent fill of an EMPTY
    /// slot is never overwritten (only non-zero slots are remapped) and a slot
    /// is never remapped twice.
    fn remap(&self, pointer_map: &cratonvm_types::PointerMap) {
        for slot in self.slots.iter() {
            let old_addr = slot.load(std::sync::atomic::Ordering::Acquire);
            if old_addr == 0 {
                continue;
            }
...
/// One VM's `IntegerCache` backing store.
///
/// Leaked (`&'static`) and never removed from [`integer_cache`], exactly like
/// the `Vec` it replaced was never removed or resized: that is what lets
/// [`INTEGER_CACHE_LAST`] hand out a reference with no lock held.
struct IntegerCacheTable {
    /// The VM identity this table is filed under.
    vm_identity: usize,
    /// `IntegerCache.high` latched for that VM; `slots.len() == high - LOW + 1`.
    high: i32,
    /// Canonical box addresses for `LOW..=high`, `0` = not yet boxed.
    slots: AtomicBoxSlots,
}

impl IntegerCacheTable {
    /// A fresh, empty table for `-128..=high`, or `None` when the backing store
    /// cannot be reserved (the caller falls back to the JDK default bound).
    fn try_new(vm_identity: usize, high: i32) -> Option<Self> {
        let len = (high as i64 - INTEGER_CACHE_LOW as i64 + 1).max(0) as usize;
        Some(Self {
            vm_identity,
            high,
            slots: AtomicBoxSlots::try_new(len)?,
        })
    }

    /// `value`'s index into [`Self::slots`]. Negative (i.e. a miss) below
    /// `LOW`; the slot lookup itself is the only upper bound, because
    /// `IntegerCache.high` is configurable and the table's LENGTH is where it
    /// was latched.
    #[inline]
    fn index_of(value: i32) -> i64 {
        value as i64 - INTEGER_CACHE_LOW as i64
    }

    /// The canonical box for `value` if its slot is filled. Never populates.
    #[inline]
    fn get(&self, value: i32) -> Option<cratonvm_types::ObjectRef> {
        self.slots.get(Self::index_of(value))
    }

    /// Publish `obj` as the canonical box for `value` unless another thread
    /// won the race; returns whichever box is canonical.
    fn install(&self, value: i32, obj: cratonvm_types::ObjectRef) -> cratonvm_types::ObjectRef {
        self.slots.install(Self::index_of(value), obj)
    }
}

/// One VM's `LongCache` backing store — [`IntegerCacheTable`]'s twin, on the
/// same terms and for the same measured reason.
///
/// Round 9 wave 15 (heapapi). `LONG_CACHE` was an
/// `OrderedPlMutex<HashMap<usize, [Option<ObjectRef>; 256]>>`, so every
/// `Long.valueOf` in `-128..=127` took a mutex and a SipHash probe of the VM
/// identity. MEASURED on the 2026-09-20 `dev` build (`BoxProbe lbox`, 10 M
/// escaping cached boxes into an `Object[1024]`): 605 ms against HotSpot
/// 25.0.4's 12 ms, where the `Integer` twin had already had this treatment.
/// See `retired/perf-a-collection-native-pays-the-checked-heap-api-per-field-20260920-RETIRED-20260921.md`
/// item 3.
///
/// Unlike `IntegerCache.high` the bound here is NOT configurable — JLS §5.1.7
/// fixes it at `-128..=127` and `jdk25src/java.base/java/lang/Long.java` has no
/// property read at all — so there is no `high` field and no bound to latch.
/// The length is a constant, which is why this table can be built on demand by
/// a reader rather than by a bound-resolution step.
struct LongCacheTable {
    /// The VM identity this table is filed under.
    vm_identity: usize,
    /// Canonical box addresses for `-128..=127`, `0` = not yet boxed.
    slots: AtomicBoxSlots,
}

/// `LongCache.low` — `-128`, and NOT configurable (see [`LongCacheTable`]).
const LONG_CACHE_LOW: i64 = -128;
/// `LongCache.high` — `127`, and NOT configurable.
const LONG_CACHE_HIGH: i64 = 127;

impl LongCacheTable {
    /// A fresh, empty table, or `None` when the backing store cannot be
    /// reserved (the caller falls back to leaving the values uncached).
    fn try_new(vm_identity: usize) -> Option<Self> {
        let len = (LONG_CACHE_HIGH - LONG_CACHE_LOW + 1) as usize;
        Some(Self {
            vm_identity,
            slots: AtomicBoxSlots::try_new(len)?,
        })
    ...
    /// The canonical box for `value` if its slot is filled. Never populates.
    /// Out-of-range values miss through [`AtomicBoxSlots::at`] rather than
    /// being range-checked here, exactly as the `Integer` twin does.
    #[inline]
    fn get(&self, value: i64) -> Option<cratonvm_types::ObjectRef> {
        if value > LONG_CACHE_HIGH {
            return None;
        }
        self.slots.get(value - LONG_CACHE_LOW)
    }

    /// Publish `obj` as the canonical box for `value` unless another thread
    /// won the race; returns whichever box is canonical.
    fn install(&self, value: i64, obj: cratonvm_types::ObjectRef) -> cratonvm_types::ObjectRef {
        if value > LONG_CACHE_HIGH {
            return obj;
        }
        self.slots.install(value - LONG_CACHE_LOW, obj)
    }
}

/// The table most recently looked up, so the steady-state `valueOf` (one VM
/// per process) is one atomic load and one compare instead of a mutex and a
/// SipHash probe. MEASURED on the wave-10 binary (`BoxProbe put`, sampler):
/// `canonical_wrapper_if_cached` 13.3 % self plus `hash_one<RandomState>` +
/// `sip::write` 5.6 %, and `integer_cache_high_for`'s mutex inside
/// `jit_integer_value_of_direct`'s 18.6 %.
///
/// Only ever stores a `&'static IntegerCacheTable` (leaked, never freed), so a
/// non-null load is always dereferenceable. A miss (another VM, or none yet)
/// falls back to the directory and repoints this at the answer.
static INTEGER_CACHE_LAST: std::sync::atomic::AtomicPtr<IntegerCacheTable> =
    std::sync::atomic::AtomicPtr::new(std::ptr::null_mut());

/// `vm_identity`'s [`IntegerCacheTable`], or `None` if no `Integer.valueOf`
/// has resolved this VM's bound yet.
#[inline]
fn integer_cache_table(vm_identity: usize) -> Option<&'static IntegerCacheTable> {
    let last = INTEGER_CACHE_LAST.load(std::sync::atomic::Ordering::Acquire);
    // SAFETY: `INTEGER_CACHE_LAST` only ever holds null or a pointer taken
    // from a leaked `&'static IntegerCacheTable`.
    if let Some(table) = unsafe { last.as_ref() } {
        if table.vm_identity == vm_identity {
            return Some(table);
        }
    }
    let table: &'static IntegerCacheTable = integer_cache().lock().get(&vm_identity).copied()?;
    INTEGER_CACHE_LAST.store(
        table as *const IntegerCacheTable as *mut IntegerCacheTable,
...
/// [`INTEGER_CACHE_LAST`]'s `Long` twin (round 9 wave 15, heapapi), on the same
/// terms: only ever a `&'static LongCacheTable`, so a non-null load is always
/// dereferenceable, and a miss falls back to the directory and repoints this.
static LONG_CACHE_LAST: std::sync::atomic::AtomicPtr<LongCacheTable> =
    std::sync::atomic::AtomicPtr::new(std::ptr::null_mut());

/// LOCK LEVEL (lock-discipline ratchet): `Scratch`, like the rest of this
/// family, and taken on the same terms as [`INTEGER_CACHE`] — on a VM switch or
/// a cold VM only, never on the steady-state `valueOf` path, and never held
/// across a `NativeContext` callback.
static LONG_CACHE: std::sync::OnceLock<
    cratonvm_types::lock_order::OrderedPlMutex<
        std::collections::HashMap<usize, &'static LongCacheTable>,
    >,
> = std::sync::OnceLock::new();

fn long_cache() -> &'static cratonvm_types::lock_order::OrderedPlMutex<
    std::collections::HashMap<usize, &'static LongCacheTable>,
> {
...
/// `vm_identity`'s [`LongCacheTable`] if one has been filed, without filing
/// one. [`integer_cache_table`]'s twin — one `Acquire` load and one compare in
/// the steady state (one VM per process).
///
/// **Read-only, and that is deliberate.** The `Integer` twin can file a table
/// because `integer_cache_bound` has to resolve a configurable bound anyway; a
/// `Long` bound is a constant, so nothing forces a cold step, and the readers
/// that would otherwise reach for one ([`long_cache_peek`],
/// [`canonical_wrapper_if_cached`]) must not allocate or take a lock they do
/// not already need. A VM that has not boxed a cached `Long` yet simply misses,
/// which is what it did before this table existed.
#[inline]
fn long_cache_table(vm_identity: usize) -> Option<&'static LongCacheTable> {
    let last = LONG_CACHE_LAST.load(std::sync::atomic::Ordering::Acquire);
    // SAFETY: `LONG_CACHE_LAST` only ever holds null or a pointer taken from a
    // leaked `&'static LongCacheTable`.
    if let Some(table) = unsafe { last.as_ref() } {
        if table.vm_identity == vm_identity {
            return Some(table);
        }
    }
    let table: &'static LongCacheTable = long_cache().lock().get(&vm_identity).copied()?;
    LONG_CACHE_LAST.store(
        table as *const LongCacheTable as *mut LongCacheTable,
        std::sync::atomic::Ordering::Release,
    );
    Some(table)
}

/// `vm_identity`'s [`LongCacheTable`], filing a fresh one on first use.
///
/// Only `native_long_value_of` calls this: filing a table is the step that
/// makes every later read lock-free, and it must happen on the path that also
/// populates slots. Returns `None` only when the backing store cannot be
/// reserved, in which case the caller leaves the value uncached exactly as the
/// pre-table code did when `try_reserve_exact` failed for `Integer`.
fn long_cache_table_or_file(vm_identity: usize) -> Option<&'static LongCacheTable> {
    if let Some(table) = long_cache_table(vm_identity) {
        return Some(table);
    }
    let mut directory = long_cache().lock();
    // Re-check under the lock: two threads can miss the fast path together.
    if let Some(existing) = directory.get(&vm_identity).copied() {
        return Some(existing);
    }
    let leaked: &'static LongCacheTable =
        Box::leak(Box::new(LongCacheTable::try_new(vm_identity)?));
    directory.insert(vm_identity, leaked);
    drop(directory);
    LONG_CACHE_LAST.store(
        leaked as *const LongCacheTable as *mut LongCacheTable,
        std::sync::atomic::Ordering::Release,
    );
    Some(leaked)
}
...
///
/// Fx-hashed (round 9 wave 10b, wire10b; collect10's measurement): the keys
/// are VM identities, so SipHash's DoS resistance buys nothing, and this memo
/// is read on every JIT `Integer.valueOf` (`integer_cache_high_for`).
static INTEGER_CACHE_HIGH: std::sync::OnceLock<
    parking_lot::Mutex<rustc_hash::FxHashMap<usize, i32>>,
> = std::sync::OnceLock::new();

fn integer_cache_high() -> &'static parking_lot::Mutex<rustc_hash::FxHashMap<usize, i32>> {
    INTEGER_CACHE_HIGH.get_or_init(|| parking_lot::Mutex::new(rustc_hash::FxHashMap::default()))
}
...
pub fn integer_cache_high_for(vm_identity: usize) -> Option<i32> {
    // Lock-free when this VM's table exists (collect11): the table is filed
    // under the memo lock with the memo's own `high` (`integer_cache_bound`),
    // so the two cannot disagree.
    if let Some(table) = integer_cache_table(vm_identity) {
        return Some(table.high);
    }
    integer_cache_high().lock().get(&vm_identity).copied()
}

/// The canonical cached `Integer` for `value` in this VM, if its slot is
/// already filled; `None` for a value outside `-128..=IntegerCache.high`, an
/// unresolved bound, or a slot no `valueOf` has filled yet. For the JIT's thin
/// `Integer.valueOf` helper, which must hand back the SAME object the native
/// would (JLS 5.1.7) without paying the native-call funnel for it. Reads the
/// table exactly as `native_integer_value_of`'s fast path does (round 9 wave
/// 10, collect10; landed by wave 10b, wire10b), through
/// [`canonical_wrapper_if_cached`], whose `I` arm already treats the backing
/// store's length as the bound.
pub fn integer_cache_peek(vm_identity: usize, value: i32) -> Option<cratonvm_types::ObjectRef> {
    // The same read `canonical_wrapper_if_cached`'s `I` arm does, minus the
    // descriptor match (collect11).
    integer_cache_table(vm_identity)?.get(value)
}

/// [`integer_cache_peek`]'s `Long` twin: the canonical cached `Long` for
/// `value` (`-128..=127`, not configurable) if its slot is already filled.
///
/// Round 9 wave 15 (heapapi): reads [`LongCacheTable`] directly rather than
/// going through [`canonical_wrapper_if_cached`], for the same reason
/// `integer_cache_peek` does — the descriptor match and the `Value` re-wrap are
/// pure overhead when the caller already knows both. That indirection is the
/// difference between a lock-free load and a `&str` compare plus a match on
/// `(desc, v)`, and this is the JIT helper's hot path.
pub fn long_cache_peek(vm_identity: usize, value: i64) -> Option<cratonvm_types::ObjectRef> {
    if !(LONG_CACHE_LOW..=LONG_CACHE_HIGH).contains(&value) {
        return None;
    }
    long_cache_table(vm_identity)?.get(value)
}
...
fn integer_cache_bound(ctx: &mut dyn NativeContext) -> i32 {
    let scope = ctx.vm_identity();
    // Steady state: the table is filed, lock-free read (collect11).
    if let Some(table) = integer_cache_table(scope) {
        return table.high;
    }
    ...
    // `try_new` is `try_reserve_exact`: a refused huge store falls back to the
    // JDK default bound (see above) rather than aborting the process.
    let fresh = match IntegerCacheTable::try_new(scope, high) {
        Some(table) => table,
        None => {
            high = 127;
            match IntegerCacheTable::try_new(scope, high) {
                Some(table) => table,
                // 256 words could not be reserved: answer the default bound
                // with nothing filed; every `valueOf` then takes the uncached
                // arm of `native_integer_value_of` until a later call files one.
                None => return high,
            }
        }
    };
    ...
    // A table already filed for this identity (it can only be one this very
    // function filed, under this lock) stays authoritative: the memo takes ITS
    // bound, so the memo, the table length and `integer_cache_high_for` agree.
    let table: &'static IntegerCacheTable =
        *integer_cache().lock().entry(scope).or_insert_with(|| {
            let leaked: &'static IntegerCacheTable = Box::leak(Box::new(fresh));
            leaked
        });
    memo.insert(scope, table.high);
    table.high
}
...
//
// Round 9 wave 15 (heapapi): its store is now a `LongCacheTable` of ATOMIC
// slots, filed once per VM identity and read with neither a lock nor a hash.
// The directory and the tables are declared with `INTEGER_CACHE`'s, next to
// the twin they copy; see `LONG_CACHE` / `long_cache_table` up there.
...
/// [`scan_one_cache`] for `INTEGER_CACHE`, whose slots are atomic (collect11).
fn scan_integer_cache(vm_identity: usize, out: &mut Vec<cratonvm_types::ObjectRef>) {
    let Some(table) = integer_cache_table(vm_identity) else {
        return;
    };
    table.slots.scan(out);
}

/// [`update_one_cache`] for `INTEGER_CACHE` (collect11).
fn update_integer_cache(vm_identity: usize, pointer_map: &cratonvm_types::PointerMap) {
    let Some(table) = integer_cache_table(vm_identity) else {
        return;
    };
    table.slots.remap(pointer_map);
}

/// [`scan_one_cache`] for `LONG_CACHE`, whose slots are atomic (wave 15).
fn scan_long_cache(vm_identity: usize, out: &mut Vec<cratonvm_types::ObjectRef>) {
    let Some(table) = long_cache_table(vm_identity) else {
        return;
    };
    table.slots.scan(out);
}

/// [`update_one_cache`] for `LONG_CACHE` (wave 15).
fn update_long_cache(vm_identity: usize, pointer_map: &cratonvm_types::PointerMap) {
    let Some(table) = long_cache_table(vm_identity) else {
        return;
    };
    table.slots.remap(pointer_map);
...
pub fn gc_scan_value_of_cache_roots(vm_identity: usize, out: &mut Vec<cratonvm_types::ObjectRef>) {
    scan_integer_cache(vm_identity, out);
    scan_one_cache(boolean_cache(), vm_identity, out);
    scan_long_cache(vm_identity, out);
...
pub fn gc_update_value_of_cache_refs(vm_identity: usize, pointer_map: &cratonvm_types::PointerMap) {
    ...
    update_integer_cache(vm_identity, pointer_map);
    update_one_cache(boolean_cache(), vm_identity, pointer_map);
    update_long_cache(vm_identity, pointer_map);
...
pub fn canonical_wrapper_if_cached(
    ...
    match (desc, v) {
        ...
        ("I", Value::Int(x)) if x >= INTEGER_CACHE_LOW => {
            // `IntegerCacheTable::slot` widens before subtracting: `x` is
            // unbounded ABOVE here (the slot lookup is the only bound), so
            // `i32::MAX - INTEGER_CACHE_LOW` would overflow an `i32` and panic
            // a debug build where it must simply MISS. Lock-free (collect11).
            integer_cache_table(vm_identity)?.get(x)
        }
        // Lock-free since wave 15 (heapapi), exactly like the `I` arm above.
        ("J", Value::Long(x)) if (LONG_CACHE_LOW..=LONG_CACHE_HIGH).contains(&x) => {
            long_cache_table(vm_identity)?.get(x)
        }
...
pub(crate) fn native_integer_value_of(
    ...
    if (INTEGER_CACHE_LOW..=high).contains(&val) {
        let scope = ctx.vm_identity();
        // `integer_cache_bound` filed this VM's table (or could not reserve
        // one, in which case the value simply goes uncached). The table's
        // `slot` lookup is bounds-checked, so a mismatched length is a miss,
        // never an index panic inside a native.
        let table = integer_cache_table(scope);
        // Fast path: lock-free read before any heap allocation (collect11).
        if let Some(cached) = table.and_then(|t| t.get(val)) {
            return Ok(Some(Value::Object(Some(cached))));
        }
        let obj = alloc_wrapper(ctx, "java/lang/Integer");
        ctx.set_field(obj, 0, Value::Int(val));
        // Publish with a CAS — another thread may have populated the slot
        // while we were allocating. If so, drop ours and return theirs (the
        // loser allocation is collectible; the race is rare and it preserves
        // the JLS identity invariant).
        let canonical = match table {
            Some(t) => t.install(val, obj),
            None => obj,
        };
        return Ok(Some(Value::Object(Some(canonical))));
    }
    let obj = alloc_wrapper(ctx, "java/lang/Integer");
    ctx.set_field(obj, 0, Value::Int(val));
...
pub(crate) fn native_long_value_of(
    ...
    if (LONG_CACHE_LOW..=LONG_CACHE_HIGH).contains(&val) {
        let scope = ctx.vm_identity();
        // Files this VM's table on first use; `None` only when the backing
        // store cannot be reserved, in which case the value goes uncached
        // rather than the native failing.
        let table = long_cache_table_or_file(scope);
        // Fast path: lock-free read before any heap allocation (wave 15,
        // heapapi) — the `Integer` twin's shape exactly.
        if let Some(cached) = table.and_then(|t| t.get(val)) {
            return Ok(Some(Value::Object(Some(cached))));
        }
        ...
        // Publish with a CAS — another thread may have populated the slot
        // while we were allocating. If so, drop ours and return theirs (the
        // loser allocation is collectible — but the race is rare and it
        // preserves the JLS identity invariant).
        let canonical = match table {
            Some(t) => t.install(val, obj),
            None => obj,
        };
...
///
/// G9-1 measured it: `JAVA_NUMERIC_VALUE_RUNS` disagrees with HotSpot 25 on
/// **1,177** supplementary code points (first `U+10107` AEGEAN NUMBER ONE,
/// HotSpot `1`, this table `-1`) — the table is BMP-only by construction, and
/// nothing above `U+FFFF` was ever measured into it. Registering `(I)I` onto
/// this body turns all 1,177 wrong on the same day; the unregistered `(I)I`
/// overload runs real `CharacterData` bytecode and must stay that way. Same
/// decision as `Character.digit(int,int)` (E17-1 §1).
...
/// is in `docs/jdk-only/E7-1-character-int-code-point-contracts.md`.
...
mod tests {
    ...
    /// Round 9 wave 10b (wire10b): the JIT helper's peeks answer exactly the
    /// object the native would, and only for a slot the native has filled.
    #[test]
    fn r9w10b_cache_peeks_answer_the_natives_object_and_nothing_else() {
        let mut ctx = mock_ctx();
        let vm = 0x5f3dusize;
        ctx.set_vm_identity(vm);
        // Nothing boxed yet in this VM: every peek is a miss.
        assert_eq!(integer_cache_peek(vm, 7), None);
        assert_eq!(long_cache_peek(vm, 7), None);
        let boxed = box_int(&mut ctx, 7);
        assert_eq!(integer_cache_peek(vm, 7), Some(boxed));
        // A cold in-range slot, and values outside the cache, stay misses.
        assert_eq!(integer_cache_peek(vm, 8), None);
        assert_eq!(integer_cache_peek(vm, 128), None);
        assert_eq!(integer_cache_peek(vm, -129), None);
        assert_eq!(integer_cache_peek(vm, i32::MIN), None);
        let long_boxed = ref_of(native_long_value_of(&mut ctx, &[Value::Long(-5)]).unwrap());
        assert_eq!(long_cache_peek(vm, -5), Some(long_boxed));
        assert_eq!(long_cache_peek(vm, 128), None);
        assert_eq!(long_cache_peek(vm, i64::MIN), None);
        // Another VM never sees this one's boxes.
        assert_eq!(integer_cache_peek(vm + 1, 7), None);
        assert_eq!(long_cache_peek(vm + 1, -5), None);
    }

    /// Round 9 wave 11 (collect11): the lock-free `INTEGER_CACHE_LAST` fast
    /// path must never answer one VM's box to another, however the lookups
    /// interleave, and every reader (peek, probe, bound memo, native) must
    /// agree on the same canonical object.
    #[test]
    fn r9w11_integer_cache_fast_table_is_vm_scoped_under_interleaving() {
        let mut a = mock_ctx();
        let mut b = mock_ctx();
        let (vm_a, vm_b) = (0x5f41usize, 0x5f42usize);
        a.set_vm_identity(vm_a);
        b.set_vm_identity(vm_b);
        b.set_system_property("java.lang.Integer.IntegerCache.high", "500");
        let a7 = box_int(&mut a, 7);
        let b7 = box_int(&mut b, 7);
        assert_ne!(a7, b7, "two VMs must not share a canonical box");
        for _ in 0..3 {
            // Alternate so the last-table pointer flips on every lookup.
            assert_eq!(integer_cache_peek(vm_a, 7), Some(a7));
            assert_eq!(integer_cache_peek(vm_b, 7), Some(b7));
            assert_eq!(integer_cache_high_for(vm_a), Some(127));
            assert_eq!(integer_cache_high_for(vm_b), Some(500));
            assert_eq!(
                canonical_wrapper_if_cached(vm_a, "I", Value::Int(7)),
                Some(a7)
            );
            assert_eq!(
                canonical_wrapper_if_cached(vm_b, "I", Value::Int(7)),
                Some(b7)
            );
            assert_eq!(box_int(&mut a, 7), a7);
            assert_eq!(box_int(&mut b, 7), b7);
        }
        // Each VM's own bound: 300 is cached only in B.
        let b300 = box_int(&mut b, 300);
        assert_eq!(box_int(&mut b, 300), b300);
        assert_eq!(integer_cache_peek(vm_b, 300), Some(b300));
        assert_eq!(integer_cache_peek(vm_a, 300), None);
        assert_ne!(box_int(&mut a, 300), box_int(&mut a, 300));
        // An identity with no table is a miss everywhere, not a stale hit.
        assert_eq!(integer_cache_high_for(0x5f43), None);
        assert_eq!(integer_cache_peek(0x5f43, 7), None);
    }

    /// The CAS publish: the first box installed for a value is canonical and
    /// a later loser gets the winner back; out-of-table values never install.
    #[test]
    fn r9w11_integer_cache_table_install_keeps_the_first_box() {
        let mut ctx = mock_ctx();
        let first = ctx.alloc_object(cratonvm_types::ClassId::new(1), 1);
        let second = ctx.alloc_object(cratonvm_types::ClassId::new(1), 1);
        let table = IntegerCacheTable::try_new(0x5f44, 127).expect("256 slots");
        assert_eq!(table.slots.len(), 256);
        assert_eq!(table.get(5), None);
        assert_eq!(table.install(5, first), first);
        assert_eq!(table.install(5, second), first);
        assert_eq!(table.get(5), Some(first));
        // Bounds: -129 and 128 are outside, `i32::MAX` must not overflow.
        assert_eq!(table.install(128, second), second);
        assert_eq!(table.get(128), None);
        assert_eq!(table.install(-129, second), second);
        assert_eq!(table.get(-129), None);
        assert_eq!(table.get(i32::MAX), None);
        assert_eq!(table.get(i32::MIN), None);
        assert_eq!(table.install(-128, second), second);
        assert_eq!(table.get(-128), Some(second));
    }
    ...
    fn math_to_radians_180() {
        ...
        match r.unwrap() {
            Some(Value::Double(v)) => {
                ...
                // anything.) W7-54-strictmath-fdlibm-family-RETIRED-20260917.md.
