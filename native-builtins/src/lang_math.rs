...
impl IntegerCacheTable {
    ...
    /// The slot for `value`, or `None` outside `LOW..=high`.
    #[inline]
    fn slot(&self, value: i32) -> Option<&std::sync::atomic::AtomicUsize> {
        // Widen before subtracting: `value` is unbounded above here and
        // `i32::MAX - LOW` overflows an `i32`.
        let idx = value as i64 - INTEGER_CACHE_LOW as i64;
        if idx < 0 {
            return None;
        }
    ...
    fn get(&self, value: i32) -> Option<cratonvm_types::ObjectRef> {
        let raw = self.slot(value)?.load(std::sync::atomic::Ordering::Acquire);
        if raw == 0 {
            return None;
        }
        // SAFETY: a non-zero slot is only ever written from a live
        // `ObjectRef::as_ptr()` (`install`) or a GC pointer-map target
        // (`gc_update_value_of_cache_refs`); both are non-null, aligned heap
        // object addresses, and the slot is a GC root, so the object is live.
    ...
    /// won the race; returns whichever box is canonical. A value outside the
    /// table (impossible for a caller that checked the bound) returns `obj`
    /// uncached, as the pre-table code did for an out-of-range index.
...
pub fn long_cache_peek(vm_identity: usize, value: i64) -> Option<cratonvm_types::ObjectRef> {
    canonical_wrapper_if_cached(vm_identity, "J", Value::Long(value))
}
...
/// Walks the WHOLE table, so a widened `IntegerCache.high` is reported too.
fn scan_integer_cache(vm_identity: usize, out: &mut Vec<cratonvm_types::ObjectRef>) {
    ...
    for slot in table.slots.iter() {
        ...
        if raw != 0 {
            // SAFETY: a non-zero slot is a live box address (see `IntegerCacheTable`).
            out.push(unsafe { cratonvm_types::ObjectRef::from_raw(raw as *mut u8) });
        }
    }
}

/// [`update_one_cache`] for `INTEGER_CACHE`. A `compare_exchange(old, new)`
/// per slot, so a concurrent fill of an EMPTY slot is never overwritten (only
/// non-zero slots are remapped) and a slot is never remapped twice.
