...
pub(crate) fn native_sb_get_chars(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    if dst_begin < 0 || copy_end > dst_len {
        ...
        return Err(
            cratonvm_types::error::RuntimeError::aioobe_index_only(bad_index)
            .into(),
        );
    }
    let buf = match buf {
...
pub(crate) fn native_string_utf16_get_chars(
    ...
    if dst_begin < 0 || copy_end > dst_len {
        ...
        return Err(
            cratonvm_types::error::RuntimeError::aioobe_index_only(index).into(),
        );
    }
    let mut bytes = vec![0u8; count * 2];
    if ctx.read_byte_array_into(value, src_begin as usize * 2, &mut bytes) != bytes.len() {
        return Err(
            cratonvm_types::error::RuntimeError::aioobe_index_only(src_begin)
            .into(),
        );
    }
