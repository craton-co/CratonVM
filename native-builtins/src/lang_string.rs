...
fn charsequence_fast_units(
    ...
    if name.starts_with("java/nio/") && name.contains("CharBuffer") {
        ...
        // `docs/jdk-only/G26-1-four-families-of-RJdkIntrinsics3-20260817.md`
...
/// `docs/jdk-only/G53-1-the-exact-exception-class-and-the-rest-of-sbidx-20260817.md`.
///
...
/// ```
///
/// The doc on `case_map::JDK_UNMAPPED_CASE_RUNS` already claimed
...
///    — can only differ on [`crate::case_map::JDK_UNMAPPED_CASE_RUNS`],
...
/// `docs/jdk-only/W7-34-formatter-family-residuals.md`): the
...
struct FmtSupport {
    ...
    /// G2-1 residual §4.1, closed. `DAY_OF_WEEK` (`%tA %ta`) answerable with
    /// **no** year, month or day at all — `java.time.DayOfWeek` is the one
    /// measured source that contradicts [`FmtSupport::full_date`]'s
    /// derivation (see there). `true` only for that source; every other
    /// constructor leaves it `false` and relies on `full_date()` instead, so
    /// `%tA`/`%ta` support is `full_date() || weekday_only` — see
    /// [`FmtSupport::weekday`].
    weekday_only: bool,
}
...
impl FmtSupport {
    ...
    /// `DAY_OF_WEEK` (`%tA %ta`) support: `full_date()`, OR the one measured
    /// exception, `weekday_only`. See [`FmtSupport::full_date`] for `%tj`
    /// (`DAY_OF_YEAR`), which stays tied to `full_date()` alone —
    /// `java.time.DayOfWeek` refuses it (measured) along with the other 29
    /// fields it does not answer.
    const fn weekday(self) -> bool {
        self.full_date() || self.weekday_only
    }
    ...
    ///
    /// **One measured source contradicts the derivation and is now decoded
    /// separately**: `java.time.DayOfWeek` answers `%tA`/`%ta` while having no
    /// year, month or day at all (and its `%tc` reports `b`, not the `a` this
    /// derivation alone would give). See [`FmtSupport::weekday`] and
    /// `extract_temporal_fields`'s `DayOfWeek` arm, which fabricates a
    /// reference `(year, month, day)` known to land on the requested weekday
    /// via [`day_of_week_sun0`] purely so `'A'`/`'a'` can read it — every
    /// other field stays refused because `year`/`month`/`day` are `false`.
    const fn full_date(self) -> bool {
        self.year && self.month && self.day
    }

    /// Whether the `(year, month, day)` this source decoded to came through
    /// `millis_to_fields`'s hybrid Julian/Gregorian calendar (pre-1582 dates
    /// Julian) rather than a `java.time` accessor's proleptic-Gregorian one —
    /// i.e. whether re-encoding those fields back to an epoch (`epoch_sec`,
    /// for `%ts`/`%tQ`) must go through the same hybrid calendar to round
    /// trip correctly.
    ///
    /// `calendar_printer` alone is not this flag: it is true for every
    /// `calendar_printer` source (`Long`/`Date`/`Calendar`, correctly hybrid)
    /// but also *false* for `java.time.Instant`, whose `(y, m, d)` are
    /// nonetheless hybrid-decoded (`extract_temporal_fields`'s `Instant` arm
    /// calls `millis_to_fields` directly) — `Instant` just has no printer that
    /// would read `%tY`/`%tD`/etc. off them, so `calendar_printer` was never
    /// set for it. `instant && !zone` catches exactly that case: the only
    /// other `instant`-capable, non-`calendar_printer` sources are
    /// `ZonedDateTime`/`OffsetDateTime`, and both carry their own zone
    /// (`zone: true`) where `Instant` does not.
    const fn hybrid_epoch(self) -> bool {
        self.calendar_printer || (self.instant && !self.zone)
    }
}
...
fn fmt_temporal_fault_char(field: char, s: FmtSupport) -> Option<char> {
    ...
    match field {
        ...
        // DAY_OF_WEEK — see [`FmtSupport::weekday`]: `full_date()`, plus the
        // one measured exception (`java.time.DayOfWeek`).
        'A' | 'a' => need(s.weekday(), field),
        // DAY_OF_YEAR needs a real date; unlike `%tA`/`%ta` it has no source
        // that answers it without one, so it stays tied to `full_date()`.
        'j' => need(s.full_date(), field),
        'R' | 'T' => need(s.time, 'H'),
        'r' => need(s.time, 'I'),
...
fn extract_temporal_fields(
    ...
    fn millis_to_fields(millis: i64) -> (i64, i32, i32, i32, i32, i32, i32) {
        ...
        // `Date`/`Long`/`Calendar` sources go through `GregorianCalendar`'s
        // hybrid Julian/Gregorian decode (pre-1582 dates are Julian), unlike
        // `java.time`'s proleptic-Gregorian sources — see `julian_date.rs`.
        let (y, m, d) = crate::julian_date::hybrid_from_epoch_day(epoch_day);
    ...
    match val {
        ...
        Value::Object(Some(obj)) => {
            ...
            } else if is_a(ctx, "java/time/DayOfWeek") {
                // G2-1 residual §4.1, closed. `DayOfWeek` has no year, month
                // or day at all — MEASURED, `%tA` of `DayOfWeek.MONDAY` is
                // `Monday`, `%ta` is `Mon`, and the other 29 fields refuse,
                // with `%tc` reporting `b`, not `a` (`FmtSupport::weekday`
                // is what makes that composite's first check pass here while
                // `full_date()` alone would not).
                //
                // `getValue()` is ISO-8601, `1..=7` for Monday..Sunday.
                // `day_of_week_sun0` wants a real calendar date, so this picks
                // one KNOWN to land on the requested weekday and read by
                // nothing else: 2001-01-01 was a Monday (a fixed historical
                // fact, not a per-run measurement), so `2001-01-<isoValue>`
                // walks Monday..Sunday for `isoValue` 1..7 in
                // `format_temporal_field`'s `'A'`/`'a'` arms. `year`, `month`
                // and `day` all stay `false` below, so nothing but those two
                // fields can ever read this fabricated triple back out —
                // exactly like `Instant`'s unread epoch defaults.
                let iso = invoke_i32(ctx, obj, "getValue");
                Ok((
                    (2001, 1, iso, 0, 0, 0, 0),
                    FmtZone::NONE,
                    FmtSupport {
...
/// 1=Monday) convention.
/// `hybrid` selects which calendar `(year, month, day)` are labelled in —
/// `true` for a `calendar_printer` source (`Date`/`Long`/`Calendar`/
/// `Instant`, decoded through `millis_to_fields`'s hybrid Julian/Gregorian
/// calendar), `false` for a `java.time` source (proleptic Gregorian by
/// specification). Weekday itself needs no branch beyond that: it is a
/// property of the physical day, not of which calendar labelled it, and
/// weekday continuity survives the 1582 reform intact (Julian Thursday
/// Oct 4 1582 was followed by Gregorian Friday Oct 15 1582 — the reform
/// deleted ten calendar dates, never a day of the week). `1970-01-01` (epoch
/// day 0) was a Thursday, index 4 in the 0=Sunday order `DAYS_ABBR`/
/// `DAYS_FULL` use.
fn day_of_week_sun0(year: i32, month: i32, day: i32, hybrid: bool) -> usize {
    let epoch_day = if hybrid {
        crate::julian_date::hybrid_to_epoch_day(year, month, day)
    } else {
...
fn format_temporal_field(
    ...
    // source through `millis_to_fields` → `julian_date::hybrid_from_epoch_day`,
    // which yields a PROLEPTIC year (Julian-calendar before the 1582 cutover,
    // Gregorian on or after it) exactly like `getYear()` does for the
    // `java.time` sources. Both halves were measured on HotSpot 25
    // (`Locale.ROOT`, `Asia/Kolkata`) at the one instant, 45 BC:
    //
    ...
    let dow = day_of_week_sun0(year32, month, day, support.calendar_printer);
    ...
    // zone, while `%tH` does.
    let epoch_sec = || {
        // See [`FmtSupport::hybrid_epoch`]: `Date`/`Long`/`Calendar`/`Instant`
        // were decoded through `millis_to_fields`'s hybrid Julian/Gregorian
        // calendar, so re-encoding them must go through the same hybrid
        // calendar or a pre-cutover instant would not round trip back to its
        // own epoch. `ZonedDateTime`/`OffsetDateTime` read their fields
        // straight off the object — genuinely proleptic Gregorian by
        // specification, never Julian — and must stay on the pure
        // civil-date encode.
        let epoch_day = if support.hybrid_epoch() {
            crate::julian_date::hybrid_to_epoch_day(year32, month, day)
        } else {
            civil_date::to_epoch_day(year32, month, day)
        };
        epoch_day * 86_400
            + hour as i64 * 3600
            + minute as i64 * 60
    ...
    let mut out = match field {
        ...
        'j' => format!(
            "{:03}",
            if support.calendar_printer {
                crate::julian_date::hybrid_day_of_year(year32, month, day)
            } else {
                civil_date::day_of_year(year32, month, day)
            }
        ),
        'm' => format!("{:02}", month),
        'd' => format!("{:02}", day),
...
///
/// Private: `format_arg_full` is the only entry point `String.format` reaches
/// (W8-F13-1 N3). No caller outside this file exists as of that record, so a
/// stray external use is now a build break instead of a silently stale
/// `pub(crate)`.
fn format_arg(
    ctx: &mut dyn NativeContext,
    val: &Value,
...
/// `case_map::JDK_UNMAPPED_CASE_RUNS`, where Rust's tables being NEWER
...
// The JDK-vs-Rust Unicode version skew — `JDK_UNMAPPED_CASE_RUNS` and
...
// [`crate::case_map::JDK_UNMAPPED_CASE_RUNS`] for the measurement and
...
fn java_char_to_upper_case(ch: u16) -> u16 {
    ...
    // to itself. See `JDK_UNMAPPED_CASE_RUNS`.
...
fn java_char_to_lower_case(ch: u16) -> u16 {
    // See the twin above and `JDK_UNMAPPED_CASE_RUNS`.
    if is_jdk_unmapped_case_code_point(u32::from(ch)) {
        return ch;
    }
    if ch == 0x0130 {
...
mod f22_utf16_formatter_tests {
    ...
    fn temporal_support_matrix_matches_hotspot() {
        ...
        // G2-1 residual §4.1, closed. The one measured source that answers
        // `%tA`/`%ta` with NO year, month or day at all.
        const DAY_OF_WEEK: FmtSupport = FmtSupport {
            year: false,
            month: false,
            day: false,
            time: false,
        ...
        // G2-1 residual §4.1, closed. `DayOfWeek` is the measured exception
        // `FmtSupport::full_date` cannot express: `%tA`/`%ta` are ANSWERED
        // with no year, month or day, `%tj` (DAY_OF_YEAR) still REFUSES
        // (needs a real date, which weekday-only does not derive), and the
        // other 29 fields refuse too. `%tc` reports `b` (the MONTH group),
        // not `a` — the weekday check passes for `DAY_OF_WEEK`, so it is the
        // first REAL gap, `%tc`'s answer under naive `full_date()`-only
        // derivation would wrongly be `a`.
        assert_eq!(fmt_temporal_fault_char('A', DAY_OF_WEEK), None);
        assert_eq!(fmt_temporal_fault_char('a', DAY_OF_WEEK), None);
        assert_eq!(fmt_temporal_fault_char('j', DAY_OF_WEEK), Some('j'));
        assert_eq!(fmt_temporal_fault_char('c', DAY_OF_WEEK), Some('b'));
        for f in FMT_DATETIME_FIELDS.chars() {
            if f == 'A' || f == 'a' {
                continue;
            }
            assert!(
                fmt_temporal_fault_char(f, DAY_OF_WEEK).is_some(),
                "%t{f} must be REFUSED on DayOfWeek"
            );
        }
