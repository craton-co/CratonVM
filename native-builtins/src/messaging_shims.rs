...
pub(crate) fn native_message_bytes_to_chars(
    ...
    message_bytes_to_chars(ctx, this)?;
    Ok(None)
}

/// `MessageBytes.toChars()`, also handing back the receiver's `charC` chunk so
/// a caller about to use it does not read the field again.
///
/// The `bool` is `true` when the conversion ALLOCATED: an allocation may move
/// objects, so the chunk reference returned alongside it (and any other the
/// caller already holds) is stale and must be re-read. It is `false` on every
/// path that does not allocate, where the returned reference is current.
pub(crate) fn message_bytes_to_chars(
    ctx: &mut dyn NativeContext,
    this: ObjectRef,
) -> Result<(Option<ObjectRef>, bool), MethodCallFailed> {
    let fields = message_bytes_conversion_fields(ctx, this);
    message_bytes_to_chars_with(ctx, this, fields)
}

/// The `[type, charC, strValue]` triple [`message_bytes_to_chars_with`] takes.
pub(crate) fn message_bytes_conversion_fields(
    ctx: &mut dyn NativeContext,
    this: ObjectRef,
) -> [Value; 3] {
    let mut fields = [Value::Object(None); 3];
    ctx.get_fields_by_name(this, &["type", "charC", "strValue"], &mut fields);
    fields
}

/// [`message_bytes_to_chars`] for a caller that has just read the receiver's
/// fields with [`message_bytes_conversion_fields`] and has run no Java since.
pub(crate) fn message_bytes_to_chars_with(
    ctx: &mut dyn NativeContext,
    this: ObjectRef,
    fields: [Value; 3],
) -> Result<(Option<ObjectRef>, bool), MethodCallFailed> {
    let ty = fields[0].as_int().unwrap_or(0);
    let char_c = match fields[1] {
        Value::Object(Some(o)) => Some(o),
        _ => None,
    };

    if ty == 3 {
        return Ok((char_c, false));
    }
    ...
    if ty == 0 {
        message_bytes_to_chars_cache()
            ...
            .unwrap()
            .remove(&this_key);
        if let Some(char_c) = char_c {
            let _ = ctx.invoke_virtual(char_c, "recycle", "()V", &[])?;
        }
        return Ok((char_c, true));
    }
    let mut str_value = fields[2];
    let mut char_c = char_c;
    let mut moved = false;
    if ty == 2 {
        let _ = ctx.invoke_virtual(this, "toString", "()Ljava/lang/String;", &[])?;
        // The callback ran Java: nothing read before it is trustworthy.
        moved = true;
        let mut again = [Value::Object(None); 2];
        ctx.get_fields_by_name(this, &["charC", "strValue"], &mut again);
        char_c = match again[0] {
            Value::Object(Some(o)) => Some(o),
            _ => None,
        };
        str_value = again[1];
    ...
    let Some(char_c) = char_c else {
        return Ok((None, moved));
    };
    ...
    // `set_char_chunk_from_units` allocated the new buffer.
    Ok((Some(char_c), true))
}
