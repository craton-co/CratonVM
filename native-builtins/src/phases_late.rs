...
fn cm_construct_via_context(
    ...
    let cls_pin = ctx.pin_native_root(cls);
    let prefix_pin = pinned_object_value(ctx, prefix);
    let dbg = cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_CMCTX");
    let result = (|| {
...
fn config_mapping_prefix(ctx: &mut dyn NativeContext, cls: ObjectRef) -> Value {
    ...
    // docs/internal/retired/oidc-common-config-auth-server-url-abstract-method-RETIRED-20260917.md.
...
/// in it. See
/// docs/jdk-only/F15-1-two-registrars-that-no-bytecode-can-name-20260813.md
...
pub(crate) fn register_p67_misc(r: &mut NativeMethodRegistry) {
    ...
    // System.Logger.Level enum — CONVERTED to a real `<clinit>` (E36-1 §4c,
    // E40-1 N3). The seven field-shaped rows this comment used to describe
    // (a field name in the method slot, `Ljava/lang/System$Logger$Level;`
    // where a method descriptor belongs) were dead in every tier — no
    // `getstatic` path in this VM consults the native registry — and
    // `p57_alloc_enum` minted a fresh instance per call, so even a
    // hypothetical caller would have failed `==` against the class's own
    // constants. `System.Logger.Level.INFO` measurably compiles to a real
    // `getstatic`, so the right end state was always a native `<clinit>`; the
    // three blockers the nomination named are now closed:
    //
    //   1. `classloading/src/class_manager.rs` now declares this class's
    //      seven statics, `$VALUES`, and the one instance field the real
    //      class adds over `java.lang.Enum` (`severity`).
    //   2. `system_logger_level_clinit` below reproduces the real `<clinit>`
    //      whole, `severity` included — verified against `javap -p
    //      'java.lang.System$Logger$Level'` and `java` on JDK 25.0.3+9-LTS:
    //      ALL=-2147483648, TRACE=400, DEBUG=500, INFO=800, WARNING=900,
    //      ERROR=1000, OFF=2147483647 (`lib.rs::system_logger_severity_of`,
    //      which `System.Logger.isLoggable` already compares against).
    //   3. `vm/src/vm/tests.rs`'s `system_logger_p67` asserts against the
    //      `<clinit>`'s output directly, in the same change.
    r.register(
        "java/lang/System$Logger$Level",
        "<clinit>",
        "()V",
        system_logger_level_clinit,
    );
    r.set_category(__prev_cat);
}

/// `java.lang.System$Logger$Level.<clinit>` — see the block comment above
/// `register_p67_misc`'s registration of this native for the full account.
///
/// Follows `stack_walker.rs::native_option_clinit`'s discipline: every
/// constant is rooted for the whole function (not just across its own
/// `create_string`), because each later iteration allocates two more objects
/// with the earlier constants live only as bare `ObjectRef`s otherwise.
fn system_logger_level_clinit(ctx: &mut dyn NativeContext, _args: &[Value]) -> MethodCallResult {
    const NAMES: [&str; 7] = ["ALL", "TRACE", "DEBUG", "INFO", "WARNING", "ERROR", "OFF"];
    let class_name = "java/lang/System$Logger$Level";

    let mut scope = cratonvm_native_api::NativeHandleScope::new(ctx);
    let mut handles = Vec::with_capacity(NAMES.len());

    for (ordinal, name) in NAMES.iter().enumerate() {
        let severity = crate::system_logger_severity_of(name)
            .expect("NAMES is exactly system_logger_severity_of's match arms");
        // 3 slots: `java.lang.Enum`'s (name, ordinal) plus this class's own
        // `severity` — see `class_manager.rs`'s entry for the field-order
        // agreement this depends on.
        let level = try_alloc_concurrent_synthetic(&mut *scope, class_name, 3)?;
        let handle = scope.root(level);
        // Pinned across `create_string`: a moving young GC there would
        // relocate the freshly allocated constant (native stale-local
        // family) — same hazard `native_option_clinit` guards against.
        let name_str = scope.create_string(name);
        let level = scope.get(&handle);
        scope.set_field_by_name(level, "name", Value::Object(Some(name_str)));
        scope.set_field_by_name(level, "ordinal", Value::Int(ordinal as i32));
        if !matches!(
            scope.get_field_by_name(level, "name"),
            Value::Object(Some(_))
        ) {
            scope.set_field(level, 0, Value::Object(Some(name_str)));
            scope.set_field(level, 1, Value::Int(ordinal as i32));
        }
        scope.set_field_by_name(level, "severity", Value::Int(severity));
        if !matches!(scope.get_field_by_name(level, "severity"), Value::Int(s) if s == severity) {
            scope.set_field(level, 2, Value::Int(severity));
        }
        scope.set_static_field_by_name(class_name, name, Value::Object(Some(level)));
        handles.push(handle);
    }
    ...
    let values_array = scope.new_ref_array(level_class, handles.len());
    let values_handle = scope.root(values_array);
    for (idx, handle) in handles.iter().enumerate() {
        let level = scope.get(handle);
        let array = scope.get(&values_handle);
        scope.set_array_element(array, idx, Value::Object(Some(level)));
    }
...
mod essential_vs_synthetic_jdk_coverage_audit {
    ...
    /// `docs/jdk-only/E16-R11-P59-MODULE-LAYER-TWIN-20260813.md`
