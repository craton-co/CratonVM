...
pub fn store_property_in_real_map(
    ...
    ctx.unpin_native_roots(this_pin);
    if r.is_ok() {
        // The key is now in the real map because THIS VM put it there, so a later
        // disappearance from that map is a `remove` by the application
        // ([`harvest_real_map`], [`removed_through_receiver`]).
        last_published()
            .lock()
...
/// The String keys the last [`replace_real_map`] published, per VM -- what
/// [`harvest_real_map`] compares the live map against to notice a `remove`.
///
