...
fn side_key_set(obj: ObjectRef) -> std::collections::HashSet<String> {
    match table().lock().get(&key_for(obj)) {
...
fn native_properties_keys(
    ...
    let mut keys: Vec<String> = snapshot_kv(this).into_iter().map(|(k, _v)| k).collect();
