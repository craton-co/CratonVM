...
pub fn replace_real_map(
    ...
    //
    // PRUNE, do not `clear()`. A `clear()` followed by a refill is a window in
    // which the LIVE system-properties map is empty, and another thread reading
    // it in that window sees no properties at all: MEASURED under `--jdk-only`,
    // `ConcurrentBeanWrapperTests` failed `The System properties must not be
    // empty` on repetition 32 of 100, the concurrent readers racing this very
    // rebuild. It also DESTROYED every entry the snapshot cannot represent -- the
    // snapshot is `String -> String`, so `System.getProperties().put("k",
    // someObject)` (which `Properties` permits, and which
    // `StandardEnvironmentTests.getSystemProperties` does) vanished at the next
    // `getProperties()` call. So the map is only ever CHANGED toward the
    // snapshot: stale String entries are removed, snapshot entries are written,
    // and a non-String value the application put there is left alone.
    if existing.is_some() {
        let snapshot_keys: std::collections::HashSet<&str> =
            entries.iter().map(|(k, _)| k.as_str()).collect();
        let key_set = ctx.invoke_virtual(
            chm_cur,
            "keySet",
        ...
        let keys_arr = match key_set {
            Ok(Some(Value::Object(Some(ks)))) => {
                match ctx.invoke_virtual(ks, "toArray", "()[Ljava/lang/Object;", &[]) {
        ...
        match keys_arr {
            Some(arr) => {
                ...
                for i in 0..len {
                    ...
                    let Value::Object(Some(k)) = ctx.get_array_element(arr_cur, i) else {
                        continue;
                    };
                    let Some(text) = ctx.read_string(k) else {
                        continue;
                    };
                    if snapshot_keys.contains(text.as_str()) {
                        continue;
                    }
                    chm_cur = ctx.read_native_pin(chm_pin, chm_cur);
                    ...
                    // Only a String value is ours to remove.
                    let ours = matches!(
                        current,
                        Ok(Some(Value::Object(Some(v)))) if ctx.read_string(v).is_some()
                    );
                    if ours {
                        chm_cur = ctx.read_native_pin(chm_pin, chm_cur);
                        let arr_cur = ctx.read_native_pin(arr_pin, arr);
                        if let Value::Object(Some(k)) = ctx.get_array_element(arr_cur, i) {
            ...
            None => trouble = Some("prune: could not enumerate the existing keys".to_string()),
        }
        chm_cur = ctx.read_native_pin(chm_pin, chm_cur);
    }
    ...
    for (key, value) in entries {
        ...
        chm_cur = ctx.read_native_pin(chm_pin, chm_cur);
        // A value the application put here that is NOT a String is not ours to
        // overwrite (see the prune note above); an unchanged String needs no
        // write at all.
        if existing.is_some() {
            let cur = ctx.invoke_virtual(
                chm_cur,
                "get",
                "(Ljava/lang/Object;)Ljava/lang/Object;",
                &[Value::Object(Some(k_cur))],
            );
            chm_cur = ctx.read_native_pin(chm_pin, chm_cur);
            if let Ok(Some(Value::Object(Some(v)))) = cur {
                let unchanged_or_foreign = match ctx.read_string(v) {
                    Some(text) => text == *value,
                    None => true,
                };
                if unchanged_or_foreign {
                    written += 1;
    ...
    if complete {
        // The whole snapshot, VALUES INCLUDED -- see [`last_published`]. The
        // map was just cleared and refilled from `entries`, so this is exactly
        // what is in it, and any later difference is the application's doing.
        last_published()
            .lock()
            .insert(ctx.vm_identity(), entries.iter().cloned().collect());
    }
    if !complete {
        this_cur = ctx.read_native_pin(this_pin, this_cur);
...
pub fn store_property_in_real_map(
    ...
    ctx.unpin_native_roots(this_pin);
    if r.is_ok() {
        // The key is now in the real map with THIS value because THIS VM put it
        // there. A later disappearance is a `remove` by the application
        // ([`removed_through_receiver`]); a later different value is a write by
        // the application ([`map_entry_is_a_receiver_write`]). Recording the
        // value is what separates those two from "the VM's own write, echoed".
        last_published()
            .lock()
            .entry(ctx.vm_identity())
            .or_default()
...
pub fn remove_property_from_real_map(ctx: &mut dyn NativeContext, this: ObjectRef, key: &str) {
    ...
    ctx.unpin_native_roots(this_pin);
    if r.is_ok() {
        // Drop it from the published table too. The key is now absent from BOTH
        // stores by the VM's own decision, and leaving it listed would make the
        // next `harvest_real_map` read that absence as an application `remove`
        // and re-issue a removal that has already happened -- harmless, but it
        // would also make `removed_through_receiver` answer `true` for a key the
        // VM cleared, which attributes the VM's own action to the program.
        if let Some(published) = last_published().lock().get_mut(&ctx.vm_identity()) {
...
/// What the VM last published into the real `map`, per VM: key -> value.
///
/// # It was a key SET until 2026-09-20, and that is why a Rust write was lost
///
/// A set answers one question -- "was this key published and then removed?" --
/// and [`harvest_real_map`] used it for exactly that. It cannot answer the
/// other one: **is the value sitting in the map now the one the VM put there,
/// or one the application wrote through the receiver?** Without that, every
/// entry in the map looked like an application write, so the harvest folded all
/// of them back into the VM store.
///
/// That is not a masking bug, it is a LOSING one. A property written into the
/// store from Rust -- `set_system_property`, no `System.setProperty` involved;
/// `user.timezone` from `TimeZone.getDefault`, `java.home` from the log
/// manager, the two `jboss_module_loader` keys -- would be overwritten by the
/// stale value still in the map at the next `System.getProperties()`, which
/// harvests BEFORE it refills. `jdk-only/the-system-properties-real-map-is-null-and-it-blocks-the-chm-retirement-20260909-RETIRED-20260921.md`
/// §7 recorded the read half of this as a residual ("masked ... until the next
/// `System.getProperties()` refills it") and understated it: the refill is the
/// call that makes it permanent, because the harvest runs first.
///
/// Storing the VALUE makes the map self-describing. A live entry equal to what
/// was published carries no opinion and the store wins; a live entry that
/// DIFFERS is an application write and wins; a published key that is gone was
/// removed through the receiver. Three answers from one table, and no
/// generation counter -- which is what §7 thought this would need.
///
/// LOCK LEVEL: `Scratch` -- a leaf map guarded only around a lookup/insert; no
/// Java call and no other lock is taken while it is held.
fn last_published() -> &'static cratonvm_types::lock_order::OrderedPlMutex<
    std::collections::HashMap<usize, std::collections::HashMap<String, String>>,
> {
    static PUBLISHED: OnceLock<
...
/// Does the live real-map entry for `key` represent a write the APPLICATION
/// made through the receiver, rather than the value this VM published?
///
/// The discriminator [`last_published`] exists for. `true` whenever the map is
/// telling us something the store does not already know:
///
/// * the key was never published -- it can only have been added through the
///   receiver;
/// * the key was published and the live value differs -- real `Properties`
///   bytecode wrote it.
///
/// `false` means the entry is the VM's own value echoed back, so the store is
/// at least as fresh and reading the map instead would mask a Rust-side write.
///
/// Answering `true` when nothing is known is the safe direction: it preserves
/// the pre-2026-09-20 behaviour (map wins) for any key this table has not seen,
/// so a path that bypasses `replace_real_map` degrades to the old rule rather
/// than to silently dropping an application write.
pub fn map_entry_is_a_receiver_write(vm: usize, key: &str, live_value: &str) -> bool {
    match last_published().lock().get(&vm) {
        Some(published) => published.get(key).map(|p| p != live_value).unwrap_or(true),
        None => true,
    }
}

/// Was `key` published to the system `Properties` and then REMOVED through the
/// receiver (`System.getProperties().remove(k)`), so that the VM store, which
/// `System.getProperty` reads, still holds a value the application deleted?
///
/// Only a key the last [`replace_real_map`] published can be "removed" -- a key
/// the VM set since is store-only and stays visible. Answers `false` whenever
/// the singleton or its real map is not there to ask.
pub fn removed_through_receiver(ctx: &mut dyn NativeContext, key: &str) -> bool {
    let vm = ctx.vm_identity();
    if !last_published()
        .lock()
        .get(&vm)
        .is_some_and(|published| published.contains_key(key))
    {
        return false;
    }
    let Some(props) = crate::lang_system::system_props_singleton(vm) else {
        return false;
    };
    if real_map_of(ctx, props).is_none() {
        return false;
    }
    lookup_in_real_map(ctx, props, key).is_none()
}
...
/// # Removals
///
/// A `remove` performed directly on the receiver is found by remembering which
/// keys the LAST [`replace_real_map`] published ([`last_published`]): a key that
/// was published and is now absent from the map was removed by the application,
/// so it is removed from the VM store too. Without it the key survived in the
/// store and was published straight back at the next `getProperties()`, so
/// `System.getProperties().remove(k)` never took -- MEASURED 2026-09-18 as the
/// cause of five `TestNG`-under-JUnit spring-test classes: the testng-engine sets
/// `testng.mode.dryrun` for discovery and clears it with exactly that `remove`,
/// so every later `TestNG` run in the process was a dry run whose test bodies
/// never executed. A key added through the receiver (never published) is a plain
/// harvest, as before. An empty snapshot removes nothing: it means the read
/// failed, not that every property was deleted.
///
/// # Updates, and the write this used to DESTROY
///
/// A live entry equal to the value [`replace_real_map`] published is the VM's
/// own value echoed back; it carries no opinion and is skipped. Until
/// 2026-09-20 it was not: every live entry was folded into the store, so a
/// property the VM had written from RUST since the last publish -- with no
/// `System.setProperty` involved, and therefore no mirror into the map -- was
/// overwritten by the stale published value at the next
/// `System.getProperties()`. Because the harvest runs BEFORE the refill, the
/// refill then re-published the value the harvest had just restored, making the
/// loss permanent rather than transient.
///
/// `jdk-only/the-system-properties-real-map-is-null-and-it-blocks-the-chm-retirement-20260909-RETIRED-20260921.md`
/// §7 recorded this as "masked ... until the next `System.getProperties()`
/// refills it" and expected closing it to need a generation counter on the map.
/// It does not: [`last_published`] storing the VALUE answers the same question,
/// and turns this function from the union that page calls it into the
/// reconciliation it wanted.
///
/// Live callers this reaches, so the paragraph above is not hypothetical:
/// `user.timezone` (`lib.rs`, from `TimeZone.getDefault`), `java.home`
/// (`logmanager.rs`), `org.graalvm.nativeimage.imagecode`
/// (`graalvm_compat.rs`), and two `jboss_module_loader.rs` keys.
pub fn harvest_real_map(ctx: &mut dyn NativeContext, this: ObjectRef) -> usize {
    let mut harvested = 0usize;
    let vm = ctx.vm_identity();
    let current = snapshot_real_map(ctx, this);
    if !current.is_empty() {
        let live: std::collections::HashSet<&str> =
            current.iter().map(|(k, _)| k.as_str()).collect();
        let gone: Vec<String> = {
            let mut published = last_published().lock();
            match published.get_mut(&vm) {
                Some(published_kv) => {
                    let gone: Vec<String> = published_kv
                        .keys()
                        .filter(|k| !live.contains(k.as_str()))
                        .cloned()
                        .collect();
                    for k in &gone {
                        published_kv.remove(k);
                    }
                    gone
                }
                None => Vec::new(),
    ...
    for (key, value) in current {
        // The entry is the VM's own published value coming back: the store is
        // the fresher of the two by construction, and writing this would undo
        // any Rust-side `set_system_property` made since the publish. See the
        // "Updates" section above -- this one test is the whole fix.
        if !map_entry_is_a_receiver_write(vm, &key, &value) {
            continue;
        }
...
fn native_properties_clone(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // is therefore a GC point, so `args[0]` may by now name a from-space
    // address. `this` was refreshed across that walk by the `&mut`; `args` was
    // not, and cloning a vacated header is a wrong answer nothing reports.
...
fn native_properties_replace_all(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // prevent.
    //
    // ...and that re-entry is a GC POINT, which is the half the ordering fix
    // left open. `func` is read out of `args` BEFORE the snapshot, so pin it
    // FIRST and refresh across the walk: pinning it afterwards, as this did,
    // registers the address the collector may already have vacated, and every
    // `read_native_pin(func_pin, ..)` below then faithfully hands that stale
    // address to `invoke_virtual`. The `&mut` on `ordered_snapshot_kv` makes
    // the RECEIVER's staleness a compile error and is silent about every other
    // ref the caller holds beside it — which is what `func` is.
...
fn build_store_text(
    ...
    // `Properties.store` writes `System.lineSeparator()` in the JDK, which is
    // the captured static -- the same value Spring's `SortedProperties.store`
    // then splits on. See `NativeContext::captured_line_separator`.
    let eol = ctx.captured_line_separator();
...
/// `docs/jdk-only/H4-1-the-cluster-that-is-not-a-tag-20260820.md`.
...
impl PropsCell {
    fn open(
        ...
        // `ConcurrentHashMap`, whose first line it is — but that first line is
        // `if (key == null) throw new NullPointerException();`, MESSAGE-LESS,
        // not the helpful-NPE `Object.hashCode()` text `props_null_key_npe()`
        // carries. That text belongs to the READ path only (`get`,
        // `getProperty`, `containsKey`, `getOrDefault`), which reaches
        // `Hashtable.get`'s own key dereference — see `props_null_key_npe`'s
        // doc comment. G1-1 RECONCILED (retirement pass): this `open` serves
        // the six write-path conditional mutators (`replace` x2, `remove(k,v)`,
        // `compute`, `computeIfPresent`, `merge`), which had all been getting
        // the read path's message via this call, overwriting the correct
        // message-less contract `native-collections` already implements for
        // `remove(k,v)`/`replace`/`replace(k,o,n)` (this registrar runs after
        // `register_collections_natives` and wins the slot — see
        // `native-collections`'s `props_init_map_half` doc comment).
...
mod tests {
    ...
    /// THE DISCRIMINATOR that turns [`harvest_real_map`] from a union into a
    /// reconciliation, tested directly because the three answers it gives are
    /// what separate three different writers.
    ///
    /// `jdk-only/the-system-properties-real-map-is-null-and-it-blocks-the-chm-retirement-20260909-RETIRED-20260921.md`
    /// §7 left this open as "a property written into the VM store from RUST is
    /// masked by an older value in the map until the next
    /// `System.getProperties()` refills it", and expected the fix to need a
    /// generation counter. It needed the published VALUE instead, and
    /// "until the next refill" understated it: the refill's harvest runs FIRST,
    /// so the stale value was written back over the VM's and the loss was
    /// permanent. `apps/probes/SysPropsVmWriteProbe.java` rows 3 and 5 are the
    /// end-to-end form; this is the unit.
    ///
    /// A VM identity nothing else uses, so a parallel test cannot collide on
    /// the process-global table.
    #[test]
    fn a_published_value_echoed_back_is_not_a_receiver_write() {
        const VM: usize = 0xF00D_6AC0;
        last_published().lock().insert(
            VM,
            [
                ("user.timezone".to_string(), String::new()),
                ("java.home".to_string(), "/opt/jdk".to_string()),
            ]
            .into_iter()
            .collect(),
        );

        // Published and unchanged: the map is echoing the VM's own value, so
        // the store is at least as fresh and must win.
        assert!(
            !map_entry_is_a_receiver_write(VM, "user.timezone", ""),
            "an entry equal to what was published carries no opinion"
        );
        assert!(!map_entry_is_a_receiver_write(VM, "java.home", "/opt/jdk"));

        // Published and CHANGED: real `Properties` bytecode wrote it.
        assert!(
            map_entry_is_a_receiver_write(VM, "user.timezone", "Etc/UTC"),
            "a value different from the published one is an application write"
        );

        // Never published at all: it can only have been added through the
        // receiver, and the safe direction is to treat it as one.
        assert!(map_entry_is_a_receiver_write(VM, "probe.added", "v"));

        // An unknown VM degrades to the pre-2026-09-20 rule (map wins) rather
        // than to silently dropping an application write.
        assert!(map_entry_is_a_receiver_write(
            VM + 1,
            "java.home",
            "/opt/jdk"
        ));

        last_published().lock().remove(&VM);
    }
