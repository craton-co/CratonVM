...
fn get_or_init_subject_set(
    ...
    // Wrap in Collections.synchronizedSet to match OpenJDK Subject.<init> invariant:
    // principals, publicCredentials, and privateCredentials are all SynchronizedSet instances.
    let pin = ctx.pin_native_root(this);
    ...
    let this = ctx.read_native_pin(pin, this);
    ctx.unpin_native_roots(pin);
    let final_set = match wrapped {
        Ok(Some(Value::Object(Some(s)))) => s,
        _ => set,
    };
    ctx.set_field(this, slot, Value::Object(Some(final_set)));
    Ok(Some(Value::Object(Some(final_set))))
}
...
fn native_subject_get_principals(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let set = if !ctx.is_class_synthetic_stub("java/util/HashSet") {
        match ctx.invoke(
            "java/util/Collections",
            "synchronizedSet",
            "(Ljava/util/Set;)Ljava/util/Set;",
