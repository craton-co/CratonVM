...
/// — but ONLY when actually running the synthetic class library.
...
///
/// 2026-09-17: `cfg!(feature = "synthetic-jdk")` alone used to decide this,
/// which is the exact mistake `NativeMethodRegistry::drops_real_layout_synthetic`'s
/// own doc warns about — the Cargo feature decides what is COMPILED, not
/// which CLASS LIBRARY is loaded. A `--features synthetic-jdk` binary run
/// against a real JDK (`--real-jdk`/default) satisfied the cfg and kept
/// installing this overlay over real, loadable `com.sun.jmx.mbeanserver`
/// bytecode — exactly the shadowing this file's own header (2026-08-11)
/// already measured unnecessary and worse than the real machinery. Now
/// gated on `registry.drops_real_layout_synthetic()`, which is true only
/// for an actual real-JDK run regardless of what was compiled in.
///
/// Mirrors `native-io`'s `real_raf_enabled()`, which flipped the same way for
/// the same reason.
pub(crate) fn real_mxbean_mapping_enabled(registry: &NativeMethodRegistry) -> bool {
...
pub fn register_jmx_openmbean_natives(registry: &mut NativeMethodRegistry) {
    let real_mapping = real_mxbean_mapping_enabled(registry);
    register_jmx_openmbean_natives_with(registry, !real_mapping);
}
