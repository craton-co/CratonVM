...
pub(crate) fn encrypt_block(kw: &[[u32; 4]], inb: &[u8], outb: &mut [u8]) {
    ...
    let o0 = sbox(r0) ^ (sbox(r1 >> 8) << 8) ^ (sbox(r2 >> 16) << 16) ^ (sbox(r3 >> 24) << 24) ^ kw[r][0];
    let o1 = sbox(r1) ^ (sbox(r2 >> 8) << 8) ^ (sbox(r3 >> 16) << 16) ^ (sbox(r0 >> 24) << 24) ^ kw[r][1];
    let o2 = sbox(r2) ^ (sbox(r3 >> 8) << 8) ^ (sbox(r0 >> 16) << 16) ^ (sbox(r1 >> 24) << 24) ^ kw[r][2];
    let o3 = sbox(r3) ^ (sbox(r0 >> 8) << 8) ^ (sbox(r1 >> 16) << 16) ^ (sbox(r2 >> 24) << 24) ^ kw[r][3];
...
pub(crate) fn decrypt_block(kw: &[[u32; 4]], inb: &[u8], outb: &mut [u8]) {
    ...
    let o0 = sibox(r0) ^ (sibox(r3 >> 8) << 8) ^ (sibox(r2 >> 16) << 16) ^ (sibox(r1 >> 24) << 24) ^ kw[0][0];
    let o1 = sibox(r1) ^ (sibox(r0 >> 8) << 8) ^ (sibox(r3 >> 16) << 16) ^ (sibox(r2 >> 24) << 24) ^ kw[0][1];
    let o2 = sibox(r2) ^ (sibox(r1 >> 8) << 8) ^ (sibox(r0 >> 16) << 16) ^ (sibox(r3 >> 24) << 24) ^ kw[0][2];
    let o3 = sibox(r3) ^ (sibox(r2 >> 8) << 8) ^ (sibox(r1 >> 16) << 16) ^ (sibox(r0 >> 24) << 24) ^ kw[0][3];
    ...
    match kc {
        4 => {
            ...
            let (mut c0, mut c1, mut c2, mut c3) =
                (le_to_u32(key, 0), le_to_u32(key, 4), le_to_u32(key, 8), le_to_u32(key, 12));
            w[0] = [c0, c1, c2, c3];
            for i in 1..=10 {
        ...
        8 => {
            ...
            let (mut c4, mut c5, mut c6, mut c7) =
                (le_to_u32(key, 16), le_to_u32(key, 20), le_to_u32(key, 24), le_to_u32(key, 28));
            w[1] = [c4, c5, c6, c7];
            let mut i = 2usize;
            let mut rcon = 1u32;
            loop {
