...
/// http-server-sslengine-identity-singleton-clobber-FIXED.md. A
...
/// Like [`throw_invalid_key_spec`], but chains `cause` — the real throwable
/// that `drive_real_rsa_keyfactory` (or its PSS/PKCS#1 siblings) actually
/// raised before this fallback discarded it.
///
/// `H13-1` §2.3 / `H13-2` N1: `kf_generate_private`/`kf_generate_public` used
/// to drive the real SunRsaSign SPI and, on failure, throw a brand-new
/// `InvalidKeySpecException` with no cause and no JDK frames — "no worse than
/// the synthetic fail-closed path" as an OUTCOME, but a diagnosis-destroying
/// regression versus HotSpot's own `RSAKeyFactory`, which chains
/// (`throw new InvalidKeySpecException(e)`). `GeneralSecurityException` (the
/// superclass of `InvalidKeySpecException`) has had the `(String, Throwable)`
/// constructor since Java 7, so this is not a new shape — see
/// `bc_public_key_contract` for the same pin/read-back-after-allocation
/// pattern used to build a caused exception.
fn throw_invalid_key_spec_caused(
    ctx: &mut dyn NativeContext,
    msg: &str,
    cause: ObjectRef,
) -> MethodCallFailed {
    ...
    let detail = ctx.create_string(msg);
    let cause_now = ctx.read_native_pin(pin, cause);
    let built = ctx.new_object_initialized(
        "java/security/spec/InvalidKeySpecException",
        "(Ljava/lang/String;Ljava/lang/Throwable;)V",
        &[Value::Object(Some(detail)), Value::Object(Some(cause_now))],
    );
    let cause_now = ctx.read_native_pin(pin, cause);
    ctx.unpin_native_roots(pin);
    match built {
        Ok(Some(Value::Object(Some(exc)))) => MethodCallFailed::ExceptionThrown(exc),
        // Could not build the caused wrapper -- the real cause is still the
        // truthful answer, and swallowing it for a plain-message exception
        // would recreate exactly the defect this function exists to fix.
        _ => MethodCallFailed::ExceptionThrown(cause_now),
    }
}

/// Pull the `Throwable` out of a failed native-SPI drive, if it was a real
/// Java exception (as opposed to an internal VM error, which is not a
/// throwable this contract may repurpose as a cause).
fn caused_by(failed: &MethodCallFailed) -> Option<ObjectRef> {
    match failed {
        MethodCallFailed::ExceptionThrown(exc) => Some(*exc),
...
fn kf_generate_public(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let mut rsa_cause: Option<(usize, ObjectRef)> = None;
    if algo == ALGO_RSA && crate::route_rsa_to_real() {
        if let Some(Value::Object(Some(spec))) = args.get(1) {
            match drive_real_rsa_keyfactory(
                ctx,
                *spec,
                "engineGeneratePublic",
                "Ljava/security/PublicKey;",
            ) {
                Ok(Some(Value::Object(Some(mut key)))) => {
                    register_rsa_pub_verify_material(ctx, &mut key);
                    return Ok(Some(Value::Object(Some(key))));
                }
                // Real SPI unavailable/refused -- fall through to the
                // synthetic DER/RSAPublicKeySpec parse below, but remember
                // WHY in case that also fails (H13-1 §2.3 / H13-2 N1).
                Ok(_) => {}
                Err(e) => {
                    if let Some(exc) = caused_by(&e) {
                        rsa_cause = Some((ctx.pin_native_root(exc), exc));
                    }
                }
            }
        }
    ...
    if algo == ALGO_RSA {
        ...
        if let Some(pk) = pk_opt {
            // The synthetic DER/RSAPublicKeySpec parse is about to succeed,
            // so whatever the real-SPI drive above failed with is moot --
            // unpin it now rather than leaking the root past this function's
            // return (every return below this point takes the synthetic
            // path, not the chained-cause one at the bottom of this fn).
            if let Some((h, _)) = rsa_cause.take() {
    ...
    if let Some((h, exc)) = rsa_cause {
        let exc = ctx.read_native_pin(h, exc);
        ctx.unpin_native_roots(h);
        return Err(throw_invalid_key_spec_caused(ctx, &msg, exc));
    }
...
fn kf_generate_private(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // the real SPI is unavailable -- CHAINED to whatever the real SPI actually
    // threw (H13-1 §2.3 / H13-2 N1), instead of discarding it as the old
    // `if let Ok(r) = ...` shape did.
    let mut rsa_cause: Option<(usize, ObjectRef)> = None;
    if algo == ALGO_RSA {
        if let Some(Value::Object(Some(spec))) = args.get(1) {
            ...
            match drive_real_rsa_keyfactory(
                ...
                *spec,
                ...
                Ok(r) => {
                    ...
                    // r was None (real SPI unavailable / produced no key) — do
                    // NOT return a null PrivateKey; fall through to the PKCS#1
                    // retry and ultimately the InvalidKeySpecException below
                    // (fail closed).
                }
                Err(e) => {
                    if let Some(exc) = caused_by(&e) {
            ...
            if is_pkcs1_rsa_private(&der) {
                ...
                if let Ok(Some(Value::Object(Some(new_spec)))) = ctx.new_object_initialized(
                    ...
                    match drive_real_rsa_keyfactory(
                        ...
                        Ok(r) => {
                            if let Some(Value::Object(Some(mut key))) = r {
                                // Succeeding here means whatever the PKCS#8
                                // attempt above failed with is moot -- unpin
                                // it rather than leaking the root past this
                                // return.
                                if let Some((h, _)) = rsa_cause.take() {
                                    ctx.unpin_native_roots(h);
                                }
                        ...
                        Err(e) => {
                            // The PKCS#1 retry, when it runs, is the more
                            // specific attempt (the spec really was PKCS#1
                            // shaped) -- prefer its cause over the PKCS#8
                            // attempt's, which is likely just a decode refusal.
                            if let Some(exc) = caused_by(&e) {
                                if let Some((h, _)) = rsa_cause.take() {
                                    ctx.unpin_native_roots(h);
                                }
...
/// `Key.equals(Object)` for our synthetic `PublicKey`/`PrivateKey` mirrors —
/// KS-2/KS-3 (`jdk-only/keystore-getkey-accepts-any-
/// password-and-the-key-does-not-round-trip-20260910.md`): every real JDK
/// `Key` implementation (`sun.security.pkcs.PKCS8Key`, `...x509.X509Key`, the
/// base class of every provider's private/public key) defines `equals` as
/// "same algorithm, same encoding" — an ENCODING comparison, not identity.
/// Leaving `equals` unregistered fell back to `Object.equals` (reference
/// identity), so a key read back from a reloaded store, or a second `getKey`
/// call on the same entry, never compared equal to itself even when the bytes
/// were identical.
///
/// Goes through `getAlgorithm()`/`getEncoded()` as virtual calls rather than
/// reading fields directly, so this works whichever mirror layout `this` is
/// (`key_get_encoded` already resolves both the 5-slot `KeyFactory` layout
/// and the compact 4-slot `KeyStore.getKey()` proxy) and even against a real
/// (bytecode) `Key` on the other side — it only needs `other` to answer both
/// methods the way any `Key` must.
///
/// FOUR sequential `invoke_virtual` calls, and a real `getEncoded()` (ASN.1 /
/// `BigInteger` encoding) can allocate and trigger a moving young GC — so
/// `this`/`other` are pinned across the whole sequence and re-read after every
/// call, the same "native stale-local" discipline `pin_native_root`'s own doc
/// comment prescribes. Without it this returned `false` for a genuinely equal
/// pair whenever the FIRST `getAlgorithm()` call happened to relocate the
/// receivers: `this`/`other` used in every call after that were stale
/// addresses, so the later `getEncoded()` calls silently read whatever now
/// lived at the old address instead of the key.
fn key_equals(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    let mut this = this_arg(args)?;
    ...
    let algo_a = match ctx.invoke_virtual(this, "getAlgorithm", "()Ljava/lang/String;", &[]) {
        Ok(Some(Value::Object(Some(s)))) => ctx.read_string(s),
        _ => None,
    };
    this = ctx.read_native_pin(this_pin, this);
    other = ctx.read_native_pin(other_pin, other);
    let algo_b = match ctx.invoke_virtual(other, "getAlgorithm", "()Ljava/lang/String;", &[]) {
        Ok(Some(Value::Object(Some(s)))) => ctx.read_string(s),
        // `other` cannot even answer `getAlgorithm()` the way a `Key` must —
        // not a `Key`, or an incompatible one. `equals` returns false rather
        // than propagating whatever `invoke_virtual` failed with: the JDK
        // contract is that `equals` never throws for the wrong type.
        _ => {
            ctx.unpin_native_roots(this_pin);
    ...
    this = ctx.read_native_pin(this_pin, this);
    other = ctx.read_native_pin(other_pin, other);
    if algo_a.is_none() || algo_a != algo_b {
        ctx.unpin_native_roots(this_pin);
        return Ok(Some(Value::Int(0)));
    }
    let der_a = match ctx.invoke_virtual(this, "getEncoded", "()[B", &[]) {
    ...
    this = ctx.read_native_pin(this_pin, this);
    other = ctx.read_native_pin(other_pin, other);
    let der_b = match ctx.invoke_virtual(other, "getEncoded", "()[B", &[]) {
        Ok(Some(Value::Object(Some(arr)))) => read_byte_array(ctx, arr),
        _ => {
            ctx.unpin_native_roots(this_pin);
            return Ok(Some(Value::Int(0)));
    ...
    let _ = (this, other); // no longer read past this point
    ctx.unpin_native_roots(this_pin);
    // An empty encoding on either side (e.g. a mirror whose DER could not be
    // resolved) never counts as a match, even against another empty one.
    Ok(Some(Value::Int(i32::from(
        !der_a.is_empty() && der_a == der_b,
    ))))
}

/// `Key.hashCode()`, paired with [`key_equals`] above so the two keep the
/// `Object` contract (equal keys must hash equal) now that `equals` is
/// content-based rather than identity-based. `Arrays.hashCode(byte[])`'s own
/// algorithm over the encoding alone — not HotSpot's exact
/// `PKCS8Key`/`X509Key` formula (which also folds in the algorithm string),
/// since nothing here needs bit-exact hash VALUES, only that keys `equals`
/// agrees on also hash the same, which an encoding-only hash already
/// guarantees.
fn key_hash_code(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    let this = this_arg(args)?;
    let der = match ctx.invoke_virtual(this, "getEncoded", "()[B", &[]) {
        Ok(Some(Value::Object(Some(arr)))) => read_byte_array(ctx, arr),
        _ => Vec::new(),
    };
    let mut h: i32 = 1;
...
pub fn register(r: &mut NativeMethodRegistry) {
    ...
    // KS-2/KS-3: `equals`/`hashCode` were unregistered, so they fell back to
    // `Object`'s identity versions — see `key_equals`'s doc comment.
    r.register(
        "java/security/PublicKey",
        "equals",
        "(Ljava/lang/Object;)Z",
    ...
    r.register("java/security/PublicKey", "hashCode", "()I", key_hash_code);
    r.register(
    ...
    r.register("java/security/PrivateKey", "hashCode", "()I", key_hash_code);
...
mod tests {
    ...
    /// Dispatches `getAlgorithm`/`getEncoded` to the real natives under test,
    /// so [`key_equals`] (which calls both virtually, exactly as bytecode
    /// would) exercises the production code path rather than a stand-in.
    fn key_accessor_hook(
        ...
        match (method_name, descriptor) {
            ("getAlgorithm", "()Ljava/lang/String;") => {
                Some(key_get_algorithm(ctx, &[Value::Object(Some(receiver))]))
            }
            ("getEncoded", "()[B") => {
                Some(key_get_encoded(ctx, &[Value::Object(Some(receiver))]))
            }
            _ => None,
        }
    }

    /// KS-2/KS-3: two `KeyStore.getKey()` proxies over the SAME stored entry
    /// (a store/reload round trip, or a second `getKey` call) must compare
    /// equal — the real `Key.equals` contract is an encoding comparison, not
    /// identity. Before `key_equals` was registered this fell back to
    /// `Object.equals` and was always `false` for two distinct mirror objects.
    #[test]
    fn compact_keystore_private_keys_with_the_same_der_are_equal() {
        let alias = "ks2-ks3-round-trip";
        let der = b"test-pkcs8-der-for-equals".to_vec();
        let store = crate::keystore::LoadedKeyStore {
            entries: [(
                alias.to_string(),
                crate::keystore::KeyStoreEntry {
        ...
        let store_id = crate::keystore::keystore_register(store);
        let mut ctx = crate::test_utils::MockNativeContext::new();
        ctx.set_invoke_virtual_hook(key_accessor_hook);
        let alias_hash = alias.as_bytes().iter().fold(0x811c_9dc5_u32, |hash, byte| {
            (hash ^ u32::from(*byte)).wrapping_mul(0x0100_0193)
        });
        let composite = ((i64::from(store_id) & 0xffff_ffff) << 32) | i64::from(alias_hash);

        // Two SEPARATE proxy objects over the same entry, exactly like two
        // `getKey(alias, pw)` calls (or one before and one after a
        // store/reload round trip) would allocate.
        let key_a = try_alloc_concurrent_synthetic(&mut ctx, "java/security/PrivateKey", 4).unwrap();
        ctx.set_field(key_a, KEY_FIELD_KEYID, Value::Long(composite));
        let key_b = try_alloc_concurrent_synthetic(&mut ctx, "java/security/PrivateKey", 4).unwrap();
        ctx.set_field(key_b, KEY_FIELD_KEYID, Value::Long(composite));

        let result = key_equals(&mut ctx, &[Value::Object(Some(key_a)), Value::Object(Some(key_b))])
            .expect("equals must not throw")
            .expect("equals must return a boolean");
        assert_eq!(result, Value::Int(1), "same-DER keys must compare equal");

        // hashCode must agree with that equals verdict.
        let hash_a = key_hash_code(&mut ctx, &[Value::Object(Some(key_a))])
            .unwrap()
            .unwrap();
        let hash_b = key_hash_code(&mut ctx, &[Value::Object(Some(key_b))])
            .unwrap()
            .unwrap();
        assert_eq!(hash_a, hash_b, "equal keys must hash equal");

        // A DIFFERENT entry's key must not compare equal.
        let other_alias = "ks2-ks3-different-entry";
        let other_der = b"a-completely-different-key".to_vec();
        let other_store = crate::keystore::LoadedKeyStore {
            entries: [(
                other_alias.to_string(),
                crate::keystore::KeyStoreEntry {
                    alias: other_alias.to_string(),
                    creation_time_ms: 0,
                    kind: crate::keystore::EntryKind::PrivateKey {
                        key_der: other_der,
                        chain: Vec::new(),
        ...
        let other_store_id = crate::keystore::keystore_register(other_store);
        let other_alias_hash = other_alias
            .as_bytes()
            .iter()
            .fold(0x811c_9dc5_u32, |hash, byte| {
                (hash ^ u32::from(*byte)).wrapping_mul(0x0100_0193)
            });
        let other_composite = ((i64::from(other_store_id) & 0xffff_ffff) << 32)
            | i64::from(other_alias_hash);
        let key_c = try_alloc_concurrent_synthetic(&mut ctx, "java/security/PrivateKey", 4).unwrap();
        ctx.set_field(key_c, KEY_FIELD_KEYID, Value::Long(other_composite));
        let result = key_equals(&mut ctx, &[Value::Object(Some(key_a)), Value::Object(Some(key_c))])
            .unwrap()
            .unwrap();
        assert_eq!(result, Value::Int(0), "different-DER keys must not compare equal");

        // Reflexivity: a key always equals itself, even without a working
        // `getEncoded` round trip (the identity fast path).
        let result = key_equals(&mut ctx, &[Value::Object(Some(key_a)), Value::Object(Some(key_a))])
            .unwrap()
            .unwrap();
