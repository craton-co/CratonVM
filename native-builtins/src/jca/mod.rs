...
///
/// H13-2 §4 / N4: this used to fabricate a fresh `Provider` unconditionally,
/// so `sig.getProvider() == myProvider` was false for every third-party
/// `Signature`/`Mac`/`SecretKeyFactory` provider, where HotSpot answers
/// `true` (`Signature` never went through `provider_chain::build_third_party_
/// engine`'s own Delegate route at all, so `record_requested_provider`'s twin
/// fix could not reach it). Delegating to `resolve_or_make_provider` closes
/// that gap here too, and folds this function's fabrication into the one
/// place that already knows to prefer the object `Security.addProvider`
/// actually registered.
pub(crate) fn make_named_provider(
    ctx: &mut dyn cratonvm_native_api::NativeContext,
    name: &str,
) -> Result<cratonvm_types::ObjectRef, cratonvm_types::error::MethodCallFailed> {
    provider_chain::resolve_or_make_provider(ctx, name)
}
