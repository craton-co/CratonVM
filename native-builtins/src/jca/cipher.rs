...
pub fn register_cipher_clinit_shim(r: &mut NativeMethodRegistry) {
    ...
    //
    // That StackWalker hole is not JCA's and is NOT fixed here — this comment
    // is left in place as the diagnosis rather than deleted, since answering
    // here (below) keeps the clinit off the path regardless of the walker's
    // own state. E36-1 N3: the cross-reference that used to point at
    // `phases_late.rs`'s three (now-deleted, duplicate-fix-shadow)
    // `StackWalker$Option` static-field rows is repointed to the publisher
    // that was always the real one — `stack_walker.rs`'s
    // `native_option_clinit`, registered on this class's `<clinit>`. It reads
    // the loaded class's own declared fields (`option_constant_names`), which
    // `classloading/class_manager.rs`'s synthetic stub for this class now
    // declares all FOUR of, `DROP_METHOD_INFO` included — the missing fourth
    // constant this comment used to flag is added there.
    //
