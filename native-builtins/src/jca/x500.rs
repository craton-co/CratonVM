...
/// one instance field — `transient X500Name thisX500Name` — of type
/// `sun.security.x509.X500Name`. Real JDK classes (such as `PKCS7` and
/// `SignerInfo` via `X500Name.asX500Name(principal)`) read this field
/// expecting a `sun.security.x509.X500Name`. We therefore construct and
/// store a real `X500Name` in that slot, falling back to string storage
/// only if X500Name construction fails.
fn populate(ctx: &mut dyn NativeContext, this: ObjectRef, canonical: &str, der: &[u8]) {
    remember_der(ctx, this, der);
    let this_pin = ctx.pin_native_root(this);
    let x500_name_obj = if !der.is_empty() {
    ...
    if let Some(x_obj) = x500_name_obj {
        ctx.set_field_by_name(this, "thisX500Name", Value::Object(Some(x_obj)));
        ctx.set_field(this, FIELD_CANONICAL, Value::Object(Some(x_obj)));
    } else {
        let s = ctx.create_string(canonical);
...
fn get_canonical(ctx: &mut dyn NativeContext, this: ObjectRef) -> Option<String> {
    let field_val = match ctx.get_field_by_name(this, "thisX500Name") {
        Value::Object(Some(o)) => Some(o),
        _ => match ctx.get_field(this, FIELD_CANONICAL) {
            Value::Object(Some(o)) => Some(o),
            _ => None,
        },
    };
    let Some(obj) = field_val else {
        return None;
    };
    ...
    // If it's a sun.security.x509.X500Name, call getRFC2253Name()
    if let Ok(Some(Value::Object(Some(s)))) =
        ctx.invoke_virtual(obj, "getRFC2253Name", "()Ljava/lang/String;", &[])
    {
...
fn get_der(ctx: &mut dyn NativeContext, this: ObjectRef) -> Vec<u8> {
    if let Value::Object(Some(x_obj)) = ctx.get_field_by_name(this, "thisX500Name") {
        if let Ok(Some(Value::Object(Some(arr)))) =
            ctx.invoke_virtual(x_obj, "getEncoded", "()[B", &[])
        {
            let len = ctx.array_length(arr);
