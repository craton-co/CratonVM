...
pub(crate) fn register_p67_structured_task_scope(r: &mut NativeMethodRegistry) {
    ...
    // docs/jdk-only/W7-14-fjp-common-factory-bound-by-name.md gave
    ...
    // (docs/jdk-only/W7-5-registrars-that-never-shipped.md), which
...
// docs/jdk-only/W7-18-structured-task-scope-jep505.md):
//
...
fn j25_sts_join_result(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    match kind {
        ...
        // docs/jdk-only/W7-18-structured-task-scope-jep505.md; not
...
// docs/jdk-only/W7-14-fjp-common-factory-bound-by-name.md landed.
//
...
/// docs/jdk-only/W7-5-registrars-that-never-shipped.md. Their
...
fn fjt_fork_run_now(ctx: &mut dyn NativeContext, this: ObjectRef) -> MethodCallResult {
    // Fast-path skip when already driven by an enclosing join()/invoke() —
    // NOT what makes this exactly-once any more. `fjp_compute_for_submit`
    // now claims the task atomically before running it
    // (`phases_early::fjp_state_claim`), so calling it unconditionally would
    // still be correct here; this check just avoids the call in the common
    // case where the answer is already known.
