...
// ---------------------------------------------------------------------------
// G19-1 — the `java.lang.foreign.FunctionDescriptor` carrier
// ---------------------------------------------------------------------------

/// Slot 0 — the return layout, or `Object(None)` for a void descriptor.
pub(crate) const P67_FD_RETURN: usize = 0;
/// Slot 1 — the argument layouts, as a `MemoryLayout[]`.
pub(crate) const P67_FD_ARGS: usize = 1;
/// Slot 2 — the `java.util.List` view of slot 1, minted ONCE.
///
/// MEASURED on 25.0.3+9-LTS: `fd.argumentLayouts() == fd.argumentLayouts()` is
/// **true** — `FunctionDescriptorImpl` stores one immutable list and hands the
/// same object back. It was `false` here because the reader built a fresh
/// `List.of` on every call, which is an identity divergence a caller can see
/// with `==` and which no amount of structural correctness hides.
const P67_FD_LIST: usize = 2;
/// How wide the carrier is.
const P67_FD_SLOTS: usize = 3;

/// Allocate a `FunctionDescriptor` carrier, refusing the members the oracle
/// refuses BEFORE one exists.
///
/// MEASURED on 25.0.3+9-LTS (G19-1 §6.6). Both refusals were missing, and a
/// descriptor built from a refused member is worse than a late error: a
/// downcall then marshals from it.
///
/// | call | oracle |
/// |---|---|
...
/// | `of(sequenceLayout(2, JAVA_INT))` | accepted, `()[2:i4]` |
///
/// The last row is the boundary that keeps the padding rule from being
/// over-applied: only PADDING is refused, not "anything that is not a value
/// layout".
fn p67_fd_new(
    ctx: &mut dyn NativeContext,
    ...
    if let Some(ret) = return_layout {
        p67_fd_check_member(ctx, ret, "return")?;
    }
    let len = ctx.array_length(params);
    for i in 0..len {
        match ctx.get_array_element(params, i) {
            Value::Object(Some(member)) => p67_fd_check_member(ctx, member, "argument")?,
            // A null INSIDE the varargs array is the same bare NPE as a null
            // in the return position — the JDK's `Objects.requireNonNull`
            // walks the array before it builds anything.
            _ => return Err(RuntimeError::NullPointerException { message: None }.into()),
        }
    }
    // Both the members and the array they live in must survive the carrier's
    // allocation; `unpin_native_roots` truncates to its argument, so the first
    // handle releases both.
    let params_pin = ctx.pin_native_root(params);
    let ret_obj = return_layout.map(|r| (ctx.pin_native_root(r), r));
    let obj =
        try_alloc_concurrent_synthetic(ctx, "java/lang/foreign/FunctionDescriptor", P67_FD_SLOTS)?;
    let params = ctx.read_native_pin(params_pin, params);
    let ret = match ret_obj {
        Some((pin, r)) => Value::Object(Some(ctx.read_native_pin(pin, r))),
        None => Value::Object(None),
    };
    ctx.unpin_native_roots(params_pin);
    ctx.set_field(obj, P67_FD_RETURN, ret);
    ctx.set_field(obj, P67_FD_ARGS, Value::Object(Some(params)));
    // The list view is minted lazily by `p67_fd_argument_list`, not here: a
    // descriptor nobody asks for the arguments of should not pay for a
    // `List.of` call, and the laziness is invisible because the slot is the
    // only place the answer is ever read from.
    ctx.set_field(obj, P67_FD_LIST, Value::Object(None));
    Ok(Some(Value::Object(Some(obj))))
}

/// The one refusal `p67_fd_new` applies per member.
fn p67_fd_check_member(
    ...
    if class_name.contains("PaddingLayout") {
        return Err(RuntimeError::IllegalArgumentException {
            message: format!(
                "Unsupported padding layout {position} in function descriptor: {}",
...
/// Slot 1, or an empty array when the carrier is not one of ours.
fn p67_fd_arguments(ctx: &mut dyn NativeContext, fd: ObjectRef) -> ObjectRef {
    match ctx.get_field(fd, P67_FD_ARGS) {
        Value::Object(Some(arr)) if ctx.object_is_array(arr) => arr,
        _ => ctx.new_array(ArrayElementType::Reference, 0),
    }
}

/// The `java.util.List` view of slot 1, minted once and cached in slot 2.
fn p67_fd_argument_list(
    ctx: &mut dyn NativeContext,
    fd: ObjectRef,
) -> Result<ObjectRef, MethodCallFailed> {
    if let Value::Object(Some(cached)) = ctx.get_field(fd, P67_FD_LIST) {
        return Ok(cached);
    }
    let array = p67_fd_arguments(ctx, fd);
    // `List.of` with an `Arrays.asList` fallback is this tree's idiom for
    // handing back an immutable list (`lang_invoke::vh_coordinate_types`), and
    // it is measured working in this binary: `RJdkForeign`'s
    // `layoutVarHandles` step asserts
    // `coordinateTypes().equals(List.of(MemorySegment.class, long.class))` and
    // is green. `List.of` is also what makes `argumentLayouts().add(...)`
    // raise the oracle's message-less `UnsupportedOperationException`.
    ...
    let fd = ctx.read_native_pin(fd_pin, fd);
    let Some(list) = list else {
        ctx.unpin_native_roots(fd_pin);
        return Err(RuntimeError::IllegalStateException {
            message: "FunctionDescriptor.argumentLayouts could not build a List".into(),
        }
        .into());
    };
    let list_pin = ctx.pin_native_root(list);
    let list = ctx.read_native_pin(list_pin, list);
    ctx.set_field(fd, P67_FD_LIST, Value::Object(Some(list)));
    ctx.unpin_native_roots(fd_pin);
    Ok(list)
}

/// `insertArgumentLayouts`/`appendArgumentLayouts` — one body, because append
/// is insert at the end and the oracle treats them identically otherwise.
fn p67_fd_insert(
    ctx: &mut dyn NativeContext,
    fd: ObjectRef,
    at: usize,
    ...
    let existing = p67_fd_arguments(ctx, fd);
    let old_len = ctx.array_length(existing);
    let add_len = ctx.array_length(added);
    let fd_pin = ctx.pin_native_root(fd);
    let existing_pin = ctx.pin_native_root(existing);
    let added_pin = ctx.pin_native_root(added);
    let merged = ctx.new_array(ArrayElementType::Reference, old_len + add_len);
    let existing = ctx.read_native_pin(existing_pin, existing);
    let added = ctx.read_native_pin(added_pin, added);
    for i in 0..at {
        let v = ctx.get_array_element(existing, i);
        ctx.set_array_element(merged, i, v);
    }
    for i in 0..add_len {
        let v = ctx.get_array_element(added, i);
        ctx.set_array_element(merged, at + i, v);
    }
    for i in at..old_len {
        let v = ctx.get_array_element(existing, i);
        ctx.set_array_element(merged, i + add_len, v);
    }
    let fd = ctx.read_native_pin(fd_pin, fd);
    let ret = match ctx.get_field(fd, P67_FD_RETURN) {
        Value::Object(Some(r)) => Some(r),
        _ => None,
    };
    ctx.unpin_native_roots(fd_pin);
    p67_fd_new(ctx, ret, merged)
}

/// The `java.lang.invoke.MethodType` a descriptor describes.
///
/// MEASURED on 25.0.3+9-LTS: `of(JAVA_LONG, ADDRESS).toMethodType()` is
/// `(MemorySegment)long`, `ofVoid(JAVA_INT)` is `(int)void`, `of(JAVA_INT)` is
/// `()int` and `of(structLayout(...))` is `()MemorySegment`. The mapping is
/// the layout's CARRIER, which `p67_layout_carrier_name` already resolves for
/// every value layout — group and sequence layouts are not value layouts and
/// carry a `MemorySegment`, which that helper spells `java/lang/Object`, so
/// they are mapped here rather than by loosening it (it serves `carrier()`,
/// where `java/lang/Object` is the right answer for a shape we cannot
/// classify).
fn p67_fd_to_method_type(ctx: &mut dyn NativeContext, fd: ObjectRef) -> MethodCallResult {
    let args = p67_fd_arguments(ctx, fd);
    let len = ctx.array_length(args);
    let ret_name = match ctx.get_field(fd, P67_FD_RETURN) {
        Value::Object(Some(r)) => p67_fd_carrier_of(ctx, r),
        _ => "void",
    };
    let mut param_names: Vec<&'static str> = Vec::with_capacity(len);
    for i in 0..len {
        param_names.push(match ctx.get_array_element(args, i) {
            Value::Object(Some(m)) => p67_fd_carrier_of(ctx, m),
            _ => "java/lang/Object",
        });
    }
    let ret_mirror = p67_class_mirror(ctx, ret_name)?;
    let ret_pin = ctx.pin_native_root(ret_mirror);
    let params = ctx.new_array(ArrayElementType::Reference, param_names.len());
    let params_pin = ctx.pin_native_root(params);
    for (i, name) in param_names.iter().enumerate() {
        let mirror = p67_class_mirror(ctx, name)?;
        let params = ctx.read_native_pin(params_pin, params);
        ctx.set_array_element(params, i, Value::Object(Some(mirror)));
    }
    let params = ctx.read_native_pin(params_pin, params);
    let ret_mirror = ctx.read_native_pin(ret_pin, ret_mirror);
    let mt = ctx.invoke(
        "java/lang/invoke/MethodType",
        "methodType",
        "(Ljava/lang/Class;[Ljava/lang/Class;)Ljava/lang/invoke/MethodType;",
        &[Value::Object(Some(ret_mirror)), Value::Object(Some(params))],
    )?;
    ctx.unpin_native_roots(ret_pin);
    Ok(mt)
}

/// The carrier CLASS NAME a layout marshals through, in the spelling
/// [`p67_class_mirror`] takes.
fn p67_fd_carrier_of(ctx: &dyn NativeContext, layout: ObjectRef) -> &'static str {
    let class_name = ctx
    ...
    p67_layout_carrier_name(&class_name)
}

/// `FunctionDescriptorImpl.toString()` — `(a8)j8`, `(i4)v`, `()[2:i4]`.
///
/// MEASURED on 25.0.3+9-LTS. CratonVM printed
/// `java.lang.foreign.FunctionDescriptor@4b8` for every descriptor, which is
/// not only a divergence but a cross-VM DIFF HAZARD: the hash is
/// nondeterministic, so any vector that prints a descriptor is a flapping
/// diff. The `v` for a void return is the JDK's own spelling and is NOT the
/// empty string.
pub(crate) fn p67_fd_render(ctx: &dyn NativeContext, fd: ObjectRef) -> String {
    let mut out = String::from("(");
    if let Value::Object(Some(args)) = ctx.get_field(fd, P67_FD_ARGS) {
        if ctx.object_is_array(args) {
            for i in 0..ctx.array_length(args) {
                if let Value::Object(Some(m)) = ctx.get_array_element(args, i) {
                    out.push_str(&p67_layout_render(ctx, m));
                }
            }
        }
    }
    out.push(')');
    match ctx.get_field(fd, P67_FD_RETURN) {
        Value::Object(Some(r)) => out.push_str(&p67_layout_render(ctx, r)),
        _ => out.push('v'),
    }
    out
}

/// `FunctionDescriptorImpl.equals` — STRUCTURAL, over the member layouts.
///
/// MEASURED: `of(JAVA_INT, JAVA_INT).equals(of(JAVA_INT, JAVA_INT))` is
/// **true** on the oracle and was `false` here (`Object.equals`, i.e.
/// identity), while `of(JAVA_INT) == of(JAVA_INT)` is false on both — the
/// descriptors are structural but NOT interned, and a fix that interned them
/// would pass the first row and fail the second.
///
/// Members are compared through their OWN `equals`, not re-derived here: the
/// layout family's structural equality is already correct on this VM
/// (measured: `JAVA_INT.withName("a").equals(JAVA_INT.withName("a"))` is
/// true), and a second comparison written here would be a second opinion about
/// layout identity for the two to drift apart on.
pub(crate) fn p67_fd_equals(ctx: &mut dyn NativeContext, a: ObjectRef, b: ObjectRef) -> bool {
    if a == b {
        return true;
    }
    if !p67_is_fd_carrier(ctx, b) {
        return false;
    }
    let (a_ret, b_ret) = (
        ctx.get_field(a, P67_FD_RETURN),
        ctx.get_field(b, P67_FD_RETURN),
    );
    match (a_ret, b_ret) {
        (Value::Object(None), Value::Object(None)) => {}
        (Value::Object(Some(x)), Value::Object(Some(y))) => {
            if !p67_layout_equals(ctx, x, y) {
                return false;
            }
        }
        _ => return false,
    }
    let (aa, ba) = (p67_fd_arguments(ctx, a), p67_fd_arguments(ctx, b));
    let n = ctx.array_length(aa);
    if n != ctx.array_length(ba) {
        return false;
    }
    for i in 0..n {
        match (ctx.get_array_element(aa, i), ctx.get_array_element(ba, i)) {
            (Value::Object(None), Value::Object(None)) => {}
            (Value::Object(Some(x)), Value::Object(Some(y))) => {
                if !p67_layout_equals(ctx, x, y) {
                    return false;
...
/// Two layouts, compared through the layout family's own `equals`.
fn p67_layout_equals(ctx: &mut dyn NativeContext, x: ObjectRef, y: ObjectRef) -> bool {
    if x == y {
        return true;
    }
    matches!(
...
/// `FunctionDescriptorImpl.hashCode` — whatever it is, it must AGREE across
/// equal descriptors, which is the only thing the oracle's contract and
/// `RJdkForeign` both actually assert. Derived from the members' own
/// `hashCode`s so that agreement follows from [`p67_fd_equals`]'s definition
/// rather than being maintained beside it.
pub(crate) fn p67_fd_hash(ctx: &mut dyn NativeContext, fd: ObjectRef) -> i32 {
    let mut h: i32 = match ctx.get_field(fd, P67_FD_RETURN) {
        Value::Object(Some(r)) => p67_layout_hash(ctx, r),
        _ => 0,
    };
    let args = p67_fd_arguments(ctx, fd);
    for i in 0..ctx.array_length(args) {
        let member = match ctx.get_array_element(args, i) {
            Value::Object(Some(m)) => p67_layout_hash(ctx, m),
            _ => 0,
        };
        h = h.wrapping_mul(31).wrapping_add(member);
    }
    h
}

fn p67_layout_hash(ctx: &mut dyn NativeContext, layout: ObjectRef) -> i32 {
    match ctx.invoke_virtual(layout, "hashCode", "()I", &[]) {
        Ok(Some(Value::Int(n))) => n,
        _ => 0,
    }
}

/// Whether `obj` is one of the carriers [`p67_fd_new`] mints.
///
/// The class name, not the width: `try_alloc_concurrent_synthetic` takes the
/// LARGER of the requested slot count and the loaded class's own field count,
/// so the width is a floor and not an identity.
pub(crate) fn p67_is_fd_carrier(ctx: &dyn NativeContext, obj: ObjectRef) -> bool {
    !ctx.object_is_array(obj)
        && ctx
            .class_name_of_id(ctx.class_id_of_object(obj))
            .is_some_and(|n| n == "java/lang/foreign/FunctionDescriptor")
        && ctx.object_num_fields(obj) >= P67_FD_SLOTS
}
...
/// The one `Arena.global()` object, minted once and kept alive by a global
/// root.
///
/// A FUNCTION rather than the body of the `global` registration, because
/// `MemorySegment.NULL` needs the same object and reaching it through
/// `ctx.invoke("java/lang/foreign/Arena", "global", ...)` does not work: that
/// is a static call on an INTERFACE, which this VM resolves through a path the
/// force-route list does not cover, so it answered nothing and the `NULL`
/// carrier's scope slot was left empty. MEASURED after that first attempt:
/// `MemorySegment.NULL` was a stable object (identity hash 8382 twice) while
/// `NULL.scope()` still minted a fresh session per call (8461, then 8464).
/// One definition of "the global arena", called directly.
pub(crate) fn p67_global_arena(ctx: &mut dyn NativeContext) -> Result<ObjectRef, MethodCallFailed> {
    ...
    // A global root: the global arena outlives every native call and must
    ...
    match crate::panama::claim_global_arena(handle) {
        ...
        Some(published) => {
            ...
            Ok(ctx.resolve_global_root(published).unwrap_or(a))
        }
    }
}
...
pub(crate) fn p67_receiver_session(
    ...
    if ctx.object_num_fields(receiver) > P67_SEGMENT_ARENA {
        if let Value::Object(Some(owner)) = ctx.get_field(receiver, P67_SEGMENT_ARENA) {
            // SLOT 2'S THIRD TENANT IS A JAVA ARRAY, AND IT MUST NOT BE READ
            // AS ONE. `panama::SEG_BACKING_ARRAY_FIELD` is 2: an `ofArray`
            // MIRROR carrier (`int[]`/`long[]`/`float[]`/`double[]`) keeps its
            // backing array there. `p67_arena_session` below asks
            // `object_num_fields`/`get_field` about it, and an array mirrors
            // its LENGTH into `num_slots`, so the index check admits the read
            // and the byte offset it computes does not name the element it
            // claims to. MEASURED on 25.0.3+9-LTS, `--jdk-only`, before this
            // guard: `MemorySegment.ofArray(new int[8]).scope()` logged
            //
            //   ERROR cratonvm::gc::guard: zgc::get_field: plain-object field
            //   access on an ARRAY receiver … index=1 length=8
            //   element_type=Int class_id=ClassId(0)
            //
            // twice per call. The guard is the receiver's KIND, which is the
            // one question `is_plain_string`'s rule says to ask first; the
            // class-name test in `pe_session_modelled` cannot stand in for it,
            // because an array's class id is `ClassId(0)` and the name lookup
            // returns before the shape read that trips the guard.
            if ctx.object_is_array(owner) {
                // …and on that carrier the session is one slot further out.
                // G19-1 §6.2. `pe_mirror_session` is panama's, not a second
                // copy of the index: the two files disagreeing about what a
                // segment slot may hold is the drift §2.4 measured.
                if let Some(session) = crate::panama::pe_mirror_session(ctx, receiver) {
                    return Ok(Value::Object(Some(session)));
                }
                return Ok(p67_memory_session(ctx)?);
            }
...
pub(crate) fn p67_segment_check_scope(
    ...
    if ctx.object_num_fields(*segment) > P67_SEGMENT_ARENA {
        if let Value::Object(Some(mut owner)) = ctx.get_field(*segment, P67_SEGMENT_ARENA) {
            // Slot 2's array tenant, checked BEFORE `p67_arena_session` asks
            // the occupant about its fields — an array mirrors its LENGTH into
            // `num_slots`, so that read passes the index check and trips the
            // GC guard. See `p67_receiver_session` for the measured log line.
            // An `ofArray` mirror's session lives one slot further out and can
            // never close, but it is checked rather than waved through so that
            // this branch stays a check and not a fourth fail-open.
            if ctx.object_is_array(owner) {
                if let Some(mut session) = crate::panama::pe_mirror_session(ctx, *segment) {
...
///
/// `pub(crate)`: also raised by `panama::pe_session_check_open`, the choke
/// point `get`/`set`/`getAtIndex`/`setAtIndex` and `Arena.allocate` share —
/// W7-89 §6.3's confinement residual, closed by giving that function the same
/// owner check `p67_session_check_valid` already had.
pub(crate) fn p67_wrong_thread(ctx: &mut dyn NativeContext) -> MethodCallFailed {
...
pub(crate) fn p67_session_check_valid(
    ctx: &mut dyn NativeContext,
...
/// Check the `MemorySessionImpl` argument at `args[idx]`, if present and
/// non-null, before a scoped memory access proceeds.
///
/// W7-89 follow-up (the `ByteBuffer.asByteBuffer()`/G residual): the JDK's own
/// `ScopedMemoryAccess.getByte`/`getInt`/… bytecode is
/// `if (session != null) session.checkValidState(); return UNSAFE.getByte(…);`
/// — but this VM's `native_scoped_memory_get_*`/`put_*`/`copy_memory`
/// (`lib.rs`) adapt straight into the `Unsafe` implementation via
/// `scoped_memory_access_unsafe_args`, which drops the session argument
/// entirely rather than checking it. That is why a `DirectByteBuffer` created
/// through `MemorySegment.asByteBuffer()` kept reading/writing its backing
/// memory after the arena that owned it had closed, with no exception at
/// all: `Buffer.session()`/`checkSession()` are real, unshadowed bytecode and
/// correctly reach this native — the native was the one link in the chain
/// that never asked.
pub(crate) fn p67_check_scoped_session_arg(
    ctx: &mut dyn NativeContext,
    args: &[Value],
    idx: usize,
) -> Result<(), MethodCallFailed> {
    if let Some(Value::Object(Some(mut session))) = args.get(idx).copied() {
        p67_session_check_valid(ctx, &mut session)?;
    }
    Ok(())
}
...
pub(crate) fn p67_var_handle_for_layout(
    ...
    p67_var_handle_for_path(ctx, layout, layout, 0, &[])
}
...
///
/// `align_layout` is separate from `layout` (the LEAF, which supplies the
/// carrier/width/endianness) because the alignment a `VarHandle` enforces is
/// NOT the leaf member's own alignment — it is the alignment of the layout
/// `.varHandle(PathElement...)` was called ON. MEASURED on Temurin 25.0.4:
/// `MemoryLayout.unionLayout(JAVA_INT.withName("u0"), JAVA_LONG.withName("u1"))
/// .varHandle(groupElement("u0"))` on a 4-byte-alignable `int[2]` segment
/// throws `IllegalArgumentException: Target offset 0 is incompatible with
/// alignment constraint 8 (of [i4(u0)|j8(u1)])` — `8`, the UNION's own
/// `byteAlignment()`, even though `u0` alone (`JAVA_INT`) only needs 4 and a
/// bare `JAVA_INT.varHandle()` on the same segment succeeds. See
...
pub(crate) fn p67_var_handle_for_path(
    ...
    let shape = crate::lang_invoke::SegmentVhShape {
        ...
        alignment: p67_layout_align_of(ctx, align_layout),
    };
...
/// width when the carrier is not one of the nine (eight primitives plus
/// `ADDRESS`) this VM's layout `VarHandle`s know how to carry.
///
/// `ADDRESS`'s carrier IS `MemorySegment` (`p67_layout_carrier_name` names
/// it), not a pointer-width integer -- see
/// `crate::lang_invoke::FFM_ADDRESS_CARRIER`'s own doc for why that used to
/// be collapsed into `J` and why doing so was wrong. See
/// `docs/internal/jdk-only/
/// ffm-varhandle-address-carrier-and-missing-alignment-check-20260921-RETIRED-20260921.md`.
pub(crate) fn p67_carrier_descriptor_byte(carrier: &str, width: i32) -> u8 {
    match carrier {
        ...
        "float" => b'F',
        "double" => b'D',
        "java/lang/foreign/MemorySegment" => crate::lang_invoke::FFM_ADDRESS_CARRIER,
        _ => {
            if width == 8 {
                b'J'
            } else {
...
pub(crate) fn p67_memory_layout_var_handle(
    ...
    Ok(Some(p67_var_handle_for_path(
        ctx,
        target.layout,
        // `this`, NOT `target.layout`: see `p67_var_handle_for_path`'s doc
        // comment. A `groupElement` path's alignment is the enclosing
        // group's, not the resolved member's own.
        this,
        target.offset,
        &target.open_strides,
    )?))
...
pub(crate) fn p67_segment_address(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // A heap segment has no machine address, mirror carrier or not — MEASURED
    // on HotSpot 25.0.3+9-LTS, `ofArray(int[]).address()` is 0, same as
    // `ofArray(byte[]).address()`. Without this check, the `>= 6` branch
    // below read slot 0 positionally on every carrier that wide, including
    // the `ofArray(int[]|long[]|float[]|double[])` MIRROR carrier
    // (`panama::alloc_mirror_carrier`), whose slot 0 is a REAL native buffer
    // address kept for `sync_heap_backed_segment` to marshal at a downcall
    // boundary — an internal detail this public accessor must not leak.
    // `crate::panama::is_heap_shaped_segment` is the broader predicate here
    // on purpose: it also covers H1/H2 for receivers that reach this function
    // some other way than the two checks below already handle.
    if crate::panama::is_heap_shaped_segment(ctx, this) {
...
pub(crate) fn register_p67_foreign_memory(r: &mut NativeMethodRegistry) {
    ...
    r.register(
        ...
        "global",
        "()Ljava/lang/foreign/Arena;",
        |ctx, _args| Ok(Some(Value::Object(Some(p67_global_arena(ctx)?)))),
    );
    r.register(
        arena,
        "allocate",
    ...
    // FunctionDescriptor
    //
    // G19-1: THE CARRIER AND EVERY READER OF IT.
    //
    // MEASURED, `--jdk-only` against 25.0.3+9-LTS, before these registrations:
    //
    ...
    // `panama.rs` is reached only from
    // `register_pe_panama`/`register_synthetic_overrides` and does not run in
    // `--jdk-only` (registry dump: the only `FunctionDescriptor` rows are the
    // ones in this file, `owns_slot=true`). So the carrier was mintable and
    // unreadable.
    //
    // Every method on the interface is declared ABSTRACT and sealed, with no
    // `Object` fallback, which is why a registration here wins — and why the
    // six that stayed unregistered after G19-1 raised `AbstractMethodError`
    // rather than answering wrongly. They are registered below; the record's
    // §6.5 transcribes the oracle row each one reproduces.
    //
    // `toString`/`equals`/`hashCode` are the exception and cannot be served
    // from here: they DO have concrete `java.lang.Object` bodies, which win
    // dispatch. They are force-routed in
    // `vm/src/runtime/interpreter/native_override.rs` (the record's NOM-2) and
    // answered by `p67_fd_*` below.
    ...
    r.register(
        ...
        |ctx, args| {
            // NOT `obj_arg`: its NPE carries the text "null object argument",
            // and the oracle's is BARE. MEASURED: `FunctionDescriptor.of((MemoryLayout) null)`
            // is `NullPointerException` with a null message on 25.0.3+9-LTS.
            let return_layout = match args.first() {
            ...
            p67_fd_new(ctx, Some(return_layout), params)
        },
    );
    r.register(
        fd,
    ...
    r.register(fd, "returnLayout", "()Ljava/util/Optional;", |ctx, args| {
        ...
        let value = ctx.get_field(this, P67_FD_RETURN);
        let opt = p67_optional(ctx, value)?;
        Ok(Some(Value::Object(Some(opt))))
    });
    r.register(fd, "argumentLayouts", "()Ljava/util/List;", |ctx, args| {
        let this = obj_arg(args, 0)?;
        Ok(Some(Value::Object(Some(p67_fd_argument_list(ctx, this)?))))
    });
    r.register(
        fd,
        "toMethodType",
        ...
        |ctx, args| {
            let this = obj_arg(args, 0)?;
            p67_fd_to_method_type(ctx, this)
        },
    );
    r.register(
        fd,
        ...
        |ctx, args| {
            ...
            let params = p67_fd_arguments(ctx, this);
            p67_fd_new(ctx, Some(layout), params)
        },
    );
    r.register(
        fd,
        "dropReturnLayout",
        "()Ljava/lang/foreign/FunctionDescriptor;",
        |ctx, args| {
            let this = obj_arg(args, 0)?;
            let params = p67_fd_arguments(ctx, this);
            p67_fd_new(ctx, None, params)
    ...
    r.register(
        ...
        |ctx, args| {
            let this = obj_arg(args, 0)?;
            let added = match args.get(1) {
                Some(Value::Object(Some(arr))) => *arr,
                _ => return Err(RuntimeError::NullPointerException { message: None }.into()),
            };
            let existing = p67_fd_arguments(ctx, this);
            let at = ctx.array_length(existing);
            p67_fd_insert(ctx, this, at, added)
        },
    );
    r.register(
        fd,
        "insertArgumentLayouts",
        "(I[Ljava/lang/foreign/MemoryLayout;)Ljava/lang/foreign/FunctionDescriptor;",
        |ctx, args| {
            let this = obj_arg(args, 0)?;
            ...
            let added = match args.get(2) {
                Some(Value::Object(Some(arr))) => *arr,
                _ => return Err(RuntimeError::NullPointerException { message: None }.into()),
            };
            let existing = p67_fd_arguments(ctx, this);
            let len = ctx.array_length(existing);
            // MEASURED: `of(JAVA_LONG, ADDRESS).insertArgumentLayouts(9, JAVA_INT)`
            // is `IllegalArgumentException: Index out of bounds: 9` — an IAE
            // naming the INDEX, not an `IndexOutOfBoundsException` and not a
            // message naming the bound.
            if at < 0 || at as usize > len {
                return Err(RuntimeError::IllegalArgumentException {
                    message: format!("Index out of bounds: {at}"),
                }
                .into());
            }
            p67_fd_insert(ctx, this, at as usize, added)
        },
    );
    // G19-1 NOM-2. These three have a concrete `java.lang.Object` body, so a
    // registration only wins once the triple is on
    // `force_native_over_real_jdk_bytecode`'s list — which it now is, for this
    // class and these three names only. Registering them without that list
    // entry would be dead code; adding the list entry without these would
    // raise `AbstractMethodError` on every `println(fd)`. They move together.
    r.register(fd, "toString", "()Ljava/lang/String;", |ctx, args| {
        let this = obj_arg(args, 0)?;
        let text = p67_fd_render(ctx, this);
        Ok(Some(Value::Object(Some(ctx.create_string(&text)))))
    });
    r.register(fd, "equals", "(Ljava/lang/Object;)Z", |ctx, args| {
        let this = obj_arg(args, 0)?;
        let other = match args.get(1) {
            Some(Value::Object(Some(o))) => *o,
            // `x.equals(null)` is false, never a throw.
            _ => return Ok(Some(Value::Int(0))),
        };
        Ok(Some(Value::Int(i32::from(p67_fd_equals(ctx, this, other)))))
    });
    r.register(fd, "hashCode", "()I", |ctx, args| {
        let this = obj_arg(args, 0)?;
        Ok(Some(Value::Int(p67_fd_hash(ctx, this))))
    });
...
mod g19_scope_tests {
    ...
    /// G19-1 §6.2: an `ofArray` MIRROR carrier keeps its session one slot
    /// past the backing array, and the reader must find it there.
    ///
    /// The negative half is the test above: with an array in slot 2 and
    /// NOTHING in the mirror slot, the answer is still a fresh session and
    /// never the array.
    #[test]
    fn a_mirror_carriers_session_lives_past_its_backing_array() {
        let mut ctx = mock_ctx();
        let session = p67_memory_session(&mut ctx).unwrap();
        let session_obj = match session {
            Value::Object(Some(obj)) => obj,
        ...
        let array = ctx.new_array(cratonvm_types::ArrayElementType::Int, 8);
        let seg = crate::panama::alloc_segment_carrier(&mut ctx, 7).unwrap();
        ctx.set_field(seg, 0, Value::Long(0x1000));
        ctx.set_field(seg, 1, Value::Long(32));
        ctx.set_field(seg, P67_SEGMENT_ARENA, Value::Object(Some(array)));
        ctx.set_field(seg, 3, Value::Int(0));
        ctx.set_field(seg, 4, Value::Int(2));
        ctx.set_field(seg, 5, Value::Long(0));
        ctx.set_field(seg, 6, session);

        let first = p67_receiver_session(&mut ctx, seg).unwrap();
        let second = p67_receiver_session(&mut ctx, seg).unwrap();
        assert_eq!(first, session, "the mirror slot's session is the scope");
        assert_eq!(first, second, "and it is the same object every time");

        // …and it is CHECKED, not merely reported.
        let mut seg = seg;
        assert!(p67_segment_check_scope(&mut ctx, &mut seg).is_ok());
        let slots = p67_session_slots(&ctx, session_obj);
        ctx.set_field(session_obj, slots.state, Value::Int(0));
        let err = p67_segment_check_scope(&mut ctx, &mut seg).unwrap_err();
        assert!(
            format!("{err:?}").contains("Already closed"),
            "a closed mirror session must refuse; got {err:?}"
        );
    }

    /// The `FunctionDescriptor` carrier renders the oracle's spelling.
    ///
    /// MEASURED on 25.0.3+9-LTS: `of(JAVA_LONG, ADDRESS).toString()` is
    /// `(a8)j8` and `ofVoid(JAVA_INT)` is `(i4)v`. CratonVM printed
    /// `java.lang.foreign.FunctionDescriptor@31e3` — an identity hash, which
    /// is a cross-VM diff hazard as well as a divergence.
    #[test]
    fn a_descriptor_renders_as_the_oracle_spells_it() {
        let mut ctx = mock_ctx();
        let long = make_value_layout(&mut ctx, "OfLong", 8);
        let int = make_value_layout(&mut ctx, "OfInt", 4);

        let with_return = make_descriptor(&mut ctx, Some(long), &[int]);
        assert_eq!(p67_fd_render(&ctx, with_return), "(i4)j8");

        let void = make_descriptor(&mut ctx, None, &[int]);
        assert_eq!(p67_fd_render(&ctx, void), "(i4)v");

        let no_args = make_descriptor(&mut ctx, Some(int), &[]);
        assert_eq!(p67_fd_render(&ctx, no_args), "()i4");
    }

    /// `argumentLayouts()` hands back ONE list, not a fresh one per call.
    ///
    /// MEASURED: `fd.argumentLayouts() == fd.argumentLayouts()` is true on the
    /// oracle. The cache slot is what makes that hold, and a reader that
    /// rebuilt the list on every call satisfied `.equals` and failed `==`.
    #[test]
    fn the_argument_list_is_minted_once() {
        let mut ctx = mock_ctx();
        let int = make_value_layout(&mut ctx, "OfInt", 4);
        let fd = make_descriptor(&mut ctx, None, &[int]);
        // The mock has no `java.util.List`, so the helper reports the refusal
        // rather than silently answering null — what matters here is that the
        // CACHE SLOT is the only place the answer is read from, which the
        // second call proves by returning the object the first one stored.
        let stored = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 1);
        ctx.set_field(fd, P67_FD_LIST, Value::Object(Some(stored)));
        assert_eq!(p67_fd_argument_list(&mut ctx, fd).unwrap(), stored);
        assert_eq!(p67_fd_argument_list(&mut ctx, fd).unwrap(), stored);
    }

    /// The two refusals `p67_fd_new` owes, and the boundary row that keeps the
    /// padding rule from being over-applied.
    #[test]
    fn a_descriptor_refuses_a_null_and_a_padding_member() {
        let mut ctx = mock_ctx();
        let int = make_value_layout(&mut ctx, "OfInt", 4);
        let padding = make_padding_layout(&mut ctx, 4);

        let params = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 1);
        ctx.set_array_element(params, 0, Value::Object(None));
        let err = p67_fd_new(&mut ctx, Some(int), params).unwrap_err();
        assert!(
            format!("{err:?}").contains("NullPointerException"),
            "a null member is a bare NPE; got {err:?}"
        );

        let empty = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 0);
        let err = p67_fd_new(&mut ctx, Some(padding), empty).unwrap_err();
        assert!(
            format!("{err:?}").contains("Unsupported padding layout return in function descriptor"),
            "a padding RETURN names the return position; got {err:?}"
        );

        let params = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 1);
        ctx.set_array_element(params, 0, Value::Object(Some(padding)));
        let err = p67_fd_new(&mut ctx, None, params).unwrap_err();
        assert!(
            format!("{err:?}")
                .contains("Unsupported padding layout argument in function descriptor"),
            "a padding ARGUMENT names the argument position; got {err:?}"
        );

        // The boundary: only PADDING is refused, not "anything that is not a
        // value layout". `of(sequenceLayout(2, JAVA_INT))` is accepted by the
        // oracle and renders `()[2:i4]`.
        let empty = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 0);
        assert!(p67_fd_new(&mut ctx, Some(int), empty).is_ok());
    }

    /// A four-slot JDK-shaped value layout: `[0]=byteSize`, `[1]=byteAlignment`,
    /// classified by CLASS NAME, which is the carrier `--jdk-only` has.
    fn make_value_layout(ctx: &mut dyn NativeContext, suffix: &str, size: i64) -> ObjectRef {
        let class = format!("jdk/internal/foreign/layout/ValueLayouts${suffix}Impl");
        let cid = ctx.ensure_class_initialized(&class).unwrap();
        let layout = ctx.alloc_object(cid, 4);
        ctx.set_field(layout, 0, Value::Long(size));
        ctx.set_field(layout, 1, Value::Long(size));
        ctx.set_field(layout, 2, Value::Object(None));
        ctx.set_field(layout, 3, Value::Object(None));
        layout
    }

    fn make_padding_layout(ctx: &mut dyn NativeContext, size: i64) -> ObjectRef {
        let cid = ctx
            .ensure_class_initialized("java/lang/foreign/PaddingLayout")
            .unwrap();
        let layout = ctx.alloc_object(cid, 4);
        ctx.set_field(layout, 0, Value::Long(size));
    ...
    fn make_descriptor(
        ctx: &mut dyn NativeContext,
        ret: Option<ObjectRef>,
        args: &[ObjectRef],
    ) -> ObjectRef {
        let params = ctx.new_array(cratonvm_types::ArrayElementType::Reference, args.len());
        for (i, a) in args.iter().enumerate() {
            ctx.set_array_element(params, i, Value::Object(Some(*a)));
        }
        match p67_fd_new(ctx, ret, params).unwrap() {
            Some(Value::Object(Some(obj))) => obj,
            other => panic!("p67_fd_new answered {other:?}"),
        }
    }

    /// `MemorySegment.address()` on `ofArray(int[])` must answer 0, the same
    /// as every other heap segment.
    ///
    /// MEASURED on HotSpot 25.0.3+9-LTS: `MemorySegment.ofArray(new
    /// int[8]).address()` is 0, the same as `ofArray(new byte[8]).address()`
    /// — the JDK never exposes a heap segment's address, whatever its element
    /// type. Between 2026-09-20 and 2026-09-21 `ofArray(int[]/long[]/float[]/
    /// double[])` minted a MIRROR carrier with a real native buffer in slot
    /// 0, and this function's `>= 6` branch read that slot positionally, the
    /// same as it does for a genuine off-heap (`Arena`-backed) segment,
    /// leaking a nondeterministic native address instead of 0 — fixed then by
    /// gating this function on `crate::panama::is_heap_shaped_segment`, and
    /// fixed for good the next day when `ofArray` stopped minting the mirror
    /// shape at all (`pe_of_array_alias` now covers int[] directly, so this
    /// test exercises the REAL registration rather than a hand-built shape).
    #[test]
    fn ofarray_ints_address_does_not_leak_a_native_buffer() {
        let mut ctx = mock_ctx();
        let array = ctx.new_array(cratonvm_types::ArrayElementType::Int, 2);
        let seg = match crate::panama::pe_of_array_alias(&mut ctx, &[Value::Object(Some(array))])
            .unwrap()
        {
        ...
        assert_eq!(
            p67_segment_address(&mut ctx, &[Value::Object(Some(seg))]).unwrap(),
            Some(Value::Long(0)),
            "ofArray(int[])'s address() must be 0 like every other heap segment"
        );
    }

    /// Negative control for the test above: a PLAIN off-heap segment — the
    /// same six-slot shape `panama::pe_arena_allocate_impl` mints, with
    /// nothing array-shaped in slot 2 — is not heap-shaped and must keep
    /// reporting its real address. Without this the mirror fix could have
    /// been "always answer 0 for a 6-or-7-field carrier", which would have
    /// broken every ordinary `Arena.allocate` segment's `address()`.
    #[test]
    fn a_plain_native_segments_address_is_unaffected() {
        let mut ctx = mock_ctx();
        let seg = crate::panama::alloc_segment_carrier(&mut ctx, 6).unwrap();
        ctx.set_field(seg, 0, Value::Long(0x7f00_0000)); // real native address
        ctx.set_field(seg, 1, Value::Long(8)); // byteSize
        ctx.set_field(seg, 2, Value::Object(None)); // arena, not an array
        ...
        assert_eq!(
            p67_segment_address(&mut ctx, &[Value::Object(Some(seg))]).unwrap(),
            Some(Value::Long(0x7f00_0000)),
            "an ordinary off-heap segment must still report its real address"
        );
    }
}
...
fn register_p67_segment_surface(r: &mut NativeMethodRegistry, ms: &str) {
    ...
    r.register(
        ...
        |ctx, _args| {
            // G19-1 §6.1: `MemorySegment.NULL` IS A SINGLETON, and its scope is
            // the GLOBAL arena's. MEASURED on 25.0.3+9-LTS:
            //
            //     MemorySegment.NULL.scope() == MemorySegment.NULL.scope()  true
            //     MemorySegment.NULL.scope() == Arena.global().scope()      true
            //
            // Both were false here, because this accessor minted a fresh
            // two-slot carrier per read and a two-slot carrier has no slot 2
            // for `p67_receiver_session` to resolve a scope from — so every
            // read also got a freshly minted, always-open session.
            //
            // Same root-handle machinery as `Arena.global()` one screen up,
            // and for the same reason: the singleton outlives every native
            // call and must survive a moving collection.
            if let Some(existing) =
                crate::panama::null_segment_handle().and_then(|h| ctx.resolve_global_root(h))
            {
                return Ok(Some(Value::Object(Some(existing))));
            }
            // The global arena through the ONE function that owns it, not a
            // second mint and not `ctx.invoke` (see `p67_global_arena`).
            let arena = p67_global_arena(ctx)?;
            let arena_pin = Some((ctx.pin_native_root(arena), arena));
            let seg = crate::panama::alloc_segment_carrier(ctx, 3)?;
            let arena = match arena_pin {
                Some((pin, a)) => {
            ...
            ctx.set_field(seg, P67_SEGMENT_ARENA, arena);
            let handle = ctx.add_global_root(seg);
            match crate::panama::claim_null_segment(handle) {
                None => Ok(Some(Value::Object(Some(seg)))),
                // Lost the race — drop ours and answer with theirs.
                Some(published) => {
                    ctx.remove_global_root(handle);
                    let winner = ctx.resolve_global_root(published).unwrap_or(seg);
