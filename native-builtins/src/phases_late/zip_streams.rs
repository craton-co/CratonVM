...
pub(crate) fn register_p71_zip_extras(r: &mut NativeMethodRegistry) {
    ...
    // `java/util/zip/ZipFile`'s `<init>`/`getEntry`/`entries`/`size`/
    // `getName`/`close` used to be registered here too, as a standalone
    // "2-field (name=0, closed=1)" synthetic layout. That was a SECOND,
    // independent registration for a triple `native-io::zip_real_jar
    // ::register_jar_natives` (`[java/util/jar/JarFile,
    // java/util/zip/ZipFile]`) already covers completely — real archive
    // reading via the `zip` crate, `getManifest`, and (load-bearing for the
    // Mockito-redefine-immunity gate in
    // `vm/src/runtime/interpreter/native_override.rs`) `known_archive_ids`
    // tracking that THIS registration never populated.
    //
    // Two competing native bodies for the same triple, registered from two
    // different crates at two different points in boot, is a hazard on its
    // own (`NativeMethodRegistry::register_inner` is last-write-wins, so
    // which one is "the" implementation depends on registration ORDER, not
    // on which one is correct) — and this one additionally assumed a
    // 2-field synthetic layout that a receiver allocated as the REAL
    // `sun.nio.fs`-adjacent `java.util.zip.ZipFile` class (JVMS 6.5,
    // real `new`+`invokespecial <init>`) does not have: writing "name"/
    // "closed" at raw slots 0/1 on a real-layout object corrupts whichever
    // real fields happen to live there instead. `ZipFile$CleanableResource
    // .clean()` NPE-ing on a null `res` at `close()`
    // (`JarUrlConnectionTests`/`UrlJarFilesTests`,
    // `zipfile-cleanableresource-npe-on-nestedjarfile-close-20260922.md`)
    // is consistent with exactly that: whichever of the two `<init>`
    // registrations won, neither one is the real constructor, and this one
    // in particular never even claimed to set `res`.
    //
    // Removed rather than fixed in place: `register_jar_natives` is the
    // complete, already-correct implementation, and keeping two half-correct
    // ones in sync forever is worse than one.
