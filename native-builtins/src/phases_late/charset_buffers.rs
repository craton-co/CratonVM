...
pub(crate) fn register_p62_char_buffer(r: &mut NativeMethodRegistry) {
    ...
    // to fall back to.
    //
    // 2026-09-17: the cfg ALONE was the wrong guard, for the reason
    // `NativeMethodRegistry::drops_real_layout_synthetic`'s own doc spells out
    // (and that `servlet.rs`'s `java/io/InputStreamReader` cluster and the
    // real-layout drop filter's `java/io/StringWriter` entry in
    // `native-api/src/registry.rs` already learned): the Cargo feature
    // decides what is COMPILED, the launcher flag decides which CLASS LIBRARY
    // is LOADED. A `--features synthetic-jdk` binary run against a real JDK
    // satisfies the cfg and registers these two `wrap` overloads OVER the
    // real `java.nio.CharBuffer.wrap` bytecode, exactly the "DELIBERATELY not
    // registered in real-JDK mode" case the comment above describes — the
    // cfg alone cannot express "in real-JDK mode".
    //
    // `wrap(CharSequence)` is the concrete defect: real `CharBuffer.wrap(csq)`
    // returns a `StringCharBuffer` that HOLDS the `CharSequence` reference —
    // read-only (`isReadOnly() == true`), `hasArray() == false`, and any
    // later mutation of a mutable `csq` (e.g. a `StringBuilder`) is visible
    // through the buffer. This native instead COPIES the sequence's chars
    // into a fresh `char[]` and hands back a mutable `HeapCharBuffer`
    // (`isReadOnly() == false`, `hasArray() == true`) — silently wrong, not a
    // crash: `put()` succeeds where HotSpot throws `ReadOnlyBufferException`,
    // and the returned buffer is frozen at wrap-time instead of tracking the
    // live sequence. This is exactly the class of defect the real-layout drop
    // filter exists for, and it feeds the already-fixed Charset family
    // directly: Tomcat's `MessageBytes.toBytes` is
    // `encoder.encode(CharBuffer.wrap(charChunk))` (see the
    // `read_wrapped_char_sequence_units` doc below), so a wrongly-mutable,
    // wrongly-copied `CharBuffer` would flow straight into the real
    // `CharsetEncoder` the registry's Charset-family entry restored.
    ...
    fn cb_to_string_range(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
        ...
        let abs_start = cur_off + start;
        let abs_end = cur_off + end;
        // A DIRECT char view (`DirectCharBufferS/U`, and their read-only `R`
        // twins) has no `hb` -- its storage is off-heap, addressed by
        // `address` -- so the fall-through below answered "" for every
        // direct-backed view (WORKER-4-NOTE-7 §6). `address` has no separate
        // `offset` field to add (unlike the heap arm), which is why this
        // branch uses `abs_start`/`abs_end` directly rather than computing
        // its own. NOTE-7 measured `get(0..2)` correct on this same view
        // class (the STORE is fine), so route through the receiver's own
        // `get(int)` -- already proven correct -- rather than re-deriving
        // the byte-order/address arithmetic a second time here.
        if cname.contains("DirectCharBuffer") {
            let lo = abs_start.max(0);
            let hi = abs_end.max(lo);
            let mut chars: Vec<u16> = Vec::with_capacity((hi - lo) as usize);
            for i in lo..hi {
                if let Ok(Some(Value::Int(c))) =
                    ctx.invoke_virtual(this, "get", "(I)C", &[Value::Int(i)])
                {
                    chars.push(c as u16);
                }
            }
