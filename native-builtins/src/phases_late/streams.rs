...
pub(crate) fn register_phase56_function_extras(r: &mut NativeMethodRegistry) {
    ...
    //
    // The `test` natives on the three minted carriers used to be left
    // registered a few lines below, dead rather than wrong. DELETED,
    // 2026-09-21 (`H24-3` N2): `--dump-native-registry` during a Compatible
    // run of `RJdkFunctionCombinators` measured all three at zero invocations
    // while their call sites (`:104-121`, before the vector's assertion
    // boundary) were exercised and passed — the factories that minted these
    // receivers were already gone (H3-1, above), so nothing could ever reach
    // them. Re-froze `stub_ratchet.rs`'s three baselines against the actual
    // count this deletion produced, per that file's own "do not hand-derive"
    // rule.
    //
    // `force_native_over_real_jdk_bytecode`
    // (`vm/src/runtime/interpreter/native_override.rs`) still names these
    // triples, per H3-1's note above; that stale entry is unchanged by this
    // deletion and remains the next lane's to clean up.
    ...
    //
    // `Consumer$AndThen.accept` used to be left registered below for the same
    // reason as the `Predicate` carriers. DELETED, 2026-09-21 (`H24-3` N2),
    // same measurement and same reasoning as the `Predicate` triple above:
    // zero invocations with the call site (`:181-214`) exercised, and no
    // factory anywhere mints this class any more (H3-1).

    // --- M3 fix: register apply/accept on synthetic composition classes ---
    //
    // `Function$AndThen`/`Function$Compose` below are NOT part of the H24-3 N2
    // deletion, despite scoring the same "registered, zero invocations in a
    // Compatible run" shape: unlike the four rows above, a factory still mints
    // both of them whenever `drops_real_layout_synthetic()` is false — which is
    // exactly `VmConfig::default()`, the synthetic-jdk build with no real image
    // to fall back to. `vm/src/vm/tests.rs`'s
    // `m3_function_and_then_creates_composite`,
    // `m3_function_and_then_apply_chains_correctly`,
    // `m3_function_compose_creates_composite` and
    // `m3_function_compose_apply_chains_correctly` all reach these two exact
    // triples through `call_native`, which panics on an absent registration.
    // Deleting either would turn a Compatible-mode cosmetic cleanup into a
    // synthetic-jdk-mode regression. REFUSED for the reason H15-3 §4 N4 already
    // recorded for two of the four tests; the other two are `compose`'s.
    ...
    // Function$Identity.apply(x) = x — used to be registered again here,
    // `SyntheticStub`, as the other half of the copy L7 item 4 missed.
    // DELETED, 2026-09-21 (`H24-3` N2 / §3): this copy never owned the slot —
    // `register_function_identity_natives` in `native-builtins/src/lib.rs`
    // registers the same triple later and `owns_slot: true` there, MEASURED
    // from `--dump-native-registry`. Deleting a shadowed, non-owning
    // duplicate changes no behaviour in any mode; the live implementation is
    // `lib.rs:41327`'s `native_function_identity_apply`.
...
//
// See docs/jdk-only/E35-R11-SYNTHETIC-WIDTH-SWEEP-20260813.md §3.1
