...
fn merge_real_message_headers(ctx: &mut dyn NativeContext, this: ObjectRef) {
...
fn start_live_fixed_stream(
    ...
    merge_real_message_headers(ctx, this);
    let req = real_reqs()
        .lock()
        .ok()
...
pub(crate) fn real_forget(ctx: &dyn NativeContext, this: ObjectRef) {
    let key = ctx.identity_hash_code(this);
    if let Ok(mut t) = real_results().lock() {
        t.remove(&key);
    }
...
/// branch below)
/// — see `docs/jdk-only/E3-1-the-cipher-name-helper-and-its-real-denominator.md`.
        ...
        if let Ok(Some(Value::Object(Some(s)))) = call {
            if let Some(full) = ctx.read_string(s) {
                if full.contains("://") {
                    ...
                    // "already connected" by both streaming setters.
                    if let Ok(mut t) = live_fixed_streams().lock() {
                        t.remove(&key);
                    }
                    if let Ok(mut t) = real_body_streams().lock() {
                        t.remove(&key);
