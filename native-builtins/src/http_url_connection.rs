...
fn huc_real_perform(
    ...
    let resp = perform(ctx, &parsed, &method, &req.headers, &body, connect_to, read_to);
    match resp {
...
fn ensure_connected(ctx: &mut dyn NativeContext, this: ObjectRef) -> MethodCallResult {
    ...
    // `perform` manages its own (fine-grained) blocking regions internally —
    // see its doc — so this caller must not wrap the whole call in one.
    let perform_result = perform(ctx, &parsed, &method, &headers, &body, connect_to, read_to);
    let (status, headers, body_bytes) =
        perform_result.map_err(|e| ioex(format!("HttpURLConnection.connect failed: {e}")))?;
