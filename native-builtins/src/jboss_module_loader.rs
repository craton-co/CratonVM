...
/// `Module.loadService(Class)` -- the public instance method used by
/// WildFly's `org.jboss.as.controller.parsing.DeferredExtensionContext`
/// (the host-excludes / domain-mode deferred extension loading path
/// exercised by `HostExcludesTestCase`; verified against the constant
/// pool of `DeferredExtensionContext.class` in
...
/// The `addUses` call is a `uses`-clause bookkeeping side effect against
/// the JDK's own JPMS module graph that CratonVM's permissive module
/// model doesn't enforce -- skip it and reimplement the observable
/// contract directly via the module's own class loader (through
