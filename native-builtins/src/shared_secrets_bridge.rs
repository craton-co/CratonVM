...
/// docs/jdk-only/W7-27-thread-exit-java-cleanup.md.
///
...
/// docs/jdk-only/W7-18-structured-task-scope-jep505.md measured 15
...
/// docs/jdk-only/W7-23-thread-container-registration.md.
///
...
fn register_java_lang_access(registry: &mut NativeMethodRegistry) {
    ...
    // `docs/jdk-only/bug-two-drift-gates-are-red-on-pristine-dev-from-a-class-parameterised-registrar-20260822.md`
...
fn jnio_new_direct_byte_buffer(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // W7-89 follow-up: the real JDK's `NativeMemorySegmentImpl.makeByteBuffer()`
    // calls the 4-arg overload as `NIO_ACCESS.newDirectByteBuffer(addr, len,
    // null, this)`, passing the backing segment as args[4] so the minted
    // buffer's `Buffer.segment` field lets `session()`/`checkValidState()` see
    // it (`panama.rs::pe_segment_as_byte_buffer` intercepts
    // `MemorySegment.asByteBuffer()` before that real bytecode runs and is
    // fixed the same way, directly, where it builds its own `DirectByteBuffer`).
    // This used to hardcode `None` here, silently dropping the segment for any
    // OTHER real-bytecode caller of this overload: the resulting buffer's
    // `segment` field went null, so its liveness check had nothing to check.
    // The 2-arg overload (`args.get(4)` absent) is unaffected: it never had a
    // segment to carry.
    let seg = args.get(4).cloned().unwrap_or(Value::Object(None));
