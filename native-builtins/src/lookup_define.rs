...
fn lk_define_hidden_class_full(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let cid = match ctx.define_class_full(&hidden_name, &class_bytes, loader_id, opts) {
        Ok(cid) => cid,
        Err(msg) => {
            // initialize=true failures: HotSpot raises `ExceptionInInitializerError`
            // wrapping the real `<clinit>` exception (JVMS 5.5). See
            // `exception_in_initializer_error`'s doc comment.
...
fn lk_define_hidden_class_with_class_data(
    ...
    let cid = match ctx.define_class_full(&hidden_name, &class_bytes, loader_id, opts) {
        Ok(cid) => cid,
        Err(msg) => {
            // See `exception_in_initializer_error`'s doc comment (JVMS 5.5).
            if msg.contains("initialize after define failed") {
                return Err(crate::lang_system::exception_in_initializer_error(
                    ctx,
                    &hidden_name,
...
// `docs/internal/retired/method-handles-lookup-find-class-RETIRED-20260917.md`
...
fn lk_find_class(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_FINDCLASS") {
        let lookup_name = lookup_class_name(ctx, this_lookup);
