...
/// Where an `ofArray` MIRROR carrier used to keep its session.
///
/// RETIRED 2026-09-21: nothing mints this shape any more. `ofArray` unified
/// all seven primitive array types onto [`pe_of_array_alias`]'s carrier
/// (`SEG_HEAP_BASE_FIELD`/`SEG_HEAP_START_FIELD`), so int[]/long[]/float[]/
/// double[] no longer need a slot of their own for the session — they use
/// slot 2 the same way byte[]/short[]/char[] always did. Kept, with
/// [`SEG_MIRROR_FIELDS`] and [`pe_mirror_session`], purely as a defensive
/// recognizer: [`pe_mirror_session`] now always answers `None` (nothing is
/// ever `SEG_MIRROR_FIELDS` wide any more), and its two callers in
/// `phases_late/foreign_ffm.rs` fall through to their own alias-carrier
/// handling, unchanged.
const SEG_MIRROR_SESSION_FIELD: usize = 6;

/// How wide an `ofArray` MIRROR carrier used to be. See
/// [`SEG_MIRROR_SESSION_FIELD`] — retired, kept only so [`pe_mirror_session`]
/// still compiles as an inert recognizer.
const SEG_MIRROR_FIELDS: usize = 7;
...
/// `MemorySegment.NULL`'s root handle, the same shape as
/// [`global_arena_cell`] and for the same reason.
///
/// G19-1 §6.1. `MemorySegment.NULL` is a `static final` field on the oracle —
/// ONE object for the life of the VM. The accessor here minted a fresh
/// zero-length carrier on every read, so MEASURED on 25.0.3+9-LTS,
/// `--jdk-only`:
///
/// ```text
///   MemorySegment.NULL.scope() == MemorySegment.NULL.scope()
///       HotSpot   true     was   false
///   MemorySegment.NULL.scope() == Arena.global().scope()
///       HotSpot   true     was   false
/// ```
///
/// `NULL == NULL` happened to answer true anyway, because a static-field read
/// of a synthetic accessor is not how `==` on it was being evaluated — which
/// is exactly the kind of coincidence that makes a per-call mint look fixed
/// from one row.
fn null_segment_cell() -> &'static cratonvm_types::lock_order::OrderedPlMutex<Option<usize>> {
    use cratonvm_types::lock_order::{LockLevel, OrderedPlMutex};
    static CELL: std::sync::OnceLock<OrderedPlMutex<Option<usize>>> = std::sync::OnceLock::new();
    CELL.get_or_init(|| OrderedPlMutex::new(None, LockLevel::Scratch))
}

/// The published `MemorySegment.NULL` handle, if any. A function so the guard
/// cannot outlive the read — see [`global_arena_handle`].
pub(crate) fn null_segment_handle() -> Option<usize> {
    *null_segment_cell().lock()
}

/// Publish `handle` unless someone already did; returns the winner if not ours.
pub(crate) fn claim_null_segment(handle: usize) -> Option<usize> {
    let mut cell = null_segment_cell().lock();
    match *cell {
        Some(existing) => Some(existing),
        None => {
            *cell = Some(handle);
...
fn register_pe_memory_segment_on(r: &mut NativeMethodRegistry, ms: &str) {
    ...
    // segment may be handed to a downcall.
    //
    ...
    //
    // `heap_segment_view` is the ONE discriminator: it recognizes every heap
    // carrier this crate mints or reads (a real `HeapMemorySegmentImpl$Of*`,
    // and this file's own alias carrier, `[6]=array, [7]=start`, which is what
    // `ofArray` mints for all seven primitive array types and what `asSlice`
    // normalizes every heap segment down to). Measured on the oracle:
    // `ofArray(new byte[32]).asSlice(3, 4).isNative()` is **false**.
    r.register(ms, "isNative", "()Z", |ctx, args| {
        let this = obj_arg(args, 0)?;
        let heap_backed = crate::panama_libffi::is_real_heap_segment(ctx, this)
    ...
    r.register(
        ...
        |ctx, args| {
            ...
            // G19-1 §2.2: A RAW-ADDRESS SEGMENT IS IN THE GLOBAL SCOPE.
            //
            // MEASURED on 25.0.3+9-LTS: `MemorySegment.ofAddress(16).scope()
            // == Arena.global().scope()` is **true**, and it was false here —
            // slot 2 was the "no arena" marker, so `p67_receiver_session`
            // minted a fresh session on every `scope()` call and no two reads
            // of the same segment's scope agreed with each other either.
            // A raw address has no owner to close it, so the global arena is
            // not a placeholder here: it is the answer.
            let arena = crate::phases_late::foreign_ffm::p67_global_arena(ctx)?;
            ...
            ctx.set_field(seg, 2, Value::Object(Some(arena)));
    ...
    // ofArray(byte[] | short[] | char[] | int[] | long[] | float[] | double[])
    // → MemorySegment ALIASING the array, for all seven.
    //
    // `java.lang.foreign.MemorySegment` declares SEVEN `ofArray` overloads
    // (`javap`, 25.0.3+9-LTS: `byte[] char[] short[] int[] float[] long[]
    // double[]` — and NO `boolean[]`). `ofArray` is on
    // `native_override.rs`'s force-route NAME list, but a forced name with no
    // registration for that descriptor falls back to real bytecode, so a
    // missing arm produced a real `HeapMemorySegmentImpl$Of*` under
    // `--jdk-only` — and until F27 this file read one's `length` as its
    // address and the VM died with `EXCEPTION_ACCESS_VIOLATION … at address
    // 0x10` (W7-89 §7.1). In synthetic-JDK mode there is no real bytecode to
    // fall back to at all.
    //
    // UNTIL 2026-09-21, int[]/long[]/float[]/double[] used a SEPARATE
    // registration (`alloc_mirror_carrier`) that copied the array into an
    // off-heap native buffer, so those four could be handed straight to a
    // downcall (`sync_heap_backed_segment` re-synced the copy around the
    // call) — CratonVM cannot hand a moving Java array to native code, and
    // Elasticsearch's bulk-vector downcalls depend on passing one of these
    // four directly. Measured, the oracle does not permit that at all:
    //
    //     strlen(MemorySegment.ofArray("hi\0".getBytes()))
    //       -> IllegalArgumentException: Heap segment not allowed: MemorySegment{ kind: heap, … }
    //     strlen(MemorySegment.ofArray(new int[]{1,2,3}))
    //       -> IllegalArgumentException: Heap segment not allowed: MemorySegment{ kind: heap, … }
    //
    // and it DOES require aliasing, for every one of the seven:
    //
    //     byte[] a = new byte[8];
    //     MemorySegment.ofArray(a).set(JAVA_INT_UNALIGNED, 0, 0x01020304);
    //     // a == [4, 3, 2, 1, 0, 0, 0, 0]
    //     MemorySegment.ofArray(new int[2]).address() == 0L
    //       // true on the oracle for EVERY ofArray segment, numeric or not
    //
    // So the mirror was a wrong capability in both directions for the four
    // numeric types too — not just the accessor gap G6-1 found and fixed
    // (`heap_segment_view`'s former H3 arm), but the identity and address()
    // semantics themselves — and it leaked its native buffer on every call
    // (`allocate_native_memory` with no matching `free_native_memory`; see
    // the retired G6-1 follow-up). All seven now go through
    // [`pe_of_array_alias`], the SAME carrier `asSlice` already normalizes
    // every heap segment down to regardless of source. The element width
    // comes from the ARRAY, through the same `heap_element_width` the
    // accessors use, so the stride cannot drift between the factory and the
    // reader.
    //
    // Elasticsearch's downcall dependency is preserved, but moved to where it
    // belongs: `pe_downcall_invoke` now stands up a SCRATCH native buffer for
    // exactly the duration of one call when an int[]/long[]/float[]/double[]
    // alias segment is passed as a downcall pointer argument — copied in
    // before the call, copied back and freed right after — rather than
    // baking a permanent native mirror into the segment `ofArray` returns.
    // byte[]/short[]/char[] get no such carve-out, matching the oracle's
    // blanket refusal for those three (unchanged from before).
    for descriptor in [
    ...
    // `SegmentFactories.fromArray([F)` is the real-bytecode internal factory
    // `ofArray(float[])` itself dispatches through on some call paths; see
    // the historical note this carried before unification (2026-09-20,
    // still-open F27 residual for the other five primitives, resolved by the
    // loop above covering `ofArray` directly).
    r.register(
...
///
/// The real JDK's own name for this class (`javap`, 25.0.3+9-LTS:
/// `jdk.internal.foreign.AbstractMemorySegmentImpl$SegmentSplitter`), not a
/// plausible-looking one under `MemorySegment`, which has no such nested
/// class at all (`javap java.lang.foreign.MemorySegment$SegmentSplitter` ->
/// `class not found`). A `--jdk-only` run refuses to fabricate a class under
/// a name that names nothing real, so the wrong string here was invisible in
/// Compatible mode — where fabrication is unconditional — and surfaced only
/// as `NoClassDefFoundError` under `--jdk-only`, on the first
/// `elements`/`spliterator` call any vector made. MEASURED: building this
/// tree's own `origin/dev` and running `RJdkFfmSegment` under `--jdk-only`
/// reproduces the crash at check B8 (`RJdkFfmSegment.java:73`) even after the
/// G2-1/G6-1/G7-1 retirement that landed everything else this file measures —
/// that retirement's own gate run did not exercise this vector with
/// `CRATONVM_ARGS="--jdk-only"` set, so the crash went unseen.
const PE_SEGMENT_SPLITTER: &str = "jdk/internal/foreign/AbstractMemorySegmentImpl$SegmentSplitter";
...
///
/// `pub(crate)`: also the alignment check for the FFM layout `VarHandle`
/// path (`lang_invoke::layout_vh_access`), which has no alignment check of
/// its own — see `docs/internal/jdk-only/
/// ffm-varhandle-address-carrier-and-missing-alignment-check-20260921-RETIRED-20260921.md`.
pub(crate) fn pe_segment_max_byte_alignment_at(
...
fn pe_segment_slice(
    ...
    // `pe_segment_session` can return. This fallback only ever sees a
    // genuine off-heap (`Arena`-backed) receiver now — every heap segment,
    // `ofArray`'s included, is caught by the `heap_segment_view` branch
...
fn pe_segment_session(ctx: &dyn NativeContext, seg: ObjectRef) -> Option<ObjectRef> {
    ...
    // An `ofArray` MIRROR carrier keeps the array in slot 2 and the session one
    // slot further out. G19-1 §6.2.
    if let Some(session) = pe_mirror_session(ctx, seg) {
        return Some(session);
    }
...
/// The session stamped into an `ofArray` MIRROR carrier's
/// [`SEG_MIRROR_SESSION_FIELD`], if `seg` is one.
///
/// RETIRED 2026-09-21: `ofArray` no longer mints the seven-field mirror shape
/// this recognizes (see [`SEG_MIRROR_SESSION_FIELD`]), so this now always
/// answers `None`. Left in place, `pub(crate)`, and still called from
/// `foreign_ffm::p67_receiver_session` — an inert recognizer costs nothing to
/// keep and removing it would mean also auditing that file's own
/// `object_is_array(owner)` guard (kept for the same reason: defensive, now
/// unreachable, not worth the extra risk to touch in this change).
///
/// The width test is an EQUALITY, not a `>`: slot 6 on an eight-field alias
/// carrier is the backing ARRAY ([`SEG_HEAP_BASE_FIELD`]), and while
/// [`pe_session_modelled`] would reject it on the class-name test anyway, an
/// exact width says which carrier we are looking at rather than relying on the
/// occupant to disqualify itself.
pub(crate) fn pe_mirror_session(ctx: &dyn NativeContext, seg: ObjectRef) -> Option<ObjectRef> {
    if ctx.object_num_fields(seg) != SEG_MIRROR_FIELDS {
        return None;
    }
    match ctx.get_field(seg, SEG_MIRROR_SESSION_FIELD) {
        Value::Object(Some(session)) if pe_session_modelled(ctx, session) => Some(session),
        _ => None,
    }
}

/// Raise `IllegalStateException` if `seg`'s scope has already been closed, or
/// `WrongThreadException` if it is confined to a different thread.
///
...
/// `Err` if this MODELLED session is confined to another thread
/// (`WrongThreadException`) or has been closed (`IllegalStateException`),
/// thread checked first — the JDK's own order (`MemorySessionImpl
/// .checkValidStateRaw()`), so a confined session touched from the wrong
/// thread reports the thread error even once it has been closed.
///
...
/// reaches here.
///
/// W7-89 §6.3: this used to check liveness only, so a confined arena's
/// `get`/`set`/`allocate` off the owning thread never raised
/// `WrongThreadException` — `p67_session_check_valid` had the owner check all
/// along, but nothing on this file's access path called it. Reusing the same
/// slot map and the same exception builder keeps the two checks from drifting
/// apart again.
fn pe_session_check_open(
    ...
    let slots = crate::phases_late::foreign_ffm::p67_session_slots(ctx, session);
    if let Value::Object(Some(owner)) = ctx.get_field(session, slots.owner) {
        if ctx.current_thread_object() != owner {
            return Err(crate::phases_late::foreign_ffm::p67_wrong_thread(ctx));
        }
    }
...
//        of these from `MemorySegment.ofBuffer(ByteBuffer.allocate(n))`
//        (measured: a `HeapMemorySegmentImpl$OfByte`) and from `asSlice(long)`
//        on a real segment `ofArray` never touches.
//
//   H2 — a CratonVM-minted alias carrier, `[6]=array, [7]=byte start`, slot 0
//        deliberately 0. `MemorySegment.ofArray(...)` mints one directly for
//        all SEVEN primitive array types (`pe_of_array_alias`); `asSlice`
//        mints one whenever its receiver is any heap segment, H1 included —
//        before that it computed `segment_address(this) + offset` and
//        stamped the RESULT into slot 0, so a slice of a heap segment was a
//        synthetic segment whose "address" was its offset — F27's fix made
//        the un-sliced read refuse and left `asSlice(3, 4).get(...)`
//        dereferencing the literal address 3.
//
// A third shape, H3 — a native-mirror carrier `ofArray(int[]/long[]/float[]/
// double[])` used to mint instead of H2, so those four types could be handed
// straight to a downcall — is retired as of 2026-09-21; see the `ofArray`
// registration site and `pe_downcall_invoke`'s scratch-buffer pass for what
// replaced it.

/// A `MemorySegment` whose bytes live in a Java primitive array, resolved into
/// the pieces an access needs.
///
/// `pub(crate)`: also the heap-addressing resolver for the FFM layout
/// `VarHandle` path (`lang_invoke::layout_vh_access`), which used to refuse
/// EVERY heap-backed segment outright — see
/// `docs/internal/jdk-only/
/// ffm-varhandle-address-carrier-and-missing-alignment-check-20260921-RETIRED-20260921.md`.
#[derive(Clone, Copy)]
pub(crate) struct HeapSegmentView {
    /// The Java primitive array holding the bytes.
    pub(crate) base: ObjectRef,
    ...
    pub(crate) elem_width: usize,
    pub(crate) elem_type: cratonvm_types::ArrayElementType,
}
...
pub(crate) fn heap_element_width(elem: cratonvm_types::ArrayElementType) -> Option<usize> {
...
///
/// A third shape, H3 — a six/seven-field MIRROR carrier that
/// `ofArray(int[]|long[]|float[]|double[])` used to mint, with the array in
/// [`SEG_BACKING_ARRAY_FIELD`] instead of [`SEG_HEAP_BASE_FIELD`] — was
/// recognized here between 2026-09-20 and 2026-09-21. `ofArray` no longer
/// mints that shape (all seven primitive array types now share H2, the same
/// carrier `asSlice` already normalized every heap segment down to), so the
/// arm was retired along with [`SEG_BACKING_ARRAY_FIELD`]'s minting sites.
pub(crate) fn heap_segment_view(ctx: &dyn NativeContext, seg: ObjectRef) -> Option<HeapSegmentView> {
    let (base, start) = if crate::panama_libffi::is_real_heap_segment(ctx, seg) {
        ...
        (base, crate::panama_libffi::heap_segment_byte_start(raw_offset))
    ...
    // Slot 3 is the read-write flag on H2's eight-field shape (the factory
    // writes `3: Int(0) // read-write`), so the field-count gate only needs
    // to keep it off the real-JDK arm (H1), which answers through
    // `heap_seg_field`'s name resolution instead.
    let read_only = matches!(heap_seg_field(ctx, seg, "readOnly"), Value::Int(n) if n != 0)
        || (ctx.object_num_fields(seg) > 3
...
/// Whether `seg` is a heap-backed segment with NO native buffer behind it: a
/// real JDK `HeapMemorySegmentImpl` (H1), or this VM's `ofArray` ALIAS carrier
/// for `byte[]`/`short[]`/`char[]`/`int[]`/`long[]`/`float[]`/`double[]` (H2,
/// [`pe_of_array_alias`], which mints all seven now).
///
/// MEASURED on HotSpot 25.0.3+9-LTS (`FfmRepro2.java`): passing either a
/// `byte[]` or an `int[]` heap segment straight to a downcall throws, in both
/// cases —
///
///     java.lang.IllegalArgumentException: Heap segment not allowed:
///         MemorySegment{ kind: heap, heapBase: [B@…, address: 0x0, byteSize: 3 }
///
/// so the oracle draws no distinction between element types at all; CratonVM
/// stays permissive for int/long/float/double ONLY at the downcall boundary
/// itself (`pe_downcall_invoke`'s scratch-buffer pass runs BEFORE this
/// predicate's caller, `marshal_arg`, ever sees those four — see that pass's
/// own comment), not by making their `ofArray` segment a different shape. Once
/// a heap segment reaches `marshal_arg` directly, it is refused regardless of
/// element type. Before this predicate existed, `marshal_arg`'s
/// downcall-pointer refusal checked only
/// [`crate::panama_libffi::is_real_heap_segment`] (H1), so an H2 alias
/// carrier fell through to `segment_address`, which answers 0 for one — a
/// legitimate C null the marshaller deliberately lets through — and a
/// `MemorySegment.ofArray(byte[])` handed to a downcall silently became
/// `NULL` instead of the oracle's refusal.
///
/// Equivalent to [`is_heap_shaped_segment`] as of 2026-09-21 (both are
/// exactly "H1 or H2" now that H3 is retired); kept as a separate, cheaper
/// field-count-only check because that is what its one caller
/// (`marshal_arg`'s hot downcall-argument path) has always wanted.
pub(crate) fn is_alias_heap_segment(ctx: &dyn NativeContext, seg: ObjectRef) -> bool {
    crate::panama_libffi::is_real_heap_segment(ctx, seg)
        || ctx.object_num_fields(seg) > SEG_HEAP_START_FIELD
}

/// Whether `seg` is ANY heap-shaped segment this crate recognizes — H1 or H2
/// (see [`is_alias_heap_segment`]'s doc for why there were briefly three).
///
/// MEASURED on HotSpot 25.0.3+9-LTS: `MemorySegment.ofArray(new
/// int[8]).address()` is 0, the same as `ofArray(new byte[8]).address()` —
/// `FfmRepro2`-style probing once found `p67_segment_address`
/// (`phases_late/foreign_ffm.rs`, the registration behind the Java-visible
/// `MemorySegment.address()`) answering a since-retired mirror carrier's real
/// native pointer instead, because its shape-detection only ever recognised
/// "6 or more fields" as the off-heap `(ptr, size, …, offset)` carrier that a
/// PLAIN native (`Arena`-backed) segment also is. A caller that needs the
/// universal "heap segments have no machine address" rule wants this
/// predicate — it validates the FULL shape via [`heap_segment_view`], not
/// just a field count.
pub(crate) fn is_heap_shaped_segment(ctx: &dyn NativeContext, seg: ObjectRef) -> bool {
    crate::panama_libffi::is_real_heap_segment(ctx, seg) || heap_segment_view(ctx, seg).is_some()
}

/// The raw bits of one array element, zero-extended into a `u64`.
pub(crate) fn heap_element_bits(ctx: &dyn NativeContext, base: ObjectRef, index: usize) -> u64 {
...
///
/// Takes `base`/`elem_type` directly rather than a [`HeapSegmentView`] so
/// [`heap_bytes_write`] can serve a caller (`lang_invoke.rs::segment_heap_set`)
/// that has an array and an `ArrayElementType` but never builds a view.
pub(crate) fn heap_store_element(
    ctx: &dyn NativeContext,
    base: ObjectRef,
    ...
    ctx.set_array_element(base, index, value);
}
...
fn heap_segment_read(
    ...
    heap_bytes_read(ctx, view.base, view.elem_width, view.start + offset, width)
}

/// The element-width-aware core of [`heap_segment_read`], taking the backing
/// array and its element width directly rather than a [`HeapSegmentView`] —
/// so `lang_invoke.rs::segment_heap_get`'s `SegmentVarHandle` path can share
/// it instead of carrying its own byte-array-only copy (NOM F35-3: that copy
/// read every backing array as if it were a `byte[]`, so a `VarHandle` over
/// an `int[]`-backed `MemorySegment` read element `k` where it should have
/// read byte `k`).
pub(crate) fn heap_bytes_read(
    ctx: &dyn NativeContext,
    base: ObjectRef,
    elem_width: usize,
    byte_start: i64,
    ...
    let mut raw: u64 = 0;
    for i in 0..width {
        let byte_index = byte_start as usize + i;
        let element = byte_index / elem_width;
        let shift_in = byte_index % elem_width;
        let byte = (heap_element_bits(ctx, base, element) >> (8 * shift_in)) & 0xff;
        raw |= byte << (8 * i);
    }
    raw
}
...
/// The element-width-aware core of [`heap_segment_write`] — see
/// [`heap_bytes_read`], its read-side twin.
pub(crate) fn heap_bytes_write(
    ctx: &dyn NativeContext,
    base: ObjectRef,
    elem_type: cratonvm_types::ArrayElementType,
    elem_width: usize,
    ...
    for i in 0..width {
        let byte_index = byte_start as usize + i;
        let element = byte_index / elem_width;
        let shift_in = 8 * (byte_index % elem_width);
        let byte = (raw >> (8 * i)) & 0xff;
        let bits = heap_element_bits(ctx, base, element);
        heap_store_element(
            ctx,
            base,
            elem_type,
...
/// `MemorySegment.ofArray(...)` for all seven primitive array types — an
/// ALIAS carrier.
///
/// Mints the H2 shape: `[0]=0` (no machine address), `[1]=byteSize`,
/// `[2]=this segment's session`, `[6]=the array`, `[7]=0`. The element width is
/// read off the array itself through [`heap_element_width`], so the factory
/// and the accessors cannot disagree about the stride. See the registration
/// site for why int[]/long[]/float[]/double[] used to mint a different,
/// copying carrier here and no longer do.
pub(crate) fn pe_of_array_alias(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    let width = match heap_element_width(ctx.heap_element_type_of(array)) {
        ...
        _ => {
            return Err(RuntimeError::IllegalArgumentException {
                message: "MemorySegment.ofArray requires a byte[], short[], char[], int[], \
    ...
    // directly", which is exactly this shape.
...
pub(crate) fn pe_downcall_invoke(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // Elasticsearch's bulk-vector downcalls depend on handing
    // `MemorySegment.ofArray(int[]|long[]|float[]|double[])` straight to a
    // downcall. The oracle refuses that outright for EVERY heap segment,
    // numeric or not (measured, `FfmRepro2.java`:
    // `strlen.invoke(MemorySegment.ofArray(new int[]{1,2,3}))` throws
    // `IllegalArgumentException: Heap segment not allowed`) — this VM stays
    // permissive for those four element types only, same as it always has.
    // `marshal_arg`'s `is_alias_heap_segment` refuses byte[]/short[]/char[]
    // (matching the oracle, no carve-out); this pass runs FIRST and only
    // ever touches the four numeric types, by substituting a scratch native
    // buffer for the argument before `marshal_arg` ever sees it, so that
    // refusal never fires for them.
    //
    // The array itself cannot be hived off to native code (moving GC), so
    // the buffer stands in for exactly this call: copied in now, copied
    // back and freed right after the call returns. This replaces the
    // permanent native mirror `ofArray` used to allocate and never free —
    // see the retired `alloc_mirror_carrier` — with one that is scoped to a
    // single call and does not leak.
    struct PermissiveHeapArg {
        alloc_id: i64,
        ptr: usize,
        size: usize,
        segment: ObjectRef,
    }
    let mut permissive_heap_args: Vec<PermissiveHeapArg> = Vec::new();
    for value in call_args.iter_mut() {
        let Value::Object(Some(segment)) = *value else {
            continue;
        };
        // A real JDK heap segment gets no carve-out either; leave it for
        // `marshal_arg` to refuse.
        if crate::panama_libffi::is_real_heap_segment(ctx, segment) {
            continue;
        }
        let Some(view) = heap_segment_view(ctx, segment) else {
            continue;
        };
        use cratonvm_types::ArrayElementType as A;
        if !matches!(view.elem_type, A::Int | A::Long | A::Float | A::Double) {
            continue; // byte[]/short[]/char[]: stays refused, matching the oracle
        }
        let size = view.size.max(0) as usize;
        let Some((alloc_id, ptr)) = ctx.allocate_native_memory(size.max(1), view.elem_width) else {
            return Err(RuntimeError::OutOfMemoryError {
                message: "Failed to allocate a scratch buffer for a downcall argument".into(),
            }
            .into());
        };
        if let Some(bytes) = heap_read_bytes(ctx, &view, 0, size) {
            // SAFETY: `ptr` was just allocated with room for `size` bytes;
            // `bytes.len() == size` by construction of `heap_read_bytes`.
            unsafe { std::ptr::copy_nonoverlapping(bytes.as_ptr(), ptr, size) };
        }
        permissive_heap_args.push(PermissiveHeapArg {
            alloc_id,
            ptr: ptr as usize,
            size,
            segment,
        });
        *value = Value::Long(ptr as i64);
    }
    ...
    // Copy each scratch buffer back into its Java array and free it — the
    // buffer's whole lifetime is this one call. `heap_segment_view` is
    // re-resolved rather than reusing the pre-call `view`: nothing else in
    // this function pins `segment` across `ffi_call` (an upcall reached
    // through a libffi closure during the call could re-enter Java and
    // allocate), which is the same discipline this function already applies
    // to `capture_state` and `handle` — not a new gap this pass introduces.
    for arg in &permissive_heap_args {
        if let Some(view) = heap_segment_view(ctx, arg.segment) {
            let mut buf = vec![0u8; arg.size];
            // SAFETY: `arg.ptr` was allocated with room for `arg.size` bytes
            // above and is still live — it is freed only after this copy.
            unsafe {
                std::ptr::copy_nonoverlapping(arg.ptr as *const u8, buf.as_mut_ptr(), arg.size)
            };
            heap_write_bytes(ctx, &view, 0, &buf);
        }
        ctx.free_native_memory(arg.alloc_id);
    }
...
fn register_pe_function_descriptor(r: &mut NativeMethodRegistry) {
    ...
    // argumentLayouts() → java.util.List
    //
    // G19-1 NOM-1. THE SIGNATURE WAS `()[Ljava/lang/foreign/ValueLayout;` —
    // the pre-JDK-22 PREVIEW spelling. MEASURED on 25.0.3+9-LTS:
    // `argumentLayouts()` returns `java.util.List`
    // (`java.util.ImmutableCollections$List12` for a one-argument
    // descriptor), so a caller writing `fd.argumentLayouts().size()` against
    // the shipping API got `AbstractMethodError: ...argumentLayouts()
    // Ljava/util/List; has no Code attribute` even where this registrar IS
    // reached — which is `--synthetic-jdk` only, since
    // `register_pe_function_descriptor` runs from `register_pe_panama` /
    // `register_synthetic_overrides` and neither shipping binary executes it.
    // The row that a `--jdk-only` binary answers is
    // `foreign_ffm::register_p67_foreign_memory`'s; the two now agree on the
    // descriptor, so whichever wins a given build answers the same question.
    r.register(fd, "argumentLayouts", "()Ljava/util/List;", |ctx, args| {
        let this = obj_arg(args, 0)?;
        let array = match ctx.get_field(this, 1) {
...
pub(crate) fn pe_segment_reinterpret(
    ...
    // G19-1 §2.2: A REINTERPRETED SEGMENT STAYS IN ITS PARENT'S SCOPE.
    //
    // MEASURED on 25.0.3+9-LTS: `cs.reinterpret(8).scope() == cs.scope()` is
    // **true** — the one-argument overload keeps the original's scope; only
    // the three-argument `reinterpret(long, Arena, Consumer)` moves a segment
    // to a different one. Slot 2 was an unconditional `Object(None)` here,
    // which is the same "no scope resolvable" hole `asSlice`'s two arms each
    // had before W7-89 and G19-1 closed them, and it has the same second
    // consequence: `p67_receiver_session` minted a fresh ALWAYS-OPEN session,
    // so a reinterpretation of a closed arena's segment reported a live scope
    // and every liveness check resolving through it passed.
    let parent_session = pe_segment_session(ctx, this);
    let session_pin = parent_session.map(|session| ctx.pin_native_root(session));
    let seg = alloc_segment_carrier(ctx, 6)?;
    // The allocation above is a safepoint and can move the session; re-read it
    // through the pin before storing it.
    let scope_value = match (parent_session, session_pin) {
        (Some(session), Some(pin)) => {
            let session = ctx.read_native_pin(pin, session);
            ctx.unpin_native_roots(pin);
    ...
    ctx.set_field(seg, 2, scope_value);
...
mod tests {
    ...
    /// `ofArray(int[])` mints an ALIAS segment, not a native copy: no
    /// machine address, and a read through the segment sees the array's own
    /// live elements. This is `pe_of_array_alias`, the real registration —
    /// not a re-simulation of the retired copy-into-native-memory logic this
    /// test used to run inline.
    #[test]
    fn test_85_2_of_array_int() {
        ...
        let seg = match pe_of_array_alias(&mut ctx, &[Value::Object(Some(arr))]).unwrap() {
            Some(Value::Object(Some(seg))) => seg,
            other => panic!("ofArray(int[]) answered {other:?}"),
        };
        assert_eq!(
            crate::panama_libffi::segment_address(&ctx, seg),
            0,
            "an ofArray segment has no machine address, int[] included"
        );
        let layout_int = make_layout(&mut ctx, LAYOUT_INT);
        for i in 0..4i64 {
            let value = pe_segment_get_impl(&mut ctx, seg, layout_int, i * 4).unwrap();
            assert_eq!(
                value,
                Some(Value::Int((i as i32 + 1) * 10)),
                "reading through the alias segment must see the array's own element {i}"
            );
        }
    }
    ...
    /// Writes `value` into every element of a native `int` buffer — standing
    /// in for a real native mutation, the way Elasticsearch's bulk-vector
    /// kernels write results back into a buffer a caller handed them.
    extern "C" fn fill_int_buffer(ptr: *mut i32, value: i32, count: i64) {
        for i in 0..count {
            unsafe { *ptr.add(i as usize) = value };
        }
    }

    /// `MemorySegment.ofArray(int[])` handed straight to a downcall must
    /// still work, end to end, through the REAL `pe_downcall_invoke` path —
    /// not just `marshal_arg` in isolation (`panama_libffi::tests::
    /// a_mirror_heap_segment_pointer_argument_still_marshals` covers that
    /// narrower claim, but the shape it feeds `marshal_arg` was hand-built
    /// and, as of this same change, no longer exists).
    ///
    /// This is Elasticsearch's own dependency, unchanged from before the
    /// int[]/long[]/float[]/double[] `ofArray` unification: the array itself
    /// cannot be handed a raw pointer into (moving GC), so
    /// `pe_downcall_invoke`'s scratch-buffer pass must stand a real native
    /// buffer in for the call, and — the round trip this test is really
    /// about — copy the native side's WRITE back into the Java array once
    /// the call returns, the same guarantee the retired mirror carrier used
    /// to provide via `sync_heap_backed_segment`.
    #[test]
    fn permissive_downcall_writes_back_into_the_ofarray_int_segment() {
        let mut ctx = mock_ctx();
        // FIX(test): grant native access for the real libffi downcall.
        let _na = NativeAccessGuard::enable();
        let fn_addr = fill_int_buffer as usize as i64;

        let array = ctx.new_array(cratonvm_types::ArrayElementType::Int, 4);
        let seg = match pe_of_array_alias(&mut ctx, &[Value::Object(Some(array))]).unwrap() {
            Some(Value::Object(Some(seg))) => seg,
            other => panic!("ofArray(int[]) answered {other:?}"),
        };
        // The segment itself has no machine address — this is exactly why a
        // scratch buffer, not the segment's own (nonexistent) address, has
        // to stand in for it at the call boundary.
        assert_eq!(crate::panama_libffi::segment_address(&ctx, seg), 0);

        let ret_layout = None;
        let p_addr = make_layout(&mut ctx, LAYOUT_ADDRESS);
        let p_int = make_layout(&mut ctx, LAYOUT_INT);
        let p_long = make_layout(&mut ctx, LAYOUT_LONG);
        let params_arr = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 3);
        ctx.set_array_element(params_arr, 0, Value::Object(Some(p_addr)));
        ctx.set_array_element(params_arr, 1, Value::Object(Some(p_int)));
        ctx.set_array_element(params_arr, 2, Value::Object(Some(p_long)));

        let descriptor =
            try_alloc_concurrent_synthetic(&mut ctx, "java/lang/foreign/FunctionDescriptor", 2)
                .unwrap();
        ctx.set_field(descriptor, 0, Value::Object(ret_layout));
        ctx.set_field(descriptor, 1, Value::Object(Some(params_arr)));

        let handle = mk_downcall_handle(&mut ctx, fn_addr, Some(descriptor), -1);

        let call_args = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 3);
        ctx.set_array_element(call_args, 0, Value::Object(Some(seg)));
        ctx.set_array_element(call_args, 1, Value::Int(0x2A2A2A2Au32 as i32));
        ctx.set_array_element(call_args, 2, Value::Long(4));
        ...
        assert!(
            result.is_ok(),
            "ofArray(int[]) must still be permitted as a downcall pointer \
             argument: {:?}",
            result.err()
        );
        ...
        for i in 0..4 {
            assert_eq!(
                ctx.get_array_element(array, i),
                Value::Int(0x2A2A2A2Au32 as i32),
                "the native write must be copied back into the Java array at index {i}"
            );
        }
    }

    /// Negative control for the test above, through the SAME real
    /// `pe_downcall_invoke` path: `ofArray(byte[])` gets no scratch-buffer
    /// carve-out and must still be refused, matching the oracle
    /// (`IllegalArgumentException: Heap segment not allowed`, measured on
    /// HotSpot 25.0.3+9-LTS for every `ofArray` element type — see
    /// `panama::is_alias_heap_segment`'s doc). The pre-pass in
    /// `pe_downcall_invoke` only ever substitutes a scratch buffer for
    /// Int/Long/Float/Double; a byte[] segment reaches `marshal_arg`
    /// unchanged and is refused there, same as before the unification this
    /// change makes to the four numeric types.
    #[test]
    fn permissive_downcall_still_refuses_an_ofarray_byte_segment() {
        let mut ctx = mock_ctx();
        let _na = NativeAccessGuard::enable();
        // Never actually invoked -- the refusal happens before `ffi_call`.
        let fn_addr = fill_int_buffer as usize as i64;

        let array = ctx.new_array(cratonvm_types::ArrayElementType::Byte, 16);
        let seg = match pe_of_array_alias(&mut ctx, &[Value::Object(Some(array))]).unwrap() {
            Some(Value::Object(Some(seg))) => seg,
            other => panic!("ofArray(byte[]) answered {other:?}"),
        };
        ...
        let descriptor =
            try_alloc_concurrent_synthetic(&mut ctx, "java/lang/foreign/FunctionDescriptor", 2)
                .unwrap();
        ctx.set_field(descriptor, 0, Value::Object(None));
        ctx.set_field(descriptor, 1, Value::Object(Some(params_arr)));
        ...
        let call_args = ctx.new_array(cratonvm_types::ArrayElementType::Reference, 3);
        ctx.set_array_element(call_args, 0, Value::Object(Some(seg)));
        ctx.set_array_element(call_args, 1, Value::Int(0x2A));
        ctx.set_array_element(call_args, 2, Value::Long(16));
        ...
        let Err(err) = result else {
            panic!("ofArray(byte[]) must be refused as a downcall pointer argument");
        };
        match err {
    ...
    /// G19-1: stamping the session into slot 2 must not disturb the slot's
    /// older tenant, the owning arena on an arena-allocated segment.
    /// `heapBase()` is the one with a visible return value, so it is the one
    /// asserted: the oracle says an alias carrier's `heapBase()` is present
    /// and holds the caller's array (`G6-1` §6, row 1) — reading it from slot
    /// 6, not slot 2, is what proves the session in slot 2 was not mistaken
    /// for something else.
    #[test]
