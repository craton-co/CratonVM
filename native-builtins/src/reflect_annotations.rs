...
pub(crate) fn register_annotation_overrides(registry: &mut NativeMethodRegistry) {
    ...
    // See docs/jdk-only/W7-5-registrars-that-never-shipped.md.
    //
    ...
    // 20260917.md` names all five as "a whole JDK 9 API absent". Pin the
...
// `docs/jdk-only/G18-1-the-proxy-invocation-contract-and-two-vectors-20260817.md`).
...
fn native_proxy_dispatch_invoke(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    if handler_class == "java/lang/annotation/AnnotationProxy" {
        ...
        if !mname.is_empty() {
            ...
            // docs/jdk-only/W7-12-strict-annotation-proxy.md)
