...
/// mismatches (item 6).
///
/// **It does NOT apply the return-value contract the live path does**
/// (`G24-1`): no NPE for a `null` result on a primitive-returning method, no
/// `ClassCastException` for a wrong-typed result. That contract lives in
/// `vm::vm_exec::proxy_coerce_handler_return`, in the `vm` crate — this crate
/// is a dependency OF `vm`, not the reverse, so it cannot call that function,
/// and duplicating its logic here would recreate exactly the five-copies
/// problem `G24-1` §4.3 collapsed. Safe only because this body stays
/// unreachable in practice: a reader relying on it for the return contract
/// would be relying on a gap, not a feature.
