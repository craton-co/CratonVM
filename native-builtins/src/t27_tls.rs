...
fn dedup_root_ders(root_ders: Vec<Vec<u8>>) -> Vec<Vec<u8>> {
...
pub(crate) fn set_pending_tm_trust_roots(root_ders: Vec<Vec<u8>>) {
    let deduped = dedup_root_ders(root_ders);
...
///
/// `resolved_tm_trust_roots` is the trust-side counterpart, resolved DIRECTLY
/// from the actual `TrustManager[]` this call received (via
/// `x509_manager::resolved_trust_roots_for_trust_manager_array`) — same
/// reason, same fix shape, same priority over the thread-local
/// `PENDING_TM_TRUST_ROOTS`: see that function's doc and
/// P4A-TOMCAT-20260812.md defect 4 for the measured order-dependence this
/// closes (`TrustOrderProbe` — a `trusting` and a `defaulting` `SSLContext`
/// built from separate `TrustManagerFactory`s before either is initialised;
/// whichever `SSLContext.init` runs first drained the OTHER one's staged
/// roots). `take_pending_tm_trust_roots` is still called unconditionally so
/// it never leaks into a later, unrelated `SSLContext.init` either.
pub(crate) fn attach_pending_identity_to_ctx(
    ctx: &mut dyn NativeContext,
    ctx_obj: ObjectRef,
    resolved_km_identity: Option<(String, String)>,
    resolved_tm_trust_roots: Option<(Vec<Vec<u8>>, Option<crate::x509_manager::RevocationConfig>)>,
) -> Result<(), MethodCallFailed> {
    let key = ctx_obj_key(ctx, ctx_obj)?;
    ...
    let pending_roots = take_pending_tm_trust_roots();
    let roots = resolved_tm_trust_roots
        .map(|(root_ders, revocation)| TlsTrustRoots {
            root_ders: dedup_root_ders(root_ders),
            revocation,
        })
        .or(pending_roots);
...
fn suite_to_java_cipher_name(suite: rustls::CipherSuite) -> String {
    ...
    match suite {
        ...
        // `docs/jdk-only/E3-1-the-cipher-name-helper-and-its-real-denominator.md`)
...
/// The protocol-version twin of [`NATIVE_TLS_UNNAMEABLE_SUITE`], for the one
/// arm that is genuinely without an accessor: `native_tls::TlsAcceptor`.
///
/// E31-1 NOMINATION 3. The `Native` arm of [`rustls_server_accept_within`]'s
/// match used to hardcode `"TLSv1.2"` for a handshake whose real version it
/// cannot read — `native-tls` 0.2.18 exposes no version accessor on
/// `TlsStream`, unlike `openssl::ssl::SslRef::version_str()`, which is why the
/// `LegacyDsa` arm beside it could be (and was) fixed to report the real
/// value. A plausible, wrong protocol version is the same species of
/// fabrication the sibling cipher comment refuses, in the *dangerous*
/// direction: a caller testing "am I on at least TLS 1.2" would be told yes
/// for a connection that may have been negotiated at TLS 1.0 or 1.1. This
/// constant is not JSSE vocabulary either — loud rather than plausible is the
/// same trade already made for the cipher suite, and the two now use the same
/// shape.
pub(crate) const NATIVE_TLS_UNNAMEABLE_PROTOCOL: &str = "TLS_UNKNOWN_NATIVE_TLS_PROTOCOL";
    ...
    > = (|| {
        Ok(match config {
            ...
            TlsServerConfig::Native(acceptor) => {
                ...
                //
                // E31-1 NOMINATION 3 — CHANGED. The `"TLSv1.2"` beside the
                // suite used to be a fabrication of the same species the
                // comment above refuses for the cipher — a real, plausible
                // protocol version asserted about a handshake whose version is
                // equally unknowable here, and worse than a conservative
                // under-report on THESE arms specifically because they are the
                // *legacy* acceptors: a caller testing "am I on at least TLS
                // 1.2" was told yes for a connection that may have been 1.0 or
                // 1.1. `native-tls` 0.2.18 exposes no version accessor and
                // this workspace has no fork or rustls-routed replacement for
                // this acceptor, so the only accessor-free choice consistent
                // with the cipher's own trade is a loud sentinel rather than a
                // silently plausible one. `getProtocol()`'s only contract
                // (`the_null_session_string_answers_are_never_null` and its
                // siblings) is non-null, which this satisfies.
                (
                    TlsServerStream::Native(stream),
                    None,
                    NATIVE_TLS_UNNAMEABLE_PROTOCOL.to_string(),
...
mod tests {
    ...
    fn scoped_trust_roots_attach_to_one_ssl_context() {
        ...
        attach_pending_identity_to_ctx(&mut mock_ctx, ctx, None, None).unwrap();
...
fn engine_session_for(
    ...
    if phase == SessionPhase::Negotiated {
        ...
        // NOM-1 (G7-1). Same epoch as the line above: this is a COMPLETED
        // handshake, so `getSessionContext()` will hand out a real carrier
        // for `ses` from this point on, and that carrier's `getIds()`/
        // `getSession(id)` need `ses` to actually be findable.
        let id_bytes = session_id_bytes(ctx, ses);
        session_id_registry().lock().insert(id_bytes, ses);
    }
...
pub fn gc_scan_tls_ctx_trust_manager_roots(roots: &mut Vec<ObjectRef>) {
    ...
    drop(cached);
    // NOM-1 (G7-1). `session_id_registry` holds the same kind of live
    // `ObjectRef` as `engine_session_table`/`client_session_cache` above, and
    // is not implied by either — a resumed connection's session can outlive
    // the engine that negotiated it.
    let by_id = session_id_registry().lock();
    for ses in by_id.values() {
        if !ses.as_ptr().is_null() {
            roots.push(*ses);
        }
    }
...
pub fn gc_update_tls_ctx_trust_manager_refs(map: &cratonvm_types::PointerMap) {
    ...
    drop(cached);
    // NOM-1 (G7-1). Remap companion to the scan added above.
    let mut by_id = session_id_registry().lock();
    for ses in by_id.values_mut() {
        let old = ses.as_ptr() as usize;
        if let Some(&new) = map.get(&old) {
            debug_assert!(new != 0, "GC pointer map contains null address");
            // SAFETY: as above.
...
/// The bytes `SSLSession.getId()` would answer for a NEGOTIATED session, as a
/// plain `Vec<u8>` — the same derivation as that registration's body (real
/// wire id first, else the per-object `SplitMix64` pseudo-id), kept as a
/// separate copy rather than a shared call because `getId`'s closure is not a
/// free function. Callers must have already checked `session_has_negotiated`;
/// this does not re-check it, matching `getId`'s own gate placement. Used to
/// key [`session_id_registry`] so it and `getId()` can never disagree about a
/// given session's id.
fn session_id_bytes(ctx: &dyn NativeContext, this: ObjectRef) -> Vec<u8> {
    let wire_key = gc_stable_objref_key(ctx, this);
    if let Some(id) = session_wire_id_table().lock().get(&wire_key).cloned() {
        return id;
    }
...
/// NOM-1 (`G7-1` §8, "the one that matters most" among that record's
/// nominations for `net_phase_e.rs`). `SSLSessionContext.getIds()` and
/// `getSession(byte[])` used to answer an always-empty enumeration and a
/// null lookup for every session, even a negotiated one — honest given the
/// context carrier itself was orphaned, but wrong once G7-1 §2 gave
/// `getSessionContext()` a real, populated carrier for exactly that state:
/// "the context a session points at does not contain that session" became
/// reachable. This table is the fix: keyed by the same 32 bytes `getId()`
/// reports, populated where [`negotiated_session_keys`] is (a completed
/// handshake) and erased by [`session_mark_invalidated`], so the two
/// collections stay in lockstep and `getSession(id)` after `invalidate()`
/// answers `null` exactly as MEASURED in G7-1 §1b.
///
/// Holds live `ObjectRef`s → scanned and remapped alongside
/// `engine_session_table`, see `gc_scan_tls_ctx_trust_manager_roots` /
/// `gc_update_tls_ctx_trust_manager_refs`.
fn session_id_registry(
) -> &'static cratonvm_types::lock_order::OrderedPlMutex<HashMap<Vec<u8>, ObjectRef>> {
    static T: OnceLock<cratonvm_types::lock_order::OrderedPlMutex<HashMap<Vec<u8>, ObjectRef>>> =
        OnceLock::new();
    T.get_or_init(|| {
        cratonvm_types::lock_order::OrderedPlMutex::new(
            HashMap::new(),
...
/// `SSLSessionContext.getIds()`'s answer: every id currently in
/// [`session_id_registry`], in no particular order — the real JDK's own
/// `getIds()` makes no ordering guarantee either.
pub(crate) fn session_id_registry_ids() -> Vec<Vec<u8>> {
    session_id_registry().lock().keys().cloned().collect()
}

/// `SSLSessionContext.getSession(byte[])`'s lookup half. `None` for both "no
/// session was ever cached under this id" and "the id has the wrong length" —
/// the caller is expected to have already refused a `null` array (MEASURED:
/// that row is a `NullPointerException`, not a lookup miss).
pub(crate) fn session_by_id(id: &[u8]) -> Option<ObjectRef> {
    session_id_registry().lock().get(id).copied()
}
...
pub(crate) fn session_mark_invalidated(ctx: &dyn NativeContext, session: ObjectRef) {
    ...
    // NOM-1 (G7-1). MEASURED (§1b): `getSession(id)` on the owning context
    // goes from SAME-OBJECT to `null` once the session invalidates. Only a
    // negotiated session can have an entry here at all (mirrors the guard
    // above), and removing by the id `session_id_bytes` derives is safe even
    // when there was never an entry to remove.
    let id_bytes = session_id_bytes(ctx, session);
    session_id_registry().lock().remove(&id_bytes);
}
...
/// `docs/jdk-only/E12-1-the-null-session-and-the-fabricated-cipher.md`
/// §1):
///
/// ```text
...
/// see `docs/jdk-only/E42-1-the-slot-that-was-never-there-and-the-predicate-that-was-its-own-negation.md`.
