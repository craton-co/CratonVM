...
fn native_unsafe_define_class(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    match ctx.define_class_full(&effective_name, &class_bytes, loader_id, opts) {
        ...
        Err(msg)
            if msg.contains("SecurityException") && msg.contains("Prohibited package name") =>
        {
            // Round 9 wave 6 (vmhelp6): a refused privileged forgery of a core
            // class surfaces as HotSpot's `SecurityException`, not as a format error.
            Err(RuntimeError::SecurityException { message: msg }.into())
        }
    ...
    r.register_with_kind(
        ...
        |ctx, _args| {
            ...
            let target = match frames.len() {
                ...
                1 => frames.first(),
                n => {
                    // Walk outward from frames[n-2], skipping java.util.stream
                    // / Spliterator / ServiceLoader plumbing frames.
                    //
                    // `ServiceLoader.load(Class)` is itself `@CallerSensitive`
                    // and calls this native directly — the "-2" above is
                    // exactly right when frames[n-1] IS `java/util/ServiceLoader`.
                    // But a `Stream.forEach(lambda)` call chain into
                    // `ServiceLoader.load` (Hibernate's
                    // `ClassLoaderServiceImpl.loadJavaServiceImplementors`,
                    // reached from JUnit's `ClassBasedTestDescriptor
                    // .invokeTestInstancePostProcessors`'s
                    // `list.forEach(postProcessor -> ...)`) is small, hot and
                    // gets JIT-compiled/inlined as one unit — MEASURED:
                    // `frames[n-1]` then comes back as
                    // `java/util/stream/ForEachOps$ForEachOp$OfRef` (the
                    // forEach machinery), not `ServiceLoader`, with every
                    // frame from the lambda down through
                    // `ServiceLoader.load`/`checkCaller` missing entirely.
                    // Plain "-2" then lands one frame INTO the same stream
                    // plumbing instead of on the real (unnamed-module) caller,
                    // and `ServiceLoader.checkCaller` reports the caller
                    // module as `java.base` — which does not declare `uses`
                    // for a caller-supplied service, and never would, because
                    // java.base is never the real caller here. Skip outward
                    // through the plumbing to the first frame that is not
                    // stream/Spliterator/ServiceLoader machinery instead of
                    // trusting one fixed offset. See
                    // docs/internal/fixed-suite-bugs/hibernate/
                    // serviceloader-collapsed-inline-chain-caller-attribution-FIXED-20260922.md.
                    let mut idx = n - 2;
                    loop {
                        let f = &frames[idx];
                        let name = f.class_name.as_ref();
                        let is_plumbing = name.starts_with("java/util/stream/")
                            || name == "java/util/ServiceLoader"
                            || (name.starts_with("java/util/") && name.contains("Spliterator"));
                        if !is_plumbing || idx == 0 {
                            break Some(f);
                        }
                        idx -= 1;
                    }
            ...
            if crate::nbflags().dbg_caller {
                eprintln!(
                    "[DBG_CALLER] jdk/internal/reflect/Reflection.getCallerClass trace ({} frames): {:?}",
                    frames.len(),
                    frames.iter().map(|f| f.class_name.as_ref()).collect::<Vec<_>>()
                );
            }
            if let Some(frame) = target {
                // Prefer the frame's own `class_id` (captured live from the
