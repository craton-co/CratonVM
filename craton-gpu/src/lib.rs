...
//! `../craton-gpu-java/...` sibling checkout, or (on Windows) a
//! `C:/craton/...` default install — then compiles it with `javac` and
//! packages it with `jar`. When no source tree is present the build
//! degrades to an empty jar and a `cargo:warning=` (it never fails), so
//! the two constants below may point at an empty directory / be empty.
//!
