...
fuzz_target!(|data: &[u8]| {
    ...
    // -------------------------------------------------------------------
    // Phase A — bounded recursion, asserted in both directions.
    //
    ...
    assert!(
        cratonvm_reader::decode_attribute("Code", &nested_code_body(ATTRIBUTE_NEST_DEEP), &cp)
            .is_err(),
    ...
    // TypeAnnotation `type_path` — a full 255-entry path is legal, and the
    // same path length with no path bytes behind it must fail rather than
    // read past the body.
    assert!(
        cratonvm_reader::decode_attribute(
            "RuntimeVisibleTypeAnnotations",
    ...
    // A deeply nested body must also be *shape-checkable* without
    // recursing: `validate_attribute_shape` walks nested `Code` headers
    // iteratively and must terminate on any depth.
    let _ = validate_attribute_shape("Code", &nested_code_body(ATTRIBUTE_NEST_DEEP));

    // -------------------------------------------------------------------
    // Phase B — raw fuzzed bodies. Panic-only oracle: an arbitrary body is
    // allowed to be `Err`, but must never panic, over-read, or recurse off
    // the stack.
    // -------------------------------------------------------------------
