...
artifacts under `fuzz/artifacts/<target>/`. Hand-built seeds for the
newer targets are committed and regenerated by
...
directly: `fuzz/corpus/fuzz_classfile/Object.class`, the bootstrap
`modules` jimage truncated to a few MiB for `fuzz_jimage`, or a self-signed
PKCS#12 for `fuzz_keystore`.
