...
- So something else handles `ArrayList.toArray(T[])` — copies elements (so it reads `al_state`) but always `new Object[]`. **Next step: add a one-shot trace (eprintln) at the top of every toArray-ish native + the interpreter's invoke path for `ArrayList.toArray([Ljava/lang/Object;)`, rebuild, and run `ToArr3` to see which Rust fn actually executes.** Then route THAT fn's too-small-template branch through `ctx.array_component_class_id` (the accessor is already in place). H2 hits this in `org.h2.result.SortOrder.sort` (`Value[][]`).
