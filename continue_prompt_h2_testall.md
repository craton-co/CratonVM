...
2. **`Collection.toArray(T[])` multi-dim** — repro `List<String[]>.toArray(new String[0][])` → CCE `Object -> [[String` (probe `C:/tmp/audit/ToArrayProbe.java`). **CORRECTED PATH (verified 2026-06-03):** the failing allocation is NOT `native_al_to_array_typed` (`native-collections/src/lib.rs:1293`) — that native is never even reached for this case (no `CRATONVM_DBG_SBLOAD` trace). The real culprit is the **`Arrays.copyOf(Object[], int, Class)` native at `native-builtins/src/phases_early.rs:442`**, which `ArrayList.toArray(T[])` calls with `a.getClass()` and which **ignores the Class arg**, always doing `ctx.new_array(Reference, len)` = bare `Object[]`. **Attempted fix (reverted):** decode the Class arg via `crate::lang_class::mirror_class_id` -> `class_name_of_id` -> strip one `[` -> `class_id_by_name(component)` -> `new_ref_array`. It did NOT work — `class_id_by_name` apparently does **not resolve array-descriptor names** like `[Ljava/lang/String;` (only plain `pkg/Class`), so it returns `None` and falls back to `Object[]`. **What's actually needed:** a way to allocate an array instance *of a given array Class* (the 3rd arg directly is `String[][].class`), or to resolve/create an array ClassId from its element type — i.e. an `new_array_of_class(array_class_id, len)` API or making `class_id_by_name` resolve `[L...;` / `[[...` descriptors (creating the array class on demand). Same fix then applies to `native_al_to_array_typed`/`native_ll_to_array_typed` (`alloc_array_like` helper) for the cases that DO route there. H2 hits this in `org.h2.result.SortOrder.sort` (`Value[][]`).
