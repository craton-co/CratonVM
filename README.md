...
[![CI](https://github.com/craton-co/CVM/actions/workflows/ci.yml/badge.svg)](https://github.com/craton-co/CVM/actions/workflows/ci.yml)
...
- **ZGC by default.** The default collector is a memory-backed, stop-the-world ZGC
  (`-XX:+UseZGC`). It has thread-local allocation buffers that the JIT allocates
  from inline, and parallel sliding compaction (kill switch
  `CRATONVM_ZGC_RELOCATE=0`).
  - Parallel marking (`CRATONVM_ZGC_PARMARK=<workers>`), concurrent marking
    (`CRATONVM_ZGC_CONC_START`) and a generational mode
    (`CRATONVM_ZGC_GENERATIONAL=1`) are opt-in.
  - The colored-pointer load barrier is built but not yet armed.
  - `-XX:+UseGenerationalGC` selects the young/old collector with a Cheney
    moving young generation; a cycle that cannot prove complete root coverage
    falls back to a non-moving sweep.
  - Region-based G1 is experimental and opt-in (`-XX:+UseG1GC`).
  - See [ARCHITECTURE.md](ARCHITECTURE.md#memory-gc-crate).
...
- **JDK-only execution (the default since 2026-09-20)** — real JDK class bytes
  are authoritative: no class is fabricated without real bytes and no
  synthetic-stub native is registered or invoked. Every compatibility
  substitution is a counted, attributed event, so the gap between "runs on
  CratonVM" and "runs on the real class bytes" is measurable rather than
  assumed. Enforcement is still at wave 1, so programs that depend on a
  substitution can opt back out with `--compatible` (see below).
...
‡ Isolated phase, one fresh process per measurement; see BENCHMARK.md for every size.
Measured on the pinned Azure EPYC host, and not re-measured since JIT review round 9.
...
| `-Xmx<SIZE>`, `--Xmx <SIZE>` | Maximum heap size (`256m`, `1g`, `8g`). Default: 1/4 of physical RAM, or of the container memory limit, clamped to 256 MB..4 GB. The cap is `CRATONVM_DEFAULT_HEAP_MAX_MB`; `CRATONVM_DEFAULT_HEAP_ERGONOMICS=0` restores a fixed 256 MB. |
| `-XX:+UseG1GC` | Use the region-based G1 collector (experimental; ZGC is the default). |
| `-XX:+UseGenerationalGC` | Use the generational (young/old, Cheney young gen) collector instead of ZGC. |
...
| `--synthetic-jdk` | Force the built-in Rust standard library even when a JDK is detected. Implies `--compatible`. |
| `--jdk-only` | Strict mode: real JDK class bytes are authoritative. **This is the default**; the flag states it explicitly. Conflicts with `--compatible` and `--synthetic-jdk`. |
| `--compatible` | Opt out of strict mode: real JDK class library with the pre-2026-09-20 compatibility substitutions. Use it when a program fails only under the default. |
...
## JDK-only mode — the default since 2026-09-20

The VM treats real JDK class bytes as authoritative: it will not fabricate a
class that has no real bytes, and it will not register or invoke a
synthetic-stub native. Enforcement is at wave 1 (class fabrication and stub
registration enforce; the rest is recorded and counted), so a program that
depends on a substitution may fail here and pass under `--compatible`. That
difference is the measurement, not a bug in either mode.

A/B a program to see what it depends on:

```bash
./target/release/cratonvm -cp . MyApp                       # default: jdk-only
./target/release/cratonvm --compatible -cp . MyApp          # pre-2026-09-20 behaviour
./target/release/cratonvm --jdk-only-report report.json -cp . MyApp
```

The report works in either mode: under `--compatible` it is a census of what
