...
| HashMap (10M put/get)             | 1,062 ms  | 1,870 ms  | 1.76x | 07-30 |
...
before any change here, `dev` runs this kernel in **4,065 ms** — 3.83x, not
...
2026-07-30 closeout cut the HotSpot gap by **73.1%** (4,065 → 1,870 ms against
1,062 ms). Both columns are medians of five interleaved fresh-process runs on
one pinned core; the host was at load 10-12, so these absolutes are inflated
relative to a quiet-host reading but the ratio is sound. See
