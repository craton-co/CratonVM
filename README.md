...
| Fibonacci(44)           | 1,438 ms  | 5,053 ms‡ | 3.51x‡    | 0.9% | 2.79x |
...
This row is re-measured post-fix, in its own quiet window (1-minute load
2.18–2.49, 9 pairs, nothing discarded, all 18 checksums matching). **Both
columns come from that window.** HotSpot read 1,438 ms in it against the
1,444 ms it records in the window the other six rows used — 0.4% apart, which
is what makes this row comparable with them rather than merely internally
consistent.
