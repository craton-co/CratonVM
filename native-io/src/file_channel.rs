...
pub fn register_file_channel_real(r: &mut NativeMethodRegistry) {
    ...
    let mmu = "java/nio/MappedMemoryUtils";
    r.register(
        mmu,
        "force0",
    ...
    r.register(mmu, "isLoaded0", "(JJJ)Z", native_mmu_is_loaded0);
    r.register(mmu, "load0", "(JJ)V", native_mmu_load0);
    r.register(mmu, "unload0", "(JJ)V", native_mmu_unload0);
...
// ---------------------------------------------------------------------------
// java/nio/MappedMemoryUtils — memory-mapped buffer synchronization
// ---------------------------------------------------------------------------

fn native_mmu_force0(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    // args[0] = FileDescriptor, args[1] = address (long), args[2] = length (long)
    // If a valid fd is present, flush through native_fc_force0
    if let Some(Value::Object(Some(fd_obj))) = args.first().copied() {
        let _ = native_fc_force0(ctx, &[Value::Object(Some(fd_obj)), Value::Int(1)]);
    }
    Ok(None)
}

fn native_mmu_is_loaded0(_ctx: &mut dyn NativeContext, _args: &[Value]) -> MethodCallResult {
    // OpenJDK's Windows MappedMemoryUtils.c always returns JNI_FALSE because
    // Windows provides no mincore(2) equivalent to query page residency.
    Ok(Some(Value::Int(0)))
}

fn native_mmu_load0(_ctx: &mut dyn NativeContext, _args: &[Value]) -> MethodCallResult {
    Ok(None)
}

fn native_mmu_unload0(_ctx: &mut dyn NativeContext, _args: &[Value]) -> MethodCallResult {
    Ok(None)
}
