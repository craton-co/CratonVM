...
fn native_fd_write0(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // The Windows `write0(fd, addr, len, boolean append)` spelling: an
    // append-mode channel writes at end-of-file whatever its cursor holds (the
    // JDK passes an all-ones overlapped offset). The seek is not atomic with
    // the write, so two appenders on one file can interleave differently from
    // HotSpot's; the single-writer case is the one measured.
    if matches!(args.get(3), Some(Value::Int(v)) if *v != 0) {
        ctx.fd_table()
            .rw_seek(fd, std::io::SeekFrom::End(0))
            .map_err(|e| io_error(format!("write0: seek to end: {e}")))?;
    }
    ctx.fd_table()
        .write_bytes(fd, &buf)
...
///
/// Registered only for the `FileDispatcherImpl` family (`FD_LEAF`/`FD_UNIX`,
/// see its call site below), which is a DIFFERENT class from
/// `java/io/FileDescriptor.close0()V` served by `lib.rs`'s own
/// `native_fd_close0`. Named distinctly (`_dispatcher_`) so the two are not
/// mistaken for the same registration when read side by side — see
/// `H8-1-three-declines-that-were-not-declines-20260820-RETIRED-20260921.md`
/// N4, which found
/// no live collision today but flagged the shared name as a trap that
/// produced exactly this shape of defect once already (`H8-A`).
fn native_fd_dispatcher_close0(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    let fd_obj = fd_arg(args, 0)?;
