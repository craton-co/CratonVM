...
//!
//! Acceptance criterion (from `wildfly-ejbca-roadmap.md` §6 WP3.5):
...
///
/// See bug-h2-largeblob-direct-memory-oom.md:
...
struct DbbElemFields {
    ...
    /// `java.nio.Buffer.segment` — the `MemorySegment` this buffer is a view
    /// of, non-null only for one minted through `MemorySegment.asByteBuffer()`
    /// (`native-builtins/src/panama.rs::pe_segment_as_byte_buffer`). `Option`
    /// for the same reason as `big_endian`: a fully-synthetic image has no FFM
    /// classes at all, and the raw fast path stays open for every buffer that
    /// cannot possibly carry a closeable arena behind it.
    ///
    /// Checked and, if set, bailed to bytecode by every accessor below. This
    /// crate cannot depend on `native-builtins` (`panama.rs`'s
    /// `pe_session_check_open` is the other side of that boundary), so the raw
    /// fast path simply declines a segment-backed buffer rather than
    /// re-implementing the liveness/thread-confinement check here — real
    /// `DirectByteBuffer` bytecode already runs `session()` →
    /// `ScopedMemoryAccess.getByte(session, …)`, and `session.checkValidState()`
    /// reaches the native this VM registers on `MemorySessionImpl` (W7-89).
    /// The buffers this can ever be true for are rare and already off the hot
    /// path (an FFM arena view, not a plain `allocateDirect()` buffer, which
    /// never sets this field — see `dbb_allocate_direct0`), so the bail costs
...
pub mod elem_fastpath {
    ...
    /// `-1` when this image has no `Buffer.segment` field (fully-synthetic-JDK
    /// mode) OR the funnel has not resolved it yet — both read as "no FFM
    /// receiver can reach this path", which is the safe default. See
    /// [`super::DbbElemFields::segment`] for why the JIT helper must check this
    /// itself rather than trust the funnel's per-class `class_is_served` alone:
    /// `DirectByteBuffer` is the one Java class both a plain `allocateDirect()`
    /// buffer and an FFM `MemorySegment.asByteBuffer()` view share, so a class
    /// the funnel marked served from the former must still be checked PER
    /// RECEIVER before the latter is let through this same compiled call site.
    static SEGMENT_SLOT: AtomicI32 = AtomicI32::new(-1);
    ...
    /// The `segment` field index, or `None` when this image has no such
    /// field, or the funnel has not resolved it yet. Checked by
    /// `dbb_direct_elem_addr` before a JIT-compiled site is allowed to read or
    /// write through a receiver — see [`SEGMENT_SLOT`].
    pub fn segment_slot() -> Option<usize> {
        let s = SEGMENT_SLOT.load(Ordering::Relaxed);
        if s < 0 {
            return None;
        }
        Some(s as usize)
    ...
    /// # Why `extern "C"`
    ///
    /// The VM publishes this function's ADDRESS to the JIT as a bare `usize`
    /// (`DirectHelperTable::nio_byte_element_served_class`), because the JIT
    /// crate does not depend on `native-io` and must not — it sits below the
    /// native layer. The compiler transmutes that word back to a fn pointer
    /// before calling it (`jit/src/direct_helpers.rs`,
    /// `nio_byte_element_class_is_served`). The `"Rust"` ABI is explicitly
    /// UNSPECIFIED between separately compiled crates and across codegen flags,
    /// and erasing the address to a `usize` is exactly the arrangement that
    /// makes the two ends separately compiled. `extern "C"` is the convention
    /// both ends can name, so this definition and the consumer's
    /// `DirectHelperFnNioByteElementServedClass` alias
    /// (`unsafe extern "C" fn(u32, bool) -> bool`) are two halves of one change.
    ///
    /// The direct Rust callers are unaffected: an `extern "C" fn` item is still
    /// an ordinary function at a Rust call site.
    pub extern "C" fn class_is_served(class_id: u32, write: bool) -> bool {
        if class_id == 0 {
            return false;
        }
    ...
    // The `extern "C"` above is load-bearing and nothing else in this crate
    // would notice if it were dropped: every Rust caller calls
    // `class_is_served` by name, and a name call is spelled identically
    // whatever the function's ABI. The one consumer that DOES care never sees
    // this signature — it sees a `usize` and transmutes it back
    // (`jit/src/direct_helpers.rs::nio_byte_element_class_is_served`).
    //
    // A fn item coerces to a fn-pointer type only when its `extern`-ness,
    // arity, argument types and return type all match, so this line turns
    // deleting the `extern "C"` into a compile error here rather than an
    // unspecified-ABI call at a JIT compile site. Spelled out rather than
    // written against `cratonvm_jit::direct_helpers::
    // DirectHelperFnNioByteElementServedClass` because this crate does not
    // depend on the JIT crate and must not. The two spellings are checked
    // against each other on the VM side, which is the one place that can name
    // both (`docs/jit/helper-abi.md` §10).
    const _: extern "C" fn(u32, bool) -> bool = class_is_served;
    ...
    pub(super) fn publish_slots(
        ...
        SEGMENT_SLOT.store(segment.map_or(-1, fit), Ordering::Relaxed);
    }
...
fn dbb_elem_fields(ctx: &mut dyn NativeContext) -> Option<DbbElemFields> {
    ...
    *CACHE.get_or_init(|| {
        ...
        let fields = DbbElemFields {
            ...
            segment: ctx.resolve_field_index(CLASS, "segment"),
        };
        elem_fastpath::publish_slots(
            fields.address,
            fields.limit,
            fields.is_read_only,
            fields.segment,
...
/// Is `this` a view of an FFM `MemorySegment` (`segment` non-null)?
///
/// Every raw-address prologue below calls this first and bails to bytecode
/// when it answers `true` — see [`DbbElemFields::segment`] for why the bail,
/// not a re-implemented check, is the fix here.
fn dbb_has_ffm_segment(ctx: &dyn NativeContext, this: ObjectRef, fields: &DbbElemFields) -> bool {
    match fields.segment {
        Some(slot) => matches!(ctx.get_field(this, slot), Value::Object(Some(_))),
        None => false,
    }
}
...
fn dbb_elem_addr(
    ...
    if dbb_has_ffm_segment(ctx, this, &fields) {
        return None;
    }
...
pub fn register_direct_buffer_real(r: &mut NativeMethodRegistry) {
    ...
    //
    // Tagged `Intrinsic`, not `Bridge` (H2 census 20260922). Both kinds are
    // registered here identically, but under `--jdk-only` `resolve_native_
    // dispatch_wave1`'s §7 step 3 sends any `Bridge` back to real bytecode
    // whenever that bytecode exists (`sun/nio/ch/Util` is a real class with a
    // real body) — measured: even after adding this triple to
    // `force_native_over_real_jdk_bytecode`, the native never ran under
    // `--jdk-only`, confirmed by instrumenting both. Only `Intrinsic` is the
    // reviewed exception that still shadows real bytecode. That is the
    // correct tag here, not a workaround: this native returns exactly what
    // the real bytecode would (a real, correctly-sized `DirectByteBuffer`)
    // and exists ONLY to avoid a genuine bug in the real bytecode's own
    // `Util$BufferCache` — several VM worker threads can observe the same
    // `ThreadLocal` instance, and its unsynchronised `count`/`start`/ring
    // array then hands out a `count` that claims a null slot, so the
    // caller's very next `buf.capacity()` NPEs with "buf is null" (or,
    // depending on timing, a stale/reused buffer throws
    // `BufferUnderflowException`). Confirmed against `TestReadOnly`,
    // `TestCluster`, `TestUrlJavaObjectSerializer`, `TestAutoReconnect`,
    // `TestJakartaServlet`, `TestPageStoreCoverage`, `TestPgServer`, and
    // `TestServlet`, all newly FAILing under `--jdk-only` (default since
    // 2026-09-20) with exactly this crash.
    let __prev_cat_tempdbb = r.current_category();
    ...
    r.set_category(__prev_cat_tempdbb);
