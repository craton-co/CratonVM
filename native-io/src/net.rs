...
pub fn register_multicast_socket_overrides(r: &mut NativeMethodRegistry) {
    ...
    // `isClosed()` — mirrors `close()` above by reading the SAME field slot 1
    // it writes. Without this, `MulticastSocket.isClosed()` is inherited from
    // `DatagramSocket` in real bytecode, which routes through
    // `net_phase_e::register_re7_datagram_socket`'s `ds_side_table` (keyed by
    // ObjectRef, unrelated to this registrar's field-slot state) instead of
    // this class's own field-based `close()` above — so a `MulticastSocket`
    // stayed `isClosed() == false` forever after `close()` ran (`close()`
    // wrote slot 1 here; the unrelated store `isClosed()` read was never
    // touched). Plain `DatagramSocket` never had this bug: it has no
    // MulticastSocket-specific registrar to compete with RE7's, so both its
    // `close()` and `isClosed()` already agreed via the same side table.
    // Measured with `McastCloseSimple.java` (regression-suite/probes/).
    ...
    // joinGroup(InetAddress) / leaveGroup(InetAddress) -- the classic 1-arg
    // overload. Unlike close()/isClosed() (inherited from DatagramSocket),
    // MulticastSocket declares these TWO methods itself, and their real
    // bytecode's first instruction is `this.delegate()`, which throws
    // `InternalError("Should not get here")` because this VM never populates
    // that field (same root cause as the H8-1/H9-1/H10-1-era delegate()
    // traps). `register_p72_datagram`'s (native-builtins
    // phases_late/net_channels.rs) own registrations for these exact
    // triples exist but are never reached by ordinary dispatch -- measured
    // directly (a debug print inside that native never fired for either
    // `joinGroup(InetAddress)` or the MulticastSocket-exact `isClosed()`
    // this file's own `isClosed()` above was added to fix); only THIS
    // registrar (run after phase-72, documented above to win for the
    // signatures it implements) reliably wins for MulticastSocket. So this
    // class needs its own copy here rather than relying on phase-72's.
    r.register(ms, "joinGroup", "(Ljava/net/InetAddress;)V", |ctx, args| {
        let this = match args.first() {
            Some(Value::Object(Some(o))) => *o,
            _ => return Ok(None),
        };
        let Some(Value::Object(Some(ia))) = args.get(1).copied() else {
            return Err(ms_socket_exception(ctx, "Invalid multicast address"));
        };
        let Some(ip) = ms_inet_host(ctx, ia).and_then(|h| h.parse::<std::net::Ipv4Addr>().ok())
        else {
            return Err(ms_socket_exception(ctx, "Invalid multicast address"));
        };
        if !ip.is_multicast() {
    ...
    r.register(ms, "leaveGroup", "(Ljava/net/InetAddress;)V", |ctx, args| {
        let this = match args.first() {
            Some(Value::Object(Some(o))) => *o,
            _ => return Ok(None),
        };
    ...
    // setLoopbackMode / getLoopbackMode -- no native existed anywhere for
    // these (unlike joinGroup/leaveGroup above, phase-72 does not even
    // attempt them), so real bytecode always ran and always threw via
    // `delegate()`. JDK semantics invert the sense of IP_MULTICAST_LOOP:
    // `setLoopbackMode(true)` DISABLES loopback delivery, and
    // `getLoopbackMode()` answers "is it disabled" -- so the fd option is
    // the logical negation of the argument/return value throughout.
    r.register(ms, "setLoopbackMode", "(Z)V", |ctx, args| {
        let this = match args.first() {
            Some(Value::Object(Some(o))) => *o,
            _ => return Ok(None),
        };
        let disable = args.get(1).and_then(|v| v.as_int()).unwrap_or(0) != 0;
        let fd = ctx.get_field(this, 3).as_int().unwrap_or(-1);
        if fd >= 0 {
            let _ = ctx.fd_table().udp_set_multicast_loop_v4(fd as u32, !disable);
        }
        Ok(None)
    });
    r.register(ms, "getLoopbackMode", "()Z", |ctx, args| {
        let this = match args.first() {
            Some(Value::Object(Some(o))) => *o,
            _ => return Ok(Some(Value::Int(0))),
        };
        ...
        let loop_enabled = ctx
            .fd_table()
            .udp_multicast_loop_v4(fd as u32)
            .unwrap_or(true);
        Ok(Some(Value::Int(if loop_enabled { 0 } else { 1 })))
    });
