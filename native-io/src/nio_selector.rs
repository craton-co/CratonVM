...
fn selector_open_native(ctx: &mut dyn NativeContext, _args: &[Value]) -> MethodCallResult {
    ...
    // See docs/jdk-only/G30-1-the-silent-reference-slot-coercion-20260817.md.
    sel_obj_ids()
        .write()
...
/// tree (docs/jdk-only/W7-9-minted-interface-abstract-methods.md §6).
///
...
/// `isRegistered()` -- `final` on `AbstractSelectableChannel`.
fn g_channel_is_registered(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    match crate::socket_channel::foreign_nio_delegate(ctx, args, "isRegistered", "()Z") {
        Some(r) => r,
        None => channel_is_registered_native(ctx, args),
    }
}
...
fn channel_register_native(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    ...
    // Keep AbstractSelectableChannel.keyCount aligned for bytecode paths
    let current_count = match ctx.get_field_by_name(channel, "keyCount") {
        Value::Int(c) => c,
        _ => 0,
    };
    ctx.set_field_by_name(channel, "keyCount", Value::Int(current_count + 1));

    Ok(Some(Value::Object(Some(key_obj))))
}

/// `SelectableChannel.isRegistered()` — return whether this channel is registered
/// with any selector.
fn channel_is_registered_native(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    let Some(Value::Object(Some(channel))) = args.first().copied() else {
        return Ok(Some(Value::Int(0)));
    };
    let has_key = {
        let t = sk_table().read();
        t.values().any(|entries| {
            entries
                .iter()
                .any(|row| row.channel == channel && !row.cancelled)
        })
    };
    if has_key {
        return Ok(Some(Value::Int(1)));
    }
    let count = match ctx.get_field_by_name(channel, "keyCount") {
        Value::Int(c) => c,
        _ => 0,
    };
...
pub fn register_nio_selector_real(r: &mut NativeMethodRegistry) {
    ...
    // SelectableChannel.register(Selector,int) — H5-1 §2.A: this used to be a
    // standalone row here AND a row of the `for c in [...]` loop below (which
    // includes `"java/nio/channels/SelectableChannel"`), same callback,
    // registered twice. Deleted; the loop already covers it.
    ...
    for c in [
        ...
        r.register(c, "isRegistered", "()Z", g_channel_is_registered);
    }
