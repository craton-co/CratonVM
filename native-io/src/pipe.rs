...
//
// The six abstract rows were DEAD WEIGHT for every receiver this VM mints
// (MEASURED `invocations: 0` while the `sun/nio/ch/*Impl` rows below took every
// call, H11-2 census), and their only reachable population was a hazard: an
// application subclass of `Pipe.SourceChannel` / `Pipe.SinkChannel` (both
// constructors are `protected`) that does not override a method would have its
// receiver's superclass walk hijacked by this fabricated pipe body instead of
// reaching real JDK bytecode further up the chain. H11-2 §4 blocked deleting
// them because `native-builtins/src/phases_late/net_channels.rs` registered the
// SAME six triples with an INCOMPATIBLE field layout (fd at slot 1, open flag at
// slot 0) and would have inherited the slot — `[2 producers, 1 slot]`.
//
// RETIRED 2026-09-21 (H11-2/H11-3 residual): `net_channels.rs`'s duplicate
// in-memory-`VecDeque` Pipe implementation was itself dead (this file's
// registrar runs later and always won `owns_slot`), so it is deleted rather
// than kept as a trap for the next person who deletes this file's rows without
// checking. With no second producer left, the six abstract rows below are gone
// too — see the (now-removed) `abstract_source`/`abstract_sink` registrations
// this comment used to defend.
...
pub fn register_pipe_real(r: &mut NativeMethodRegistry) {
    ...
    //   * `java/nio/channels/Pipe` — kept as the fallback class for when
    //     `sun.nio.ch.PipeImpl` is absent from the image (e.g. a synthetic-JDK
    //     boot). `native-builtins/src/phases_late/net_channels.rs` used to
    //     register the same triples with an incompatible field layout and win
    //     the slot away from this file in that case; that duplicate registrar
    //     was dead code (this file's registrar always ran later) and was
    //     deleted 2026-09-21 (H11-2 §4 residual), so there is no longer a
    //     second producer to lose the slot to.
    ...
    // The abstract `java/nio/channels/Pipe$SourceChannel` rows (read/isOpen/
    // close) were RETIRED 2026-09-21 (H11-2 §4 / H11-3 N2): `invocations: 0`
    // measured while the `sun/nio/ch/SourceChannelImpl` rows above took every
    // real call, and the only population that could still reach them was the
    // superclass-walk hijack of an application subclass — a hazard, not a
    // service. Blocked until now by `native-builtins/src/phases_late/
    // net_channels.rs` registering the same triples with an incompatible body;
    // that duplicate was dead code and is deleted in the same commit.
    ...
    // The abstract `java/nio/channels/Pipe$SinkChannel` rows (write/isOpen/
    // close) were RETIRED alongside `Pipe$SourceChannel` above -- same
    // measurement, same hazard, same blocker, same commit.
