...
/// `provider()` -- guarded; final on `AbstractSelectableChannel`.
fn g_selectable_channel_provider(ctx: &mut dyn NativeContext, args: &[Value]) -> MethodCallResult {
    match foreign_nio_delegate(
        ...
        Some(r) => r,
        None => selectable_channel_provider(ctx, args),
    }
}

fn selectable_channel_provider(ctx: &mut dyn NativeContext, _args: &[Value]) -> MethodCallResult {
    ctx.invoke(
...
mod tests {
    ...
    fn h3b_resolve_and_vet_blocks_link_local_literal() {
        ...
        assert!(
            ...
            "expected AWS ECS task-role-credentials address to be denied"
        );
        assert!(
