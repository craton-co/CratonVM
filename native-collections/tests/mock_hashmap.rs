...
#[test]
fn concurrent_resize_uses_a_balanced_gc_safe_stripe_wait() {
    let reg = build_registry();
    let mut ctx = MockCtx::new();
    let chm = new_concurrent_hashmap(&reg, &mut ctx);

    // Enough entries to grow multiple native CHM segments. The mock records
    // NativeContext blocking transitions, making this a direct regression for
    // a resize that used to call parking_lot::RwLock::write while NativeRunning.
    for i in 0..1_000 {
        let key = boxed_int(&mut ctx, i);
        let value = boxed_int(&mut ctx, i * 3);
        call(
            &reg,
            &mut ctx,
            CHM,
            "put",
            PUT,
            &[Value::Object(Some(chm)), key, value],
        )
        .unwrap();
    }

    let (enters, exits, depth) = ctx.blocking_region_counts();
    assert!(
        enters > 0,
        "a CHM resize must use the GC-safe stripe protocol"
    );
    assert_eq!(enters, exits, "every stripe wait must leave the protocol");
    assert_eq!(depth, 0, "no completed resize may leave the mock blocked");
}
