...
pub struct MockCtx {
    ...
    /// Native blocking-protocol transitions. The collection resize tests use
    /// this to prove a CHM growth enters and leaves the GC-safe wait protocol
    /// rather than directly parking on a stripe lock as `NativeRunning`.
    blocking_region_entries: UnsafeCell<usize>,
    blocking_region_exits: UnsafeCell<usize>,
    blocking_region_depth: UnsafeCell<usize>,
}
...
impl MockCtx {
    pub fn new() -> Self {
        ...
        Self {
            ...
            blocking_region_entries: UnsafeCell::new(0),
            blocking_region_exits: UnsafeCell::new(0),
            blocking_region_depth: UnsafeCell::new(0),
        }
    }
    ...
    /// Counts of `NativeContext::{begin,end}_blocking_region` calls recorded
    /// by this mock, plus the current nesting depth.
    pub fn blocking_region_counts(&self) -> (usize, usize, usize) {
        // SAFETY: single-threaded test code.
        unsafe {
            (
                *self.blocking_region_entries.get(),
                *self.blocking_region_exits.get(),
                *self.blocking_region_depth.get(),
            )
        }
    }
...
impl cratonvm_native_api::NativeThreadAccess for MockCtx {
    ...
    fn begin_blocking_region(&mut self) {
        // SAFETY: single-threaded test code.
        unsafe {
            *self.blocking_region_entries.get() += 1;
            *self.blocking_region_depth.get() += 1;
        }
    }
    fn end_blocking_region(&mut self) {
        // SAFETY: single-threaded test code.
        unsafe {
            let depth = &mut *self.blocking_region_depth.get();
            assert!(*depth > 0, "end_blocking_region without a matching begin");
            *depth -= 1;
            *self.blocking_region_exits.get() += 1;
        }
    }
