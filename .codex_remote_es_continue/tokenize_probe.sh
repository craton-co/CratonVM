...
BIN=/data/data/bin/cratonvm-es-suite-continue-20260709-124629-r4
CP=$(tr -d '\r' < "$ES/libs/core/build/craton-testcp.txt" | paste -sd: -)
PROBEDIR=/tmp/craton-es-tokenize-probe-r4
rm -rf "$PROBEDIR"
mkdir -p "$PROBEDIR"
...
public class RegexProbe {
  public static void main(String[] args) {
    ...
    while (tm.find(start)) {
      System.out.println("token " + tm.start() + "-" + tm.end() + " g1=" + tm.group(1));
      start = tm.end();
...
for cls in TokenizeProbe RegexProbe; do
  echo "HOTSPOT $cls"
  timeout 15s java -cp "$PROBEDIR:$CP" "$cls"
  echo "CRATON $cls"
  set +e
  timeout 20s "$BIN" --java-home /usr/lib/jvm/java-21-openjdk-amd64 --Xmx 512m -cp "$PROBEDIR:$CP" "$cls" >"$PROBEDIR/$cls.craton.out" 2>"$PROBEDIR/$cls.craton.err"
  rc=$?
  set -e
  echo "rc=$rc"
  sed -n '1,80p' "$PROBEDIR/$cls.craton.out"
  sed -n '1,80p' "$PROBEDIR/$cls.craton.err"
done