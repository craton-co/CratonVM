...
//! hand-written shapes. It is an extra arm, not a replacement: the lattice is
//! path-insensitive, so it cannot see the guard-dominated induction variable
//! the unit-stride rule exists for, and that rule cannot see `(char) p`.
//!
...
pub fn analyze(graph: &Graph, schedule: &Schedule) -> CheckElision {
    ...
    let node_ranges = if range_enabled() {
        let base = crate::range_analysis::analyze_graph(graph);
        Some(if range_edges_enabled() {
            crate::range_analysis::analyze_graph_pathwise(graph, schedule)
        } else {
            crate::range_analysis::PathRanges::flow_insensitive(base)
        })
    } else {
        None
    };
...
///
/// Quadratic in the block count, which the module header already notes is small
/// in this tier; the dominance query behind it is an array index.
fn dominator_order(schedule: &Schedule) -> Vec<usize> {
    ...
    let depth: Vec<usize> = (0..n)
        .map(|b| {
            (0..n)
                .filter(|&d| d != b && schedule.dom.dominates(d, b))
                .count()
        })
        .collect();
...
pub fn range_edges_enabled() -> bool {
