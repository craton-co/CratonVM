...
impl CompilerCore {
    ...
    fn admit(&self, state: &mut MethodState, task: CompilationTask) -> bool {
        ...
        if state.ineligible || state.tier_fail_count >= MAX_TIER_FAIL_RETRIES {
...
impl TieredCompilationManager {
    ...
    /// `Interpreter`. `ineligible`, `c2_bailout`, `tier_fail_count` and the
...
mod tests {
    ...
    fn a_compile_overtaken_by_an_epoch_move_spends_no_retry() {
        ...
        assert_eq!(mgr.core.methods.lock()[&key].tier_fail_count, 1);
    }
