...
mod linux {
    ...
    fn dump_path(pid: u32) -> std::path::PathBuf {
        let dir = cratonvm_types::flags::runtime_var_os("JITDUMPDIR")
            .filter(|d| !d.is_empty())
    ...
    fn open() -> Option<DumpFile> {
        ...
        //
        // `O_NOFOLLOW` for the same reason as `perf_map::open_map`: the path is
        // predictable (`$JITDUMPDIR/jit-<pid>.dump`, defaulting to `/tmp`) and
        // the default directory is world-writable, so a pre-planted symlink
        // there would be followed and TRUNCATED with this process's privileges.
        // The sink is opt-in, so the exposure is small, and the flag costs
        // nothing: `ELOOP` reaches `fail` and disables the sink with one stderr
        // line, which is the right answer to "someone has arranged for me to
        // truncate a file I did not name".
        //
        // The DIRECTORY is not moved to `$XDG_RUNTIME_DIR`. `JITDUMPDIR` is
        // already the operator's lever, and unlike the perf map this file must
        // additionally be mappable `PROT_EXEC` — a constraint many runtime
        // directories fail, which would turn a security default into a silent
        // loss of profiling.
        let mut options = std::fs::OpenOptions::new();
        options.read(true).write(true).create(true).truncate(true);
        {
            use std::os::unix::fs::OpenOptionsExt;
            options.custom_flags(crate::perf_map::o_nofollow());
        }
