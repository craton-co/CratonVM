...
pub fn classify_exit_site(
    ...
    let reason = match deopt_reason_at_bci(deopt_points, bci) {
        ...
        // Several non-`RETHROW` points name different reasons. Take the first
        // in list order, **only to NAME the site**: this arm feeds a counter,
        // not a decision, and dropping the count would hide the exit entirely.
        // Nothing that escalates a method or de-specs a site may take this
        // shortcut — that is precisely what `ReasonAtBci::Ambiguous` exists to
        // stop.
        ReasonAtBci::Ambiguous => deopt_points
            .iter()
            .find(|p| p.bci == bci && !p.semantics.rethrow_exception)
            .map_or(DeoptReason::UnreachedCode, |p| p.reason),
...
/// cannot prove which of the two candidates fired. A lookup keyed on
/// `native_offset` (`CompiledMethod::find_deopt_point`) is unique per point and
/// is the right answer wherever the offset is available; this exists for the
/// sinks that only ever see a bci.
