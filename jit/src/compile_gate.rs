...
//!
//! **The OSR row names the FALLBACK, not the only route.** `try_osr` — the
//! caller above `compile_osr_artifact` — first asks
//! `compile_optimizing_artifact` for an optimizing body and enters it with
//! `CompiledMethod::ir_osr_enter` when it carries a stub for this bci and is
//! `ir_osr_sentinel_free`. That path is gated by `CRATONVM_JIT_OSR_OPTIMIZING`,
//! **default ON since 2026-09-05**. `compile_osr_artifact` is what runs when
//! it declines, which for any method containing a call is always (see
//! `OSR_OPTIMIZING_REFUSED` in `jit_bridge.rs` for why the sentinel-free
//! conjunct is that narrow). Both routes go through `admit`; the table lists
//! the single-pass one because that is the door whose *admission* this module
//! gates.
//!
...
impl CompileDoor {
    ...
    pub const fn builds_direct_calls(self) -> bool {
        match self {
            ...
            // this door was *incidentally* correct and structurally unguarded
            // until `H12-1` O2 (2026-09-22); both binds now go through
            // `vm::jit::helpers::admit_direct_native_entry` and it declares
            // its policy.
            CompileDoor::EagerFirstCall => true,
            // `direct_calls2` — the door H12-1 measured binding `bridge` rows
            // in strict mode. Since `H12-1` O1 every shadowing bind here goes
            // through `admit_direct_native_entry` as well; the eight
            // `jdk/internal/misc/Unsafe` scalar accessors are the remaining
            // raw binds and shadow no bytecode (see that arm's comment for
            // what they DO bypass, and where it is tracked).
    ...
    /// measurement.
    ///
    /// All three doors answer `true` as of 2026-09-01 (the arms below say how
    /// each one asks). This paragraph used to say `MethodEntry` was the only
    /// `true` -- the state before the OSR and eager-first-call doors were
    /// taught -- which contradicted the `match` it sits on. A door that ever
    /// answers `false` here is COUNTED by [`string_pin_not_asked`] rather than
    /// assumed harmless: a `false` is a statement about what the door does,
    /// not a claim that its answer would not have mattered.
    ///
...
/// exactly the shape of the defect — the OSR door passed an implicit
...
/// count it.
///
/// All three doors declare as of 2026-09-22 (`H12-1` O1/O2), so the third
/// state is unoccupied today. It stays representable because the next door
/// will start there, and because "unoccupied" is a measurement, not a
/// guarantee -- `undeclared_direct_bind_rows` is what says so per run.
///
...
pub enum DirectCallPolicy {
    ...
    Compatible,
    /// `--jdk-only`. A row may bind iff `vm_exec::invoke_or_native` -- the
    /// tail a refusal hands the call to -- would run that native itself.
    /// `SyntheticStub` and unregistered triples refuse; a `Bridge` binds
    /// unless §7 step 3 has real bytecode to prefer. This variant's doc said
    /// "only a reviewed `NativeKind::Intrinsic` row may bind" until
    /// 2026-09-22; see [`admits_shadow_bind`] for why that was a refusal
    /// which prevented nothing.
    ///
    /// [`admits_shadow_bind`]: DirectCallPolicy::admits_shadow_bind
    JdkOnly,
}
...
impl DirectCallPolicy {
    ...
    /// native, given the VM's answer to *would the generic dispatch tail run
    /// that native?*
    ///
    /// The rule is `crate::direct_native_helper`'s, stated once: `Compatible`
    /// admits everything; `JdkOnly` defers to the caller's answer, which is
    /// `jit::helpers::direct_native_bind_admitted` at every live call site and
    /// is the same predicate the callee-side gate runs per call.
    ///
    /// The parameter was `is_reviewed_intrinsic` until 2026-09-22 and the
    /// narrower question was wrong: refusing a `Bridge` bind left the site's
    /// `invoke_info` in place and `vm_exec::invoke_or_native` ran that same
    /// `Bridge`, so the refusal bought no policy and cost a three-string
    /// registry hash per call. `SyntheticStub` and unregistered triples still
    /// refuse, which is the fail-closed direction and is what a `false` here
    /// means now.
    pub const fn admits_shadow_bind(self, tail_would_run_the_native: bool) -> bool {
        match self {
            DirectCallPolicy::Compatible => true,
            DirectCallPolicy::JdkOnly => tail_would_run_the_native,
        }
    }
}
...
pub enum CompileRefusal {
    ...
    /// restores headroom.
    CodeCacheAtCapacity,
    /// This method's speculations keep failing at runtime
    /// ([`RuntimeDespecRegistry::mark`]). A verdict about the PROGRAM, not
    /// about the backend, and therefore **not** raised at
    /// [`CompileDoor::Osr`] — see [`RuntimeDespecRegistry`].
    RuntimeDespeculated,
}
...
impl CompileRefusal {
    ...
    /// bytecode; the others are configuration or resource state.
    ///
    /// `RuntimeDespeculated` is `false` for the same reason
    /// `PermanentlyBailListed` is: it is a *sticky* verdict that only the
    /// install epoch clears, so re-asking at the same door in the same epoch
    /// cannot answer differently. That it answers differently at a DIFFERENT
    /// door is not transience — transience is about time, and a caller that
    /// treats it as transient would re-run the whole pipeline every stride.
...
impl std::fmt::Display for CompileRefusal {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            CompileRefusal::RuntimeDespeculated => "de-speculated at runtime",
        };
        f.write_str(s)
    }
}
...
// ---------------------------------------------------------------------------
// A runtime de-speculation verdict is NOT a backend bail — N3
// ---------------------------------------------------------------------------

/// Why [`RuntimeDespecRegistry`] is a third registry and not the bail-list.
///
/// # The two claims
///
/// `crate::mark_jit_bail_listed`'s own contract is *"called when the heavy
/// `x64::compile` path returns None (typically because of an unsupported
/// backend pattern that won't change on retry)"*, and check 2 of [`admit`]
/// consults it at **every** door, OSR included. So when
/// `DeoptimizationController::deoptimize` converted a runtime
/// `DeoptAction::MakeNotCompilable` into a bail-list entry, it was answering a
/// question nobody asked: *the backend cannot emit this method*, on the
/// evidence that *one call site in this method mis-speculated*.
///
/// Those are different claims and the difference is expensive. One cold
/// `assert x : "msg " + y;` under `-ea` is a non-bridged `invokedynamic`, an
/// unconditional reason-8 trap, and `recommend_action(UnreachedCode)` is
/// `MakeNotCompilable` on the FIRST occurrence — so a single cold assertion
/// denied **every unrelated hot loop in that method** its OSR entry for the
/// rest of the process. A loop in a method may be perfectly compilable while
/// one call site elsewhere in it keeps trapping; the OSR door compiles the
/// loop, not the call site.
///
/// Dropping the bail-list call outright was the other candidate and it is
/// worse: the tomcat `Response.toAbsolute()` fix needed the *callee-dispatch*
/// door to honour the verdict too, and `jit_skip_set` (the VM-side set kept
/// beside this one) is consulted only by the interpreter's own per-call
/// hotness path. Hence a registry with the same reach at the entry doors and
/// none at the OSR door.
///
/// Staleness and keying are on [`RuntimeDespecRegistry`] itself.
type RuntimeDespecKey = (cratonvm_types::ClassId, Box<str>, Box<str>, Box<str>);

/// `(redefine_epoch, install_epoch)` at which the verdict was recorded.
type RuntimeDespecStamp = (u32, u64);

fn runtime_despec_stamp_now() -> RuntimeDespecStamp {
    (crate::redefine_epoch(), crate::jit_install_epoch())
}

/// One VM's runtime de-speculation verdicts. `NOTES-deopt2.md` M7.
///
/// **This used to be a process-global `OnceLock<RwLock<FxHashMap<..>>>`**, and
/// that was wrong for the reason `AGENTS.md` forbids process globals for
/// per-VM state: the key is `(ClassId, class, method, descriptor)`, and two
/// VMs in one process routinely produce the same tuple — `ClassId`s are
/// allocated per `ClassStore`, from 0, so `(ClassId(41),
/// "com/x/Service", "handle", "()V")` names a different method in each VM and
/// the same key in the map. One VM's give-up decision then shut the
/// method-entry and callee-dispatch doors on the other VM's method, which had
/// never mis-speculated. The tuple key was chosen to keep two LOADERS apart and
/// does that; it never separated two VMs, and the note that claimed it did was
/// reasoning about the wrong axis.
///
/// It lives on `JitRealm` beside `despec_registry` now, behind an `Arc`,
/// and reaches a compile through `CompileRequest::runtime_despec` /
/// [`admit`]'s `despec` parameter — the same route the per-bci registry took
/// in 2026-09-12.
///
/// # Lifecycle
///
/// Every verdict is stamped with both process epochs, exactly as
/// `crate::VerdictStamp::now(true)` stamps an OSR denial: a verdict about a
/// *speculation* depends on the code that was installed when it failed, so a
/// code-cache flush (`JitCache::clear_all`, which advances `JIT_INSTALL_EPOCH`)
/// and a redefinition both retire it. An ordinary per-method eviction moves
/// neither epoch, which is what makes the verdict stick across exactly the
/// recompile churn it exists to stop.
///
/// Keyed by the full tuple rather than by a hash, so — unlike `JIT_VERDICTS`,
/// which documents the trade — no collision can refuse an innocent method. The
/// population is bounded by the number of methods that reach
/// `MakeNotCompilable` at runtime, which is small by construction.
pub struct RuntimeDespecRegistry {
    verdicts: parking_lot::RwLock<rustc_hash::FxHashMap<RuntimeDespecKey, RuntimeDespecStamp>>,
}

impl RuntimeDespecRegistry {
    /// A new, empty registry, weakly registered in [`LIVE_REGISTRIES`] so the
    /// two remaining VM-blind free functions can reach it.
    ///
    /// Deliberately the ONLY constructor, and deliberately not `new` returning
    /// `Self`: a registry that is not in that table is invisible to
    /// [`forget_runtime_not_compilable_for_class`], so it would keep verdicts
    /// about classes the VM has since unloaded and refuse compiles of the next
    /// class under a recycled `ClassId`. There is no shape of this type that is
    /// correct to build unregistered, so there is no way to build one.
    pub fn new_registered() -> std::sync::Arc<Self> {
        let registry = std::sync::Arc::new(RuntimeDespecRegistry {
            verdicts: parking_lot::RwLock::new(rustc_hash::FxHashMap::default()),
        });
        live_registries()
            .write()
    ...
    /// Record a verdict about the PROGRAM, not about the backend: this method's
    /// speculations keep failing at runtime.
    ///
    /// Consulted by the method-entry and callee-dispatch doors and **not** by
    /// the OSR door, because a loop in the method may be perfectly compilable
    /// while one call site mis-speculates. See the type's doc for the whole
    /// argument.
    ///
    /// The one production caller is
    /// `vm/src/jit/helpers.rs::DeoptimizationController::deoptimize`, on
    /// `DeoptAction::MakeNotCompilable` — today via
    /// [`mark_runtime_not_compilable`], which is the one piece of M7 that could
    /// not land here (see that function).
    pub fn mark(
        &self,
        ...
        self.verdicts.write().insert(
            ...
            runtime_despec_stamp_now(),
        );
    }

    /// Whether a verdict recorded for this method is still current.
    pub fn is_not_compilable(
        &self,
        class_id: cratonvm_types::ClassId,
        class_name: &str,
        method_name: &str,
        ...
        // The overwhelmingly common registry is EMPTY -- a verdict needs a
        // runtime `MakeNotCompilable` -- and every method-entry and
        // callee-dispatch admission asks. Answer that case before building the
        // owned key below, which is three heap allocations and three frees per
        // compile attempt for a lookup that cannot hit.
        if verdicts.is_empty() {
            return false;
        }
        // The borrow-free lookup would need an owned key; the map is small (see
        // the type's doc) and this runs once per compile attempt, not per call.
        verdicts
            .get(&(
                class_id,
                Box::from(class_name),
            ...
            .is_some_and(|&stamp| stamp == runtime_despec_stamp_now())
    }

    /// Forget every verdict about methods of the loaded class
    /// `(class_id, class_name)`. Returns the number of entries dropped.
    ///
    /// Identity decides when both sides carry one, so a same-named class in
    /// another loader keeps its verdict; an id-less side falls back to the
    /// name, which errs towards forgetting too much — costing at most one more
    /// compile attempt.
    pub fn forget_for_class(&self, class_id: cratonvm_types::ClassId, class_name: &str) -> usize {
        let mut verdicts = self.verdicts.write();
        let before = verdicts.len();
        verdicts.retain(|(entry_id, entry_class, _, _), _| {
            if entry_id.as_u32() != 0 && class_id.as_u32() != 0 {
                *entry_id != class_id
            } else {
    ...
    /// How many methods in THIS VM currently carry a current verdict.
    pub fn current_len(&self) -> usize {
        let now = runtime_despec_stamp_now();
        self.verdicts
            .read()
            .values()
...
/// Every [`RuntimeDespecRegistry`] still alive in this process, weakly.
///
/// **Not** storage for verdicts — each registry owns its own — but the address
/// book the two remaining free functions need, because their callers cannot
/// name a VM:
///
///  * [`mark_runtime_not_compilable`] is called from
///    `vm/src/jit/helpers.rs::DeoptimizationController::deoptimize`, which
///    *does* have the `SharedVm` in scope and wants
///    `vm.jit.runtime_despec.mark(..)` — a one-line change this lane could not
///    make, since it does not own that file (recorded in `NOTES-cha.md`).
///  * [`forget_runtime_not_compilable_for_class`] is called from
///    `TieredCompilationManager::invalidate_class` / `on_class_redefined`
///    (`jit/src/tiered.rs`), which has no VM in scope at all and belongs to
///    another lane this cycle.
///
/// Fanning out is safe in wildly different degrees for the two, and the
/// difference is the whole reason they are documented apart:
///
///  * FORGETTING in every VM is harmless in principle. A dropped verdict costs
///    at most one more compile attempt, which is the same argument the
///    name-fallback inside [`RuntimeDespecRegistry::forget_for_class`] already
///    makes.
///  * MARKING in every VM is exactly the bug M7 is about, so it is done only
///    when the process cannot tell which VM meant it: with exactly one live
///    registry — the overwhelmingly common case, and every single-VM process —
///    the mark is routed to that one and is precisely per-VM. With two or more
///    it falls back to marking all of them and counts the event in
///    [`AMBIGUOUS_RUNTIME_MARKS`], so the residual is measurable rather than
///    assumed absent.
///
/// Dead `Weak`s are reaped on each pass; the vector's length is the number of
/// VMs ever created in the process, which is 1 outside the embedding tests.
static LIVE_REGISTRIES: std::sync::OnceLock<
    parking_lot::RwLock<Vec<std::sync::Weak<RuntimeDespecRegistry>>>,
> = std::sync::OnceLock::new();

fn live_registries() -> &'static parking_lot::RwLock<Vec<std::sync::Weak<RuntimeDespecRegistry>>> {
    LIVE_REGISTRIES.get_or_init(|| parking_lot::RwLock::new(Vec::new()))
}

/// Snapshot of the live registries, reaping dead `Weak`s as it goes.
fn live_registry_snapshot() -> Vec<std::sync::Arc<RuntimeDespecRegistry>> {
    let mut slots = live_registries().write();
    slots.retain(|w| w.strong_count() > 0);
    slots.iter().filter_map(std::sync::Weak::upgrade).collect()
}

/// How many times [`mark_runtime_not_compilable`] could not tell which VM a
/// runtime de-speculation verdict belonged to and marked every live registry.
///
/// Non-zero means the residual described on [`LIVE_REGISTRIES`] is live in this
/// run: two or more VMs share the process and one of them may have had a
/// verdict recorded that it did not earn. Zero — every single-VM process — means
/// M7's property holds exactly.
pub static AMBIGUOUS_RUNTIME_MARKS: AtomicU64 = AtomicU64::new(0);

/// Record a runtime de-speculation verdict without naming a VM.
///
/// The VM-naming form is [`RuntimeDespecRegistry::mark`] and is what every
/// caller should use. This exists for the single production call site that
/// cannot be changed from this lane —
/// `vm/src/jit/helpers.rs::DeoptimizationController::deoptimize`, which holds a
/// `&SharedVm` and needs one line changed to `vm.jit.runtime_despec.mark(..)`.
/// See [`LIVE_REGISTRIES`] for what routing is possible without it, and
/// `NOTES-cha.md` for the edit.
pub fn mark_runtime_not_compilable(
    class_id: cratonvm_types::ClassId,
    class_name: &str,
    method_name: &str,
    descriptor: &str,
) {
    let live = live_registry_snapshot();
    if live.len() > 1 {
        AMBIGUOUS_RUNTIME_MARKS.fetch_add(1, Ordering::Relaxed);
    }
    for registry in &live {
        registry.mark(class_id, class_name, method_name, descriptor);
    }
}

/// Forget every runtime de-speculation verdict about methods of the loaded
/// class `(class_id, class_name)`, in every live VM.
///
/// Called by the tiered manager beside `crate::forget_jit_verdicts_for_class`
/// on class unload and on redefinition. A redefinition is already caught by the
/// stamp; an UNLOAD is not, and `ClassId`s are recycled — without this, the
/// next class loaded under a recycled id and a matching name would inherit a
/// verdict about bytecode it has never run.
///
/// Fans out across VMs deliberately: see [`LIVE_REGISTRIES`] for why forgetting
/// too much costs at most one compile attempt while marking too much does not.
/// Returns the total number of entries dropped.
pub fn forget_runtime_not_compilable_for_class(
    class_id: cratonvm_types::ClassId,
    class_name: &str,
) -> usize {
    live_registry_snapshot()
        .iter()
        .map(|r| r.forget_for_class(class_id, class_name))
        .sum()
}

/// Diagnostic: how many methods across every live VM currently carry a current
/// runtime de-speculation verdict.
///
/// Read beside `crate::jit_bail_list_size()`. Before N3 these two were the same
/// number by construction, because every runtime verdict was also written to
/// the bail-list; a run in which they differ is a run in which some method kept
/// its OSR door open.
pub fn runtime_not_compilable_size() -> usize {
    live_registry_snapshot()
        .iter()
        .map(|r| r.current_len())
        .sum()
...
impl CompileAdmission {
    ...
    ///
    /// `None` was the state the OSR and eager first-call doors were in when
    /// this landed; both declare as of 2026-09-22 (`H12-1` O1/O2). It is the
    /// state [`undeclared_direct_bind_rows`] counts, and the reason that
    /// counter is expected to read zero now rather than to have been deleted.
    ...
    /// registered native?
    ///
    /// `tail_would_run_the_native` is the VM's answer for the triple whose
    /// native actually runs: *would `vm_exec::invoke_or_native` run it?* It is
    /// a closure because only the strict arm needs it and only for a row that
    /// is otherwise bindable, and because this crate cannot see `NativeKind`
    /// at all. It was `is_reviewed_intrinsic` — `NativeKind == Intrinsic`
    /// alone — until 2026-09-22; see [`DirectCallPolicy::admits_shadow_bind`]
    /// for why the narrower question was a refusal that prevented nothing.
    ///
    /// The only production caller is
    /// `vm/src/jit/helpers.rs::admit_direct_native_entry`, which asks this
    /// AND the policy its VM is actually executing under, so a door whose
    /// declaration drifts from its VM cannot widen the gate.
    ///
    ...
    pub fn admits_direct_bind(
        &self,
        tail_would_run_the_native: impl FnOnce() -> bool,
    ) -> bool {
        match self.direct_call_policy() {
            Some(p) => p.admits_shadow_bind(tail_would_run_the_native()),
            None => true,
        }
    }
...
/// this one's verdict.
///
/// `despec` is the COMPILING VM's runtime de-speculation registry
/// ([`RuntimeDespecRegistry`], owned by `JitRealm::runtime_despec` and threaded
/// through `CompileRequest::runtime_despec`). `None` — no VM in scope, which is
/// every backend test — consults nothing and so can never raise
/// [`CompileRefusal::RuntimeDespeculated`]. That is the fail-OPEN direction on
/// purpose: the verdict is about a program's behaviour at runtime, and a caller
/// with no VM has no runtime to have observed it in. Passed as a REQUIRED
/// parameter rather than defaulted, which is the shape `H20-1 §6` specifies as
/// O1, so a new door cannot acquire the check by accident or lose it by
/// silence.
pub fn admit(
    class_id: cratonvm_types::ClassId,
    class_name: &str,
    ...
    descriptor: &str,
    door: CompileDoor,
    despec: Option<&RuntimeDespecRegistry>,
    ...
    } else if door != CompileDoor::Osr
        && despec
            .is_some_and(|d| d.is_not_compilable(class_id, class_name, method_name, descriptor))
    {
        // N3. Deliberately NOT asked at `CompileDoor::Osr`. The verdict says
        // "this method's speculations keep failing at runtime", which is
        // evidence about one call site; the OSR door is compiling a LOOP, and
        // a loop in a method with one bad call site is the ordinary shape of
        // `assert` under `-ea` — one cold non-bridged `invokedynamic`, an
        // unconditional reason-8 trap, `MakeNotCompilable` on the first
        // occurrence. Before this check existed at all the same verdict went
        // into the permanent bail-list, which check 2 above DOES ask at every
        // door, so that cold assertion denied the method's hot loops their OSR
        // entry for the rest of the process.
        Some(CompileRefusal::RuntimeDespeculated)
...
/// instrument.
///
/// # H12-1 N2: this was the first counter that could see the OSR door's binds
///
/// `THREAD_CURRENT_THREAD_SITES_OSR` was the only per-door bind counter in the
/// tree that reached the OSR door, and it covers one triple. `H7-1` §6c
...
///
/// N2 asked for the specific counter as well, and it landed 2026-09-22:
/// `HASHMAP_{GET,PUT}_DIRECT_SITES_OSR`, printed beside the single-pass triple
/// on the `compiled collection direct-helper binds` line. This counter stays
/// the general one — it answers "did a door bind rows without declaring",
/// which no per-triple counter can.
...
/// [`DirectCallPolicy`].
///
/// **Expected to be zero at every door since 2026-09-22**, when `H12-1` O1/O2
/// landed and the OSR and eager first-call doors began declaring. Until then
/// it was the size of the bypass, per door, which no instrument in this tree
/// reported before. A non-zero now is a door that builds `direct_calls`
/// without declaring — a fourth one, or a regression in one of the three.
///
/// # Reading a zero
///
...
pub(crate) fn reset_for_test() {
    ...
    // The runtime de-speculation verdicts are per VM now (M7), so there is no
    // process-wide map to clear here. Each test that uses them builds its own
    // `RuntimeDespecRegistry` and drops it, which also drops it out of
    // `LIVE_REGISTRIES` at the next snapshot.
    AMBIGUOUS_RUNTIME_MARKS.store(0, Ordering::Relaxed);
}
...
mod tests {
    ...
    /// binary: three tests here call `admit(.., CompileDoor::Osr, None)` and three
    ...
    fn an_admission_opens_and_closes_the_thread_scope() {
        ...
        let a =
            admit(NO_ID, &cls, "m", "()V", CompileDoor::Osr, None).expect("clean method admits");
        assert!(admission_is_open());
    ...
    fn a_bail_listed_method_is_refused_at_every_door() {
        ...
        for door in CompileDoor::ALL {
            assert_eq!(
                admit(NO_ID, &cls, "m", "()V", door, None).err(),
                Some(CompileRefusal::PermanentlyBailListed),
    ...
    fn only_the_cap_refusal_is_transient() {
        ...
        assert!(!CompileRefusal::RuntimeDespeculated.is_transient());
    }

    /// **The N3 fix, stated as the difference between the two verdicts.**
    ///
    /// The bail-list means *the backend cannot emit this method* and is
    /// therefore honoured at all three doors — that is
    /// `a_bail_listed_method_is_refused_at_every_door` above. A runtime
    /// de-speculation verdict means *one call site in this method keeps
    /// mis-speculating*, and the OSR door is not compiling that call site: it
    /// is compiling a loop. Converting the second into the first is what let
    /// one cold `assert x : "msg " + y;` under `-ea` — a non-bridged
    /// `invokedynamic`, an unconditional reason-8 trap, `MakeNotCompilable` on
    /// the FIRST occurrence — deny every unrelated hot loop in the method its
    /// OSR entry for the rest of the process.
    ///
    /// Written as a per-door table rather than as "OSR admits", because the
    /// half that is easy to lose in a later edit is that the other two doors
    /// still refuse: a verdict nobody honours anywhere is the tomcat
    /// `Response.toAbsolute()` recompile thrash coming back.
    #[test]
    fn a_runtime_despeculated_method_is_refused_everywhere_except_the_osr_door() {
        let _guard = COUNTER_LOCK.lock();
        let cls = unique("runtime-despec");
        let despec = RuntimeDespecRegistry::new_registered();
        assert!(!despec.is_not_compilable(NO_ID, &cls, "m", "()V"));
        despec.mark(NO_ID, &cls, "m", "()V");
        assert!(despec.is_not_compilable(NO_ID, &cls, "m", "()V"));

        for door in CompileDoor::ALL {
            let got = admit(NO_ID, &cls, "m", "()V", door, Some(&despec)).err();
            let want = match door {
                // A loop in this method may be perfectly compilable.
                CompileDoor::Osr => None,
                _ => Some(CompileRefusal::RuntimeDespeculated),
            };
            assert_eq!(
                got,
                want,
                "{} answered the runtime de-speculation verdict wrongly",
                door.label()
            );
        }

        // And an unrelated method of the same class keeps every door.
        assert_eq!(
            admit(
                NO_ID,
                &cls,
            ...
            .err(),
            None,
            "the verdict is per method, not per class"
        );

        // A door asked with NO registry cannot raise the verdict at all. That
        // is the fail-OPEN direction and it is deliberate: a caller with no VM
        // has no runtime in which this method could have mis-speculated.
        assert_eq!(
            admit(NO_ID, &cls, "m", "()V", CompileDoor::MethodEntry, None).err(),
            None
        );
    }

    /// **The M7 fix, stated as the property the process global did not have.**
    ///
    /// Two VMs in one process allocate `ClassId`s independently, from 0, so
    /// `(ClassId(41), "com/x/Service", "handle", "()V")` names a different
    /// method in each and the SAME key in a process-wide map. One VM reaching
    /// `MakeNotCompilable` then shut the method-entry and callee-dispatch doors
    /// on the other VM's method, which had never mis-speculated. The tuple key
    /// does keep two LOADERS apart — `a_runtime_verdict_is_per_loaded_class_identity`
    /// asserts that — but it never separated two VMs, and the note that claimed
    /// it did was reasoning about the wrong axis.
    #[test]
    fn one_vms_runtime_verdict_does_not_reach_another_vm() {
        let _guard = COUNTER_LOCK.lock();
        let cls = unique("despec-two-vms");
        let first = RuntimeDespecRegistry::new_registered();
        let second = RuntimeDespecRegistry::new_registered();
        // The same class id on purpose: that is what two VMs in one process
        // routinely produce, and it is the state the old key could not tell
        // apart.
        first.mark(NO_ID, &cls, "m", "()V");

        assert!(first.is_not_compilable(NO_ID, &cls, "m", "()V"));
        assert!(
            !second.is_not_compilable(NO_ID, &cls, "m", "()V"),
            "a second VM must not inherit a verdict it never earned"
        );
        assert_eq!(
            admit(
                NO_ID,
            ...
            .err(),
            None,
            "and the door it reaches must still open"
        );
    }

    /// A class that is gone must not leave a runtime verdict behind for the
    /// next class loaded under its recycled id or its name.
    ///
    /// The stamp catches a redefinition on its own — the redefine epoch moves —
    /// but an UNLOAD moves neither epoch, and `ClassId`s are recycled. This is
    /// why the tiered manager calls
    /// [`forget_runtime_not_compilable_for_class`] beside
    /// `crate::forget_jit_verdicts_for_class` on both paths.
    #[test]
    fn forgetting_a_class_drops_its_runtime_despeculation_verdicts() {
        let _guard = COUNTER_LOCK.lock();
        let gone = unique("despec-unloaded");
        let kept = unique("despec-survivor");
        let despec = RuntimeDespecRegistry::new_registered();
        despec.mark(NO_ID, &gone, "m", "()V");
        despec.mark(NO_ID, &gone, "n", "()I");
        despec.mark(NO_ID, &kept, "m", "()V");

        assert_eq!(despec.forget_for_class(NO_ID, &gone), 2);
        assert!(!despec.is_not_compilable(NO_ID, &gone, "m", "()V"));
        assert!(!despec.is_not_compilable(NO_ID, &gone, "n", "()I"));
        assert!(
            despec.is_not_compilable(NO_ID, &kept, "m", "()V"),
            "another class's verdict must survive"
        );
        assert_eq!(despec.forget_for_class(NO_ID, &kept), 1);
    }

    /// Two same-named classes in different loaders keep separate verdicts, and
    /// a verdict recorded against one copy does not refuse the other.
    ///
    /// The same property `mark_jit_bail_listed` has, asserted here because this
    /// registry is keyed by a whole tuple rather than by `JIT_VERDICTS`' hash
    /// and so cannot inherit that store's test.
    #[test]
    fn a_runtime_verdict_is_per_loaded_class_identity() {
        let _guard = COUNTER_LOCK.lock();
        let cls = unique("despec-loaders");
        let app = cratonvm_types::ClassId::new(7001);
        let plugin = cratonvm_types::ClassId::new(7002);
        let despec = RuntimeDespecRegistry::new_registered();
        despec.mark(app, &cls, "m", "()V");
        assert!(despec.is_not_compilable(app, &cls, "m", "()V"));
        assert!(
            !despec.is_not_compilable(plugin, &cls, "m", "()V"),
            "the other loader's copy is different bytecode and keeps its doors"
        );
        // And forgetting by identity leaves the other copy alone.
        despec.mark(plugin, &cls, "m", "()V");
        assert_eq!(despec.forget_for_class(app, &cls), 1);
        assert!(despec.is_not_compilable(plugin, &cls, "m", "()V"));
        assert_eq!(despec.forget_for_class(plugin, &cls), 1);
    }

    /// The verdict stops counting once the install epoch moves, like an OSR
    /// denial — a code-cache flush replaced every body it was recorded about.
    #[test]
    fn a_runtime_verdict_expires_when_the_install_epoch_moves() {
        let _guard = COUNTER_LOCK.lock();
        let cls = unique("despec-epoch");
        let despec = RuntimeDespecRegistry::new_registered();
        despec.mark(NO_ID, &cls, "m", "()V");
        assert!(despec.is_not_compilable(NO_ID, &cls, "m", "()V"));
        // Age the entry in place rather than calling `bump_jit_install_epoch`.
        // That counter is process-wide and `JitCache::flush_barrier` compares
        // against it, so a test that moves it can refuse an unrelated
        // concurrent test's `put` — the shape that reads as flakiness and is
        // not. `COUNTER_LOCK` serialises this module's tests and nothing else.
        {
            let mut v = despec.verdicts.write();
            let (redefine, install) = runtime_despec_stamp_now();
            for (_, stamp) in v.iter_mut() {
                *stamp = (redefine, install.wrapping_sub(1));
            }
        }
        assert!(
            !despec.is_not_compilable(NO_ID, &cls, "m", "()V"),
            "a flush retires the speculation the verdict was about"
        );
        assert_eq!(
            admit(
                NO_ID,
    ...
    fn the_direct_bind_rule_refuses_exactly_one_cell() {
        let _guard = COUNTER_LOCK.lock();
        let a = admit(NO_ID, &unique("rule"), "m", "()V", CompileDoor::Osr, None).expect("admits");
