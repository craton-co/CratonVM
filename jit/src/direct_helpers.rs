...
///
/// Nine: the eight `ScopedMemoryAccess` accessors and `varhandle_cas`. Every
/// one is [`DirectHelperCallPath::JavaArgMarshalled`], which is the property
/// the const assertion below actually pins — the count alone would be satisfied
...
const _: () = {
    ...
    assert!(
        DIRECT_HELPERS_NEEDING_WIN64_STACK_ARGS == 9,
        "the number of direct helpers spilling arguments on Win64 changed. The \
         nine are the eight ScopedMemoryAccess accessors and varhandle_cas, all \
         reached through JitDirectCall's `emit_stack_arg_setup`. A new one is \
         fine on that route and is NOT fine on the hand-written route — say \
         which by giving the new row its DirectHelperCallPath, then update this \
         count.",
    );
    assert!(
        DIRECT_HELPERS_NEEDING_SYSV_STACK_ARGS == 4,
        "the number of direct helpers spilling arguments on SysV changed. The \
         four are the ScopedMemoryAccess `put` accessors at seven words each.",
    );
};
...
//
// 117 words = 32 scalar slots + 40 read + 36 write + 9 CAS.
// 31 signature rows = 32 scalar slots MINUS the four that are not baked
// callables (`static_base_resolver`, `static_base_resolver_ctx` and the two
// served-class predicates) PLUS one row per VarHandle array (every element of
// an array is a monomorphisation of one function and shares its signature).
const _: () = {
    assert!(
        core::mem::size_of::<DirectHelperTable>() == 117 * core::mem::size_of::<usize>(),
