...
pub fn unregister_range(start: usize, len: usize) {
    if !EVER_REGISTERED.load(Ordering::Relaxed) {
        return;
    }
    #[cfg(target_os = "linux")]
    linux::unregister_range(start, len);
    #[cfg(not(target_os = "linux"))]
    let _ = (start, len);
}
...
mod linux {
    ...
    pub(super) fn unregister_range(start: usize, len: usize) {
        let mut reg = registry().lock().unwrap_or_else(PoisonError::into_inner);
        let end = start.saturating_add(len);
        let inside: Vec<usize> = reg
            .by_start
            .keys()
            .copied()
            ...
            .collect();
        for k in inside {
            if let Some(entry) = reg.by_start.remove(&k) {
                // SAFETY: `entry` came from `Box::into_raw` in `register` and
                // is still linked; the registry lock is held.
