...
//! unmapping passes through. Only the GDB sink acts on it: a perf map is
...
/// The buffer starting at `start` is about to be unmapped.
pub fn retire(start: usize) {
    if start == 0 {
        return;
    }
    crate::gdb_jit::unregister(start);
}
