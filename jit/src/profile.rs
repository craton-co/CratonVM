...
mod branch_counter_tests {
    ...
    /// A per-method arm is sticky, reports only its first call, and survives
    /// the array being regrown for a longer code length.
    #[test]
    fn a_method_arm_is_sticky_and_survives_a_regrow() {
        let c = BranchCounters::new(4);
        assert!(!c.is_armed());
        ...
        let small = p.flat_branches_for(4);
        assert!(!small.is_armed());
        small.arm();
        let grown = p.flat_branches_for(16);
        assert!(!Arc::ptr_eq(&small, &grown), "regrown");
        assert!(grown.is_armed(), "the arm carries over");
    }
...
impl MethodProfile {
    ...
    /// time. There is no provenance *per direction*: a branch hint is a layout
    /// choice, a wrong one costs a mis-laid-out block, and nothing downstream
    /// needs to know which observations produced the ratio — the caller's job
    /// is to keep the seed small enough that live observations can outvote it,
    /// which `crate::profile_store::ReplaySeedPolicy` documents and does.
