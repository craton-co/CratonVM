...
///
/// # There is a second copy of this computation (R10)
///
/// `regalloc::ls_loop_depths` is it, and it is identical in definition — the
/// same back-edge test and the same predecessor walk, over that module's own
/// `Schedule`. Its doc comment says so; this one did not, and a back-pointer
/// at only one end is how the next person to find two loop-depth models
/// deletes the wrong one.
///
/// **Neither is redundant.** This one takes `(&[Block], &Dominators)`, both
/// private to this module, and `sink_pure_nodes` calls it while the block list
/// is still being built — from the middle of `schedule_with_options`, long
/// before the `Schedule` that function returns is constructed. `regalloc`'s
/// caller does have a full `Schedule` (and therefore `schedule.dom`), so it
/// *could* call a shared form; the bytecode colourer named in
/// `NOTES-regalloc.md` §7 has neither a `Schedule` nor a dominator relation
/// and could not. The shared form, when someone takes it, is a free function
/// over `(successors: &[Vec<usize>], predecessors: &[Vec<usize>], dominates:
/// impl Fn(usize, usize) -> bool)` with both callers adapting — which is a
/// refactor of two files, not of this one.
///
...
///
/// `regalloc::ls_loop_depths` still counts per edge; the change it needs is
/// `docs/known-issues/jit/regalloc-ls-loop-depths-counts-back-edges-not-loops-20260918.md`.
...
fn sink_pure_nodes(
    ...
    if !ok && equal_depth {
        ...
        moved = place_sunk_nodes(graph, blocks, node_to_block, dom, &depth, &innermost, false);
...
impl Schedule {
    /// Scheduled data nodes that read an input whose block does not dominate
    /// their own, in block order.
    ///
    /// Phis are skipped: they read each value input on its own incoming edge,
    /// so their inputs need not dominate the merge. An input with no block
    /// (`usize::MAX`) is skipped too; `ir_lower::verify_data_locations`
    /// already names that case.
    ///
    /// Empty for every graph the scheduler can place legally. A non-empty
    /// answer is the shape `find_best_block` falls back on (a node whose inputs
    /// live in incomparable blocks, which well-formed SSA does not have), and
    /// it names the node so a refusal can say which pass broke the graph
    /// instead of reporting a generic def-after-use.
    pub fn misplaced_data_nodes(&self, graph: &Graph) -> Vec<NodeId> {
        ...
        for (b, block) in self.blocks.iter().enumerate() {
            for &id in &block.nodes {
                ...
                let bad = node.inputs.iter().any(|&inp| {
                    if inp == NO_NODE {
                        return false;
                    }
                    match self.node_to_block.get(inp as usize) {
                        Some(&ib) if ib != usize::MAX => !self.dom.dominates(ib, b),
                        _ => false,
                    }
                });
                if bad {
...
///
/// Computed once per method by `schedule_with_options` and handed to every
/// block's [`priority_sort_block`].
