...
pub(super) struct ArrayLenHoist {
    ...
    /// The bci the pre-header's null check reports when it traps: the
    /// `arraylength` of the site that OPENED this record (the one the original
    /// program evaluates first on entry). For a top-tested loop that is the
    /// first site; for a relocated rotated loop it is the COND site
    /// ([`find_array_len_hoists_with_rotation`]).
    pub(super) trap_bci: usize,
}
...
/// Every pc the interpreter can enter an OSR body at: the target of a
/// backward branch. Its branch arms offer OSR exactly when the taken offset is
/// negative (`vm/src/runtime/interpreter.rs`: `if offset < 0 { ...
/// backward_count += 1; let entry_pc = frame.pc; ... }`), and
/// `bytecode_analysis::back_edges` is that set -- switches and `goto_w`
/// included, which is wider than `detect_loops` and the right direction here.
pub(super) fn osr_entry_pcs(code: &[u8], code_len: usize) -> Vec<usize> {
...
/// Whether an OSR entry can land STRICTLY inside the loop `[header, loop_end)`
/// -- at an inner loop's header, typically -- given [`osr_entry_pcs`].
///
/// A pre-header transform (a hoist slot, a speculative guard) is written only
/// on a pass through `header`, so such an entry reaches the body with it
/// unwritten. The walk answers that for the hoists by publishing every pc
/// strictly inside a hoisted loop OSR-ineligible (`inside_*_hoisted` in
/// `bytecode_walk.rs`) -- sound, but it takes the OSR entry away from the
/// INNER loop, which is the one hot enough to want it. A method that runs a
/// long inner loop once then stays in the interpreter until that inner loop
/// ends: measured round 9 wave 8 on the w7b binary
/// (`C:\craton\jitr9-probes\review8a\src\OsrHoistCliff.java`, two outer
/// iterations of a 150M-iteration inner loop, called once), an outer
/// `i < rows.length` hoist cost 18 151 ms against 210 ms without it, an
/// arithmetic hoist 25 220 ms against 202 ms, an `aaload` hoist 28 909 ms
/// against 229 ms (HotSpot: ~100 ms each). So the hoist finders do not attach
/// a hoist to such a loop at all; the `aaload` and arithmetic finders then
/// attach it to the inner loop instead, where it is re-evaluated once per inner
/// loop entry, and the inner header keeps its OSR entry (an entry AT a hoisted
/// header runs the pre-header: `osr_entry_native[header]` points before it).
pub(super) fn span_holds_inner_osr_entry(
    entries: &[usize],
    header: usize,
    loop_end: usize,
) -> bool {
    entries.iter().any(|&t| t > header && t < loop_end)
}
...
/// * no OSR entry can land strictly inside the body
///   ([`span_holds_inner_osr_entry`]): an outer loop whose inner loop is hot
///   would otherwise lose that inner loop's OSR entry, and
...
// Only the tests call the rotation-free form since the driver switched to
// `find_array_len_hoists_with_rotation` (round 9 wave 3).
...
pub(super) fn find_array_len_hoists(
    ...
    find_array_len_hoists_with_rotation(code, code_len, loops, &FxHashMap::default())
}

/// [`find_array_len_hoists`], aware of ROTATED loops whose pre-header the
/// driver relocates to the loop's entry `goto` (`rotation`: header -> entry
/// `goto` pc, i.e. `find_rotation_preheader_entries`' answer; empty unless
/// `CRATONVM_JIT_ROTATED_PREHEADER` is armed, and then this is exactly
/// [`find_array_len_hoists`]).
///
/// A rotated loop (`goto COND; BODY: ...; COND: ... if<cond> BODY`) is entered
/// only through its `goto`, which lands on COND, not on the header. So the
/// dominance restriction that licenses an eager `arraylength` in the
/// pre-header is proved against COND's straight-line prefix instead of the
/// header's: a site there is evaluated on the FIRST pass through COND on every
/// entry, zero-trip included, so a pre-header at the `goto` may throw its NPE
/// with nothing Java-visible having happened in between. A site that is only
/// on the HEADER's prefix does NOT open a record for a rotated loop -- a
/// zero-trip rotated loop never runs the body. Once opened, every other
/// invariant site of the same array joins, exactly as for a top-tested loop;
/// COND is scanned first so body sites textually above it can join.
///
/// `trap_bci` is the opening site's `arraylength`: the one the original
/// program would have executed first.
pub(super) fn find_array_len_hoists_with_rotation(
    code: &[u8],
    code_len: usize,
    loops: &[(usize, usize)],
    ...
    // A `FxHashSet`, not a `Vec`. This set grows across ALL loops and was
    // scanned linearly at every pc of every loop, which made the claim test
    // alone O(sites × body × loops). `licm_int::find_arith_loop_hoists` already
    // uses a set for the same job, so this was also an inconsistency between
    // two neighbouring passes doing the same bookkeeping.
    let mut claimed_pcs: FxHashSet<usize> = FxHashSet::default();
    ...
    for &(header, back_edge) in &sorted_loops {
        ...
        // A loop an OSR entry can land inside keeps its `arraylength` in the
        // body: see `span_holds_inner_osr_entry` for the measured cost of the
        // alternative (the walk vetoing the inner loop's OSR entry).
        if span_holds_inner_osr_entry(&osr_entries, header, loop_end) {
            continue;
        }

        // Where the loop is ENTERED: the header, or for a relocated rotated
        // loop the entry `goto`'s target (COND). A rotation whose target is
        // not inside the loop is not one this analysis understands.
        let entry = match rotation.get(&header) {
            None => header,
            Some(&goto_pc) => match bytecode_analysis::offset_branch_target(code, goto_pc) {
                Some(cond) if cond >= header && cond < loop_end => cond,
                _ => continue,
            },
        };
        ...
        // COND first for a rotated loop (see the doc comment); one pass over
        // the whole body otherwise, which is the historical scan exactly.
        let ranges: [(usize, usize); 2] = if entry == header {
            [(header, loop_end), (loop_end, loop_end)]
        } else {
            [(entry, loop_end), (header, entry)]
        };
        for (from, to) in ranges {
            let mut pc = from;
            while pc < to && pc < code_len {
                ...
                // `find_modified_locals` saturates every local index at bit 63,
                // so bit 63 means "some local at or above 63 was stored" and
                // proves nothing about local 63 itself.
                ...
                // Dominance is a condition on OPENING a record, not on joining
                // one. It licenses the pre-header's eager evaluation; once some
                // site has licensed it, the slot holds this array's true length
                // for the whole body, and any other invariant read of the same
                // length -- including one behind a conditional -- may take it.
                // Such a site's own null check goes with it, which is sound
                // because the pre-header already proved the receiver non-null
                // on this path.
                let admissible = invariant
                    && !interior_entered
                    && seq_end <= loop_end
                    && (existing_idx.is_some()
                        || straight_line_prefix_of_header(code, entry, pc, &targets));
                if admissible {
                    claimed_pcs.insert(pc);
                    match existing_idx {
    ...
    // Sites in pc order (a rotated scan visits COND first). A no-op for every
    // top-tested loop, whose single pass already produced them in order.
    for h in &mut hoists {
        h.sites.sort_unstable();
    }
    hoists
}
...
/// The branch targets of a loop body: the exact ones, plus the spans a
/// `switch` makes wholly untrustworthy.
///
/// Two representations because the two sources are different in kind. A
/// `goto`/`if_*` names ONE pc. A `tableswitch`/`lookupswitch` is not decoded
/// here at all (that would duplicate `bytecode_analysis::step`), so its
/// contribution is "everything from the switch to the end of the body may be
/// entered" — a claim about a RANGE, which materialising one pc at a time turns
/// into a vector the size of the body.
#[derive(Default)]
struct BranchTargets {
    /// Exact targets, from the offset-carrying branches.
    pcs: FxHashSet<usize>,
    /// `[from, to)` spans a `switch` may enter anywhere in.
    covered: Vec<(usize, usize)>,
}

impl BranchTargets {
    /// Can control reach `pc` other than by falling into it?
    ///
    /// Conservative: a pc inside a switch-covered span answers `true` whether
    /// or not it is a real case label, which is the same answer the old
    /// materialised form gave and for the same reason.
    fn contains(&self, pc: usize) -> bool {
        self.pcs.contains(&pc) || self.covered.iter().any(|&(lo, hi)| pc >= lo && pc < hi)
    }
}
...
/// landing pad.
fn branch_target_pcs(code: &[u8], start: usize, end: usize) -> BranchTargets {
    ...
    while pc < end {
        ...
        match op {
            ...
            //
            // Recorded as a RANGE rather than materialised one PC at a time.
            // The old form pushed `end - pc` entries into a `Vec` that every
            // `contains` below then scanned linearly, so one switch in a 4 KB
            // loop body produced a ~4000-element vector and turned the per-PC
            // walk into O(body²) — per loop, times the number of loops. The
            // covered span is what the analysis actually means, and testing a
            // PC against it is two comparisons.
            0xaa | 0xab => {
                targets.covered.push((pc, end));
            }
            _ => {}
        }
        pc += len;
...
pub fn inline_putfield_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| !cratonvm_types::flags::runtime_flag_on("CRATONVM_NO_JIT_INLINE_PUTFIELD"))
}
...
pub fn g1_inline_barrier_enabled() -> bool {
    if let Some(forced) = g1_inline_barrier_forced() {
        return forced;
    }
    cratonvm_types::flags().gc.g1_inline_barrier
}

/// The production half of the test-only override `G1_INLINE_BARRIER_FORCED`
/// (declared, with its test-build twin of this function, at the bottom of this
/// file, below every production item, so the panic-free ratchet -- which stops
/// at the first test gate -- scans the whole of this module): never forced.
#[cfg(not(test))]
#[inline(always)]
fn g1_inline_barrier_forced() -> Option<bool> {
    None
}
...
pub fn sp_ref_store_trace_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_SP_REF_STORE_TRACE"))
}

pub fn ir_ref_store_trace_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_IR_REF_STORE_TRACE"))
}
...
pub fn inline_tlab_new_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| !cratonvm_types::flags::runtime_flag_on("CRATONVM_NO_JIT_INLINE_TLAB_NEW"))
}
...
pub fn inline_tlab_newarray_enabled() -> bool {
    ...
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| {
        !cratonvm_types::flags::runtime_flag_on("CRATONVM_NO_JIT_INLINE_TLAB_NEWARRAY")
    })
}
...
pub fn inline_tlab_zero_elision_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| !cratonvm_types::flags::runtime_flag_on("CRATONVM_NO_JIT_TLAB_ZERO_ELISION"))
}
...
pub fn inline_getfield_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_INLINE_GETFIELD"))
}
...
pub fn guarded_inline_getfield_enabled() -> bool {
    ...
    !cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_GETFIELD_HELPER")
}
...
pub fn inline_self_guard_enabled() -> bool {
    ...
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| {
        cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_INLINE_SELF_GUARD")
    })
}
...
pub(super) fn self_cache_inherit_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| !cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_NO_SELF_CACHE_INHERIT"))
}
...
pub fn precise_inline_frame_record_enabled() -> bool {
    ...
    *G.get_or_init(|| {
        precise_jit_maps_enabled()
            && !cratonvm_types::flags::runtime_flag_on("CRATONVM_NO_PRECISE_INLINE_FRAME_RECORD")
    })
}
...
pub fn verify_inline_frame_record_enabled() -> bool {
    ...
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| {
        cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_VERIFY_INLINE_FRAME_RECORD")
    })
}
...
pub fn inline_rbp_tls_disp() -> usize {
    ...
    *DISP.get_or_init(|| {
        ...
        if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_INLINE_FR") {
            if disp != 0 {
                eprintln!(
...
pub fn jit_thread_tls_disp() -> usize {
    ...
    *DISP.get_or_init(|| {
        ...
        LINUX_JIT_THREAD_MIRROR.with(|cell| {
            // SAFETY: `fs:[0]` is the ABI-guaranteed self-pointer on x86_64 Linux TCB.
            let fs_base = unsafe { read_fs_qword(0) };
            let cell_addr = cell as *const std::cell::Cell<usize> as usize;
            let delta = (cell_addr as i128) - (fs_base as i128);
            let Ok(delta32) = i32::try_from(delta) else {
            ...
            // SAFETY: reads back the sentinel value just written to verify fs:[delta32] maps to cell.
            let probed = unsafe { read_fs_qword(delta32 as isize) };
            cell.set(old);
...
pub fn jit_thread_mirror_read() -> Option<usize> {
    ...
    #[cfg(all(target_os = "linux", target_arch = "x86_64"))]
    {
        // SAFETY: `disp` came from the startup probe, which proved `fs:[disp]` is this thread's JIT-thread mirror slot.
...
pub fn inline_cm_tls_disp() -> usize {
    ...
    *DISP.get_or_init(|| {
        ...
        let disp = LINUX_INLINE_CM_MIRROR.with(|cell| {
            // SAFETY: identical to `inline_rbp_tls_disp`'s Linux
...
pub fn scratch_flush_at_safepoint_enabled() -> bool {
    cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_MY_SCRATCH_FLUSH")
}
...
pub fn shadow_emission_moving_implication_enabled() -> bool {
    cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_MY_SHADOW_EMISSION")
}
...
pub fn self_call_moving_proof_enabled() -> bool {
    ...
    cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_MY_SELFCALL_PROOF")
}
...
pub(super) fn shadow2_diag_enabled(method_label: &str) -> bool {
    if !cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_SHADOW2") {
        return false;
    }
...
pub(super) fn shadow_reload_dbg() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_SHADOW_RELOAD"))
}
...
pub(super) fn shadow_nopush() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_SHADOW_NOPUSH"))
}
...
pub(super) fn shadow_noreload() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_SHADOW_NORELOAD"))
}
...
pub(super) fn shadow_pin_codegen() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_SHADOW_PIN"))
}
...
pub(super) fn shadow_sentinel() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_SHADOW_SENTINEL"))
}
...
pub fn inline_getstatic_enabled() -> bool {
    ...
    !cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_GETSTATIC_HELPER")
}
...
pub(super) fn shadow_reload_raw() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_SHADOW_RAW_RELOAD"))
}
...
pub(super) fn self_tailcall_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_SELF_TAILCALL"))
}
...
pub(super) fn shadow_overflow_diag() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_SHADOW_OVERFLOW_DIAG"))
}
...
pub(super) fn shadow_no_savebase() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_SHADOW_NO_SAVEBASE"))
}
...
pub(super) fn precise_reg_spill_disabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_NO_PRECISE_REG_SPILL"))
}
...
pub(super) fn full_self_call_spill_requested() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_FULL_SELF_CALL_SPILL"))
}
...
pub(super) fn alloc_spill_sink_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| !cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_NO_ALLOC_SPILL_SINK"))
}
...
pub(super) fn flush_callee_saved_oops_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| !cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_NO_CALLEE_OOP_FLUSH"))
}
...
pub fn kernel_reg_locals_enabled() -> bool {
    ...
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| {
        cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_KERNEL_REG_LOCALS")
    })
}
...
pub(super) fn kernel_reg_osr_enabled() -> bool {
    use std::sync::OnceLock;
    static G: OnceLock<bool> = OnceLock::new();
    *G.get_or_init(|| cratonvm_types::flags::runtime_flag_default_on("CRATONVM_JIT_KERNEL_REG_OSR"))
}
...
fn compute_local_oop_masks_window(
    ...
    //
    // The bound is a real one, not a guess: a pc is pushed once when first
    // reached and again only when its mask LOSES a bit, which a 64-bit mask
    // can do at most 64 times. So there are at most `65 * code_len + 1`
    // pushes, and a budget of `66 * code_len + 64` pops can only run out on a
    // decode that is not making progress. It used to be `64 * code_len + 64`,
    // which a long method whose loops shed their bits one merge at a time
    // could genuinely exhaust.
    //
    // And running out must REFUSE, not `break`. This is a must-analysis
    // descending from TOP: stopped short of its fixpoint, a mask still carries
    // bits a later merge would have cleared, i.e. it names as an oop a slot
    // that holds an int on some path — and a moving collector rewrites that
    // "oop". The `break` handed the partial fixpoint to the oop maps. An empty
    // result is this function's documented refusal
    // (`compute_local_oop_masks_windowed`, "An empty result is a REFUSAL"),
    // and every caller already falls back to the conservative scan on it.
    let mut guard = code_len.saturating_mul(66).saturating_add(64);
...
//     `goto_w`: the emitter does poll a backward `goto_w`, but a method
//     containing one is refused outright (`OpaqueControlFlow`, below), so a
//     widened rewrite would refuse its own output.
//
...
// (`0xaa`/`0xab`), `ifnull`/`ifnonnull` (`0xc6`/`0xc7`) and `goto_w` (`0xc8`),
// if any decoded target is `<= pc` it emits [`emit_safepoint_poll`] *before*
// the compare and branch, so the poll runs whether or not the branch is taken.
// [`poll_bearing_opcode`] is that opcode set, transcribed. `goto_w` joined it
// in round 9: `jit_scan` admits it and `walk_control`'s `0xc8` arm polls a
// backward one (`x64/op_control.rs`). `jsr`/`jsr_w`/`ret` stay absent — the
// emitter has no poll for them, and a rewriter that introduced one would
// introduce an unpolled backward branch.
//
...
// The check is not vacuous — it fails on a backward `jsr_w`, on `jsr`/`ret`,
...
/// `if_acmpeq/ne`, `goto`, `tableswitch`, `lookupswitch`, `ifnull`,
/// `ifnonnull` and `goto_w` arms (`goto_w`: `walk_control`'s `0xc8` arm in
/// `x64/op_control.rs`, admitted by `jit_scan` since round 9).
/// `jsr_w`/`jsr`/`ret` are deliberately absent: the emitter has no poll for
/// them, so a backward one would be an unpolled cycle.
#[allow(dead_code)]
pub(super) fn poll_bearing_opcode(op: u8) -> bool {
    matches!(op, 0x99..=0xa7 | 0xaa | 0xab | 0xc6 | 0xc7 | 0xc8)
}
...
/// and on a backward branch at an opcode the emitter does not poll.
#[allow(dead_code)]
...
pub(crate) enum LoopXformRefusal {
    ...
    /// refuse rather than widen to `goto_w`, which this rewriter refuses as
    /// input (`OpaqueControlFlow`).
    OffsetOverflow,
...
impl LoopXform {
    ...
    /// [`Self::outputs_for_bci`] for every bci at once: `v[bci]` lists, in
    /// ascending order, the output PCs that are images of `bci`, with the
    /// versioning guard's bytes skipped exactly as `outputs_for_bci` skips
    /// them. One pass over `bci_of`.
    fn images_by_bci(&self) -> Vec<Vec<usize>> {
        let guard = self.guard_span();
        let mut v: Vec<Vec<usize>> = vec![Vec::new(); self.orig_code_len];
        for (pc, &b) in self.bci_of.iter().enumerate() {
            if let Some((from, to)) = guard {
                if pc >= from && pc < to {
                    continue;
            ...
            // Widening: u32 -> usize
            if let Some(slot) = v.get_mut(b as usize) {
                slot.push(pc);
            }
        }
        v
    ...
    ///
    /// Cost: one pass over the output to build the inverse index, then one
    /// lookup per entry. It used to call [`Self::outputs_for_bci`] — itself a
    /// full scan of `bci_of` — once per ENTRY, i.e. O(entries x code_len) per
    /// table and 21 tables per compile. The answer is the same list in the
    /// same order.
    pub(super) fn replicate_pc_keyed<T: Clone>(&self, table: &[(usize, T)]) -> Vec<(usize, T)> {
        let images = self.images_by_bci();
        let mut out: Vec<(usize, T)> = Vec::with_capacity(table.len());
        for (pc, payload) in table {
            let Some(list) = images.get(*pc) else {
                continue;
            };
            for &image in list {
...
fn rewrite_loop_copies(
    ...
    for &at in cfg.nodes() {
        ...
        // not. The emitter polls a backward `goto_w` (round 9), but the
        // rewriter's offset arithmetic only knows 2-byte branches, so a
        // method containing one is still refused whole rather than reasoned
        // about.
...
mod loop_xform_tests {
    ...
    /// A one-instruction loop whose header is reached by a `goto_w`, which
    /// the rewriter refuses as input. Header 0, back edge 5.
    ...
    #[test]
    fn a_backward_goto_w_is_polled_but_still_refused_by_the_rewriter() {
        // Since round 9 `jit_scan` admits `goto_w` and `walk_control` polls a
        // backward one, so the model counts it as polled.
        ...
        // The model is still not vacuous: a backward `jsr_w` (never polled)
        // is refused.
        let jsr_w_spin = vec![0xc9, 0x00, 0x00, 0x00, 0x00, 0xb1];
        assert!(!all_backward_edges_are_polled(
            &jsr_w_spin,
            jsr_w_spin.len()
        ));
        assert!(!poll_bearing_opcode(0xc9));
    ...
    /// The one-pass inverse index `replicate_pc_keyed` now builds answers
    /// EXACTLY what the per-entry `outputs_for_bci` scan it replaced answered:
    /// the same images, in the same order, for every bci including ones past
    /// the method's end (which have none).
    #[test]
    fn the_inverse_index_is_outputs_for_bci_for_every_bci() {
        for (name, code, header, back_edge) in admissible_fixtures() {
            let len = code.len();
            for k in 1..=LOOP_XFORM_MAX_COPIES {
                for planned in [
                    ...
                    let Ok(x) = planned else { continue };
                    let index = x.images_by_bci();
                    for bci in 0..len + 3 {
                        let want = x.outputs_for_bci(bci);
                        let got = index.get(bci).cloned().unwrap_or_default();
                        assert_eq!(got, want, "{name} k={k}: bci {bci}");
                    }
                }
            }
        }
...
#[cfg(test)]
mod branch_target_shape_tests {
    use super::*;

    /// A `switch` in the loop body contributes "everything from here to the end
    /// of the body may be entered". That is a claim about a RANGE, and it used
    /// to be materialised one pc at a time into a `Vec` that every `contains`
    /// in the per-pc walk then scanned linearly — so a 4 KB body with one
    /// switch produced a ~4000-element vector and made the walk O(body²), per
    /// loop, times the number of loops.
    ///
    /// The answers must be identical; only the representation changed.
    #[test]
    fn a_switch_covers_its_span_without_materialising_every_pc() {
        // 0: nop
        // 1: tableswitch — two padding bytes take the payload to offset 4,
        //    then default / low / high / one jump offset, four bytes each. The
        //    instruction spans [1, 20) and a trailing nop sits at 20.
        ...
        code.push(0x00); // trailing nop
        let end = code.len();

        let targets = branch_target_pcs(&code, 0, end);
        assert!(
            targets.covered.iter().any(|&(lo, hi)| lo == 1 && hi == end),
            "the switch must cover [1, {end}), got {:?}",
            targets.covered
        );
        assert!(
            targets.pcs.is_empty(),
            "and must not have materialised the span as individual targets"
        );
        // The observable contract, unchanged: everything from the switch on is
        // treated as enterable, and everything before it is not.
        assert!(!targets.contains(0));
        for pc in 1..end {
            assert!(targets.contains(pc), "pc {pc} is inside the covered span");
        }
    }

    /// The exact targets still land in the set, and nothing else does.
    #[test]
    fn an_ordinary_branch_records_exactly_its_target() {
        // 0: nop ; 1: goto +4 -> 5 ; 4: nop ; 5: nop
        let code = vec![0x00, 0xa7, 0x00, 0x04, 0x00, 0x00];
        let targets = branch_target_pcs(&code, 0, code.len());
        assert!(targets.contains(5));
        assert!(!targets.contains(0));
        assert!(!targets.contains(4));
        assert!(targets.covered.is_empty(), "no switch, no covered span");
    }
}

#[cfg(test)]
mod rotated_array_len_hoist_tests {
    use super::*;

    /// ecj/kotlinc-style ROTATED `for (i = 0; i < a.length; i++) s += a[i]`:
    /// `goto COND; BODY (header 7); COND (16): iload i; aload a; arraylength;
    /// if_icmplt BODY`.
    ...
    fn rotation() -> FxHashMap<usize, usize> {
        let mut m = FxHashMap::default();
        m.insert(7usize, 4usize);
        m
    }

    #[test]
    fn the_cond_site_opens_a_hoist_only_when_the_loop_is_relocated() {
        let loops = vec![(7usize, 19usize)];
        // Unrelocated: COND is not on the HEADER's straight-line prefix (the
        // body's `iaload` is in the way), so nothing is hoisted.
        assert!(find_array_len_hoists(&ROTATED_SUM, ROTATED_SUM.len(), &loops).is_empty());
        let hoists = find_array_len_hoists_with_rotation(
            &ROTATED_SUM,
            ROTATED_SUM.len(),
            &loops,
            &rotation(),
        );
        assert_eq!(hoists.len(), 1, "{hoists:?}");
        assert_eq!(hoists[0].loop_header, 7);
        assert_eq!(hoists[0].array_local, 0);
        assert_eq!(hoists[0].sites, vec![(17, 19)]);
        assert_eq!(hoists[0].trap_bci, 18, "the COND arraylength traps first");
    }

    #[test]
    fn a_body_only_site_never_opens_a_rotated_hoist() {
        // `goto COND; BODY: aload_0; arraylength; istore_1; iinc 2 1;
        //  COND: iload_2; iload_3; if_icmplt BODY; return` -- a zero-trip
        // rotated loop never evaluates the body's `a.length`.
        let code: [u8; 18] = [
            0xa7, 0x00, 0x09, // 0: goto +9 -> 9
            0x2a, // 3: aload_0          ; header
            0xbe, // 4: arraylength
            0x3c, // 5: istore_1
        ...
        let loops = vec![(3usize, 11usize)];
        let mut rotation = FxHashMap::default();
        rotation.insert(3usize, 0usize);
        assert!(
            find_array_len_hoists_with_rotation(&code, 15, &loops, &rotation).is_empty(),
            "the body site is on the HEADER's prefix, which a rotated loop may skip"
        );
        // The same site IS hoistable for a top-tested reading of the header,
        // which is what the unrelocated finder answers (the driver then drops
        // it at a bypassable header).
        let top = find_array_len_hoists(&code, 15, &loops);
        assert_eq!(top.len(), 1);
        assert_eq!(top[0].trap_bci, 4);
    }

    #[test]
    fn a_top_tested_loop_is_unchanged_by_an_empty_rotation_map() {
        // `iload_1; aload_0; arraylength; if_icmpge +10; iinc 1 1; goto -9`
        let code: [u8; 14] = [
            0x1b, // 0: iload_1   ; header
            0x2a, // 1: aload_0
            0xbe, // 2: arraylength
            0xa2, 0x00, 0x09, // 3: if_icmpge +9 -> 12
            0x84, 0x01, 0x01, // 6: iinc 1, 1
            0xa7, 0xff, 0xf7, // 9: goto -9 -> 0
            0xb1, // 12: return
            0x00,
        ];
        let loops = vec![(0usize, 9usize)];
        let plain = find_array_len_hoists(&code, 13, &loops);
        let with = find_array_len_hoists_with_rotation(&code, 13, &loops, &FxHashMap::default());
        assert_eq!(plain.len(), 1);
        assert_eq!(with.len(), 1);
        assert_eq!(plain[0].sites, with[0].sites);
        assert_eq!(plain[0].trap_bci, 2);
        assert_eq!(with[0].trap_bci, 2);
    }
}

#[cfg(test)]
mod osr_entry_attribution_tests {
    use super::super::{detect_loops, find_arith_loop_hoists, find_loop_hoists};
    use super::*;

    #[test]
    fn osr_entries_are_the_backward_branch_targets_and_only_inner_ones_count() {
        // `test_licm_nested_loops`' shape: outer header 0, inner header 6.
        let code: Vec<u8> = vec![
            0x15, 0x05, 0x1c, 0xa2, 0x00, 0x17, 0x15, 0x07, 0x1c, 0xa2, 0x00, 0x0a, 0x2a, 0x15,
            0x04, 0x32, 0x57, 0x84, 0x07, 0x01, 0xa7, 0xff, 0xf2, 0x84, 0x05, 0x01, 0xa7, 0xff,
            0xe6, 0, 0,
        ];
        let mut pcs = osr_entry_pcs(&code, 29);
        pcs.sort_unstable();
        assert_eq!(pcs, vec![0, 6]);
        assert!(
            span_holds_inner_osr_entry(&pcs, 0, 29),
            "the inner header is inside the outer loop"
        );
        assert!(
            !span_holds_inner_osr_entry(&pcs, 6, 23),
            "a loop's own header is not an inner entry"
        );
    }

    /// The `aaload` hoist of an invariant `a[i]` inside a nested loop goes to
    /// the INNER loop: on the outer one the walk would publish the inner header
    /// OSR-ineligible, which measured 28.9 s against 0.23 s for a method that
    /// runs a long inner loop once (`OsrHoistCliff.aaload`).
    #[test]
    fn a_nested_aaload_hoist_is_attached_to_the_inner_loop() {
        let code: Vec<u8> = vec![
            0x15, 0x05, // 0: iload 5 (j)      outer header
            0x1c, // 2: iload_2 (n)
            0xa2, 0x00, 0x17, // 3: if_icmpge -> 26
            0x15, 0x07, // 6: iload 7 (k)      inner header
            0x1c, // 8: iload_2
            0xa2, 0x00, 0x0a, // 9: if_icmpge -> 19
            0x2a, // 12: aload_0           invariant a[i]
            0x15, 0x04, // 13: iload 4
            0x32, // 15: aaload
            0x57, // 16: pop
            0x84, 0x07, 0x01, // 17: iinc 7, 1
            0xa7, 0xff, 0xf2, // 20: goto -> 6
            0x84, 0x05, 0x01, // 23: iinc 5, 1
            0xa7, 0xff, 0xe6, // 26: goto -> 0
            0, 0,
        ];
        let loops = detect_loops(&code, 29);
        assert_eq!(loops.len(), 2, "{loops:?}");
        let hoists = find_loop_hoists(&code, 29, &loops);
        assert_eq!(hoists.len(), 1);
        assert_eq!(
            hoists[0].loop_header, 6,
    ...
    /// The same for an invariant arithmetic run: `s += base * 3 + 1` in the
    /// inner loop is hoisted to the inner header (`OsrHoistCliff.arith`, 25.2 s
    /// against 0.20 s with the outer attribution).
    #[test]
    fn a_nested_arith_hoist_is_attached_to_the_inner_loop() {
        // locals: 0=base 1=n 2=m 3=s 4=i 5=j
        let code: Vec<u8> = vec![
            0x15, 0x04, // 0: iload 4 (i)      outer header
            0x1b, // 2: iload_1
            0xa2, 0x00, 0x20, // 3: if_icmpge -> 35
            0x03, // 6: iconst_0
            0x36, 0x05, // 7: istore 5
            0x15, 0x05, // 9: iload 5 (j)      inner header
            0x1c, // 11: iload_2
            0xa2, 0x00, 0x11, // 12: if_icmpge -> 29
            0x1d, // 15: iload_3
            0x1a, // 16: iload_0           run start
            0x06, // 17: iconst_3
            0x68, // 18: imul
            0x04, // 19: iconst_1
            0x60, // 20: iadd              run end (21)
            0x60, // 21: iadd
            0x3e, // 22: istore_3
            0x84, 0x05, 0x01, // 23: iinc 5, 1
            0xa7, 0xff, 0xef, // 26: goto -> 9
            0x84, 0x04, 0x01, // 29: iinc 4, 1
            0xa7, 0xff, 0xe0, // 32: goto -> 0
            0xb1, // 35: return
        ];
        let loops = detect_loops(&code, code.len());
        assert_eq!(loops.len(), 2, "{loops:?}");
        let hoists = find_arith_loop_hoists(&code, code.len(), &loops);
        assert_eq!(hoists.len(), 1, "one run");
        assert_eq!(
            hoists[0].loop_header, 9,
            "the inner loop, not the outer one"
        );
        assert_eq!((hoists[0].seq_start, hoists[0].seq_end), (16, 21));
    }

    /// An outer loop's `i < rows.length` stays in the body when an inner loop
    /// sits inside it (`OsrHoistCliff.len`, 18.2 s against 0.21 s hoisted);
    /// with the inner back edge `nop`ed out the same site is hoisted.
    #[test]
    fn an_outer_arraylength_is_not_hoisted_over_an_inner_loop() {
        // locals: 0=rows 1=m 2=s 3=i 4=j
        let nested: Vec<u8> = vec![
            0x1d, // 0: iload_3            outer header
            0x2a, // 1: aload_0
            0xbe, // 2: arraylength
            0xa2, 0x00, 0x1b, // 3: if_icmpge -> 30
            0x03, // 6: iconst_0
            0x36, 0x04, // 7: istore 4
            0x15, 0x04, // 9: iload 4          inner header
            0x1b, // 11: iload_1
            0xa2, 0x00, 0x0c, // 12: if_icmpge -> 24
            0x84, 0x02, 0x01, // 15: iinc 2, 1
            0x84, 0x04, 0x01, // 18: iinc 4, 1
            0xa7, 0xff, 0xf4, // 21: goto -> 9
            0x84, 0x03, 0x01, // 24: iinc 3, 1
            0xa7, 0xff, 0xe5, // 27: goto -> 0
            0xb1, // 30: return
        ];
        let loops = detect_loops(&nested, nested.len());
        assert_eq!(loops.len(), 2, "{loops:?}");
        assert!(
            find_array_len_hoists(&nested, nested.len(), &loops).is_empty(),
            "the outer header's arraylength must stay in the body"
        );
        ...
        flat[22] = 0x00;
        flat[23] = 0x00;
        let flat_loops = detect_loops(&flat, flat.len());
        assert_eq!(flat_loops, vec![(0, 27)]);
        let hoists = find_array_len_hoists(&flat, flat.len(), &flat_loops);
        assert_eq!(hoists.len(), 1, "{hoists:?}");
        assert_eq!(hoists[0].loop_header, 0);
        assert_eq!(hoists[0].sites, vec![(1, 3)]);
    }
}
...
/// Test-build twin of the production `g1_inline_barrier_forced` (see
/// `g1_inline_barrier_enabled`): the override above, when a test set one.
#[cfg(test)]
fn g1_inline_barrier_forced() -> Option<bool> {
    G1_INLINE_BARRIER_FORCED.with(|c| c.get())
}
