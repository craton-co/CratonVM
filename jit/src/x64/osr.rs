...
impl Compiler {
    ...
    pub(super) fn emit_osr_exit_map_at_reason(
        ...
        // The point is recorded either way; only the bare-bci key is withheld
        // inside a splice, so a reason-7/8 stub then finds no entry and takes the
        // shared exit. Both callers (`bytecode_walk`'s loop header and
        // `op_invoke`'s `invokedynamic` trap) are on the OUTER walk, so the
        // refusal arm is unreachable today — see `Compiler::may_file_by_bci` for
        // why a key space with no owner component gets a guard rather than a
        // widened key.
        if self.may_file_by_bci("osr_exit_box_ptr_by_bci", bci) {
    ...
    if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_OSR_SEED_COLLISION") {
        ...
        for &(pc, live_in) in osr_block_live_in {
            ...
            for c in seed_collisions_at(m, num_locals, &osr_local_assignments, &osr_xmm_assignments)
            {
                ...
                hits += 1;
                eprintln!(
                    "[osr-seed-collision] {method_label} entry_pc={pc} {} {} holds LIVE local {} \
                     then OVERWRITTEN by local {} (live_in={live_in:#x})",
            ...
            for i in 0..num_locals.min(64) {
                ...
                if let Some(home) = body_home {
                    if seeded.is_none() {
                        stripped_live += 1;
                        eprintln!(
                            "[osr-seed-stripped] {method_label} entry_pc={pc} LIVE local {i} \
                             reads r{home} in the body but the trampoline seeds no register for it"
                        );
                    }
                }
            }
        ...
        eprintln!(
            "[osr-seed-scan] {method_label} takeable_entries={takeable} \
             live_collisions={hits} stripped_live={stripped_live}"
        );
    }
    // ── The seed-collision invariant ─────────────────────────────────
    //
