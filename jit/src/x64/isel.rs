...
//! (`mov_r64_imm0_xor`, `mov_r32_imm32_zx`, `mov_r64_imm32`, `mov_r64_imm64`) rather than as `if`
//! statements, the selector reproduces `emit_mov_imm64`'s four-way shrink by
...
//!
//! The single-pass emitters in `x64.rs` do not call this module; migrating
//! them onto it is a separate change (see `docs/jit/instruction-patterns.md`
//! for the migration order). The IR tier does: `ir_lower::build_mir_plan`
//! runs [`select_block`], and under `CRATONVM_JIT=ir-isel-emit` its
//! `Rule::AluReg` tiles are encoded through [`select`] and are the emitted
//! bytes (`ir-isel-verify` compares them against the per-opcode arm instead).
//! A wrong row here is therefore wrong code behind that flag, not only a
//! failing test.
//!
//! # Displacements
//!
...
//! test code. Every fallible operation returns [`SelError`].
//!
//! (Do not spell the test-gate attribute literally in prose above production
//! code: `jit/tests/panic_free_compile_ratchet.rs` stops scanning a file at
//! its first occurrence, and this sentence used to end the scan of this whole
//! file at line 71.)
...
pub enum ImmForm {
    ...
    Imm32,
    /// Four bytes, little-endian, ZERO-extended by the instruction (`MOV r32,
    /// imm32` writing a 32-bit register). The value must fit `u32`.
    ImmU32,
...
impl ImmForm {
    ...
    pub fn byte_len(self) -> usize {
        match self {
            ...
            ImmForm::Imm32 | ImmForm::ImmU32 => 4,
            ImmForm::Imm64 => 8,
        }
    }
}
...
impl Pattern {
    ...
    pub fn check(&self, a: &Args) -> Result<(), SelError> {
        ...
        if self.is_memory() {
            ...
            if let Some(ix) = a.mem.index {
                ...
                scale_bits(ix.scale)?;
                // Structural, not per-row: SIB `index=100` with REX.X clear
                // means "no index", so an RSP index would silently become
                // `[base + disp]` on EVERY memory row, not only the ones that
                // remembered to declare `Constraint::IndexNotRsp` (the two
                // `LEA` rows, which `IrAddr` lowering feeds, did not). Reported
                // as that constraint so a row that does declare it answers
                // exactly as before.
                if ix.reg == RSP {
    ...
    pub fn encode(&self, a: &Args) -> Result<Encoded, SelError> {
        ...
        match self.imm {
            ...
            ImmForm::ImmU32 => {
                let v = u32::try_from(a.imm).map_err(|_| SelError::Immediate {
                    pattern: self.name,
                    form: ImmForm::ImmU32,
                    value: a.imm,
                })?;
...
pub static PATTERNS: &[Pattern] = &[
    ...
    // `MOV r32, imm32` — writing the 32-bit register zero-extends, so for any
    // value in `0..=u32::MAX` the 64-bit result is exact. 5 bytes (6 with
    // REX.B). `select` prefers it over `mov_r64_imm32` (7) for a positive
    // value and over `mov_r64_imm64` (10) for the `u32` range; zero stays
    // `XOR` (2). The `ImmU32` field refuses a negative or wider value.
    Pattern {
        name: "mov_r32_imm32_zx",
        emitter: "emit_mov_r32_imm32_zx",
        src: OpKind::Imm,
        enc: Enc {
            rex_w: false,
            rex: RexMode::OnDemand,
    ...
    // The wide sibling of `cmp_r32_imm8`. Without it a `CMP r32, imm32` had no
    // row at all: `select` answered `Err(Constraint(ImmFitsI8))` and
    // `MInst::CmpRI { ty: I32, form: Imm32 }` mapped to no name, so
    // `require_encodable` DISCARDED every compare against a constant outside
    // `-128..=127` — `if (x > 1000)` is the shape, and it is not rare.
    Pattern {
        name: "cmp_r32_imm32",
        emitter: "x64/simd.rs emit_strip_clamp `[0x41, 0x81, 0xF8] + imm32`",
        op: Op::Cmp,
        ty: Ty::I32,
        src: OpKind::Imm,
        enc: Enc {
    ...
    // The `Int` bitwise forms. `ir_lower` emits these (not the REX.W rows
    // below) for an `IrType::Int` node, to keep the int home word the
    // zero-extension of its low half; see `alu_op_of`.
    Pattern {
        name: "and_r32_r32",
        emitter: "ir_lower.rs inline `[0x21, 0xC8]` (Op::And, Int)",
        op: Op::And,
        ty: Ty::I32,
        enc: Enc {
    ...
    Pattern {
        name: "or_r32_r32",
        emitter: "ir_lower.rs inline `[0x09, 0xC8]` (Op::Or, Int)",
        op: Op::Or,
        ty: Ty::I32,
        enc: Enc {
    ...
    Pattern {
        name: "xor_r32_r32",
        emitter: "ir_lower.rs inline `[0x31, 0xC8]` (Op::Xor, Int)",
        op: Op::Xor,
        ty: Ty::I32,
        enc: Enc {
            opcode: Opcode::One(0x31),
    ...
    Pattern {
        name: "and_r64_r64",
        emitter: "ir_lower.rs inline `[0x48, 0x21, 0xC8]` (Op::And, Long)",
        op: Op::And,
        enc: Enc {
            rex_w: true,
            rex: RexMode::Always,
    ...
    // ── 64-bit compare against a constant ────────────────────────────────
    //
    // `Op::Cmp` at `Ty::I64` had NO immediate row of either width, so
    // `MInst::CmpRI { ty: Ty::I64, .. }` named nothing and every `long` or
    // reference comparison against a non-zero constant was discarded by
    // `require_encodable` — which is also what pinned `CmpRI`/`I64`/`Imm8` in
    // `the_rows_the_selector_still_lacks_are_exactly_these`. Both widths are
    // anchored to byte literals this backend already emits.
    Pattern {
        name: "cmp_r64_imm8",
        emitter: "x64/arith.rs emit_fp_to_int_nan_fixup `[0x48, 0x83, 0xF8, 0x01]`",
        op: Op::Cmp,
        src: OpKind::Imm,
        enc: Enc {
            rex_w: true,
    ...
    Pattern {
        name: "cmp_r64_imm32",
        emitter: "x64/objects.rs ZGC TLAB guard `[0x49, 0x81, 0xF8] + imm32`",
        op: Op::Cmp,
        src: OpKind::Imm,
        enc: Enc {
            rex_w: true,
...
// # Wiring
//
// Two callers, both off by default: `shadow_select_method` (counts only,
// `CRATONVM_JIT_IR_ISEL_SHADOW`) and `ir_lower::build_mir_plan`, whose
// `Rule::AluReg` tiles become emitted bytes under `CRATONVM_JIT=ir-isel-emit`.
// See `docs/jit/instruction-selection.md` for what the rest of the wiring has
// to do and what is still unvalidated.
    ...
    pub fn pattern_name(&self) -> Option<&'static str> {
        match *self {
            MInst::Imm {
                ...
                0 => "mov_r64_imm0_xor",
                // The rows `select` picks (shortest wins): zero-extending for
                // `1..=u32::MAX`, sign-extending for the negative i32 range.
                v if u32::try_from(v).is_ok() => "mov_r32_imm32_zx",
            ...
            MInst::AluRR { op, ty, .. } => match (op, ty) {
                ...
                (Op::And, Ty::I32) => Some("and_r32_r32"),
                (Op::Or, Ty::I64) => Some("or_r64_r64"),
                (Op::Or, Ty::I32) => Some("or_r32_r32"),
                (Op::Xor, Ty::I64) => Some("xor_r64_r64"),
                (Op::Xor, Ty::I32) => Some("xor_r32_r32"),
            ...
            MInst::CmpRR { .. } => None,
            MInst::CmpRI { ty, form, .. } => match (ty, form) {
                (Ty::I32, ImmForm::Imm8) => Some("cmp_r32_imm8"),
                (Ty::I32, ImmForm::Imm32) => Some("cmp_r32_imm32"),
                (Ty::I64, ImmForm::Imm8) => Some("cmp_r64_imm8"),
                (Ty::I64, ImmForm::Imm32) => Some("cmp_r64_imm32"),
                _ => None,
            },
...
impl Tile {
    ...
    // `Tile::for_test` (the tests-only constructor) is defined in this file's
    // bottom test module, so no test gate sits above production code here.
...
/// perform it at.
///
/// Every op is performed at the node's own width, the bitwise three included.
/// They used to come back as [`Ty::I64`] for an `Int` node, on the reasoning
/// that "the low 32 bits of a 64-bit bitwise operation are the 32-bit result".
/// The low half is — the *high* half is not. `ir_lower`'s `Int` arms keep the
/// tier's convention that an int's home word is the zero-extension of its low
/// half, and a REX.W `AND`/`OR`/`XOR` over two operands that disagree about
/// that half (one sign-extended out of `iaload`/`F2I`/a negative `Const`, one
/// zero-extended from any 32-bit ALU result) produces a word that is neither
/// — `0xFFFF_FFFF_0000_0006` — which is an out-of-bounds element address that
/// passes a low-half bounds check. `ir_lower` fixed its own arms to the 32-bit
/// form for exactly that reason (see its `Op::And` comment); the selector kept
/// choosing `and_r64_r64`, so under `CRATONVM_JIT=ir-isel-emit` an `Int` `&`,
/// `|` or `^` tile re-introduced the bug, and under `ir-isel-verify` every one
/// of them was a byte mismatch that refused the compile. `OR r64, imm8` with a
/// negative immediate is the same defect with an immediate: `x | -1` set all
/// 64 bits of an `Int`.
...
mod tests {
    ...
    impl Tile {
        ...
        ///
        /// In production only the rules construct tiles, so a tile's cover list
        /// and its instructions always come from one place. A test that needs to
        /// hand a *deliberately inconsistent* tile to a consumer — `ir_lower::
        /// mir_tile_is_emittable`'s absorbed-node guard is the one that does —
        /// cannot obtain one from a rule by construction, which is exactly why
        /// that guard needs this.
        ///
        /// Defined here, not beside `Tile::generic`: a test gate above
        /// production code ends the panic-free ratchet's scan of the file.
    ...
    /// The `*_mem_disp32` emitters now agree with the table on the SIB bases
    /// too. They used to emit `mod=10, r/m=100` with no SIB byte, so an R12
    /// base (a LOCAL_REGS register) made the CPU read the first displacement
    /// byte as a SIB; the equivalence sweep above skips RSP/R12 because the
    /// emitters were wrong there. This is the half it skipped.
    #[test]
    fn disp32_memory_family_matches_the_table_on_sib_bases() {
        let mut r = Reference::new();
        for &base in [RSP, R12].iter() {
            for &dst in [RAX, RDX, R8, R11, R13].iter() {
                for disp in [0i32, 8, 128, -4096] {
                    let mem = Mem::base_disp32(base, i64::from(disp));
                    let rm =
                        |op: Op, ty: Ty| Req::new(op, ty, Operand::Gpr(dst), Operand::Mem(mem));
                    ...
                    assert_eq!(sel(&rm(Op::Mov, Ty::I64)), want, "MOV r64 [r{base}]");
                    // ModRM r/m=100, then the index-free SIB 0x24.
                    assert_eq!(want[3], 0x24, "SIB byte for r{base}");

                    let want = r.emit(|c: &mut super::super::Compiler| {
                        c.emit_movsxd_r64_mem_disp32(dst, base, disp)
                    });
                    assert_eq!(sel(&rm(Op::Movsxd, Ty::I32)), want, "MOVSXD [r{base}]");

                    let want = r.emit(|c: &mut super::super::Compiler| {
                        c.emit_mov_r32_mem_disp32(dst, base, disp)
                    });
                    assert_eq!(sel(&rm(Op::Mov, Ty::I32)), want, "MOV r32 [r{base}]");

                    let want = r.emit(|c: &mut super::super::Compiler| {
                        c.emit_lea_r64_mem_disp32(dst, base, disp)
                    });
                    assert_eq!(sel(&rm(Op::Lea, Ty::I64)), want, "LEA [r{base}]");

                    let want = r.emit(|c: &mut super::super::Compiler| {
                        c.emit_cmp_r64_mem_disp32(dst, base, disp)
                    });
                    assert_eq!(sel(&rm(Op::Cmp, Ty::I64)), want, "CMP [r{base}]");
                    ...
                    assert_eq!(got, want, "MOV [r{base}], r64");
                    ...
                    for (bits, signed, op, ty) in [
                        ...
                        assert_eq!(sel(&rm(op, ty)), want, "MOVX{bits} [r{base}]");
                    }
                }
                let want = r.emit(|c: &mut super::super::Compiler| {
                    c.emit_mov_dword_mem_disp32_imm32(base, 64, -1)
                });
                let got = sel(&Req::new(
                    Op::Mov,
                    Ty::I32,
                    Operand::Mem(Mem::base_disp32(base, 64)),
                    Operand::Imm(-1),
                ));
                assert_eq!(got, want, "MOV DWORD [r{base} + 64], -1");
            }
        }
    }
    ...
    fn indexed_byte_store_matches_emit_mov_mem8_indexed_imm8() {
        let mut r = Reference::new();
        // The emitter used to write a literal `mod=00` ModRM byte, which for
        // an RBP/R13 base means "no base, disp32 follows"; it now takes the
        // `mod=01` + `disp8(0)` form the row always produced, so every base is
        // in the sweep.
    ...
    fn immediate_size_selection_picks_the_smallest_legal_form() {
        // MOV r64, imm: zero -> XOR, 1..=u32::MAX -> zero-extending B8+rd
        // imm32, negative i32 -> C7, wider -> B8+rd imm64.
        assert_eq!(
        ...
        for v in [-1i64, -128, i64::from(i32::MIN)] {
            assert_eq!(
                select(&gpr_imm(Op::Mov, Ty::I64, RAX, v))
                    .expect("negative i32 range")
                    .pattern
                    .name,
                "mov_r64_imm32",
                "value {v}"
        ...
        for v in [i64::from(u32::MAX) + 1, i64::from(i32::MIN) - 1, i64::MAX] {
            assert_eq!(
                select(&gpr_imm(Op::Mov, Ty::I64, RAX, v))
                    .expect("wide")
                    .pattern
        ...
        assert!(!pattern("mov_r32_imm32_zx").expect("row").flags.writes);
    ...
    fn the_new_alu_rows_reproduce_the_ir_lower_byte_literals() {
        ...
        // The `Int` bitwise arms are 32-bit (no REX.W) — see `alu_op_of`.
        assert_eq!(sel(&gpr(Op::And, Ty::I32, RAX, RCX)), vec![0x21, 0xC8]);
        assert_eq!(sel(&gpr(Op::Or, Ty::I32, RAX, RCX)), vec![0x09, 0xC8]);
        assert_eq!(sel(&gpr(Op::Xor, Ty::I32, RAX, RCX)), vec![0x31, 0xC8]);
        assert_eq!(
    ...
    /// An `Int` `&` / `|` / `^` selects the 32-bit row, never the REX.W one.
    ///
    /// `alu_op_of` used to widen the three bitwise ops to `Ty::I64` for an
    /// `Int` node. `ir_lower` emits the 32-bit form for exactly those nodes so
    /// the int home word stays the zero-extension of its low half; the REX.W
    /// form over a sign-extended and a zero-extended operand produces a word
    /// that is neither (an out-of-bounds index that passes a low-half bounds
    /// check). Under `ir-isel-emit` that tile was the emitted code.
    ///
    /// The exact edit that trips it: put `Ty::I64` back in any of the three
    /// `alu_op_of` arms.
    #[test]
    fn an_int_bitwise_node_selects_the_32bit_form() {
        for (op, want) in [
            (IrOp::And, [0x21u8, 0xC8]),
            (IrOp::Or, [0x09, 0xC8]),
            (IrOp::Xor, [0x31, 0xC8]),
        ] {
            let mut graph = g();
            ...
            let n = graph.add(op.clone(), IrType::Int, vec![p, q], None);
            let block = vec![n];
            let s = select_block(&graph, &block, None, &SelectOptions::default());
            assert!(s.covers(&block), "coverage: {s:?}");
            let t = s.tiles.iter().find(|t| t.root == n).expect("root");
            let alu = t.insts.iter().find_map(|i| match *i {
                MInst::AluRR { ty, .. } => Some((i, ty)),
                _ => None,
            });
            let (inst, ty) = alu.unwrap_or_else(|| panic!("{op:?}: no AluRR in {t:?}"));
            assert_eq!(ty, Ty::I32, "{op:?} on an Int node must stay 32-bit");
            // Placeholder registers RAX/RCX: exactly `ir_lower`'s literal.
            let bytes = inst
                .probe()
                .unwrap_or_else(|e| panic!("{op:?}: {e:?}"))
                .bytes;
            assert_eq!(bytes, want.to_vec(), "{op:?}: no REX.W on an Int");
        }
        // And a Long node still takes the wide form.
        let mut graph = g();
        let p = param(&mut graph, 0);
        let q = param(&mut graph, 1);
        let n = bin(&mut graph, IrOp::And, p, q);
        let s = select_block(&graph, &[n], None, &SelectOptions::default());
        let t = s.tiles.iter().find(|t| t.root == n).expect("root");
        assert!(t
            .insts
            .iter()
            .any(|i| matches!(i, MInst::AluRR { ty: Ty::I64, .. })));
    }

    /// RSP is never a SIB index, on ANY memory row — including the two `LEA`
    /// rows that never declared `Constraint::IndexNotRsp` and would otherwise
    /// have encoded `[base + rsp*1]` as `[base]`. R12 *is* a legal index
    /// (REX.X distinguishes it), so it must still encode.
    #[test]
    fn an_rsp_index_is_refused_by_every_indexed_row() {
        for name in ["lea_r64_m", "lea_r32_m", "mov_m8_index_imm8"] {
            let p = pattern(name).unwrap_or_else(|| panic!("{name} missing"));
            let mut a = Args {
                dst: RAX,
                src: RCX,
                mem: Mem::base_index(RAX, RSP, 1, 0),
                imm: 1,
                op_byte: 0,
            };
            assert_eq!(
                ...
                "{name} must refuse an RSP index"
            );
            a.mem = Mem::base_index(RAX, R12, 1, 0);
            let e = p
                .encode(&a)
                .unwrap_or_else(|e| panic!("{name} must accept an R12 index: {e}"));
            let d = decode(&e.bytes, p.enc.modrm).expect("decodable");
            assert_eq!(d.index(), Some(R12), "{name}: R12 index round-trips");
        }
    }
    ...
    /// The three `CMP r, imm` rows this round added, each against the byte
    /// literal it is anchored to.
    ///
    /// The gap they close is not cosmetic: `MInst::CmpRI` named a row only at
    /// `(I32, Imm8)`, so under `SelectOptions::require_encodable` every compare
    /// against a constant outside `-128..=127`, and every `long`/reference
    /// compare against any non-zero constant, was DISCARDED — the tile fell
    /// back to `Rule::Generic` and the measurement read as "the rule never
    /// fires".
    ///
    /// The exact edit that trips it: change any `RegF::Ext(7)` on the three new
    /// rows. `/7` is what makes the shared `0x81`/`0x83` group mean `CMP`
    /// rather than `ADD` (`/0`) or `SUB` (`/5`) — a valid instruction that
    /// computes something else and then branches on it.
    #[test]
    fn the_new_cmp_immediate_rows_reproduce_their_byte_literals() {
        // `x64/simd.rs` emit_strip_clamp: `CMP R8D, MAX_BULK_BYTE_LOOP_SPAN`.
        assert_eq!(
            named(
                "cmp_r32_imm32",
                &Args {
            ...
            vec![0x41, 0x81, 0xF8, 0xA0, 0x86, 0x01, 0x00],
        );
        // `x64/arith.rs` emit_fp_to_int_nan_fixup: `CMP RAX, 1`.
        assert_eq!(
            named(
                "cmp_r64_imm8",
                &Args {
            ...
            vec![0x48, 0x83, 0xF8, 0x01],
        );
        // `x64/objects.rs` ZGC TLAB guard: `CMP R8, 512`.
        assert_eq!(
            named(
                "cmp_r64_imm32",
                &Args {
            ...
            vec![0x49, 0x81, 0xF8, 0x00, 0x02, 0x00, 0x00],
        );
        // The smallest form still wins where both apply, so no existing call
        // site's bytes move.
        assert_eq!(
            sel(&gpr_imm(Op::Cmp, Ty::I64, RAX, 1)),
            vec![0x48, 0x83, 0xF8, 0x01],
        );
        assert_eq!(
            sel(&gpr_imm(Op::Cmp, Ty::I32, RAX, 100_000)),
            vec![0x81, 0xF8, 0xA0, 0x86, 0x01, 0x00],
        );
        // And the tiles that name them really encode.
        for (ty, form, imm) in [
            (Ty::I32, ImmForm::Imm8, 7i64),
            (Ty::I32, ImmForm::Imm32, 100_000),
            (Ty::I64, ImmForm::Imm8, 1),
            (Ty::I64, ImmForm::Imm32, 512),
        ] {
            let inst = MInst::CmpRI {
                ty,
                lhs: 0,
                imm,
                form,
            ...
            let name = inst
                .pattern_name()
                .unwrap_or_else(|| panic!("{ty:?}/{form:?} has a row but no mapping"));
            assert!(pattern(name).is_some(), "`{name}` is not in PATTERNS");
            inst.probe()
                .unwrap_or_else(|e| panic!("{ty:?}/{form:?}: {e:?}"));
        }
    }
    ...
    fn the_rows_the_selector_still_lacks_are_exactly_these() {
        ...
        assert_eq!(
            missing.len(),
            3,
            "these three instruction shapes have no PATTERNS row; see \
             docs/jit/instruction-selection.md. `CmpRI` at `Ty::I64` used to be \
             the fourth and is now covered by `cmp_r64_imm8`/`cmp_r64_imm32`."
        );
