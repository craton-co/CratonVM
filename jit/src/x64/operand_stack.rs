...
/// Kill switch for the round-9 wave-5 operand folding in the single-pass
/// backend: `CRATONVM_JIT_NO_OPERAND_FOLD=1` restores the old shapes, in which
/// every ALU/compare/store operand is first copied into RAX/RCX and every
/// result is parked through RAX.
///
/// What it switches (all value-preserving rewrites of the same operand-stack
/// model; no new `StackSlot` kind, no new merge layout):
///
/// * a popped `Scratch` operand is computed on IN PLACE, and a result is
///   produced straight into a free scratch register
///   (`Compiler::pop_to_work_reg`, `Compiler::push_frame_word`,
///   `Compiler::push_imm32`);
/// * a register or frame operand is read by the ALU/CMP instruction itself
///   (`add r8d, r13d`, `cmp r12d, [rbp-x]`) instead of through RCX
///   (`Compiler::emit_gpr_binop`, `Compiler::alu_src_for` in `arith.rs`);
/// * `x = x op y;` with `x` register-homed becomes one `op home, y`, and a
///   store to a register-homed local moves from the operand's own register;
/// * a forward `if*`/`if_icmp*` whose operands are the whole stack no longer
///   flushes them to frame words first (`op_control.rs`).
///
/// Read on every call rather than latched in a `OnceLock`, because
/// `jit/tests/process_global_statics_ratchet.rs` admits no new `static`; once
/// the name is declared the read is one flag-snapshot lookup per folded op.
/// The switch is default OFF, i.e. the folding is ON. See
/// `perf-single-pass-scalar-loop-round-trips-every-operand-through-a-scratch-register-20260918.md`.
pub(super) fn operand_fold_enabled() -> bool {
    !cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_NO_OPERAND_FOLD")
}
...
impl Compiler {
    ...
    /// precise oop map entries (see `jit-safepoint-revert.md` and
    /// `precise-jit-stack-maps-design.md`). The desync was previously
    ...
    ///
    /// # A second record must AGREE with the first
    ///
    /// The first record wins (`or_insert`), and every consumer — the dead-code
    /// revival, the live-merge canonicalisation in the walk — trusts it to
    /// describe EVERY edge into `target_pc`. In verified bytecode it does: JVMS
    /// 4.10.1 gives a merge point one stack shape, and this model holds one
    /// entry per value, so every predecessor has the same entry count. A
    /// second edge arriving with a different depth therefore means the model
    /// and the bytecode have diverged, and whichever layout the target was
    /// emitted for is wrong for the other edge. That used to be silent; it is
    /// now a refusal, named so a census can find it.
    ///
    /// The marks vector is cloned only when this is the FIRST record for the
    /// target. The old `or_insert(marks)` built the clone unconditionally, so
    /// every re-branch to an already-recorded target (each case of a switch
    /// that shares a target, every `continue`) paid an allocation to discard.
    pub(super) fn record_branch_target_depth(&mut self, target_pc: usize) {
        let stack_len = self.stack.len();
        match self.branch_target_stack_depth.get(&target_pc).copied() {
            Some(recorded) if recorded != stack_len => {
                self.fail("singlepass-codegen/branch-target-depth-disagreement");
            }
            Some(_) => {}
            None => {
                self.branch_target_stack_depth.insert(target_pc, stack_len);
            }
        }
        if !self.branch_target_stack_oop_marks.contains_key(&target_pc) {
            let marks = self.stack_oop_marks.clone();
            self.branch_target_stack_oop_marks.insert(target_pc, marks);
        }
    }
    ...
    /// resolved.
    ///
    /// `insn_starts` is the walk's own `bytecode_analysis::instruction_starts`
    /// map (built once per compile in `compile_bytecode`). The two preceding
    /// boundaries are found by scanning it BACKWARD from `dup2_pc`, a few
    /// bytes, instead of re-walking the method from pc 0 at every call: this
    /// runs for every `pop2`/`dup2`/`dup_x2`/`dup2_x1`/`dup2_x2`, so the walk
    /// made the compile quadratic in the number of them (`NOTES-irfront2` §6
    /// left that open). The map IS that walk — same `step`, same origin — so
    /// the boundaries are identical. A map too short to cover `dup2_pc` (a
    /// caller without one) falls back to the linear walk.
    pub(super) fn dup2_top_cat2(
        &self,
        code: &[u8],
        dup2_pc: usize,
        insn_starts: &[bool],
        ...
        let (prev, prev2) = if dup2_pc < insn_starts.len() {
            if !insn_starts[dup2_pc] {
                return None; // dup2_pc is not on an instruction boundary
            }
            let before = |at: usize| (0..at).rev().find(|&p| insn_starts[p]);
            let prev = before(dup2_pc);
            (prev, prev.and_then(before))
        } else {
            let mut p = 0usize;
    ...
    /// `obj_ptr=0x1`, see real-raf-segv-root-cause.md).
        ...
        while !pending.is_empty() {
            ...
            match unblocked {
                ...
                None => {
                    ...
                    // Both lookups hold by construction (`walk` was reached
                    // through `pending`, and only a parked move has a `None`
                    // source, of which there is none yet). They bail rather
                    // than panic because this runs on the compiler thread
                    // inside the no-panic compile path, where the answer to a
                    // broken invariant is "stay interpreted".
                    let Some(entry) = pending.iter_mut().find(|(i, _)| *i == walk) else {
                        self.fail("singlepass-codegen/parallel-move-cycle-node-missing");
                        return;
                    };
                    let Some(slot) = entry.1.take() else {
                        self.fail("singlepass-codegen/parallel-move-cycle-node-parked");
                        return;
                    };
    ...
    pub(super) fn dbg_note_slot_load(&mut self, offset: i32) {
        if !crate::dbg_jit_slot_overlap() {
            return;
        }
    ...
    pub(super) fn push_from_rax(&mut self) {
        ...
        //
        // 2026-09-02, the eight-finding pass: re-examined as that pass's
        // finding 6, and the recommendation is AGAINST widening this. The
        // ARG_REGS collision is fixable — a dynamic pool of the callee-saved
        // registers `alloc_used_regs` did not hand out contains no argument
        // register, and callee-saved means a call preserves the value by the
        // ABI rather than by a flush the emitter has to remember. What changed
        // is the size of the prize: `slot_mirror` already elides the RELOAD
        // half of the round-trip (the step-7 row of
        // `known-issues/perf/array-element-load-baseline-codegen-20260901.md`),
        // so a widened cache still buys one STORE per push. Set that against
        // the constraint above — the adjacent change shipped a nondeterministic
        // heap corruption and was reverted the same day — and the expected
        // value is negative. Left as it is, deliberately.
        if let Some(reg) = self.free_scratch_reg(None) {
    ...
    /// The first `SCRATCH_REGS` register no operand-stack entry names, when
    /// the deferred operand cache is on for this compile: the register
    /// [`Self::push_from_rax`] would park a value in. `None` when the cache is
    /// off (every non-kernel method) or both registers are taken.
    ///
    /// `avoid` is excluded as well. A caller that writes its result register
    /// BEFORE it has read a popped operand names that operand's register
    /// here: the operand is off the model already, so its register reads as
    /// free, and choosing it would overwrite the value about to be read.
    pub(super) fn free_scratch_reg(&self, avoid: Option<u8>) -> Option<u8> {
        if !(self.kernel_operand_cache || operand_cache_enabled()) {
            return None;
        }
        SCRATCH_REGS.iter().copied().find(|&candidate| {
    ...
    /// Does no entry still on the operand stack name `reg`, as a scratch
    /// operand or as a callee-saved (local home) one?
    ///
    /// The question every in-place write asks first. A popped `Scratch(r)`
    /// operand may have its register overwritten with the result only when no
    /// other entry is a copy living in the same register; a register-homed
    /// local may be written directly only when no entry still reads its OLD
    /// value (what `invalidate_callee_saved` would otherwise spill).
    pub(super) fn reg_is_unshared(&self, reg: u8) -> bool {
        !self.stack.iter().any(
            |slot| matches!(slot, StackSlot::Scratch(r, ..) | StackSlot::CalleeSaved(r) if *r == reg),
        )
    }

    /// Pop the top operand into a register the caller may overwrite with its
    /// result, and return that register. Pair it with [`Self::push_work_reg`].
    ///
    /// A `Scratch` operand that no other entry names is used IN PLACE: its
    /// register already holds the value and this pop is what vacates it, so
    /// computing there removes the `mov rax, r8` ... `mov r8, rax` pair the
    /// cache otherwise wraps around every cached operand
    /// (`perf-single-pass-scalar-loop-round-trips-every-operand-through-a-scratch-register-20260918.md`).
    /// Any other operand is loaded into a free scratch register when the
    /// cache has one, else into RAX — the old `pop_to_rax` shape, byte for
    /// byte, which is what every non-kernel method still gets.
    ///
    /// Under `CRATONVM_JIT_NO_OPERAND_FOLD=1` this IS `pop_to_rax`.
    pub(super) fn pop_to_work_reg(&mut self) -> u8 {
        let slot = self.pop_stack();
        ...
        self.load_slot_to_reg(dst, slot);
        dst
    }

    /// Push the value in `reg`: RAX goes through [`Self::push_from_rax`]
    /// (a scratch register or a frame word), a scratch register becomes the
    /// entry's home directly with no move. Only ever handed a register
    /// [`Self::pop_to_work_reg`], [`Self::free_scratch_reg`] or RAX produced;
    /// anything else is copied through RAX rather than recorded as a
    /// `Scratch` home it is not.
    pub(super) fn push_work_reg(&mut self, reg: u8) {
        if reg != RAX && SCRATCH_REGS.contains(&reg) {
            self.stack_push(StackSlot::Scratch(reg), false);
            return;
        }
        self.emit_mov_reg_reg(RAX, reg); // nothing when `reg` is RAX
        self.push_from_rax();
    }

    /// Push the word at `[rbp - offset]` — a frame-homed local or a LICM
    /// hoist slot. With the operand cache on it is loaded straight into a
    /// free scratch register (`mov r8, [rbp-x]`) instead of into RAX and then
    /// copied (`mov rax, [rbp-x]; mov r8, rax`); otherwise the bytes are the
    /// old `emit_load_local(RAX, ..); push_from_rax()` ones exactly.
    ///
    /// The value is pushed unmarked; an `aload` caller marks it.
    pub(super) fn push_frame_word(&mut self, offset: i32) {
        let dst = if operand_fold_enabled() {
            self.free_scratch_reg(None).unwrap_or(RAX)
        } else {
            RAX
        };
        self.emit_load_local(dst, offset);
        self.push_work_reg(dst);
    }

    /// Push the int constant `val` (sign-extended to 64 bits), straight into
    /// a free scratch register when the operand cache has one.
    pub(super) fn push_imm32(&mut self, val: i32) {
        let dst = if operand_fold_enabled() {
            self.free_scratch_reg(None).unwrap_or(RAX)
        } else {
            RAX
        };
        self.emit_mov_imm32_sx(dst, val);
        self.push_work_reg(dst);
    }

    /// Pop the top operand into local `idx`'s home — the `istore`/`lstore`/
    /// `astore` (and non-XMM `fstore`/`dstore`) store itself.
    ///
    /// A register home is written from wherever the operand is
    /// (`mov r13, r8`, `mov r13, [rbp-x]`); a frame home from the operand's
    /// own register when it has one (`mov [rbp-x], r8`). The old shape copied
    /// every operand through RAX first. Under `CRATONVM_JIT_NO_OPERAND_FOLD=1`
    /// it still does.
    ///
    /// The caller has ALREADY run `invalidate_callee_saved` on a register
    /// home, before this pop: the spill word that reserves could otherwise be
    /// the word this pop releases, overwritten before it is read here.
    pub(super) fn pop_into_local(&mut self, idx: usize) {
        let slot = self.pop_stack();
        let home = self.reg_for_local(idx);
        if !operand_fold_enabled() {
            self.load_slot_to_reg(RAX, slot);
            match home {
                Some(reg) => self.emit_mov_reg_reg(reg, RAX),
                None => {
        ...
        match home {
            Some(reg) => self.load_slot_to_reg(reg, slot),
            None => {
                let off = self.local_offset(idx);
                match slot {
                    StackSlot::CalleeSaved(src) | StackSlot::Scratch(src, ..) => {
                        self.emit_store_local(off, src);
                    }
    ...
    /// The pc to continue the walk at after an arm that has just pushed an
    /// int it KNOWS is held sign-extended through all 64 bits (`next_pc` is
    /// that arm's own successor): `next_pc + 1` when the bytecode at
    /// `next_pc` is an `i2l` this consumes, else `next_pc`.
    ///
    /// `i2l` is `movsxd` of the low half, which on such a value rewrites the
    /// register with what it already holds. The callers are the int array
    /// loads (`movsxd`/`movsx`/`movzx` of the element: `op_array.rs`) and the
    /// int ALU binops (`op; movsxd`: `op_arith.rs` `walk_gpr_binop`), and the
    /// fact is about the code THEY just emitted, not a bytecode-order guess —
    /// an LICM or hoist replacement standing in for the load at its pc never
    /// reaches those arms, so never reaches here. The model is unchanged: an
    /// int and a long are both one 64-bit entry, unmarked.
    ///
    /// Only when nothing else can arrive at the `i2l`: it must not be a branch
    /// or handler target (an edge landing there brings its own int, whose
    /// upper half nothing here vouches for), and none of the per-pc work the
    /// walk does before dispatch (the debug deopt/OSR-exit triggers, the three
    /// LICM replacements) may be keyed to it. Its `pc_to_native` entry is the
    /// end of the producing instruction, as for the consumed store of
    /// `walk_gpr_binop`. Off under `CRATONVM_JIT_NO_OPERAND_FOLD=1`.
    ///
    /// Round 9 wave 7 (spstack7): fix 4 of
    /// `perf-single-pass-scalar-loop-round-trips-every-operand-through-a-scratch-register-20260918.md`.
    pub(super) fn skip_redundant_i2l(
        &mut self,
        code: &[u8],
        code_len: usize,
        next_pc: usize,
        ...
        const I2L: u8 = 0x85;
        if !operand_fold_enabled()
            || next_pc >= code_len.min(code.len())
            || code[next_pc] != I2L
            || branch_targets.get(next_pc).copied().unwrap_or(true)
            || self.deopt_eager_bci == Some(next_pc)
            || self.osr_exit_test_trigger_bci == Some(next_pc)
            || !matches!(
                self.stack.last(),
                Some(StackSlot::Frame(_) | StackSlot::CalleeSaved(_) | StackSlot::Scratch(..))
            )
            || self.hoist_info.iter().any(|h| h.seq_start == next_pc)
            || self.arith_hoist_info.iter().any(|h| h.seq_start == next_pc)
            || self
                .array_len_hoist_info
                .iter()
                .any(|h| h.sites.iter().any(|&(start, _)| start == next_pc))
        {
            return next_pc;
        }
        let native = self.buf.pos() as i32; // Cast: x86-64 immediate encoding
        if let Some(slot) = self.pc_to_native.get_mut(next_pc) {
            *slot = native;
        }
        next_pc + 1
    }

    /// The register the adjacent-reload elision (`slot_mirror`) says holds
    /// `[rbp - offset]` right now, if any — the same test `emit_load_local`
    /// applies before it substitutes a register move for a reload. For a
    /// consumer that can read a register OR memory operand, this is the
    /// register it can read instead of the frame word, with no move at all.
    pub(super) fn mirrored_reg(&self, offset: i32) -> Option<u8> {
        if self.slot_mirror_suppressed || !slot_mirror_enabled() {
            return None;
        }
        match self.slot_mirror {
            Some((moff, mreg, mpos)) if moff == offset && mpos == self.buf.pos() => Some(mreg),
            _ => None,
        }
    }
    ...
    /// [`Self::invalidate_callee_saved`] for the XMM file: materialise every
    /// operand-stack entry that still names `xmm` before the register is
    /// overwritten.
    ///
    /// `fload`/`dload` of an XMM-homed local is a zero-cost push — the entry is
    /// `StackSlot::Xmm(home)`, a promise that the value is still in the local's
    /// register. The GPR store arms keep the matching promise by calling
    /// `invalidate_callee_saved` before they write a register-homed local; the
    /// `fstore`/`dstore` arms wrote the XMM home with no such call. So
    /// `f + (f = 2.0f)` — `fload_1; fconst_2; dup; fstore_1; fadd` — read the
    /// NEW value through the stale `Xmm(home)` entry and computed `4.0` where
    /// JVMS §6.5 `fadd` over the pushed operands gives `old + 2.0`. XMM homes
    /// exist only where the ABI makes them callee-saved (Win64), so that is
    /// the platform the wrong answer shipped on.
    ///
    /// Same shape as the GPR helper: one shared spill word, every aliasing
    /// entry repointed at it, and a no-op (nothing emitted, nothing reserved)
    /// when no entry names the register.
    pub(super) fn invalidate_xmm_home(&mut self, xmm: u8) {
        ...
        // MOVQ [rbp - off], xmm — the full 64 bits, so a `double` survives.
        self.emit_movq_mem_rbp_from_xmm(off, xmm);
        for slot in &mut self.stack {
            if matches!(*slot, StackSlot::Xmm(x) if x == xmm) {
                *slot = StackSlot::Frame(off);
            }
        }
        // A scratch XMM that no entry names any more is free again; a local
        // home is not in `SCRATCH_XMMS`, and this is then a no-op.
        self.free_scratch_xmm(xmm);
    }
}
