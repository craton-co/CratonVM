...
impl Compiler {
    ...
    pub(super) fn emit_mov_imm64(&mut self, reg: u8, imm: i64) {
        ...
        // `i32::MAX < imm <= u32::MAX`: the zero-extending `MOV r32, imm32`
        // is exact (5-6 bytes instead of the 10-byte MOVABS).
        if let Ok(imm32) = u32::try_from(imm) {
            self.emit_mov_r32_imm32_zx(reg, imm32);
            return;
        }
        self.rex_w_b(reg);
    ...
    /// `MOV r32, imm32` — `[REX.B] B8+rd id`, 5 bytes (6 for R8-R15). Writing
    /// a 32-bit register zeroes bits 63:32, so the full 64-bit register holds
    /// `imm` zero-extended. Exact for any value in `0..=u32::MAX`; the shrinking
    /// emitters (`emit_mov_imm32_sx` for `imm > 0`, `emit_mov_imm64` for the
    /// `u32` range) use it instead of the 7-byte sign-extending
    /// `REX.W C7 /0 id` or the 10-byte MOVABS
    /// (`mov-imm-non-negative-uses-sign-extending-c7-form-20260918.md`).
    ///
    /// The imm32 is the instruction's LAST four bytes, as it is in the C7
    /// form, so a patcher that addresses it from the end of the instruction
    /// would still find it — but a value patched in later is zero-extended,
    /// not sign-extended. No current site patches a shrinking-form immediate
    /// (round 9 wave 2 audit: every `try_patch_i32` target is a rel32 branch,
    /// call or RIP-relative displacement, and the IC sites use
    /// `emit_mov_imm64_full`); a new one must use `emit_mov_imm64_full`.
    pub(super) fn emit_mov_r32_imm32_zx(&mut self, reg: u8, imm: u32) {
        if reg >= 8 {
            self.buf.emit_byte(0x41); // REX.B
        }
        self.buf.emit_byte(0xB8 + (reg & 7)); // MOV r32, imm32
        self.buf.emit(&imm.to_le_bytes());
    }
    ...
    /// MOV reg, imm32 (sign-extended to 64-bit). Uses XOR for zero and the
    /// zero-extending `MOV r32, imm32` for a positive value (identical 64-bit
    /// result, 1-2 bytes shorter); only a negative value takes the 7-byte
    /// `REX.W C7 /0 id`.
    pub(super) fn emit_mov_imm32_sx(&mut self, reg: u8, imm: i32) {
        ...
        if imm > 0 {
            // Cast: positive i32 -> u32, value unchanged.
            self.emit_mov_r32_imm32_zx(reg, imm as u32);
            return;
        }
    ...
    /// VEX prefix (map `0F`, `W0`) for a register-direct operation whose ModRM
    /// `reg` field is `reg` and whose `r/m` field is `rm`.
    ///
    /// The 2-byte `C5` form has VEX.R but no VEX.B, so it cannot name an
    /// `r/m` register >= 8: the reg-reg emitters below used to call
    /// `emit_vex2` unconditionally and a `src2` of YMM9 would have silently
    /// read YMM1. This takes the 3-byte `C4` form exactly when `rm >= 8` and is
    /// byte-identical to the old `emit_vex2` call otherwise. (`vex3`'s R/X/B
    /// arguments are the *inverted* bits — `true` means "not extended".)
    fn emit_vex_rr_0f(&mut self, reg: u8, vvvv: u8, rm: u8, l: bool, pp: u8) {
        if rm >= 8 {
            self.emit_vex3(reg < 8, true, false, 0x01, false, vvvv, l, pp);
        } else {
            self.emit_vex2(reg < 8, vvvv, l, pp);
        }
    }
    ...
    pub(super) fn emit_vpxor_ymm(&mut self, dst: u8, src1: u8, src2: u8) {
        self.emit_vex_rr_0f(dst, src1, src2, true, 1); // L=1(256-bit), pp=01(66)
        self.buf.emit_byte(0xEF);
    ...
    fn emit_vpaddd_ymm(&mut self, dst: u8, src1: u8, src2: u8) {
        self.emit_vex_rr_0f(dst, src1, src2, true, 1);
    ...
    fn emit_vpaddd_xmm(&mut self, dst: u8, src1: u8, src2: u8) {
        self.emit_vex_rr_0f(dst, src1, src2, false, 1); // L=0 for 128-bit
        self.buf.emit_byte(0xFE);
        self.buf.emit_byte(0xC0 | ((dst & 7) << 3) | (src2 & 7));
    }
    ...
    fn emit_vpshufd_xmm(&mut self, dst: u8, src: u8, imm: u8) {
        self.emit_vex_rr_0f(dst, 0, src, false, 1); // vvvv=0 → 1111 (unused)
    ...
    fn emit_vmovd_to_gpr(&mut self, dst_gpr: u8, src_xmm: u8) {
        self.emit_vex_rr_0f(src_xmm, 0, dst_gpr, false, 1); // vvvv=0 → 1111 (unused)
        self.buf.emit_byte(0x7E);
        self.buf
    ...
    pub(super) fn emit_fetch_current_thread_into_rax(&mut self) {
        ...
        if tls_disp != 0 {
            ...
            // ABI-3 — () -> *mut JvmThread, in RAX. This arm and the TLS-mirror
            // arm above have to agree on the answer's register and on there being
            // no arguments; the assertion pins the half that is a call.
    ...
    /// Emit `ADD qword ptr [base + disp8], imm8` — a read-modify-write of one
    /// 64-bit memory word by a small signed constant, with NO `lock` prefix.
    ///
    /// The one consumer is the per-method entry counter the prologue bumps
    /// (`crate::entry_counter`), which is why the unlocked form is the one
    /// offered: that counter is documented as deliberately racy, and a `lock`
    /// prefix on a word touched by every entry to every compiled method is the
    /// expensive thing precisely where the cheap thing is good enough. Anything
    /// needing an ATOMIC increment must not reach for this emitter — add a
    /// separate `lock`-prefixed one and say so at its call site.
    ///
    /// Encoding (5 bytes for a low base register, 5 for a high one — the REX
    /// byte is unconditional because the operand is 64-bit):
    ///
    /// ```text
    ///   REX.W [+ REX.B if base >= 8]     (1 byte)
    ///   0x83                             (1 byte)  ADD r/m64, imm8 (group 1 /0)
    ///   ModRM: mod=01 reg=/0 rm=base&7   (1 byte)
    ///   [SIB 0x24 when base&7 == 4]      (0 or 1)  RSP/R12 have no mod!=00 form
    ///   disp8                            (1 byte)
    ///   imm8                             (1 byte)  sign-extended to 64 bits
    /// ```
    ///
    /// `mod=01` (an explicit zero displacement) rather than the one-byte-shorter
    /// `mod=00`, for two reasons worth the byte: `mod=00` with `rm=101`
    /// (RBP/R13) is not `[rbp]` at all but RIP-relative, so the short form is
    /// wrong for two of the sixteen registers and right for the rest — exactly
    /// the shape of encoding bug that is invisible until someone changes which
    /// scratch register a caller uses. The uniform form is also
    /// length-deterministic, which is what lets the prologue test assert on
    /// exact bytes.
    ///
    /// `disp` is an `i32` for call-site convenience (the offset constants are
    /// `i32` everywhere else in this backend) but MUST fit in an `i8`; a value
    /// that does not refuses the method (`mark_codegen_unencodable`), it is
    /// never truncated.
    pub(super) fn emit_add_mem_disp8_imm8(&mut self, base: u8, disp: i32, imm: i8) {
        // Checked, not asserted: a `debug_assert!` is nothing in release, and
        // the `disp as i8` below would then address BEFORE the base (the
        // exact hazard `disp.rs` documents). Refuse the method instead.
        let Ok(disp8) = i8::try_from(disp) else {
            self.buf
                .mark_codegen_unencodable("add-mem-disp8-displacement-unencodable");
            return;
        };
        let mut rex = 0x48u8; // REX.W — 64-bit operand
        if base >= 8 {
            rex |= 0x01; // REX.B — extends the r/m (base) field
        }
        self.buf.emit_byte(rex);
        self.buf.emit_byte(0x83); // group 1, r/m64, imm8
        self.buf.emit_byte(0x40 | (base & 7)); // mod=01, reg=/0 (ADD), rm=base
        if (base & 7) == 4 {
            // rm=100 means "a SIB byte follows"; 0x24 is scale=0, index=none,
            // base=rm — i.e. plain `[base]`, which is what the ModRM would have
            // meant had 100 not been stolen for the SIB escape.
            self.buf.emit_byte(0x24);
        }
        // Cast: x86-64 displacement byte — range-checked as a signed i8 above.
        self.buf.emit_byte(disp8 as u8);
        self.buf.emit_byte(imm as u8);
    }
    ...
    pub(super) fn emit_mov_r64_mem_disp32(&mut self, dst: u8, base: u8, disp: i32) {
        ...
        self.buf.emit_byte(rex);
        self.buf.emit_byte(0x8B); // MOV r64, r/m64
        self.emit_modrm_sib_disp32(dst, base, disp);
    }

    /// ModRM (`mod=10`) + disp32 for `[base + disp32]`, with the index-free
    /// SIB byte (`0x24`) when `base` is RSP/R12.
    ///
    /// Every `*_mem_disp32` emitter below used to write
    /// `0x80 | reg << 3 | base & 7` and then the displacement, with a comment
    /// saying RSP/R12 "is not used here". `r/m=100` means "a SIB byte
    /// follows", so for an R12 base — a callee-saved LOCAL_REGS register the
    /// allocator hands out — the CPU would have read the first displacement
    /// byte as the SIB and every byte after it one position early: a wrong
    /// address and a desynchronised instruction stream, silently. The
    /// declarative table (`isel::mov_r64_m_disp32` and friends) always
    /// emitted the SIB; this makes the emitters agree with it. For every base
    /// that is not RSP/R12 the bytes are unchanged.
    fn emit_modrm_sib_disp32(&mut self, reg: u8, base: u8, disp: i32) {
        self.buf.emit_byte(0x80 | ((reg & 7) << 3) | (base & 7));
        if base_requires_sib(base) {
            // scale=00, index=100 (none), base=100 — plain `[base]`.
            self.buf.emit_byte(0x24);
        }
        self.buf.emit(&disp.to_le_bytes());
    }
    ...
    ///
    /// `dst` and `base` are register numbers 0..=15; an RSP/R12 base gets its
    /// SIB byte from [`Self::emit_modrm_sib_disp32`].
    ...
    pub(super) fn emit_mov_mem_disp32_r64(&mut self, base: u8, src: u8, disp: i32) {
        ...
        self.buf.emit_byte(rex);
        self.buf.emit_byte(0x89); // MOV r/m64, r64
        self.emit_modrm_sib_disp32(src, base, disp);
    }
    ...
    pub(super) fn emit_mov_dword_mem_disp32_imm32(&mut self, base: u8, disp: i32, imm: i32) {
        ...
        self.emit_modrm_sib_disp32(0, base, disp);
        self.buf.emit(&imm.to_le_bytes());
    }
    ...
    pub(super) fn emit_cmp_r64_mem_disp32(&mut self, lhs: u8, base: u8, disp: i32) {
        ...
        self.emit_modrm_sib_disp32(lhs, base, disp);
    }
    ...
    ///
    /// `base & 7 == 0b100` (RSP/R12) needs a SIB byte, which this short form
    /// does not emit; such a base falls back to the disp32 form, whose
    /// [`Self::emit_modrm_sib_disp32`] tail supplies it. `base & 7 == 0b101` (RBP/R13) has no `mod=00` form, so a
    ...
    /// memory-resident threshold.
    ///
    /// Any of the sixteen low-byte registers. Without a REX prefix the byte
    /// encodings 4..=7 name AH/CH/DH/BH, not SPL/BPL/SIL/DIL, so a REX byte —
    /// the bare `0x40` if nothing else — is emitted for every `lhs >= 4`. This
    /// used to be a `debug_assert!(lhs < 4)` and nothing else: in release an
    /// `lhs` of RSI compared the threshold against DH, silently, and an `lhs`
    /// of R9 compared it against CL.
    pub(super) fn emit_cmp_r8_mem8(&mut self, lhs: u8, base: u8, disp: i32) {
        let rex = 0x40 | (u8::from(lhs >= 8) << 2) | u8::from(base >= 8);
        if rex != 0x40 || lhs >= 4 {
            self.buf.emit_byte(rex);
        }
    ...
    /// `TEST r8, imm8` for any low-byte register.
    ///
    /// Same REX rule as [`Self::emit_cmp_r8_mem8`]: registers 4..=7 need the
    /// bare `0x40` or they decode as AH/CH/DH/BH, and 8..=15 need REX.B. The
    /// three-byte `F6 C0|r ib` shape is unchanged for AL/CL/DL/BL.
    pub(super) fn emit_test_r8_imm8(&mut self, reg: u8, imm: u8) {
        if reg >= 4 {
            self.buf.emit_byte(0x40 | u8::from(reg >= 8));
        }
    ...
    ///
    /// Two SIB special cases, both silent mis-encodings if written as the
    /// plain `mod=00` literal this used to be:
    ///
    /// * SIB `base=101` with `mod=00` means "no base, disp32 follows", so an
    ///   RBP/R13 base takes `mod=01` with an explicit `disp8` of 0 (what
    ///   `isel::mov_m8_index_imm8` has always emitted through
    ///   `Disp::encode_for_base`).
    /// * SIB `index=100` with REX.X clear means "no index", so RSP cannot be
    ///   an index at all; that is refused (`mark_codegen_unencodable`) rather
    ///   than turned into a store to `[base]`.
    ///
    /// The one caller (the card-mark store, `R11` base / `RCX` index) is
    /// byte-for-byte unchanged.
    pub(super) fn emit_mov_mem8_indexed_imm8(&mut self, base: u8, index: u8, value: u8) {
        if index == RSP {
            self.buf.mark_codegen_unencodable("mem8-index-is-rsp");
            return;
        }
        let mut rex = 0x40u8;
        if index >= 8 {
        ...
        let needs_disp8 = base_requires_displacement(base);
        // mod=00 (or mod=01 for RBP/R13), /0, r/m=100 (SIB follows).
        self.buf.emit_byte(if needs_disp8 { 0x44 } else { 0x04 });
...
pub(super) mod tests {
    use super::*;
    use crate::x64::{R10, R11, R12, R13, R9, RAX, RBP, RBX, RCX, RSI, RSP};

    /// A fresh single-pass `Compiler` whose hand-written emitters can be
    /// driven directly. Same construction as `isel`'s reference harness.
    ///
    /// `Compiler::new` has already emitted a method prologue; callers that
    /// execute what they emit must enter at the `buf.pos()` they recorded.
    pub(in crate::x64) fn test_compiler() -> Compiler {
        ...
        Compiler::new(
            "emit-unit".to_string(),
            crate::ExecutableBuffer::new(1 << 16).expect("test executable buffer"),
            0,
            0,
            1,
            false,
    ...
    /// The bytes one emitter call appends.
    fn bytes(c: &mut Compiler, f: impl FnOnce(&mut Compiler)) -> Vec<u8> {
        let start = c.buf.pos();
        f(c);
        c.buf.as_slice()[start..].to_vec()
    }

    /// An R12/RSP base gets the index-free SIB byte in every `*_disp32`
    /// emitter; every other base keeps exactly the bytes it always had.
    #[test]
    fn disp32_family_emits_the_sib_byte_for_rsp_class_bases() {
        let mut c = test_compiler();
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_r64_mem_disp32(RAX, R12, 8)),
            vec![0x49, 0x8B, 0x84, 0x24, 0x08, 0x00, 0x00, 0x00],
            "MOV RAX, [R12 + 8]"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_mem_disp32_r64(RSP, RCX, 16)),
            vec![0x48, 0x89, 0x8C, 0x24, 0x10, 0x00, 0x00, 0x00],
            "MOV [RSP + 16], RCX"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_dword_mem_disp32_imm32(R12, 4, 7)),
            vec![0x41, 0xC7, 0x84, 0x24, 0x04, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00],
            "MOV DWORD [R12 + 4], 7 — the SIB sits between ModRM and disp32"
        );
        // Unchanged for a non-SIB base.
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_r64_mem_disp32(RAX, RBX, 8)),
            vec![0x48, 0x8B, 0x83, 0x08, 0x00, 0x00, 0x00],
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_r64_mem_disp32(RAX, R13, 0)),
            vec![0x49, 0x8B, 0x85, 0x00, 0x00, 0x00, 0x00],
        );
        assert!(!c.buf.overflowed());
    }

    /// Byte registers 4..=7 need a bare REX or they are AH/CH/DH/BH; 8..=15
    /// need REX.R / REX.B. AL..BL keep their REX-less encodings.
    #[test]
    fn byte_register_forms_encode_every_register() {
        let mut c = test_compiler();
        assert_eq!(
            bytes(&mut c, |c| c.emit_test_r8_imm8(RCX, 1)),
            vec![0xF6, 0xC1, 0x01]
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_test_r8_imm8(RSI, 1)),
            vec![0x40, 0xF6, 0xC6, 0x01],
            "SIL, not DH"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_test_r8_imm8(R9, 1)),
            vec![0x41, 0xF6, 0xC1, 0x01],
            "R9B, not CL"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_cmp_r8_mem8(RCX, R11, 0)),
            vec![0x41, 0x3A, 0x0B],
            "the one existing caller is unchanged"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_cmp_r8_mem8(RSI, RAX, 0)),
            vec![0x40, 0x3A, 0x30],
            "CMP SIL, [RAX]"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_cmp_r8_mem8(R9, RAX, 0)),
            vec![0x44, 0x3A, 0x08],
            "CMP R9B, [RAX]"
        );
        assert!(!c.buf.overflowed());
    ...
    /// `[base + index]` byte store: RBP/R13 bases take `mod=01` + `disp8(0)`
    /// (a `mod=00` SIB with base=101 is "no base, disp32"), and an RSP index —
    /// which SIB cannot express — refuses the method.
    #[test]
    fn indexed_byte_store_handles_both_sib_special_cases() {
        let mut c = test_compiler();
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_mem8_indexed_imm8(R11, RCX, 1)),
            vec![0x41, 0xC6, 0x04, 0x0B, 0x01],
            "the card-mark store is unchanged"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_mem8_indexed_imm8(RBP, RCX, 1)),
            vec![0xC6, 0x44, 0x0D, 0x00, 0x01],
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_mem8_indexed_imm8(R13, RCX, 1)),
            vec![0x41, 0xC6, 0x44, 0x0D, 0x00, 0x01],
        );
        assert!(!c.buf.overflowed());
        c.emit_mov_mem8_indexed_imm8(RAX, RSP, 1);
        assert!(
            c.buf.overflowed(),
            "an RSP index must refuse, not store to [base]"
        );
    }

    /// A displacement past `i8` refuses the method instead of wrapping.
    #[test]
    fn add_mem_disp8_refuses_a_wide_displacement() {
        let mut c = test_compiler();
        assert_eq!(
            bytes(&mut c, |c| c.emit_add_mem_disp8_imm8(R10, 8, 1)),
            vec![0x49, 0x83, 0x42, 0x08, 0x01],
        );
        assert!(!c.buf.overflowed());
        c.emit_add_mem_disp8_imm8(R10, 128, 1);
        assert!(
            c.buf.overflowed(),
            "disp 128 would have been encoded as -128"
        );
    }

    /// The register-direct VEX emitters take the 3-byte prefix when the
    /// `r/m` register is >= 8 (the 2-byte form has no VEX.B), and are
    /// byte-identical to the old 2-byte form otherwise.
    #[test]
    fn vex_reg_reg_forms_reach_the_high_registers() {
        let mut c = test_compiler();
        assert_eq!(
            bytes(&mut c, |c| c.emit_vpxor_ymm(0, 0, 0)),
            vec![0xC5, 0xFD, 0xEF, 0xC0],
            "the existing SIMD call site is unchanged"
        );
        // VPXOR YMM0, YMM0, YMM9: C4 (R X !B, map 0F) (W0 vvvv=~0 L=1 pp=66).
        assert_eq!(
            bytes(&mut c, |c| c.emit_vpxor_ymm(0, 0, 9)),
            vec![0xC4, 0xC1, 0x7D, 0xEF, 0xC1],
        );
        // VPSHUFD XMM1, XMM10, 0x4E.
        assert_eq!(
            bytes(&mut c, |c| c.emit_vpshufd_xmm(1, 10, 0x4E)),
            vec![0xC4, 0xC1, 0x79, 0x70, 0xCA, 0x4E],
        );
        assert!(!c.buf.overflowed());
    }

    /// Non-negative constants take the zero-extending `MOV r32, imm32`
    /// (`mov-imm-non-negative-uses-sign-extending-c7-form-20260918.md`);
    /// negative ones keep the sign-extending `REX.W C7 /0`, wider ones MOVABS,
    /// zero keeps `XOR`, and the IC form stays ten bytes.
    #[test]
    fn non_negative_immediates_take_the_zero_extending_form() {
        let mut c = test_compiler();
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm32_sx(R9, 1)),
            vec![0x41, 0xB9, 0x01, 0x00, 0x00, 0x00],
            "MOV R9D, 1"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm32_sx(RCX, i32::MAX)),
            vec![0xB9, 0xFF, 0xFF, 0xFF, 0x7F],
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm32_sx(RCX, -1)),
            vec![0x48, 0xC7, 0xC1, 0xFF, 0xFF, 0xFF, 0xFF],
            "a negative value still sign-extends"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm32_sx(R10, i32::MIN)),
            vec![0x49, 0xC7, 0xC2, 0x00, 0x00, 0x00, 0x80],
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm64(RAX, 0xFFFF_FFFF)),
            vec![0xB8, 0xFF, 0xFF, 0xFF, 0xFF],
            "MOV EAX, 0xFFFFFFFF zero-extends to 0x0000_0000_FFFF_FFFF"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm64(R11, i64::from(i32::MAX) + 1)),
            vec![0x41, 0xBB, 0x00, 0x00, 0x00, 0x80],
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm64(RAX, 1 << 32)),
            vec![0x48, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00],
            "past u32 needs MOVABS"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm64(RAX, i64::from(i32::MIN) - 1)).len(),
            10,
            "below i32::MIN needs MOVABS"
        );
        assert_eq!(
            bytes(&mut c, |c| c.emit_mov_imm64(RAX, 0)),
            vec![0x31, 0xC0]
        );
        assert_eq!(bytes(&mut c, |c| c.emit_mov_imm64_full(RAX, 1)).len(), 10);
        assert!(!c.buf.overflowed());
    }
}
