...
impl StackKindMap {
    ...
    // The test-only constructor (`from_entries_for_test`) lives in the bottom
    // test module: a test gate this high up used to end the panic-free
    // ratchet's scan of this file here, leaving the analysis below unchecked.
}
...
pub(crate) fn analyze(code: &[u8], code_len: usize, inputs: &StackKindInputs<'_>) -> StackKindMap {
    ...
    while let Some(pc) = work.pop() {
        ...
        if len == 0 {
            map.at.insert(pc, state);
            continue;
        }
        let next = pc + len;

        // Apply this opcode's typed stack effect. `None` = poison. Computed
        // BEFORE the state is published so the map can take `state` by value:
        // this loop is on every compile's path, and the published copy was a
        // second clone of the same vector per visit.
...
/// [`bytecode_analysis::lenient_successors`], plus the textual fall-through of
/// `ret` and `wide ret`, which this walk has always poisoned (the lenient view
/// gives both no successors at all).
fn successors(code: &[u8], pc: usize, op: u8, len: usize, code_len: usize) -> Vec<usize> {
    ...
    if op == 0xa9 || bytecode_analysis::is_wide_ret(code, pc) {
        out.push(pc + len);
    }
    out
}
...
fn transfer(
    ...
    match op {
        ...
        // `dup`, `dup_x1` and `swap` have ONE form each: JVMS admits only
        // category-1 operands, so their compact stack effect does not depend
        // on an operand's category. An `Unknown` operand (an unresolved `ldc`)
        // therefore leaves the DEPTH perfectly known and must not poison —
        // rule 3 of the module doc poisons on depth ambiguity only, and
        // poisoning here used to cost every pc downstream of the shuffle its
        // answer (and with it the `dup2`/`pop2` width oracle). A KNOWN
        // category-2 operand is malformed bytecode and still poisons.
        0x59 => {
            // dup — cat-1 only per JVMS.
            let top = *s.last()?;
            if top.is_category_2() == Some(true) {
                return None;
            }
            push!(top);
        }
        0x5a => {
            ...
            let top = s[s.len() - 1];
            let below = s[s.len() - 2];
            if top.is_category_2() == Some(true) || below.is_category_2() == Some(true) {
                return None;
            }
        ...
        0x5b => {
            ...
            // two entries). v1 is cat-1 in both forms, so only a KNOWN
            // category-2 top poisons (see `dup` above); the entry below
            // decides how deep the copy is inserted and must be known.
            if s.len() < 2 {
                return None;
            }
            let top = s[s.len() - 1];
...
mod tests {
    ...
    impl StackKindMap {
        /// A map with exactly these answers, for consumer-side unit tests (the
        /// deopt snapshot's width selection) that must not depend on the
        /// analysis modelling a whole bytecode sequence.
        ///
        /// Defined here rather than beside `get`: a test gate above production
        /// code ends the panic-free ratchet's scan of the file (round 9 wave 7).
        pub(crate) fn from_entries_for_test(
            entries: impl IntoIterator<Item = (usize, Vec<StackKind>)>,
        ) -> Self {
            Self {
                at: entries.into_iter().collect(),
            }
        }
    }
    ...
    /// `dup`, `dup_x1` and `swap` have one JVMS form each, so an `Unknown`
    /// operand leaves their compact stack effect — and therefore the depth —
    /// fully known. They used to poison on it, which withdrew the answer at
    /// every pc downstream of the shuffle.
    #[test]
    fn single_form_shuffles_over_an_unknown_operand_keep_the_depth() {
        // 0: ldc #1 (unresolved -> Unknown)  2: dup  3: pop  4: pop  5: return
        let dup = vec![0x12, 0x01, 0x59, 0x57, 0x57, 0xb1];
        let m = run(&dup, FxHashMap::default());
        assert_eq!(
            m.get(3),
            Some(&[StackKind::Unknown, StackKind::Unknown][..])
        );
        assert_eq!(m.get(5).map(<[StackKind]>::len), Some(0));

        // 0: iconst_0  1: ldc #1  3: swap  4: pop  5: pop  6: return
        let swap = vec![0x03, 0x12, 0x01, 0x5f, 0x57, 0x57, 0xb1];
        let m = run(&swap, FxHashMap::default());
        assert_eq!(m.get(4), Some(&[StackKind::Unknown, StackKind::Int][..]));
        assert_eq!(m.get(6).map(<[StackKind]>::len), Some(0));

        // 0: iconst_0  1: ldc #1  3: dup_x1  4: pop  5: pop  6: pop  7: return
        let dup_x1 = vec![0x03, 0x12, 0x01, 0x5a, 0x57, 0x57, 0x57, 0xb1];
        let m = run(&dup_x1, FxHashMap::default());
        assert_eq!(
            m.get(4),
            Some(&[StackKind::Unknown, StackKind::Int, StackKind::Unknown][..])
        );

        // A KNOWN category-2 operand is malformed for all three and still
        // poisons: 0: lconst_0  1: dup  2: pop  3: return
        let bad = vec![0x09, 0x59, 0x57, 0xb1];
        let m = run(&bad, FxHashMap::default());
        assert_eq!(m.get(1), Some(&[StackKind::Long][..]));
