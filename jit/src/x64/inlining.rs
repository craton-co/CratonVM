...
/// publishes any point this answers `true` for. Two independent things have to
...
///    — a wrong-code bug, not a missed optimisation.
///
...
/// longer stamps `self.method_key` unconditionally: `Compiler::inline_callee_identity`
...
/// omits `snapshot_pre_intrinsic_call`), so the population this admits is still
/// empty. What changed is that the FIRST edit to publish one produces
/// well-formed metadata instead of being refused, and the refusal that remains
/// is now about a real defect rather than about an unimplemented stamp.
///
...
fn splice_point_is_misidentified(
    ...
    // The innermost frame must name a spliced CALLEE. At this splice's own
    // level that is `callee_key`; a point published by a NESTED splice inside
    // this body legitimately names the INNER callee, and was already held to
    // this same rule by that splice's own postcondition, so refusing it here
    // would make nesting and publishing mutually exclusive for no safety gain.
    //
    // What must never appear is `enclosing_method_key`, which is what
    // `build_frame_state_at` stamps when no splice is active. That is the
    // caller-named / callee-bci'd pair this check exists to exclude, and it is
    // the only other identity the producer can stamp.
    point.frame_state.method_key != callee_key
        && point.frame_state.method_key == enclosing_method_key
}
...
impl Compiler {
    ...
    pub(super) fn try_emit_inline_site(&mut self, pc: usize, site: &crate::InlineSite) -> bool {
        ...
        self.push_inline_scope(pc, site.callee_num_args, callee_key.clone());
        ...
        //
        // **What it checks changed 2026-08-18, and was WRONG until now.** The
        ...
        //
        // It does not. `build_frame_state_at` stamps
        // `method_key: self.method_key` unconditionally, and `self.method_key`
        // is assigned once per compile (`x64::driver`) and never swapped for
        // the callee during a splice. `build_and_record_deopt_point` then
        // publishes `bci: self.orig_bci(bci)`, mapping a CALLEE pc through the
        // ENCLOSING method's loop-rewrite table. So a point published from
        // inside a splice names the caller's method at a bci belonging to the
        // callee — the malformed pair this whole exercise exists not to
        // produce, and one the VM's identity gate
        // (`deopt_resume::deopt_frame_matches_method`) would ACCEPT, because
        // the key does name the method being resumed. It would then resume the
        // caller's bytecode at a foreign bci.
        //
        // So the postcondition asks what it was always meant to ask: a point
        // published from inside this splice must carry a caller chain AND an
        // innermost `method_key` naming the spliced callee. Until
        // `build_frame_state_at` takes the identity as a parameter, the second
        // half is never true, so this is the strict "no published metadata from
        // inside a splice" rule again — which is the correct conservative state
        // and exactly what the relaxation silently gave up. The difference from
        // simply reverting is that the check now states the property, so the
        // edit that makes callee-identified metadata possible is admitted
        ...
        // `snapshot_pre_intrinsic_call`), so this is a gate on a future edit,
        ...
        let published_misidentified_point = self.deopt_points[deopt_points_checkpoint..]
            .iter()
            .any(|p| splice_point_is_misidentified(p, &callee_key, &self.method_key));
    ...
    ///
    ///    **M1 closed half of that and not the other half, and the difference
    ///    is what keeps this rule in place.** `current_bytecode_owner` /
    ///    `resume_bci_for` now give a point published inside a splice the right
    ///    IDENTITY and the right BCI SPACE. The LOCALS are still read out of
    ///    the enclosing method's `local_liveness` / `local_kinds` /
    ///    `sr_local_prov_at` tables, which are analyses of that method's
    ///    bytecode and say nothing about a callee's slots. So a scope built
    ///    here would now be correctly labelled and still wrong about its
    ///    contents, which is strictly worse than not building one.
...
mod splice_postcondition_tests {
    ...
    fn a_caller_scoped_point_naming_the_caller_is_still_refused() {
        ...
        assert!(
            splice_point_is_misidentified(&p, CALLEE, CALLER),
    ...
    fn a_callee_identified_point_under_a_caller_scope_is_admitted() {
        ...
        assert!(!splice_point_is_misidentified(&p, CALLEE, CALLER));
    }
    ...
    fn a_nested_splices_point_is_admitted_by_the_enclosing_splice() {
        ...
        assert!(
            !splice_point_is_misidentified(&p, NESTED, CALLER),
