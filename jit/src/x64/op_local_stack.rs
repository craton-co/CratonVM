...
impl Compiler {
    ...
    pub(super) fn walk_local_stack(
        ...
        _dead: &mut bool,
        branch_targets: &[bool],
        // The walk's instruction-start map (see `compile_bytecode`), for the
        // category-2 peephole's backward boundary scan.
        insn_starts: &[bool],
    ) -> WalkStep {
        match op {
            // nop
            ...
            // lconst_0 / lconst_1 — through the same `l{mul,div,rem,add,sub}`
            // constant fusion as `ldc2_w`: `lload i; lconst_1; ladd; lstore i`
            // is javac's `i++` on a `long` (every `for (long i ...)` loop), and
            // unfused it materialises the 1, parks it, and reloads it into RCX
            // for a register-register ADD.
            0x09 | 0x0a => {
                let val = i64::from(op - 0x09);
                if self.try_const_arith_peephole_long(val, pc + 1, code, code_len, &branch_targets)
                {
                    pc += 2;
            ...
            0x36..=0x3a => {
                ...
                if matches!(op, 0x38 | 0x39) {
                    if let Some(dst_xmm) = self.xmm_for_local(idx) {
                        // Entries still naming the home hold the local's OLD value;
                        // materialise them before the write below replaces it. BEFORE
                        // the pop: the spill word it reserves could otherwise be the
                        // one the pop just released, clobbering a `Frame` source
                        // before the arm below reads it.
                        self.invalidate_xmm_home(dst_xmm);
                ...
                // Invalidate BEFORE the pop, as the `_0..3` arms do: the spill
                // word `invalidate_callee_saved` reserves could otherwise be the
                // one the pop just released, and `pop_into_local` reads the
                // popped operand after it.
                if let Some(local_reg) = self.reg_for_local(idx) {
                    self.invalidate_callee_saved(local_reg);
                }
                self.pop_into_local(idx);
                pc += 2;
            }
            ...
            // (two category-1 values) pops twice. `top_entry_is_cat2` is the
            // same width question `dup2` asks, answered by the one consult
            // point the five category-dependent arms now share.
            0x58 => {
                match self.top_entry_is_cat2(code, pc, insn_starts) {
            ...
            0x5b => {
                ...
                let peephole_top = self.dup2_top_cat2(code, pc, insn_starts);
                ...
                let depth = cats
                    ...
                    .and_then(|cats| {
                        ...
                        // independent analyses is wrong; use neither — and
                        // COUNT it, which is the whole difference between a
                        // divergence that is known about and one that is not.
                        reconcile_top_cat2(pc, top, peephole_top)?;
                        if top {
            ...
            // decided by the top operand's width, which `top_entry_is_cat2`
            // answers from the `x64::stack_kinds` abstract interpretation
            // cross-checked against the `dup2_top_cat2` peephole (the
            // immediately-preceding op + resolved field/invoke metadata).
            // FORM-2 is exactly `dup` of the one slot. If the width can't be
            // proven, bail to the interpreter rather than risk a category
            // miscompile (the historic FORM-1-on-cat-2 hard abort). This is
            // the codegen-side replacement for the `dup2_category_safe`
            // reject gate, which over-rejected FORM-1 methods (regressing
            // bintrees18) because the CP-less scan could not resolve a
            // `<getfield/invoke>; dup2` top to category-1 — and the analysis
            // now also covers the `<shuffle>; dup2` shape the peephole never
            // could.
            0x5c => {
                match self.top_entry_is_cat2(code, pc, insn_starts) {
                    Some(true) => {
            ...
            0x5e => {
                ...
                let shape = cats.as_ref().and_then(|cats| {
                    ...
                    // independent analyses is wrong; use neither — and count
                    // it (`cat2_model_disagreements`).
                    reconcile_top_cat2(pc, top, peephole_top)?;
                    let second = (*cats.get(n.checked_sub(2)?)?)?;
                    if top {
            ...
            0x5f => {
                ...
                // The physical exchange below rewrites BOTH words, so it is
                // only sound when nothing else still reads either of them.
                // `invalidate_callee_saved` (every `istore`/`astore`/`iinc`
                // to a register-homed local) repoints ALL aliasing entries at
                // one shared word, and `flush_xmm0_slots` does the same, so a
                // deeper entry can name `off_a` or `off_b`: exchanging then
                // silently hands that entry the other operand's value. With
                // two copies of one register-homed local on the stack and a
                // store to it in between — `iload_1; iload_1; ...; istore_1;
                // <push>; swap` — that entry is live. Such a pair (and a
                // pair sharing ONE word, where the exchange is a no-op
                // anyway) takes the entry-reorder arm instead, which moves no
                // memory and is value-correct for any homes.
                let exchange_is_private =
                    match (a, b) {
                        (StackSlot::Frame(off_a), StackSlot::Frame(off_b)) => off_a != off_b
                            && !self.stack.iter().any(
                                |s| matches!(*s, StackSlot::Frame(o) if o == off_a || o == off_b),
                            ),
                        _ => false,
                    };
                match (a, b) {
                    (StackSlot::Frame(off_a), StackSlot::Frame(off_b)) if exchange_is_private => {
    ...
    /// **Is the top operand-stack entry at `pc` a category-2 value?**
    /// `None` = not provable here, and the caller must stay interpreted.
    ///
    /// # Why this function exists
    ///
    /// Three analyses used to answer this question and nothing reconciled
    /// them (`NOTES-irfront.md` §6):
    ///
    /// 1. the IR tier reads `graph.nodes[v].ty` — it knows an `Op::Call`'s
    ///    width from the resolved descriptor;
    /// 2. this backend asked [`Compiler::dup2_top_cat2`], a *producing
    ///    instruction* peephole that walks back one opcode and fails the
    ///    compile whenever the producer is a shuffle, a block boundary, an
    ///    `invokedynamic`, a `wide`, a `checkcast` or an `iinc`;
    /// 3. `x64::stack_kinds` is a third, wholly independent abstract
    ///    interpretation of the same question over the whole method.
    ///
    /// They can disagree, and where they merely *differ in reach* a method
    /// ends up compilable by exactly one tier for reasons nothing records.
    /// The `dup_x2`, `dup2_x1` and `dup2_x2` arms already consulted (3) with
    /// (2) as a cross-check; `pop2` and `dup2` still asked (2) alone, so
    /// `<shuffle>; dup2` refused a method the same compile could have typed.
    /// This is the one consult point for all five.
    ///
    /// # The rules, in precedence order
    ///
    /// * **The analysis answers first.** `stack_entry_categories` admits it
    ///   only when its depth equals this emitter's live stack depth AND every
    ///   typed entry's ref-ness matches this emitter's own oop marks — two
    ///   independent opinions maintained for the GC, which a modelling error
    ///   that shifted the stack could not survive.
    /// * **A disagreement is a refusal, not a vote.** If the peephole also
    ///   answers and answers differently, one of two independent analyses is
    ///   wrong and there is no basis for choosing; the compile bails to the
    ///   interpreter, which is always semantically legal, and the divergence
    ///   is COUNTED. That counter is the point of this arrangement: the
    ///   NOTES' complaint is that the models diverge "for reasons nothing
    ///   records", and a disagreement that nobody can observe is worse than
    ///   one that costs a method its compile.
    /// * **The peephole is the fallback.** Where the analysis declines — it
    ///   poisons totally on an unmodelled opcode, and one such opcode
    ///   anywhere upstream costs every pc after it — the peephole still
    ///   proves the local case. Nothing that compiled before stops
    ///   compiling.
    pub(super) fn top_entry_is_cat2(
        &self,
        code: &[u8],
        pc: usize,
        insn_starts: &[bool],
    ) -> Option<bool> {
        let peephole = self.dup2_top_cat2(code, pc, insn_starts);
        match self
            .stack_entry_categories(pc)
            .and_then(|cats| cats.last().copied().flatten())
        {
            // The analysis answered: it is the answer, once the peephole has
            // had its say.
            Some(analysis) => reconcile_top_cat2(pc, analysis, peephole),
            // It declined (poisoned upstream, or its depth/ref-ness did not
            // match this emitter's). The peephole still proves the local case.
            None => peephole,
        }
    }
}

/// The reconciliation rule the five category-dependent arms share: the
/// abstract interpretation's opinion about the TOP entry, accepted only if the
/// peephole does not contradict it.
///
/// A contradiction returns `None` — refuse, do not vote — and increments
/// [`cat2_model_disagreements`]. Two independent analyses of the same question
/// disagreeing is a defect in one of them; which one is not something this
/// call site can know, and a compile that bails is always semantically legal
/// while a compile that guesses is not.
///
/// A free function rather than a method because it reads nothing off the
/// compiler: it is the rule itself, and it is testable without building one.
pub(crate) fn reconcile_top_cat2(
    pc: usize,
    analysis_top: bool,
    ...
    if let Some(p) = peephole {
        if p != analysis_top {
            note_cat2_model_disagreement(pc, analysis_top, p);
            return None;
        }
    }
    Some(analysis_top)
...
/// How many times the two category-2 models answered this compile's question
/// differently. See [`Compiler::top_entry_is_cat2`].
///
/// Process-wide and monotonic, like the rest of this crate's model counters:
/// the number that matters is "did this ever happen", and a non-zero value is
/// a bug in exactly one of `x64::stack_kinds` and `Compiler::dup2_top_cat2`.
static CAT2_MODEL_DISAGREEMENTS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);

/// Reader for [`CAT2_MODEL_DISAGREEMENTS`] — the number a soak run should find
/// at zero.
pub(crate) fn cat2_model_disagreements() -> u64 {
    CAT2_MODEL_DISAGREEMENTS.load(std::sync::atomic::Ordering::Relaxed)
}

/// Record that the abstract interpretation and the peephole disagreed about
/// the width of the top of the stack at `pc`.
///
/// Printed under the existing `CRATONVM_DBG_JITC` codegen switch rather than
/// unconditionally: the compile has already refused the method by the time
/// this is called, so the divergence costs throughput and not correctness, and
/// a compiler that writes to stderr on its own initiative is a compiler nobody
/// runs with diagnostics on.
fn note_cat2_model_disagreement(pc: usize, analysis: bool, peephole: bool) {
    CAT2_MODEL_DISAGREEMENTS.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JITC") {
        eprintln!(
            "[cratonvm-jit] cat-2 model disagreement at pc {pc}: stack_kinds says \
             cat2={analysis}, dup2_top_cat2 says cat2={peephole} — refusing the method"
        );
    }
}

#[cfg(test)]
mod category_two_reconciliation_tests {
    use super::{cat2_model_disagreements, reconcile_top_cat2};

    /// The ordinary case: the abstract interpretation answers and the
    /// peephole, where it answers at all, says the same thing.
    #[test]
    fn the_analysis_answers_when_the_peephole_agrees_with_it() {
        assert_eq!(reconcile_top_cat2(0, true, Some(true)), Some(true));
        assert_eq!(reconcile_top_cat2(0, false, Some(false)), Some(false));
    }

    /// The reach the unification buys. The peephole declines whenever the
    /// producing instruction is a shuffle, a block boundary, an
    /// `invokedynamic`, a `wide`, a `checkcast` or an `iinc`; the analysis
    /// carries the width through all of them, and `<shuffle>; dup2` is a
    /// method that used to be refused by this tier alone.
    #[test]
    fn the_analysis_answers_where_the_peephole_has_no_opinion() {
        assert_eq!(reconcile_top_cat2(0, true, None), Some(true));
        assert_eq!(reconcile_top_cat2(0, false, None), Some(false));
    }

    /// A disagreement is a refusal and not a vote: two independent analyses of
    /// one question cannot both be right, this call site cannot know which is
    /// wrong, and a compile that bails is always semantically legal.
    #[test]
    fn a_disagreement_refuses_rather_than_picking_a_side() {
        assert_eq!(reconcile_top_cat2(7, true, Some(false)), None);
        assert_eq!(reconcile_top_cat2(7, false, Some(true)), None);
    }

    /// …and it is COUNTED. The complaint in `NOTES-irfront.md` §6 is that the
    /// three models could diverge "for reasons nothing records"; a divergence
    /// nobody can observe is worse than one that costs a method its compile.
    #[test]
    fn a_disagreement_is_recorded_where_a_soak_run_can_find_it() {
        let before = cat2_model_disagreements();
        assert_eq!(reconcile_top_cat2(11, true, Some(false)), None);
        assert!(
            cat2_model_disagreements() > before,
            "the counter is what makes the divergence observable"
        );
        // The counter is a defect signal, not a traffic meter: an agreement
        // answers without touching it. Asserted as "the agreement still
        // answers" rather than as an exact count, because the counter is
        // process-wide and the test binary runs these in parallel.
        assert_eq!(reconcile_top_cat2(11, true, Some(true)), Some(true));
        assert_eq!(reconcile_top_cat2(11, true, None), Some(true));
    }
}
