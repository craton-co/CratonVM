...
impl Compiler {
            ...
            0xb8 => {
                    ...
                    && code[pc + 11] == 0xb6
                {
                    if self.stack.len() >= 2 {
                        ...
                        // ABI-3 — (vm_ptr, proxy_raw, index). `emit_call_absolute` takes a
                        // bare address, so this assertion is the only thing connecting the
                        // three registers written above to the helper's declared arity.
                ...
                // Round 9 wave 7 (`mutrec7`): a row marked
                // `JIT_CYCLE_EDGE_CELL_GUARD` carries a `JitCycleEdgeCell`
                // ADDRESS, not an entry. It is lowered by
                // `emit_cycle_edge_cell_call` below and must never reach the
                // direct-call ladder, which would `CALL` the cell.
                let cycle_cell = if site_is_gpu_kernel {
                    None
                } else {
                    self.direct_calls_idx.get(&pc).and_then(|&i| {
                        let dc = &self.direct_calls[i].1;
                        (dc.guard_class_id == crate::JIT_CYCLE_EDGE_CELL_GUARD).then_some(dc.entry)
                    })
                };
                // Round 9 wave 9 (`evict9`): a row marked
                // `JIT_RETIRE_CELL_GUARD` is an ordinary baked direct call
                // whose target is read through a `JitCycleEdgeCell` PINNED to
                // that one entry, so an invalidation that retires the callee
                // sends this (possibly still running) caller to dispatch
                // instead of back into the withdrawn body. Its `entry` is the
                // cell's ADDRESS; the entry it guards is the cell's pinned one,
                // which is what the direct-call ladder below is handed.
                let retire_cell = if site_is_gpu_kernel || cycle_cell.is_some() {
                    None
                } else {
                    self.direct_calls_idx.get(&pc).and_then(|&i| {
                        let dc = &self.direct_calls[i].1;
                        (dc.guard_class_id == crate::JIT_RETIRE_CELL_GUARD).then_some(dc.entry)
                    })
                };
                let direct = if site_is_gpu_kernel || cycle_cell.is_some() {
                    None
                } else {
                    self.direct_calls_idx.get(&pc).and_then(|&i| {
                        let dc = &self.direct_calls[i].1;
                        // Round 9 wave 10 (`call10`): an INSTANCE self-call
                        // marker (`walk_invoke_instance` hands its site here)
                        // names no entry; it takes the self-recursive arm
                        // below, which it reaches with no direct and no info.
                        if dc.guard_class_id == crate::JIT_SELF_CALL_GUARD {
                            return None;
                        }
                        let entry = if dc.guard_class_id == crate::JIT_RETIRE_CELL_GUARD {
                            // SAFETY: the planner that marked this row owns the
                            // cell (`SinglePassParts::owned_cycle_cells`, or the
                            // OSR door's cell vector) for the whole compile.
                            unsafe { crate::JitCycleEdgeCell::pinned_entry_at(dc.entry) }
                        } else {
                            dc.entry
                        };
                        // A retire row whose cell pins nothing would CALL 0:
                        // leave the site to the dispatch arm (its info is
                        // registered) rather than emit that.
                        if entry == 0 {
                            return None;
                        }
                        note_emit_direct(&self.method_key, pc, entry);
                        Some((entry, dc.needs_context, dc.num_params, dc.return_type))
                    })
                };
                // The guarded cell call needs the frame's VM context (the
                // stack guard and the dispatch fallback both take it), both
                // helpers, and the site's `JitInvokeInfo` (the fallback
                // dispatches through it). Missing any of them, the site takes
                // the plain dispatch arm below -- `direct` is already `None`.
                if let (Some(cell_addr), Some(info)) = (cycle_cell, info_ptr) {
                    if self.needs_heap
                        && self.helpers.self_call_stack_guard != 0
                        && self.helpers.invoke_dispatch != 0
                    {
                        if !self.emit_cycle_edge_cell_call(pc, cell_addr, info) {
                            return WalkStep::Return(false);
                        }
                        pc += 3;
                        return WalkStep::Next(pc);
                    }
                } else if cycle_cell.is_some() {
                    // A cell row with no `JitInvokeInfo` would fall to the
                    // SELF-recursive arm (no info, no direct) and CALL this
                    // method's own entry. `try_compile` always registers one,
                    // so this is a planner/emitter disagreement: refuse.
                    self.fail("singlepass-codegen/cycle-edge-cell-without-invoke-info");
                    return WalkStep::Return(false);
                }
                ...
                if let Some((callee_entry, callee_needs_ctx, callee_params, ret_type)) = direct {
                    if callee_entry == crate::MATH_SQRT_INTRINSIC {
                        ...
                        // (the arg itself is fine — it's consumed). `_keeping`,
                        // here and in the five sibling arms below: a bare flush
                        // may relocate a live Xmm(0) INTO the scratch register
                        // `pop_stack` just freed — the popped operand's own —
                        // before the arm reads it
                        // (`single-pass-math-intrinsics-flush-xmm0-after-pop-20260918.md`).
                        // Unreachable today, since `flush_scratch_registers`
                        // at the top of this arm leaves no XMM0-7 entry on the
                        // stack, but not by anything the arm itself states.
                        self.flush_xmm0_slots_keeping(&[arg_slot]);
                        match arg_slot {
                            StackSlot::Xmm(xmm) => {
                                if xmm != 0 {
                                    // MOVSD XMM0, XMMn
                        ...
                        || callee_entry == crate::MATH_RINT_INTRINSIC
                    {
                        ...
                        // MOVSXD RAX, ECX: 48 63 C1. NOT `MOV EAX, ECX`: an
                        // `int` is held sign-extended in its 64-bit slot (see
                        // the Math.min/max int arm below), and the one input
                        // whose result is negative — `Math.abs(MIN_VALUE)`,
                        // which the JLS defines as MIN_VALUE — came out of
                        // the 32-bit move as +2^31 to every 64-bit consumer
                        // (a spliced `i2l`, a 64-bit compare, a helper's i64
                        // argument).
                        ...
                        // through the integer ARG_REGS file, and `jit_math_fma_double` is
                        // `(f64, f64, f64) -> f64` with its arguments in
                        // XMM0/XMM1/XMM2 on both ABIs. Asserting the integer shape here would
                        // be worse than asserting nothing, so what is pinned is the fact the
                        // sequence above actually depends on.
                        const _: () = {
                            let sig =
                                cratonvm_jit_api::helpers_abi::helper_sig_of("math_fma_double");
                            assert!(
                                sig.arity == 3 && sig.float_args == 3 && sig.returns_value,
                                "`math_fma_double` no longer takes three floating-point \
                                 operands; this site stages them into XMM0/XMM1/XMM2 and \
                                 would now be loading the wrong register file",
                            );
                        };
                        ...
                        // ABI-3, deliberately NOT `assert_helper_call_shape!`: that macro
                        // asserts `float_args == 0`, because every site it guards marshals
                        // through the integer ARG_REGS file, and `jit_math_fma_float` is
                        // `(f32, f32, f32) -> f32` with its arguments in
                        // XMM0/XMM1/XMM2 on both ABIs. Asserting the integer shape here would
                        // be worse than asserting nothing, so what is pinned is the fact the
                        // sequence above actually depends on.
                        const _: () = {
                            let sig =
                                cratonvm_jit_api::helpers_abi::helper_sig_of("math_fma_float");
                            assert!(
                                sig.arity == 3 && sig.float_args == 3 && sig.returns_value,
                                "`math_fma_float` no longer takes three floating-point \
                                 operands; this site stages them into XMM0/XMM1/XMM2 and \
                                 would now be loading the wrong register file",
                            );
                        };
                        ...
                        || callee_entry == crate::MATH_MAX_DOUBLE_INTRINSIC
                    {
                        ...
                        // `stack_push`'s second argument is `is_oop`, NOT
                        // "is this a double". It used to be passed
                        // `is_double`, which marked every `Math.min/max
                        // (double,double)` result as an object reference:
                        // a precise oop map / shadow push then named a
                        // double's bit pattern as a live oop, and a moving
                        // collector would try to relocate it. An FP result
                        // is never an oop, exactly as the sqrt/floor/abs
                        // arms above push it.
                        self.stack_push(StackSlot::Xmm(0), false);
                    ...
                    // the resolver and so cannot reach here.
                        ...
                        let arg = self.pop_stack();
                        self.flush_xmm0_slots_keeping(&[arg]);
                        ...
                        == crate::JitIntrinsic::IntNumberOfTrailingZeros.as_entry()
                    {
                        ...
                        // MOVSXD RAX, EAX: the swapped value is negative
                        // whenever the input's low byte has bit 7 set
                        // (`reverseBytes(0x80) == 0x80000000`, `(-1) == -1`),
                        // and the 32-bit BSWAP zero-extended it. Same
                        // sign-extension contract as `rotateLeft` below.
                        self.buf.emit(&[0x48, 0x63, 0xC0]);
                        self.push_from_rax();
                        ...
                        // MOVSXD RAX, EAX: every negative input answers
                        // MIN_VALUE, which the 32-bit ops left zero-extended.
                        self.buf.emit(&[0x48, 0x63, 0xC0]);
                        self.push_from_rax();
                        ...
                        // MOVSXD RAX, EAX: `lowestOneBit(MIN_VALUE)` is
                        // MIN_VALUE, negative.
                        self.buf.emit(&[0x48, 0x63, 0xC0]);
                        self.push_from_rax();
                        ...
                        self.buf.emit(&[0x0F, 0xC8]);
                        // MOVSXD RAX, EAX: any odd input reverses to a
                        // negative int.
                        self.buf.emit(&[0x48, 0x63, 0xC0]);
                        self.push_from_rax();
                        ...
                        // MOVSXD RAX, EAX: `-1` came out of the 32-bit SUB as
                        // 0x00000000_FFFFFFFF. An `int` slot is held
                        // sign-extended, and the inline mini-emitter treats
                        // `i2l` as a no-op on that promise.
                        self.buf.emit(&[0x48, 0x63, 0xC0]);
                        self.push_from_rax();
                        ...
                        || callee_entry == crate::JitIntrinsic::ArraysFill8.as_entry()
                    {
                        ...
                        self.emit_null_check_array_load(npe_action::NONE, trap_key, pc);
                    ...
                    // ===== INTRINSIC REGION END: ARRAYS_SORT =====
                    else if crate::JitIntrinsic::is_sentinel_entry(callee_entry) {
                        // An intrinsic sentinel no arm above emitted — a
                        // registration/emission disagreement (a CPU-feature
                        // gate, a despec filter, a family added to the
                        // resolver but not to this ladder). The plain arm
                        // below would `CALL` the sentinel address. Refuse the
                        // method; the interpreter runs the call correctly.
                        self.fail("singlepass-codegen/unhandled-intrinsic-sentinel");
                        return WalkStep::Return(false);
                    } else {
                        ...
                        // exception table must run. (A retire-cell site also uses
                        // this range as its dispatch fallback's args buffer.)
                        ...
                        // Emit direct CALL to callee entry point -- or, for a
                        // retire-cell site (round 9 wave 9, `evict9`), the same
                        // CALL made through the cell, with the dispatch helper
                        // as the target once an invalidation has cleared it.
                        // Taken only with everything the fallback needs: the
                        // frame's VM context, the helper, the site's
                        // `JitInvokeInfo` and the service range as its args
                        // buffer. Anything missing keeps the plain CALL to the
                        // pinned entry, i.e. exactly the pre-wave-9 code.
                        let info_arity_matches = info_ptr.is_some_and(|info| {
                            // SAFETY: `invoke_info` owns every pointer it hands
                            // out for the life of this compile.
                            let want = unsafe { (*info).num_jit_args };
                            want == arg_slots.len()
                        });
                        let retire_target = match (retire_cell, info_ptr, service_args_base) {
                            (Some(cell_addr), Some(info), Some(args_base))
                                if self.needs_heap
                                    && self.helpers.invoke_dispatch != 0
                                    && info_arity_matches =>
                            {
                                Some((cell_addr, info, args_base))
                            }
                            _ => None,
                        };
                        if let Some((cell_addr, info, args_base)) = retire_target {
                            self.emit_retire_cell_call_target(
                                cell_addr,
                                info,
                                arg_slots.len(),
                                args_base,
                    ...
                    let is_tail_call = pc + 3 < code_len
                        // An instance self-call handed over by
                        // `walk_invoke_instance` (round 9 wave 10) keeps the
                        // raw CALL: the tail form is the opt-in static shape.
                        && code[pc] == 0xb8
                        && matches!(code[pc + 3], 0xac..=0xb0) // ireturn..areturn
                        && !branch_targets[pc + 3]
                        && !self.pc_is_protected(pc)
                        // OPT-IN: `CRATONVM_JIT=self-tailcall`
                        // (`CRATONVM_JIT_SELF_TAILCALL`, a `runtime_flag_on`
                        // read -- default OFF since 2026-08-20, see
                        // `licm.rs::self_tailcall_enabled`). Off keeps the raw
                        // self-recursive CALL below, one native frame per
                        // activation, which is the HotSpot-faithful answer.
                        // (This comment used to say "on is the default".)
                        && self_tailcall_enabled();

                    // jit-invokedynamic-groovy-regression fix: a method
                    // containing an invokedynamic site THAT LOWERS TO THE
                    ...
                    // whole compile — correctness first.
                    //
                    // The test is `has_trapping_indy_site`, not "has any indy
                    // site". Since the indy BRIDGE landed (the 0xba arm below,
                    // `StringConcatFactory` / `LambdaMetafactory` /
                    // `SwitchBootstraps` / `ObjectMethods`), a bridged site
                    // emits a plain fallible helper call and NO trap at its
                    // bci — the same shape as every other call in this method,
                    // whose deopt points this arm has never refused. So there
                    // is nothing for the frame-identity hazard above to be
                    // about, and `!indy_info.is_empty()` refused the whole
                    // compile of every self-recursive method that creates a
                    // lambda or concatenates a string. `driver.rs` already
                    // derives `CompiledMethod::has_indy_trap` from exactly this
                    // predicate for the CALLERS of such a method; this is the
                    // same question asked about its own body.
                    if self.has_trapping_indy_site()
                    ...
                    // `pop_invoke_args` hands the argument SLOTS back to the spill
                    // allocator while they are still live sources for
                    // `emit_stack_arg_setup` far below, so the deopt-service copy
                    // has to be placed ABOVE them. Remember the pre-pop top, and
                    // separately the post-pop top the return value belongs at —
                    // the two direct-call arms' `args_frame_top`/`post_pop_spill`
                    // pair, for the same two reasons.
                    let args_frame_top = self.next_spill_offset;
                    let (arg_slots, arg_oops) = self.pop_invoke_args(n);
                    let post_pop_spill = self.next_spill_offset;
                    ...
                    // The tail form below un-stages this site (it moves the
                    // arguments into the parameter homes the entry mask names),
                    // so it needs the fail-closed bit as it was BEFORE this
                    // site raised it, not just the list truncated.
                    let staged_self_args_unmapped_before = self.pending_staged_args_unmapped;
                    ...
                    if is_tail_call && self.body_entry_offset > 0 {
                        ...
                        // and JMP back to body entry (skip prologue). The
                        // homes, and the order the moves may be made in, are
                        // the prologue's — see `emit_self_tail_param_moves`.
                        self.emit_self_tail_param_moves(&arg_slots);
                        // Un-stage BEFORE the back-edge poll below, not after
                        // the JMP where the truncation used to sit: the poll is
                        // a safepoint, and its map would otherwise consume the
                        // argument homes this site staged -- homes the moves
                        // above have just vacated -- and, for an argument that
                        // was not frame-resident, the fail-closed bit, marking
                        // the poll's map incomplete for a state the entry mask
                        // describes exactly. See the comment after the JMP for
                        // why the staged list must not outlive this site.
                        self.pending_staged_arg_oops.truncate(staged_self_args_mark);
                        self.pending_staged_args_unmapped = staged_self_args_unmapped_before;
                        // The JMP below is a BACK-EDGE that skips the
                        // prologue, and with it the method-entry poll, so a
                        // self-tail-recursive method whose only loop is the
                        // recursion ran with no poll at all
                        // (`NOTES-invoke.md` proposal 5). See
                        // `emit_self_tail_safepoint_poll`.
                        self.emit_self_tail_safepoint_poll();
                        ...
                        // THE TAIL FORM PUBLISHES NO STAGED ARGUMENT. The
                        // arguments were just loaded into this method's own
                        // parameter locals, which the entry mask names from here
                        // on, and `reset_spills` below hands their old slots
                        // straight back to the spill allocator. Anything left
                        // pending would be consumed by a LATER, unrelated
                        // safepoint and would name a slot the cursor has already
                        // given to something else — the mirror of the defect
                        // this staging fixes. The truncation to the mark (and
                        // the fail-closed bit's restore) now happen BEFORE the
                        // JMP, ahead of the back-edge poll, which is the one
                        // safepoint that could otherwise have taken them.
                    ...
                    // Preserve the outgoing Java arguments in a contiguous frame
                    // range for the cold callee-sentinel service below. A raw
                    // `CALL` has no dispatch-helper frame to recover them from
                    // when the callee's own exception table has to run, and this
                    // callee IS this method, so the range is what its handler is
                    // re-entered with. The two direct-call arms and the inline
                    // splice all do exactly this; only the self-call arm did not.
                    //
                    // `None` (no room, or a range that would overlap the argument
                    // slots it reads) simply keeps the pre-fix route for this one
                    // site: `emit_inline_callee_deopt_check` is skipped and the
                    // sentinel propagates as before. That is a throughput/
                    // correctness trade the other arms resolve by failing the
                    // compile, but here failing would refuse an artifact for
                    // every self-recursive method whose frame is merely full,
                    // and the propagating route is the one this backend shipped.
                    //
                    // `needs_heap` is a HARD precondition, not a nicety: the
                    // service is called with this frame's `vm_ptr` slot
                    // (`emit_inline_callee_deopt_check` loads ARG0 from
                    // `heap_local_offset`), and a frame that reserved no such
                    // slot would pass whatever that offset happens to address.
                    // Both planner arms that bind a self-call set it — but the
                    // TAIL-shaped arm does not, and its site still reaches this
                    // code as an ordinary raw `CALL` whenever
                    // `self_tailcall_enabled()` is off (the default), so the
                    // info can be staged for a frame that has no slot.
                    let self_service_args_base = (self.self_call_invoke_info != 0
                        && self.needs_heap)
                        .then(|| self.reserve_direct_call_service_slots(args_frame_top, &arg_slots))
                        .flatten();
                    if let Some(base) = self_service_args_base {
                        for (i, slot) in arg_slots.iter().enumerate() {
                            self.load_slot_to_reg(R11, *slot);
                            // Cast: x86-64 immediate encoding.
                            let off = base + ((arg_slots.len() - 1 - i) as i32) * 8;
                            self.emit_store_local(off, R11);
                            // The direct-call arms' channel verbatim: a contiguous
                            // frame range written before the CALL and read after
                            // it, so each reference in it is named by this call's
                            // safepoint map and published on the shadow stack.
                            if direct_call_arg_maps_enabled()
                                && arg_oops.get(i).copied().unwrap_or(false)
                            {
                                self.pending_staged_arg_oops.push(off);
                            }
                        }
                    ...
                    let guard_skip_patch =
                        if self.helpers.self_call_stack_guard != 0 && self.needs_heap {
                            ...
                            self.emit_load_local(ARG_REGS[0], self.heap_local_offset);
                            // (vm_ptr) -> 0 | i64::MIN. One register: the guard reads the
                            // native stack pointer itself rather than being told it.
                            cratonvm_jit_api::assert_helper_call_shape!(
                    ...
                    // Round 9 wave 2 (call overhead): on the inline-TLS
                    // frame-record shape, republish ONLY the RBP half. The
                    // callee of this CALL is this very buffer's entry
                    // (`patch_self_calls`), whose prologue published THIS
                    // compile's `compile_id` as the identity half; every
                    // later writer inside the callee (its own post-call
                    // republishes, the Rust dispatch brackets'
                    // `top_cm_id_mirror_write` restore) writes that same
                    // id back before control returns here. So the identity
                    // store `emit_post_call_rbp_republish` would add is a
                    // byte-for-byte no-op (see
                    // `docs/internal/performance/c2-fib-per-call-budget-20260912.md`
                    // §3). The helper-call fallback shape is unchanged.
                    ...
                    self.emit_stack_arg_cleanup(total_sub);
                    // Give the CALLEE's own exception table one last look before
                    // the sentinel below is treated as THIS frame's unwind.
                    //
                    // This is the fix for `R10SelfRecCatch`. The callee here is
                    // this same method one activation down, and it returns
                    // `i64::MIN` both for a trap inside itself and for a rethrow
                    // out of ITS nested calls — exactly what every other raw-CALL
                    // route in this backend services (`emit_inline_direct_call`,
                    // the two `invokestatic`/instance direct arms, the retire
                    // cell). This arm never did, because the planner gives a
                    // self-call site no `invoke_info` row; `self_call_invoke_info`
                    // is the channel that carries one anyway.
                    //
                    // Without it, `emit_post_invoke_exception_check` below is the
                    // only consumer, and at a bci with no precise exceptional
                    // frame it is the SHARED SENTINEL EXIT: it stamps this
                    // frame's own throw bci over the callee's and returns the
                    // sentinel through the epilogue. Correct when the caller is
                    // the interpreter, which then routes through this method's
                    // table — but the caller of a self-recursive `CALL` is
                    // compiled code, which repeats the same overwrite. So the
                    // exception skipped every compiled activation's `catch` and
                    // was first resolved at the topmost compiled frame.
                    //
                    // Deliberately BEFORE the stack-guard merge point: on the
                    // guard's overflow edge the callee never ran, so there is no
                    // callee stash to service and the pending `StackOverflowError`
                    // is genuinely this frame's. That edge keeps its documented
                    // route straight into the sentinel check.
                    if let (info, Some(base)) = (self.self_call_invoke_info, self_service_args_base)
                    {
                        self.emit_inline_callee_deopt_check(
                            info as *const crate::JitInvokeInfo, // Cast: staged address
                            arg_slots.len(),
                            base,
                        );
                    }
                    ...
                    // and past the service above for the reason given there.
                    ...
                    // the method's own, and `method_key`
                    // ("Class.name:descriptor") carries it. This used to pass
                    // `b'I'` on the claim that single-pass does not have the
                    // descriptor — while the void-push fix eleven lines below
                    // was already reading it out of `method_key`. `b'I'` is
                    // the plain `CMP; JE` arm, so a self-recursive
                    // `long`/`double` method that legitimately returned
                    // `Long.MIN_VALUE` — or `-0.0`, whose bits ARE `i64::MIN`
                    // — was taken for a throw. The `J`/`D`/`F` arm
                    // disambiguates with the out-of-band `dispatch_threw`
                    // peek on the (cold) sentinel-equal edge only. An empty
                    // key (the legacy test wrapper) keeps `b'I'`.
                    let self_ret_ty = self
                        .method_key
                        .rfind(')')
                        .and_then(|i| self.method_key.as_bytes().get(i + 1))
                        .copied();
                    let sentinel_check_ty = match self_ret_ty {
                        Some(t @ (b'J' | b'D' | b'F')) => t,
                        _ => b'I',
                    };
                    self.emit_post_invoke_exception_check(sentinel_check_ty);
                    // Hand the service range back BEFORE the result is pushed.
                    //
                    // `reserve_direct_call_service_slots` parks its copy ABOVE
                    // the argument slots (it must: those are still live sources
                    // for `emit_stack_arg_setup`), which leaves the cursor n
                    // slots past the pre-pop top. Pushing the return value from
                    // there parks it above its semantic operand-stack depth and
                    // every later push in this block inherits the shift — self-
                    // consistent until the first branch target, whose depth is
                    // re-established from the bytecode, after which writer and
                    // reader address different slots. That is the ECJ
                    // `OperandStack.pop` miscompile the invokestatic direct arm
                    // documents at length; this arm now reserves too, so it
                    // needs the same reclaim. Safe: the range's only consumer is
                    // the `emit_inline_callee_deopt_check` emitted just above.
                    //
                    // Unconditional — with no reservation the cursor is already
                    // exactly `post_pop_spill`, so this is a no-op on that path.
                    self.next_spill_offset = post_pop_spill;
                    ...
                    // self-recursive methods. (`self_ret_ty` is read out of
                    // `method_key` just above, for the sentinel check.)
    ...
    fn walk_invoke_instance(
        ...
        match op {
            ...
            0xb6 | 0xb7 | 0xb9 => {
                ...
                // Round 9 wave 10 (`call10`,
                // `perf-private-instance-self-recursion-dispatches-every-call-FIXED-20260918.md`):
                // a statically bound INSTANCE self-call, planned as a
                // `JIT_SELF_CALL_GUARD` marker row (and no `JitInvokeInfo`) by
                // `lib.rs`. JVMS 6.5 first: a null receiver raises NPE AT THE
                // INVOKE -- the same precise check a spliced `invokespecial`
                // body gets, emitted while the receiver is still on the
                // operand stack. Then the site is exactly a static self-call
                // whose first argument is the receiver (this method's own
                // entry ABI: context, `this`, parameters), so it is lowered by
                // `walk_invokestatic`'s self-recursive arm: stack guard, raw
                // `CALL` to offset 0, sentinel check, own return type.
                let self_call_row = self.direct_calls_idx.get(&pc).is_some_and(|&i| {
                    self.direct_calls[i].1.guard_class_id == crate::JIT_SELF_CALL_GUARD
                });
                if self_call_row {
                    let n = self.num_params;
                    if op == 0xb9
                    ...
                    {
                        // A planner/emitter disagreement: the static arm would
                        // take an info or an inline site instead of the self
                        // call, or pop the wrong operands.
                        self.fail("singlepass-codegen/instance-self-call-shape");
                        return WalkStep::Return(false);
                    }
                    self.emit_spliced_receiver_null_check(false, false, n);
                    return self.walk_invokestatic(
                        _code,
                        _code_len,
                        0xb8,
                        pc,
                ...
                if op == 0xb7 && self.inline_sites.contains_key(&pc) {
                    ...
                    // The direct-call arm below learned this the hard way
                    // (`return 3;` / `return helper();` callees returned
                    // normally on a null receiver — see its comment); a
                    // SPLICED body is the same body with the CALL removed,
                    // and had no check at all. Emitted before the splice and
                    // kept if the splice rolls back: the fall-through arm's
                    // own check is then redundant, never wrong.
                    if let Some(site) = self.inline_sites.get(&pc) {
                        let (is_static, is_init, recv_depth) = (
                            site.callee_is_static,
                            site.method_name == "<init>",
                            site.callee_num_args,
                        );
                        self.emit_spliced_receiver_null_check(is_static, is_init, recv_depth);
                    }
                    if self.try_emit_inline(pc) {
                        pc += 3;
                        return WalkStep::Next(pc);
                ...
                // + `inline_guard_variants` are populated TOGETHER, for an
                // admitted plan whose verdict is `InlineVerdict::Monomorphic`,
                // `Bimorphic`, or — since CHA landed (`NOTES-deopt2.md` M5) —
                // `UniqueConcreteMethod`. The third is lowered by exactly this
                // code and deliberately has no arm of its own: its guard is the
                // same exact class-id compare with the same
                // fall-through-to-dispatch miss edge, and only the EVIDENCE
                // behind the class id differs (the loaded hierarchy rather than
                // the receiver profile), which is a planner-side distinction
                // with no lowering consequence. `InlinePlan::is_speculative` is
                // the predicate that decides what lands in the map; keep it and
                // this comment in step. All three require
                // a `class_id_name_resolver` (see `InlineBackendCaps`
                // in jit/src/lib.rs). The VM threads one in unless
                // `CRATONVM_JIT_GUARDED_VIRTUAL_INLINE=0` — **default ON since
                // N8**, where it was default OFF before, so this block used to
                // cost one HashMap probe over an always-empty map and now
                // carries the tree's only devirtualization. Splices the callee
                // body via
                ...
                // Each hit path's `JMP done`, with the operand-stack model it
                // left behind. The join below reconciles that model with the
                // dispatch path's: see `reconcile_guarded_inline_join`.
                let mut guarded_virtual_done_patches: Vec<(usize, Vec<StackSlot>)> = Vec::new();
                if op != 0xb7 {
                    if let Some(variants) = self.inline_guard_variants.get(&pc).cloned() {
                        ...
                        // A planned, budget-charged guarded inline that fails
                        // this test evaporates with no code emitted for it. It
                        // used to do so with no `else`, no counter and no
                        // diagnostic line, so the site still reported as
                        // inlined; see the refusal at the bottom of this block
                        ...
                        if !(uniform && recv_depth >= 1 && self.stack.len() >= recv_depth) {
                            ...
                            if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JITC") {
                                eprintln!(
                                    "[cratonvm-jitc] guarded-virtual refused at pc={pc}: \
                                     uniform={uniform} recv_depth={recv_depth} \
                                     stack_depth={} variants={}",
                                    self.stack.len(),
                                    variants.len(),
                                );
                            }
                        ...
                        if uniform && recv_depth >= 1 && self.stack.len() >= recv_depth {
                            ...
                            // ARRAY-RECEIVER GUARD — the one the MIC/PIC
                            // cascades and the hashed stub already carry, and
                            // this chain did not. A reference array's header
                            // holds its COMPONENT's class id at offset 0, so
                            // a `Foo[]` receiver passes `CMP [RAX], Foo` and
                            // would run `Foo`'s spliced body on an array.
                            // Only a site whose static receiver type an array
                            // can inhabit needs it — `Object` itself and the
                            // two interfaces every array implements; the
                            // verifier keeps arrays away from any other
                            // receiver type — so every other guarded site
                            // stays byte-identical.
                            let recv_may_be_array = self
                                .invoke_info_idx
                                .get(&pc)
                                .map(|&i| self.invoke_info[i].1)
                                .map_or(true, |ip| {
                                    // SAFETY: `invoke_info` pointers are owned
                                    // by the enclosing compile and outlive
                                    // codegen (same as every other deref of
                                    // them in this file).
                                    let info = unsafe { &*ip };
                                    crate::x64::inlining::static_receiver_admits_arrays(
                                        info.class_name,
                                    )
                                });
                            let kind_miss_patch = if recv_may_be_array {
                                ...
                                Some(self.emit_jcc_rel32_patch(0x85)) // JNE
                            } else {
                                None
                            };
                            ...
                            for (guard_class_id, site) in &variants {
                                ...
                                let variant_late_patches = self.late_patch_checkpoint();
                                ...
                                if self.try_emit_inline_site(pc, site) {
                                    ...
                                    let hit_stack = self.stack.clone();
                                    guarded_virtual_done_patches
                                        .push((self.emit_jmp_rel32_patch(), hit_stack));
                                    spliced_any = true;
                                    ...
                                    self.restore_late_patches(variant_late_patches);
                                }
                            }
                            ...
                            if spliced_any {
                                ...
                                if let Some(kind_miss) = kind_miss_patch {
                                ...
                                //
                                // Named, not just rewound. The planner has
                                // already charged `plan.expansion_cost` against
                                // `inline_budget_remaining` and recorded the
                                // invalidation dependencies, so without this
                                // the site reports as inlined in
                                // `InlineDecisionTally` while emitting a plain
                                // dispatch — "did not help" and "could not be
                                // emitted" become the same number. Every other
                                // refusal in this area is named; these two were
                                // not.
                                crate::metrics::note_inline_call_arm(4);
                                if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JITC") {
                                    eprintln!(
                                        "[cratonvm-jitc] guarded-virtual refused at pc={pc}: \
                                         no variant of {} could be spliced",
                                        variants.len(),
                                    );
                                }
                ...
                // A cell row (`JIT_CYCLE_EDGE_CELL_GUARD` / `JIT_RETIRE_CELL_GUARD`)
                // carries a cell ADDRESS in `entry`. The planners place them on
                // `invokestatic` only; one reaching an instance site would be
                // CALLed as code, so refuse the method (round 9 wave 9, `evict9`).
                if direct.is_some_and(|d| {
                    d.4 == crate::JIT_CYCLE_EDGE_CELL_GUARD || d.4 == crate::JIT_RETIRE_CELL_GUARD
                }) {
                    self.fail("singlepass-codegen/cell-row-on-an-instance-site");
                    return WalkStep::Return(false);
                }
                ...
                )) = direct
                {
                            ...
                            || callee_entry == crate::JitIntrinsic::FfmSegmentSetAtIndex.as_entry())
                    {
                        ...
                        match (info_ptr, kind) {
                            (Some(info), Some(kind)) if helper != 0 && !info.is_null() => {
                                ...
                                // The FFM accessors, both reached through this one sequence.
                                // `ffm_segment_get`'s fourth argument is a `*mut i64` out-pointer
                                // and `ffm_segment_set`'s is the value; the macro counts either as
                                // one integer argument register, which is what this site writes.
                                cratonvm_jit_api::assert_helper_call_shape!(
                                    "ffm_segment_get",
                                    int_args = 4,
                                    returns_value = true
                    ...
                    if !intrinsic_handled
                        && (callee_entry == crate::JitIntrinsic::StringBuilderLength.as_entry()
                            ...
                            || callee_entry == crate::JitIntrinsic::StringBuilderCharAt.as_entry()
                            || callee_entry
                                == crate::JitIntrinsic::StringBuilderAppendString.as_entry()
                            || callee_entry
                                == crate::JitIntrinsic::StringBuilderAppendInt.as_entry())
                    {
                        let is_append =
                            callee_entry == crate::JitIntrinsic::StringBuilderAppendChar.as_entry();
                        let is_char_at =
                            callee_entry == crate::JitIntrinsic::StringBuilderCharAt.as_entry();
                        let is_append_string = callee_entry
                            == crate::JitIntrinsic::StringBuilderAppendString.as_entry();
                        let is_append_int =
                            callee_entry == crate::JitIntrinsic::StringBuilderAppendInt.as_entry();
                        let info_ptr = self
                            .invoke_info_idx
                            .get(&pc)
                            .map(|&i| self.invoke_info[i].1);
                        ...
                        // The `append(String)` argument's OWN field layout —
                        // `java/lang/String`, not the builder — read from the
                        // same `self.string_layout` the builder half came
                        // from. Registration already refused this site when
                        // `has_coder` is false, so a `Some` layout here has
                        // one.
                        let outer_string_layout = self.string_layout;
                        match (info_ptr, layout) {
                            (Some(info), Some(b)) if !info.is_null() => {
                                self.flush_scratch_registers();
                                // Operands, deepest first: receiver, then
                                // (append/charAt/appendString/appendInt
                                // only) the one argument.
                                let arg_slot =
                                    if is_append || is_char_at || is_append_string || is_append_int
                                    {
                                        Some(self.pop_stack())
                                    } else {
                                        None
                                ...
                                // Matched rather than `expect`ed: this file
                                // denies production panics
                                // (`hot_files_have_no_production_panics`),
                                // and `(is_append, is_char_at)` versus
                                // `arg_slot.is_some()` are the same fact —
                                // the operand is popped under exactly the
                                // condition one of the first two is true.
                                // The wildcard arm cannot be reached in
                                // practice, but the compile fails safely
                                // (drops to the interpreter) rather than
                                // panicking if it ever were.
                                let done = if is_append {
                                    let Some(ch) = arg_slot else {
                                        self.fail("singlepass-codegen/sb-append-no-arg-slot");
                                        return WalkStep::Return(false);
                                    };
                                    ...
                                    let Some(idx) = arg_slot else {
                                        self.fail("singlepass-codegen/sb-charat-no-arg-slot");
                                        return WalkStep::Return(false);
                                    };
                                    // R9 = the index. `emit_sb_char_at_body`
                                    // reads it and leaves it untouched, so
                                    // loading it once here serves both the
                                    // compact and legacy arm below — same
                                    // shape as `ch` above, one register load
                                    // shared by both layout bodies.
                                    self.load_slot_to_reg(RCX, idx);
                                    self.buf.emit(&[0x49, 0x89, 0xC9]); // MOV R9,RCX
                                    ...
                                    self.patch_rel32_to_here(joined);
                                    // `charAt` returns a char derived from
                                    // the receiver, already computed into
                                    // RAX by the body — nothing here
                                    // allocates or moves the receiver.
                                    self.emit_jmp_rel32_patch()
                                } else if is_append_string {
                                    let Some(src) = arg_slot else {
                                        self.fail("singlepass-codegen/sb-appendstring-no-arg-slot");
                                        return WalkStep::Return(false);
                                    };
                                    let Some(outer) = outer_string_layout else {
                                        self.fail(
                                            "singlepass-codegen/sb-appendstring-no-outer-layout",
                                        );
                                        return WalkStep::Return(false);
                                    };
                                    // RDX = src String ref; null takes the
                                    // call — `append(String)` on a null
                                    // argument appends the four characters
                                    // "null", which this path does not
                                    // reproduce.
                                    self.load_slot_to_reg(RDX, src);
                                    self.emit_test_r64_r64(RDX);
                                    decline.push(self.emit_jcc_rel32_patch(0x84)); // JZ

                                    // R8 = src.value ptr (the generic
                                    // String-layout emitter branches on
                                    // RDX's own compact/legacy bit
                                    // internally, so this needs no outer
                                    // branch of its own — unlike the
                                    // builder reads below, which are raw
                                    // per-offset loads).
                                    self.emit_load_string_value_ptr(
                                        R8,
                                        RDX,
                                        outer.value_compact_offset,
                                        outer.value_legacy_offset,
                                    );
                                    ...
                                    self.emit_load_string_i32_field(
                                        R10,
                                        RDX,
                                        outer.coder_compact_offset,
                                        outer.coder_compact_is_byte,
                                        outer.coder_legacy_offset,
                                    );

                                    // R9D = src.value.length (bytes) ->
                                    // src.count (chars). Branch-on-the-bit
                                    // rather than a variable `SHR ..,CL`:
                                    // coder is always exactly 0 or 1, and
                                    // this keeps RCX free for the rest of
                                    // this site instead of tying it up as
                                    // the one register a variable shift
                                    // count must live in.
                                    self.buf.emit(&[
                                        0x45,
                                        0x8B,
                                        0x48,
                                        cratonvm_types::ARRAY_LENGTH_OFFSET as u8,
                                    ]); // MOV R9D,[R8+ARRAY_LENGTH_OFFSET]
                                    self.buf.emit(&[0x45, 0x85, 0xD2]); // TEST R10D,R10D
                                    let src_len_done = self.emit_jcc_rel32_patch(0x84); // JZ
                                    self.buf.emit(&[0x41, 0xD1, 0xE9]); // SHR R9D,1
                                    self.patch_rel32_to_here(src_len_done);

                                    // One compact/legacy branch for the
                                    // whole body, same shape as `append`
                                    // and `charAt` above.
                                    self.emit_test_mem8_imm8(
                                        RAX,
                                        cratonvm_types::GC_FLAGS_BYTE_OFFSET as i32,
                                    ...
                                    let legacy = self.emit_jcc_rel32_patch(0x84); // JZ
                                    self.emit_sb_append_string_body(
                                        b.count_compact_offset,
                                        b.value_compact_offset,
                                        b.coder_compact_offset,
                                        b.coder_compact_is_byte,
                                        true, // src is a real String's heap byte[]
                                        &mut decline,
                                    );
                                    let joined = self.emit_jmp_rel32_patch();
                                    self.patch_rel32_to_here(legacy);
                                    ...
                                    self.patch_rel32_to_here(joined);
                                    // `append` returns its receiver, still
                                    // in RAX and never moved: this path
                                    // allocates nothing (a same-coder copy
                                    // or a LATIN1-into-UTF16 widen, both
                                    // in place — never a growth or a
                                    // narrowing inflate, which decline).
                                    self.emit_jmp_rel32_patch()
                                } else if is_append_int {
                                    let Some(val) = arg_slot else {
                                        self.fail("singlepass-codegen/sb-appendint-no-arg-slot");
                                        return WalkStep::Return(false);
                                    };
                                    // A small stack scratch buffer for the
                                    // decimal digits — 2 slots (16 bytes) is
                                    // more than the 10 digits a 32-bit int
                                    // ever needs. Not an argument buffer for
                                    // any call (the fast path never calls
                                    // anything): reusing `HelperArgs`'
                                    // accounting is what keeps this bump
                                    // from colliding with the ACTUAL
                                    // helper-args reservation below, since
                                    // both share one cursor.
                                    //
                                    // Reserved ABOVE the popped operands'
                                    // own frame homes, the way the arraycopy
                                    // intrinsic places its scratch. The two
                                    // pops just rewound the cursor over those
                                    // homes, so a reservation taken at the
                                    // cursor IS them — and the decline edge
                                    // below re-reads `recv_slot`/`val` from
                                    // their homes AFTER the digit loop has
                                    // written the buffer (a capacity miss
                                    // inside the append body declines there),
                                    // so it would have dispatched on digit
                                    // bytes as a receiver.
                                    let post_pop_spill = self.next_spill_offset;
                                    let mut scratch_floor = post_pop_spill;
                                    for slot in [recv_slot, val] {
                                        if let StackSlot::Frame(off) = slot {
                                            scratch_floor =
                                                scratch_floor.max(off.saturating_add(8));
                                        }
                                    }
                                    self.next_spill_offset = scratch_floor;
                                    let Some(scratch_base) =
                                        self.reserve_spill_slots(2, SpillReason::HelperArgs)
                                    else {
                                        self.fail(
                                            "singlepass-codegen/sb-appendint-spill-exhausted",
                                        );
                                        return WalkStep::Return(false);
                                    };
                                    // One past the HIGHEST byte of the two
                                    // reserved words. A word at offset `o`
                                    // occupies `[rbp - o, rbp - o + 8)`, so
                                    // the pair `scratch_base`, `scratch_base
                                    // + 8` spans `[rbp - scratch_base - 8,
                                    // rbp - scratch_base + 8)` and its end is
                                    // the frame address of offset
                                    // `scratch_base - 8`. This used to be
                                    // `scratch_base + 16` — 24 bytes below
                                    // that end and 8 below the reservation's
                                    // bottom — so every digit landed in
                                    // UNRESERVED words beneath the cursor:
                                    // an open inline scope's locals, or, with
                                    // the spill area nearly full, the
                                    // callee-saved register save area that
                                    // sits directly past it
                                    // (`callee_saved_base`).
                                    let digits_end = scratch_base - 8;

                                    // Stash the receiver: RAX is the DIV
                                    // dividend/quotient register, and the
                                    // digit loop below runs one DIV per
                                    // digit. Without this, RAX comes out of
                                    // the loop holding the LAST quotient —
                                    // always 0, since the loop keeps
                                    // dividing until the quotient IS 0 —
                                    // and `emit_sb_append_string_body` below
                                    // would then dereference a null
                                    // receiver. (Measured: this was a real
                                    // bug, not a hypothetical one — SIGSEGV
                                    // addr=0xf, rax=0x0 at the crash, `0xf`
                                    // being one of the builder's own small
                                    // field offsets applied to a null base.)
                                    self.buf.emit(&[0x49, 0x89, 0xC0]); // MOV R8,RAX

                                    // ECX = the int. Negative declines: the
                                    // digit loop below is UNSIGNED divide,
                                    // so a negative value would read as its
                                    // huge unsigned reinterpretation rather
                                    // than as a negative number, and it is
                                    // the ordinary call's job to print the
                                    // sign anyway.
                                    self.load_slot_to_reg(RCX, val);
                                    self.buf.emit(&[0x85, 0xC9]); // TEST ECX,ECX
                                    decline.push(self.emit_jcc_rel32_patch(0x88)); // JS

                                    // R11 = write cursor, one past the last
                                    // reserved byte; decremented before
                                    // every store so it always points at
                                    // the last-written (i.e. so-far-most-
                                    // significant) digit.
                                    self.emit_lea_frame_slot(R11, digits_end);
                                    self.emit_mov_imm32_sx(R9, 10); // divisor

                                    let digit_loop = self.buf.pos();
                                    self.buf.emit(&[0x49, 0xFF, 0xCB]); // DEC R11
                                    self.buf.emit(&[0x89, 0xC8]); // MOV EAX,ECX
                                    self.emit_xor_reg_self(RDX);
                                    self.buf.emit(&[0x41, 0xF7, 0xF1]); // DIV R9D  (EAX=q, EDX=r)
                                    self.buf.emit(&[0x83, 0xC2, 0x30]); // ADD EDX,0x30 ('0')
                                    self.buf.emit(&[0x41, 0x88, 0x13]); // MOV [R11],DL
                                    self.buf.emit(&[0x89, 0xC1]); // MOV ECX,EAX
                                    self.buf.emit(&[0x85, 0xC9]); // TEST ECX,ECX
                                    let back = self.emit_jcc_rel32_patch(0x85); // JNZ
                                    let rel = digit_loop as i32 - (back as i32 + 4); // Cast: rel32 displacement
                                    self.buf.try_patch_i32(back, rel).ok(); // on Err try_patch_i32 set buf.overflowed; compile bails

                                    // RAX = buffer end, recomputed (cheaper
                                    // than keeping it alive across the loop
                                    // above, which needed EAX itself for
                                    // every division). R9D = digit count =
                                    // buffer_end - R11 (both frame
                                    // addresses a few bytes apart, so the
                                    // 32-bit truncation is exact).
                                    self.emit_lea_frame_slot(RAX, digits_end);
                                    self.buf.emit(&[0x49, 0x89, 0xC1]); // MOV R9,RAX
                                    self.buf.emit(&[0x4D, 0x29, 0xD9]); // SUB R9,R11
                                                                        // RAX = the receiver, restored.
                                    self.buf.emit(&[0x4C, 0x89, 0xC0]); // MOV RAX,R8
                                                                        // R8 = the first (most-significant) digit.
                                    self.buf.emit(&[0x4D, 0x89, 0xD8]); // MOV R8,R11
                                                                        // R10D = 0 — the scratch buffer's
                                                                        // "coder" is always LATIN1: every
                                                                        // digit and the sign are pure ASCII.
                                    self.emit_xor_reg_self(R10);

                                    // From here it is exactly `append(String)`'s
                                    // tail: R8/R9D/R10D already hold what
                                    // that body expects a source String's
                                    // value ptr/count/coder to be, and it
                                    // never re-reads the source object
                                    // itself, so a scratch buffer serves
                                    // just as well as a real `String`.
                                    self.emit_test_mem8_imm8(
                                        RAX,
                                        cratonvm_types::GC_FLAGS_BYTE_OFFSET as i32,
                                        cratonvm_types::GC_FLAG_COMPACT,
                                    ...
                                    self.emit_sb_append_string_body(
                                        ...
                                        b.coder_compact_offset,
                                        b.coder_compact_is_byte,
                                        false, // src is the stack scratch buffer, no header
                                        &mut decline,
                                    );
                                    let joined = self.emit_jmp_rel32_patch();
                                    self.patch_rel32_to_here(legacy);
                                ...
                                let nargs = if arg_slot.is_some() { 2 } else { 1 };
                                let args_base = match self
                                    .reserve_spill_slots(nargs, SpillReason::HelperArgs)
                                {
                                    Some(base) => base,
                                ...
                                self.emit_call_absolute(self.helpers.invoke_dispatch);
                                self.emit_oop_map_for_safepoint();
                                // `append` returns a reference; `length`
                                // and `charAt` return an int-category value
                                // (a `char` is widened to `I` on the
                                // operand stack). None of the three shares
                                // the `i64::MIN` pending-exception sentinel
                    ...
                    if let Some(layout) = self.string_layout {
                        ...
                        if let Some(kind) = acc {
                            ...
                            if kind == 3 {
                                ...
                                //   R11 = char_count
                                //   R10 = coder
                                if let Some(idx) = index_slot {
                                    self.load_slot_to_reg(R9, idx);
                                }
                                // R11D = value.length (zero-extended) —
                    ...
                    //   * this == null              → deopt (E27-1 N3-obs:
                    //                                the intrinsic bypasses
                    //                                ordinary virtual
                    //                                dispatch, so nothing
                    //                                upstream has null-
                    //                                checked the receiver
                    //                                yet — `compareTo` and
                    //                                `indexOf` already guard
                    //                                this and `equals` must
                    //                                too, or `((String)
                    //                                null).equals(x)`
                    ...
                    if let (Some(layout), true) = (
                        ...
                        // this == null → deopt (re-run in the interpreter,
                        // which raises NPE through ordinary dispatch). Must
                        // come before the class-id check below dereferences
                        // RAX — see the N3-obs note above.
                        self.emit_test_r64_r64(RAX);
                        bail.push(self.emit_jcc_rel32_patch(0x84)); // JZ
                        ...
                        //
                        // An ARRAY argument must be screened out first: a
                        // reference array's header carries its COMPONENT's
                        // class id at offset 0, so `"a".equals(new
                        // String[]{..})` passes the class-id compare below
                        // and the body then decodes the array as a String —
                        // reading a `value` pointer out of array memory.
                        // Java answers `false` for any array, so a non-plain
                        // object takes the result-0 path directly (no deopt:
                        // the answer is known). Same KIND_TAGS test the
                        // MIC/PIC cascades and the inline `checkcast` make.
                        //   CMP BYTE [RDX + KIND_TAGS_BYTE_OFFSET], Object
                        self.buf.emit(&[
                            0x80,
                            0x7A,
                            cratonvm_types::KIND_TAGS_BYTE_OFFSET as u8,
                        ...
                        let other_not_plain = self.emit_jcc_rel32_patch(0x85); // JNE
                        ...
                        // result 0 path (other null / an array / length
                        // mismatch).
                        self.patch_rel32_to_here(other_null);
                        self.patch_rel32_to_here(other_not_plain);
                    ...
                    if let (Some(layout), true) = (
                        ...
                        // THE CONSTANT-NEEDLE SCREEN NEVER REACHED THIS ARM.
                        // The comment above says "the `direct.filter` far
                        // above is what enforces that", and that filter sits
                        // in the INVOKESTATIC ladder — while `indexOf(I)` is
                        // an instance method and arrives here through
                        // `invokevirtual`, unscreened. So a variable needle
                        // was intrinsified too, and the mask below turned
                        // `"abc".indexOf(0x10061)` into a search for 'a'
                        // (answer 0; the JDK answers -1) and `indexOf(-1)`
                        // into a search for U+FFFF.
                        //
                        // A declined site cannot fall back to dispatch from
                        // here — the resolver registered it as an intrinsic,
                        // so it has no `invoke_info` — so a needle this
                        // compile cannot prove is in `0..=0xFFFF` gets a
                        // runtime screen whose miss takes the same
                        // uncommon trap as a null receiver: slow for an
                        // out-of-range needle, and right. A proven-constant
                        // site emits nothing extra.
                        //
                        // Round 9 wave 2: the resolver now screens
                        // (`try_compile_inner` declines the `StringIndexOfChar`
                        // registration unless
                        // `prev_insn_int_const_for_test` proves the needle is
                        // in `0..=0xFFFF`; see
                        // `string-indexof-char-screen-belongs-at-the-resolver-20260918.md`),
                        // so a variable-needle site gets `invoke_info` and never
                        // reaches this arm. This runtime screen is KEPT as the
                        // backstop (defence in depth: two instructions, and
                        // only when the two screens disagree about what a
                        // constant is).
                        if !matches!(prev_insn_int_const(_code, _code_len, pc), Some(0..=0xFFFF)) {
                            // CMP R9D, 0xFFFF (REX.B + 81 /7 id) ; JA bail
                            // (unsigned, so a negative needle bails too).
                            self.buf.emit(&[0x41, 0x81, 0xF9]);
                            self.buf.emit(&0xFFFFu32.to_le_bytes());
                            bail.push(self.emit_jcc_rel32_patch(0x87)); // JA
                        }
                    ...
                    // override, which raises the exact exception.
                    {
                        ...
                        if is_crc32c {
                            ...
                            // from the same base.
                            //
                            // The pins start ABOVE every operand's own frame
                            // home — the arraycopy intrinsic's rule, for the
                            // same reason. The pops rewound the cursor over
                            // those homes, so pins based on the bare cursor
                            // alias them: with a register-homed receiver
                            // (`CalleeSaved`, no frame word) and a
                            // frame-resident array, `s_recv` IS the array's
                            // home, the receiver store overwrites it before
                            // it is read, and `s_a` then pins the CRC32C
                            // object as the "array". The deopt snapshot taken
                            // above also names those original homes and reads
                            // them when a guard bails.
                            let scratch_slots = if is_byte_form { 2 } else { 4 };
                            let mut crc_scratch_base = self.next_spill_offset;
                            for slot in [recv_slot, arr_slot, off_slot, b_or_len_slot] {
                                if let StackSlot::Frame(off) = slot {
                                    crc_scratch_base = crc_scratch_base.max(off.saturating_add(8));
                                }
                            }
                            if !self.spill_range_fits(crc_scratch_base, scratch_slots) {
                                self.fail("singlepass-codegen/intrinsic-pin-spill-exhausted");
                                return WalkStep::Return(false);
                            }
                            let s_recv = crc_scratch_base;
                            let s_a = crc_scratch_base + 8;
                            let s_b = crc_scratch_base + 16;
                            let s_c = crc_scratch_base + 24;
                            self.load_slot_to_reg(RAX, recv_slot);
                    ...
                    // An intrinsic sentinel that no region above emitted.
                    // Several regions decline by leaving `intrinsic_handled`
                    // false — the `Atomic*` / box-unbox arms when their field
                    // layout does not resolve at emission, the String arms
                    // when `string_layout` is absent — each on a comment that
                    // says declining "keeps the plain direct-call path rather
                    // than emitting a CALL to an intrinsic sentinel". The
                    // plain path below IS a `CALL callee_entry`, i.e. to the
                    // sentinel. Refuse the method instead.
                    if !intrinsic_handled && crate::JitIntrinsic::is_sentinel_entry(callee_entry) {
                        self.fail("singlepass-codegen/unhandled-intrinsic-sentinel");
                        return WalkStep::Return(false);
                    }
                    ...
                    if let Some(info) = info_ptr {
                        ...
                        // Round 9 wave 8 (`arraylist8`): an exact-`ArrayList`
                        // `get(int)` / `size()` answered inline as a guarded
                        // PREFIX of this unchanged dispatch. Every decline edge
                        // lands on the first byte of the dispatch code below;
                        // the hit edge lands at the result push at the bottom
                        // of this arm with the answer in RAX. See
                        // `try_emit_arraylist_access_prefix`.
                        let arraylist_hit_patch =
                            self.try_emit_arraylist_access_prefix(pc, info_ref);
                        ...
                        //   jc    .call           ; context ABI already live
                        //   jmp   .noctx          ; (last slot falls through)
                        //   ; --- end per-slot ---
                        // .noctx:                 ; ONE shared tail (r9 wave 2)
                        //   <load context-free ABI: arg_slots[0..n]>
                        // .call:
                        //   call  r11
                        //   <rbp republish> <callee-deopt check>
                        //   jmp   .done
                        // .miss:
                        ...
                        // Empty PIC entries hold `EMPTY_WAY_CLASS_ID`, which
                        // no receiver matches, so an empty slot fails its
                        // CMP and falls through. They used to hold 0, on the
                        // claim that ClassId 0 (`java.lang.Object`) is never
                        // a receiver here. It is one — a plain `new
                        // Object()` and the untyped ClassId(0) synthetic
                        // containers both carry it — and a class-0 receiver
                        // matched an empty way while that way was being
                        // published word-first. See `JitPICSlot::class_ids`.
                        // The MIC's empty slot now holds
                        // `JitMICSlot::EMPTY_CLASS_ID` (round 9 wave 2; see
                        // `mic-empty-slot-class-zero-window-20260918.md`),
                        // which is likewise no receiver's class id, so the
                        // MIC arm's single `CMP EAX,[R10]` below needs no
                        // emptiness test of its own: an empty MIC simply
                        // misses.
                        //
                        ...
                        if let (true, Some(pic)) = (pic_inline, pic_ptr) {
                            ...
                            // rel32 sites of each way's `JC → .call` and
                            // `JMP → .noctx`, patched at the shared tail
                            // emitted after the cascade.
                            let mut pic_call_patches: Vec<usize> =
                                Vec::with_capacity(crate::JIT_PIC_ENTRIES);
                            let mut pic_noctx_patches: Vec<usize> =
                                Vec::with_capacity(crate::JIT_PIC_ENTRIES);
                            ...
                            for i in 0..crate::JIT_PIC_ENTRIES {
                                ...
                                // SHARED POST-CALL TAIL (round 9 wave 2,
                                // `pic-cascade-repeats-the-post-call-tail-per-way`).
                                // Every way used to carry its own no-context
                                // marshalling block, `CALL R11`, innermost-RBP
                                // republish, callee-deopt service check and
                                // `JMP .done` — four copies of a ~100+ byte
                                // tail at every virtual/interface site, three
                                // of them dead at a monomorphic site. The ways
                                // differ only in which ABI shape the tag
                                // selects; the target is already in R11 and the
                                // context ABI is already live in ARG_REGS (the
                                // hoist above), so every way now branches to ONE
                                // `.noctx` / `.call` pair emitted after the
                                // cascade:
                                //
                                //   JC  rel32 → .call    ; tag set: context ABI live
                                //   JMP rel32 → .noctx   ; (the last way falls in)
                                //
                                // The hot path is unchanged in length: a way-0
                                // context hit takes one taken `JC` where it took
                                // a not-taken `JNC` plus a taken `JMP`; a
                                // no-context hit takes one `JMP` where it took a
                                // taken `JNC`. INVARIANT (unchanged): nothing
                                // between the entry-word capture above and the
                                // shared `CALL R11` writes R11 — the path is
                                // `BTR`, a jump, and (no-context only) the
                                // ARG_REGS reloads.
                                //
                                // JC rel32 → .call (6 bytes: 0F 82 + i32).
                                self.buf.emit(&[0x0F, 0x82, 0x00, 0x00, 0x00, 0x00]);
                                pic_call_patches.push(self.buf.pos() - 4);
                                if i + 1 < crate::JIT_PIC_ENTRIES {
                                    // JMP rel32 → .noctx (5 bytes: E9 + i32).
                                    self.buf.emit(&[0xE9, 0x00, 0x00, 0x00, 0x00]);
                                    pic_noctx_patches.push(self.buf.pos() - 4);
                                }
                                // The last way falls through into `.noctx`.
                            }

                            // .noctx: Java args start at ARG_REGS[0].
                            let noctx_off = self.buf.pos();
                            for patch in &pic_noctx_patches {
                                // Widening: usize offset -> i64 (rel math)
                                let rel = (noctx_off as i64) - (*patch as i64 + 4);
                                debug_assert!((i32::MIN as i64..=i32::MAX as i64).contains(&rel));
                                // Cast: rel32 displacement; on Err the buffer
                                // is marked overflowed and the compile bails.
                            ...
                            // .call — the context ABI arrives here with the
                            // hoisted ARG_REGS still live (the cascade touched
                            // only RAX, R10 and R11 since the hoist).
                            let call_off = self.buf.pos();
                            for patch in &pic_call_patches {
                                // Widening: usize offset -> i64 (rel math)
                                let rel = (call_off as i64) - (*patch as i64 + 4);
                                debug_assert!((i32::MIN as i64..=i32::MAX as i64).contains(&rel));
                                // Cast: rel32 displacement (see above).
                                self.buf.try_patch_i32(*patch, rel as i32).ok();
                            }
                            ...
                            // R11 by the way that hit (a caller-saved
                            // scratch reg that is NOT in ARG_REGS /
                            // SCRATCH_REGS / LOCAL_REGS and is clobbered
                            // by the call anyway) and R10 is not read
                            // again.
                            //
                            ...
                            // JMP rel32 → .done (5 bytes: E9 + i32). The
                            // hashed stub and the slow path sit between
                            // here and `.done`.
                            self.buf.emit(&[0xE9, 0x00, 0x00, 0x00, 0x00]);
                        ...
                        } else if let (true, Some(mic)) = (mic_inline, mic_ptr) {
                        ...
                        if let Some(mic) = mic_ptr {
                            ...
                            // ABI-3 — the ONE hand-written helper call site that must not use
                            // `assert_helper_call_shape!`. `invoke_virtual_mic` takes six integer
                            // words; the macro asserts `win64_stack_args() == 0` because every
                            // other guarded site marshals into the register file alone, and this
                            // sequence writes arguments five and six itself — `[RSP+32]`/`[RSP+40]`
                            // on Win64, R8/R9 on SysV. The arity is instead pinned in the shape
                            // this site can honestly make, which is that it overflows by exactly
                            // the two words it hand-writes.
                            const _: () = {
                                let sig = cratonvm_jit_api::helpers_abi::helper_sig_of(
                                    "invoke_virtual_mic",
                                );
                                assert!(
                                    sig.arity == 6 && sig.float_args == 0 && sig.returns_value,
                                    "`invoke_virtual_mic` no longer takes six integer words; \
                                     the two-word hand-written overflow above is written \
                                     against that arity and nothing else would notice",
                                );
                                assert!(
                                    sig.win64_stack_args() == 2,
                                    "`invoke_virtual_mic` no longer overflows Win64's four \
                                     argument registers by exactly two words; this site writes \
                                     `[RSP+32]` and `[RSP+40]` by hand and would now be \
                                     writing the wrong slots",
                                );
                            };
                            self.emit_call_absolute(self.helpers.invoke_virtual_mic);
                        } else {
                            // Plain dispatch without MIC
                            // (vm_ptr, info_ptr, args_ptr, num_args) — the no-MIC arm, which
                            // DOES fit the register file and so can use the macro.
                            cratonvm_jit_api::assert_helper_call_shape!(
                                "invoke_dispatch",
                                int_args = 4,
                                returns_value = true
                        ...
                        // ARRAYLIST_ACCESS hit edge: the inline answer is in
                        // RAX, exactly where the dispatch leaves its result, so
                        // the push below serves both paths. The hit path made
                        // no call, so it owes neither the safepoint map / reload
                        // nor the exception check above, and skips them — the
                        // shape of the guarded-virtual inline's hit edge.
                        if let Some(hit) = arraylist_hit_patch {
                            self.patch_rel32_to_here(hit);
                        }
                        ...
                        // Neither a direct target nor dispatch metadata. This
                        // arm used to fall out here having emitted NOTHING —
                        // no call, and no pop of the receiver and arguments —
                        // so the simulated operand stack kept them and every
                        // later bytecode in the method was compiled against
                        // the wrong stack. The invokestatic arm's matching
                        // case is a self-recursive call; an instance invoke
                        // has no such meaning, so refuse the method rather
                        // than emit it mis-modelled.
                        self.fail("singlepass-codegen/instance-invoke-without-target");
                        return WalkStep::Return(false);
                    }
                }
                if !guarded_virtual_done_patches.is_empty() {
                    self.reconcile_guarded_inline_join(guarded_virtual_done_patches);
                }
                if op == 0xb9 {
                    pc += 5;
                } else {
    ...
    /// ARRAYLIST_ACCESS (round 9 wave 8, `arraylist8`): emit the inline
    /// `ArrayList.get(int)` / `size()` fast path as a guarded PREFIX of the
    /// site's ordinary virtual/interface dispatch, and return the hit edge's
    /// `JMP rel32` patch. `None` emits nothing.
    ///
    /// Page: `perf-collection-natives-cost-a-generic-native-dispatch-per-call`.
    ///
    /// # Contract with the caller
    ///
    /// Called at the top of the dispatch arm, after `info_ref`/`n` are known and
    /// before a single byte of the dispatch is emitted. It only PEEKS at the
    /// operand stack -- the receiver and the index stay where the dispatch will
    /// pop them -- and it only writes RAX, RCX, RDX, R8, R9 (and R11 inside the
    /// layout epoch guard's fallback form), all of which the dispatch below
    /// overwrites before reading: after the walk's `flush_scratch_registers` no
    /// operand lives in any of them. Every decline edge is patched to the byte
    /// after the prefix, i.e. the first byte of the UNCHANGED dispatch, which
    /// then runs the registered native exactly as today (MIC/PIC and the
    /// site-cached native leaf included). The hit edge must be patched at the
    /// dispatch arm's result push, with the answer in RAX.
    ///
    /// # What the fast path answers
    ///
    /// Exactly what `native_al_get` / `native_al_size`'s wave-3 fast path
    /// (`al_fast_state` plus an in-range index) answers, and nothing else:
    ///
    /// ```text
    ///   [layout epoch guard]                           ; JNE decline
    ///   RAX = recv ; TEST RAX,RAX ; JZ decline         ; the NPE is the native's
    ///   CMP BYTE  [RAX+KIND_TAGS], Object ; JNE decline ; an array carries its
    ///                                                   ; COMPONENT's id at +0
    ///   CMP DWORD [RAX+class_id], ArrayList ; JNE decline ; exact: no subclass
    ///   TEST BYTE [RAX+GC_FLAGS], COMPACT ; JZ legacy
    ///     R8D = [RAX+size_c] ; RDX = [RAX+data_c] ; JMP have
    ///   legacy:
    ///     CMP DWORD [RAX+NUM_SLOTS], min ; JB decline   ; both cells present
    ///     CMP DWORD [RAX+size_tag], Int  ; JNE decline
    ///     CMP DWORD [RAX+data_tag], Object ; JNE decline
    ///     R8D = [RAX+size_l] ; RDX = [RAX+data_l]
    ///   have:
    ///   TEST R8D,R8D ; JS decline                       ; size >= 0
    ///   TEST RDX,RDX ; JZ decline                       ; elementData != null
    ///   CMP BYTE [RDX+KIND_TAGS], Array|Reference ; JNE decline
    ///   R9D = elementData.length
    ///   get:  ECX = index ; CMP ECX,R8D ; JAE decline ; CMP ECX,R9D ; JAE decline
    ///   CMP R8D,R9D ; JAE nomark                        ; `view_source_in`:
    ///   CMP QWORD [RDX+R9*8+DATA-8],0 ; JNE decline     ; a trailing non-null slot
    ///   nomark:
    ///   get:  RAX = [RDX+RCX*8+DATA]    size: MOVSXD RAX,R8D
    ///   JMP hit
    ///   decline:  <the dispatch>
    /// ```
    ///
    /// The index compares are unsigned, so a negative index declines too, and
    /// the IndexOutOfBoundsException is the native's -- this path never builds
    /// an exception and never deopts: an out-of-range index is ordinary control
    /// flow in real code, and a deopt would re-run the method in the
    /// interpreter every time.
    ///
    /// The reference loads are raw 8-byte loads, the representation the inline
    /// `aaload` reads; `ArrayListFieldLayout::new` refuses narrow oops and an
    /// armed ZGC read barrier, and this re-checks the barrier at emission, the
    /// refusal the inline `getfield` takes.
    ///
    /// # Why a prefix and not a `JitDirectCall` sentinel
    ///
    /// A sentinel must be registered at BOTH compile doors, with a
    /// `JitInvokeInfo` minted for its decline edge, or the method drops to the
    /// interpreter (`string_intrinsic_declines_to_a_call`'s history), and it
    /// would take the site off its MIC/PIC. As a prefix the site keeps its
    /// ordinary registration and its inline caches, the optimizing tier (which
    /// never runs this emitter) is untouched, and there is nothing for
    /// registration and emission to disagree about: every refusal is a `None`
    /// here, which emits the plain dispatch.
    fn try_emit_arraylist_access_prefix(
        &mut self,
        pc: usize,
        info: &crate::JitInvokeInfo,
    ) -> Option<usize> {
        let layout = self.string_layout.and_then(|l| l.array_list)?;
        let (entry, num_params, _ret, guard) = crate::try_resolve_arraylist_intrinsic(
            info.class_name,
            info.method_name,
            info.descriptor,
            Some(layout),
        )?;
        // Virtual (0) and interface (2) only: `invokespecial` of `get` names a
        // statically bound body, which is not the question this answers.
        if !matches!(info.invoke_kind, 0 | 2) || guard == 0 {
            return None;
        }
        if crate::x64::zgc_read_barrier_blocks_inline_fields() {
            return None;
        }
        let is_get = entry == crate::JitIntrinsic::ArrayListGet.as_entry();
        let depth = num_params + 1;
        if info.num_jit_args != depth || self.stack.len() < depth {
            return None;
        }
        let recv_slot = *self.stack.get(self.stack.len() - depth)?;
        let index_slot = if is_get {
            Some(*self.stack.last()?)
        } else {
            None
        };
        if matches!(recv_slot, StackSlot::Xmm(..)) || matches!(index_slot, Some(StackSlot::Xmm(..)))
        {
            return None;
        }
        // The prefix writes R8/R9, the deferred operand cache's registers. The
        // walk flushed them before this arm, so this never fires; it is here so
        // a future caller that did not flush gets the plain dispatch rather
        // than a clobbered operand.
        if self
            .stack
            .iter()
            .any(|slot| matches!(slot, StackSlot::Scratch(..)))
        {
            return None;
        }
        if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JITC") {
            eprintln!(
                "[cratonvm-jitc] arraylist-intrinsic {} {}.{}{} @pc={pc} guard_class_id={guard}",
                self.method_label, info.class_name, info.method_name, info.descriptor,
            );
        }

        let mut decline: Vec<usize> = Vec::new();

        // The baked compact offsets are a compile-time claim about ArrayList's
        // layout; a layout REPLACEMENT bumps the process epoch and this site
        // then declines for good. Emitted first: its fallback form clobbers
        // RCX and R11.
        if jit_sp_field_layout_guard_enabled() {
            if let Some(p) = self.emit_layout_epoch_guard() {
                decline.push(p);
            }
        }

        // RAX = receiver; null declines (the native reports the NPE).
        self.load_slot_to_reg(RAX, recv_slot);
        self.emit_test_r64_r64(RAX);
        decline.push(self.emit_jcc_rel32_patch(0x84)); // JZ
                                                       // A plain object only: a reference array's header carries its
                                                       // COMPONENT's class id at +0, so an `ArrayList[]` would pass the id
                                                       // compare below.
        self.emit_cmp_mem8_imm8(
            RAX,
            cratonvm_types::KIND_TAGS_BYTE_OFFSET as i32, // Cast: small header constant
            cratonvm_types::ObjectKind::Object as u8,
        );
        decline.push(self.emit_jcc_rel32_patch(0x85)); // JNE
                                                       // CMP DWORD [RAX + class_id_off], ArrayList -- `81 /7 id` with
                                                       // ModRM mod=10 rm=RAX, the form the inline `instanceof` uses.
        ...
        self.buf.emit(&guard.to_le_bytes());
        decline.push(self.emit_jcc_rel32_patch(0x85)); // JNE

        // One compact/legacy branch for both fields.
        self.emit_test_mem8_imm8(
            RAX,
            cratonvm_types::GC_FLAGS_BYTE_OFFSET as i32, // Cast: small header constant
            cratonvm_types::GC_FLAG_COMPACT,
        );
        let legacy = self.emit_jcc_rel32_patch(0x84); // JZ -> legacy
                                                      // COMPACT: `size` is a bare 4-byte int, `elementData` a bare pointer.
        self.emit_mov_r32_mem_disp32(R8, RAX, layout.size_compact_offset);
        self.emit_mov_r64_mem_disp32(RDX, RAX, layout.data_compact_offset);
        let have = self.emit_jmp_rel32_patch();

        // LEGACY: 16-byte `Value` cells. Both cells must exist, and each must
        // carry the tag its payload is read as.
        self.patch_rel32_to_here(legacy);
        // CMP DWORD [RAX + NUM_SLOTS_OFFSET], min_num_slots ; JB decline
        self.buf.emit(&[0x81, 0xB8]);
        self.buf
            .emit(&(cratonvm_types::NUM_SLOTS_OFFSET as i32).to_le_bytes()); // Cast: small header constant
        self.buf.emit(&layout.min_num_slots.to_le_bytes());
        decline.push(self.emit_jcc_rel32_patch(0x82)); // JB
                                                       // CMP DWORD [RAX + size_tag], 0 (`Value::Int`) ; JNE decline
        self.buf.emit(&[0x81, 0xB8]);
        self.buf.emit(&layout.size_legacy_tag_offset.to_le_bytes());
        self.buf.emit(&0u32.to_le_bytes());
        decline.push(self.emit_jcc_rel32_patch(0x85)); // JNE
                                                       // CMP DWORD [RAX + data_tag], FIELD_CELL_TAG_OBJECT ; JNE decline
        self.buf.emit(&[0x81, 0xB8]);
        self.buf.emit(&layout.data_legacy_tag_offset.to_le_bytes());
        self.buf
            .emit(&cratonvm_types::FIELD_CELL_TAG_OBJECT.to_le_bytes());
        decline.push(self.emit_jcc_rel32_patch(0x85)); // JNE
        self.emit_mov_r32_mem_disp32(R8, RAX, layout.size_legacy_offset);
        self.emit_mov_r64_mem_disp32(RDX, RAX, layout.data_legacy_offset);

        self.patch_rel32_to_here(have);
        // TEST R8D,R8D ; JS decline -- `al_fast_state` requires size >= 0.
        self.buf.emit(&[0x45, 0x85, 0xC0]);
        decline.push(self.emit_jcc_rel32_patch(0x88)); // JS
                                                       // elementData must be a live REFERENCE array.
        self.emit_test_r64_r64(RDX);
        decline.push(self.emit_jcc_rel32_patch(0x84)); // JZ
        let ref_array_tags = (cratonvm_types::ObjectKind::Array as u8)
        ...
        decline.push(self.emit_jcc_rel32_patch(0x85)); // JNE
                                                       // R9D = elementData.length (zero-extends into R9).
        self.emit_mov_r32_mem_disp32(
            R9,
            RDX,
            cratonvm_types::ARRAY_LENGTH_OFFSET as i32, // Cast: small header constant
        );
        if let Some(idx) = index_slot {
            self.load_slot_to_reg(RCX, idx);
            // MOV ECX,ECX: the element load below scales the FULL register,
            // so clear whatever the upper half of the int slot holds.
            self.buf.emit(&[0x89, 0xC9]);
            // Unsigned: a negative index is a huge one and declines.
            self.emit_cmp_r32_r32(RCX, R8); // index vs size
            decline.push(self.emit_jcc_rel32_patch(0x83)); // JAE
            self.emit_cmp_r32_r32(RCX, R9); // index vs length: never an unchecked read
            decline.push(self.emit_jcc_rel32_patch(0x83)); // JAE
        }
        // `view_source_in`, verbatim: with spare capacity, a non-null LAST slot
        // names a source map this list is a stale view of, which the native
        // must resync first.
        self.emit_cmp_r32_r32(R8, R9); // size vs length
        let no_marker = self.emit_jcc_rel32_patch(0x83); // JAE -> no spare capacity
                                                         // CMP QWORD [RDX + R9*8 + (ARRAY_DATA_OFFSET - 8)], 0
                                                         //   REX.W|REX.X, 83 /7 ib, ModRM mod=01 rm=SIB, SIB *8 idx=R9 base=RDX.
        self.buf.emit(&[
            0x4A,
            0x83,
            0x7C,
            0xCA,
            (cratonvm_types::ARRAY_DATA_OFFSET - cratonvm_types::REF_ELEMENT_SIZE) as u8, // Cast: x86-64 disp8
            0x00,
        ]);
        decline.push(self.emit_jcc_rel32_patch(0x85)); // JNE
        self.patch_rel32_to_here(no_marker);

        if is_get {
            // MOV RAX, [RDX + RCX*8 + ARRAY_DATA_OFFSET]
            self.buf.emit(&[
                0x48,
                0x8B,
                0x44,
                0xCA,
                cratonvm_types::ARRAY_DATA_OFFSET as u8, // Cast: x86-64 disp8
            ]);
        } else {
            // MOVSXD RAX, R8D -- `size()` into the 64-bit operand slot.
            self.buf.emit(&[0x49, 0x63, 0xC0]);
        }
        let hit = self.emit_jmp_rel32_patch();

        // ---- decline: the first byte of the unchanged dispatch -------------
        for p in decline {
            self.patch_rel32_to_here(p);
        }
        // The dispatch below starts at a join of many edges; no register
        // mirrors a frame slot here.
        self.slot_mirror = None;
        Some(hit)
    }

    /// Does any `invokedynamic` site in this method lower to the reason-8
    /// uncommon trap rather than to the bridge call?
    ///
    /// One question, asked of the whole body, by the self-recursive arm in
    /// `walk_invokestatic`: a raw `CALL` to this method's own entry is
    /// unsound only while a trap inside the inner invocation can stash a
    /// frame under this method's identity. It is the negation of
    /// [`indy_site_is_bridged`] over `indy_info`, i.e. the same three terms
    /// the 0xba arm admits a bridged site on, so the two cannot drift.
    ///
    /// `driver.rs` computes the identical predicate for
    /// `CompiledMethod::has_indy_trap`, which is the CALLER-side half of the
    /// same fact; if that copy is ever unified with this one, this is the
    /// function to call.
    pub(super) fn has_trapping_indy_site(&self) -> bool {
        let bridge_entry = self.direct_helpers.indy_bridge;
        self.indy_info
            .iter()
            .any(|(_pc, _arg_slots, ret_type, _tags, bridge_site)| {
                !indy_site_is_bridged(*bridge_site, bridge_entry, *ret_type)
            })
    }
    ...
    fn walk_invokedynamic(
        ...
        match op {
            ...
            0xba => {
                ...
                // the trap.
                let bridge_entry = self.direct_helpers.indy_bridge;
                if indy_site_is_bridged(bridge_site, bridge_entry, ret_type) {
                    if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JITC") {
    ...
    /// Round 9 wave 7 (lane `mutrec7`): lower the CLOSING edge of a
    /// compile-time recursion cycle as a guarded call through its
    /// [`crate::JitCycleEdgeCell`] (see that type for the design and the
    /// soundness argument, and
    /// `perf-mutual-recursion-forces-every-call-through-invoke-dispatch-20260918.md`).
    ///
    /// ```text
    ///   [cmp rsp,[rbp-floor] ; ja .ok]              ; only with a floor slot
    ///   <spill> mov ARG0,[rbp-heap] ; call self_call_stack_guard ; <map>
    ///   test rax,rax ; jnz .exc                     ; i64::MIN = SOE stashed
    /// .ok:
    ///   <pop args ; copy them to the service range> ; = dispatch args buffer
    ///   <direct-entry ABI staging, context in ARG0> ; <spill>
    ///   mov r11,cell ; mov r11,[r11] ; test r11,r11 ; jnz .call
    ///   mov ARG0,[rbp-heap] ; mov ARG1,info ; lea ARG2,[args] ; mov ARG3,n
    ///   mov r11,jit_invoke_dispatch
    /// .call:
    ///   call r11                                    ; one return pc, one map
    ///   <republish rbp> <map> <stack-arg cleanup>
    ///   <sentinel fast skip> <callee-deopt service check>
    /// .exc:
    ///   <post-invoke exception check>
    /// ```
    ///
    /// * The guard runs BEFORE the arguments leave the simulated operand
    ///   stack, so its own safepoint map names them where they are.
    /// * The cell word is loaded ONCE, after every other instruction of the
    ///   staging, and called through the same register with no blocking point
    ///   in between: a thread that saw a body cannot be quiescent until that
    ///   body's frame exists, so the retirement queue cannot free it. A word
    ///   cleared to `0` in the meantime just selects the dispatch helper.
    /// * Both targets return to the same `call r11`, so the post-call sequence
    ///   (and its oop map) is shared. The dispatch helper ignores the staged
    ///   direct-entry registers and the outgoing stack-argument block (its
    ///   shadow space lies inside that block on Win64); its args buffer is the
    ///   service range, laid out exactly as the dispatch arm lays its own out.
    ///   A dispatch that returns the sentinel reaches the callee-deopt service
    ///   check exactly as a direct callee that threw does.
    fn emit_cycle_edge_cell_call(
        &mut self,
        pc: usize,
        cell_addr: usize,
        info: *const crate::JitInvokeInfo,
    ) -> bool {
        // SAFETY: `info` comes from `self.invoke_info`, whose boxes the caller
        // keeps alive for the whole compile.
        let (n, ret_type) = unsafe { ((*info).num_jit_args, (*info).return_type) };
        if crate::deopt_real_enabled() {
            self.snapshot_pre_intrinsic_call(pc, crate::deopt::DeoptReason::ReceiverTypeChanged);
        }

        // 1. Stack-depth guard -- the self-call one, byte for byte.
        let fast_skip = if self.stack_floor_slot_off != 0 {
            self.emit_cmp_r64_rbp_local(RSP, self.stack_floor_slot_off);
            Some(self.emit_jcc_rel32_patch(0x87)) // JA .ok
        } else {
            None
        };
        self.emit_pre_safepoint_spill();
        ...
        self.emit_test_r64_r64(RAX);
        let overflow = self.emit_jcc_rel32_patch(0x85); // JNE .exc
        if let Some(skip) = fast_skip {
            self.patch_rel32_to_here(skip);
        }

        // 2. Arguments: into the service range (the dispatch fallback's args
        // buffer and the deopt service's copy), then the direct-entry ABI.
        let args_frame_top = self.next_spill_offset;
        let (arg_slots, arg_oops) = self.pop_invoke_args(n);
        let post_pop_spill = self.next_spill_offset;
        let Some(base) = self.reserve_direct_call_service_slots(args_frame_top, &arg_slots) else {
            self.fail("singlepass-codegen/cycle-edge-cell-service-slots");
            return false;
        };
        for (i, slot) in arg_slots.iter().enumerate() {
            self.load_slot_to_reg(R11, *slot);
            // Cast: argument index bounded by the operand stack; x86-64 displacement.
            let off = base + ((arg_slots.len() - 1 - i) as i32) * 8;
            self.emit_store_local(off, R11);
            // Named unconditionally, as the dispatch arm names its buffer: the
            // range is live across the call on BOTH targets.
            if arg_oops.get(i).copied().unwrap_or(false) {
                self.pending_staged_arg_oops.push(off);
            }
        }
        let total_sub = self.emit_stack_arg_setup(&arg_slots, true);
        // Every Java argument is in the named service range and ARG_REGS carry
        // only copies of them (plus the context); RAX may hold a stack-argument
        // copy, so it stays in the spill.
        self.emit_pre_safepoint_spill_args_published(true, false);

        // 3. The target: the cell's entry, else the dispatch helper.
        self.emit_mov_imm64(R11, cell_addr as i64); // Cast: cell address as imm64
        self.emit_mov_r64_mem_disp32(R11, R11, crate::JIT_CYCLE_EDGE_CELL_ENTRY_OFFSET);
        self.emit_test_r64_r64(R11);
        let resolved = self.emit_jcc_rel32_patch(0x85); // JNZ .call
        self.emit_load_local(ARG_REGS[0], self.heap_local_offset);
        self.emit_mov_imm64(ARG_REGS[1], info as i64); // Cast: pointer as imm64
        if n > 0 {
            // Lowest address = arg 0, exactly the dispatch arm's buffer start.
            let buf_start = base + ((n as i32) - 1) * 8; // Cast: x86-64 displacement
            self.emit_lea_frame_slot(ARG_REGS[2], buf_start);
        } else {
            self.emit_xor_reg_self(ARG_REGS[2]);
        }
        self.emit_mov_imm32_sx(ARG_REGS[3], n as i32); // Cast: x86-64 immediate
                                                       // (vm_ptr, info_ptr, args_ptr, num_args) -> result | sentinel.
        cratonvm_jit_api::assert_helper_call_shape!(
            "invoke_dispatch",
            int_args = 4,
        ...
        self.emit_mov_imm64(R11, self.helpers.invoke_dispatch as i64); // Cast: fn address as imm64
        self.patch_rel32_to_here(resolved);
        // CALL R11 (REX.B + FF /2 + ModRM(11,/2,R11)), as the inline PIC does.
        self.buf.emit(&[0x41, 0xFF, 0xD3]);
        self.emit_post_call_rbp_republish();
        self.emit_oop_map_for_safepoint();
        self.emit_stack_arg_cleanup(total_sub);
        let merged_keep =
            merged_call_sentinel_enabled().then(|| self.emit_call_sentinel_fast_skip());
        self.emit_inline_callee_deopt_check(info, n, base);
        // The guard's overflow edge joins here with RAX = i64::MIN and its
        // stack balanced (it left before the stack-argument block existed).
        self.patch_rel32_to_here(overflow);
        self.emit_post_invoke_exception_check(ret_type);
        if let Some(keep) = merged_keep {
            self.patch_rel32_to_here(keep);
        }
        // See the direct-call arm: the result belongs at the popped depth, not
        // above the service range.
        self.next_spill_offset = post_pop_spill;
        if ret_type != b'V' {
            if matches!(ret_type, b'D' | b'F') {
                self.push_from_rax_as_xmm0();
    ...
    /// Round 9 wave 9 (lane `evict9`): the CALL of a baked direct call made
    /// through its retire cell ([`crate::JIT_RETIRE_CELL_GUARD`],
    /// `evicted-callee-stays-reachable-through-a-compiled-callers-binding-FIXED-20260918.md`).
    ///
    /// Emitted by the ordinary direct-call arm in place of its
    /// `emit_call_absolute(callee_entry)`, AFTER the arguments are staged for
    /// the direct entry ABI, copied to the service range (`args_base`) and the
    /// pre-call spill / safepoint metadata is emitted:
    ///
    /// ```text
    ///   mov r11,cell ; mov r11,[r11] ; test r11,r11 ; jnz .call
    ///   mov ARG0,[rbp-heap] ; mov ARG1,info ; lea ARG2,[args] ; mov ARG3,n
    ...
    /// ```
    ///
    /// The caller's post-call sequence (republish, oop map, stack-argument
    /// cleanup, sentinel skip, callee-deopt service check, exception check) is
    /// shared by both targets, exactly as for the cycle-edge cell
    /// ([`Self::emit_cycle_edge_cell_call`], whose soundness notes apply):
    ///
    /// * The word is loaded ONCE, as the last thing before the `CALL`, with no
    ///   blocking point in between, so the body it names cannot be reclaimed
    ///   under this thread (the inline-cache argument).
    /// * The cell only ever holds its PINNED entry -- the one this code would
    ///   otherwise have baked -- or `0`, so the acyclicity of baked edges that
    ///   stack safety rests on is unchanged: no stack-depth guard is needed,
    ///   and a filled cell costs one load, a test and a taken branch.
    /// * The dispatch helper ignores the staged direct-entry registers and the
    ///   outgoing stack-argument block; its args buffer is the service range,
    ///   laid out as the dispatch arm lays out its own.
    fn emit_retire_cell_call_target(
        &mut self,
        cell_addr: usize,
        info: *const crate::JitInvokeInfo,
        n: usize,
        ...
        if n > 0 {
            // Lowest address = arg 0, exactly the dispatch arm's buffer start.
            let buf_start = args_base + ((n as i32) - 1) * 8; // Cast: x86-64 displacement
            self.emit_lea_frame_slot(ARG_REGS[2], buf_start);
        } else {
            self.emit_xor_reg_self(ARG_REGS[2]);
        }
        ...
        self.emit_mov_imm64(R11, self.helpers.invoke_dispatch as i64); // Cast: fn address as imm64
        self.patch_rel32_to_here(resolved);
        // CALL R11 (REX.B + FF /2 + ModRM(11,/2,R11)), as the cycle-edge cell does.
        self.buf.emit(&[0x41, 0xFF, 0xD3]);
    }

    /// The safepoint poll of a self-recursive TAIL call's back-edge.
    ///
    /// The tail form `JMP`s to `body_entry_offset`, which is taken AFTER the
    /// prologue (`driver.rs`), and the prologue's last act is the method-entry
    /// poll (`frames.rs`, `emit_safepoint_poll_prologue`). So the jump skipped
    /// the only poll a method whose sole loop is its own tail recursion has —
    /// `static int spin(int n) { return n == 0 ? 0 : spin(n - 1); }` ran
    /// `spin(Integer.MAX_VALUE)` as a poll-free loop and held off every
    /// stop-the-world for its whole length. HotSpot has no tail calls, so there
    /// the same method overflows the stack. Here it is a loop, and a loop polls.
    /// (`NOTES-invoke.md`, proposal 5.) The tail form itself is opt-in
    /// (`CRATONVM_JIT=self-tailcall`), so this is the poll of a configuration a
    /// user chose, not of the default one.
    ///
    /// # Why this is the ENTRY poll, re-emitted, and not a back-edge poll at `pc`
    ///
    /// By the time this runs, `emit_self_tail_param_moves` has put every
    /// argument in the parameter home the prologue would have put it in, and
    /// the jump target is the state right after that prologue. That is exactly
    /// the state the entry poll describes, and describes EXACTLY:
    /// `local_oop_mask_at_current_pc` answers `param_oop_mask` for
    /// `ENTRY_POLL_BC_PC` with an empty operand stack. The invoke's own pc is
    /// the wrong key: its `local_oop_masks` entry describes the locals BEFORE
    /// the moves, and a parameter slot the body reuses for a value of another
    /// type would be claimed (or not) by the old type while it holds the new
    /// argument.
    ///
    /// The prologue poll and this one therefore publish the same bytecode pc
    /// with the same claim — the parameter mask, nothing on the operand stack
    /// — which is why the operand stack must be EMPTY here. `ireturn` does not
    /// require it (values below the return value are discarded), so a
    /// hand-written tail site can leave some; it gets no poll, which is
    /// today's behaviour, rather than a second map under the entry key that
    /// disagrees with the first.
    ///
    /// # GC-inert methods
    ///
    /// A `gc_inert_selfrec` method has no entry poll at all and its raw
    /// self-CALLs publish nothing, so claiming moving-young coverage at a
    /// safepoint inside one would be a lie about the frames above it. It gets
    /// the protocol its native-stack guard already uses at the same site —
    /// `emit_pre_safepoint_spill_without_shadow`, then a map — under which a
    /// collection that lands there takes the non-moving fallback. That is the
    /// one method shape where this poll matters most: a spin like the one
    /// above is GC-inert (no allocation, no bytecode back-edge, no other call).
    ///
    /// Costs one `TEST BYTE [rip+flag]` + `JZ` per tail iteration. Disabled by
    /// the same switch as every other poll, `CRATONVM_JIT_SAFEPOINT_POLLS=0`.
    fn emit_self_tail_safepoint_poll(&mut self) {
        if self.failed || !self.stack.is_empty() {
            return;
        }
        if !self.gc_inert_selfrec {
            // Swaps `cur_bc_pc` to `ENTRY_POLL_BC_PC` and back, and applies
            // the flag / helper-address gates itself.
            self.emit_safepoint_poll_prologue();
            return;
        }
        if !jit_safepoint_polls_enabled()
            || self.helpers.safepoint_flag_addr == 0
            || self.helpers.safepoint_slow_path == 0
        {
            return;
        }
        let saved_pc = self.cur_bc_pc;
        self.cur_bc_pc = super::safepoint::ENTRY_POLL_BC_PC;
        // The flag test, exactly as `emit_safepoint_poll` emits it.
        if !jit_rip_safepoint_poll_enabled()
            || !self.emit_test_mem8_abs_imm8(self.helpers.safepoint_flag_addr, 0xFF)
        {
            self.emit_mov_imm64(R11, self.helpers.safepoint_flag_addr as i64); // Cast: x86-64 immediate encoding
            self.emit_test_mem8_imm8(R11, 0, 0xFF);
        }
        let no_poll = self.emit_jcc_rel32_patch(0x84); // JZ (flag clear) -> skip
        self.emit_pre_safepoint_spill_without_shadow();
        cratonvm_jit_api::assert_helper_call_shape!(
            "safepoint_slow_path",
            int_args = 0,
    ...
    /// Move a self-recursive TAIL call's arguments into this method's own
    /// parameter homes, ahead of the `JMP` back to the body entry.
    ///
    /// The homes must be exactly the ones the prologue deposits incoming
    /// arguments in (`frames.rs`, `emit_prologue`), because the body the jump
    /// lands in reads them from there. This used to write argument `i` to local
    /// `i`'s GPR home or frame slot, which is right only for a method whose
    /// parameters are all category-1 and none of which has an XMM home:
    ///
    ///   * a `long`/`double` parameter spans TWO JVM local slots, so every
    ///     argument after it belongs one slot further on
    ///     (`param_jvm_slots`) — `f(long a, int b)` wrote `b` into `a`'s dead
    ///     high half and the body kept reading the OLD `b` from slot 2;
    ///   * a `float`/`double` parameter the allocator homed in an XMM register
    ///     is read by the body from that register, and the store went to a
    ///     frame slot nothing reads.
    ///
    /// It also moved the arguments one at a time, which is a PARALLEL-MOVE
    /// hazard the prologue never has (its sources are the ABI registers, which
    /// no parameter home aliases). Here a source can be a register-homed local
    /// — `return f(b, a);` pushes `CalleeSaved(home of b)` then
    /// `CalleeSaved(home of a)` — and writing parameter 0 destroys the source
    /// of parameter 1. When any later source reads a home an earlier move
    /// writes, every value is first staged on the machine stack (`PUSH` in
    /// order, `POP` in reverse): no call and no safepoint sits between them, so
    /// the transient `RSP` adjustment is invisible to the GC and to the frame
    /// layout. Without such an overlap the moves are emitted directly, which is
    /// byte-identical to the old sequence for a category-1, non-XMM signature.
    fn emit_self_tail_param_moves(&mut self, arg_slots: &[StackSlot]) {
        ...
        let homes: Vec<Home> = (0..arg_slots.len())
            .map(|i| {
                let slot = self.param_jvm_slots.get(i).copied().unwrap_or(i);
                if let Some(xmm) = self.xmm_for_local(slot) {
                    Home::Xmm(xmm)
                } else if let Some(reg) = self.reg_for_local(slot) {
                    Home::Gpr(reg)
                } else {
                    Home::Frame(self.local_offset(slot))
                }
            })
            .collect();
        let source_home = |slot: &StackSlot| -> Home {
            match *slot {
                StackSlot::Frame(off) => Home::Frame(off),
                StackSlot::CalleeSaved(reg) | StackSlot::Scratch(reg, ..) => Home::Gpr(reg),
                StackSlot::Xmm(xmm) => Home::Xmm(xmm),
            }
        };
        // Parameter `i` is written before the source of every `k > i` is read.
        let overlaps = arg_slots.iter().enumerate().any(|(k, slot)| {
            let src = source_home(slot);
            homes[..k].iter().any(|dst| *dst == src)
        });
        if overlaps {
            ...
            for i in (0..arg_slots.len()).rev() {
                self.buf.emit_byte(0x58); // POP RAX
                match homes[i] {
                    Home::Xmm(xmm) => self.emit_movq_xmm_from_rax(xmm),
                    Home::Gpr(reg) => self.emit_mov_reg_reg(reg, RAX),
                    Home::Frame(off) => self.emit_store_local(off, RAX),
                }
            }
            return;
        }
        for (i, slot) in arg_slots.iter().enumerate() {
            match homes[i] {
                Home::Gpr(reg) => self.load_slot_to_reg(reg, *slot),
                Home::Xmm(xmm) => {
                    self.load_slot_to_reg(RAX, *slot);
...
/// Does this `invokedynamic` site lower to the BRIDGE call rather than to the
/// reason-8 uncommon trap?
///
/// The three terms the 0xba arm admits a bridged site on, in one place because
/// two callers now ask: the arm itself, and
/// [`Compiler::has_trapping_indy_site`], which the self-recursive
/// `invokestatic` arm consults before it bakes a raw `CALL` to this method's
/// own entry. A site needs a resolved bootstrap (`bridge_site`), the bridge
/// helper wired (`bridge_entry`), and a descriptor that returns a value —
/// `void` is refused because the arm's result push has nothing to model.
#[inline]
fn indy_site_is_bridged(bridge_site: usize, bridge_entry: usize, ret_type: u8) -> bool {
