...
/// The IR tier's inline TLAB bump (`runtime_lowering::emit_inline_tlab_new_ir`)
/// is held to the same header contract as the single-pass one.
///
/// It is a second implementation of `emit_inline_tlab_new`, and until round 9
/// wave 2 no scan looked at it: `backend_sources()` covers `x64.rs` and the
/// `x64/*.rs` submodules only (and must not grow `runtime_lowering.rs`, which
/// would shift the pinned site inventory). So it gets its own scan, over its
/// own body, pinning what the two heap corruptions this module exists for were
/// made of:
///
/// * `class_id`, `shape` and the mark word are all stored, each
///   UNCONDITIONALLY (at the function's own brace depth) — a mark-word store
///   inside a conditional is the 2026-08-07 `read_slot: corrupt Value cell`;
/// * the `gc_flags` byte is written AFTER the mark-word store that would erase
///   it, and carries `GC_FLAG_HEADER` on every arm (the zero-header
///   `new Object()` the young sweep cannot parse);
/// * every one of those stores precedes the cursor commit
///   (`MOV [R10 + cursor_off], RAX`, bytes `49 89 82`) — the BinTrees-18
///   ordering bug.
///
/// Source-level for the reason its siblings give: the ordering is structural
/// and every store is just a `MOV`.
#[test]
fn ir_inline_tlab_writes_the_whole_header_before_the_commit() {
    // No `.expect(` / `.unwrap()` / `panic!` in this file's new tests: the
    // whole file is counted by `jit/tests/panic_free_compile_ratchet.rs` (its
    // `#[cfg(test)]` gate lives in `x64.rs`), frozen at the older tests' total.
    let src = include_str!("../runtime_lowering.rs");
    let found = src.find("pub(crate) fn emit_inline_tlab_new_ir(");
    assert!(
        found.is_some(),
        "emit_inline_tlab_new_ir is in runtime_lowering.rs"
    );
    let rest = &src[found.unwrap_or(0)..];
    let body = rest.split("\npub(crate) fn ").next().unwrap_or(rest);

    // (line index, brace depth at the start of the line) of each needle.
    let mut class_id: Option<(usize, i32)> = None;
    let mut shape: Option<(usize, i32)> = None;
    let mut mark: Option<(usize, i32)> = None;
    let mut flags: Option<(usize, i32)> = None;
    ...
    for (n, line) in body.lines().enumerate() {
        ...
        let here = Some((n, depth));
        if class_id.is_none() && code.contains("0x41, 0xC7, 0x43, 0x00") {
            class_id = here;
        }
        if shape.is_none() && code.contains("NUM_SLOTS_OFFSET as u8") {
            shape = here;
        }
        if mark.is_none() && code.contains("MARK_WORD_OFFSET as u8") {
            mark = here;
        }
        if flags.is_none() && code.contains("GC_FLAGS_BYTE_OFFSET as u8") {
            flags = here;
        }
        if commit.is_none() && code.contains("0x49, 0x89, 0x82") {
            commit = Some(n);
        }
        for ch in code.chars() {
            match ch {
    ...
    assert!(
        class_id.is_some()
            ...
            && flags.is_some()
            && commit.is_some(),
        "the IR inline bump must store class_id at +0 ({class_id:?}), the shape \
         word ({shape:?}), the mark word ({mark:?}) and the gc_flags byte \
         ({flags:?}), and commit the TLAB cursor with `49 89 82` ({commit:?})"
    );
    let (Some(class_id), Some(shape), Some(mark), Some(flags), Some(commit)) =
        (class_id, shape, mark, flags, commit)
    else {
        return;
    };
    ...
    for (name, (line, d)) in [
        ...
        assert_eq!(
            d, 1,
            "the IR inline bump's {name} store (line +{line}) sits inside a \
             conditional block (depth {d}); every header store must be \
             unconditional, or some allocation publishes whatever the TLAB \
             slot held"
        );
        assert!(
            line < commit,
            "the IR inline bump's {name} store (line +{line}) follows the cursor \
             commit (line +{commit}): a walker can see the object before its \
             header exists"
        );
    }
    assert!(
        mark.0 < flags.0,
        "the gc_flags byte (line +{}) must be written AFTER the mark-word store \
         (line +{}) that overlaps and would erase it",
        flags.0,
        mark.0
    );

    // `GC_FLAG_HEADER` on every arm of the flags byte's value.
    let span = code_lines[flags.0..commit].concat();
    let header_bits = span.matches("GC_FLAG_HEADER").count();
    let arms = span.matches("else").count() + 1;
    assert!(
        header_bits >= arms,
        "the IR inline bump's gc_flags byte must carry GC_FLAG_HEADER on every \
         arm ({arms} arm(s), {header_bits} mention(s)); without it a JIT-inline \
         `new Object()` is sixteen zero bytes the young sweep cannot parse"
    );
    for line in &code_lines[flags.0..commit] {
        assert!(
            !line.contains("GC_FLAG_COMPACT") || line.contains("GC_FLAG_HEADER"),
            "a compact-arm gc_flags value lost GC_FLAG_HEADER: {line}"
        );
    }
}

/// Every element address in `x64/simd.rs` is `ARRAY_DATA_OFFSET`-based.
///
/// The two constants are equal today and separate when arrays grow their
/// length prefix (`heap_types.rs`: "objects 16, array data still at 24"). The
/// pre-headers in `simd.rs` address nothing BUT array elements, so any
/// object-header-size spelling there is an element address that would read
/// eight bytes early — into the length word — the day the prefix lands, with
/// no fault. Round 9 wave 2 migrated all fourteen; this keeps it that way.
#[test]
fn simd_element_addresses_name_the_array_data_offset() {
    let src = include_str!("simd.rs");
    // Assembled at runtime so this test's own text is not a hit when the
    // inventory test above scans this file.
    let needle = ["HEADER", "_SIZE"].concat();
    ...
    assert!(
        offenders.is_empty(),
        "x64/simd.rs addresses array elements with the object header size; use \
         ARRAY_DATA_OFFSET (and move the site between rows of \
         header_offset_emission_site_inventory_matches_the_doc): {offenders:?}"
    );
    let data = ["ARRAY_DATA", "_OFFSET"].concat();
    assert!(
        src.matches(data.as_str()).count() >= 14,
        "x64/simd.rs lost element-address sites; update this test and the inventory"
    );
}
...
fn header_offset_emission_site_inventory_matches_the_doc() {
    ...
    let cases: [(&str, &str, usize); 7] = [
        ...
        //
        // 2026-09-18 (round 9 wave 2, spsimd): 21 -> 17. The four disp8
        // element bases in `simd.rs` (matrix-dot `IMUL`, zero-fill, stride and
        // sieve `ADD`s) now name the array-data constant; see its row below.
        ("HEADER_SIZE", " as u8", 17),
        ...
        //
        // 2026-09-18 (round 9 wave 2, spsimd): 11 -> 1. Ten of the eleven were
        // `simd.rs` element addresses (int-sum pointer and tail, matrix-dot A/B
        // displacements, element-wise pointers and tail) and moved to the
        // array-data row. The one left is not an array element address.
        ("HEADER_SIZE", " as i32", 1),
        ...
        //
        // 2026-09-17: +2 for `append(String)`/`append(int)` (lane
        // lane-stringbuilder-jit-intrinsic-20260917) — `objects.rs`'s
        // `emit_sb_append_string_copy` reads the destination's capacity the
        // same way the append(char) body above it does, and `op_invoke.rs`'s
        // `append(int)` driver reads the source String's own `value.length`
        // when computing its char count. Both disp8, both behind the same
        // allocator screen (a live array's header is what either constant
        // describes either way).
        ("ARRAY_LENGTH_OFFSET", " as u8", 27),
        ("ARRAY_LENGTH_OFFSET", " as i32", 6),
        // 2026-09-18 (round 9 wave 2, spsimd): 3 -> 5. The int-sum and
        // element-wise SIMD pre-headers now guard their own arrays (null,
        // `length >= bound`) and load an `a.length` bound themselves; each
        // spells the length offset once, through the checked `disp8_const`.
        ("ARRAY_LENGTH_OFFSET", " as i64", 5),
        ...
        //
        // 2026-09-17: +4 for `charAt(I)C`/`append(String)`/`append(int)`
        // (lane lane-stringbuilder-jit-intrinsic-20260917), all in
        // `objects.rs`: `emit_sb_char_at_body`'s LATIN1 and UTF16 element
        // reads (`emit_sb_char_at_body` in `objects.rs`), and
        // `emit_sb_append_string_copy`'s two `LEA`s that fold the
        // destination's old `count` into the write base, one per
        // destination element width. All four disp8, all four behind the
        // same screen — every one addresses a live `byte[]` the allocator
        // already stamped.
        //
        // 2026-09-17 (same lane, second commit): +1, 19 -> 20.
        // `emit_sb_append_string_copy`'s copy-loop READ was missing this
        // offset entirely — it read `[R8+index*scale]` with R8 sitting at
        // the source array's OWN OBJECT HEADER (class id / GC flags /
        // length), not its element data, which starts `ARRAY_DATA_OFFSET`
        // bytes later. `append("alpha")` measured the bug directly: 5
        // bytes copied (the length header answers `.length` correctly) and
        // read back as `[0, 0, 0, 0, 5]` — header bytes, not `"alpha"`.
        // Fixed by gating the read behind `src_is_heap_array` (`true` for
        // `append(String)`'s real `String.value`, `false` for
        // `append(int)`'s stack scratch digit buffer, which has no header
        // to skip and was the one case the bug happened not to reach).
        // Disp8, behind the same allocator screen as every sibling row here.
        //
        // 2026-09-18 (round 9 wave 2, spsimd): 20 -> 24 and 0 -> 10 — the
        // fourteen `simd.rs` element addresses that left the two header-size
        // rows above. Moved between rows, none added or removed.
        //
        // 2026-09-18 (round 9 wave 8, arraylist8): 24 -> 25 (and the i32
        // array-length row 5 -> 6). The single-pass ArrayList
        // `get`/`size` prefix (`op_invoke.rs::try_emit_arraylist_access_prefix`)
        // bounds the index by the backing `Object[]`'s own length (disp32) and
        // loads the element at `ARRAY_DATA_OFFSET` (disp8) -- a live array the
        // allocator stamped, like every sibling row.
        ("ARRAY_DATA_OFFSET", " as u8", 25),
        ("ARRAY_DATA_OFFSET", " as i32", 10),
    ];
...
pub(super) fn bounds_check_test_compiler() -> Compiler {
