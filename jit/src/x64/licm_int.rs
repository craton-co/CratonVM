...
/// which all its operands are invariant (largest span first) AND that no OSR
/// entry can land inside ([`span_holds_inner_osr_entry`]) -- in practice the
/// innermost loop containing the run, and only when that loop has no inner
/// loop of its own -- so it is computed as few times as possible without
/// taking an inner loop's OSR entry away. (A run in an outer body beside an
/// inner loop is therefore not hoisted at all; it costs a few ALU operations
/// per OUTER iteration.) A run already claimed by an outer loop is not
/// re-hoisted by an inner one.
pub(super) fn find_arith_loop_hoists(
    ...
    for &(header, back_edge) in &sorted_loops {
        ...
        // "Outermost" stops at the first loop an OSR entry cannot land
        // inside. An outer loop holding an inner loop's header is skipped, so
        // the run falls to that inner loop on its turn (its pcs are still
        // unclaimed) and is recomputed once per inner loop entry, instead of
        // costing the inner header its OSR entry (`inside_arith_hoisted` in
        // the walk). Measured cost of the old attribution:
        // `licm::span_holds_inner_osr_entry`.
