        ...
        match op {
            ...
            0xc5 => {
                ...
                let ndims = code[pc + 3] as usize; // Widening: always safe
                                                   // `jit_multianewarray_2d` is the only lowering, and it pops
                                                   // exactly two dimensions. `jit_scan` refuses every other
                                                   // `ndims`, but a `debug_assert!` is no guard in a release
                                                   // build: a 3-D site reaching here would pop two of its three
                                                   // counts and leave the model one entry high. Refuse instead
                                                   // (round 9 wave 6, x64misc6).
                if ndims != 2 {
                    self.fail("singlepass-codegen/multianewarray-not-2d");
                    return WalkStep::Return(false);
                }
