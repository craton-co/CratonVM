...
pub(crate) enum LoopRewriteRefusal {
    /// The default: [`bytecode_loop_xform_rewrites_bytecode`] answered `false`.
    ///
    /// That is BOTH ways in, not one. This thread never called
    /// [`set_bytecode_loop_rewriter_armed`] **and**
    /// `CRATONVM_JIT_BYTECODE_LOOP_XFORM` is off. The doc used to name only the
    /// thread-local, which is the way in that no production code takes — the
    /// only callers of that setter in the tree are in the `#[cfg(test)] mod
    /// loop_unroll_admission`. The env flag is the production opt-in and is what
    /// makes every refusal in this enum reachable outside a unit test.
    NotArmed,
    ...
    ///
    /// **Nothing counts this refusal.** `metrics::LOOP_XFORM_EVENTS` has no row
    /// for it, the three `return Err(R::ProvenanceNotTotal)` sites in
    /// `plan_bytecode_loop_xform` all leave before the tally at the end of that
    /// function, and `driver.rs`'s `Err(refusal)` arm only `eprintln!`s, behind
    /// `CRATONVM_DBG_JIT_GEN`. So a soundness backstop that fires is
    /// indistinguishable from one that never fires — the round-10 defect class,
    /// one level in from an instrument that reads zero. Fixing it needs a new
    /// row in `jit/src/metrics.rs`, which lane `offsetkey` did not own:
    /// `record_loop_xform_event` ignores a name absent from the table, so adding
    /// the `tally` call alone would produce a counter that silently no-ops
    /// forever, which is the same defect again. The two edits must land
    /// together; see
...
fn trip_count_guard(
    ...
    //
    // The exception table is the method's real one: the proof's constant
    // entry value and once-per-iteration advance are checked along handler
    // entries too, and a loop an exception can enter from outside is refused
    // (docs/known-issues/jit/bytecode-loop-scans-do-not-model-exception-handler-entries-20260918.md).
    let (counted, test_pc) = crate::loop_analysis::analyze_counted_loop_at_with_handlers(
        code,
        code_len,
        header,
        back_edge,
...
fn plan_versioned(
    ...
    if let Some(guard) = trip_count_guard(
        code,
        code_len,
        header,
        back_edge,
    ...
    for &(header, back_edge) in &loops {
        ...
        if bypassable.contains(&header) {
            ...
            match plan_versioned(
                ...
                Ok(x) if !x.provenance_is_total() => {
                    tally("loop_xform_provenance_not_total");
                    return Err(R::ProvenanceNotTotal);
                }
                Ok(x) => return Ok(x),
                Err(e) => {
                    last_refusal = Some(e);
        ...
        if let Some(&pgo_factor) = loop_unroll_hints.get(&back_edge) {
            if body_size <= 50 || (body_size <= 100 && pgo_factor <= 2) {
                return match plan_versioned(
                    ...
                    Ok(x) if !x.provenance_is_total() => {
                        tally("loop_xform_provenance_not_total");
                        Err(R::ProvenanceNotTotal)
                    }
                    Ok(x) => Ok(x),
                    Err(e) => {
                        // Counted HERE because this arm returns rather than
                        // falling through to the tally at the end of the
                        // function (the `return` is deliberate — a PGO refusal
                        // must not drop back to the static band, same as the
                        // native unroller's). Without this, a PGO-path planner
                        // refusal was the only `LoopXformRefusal` in this
                        // function that reached the caller uncounted, so
                        // `loop_xform_planner_refused` read as if the PGO arm
                        // never refused anything — and on a PGO run that is the
                        // arm most loops take. Found by reading, round 10; no
                        // run has been observed with and without it.
                        tally("loop_xform_planner_refused");
                        Err(R::Planner(e))
                    }
                };
            }
...
enum PointDifference {
    ...
    /// that RECONSTRUCTS a frame finds its point through the box pointer the
    /// copy's own deopt stub bakes — never by bci and never by offset.
    ///
    /// **The `find_deopt_point` disjunct was removed in round 10, and the
    /// conclusion above does not need it.** It used to read "finds its point by
    /// native offset (`CompiledMethod::find_deopt_point`) or through the box
    /// pointer …". The first half was false twice over: that search had no
    /// production caller (it was deleted outright in wave 9), and
    /// `native_offset` is not an identity — two
    /// metadata-only records at one emitter pc land on one offset, which the
    /// verifier's ordering lane deliberately tolerates
    /// (`deopt::DeoptMetadataError::DeoptPointsUnsorted` is `>`, not `>=`). See
    /// `docs/internal/fixed-bugs/r10-interner-native-offset-is-not-a-unique-deopt-point-key-RESOLVED-20260922.md`.
    ///
    /// The surviving disjunct is the whole argument, and it is an identity in the
    /// strict sense: `emit_deopt_stubs` resolves each stub's key through
    /// `x64::deopt_stubs::resolve_baked_point`, which maps the key to a position
    /// in `deopt_points`, cross-checks the box's `native_offset`/`bci`/`reason`
    /// against that element's, and **fails the compile closed** if they
    /// disagree; the stub then bakes that element's ADDRESS as an imm64 and
    /// `deopt::x64_deopt_entry` receives it in arg0. Per-copy by construction:
    /// the `*_box_ptr_by_bci` maps are keyed by EMITTER pc (despite the name —
    /// `build_and_record_deopt_point`'s `bci` parameter is documented as an
    /// emitter pc), and the copies of one bytecode occupy distinct output pcs,
    /// so each copy gets its own map entry and its own baked pointer. A copy's
    /// stub can therefore not reach another copy's frame description.
    ///
    /// That leaves exactly three bci-keyed readers, which is why the split above
    /// is drawn where it is: `OsrEntryPlan::resume_after_exit` (through
    /// `osr_exit::resume_image`) reads `semantics` — refused as `Fatal`; the
    /// de-speculation lookup reads `reason` — the grouping key, so it never
    /// reaches here, and `osr_exit::deopt_reason_at_bci` refuses rather than
    /// picks in any case; and `osr_entry_frame_state` reads the slot
    /// descriptors — re-verified against the live frame, which is this variant.
    ///
