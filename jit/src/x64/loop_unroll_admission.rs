...
fn the_empty_stack_rule_has_an_off_switch_and_it_is_not_inert() {
    ...
    // Folding off in BOTH arms (see the `off` arm below), so the two
    // artifacts differ only in the rule and their offsets stay comparable.
    let on = cratonvm_types::flags::with_thread_overrides(
        &[("CRATONVM_JIT_NO_OPERAND_FOLD", Some("1"))],
        rebuild,
    );
    for bci in MID_EXPRESSION {
    ...
    // Operand folding (round 9 wave 5) fuses `iadd; istore` into one store to
    // the local's register, so bci 12 has no instruction boundary of its own
    // to enter at; it is switched off here so this arm still measures the
    // empty-stack rule and nothing else.
