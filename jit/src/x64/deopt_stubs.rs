...
/// Whether the four `*_by_bci` publication maps' single, bare-`usize` key space
/// has exactly one owner at splice depth `splice_depth`.
///
/// Kept as a free function, for the same reason `splice_point_is_misidentified`
/// is one: the property is a one-line rule about a number, and a rule that can be
/// stated without a `Compiler` can be tested without building one. See
/// [`Compiler::may_file_by_bci`] for the whole argument; this is only the rule.
///
/// `splice_depth == 0` — nothing is being spliced — is the only configuration in
/// which a bare bci names exactly one method's bytecode. At any depth the answer
/// is `false` and stays `false`: there is no depth at which the ambiguity gets
/// better, because the maps are shared across the WHOLE compile and an entry a
/// depth-1 splice files outlives that splice (`truncate_deopt_points_to` purges
/// only a ROLLED-BACK splice's entries, never a kept one's).
fn by_bci_key_space_has_one_owner(splice_depth: usize) -> bool {
    splice_depth == 0
}
...
impl Compiler {
    ...
    pub(super) fn emit_deopt_snapshot_at_guard(&mut self, bci: usize) {
        // This publisher's reason is fixed — it is the speculative-BCE / hoist
        // guard block at a loop header. It is bound rather than inlined because
        // it is now half of the map KEY as well as the point's stamp, and the
        // two must not be able to drift apart.
        let reason = crate::deopt::DeoptReason::BoundsCheck;
        let box_ptr = self.build_and_record_deopt_point(bci, reason);
        // The point is still RECORDED when the filing is refused — only the key
        // is withheld. See [`Self::may_file_by_bci`]. Recording unconditionally
        // is deliberate and is not laziness: `inlining.rs`'s splice postcondition
        // inspects the points a splice appended (`deopt_points_checkpoint`) and
        // rolls the splice back if one is misidentified, so a publisher that
        // skipped the BUILD would remove that backstop's only evidence. Every
        // caller of this reaches it from the OUTER walk, so the refusal arm is
        // unreachable today and this costs one already-loaded length test.
        if self.may_file_by_bci("deopt_box_ptr_by_bci", bci) {
            self.note_by_bci_reason_collision(bci, reason);
            self.deopt_box_ptr_by_bci.insert((bci, reason), box_ptr);
        }
    }

    /// Name, under `CRATONVM_DBG_DEOPT`, a pc that is about to carry a SECOND
    /// `deopt_box_ptr_by_bci` entry under a different reason.
    ///
    /// This is the probe `docs/feature-designs/jit-r10-bybci-proposals.md` §2
    /// calls "the actual first task": until the key carried a reason, such a pc
    /// kept only whichever point filed first, and no trace said so — which is
    /// why the filing page could state the structural gap with certainty and the
    /// REACHABILITY not at all. Called from both producers, so it fires whichever
    /// reason arrives second; `snapshot_pre_intrinsic_call` alone would have seen
    /// only the loop-header-first ordering, which is the one emission order makes
    /// likely but not the only one.
    ///
    /// Read-only and diagnostic. No counter — a count with no reader is the
    /// defect class this round spent nine lanes on, and this question is answered
    /// by one run, not by a gauge.
    fn note_by_bci_reason_collision(&self, bci: usize, reason: crate::deopt::DeoptReason) {
        ...
        for other in [
            ...
            if other != reason && self.deopt_box_ptr_by_bci.contains_key(&(bci, other)) {
                eprintln!(
                    "[DBG_DEOPT] by-bci reason collision: {} pc={bci} now carries \
                     {reason:?} beside {other:?} in deopt_box_ptr_by_bci (before the \
                     reason component the second of these was dropped and the first \
                     was baked into BOTH stubs)",
                    self.method_key,
                );
            }
        }
    ...
    /// Idempotent PER `(bci, reason)`: a second call for the same pair is a
    /// no-op.
    /// Only called when `deopt_real_enabled()`; production builds are unaffected.
    ///
    /// **The idempotence test names the reason, and that is the whole point.**
    /// It used to be `contains_key(&bci)`, which made a reason-6 snapshot at a
    /// pc that already carried a reason-2 one (a loop header that is also a
    /// guarded invoke: the header's guard block is emitted BEFORE the
    /// instruction at that pc, so `BoundsCheck` files first and the invoke's
    /// `ReceiverTypeChanged` is the one that loses) a silent no-op — and then
    /// `emit_deopt_stubs`' `6 =>` arm read the surviving `BoundsCheck` box.
    /// See the map's declaration in `x64.rs` for what that costs; in short,
    /// `osr_exit::deopt_reason_at_bci` answered `Unique(BoundsCheck)` where
    /// `Ambiguous` is the truth, and the receiver guard never got its
    /// de-speculation policy.
    pub(super) fn snapshot_pre_intrinsic_call(
        &mut self,
        bci: usize,
        reason: crate::deopt::DeoptReason,
    ) {
        if self.deopt_box_ptr_by_bci.contains_key(&(bci, reason)) {
            return;
        }
        let box_ptr = self.build_and_record_deopt_point(bci, reason);
        // See the note on `emit_deopt_snapshot_at_guard`: the point is recorded
        // either way, only the key is withheld. A reason-2/6 stub that then finds
        // no entry takes the shared sentinel exit, which is exactly what it did
        // before precise frames existed. `emit_inline_invoke_into_rax`
        // deliberately omits this call, so no splice reaches it today.
        if self.may_file_by_bci("deopt_box_ptr_by_bci", bci) {
            // The collision this key space used to swallow, made observable.
            // See [`Self::note_by_bci_reason_collision`]; it is called from both
            // producers so that neither emission order hides it.
            self.note_by_bci_reason_collision(bci, reason);
            self.deopt_box_ptr_by_bci.insert((bci, reason), box_ptr);
        }
    }
    ...
    /// to be spliced. It is pushed onto [`Compiler::inline_callee_scopes`]
    ...
    ///
    /// `callee_max_locals` is `InlineSite::callee_max_locals` — the callee's own
    /// local count, which is what a point published INSIDE this splice must
    /// describe. See [`super::InlineCalleeScope::max_locals`] for why it is not
    /// the reserved `callee_locals_size`, and `build_frame_state_at` for what is
    /// done with it.
    pub(super) fn push_inline_scope(
        ...
        // The floor is taken from the LIVE stack, not from the captured
        // snapshot: outside a splice the two agree, but inside one (a caller
        // scope captured for a nested splice) the snapshot's stack is already
        // the enclosing callee's window while `self.stack` is the shared vector
        // every later `self.stack.len()` is measured against. Deriving the floor
        // from the snapshot would give an index into the wrong coordinate space.
        let caller_stack_floor = self.stack.len().saturating_sub(arg_slots);
        self.inline_scope_stack.push(fs);
        self.inline_callee_scopes.push(super::InlineCalleeScope {
            key: callee_key,
    ...
    ///
    /// M1. One accessor for the three places that must agree —
    /// [`Self::build_frame_state_at`]'s `method_key`, [`Self::resume_bci_for`]'s
    /// choice of bci space, and (round 10 wave 7) the frame GEOMETRY
    /// `build_frame_state_at` builds to — because a point whose identity, bci
    /// and locals come from different methods is precisely the malformed object
    /// the splice postcondition exists to keep out of artifacts.
    ///
    /// `pub(super)` because `inlining.rs`'s splice postcondition needs the
    /// answer as it stood BEFORE the splice opened — the one identity a point
    /// published inside the splice must never carry. Reading it there rather
    /// than assuming `self.method_key` is what makes the check correct for a
    /// nested splice, whose enclosing owner is a callee.
    pub(super) fn current_bytecode_owner(&self) -> &str {
        match self.inline_callee_scopes.last() {
            Some(scope) => &scope.key,
            None => &self.method_key,
        }
    }

    /// The innermost spliced callee's frame geometry, or `None` when nothing is
    /// being spliced and the frame being built is this compile's own.
    ///
    /// The third answer to [`Self::current_bytecode_owner`]'s question, read off
    /// the same stack entry so it cannot name a different method than the
    /// identity does.
    fn current_callee_scope(&self) -> Option<&super::InlineCalleeScope> {
        self.inline_callee_scopes.last()
    }

    /// Whether a by-bci deopt publication may be FILED under a bare bci right
    /// now — i.e. whether the four `*_by_bci` maps' single key space still has
    /// exactly one owner.
    ///
    /// # The invariant, and why it needed a name
    ///
    /// `deopt_box_ptr_by_bci`, `exc_frame_box_ptr_by_bci`,
    /// `osr_exit_box_ptr_by_bci` and `precise_npe_action_by_bci` are keyed by a
    /// bare `usize`. Outside a splice that is exactly right and is load-bearing:
    /// one bci, one snapshot, shared by every guard at that instruction, which is
    /// what makes two copies of one guard under a loop rewrite resolve to one
    /// point. What the key does NOT carry is which METHOD's bytecode the number
    /// indexes, and inside a splice the emitter is walking a second method's
    /// code on the same maps. Both directions of collision follow, and the
    /// asymmetry matters:
    ///
    ///   * **suppression** — an entry already present at that key makes the
    ///     second publisher's `contains_key` test skip its own insert. The guard
    ///     is left with no snapshot and `emit_deopt_stubs` routes it to the
    ///     shared sentinel exit instead of the frame trampoline. A lost precise
    ///     frame: worse code, not wrong code.
    ///   * **substitution** — the reverse order. The second publisher skips, and
    ///     then `emit_deopt_stubs` BAKES the first publisher's pointer as an
    ///     imm64 into its stub (`resolve_baked_point`). A guard in one method
    ///     deopting through a frame that describes another one is a wrong
    ///     answer, silently.
    ///
    /// Refusing the FILING converts the second into the first. That is the whole
    /// content of this predicate: the bare-bci key space belongs to the method
    /// this compile is compiling, and nothing else may file in it. A publisher
    /// that cannot file falls back to whatever it does when no snapshot exists,
    /// which at every one of its call sites is the shared, frame-less exit that
    /// predates precise frames entirely.
    ///
    /// # This cannot fire today, and the guard is still the right shape
    ///
    /// Round 10 wave 8 walked it: only TWO of the ten by-bci filing sites are
    /// reachable from the splice walk at all — `emit_post_invoke_exception_check`
    /// and `arrays.rs::emit_precise_null_check_field_store`, which are the only
    /// two names `x64/inlining.rs` calls that publish (the whole cross-file call
    /// surface from `inlining.rs` into the seven emission files is six names).
    /// Both are armed only by `self.precise_exception_frames`, and a compile with
    /// `precise_exception_frames` splices nothing, stated four times over:
    /// `plan_inline`'s `InlineRefusal::PreciseExceptionFrames`,
    /// `build_single_pass_tables` clearing BOTH `inline_sites` and
    /// `inline_guard_variants`, and `try_emit_inline_site`'s own refusal. So
    /// `inline_callee_scopes` is empty at all ten sites in every compile this
    /// tree can produce, this returns `true` every time, and no artifact changes
    /// by one byte.
    ///
    /// The alternative — widening the four keys to `(owner, bci)` — is what
    /// `r10-splice-by-bci-deopt-maps-have-no-owner-FIXED-20260922.md` proposes, and it
    /// is NOT taken, for three reasons argued in full in that page's resolution:
    /// the map declarations live in `x64.rs` and `emit_deopt_stubs`' stub-list
    /// tuples would have to carry the depth, so no single lane can do it; the
    /// owner component the page picks (`inline_callee_scopes.len()`) does not
    /// satisfy the page's own requirement, because two splices of one callee at
    /// two caller pcs are both depth 1; and a widened key is exercised by
    /// nothing, whereas this predicate is exercised directly by
    /// `r9w2_spdeopt_tests::a_splice_cannot_file_in_the_bare_bci_key_space`.
    ///
    /// `map` and `bci` are for the diagnostic only. `CRATONVM_DBG_EXCFRAME` is
    /// deliberately the same channel `build_frame_state_at`'s `SPLICE FRAME` line
    /// uses: the two lines together say "a splice published" and "the filing was
    /// refused", and a reader hunting the first will not have to find a second
    /// flag to see the second.
    pub(super) fn may_file_by_bci(&self, map: &'static str, bci: usize) -> bool {
        if by_bci_key_space_has_one_owner(self.inline_callee_scopes.len()) {
            return true;
        }
        if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_EXCFRAME") {
            eprintln!(
                "[excframe] BY-BCI FILING REFUSED map={map} pc={bci} \
                 splice_owner={} enclosing={} depth={} \
                 (the guard falls back to its frame-less exit; \
                 see Compiler::may_file_by_bci)",
                self.current_bytecode_owner(),
                self.method_key,
                self.inline_callee_scopes.len(),
            );
        }
        false
    }
    ...
    fn resume_bci_for(&self, bci: usize) -> usize {
        if self.inline_callee_scopes.is_empty() {
            self.orig_bci(bci)
        } else {
            bci
        }
    ...
    pub(super) fn build_and_record_deopt_point(
        ...
        // splice publishes.
        //
        // **And none does — but not for the reason this comment used to give.**
        // It said "`inlining.rs`'s splice postcondition rolls back any splice
        // that published a point". That stopped being true on 2026-08-18: the
        // postcondition now rolls back a splice whose published points are
        // MISIDENTIFIED, and a point stamped by `current_bytecode_owner` under a
        // caller scope is not. What keeps the population empty is a
        // compilation-wide interlock, and it is worth naming precisely, because
        // the splice walk is not free of publishers (round 10 wave 7):
        //
        //   * TWO publishers, not four. Round 10 wave 8 enumerated the whole
        //     cross-file call surface from `x64/inlining.rs` into the seven
        //     emission files and it is six names, of which exactly two publish:
        //     `emit_post_invoke_exception_check` (five call sites, the
        //     nested-invoke and return arms) and
        //     `arrays.rs::emit_precise_null_check_field_store` (two call sites,
        //     the spliced `putfield` arms). Both publish only under
        //     `self.precise_exception_frames`.
        //
        //     Wave 7 also named `emit_post_alloc_oom_check` and the two array
        //     arms (`emit_precise_array_npe_check`, the reason-11 bounds arm).
        //     `inlining.rs` contains no call to any of the three, directly or
        //     through the six names that do cross, so the splice walk cannot
        //     reach them: its `new` arm calls the allocation helper without the
        //     OOM check and it has no array lowering at all. The claim is
        //     repeated in `x64/inlining.rs`'s postcondition doc and at
        //     `try_emit_inline_site`, which this lane does not own — the
        //     correction is recorded in
        //     `docs/internal/fixed-bugs/r10-bybci-splice-publisher-count-is-overstated-FIXED-20260922.md`.
        //     It changes no conclusion: two gated publishers keep the population
        //     as empty as four would.
        //   * `precise_exception_frames` and inlining are mutually exclusive
        //     FOUR times over: `plan_inline` refuses every site with
        //     `InlineRefusal::PreciseExceptionFrames`; `build_single_pass_tables`
        //     clears `inline_sites`; the same block also clears
        //     `inline_guard_variants`; and `try_emit_inline_site` refuses the
        //     splice outright.
        //
        //     Two corrections to what wave 7 wrote here, both read off `lib.rs`
        //     in wave 8 and neither changing the conclusion. The clear is in
        //     `build_single_pass_tables` (`jit/src/lib.rs:33038`), not in
        //     `try_compile_inner` — there is exactly one `inline_sites.clear()`
        //     in the crate and it is inside the former. And the parenthetical
        //     "the middle one alone would not do: it does not clear
        //     `inline_guard_variants`" is no longer true: the same `if
        //     precise_exception_frames` block clears both, which is what closed
        //     the guarded-virtual splice path (on by default since N8) that
        //     clearing only `inline_sites` left open. The same stale attribution
        //     is in `jit/src/lib.rs:7329` and
        //     `jit/src/x64/inlining.rs:1938`, neither owned by this lane.
        //   * `emit_inline_invoke_into_rax` additionally omits
        //     `snapshot_pre_intrinsic_call`, and `try_emit_nested_inline` rolls
        //     back on ANY publication at all.
        //
        // Note what that means for the shape of a future bug: those publishers
        // key on `self.dbg_last_pc`, which the OUTER walk owns, so the pc they
        // hand this function is an ENCLOSING-method pc even though a splice is
        // open. `resume_bci_for`/`current_bytecode_owner` would publish it
        // verbatim under the CALLEE's name. Relaxing the interlock therefore
        // needs more than a geometry fix — it needs those sites to say whose pc
        // they are publishing. The postcondition's callee-code-range check
        // (`splice_point_is_misidentified`) is the backstop that refuses the
        // splice if one ever does reach here.
        //
        // The consumer side is load-bearing too:
        // `build_deopt_frame_inner` — the sink this backend's traps
        ...
        // defect is that the interlock above keeps the population empty.
        ...
        // is what the VM's resume sinks and — since this
    ...
    /// The frame state at `bci` of whichever method owns that pc — locals,
    /// operand stack and held monitors, with no caller chain.
    ///
    ...
    /// sinks then reject.
    ///
    /// # Inside a splice the frame is built to the CALLEE's geometry
    ///
    /// Round 10 wave 7, and the whole content of
    /// `r10-deoptverify-splice-published-point-would-carry-the-callers-locals-FIXED-20260922.md`.
    /// Everything below the early return is an analysis OF THIS COMPILE'S OWN
    /// METHOD, indexed by that method's pcs: `self.num_locals` (the
    /// `compile_with_param_slots` parameter), `reg_for_local`/`xmm_for_local`/
    /// `local_offset` (the root's local homes), `local_oop_masks`/
    /// `local_oop_reached`, `local_liveness`, `local_kinds`/`local_kinds_refined`,
    /// `sr_local_prov_at`, `sr_monitor_at`, `stack_kinds`,
    /// `indy_stack_arg_types`/`invoke_stack_arg_types`. Inside a splice `bci` is
    /// in the CALLEE's bytecode space, so every one of them is being read at an
    /// unrelated instruction of a different method — and `current_bytecode_owner`
    /// then stamps the callee's name on the result. M1 made the identity and the
    /// bci move together and left the CONTENTS behind, which produced exactly
    /// the object the splice postcondition exists to exclude: a well-formed
    /// description of a stack that never existed, malformed on a different axis.
    ///
    /// So a splice-published frame is built from the one thing that IS known
    /// about the callee — its geometry ([`super::InlineCalleeScope`]) — with
    /// every slot `FrameValue::Unsupported`. That is not a placeholder, it is
    /// the house encoding for "this frame cannot be described": it is what
    /// `ir_lower` writes for an unresolvable caller scope, it makes
    /// `deopt::frame_state_is_resumable` false, and every VM sink then refuses
    /// the resume and takes the safe re-run instead of reconstructing a callee
    /// frame out of the caller's locals. `docs/jit/deopt-inline-scopes.md` makes
    /// the same argument for the IR side: "it lowers to `[Unsupported]`
    /// precisely so this consumer refuses it rather than inventing an empty
    /// frame".
    ///
    /// **What would make it resumable, for whoever needs that.** The oop-ness of
    /// a callee local IS available — `Compiler::inline_oop_scopes` runs
    /// `compute_local_oop_masks` over the callee's own bytecode and
    /// [`super::InlineOopScope::mask_at_cur`] answers at the callee pc being
    /// emitted, which is what the safepoint oop maps already use — and the
    /// callee's locals are memory-homed at `local_base + i*8`, so a ref slot
    /// could be published as a precise `StackSlotRef`. What is missing is a
    /// WIDTH source for the non-ref slots: there is no `classify_local_kinds`
    /// and no liveness for a callee, so an `int`-vs-`long`-vs-`double` slot
    /// cannot be typed, and one `Unsupported` slot makes the whole frame
    /// unresumable anyway. Partial precision therefore buys nothing until a
    /// callee-side kind analysis exists; adding one is the next step, not this
    /// one.
    ///
    /// Unreachable on a production compile today, and deliberately kept anyway:
    /// nothing publishes a point from inside a splice (see
    /// [`Self::build_and_record_deopt_point`]), so this branch is a ratchet for
    /// the first edit that does.
    ...
    fn build_frame_state_at(
        ...
        // ── INSIDE A SPLICE: the callee's geometry, honestly undescribed ──
        //
        // See the "Inside a splice" section of this method's doc for the
        // argument. Three fields, each derived from the SAME stack entry
        // `current_bytecode_owner` took the identity from:
        //
        //  * `locals` — exactly `max_locals` entries, which is both what an
        //    interpreter frame for this callee has and the bound `driver.rs`
        //    registers for this key, so the verifier's `LocalCountMismatch` lane
        //    becomes meaningful instead of comparing the root's count against
        //    the callee's;
        //  * `stack` — the callee's OWN window of the shared abstract stack.
        //    Entries below `caller_stack_floor` belong to the caller and are
        //    described by the caller scope `inline_caller_chain` attaches, not
        //    twice here;
        //  * `monitors` — empty, and that is a statement, not a default. The
        //    only monitor source is `sr_monitor_at`, an analysis of the ROOT
        //    method keyed by root pcs; consulting it at a callee pc would
        //    publish a `MonitorInfo` for a lock this frame does not hold, and
        //    the resume would re-acquire it on a re-materialised object of the
        //    wrong method. An empty list cannot under-report either, because the
        //    frame is unresumable: no consumer relocks from it.
        if let Some(scope) = self.current_callee_scope() {
            let depth = self.stack.len().saturating_sub(scope.caller_stack_floor);
            let fs = FrameState {
                method_key: scope.key.clone(),
                bci: resume_bci,
                locals: vec![FrameValue::Unsupported; scope.max_locals],
                stack: vec![FrameValue::Unsupported; depth],
                monitors: Vec::new(),
                // Filled by `build_and_record_deopt_point` from the scope stack,
                // exactly as for a root frame.
                caller: None,
            };
            if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_EXCFRAME") {
                // No run that has ever existed prints this line: reaching it
                // means something started publishing from inside a splice, which
                // is the edit this branch was written for. Say which callee and
                // at which of ITS bci, because the two numbers a reader would
                // otherwise reach for -- `self.method_key` and an interpreter
                // bci -- belong to the enclosing method.
                eprintln!(
                    "[excframe] SPLICE FRAME callee={} callee_bci={bci} reason={reason:?} \
                     locals={} stack={depth} (all Unsupported; enclosing={})",
                    scope.key, scope.max_locals, self.method_key,
                );
            }
            return fs;
        }
        ...
        FrameState {
            ...
            // about.
            //
            // **This line is only ever reached OUTSIDE a splice**, as of round 10
            // wave 7: a splice-published frame returns from the early branch
            // above, built to the callee's own geometry. So
            // `current_bytecode_owner` here always answers `self.method_key`
            // today. It is kept as the accessor rather than collapsed back to
            // the field because the accessor is what states the property — whose
            // bytecode does this pc index? — and a nested splice that ever
            // starts pushing its own scope must not have to rediscover that this
            // stamp was the answer to it.
            //
            // (An earlier revision of this block claimed, two paragraphs apart,
            // BOTH that the stamp is "the ENCLOSING method's key … a known hole"
            // and that it "used to be `self.method_key` unconditionally". The
            // first was the pre-M1 text left behind by the fix; deleted rather
            // than annotated, because a comment that contradicts itself is worse
            ...
            // The LOCALS have to move with them too, which is what the splice
            // branch above does and what M1 left undone.
            //
    ...
    pub(super) fn emit_inline_callee_deopt_check(
        ...
        // The whole `n` — no flag bits. Bit 32 used to mark this call as the
        // self-recursive arm's own per-level service, which `run_jit_callee_handler`
        // read as `trust_throw_pc` to opt OUT of a stamp downgrade that no longer
        // exists (the downgrade itself was the defect; see that function). With
        // nothing left to read it, an ABI bit nobody decodes is exactly the
        // "guard that is in place and has never run" shape round 10 spent its
        // time on, so it is gone rather than carried.
        // Cast: x86-64 immediate encoding
    ...
    pub(super) fn emit_bounds_check_stubs(&mut self) {
        ...
        for (patch_off, bc_pc) in sites {
            ...
            // This method's own `catch` may take it without leaving the frame.
            // The probe clobbers RAX, so the miss edge re-materialises the
            // sentinel `jit_throw_aioobe` returned before running the epilogue
            // the pad always ran.
            if self.emit_local_handler_implicit_probe(bc_pc) {
    ...
    pub(super) fn emit_null_check_store_stubs(&mut self) {
        ...
        for &(action, _, _, bci) in &entries {
            ...
            // This method's own `catch` may take the NPE without leaving the
            // frame. A miss falls through to the sentinel below, which is the
            // route that ran before — and which also repairs the RAX the
            // probe's `CALL` clobbered.
            //
            // `bci` is the trapping opcode's ORIGINAL bytecode index (the same
            // value the `set_throw_bci` stamp above passes), so it range-tests
            // against the classfile exception table directly.
            self.emit_local_handler_implicit_probe(bci);
    ...
    pub(super) fn emit_post_invoke_exception_check(&mut self, ret_type: u8) {
        ...
        // `may_file_by_bci` is part of the ARMING condition, not a wrapper on the
        // insert, and that is the point. This is one of exactly two publishers the
        // splice walk reaches (`inlining.rs` calls it from five places), and
        // `throw_bci` here is `dbg_last_pc` — assigned only by the OUTER walk, so
        // inside a splice it is the CALLER's invoke pc while
        // `current_bytecode_owner` names the callee. Withholding only the key
        // would leave the reason-9 stub pushed below and let
        // `emit_deopt_stubs` resolve it against whatever the enclosing method
        // later files at that same pc — the guarded-virtual miss edge at this very
        // pc is one such publisher. Flipping the whole arm instead takes the
        // shared sentinel exit, which is the pre-precise-frames path and the one
        // every non-protected invoke in the tree already uses.
        let precise_exc_stub = self.precise_exception_frames
            && self.pc_is_protected(throw_bci)
            && self.may_file_by_bci("exc_frame_box_ptr_by_bci", throw_bci);
    ...
    pub(super) fn emit_post_alloc_oom_check(&mut self) {
        ...
        let throw_bci = self.dbg_last_pc;
        // Same third clause, same reason, as `emit_post_invoke_exception_check`.
        // `inlining.rs` does NOT call this one — round 10 wave 8 checked, and the
        // claim in three comment blocks that the splice walk's `new` arm reaches
        // it is wrong (see this file's `may_file_by_bci`) — so the clause is pure
        // insurance here rather than a live edge. It is still worth having in the
        // arming condition and not on the insert: the two functions are read as a
        // pair, and one of them silently keeping a weaker rule is how the pair
        // stops being a pair.
        let precise_exc_stub = self.precise_exception_frames
            && self.pc_is_protected(throw_bci)
            && self.may_file_by_bci("exc_frame_box_ptr_by_bci", throw_bci);
    ...
    pub(super) fn local_handler_site_for(&mut self, throw_bci: usize) -> Option<usize> {
        ...
        self.local_handler_sites
            .push(Box::new(crate::JitLocalHandlerSite {
                ...
                implicit_cache: crate::JitLocalHandlerSite::empty_implicit_cache(),
            }));
        self.local_handler_site_by_bci.insert(throw_bci, idx);
        Some(idx)
    }

    /// Ask this method's own `catch` blocks to take an IMPLICIT exception,
    /// from inside the cold pad that just signalled it.
    ///
    /// Emitted in-line at the current buffer position, i.e. between the
    /// `jit_throw_aioobe` / `jit_npe_with_action` / `jit_throw_arithmetic`
    /// call and the epilogue that pad was going to run. On a hit the helper
    /// has built the throwable, cleared every signal it consumed and stored
    /// the throwable into the frame slot the handler's operand stack starts
    /// at, and the ladder jumps straight into the compiled handler block. On a
    /// miss the helper has put every signal back exactly as it found it and
    /// this falls THROUGH, so the pad's own epilogue runs and the pre-feature
    /// route is unchanged.
    ///
    /// Returns `true` when anything was emitted -- which is also "RAX has been
    /// clobbered by a `CALL`", so a caller whose epilogue returns a sentinel
    /// the throw helper left in RAX has to re-materialise it.
    ///
    /// ```text
    ...
    ///   ...
    ///   JMP  .miss                             ; unreachable; fail-closed
    /// .miss:
    /// ```
    ///
    /// `throw_bci` must be an INTERPRETER bci: the candidate list is filtered
    /// against the classfile exception table's `[start_pc, end_pc)`, so a
    /// caller holding an emitter pc passes it through [`Self::orig_bci`]
    /// first.
    ///
    /// # Why the state at an implicit trap is enterable
    ///
    /// The three obligations a jump into a handler block has, and why each
    /// holds here:
    ///
    ///  * **The handler block exists.** `local_handler_site_for` answers
    ///    `None` unless `local_handler_table` is non-empty, and that is
    ///    exactly the condition under which `bytecode_walk` roots every
    ///    `handler_pc` as live code and a branch target.
    ///  * **The operand stack is the one the block was compiled for.** A
    ///    revived handler block starts on the JVMS `[exception]`: depth one,
    ///    at `base_spill_offset`, marked as an oop. Whatever this method had
    ///    on its operand stack at the trap is discarded, which is what JVMS
    ///    says happens. No flush is therefore owed -- unlike the propagate
    ///    edge, which is why [`Self::local_handler_propagate_survives_a_call`]
    ///    guards the post-invoke route and nothing guards this one: the miss
    ///    edge here is the pad's own epilogue, which reads RAX and nothing
    ///    else.
    ///  * **The locals survive the helper `CALL`.** A Java local is homed in
    ///    `LOCAL_REGS` (callee-saved on both ABIs) or in a frame slot, and
    ///    `LOCAL_XMMS` homes exist only where the ABI makes XMM8-15
    ///    non-volatile (`xmm_local_homes_enabled` is `cfg!(windows)`). The pad
    ///    already ran one C-ABI call before this one.
    pub(super) fn emit_local_handler_implicit_probe(&mut self, throw_bci: usize) -> bool {
        if !crate::implicit_local_handlers_enabled() {
            return false;
        }
        // The helper's first argument is the `SharedVm` this frame was entered
        // with, and the only place it lives is `heap_local_offset`. Without
        // `needs_heap` that slot was never reserved, `heap_local_offset` is 0,
        // and `[rbp - 0]` is the caller's saved RBP — a stack address the
        // helper would dereference as a `SharedVm`.
        //
        // This is NOT theoretical for these pads, which is why the guard is
        // here and not on `emit_local_handler_stubs`: that emitter's sites are
        // all post-invoke, and every invoke opcode sets `needs_heap` because
        // `jit_invoke_dispatch` needs the same pointer. A primitive array
        // load/store and an `idiv` set nothing (`x64/bytecode_compat.rs` —
        // only `aastore` among the array stores does, for its write barrier),
        // so a method whose only fallible sites are these CAN reach here with
        // no VM pointer in the frame. It keeps the pre-feature pad; the
        // alternative, passing the null the helper does check for, would buy a
        // guaranteed `-1` for the price of a call.
        if !self.needs_heap {
            return false;
        }
        let Some(site_idx) = self.local_handler_site_for(throw_bci) else {
            return false;
        };
        // Out-of-line code reached by a branch: no register/slot pairing
        // recorded in the body describes it. Same reason
        // `emit_local_handler_stubs` clears it.
        self.slot_mirror = None;
        crate::metrics::note_local_handler(crate::metrics::LOCAL_HANDLER_IMPLICIT_SITE_EMITTED);
        let operand_slot_0 = self.base_spill_offset;
        self.emit_load_local(ARG_REGS[0], self.heap_local_offset);
        // Owned by `local_handler_sites`, which is moved onto the published
        // `CompiledMethod`: the address outlives the machine code that bakes
        // it and is freed with it. `Vec<Box<..>>`, so a later `push` cannot
        // move the pointee.
        ...
        // TEST EAX, EAX — the result is a small index or -1, so the 32-bit
        // form is enough and the sign flag carries the answer.
        self.buf.emit(&[0x85, 0xC0]);
        // JS .miss (0F 88 rel32)
        self.buf.emit(&[0x0F, 0x88]);
        let miss_patch = self.buf.pos();
        ...
        for (i, handler_bci) in handlers.iter().enumerate() {
            // CMP EAX, imm8 (83 F8 ib) — bounded by MAX_LOCAL_HANDLER_CANDIDATES.
        ...
        // Unreachable: the helper only ever returns an index it was given.
        // Emitted anyway so a future candidate-list change cannot walk off the
        // end of the ladder into the epilogue with a live index in EAX.
        self.buf.emit(&[0xE9]);
        let tail_patch = self.buf.pos();
        self.buf.emit(&[0x00, 0x00, 0x00, 0x00]);
        // .miss — both the `JS` and the fail-closed `JMP` land here, which is
        // wherever the caller continues emitting.
        let miss = self.buf.pos();
        // Cast: x86-64 rel32 displacements, both ends buffer positions.
        let rel_js = (miss as i32) - (miss_patch as i32 + 4);
        self.buf.try_patch_i32(miss_patch, rel_js).ok(); // on Err the buffer is marked overflowed; the compile bails
        let rel_tail = (miss as i32) - (tail_patch as i32 + 4); // Cast: x86-64 rel32 displacement
        self.buf.try_patch_i32(tail_patch, rel_tail).ok(); // ditto
        true
    }
    ...
    pub(super) fn emit_deopt_stubs(&mut self) {
        ...
        for &(patch_off, site_pc, reason) in &stubs {
            ...
            if reason == 3 {
                ...
                // This method's own `catch (ArithmeticException)` may take it
                // without leaving the frame; a miss falls through. The probe's
                // `CALL` clobbers the sentinel the helper returned, so the
                // miss edge re-materialises it.
                if self.emit_local_handler_implicit_probe(self.orig_bci(site_pc)) {
                    self.emit_mov_imm64(RAX, i64::MIN);
                }
                // RAX holds i64::MIN (from the helper return, or restored
                // above); run the standard epilogue (preserves RAX) and return
                // to the caller.
                self.emit_epilogue();
                let rel32 = (stub_off as i32) - (patch_off as i32 + 4); // Cast: x86-64 rel32 displacement
                self.buf.try_patch_i32(patch_off, rel32).ok(); // on Err try_patch_i32 set buf.overflowed; compile bails
            ...
            let frame_box_ptr = if crate::deopt_real_enabled() || matches!(reason, 8..=11) {
                match reason {
                    // The stub TAG IS the reason, so each lookup names its own
                    // half of the key. Before the key carried a reason these two
                    // arms were textually identical and read one entry: at a pc
                    // reached by both producers, whichever filed first was baked
                    // into BOTH stubs, and the other reason's point was never
                    // built at all (`snapshot_pre_intrinsic_call` returned early).
                    // A miss here is the same benign fall-through it always was —
                    // the shared, frame-less sentinel exit.
                    2 => self
                        .deopt_box_ptr_by_bci
                        .get(&(site_pc, crate::deopt::DeoptReason::BoundsCheck))
                        .copied(),
                    ...
                    6 => self
                        .deopt_box_ptr_by_bci
                        .get(&(site_pc, crate::deopt::DeoptReason::ReceiverTypeChanged))
                        .copied(),
            ...
            if let Some(box_ptr) = frame_box_ptr {
                ...
                if reason == 11 {
                    ...
                    self.emit_call_absolute(self.helpers.throw_aioobe);
                    // This method's own `catch` may take the AIOOBE without
                    // leaving the frame -- which is the whole reason a precise
                    // frame was published here, so a hit skips reconstructing
                    // one. A miss falls through to the entry call below, which
                    // reads every register home out of the spill region taken
                    // ABOVE this probe, so nothing the probe's `CALL` clobbers
                    // can reach it.
                    self.emit_local_handler_implicit_probe(bci);
                }
                if reason == 10 {
                    ...
                    self.emit_call_absolute(self.helpers.jit_npe_with_action);
                    // Same as reason 11: this method's own `catch` may take
                    // the NPE in compiled code, and a miss falls through to
                    // the spill-region-driven entry call below.
                    self.emit_local_handler_implicit_probe(bci);
                }
...
#[cfg(test)]
mod implicit_local_handler_pad_tests {
    //! The implicit half of the compiled-local-handler feature
    //! (`CRATONVM_JIT_IMPLICIT_LOCAL_HANDLERS`, 2026-09-20): a bounds check, an
    //! inline null check or a zero-divisor guard whose bci this method can
    //! catch asks `jit_local_handler_lookup` before running its epilogue.
    //!
    //! These assert the BYTES, because the behavioural half
    //! (`vm/tests/jit_implicit_local_handler_tests.rs`) needs a built binary
    //! and a JDK and skips itself without them.

    use crate::x64::{Compiler, RAX};

    /// A bare `Compiler` with a wired `throw_aioobe` / `local_handler_lookup`
    /// and the exception table the test wants. Shaped like
    /// `flag_and_header_contracts::bounds_check_test_compiler`, which it
    /// deliberately does not reuse: that fixture is the no-table control and
    /// must keep emitting exactly what it emitted before this feature.
    fn pad_compiler(table: Vec<(usize, usize, usize, &'static str)>) -> Compiler {
        ...
        // Any non-zero, in-reach addresses: the pads only have to be
        // emittable, they are never executed here.
        helpers.throw_aioobe = pad_compiler as usize;
        helpers.local_handler_lookup = pad_compiler as usize + 0x40;
        let mut c = Compiler::new(
            "implicit-local-handler-pad-test".to_string(),
            crate::ExecutableBuffer::new(8192).expect("test executable buffer"),
            0,
            0,
            8,
            // `needs_heap`: the probe refuses a frame with no VM pointer, so a
            // fixture that wants to see it emitted has to reserve the slot.
            true,
            Vec::new(),
            Vec::new(),
            Vec::new(),
        ...
        c.local_handler_table = table;
        c.local_handler_class_id = 7;
        c
    }

    /// [`pad_compiler`] with `needs_heap` false: a method whose only fallible
    /// sites are a primitive array access and an `idiv` reserves no VM-pointer
    /// slot, which is the shape the probe's own guard exists for.
    fn heapless_pad_compiler(table: Vec<(usize, usize, usize, &'static str)>) -> Compiler {
        let alloc_result = crate::regalloc::RegAllocResult {
            assignments: Vec::new(),
            xmm_assignments: Vec::new(),
            used_callee_saved: Vec::new(),
        ...
        helpers.local_handler_lookup = pad_compiler as usize + 0x40;
        let mut c = Compiler::new(
            "implicit-local-handler-heapless-test".to_string(),
            crate::ExecutableBuffer::new(8192).expect("test executable buffer"),
            0,
            0,
            8,
    ...
    /// `MOV RAX, imm64` with the deopt sentinel — what the miss edge owes the
    /// epilogue once the probe's `CALL` has clobbered RAX.
    fn sentinel_reload() -> Vec<u8> {
        let mut v = vec![0x48, 0xB8];
        v.extend_from_slice(&(i64::MIN as u64).to_le_bytes());
        v
    }
    ...
    /// The control. No exception table ⇒ no site ⇒ the pad is byte-for-byte
    /// the pre-feature pad: the throw helper's sentinel goes straight to the
    /// epilogue and RAX is never reloaded.
    #[test]
    fn a_method_with_no_exception_table_emits_the_pre_feature_bounds_pad() {
        let mut c = pad_compiler(Vec::new());
        c.emit_bounds_check(0);
        let start = c.buf.pos();
        c.emit_bounds_check_stubs();
        let pad: Vec<u8> = c.buf.as_slice()[start..c.buf.pos()].to_vec();
        assert!(
            !contains(&pad, &sentinel_reload()),
            "no table, so nothing clobbers RAX and nothing may reload it"
        );
        assert_eq!(c.local_handler_sites.len(), 0);
        let _ = RAX;
    }

    /// A `try` that does NOT cover the trapping bci is the same control: the
    /// candidate filter is a range test, and an empty candidate list is no
    /// site.
    #[test]
    fn a_range_that_does_not_cover_the_bci_arms_nothing() {
        // Protected [4, 9); the bounds check is at 0.
        let mut c = pad_compiler(vec![(4, 9, 12, "java/lang/RuntimeException")]);
        c.emit_bounds_check(0);
        let start = c.buf.pos();
        c.emit_bounds_check_stubs();
        let pad: Vec<u8> = c.buf.as_slice()[start..c.buf.pos()].to_vec();
        assert!(!contains(&pad, &sentinel_reload()));
        assert_eq!(c.local_handler_sites.len(), 0);
    }

    /// The armed case: one covering entry ⇒ one site, a `CALL` to the lookup
    /// helper, one `CMP EAX, 0 ; JE` rung resolved through `forward_patches`,
    /// and the sentinel reload the epilogue needs on the miss edge.
    #[test]
    fn a_covering_handler_arms_the_bounds_pad_with_a_lookup_and_a_ladder() {
        if !crate::implicit_local_handlers_enabled() {
            // The kill switch is set in this process; the pad is the control
            // shape and the assertions below would be testing it against
            // itself.
            return;
        }
        let mut c = pad_compiler(vec![(0, 9, 12, "java/lang/ArrayIndexOutOfBoundsException")]);
        c.emit_bounds_check(0);
        let start = c.buf.pos();
        let patches_before = c.forward_patches.len();
        c.emit_bounds_check_stubs();
        let pad: Vec<u8> = c.buf.as_slice()[start..c.buf.pos()].to_vec();

        assert_eq!(
            c.local_handler_sites.len(),
            1,
            "one covering entry is one site"
        );
        assert_eq!(
            c.local_handler_sites[0].candidates.len(),
            1,
            "the candidate list is the table filtered by the throwing bci"
        );
        assert_eq!(c.local_handler_sites[0].throw_bci, 0);
        // TEST EAX, EAX ; JS rel32 — the "-1, not ours" edge.
        assert!(
            contains(&pad, &[0x85, 0xC0, 0x0F, 0x88]),
            "the pad must branch out on the helper's -1"
        );
        // CMP EAX, 0 ; JE rel32 — the rung for candidate 0.
        assert!(
            contains(&pad, &[0x83, 0xF8, 0x00, 0x0F, 0x84]),
            "the pad must test the returned candidate index"
        );
        assert!(
            contains(&pad, &sentinel_reload()),
            "the probe's CALL clobbers RAX, so the miss edge owes the epilogue \
             the i64::MIN the throw helper returned"
        );
        assert_eq!(
            c.forward_patches.len(),
            patches_before + 1,
            "the handler jump resolves against pc_to_native like any branch"
        );
        assert_eq!(
            c.forward_patches.last().map(|&(_, target)| target),
            Some(12),
            "and it targets the handler bci"
        );
    }

    /// The probe must drop the body's register/slot pairing before it emits:
    /// a mirror recorded in the body describes a position in the body, and
    /// this pad is reached by a branch from somewhere else entirely, so a
    /// surviving pairing would let `emit_load_local` elide a reload that is
    /// not actually redundant here.
    ///
    /// It does NOT have to end `None`: the probe's own
    /// `emit_load_local(heap_local_offset)` records a fresh, correct mirror
    /// for the instruction it just emitted. What must not survive is the
    /// STALE one.
    #[test]
    fn the_probe_drops_the_bodys_stale_slot_mirror() {
        if !crate::implicit_local_handlers_enabled() {
            return;
        }
        let mut c = pad_compiler(vec![(0, 9, 12, "")]);
        let stale = (0x18i32, RAX, 0usize);
        c.slot_mirror = Some(stale);
        assert!(c.emit_local_handler_implicit_probe(0));
        assert_ne!(
            c.slot_mirror,
            Some(stale),
            "the body's pairing survived into out-of-line code"
        );
        if let Some((off, _, pos)) = c.slot_mirror {
            assert_eq!(
                off, c.heap_local_offset,
                "the only mirror the probe may leave is its own VM-pointer load"
            );
            assert_ne!(pos, 0, "and it names a position inside the pad");
        }
    }

    /// Two fallible operations at one bci share ONE site, so they share one
    /// monomorphic cache — the property `local_handler_site_by_bci` exists
    /// for, restated for the implicit pads that now also create sites.
    #[test]
    fn two_probes_at_one_bci_share_one_site() {
        if !crate::implicit_local_handlers_enabled() {
            return;
        }
        let mut c = pad_compiler(vec![(0, 9, 12, "")]);
        assert_eq!(c.emit_local_handler_implicit_probe(0), true);
        assert_eq!(c.emit_local_handler_implicit_probe(0), true);
        assert_eq!(c.local_handler_sites.len(), 1);
    }

    /// A frame with no VM pointer cannot call the lookup helper at all, and
    /// the pad must not pass it `[rbp - 0]` — the caller's saved RBP. The one
    /// difference from the armed fixture is `needs_heap`.
    #[test]
    fn a_frame_with_no_heap_slot_refuses_the_probe() {
        if !crate::implicit_local_handlers_enabled() {
            return;
        }
        let mut c = heapless_pad_compiler(vec![(0, 9, 12, "")]);
        assert!(!c.emit_local_handler_implicit_probe(0));
        assert_eq!(
            c.local_handler_sites.len(),
            0,
            "a refused probe must not create a site either"
        );
    }

    /// A fresh site starts with every implicit kind unresolved — `-1` is the
    /// resolved answer "this frame does not catch this kind" and must not be
    /// what a never-asked slot reads as.
    #[test]
    fn a_new_sites_implicit_cache_is_unresolved_not_no_handler() {
        if !crate::implicit_local_handlers_enabled() {
            return;
        }
        let mut c = pad_compiler(vec![(0, 9, 12, "")]);
        assert!(c.emit_local_handler_implicit_probe(0));
        for slot in [
            crate::JitLocalHandlerSite::IMPLICIT_KIND_AIOOBE,
            crate::JitLocalHandlerSite::IMPLICIT_KIND_NPE,
            crate::JitLocalHandlerSite::IMPLICIT_KIND_ARITHMETIC,
        ] {
            assert_eq!(
                c.local_handler_sites[0].implicit_cache[slot]
                    .load(std::sync::atomic::Ordering::Relaxed),
                crate::JitLocalHandlerSite::IMPLICIT_UNRESOLVED,
            );
        }
    }
}
...
mod r9w2_spdeopt_tests {
    ...
    // -----------------------------------------------------------------------
    // Round 10 wave 8 (`bybci`): the bare-bci key space has exactly one owner.
    // -----------------------------------------------------------------------

    /// The rule, in isolation. `splice_depth == 0` is the ONLY configuration in
    /// which a bare bci names one method's bytecode; every depth is ambiguous and
    /// none is less ambiguous than another.
    ///
    /// Worth a test of its own precisely because the answer looks too obvious to
    /// need one: `r10-splice-by-bci-deopt-maps-have-no-owner-FIXED-20260922.md` proposes
    /// keying the four maps on this same depth, which would make depth 1 look like
    /// an owner — and it is not one, because two splices of the same callee at two
    /// different caller pcs are BOTH depth 1 and would share every entry. The
    /// predicate deliberately says "one owner or none", not "which owner".
    #[test]
    fn only_depth_zero_owns_the_bare_bci_key_space() {
        assert!(super::by_bci_key_space_has_one_owner(0));
        for depth in 1..8usize {
            assert!(
                !super::by_bci_key_space_has_one_owner(depth),
                "depth {depth} shares the key space with the enclosing method"
            );
        }
    }

    /// The consequence, through a real emitter, on the population that no
    /// workload can produce.
    ///
    /// This is the half the known-issues page said could not be had ("there is
    /// nothing to observe at runtime … the population is empty, so no counter,
    /// trace or census can distinguish this from 'cannot happen'"). That is true
    /// of a *workload*, and it is not true of a unit test: `inline_callee_scopes`
    /// is a plain field, so seeding it reaches exactly the state four interlocks
    /// keep a compile out of, and the guard can be exercised rather than reasoned
    /// about. A widened `(owner, bci)` key would have no equivalent — there is no
    /// second method's pc to collide with here, only the invariant to check.
    ///
    /// `emit_bounds_check` is the vehicle because its refusal is observable in
    /// three independent places: no `exc_frame_box_ptr_by_bci` entry (the key
    /// space stayed clean), no reason-11 entry in `deopt_stubs` (the frame
    /// trampoline was not armed), and one entry in `bounds_check_stubs` (the
    /// shared, frame-less AIOOBE pad took it instead). A guard that silently
    /// emitted nothing at all would pass the first two and fail the third.
    #[test]
    fn a_splice_cannot_file_in_the_bare_bci_key_space() {
        const PC: usize = 0x20;
        let mut c = test_compiler(true, vec![(0, 0x100)]);

        // Baseline: outside a splice this pc DOES get the precise arm. Without
        // this half the test below would pass on a compiler that had stopped
        // emitting precise bounds checks altogether.
        c.emit_bounds_check(PC);
        assert!(
            c.exc_frame_box_ptr_by_bci.contains_key(&PC),
            "the precise arm must still fire outside a splice"
        );
        assert_eq!(c.deopt_stubs.iter().filter(|s| s.2 == 11).count(), 1);
        assert!(c.bounds_check_stubs.is_empty());

        // Now the same emitter at a DIFFERENT pc with a splice open. `key` and
        // `max_locals` are the shape `push_inline_scope` builds from an
        // `InlineSite`; `caller_stack_floor` is 0 because this bare compiler has
        // no abstract operand stack.
        const SPLICED_PC: usize = 0x40;
        c.inline_callee_scopes.push(crate::x64::InlineCalleeScope {
            key: "Callee.leaf:(I)I".to_string(),
            max_locals: 2,
            caller_stack_floor: 0,
        });
        let points_before = c.deopt_points.len();
        c.emit_bounds_check(SPLICED_PC);

        assert!(
            !c.exc_frame_box_ptr_by_bci.contains_key(&SPLICED_PC),
            "a splice filed in the bare-bci key space: the enclosing method's own \
             guard at this bci would find this entry, skip its own publication, \
             and bake a pointer to a frame describing the spliced callee"
        );
        assert_eq!(
            c.deopt_points.len(),
            points_before,
            "the refused arm must not record a point either — it never reaches \
             `build_and_record_deopt_point`"
        );
        assert_eq!(
            c.deopt_stubs.iter().filter(|s| s.2 == 11).count(),
            1,
            "no second reason-11 stub: the refused bounds check must not arm the \
             frame trampoline it has no frame for"
        );
        assert_eq!(
            c.bounds_check_stubs.len(),
            1,
            "the refusal must FALL BACK to the shared AIOOBE pad, not emit an \
             unguarded access"
        );
        assert_eq!(c.bounds_check_stubs[0].1, SPLICED_PC);

        // And the original entry is untouched, which is the substitution the
        // known-issues page calls the worse direction: had the splice filed at
        // `PC` instead, this pointer would now name the callee's frame.
        assert!(c.exc_frame_box_ptr_by_bci.contains_key(&PC));
        assert_eq!(c.exc_frame_box_ptr_by_bci.len(), 1);
    }

    /// Round 10 wave 8 follow-up: one bci carries a reason-2 AND a reason-6
    /// snapshot, and the two are told apart by the key rather than by which
    /// producer ran first.
    ///
    /// The shape this reproduces is the one the filing page could not establish
    /// by reading: a loop header whose guard block files `BoundsCheck` at a pc,
    /// followed by the invoke AT THAT SAME pc asking for a `ReceiverTypeChanged`
    /// snapshot. Emission order puts the header's guard block first, so before
    /// the key carried a reason it was the receiver guard that was dropped —
    /// `snapshot_pre_intrinsic_call`'s `contains_key(&bci)` returned early, the
    /// point was never built, and `emit_deopt_stubs`' `6 =>` arm read the
    /// `BoundsCheck` box.
    ///
    /// The assertion that matters is the last one. The frame CONTENTS were never
    /// wrong (both producers describe the same bci from the same provenance), so
    /// a test on the box alone would have passed before the fix too. What was
    /// wrong is what a bci-keyed reader concludes: `deopt_reason_at_bci`
    /// answered `Unique(BoundsCheck)` for a pc that genuinely carries both, and
    /// `recommend_action` then denied the receiver guard its de-spec sparing
    /// path. `Ambiguous` is the honest answer and every caller in the tree
    /// handles it conservatively.
    #[test]
    fn one_bci_carries_a_reason_2_and_a_reason_6_snapshot_independently() {
        use crate::deopt::DeoptReason::{BoundsCheck, ReceiverTypeChanged};
        const PC: usize = 0x30;
        let mut c = test_compiler(true, Vec::new());

        // The loop-header guard block, emitted before the instruction at PC.
        c.emit_deopt_snapshot_at_guard(PC);
        assert!(c.deopt_box_ptr_by_bci.contains_key(&(PC, BoundsCheck)));
        let after_first = c.deopt_points.len();
        assert_eq!(after_first, 1);

        // The invoke at the same pc. Before the reason component this call was a
        // silent no-op.
        c.snapshot_pre_intrinsic_call(PC, ReceiverTypeChanged);
        assert!(
            c.deopt_box_ptr_by_bci
                .contains_key(&(PC, ReceiverTypeChanged)),
            "the receiver guard's snapshot was suppressed by the bounds check's \
             entry at the same bci — the collision this key component exists for"
        );
        assert_eq!(
            c.deopt_points.len(),
            after_first + 1,
            "the second point must be BUILT, not skipped"
        );

        // Two distinct boxes, each stamped with its own reason. `emit_deopt_stubs`
        // bakes these pointers as imm64 into the tag-2 and tag-6 stubs
        // respectively, so this is the property those two lookups rely on.
        let bounds = c.deopt_box_ptr_by_bci[&(PC, BoundsCheck)];
        let receiver = c.deopt_box_ptr_by_bci[&(PC, ReceiverTypeChanged)];
        assert_ne!(bounds, receiver);
        // SAFETY: both pointers name `deopt_boxes` elements owned by `c`, which
        // outlives this block and is not mutated between the reads.
        unsafe {
            assert_eq!((*bounds).reason, BoundsCheck);
            assert_eq!((*receiver).reason, ReceiverTypeChanged);
        }

        // Idempotence survives, PER PAIR: a second call for the same
        // `(bci, reason)` is still a no-op.
        c.snapshot_pre_intrinsic_call(PC, ReceiverTypeChanged);
        assert_eq!(c.deopt_points.len(), after_first + 1);

        // The consumer's answer, which is the whole cost of the old key.
        assert!(
            matches!(
                crate::osr_exit::deopt_reason_at_bci(&c.deopt_points, PC as u32),
                crate::osr_exit::ReasonAtBci::Ambiguous
            ),
            "with both points present the bci-keyed reader must refuse to guess; \
             a `Unique(_)` here is the confident wrong answer the suppression \
             used to manufacture"
        );
    }
}
