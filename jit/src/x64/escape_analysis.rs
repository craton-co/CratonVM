...
pub(super) fn analyze_escapes(
    ...
    analyze_escapes_with_handlers(code, code_len, invokespecial_shapes, None)
}

/// The `goto`-over-handler straight-line map for one method, plus the targets
/// it exempts from the merge barrier.
///
/// `None` handlers means "the caller does not have this method's exception
/// table", and the answer is then a pair of all-`false` maps: every `goto`
/// stays a hard barrier, which is the pre-2026-09-22 behaviour. That is the
/// conservative direction and it is deliberate — see
/// `bytecode_analysis::straight_line_gotos` for why an empty slice must not be
/// used to mean "no handlers" here.
///
/// The second map is what makes the first usable. A straight-line `goto`'s
/// target is still a branch target, so the merge barrier at the TOP of both
/// walks would undo the see-through the instant the walk arrived. Exempting it
/// is sound for exactly the reason the `goto` qualified: the target has exactly
/// one ordinary predecessor, so the linear state on arrival is the only state
/// that reaches it, and there is nothing to merge.
fn straight_line_goto_maps(
    code: &[u8],
    code_len: usize,
    handler_pcs: Option<&[usize]>,
) -> (Vec<bool>, Vec<bool>) {
    let Some(handler_pcs) = handler_pcs else {
        // EMPTY, not `vec![false; code_len]`. Both maps are only ever read
        // through `.get(pc).copied().unwrap_or(false)`, which answers `false`
        // for an empty vec exactly as it does for an all-`false` one — and this
        // arm is the hot one: `x64/bytecode_compat`'s conservative pre-pass
        // runs on every method the scanner looks at and passes `None`.
        return (Vec::new(), Vec::new());
    };
    let straight = bytecode_analysis::straight_line_gotos(code, code_len, handler_pcs);
    let mut exempt = vec![false; code_len];
    for (pc, ok) in straight.iter().enumerate() {
        if *ok {
            if let Some(t) =
...
/// [`analyze_escapes`] with the method's exception-handler entry pcs, which is
/// what licenses the `goto`-over-handler see-through (`EA-2` in
/// `docs/feature-designs/jit-r10-earelock-proposals.md`).
///
/// `handler_pcs = None` is the caller saying it does not have the exception
/// table. Every `goto` then stays a hard barrier and this behaves exactly as it
/// did before the see-through existed.
pub(super) fn analyze_escapes_with_handlers(
    code: &[u8],
    code_len: usize,
    invokespecial_shapes: &FxHashMap<usize, InvokeSpecialShape>,
    handler_pcs: Option<&[usize]>,
) -> std::collections::HashSet<usize> {
    let (straight_gotos, barrier_exempt) = straight_line_goto_maps(code, code_len, handler_pcs);
    ...
    while pc < code_len {
        ...
        // EA-2: a target whose ONLY ordinary predecessor is a straight-line
        // `goto` this walk is about to step through is not a merge. See
        // `straight_line_goto_maps`.
        if branch_targets.get(pc).copied().unwrap_or(false)
            && !barrier_exempt.get(pc).copied().unwrap_or(false)
        {
            escape_all!();
        ...
        match op {
            ...
            //
            // Byte for byte the `pop` arm above, and that is the entire stack
            // contract of both opcodes: each consumes exactly one reference and
            // pushes nothing. No descriptor, no constant-pool resolution,
            // nothing this single linear pass cannot know. It is the easiest
            // exact arm in this function.
            //
            // # This is a LOOSENING, and the note it replaces said so
            //
            // Until 2026-09-22 both opcodes fell into the catch-all below,
            // which does not pop: it escapes every tracked operand, overwrites
            // every slot with `None`, and advances. Because a `synchronized(f)`
            // block always has `f` on the operand stack at the
            // `monitorenter`/`monitorexit` (`aload f; monitorenter`), EVERY
            // object that was ever the target of a `synchronized` block was
            // unconditionally escaped, however local its lifetime.
            //
            // Giving an opcode an exact arm removes the catch-all's incidental
            // SHIELD (see the invariant written on it), so every exact arm here
            // is justified by a named defect rather than by neatness. This one's
            // justification is `docs/internal/fixed-bugs/
            // r10-ea-single-pass-monitor-scalar-relock-is-unreachable-FIXED-20260922.md`:
            // the backend carries an entire dedicated feature for relocking a
            // scalar-replaced object's monitor on deopt ("Phase C", below, plus
            // `x64/deopt_stubs.rs` and `op_object.rs`'s elision arm), and this
            // catch-all was the reason it could never receive a candidate.
            //
            // # Why keeping the receiver non-escaping is sound
            //
            // Only because of a coupling the surrounding code already enforces:
            // scalar-replacing a monitored object is safe ONLY if the monitor op
            // is also elided. A scalar-replaced `new` pushes a dummy zero
            // reference, so a surviving `jit_monitor_enter` on that receiver
            // would be a call on a null. `op_object.rs`'s `0xC2`/`0xC3` arm keys
            // its elision on `sr_monitor_scalar_ops`, which is derived from
            // `final_objects` — the same set `objects` is pruned to — so an
            // object that survives as a scalar has ALL of its monitor ops
            // elided, and one that is pruned or poisoned keeps both its real
            // allocation and its real locks. The two halves cannot disagree.
            //
            // Given that, each hazard resolves:
            //
            //  * CONTENTION / INFLATION. Impossible for a non-escaping object:
            //    no other thread holds a reference, so none can contend, and
            //    nothing can inflate a lock nobody else can reach.
            //  * `Object.wait()` / `notify()` / `hashCode()` /
            //    `Thread.holdsLock(o)`. All are `invokevirtual`/`invokestatic`,
            //    which hit this function's `escape_all!` arms. An object any of
            //    them is applied to is escaped, is not scalar-replaced, and
            //    keeps its real lock. Existing behaviour, not a new argument.
            //  * A SAFEPOINT INSIDE THE MONITOR. A GC has nothing to scan for an
            //    object that was never allocated; the lock it does not hold
            //    needs no root.
            //  * DEOPT WHILE HOLDING THE MONITOR. This is what Phase C exists
            //    for: `monitor_at[bci]` becomes
            //    `MonitorInfo { object: VirtualObjectRef(new_pc), lock_depth,
            //    relock: true }`, and the resume re-acquires the lock on the
            //    re-materialized object `lock_depth` times.
            //  * `IllegalMonitorStateException` FROM UNBALANCED LOCKING.
            //    Eliding removes a possible IMSE. For an object whose whole
            //    `new -> monitorenter -> monitorexit` lifetime is inside one
            //    region this pass admits at all, the locking is balanced by
            //    construction.
            //
            // # The arm is necessary and was NOT sufficient
            //
            // On its own it changed nothing for javac output, because a second
            // gate closed on the same population first: javac's mandatory
            // monitor handler makes `local_handler_reads_unsafe_local` true,
            // hence `precise_exception_frames` true, hence `non_escaping_for_sr`
            // the empty set in `x64/driver.rs`; and independently the `goto`
            // javac emits over that handler was a hard barrier that escaped the
            // receiver out of its locals. Both were removed in the same change
            // as this arm — EA-2 above for the `goto`, and the
            // precise-exception-frame admission in `x64/driver.rs` — which is
            // why this landed as one piece of work and not three.
            0xC2 | 0xC3 => {
                abs_stack.pop();
                pc += 1;
            }
            ...
            0x99..=0xa9 | 0xaa | 0xab | 0xbf | 0xc6 | 0xc7 | 0xc8 | 0xc9 => {
                // EA-2 (2026-09-22) — a forward `goto` whose target has exactly
                // one ordinary predecessor, and which skips only handler-only
                // code, is a straight-line continuation rather than a transfer.
                // `bytecode_analysis::straight_line_gotos` proves all three
                // conditions; the caller proves the fourth by passing the
                // handler pcs at all.
                //
                // This is the shape javac emits around every `synchronized`
                // block and every `try`/`finally`: the normal path jumps over
                // the catch-all handler to the instruction after it. Nothing
                // merges at that instruction and nothing in between runs, so the
                // state there IS the state here — but the barrier could not see
                // it, and escaped every object held in a local for a transfer
                // that transfers nothing.
                //
                // The `goto` is followed rather than stepped over: `pc` becomes
                // the target, so the handler-only region in between is never
                // walked. That matters as much as keeping the state — the
                // handler's own `astore <e>` would otherwise overwrite a local's
                // provenance on a path that cannot reach it.
                if straight_gotos.get(pc).copied().unwrap_or(false) {
            ...
            //
            // # The depth invariant this arm maintains (r10-earelock)
            //
            // This arm does NOT pop. An unmodeled opcode that really consumes
            // `k` slots leaves `abs_stack` exactly `k` entries deeper than the
            // real verification-time stack from here on. That is stated as an
            // invariant rather than a bug because it is load-bearing for the
            // soundness argument above, and because a future arm that "fixes"
            // the depth is a LOOSENING, not a tidy-up:
            //
            //   * every surplus entry is `None` (this arm overwrites the whole
            //     stack with `None` before leaving), and
            //   * `Vec::pop` on an exhausted stack yields `None` too,
            //
            // so a later consumer that reaches into the surplus reads the same
            // `None` it would have read from an exact model, and the surplus can
            // never turn a tracked object into an untracked one. The one thing
            // it CAN do is shield a tracked object from a consumer that would
            // not have escaped it (`pop`, `astore`) — and that direction ends in
            // MORE escapes, which is the safe one. Giving an opcode an exact arm
            // removes that shield, which is why every exact arm in this function
            // is justified by a named miscompile rather than by neatness.
            //
            // # `monitorenter` (0xC2) / `monitorexit` (0xC3) NO LONGER land here
            //
            // They had their own arm as of 2026-09-22, and it is worth knowing
            // why they were here for so long, because the answer was never
            // "the arm would be unsound". It was "the arm would change
            // nothing": two further gates closed on the same population first,
            // and removing one of three gates buys nothing measurable. All
            // three went in one change — see the arm's own comment, and
            // `docs/internal/fixed-bugs/` for the page that had them counted.
            //
            // The lesson that outlives them: an opcode sitting in this
            // catch-all is not evidence that modelling it is hard, and a
            // catch-all entry is not a decision anybody made. Adding an exact
            // arm is a LOOSENING (it removes the shield described above), so it
            // needs a named defect behind it -- but the absence of an arm needs
            // a reason too, and "nobody noticed" is how a whole dedicated
            // feature shipped unreachable for a year.
            _ => {
...
pub(super) struct ScalarReplacementPlan {
    ...
    /// (relockable on deopt). A monitor op NOT in this set operated on a
    /// non-scalar object, which WOULD keep the method off the resume path — but
    /// no such elision happens: `has_elided_monitor` is never set to `true`
...
pub(super) fn plan_scalar_replacement(
    ...
    plan_scalar_replacement_with_handlers(
        code,
        code_len,
        non_escaping_new,
        new_info,
...
/// [`plan_scalar_replacement`] with the method's exception-handler entry pcs.
///
/// This walk and [`analyze_escapes`] are a matched pair over the same bytecode,
/// and their agreement is load-bearing: an object the first admits and the
/// second models differently is half-replaced, which is a real field access on
/// a dummy-zero receiver. So the `goto`-over-handler see-through EA-2 gave
/// `analyze_escapes` has to arrive here in the same change, with the same
/// conditions computed by the same function, or the pair drifts in exactly the
/// direction the `poisoned` comment below is four paragraphs about.
///
/// `handler_pcs = None` disables the see-through, as it does there. Callers
/// must pass the SAME value to both, which `x64/driver.rs` does.
pub(super) fn plan_scalar_replacement_with_handlers(
    code: &[u8],
    code_len: usize,
    non_escaping_new: &std::collections::HashSet<usize>,
    new_info: &[(usize, u32, usize, bool, bool)],
    ...
    // AFTER the early return, not before it. `x64/driver.rs` calls this on every
    // compile, and the overwhelming majority hand it an empty set -- paying for
    // a reachability walk and a predecessor pass to then return `empty` is a
    // per-compile cost for nothing.
    let (straight_gotos, barrier_exempt) = straight_line_goto_maps(code, code_len, handler_pcs);
    ...
    // a scalar object is relockable on deopt (recorded here); one over a
    // non-scalar (escaping) object is the pre-existing blanket-elision case,
    // which WOULD keep the method off the resume path via `has_elided_monitor`
    // in codegen — except that flag is never set, because that elision never
    // happens (`r10-earelock-has-elided-monitor-is-never-set-FIXED-20260922.md`).
    ...
    let mut pc = 0usize;
    while pc < code_len {
        // EA-2: exempt exactly the targets `analyze_escapes` exempts. See
        // `straight_line_goto_maps`.
        if branch_targets.get(pc).copied().unwrap_or(false)
            && !barrier_exempt.get(pc).copied().unwrap_or(false)
        {
        ...
        //
        // r10-earelock: an entry recorded here becomes
        // `MonitorInfo { object: FrameValue::VirtualObjectRef(new_pc), .. }` in
        // `x64/deopt_stubs.rs`'s `build_frame_state_at`. A `VirtualObjectRef` is
        // an INTRA-FRAME id edge, not a value: it resolves only against the
        // `FrameValue::VirtualObject(new_pc)` that some LOCAL of the same frame
        // state defines, and the locals lane of that frame state is built from
        // `local_prov_at[bci]` alone. So a pc at which a scalar monitor is held
        // but NO local holds the object publishes a monitor entry pointing at a
        // shell nothing defines — `deopt::DeoptMetadataError::
        // UndefinedVirtualObjectRef` is exactly that defect.
        //
        // That defect is now caught on BOTH backends, which it was not when the
        // demotion below was written: round 10 wave 6 wired `DeoptVerifier` into
        // the single-pass finalize path too
        // (`x64/driver.rs`'s `verify_deopt_metadata` call, whose refusal records
        // the `deopt-metadata-violation` bail site), so the sentence that used
        // to sit here — "nothing in the single-pass backend would catch it:
        // `DeoptVerifier` is only ever run by `ir_lower.rs`" — is no longer
        // true. See
        // `docs/internal/fixed-bugs/r10-earelock-singlepass-deopt-metadata-is-never-verified-FIXED-20260922.md`.
        // The demotion stays: a producer that does not emit the malformed frame
        // is better than an install-time refusal of the whole artifact, and the
        // verifier is the backstop for the shapes this reasoning missed.
        //
        // The shape that produces it is `new; dup; <init>()V; dup; monitorenter`
        // — the receiver is locked straight off the operand stack and never
        // stored. `analyze_escapes` cannot currently hand such an object over
        // (see the `0xC2`/`0xC3` note on its catch-all), so this is a latent
        // defect rather than an observed one, and it is fixed by DEMOTION: the
        // object is poisoned, so the pruning below drops it from `final_objects`
        // and with it every monitor op, field op and init skip that named it.
        // The `new` is then allocated for real and the lock is really taken —
        // the same all-or-nothing coupling `final_monitor_scalar_ops` already
        // enforces in the other direction. Read, not executed.
        {
            ...
            if !held.is_empty() {
                for &(np, _) in &held {
                    if !local_prov.iter().any(|p| *p == Some(np)) {
                        poisoned.insert(np);
                    }
                }
        ...
        match op {
            ...
            // aload
            //
            // r10-earelock: the operand byte is read through a bounds test, as
            // `analyze_escapes`' own `0x19` arm has always done. This walk read
            // `code[pc + 1]` bare, so a method whose LAST byte is a truncated
            // `aload` panicked the compiler instead of bailing it. That is a
            // permanent decline rather than one lost compile attempt
            // (`jit/tests/panic_free_compile_ratchet.rs` argues why the two are
            // not the same outcome), and this module's header declares a
            // no-panic gate. The guard cannot change the answer for any
            // well-formed method: `pc + 1 < code_len` fails only at a final
            // instruction the verifier would already have rejected. (The old
            // `idx < 256` test was dead — a `u8` index is always in range for a
            // 256-entry array.) Read, not executed.
            0x19 => {
                abs_stack.push(if pc + 1 < code_len {
                    local_prov[code[pc + 1] as usize] // Widening: always safe
                } else {
                    None
                });
                pc += 2;
            ...
            //
            // r10-earelock: bounds-guarded, as the twin arm in
            // `analyze_escapes` already is — see the `0x19` arm above for why a
            // truncated tail instruction must bail rather than panic.
            0x36..=0x39 => {
                if let Some(p) = abs_stack.pop().flatten() {
                    poisoned.insert(p);
                }
            ...
            // astore (r10-earelock: bounds-guarded, as above; `analyze_escapes`
            // likewise pops only inside the guard, so the two walks still agree
            // instruction for instruction on a truncated tail).
            0x3A => {
                if pc + 1 < code_len {
                    let idx = code[pc + 1] as usize; // Widening: always safe
                    let val = abs_stack.pop().flatten();
            ...
            // goto — control transfer; same hard barrier as above, EXCEPT for
            // the straight-line case EA-2 admits.
            0xA7 => {
                // The identical test `analyze_escapes` makes, from the identical
                // map. If these two ever disagree about a `goto`, one walk keeps
                // a provenance the other dropped, and a field op past the jump
                // binds to a scalar object the other pass did not admit.
                if straight_gotos.get(pc).copied().unwrap_or(false) {
                    if let Some(t) = bytecode_analysis::offset_branch_target(code, pc) {
                        pc = t;
                        continue;
...
mod r9_ea_tests {
    use super::{
        analyze_escapes, analyze_escapes_with_handlers, find_modified_locals,
        match_invariant_aaload, plan_scalar_replacement, InvokeSpecialShape,
    };
    ...
    /// `Foo f = new Foo(); synchronized(f) { f.x = 1; }` — the exact bytecode
    /// `s32_phase_c_plan_tracks_scalar_monitors` (`jit/src/x64/tests.rs`) uses
    /// to exercise `plan_scalar_replacement`'s monitor-relock ("Phase C")
    /// code. That test hands `plan_scalar_replacement` a HAND-BUILT
    /// `non_escaping` set rather than this function's real output — it never
    /// asks what `analyze_escapes` itself says about this bytecode. This test
    /// closes that gap.
    ///
    /// **THE ASSERTION FLIPPED ON 2026-09-22.** It was written as `!contains`,
    /// with a doc comment saying it was "expected to FLIP the day
    /// `analyze_escapes` gains precise `monitorenter`/`monitorexit` arms that
    /// pop the receiver without escaping it". Those arms exist now, so the
    /// answer is `contains`: `f` is confirmed NON-escaping, Phase C can receive
    /// a candidate, and `plan_scalar_replacement`'s monitor-relock machinery —
    /// real, tested, and consumed by `x64/op_object.rs`'s elision arm — is
    /// reachable in production for the first time.
    ///
    /// Kept rather than deleted, and kept under its original name, because a
    /// flipped pin is the cheapest possible detector for the change being
    /// reverted or walked back: if this reads `!contains` again, the whole
    /// feature below it is inert again and nothing else in the tree would say
    /// so. The three-line history is the point.
    ///
    /// What it does NOT prove, and what its sibling tests are for: this is the
    /// shape with no exception table, which javac never emits for a
    /// `synchronized` block. `the_goto_over_javacs_monitor_handler_is_walked_through_when_the_handlers_are_known`
    /// covers the real javac shape, and it needed EA-2 as well as these arms —
    /// the arms alone were provably worth nothing on compiler output, which is
    /// why they had not been written.
    #[test]
    fn monitorenter_on_an_otherwise_local_receiver_no_longer_escapes_it() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            ...
            0xc2, // 11: monitorenter       -> lock f
            0x2b, // 12: aload_1            -> [f]
            0x04, // 13: iconst_1           -> [f, 1]
            0xb5, 0x00, 0x03, // 14: putfield #3
            0x2c, // 17: aload_2            -> [f]
            0xc3, // 18: monitorexit        -> unlock
            0xb1, // 19: return
        ];
        let len = code.len();
        assert_eq!(
            len, 20,
            "sanity: this must match the bytecode s32_phase_c_plan_tracks_scalar_monitors uses"
        );
        assert!(
            analyze_escapes(&code, len, &trivial_init_at(4)).contains(&0),
            "`monitorenter`/`monitorexit` have exact arms: they pop one slot \
             and do not escape what they popped. `analyze_escapes` returns the \
             CONFIRMED NON-ESCAPING set, so the receiver's `new` pc must be \
             PRESENT in it. A failure here means the arms were reverted and \
             Phase C is inert again -- see this test's own doc comment."
        );
    }

    /// The second half of the round-10 monitor finding, KEPT AS A HISTORICAL
    /// PIN after the fix: giving `analyze_escapes` the exact
    /// `monitorenter`/`monitorexit` arms that page asked for would NOT, ON ITS
    /// OWN, make the synchronized receiver non-escaping in the shape javac
    /// actually emits.
    ///
    /// That is still true and it is still worth executing, but read the
    /// assertions below with the 2026-09-22 change in mind: the arms exist now,
    /// so the `javac` array's receiver no longer escapes at its `monitorenter`
    /// — it escapes at the `goto`, which is the finding. This test calls
    /// `analyze_escapes`, i.e. the see-through DISABLED (`handler_pcs = None`),
    /// which is why the answer is unchanged. Its sibling
    /// `the_goto_over_javacs_monitor_handler_is_walked_through_when_the_handlers_are_known`
    /// runs the same shape with the handlers supplied and gets the opposite
    /// answer; the pair is what shows the two gates were independent.
    ///
    /// The bytecode below is the real javac shape for
    /// `void m() { Foo f = new Foo(); synchronized (f) { f.x = 1; } }` — the
    /// one the sibling test above omits. javac must protect the body with a
    /// catch-all handler that re-exits the monitor and rethrows, and it must
    /// jump the normal path over that handler. That `goto` is the point of this
    /// test.
    ///
    /// # Why substituting `pop` is the right experiment
    ///
    /// `analyze_escapes` has no `0xC2`/`0xC3` arm, so "what would the ideal arm
    /// do?" cannot be asked of the function directly. But the ideal arm is
    /// fully specified: `monitorenter` and `monitorexit` each pop exactly one
    /// reference and push nothing, and the whole point of the proposed fix is
    /// that the popped provenance must NOT be escaped. That is byte for byte
    /// the contract of this function's `pop` (`0x57`) arm:
    /// `abs_stack.pop(); pc += 1;`. So the `ideal` copy below — the same
    /// bytecode with `0xC2`/`0xC3` rewritten to `0x57` — is an exact simulation
    /// of a fixed `analyze_escapes` running on the javac shape, and it needs no
    /// change to the function under test.
    ///
    /// # What each assertion follows from
    ///
    /// `analyze_escapes` returns the set of `new` PCs it CONFIRMED
    /// non-escaping, so "the receiver escapes" is `!contains(&0)`.
    ///
    /// 1. `plain` (the same allocation with the `synchronized` wrapper removed)
    ///    is non-escaping — `contains(&0)` — which is what stops the two
    ///    assertions below from being vacuously true about some unrelated
    ///    property of this `new; dup; <init>; astore; putfield` sequence.
    /// 2. `javac` escapes it today, at the `monitorenter` catch-all (pc 11).
    /// 3. `ideal` escapes it TOO, and this is the finding: with the monitor
    ///    opcodes modelled perfectly, the walk reaches pc 19 (`goto`) holding
    ///    `f` in locals 1 and 2, and the control-transfer barrier
    ///    (`0x99..=0xa9`) escapes every object held in a local. Fixing the
    ///    monitor arms alone therefore buys nothing for javac's output.
    ///
    /// Read, not executed — this lane could not build.
    #[test]
    fn an_exact_non_escaping_monitor_arm_would_not_rescue_the_javac_synchronized_shape() {
        // `Foo f = new Foo(); f.x = 1;` with no `synchronized` at all.
        ...
        assert!(
            analyze_escapes(&plain, plain.len(), &trivial_init_at(4)).contains(&0),
            "control: without the synchronized wrapper this allocation IS \
             confirmed non-escaping, so the two assertions below are about the \
             wrapper and nothing else"
        );

        // The full javac shape, including the monitor handler and the `goto`
        // that jumps the normal path over it.
        let javac: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V
            0x4c, // 7: astore_1            -> f in local 1
            0x2b, // 8: aload_1
            0x59, // 9: dup
            0x4d, // 10: astore_2           -> the synthetic monitor temp
            ...
            0x2c, // 17: aload_2
            0xc3, // 18: monitorexit
            0xa7, 0x00, 0x08, // 19: goto 27 -- jumps over the handler
            0x4e, // 22: astore_3           -- handler: catch any
            0x2c, // 23: aload_2
            0xc3, // 24: monitorexit
            0x2d, // 25: aload_3
            0xbf, // 26: athrow
        ...
        assert_eq!(javac.len(), 28, "sanity: the goto at 19 must target pc 27");
        assert_eq!(javac[27], 0xb1, "sanity: pc 27 is the return");
        assert!(
            !analyze_escapes(&javac, javac.len(), &trivial_init_at(4)).contains(&0),
            "the receiver is absent from the confirmed non-escaping set. Until 2026-09-22 it was the catch-all at the `monitorenter` (pc 11) that put it there; since the exact monitor arms landed it is the `goto` at pc 19, which THIS entry point still treats as a hard barrier because it was given no handler pcs. The `ideal` run below separates the two causes. Escaped the \
             receiver, so its `new` pc is absent from the confirmed \
             non-escaping set"
        );

        // The same bytecode with both monitor opcodes rewritten to `pop` — an
        // exact stand-in for the precise, non-escaping arms the known-issue
        // page proposes.
        let ideal: Vec<u8> = javac
            .iter()
            .map(|&b| if b == 0xc2 || b == 0xc3 { 0x57 } else { b })
            .collect();
        assert_eq!(
            ideal.len(),
            javac.len(),
            "sanity: monitorenter, monitorexit and pop are all one byte, so the \
             substitution cannot move any pc or retarget the goto"
        );
        assert!(
            !analyze_escapes(&ideal, ideal.len(), &trivial_init_at(4)).contains(&0),
            "THE FINDING, and why the monitor arms had to land WITH EA-2 rather than before it: even with the monitor opcodes modelled as an exact, \
             non-escaping one-slot pop, the receiver still escapes -- the \
             `goto` at pc 19 that javac must emit to skip the monitor handler \
             is a hard control-transfer barrier, and it escapes every object \
             held in a local (here `f`, in locals 1 and 2). Adding \
             monitorenter/monitorexit arms to `analyze_escapes` is therefore \
             necessary but NOT sufficient to reach Phase C. Supply the handler pcs and the answer flips; that is the sibling test."
        );
    }

    // -----------------------------------------------------------------------
    // EA-2 (2026-09-22) — the `goto`-over-handler see-through.
    //
    // `docs/feature-designs/jit-r10-earelock-proposals.md` §EA-2. The barrier
    // these exercise is the one named in this file's own comment history for
    // the bc `org.bouncycastle.math.ec` SEGV, so the loosening is pinned by
    // FOUR refusals below, one per condition, and the two tests that do admit
    // an object carry a control that fails if the admission came from anywhere
    // else.
    // -----------------------------------------------------------------------

    /// The javac `synchronized` shape with the monitor opcodes rewritten to
    /// `pop` — the same byte-exact simulation of the ideal monitor arm that
    /// `an_exact_non_escaping_monitor_arm_would_not_rescue_the_javac_synchronized_shape`
    /// uses. That test's third assertion is the finding EA-2 exists to remove:
    /// with perfect monitor arms the receiver STILL escaped, at the `goto` that
    /// jumps over the monitor handler.
    ///
    /// With the handler pcs supplied, the `goto` is proven to be a straight-line
    /// continuation and the receiver survives.
    #[test]
    fn the_goto_over_javacs_monitor_handler_is_walked_through_when_the_handlers_are_known() {
        // `void m() { Foo f = new Foo(); synchronized (f) { f.x = 1; } }`, with
        // monitorenter/monitorexit -> pop. Byte-for-byte the `ideal` array of
        // the sibling test.
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            ...
            0x59, // 9: dup
            0x4d, // 10: astore_2           -> the synthetic monitor temp
            0x57, // 11: pop                (monitorenter, modelled exactly)
            0x2b, // 12: aload_1
            0x04, // 13: iconst_1
            0xb5, 0x00, 0x03, // 14: putfield #3
            0x2c, // 17: aload_2
            0x57, // 18: pop                (monitorexit, modelled exactly)
            0xa7, 0x00, 0x08, // 19: goto 27 -- jumps over the handler
            0x4e, // 22: astore_3           -- handler: catch any
            0x2c, // 23: aload_2
            0x57, // 24: pop
        ...
        let len = code.len();
        // javac's own handler for this shape: one catch-all at pc 22, named by
        // both the body range and the handler's own range.
        let handlers = [22usize];

        // The control, and the reason the assertion below is about EA-2 rather
        // than about some other property of this byte sequence: told nothing
        // about handlers, the walk keeps the `goto` as a hard barrier and the
        // receiver still escapes — exactly as the sibling test pins.
        assert!(
            !analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), None).contains(&0),
            "control: with no handler pcs the see-through is disabled, which is \
             the pre-EA-2 behaviour -- so the assertion below measures the \
             see-through and not the bytecode"
        );

        assert!(
            analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), Some(&handlers))
                .contains(&0),
            "EA-2: the `goto` at 19 targets 27, whose only ordinary predecessor \
             it is; it skips only handler-only code (22..27); and 27 is not \
             itself a handler entry -- so the walk steps through it and `f` \
             survives in its locals"
        );
    }

    /// The barrier EA-2 must NOT have removed: a CONDITIONAL branch.
    ///
    /// This is the shape behind the miscompile this file's comments name. A
    /// two-armed `if` is a real merge and the linear state at the join is not
    /// the state on every incoming edge, so the receiver must still escape even
    /// with the handler pcs supplied.
    #[test]
    fn a_conditional_branch_is_still_a_hard_barrier_under_ea2() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V
        ...
        let len = code.len();
        assert!(
            !analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), Some(&[])).contains(&0),
            "a conditional branch is a merge; EA-2 admits only an unconditional \
             forward `goto` whose target has exactly one ordinary predecessor"
        );
    }

    /// Condition 1: a BACK edge is never a straight-line continuation.
    #[test]
    fn a_backward_goto_is_still_a_hard_barrier_under_ea2() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V
            ...
            0x04, // 9: iconst_1
            0xb5, 0x00, 0x03, // 10: putfield #3
            0xa7, 0xff, 0xfb, // 13: goto -5 -> 8   (a loop over the body)
        ];
        let len = code.len();
        assert!(
            !analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), Some(&[])).contains(&0),
            "a back edge makes its target a loop header, i.e. a real merge"
        );
    }

    /// Condition 2: the target must have exactly ONE ordinary predecessor. Here
    /// a conditional branch lands on the same instruction, so the target is a
    /// join and the linear state is not the only state that reaches it.
    #[test]
    fn a_goto_whose_target_has_a_second_predecessor_is_still_a_barrier() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V
            0x4c, // 7: astore_1
            0x1a, // 8: iload_0
            0x99, 0x00, 0x06, // 9: ifeq 15        -> second edge into 15
            0xa7, 0x00, 0x03, // 12: goto 15       -> the candidate
            0xb1, // 15: return
        ];
        let len = code.len();
        assert!(
            !analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), Some(&[])).contains(&0),
            "pc 15 has two ordinary predecessors (the `ifeq` at 9 and the `goto` \
             at 12), so it is a join and the barrier must stand"
        );
    }

    /// Condition 3: the region the `goto` skips must be reachable ONLY through
    /// the exception table. Here the skipped instructions are on an ordinary
    /// path, so stepping through the `goto` would jump over code that really
    /// runs — including its `astore`, which kills the receiver's local.
    #[test]
    fn a_goto_over_ordinarily_reachable_code_is_still_a_barrier() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V
            0x4c, // 7: astore_1
            0x1a, // 8: iload_0
            0x99, 0x00, 0x05, // 9: ifeq 14        -> into the "skipped" region
            0xa7, 0x00, 0x05, // 12: goto 17
            0x01, // 14: aconst_null   <- reachable from the ifeq
            0x4c, // 15: astore_1      <- and it clobbers the receiver's local
            0x00, // 16: nop
            0xb1, // 17: return
        ];
        let len = code.len();
        assert!(
            !analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), Some(&[])).contains(&0),
            "pcs 14..17 are ordinarily reachable, so this `goto` skips code that \
             runs -- which is exactly how a stale provenance would survive a \
             store that killed it"
        );
    }

    /// Condition 4: the target must not itself be an exception-handler entry,
    /// which is reached on an edge carrying a different state.
    ///
    /// The control is the SAME bytecode with an empty handler set, which does
    /// qualify — without it this test would pass for any reason at all.
    #[test]
    fn a_goto_into_a_handler_entry_is_still_a_barrier() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V
            ...
            0x04, // 9: iconst_1
            0xb5, 0x00, 0x03, // 10: putfield #3
            0xa7, 0x00, 0x03, // 13: goto 16
            0xb1, // 16: return
        ];
        let len = code.len();
        assert!(
            analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), Some(&[])).contains(&0),
            "control: with pc 16 an ordinary instruction the `goto` qualifies, \
             so the refusal below is about the handler set and nothing else"
        );
        assert!(
            !analyze_escapes_with_handlers(&code, len, &trivial_init_at(4), Some(&[16]))
                .contains(&0),
            "pc 16 is a handler entry, so control can arrive there on an \
             exception edge carrying a state this walk never modelled"
        );
    }

    /// Round 10 (`earelock`), Phase C hardening: a scalar object whose monitor
    /// is held at a PC where NO local holds it must be demoted, not planned.
    ///
    /// `x64/deopt_stubs.rs` turns each `monitor_at[bci]` entry into
    /// `MonitorInfo { object: FrameValue::VirtualObjectRef(new_pc), .. }`, and a
    /// `VirtualObjectRef` resolves only against the `VirtualObject(new_pc)` that
    /// the frame state's LOCALS lane defines — and that lane is built from
    /// `local_prov_at[bci]` alone. `new; dup; <init>()V; dup; monitorenter`
    /// locks the receiver straight off the operand stack and never stores it, so
    /// before this fix the plan published monitor entries at pcs 9..14 while
    /// `local_prov_at` stayed empty: a monitor naming a shell nothing defines.
    ///
    /// # What this asserts and why it follows
    ///
    /// `plan_scalar_replacement` returns a plan whose `objects`, `field_ops`,
    /// `init_skips`, `monitor_at` and `monitor_scalar_ops` have ALL been
    /// filtered through `final_objects`, which excludes every `poisoned` object.
    /// The fix poisons this receiver at the first monitor snapshot that finds no
    /// local home (pc 9), so `final_objects` is empty and every one of those
    /// five maps is empty with it. That is the demotion: the `new` is allocated
    /// for real, `<init>` is really called, and the lock is really taken — which
    /// is always correct, and is the same all-or-nothing coupling
    /// `final_monitor_scalar_ops` already enforces from the other side.
    ///
    /// Read, not executed.
    #[test]
    fn a_monitored_scalar_with_no_local_home_is_demoted_instead_of_dangling() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1          -> [f]
            0x59, // 3: dup                         -> [f, f]
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V -> [f]
            0x59, // 7: dup                         -> [f, f]
            0xc2, // 8: monitorenter                -> locks f, [f]
            0x59, // 9: dup                         -> [f, f]
            0x04, // 10: iconst_1                   -> [f, f, 1]
            0xb5, 0x00, 0x03, // 11: putfield #3    -> [f]
            0xc3, // 14: monitorexit                -> unlocks f, []
            0xb1, // 15: return
        ];
        let len = code.len();
        let new_info = vec![(0usize, 1u32, 1usize, true, true)];
        let invoke_info = vec![(4usize, init_void())];
        // Handed the receiver as non-escaping on purpose: this test is about
        // what the PLAN does with such a candidate, not about whether
        // `analyze_escapes` can currently produce one (it cannot — see the two
        // sibling tests above).
        let plan =
            plan_scalar_replacement(&code, len, &non_escaping(0), &new_info, &invoke_info, 0);
        assert!(
            plan.objects.is_empty(),
            "the receiver is locked off the operand stack and never stored, so \
             no deopt snapshot could define its VirtualObject shell -- it must \
             be demoted to a real allocation rather than scalar-replaced"
        );
        assert!(
            plan.monitor_at.is_empty() && plan.monitor_scalar_ops.is_empty(),
            "both Phase C maps are filtered through `final_objects`, which the \
             demotion just emptied, so neither may name a lock to elide"
        );
        assert!(
            plan.field_ops.is_empty() && plan.init_skips.is_empty(),
            "the same filter governs the field ops and the init skip: a \
             demoted object keeps its real putfield and its real <init> call"
        );
        assert_eq!(plan.total_slots, 0, "and reserves no frame slots");
    }

    /// The control for the test above: the SAME Phase C machinery, on the shape
    /// where the receiver does have a local home for the whole locked region.
    /// Without this, the demotion fix could be "poison every monitored object"
    /// and still pass.
    ///
    /// This is the bytecode `x64/tests.rs::s32_phase_c_plan_tracks_scalar_monitors`
    /// uses, fed the same hand-built non-escaping set for the same reason (see
    /// `monitorenter_on_an_otherwise_local_receiver_still_escapes_it_today` for
    /// why the real `analyze_escapes` cannot supply it).
    ///
    /// The last assertion is the invariant the demotion fix establishes, stated
    /// directly: every `(new_pc, depth)` a plan publishes in `monitor_at[bci]`
    /// has a `(local, new_pc)` pair in `local_prov_at[bci]` to define its
    /// `VirtualObject` shell. Read, not executed.
    #[test]
    fn a_monitored_scalar_kept_in_a_local_still_plans_its_relock() {
        let code: Vec<u8> = vec![
            0xbb, 0x00, 0x01, // 0: new #1
            0x59, // 3: dup
            0xb7, 0x00, 0x02, // 4: invokespecial #2 <init>()V
            ...
            0x2b, // 8: aload_1
            0x59, // 9: dup
            0x4d, // 10: astore_2           -> local 2 = f (the monitor temp)
            0xc2, // 11: monitorenter
            0x2b, // 12: aload_1
            0x04, // 13: iconst_1
            0xb5, 0x00, 0x03, // 14: putfield #3
        ...
        assert!(
            ...
            "locals 1 and 2 hold the receiver across the whole locked region, \
             so the demotion must NOT fire here"
        );
        assert!(
            plan.monitor_scalar_ops.contains(&11) && plan.monitor_scalar_ops.contains(&18),
            "both monitor ops name a surviving scalar object, so both are \
             elidable at `op_object.rs`'s 0xC2/0xC3 arm"
        );
        assert!(
            !plan.monitor_at.is_empty(),
            "the lock is held across pcs 12..18, so a deopt there has to record it"
        );
        for (bci, held) in &plan.monitor_at {
            for &(new_pc, depth) in held {
                assert!(
                    depth >= 1,
                    "a depth-0 entry is a lock nobody holds \
                     (deopt::DeoptMetadataError::UnbalancedMonitor)"
                );
                let defines_shell = plan
                    .local_prov_at
                    .get(bci)
                    .is_some_and(|live| live.iter().any(|&(_, np)| np == new_pc));
                assert!(
                    defines_shell,
                    "monitor_at[{bci}] names VirtualObjectRef({new_pc}) but no \
                     local at that bci defines its VirtualObject shell"
                );
            }
        }
    }
