...
impl Compiler {
    ...
    /// aligned `region_size` block of the address space. G1's arena comes from
    /// a `Vec<u8>` and is only malloc-aligned, so an aligned block is NOT a
    /// region: exactly one region boundary falls inside each block, and two
    /// addresses straddling it would be called "same region", the barrier
    /// skipped, and a live cross-region edge lost. That is a use-after-free, so
    /// the two subtractions are load-bearing rather than tidy.
    ///
