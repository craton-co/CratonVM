...
impl Compiler {
    ...
    pub(super) fn frame_layout(&self) -> crate::FrameLayout {
        ...
        crate::FrameLayout {
            ...
            // block. `deopt_regs_base` is the block's DEEPEST slot -- `gpr[0]`,
            // the RAX image, at `[rbp - deopt_regs_base]` (see `deopt_gpr_hi`
            // below, which is `base + 8` for exactly that reason) -- so the
            // first slot past the block is `base + 8`, not `base`.
            //
            // Round 9 wave 7 (review7b): this used to publish `base` itself,
            // which put `gpr[0]` inside the outgoing reserve, and
            // `conservative_roots::is_dead_outgoing_reserve` then skipped the
            // saved RAX image as dead whenever the safepoint carried a
            // register mask. `0` when no block was reserved, so with deopt off
            // this is exactly the end of the blind spill.
    ...
    pub(super) fn emit_post_call_rbp_republish(&mut self) {
        ...
        // ABI-3 — (rbp) -> (). `emit_call_absolute` takes a bare address, so
        // this assertion is the only thing tying the single register written
        // above to the helper's declared arity. `returns_value = false`
        // matters here: the sequence brackets the call with PUSH/POP RAX
        // precisely because the callee's RAX is meaningless.
    ...
    pub(super) fn emit_prologue(&mut self) {
        ...
        // Indexed rather than iterating a clone: the loop body needs `&mut self`,
        // and cloning the Vec for that cost an allocation per prologue/epilogue
        // (every deopt, exception and bounds stub emits an epilogue).
        ...
        for i in 0..self.alloc_used_xmms.len() {
            let xmm = self.alloc_used_xmms[i];
        ...
        // (neither GPR nor XMM assigned). ONE `XOR EAX, EAX` for the whole run:
        // `emit_store_local` only reads RAX, so re-zeroing it before every
        // store (the pre-r9 shape) cost two bytes and an instruction per
        // frame-homed local on every invocation of every method.
        ...
        if self.precise_maps && self.helpers.frame_record != 0 {
            if self.inline_rbp_tls_disp != 0 {
                ...
                if self.verify_inline_frame_record {
                    self.emit_mov_reg_reg(ARG_REGS[0], RBP);
                    // (rbp) -> (). `build_helpers` wires EITHER `jit_frame_record` or
                    // `jit_verify_inline_frame_record` into this one slot, which is why
                    // `vm/src/jit/helpers.rs` coerces both to `HelperFnFrameRecord`.
                    cratonvm_jit_api::assert_helper_call_shape!(
                        "frame_record",
                        int_args = 1,
                        returns_value = false
                ...
                self.emit_mov_reg_reg(ARG_REGS[0], RBP);
                // (rbp) -> (), the non-verifying arm of the same slot. Asserted
                // separately so "the same shape as the one above" stops being a
                // comment.
                cratonvm_jit_api::assert_helper_call_shape!(
                    "frame_record",
                    int_args = 1,
            ...
            && self.shadow_thread_slot_off != 0
        {
            ...
            if tls_disp != 0 {
                ...
                // () -> *mut JvmThread, in RAX. Zero argument registers: a site
                // that staged one would be writing into a register the callee
                // never reads. See the ABI-4 pair in `vm/src/jit/helpers.rs` for
                // why this slot's producer coercion is spelled differently from
                // every other one.
                cratonvm_jit_api::assert_helper_call_shape!(
                    "get_current_thread",
                    int_args = 0,
        ...
        if shadow_watch() && self.shadow_savebase_slot_off != 0 {
            ...
            if h != 0 {
                ...
                // A DIRECT-helper slot, so the direct table's census is what is
                // asserted: `arm_savebase_watch` is `HandWritten`, which is the
                // route with no stack-argument marshalling at all, and the macro
                // pins that as well as the arity.
                crate::assert_direct_helper_call_shape!(
                    "arm_savebase_watch",
                    int_args = 1,
                    returns_value = false
                );
        ...
        // Per-method invocation counter (CRATONVM_JIT_ENTRY_COUNTER, default
        // ON since round 9 wave 3; `=0` is the kill switch, and a thread
        // override can disarm one compile). See `crate::entry_counter` for the whole argument; the short
        // version is that the tiered manager's `invocation_count` has no writer
        // on the compiled path, so it freezes the moment a C1 body is published
        // and `should_compile`'s C1 -> C2 arm can never be satisfied again. This
        // is the writer.
        //
        // Emitted HERE, second-to-last, for the same reason the safepoint poll
        // below is last and the shadow-stack fetch above is late: by this point
        // every parameter is homed, no ABI argument is staged, and RAX/R10/R11
        // are all dead — the two blocks immediately above already clobber R10
        // and R11 freely. Emitting at the TOP of the prologue would also be
        // register-safe (R10 is an argument register under neither the SysV nor
        // the Win64 ABI) but would count invocations that then die in the stack
        // bang, and would sit ahead of the `self.failed` early return, which is
        // the one place in this function where "we emitted something" and "this
        // compile happened" stop agreeing.
        //
        // Deliberately BEFORE the poll: a thread that parks at the entry
        // barrier has entered this method, and a counter that skipped every
        // invocation that happened to arrive during a GC would undercount
        // exactly the allocation-heavy methods the C2 tier exists for.
        self.emit_entry_invocation_counter();
    ...
    /// Bump this method's entry counter — two instructions, no call, no branch,
    /// no lock.
    ///
    /// ```text
    ///   mov  r10, imm64(&counter.count)
    ///   add  qword ptr [r10 + 0], 1
    /// ```
    ///
    /// ## Why R10
    ///
    /// R10 is caller-saved and is an argument register under NEITHER x86-64 ABI
    /// this backend targets (SysV passes in RDI/RSI/RDX/RCX/R8/R9; Win64 in
    /// RCX/RDX/R8/R9), so it can never hold an incoming Java parameter. It is
    /// also already used as scratch twice in this same prologue — the
    /// self-cache-inherit proof loads two `imm64` code-range bounds into it, and
    /// the epilogue reloads the shadow thread pointer into it — so this
    /// introduces no new assumption about the register file, it reuses an
    /// existing one.
    ///
    /// ## Why `emit_mov_imm64_full` and not `emit_mov_imm64`
    ///
    /// The shrinking form collapses to `XOR r,r` at zero and to a 7-byte
    /// `MOV r/m64, imm32` for sign-extendable immediates. A heap address is
    /// neither, so the two forms agree in practice — but "in practice" is not
    /// something a byte-exact test can assert, and the fixed 10-byte form makes
    /// the emitted sequence a constant 15 bytes regardless of where the
    /// allocator happened to put the box. `prologue_entry_counter_emits_the_
    /// documented_two_instructions` asserts exactly that.
    ///
    /// ## Why nothing is emitted for an unarmed compile
    ///
    /// The counter is default-ON since round 9 wave 3 (priced in
    /// `docs/internal/jit-review-r9/NOTES-w3-tiering3.md`). Whether THIS compile
    /// emits it is `entry_counter_enabled_for_this_compile`: the process answer
    /// (`entry_counter_enabled`, memoised; `CRATONVM_JIT_ENTRY_COUNTER=0` turns it
    /// off) narrowed by a thread override. When it answers false this function
    /// allocates nothing and emits nothing, so a disarmed prologue is
    /// byte-identical to the one that shipped before the counter existed. The
    /// byte-exact / byte-determinism JIT tests rely on that: an armed prologue
    /// bakes the address of its own freshly allocated box, so they disarm the
    /// counter through the thread override.
    pub(super) fn emit_entry_invocation_counter(&mut self) {
        if !crate::entry_counter::entry_counter_enabled_for_this_compile() {
            return;
        }
        // Allocated on first use rather than at construction: every `Compiler`
        // is built, including the ones for compiles that bail before reaching a
        // prologue, and there is no reason to charge those an allocation.
        let counter = self
            .entry_counter
            .get_or_insert_with(|| Box::new(crate::entry_counter::JitEntryCounter::new()));
        // Cast: a heap address always fits in the imm64 field; the `as i64`
        // reinterprets the bit pattern, which is what the encoder wants.
        let addr = counter.count_addr() as i64;
        self.emit_mov_imm64_full(R10, addr);
        self.emit_add_mem_disp8_imm8(R10, crate::entry_counter::JitEntryCounter::COUNT_OFFSET, 1);
    }
    ...
    pub(super) fn emit_epilogue(&mut self) {
        ...
        if shadow_watch() && self.shadow_savebase_slot_off != 0 {
            ...
            if h != 0 {
                ...
                // () -> (). RAX is live across this call, which is why the
                // sequence stashes it in the dead savebase slot; a helper that
                // started returning a value would not change that, but one that
                // started taking an argument would clobber ARG_REGS[0] here.
                crate::assert_direct_helper_call_shape!(
...
#[cfg(test)]
mod r9w7_frame_layout_tests {
    //! Round 9 wave 7 (review7b): the published `FrameLayout` must not put any
    //! slot of the frame-deopt `SavedRegisters` image inside the outgoing
    //! reserve, which the conservative scanner may skip as dead.

    /// `gpr[0]` (the RAX image) is the block's DEEPEST slot, at
    /// `[rbp - deopt_regs_base]`. The outgoing reserve therefore starts one slot
    /// past it; publishing `deopt_regs_base` itself classified the saved RAX as
    /// outgoing-argument storage.
    #[test]
    fn the_outgoing_reserve_starts_past_the_deepest_saved_register() {
        let c = super::super::emit::tests::test_compiler();
        let l = c.frame_layout();
        if l.deopt_gpr_hi == 0 {
            // No `SavedRegisters` block in this configuration
            // (`CRATONVM_DEOPT_REAL=0`): nothing to classify.
            return;
        }
        let gpr0 = l.deopt_gpr_hi - 8;
        assert!(
            l.is_deopt_saved_gpr_image(gpr0),
            "gpr[0] at {gpr0} must be classified as the saved GPR image"
        );
        assert!(
            l.outgoing_lo >= l.deopt_gpr_hi,
            "outgoing_lo {} overlaps the saved GPR image [{}, {})",
            l.outgoing_lo,
            l.deopt_gpr_lo,
            l.deopt_gpr_hi
        );
        assert!(
            gpr0 < l.outgoing_lo,
            "the saved RAX image at {gpr0} would read as dead outgoing reserve"
        );
    }

    /// Round 9 wave 7b (wire7b):
    /// `single-pass-deopt-register-image-sits-one-slot-too-deep-20260918.md`.
    /// `Compiler::new` placed `gpr[0]` one slot too deep, on the first
    /// outgoing-argument word (`[RSP + 40]`, the Win64 6th-argument slot, when
    /// the frame total is already 16-aligned), and left the block's shallowest
    /// slot unused. The block must sit wholly above the 48-byte outgoing area
    /// (32 bytes of shadow space + the 16-byte stack-argument reserve) and abut
    /// the blind spill with no gap. The second assertion fails on the old
    /// placement whatever the frame's alignment padding is.
    #[test]
    fn the_saved_register_image_stays_above_the_outgoing_area() {
        let c = super::super::emit::tests::test_compiler();
        let l = c.frame_layout();
        if l.deopt_gpr_hi == 0 {
            return;
        ...
        let gpr0 = l.deopt_gpr_hi - 8;
        assert!(
            gpr0 <= l.frame_size - 48,
            "gpr[0] at {gpr0} is inside the outgoing area (frame_size {})",
            l.frame_size
        );
        assert_eq!(
            l.deopt_xmm_lo, l.reg_spill_hi,
            "the SavedRegisters block must start at the first slot past the blind spill"
        );
    }
}
