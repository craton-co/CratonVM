...
non_escaping_new={nen:?} scalar_new={news:?} field_ops={fops:?} init_skips={skips:?}",
    ...
    if !implicit_null_sites.is_empty() {
        ...
        // guarantee. Refuse the artifact: dropping `cm` runs
        // `CompiledMethod::drop`, which retires `[entry, entry + pos)` -- every
        // site this loop DID register -- before the buffer is unmapped.
