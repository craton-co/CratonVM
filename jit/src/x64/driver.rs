...
pub fn compile_with_request(
    ...
    // this crate are two test-only fixtures in `vm/src/vm.rs`). Threading
...
pub fn compile_with_param_slots(
    ...
    let BackendRequest {
        ...
        allow_scalar_under_precise_frames,
        protected_ranges,
        baseline_mode,
        volatile_field_pcs,
        ldc_slots,
        self_call_invoke_info: staged_self_call_invoke_info,
    } = backend;
    ...
    // The volatile `getfield`/`putfield` sites travel on the request rather
    // than as a 23rd positional table, so they are lifted here, by the same
    // primitive: a copy of a volatile `putfield` that lost its entry would
    // compile as a plain store with no StoreLoad fence, silently
    // (`volatile-instance-fields-are-plain-accesses-in-compiled-code`).
    let volatile_field_pcs: FxHashSet<usize> = match &loop_xform {
        None => volatile_field_pcs.into_iter().collect(),
        Some(x) => x
            .replicate_pc_keyed(
                &volatile_field_pcs
                    .into_iter()
    ...
    // Rotated (`goto cond`) loops whose ONLY entry is the `goto` right before
    // the header: the walk emits the header's relocatable pre-header
    // transforms (speculative-BCE entry guards, `aaload` and integer-arithmetic
    // hoists) immediately before that `goto`, so every entry runs them, and
    // those three transforms may keep such a header. So may the `arraylength`
    // hoist when it was proved against COND's prefix (wave 3,
    // `find_array_len_hoists_with_rotation`), and the batch pre-headers (SIMD
    // sum / element-wise, bulk byte, sieve, matrix dot), which are detected on
    // the loop's unrotated copy and emitted at the `goto` too
    // (`detect_batch_loop`, wave 4). FP hoists and unrolling keep the full
    // veto. Empty (and every filter below byte-identical) unless
    // `CRATONVM_JIT_ROTATED_PREHEADER` is on.
    // (`rotated-loops-lose-every-preheader-transform-20260918.md`.)
    let rotation_entries: FxHashMap<usize, usize> = if rotated_preheader_enabled() {
    ...
    if !rotation_entries.is_empty()
        && cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JIT_GEN")
    {
        let mut relocated: Vec<(usize, usize)> = rotation_entries
            .iter()
            .map(|(&header, &goto_pc)| (header, goto_pc))
            .collect();
        relocated.sort_unstable();
        eprintln!(
            "[JIT_GEN] rotated-preheader relocations (header, entry goto)={relocated:?} \
             (aaload / arith / arraylength hoists, BCE guards, batch pre-headers)"
        );
    }
    let relocatable_preheader_skipped = |header: &usize| {
        bypassable_headers.contains(header) && !rotation_entries.contains_key(header)
    };
    let hoist_info =
    ...
    let hoist_info: Vec<LoopHoist> = hoist_info
        .into_iter()
        .filter(|h| !relocatable_preheader_skipped(&h.loop_header))
        .collect();
    // A volatile access inside the loop is a JMM acquire (a read) or a full
    // fence (a write): no plain read may move above it, and the pre-header
    // runs before every in-loop one. `find_loop_hoists` already refuses call-
    // and monitor-bearing loops for this reason, but cannot see volatility,
    // so the volatile sites are checked here. Without it `while (!ready) {}`
    // followed by a hoistable `rows[i]` read the row published before `ready`
    // was set too early (`aaload-loop-hoist-crosses-volatile-reads`).
    let hoist_info: Vec<LoopHoist> = hoist_info
        .into_iter()
        .filter(|h| {
            let in_loop = |pc: usize| h.loop_header <= pc && pc < h.loop_end;
            !volatile_field_pcs.iter().any(|&pc| in_loop(pc))
                && !static_field_info
                    .iter()
                    .any(|&(pc, _, _, _, is_volatile)| is_volatile && in_loop(pc))
        })
        .collect();
        ...
        || cratonvm_types::flags::runtime_flag_on("CRATONVM_DISABLE_ARRAYLEN_LICM")
    {
        ...
        // Rotation-aware: for a relocated rotated header the eager-evaluation
        // proof is taken against COND's prefix (`find_array_len_hoists_with_rotation`).
        // `rotation_entries` is empty unless `CRATONVM_JIT_ROTATED_PREHEADER`
        // is armed, and then this is `find_array_len_hoists` exactly.
        find_array_len_hoists_with_rotation(code, code_len, &loops, &rotation_entries)
    };
    ...
    //
    // A ROTATED header (`rotation_entries`, armed only) is exempt: its hoists
    // were proved against COND's prefix above and are emitted at the entry
    // `goto` (`emit_array_len_hoists(header, goto_pc)` in the walk), which
    // every entry into the loop passes.
    let array_len_hoist_info: Vec<ArrayLenHoist> = array_len_hoist_info
        .into_iter()
        .filter(|h| !relocatable_preheader_skipped(&h.loop_header))
        .collect();
    // Precise exception frames: the pre-header's null check goes through the
    // SHARED, frame-less NPE stub (`emit_null_check_array_load`), while the
    // in-loop `arraylength` it replaces publishes a precise frame when its bci
    // is inside a `try` (`emit_precise_array_npe_check`, reason-10 exit). So
    // a hoist any of whose sites is protected would hand the handler an
    // unpublished frame. Keep those `arraylength`s in the loop instead; the
    // body then pays one length load per iteration but the method compiles.
    // Tested per SITE, not per trap bci: every site's own lowering is what
    // the hoist would replace. Ranges are in interpreter bcis, so an output
    // pc under the loop rewriter is mapped back first (`despec_bci`, identity
    // when unarmed). This is what lets `first_unsupported_precise_frame_site`
    // (`jit/src/lib.rs`) drop its "`aload; arraylength` inside a back-edge
    // span" refusal (r9 wave 2 runtime2 request 3).
    let array_len_hoist_info: Vec<ArrayLenHoist> = if precise_exception_frames {
        array_len_hoist_info
            .into_iter()
            .filter(|h| !array_len_hoist_site_is_protected(h, &protected_ranges, &despec_bci))
            .collect()
    } else {
        array_len_hoist_info
    };
    ...
    // T5.2.1 — SCEV induction variable analysis: REMOVED from this path on
    // 2026-09-17 (`NOTES-opts.md` §4 A3).
    //
    // It read:
    //
    // ```ignore
    ...
    // ```
    //
    // and its stated purpose was "stored on the Compiler so downstream passes
    // (unrolling, vectorization, range-check elimination) can query stride,
    // bound and trip count without re-walking the bytecode". No downstream
    // pass ever did: the only reader was `Compiler::induction_var_for`, which
    // carried `#[allow(dead_code)]` and had no caller anywhere in the
    // workspace. So this re-walked every counted loop of every method this
    // tier compiles, to fill a `Vec` nothing read.
    //
    // `scev::analyze_induction_variables` itself is NOT dead and is not
    // touched: `ir_optimize`'s unroller calls it, and that caller reads the
    // result. What is deleted is this tier's unused second call, plus the
    // field and accessor it fed.
    //
    // Restoring it is this one line plus the field, and the moment to do so is
    // when a pass on this tier actually wants an `InductionVar` — at which
    // point the caller and the computation land together and the compile time
    // buys something.
    ...
    // Every batch pre-header below (matrix dot, SIMD sum / element-wise, the
    // bulk byte loops) goes through `detect_batch_loop`: a header an outside
    // edge can bypass is refused, except a rotated header in
    // `rotation_entries` (armed only), whose loop is detected on its unrotated
    // copy and whose pre-header the walk emits at the entry `goto`
    // (`rotated-loops-lose-every-preheader-transform-20260918.md`, wave 4).
        ...
        loops
            ...
            .filter_map(|&(header, back_edge)| {
                detect_batch_loop(
                    ...
                    &rotation_entries,
                    |c, _, h, b| {
                        let loop_end = b + bytecode_analysis::step(c, b);
                        let iv = find_induction_variable(c, h, loop_end)?;
    ...
    let simd_loops = if has_avx2() && !no_bce {
        ...
        for &(header, back_edge) in &loops {
            let found = detect_batch_loop(
                ...
                &rotation_entries,
                |c, _, h, b| {
                    let back_edge_end = b + bytecode_analysis::step(c, b);
                    let iv = find_induction_variable(c, h, back_edge_end)?;
                    detect_int_array_sum(c, h, b, iv)
                },
            );
        ...
        for &(header, back_edge) in &loops {
            let found = detect_batch_loop(
                ...
                |c, _, h, b| {
                    let back_edge_end = b + bytecode_analysis::step(c, b);
                    let iv = find_induction_variable(c, h, back_edge_end)?;
                    detect_int_array_element_wise(c, h, b, iv)
                },
            );
            if let Some(info) = found {
                ewise.push(info);
    ...
    // EA-2 (2026-09-22) — this method's exception-handler entry pcs, shared by
    // BOTH escape-analysis walks below. They must be given the same answer:
    // `analyze_escapes` and `plan_scalar_replacement` are a matched pair over
    // the same bytecode, and a `goto` one steps through and the other does not
    // is how a field op past the jump binds to an object the other pass never
    // admitted. Computed once, here, for that reason and not for the
    // allocation.
    //
    // `exception_ranges` has already been lifted into the rewritten pc space by
    // the bytecode loop rewriter above, so these are in the same coordinates as
    // `code`.
    let handler_pcs: Vec<usize> = exception_ranges.iter().map(|&(_, _, h)| h).collect();
    let non_escaping_new: std::collections::HashSet<usize> = if is_baseline || new_info.is_empty() {
        ...
        // EA-2 (2026-09-22) — with the handler pcs supplied, this walk may step
        // THROUGH javac's `goto`-over-handler instead of treating it as a
        // merge.
        //
        // The parameter is an `Option` and an empty slice is NOT the same as
        // `None`: a method with no exception table genuinely has no handlers,
        // and its `goto`s are eligible on their own merits. `None` is for a
        // caller that does not HAVE the table — see
        // `x64::bytecode_compat`'s conservative pre-pass.
        analyze_escapes_with_handlers(code, code_len, &invokespecial_shapes, Some(&handler_pcs))
    };
    ...
    // ── SCALAR REPLACEMENT UNDER PRECISE EXCEPTION FRAMES ─────────────────
    //
    // `precise_exception_frames` used to be an unconditional third disjunct
    // here, and that one word is what
    // `docs/internal/fixed-bugs/r10-ea-single-pass-monitor-scalar-relock-is-unreachable-FIXED-20260922.md`
    // is about. `lib.rs::local_handler_reads_unsafe_local` is true for EVERY
    // javac `synchronized` block — the synthesised monitor temporary is
    // allocated above every parameter slot, so the mandatory
    // `astore <e>; aload <mon>; monitorexit; aload <e>; athrow` handler always
    // reads a non-parameter local — so the flag is set for every one of them,
    // and this line then switched scalar replacement off wholesale. The
    // backend's entire monitor-relock feature ("Phase C" in
    // `escape_analysis.rs`, its deopt half in `deopt_stubs.rs`, its elision arm
    // in `op_object.rs`) could never receive a candidate.
    //
    // It cost far more than the monitors it was filed about: the same flag is
    // set by any handler that reads a local first assigned inside the method,
    // i.e. most `try`/`catch`.
    //
    // Admitting it is sound only because three things now hold, and each is
    // load-bearing:
    //
    //  1. **The reason-9 sinks can resume on a frame that names a deleted
    //     object.** `exception_dispatch::materialize_and_relock_precise_frame`
    //     rebuilds the shells and re-acquires the elided locks immediately
    //     before the handler frame is built. Without it an exception inside the
    //     protected range PROPAGATED past a handler that should have caught it,
    //     because `ir_deopt_locals` answers `None` for a `VirtualObject` and
    //     both sinks read that as "cannot resume".
    //  2. **The method is not `ACC_SYNCHRONIZED`**, which is what
    //     `allow_scalar_under_precise_frames` carries — the backend cannot see
    //     the access flags, and the sink in (1) refuses such a method.
    //  3. **Compiled local handler blocks are disarmed** for a method that
    //     scalar-replaces anything (see `local_handlers_armed` below). Such a
    //     block runs the handler IN the compiled frame, where the elided `new`
    //     left a dummy zero in the local javac's handler reloads — a real
    //     `jit_monitor_exit(null)`. Disarming sends the exception through the
    //     reason-9 route instead, which is the one (1) taught to rebuild.
    //
    // `CRATONVM_JIT_SCALAR_UNDER_PRECISE_FRAMES=0` closes it again, restoring
    // this line's pre-2026-09-22 behaviour exactly.
    let sr_admitted_under_precise_frames =
        precise_exception_frames && allow_scalar_under_precise_frames;
    let non_escaping_for_sr = if is_baseline
        || (precise_exception_frames && !sr_admitted_under_precise_frames)
    ...
    let sr_plan = plan_scalar_replacement_with_handlers(
        code,
        code_len,
        non_escaping_for_sr,
        &new_info,
    ...
    //
    // A recursion-cycle closing edge (`CRATONVM_JIT_CYCLE_EDGE_CELL`, a direct
    // call carrying `JIT_CYCLE_EDGE_CELL_GUARD`) runs the same stack-depth
    // guard (`op_invoke.rs::emit_cycle_edge_cell_call`), so it reserves the
    // floor too and takes the inline `CMP RSP, [rbp - floor]; JA` fast path
    // instead of the helper plus a full spill per crossing (mutrec7 request 2,
    // wired round 9 wave 7b).
            ...
            while pc < code_len {
                if (code[pc] == 0xb8
                    && ((!invoke_info.iter().any(|(p, _)| *p == pc)
                        ...
                        || direct_calls.iter().any(|(p, dc)| {
                            *p == pc && dc.guard_class_id == crate::JIT_CYCLE_EDGE_CELL_GUARD
                        })))
                    // An INSTANCE self-call (`JIT_SELF_CALL_GUARD` marker row, at
                    // an `invokevirtual`/`invokespecial`; round 9 wave 10 call10,
                    // landed by wave 10b wire10b) is lowered by the same
                    // self-recursive arm, so it takes the same inline floor
                    // check instead of the guard helper plus a spill per level.
                    || direct_calls.iter().any(|(p, dc)| {
                        *p == pc && dc.guard_class_id == crate::JIT_SELF_CALL_GUARD
                    })
                {
                    found = true;
                    break;
    ...
    // Site-keyed (not pc-keyed), so the loop rewriter above needs no lifting.
    compiler.ldc_slots = ldc_slots;
    ...
    //  * this method scalar-replaced nothing. A compiled handler block runs the
    //    `catch` IN THIS FRAME, and a scalar-replaced `new` leaves a dummy zero
    //    in every local that held it — including the monitor temporary javac's
    //    handler reloads and exits. The plan's walk does not model the handler
    //    region (it is reachable only through the exception table, which is
    //    exactly why `analyze_escapes`' `goto` see-through may skip it), so no
    //    field op or monitor op in a handler is ever mapped to a frame slot and
    //    each would execute against that zero. Disarming sends the exception
    //    through the reason-9 route instead, which
    //    `exception_dispatch::materialize_and_relock_precise_frame` rebuilds
    //    the object and re-acquires the lock for. Before 2026-09-22 this
    //    conjunct was vacuous — `precise_exception_frames` above already
    //    guaranteed an empty plan — and it is written out rather than left
    //    implicit because that is no longer true;
    let local_handlers_armed = crate::local_handlers_enabled()
        && helpers.local_handler_lookup != 0
        && precise_exception_frames
        && needs_heap
        && loop_xform.is_none()
        && sr_plan.objects.is_empty()
    ...
    // Unconditional, unlike the handler table above: the callee-deopt service
    // at a raw self-recursive CALL is a correctness obligation, not a
    // throughput feature, and it is needed EXACTLY when the local-handler
    // route is unavailable — `local_handlers_armed` requires
    // `precise_exception_frames`, which RBC.6 only requests for a handler that
    // reads a local beyond the parameters. A handler that reads only its
    // parameters (the common shape) gets neither, and without the service its
    // frame's `catch` was skipped entirely. See `Compiler::self_call_invoke_info`.
    compiler.self_call_invoke_info = staged_self_call_invoke_info;
    ...
    let speculative_bce_guards: Vec<SpeculativeBCEGuard> = speculative_bce_guards
        .into_iter()
        .filter(|g| {
            let bypassable = relocatable_preheader_skipped(&g.loop_header);
            if bypassable {
                for covered_pc in &g.covered_pcs {
                    bounds_safe_pcs.remove(covered_pc);
                }
    ...
    // The rule itself lives in `simd_analysis.rs` (`simd_array_covered`), shared
    // with `single_pass_only.rs`'s IR-tier veto so the two cannot disagree. For
    // an `iload`-bounded loop it is the provenance-or-guard test this closure
    // used to spell inline; an `a.length`-bounded loop (`SimdLoopBound::ArrayLength`,
    // detected only under `CRATONVM_JIT=vectorize`) is covered by the
    // pre-header's own self-guards in `simd.rs` (every array non-null,
    // `iv >= 0`, `bound <= length` per array, else fall into the scalar loop).
    let simd_loops: Vec<SimdIntArraySum> = simd_loops
        .into_iter()
        .filter(|s| !no_bce && simd_sum_covered(s, code, code_len, &speculative_bce_guards))
        .collect();
    let simd_element_wise_loops: Vec<SimdArrayElementWise> = simd_element_wise_loops
        .into_iter()
        .filter(|e| {
            !no_bce && simd_element_wise_covered(e, code, code_len, &speculative_bce_guards)
        })
        .collect();
    ...
    // unless it is a rotated header, whose pre-header the walk emits at the
    // entry `goto` (`detect_batch_loop` already applied this rule; kept as the
    // emission-side statement of it).
    let simd_loops: Vec<SimdIntArraySum> = simd_loops
        .into_iter()
        .filter(|s| !relocatable_preheader_skipped(&s.header_pc))
        .collect();
    let simd_element_wise_loops: Vec<SimdArrayElementWise> = simd_element_wise_loops
        .into_iter()
        .filter(|e| !relocatable_preheader_skipped(&e.header_pc))
        .collect();
    ...
    // The walk emits a rotated header's relocatable pre-header at its entry
    // `goto`; it looks the header up by the `goto`'s pc.
    compiler.rotation_preheader_by_goto = rotation_entries
        .iter()
        .map(|(&header, &goto_pc)| (goto_pc, header))
        .collect();
    compiler.compact_field_off = compact_field_info
        .into_iter()
        .map(|(pc, off, is_ref)| (pc, (off, is_ref)))
        .collect();
    compiler.volatile_field_pcs = volatile_field_pcs;
    ...
    // The strip-mined SIMD pre-headers (int-array sum, element-wise) sit AT
    // `pc_to_native[header]` (`emit_strip_mined_preheaders`, round 9 wave 4),
    // and the native unroller duplicates the machine code from
    // `pc_to_native[header]` to the back edge, so it would copy the whole
    // pre-header into every unrolled iteration. The scalar loop behind such a
    // pre-header only runs after a clamped strip or a refused self-guard, so
    // unrolling it buys nothing: drop those plans. (Without AVX2 neither SIMD
    // pre-header is emitted and their plans stay.) Since round 9 wave 7
    // (bulkplace7) the bulk byte pre-headers (zero fill, stride store, byte
    // sieve) sit there too (`emit_batch_preheaders`, called from
    // `emit_strip_mined_preheaders`); they need no AVX2, so their plans are
    // dropped unconditionally. The matrix dot stays fall-through-only (before
    // `pc_to_native[header]`), so its plans are unaffected.
    let unroll_loops: Vec<(usize, usize, usize)> = unroll_loops
        .into_iter()
        .filter(|&(header, _, _)| {
            let simd = has_avx2()
                && (simd_loops.iter().any(|s| s.header_pc == header)
                    || simd_element_wise_loops
                        .iter()
                        .any(|e| e.header_pc == header));
            let bulk = bulk_zero_byte_fill_loops
                .iter()
                .any(|f| f.header_pc == header)
                || bulk_set_byte_stride_loops
                    .iter()
                    .any(|f| f.header_pc == header)
                || byte_sieve_loops.iter().any(|s| s.header_pc == header);
            !(simd || bulk)
        })
        .collect();
    ...
    // The two monitor columns of the `CRATONVM_DBG_SCALAR_DEOPT` trace below,
    // taken from the PLAN here rather than read back off the compiler there.
    //
    // Not a style choice. `jit/tests/r10_deoptverify_elided_monitor_contract.rs`
    // counts every non-binding read of `compiler.sr_monitor_scalar_ops` and
    // requires exactly ONE -- the membership test in `op_object.rs`, which is
    // the only code in this backend that removes a lock. That count is the
    // standing contract for the next elision ("record a `MonitorInfo` or set
    // `has_elided_monitor`"), and a diagnostic read would have spent it on a
    // line that cannot remove anything. Reading the plan instead leaves the
    // contract with all its teeth: a second `.contains()` on the proof set
    // still fails that test.
    //
    // Empty on every compile that scalar-replaced nothing, which is the
    // overwhelming majority, and two `Vec`s of a handful of `usize` on the
    // rest -- paid only under the flag would be better, but the values are
    // moved on the next two lines and this is the last point they exist.
    let (dbg_mon_elided, dbg_mon_at): (Vec<usize>, Vec<usize>) =
        if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_SCALAR_DEOPT") {
            let mut elided: Vec<usize> = sr_plan.monitor_scalar_ops.iter().copied().collect();
            elided.sort_unstable();
            let mut at: Vec<usize> = sr_plan.monitor_at.keys().copied().collect();
            at.sort_unstable();
            (elided, at)
        } else {
            (Vec::new(), Vec::new())
...
non_escaping_new={nen:?} scalar_new={news:?} field_ops={fops:?} init_skips={skips:?}",
    ...
    // ── THE PER-COMPILE SCALAR-REPLACEMENT TRACE ─────────────────────────
    //
    // MOVED HERE 2026-09-22, and the move is the fix. This block used to sit
    // ~140 lines above, gated on `!compiler.scalar_replaced.is_empty()` — and
    // `compiler.scalar_replaced` is assigned on the line above this comment,
    // i.e. AFTER it ran. The field is empty at that point in every compile that
    // has ever happened, so the gate was false every time and this line has
    // never been printed. It is the defect
    // `scripts/check-orphan-instruments.sh` exists for, in the one trace whose
    // job is to say what scalar replacement did: a diagnostic whose silence is
    // indistinguishable from "the thing it watches never happened".
    //
    // `monitor_elided_pcs` and `monitor_at_pcs` are new, and they are the
    // columns the round-10 monitor page could not read. Together they answer
    // the two halves that have to agree:
    //
    //   * `monitor_elided_pcs` — the `monitorenter`/`monitorexit` sites
    //     `op_object.rs` will emit NOTHING for, because their receiver is a
    //     proven scalar. Non-empty is what "Phase C fired" looks like, and it
    //     was empty on every run until the escape analysis learned to model
    //     `0xC2`/`0xC3` and to walk through javac's `goto`-over-handler;
    //   * `monitor_at_pcs` — the bcis whose published frame state carries a
    //     `MonitorInfo { relock: true }`, i.e. where a resume must re-acquire
    //     what was elided. An elision with no matching record here is the
    //     unsound half of the pair, which is why they are printed together
    //     rather than either alone.
    //
    // `elided_monitor=` is a CONSTANT `false` on every run that has ever
    // existed — nothing assigns the field `true` (see the note on
    // `can_deopt_resume` below). It is printed anyway, and named here, so a
    // reader does not mistake it for a measurement of Phase C: an elision that
    // RECORDS a `MonitorInfo` deliberately does not set it, and the two columns
    // above are the ones that say what happened.
    if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_SCALAR_DEOPT")
        && !compiler.scalar_replaced.is_empty()
    {
        let mut keys: Vec<usize> = compiler.sr_local_prov_at.keys().copied().collect();
        keys.sort();
        let (mon_elided, mon_at) = (&dbg_mon_elided, &dbg_mon_at);
        eprintln!(
            "[DBG_SCALAR_DEOPT] x64 single-pass compile: scalar_replaced={} \
             sr_local_prov_at_pcs={keys:?} monitor_elided_pcs={mon_elided:?} \
             monitor_at_pcs={mon_at:?} eager_bci={:?} elided_monitor={}",
    ...
    // T5.2.14 — transfer the pre-computed analysis. (T5.2.1's SCEV result used
    // to be transferred here too; see the removal note at its old site.)
    ...
    // And the SIMD sum (chunk counter in R8) and element-wise (R8 chunk
    // counter, R9 element pointer) pre-headers, which since round 9 wave 4
    // also run at the loop header's join point, on back edges.
    if !compiler.simd_loops.is_empty() || !compiler.simd_element_wise_loops.is_empty() {
        compiler.kernel_operand_cache = false;
    }
    ...
    // Compile bytecode. (The baked-point record lives on `compiler`, so the
    // finalize check below judges only this compile's stubs.)
    ...
    // The per-method entry counter, if this compile was armed for one. Taken
    // alongside `implicit_null_sites` and for the same reason: `compiler.buf`
    // is about to move into the artifact, and everything the artifact must own
    // has to be lifted out before that partial move.
    //
    // `Some` on a default-configured compile (the counter is default-on since
    // round 9 wave 3); `None` when `entry_counter_enabled_for_this_compile`
    // answered false (`CRATONVM_JIT_ENTRY_COUNTER=0`, or a thread override
    // disarmed this compile), in which case this is a single `Option` move
    // and the artifact's counter field stays empty — which is what makes
    // `CompiledMethod::entry_count` answer 0 for a body whose prologue does
    // not bump anything, rather than answering a stale value some other
    // artifact wrote.
    let entry_counter = compiler.entry_counter.take();
    // The fallible constructors: a code buffer whose W^X flip to executable
    // fails is refused with a named bail rather than panicking the compiler
    // thread (r9 wave 2 runtime2 request 2).
    let built = if needs_heap {
        CompiledMethod::try_new_with_context(compiler.buf)
    } else {
        CompiledMethod::try_new(compiler.buf)
    };
    let Some(mut cm) = built else {
        crate::note_jit_bail_site_at("code-buffer-protect-failed", 0, 0);
        return None;
    };
    // The prologue baked this box's address as an `imm64`. From here the box is
    // owned by the artifact, so the word compiled code increments lives exactly
    // as long as the machine code that increments it — the same contract
    // `_jit_pic_slots` and `_jit_local_handler_sites` carry, and the reason a
    // retired body's counter dies with the body instead of being inherited by
    // whatever replaces it.
    cm._jit_entry_counter = entry_counter;
    cm.has_dispatch = has_dispatch;
    if !implicit_null_sites.is_empty() {
        let base = cm.entry as usize;
        // The entry MUST be the buffer base. Retirement itself no longer needs
        // that: `ExecutableBuffer::drop` retires `[ptr, ptr + capacity)`, the
        // whole mapping, so a moved prologue could not strand a site below it.
        // What still needs it is this registration, which computes each site as
        // `entry + fault_off` where `fault_off` is an offset from the BUFFER
        // base -- and the OSR-trampoline purge in `CompiledMethod::drop`, which
        // scans `[entry, entry + pos())`. Today they agree (`entry_offset = 0`
        // above). If a future prologue moves the entry, every registered
        // address is wrong by that delta: the fault handler looks up a PC no
        // entry covers, and a null receiver at an elided site becomes a process
        // crash instead of a `NullPointerException`.
        //
        // So it is checked, and a mismatch installs NOTHING. The sites' checks
        // are already elided in the emitted code, so registering none of them
        // and installing the artifact anyway would turn every null receiver at
        // those sites into a process crash; the compile is refused instead.
        // `note_registration_base` counts the mismatch (and asserts in debug).
        if !crate::implicit_null::note_registration_base(base, cm.code_bytes().as_ptr() as usize) {
            crate::note_jit_bail_site_at("implicit-null-entry-not-base", 0, 0);
            return None;
        }
        // A DECLINED registration (table past its load cap, or a probe run
        // over `MAX_PROBE`) is not a graceful degradation: the load was
        // emitted without its `TEST`/`JZ`, so an unregistered site turns a
        // Java `NullPointerException` into an access violation. `active()`'s
        // headroom keeps this from happening in practice; this makes it a
        // guarantee. Refuse the artifact: dropping `cm` drops the buffer it
        // owns, and `ExecutableBuffer::drop` retires `[ptr, ptr + capacity)` --
        // a superset of every site this loop DID register -- immediately before
        // the mapping is unmapped and its address becomes reusable.
        // (`implicit-null-declined-registration-installs-unguarded-code`.)
        if let Err(fault_off) =
            register_implicit_null_sites(base, cm.code_bytes().len(), &implicit_null_sites)
        {
            crate::note_jit_bail_site_at("implicit-null-declined", fault_off, 0);
            if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JITC") {
                eprintln!(
                    "[cratonvm-jitc] implicit-null-declined fault_off={fault_off}: \
                     artifact refused"
                );
            }
            return None;
    ...
    // (nothing resolves a point by offset at all; the i64::MIN re-run is
    // unchanged).
    //
    // SINGLE STORE (r9 wave 6, `every-x64-deopt-point-is-stored-twice`): the
    // frame-deopt stubs baked the addresses of `compiler.deopt_points`'
    // ELEMENTS (`emit_deopt_stubs`), so this move — which keeps the heap
    // buffer — is what makes those imm64s point into the artifact. Refuse the
    // artifact if the list was reallocated or resized after the stubs were
    // emitted: every baked pointer would dangle. `compiler.deopt_boxes` are
    // identity keys only now and are dropped with `compiler`;
    // `cm._deopt_point_boxes` stays empty on a single-pass artifact.
    if !super::deopt_stubs::baked_point_table_is(compiler.baked_point_table, &compiler.deopt_points)
    {
        crate::note_jit_bail_site_at("deopt-point-table-moved-after-stubs", 0, 0);
        return None;
    }
    ...
    //
    // `DeoptVerifier` used to run from `ir_lower.rs` and nowhere else, so the
    // metadata THIS backend publishes was never checked at all
    // (`r10-earelock-singlepass-deopt-metadata-is-never-verified-FIXED-20260922.md`).
    // That was a blind spot with a demonstrated inhabitant: round 10 wave 5
    // found Phase C publishing a `MonitorInfo::object` of
    // `VirtualObjectRef(new_pc)` for a receiver the locals lane never defined
    // (`monitor_at` and `local_prov_at` are different maps), which is exactly
    // `DeoptMetadataError::UndefinedVirtualObjectRef`. That instance was fixed
    // at the producer; this closes the class.
    //
    // The two backends build the SAME types — `deopt_stubs::build_frame_state_at`
    // produces `FrameState`/`FrameValue` and `build_and_record_deopt_point`
    // produces `DeoptimizationPoint` — so the verifier needs no adapter. Which
    // lanes are active is decided, as in `ir_lower.rs`, by which facts THIS
    // backend actually has:
    //
    //   * **scope** — the root method's real limits, under the key the producer
    //     stamps. `build_frame_state_at` writes
    //     `method_key: self.current_bytecode_owner()`, which is
    //     `Compiler::method_key` outside a splice, so `compiler.method_key` is
    //     the key to register and not `method_key` re-derived here: an
    //     `UnknownMethod` for the root scope would mean the two had drifted.
    //     - `code_len` is `orig_code_len`, because `DeoptimizationPoint::bci` and
    //       `FrameState::bci` are published through `resume_bci_for`/`orig_bci`
    //       into INTERPRETER-bci space (the rewritten length would be the wrong
    //       bound and, under a rewrite, a larger one).
    //     - `max_locals` is this compile's own: `Compiler::num_locals` IS this
    //       parameter (`Compiler::new`), and `build_frame_state_at` fills
    //       exactly `num_locals` entries, so the root scope can only violate
    //       this if one of those two facts changes.
    //     - `max_stack` is the READER/verifier figure, not the local
    //       `max_stack` above — that one has already been inflated by
    //       `max_invoke_args` and the inline reserve for frame SIZING and is not
    //       a bytecode bound. The comparison is sound in the safe direction
    //       because this backend's abstract operand stack is COMPACT (a `long`
    //       occupies one entry, not two — see `index_invoke_arg_types`), so a
    //       published `stack.len()` is at most the JVM's slot-counted
    //       `max_stack`. `None` (hand-built test call sites) saturates, which
    //       disarms the two count checks rather than inventing a bound.
    //   * **inlined-callee scopes** — every spliced body's real
    //     `callee_code_len`/`callee_max_locals`, under the SAME key
    //     `inlining.rs` builds for `push_inline_scope`
    //     (`"{class}.{method}:{descriptor}"`). No `max_stack` reaches us per
    //     callee, so that one saturates.
    //
    //     Nothing publishes a point from inside a splice today — the splice
    //     postcondition (`splice_point_is_misidentified`) rolls back any splice
    //     that did, unless the point names the callee under a caller scope — so
    //     these registrations are inert now and are the reason the FIRST edit
    //     that publishes one does not immediately lose every artifact that
    //     inlines to `UnknownMethod`. They also arm the check that edit most
    //     needs — though not for the reason first written here. It used to say
    //     `build_frame_state_at` fills `self.num_locals`, the ROOT method's
    //     count, under whatever identity `current_bytecode_owner` returns, so a
    //     callee-identified point carries the CALLER's locals. That was true and
    //     is no longer: the producer now detects an open splice and builds to the
    //     spliced callee's own geometry. These registrations are still exactly
    //     right, and the reason is the other way round — a callee-identified
    //     point can be BOUNDED by the callee's own limits, which is what
    //     `LocalCountMismatch` then checks against something real.
    //   * **removed-node** — not registered. This backend has no IR node ids;
    //     a scalar-replaced object is keyed by its `new` pc, which is not the
    //     same namespace, and registering it would compare two unrelated
    //     numbering schemes.
    //   * **oop-map agreement** — not registered, deliberately. `cm.oop_maps`
    //     are keyed by SAFEPOINT native offset and a deopt point's
    //     `native_offset` is the buffer position of its GUARD; the lane matches
    //     on exact key equality, so registering them would either never match
    //     or match by layout accident. Arming it needs a proven correspondence
    //     first — see `docs/feature-designs/jit-r10-deoptverify-proposals.md`.
    //   * **structural** — always on, and the reason this is worth the call:
    //     virtual-object definition/reference integrity, monitor balance,
    //     frame-slot sign and one-word-one-type agreement, register-file
    //     bounds, and the sortedness the emitters' forward walk implies -- a
    //     strictly-decreasing pair means a producer recorded out of order.
    //     (This clause used to be justified by `find_deopt_point`'s binary
    //     search; that search had no production caller and was deleted in
    //     round 10 wave 9. The lane outlived it, because the invariant is the
    //     producer's, not a consumer's.)
    //
    // WHY A FAILURE REFUSES THE ARTIFACT INSTEAD OF DEMOTING IT.
    // The obvious cheaper answer — keep the code, drop the points, leave
    // `can_deopt_resume` false, as wave 5's producer-side fix demoted one
    // object — does not work HERE, for two independent reasons, both read out
    // of this file and `deopt_stubs.rs`:
    //
    //   1. The frame-deopt stubs have already baked the ADDRESSES of
    //      `deopt_points`' elements as imm64 operands (`resolve_baked_point`,
    //      and the `baked_point_table_is` check immediately above). Clearing or
    //      shrinking the list after that point is precisely what that check
    //      exists to forbid: every baked pointer would dangle.
    //   2. Clearing the flags would not cover the consumers anyway. Reasons
    //      8..=11 take the frame path in `emit_deopt_stubs` UNCONDITIONALLY —
    //      not under `deopt_real_enabled()` — and a `PendingException` frame is
    //      consumed through the interpreter's exception route, which
    //      `can_deopt_resume` does not gate. A malformed frame would still be
    //      reconstructed: a `StackSlotRef` at a positive offset reads the
    //      caller's frame and hands the result to the GC as an object address.
    //
    // So: refuse. `DeoptVerifier::verify`'s own doc says a failure means the
    // artifact must not be installed, and refusal is the established outcome at
    // exactly this point — the `baked_point_table_is` check three lines up does
    // the same thing, for the same list, with the same `cm` half-built. Dropping
    // `cm` drops the buffer it owns and `ExecutableBuffer::drop` retires the
    // mapping; the method stays interpreted. A refusal costs one interpreted
    // method. A resume from metadata that contradicts itself costs a wrong
    // answer or a crash, arbitrarily far from here.
    //
    // The refusal is NAMED (`note_jit_bail_site_at`) rather than silent,
    // because a compile that vanishes without a reason is the one failure mode
    // worse than the bug: `jit-method-stats` reports bail sites, so a producer
    // defect this lane has not thought of shows up as a census row instead of
    // an unexplained perf cliff.
    //
    // Cost: one pass over a list that is a handful of entries on a typical
    // method, on a path that has just walked the same list twice.
    //
    // Bound to a local first, so the shared borrow of `cm.deopt_points` in the
    // argument ends at this statement rather than spanning the refusal block.
    let deopt_metadata_verified = crate::deopt::verify_deopt_metadata(
        &cm.deopt_points,
        std::iter::once(crate::deopt::MethodFrameLimits::new(
            compiler.method_key.clone(),
            u32::try_from(orig_code_len).unwrap_or(u32::MAX),
            u16::try_from(max_locals).unwrap_or(u16::MAX),
            verified_max_stack
            ...
            compiler
                ...
                .chain(
                    compiler
                        .inline_guard_variants
                        .values()
                        .flat_map(|v| v.iter().map(|(_class_id, site)| site)),
                )
                .map(|site| {
                    crate::deopt::MethodFrameLimits::new(
                        format!(
                            "{}.{}:{}",
                            site.class_name, site.method_name, site.descriptor
                        ),
                        u32::try_from(site.callee_code_len).unwrap_or(u32::MAX),
                        u16::try_from(site.callee_max_locals).unwrap_or(u16::MAX),
                        u16::MAX,
                    )
                }),
        ),
        std::iter::empty::<(u32, crate::deopt::OopCoverage)>(),
    );
    if let Err(bailout) = deopt_metadata_verified {
        crate::note_jit_bail_site_at("deopt-metadata-violation", 0, 0);
        if cratonvm_types::flags::runtime_flag_on("CRATONVM_DBG_JITC") {
            eprintln!(
                "[cratonvm-jitc] {bailout} method={} points={}",
                compiler.method_key,
                cm.deopt_points.len(),
            );
        }
        tracing::warn!(
            method = method_key,
            deopt_points = cm.deopt_points.len(),
            bailout = %bailout,
            "JIT compile bailed: the single-pass backend's deopt metadata did not verify; \
             method stays interpreted"
        );
        return None;
    }
    ...
    //
    // `has_elided_monitor` IS A CONSTANT `false` TODAY, and the sentence above
    // reads as though it were not — which is the whole content of
    // `r10-earelock-has-elided-monitor-is-never-set-FIXED-20260922.md`. The field is
    // assigned exactly once, in `Compiler::new` (`x64.rs`), and nothing in the
    // workspace ever assigns `true`. So this conjunct, and its twin on
    // `can_osr_exit` below, currently reduce to their first term.
    //
    // That is CORRECT, not a missing write, and the distinction matters to
    // anyone about to "fix" it. Phase B's blanket rule — "this method scalar-
    // replaced something, so drop its monitor ops" — could elide a lock on an
    // unrelated, genuinely escaping receiver, and left no trace in the frame, so
    // a method that did it had to be kept off the resume path. Phase C replaced
    // that rule with a per-PC proof (`sr_monitor_scalar_ops`) and RECORDS what it
    // elides (`sr_monitor_at` → `MonitorInfo { relock: true }`, which
    // `build_frame_state_at` publishes and the resume replays). An elision that
    // leaves a `MonitorInfo` must NOT set this flag, or Phase C would exclude the
    // very methods it was built to keep resumable. Every other receiver keeps a
    // real `jit_monitor_enter`/`jit_monitor_exit`. There is no untraceable
    // elision left to report, which is why the flag is never set.
    //
    // The conjunct is KEPT, as the standing contract for the next elision:
    // **any lock elision added to this backend that does not record a
    // `MonitorInfo` must set `has_elided_monitor`.** A flag that is false
    // forever is indistinguishable from a flag whose condition never happens,
    // so the contract is also pinned by a test rather than only asserted here —
    // `jit/tests/r10_deoptverify_elided_monitor_contract.rs` fails if a second
    // elision site appears in the monitor lowering, or if the recording call
    // that makes the present one traceable is removed.
    ...
    //
    // Like its twin, the second conjunct is a constant `true` today
    // (`!false`) — see the long note on `can_deopt_resume` above for why that
    // is the correct state and what the term is still here to catch. It is
    // deliberately NOT collapsed to `!cm.osr_exit_points.is_empty()`: the two
    // gates must keep agreeing, and an elision that stopped recording its
    // monitor has to be excluded from both or the OSR-exit transfer resumes
    // without the re-lock.
...
/// Register every implicit null-check site of an artifact whose code starts at
/// `base` (`sites` are `(fault_off, recover_off)` pairs relative to it).
///
/// All or nothing. `Err(fault_off)` names the first site the table declined;
/// by then every site registered before it has been retired again
/// (`unregister_range(base, code_len)`), so the caller can refuse the artifact
/// with no entry left behind. `CompiledMethod::drop` retires the same range
/// again when the refused artifact is dropped, which is harmless.
///
/// A decline cannot be absorbed: the site's `TEST`/`JZ` was already elided
/// when the code was emitted, so installing the artifact would turn a null
/// receiver there into an access violation instead of a
/// `NullPointerException`
/// (`implicit-null-declined-registration-installs-unguarded-code-20260918`).
fn register_implicit_null_sites(
    base: usize,
    code_len: usize,
    sites: &[(usize, usize)],
) -> Result<(), usize> {
    for &(fault_off, recover_off) in sites {
        if !crate::implicit_null::register(base + fault_off, base + recover_off) {
            crate::implicit_null::unregister_range(base, code_len);
            return Err(fault_off);
        }
    }
    Ok(())
}
...
#[cfg(test)]
mod implicit_null_registration_tests {
    use super::register_implicit_null_sites;

    #[test]
    fn a_declined_site_retires_every_site_the_artifact_registered() {
        // `register` refuses a fault PC equal to a table sentinel
        // (`usize::MAX - 1`), which is how this test makes the SECOND site
        // decline without filling the process-wide table.
        let base = usize::MAX - 0x31;
        let first_fault = base + 0x10;
        let sites = [(0x10usize, 0x20usize), (0x30, 0x28)];
        assert_eq!(
            register_implicit_null_sites(base, 0x31, &sites),
            Err(0x30),
            "the sentinel-keyed site must decline"
        );
        assert_eq!(
            crate::implicit_null::recover(first_fault, 0x10),
            None,
            "the site registered before the decline must have been retired"
        );
    }

    #[test]
    fn an_accepted_artifact_registers_every_site() {
        let base = 0x4f00_0000_0000usize;
        let sites = [(0x10usize, 0x40usize), (0x18, 0x48)];
        assert_eq!(register_implicit_null_sites(base, 0x100, &sites), Ok(()));
        assert_eq!(
            crate::implicit_null::recover(base + 0x10, 0x8),
            Some(base + 0x40)
        );
        assert_eq!(
            crate::implicit_null::recover(base + 0x18, 0x8),
            Some(base + 0x48)
        );
        crate::implicit_null::unregister_range(base, 0x100);
        assert_eq!(crate::implicit_null::recover(base + 0x10, 0x8), None);
    }
}

/// Whether any `arraylength` site of `hoist` lies in an exception-table
/// protected range, under a precise-exception-frames compile.
///
/// `protected` is `[start, end)` in INTERPRETER bcis (the method's own
/// exception table); `bci_of` maps an emitter pc back to one (the loop
/// rewriter's provenance, identity on an ordinary compile). A site is
/// `(seq_start, seq_end)` with the `arraylength` at `seq_end - 1`.
///
/// Such a hoist must be dropped: its pre-header null check takes the shared,
/// frame-less NPE stub, while the in-loop `arraylength` it replaces publishes
/// a precise frame at a protected bci.
fn array_len_hoist_site_is_protected(
    hoist: &ArrayLenHoist,
    protected: &[(u32, u32)],
    bci_of: &dyn Fn(usize) -> u32,
) -> bool {
    hoist.sites.iter().any(|&(_, seq_end)| {
        let bci = bci_of(seq_end.saturating_sub(1));
        protected
            .iter()
            .any(|&(start, end)| bci >= start && bci < end)
    })
}

#[cfg(test)]
mod array_len_hoist_protection_tests {
    use super::{array_len_hoist_site_is_protected, ArrayLenHoist};

    fn hoist(sites: Vec<(usize, usize)>) -> ArrayLenHoist {
        ArrayLenHoist {
            loop_header: 4,
            loop_end: 22,
            array_local: 0,
            trap_bci: sites.first().map_or(4, |&(_, seq_end)| seq_end - 1),
            sites,
        }
    }

    #[test]
    fn a_protected_site_drops_the_hoist_and_an_unprotected_one_keeps_it() {
        let identity = |pc: usize| pc as u32;
        // `aload_0; arraylength` at 5..7: the arraylength is bci 6.
        let h = hoist(vec![(5, 7)]);
        assert!(array_len_hoist_site_is_protected(&h, &[(0, 22)], &identity));
        assert!(array_len_hoist_site_is_protected(&h, &[(6, 7)], &identity));
        // `end` is exclusive, and a range ending AT the arraylength misses it.
        assert!(!array_len_hoist_site_is_protected(&h, &[(0, 6)], &identity));
        assert!(!array_len_hoist_site_is_protected(
            &h,
            &[(7, 30)],
            &identity
        ));
        assert!(!array_len_hoist_site_is_protected(&h, &[], &identity));
    }

    #[test]
    fn any_protected_site_is_enough_and_bcis_are_mapped_back() {
        let identity = |pc: usize| pc as u32;
        // Two sites; only the body one (arraylength at 12) is protected.
        let h = hoist(vec![(5, 7), (11, 13)]);
        assert!(array_len_hoist_site_is_protected(
            &h,
            &[(10, 14)],
            &identity
        ));
        // Under a rewrite, output pc 12 is a copy of interpreter bci 40.
        let rewritten = |pc: usize| if pc == 12 { 40 } else { pc as u32 };
        assert!(!array_len_hoist_site_is_protected(
            &h,
            &[(10, 14)],
            &rewritten
