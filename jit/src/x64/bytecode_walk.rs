...
impl Compiler {
    ...
    pub(super) fn compile_bytecode(&mut self, code: &[u8], code_len: usize) -> bool {
        ...
        //
        // The shared LENIENT decoder, not a private copy: the walk's own
        // pre-pass knew only the 16-bit branches and the two switches, so a
        // `goto_w` target was never marked (and `jit_scan` refused the opcode
        // for that and its other missing arms —
        // `single-pass-refuses-every-method-containing-goto-w-20260918.md`).
        // `branch_target_map` marks every explicit target, `goto_w`/`jsr_w`
        // included, and decodes switches exactly as the old copy did (a
        // truncated or unparseable table contributes what it can; `jit_scan`
        // has already refused such a method). Sized `code_len + 1`: the
        // walk indexes `branch_targets[pc]` for `pc <= code_len`.
        let mut branch_targets = bytecode_analysis::branch_target_map(code, code_len);
        branch_targets.resize(code_len + 1, false);
        ...
        // ── The instruction-start map, built ONCE for the whole walk ──────
        //
        // Three of the seven families need to know whether some pc is a real
        // instruction boundary: the array null checks (`walk_array`), the
        // receiver null checks (`walk_field`), and the `ifnull`/`ifnonnull`
        // arms (`walk_control`). Each of them used to build this vector
        // itself, per SITE — `bytecode_analysis::instruction_starts` allocates
        // `code.len()` bools and re-walks the whole method — so a loop body
        // doing `a[i]` paid one full map per access. On a 4 KB method with 200
        // array accesses that is ~800 K redundant writes per compile.
        //
        // A PARAMETER, not a cache. `x64/null_check_elim.rs`'s previous
        // comment refused a pointer-keyed cache and was right to: a `Vec<u8>`
        // of bytecode can be dropped and another method's allocated at the
        // same address inside one compile (the loop-rewrite path in
        // `driver.rs` does exactly that), so a pointer hit does not imply a
        // content hit. A parameter has no key and therefore no ABA question —
        // the borrow ties the map to the `code` still in scope.
        //
        // Built from `code.len()`, NOT from `code_len`. They differ: `code_len`
        // is threaded from `driver.rs` and re-bound by the loop rewriter,
        // while every helper that consumes this map indexes it with a raw
        // bytecode pc and treats a missing entry as "not a start". A shorter
        // map would quietly turn provable sites into unprovable ones — a
        // silent de-optimization no test catches. See the REVIEW-NOTE at the
        // bottom of `x64/null_check_elim.rs`.
        let insn_starts = bytecode_analysis::instruction_starts(code, code.len());
        ...
        while pc < code_len {
            ...
            if dead {
                ...
                if revive {
                    ...
                    //
                    // ...unless the operand-stack kind analysis can answer.
                    // That shape is not exotic: it is every ROTATED loop —
                    // `goto test; body: ...; test: ...; if<cond> body` — whose
                    // body follows an unconditional `goto` and is named only
                    // by the bottom test. ecj emits every `while`/`for` that
                    // way (Jasper-compiled JSPs, anything built with the
                    // Eclipse compiler), so each such loop cost its method
                    // the compile. `x64::stack_kinds` is a whole-method
                    // forward analysis and has the body's entry state
                    // already; it is admitted here only when it types EVERY
                    // entry (so the oop marks rebuilt from it are exact, not
                    // guessed), and the answer is then RECORDED, so the back
                    // edge that arrives later is checked against it by
                    // `record_branch_target_depth` — a disagreement refuses
                    // the method rather than jumping into a mismatched
                    // layout. javac's top-tested loops never reach this.
                    if !self.branch_target_stack_depth.contains_key(&pc) {
                        let typed = self.stack_kinds.get(pc).and_then(|kinds| {
                            kinds
                                .iter()
                                .map(|k| k.is_category_2().map(|_| k.is_ref()))
                                .collect::<Option<Vec<bool>>>()
                        });
                        if let Some(marks) = typed {
                            self.branch_target_stack_depth.insert(pc, marks.len());
                            self.branch_target_stack_oop_marks.insert(pc, marks);
                        }
                    }
            ...
            if !dead && branch_targets[pc] {
                ...
                // Canonicalise at EVERY live branch target with a non-empty
                // stack, not only at one an earlier branch has recorded.
                //
                // The recorded-depth condition this replaces was right for a
                // forward target — its branch is emitted first, so the depth
                // is known on arrival — and silently skipped a LOOP HEADER,
                // whose only recorded edge is the back edge still to come.
                // The header was then emitted for whatever homes its live
                // operands had on entry, and the back edge arrived with
                // whatever homes the loop body left them in: a scratch
                // register a later push reused, a callee-saved local an
                // `istore` in the body overwrote. javac never leaves an
                // operand live across a loop header; kotlinc does, for an
                // inline lambda's loop in argument position. Canonical here
                // plus canonical at every back edge (`op_control.rs`) makes
                // the two agree by construction. For an empty stack — every
                // javac loop header — this emits nothing.
                //
                // A recorded depth that DISAGREES with the fall-through's is
                // the model and the bytecode diverging, and whichever layout
                // the target gets is wrong for one of its edges; that used to
                // skip canonicalisation and carry on. Refuse instead.
                match self.branch_target_stack_depth.get(&pc).copied() {
                    Some(expected_depth) if expected_depth != self.stack.len() => {
                        self.fail("singlepass-codegen/merge-depth-disagreement");
                        return false;
                    }
                    Some(_) => {}
                    None => {
                        // First edge in: record it, so the back edge that
                        // arrives later is CHECKED against it by
                        // `record_branch_target_depth` rather than recorded
                        // unchallenged.
                        self.record_branch_target_depth(pc);
                    }
                }
            ...
            // === Relocatable pre-header: speculative-BCE entry guards, then
            // the `aaload` and integer-arithmetic LICM hoists ===
            // (`emit_relocatable_preheader`.) For a loop header this is where
            // they always ran. At the entry `goto` of a ROTATED loop
            // (`goto COND; BODY: ...; COND: if<cond> BODY`, only when
            // `CRATONVM_JIT_ROTATED_PREHEADER` armed it in the driver) the
            // header's set runs HERE instead, deopting at the `goto`'s own bci:
            // that `goto` is the loop's only entry, so every entry passes it,
            // while the copy at the header itself is reached only by an OSR
            // entry (the `goto` does not fall through).
            if let Some(&rotated_header) = self.rotation_preheader_by_goto.get(&pc) {
                // The pre-header clobbers RAX/RCX/RDX/R10/R11. javac-style
                // entries have an empty operand stack; anything else is moved
                // to the frame first, exactly as the `goto` arm below would.
                if !self.stack.is_empty() {
                    self.flush_scratch_registers();
                }
                self.emit_relocatable_preheader(rotated_header, pc);
                // And the header's `arraylength` hoists, which the driver
                // admits at a rotated header only when they were proved
                // against COND's straight-line prefix
                // (`find_array_len_hoists_with_rotation`): evaluated here,
                // they throw exactly the NPE COND's first `arraylength` would.
                self.emit_array_len_hoists(rotated_header, pc);
                // Then the header's batch pre-headers, which the driver
                // detected on the loop's unrotated copy
                // (`escape_analysis::detect_batch_loop`). Each is
                // self-contained: it finishes the loop's work (or one
                // strip of it) and publishes the induction variable (and
                // accumulator) at that point, or changes nothing
                // Java-visible, and falls through into this `goto`, whose
                // COND then tests the published state. Only with an empty
                // operand stack: they use registers well beyond the scratch
                // set `flush_scratch_registers` frees. Here they are all
                // fall-through-only (see the header copy below for why a
                // rotated header gets none), so only the first strip of an
                // over-cap span is bulk.
                if self.stack.is_empty() {
                    self.emit_strip_mined_preheaders(rotated_header);
                    self.emit_matrix_dot_preheaders(rotated_header);
                }
                // Code emitted here is fall-through-only (the `goto` below
                // jumps to COND, a join point), and the batch pre-headers
                // branch internally: no reload elision may carry past them.
                self.slot_mirror = None;
            }
            self.emit_relocatable_preheader(pc, pc);
            ...
            // entry at the header runs it. Soundness argument (why the null
            // case THROWS here instead of deopting) on `emit_array_len_hoists`.
            self.emit_array_len_hoists(pc, pc);

            // === Matrix-dot pre-header: `emit_matrix_dot_preheaders`. Runs
            // once on initial loop entry; back-edges skip to the scalar loop
            // (it reads `a[row]` from the LICM ref-hoist slot, which no
            // precise oop map describes, so it is not re-entered after the
            // back-edge poll). A ROTATED header's batch pre-headers ran at
            // its entry `goto` instead (above), and must not run here: this
            // copy is reached only by an OSR entry, which arrives
            // mid-iteration (after COND's test), where running the rest of
            // the loop and then falling into the body would execute one
            // iteration too many.
            let rotated_header = self.rotation_preheader_by_goto.values().any(|&h| h == pc);
            if !rotated_header {
                self.emit_matrix_dot_preheaders(pc);
            }
            // The strip-mined pre-headers (SIMD sum and element-wise, and
            // since round 9 wave 7 the bulk zero fill, stride store and byte
            // sieve) go AFTER `pc_to_native[pc]` (below), so the loop's back
            // edge re-enters them for the next strip. A non-empty operand
            // stack at the join (never javac's) and the two test-only
            // triggers that must fire at exactly `pc_to_native[pc]` keep them
            // fall-through-only here (then only the first strip is bulk,
            // which is still exact).
            let strip_on_back_edge = !rotated_header
                && self.stack.is_empty()
                && self.osr_exit_test_trigger_bci != Some(pc)
                && self.deopt_eager_bci != Some(pc);
            if !rotated_header && !strip_on_back_edge {
                self.emit_strip_mined_preheaders(pc);
            }
            ...
            // (AFTER speculative-BCE/hoisted/matrix-dot code, so back-edges
            // skip the preheader; the strip-mined pre-headers come after it)
            ...
            // The strip-mined pre-headers (see above and
            // `emit_strip_mined_preheaders`): on every arrival at the header,
            // fall-through and back edge alike. They branch internally, so no
            // reload elision may carry past them either.
            if strip_on_back_edge && self.emit_strip_mined_preheaders(pc) {
                self.slot_mirror = None;
            }
            ...
            if self.deopt_eager_bci == Some(pc) {
                ...
                // Reason 2 specifically: the stub pushed just below is tagged
                // 2, and since the key carries the reason a reason-6 snapshot at
                // this pc is no longer mistaken for this one (see the map's
                // declaration in `x64.rs`).
                if !self
                    .deopt_box_ptr_by_bci
                    .contains_key(&(pc, crate::deopt::DeoptReason::BoundsCheck))
                {
            ...
            // === LICM: Replace hoisted sequences with spill slot loads ===
            {
                ...
                if let Some((seq_end, hoist_offset)) = hoist_replace {
                    ...
                    self.push_frame_word(hoist_offset);
            ...
            // hoisted).
            {
                ...
                if let Some((seq_end, result_offset)) = arith_replace {
                    ...
                    if interior_safe {
                        self.push_frame_word(result_offset);
                        let native_pos = self.buf.pos() as i32; // Cast: x86-64 immediate encoding
                        let mut skip_pc = pc + bytecode_analysis::step(code, pc);
                        while skip_pc < seq_end {
                            self.pc_to_native[skip_pc] = native_pos;
            ...
            let family = match op {
                ...
                // goto_w: `goto` with a 32-bit offset (see the `0xc8` arm of
                // `walk_control`).
                0xc8 => WalkFamily::Control,
            ...
            let step = match family {
                WalkFamily::LocalStack => self.walk_local_stack(
                    code,
                    code_len,
                    op,
                    pc,
                ...
                WalkFamily::Array => self.walk_array(
                    code,
                    code_len,
                    op,
                    pc,
                ...
                WalkFamily::Control => self.walk_control(
                    code,
                    code_len,
                    op,
                    pc,
                ...
                WalkFamily::Field => self.walk_field(
                    code,
                    code_len,
                    op,
                    pc,
        ...
        // Operand-home bounds checks (`emit_bounds_check_in`) first get their
        // cold `mov rax, <array> ; mov rcx, <index> ; jmp` prologue; the pads
        // below read RAX/RCX. After the walk (every unrolled copy is in the
        // table), before the pads (the prologue's `jmp` is what they patch).
        self.emit_bounds_check_home_prologues();
...
impl Compiler {
    /// Emit the loop-invariant `arraylength` hoists of the loop at `header`
    /// (`ArrayLenHoist`), standing at bci `at`: the header itself, or a
    /// relocated rotated loop's entry `goto` (see the call sites).
    ///
    /// SOUNDNESS: the pre-header executes UNCONDITIONALLY, including when the
    /// loop is zero-trip. `find_array_len_hoists` therefore only offers sites
    /// in the straight-line prefix of the point the loop is ENTERED at -- the
    /// header, or COND for a relocated rotated loop -- reached through nothing
    /// but local/constant pushes, so the original program was always going to
    /// evaluate this `arraylength` at this exact moment anyway. That is why the
    /// null case here THROWS (`npe_action::ARRAY_LENGTH`, the same JEP-358
    /// action and the same shared stub the in-loop `arraylength` would have
    /// used) rather than deopting the way the aaload hoist's unconstrained
    /// sites must. It also keeps the emitted bytes address-independent: a
    /// deopt snapshot bakes a `Box` pointer, and two compiles of one method
    /// would stop producing identical code.
    ///
    /// The reported bci is `ArrayLenHoist::trap_bci`, the `arraylength` of the
    /// site that opened the record -- the one that would have trapped first
    /// had nothing moved. The check no longer stands where the programmer
    /// wrote it, so it has to name a site explicitly.
    ///
    /// The cached value is an int, so unlike the aaload hoist's row pointer it
    /// is not a GC root, needs no oop map, and is not invalidated by
    /// relocation: an array's length is immutable and no bytecode can write it.
    ///
    /// Same placement contract as the relocatable pre-header: emitted BEFORE
    /// `pc_to_native[at]` is set, so the back edge skips it, while
    /// `osr_entry_native[header]` points before the header's copy, so a cold
    /// OSR entry at the header runs it.
    fn emit_array_len_hoists(&mut self, header: usize, at: usize) {
        let len_hoists: Vec<(usize, i32, usize)> = self
            .array_len_hoist_info
            .iter()
            .enumerate()
            .filter(|(_, h)| h.loop_header == header)
            .map(|(idx, h)| (h.array_local, self.array_len_hoist_offsets[idx], h.trap_bci))
            .collect();
        for (array_local, hoist_offset, trap_bci) in len_hoists {
            ...
            // The in-loop null check, moved here whole: same TEST/JZ, same
            // shared stub, same "Cannot read the array length" action. It is
            // elided outright when the dataflow already proves the receiver
            // non-null where this copy stands.
            if !self.is_local_nonnull(at, array_local) {
                let key = crate::x64::inlining::record_npe_trap_site(trap_bci);
                self.emit_null_check_array_load(npe_action::ARRAY_LENGTH, key, trap_bci);
            }
            // MOV EAX, [RAX + array length offset] -- zero-extends into RAX,
            // and a length is non-negative, so the 64-bit slot below holds the
    ...
    /// The SIMD int-array sum and element-wise pre-headers of the loop at
    /// `header`, then its bulk byte pre-headers ([`Self::emit_batch_preheaders`];
    /// at most one family matches: their bodies are exact and different).
    /// Returns whether anything was emitted.
    ///
    /// All of them are STRIP-MINED (the SIMD pair by `emit_strip_clamp`, round
    /// 9 wave 4; the bulk byte ones since wave 6) — a pass covers at most
    /// `MAX_BULK_BYTE_LOOP_SPAN` elements and leaves the rest to the original
    /// loop — so the walk emits them AT `pc_to_native[header]`, where the
    /// loop's back edge (which polls) re-enters them for the next strip. That
    /// is sound because each pass computes exactly what the loop computes from
    /// ANY state at its header: the detectors admit only bodies with no other
    /// effect and no other exit, and every input is re-read from its home.
    /// Re-entry is also cheap: a pass that is not clamped finishes the loop,
    /// so the back edge is taken again only after a clamped pass, or when a
    /// self-guard refused (a loop that is about to throw).
    fn emit_strip_mined_preheaders(&mut self, header: usize) -> bool {
        let start = self.buf.pos();
        // === SIMD: Emit vectorized preheader for int-array-sum loops ===
        // At `pc_to_native[header]`: runs on entry and, strip-mined, again on
        // the back edge after each clamped pass (`emit_strip_clamp`).
        {
            let simd_match = self
                .simd_loops
                .iter()
            ...
            if let Some((iv_local, acc_local, array_local, bound_local, acc_is_long)) = simd_match {
                ...
                self.emit_simd_int_array_sum(header, acc_offset, acc_is_long);
        ...
        if has_avx2() {
            ...
            if let Some((iv_local, out_local, a_local, b_local, bound_local, op)) = ewise_match {
                ...
                self.emit_simd_int_array_element_wise(header, op);
        ...
        // The bulk byte pre-headers (sieve, zero fill, stride store),
        // strip-mined since round 9 wave 6 (simd6) and re-entered from the
        // back edge since wave 7 (bulkplace7).
        self.emit_batch_preheaders(header);

        self.buf.pos() != start
    }

    /// The bulk byte batch pre-headers of the loop at `header`: the byte
    /// sieve nest, the bulk zero fill and the stride store. Each is found by
    /// `header` in its list; a header with none emits nothing. Called only
    /// from [`Self::emit_strip_mined_preheaders`], so they are placed exactly
    /// where the two SIMD pre-headers are: at `pc_to_native[header]`, where
    /// the loop's back edge re-enters them, or fall-through-only (a rotated
    /// loop's entry `goto`, a non-empty operand stack, the test triggers).
    ///
    /// Re-entry from the back edge is sound for the same reasons as for the
    /// SIMD pair. Every one of them reads the loop's locals and array bases
    /// from their homes on each entry (no raw pointer survives the back-edge
    /// poll), guards everything it relies on, and either runs one strip --
    /// the loop's remaining iterations, or, past `MAX_BULK_BYTE_LOOP_SPAN`
    /// (the sieve: `SIEVE_STRIP_BUDGET`), the iterations up to a state the
    /// original loop reaches at its header -- and publishes that state, or
    /// falls through untouched. The detectors admit only bodies with no
    /// other effect and no other exit, so a pass from ANY state at the header
    /// computes exactly what the loop would. None records a deopt or trap
    /// site, so the bci it stands at does not matter. The driver drops the
    /// native-unroll plans of these headers (the unroller copies from
    /// `pc_to_native[header]`) and disables the R8/R9 kernel operand cache
    /// for the sieve and the stride store; the zero fill touches neither.
    fn emit_batch_preheaders(&mut self, header: usize) {
        if let Some(sieve) = self
            .byte_sieve_loops
            .iter()
            .find(|sieve| sieve.header_pc == header)
            .cloned()
        {
        ...
        if let Some(fill) = self
            ...
            .iter()
            .find(|fill| fill.header_pc == header)
            .cloned()
        {
    ...
    /// The matrix-dot pre-header of the loop at `header` (guarded `int[][]`
    /// dot-product replacement), FALL-THROUGH-ONLY: called at the header
    /// BEFORE `pc_to_native[header]`, so normal back edges skip it, or at a
    /// rotated loop's entry `goto`. It is strip-mined like the bulk byte
    /// pre-headers (round 9 wave 6), but deliberately not re-entered from the
    /// back edge: it reads `a[row]` from the LICM `aaload` hoist slot, and
    /// ref-hoist slots are in no precise oop map (the GC reaches them only
    /// through the conservative frame-band scan; see
    /// `vm/src/jit/conservative_roots.rs` `region_is_dataflow_modelled`). An
    /// inner dimension over 2^20 is not a realistic matrix, so bulk-only-on-
    /// the-first-strip costs nothing in practice. Emits nothing for a header
    /// without one.
    fn emit_matrix_dot_preheaders(&mut self, header: usize) {
        if let Some(dot) = self
            .matrix_dot_loops
            .iter()
            .find(|dot| dot.header_pc == header)
            .cloned()
        {
    ...
    /// The part of a loop's pre-header that may run at a place other than its
    /// header: the speculative-BCE entry guards, then the `aaload` hoists, then
    /// the integer-arithmetic hoists — in that order (the guards must deopt
    /// before any speculative code runs).
    ///
    /// `header` selects WHICH loop's transforms (they are keyed by header pc);
    /// `at` is the bci this code stands at, and is the bci every deopt it
    /// records resumes at. For an ordinary header the two are equal. For a
    /// rotated loop's entry `goto` (`rotation_preheader_by_goto`) `at` is the
    /// `goto`: a failed guard must resume the interpreter AT the `goto` —
    /// resuming at the header (the body) would run one iteration without its
    /// test. The machine state is the same at both, because nothing executes
    /// between the `goto` and the loop's first test.
    ///
    /// Emits nothing for a `header` with none of the three transforms.
    fn emit_relocatable_preheader(&mut self, header: usize, at: usize) {
        ...
        //   JB deopt_stub  (if array.length < loop_bound, deopt)
        {
            ...
            if let Some(first) = guards.first() {
                ...
                self.buf.emit(&[0x00, 0x00, 0x00, 0x00]);
                self.deopt_stubs.push((patch_offset, at, 2)); // 2 = DEOPT_REASON_BOUNDS_CHECK
                ...
                if first.inclusive {
                    ...
                    let p = self.buf.pos();
                    self.buf.emit(&[0x00, 0x00, 0x00, 0x00]);
                    self.deopt_stubs.push((p, at, 2)); // 2 = DEOPT_REASON_BOUNDS_CHECK
                }
            ...
            for guard in guards {
                ...
                self.buf.emit(&[0x00, 0x00, 0x00, 0x00]);
                self.deopt_stubs.push((null_patch, at, 2)); // 2 = DEOPT_REASON_BOUNDS_CHECK
            ...
            if had_guards {
                self.emit_deopt_snapshot_at_guard(at);
            }
        }
        ...
        // LicmHoistRepro.java).
        {
            ...
            for (array_local, index_local, hoist_offset) in loop_hoists {
                ...
                self.buf.emit(&[0x00, 0x00, 0x00, 0x00]);
                self.deopt_stubs.push((bounds_patch, at, 2)); // 2 = DEOPT_REASON_BOUNDS_CHECK
            ...
            if had_hoists
                && !self
                    .deopt_box_ptr_by_bci
                    .contains_key(&(at, crate::deopt::DeoptReason::BoundsCheck))
            {
                self.emit_deopt_snapshot_at_guard(at);
            }
        }
