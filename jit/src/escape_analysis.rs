...
//!   reached only from the tier-2 IR pipeline in `jit/src/lib.rs`, through the
//!   bridge in `jit/src/ea_ir_bridge.rs` (`escape_analysis_from_ir` builds the
//!   graph; `analyze_escapes(&ea_graph)` runs it). Its results drive IR-level
//!   scalar replacement and lock elision.
//! * **The single-pass x64 backend has its own**, private, *bytecode-level*
//!   `analyze_escapes(code, code_len, &invokespecial_shapes)` in
//!   `jit/src/x64/escape_analysis.rs`. That is the one that runs for the overwhelming majority
...
//! `jit/src/ea_ir_bridge.rs` (`apply_ea_to_ir_pinned`) reads only two:
//!
...
//!   (`apply_ea_to_ir_pinned` in `jit/src/ea_ir_bridge.rs`) reads those per-object plans
...
//! EA graph cannot ask it: the bridge in `jit/src/ea_ir_bridge.rs`
...
pub enum Op {
    ...
    ///
    /// Produced since cov-07: the bridge maps `ir::Op::Throw` here, forwarding
    /// `[ctrl, exc]` (the memory token is dropped, r9). The escape rule filters
    /// its inputs by `is_ref_producer`, so the control input is harmless.
    Throw,
    ArrayLength,
    ...
    ///
    /// Produced by `escape_analysis_from_ir` (`jit/src/ea_ir_bridge.rs`) for an
    /// `ir::Op::Cmp` whose two operands are `IrType::Ref` and neither is the
    /// null literal (`ir_cmp_is_ref_compare`). A null compare stays
    /// [`Op::Other`]. (This comment used to say the variant had no producer;
    /// corrected 2026-09-18.)
    RefCompare,
    /// Identity-hash observation.  Input `[obj]`.
    ///
    /// Produced by the bridge for a static `System.identityHashCode` call ONLY
    /// (`ir_call_is_identity_hash`). An `invokespecial Object.hashCode()I` is
    /// deliberately not recognised: under `ACC_SUPER` it can select an
    /// override, and this variant escapes nothing (r9, 2026-09-18).
    IdentityHash,
    Dead,
    Other,
}
...
pub struct Graph {
    ...
    /// For each field/element [`Op::Load`], the node its incoming MEMORY TOKEN
    /// names in the producer's graph (mapped into EA ids): the latest memory
    /// operation the load is ordered after.
    ///
    /// # Why the analysis needs this (r9-ea)
    ///
    /// Both dominance stand-ins — [`program_order_proves_dominance`] and
    /// `one_block_proves_dominance` — read "store id < load id" as "the store
    /// ran first". That holds for nodes the builder created in walk order, but
    /// not for a node a later pass CREATED: the full unroller
    /// (`ir_optimize::unroll`) clones a loop's `Op::Load`s with fresh, HIGH ids
    /// and pins every clone to the preheader, and it rewires the loop's exit
    /// edge to that same preheader. After a full unroll, a store that runs
    /// AFTER the old loop has a LOWER id than the cloned loads that run before
    /// it, in the same block — so a load could be answered with a store from
    /// its own future, which is the miscompile the positional record exists to
    /// prevent.
    ///
    /// The token is an order the graph actually carries: the builder threads
    /// every memory operation — stores, loads, calls, monitors — onto one
    /// chain, and a cloned load keeps its original token. So a store `S` ran
    /// before load `L` on that chain iff `S` is `L`'s token or earlier on the
    /// chain. `resolve_field_load` asks BOTH questions and refuses (answers
    /// `Unknown`) whenever they disagree; it never forwards a value the id
    /// order would not have forwarded. On a builder-order graph they always
    /// agree, so this costs nothing there.
    ///
    /// **Empty by default**, and a load with no entry is answered by id order
    /// alone, exactly as before this existed.
    pub load_mem_tokens: HashMap<NodeId, NodeId>,
}
...
/// `CRATONVM_JIT_EA_CROSS_BLOCK_LOADS=1` (`CRATONVM_JIT=ea-cross-block-loads`)
/// — answer a replaced object's loads OUTSIDE its allocation block when every
/// store to it is inside that block ([`alloc_block_holding_every_store`]).
///
/// Default OFF while it is A/B'd (JIT review round 9, wave 2). Read per
/// analysis, not cached: this is compile-time work, and the snapshot lookup is
/// cheap next to the analysis it gates.
pub fn ea_cross_block_loads_enabled() -> bool {
    cratonvm_types::flags::runtime_flag_on("CRATONVM_JIT_EA_CROSS_BLOCK_LOADS")
}

/// The allocation's own block, when EVERY store to the object is in it —
/// the premise that lets a load in ANY other block be answered.
///
/// # The argument
///
/// Let *B* be the allocation's block, and require (as
/// [`one_block_proves_dominance`] does) that the reference has no φ alias
/// (`transparent` is the allocation alone) and that every store in `stores`
/// is in *B*. Take a load *L* of this object in a different block *C*.
///
/// 1. *L*'s holder input is the allocation itself (no φ), and the producer's
///    graph is SSA, so *C* is dominated by *B*: the object *L* reads is the one
///    created by the most recent execution of *B*.
/// 2. Each execution of *B* creates a fresh object (the allocation is in *B*),
///    and *B* is straight-line, so every store that execution makes to that
///    object has completed before control leaves *B* — before *L*.
/// 3. No other store to it exists: the use walk in
///    [`find_scalar_replacements`] is exhaustive, and a non-escaping object
///    cannot be written through a call or another thread.
///
/// So *L* reads the value of the LAST store to its field in *B* (program
/// order within one block, as in [`one_block_proves_dominance`]), or the zero
/// default when the field has no store. Loads inside *B* keep the ordinary
/// positional answer. A load with no recorded block is still `Unknown`.
///
/// `None` whenever the allocation or any store has no recorded block, or the
/// stores are not all in the allocation's block — a missing fact is never
/// read as agreement.
fn alloc_block_holding_every_store(
    graph: &Graph,
    alloc: NodeId,
    transparent: &HashSet<NodeId>,
    stores: &[Vec<FieldStore>],
    ...
    let block = *graph.blocks.get(&alloc)?;
    stores
        .iter()
        .flatten()
        .all(|fs| graph.blocks.get(&fs.store) == Some(&block))
        .then_some(block)
}

/// A load OUTSIDE the allocation block, under
/// [`alloc_block_holding_every_store`]'s premise: the last store to the field
/// (`stores` ascending by id, all in that block), or the zero default.
///
/// `mem_token` is cross-checked the way [`resolve_field_load`] does it: the
/// memory chain must also place that store at or before the load, or the
/// answer is `Unknown`. It can only refuse.
fn resolve_cross_block_load(stores: &[FieldStore], mem_token: Option<NodeId>) -> LoadResolution {
    let last = stores.last();
    if let Some(token) = mem_token {
        let by_token = stores.iter().rev().find(|s| s.store <= token);
        if by_token.map(|s| s.store) != last.map(|s| s.store) {
...
/// (`escape_analysis_from_ir` in `jit/src/ea_ir_bridge.rs`) rewrites `Op::Load` /
...
pub struct ScalarReplacementInfo {
    ...
    /// from today (via `jit/src/ea_ir_bridge.rs::virtual_object_info_for`). **For load
    ...
    /// graph, or [`one_block_proves_dominance`] for this object alone, or
    /// (opt-in, [`ea_cross_block_loads_enabled`]) every store to it being in
    /// its allocation block ([`alloc_block_holding_every_store`]).
...
impl ScalarRefusal {
    /// Stable index into [`SCALAR_REFUSAL_CENSUS`]. Kept next to the enum so a
    /// new variant that forgets a row here fails the exhaustive `match` rather
    /// than silently landing in another reason's bucket.
    pub const fn ordinal(self) -> usize {
        match self {
            ScalarRefusal::Escapes(EscapeState::NoEscape) => 0,
            ScalarRefusal::Escapes(EscapeState::ArgEscape) => 1,
            ScalarRefusal::Escapes(EscapeState::GlobalEscape) => 2,
            ScalarRefusal::Escapes(EscapeState::PartialEscape) => 12,
            ScalarRefusal::IdentityObserved => 3,
            ScalarRefusal::StoredIntoAnotherObject => 4,
            ScalarRefusal::FieldIndexOutOfRange => 5,
            ScalarRefusal::PhiNotTransparentCopy => 6,
            ScalarRefusal::MonitorOperation => 7,
            ScalarRefusal::OpaqueUse => 8,
            ScalarRefusal::SelfReference => 9,
            ScalarRefusal::LoadNotAnswerable => 10,
            ScalarRefusal::ArrayNotReplaceable(_) => 11,
        }
    }

    /// The census row names, in [`ScalarRefusal::ordinal`] order.
    pub const NAMES: [&'static str; 13] = [
        "escapes:no-escape(unreachable)",
        "escapes:arg-escape",
        "escapes:global-escape",
        "identity-observed",
        "stored-into-another-object",
        "field-index-out-of-range",
        "phi-not-transparent-copy",
        "monitor-operation",
        "opaque-use",
        "self-reference",
        "load-not-answerable",
        "array-not-replaceable",
        "escapes:partial-escape",
    ];
}

/// Why scalar replacement refused, counted across the process.
///
/// Added 2026-09-16. `scalar_replaceable` came back EMPTY on a probe written
/// specifically to be replaceable, and the candidates/planned pair could say
/// only that the analysis offered nothing -- not WHY. These twelve numbers are
/// the why, and they separate explanations this module's own doc comments
/// already name as plausible: `escapes:arg-escape` is "every `new` is passed
/// to its own `<init>`, and `Op::Call` arg-escapes every reference operand",
/// while `load-not-answerable` is the one `ScalarRefusal::LoadNotAnswerable`
/// documents as "the refusal a hot loop hits". Without the split, a zero
/// candidate count is compatible with both and actionable on neither.
static SCALAR_REFUSAL_CENSUS: [std::sync::atomic::AtomicU64; 13] =
    [const { std::sync::atomic::AtomicU64::new(0) }; 13];

/// `(name, count)` per refusal reason, in [`ScalarRefusal::ordinal`] order.
pub fn scalar_refusal_census() -> Vec<(&'static str, u64)> {
    ScalarRefusal::NAMES
        .iter()
        .enumerate()
        .map(|(i, name)| {
            (
                *name,
                SCALAR_REFUSAL_CENSUS[i].load(std::sync::atomic::Ordering::Relaxed),
            )
        })
        .collect()
}

/// `[analyses run, allocation nodes seen]`.
///
/// Added 2026-09-16, immediately after the refusal census came back with every
/// one of its thirteen rows at ZERO alongside `candidates=0`. That combination
/// refutes both explanations this module's comments offer: escape analysis is
/// not refusing these allocations for any reason, because it is not seeing an
/// allocation to refuse. Three cases remain and the refusal rows cannot
/// separate them -- the analysis never ran, it ran on a graph with no
/// `Op::New`/`Op::NewArray` node in it, or it ran on the wrong graph. These
/// two numbers do: `analyses = 0` is the first, `analyses > 0` with
/// `allocations = 0` is the second.
static SCALAR_SCOPE_CENSUS: [std::sync::atomic::AtomicU64; 2] =
    [const { std::sync::atomic::AtomicU64::new(0) }; 2];

/// `(analyses_run, allocation_nodes_seen)`.
pub fn scalar_scope_census() -> (u64, u64) {
    (
        SCALAR_SCOPE_CENSUS[0].load(std::sync::atomic::Ordering::Relaxed),
        SCALAR_SCOPE_CENSUS[1].load(std::sync::atomic::Ordering::Relaxed),
    )
}

/// Record one refusal. Unconditional: a counter that only runs under a flag
/// cannot answer a question asked after the fact, and this is one `Relaxed`
/// add on a path that has already walked an object's whole use set.
fn note_scalar_refusal(reason: ScalarRefusal) {
    SCALAR_REFUSAL_CENSUS[reason.ordinal()].fetch_add(1, std::sync::atomic::Ordering::Relaxed);
}
...
pub struct EscapeAnalysisResult {
    ...
    /// sound applied a whole plan at a time.** Kept for tests and diagnostics;
    /// `apply_ea_to_ir` reads the grouped [`Self::lock_elisions`] instead.
    ...
    ///
    /// This module *offers* elisions; `apply_ea_to_ir` in `jit/src/ea_ir_bridge.rs`
    ...
    /// The production consumer (`apply_ea_to_ir_pinned`) reads THIS grouped
    /// view and applies each plan all-or-nothing (verified 2026-09-18); the flat
    /// list is kept for tests and diagnostics.
...
fn build_connection_graph(graph: &Graph) -> ConnectionGraph {
    ...
    for (id, node) in graph.nodes.iter().enumerate() {
        match &node.op {
            ...
            // catch-all. `jit/src/ea_ir_bridge.rs::ir_op_to_ea_op` funnels every
...
fn propagate_escape_states(cg: &mut ConnectionGraph, graph: &Graph) {
    ...
    while changed && iteration < MAX_ITERATIONS {
        ...
        //
        // COMPILE-TIME COST (r9-ea). This used to be a loop over every
        // allocation with, inside it, a loop over every node that re-ran
        // `resolve_points_to` (a BFS with two `HashSet`s) and the whole store
        // rule below — `O(allocations × nodes)` BFS walks per iteration, and
        // the store rule evaluated once per allocation for the same store. The
        // rules are unchanged; each is now evaluated once per node per
        // iteration. Points-to sets do not change during this phase (only
        // escape states do), so a node's set is resolved once, and a node with
        // neither a points-to nor a deferred entry resolves to the empty set
        // and is skipped without a walk. Every rule here joins upward and the
        // loop runs to a fixed point, so visiting in a different order reaches
        // the same (least) fixed point — the only observable difference is
        // that the iteration bound is reached less often, i.e. fewer
        // fail-closed `escalate_all_to_global` escalations.
        //
        // The allocation-less graph used to skip the store rule entirely (its
        // enclosing loop had nothing to iterate). With no allocation there is
        // no object for any consumer to act on, so running it is a no-op for
        // every result this module reports.
        for id in 0..graph.nodes.len() {
            if !cg.points_to.contains_key(&id) && !cg.deferred_edges.contains_key(&id) {
                continue;
            }
...
/// consumer (`apply_ea_to_ir` in `jit/src/ea_ir_bridge.rs`) can and does refuse elisions
...
/// # Producer status
///
/// Both variants are produced (verified 2026-09-18): the bridge
/// (`escape_analysis_from_ir`) emits [`Op::RefCompare`] for a reference
/// `ir::Op::Cmp` that is not a null compare, and [`Op::IdentityHash`] for a
/// static `System.identityHashCode` call. See `docs/jit/escape-analysis.md` §4.
...
fn find_scalar_replacements(
    ...
    find_scalar_replacements_with(cg, graph, ea_cross_block_loads_enabled())
}

/// [`find_scalar_replacements`] with the cross-block load rule
/// ([`alloc_block_holding_every_store`]) as an argument, so it can be tested
/// without process-global flag state.
fn find_scalar_replacements_with(
    cg: &ConnectionGraph,
    graph: &Graph,
        ...
        if let Some((class_id, num_fields, array_element_type)) = shape {
            ...
            while let Some(use_id) = worklist.pop() {
                ...
                match &use_node.op {
                    ...
                    // `jit/src/ea_ir_bridge.rs::apply_ea_to_ir`). Each
            ...
            // Opt-in: every store in the allocation's own block answers loads
            // in every other block too. Only consulted when program order is
            // not already dominance for this object.
            let store_block = if cross_block_loads && !dominance_proved && can_replace {
                alloc_block_holding_every_store(graph, id, &transparent, &field_stores)
            } else {
                None
            };
            let dominance_proved = dominance_proved || store_block.is_some();
            ...
            if can_replace {
                for (&load, &field) in replaced_loads.iter().zip(load_fields.iter()) {
                    let mem_token = graph.load_mem_tokens.get(&load).copied();
                    let resolution = match store_block {
                        // Outside the allocation block: the block's last store.
                        Some(b) if graph.blocks.get(&load).is_some_and(|&lb| lb != b) => {
                            resolve_cross_block_load(&field_stores[field], mem_token)
                        }
                        // No recorded block: nothing orders it.
                        Some(_) if !graph.blocks.contains_key(&load) => LoadResolution::Unknown,
                        // Inside the block (or the ordinary proofs).
                        _ => resolve_field_load(
                            &field_stores[field],
                            load,
                            dominance_proved,
                            mem_token,
    ...
    refusals.dedup();
    // Counted HERE, at the single exit, rather than at each of the fourteen
    // `refusals.push` sites: a new refusal arm added later cannot forget to
    // register itself, which is the failure mode that makes a census lie by
    // omission. After the dedup, so one object contributes one reason -- the
    // walk stops at an object's first refusal anyway, but the dedup is what
    // makes that a guarantee rather than an observation.
    for &(_, reason) in &refusals {
        note_scalar_refusal(reason);
    }
    // Scope, not outcome: how many times this ran, and how many allocation
    // nodes it had to look at. Both are needed -- a zero refusal count means
    // something completely different depending on whether this walk ever saw
    // an `Op::New` at all.
    SCALAR_SCOPE_CENSUS[0].fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    let allocations = graph
        .nodes
        .iter()
        .filter(|n| matches!(n.op, Op::New { .. } | Op::NewArray { .. }))
        .count() as u64;
    SCALAR_SCOPE_CENSUS[1].fetch_add(allocations, std::sync::atomic::Ordering::Relaxed);
    (results, refusals)
}
...
///
/// `mem_token` is the load's entry in [`Graph::load_mem_tokens`], when the
/// producer recorded one. Then the store that precedes the load by id must be
/// the same store that precedes it on the memory chain (a store at or before
/// the token); if the two orders disagree — a pass created the load after the
/// stores it runs before — the answer is `Unknown`. This can only refuse, never
/// change a forwarded value.
fn resolve_field_load(
    stores: &[FieldStore],
    load: NodeId,
    dominance_proved: bool,
    mem_token: Option<NodeId>,
) -> LoadResolution {
    if stores.is_empty() {
    ...
    let by_id = stores.iter().rev().find(|s| s.store < load);
    if let Some(token) = mem_token {
        let by_token = stores.iter().rev().find(|s| s.store <= token);
        if by_token.map(|s| s.store) != by_id.map(|s| s.store) {
            return LoadResolution::Unknown;
        }
    }
    match by_id {
...
///
/// Kept because it is the shape [`EscapeAnalysisResult::elide_locks`] has (a
/// test and diagnostic view; `apply_ea_to_ir` reads the plans). It loses the
/// grouping, and the grouping is
...
fn find_lock_coarsening_plans(
    ...
    for object in objects {
        ...
        for pair in outer.windows(2) {
            ...
            // A load whose MEMORY TOKEN lies in the gap executes in the gap,
            // whatever its id (r9-ea). The id-range scan above cannot see one a
            // later pass created — a full unroll's clones carry high ids but
            // keep their original token (see `Graph::load_mem_tokens`) — and a
            // cloned load of a caller-supplied holder can throw NPE inside what
            // would become the merged critical section. Such a load gets the
            // same allowlist test as any gap node.
            if refusal.is_none() {
                let mut late: Vec<NodeId> = graph
                    .load_mem_tokens
                    .iter()
                    .filter(|&(&l, &t)| {
                        t >= first.exit && t < second.enter && !(l > first.exit && l < second.enter)
                    })
                    .map(|(&l, _)| l)
                    .collect();
                late.sort_unstable();
                refusal = late
                    .into_iter()
                    .find_map(|l| gap_node_refusal(cg, graph, l));
            }
            match refusal {
...
pub fn analyze_escapes(graph: &Graph) -> EscapeAnalysisResult {
    ...
    for (id, node) in graph.nodes.iter().enumerate() {
        if matches!(node.op, Op::New { .. } | Op::NewArray { .. }) {
            ...
            let mut state = raw;
            // `escape_sites_for` walks every node (with a points-to BFS per
            // reference operand) for every escaping allocation. With no cold
            // node the answer is fixed before it is computed -- a non-empty
            // site list cannot be all-cold, and an empty one is refused -- so
            // skip the walk (r9-ea). `cold_nodes` is empty unless profile
            // branch hints were fed in, i.e. on every default compile.
            if raw.may_escape() && !graph.cold_nodes.is_empty() {
...
pub fn apply_scalar_replacement(graph: &mut Graph, info: &ScalarReplacementInfo) {
    // NOTE (2026-09-16): this function has no production caller -- every call
    // site is a unit test in this module. The shipping pipeline plans here and
    // applies on the IR graph (`lib.rs`, `plan_scalar_replacement` plus the
    // victim splice), so the census note that used to live on this line was
    // counting a path nothing takes and reported zero unconditionally. It now
    // lives on the applier that runs. Left uncounted here on purpose: a note
    // on both would double-count if this applier were ever wired up.
...
mod r9_ea_tests {
    ...
    fn one_field_alloc(g: &mut Graph) -> NodeId {
        g.add_node(
            Op::New {
                class_id: 1,
                num_fields: 1,
    ...
    /// A full unroll clones a loop's loads with HIGH ids into the preheader,
    /// where a store that runs after the old loop has a LOWER id. The memory
    /// token still says the load ran first; the two orders disagree, so the
    /// load is refused rather than forwarded a value from its own future.
    #[test]
    fn a_load_the_memory_chain_orders_before_a_store_is_not_forwarded_that_store() {
        let mut g = Graph::new();
        // Stands for the memory state before the (unrolled) loop.
        let mem0 = g.add_node(Op::Other, vec![]);
        let alloc = one_field_alloc(&mut g);
        let c42 = g.add_node(Op::Const(42), vec![]);
        let _store = g.add_node(Op::Store(0), vec![alloc, c42]);
        // Higher id than the store, but chained BEFORE it.
        let load = g.add_node(Op::Load(0), vec![alloc]);
        g.load_mem_tokens.insert(load, mem0);
        ...
        assert!(
            result
                ...
                .iter()
                .all(|s| s.alloc_node != alloc),
            "id order and chain order disagree: refuse, never guess"
        );
        assert!(result
            .scalar_refusals
            .contains(&(alloc, ScalarRefusal::LoadNotAnswerable)));
    }

    /// The agreeing case is unchanged: a load whose token IS the store reads it.
    #[test]
    fn a_load_whose_memory_token_is_the_store_reads_that_store() {
        let mut g = Graph::new();
        let alloc = one_field_alloc(&mut g);
        let c42 = g.add_node(Op::Const(42), vec![]);
        let store = g.add_node(Op::Store(0), vec![alloc, c42]);
        let load = g.add_node(Op::Load(0), vec![alloc]);
        g.load_mem_tokens.insert(load, store);
        ...
        let sr = result
            ...
            .iter()
            .find(|s| s.alloc_node == alloc)
            .expect("the orders agree, so the object is replaced");
        assert_eq!(sr.load_value(load), LoadResolution::Value(c42));
    }

    /// And a load before every store, by both orders, still reads the zero
    /// default.
    #[test]
    fn a_load_before_the_store_by_both_orders_reads_the_zero_default() {
        let mut g = Graph::new();
        let mem0 = g.add_node(Op::Other, vec![]);
        let alloc = one_field_alloc(&mut g);
        let load = g.add_node(Op::Load(0), vec![alloc]);
    ...
    /// The restructured propagation still reaches the fixed point through a
    /// chain of stores: `c` is returned, `b` is stored in `c`, `a` in `b` —
    /// every one of them escapes globally, however the nodes are ordered.
    #[test]
    fn escape_propagates_down_a_chain_of_stores_written_in_reverse() {
        let mut g = Graph::new();
        let a = one_field_alloc(&mut g);
        let b = one_field_alloc(&mut g);
        let c = one_field_alloc(&mut g);
        // Written innermost-first, so a single forward sweep cannot finish it.
        let _s_ab = g.add_node(Op::Store(0), vec![b, a]);
        let _s_bc = g.add_node(Op::Store(0), vec![c, b]);
        g.nodes[g.exit].inputs.push(c);
        g.nodes[c].uses.push(g.exit);

        let result = analyze_escapes(&g);
        for alloc in [a, b, c] {
            assert_eq!(
                result.escape_states.get(&alloc).copied(),
                Some(EscapeState::GlobalEscape),
                "node {alloc} is reachable from the returned object"
            );
        }
    }

    /// A load that executes in a coarsening gap by its memory token, but was
    /// given an id outside the gap's id range (a full unroll's clone), is still
    /// held to the gap allowlist: here it reads a caller-supplied holder, so it
    /// can throw, and the merge is refused.
    #[test]
    fn a_cloned_load_in_the_gap_is_seen_by_its_memory_token() {
        let mut g = Graph::new();
        let this = g.add_node(Op::Param(0), vec![]);
        let o = one_field_alloc(&mut g);
        let _e1 = g.add_node(Op::MonitorEnter, vec![o]);
        let x1 = g.add_node(Op::MonitorExit, vec![o]);
        let _e2 = g.add_node(Op::MonitorEnter, vec![o]);
        let _x2 = g.add_node(Op::MonitorExit, vec![o]);
        // Created after both regions, ordered right after the first exit.
        let late = g.add_node(Op::Load(0), vec![this]);
        g.load_mem_tokens.insert(late, x1);

        let r = analyze_escapes(&g);
        assert!(
            r.lock_coarsening.is_empty(),
            "a possibly-throwing load runs between the regions"
        );
        assert!(r.lock_refusals.contains(&(o, LockRefusal::ObservableGap)));
    }
}
...
#[cfg(test)]
mod r9w2_cross_block_tests {
    use super::*;

    /// A branchy graph (so the whole-graph stand-in fails) with an object
    /// allocated and stored in `block` and read in `other` — the IrEscapeProbe
    /// shape once `consume(mine)` is spliced: `mine.v = depth` in the entry
    /// block, the read after the `down(...)` call in a later one.
    ///
    /// With `late_store`, a second store to the field sits in `other` too.
    fn build(late_store: bool) -> (Graph, NodeId, NodeId, NodeId) {
        let mut g = Graph::new();
        let block = g.add_node(Op::Merge, vec![]);
        let other = g.add_node(Op::Merge, vec![]);
        let obj = g.add_node(
        ...
        g.blocks.insert(obj, block);
        g.blocks.insert(st, block);
        g.blocks.insert(ld, other);
        if late_store {
            let w = g.add_node(Op::Const(7), vec![]);
            let st2 = g.add_node(Op::Store(0), vec![obj, w]);
            g.blocks.insert(st2, other);
        }
        (g, obj, v, ld)
    }
    ...
    fn analyse(
        ...
        let mut cg = build_connection_graph(g);
        propagate_escape_states(&mut cg, g);
        find_scalar_replacements_with(&cg, g, cross)
    }

    #[test]
    fn a_load_outside_the_block_holding_every_store_is_answered_under_the_opt_in() {
        let (g, obj, v, ld) = build(false);
        assert!(
            !program_order_proves_dominance(&g),
            "precondition: branchy graph"
        );

        // Default: refused, exactly as before.
        let (found, refusals) = analyse(&g, false);
        assert!(found.is_empty());
        assert_eq!(refusals, vec![(obj, ScalarRefusal::LoadNotAnswerable)]);

        // Opt-in: the block's last store answers it.
        let (found, _) = analyse(&g, true);
        let info = found
            .iter()
            .find(|i| i.alloc_node == obj)
            .expect("every store is in the allocation block");
        assert!(info.dominance_proved);
        assert_eq!(info.load_value(ld), LoadResolution::Value(v));
    }

    /// A store OUTSIDE the allocation block breaks the premise: the load's
    /// answer then depends on the path, which this graph cannot see.
    #[test]
    fn a_store_outside_the_allocation_block_still_refuses() {
        let (g, obj, _, _) = build(true);
        let (found, refusals) = analyse(&g, true);
        assert!(found.is_empty());
        assert_eq!(refusals, vec![(obj, ScalarRefusal::LoadNotAnswerable)]);
    }

    /// A load with no recorded block is never answered by the cross-block
    /// rule — a missing fact is not agreement.
    #[test]
    fn a_load_with_no_block_is_not_answered() {
        let (mut g, obj, _, ld) = build(false);
        g.blocks.remove(&ld);
        let (found, refusals) = analyse(&g, true);
        assert!(found.is_empty());
        assert_eq!(refusals, vec![(obj, ScalarRefusal::LoadNotAnswerable)]);
    ...
    /// A field with no store at all reads the zero default from any block.
    #[test]
    fn a_never_stored_field_is_the_zero_default_across_blocks() {
        let stores: Vec<FieldStore> = Vec::new();
        assert_eq!(
            resolve_cross_block_load(&stores, None),
            LoadResolution::ZeroDefault
        );
        let stores = vec![
            FieldStore { store: 5, value: 3 },
            FieldStore { store: 8, value: 4 },
        ];
        assert_eq!(
            resolve_cross_block_load(&stores, None),
            LoadResolution::Value(4)
        );
        // The memory chain places the load after store 5 only: disagreement
        // refuses rather than forwarding the later store.
        assert_eq!(
            resolve_cross_block_load(&stores, Some(6)),
            LoadResolution::Unknown
        );
        assert_eq!(
            resolve_cross_block_load(&stores, Some(9)),
            LoadResolution::Value(4)
        );
    }
}
