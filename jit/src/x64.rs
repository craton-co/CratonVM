...
//!
//! Tests inside the test-gated `mod tests { ... }` are exempt — assertion
...
mod osr;
// Round 10: `osr_empty_stack_refusals` is a COMPILE-time census that had no
// reader, and could not get one — `mod osr` is private and `jit/src/lib.rs` is
// this module's ANCESTOR, so a crate-root re-export does not reach into it
// either. The lane that wanted to read it wrote the reader, found it would not
// compile, removed it and filed the one line that makes it possible rather than
// reaching for a `cfg`-gated call that would have retired the orphan entry by
// exploiting a documented limit of the gate. This is that line.
pub use osr::osr_empty_stack_refusals;
mod simd;
...
struct Compiler {
    ...
    /// callee's arguments removed, which is the shape
    /// `ResumeSemantics::for_caller_scope()` (`RESUME`) describes and what the
    /// VM's `caller_resume_pc` requires.
    ///
    /// The shape, not the recorded flag: an owned `FrameState` has no semantics
    /// field, so nothing in the published chain SAYS "parked mid-invoke" — only
    /// the interned form carries per-scope semantics (`FrameStateInterner::intern`
    /// stamps `for_caller_scope()` on every scope reached through `caller`), and
    /// no production path interns. A consumer of this backend's chain therefore
    /// has to know the convention rather than read it, which is one more reason
    /// the population is still deliberately empty.
    ///
    ...
    /// The method whose BYTECODE the emitter is currently walking, innermost
    /// last — pushed and popped in lockstep with [`Self::inline_scope_stack`].
    ///
    /// `NOTES-deopt2.md` M1 / `NOTES-deopt.md` N5, the producer half.
    /// `build_frame_state_at` stamped `self.method_key` unconditionally, and
    /// that field is assigned once per compile (`x64/driver.rs`) and never
    /// swapped for a callee. So a deopt point published from inside a splice
    /// named the CALLER's method at a bci belonging to the CALLEE — and the
    /// VM's identity gate (`deopt_resume::deopt_frame_matches_method`) ACCEPTS
    /// that pair, because the key does name the method being resumed. It would
    /// then run the caller's bytecode at a foreign bci.
    ///
    /// THREE things ride on this stack, and they are the same question asked
    /// three times: WHOSE bytecode does an emitter pc index?
    ///
    ///  * the `FrameState::method_key` / `DeoptimizationPoint` identity
    ///    ([`InlineCalleeScope::key`]);
    ///  * whether the pc goes through `orig_bci`. That map is the ENCLOSING
    ///    method's loop-rewrite table; a callee pc has no entry in it and must
    ///    be published verbatim, in the callee's own bci space;
    ///  * **the FRAME GEOMETRY the published `FrameState` describes** — the
    ///    number of local slots, and which operand-stack entries belong to the
    ///    method being named. This is the axis round 10 wave 7 added, and it
    ///    used to be answered by the ROOT method's `num_locals` and the whole
    ///    shared `stack` whatever identity was stamped
    ///    (`r10-deoptverify-splice-published-point-would-carry-the-callers-locals-FIXED-20260922.md`).
    ///
    /// It was two parallel `Vec`s (a `Vec<String>` of identities beside a
    /// geometry the producer did not have) for exactly as long as it took to
    /// notice that a third parallel stack is a third thing that can be pushed
    /// without its partner. One stack of one struct cannot drift: the identity
    /// a point is stamped with and the geometry its frame is built to are the
    /// same entry.
    ///
    /// Empty on every compile that splices nothing, in which case all three
    /// answers are what they always were and the published metadata is
    /// byte-identical.
    pub(super) inline_callee_scopes: Vec<InlineCalleeScope>,
    ...
    /// its own raw self-recursive `CALL` sites, or `0` when this compile has
    /// none (no self-call row, or a caller with no method identity).
    ///
    /// Staged by the planner on its own channel because a row in `invoke_info`
    /// would route the site to the dispatch helper instead of the raw `CALL`.
    /// See `BackendRequest::self_call_invoke_info` and
    /// `crate::stage_self_call_invoke_info`.
    pub(super) self_call_invoke_info: usize,
    ...
    /// An entry whose branch is an operand-home compare
    /// (`arrays.rs` `emit_bounds_check_in`) is rewritten by
    /// `emit_bounds_check_home_prologues` just before that, to name the `jmp`
    /// of its cold `mov rax, <array> ; mov rcx, <index>` prologue -- the pads
    /// always read RAX/RCX. Recognised from the code bytes, so the entry needs
    /// no extra field and every rollback truncation / unroller shift of this
    /// vector covers it unchanged (round 9 wave 9, arr9).
    ...
    ///
    /// Each entry is `(action, patch_offset, trap_key, bci)`: `action` is the JEP-358
    ...
    /// [`Self::emit_null_check_store_stubs`] groups entries by `(action, bci)` and
    /// emits one cold stub per pair. It stamps the bci before calling
    /// `jit_npe_with_action(code)`
    ...
    /// own cold trampoline, which stamps the bci and then packs
    /// `action | key << 8` into the helper's single argument so the NPE
    /// snapshot can put a LINE on the frame that raised. `0` keeps the
    /// historical shape exactly: the `JZ` goes straight to the shared
    /// `(action, bci)` stub.
    null_check_store_stubs: Vec<(u8, usize, u32, usize)>,
    ...
    /// Rotated loops whose relocatable pre-header (speculative-BCE entry
    /// guards, `aaload` and integer-arithmetic hoists) is emitted at the loop's
    /// entry `goto` instead of at the header: `entry goto pc -> header pc`.
    /// Empty unless `CRATONVM_JIT_ROTATED_PREHEADER` is on; see
    /// `escape_analysis::find_rotation_preheader_entries`.
    rotation_preheader_by_goto: FxHashMap<usize, usize>,
    ...
    /// Site -> GC-maintained constant slot (`BackendRequest::ldc_slots`). Read
    /// by `objects.rs::ldc_site_slot`; empty on every path that has none.
    ldc_slots: std::collections::HashMap<(u32, u16), usize>,
    ...
    /// Phase B: WOULD be set when a `monitorenter`/`monitorexit` was ELIDED over
    /// a NON-scalar object (the pre-existing blanket-elision case) — an elided
    /// lock over a non-scalar object leaves no recordable trace, so such a
    /// method must stay off the deopt-resume path. Phase C: an elision over a
    /// SCALAR object does not set this; it is recorded in `sr_monitor_at` and
    /// relocked on resume. (`ACC_SYNCHRONIZED` is separately caught by the
    /// consumer's `is_synchronized` bail.)
    ///
    /// NOTHING ASSIGNS `true` TODAY. Round 10 checked every write: `false` once,
    /// `true` never, so the `!has_elided_monitor` conjunct in the resume gate
    /// constant-folds. That is currently CORRECT — the blanket-elision path this
    /// field was meant to report does not fire — and the field is kept on
    /// purpose, because the hazard is real and its failure mode is a silent hang.
    /// Read
    /// `docs/internal/fixed-bugs/r10-earelock-has-elided-monitor-is-never-set-FIXED-20260922.md`
    /// before treating a `false` here as evidence that no monitor was elided.
    ...
    /// codegen monitor handler consults this to decide whether an elided lock is
    /// relockable on resume. Membership means relockable; absence does NOT mean
    /// the other way is taken, because there is no other way: nothing ever sets
    /// `has_elided_monitor`, so a monitor op outside this set is simply not
    /// elided at all.
    ...
    /// How many 64-local WINDOWS `local_oop_masks` holds, and the stride
    /// between them.
    ///
    /// `local_oop_masks[w * local_oop_stride + pc]` describes JVM locals
    /// `w*64 .. w*64+64` at `pc`. One window (`stride == code_len`, the only
    /// case below 64 locals) is laid out exactly as the single-window vector
    /// always was, so every reader that indexes by `pc` alone still gets
    /// window 0.
    ///
    /// Zero windows means the analysis REFUSED this method, which
    /// `map_incomplete_cause::LOCAL_MASK_UNSUPPORTED` then fails closed on.
    local_oop_windows: usize,
    /// The stride between windows in `local_oop_masks` (the method's code
    /// length). Kept beside the vector because `code_len` is not otherwise
    /// available where the masks are read.
    local_oop_stride: usize,
    ...
    /// This compilation's per-method entry counter, allocated lazily by
    /// `emit_prologue` when `CRATONVM_JIT_ENTRY_COUNTER` is armed and `None`
    /// otherwise (which is the default, and which is why this costs an
    /// `Option` discriminant and nothing else on the normal path).
    ///
    /// Allocated DURING codegen for the same reason `compile_id` is reserved
    /// before it: the prologue bakes the counter's address as an `imm64` while
    /// the `CompiledMethod` that will own the box does not exist yet. The
    /// driver moves it onto `CompiledMethod::_jit_entry_counter` at finalize —
    /// the same "allocate a `Box`, bake its address, hand the box to the
    /// artifact" pattern `_jit_pic_slots` and `_jit_local_handler_sites` use,
    /// and for the same reason: the machine code that reads the address must
    /// not outlive the allocation. A compile that bails after the prologue
    /// drops the `Compiler` and the box with it.
    entry_counter: Option<Box<crate::entry_counter::JitEntryCounter>>,
    ...
    // T5.2.1 — `induction_vars: Vec<crate::scev::InductionVar>` was here, and
    // was removed on 2026-09-17 (`NOTES-opts.md` §4 A3) together with the
    // `driver.rs` call that filled it and the `induction_var_for` accessor
    // that was its only reader. Its doc claimed it was "consumed by downstream
    // passes … unroll factor decisions, SIMD remainder handling, and
    // range-check elimination"; none of the three ever read it, and the
    // accessor carried `#[allow(dead_code)]`, which is what kept the claim
    // unfalsifiable. See the removal note in `x64/driver.rs`.
    ...
    /// `getfield` / `putfield` pcs whose field is `volatile`
    /// (`BackendRequest::volatile_field_pcs`, lifted into this walk's pc
    /// space by the driver). A `putfield` pc here is followed by `MFENCE`.
    volatile_field_pcs: FxHashSet<usize>,
    ...
    ///
    /// The single store: its elements' addresses are baked by the frame-deopt
    /// stubs, so it must not grow after `emit_deopt_stubs` (checked at finalize
    /// by `baked_point_table_is`).
    deopt_points: Vec<crate::deopt::DeoptimizationPoint>,
    /// Identity keys, one per `deopt_points` entry and in lockstep with it: the
    /// `*_box_ptr_by_bci` maps store these addresses and `inlining.rs`'s
    /// rollback purges by them. NOT a copy of the point (empty frame state since
    /// r9 wave 6); `emit_deopt_stubs` resolves a key to the `deopt_points`
    /// element and bakes THAT, and the boxes are dropped with the compiler.
    deopt_boxes: Vec<Box<crate::deopt::DeoptimizationPoint>>,
    /// `(address, len)` of the `deopt_points` buffer whose elements
    /// `emit_deopt_stubs` baked into code (round 9 wave 6, deopt6). `driver.rs`
    /// refuses the artifact at finalize unless the published list is that
    /// buffer at that length (`deopt_stubs::baked_point_table_is`).
    baked_point_table: Option<(usize, usize)>,
    ...
    /// The emitter pc each entry of `deopt_points` was recorded at, in the
    /// same order.
    ///
    /// **Read POSITIONALLY, by index, against `deopt_points` and
    /// `deopt_boxes`.** `build_and_record_deopt_point` pushes all three
    /// together and `loop_rewrite::verify_deopt_point_coordinates` zips the
    /// first two — its own doc says a producer that forgets this one "would
    /// silently misalign every check below". Anything that truncates one
    /// therefore has to truncate all three: the splice rollbacks in
    /// `inlining.rs` truncated only `deopt_points`, so a rolled-back splice
    /// that had published left the other two one entry long and every later
    /// point in the method was verified against the wrong recording pc. Latent
    /// only because the splice postcondition refuses every splice that
    /// publishes; M1 is what makes publishing possible, so the truncations had
    /// to be correct before the first artifact could be wrong rather than
    /// after.
    ...
    deopt_regs_base: i32,
    /// deopt-osr Step 2: `(bci, reason)` → stable pointer to the boxed
    /// `DeoptimizationPoint` for that guard, baked as arg0 (imm64) by the
    /// frame-deopt stub. Populated by `emit_deopt_snapshot_at_guard` (always
    /// `DeoptReason::BoundsCheck`) and by `snapshot_pre_intrinsic_call` (16 call
    /// sites pass `ReceiverTypeChanged`, 2 pass `BoundsCheck`).
    ///
    /// **The `reason` component is not decoration.** It was added in round 10
    /// wave 8's follow-up, after the map was found keyed on the bare bci with
    /// three producers filing two distinct reasons into it and an idempotence
    /// test (`snapshot_pre_intrinsic_call`) that did not mention the reason. One
    /// pc reached by a reason-2 producer and a reason-6 producer — a loop header
    /// that is also a guarded invoke is the shape — kept only whichever point
    /// got there first, and `emit_deopt_stubs`' two lookups (`2 =>` and `6 =>`)
    /// both read it, so the survivor was handed to the other reason's stub.
    ///
    /// The frame CONTENTS are the same either way (both producers describe the
    /// same bci from the same regalloc provenance), so nothing resumed on wrong
    /// state; what was lost is the point's `reason`, which
    /// `osr_exit::deopt_reason_at_bci` recovers BY BCI. With one point
    /// suppressed that function answered `Unique(the survivor)` where the honest
    /// answer is `Ambiguous`, and `deopt::recommend_action` then applied the
    /// wrong de-speculation policy: a `ReceiverTypeChanged` guard reported as a
    /// `BoundsCheck` never takes the receiver-de-spec sparing path and keeps
    /// re-trapping until the raw per-method deopt count crosses its threshold.
    /// The same mis-labelling in this machinery, one map over, is what
    /// `x64/osr.rs`'s `emit_osr_exit_map_at_reason` doc records costing ~2000
    /// frame-deopt entries for one Groovy script evaluation.
    ///
    /// Two points at one bci is a SUPPORTED configuration, not a workaround:
    /// `speculation_id_for` derives its id from `(resume_bci, reason)` precisely
    /// so `DespecRegistry` (keyed `(method_key, bci, speculation_id)`) can
    /// suppress one guard at a bci without suppressing the other, and the
    /// reason-6/reason-9 pair already relies on it — those two only survived
    /// because they live in DIFFERENT maps. This pair gets the same separation
    /// inside one map instead of a second map, because unlike reason 9 the two
    /// agree on what the key means: both resume at their bci.
    ///
    /// The key still carries no OWNER component; that is a different property
    /// and it is enforced by `may_file_by_bci` — see
    /// `jit/tests/r10_bybci_publication_key_space.rs`.
    deopt_box_ptr_by_bci: rustc_hash::FxHashMap<
        (usize, crate::deopt::DeoptReason),
...
mod deopt_snapshot_tests {
    ...
    fn classify_local_kinds_types_a_reference_above_slot_63() {
        ...
        // THE PREMISE, REWRITTEN 2026-09-03 AS THIS TEST ASKED.
        //
        // It used to assert that nothing answers above 64 locals, and warned
        // that if anything ever started answering, "the deopt snapshot's
        // `is_oop` gains a second source and this test's premise needs
        // rewriting rather than deleting". `CRATONVM_JIT_WIDE_LOCAL_OOP_MAPS`
        // is that change, so here is the rewrite.
        //
        // Two entry points now, and the split is the invariant:
        //
        //   * the SPLICE path (`compute_local_oop_masks`) still answers nothing
        //     above 64, because `InlineOopScope::masks` is a single `u64` by
        //     construction and a spliced callee past the cliff must keep failing
        //     its safepoints closed through `mask_at_cur() == None`;
        //   * the METHOD path (`compute_local_oop_masks_windowed`) answers, one
        //     word per 64 slots.
        let (masks, reached) = crate::x64::licm::compute_local_oop_masks(&code, code.len(), 84, 0);
        assert!(
            masks.is_empty() && reached.is_empty(),
            "the SPLICE entry point must still answer nothing above 64 locals --              `InlineOopScope` cannot hold a second word"
        );
        let (wmasks, wreached, windows) =
            crate::x64::licm::compute_local_oop_masks_windowed(&code, code.len(), 84, 0);
        assert_eq!(windows, 2, "84 locals is two 64-slot windows");
        assert_eq!(wmasks.len(), windows * code.len());
        // `wide astore 74` at pc 0 makes local 74 an oop from pc 4 onward.
        // Local 74 is window 1 (base 64), bit 10.
        let w1_at_pc4 = wmasks[code.len() + 4];
        assert_ne!(
            w1_at_pc4 & (1u64 << 10),
            0,
            "local 74 must be a proven oop after its `wide astore` -- this is the              coverage a >64-local method used to have none of"
        );
        ...
        // The DEOPT COUPLING this test exists to hold onto. `is_oop` there reads
        // bits `< 64` only, so for a local above slot 63 `classify_local_kinds`
        // remains the sole authority -- exactly as asserted at the top. What
        // changed is that locals BELOW 64 in such a method now have a second,
        // strictly more precise source, which is the same arrangement every
        // method under the cliff already had.
        assert_eq!(
            kinds[74],
            LocalKind::Ref,
            "the classifier is still the only thing that types a local above slot 63              for the deopt snapshot"
        );
...
impl Compiler {
    ...
    fn new(
        ...
        // A slot offset names the LOW byte of its word at `[rbp - off]`, so the
        // block's slots are `reg_spill_base + reg_spill_size ..= that + 248`
        // and its deepest (`gpr[0]`) is `+ deopt_regs_size - 8`. Round 9 wave
        // 7b (wire7b): this used to omit the `- 8`, which put `gpr[0]` on the
        // first outgoing-argument word (`[RSP + 40]` when the total is already
        // 16-aligned: the Win64 6th-argument slot) and left the block's
        // shallowest slot unused.
        // `single-pass-deopt-register-image-sits-one-slot-too-deep-20260918.md`.
        let deopt_regs_base = if deopt_regs_size != 0 {
            reg_spill_base + reg_spill_size + deopt_regs_size - 8
        } else {
            0
        };
        ...
        Self {
            ...
            inline_callee_scopes: Vec::new(),
            protected_ranges,
            local_handler_table: Vec::new(),
            local_handler_class_id: 0,
            self_call_invoke_info: 0,
            ...
            rotation_preheader_by_goto: FxHashMap::default(),
            new_info: Vec::new(),
            ...
            ldc_slots: std::collections::HashMap::new(),
            ldc2w_info: Vec::new(),
            ...
            inline_cm_tls_disp,
            compile_id,
            // Allocated lazily by `emit_prologue`, and only when the counter
            // flag is armed — so a default-configured compile never touches the
            // allocator for this at all.
            entry_counter: None,
            ...
            volatile_field_pcs: FxHashSet::default(),
    ...
    // T5.2.1 — `induction_var_for` was here and was removed on 2026-09-17
    // (`NOTES-opts.md` §4 A3). It was the ONLY reader of `induction_vars`, it
    // carried `#[allow(dead_code)]`, and it had no caller in the workspace —
    // so the `#[allow]` was what stopped the compiler from reporting the whole
    // chain, from the driver's per-compile `scev::analyze_induction_variables`
    // call down to here, as the dead code it was. See the removal note in
    // `x64/driver.rs` for what to restore, and when.
...
/// One spliced callee, while the emitter is inside its body: the identity a
/// deopt point published there must be stamped with, and the geometry of the
/// frame that point describes.
///
/// Pushed by `deopt_stubs::push_inline_scope` and popped by `pop_inline_scope`,
/// in lockstep with [`Compiler::inline_scope_stack`] (which holds the CALLER's
/// frame for the same splice). See that field's doc for why the identity and
/// the geometry are one entry rather than two stacks.
///
/// Nothing in the emitter publishes a deopt point from inside a splice today,
/// so nothing reads [`Self::max_locals`] or [`Self::caller_stack_floor`] on a
/// production compile — the two `precise_exception_frames` publishers the
/// splice walk does contain (`emit_post_invoke_exception_check`,
/// `emit_post_alloc_oom_check`, and their array-lowering siblings) cannot run
/// inside one, because `precise_exception_frames` refuses every inline plan
/// (`InlineRefusal::PreciseExceptionFrames`), clears `inline_sites` in
/// `try_compile_inner`, and is refused a third time at
/// `try_emit_inline_site`. They are here so that the FIRST edit that does
/// publish describes the callee's frame rather than the root's.
#[derive(Clone, Debug)]
pub(crate) struct InlineCalleeScope {
    /// `"<class>.<method>:<descriptor>"` of the callee being spliced — the key
    /// the deopt log, `method_epochs` and `ReconstructedFrame::method_key` all
    /// use, and the one `x64/driver.rs` registers this callee's
    /// `MethodFrameLimits` under.
    pub(crate) key: String,
    /// The callee's own `max_locals`, i.e. how many local slots an interpreter
    /// frame for [`Self::key`] has. It is `InlineSite::callee_max_locals`, which
    /// is also the bound `driver.rs` hands the verifier's scope lane for this
    /// key, so a frame built to this count cannot report `LocalCountMismatch`.
    ///
    /// NOT `callee_locals_size` (`max_locals.max(param_slot_span)`, what
    /// `try_emit_inline_body` RESERVES): that is a frame-sizing figure, and
    /// publishing it would exceed the verifier's bound whenever the two differ.
    pub(crate) max_locals: usize,
    /// Depth of the shared abstract operand stack that belongs to the CALLER,
    /// with this call's arguments already removed — so entries at or above this
    /// index are the callee's own operands.
    ///
    /// Frozen at the splice rather than recomputed: `push_inline_scope` knows
    /// the argument count (`InlineSite::callee_num_args`, exactly how many
    /// `pop_stack` calls `try_emit_inline_body` makes), and the splice walks on
    /// the same `self.stack` from there, pushing above this floor and never
    /// below it.
    pub(crate) caller_stack_floor: usize,
}
