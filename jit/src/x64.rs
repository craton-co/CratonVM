...
struct Compiler {
    ...
    ///
    /// Two things ride on this stack, and they are the same question asked
    /// twice: WHOSE bytecode does an emitter pc index?
    ///
    ///  * the `FrameState::method_key` / `DeoptimizationPoint` identity, and
    ...
    ///
    /// Empty on every compile that splices nothing, in which case both answers
    /// are what they always were and the published metadata is byte-identical.
    ///
    /// Entries are the `"<class>.<method>:<descriptor>"` key the deopt log,
    /// `method_epochs` and `ReconstructedFrame::method_key` all use.
    pub(super) inline_callee_identity: Vec<String>,
...
impl Compiler {
    ...
    fn new(
        ...
        Self {
            ...
            inline_callee_identity: Vec::new(),
            protected_ranges,
