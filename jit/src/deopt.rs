...
/// published a point, so no finished artifact carries one yet.) The interned
/// counterparts are [`FrameStateInterner::is_resumable`] (deliberately
/// scope-local — it answers "is *this* scope clean") and
/// [`FrameStateInterner::chain_is_resumable`], which is the handle-side
...
/// but `PendingException` — and that one is routed to [`LAST_EXCEPTIONAL`]
/// before this is reached. A producer that later stashes a resume-AFTER point
/// (a caller parked mid-invoke, a point after a returning call) would make
/// that argument false silently; this makes it loud and safe instead.
///
...
mod stash_nesting_tests {
    ...
    /// an invoke) correct.
    ///
    /// A resume-after point (a caller parked mid-invoke) and a rethrow point
    /// are both turned into the identity-less re-run sentinel every sink
    /// refuses, and counted. Through the real trampoline entry, so a later
    /// producer cannot route around the check.
    #[test]
    fn only_a_reexecute_point_enters_the_ordinary_stash() {
        on_a_fresh_thread(|| {
            ...
            for semantics in [ResumeSemantics::RESUME, ResumeSemantics::RETHROW] {
                let before = deopt_stash_non_reexecute_refused();
                let point = point_with(semantics);
                assert_eq!(ir_deopt_entry(&point, 0, std::ptr::null()), i64::MIN);
                let f = take_last_deopt().expect("the sentinel is stashed, not nothing");
                assert!(
                    f.method_key.is_empty() && f.bci == u32::MAX,
                    "{semantics} must stash the re-run sentinel, got {} bci={}",
                    f.method_key,
                    f.bci
                );
...
pub extern "C" fn ir_deopt_entry(
    ...
    let frame = ordinary_stash_frame(
        point,
        reconstruct_frame_from_machine_state(point, regs, rbp),
    );
...
fn hash_frame_value<H: Hasher>(v: &FrameValue, state: &mut H) {
    ...
    match v {
        ...
        FrameValue::VirtualObject(vo) => {
            // Every field the derived `PartialEq` compares — including
            // `array_element_type`, which this used to skip, so an object
            // recipe and an array recipe with the same id/class/length always
            // landed in one bucket and were told apart only by the equality.
            vo.array_element_type.hash(state);
            vo.id.hash(state);
