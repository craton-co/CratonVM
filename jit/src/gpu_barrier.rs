...
///
/// # A late arming is safe, and here is the proof (RT-7, closed 2026-09-17)
///
/// "Before any method can be compiled" is true of the VM that calls this and
/// not of the process. An embedding that creates VM #1 without `--gpu` --
/// compiling array writers with no 51-byte barrier -- and only then VM #2 with
/// `--gpu` reaches this with code already committed. Until 2026-09-17 that
/// arming was REFUSED, so VM #2 fell back to `KeepCache` and left its
/// array-writing methods interpreted.
///
/// The refusal guarded one question, which `NOTES-abi8.md` left unsigned: can
/// an array written by VM #1's barrier-less stores be an entry in the residency
/// cache VM #2 relies on? If it could, VM #2 would read stale device data. It
/// cannot, and the chain is:
///
/// 1. **The cache's TABLE is per-VM.** `offload::input_cache`'s entries live in
///    a map keyed by `SharedVm::vm_identity` first and `ObjectRef` second. The
///    earlier write-up called the table "process-wide and keyed by ADDRESS";
///    only the filter word and dirty buckets are, and those can only
///    OVER-evict or cost a false-positive lookup, never under-evict.
/// 2. **Every VM identity owns its own heap.** `SharedVm::new` takes only a
///    config and builds a fresh `VmHeap::new_with_overrides(..)`; no
///    constructor accepts an existing heap. Live heaps are disjoint
///    allocations, and a VM's Java code can hold references only into its own.
/// 3. **Identities are never reused.** `vm_identity` is a monotonic
///    `fetch_add` from 1, so a dead VM's table -- even one `clear_all` never
///    removed -- cannot be adopted by a later VM.
/// 4. **A VM with a cache arms before its first compile.** This is called at
///    the top of `SharedVm::new`. So at the moment of a FIRST arming with code
///    already committed, every committed store belongs to a VM that had not
///    armed -- which is to say, a VM with no residency cache at all.
///
/// So the barrier-less stores write only heaps that no cache table references,
/// and publishing the pair cannot make any entry stale. The refusal is gone and
/// every `--gpu` VM now arms, which gives a late-created one its compiled
/// array-writing path back.
///
/// `NOTES-abi7.md` expected this to become a `debug_assert`. It does not,
/// because there is nothing left to assert here: the invariant that matters
/// (point 4) is established by where this is CALLED, and the one component that
/// could violate it -- a constructor sharing a heap between identities -- would
/// have to be added to `vm_init.rs`, which is where that argument now lives.
///
/// `CRATONVM_JIT_GPU_ARRAY_BARRIER=0` remains the lever that restores
/// `KeepCache` for every VM, should this reasoning ever turn out to have a
/// hole.
///
/// Returns whether the barrier is armed after the call: `true` for a complete
/// pair, `false` for a disarm (`arm(0, 0)`) or a half-published pair, which
/// also disarms. `vm_init` records it as `SharedVm::gpu_array_barrier_armed`
/// and only ever passes the real pair.
pub fn arm(filter_addr: usize, dirty_addr: usize) -> bool {
    FILTER_ADDR.store(filter_addr, Ordering::Release);
    DIRTY_ADDR.store(dirty_addr, Ordering::Release);
    pair_is_armed(filter_addr, dirty_addr)
}
