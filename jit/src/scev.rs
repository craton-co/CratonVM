...
//! T5.2.1 — Scalar Evolution (SCEV) for counted bytecode loops.
//!
//! The types here ([`CountedLoop`], [`IntRange`], [`AffineIv`],
//! [`BoundSource`], …) describe a counted loop — an induction variable
//! advanced by a constant (or guarded) stride once per iteration, an exit
//! comparison [`ExitCmp`] and a bound — and prove things about it: index
//! spans, bounds-check elision and trip counts. The recognition that
//! produces a `CountedLoop` from bytecode is
//! `loop_analysis::analyze_counted_loop_at`.
//!
//! ## Limitations
//!
//! This is a bytecode-level analysis, not an SSA-level one. Complex
//! induction variables (e.g. `i = i * 2`) are not detected — those require
//! a full SSA-based SCEV like LLVM's.
//!
//! ## History (round 9 wave 3)
//!
//! This module used to carry a second, older recognition as well:
//! `InductionVar` / `analyze_induction_variables`, an `iinc` scan with its
//! own exit-comparison matcher. Its only caller was the uncalled
//! `ir_optimize::licm_scev_corroborates`, and it answered confidently on
//! shapes the `CountedLoop` path had learned to refuse. Both were deleted;
//! see `scev-legacy-induction-variable-path-unverified-and-uncalled-20260918.md`.
...
impl ExitCmp {
    ...
    /// The exit-when-true comparison of the complement of `self`.
    ///
    /// A branch that is taken to CONTINUE the loop (the rotated shape, where
    /// the back edge is the comparison and the fall-through leaves) exits
    /// when its own test is false, so its exit polarity is the negation of
    /// what [`ExitCmp::from_opcode`] decodes. `if_icmplt body` continues while
    /// `iv < bound`, i.e. exits when `iv >= bound` — `Lt.negate() == Ge`.
    pub fn negate(self) -> ExitCmp {
        match self {
...
impl IntRange {
    ...
    /// is `>= 0`.
    ///
    /// The same predicate `range_analysis::Range` offers under the same name,
    /// and the only one either type offers. Both used to carry a vacuous twin,
    /// `is_non_negative`, that answered `true` at `Empty` — logically right
    /// (there are no values to violate the claim) and a trap as an API, because
    /// what a caller does with "the index is non-negative" is delete a bounds
    /// check, and here that permission would rest on the analysis being right
    /// that the index cannot occur at all.
    ///
    /// `prove_index_in_bounds_of_array` was the only production caller and was
    /// already safe: it returns `BoundsProof::Static` on an empty span before
    /// reaching the test. It is safe by CONSTRUCTION now — the predicate no
    /// longer has a vacuous answer to give — and a caller that adds a new path
    /// to it does not have to rediscover the argument.
...
pub struct TripCount {
    /// Fewest executions.
    pub min: u64,
    /// Most executions. `u64::MAX` when the induction variable can wrap past
    /// the `int` range with the exit test still passing — i.e. no bound.
    pub max: u64,
}
...
impl CountedLoop {
    ...
    pub fn prove_index_in_bounds_of_array(
        ...
        //
        // `definitely_non_negative` answers `false` at `Empty`, which sends an
        // empty span into the witness walk rather than out through the proof.
        // Unreachable — the `span.numeric.is_empty()` test above has already
        // returned `BoundsProof::Static` — and it is the right direction if it
        // ever becomes reachable: the walk refuses or asks for a guard, where
        // the vacuous reading would have handed out the proof.
        if !span.numeric.definitely_non_negative() {
    ...
    ///
    /// ## `max` under a possible wrap
    ///
    /// `max` counts the executions of `init, init±|stride|, …` that stay on
    /// the near side of the limit — which is the whole story only while the
    /// IV cannot step past the `int` range. `for (i = 0; i <= n; i++)` with
    /// `n == Integer.MAX_VALUE` never terminates (`i++` wraps to `MIN_VALUE`,
    /// which is `<= n`), yet the interval arithmetic used to report it as
    /// exactly `2^31` trips, and [`TripCount::exact`] is what
    /// `x64/simd_analysis.rs` sizes its no-tail decision on. When the extreme
    /// executed value plus the stride can leave the `int` range, `max` is now
    /// `u64::MAX` ("unbounded"). `min` survives a wrap — a wrapping IV makes
    /// the loop run longer, never shorter — and is unchanged.
    ///
    /// A limit the body can rewrite makes both numbers fiction, so that is
    /// `None` here exactly as it is a refusal in the other proofs.
    pub fn trip_count(&self, env: &RangeEnv) -> Option<TripCount> {
        ...
        // The step after the extreme executed value must stay in `int`, or the
        // IV wraps with the exit test still passing and `max` bounds nothing.
        let may_wrap = if increasing {
            gov_hi + mag > i32::MAX as i64
        } else {
            gov_lo - mag < i32::MIN as i64
        };
        let max = if may_wrap && max > 0 { u64::MAX } else { max };
...
mod range_tests {
    ...
    fn lattice_arithmetic_refuses_to_pretend_wrapping_did_not_happen() {
        ...
        assert!(IntRange::array_length().definitely_non_negative());
    }

    /// `IntRange::Empty` is not non-negative, for the same reason
    /// `range_analysis::Range`'s bottom is not: the only thing a caller does
    /// with the answer is drop a bounds check, and at `Empty` there is no index
    /// that was checked. The vacuous twin this replaces answered `true`.
    #[test]
    fn an_empty_int_range_is_not_definitely_non_negative() {
        assert!(!IntRange::Empty.definitely_non_negative());
        assert!(IntRange::new(0, 0).definitely_non_negative());
        assert!(!IntRange::new(-1, 9).definitely_non_negative());
        assert!(!IntRange::unknown().definitely_non_negative());
    }
    ...
    fn min_and_max_bounds_fold_through_the_lattice() {
        ...
        assert!(span.span.numeric.definitely_non_negative());
    ...
    #[test]
    fn a_loop_that_can_wrap_has_no_finite_maximum_trip_count() {
        let env = RangeEnv::new();
        // MUST REFUSE a finite max: `for (i = 0; i <= MAX; i++)` never ends.
        let forever = mk(
            IntRange::constant(0),
            Stride::Const(1),
            ExitCmp::Gt,
            BoundSource::Const(i32::MAX),
        );
        let t = forever.trip_count(&env).expect("min survives a wrap");
        assert_eq!(t.max, u64::MAX);
        assert_eq!(t.exact(), None);
        assert_eq!(t.min, i32::MAX as u64 + 1);
        // MUST ACCEPT twin: one below the edge and the count is exact again.
        let ends = mk(
            IntRange::constant(0),
            Stride::Const(1),
            ExitCmp::Gt,
            BoundSource::Const(i32::MAX - 1),
        );
        assert_eq!(
            ends.trip_count(&env).and_then(|t| t.exact()),
            Some(i32::MAX as u64)
        );
        // Descending, the same edge at MIN.
        let down = mk(
            IntRange::constant(0),
            Stride::Const(-2),
            ExitCmp::Lt,
            BoundSource::Const(i32::MIN + 1),
        );
        assert_eq!(down.trip_count(&env).map(|t| t.max), Some(u64::MAX));
    }

    #[test]
    fn a_limit_the_body_rewrites_has_no_trip_count() {
        let mut l = mk(
            IntRange::constant(0),
            Stride::Const(1),
            ExitCmp::Ge,
        ...
        assert!(l.trip_count(&RangeEnv::new()).is_some());
        l.modified_locals |= 1u64 << N;
        assert_eq!(l.trip_count(&RangeEnv::new()), None);
    }
