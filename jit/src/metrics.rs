...
//!   exactly one path: `ir_lower::lower_inner_with_array_list` with
...
//!
//!   The allocation's own figures — what it PLANNED, before
//!   `plan_register_residency` decided how much of the plan it could prove —
//!   have their own pair, [`CompilationReport::planned_spills`] /
//!   [`CompilationReport::planned_reloads`], written through
//!   [`CompileRecorder::set_planned_spills`]. Two fields rather than two
//!   writers of one field: until the whole plan is emitted the numbers
//!   disagree, and a report that cannot say which of them it is carrying is
//!   not a measurement. The gap between the pairs IS the residency wiring's
//!   coverage.
//!
//!   **The planned pair has no production writer**, so it reads `NotMeasured`
//!   on every compile. `regalloc::record_allocation_metrics` was the intended
//!   one and was deleted; the record is in `regalloc.rs`'s metrics section.
//!   It wrote the EMITTED pair with PLANNED numbers — the collision these four
//!   fields exist to prevent — and it had no caller in the workspace. The
//!   producer the pair still wants is `ir_lower::plan_register_residency`,
//!   which holds the `Allocation` and, being below `lower_inner_with_array_list`
//!   rather than beside it, holds no [`CompileRecorder`]. See
//!   [`CompileRecorder::set_planned_spills`] for why that is not by itself an
//!   argument for a third `note_current_*` form.
//!
...
// ── The `-XX:+PrintCompilation` analogue ─────────────────────────────
//
// This VM had IR-graph dumping (`CRATONVM_DBG_IR_GRAPH`), x86 disassembly
// (`CRATONVM_DBG_JIT_DISASM`) and the structured per-compile reports in this
// module (`CRATONVM_JIT_METRICS`), and NO one-line-per-compilation stream —
// the single most-asked-for JIT diagnostic, and the one that answers "what is
// it compiling, in what order, and what is it refusing" without a parser.
//
// Deliberately INDEPENDENT of `enabled()`. The metrics ring allocates a
// `CompilationReport` per compile and retains a window of them; this prints a
// line and keeps nothing. An operator who wants the stream should not have to
// turn on the ring to get it, and an operator who wants the ring should not be
// spammed on stderr to keep it.

/// Whether the one-line-per-compilation stream is on. Latched on first read.
///
/// Accepts `CRATONVM_JIT_PRINT_COMPILATION` as a boolean, and also the
/// `CRATONVM_JIT=print-compilation` spelling, for operators who already keep
/// one comma-separated knob for JIT options.
pub fn print_compilation_enabled() -> bool {
    static ON: OnceLock<bool> = OnceLock::new();
    *ON.get_or_init(|| {
        if let Some(on) = env_flag("CRATONVM_JIT_PRINT_COMPILATION") {
            return on;
        }
        cratonvm_types::flags::runtime_var_os("CRATONVM_JIT")
            .and_then(|v| v.to_str().map(|s| s.to_ascii_lowercase()))
            .is_some_and(|s| s.split(',').any(|p| p.trim() == "print-compilation"))
    })
}

/// The origin of the millisecond column: the first compilation this process
/// recorded.
///
/// Process start would be better and is not reachable from here without a
/// second global. A fixed origin is all the column is for — consecutive lines
/// have to be comparable with each other, not with the clock.
fn print_epoch() -> Instant {
    static EPOCH: OnceLock<Instant> = OnceLock::new();
    *EPOCH.get_or_init(Instant::now)
}

/// Monotonic per-line id, so two lines whose millisecond columns agree are
/// still orderable.
static PRINT_SEQ: AtomicU64 = AtomicU64::new(0);

/// What one printed compilation needs, held only when the stream is on.
///
/// `Cell`/`RefCell` rather than atomics: a [`CompileRecorder`] holds an `Rc`
/// and so is `!Send`, and never leaves the compiling thread.
struct PrintCompilation {
    id: u64,
    /// `class.method(descriptor)`, built once at `begin` because the three
    /// borrowed strings do not outlive it.
    name: String,
    optimizing: bool,
    /// The first bailout reason seen, for the line `Drop` prints when no body
    /// was installed.
    bailout: RefCell<Option<String>>,
    /// Set by `installed`, so `Drop` does not print a second line for a
    /// compilation that succeeded.
    printed: std::cell::Cell<bool>,
}

impl PrintCompilation {
    fn begin(class_name: &str, method_name: &str, descriptor: &str, optimizing: bool) -> Self {
        // Touch the epoch here, so the FIRST line's millisecond column reads 0
        // rather than however long the process took to reach its first compile.
        let _ = print_epoch();
        PrintCompilation {
            id: PRINT_SEQ.fetch_add(1, Ordering::Relaxed),
            name: format!("{class_name}.{method_name}{descriptor}"),
            optimizing,
            bailout: RefCell::new(None),
            printed: std::cell::Cell::new(false),
        }
    }

    /// `<ms> <id> <tier> <class>.<method><descriptor> <tail>`.
    ///
    /// `writeln!` on a locked handle rather than `eprintln!`: the macro panics
    /// when stderr is closed, and this runs inside the compiler, where
    /// `helper_guard.rs` makes the same choice for the same reason.
    fn emit(&self, tail: std::fmt::Arguments<'_>) {
        use std::io::Write as _;
        self.printed.set(true);
        let ms = print_epoch().elapsed().as_millis();
        let tier = if self.optimizing { "c2" } else { "c1" };
        let mut err = std::io::stderr().lock();
        let _ = writeln!(err, "{ms:>7} {:>6} {tier} {} {tail}", self.id, self.name);
    }
}
...
pub struct CompilationReport {
    ...
    /// `ir_lower::lower_inner_with_array_list` when `CRATONVM_JIT_IR_LINEAR_SCAN`
    /// is on. `NotMeasured` on every path that allocates no registers, which
    /// is the default build: absence of an allocator is not a spill count of
    /// zero.
    ///
    /// **This field carries the EMITTED number and only the emitted number.**
    /// It used to name a second supplier in `regalloc` (since deleted — see
    /// that file's metrics section), and that was the bug R5 (`NOTES-misc.md`
    /// §2) describes: the
    /// allocation's `alloc.spills` is what the allocator *planned*, and
    /// `ir_lower::plan_register_residency` executes only the subset of that
    /// plan it can prove, so the two are different numbers. Pointing both at
    /// one field would make a report mean "one of two things, and the reader
    /// cannot tell which". The planned figures have their own pair —
    /// [`planned_spills`](Self::planned_spills) /
    /// [`planned_reloads`](Self::planned_reloads) — so the gap between plan
    /// and emission is itself the measurement.
    ...
    /// Register → memory transitions the allocation **planned**.
    ///
    /// `Allocation::spills` — the full split/spill/remat schedule the
    /// allocator produced, before the backend decided how much of it it could
    /// prove. Written through [`CompileRecorder::set_planned_spills`], and by
    /// **nothing in production**: see the `NotMeasured` paragraph below.
    ///
    /// Deliberately a SECOND pair of fields rather than a second writer of
    /// [`spills`](Self::spills). Until `plan_register_residency` executes the
    /// whole plan (R4, `NOTES-regalloc.md` §1) the plan and the emission
    /// disagree, and only one of them describes machine code that exists. A
    /// reader comparing `planned_spills` against `spills` is measuring exactly
    /// that gap; a reader of a single field that either producer might have
    /// written is measuring nothing it can name.
    ///
    /// `NotMeasured` whenever no caller held a [`CompileRecorder`] at
    /// allocation time, which is **every path today**: this field pair and its
    /// setters landed (`NOTES-opts7.md` §2) but the producer did not, and the
    /// function that was to be it — `regalloc::record_allocation_metrics` — was
    /// deleted rather than repointed, because at the time it wrote
    /// [`spills`](Self::spills) with a planned number and had no caller either
    /// way. The record is in `regalloc.rs`'s metrics section.
    ///
    /// The open question is not which setter to call but *who calls it*.
    /// `ir_lower::plan_register_residency` is the only place that holds an
    /// `Allocation`, and it holds no recorder; `NOTES-opts7.md` §2 proposes a
    /// recorder-taking free function in `regalloc.rs` and `NOTES-regalloc7.md`
    /// §3 proposes ambient `note_*` twins instead. See
    /// [`CompileRecorder::set_planned_spills`], which argues against the second.
    pub planned_spills: Measured<u32>,
    /// Memory → register transitions the allocation **planned**. See
    /// [`planned_spills`](Self::planned_spills).
    pub planned_reloads: Measured<u32>,
    ...
    /// Of [`inlined_sites`](Self::inlined_sites), those bound by CLASS-HIERARCHY
    /// ANALYSIS rather than by a receiver profile
    /// (`InlineDecisionTally::hierarchy_bound_sites`).
    ///
    /// This is the only number that answers whether CHA is doing anything. A
    /// compile with no profile at all can show
    /// [`speculative_inlined_sites`](Self::speculative_inlined_sites) above zero
    /// ONLY through this counter, and a soak on which it stays flat has a CHA
    /// that is planning nothing, however many virtual sites the methods have.
    ///
    /// The tally has carried it since CHA landed; nothing harvested it, so the
    /// soak `NOTES-cha.md` asks for had no number to watch. That is what this
    /// field is for.
    pub hierarchy_bound_sites: Measured<u32>,
...
impl CompilationReport {
    ...
    pub fn new(class_name: &str, method_name: &str, descriptor: &str, optimizing: bool) -> Self {
        CompilationReport {
            ...
            planned_spills: Measured::NotMeasured,
            planned_reloads: Measured::NotMeasured,
            ...
            hierarchy_bound_sites: Measured::NotMeasured,
    ...
    pub fn to_json(&self) -> String {
        ...
        // Their own keys, beside the emitted pair rather than instead of it.
        // A sink that only ever knew `spills` keeps reading the emitted
        // number; one that wants to see how much of the allocation survived
        // lowering subtracts. Both render `null` when unmeasured, which is the
        // honest answer on a build with no recorder at allocation time.
        let _ = write!(
            s,
            ",\"planned_spills\":{},\"planned_reloads\":{}",
            self.planned_spills.json(),
            self.planned_reloads.json(),
        );
        let _ = write!(
            s,
        ...
        let _ = write!(
            s,
            ",\"inline_candidates\":{},\"inlined_sites\":{},\"speculative_inlined_sites\":{},\"hierarchy_bound_sites\":{}",
            self.inline_candidates.json(),
            self.inlined_sites.json(),
            self.speculative_inlined_sites.json(),
            self.hierarchy_bound_sites.json(),
        );
        let _ = write!(
            s,
...
/// on this thread.
///
/// The ambient form, for a caller whose signature is pinned and cannot take a
/// [`CompileRecorder`]. `ir_lower::lower_inner_with_array_list` is that caller: it
/// already reports [`note_current_peak_live_values`] the same way, so wiring
/// the allocator into it must not widen its parameter list. It passes what it
/// **emitted**.
///
/// The recorder-holding twin is [`CompileRecorder::set_spills`], and it means
/// the same thing — emitted transitions. What an *allocation* planned is a
/// different number and goes somewhere else entirely
/// ([`CompileRecorder::set_planned_spills`]): the residency wiring executes
/// only the subset of the plan it can prove, so the plan is the larger figure
/// and describes instructions that may not exist.
///
...
pub struct CompileRecorder {
    state: Option<Rc<RecorderState>>,
    /// The `-XX:+PrintCompilation` analogue's per-compilation state, present
    /// only when [`print_compilation_enabled`]. Independent of `state`: the
    /// one-line stream and the metrics ring are separately switched, and each
    /// has to work with the other off.
    print: Option<Box<PrintCompilation>>,
}
...
impl CompileRecorder {
    ...
    pub fn begin(
        ...
        // Built first and unconditionally, because the stream is switched
        // separately from the ring: a `CRATONVM_JIT_PRINT_COMPILATION` run
        // with metrics off must still get its lines.
        let print = print_compilation_enabled().then(|| {
            Box::new(PrintCompilation::begin(
                class_name,
                method_name,
                descriptor,
                optimizing,
        ...
        if !enabled() {
            return CompileRecorder { state: None, print };
        }
    ...
    ///
    /// Opts out of the printed stream too: an explicit "record nothing" must
    /// mean nothing, or a test that asks for silence gets stderr noise from a
    ...
    ///
    /// Answers for the metrics ring only. The printed stream is deliberately
    /// not visible here: every existing caller uses this to decide whether
    /// building a report input is worth the work, and the stream needs none of
    /// them.
    #[inline]
    ...
    /// Emitted spill count, recorded against *this* recorder, for a backend
    /// that holds one. A caller whose signature cannot carry a recorder uses
    /// [`note_current_spills`] instead. See [`CompilationReport::spills`].
    ///
    /// Note what this is NOT for: an allocation's `alloc.spills`. That is the
    /// plan, not the emission, and it goes to
    /// [`set_planned_spills`](Self::set_planned_spills).
    pub fn set_spills(&self, n: usize) {
        self.with_report(|r| r.spills = Measured::Value(n.min(u32::MAX as usize) as u32));
    }

    /// Emitted reload count. Same two forms as
    /// [`set_spills`](Self::set_spills); see [`CompilationReport::spills`].
    pub fn set_reloads(&self, n: usize) {
        self.with_report(|r| r.reloads = Measured::Value(n.min(u32::MAX as usize) as u32));
    }

    /// Planned spill count — `Allocation::spills`. See
    /// [`CompilationReport::planned_spills`] for why it is a separate field
    /// from the emitted one and not a second writer of it, and for why it has
    /// no production caller yet.
    ///
    /// There is no ambient (`note_current_*`) twin, and that was deliberate:
    /// the ambient forms exist for `ir_lower::lower_inner_with_array_list`, whose
    /// signature is pinned, and a producer of a *plan* holds the `Allocation`
    /// and so can be given a recorder parameter instead.
    ///
    /// **That argument has not survived contact with the only candidate
    /// producer.** `ir_lower::plan_register_residency` holds the `Allocation`,
    /// but it is called from `lower_inner_with_array_list` — the pinned-signature
    /// function — and takes `(graph, schedule, plan)`. Giving it a recorder
    /// means threading one down, or carrying the two numbers back up in
    /// `RegResidency`, or an ambient twin after all. Whoever wires it picks
    /// one; nothing here forecloses any of the three.
    pub fn set_planned_spills(&self, n: usize) {
        self.with_report(|r| r.planned_spills = Measured::Value(n.min(u32::MAX as usize) as u32));
    }

    /// Planned reload count — `Allocation::reloads`. See
    /// [`set_planned_spills`](Self::set_planned_spills).
    pub fn set_planned_reloads(&self, n: usize) {
        self.with_report(|r| r.planned_reloads = Measured::Value(n.min(u32::MAX as usize) as u32));
    }
    ...
    pub fn note_bailout_reason(&self, reason: BailoutReason, phase: &str) {
        // Rendered BEFORE `Bailout::new` takes ownership of `reason`.
        // `BailoutReason` is not `Copy`, so the other order borrows a moved
        // value; reordering keeps the single `reason` argument instead of
        // cloning one copy for the printer.
        if let Some(p) = self.print.as_ref() {
            // FIRST reason only. A bail cascades — the phase that refuses
            // reports, and so does each caller unwinding past it — and the
            // line wants the cause, not the last frame to notice.
            if let Ok(mut slot) = p.bailout.try_borrow_mut() {
                if slot.is_none() {
                    *slot = Some(format!("{reason} @{phase}"));
                }
            }
        }
    ...
    pub fn installed(&self, cm: &crate::CompiledMethod) {
        if let Some(p) = self.print.as_ref() {
            // `used_ir_backend` and not the requested tier: an `optimize=true`
            // compile that fell through to the single-pass backend is exactly
            // the event an operator reading this stream is looking for, and
            // the `c1`/`c2` column would hide it.
            let actual = if cm.used_ir_backend { "" } else { " (c1-body)" };
            let bytes = cm.code_bytes().len();
            p.emit(format_args!("({bytes} bytes){actual}"));
        }
        ...
        self.with_report(|r| {
            ...
            r.hierarchy_bound_sites = Measured::Value(tally.hierarchy_bound_sites);
...
impl Drop for CompileRecorder {
    fn drop(&mut self) {
        // The printed line for a compilation that produced no body. Taken
        // BEFORE the `state` early-return, because the stream is switched
        // independently of the ring: with metrics off there is no `state` at
        // all, and a refusal would otherwise be the one outcome the stream
        // never showed — which is the outcome it is most often turned on for.
        if let Some(p) = self.print.take() {
            let why = p
                .bailout
                .try_borrow()
                .ok()
            ...
            if !p.printed.get() {
                match why.as_deref() {
                    Some(why) => p.emit(format_args!("bailout:{why}")),
                    // No body and no recorded reason: the compile was
                    // abandoned somewhere that does not report one. Naming it
                    // is the point — a silent gap here is a `return None` with
                    // no `note_bailout_reason` beside it.
                    None => p.emit(format_args!("abandoned")),
                }
            }
        }
...
/// running a compile.
pub(crate) fn record_report(mut report: CompilationReport) {
...
pub const OSR_EVENTS: [&str; 15] = [
    ...
    // The optimizing-tier OSR door served this (method, pc) from its own
    // cache instead of compiling again -- the optimizing-tier analogue of the
    // single-pass OSR door's `OSR-reuse` report
    // (`vm::runtime::interpreter::jit_bridge`, `osr_optimizing_cached`).
    //
    // This row previously did not exist: the call site
    // (`jit_bridge.rs::osr_optimizing_cached` hit) recorded
    // `"osr_optimizing_artifact_reused"`, a string absent from this array, so
    // `record_osr_event` silently found no matching row and dropped every
    // occurrence -- forever, with no error, because an unknown name is
    // deliberately ignored (see `record_osr_event`). A door that has cached
    // and is reusing an optimizing-tier OSR artifact on every later entry
    // reported the exact same zero as a door that never got a cache hit at
    // all. Added here so the string now resolves to a real row.
    "osr_optimizing_artifact_reused",
...
///
/// See `loop-02-planner-admission-gates.md`.
pub const LOOP_XFORM_EVENTS: [&str; 13] = [
    ...
    // The rewriter produced a transform whose bytecode-provenance map does not
    // cover every rewritten instruction, so the coordinate change cannot be
    // audited and the transform is REFUSED. This row exists because round 10
    // found the refusal firing at three sites in `x64::loop_rewrite` with
    // nothing counting it: all three `return Err(R::ProvenanceNotTotal)` before
    // the tally at the end of the function, so the loss was visible only under
    // an off-by-default `eprintln!`. A non-zero value is a lost optimization,
    // not a wrong answer — the refusal is the fail-closed side.
    "loop_xform_provenance_not_total",
];
...
pub const SCHEDULING_EVENTS: [&str; 8] = [
    ...
    "worker_panic",
    // A request was refused by `CompilerCore::admit` because its method is
    // marked ineligible, or has already burned `MAX_TIER_FAIL_RETRIES` compile
    // attempts. The sibling arm of the `queue_deduplicated` test above, and the
    // process-wide mirror of `TieredCompilationManager::ineligible_refusals()`,
    // which is a per-manager accessor no dump reads.
    //
    // Expected to stay at zero in a healthy run: every policy door
    // (`should_compile`, `request_osr`, `request_c2_upgrade`) asks the same two
    // questions before it builds a task, so a count here is a door that pushed
    // without asking — the shape that let the deopt re-queue path retry a
    // method whose compile keeps failing, forever. It is counted here rather
    // than only on the manager because "expected to stay at zero" is a claim
    // only a printed number can support.
    //
    // The producer is one line in `CompilerCore::admit` (`jit/src/tiered.rs`),
    // immediately below the `queue_deduplicated` recording it mirrors. It
    // landed 2026-09-17; until then this row read zero for want of a PRODUCER
    // rather than for want of refusals, which is exactly the ambiguity the
    // fixed row set exists to remove. A zero here now means zero refusals.
    "queue_refused_ineligible",
];
...
pub(crate) fn scheduling_counts() -> Vec<(&'static str, u64)> {
    SCHEDULING_EVENTS
        .iter()
...
/// the stale-epoch, class-invalidated, shutdown and deoptimized drops
/// (indices 0, 1, 3 and 5).
///
...
/// scheduler's. The two refusal rows (`queue_deduplicated`,
/// `queue_refused_ineligible`) are excluded because nothing was ever queued.
///
/// `queue_dropped_deoptimized` (index 5) was missing from this sum while
/// `TieredCompilationManager::dropped_requests` -- documented as this
/// function's per-manager mirror -- counted it, so the two disagreed on
/// exactly the workloads that deoptimize, and a queued request thrown away
/// by a counted trap was not "discarded" by the process-wide total.
pub fn scheduling_dropped_total() -> u64 {
    scheduling_count(SCHEDULING_EVENTS[0]).unwrap_or(0)
        ...
        + scheduling_count(SCHEDULING_EVENTS[5]).unwrap_or(0)
}
...
pub struct MetricsSummary {
    ...
    /// [`crate::deopt::deopt_stash_nesting_counts`] as named rows:
    /// `deopt_stash_nested` and `deopt_stash_dropped_unclaimed`. Process-wide
    /// and metrics-flag-independent like the four above.
    ///
    /// **Both are expected to read zero**, and that is the entire reason they
    /// are here. A non-zero `deopt_stash_nested` means a trap arrived while an
    /// earlier trap's frame was still stashed — some consumer handled a trap
    /// without taking its frame — and `deopt_stash_dropped_unclaimed` counts
    /// those stale frames as the next take discards them. They are never a
    /// live caller chain: only re-execute points enter the stash and no caller
    /// stashes on its callee's sentinel (see
    /// [`crate::deopt::deopt_stash_nesting_counts`]).
    ///
    /// A counter whose whole value is "it stays at zero" is worth nothing until
    /// something prints it: this row is the pull, and `vm-cli` prints both at
    /// exit whenever either is non-zero.
    pub deopt_stash: Vec<(&'static str, u64)>,
}

/// The two [`MetricsSummary::deopt_stash`] row names, in the order
/// [`deopt_stash_counts`] emits them.
///
/// Named here rather than spelled at the call site for the same reason
/// [`SCHEDULING_EVENTS`] is: a sink wants a stable row set, and a renamed row
/// that silently stops matching is how a dashboard starts reading zero for a
/// number that is not zero.
pub const DEOPT_STASH_EVENTS: [&str; 2] = ["deopt_stash_nested", "deopt_stash_dropped_unclaimed"];

/// [`crate::deopt::deopt_stash_nesting_counts`] as named rows, in
/// [`DEOPT_STASH_EVENTS`] order.
///
/// A pull, not a push: the two counters live in `crate::deopt` as plain
/// statics that the stash path bumps directly, so there is nothing to register
/// and nothing that can be forgotten at a call site. Relaxed loads, so a
/// sample rather than an atomic snapshot — the same contract
/// [`scheduling_counts`] gives.
pub fn deopt_stash_counts() -> Vec<(&'static str, u64)> {
    let (nested, dropped) = crate::deopt::deopt_stash_nesting_counts();
    vec![
        (DEOPT_STASH_EVENTS[0], nested),
        (DEOPT_STASH_EVENTS[1], dropped),
    ]
}
...
pub fn summary() -> MetricsSummary {
    ...
    MetricsSummary {
        ...
        deopt_stash: deopt_stash_counts(),
    }
}
...
impl MetricsSummary {
    ...
    pub fn to_json(&self) -> String {
        ...
        let _ = write!(s, ",\"deopt_stash\":{}", pairs(&self.deopt_stash));
        s.push('}');
        s
    }
}
...
mod tests {
    ...
    fn inline_and_allocation_fields_stay_distinguishable_from_zero() {
        ...
        for (name, m) in [
            ...
            ("hierarchy_bound_sites", fresh.hierarchy_bound_sites),
            ("inlined_bytecodes", fresh.inlined_bytecodes),
            ("spills", fresh.spills),
            ("reloads", fresh.reloads),
            ("planned_spills", fresh.planned_spills),
            ("planned_reloads", fresh.planned_reloads),
        ] {
    ...
    /// The planned pair and the emitted pair are four independent fields.
    ///
    /// R5 (`NOTES-misc.md` §2) is the decision this pins: an allocation's
    /// `spills` is what it PLANNED and `plan_register_residency` emits only the
    /// subset it can prove, so the two figures disagree and a report that
    /// pointed both producers at one field would say "one of two numbers" and
    /// leave the reader no way to tell which. Writing either pair must leave
    /// the other alone, and the gap between them is the measurement.
    #[test]
    fn the_planned_allocation_figures_do_not_overwrite_the_emitted_ones() {
        let _guard = TEST_LOCK.lock();
        set_enabled_for_test(true);
        clear_reports();
        {
            let rec = CompileRecorder::begin("metrics/AllocPlan", "m", "()V", true);
            // The plan: nine spills, five reloads.
            rec.set_planned_spills(9);
            rec.set_planned_reloads(5);
            // What lowering actually emitted: a strict subset.
            note_current_spills(4);
            note_current_reloads(1);
            drop(rec);
        }
        let r = last_compilation_report(Some("metrics/AllocPlan")).expect("published");
        assert_eq!(r.planned_spills, Measured::Value(9));
        assert_eq!(r.planned_reloads, Measured::Value(5));
        assert_eq!(
            r.spills,
            Measured::Value(4),
            "the emitted pair is untouched"
        );
        assert_eq!(r.reloads, Measured::Value(1));
        let json = r.to_json();
        assert!(json.contains("\"planned_spills\":9"), "{json}");
        assert!(json.contains("\"planned_reloads\":5"), "{json}");
        assert!(json.contains("\"spills\":4"), "{json}");
        assert!(json.contains("\"reloads\":1"), "{json}");
        clear_reports();
        set_enabled_for_test(false);
    }

    /// A compile that emitted a measurement but had no recorder at allocation
    /// time reports the planned pair as `null`, not as zero.
    ///
    /// This is every compile today — the planned pair still has no production
    /// writer — and "no plan was harvested" must stay distinct from "the
    /// allocator planned nothing", for the same reason [`Measured`] exists at
    /// all.
    #[test]
    fn an_unharvested_allocation_plan_is_null_and_not_zero() {
        let _guard = TEST_LOCK.lock();
        set_enabled_for_test(true);
        clear_reports();
        {
            let rec = CompileRecorder::begin("metrics/AllocNoPlan", "m", "()V", true);
            note_current_spills(0);
            drop(rec);
        }
        let r = last_compilation_report(Some("metrics/AllocNoPlan")).expect("published");
        assert_eq!(r.spills, Measured::Value(0));
        assert_eq!(r.planned_spills, Measured::NotMeasured);
        assert_eq!(r.planned_reloads, Measured::NotMeasured);
        let json = r.to_json();
        assert!(json.contains("\"spills\":0"), "{json}");
        assert!(json.contains("\"planned_spills\":null"), "{json}");
        assert!(json.contains("\"planned_reloads\":null"), "{json}");
        clear_reports();
        set_enabled_for_test(false);
    }
    ...
    /// Regression for the string the optimizing-tier OSR door actually sends
    /// (`vm/src/runtime/interpreter/jit_bridge.rs`, `osr_optimizing_cached`
    /// hit): `"osr_optimizing_artifact_reused"` must resolve to a real row,
    /// not be silently dropped as an unknown name. It once was — the row did
    /// not exist in [`OSR_EVENTS`], so every cache-hit reuse of an
    /// optimizing-tier OSR artifact was counted nowhere and looked identical
    /// to a door with no cache hits at all.
    #[test]
    fn osr_optimizing_artifact_reused_is_a_real_row() {
        let _guard = METRICS_TEST_LOCK.lock();
        reset_osr_counts();
        assert!(OSR_EVENTS.contains(&"osr_optimizing_artifact_reused"));
        record_osr_event("osr_optimizing_artifact_reused");
        record_osr_event("osr_optimizing_artifact_reused");
        let counts: Vec<(&str, u64)> = osr_counts();
        let (_, n) = counts
            .iter()
            .find(|(name, _)| *name == "osr_optimizing_artifact_reused")
            .expect("row present");
        assert_eq!(*n, 2);
        reset_osr_counts();
    }
    ...
    fn recording_a_scheduling_event_is_visible_without_metrics_enabled() {
        ...
        // A request dropped by a counted trap WAS queued and is thrown away,
        // exactly as the manager's `dropped_requests` mirror counts it.
        record_scheduling_events(SCHEDULING_EVENTS[5], 2);
        assert_eq!(scheduling_dropped_total(), 9);
        // Refusals (nothing queued) and a contained panic (the compile ran)
        // are not drops.
        record_scheduling_event(SCHEDULING_EVENTS[4]);
        record_scheduling_event(SCHEDULING_EVENTS[6]);
        record_scheduling_event(SCHEDULING_EVENTS[7]);
        assert_eq!(scheduling_dropped_total(), 9);

        reset_scheduling_counts_for_test();
    }

    /// The ineligible-refusal row must exist, and at the index `tiered.rs`
    /// will record by.
    ///
    /// `CompilerCore::admit` records `queue_deduplicated` by INDEX
    /// (`SCHEDULING_EVENTS[4]`), so the index of its sibling arm is part of the
    /// contract and not an implementation detail. Pinning the name at index 7
    /// here is what makes the one-line companion edit in `tiered.rs` safe to
    /// write against a constant instead of against a literal.
    #[test]
    fn the_ineligible_refusal_row_exists_at_the_index_the_scheduler_records_by() {
        let _guard = TEST_LOCK.lock();
        set_enabled_for_test(false);
        reset_scheduling_counts_for_test();

        assert_eq!(SCHEDULING_EVENTS[7], "queue_refused_ineligible");
        // A row that exists but cannot be recorded into is the bug this test
        // exists to prevent: the name must resolve through the same lookup
        // `record_scheduling_event` uses.
        assert_eq!(scheduling_count(SCHEDULING_EVENTS[7]), Some(0));
        record_scheduling_events(SCHEDULING_EVENTS[7], 2);
        assert_eq!(scheduling_count(SCHEDULING_EVENTS[7]), Some(2));
        // It is a REFUSAL, not a drop: nothing was ever queued, so it must not
        // move the total that counts thrown-away queued work — exactly the rule
        // `queue_deduplicated` (index 4) already follows.
        assert_eq!(scheduling_dropped_total(), 0);

        reset_scheduling_counts_for_test();
    }

    /// The deopt stash's two residual counters must reach a dump.
    ///
    /// Both are expected to read zero forever; that claim is only worth
    /// something if a run can print them, which is the whole point of the row.
    /// The test asserts the plumbing (names, order, JSON key) rather than the
    /// values, because the values are process-wide and another test in this
    /// binary may legitimately have pushed a nested stash.
    #[test]
    fn the_summary_prints_the_deopt_stash_residual_counters() {
        let _guard = TEST_LOCK.lock();
        set_enabled_for_test(false);

        let rows = deopt_stash_counts();
        let names: Vec<&str> = rows.iter().map(|(n, _)| *n).collect();
        assert_eq!(names, DEOPT_STASH_EVENTS.to_vec());
        // Names and order, not values: `crate::deopt`'s counters are
        // process-wide and `deopt.rs`'s own tests push nested stashes, so a
        // value comparison across two reads is a race this lock does not cover.
        let summary_names: Vec<&str> = summary().deopt_stash.iter().map(|(n, _)| *n).collect();
        assert_eq!(summary_names, DEOPT_STASH_EVENTS.to_vec());

        let json = summary().to_json();
        assert!(
            json.contains("\"deopt_stash\":{\"deopt_stash_nested\":"),
            "{json}"
        );
        assert!(
            json.contains("\"deopt_stash_dropped_unclaimed\":"),
            "{json}"
        );
    }
...
///
/// Six numbers, because most single readings of "the feature is on" are wrong
...
///
/// The last two split out the IMPLICIT half (2026-09-20): a bounds check, an
/// inline null check or a zero-divisor guard entering this method's own
/// `catch`. It is a different pad, a different runtime arm and a different
/// kill switch (`CRATONVM_JIT_IMPLICIT_LOCAL_HANDLERS`), so a reading of
/// `entered` alone cannot say whether it engaged. `propagated` is shared: the
/// miss edge is the same answer whichever pad asked.
pub const LOCAL_HANDLER_NAMES: [&str; 6] = [
    "methods-armed",
    "sites-emitted",
    "entered",
    "propagated",
    "implicit-sites-emitted",
    "implicit-entered",
];

static LOCAL_HANDLER_COUNTS: [std::sync::atomic::AtomicU64; 6] = [
    std::sync::atomic::AtomicU64::new(0),
    std::sync::atomic::AtomicU64::new(0),
    std::sync::atomic::AtomicU64::new(0),
    std::sync::atomic::AtomicU64::new(0),
...
/// Index: one IMPLICIT-trap local-handler probe emitted (a bounds check, an
/// inline null check or a zero-divisor guard whose bci this method can catch).
/// Separate from `sites-emitted` because the two arm different pads and a zero
/// in one says nothing about the other.
pub const LOCAL_HANDLER_IMPLICIT_SITE_EMITTED: usize = 4;
/// Index: a `catch` entered in compiled code for an exception the compiled
/// code itself had to BUILD — the counter that says the implicit half of the
/// feature engaged, as opposed to a dispatched callee's throwable.
pub const LOCAL_HANDLER_IMPLICIT_ENTERED: usize = 5;
