...
//!   which holds the `Allocation` and, being below `lower_inner_with_scopes`
...
/// [`CompileRecorder`]. `ir_lower::lower_inner_with_scopes` is that caller: it
...
impl CompileRecorder {
    ...
    /// the ambient forms exist for `ir_lower::lower_inner_with_scopes`, whose
    ...
    /// but it is called from `lower_inner_with_scopes` — the pinned-signature
