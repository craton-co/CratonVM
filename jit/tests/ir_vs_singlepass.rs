...
fn wide_statics_stay_on_single_pass_with_their_value_tier_off() {
    ...
    // That is the shape in which a wide or floating-point static appears in a
    // method containing no category-2 or FP OPCODE, and it is also what
    ...
    // the admission gate refuse the method and the test pass vacuously.
    //
    // (The one non-category-2 *discard* of a wide value, `pop2` (0x58), is the
    // other body that reaches this rung. This comment used to claim `pop2` was
    // "implemented by neither backend", which stopped being true when the stack
    // shuffles landed: the IR tier lowers it in `ir.rs`'s 0x58 arm and the
    // single-pass tier in `x64/op_local_stack.rs`. The shape pinned here is
    // still the one worth pinning, because it is the one ordinary Java
    // produces — javac does not emit `getstatic; pop2; return`.)
    //
...
// ── switches: the three `Op::Switch` shapes, against the single-pass body ──
//
// `IrBuilder` now emits an `ir::Op::Switch` multi-way terminator wherever the
// key range is dense enough to be worth one (`ir::switch_is_representable`),
// and `ir_lower` picks a compare chain, a jump table or a binary search from
// it. Three shapes, an `i32`-overflowing span, and a budget boundary — every
// one of them a place an off-by-one in a table index or a sign in a compare
// would show up as a wrong ARM rather than as a crash, which is exactly the
// failure no unit test on the IR shape catches.
//
// Every fixture takes every case AND the default, and the single-pass body is
// the oracle for all of them (`check` also anchors on a host-computed value,
// so a bug shared by both backends still fails).

/// `int f(int x) { switch (x) { … } }` as a `tableswitch` over the slots
/// `low ..= low + arms.len() - 1`.
///
/// `arms[i]` is the value slot `low + i` returns, or `None` for a slot wired
/// to the DEFAULT target — which is how javac pads a `tableswitch` whose cases
/// have holes, and the shape the live-case count and the range trimming are
/// about.
///
/// Layout: `iload_0` at 0, `tableswitch` at 1, JVMS padding to the next
/// multiple of 4 (operands start at 4), then `default`, `low`, `high`, one
/// offset per slot, and the arms as `sipush v; ireturn`.
fn tableswitch_method(low: i32, arms: &[Option<i16>], default_value: i16) -> Vec<u8> {
    let n = arms.len();
    let arm_base = 16 + 4 * n;
    let live: Vec<usize> = (0..n).filter(|&i| arms[i].is_some()).collect();
    let default_at = arm_base + 4 * live.len();
    ...
    code.extend_from_slice(&low.wrapping_add(n as i32 - 1).to_be_bytes());
    for i in 0..n {
    ...
    for &i in &live {
        ...
        code.extend_from_slice(&arms[i].unwrap_or(0).to_be_bytes());
        code.push(0xac); // ireturn
    }
    code.push(0x11);
    code.extend_from_slice(&default_value.to_be_bytes());
    code.push(0xac);
    code
}

/// The same method shape as a `lookupswitch` over `(key, returned value)`
/// pairs, in the order given — which JVMS requires to be ascending.
fn lookupswitch_method(pairs: &[(i32, i16)], default_value: i16) -> Vec<u8> {
    let n = pairs.len();
    let arm_base = 12 + 8 * n;
...
/// TINY — two cases. `ir_lower` keeps the compare chain here however dense the
/// range is: an indirect branch through a table is one the predictor gets
/// wrong, and two `CMP`/`JE` pairs are not.
#[test]
fn ir_vs_singlepass_a_two_case_tableswitch_keeps_the_compare_chain() {
    check(
        "tswitch_tiny",
        "(I)I",
        tableswitch_method(5, &[Some(50), Some(60)], -1),
        1,
        1,
        &[
            (vec![5], 50),
...
/// DENSE — eight contiguous cases, no waste at all, so `ir_lower` emits the
/// jump table. Every key in range, both keys just outside it, and the two ends
/// of the `int` range: the out-of-range ones are the `JAE` that the single
/// unsigned compare is supposed to catch at BOTH ends.
#[test]
fn ir_vs_singlepass_a_dense_tableswitch_lowers_through_a_jump_table() {
    let arms: Vec<Option<i16>> = (0..8).map(|i| Some(100 + i as i16)).collect();
    check(
        "tswitch_dense",
        "(I)I",
        tableswitch_method(0, &arms, -1),
        1,
        1,
        &[
            (vec![0], 100),
...
/// A jump table WIDER THAN 256 SLOTS, executed, every slot checked.
///
/// Until 2026-09-17 `Op::Proj` carried a `u8`, so no switch over 255 dense
/// cases ever reached the lowerer's jump-table path -- the builder refused it
/// into a compare chain. That path is `Vec`-backed and sized from
/// `SWITCH_MAX_PROJECTIONS`, so nothing in it assumed 256, but "nothing assumed
/// it" is a claim about code that had never run at that size. This runs it.
///
/// 300 live arms, each returning a distinct value, so a table slot routed to
/// the wrong arm returns the wrong number rather than a coincidentally right
/// one. EVERY key in the range is checked, not a sample: the defect this could
/// have is one wrong slot, and a sample would miss it. Keys 255, 256 and 257
/// straddle the old `u8` boundary, where an index that wrapped would have
/// landed on slot 0.
///
/// `ir::switch_tests::a_switch_past_the_old_u8_ceiling_now_gets_a_switch_node`
/// proves the builder makes this an `Op::Switch`; an all-live range is dense,
/// so `ir_lower` puts it in a table. This proves the table is right.
#[test]
fn ir_vs_singlepass_a_tableswitch_wider_than_a_u8_lowers_through_a_jump_table() {
    const ARMS: i32 = 300;
    let arms: Vec<Option<i16>> = (0..ARMS).map(|i| Some(1000 + i as i16)).collect();
    let mut cases: Vec<(Vec<i64>, i32)> =
        (0..ARMS).map(|k| (vec![i64::from(k)], 1000 + k)).collect();
    // Outside the range: the default.
    for k in [ARMS, ARMS + 1, -1, i32::MIN, i32::MAX] {
        cases.push((vec![i64::from(k)], -1));
    }
    assert!(
        cases.iter().any(|(a, _)| a == &vec![256]) && cases.iter().any(|(a, _)| a == &vec![257]),
        "the keys either side of the old u8 boundary must be among those checked",
    );
    check(
        "tswitch_wider_than_u8",
        "(I)I",
        tableswitch_method(0, &arms, -1),
        1,
        1,
...
/// SPARSE — every third slot of a 21-slot range is live, so the range is too
/// thin for a table to be worth 21 entries and `ir_lower` emits the binary
/// search instead. The holes are still edges: each one has its own projection
/// wired to the default target, and each one must come back as the default.
#[test]
fn ir_vs_singlepass_a_sparse_tableswitch_lowers_through_a_binary_search() {
    let arms: Vec<Option<i16>> = (0..21)
        .map(|i| {
            if i % 3 == 0 {
                Some(200 + i as i16)
            } else {
        ...
        .collect();
    let mut cases: Vec<(Vec<i64>, i32)> = Vec::new();
    for i in 0..21i64 {
        let expect = if i % 3 == 0 { 200 + i as i32 } else { -7 };
        cases.push((vec![i], expect));
    }
    cases.push((vec![-1], -7));
    cases.push((vec![21], -7));
    cases.push((vec![i32::MIN as i64], -7));
    cases.push((vec![i32::MAX as i64], -7));
    check(
        "tswitch_sparse",
        "(I)I",
        tableswitch_method(0, &arms, -7),
        1,
        1,
        &cases,
    );
...
/// A `lookupswitch` whose keys are NEGATIVE. Two separate things have to be
/// right: the binary search compares signed (an unsigned split would put every
/// negative key above every positive one), and the jump table's `SUB EAX, low`
/// is a subtraction of a negative immediate.
#[test]
fn ir_vs_singlepass_a_lookupswitch_over_negative_keys() {
    check(
        "lswitch_negative",
        "(I)I",
        lookupswitch_method(
        ...
        &[
            ...
            (vec![-6], 0),
            (vec![2], 0),
            (vec![i32::MIN as i64], 0),
            (vec![i32::MAX as i64], 0),
        ],
    );
}

/// The span that OVERFLOWS `i32`. `high - low + 1` for these two keys is 2^32,
/// which wraps to 0 at 32 bits and reads as "perfectly dense" — so the shape
/// decision has to be made in `i64`, and the answer has to be the compare
/// chain rather than a jump table with four billion entries.
#[test]
fn ir_vs_singlepass_a_lookupswitch_spanning_the_whole_int_range() {
    check(
        "lswitch_full_range",
        "(I)I",
        lookupswitch_method(&[(i32::MIN, 11), (i32::MAX, 22)], 33),
        1,
        1,
        &[
            (vec![i32::MIN as i64], 11),
            (vec![i32::MAX as i64], 22),
            (vec![0], 33),
            (vec![-1], 33),
            (vec![(i32::MIN + 1) as i64], 33),
            (vec![(i32::MAX - 1) as i64], 33),
        ],
    );
}

/// A `tableswitch` sitting at the BOTTOM of the `int` range, where `key - low`
/// is the subtraction most likely to be got wrong. Small enough to keep the
/// chain, so this is about the key arithmetic and not about the table.
#[test]
fn ir_vs_singlepass_a_tableswitch_at_the_bottom_of_the_int_range() {
    check(
        "tswitch_int_min",
        "(I)I",
        tableswitch_method(i32::MIN, &[Some(1), Some(2), Some(3)], 9),
        1,
        1,
        &[
            (vec![i32::MIN as i64], 1),
            (vec![(i32::MIN + 1) as i64], 2),
            (vec![(i32::MIN + 2) as i64], 3),
            (vec![(i32::MIN + 3) as i64], 9),
            (vec![-1], 9),
            (vec![0], 9),
            (vec![i32::MAX as i64], 9),
        ],
    );
}

/// javac's padding: a `tableswitch` whose first three and last two slots carry
/// the default offset. The builder trims the key range to the live slots, so
/// those keys arrive at the default edge through the RANGE CHECK rather than
/// through a projection — and they must still answer the default.
#[test]
fn ir_vs_singlepass_a_tableswitch_padded_with_default_slots_on_both_sides() {
    let arms = [
        None,
        None,
        None,
...
/// The projection budget, exactly: `SWITCH_MAX_SPAN` slots spend indices
/// `0..SWITCH_MAX_SPAN` on their cases and the last index on the default -- the
/// widest `Op::Switch` the builder will make. One entry off in the table, or one
/// projection index off in the builder, lands on the wrong arm.
///
/// Derived from the constant, not written as a number. This used to hard-code
/// 255, which was the `u8` width of `Op::Proj`; when the budget became 2048 it
/// kept passing while testing a switch nowhere near the boundary its name
/// claims.
#[test]
fn ir_vs_singlepass_a_tableswitch_at_the_projection_budget() {
    let span = cratonvm_jit::ir::SWITCH_MAX_SPAN;
    let arms: Vec<Option<i16>> = (0..span).map(|i| Some(i as i16)).collect();
    let mut cases: Vec<(Vec<i64>, i32)> = Vec::new();
    // Every slot, so an off-by-one anywhere in the table is a wrong answer
    // rather than a lucky one.
    for i in 0..span {
        cases.push((vec![i], i as i32));
    }
    cases.push((vec![span], -2));
    cases.push((vec![-1], -2));
    cases.push((vec![i32::MIN as i64], -2));
    cases.push((vec![i32::MAX as i64], -2));
    check(
        "tswitch_budget",
        "(I)I",
        tableswitch_method(0, &arms, -2),
        1,
        1,
        &cases,
    );
...
/// PAST the projection budget: one slot more than `SWITCH_MAX_SPAN`, so the
/// builder refuses the dense form and keeps the five-nodes-per-case compare
/// chain. The chain has to stay CORRECT, which is the whole reason it is kept
/// reachable instead of deleted -- and the single-pass backend lowers this one
/// through a jump table, so the two tiers are genuinely different code here.
///
/// Derived from the constant. This used to hard-code 300 slots, which was past
/// a `u8` budget of 256. When the budget became 2048 a 300-slot switch started
/// getting a JUMP TABLE, so this test kept passing while the compare-chain path
/// it is named for lost its only end-to-end execution check. The 300-slot table
/// is now `..._wider_than_a_u8_lowers_through_a_jump_table`.
#[test]
fn ir_vs_singlepass_a_tableswitch_past_the_projection_budget_keeps_the_chain() {
    let slots = cratonvm_jit::ir::SWITCH_MAX_SPAN + 1;
    let arms: Vec<Option<i16>> = (0..slots).map(|i| Some(i as i16)).collect();
    let last = slots - 1;
    check(
        "tswitch_over_budget",
        "(I)I",
        tableswitch_method(0, &arms, -4),
        1,
        1,
        &[
            (vec![0], 0),
            ...
            (vec![255], 255),
            (vec![256], 256),
            (vec![slots / 2], (slots / 2) as i32),
            (vec![last - 1], (last - 1) as i32),
            (vec![last], last as i32),
            (vec![slots], -4),
            (vec![-1], -4),
        ],
    );
