...
///
/// * `ir_verify.rs` `sub_check_defaults_follow_the_master_gate` — a test
///   helper, `runtime_var_os(n).is_none()`, asking whether the harness left a
///   variable unset before it asserts a default. (Renamed 2026-09-16 from
///   `options_from_env_defaults_to_structural_only`, when the four optional
///   lanes stopped defaulting to `false` in a debug build; the COUNT is
///   unchanged, only the row label.)
...
///   (`nostore` / `all`).
/// * `intrinsic_catalogue.rs` — `CRATONVM_DBG_ALTRACE` (round 9 wave 8): the
///   ArrayList intrinsic stands down whenever `native-collections`' own
///   `altrace_enabled` (a presence read) would trace, so the two must ask the
///   same question the same way or a traced run would skip the natives it
///   meant to trace.
///
/// (`tiered.rs` `CRATONVM_TIER_C2_THRESHOLD` left the list 2026-09-18: the
/// C1->C2 upgrade gate now parses the value the way the tier policy does,
/// instead of arming on the variable's mere presence.)
