...
//!
//! # What is allowed to remain, on 2026-09-18
//!
//! * `aarch64_backend.rs` `compile_pass` — the aarch64 emitter. It reads a
//!   switch's operands to lay out its jump table in machine code.
...
//!
//! `x64/bytecode_walk.rs` `compile_bytecode` left the list in round 9 wave 2:
//! its private branch-target pre-pass (the one switch decode left in the x64
//! walk — its emitter arms already use `bytecode_analysis::switch_table`) was
//! replaced by `bytecode_analysis::branch_target_map`, which also marks
//! `goto_w` targets.
//!
//! [`ALLOWED`] pins the count in both directions: a new decoder fails, and so
//! does migrating one of the four until the number is lowered. The needles are
