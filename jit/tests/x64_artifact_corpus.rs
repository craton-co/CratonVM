...
//! constant.
//!
//! # THIS FILE DETECTS CHANGE, NOT CORRECTNESS
//!
//! Stated plainly because the shape of this file invites the opposite reading:
//! it compiles a wide corpus, it runs in CI, and it passes. None of that is a
//! statement about whether the emitted code computes the right answer.
//!
//! * Nothing here is ever EXECUTED. The helper table is fabricated
//!   (`fake_helpers`) and every address in it is a constant, so a body that
//!   would fault on its first instruction fingerprints exactly as well as a
//!   correct one.
//! * Nothing here compares against an ORACLE. There is no interpreter run, no
//!   HotSpot run, and no host-computed expected value anywhere in this file.
//! * A wrong change that MOVES the bytes is indistinguishable from a refactor
//!   that moves the bytes. The differ answers "did the bytes move"; only a
//!   human reading the diff tells the two apart.
//! * A wrong change that emits IDENTICAL bytes — a metadata error outside
//!   `metadata_hash`'s reach, or a runtime-policy change — produces no diff at
//!   all.
//!
//! What answers correctness is elsewhere, and a change to this backend wants
//! both: `jit/tests/differential.rs` (both tiers against a host anchor),
//! `jit/tests/ir_vs_singlepass.rs` (the two tiers against each other,
//! executed) and `difftest/` (generated programs against the interpreter as
//! oracle). This file's job is to make a refactor's claim of "changes nothing"
//! checkable and an optimisation's blast radius visible. That is a real job,
