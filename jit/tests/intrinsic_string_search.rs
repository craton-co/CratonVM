...
fn string_index_of_const_char_differential() {
    ...
    let needles: [u16; 6] = [
        ...
        // The largest needle `sipush` can carry. It used to be 0xFFFF, but
        // `sipush 0xFFFF` is -1, and a compiled `indexOf(-1)` must not be
        // masked into a search for U+FFFF (round 9 added the runtime screen
        // that sends it to the uncommon trap, which this harness reads as 0).
        0x7FFF,
    ];
    for &ch in &needles {
...
/// E27-1 N3-obs, settled: unlike `compareTo` and `indexOf(I)`, the `equals`
/// intrinsic dereferenced `this` (the class-id check's `MOV ECX,[RAX]`) with
/// no null-receiver guard in front of it. `((String) null).equals(x)` must
/// raise NullPointerException on the oracle; a null `this` here must deopt
/// and let the interpreter's ordinary dispatch raise it, not fault on the
/// null page the way an un-registered implicit-null site would.
#[test]
fn string_equals_null_receiver_deopts() {
    let _guard = deopt_lock();
    let f = compile_equals();
    let (b, _bb) = string_of("hello");
    let before = clear_deopt_signals();
    let r = f(0, b.ptr());
    assert_eq!(r, i64::MIN, "null receiver must return the deopt sentinel");
    assert_one_deopt_after(before, "equals null receiver");
}
