...
//! 2. in every other file, scanning stops at the **first line carrying a
//!    configuration attribute that selects the test configuration** — the
//!    bare one, and also a compound one such as
...
//!    production code. See [`opens_test_code`]. Everything from the stopping
//!    line on is test code;
...
fn opens_test_code(line: &str, gate: &str) -> bool {
    ...
    let rest = &line[at + cfg_open.len()..];
    if rest.contains(&format!("{}(", ["no", "t"].concat())) {
        return false;
    }
