...
fn arraycopy_despec_uses_dispatch_not_intrinsic_sentinel() {
    ...
    // M4: the registry key carries the speculation id now. This site records
    // the wildcard (`SPECULATION_ID_ANY`), because the `ArraycopyPrimitive`
    // filter asks the COARSE `contains` — it is deciding about the whole
    // bytecode index, not about one of two guards sharing it.
    despec.insert(method_key, 6, cratonvm_jit::deopt::SPECULATION_ID_ANY);
    {
        let mut record = DISPATCH_RECORD
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
