...
The VM is the core of the project (~2,343,000 Rust LoC across the 22 workspace
...
which reports roughly 2,403,000 lines (2,402,770 on 2026-09-21 after the
four-wave G1 design and performance round merged with dev, which is also what
moved `gc` in the table below to 242,000 - it was 195,000 when that round
started, a 22% growth in one round; 2,342,837 on 2026-09-19 after the round-9
JIT review merged with dev; 2,273,319 on 2026-09-17, up from 2,204,853 on
2026-09-12 - the jit and native-api growth recorded below, plus the 62-commit
dev merge landed during the lane-4 WindowsPath.type/.root session) across
1,126 files. Re-measure before quoting it: this figure and the
table below stood at 1,350,000 for long enough to be wrong by 49%, because
nothing regenerates them. If you change this paragraph, change the table too
— they are derived from the same command.
...
throwable-family lane record) brought it to 49,223 on the merge of all of
the above; `dev` and lane 4's waves 4-5 carried it to 51,358, and lane 4 wave
6's two `java/io/Print*` tables made it 51,618 on 2026-09-12 -- the wave that
crossed the tolerance line, by 168 lines, and was the one that re-measured
this row. Lane 0+3's combination build (L0+L3 wave 3+L4+L6+LT) re-measured it
again on 2026-09-16 against dev as it stood then (51,709); merging that into
the dev tip reached here brings it to 52,893, and by lane 4 wave 13's
WindowsPath retirement (2026-09-18) it had drifted to 55,744 -- past the
tolerance line again by 5.17%, this time caught by `doc_numeric_claims`
itself rather than noticed in passing. Lane 4 wave 14's own two-row addition
(`toRealPath`) brings it to **55,774**. **The 5% tolerance on the
row below is roughly two waves wide**, so expect to re-measure it about every
second wave rather than treating a red here as a surprise.

The `jit` row moved the same way and for a different reason, and it is
recorded so the next re-measure is not mistaken for drift: 269,000 -> ~286,000
over the three days to 2026-09-12 (the IR tier, the JIT review and the deopt
work, not one lane's table) -> **292,656**, then **323,961** on the round-6
review merge. That last step is ~31,000 lines and almost none of it is new
capability: it is the fixes for seven wrong-code and GC-safety defects, the
regression tests that reproduce them, and the explanations of why each was
wrong. `jit/src` alone is 304,372. Re-measured with the command above.
JIT review round 9 (2026-09-18) took it to **355,786** (`jit/src` 333,471): the
same shape again -- roughly two hundred defect fixes, the executed-code tests
that pin them, and the reasoning at each site.

| Crate | LoC | Crate | LoC |
|-------|----:|-------|----:|
| `native-builtins` | 776,000 | `native-awt` | 18,000 |
| `vm` | 505,000 | `types` | 45,000 |
| `jit` | 384,000 | `native-api` | 56,000 |
| `gc` | 242,000 | `reader` | 17,000 |
| `native-collections` | 90,000 | `jfr` | 20,000 |
| `native-io` | 87,000 | `jit-cuda` | 14,000 |
...
Garbage collectors, extracted into the `cratonvm-gc` crate. **ZGC is the default collector**, and has been since 2026-08-10: `gc/Cargo.toml` carries `default = ["zgc"]` and `vm/src/config.rs` defaults `gc_algorithm` to `GcAlgorithm::Zgc`, so a stock `cargo build` both contains ZGC and runs it. `ZgcRealHeap` (`gc/src/zgc.rs`, with `gc/src/zgc_concurrent.rs` and the seventeen modules of `gc/src/zgc/`) is a real, memory-backed collector — `Arena` storage, real `ObjectHeader`s, real `java.lang.ref` processing, a default-ON TLAB serving `alloc_object` / `alloc_array` — wired end to end as `GcAlgorithm::Zgc` → `GcBackend::Zgc` → `VmHeap::Zgc`, so `-XX:+UseZGC` also names it explicitly. It **compacts by default** — a stop-the-world arena slide, `ZgcRealHeap::relocate_stw` in `gc/src/zgc.rs`, kill switch `CRATONVM_ZGC_RELOCATE=0`, with a separate compactor for the large-object end under `CRATONVM_ZGC_HIGH_COMPACTION=0` — and has an opt-in generational mode (`zgc/generation.rs`, `CRATONVM_ZGC_GENERATIONAL=1`) and opt-in concurrent marking (`zgc_concurrent.rs`, `CRATONVM_ZGC_CONC_START=<percent>` or `auto`; the default `0` marks stop-the-world). It *contains* a colored-pointer encoding (`zgc/vaddr.rs`), a load barrier (`zgc/barrier.rs::z_load`) and a second, page-based evacuator (`zgc/relocate.rs`) — **none of which runs**: no slot ever holds a colored pointer, the barrier is never armed (see the note below), and `zgc/relocate.rs` has no production caller, its `CRATONVM_ZGC_PAGE_EVAC` switch being declared but unwired. Do not read the shipping compaction as `zgc/relocate.rs`; that mis-attribution is tracked in `docs/internal/zgc-round-20260920/gap-b-two-relocators-one-of-them-unreachable.md`. The generational collector — young + old gen, with the Cheney moving young gen *requested* by default on that backend, but see **"When the generational backend does not compact"** below — stays selectable with `-XX:+UseGenerationalGC` and is what a `--no-default-features` build falls back to. A region-based G1 collector is opt-in via `-XX:+UseG1GC` (experimental — see `docs/feature-designs/concurrent-gc-maturation.md`). *(Until 2026-09-21 this paragraph ended by warning readers off a metadata-only *simulation* of OpenJDK's colored-pointer model — `ZgcCollector` / `ColoredPointer` / `LoadBarrier` / `ZgcHeap` / `GenerationalZgc` — said to sit above `ZgcRealHeap` in the same file. **Those types were deleted in `78c20bd3d`**, "delete the simulation, and make the adoption census self-checking"; `gc/src/zgc.rs:182`–`:197` is the tombstone, mapping each removed name to the real type that replaced it. There is no simulation left to mistake for the shipping path, and `gc/src/zgc/vaddr.rs:49`–`:61` still carries a ⚠ block warning about its constants.)*
...
2. Requesting one is not running one: every cycle must carry a **per-cycle
   ...
   print `moving_young: moving_cycles_total=N non_moving_cycles_total=M
   refused_cycles_total=R moving_cycles_under_live_jit=J coverage_fallbacks=F`
   plus a per-reason histogram. (`refused_cycles_total` was added 2026-09-21: a
   young cycle can also refuse to run at all, and those used to be folded into
   `non_moving_cycles_total`, claiming a sweep that never ran.) (The field was `cycles=N` until 2026-09-20, when it was renamed
   because it was structurally pinned at 0 while the collector copied — see
...
crate (about 304,000 lines of Rust under `jit/src/`, including `jit/src/x64/`).
...
1. **Two standard libraries, and the default one is the real JDK's.** This
   ...
   * **Synthetic mode.** A large Rust stub library stands in for `java.*`, so
   ...
   `--compatible` (deprecated alias `--real-jdk`) and `--synthetic-jdk` are
...
8. **Compatibility policy is a runtime token in `types`, not a build feature.**
   `CompatibilityMode::JdkOnly` says *real class bytes are authoritative*;
   `CompatibilityMode::Compatible` is the pre-2026-09-20 behaviour, byte for
   byte. It is **orthogonal** to `JdkMode` — that picks *which class library*
   boots, this picks *which substitutions are permitted* — and `JdkMode` was
   deliberately not overloaded with strictness.

   The default **differs by entry point** (`vm/src/config.rs`): `JdkOnly` for
   the `cratonvm` launcher (`LAUNCHER_DEFAULT_COMPATIBILITY_MODE`, opt out with
   `--compatible`), `Compatible` for every embedding path
   (`EMBEDDED_DEFAULT_COMPATIBILITY_MODE`,
   `EMBEDDED_HOST_JDK_DEFAULT_COMPATIBILITY_MODE`). The split is on *who
   asked*: a person at a prompt can read a refusal and pass `--compatible`, a
   host program that linked `libcratonvm` cannot, and that crate's
   `DEFAULT_COMPATIBILITY_MODE` is ABI. `VmConfig::with_host_jdk_default`
   therefore stopped being an alias for `VmConfig::for_launcher` on the same
   day. Contract:
