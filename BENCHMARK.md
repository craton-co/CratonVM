...
### LICM guard attribution: what the blanket refusal cost (2026-09-18)

`tools/probes/GuardedDivHoist.java` is a counted loop with an `idiv`, whose
divide-by-zero `Op::Guard` sits in the body, and an invariant `this.bias`
read. `ir_optimize::LoopGuards` attributes that guard to `d`, not `this`, so
the read is hoisted. `CRATONVM_JIT_IR_LICM_GUARD_ATTRIBUTION=0` restores the
blanket refusal in the same binary. Release build, Windows desktop, three runs
per arm, same checksum everywhere:

| arm | ns/iter |
|---|---|
| attribution on (default) | 1.216 / 1.227 / 1.220 |
| attribution off | 1.414 / 1.430 / 1.413 |
| HotSpot 25, same host | 1.21–1.25 |

The attribution is the whole gap to HotSpot on this shape (about 14%).
`String.charAt` loops cannot show it: they are pinned to the single-pass tier
and never reach LICM. `NOTES-opts8.md` §4 has the guard-token arms and the
negative control.
