...
| Mode | Meaning |
|---|---|
| `CompatibilityMode::Compatible` | Bridges, intrinsics **and** compatibility shims. The default on every entry point reachable from this crate — deliberately not on the `cratonvm` launcher, which has defaulted to jdk-only since 2026-09-20. |
...
// with_host_jdk_default() is the embedding base: real JDK, deterministic, and
// `Compatible` -- unlike for_launcher(), which is strict.
...
- **The default here is `Compatible`, and you reach it by doing nothing.** Strict
  mode is never inferred from a Cargo feature, from `CRATONVM_REAL` /
  `CRATONVM_NO_STUBS`, from what the host machine has installed, or from the
  launcher's default — the first three select a native-registry filter only and
  cannot express the class-loading or dispatch half of the contract, and the
  last belongs to a different entry point.
- **The launcher's default is not this crate's, and re-aligning them is an ABI
  break.** `VmConfig::default()` (the embedding / in-tree-test path) is
  `JdkMode::Synthetic` while `VmConfig::with_host_jdk_default()` (the base
  config `libcratonvm`'s C entry points build on) is `JdkMode::Real`, because
  the two paths genuinely want different class libraries. Both are
  `Compatible`; only `VmConfig::for_launcher()` is `JdkOnly`, and
  `with_host_jdk_default` stopped aliasing it on 2026-09-20 for that reason.
  Do not "align" either split.

JDK-only enforcement is at stage 1 of 4, so a host that enables it should
expect failures on programs that run fine under `Compatible`. See
