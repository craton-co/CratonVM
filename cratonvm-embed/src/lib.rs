...
//!
//! **Both [`JdkMode`] and compatibility mode are asymmetric between entry
//! points, and they split along different lines.** [`VmConfig::default`] (the
//! embedding / in-tree-test path) is [`JdkMode::Synthetic`] while
//! [`VmConfig::with_host_jdk_default`] (the base config `libcratonvm`'s C
//! entry points build on) is [`JdkMode::Real`], because the two paths
//! genuinely want different class libraries.
//!
//! For compatibility mode, **every entry point reachable from this crate is
//! [`CompatibilityMode::Compatible`]**, and that is now a deliberate
//! *difference* from the `cratonvm` launcher, which defaults to
//! [`CompatibilityMode::JdkOnly`] as of 2026-09-20. A person at a prompt can
//! read a refusal and pass `--compatible`; an embedded host cannot. A caller
//! that did not ask for strictness must never be given a policy that rejects
//! work it expects to succeed, so do not "align" these with the launcher.
//! `VmConfig::for_launcher` is the launcher's config and is not what this
//! crate hands you.
//!
//! JDK-only is at stage 1 of 4 of its rollout, so expect failures on programs
//! that run fine under `JdkMode::Real` + `Compatible`. See `docs/EMBEDDING.md`
//! ("Choosing a compatibility mode") and
...
//! }
//!
//! // Doing nothing is `Compatible`, on both embedding entry points...
//! assert_eq!(
...
//!     CompatibilityMode::Compatible
//! );
//! // ...and deliberately NOT on the `cratonvm` launcher's config, which has
//! // defaulted to strict since 2026-09-20.
//! assert_eq!(
//!     VmConfig::for_launcher().compatibility_mode,
...
mod tests {
    ...
    /// feature, an environment variable, or **another entry point's default**
    /// must never supply it — and since 2026-09-20 there is such an entry
    /// point to not inherit from: `VmConfig::for_launcher` is `JdkOnly`.
    #[test]
    fn embedded_default_is_compatible() {
        ...
        assert_eq!(
            ...
            JdkMode::Real,
            "the host-JDK base config boots the host JDK"
        );
        // And the launcher's config, which this crate does not hand out, is
        // the one that is strict. Pinned here so an "alignment" cleanup of
        // `with_host_jdk_default` back into an alias fails in this crate too.
        assert_eq!(
            VmConfig::for_launcher().compatibility_mode,
            CompatibilityMode::JdkOnly
        );
