...
pub const JIT_HELPERS_ABI_VERSION: u32 = 14;
...
impl_helper_arg_abi! {
    ...
    *mut i64 => false,
    // A read-only word-array argument: the `invokedynamic` bridge's staged
    // argument block (`jit_indy_bridge(vm, site, args_ptr, argc)`). Classified
    // here rather than in `jit/` because the classification is a
    // calling-convention decision and this is where those live — see
    // `DIRECT_HELPER_FN_SIGS` in `jit/src/direct_helpers.rs`, which is the
    // only consumer today.
    *const i64 => false,
}
...
helper_fn_slots! {
    ...
    HelperFnAnewarrayObjectCp, anewarray_object_cp, anewarray_object_cp_fn,
        (i64, i64, i64, i64) -> i64;
    // (vm_ptr, obj) -> possibly-remapped obj | i64::MIN.
    //
    // This row used to say the return "is not advisory: a contended acquire can
    // move the object while the thread is parked", which reads as though the
    // tier that discards RAX were losing a relocation. That is true of the
    // SENTINEL and overstated for the remap: the collector's frame rewrite
    // reaches the same address, so the store-back is idempotent rather than
    // load-bearing. The determination is written out once, at the seam the two
    // tiers share, rather than restated here — see
    // `cratonvm_jit::runtime_lowering::emit_monitor_stub`'s doc, which also
    // says why `monitor_exit`'s `1` return makes any store-back enter-only.
    ...
    // v13: ZGC inline-TLAB announce -- (vm_ptr, obj_ptr, size_bytes) ->
    // obj_ptr, or `0` after publishing a pending exception. Logically the
    // sibling of `tlab_post_init` above; the slot is appended at the end of the
    // table because the ABI is append-only. See
    // `JitRuntimeHelpers::zgc_note_tlab_object`.
    HelperFnZgcNoteTlabObject, zgc_note_tlab_object, zgc_note_tlab_object_fn, (i64, i64, i64) -> i64;
    // v14: `multianewarray` of any arity -- (vm_ptr, site, ndims, dims_ptr) ->
    // the array, or `0` with an exception published. The fourth argument is a
    // READABLE frame address, not a value: it points at `ndims` consecutive
    // `i64` dimension counts, outermost first. That indirection is what lifts
    // the arity limit -- `multianewarray_2d`'s `(vm_ptr, site, dim1, dim2)`
    // spends all four Win64 integer argument registers on two dimensions, so
    // three was not expressible. See `JitRuntimeHelpers::multianewarray_n`.
    HelperFnMultianewarrayN, multianewarray_n, multianewarray_n_fn, (i64, i64, i64, *const i64) -> i64;
}
...
helper_field_table! {
    ...
    // v12: the post-barrier SKIP MASK, as a VALUE not an address.
    //
    // v10's `ref_store_post_young_floor` can only express "young enough" -- an
    // unsigned compare against `(gc_age << 4) | gc_flags`. The generational
    // collector's post barrier asks whether the receiver is in the OLD
    // GENERATION, which is a flag BIT, and the two do not order: an object
    // allocated straight into old gen has `gc_age == 0`, so its flags byte sits
    // BELOW the age-zero floor. A value rather than an address because which
    // collector is running does not change after start-up, so the emitter bakes
    // it as an immediate; the gate BYTES stay addresses because they do change.
    (ref_store_post_skip_mask,       Constant, false),
    // v13: the ZGC inline-TLAB announce helper. Optional: 0 keeps every inline
    // allocation on its `tlab_post_init` call, which announces the object too.
    (zgc_note_tlab_object,           Function, false),
    // v14: the N-dimensional `multianewarray` helper. Required -- see the
    // field's doc comment for why `multianewarray_2d` cannot stand in.
    (multianewarray_n,               Function, true),
}
...
const _: () = assert!(
    JIT_HELPERS_ABI_SIZE == 616,
    "JitRuntimeHelpers size changed (expected 77 * 8 = 616) — the JIT's baked \
...
pub const GOLDEN_HELPER_OFFSETS: [(&str, usize); NUM_HELPER_FIELDS] = [
    ...
    ("ref_store_post_skip_mask", 592),
    ("zgc_note_tlab_object", 600),
    ("multianewarray_n", 608),
];
...
pub const ABI_REVISIONS: &[HelperAbiRevision] = &[
    ...
    // v12 appends `ref_store_post_skip_mask`, which is what lets the
    // GENERATIONAL collector publish a reference-store plan at all: v10's age
    // floor cannot express "is the receiver in the old generation", so under
    // `-XX:+UseGenerationalGC` every compiled reference store was still paying
    // a full `jit_putfield_object` call.
    HelperAbiRevision {
        version: 12,
        num_fields: 75,
        size: 600,
    },
    // v13 appends `zgc_note_tlab_object`, the ZGC inline-TLAB announce helper.
    // Under ZGC every inline bump allocation must be announced to the
    // collector; when the class's `tlab_post_init` work is otherwise a no-op
    // this narrower call does only that. Optional: 0 keeps the
    // `tlab_post_init` call every inline allocation makes today.
    HelperAbiRevision {
        version: 13,
        num_fields: 76,
        size: 608,
    },
    // v14 appends `multianewarray_n`, which allocates a multi-dimensional
    // array of ANY arity. `multianewarray_2d` -- appended long before the
    // ledger existed -- takes its two dimensions BY VALUE, and with `vm_ptr`
    // and the site descriptor that is all four Win64 integer argument
    // registers, so a third dimension had nowhere to go. The x64 scanner
    // refused every `multianewarray` with `ndims != 2` for exactly that
    // reason, and a refusal there bail-lists the WHOLE method permanently:
    // one `new byte[3][4][4]` made `RomulusEngine.skinny_128_384_plus_enc`
    // (138x HotSpot, 63% of `RomulusTest`'s samples) uncompilable at every
    // door. This slot takes a POINTER to the dimensions instead, so arity
    // costs no registers. `multianewarray_2d` stays -- the table is
    // append-only -- and is no longer emitted.
    HelperAbiRevision {
        version: 14,
        num_fields: 77,
        size: 616,
...
const _: () = {
    ...
    assert!(functions == 62, "callable-slot count changed");
    assert!(
        offsets == 4,
    ...
    // 43 -> 44 in v14: `multianewarray_n` is a required call target.
    assert!(required == 44, "required-slot count changed");
    ...
    // 17 -> 18 in v13: `zgc_note_tlab_object`. Its emitter call site (the
    // x64 inline-TLAB third post-init mode) is gated on the slot being non-zero.
    assert!(
        optional_fns == 18,
...
// ---------------------------------------------------------------------
// The emitter-side lookup (`docs/jit/helper-abi.md` §3, row 1).
//
// `HELPER_FN_SIGS` existed from the day it was written with **no consumer**:
// `grep -rn HELPER_FN_SIGS` outside this file returned the re-export line in
// `lib.rs` and nothing else, and the doc said so in as many words. A table
// that describes the hazard without ever being consulted closes nothing.
//
// These two functions are the missing half. They are `const fn` on purpose:
// the check they enable is meant to run at COMPILE time at each hand-written
// call site, not as a runtime assertion nobody arms. See
// [`assert_helper_call_shape!`].
// ---------------------------------------------------------------------

/// Signature shape of the callable slot named `field`, or `None`.
///
/// `const fn`, so a call site can resolve its own helper's declared arity
/// while the compiler is still running and assert against it. Linear over
/// [`HELPER_FN_SIGS`]; there is no hot path here because every intended use
/// is folded away before codegen.
pub const fn helper_sig(field: &str) -> Option<HelperFnSig> {
    let mut i = 0;
    while i < HELPER_FN_SIGS.len() {
        if str_eq(HELPER_FN_SIGS[i].field, field) {
            return Some(HELPER_FN_SIGS[i]);
        }
        i += 1;
    }
    None
...
/// [`helper_sig`] for a field the caller names as a literal, panicking (at
/// compile time, in a `const` context) when no such callable slot exists.
///
/// A typo in the field name is therefore a build failure rather than a check
/// that silently does nothing — which is the failure mode an `Option`-shaped
/// assertion would have.
pub const fn helper_sig_of(field: &str) -> HelperFnSig {
    match helper_sig(field) {
        Some(sig) => sig,
        None => panic!(
            "no `helper_fn_slots!` row declares this helper field — either the \
             name is misspelled at the call site, or a callable slot was added \
             to the struct without a signature alias",
        ),
    }
}

/// Assert, at compile time, that a hand-written helper call site agrees with
/// the helper's declared C signature.
///
/// # The hazard this closes
///
/// The backend loads N argument registers by hand and then jumps to a bare
/// address (`emit_call_absolute`). Nothing in the type system connects that
/// hand-written setup to the callee's real arity: appending a parameter to a
/// helper *and* to its `helper_fn_slots!` row compiles clean on both sides —
/// the producer-side `let _: HelperFn* = jit_foo;` coercion pins the CALLEE's
/// signature and says nothing about the CALLER's register setup — and the
/// stub then passes an uninitialised register as the new argument.
///
/// Invoking this macro next to the call site makes the two disagree loudly:
/// the argument count the emitter actually writes is a literal here, and the
/// declared arity comes from [`HELPER_FN_SIGS`], which is derived from the
/// alias rather than transcribed.
///
/// `int_args` is the number of integer ARG_REGS the site loads, `returns_value`
/// is whether the site reads a result out of RAX.
///
/// ```
/// use cratonvm_jit_api::assert_helper_call_shape;
///
/// // `jit_monitor_enter(vm_ptr, obj) -> possibly-remapped obj`
/// assert_helper_call_shape!("monitor_enter", int_args = 2, returns_value = true);
/// ```
#[macro_export]
macro_rules! assert_helper_call_shape {
    ($field:literal, int_args = $n:expr, returns_value = $r:expr $(,)?) => {
        const _: () = {
            let sig = $crate::helpers_abi::helper_sig_of($field);
            assert!(
                sig.int_args() == $n,
                concat!(
                    ...
                    $field,
                    "` helper \
                     loads a number of integer argument registers that is not \
                     the helper's declared arity. Either the emitter is now \
                     passing an uninitialised register, or it is dropping an \
                     argument the callee reads.",
                ),
            );
            assert!(
                sig.float_args == 0,
                concat!(
                    "the `",
                    $field,
                    "` helper declares a floating-point \
                     argument, but this call site marshals integer ARG_REGS \
                     only — the value would arrive in the wrong register file.",
                ),
            );
            assert!(
                sig.returns_value == $r,
                concat!(
                    ...
                    $field,
                    "` helper \
                     disagrees with the declared return-ness. Reading RAX after \
                     a `-> ()` helper reads whatever the callee happened to \
                     leave there.",
                ),
            );
            // Every site that uses this macro marshals into ARG_REGS alone and
            // has no stack-argument path. `invoke_virtual_mic` is the one
            // helper above four arguments and its call site writes args 5 and 6
            // to `[RSP+32]`/`[RSP+40]` by hand, so it deliberately does NOT go
            // through this macro.
            assert!(
                ...
                concat!(
                    "the `",
                    $field,
                    "` helper takes more arguments than the \
                     Win64 integer register file holds; a site asserting its \
                     shape with this macro has no stack-argument marshalling \
                     and would pass garbage in the overflow positions.",
                ),
            );
        };
    };
...
mod tests {
    ...
    fn helper_fields_offsets_match_offset_of() {
        let probes: [(&str, usize); H::NUM_FIELDS] = [
            ...
            ("zgc_note_tlab_object", offset_of!(H, zgc_note_tlab_object)),
            ("multianewarray_n", offset_of!(H, multianewarray_n)),
        ];
    ...
    fn helper_table_size_and_align_are_the_literal_abi_numbers() {
        assert_eq!(core::mem::size_of::<H>(), 616);
        assert_eq!(core::mem::align_of::<H>(), 8);
        assert_eq!(JIT_HELPERS_ABI_SIZE, 616);
        assert_eq!(JIT_HELPERS_ABI_ALIGN, 8);
        assert_eq!(HELPER_FIELD_STRIDE, 8);
        assert_eq!(NUM_HELPER_FIELDS, 77);
        assert_eq!(H::NUM_FIELDS, 77);
        ...
        // v12 appends `ref_store_post_skip_mask`, a Constant (a VALUE this
        // time, not an address), so the callable count stands still again.
        // v13 appends `zgc_note_tlab_object`, an optional call target.
        // v14 appends `multianewarray_n`, a REQUIRED call target, so both the
        // callable count and the required count move by one.
        assert_eq!(H::NUM_HELPER_FN_FIELDS, 62);
        assert_eq!(JIT_HELPERS_ABI_VERSION, 14);
    }
    ...
    fn golden_offsets_are_the_struct_offsets() {
        ...
        assert_eq!(last_name, "multianewarray_n");
    ...
    fn slot_census_matches_the_pinned_counts() {
        ...
        assert_eq!(functions, 62, "callable slots");
        ...
        assert_eq!(required, 44, "required slots");
        // v13's `zgc_note_tlab_object` is an optional call target.
        assert_eq!(functions - required, 18, "optional callable slots");
    ...
    fn as_words_matches_the_struct_fields() {
        let mut h = H::default();
        h.newarray = 1;
        // The LAST field, whatever it currently is — `multianewarray_n`
        // since v14 appended the N-dimensional `multianewarray` helper after
        // v13's ZGC inline-TLAB announce helper.
        h.multianewarray_n = 2;
        let w = h.as_words();
