...
pub const ABI_REVISIONS: &[HelperAbiRevision] = &[
    ...
    // v7 — appended `ldc_string_cp`, the CP-INDEXED form of the string `ldc`.
    // `ldc_string` bakes the literal's bytes and so has no key to answer a
    // recorded resolution from; it re-derived the constant on every execution
...
mod tests {
    ...
    fn helper_table_size_and_align_are_the_literal_abi_numbers() {
        assert_eq!(core::mem::size_of::<H>(), 528);
        assert_eq!(core::mem::align_of::<H>(), 8);
        assert_eq!(JIT_HELPERS_ABI_SIZE, 528);
        assert_eq!(JIT_HELPERS_ABI_ALIGN, 8);
        assert_eq!(HELPER_FIELD_STRIDE, 8);
        assert_eq!(NUM_HELPER_FIELDS, 66);
        assert_eq!(H::NUM_FIELDS, 66);
        assert_eq!(H::NUM_HELPER_FN_FIELDS, 56);
        assert_eq!(JIT_HELPERS_ABI_VERSION, 7);
    }
    ...
    fn slot_census_matches_the_pinned_counts() {
        ...
        assert_eq!(functions, 56, "callable slots");
