...
pub const JIT_HELPERS_ABI_VERSION: u32 = 13;
...
const _: () = assert!(
    JIT_HELPERS_ABI_SIZE == 608,
    "JitRuntimeHelpers size changed (expected 76 * 8 = 608) — the JIT's baked \
...
const _: () = {
    ...
    assert!(functions == 61, "callable-slot count changed");
    assert!(
        offsets == 4,
...
mod tests {
    ...
    fn helper_table_size_and_align_are_the_literal_abi_numbers() {
        assert_eq!(core::mem::size_of::<H>(), 608);
        assert_eq!(core::mem::align_of::<H>(), 8);
        assert_eq!(JIT_HELPERS_ABI_SIZE, 608);
        assert_eq!(JIT_HELPERS_ABI_ALIGN, 8);
        assert_eq!(HELPER_FIELD_STRIDE, 8);
        assert_eq!(NUM_HELPER_FIELDS, 76);
        assert_eq!(H::NUM_FIELDS, 76);
        ...
        assert_eq!(H::NUM_HELPER_FN_FIELDS, 61);
        assert_eq!(JIT_HELPERS_ABI_VERSION, 13);
    }
    ...
    fn golden_offsets_are_the_struct_offsets() {
        ...
        assert_eq!(last_name, "zgc_note_tlab_object");
    ...
    fn slot_census_matches_the_pinned_counts() {
        ...
        assert_eq!(functions, 61, "callable slots");
    ...
    fn as_words_matches_the_struct_fields() {
        let mut h = H::default();
        h.newarray = 1;
        // The LAST field, whatever it currently is — `zgc_note_tlab_object`
        // since v13 appended the ZGC inline-TLAB announce helper after v12's
        // post-barrier mask.
        h.zgc_note_tlab_object = 2;
        let w = h.as_words();
