...
pub use helpers_abi::{
    accessor_name_matches_field, helper_field, helper_field_offset, helper_sig, helper_sig_of,
    str_eq, HelperAbiError, HelperAbiRevision, HelperArgAbi, HelperFieldDesc, HelperFnSig,
    HelperKind, HelperRetAbi, ABI_REVISIONS, GOLDEN_HELPER_OFFSETS,
    HELPERS_NEEDING_WIN64_STACK_ARGS, HELPER_FIELDS, HELPER_FIELD_STRIDE, HELPER_FN_SIGS,
    JIT_HELPERS_ABI_ALIGN, JIT_HELPERS_ABI_SIZE, JIT_HELPERS_ABI_VERSION, MAX_PLAUSIBLE_OFFSET,
    NUM_HELPER_FIELDS, SYSV_INT_ARG_REGS, WIN64_INT_ARG_REGS,
};
...
pub struct CachedBytecodeMethod {
    ...
    /// Per-method branch-profile arm, cached on this entry so the interpreter's
    /// conditional-branch arms can test it with one relaxed load.
    ///
    /// Set by the VM's first-nomination door
    /// (`jit_bridge::offer_invocation_to_tiered_manager`) when the tiered
    /// manager queues this method's first compile while it is still
    /// interpreted: the door arms the method's own lock-free counters
    /// (`ProfileStore::arm_branch_counters_borrowed`) and sets this flag, and
    /// from then on the interpreter records this method's branches even with
    /// the process-wide PGO gate off. See
    /// `docs/known-issues/jit/c2-branch-window-opens-after-the-method-leaves-the-interpreter-20260918.md`.
    ///
    /// Sticky (nothing clears it) and advisory: a `false` on an entry of a
    /// method armed through another entry only loses observations, never
    /// changes what the program computes. Every struct literal initialises it
    /// to `false`; [`Clone`] carries it forward, since the clone names the
    /// same method.
    pub branch_profile_armed: std::sync::atomic::AtomicBool,
...
impl Clone for CachedBytecodeMethod {
    fn clone(&self) -> Self {
        Self {
            ...
            // The arm is a property of the method, which the clone shares.
            branch_profile_armed: std::sync::atomic::AtomicBool::new(
                self.branch_profile_armed
                    .load(std::sync::atomic::Ordering::Relaxed),
            ),
...
pub struct JitRuntimeHelpers {
    ...
    /// ZGC inline-TLAB announce helper — `extern "C" fn(vm_ptr, obj_ptr,
    /// size_bytes) -> i64`.
    ///
    /// Under ZGC with `jit_tlab_registration_required()` the collector must
    /// learn about every object the inline bump pointer carves (the page's
    /// start bit, allocate-black while marking). When the class's
    /// `tlab_post_init` work is otherwise a no-op, compiled code can call this
    /// narrower helper instead: it announces the object and returns `obj_ptr`
    /// unchanged, or `0` after publishing a pending exception.
    ///
    /// `0` = not wired: the emitter keeps calling `tlab_post_init`, which
    /// already announces the object itself. Logically it belongs beside
    /// `tlab_post_init`; it is appended at the END of the struct (ABI v13) so
    /// all prior golden offsets stay stable.
    pub zgc_note_tlab_object: usize,
    /// `multianewarray` of ANY arity - `extern "C" fn(vm_ptr, site, ndims,
    /// dims_ptr) -> i64`.
    ///
    /// The N-dimensional successor to `multianewarray_2d`, which could only
    /// ever describe two dimensions because four integer argument registers is
    /// all Win64 has and `(vm_ptr, site, dim1, dim2)` spends all four. This one
    /// spends the last register on a POINTER instead: `dims_ptr` addresses
    /// `ndims` consecutive `i64` dimension counts, outermost first, which the
    /// caller has written into its own frame. Arity then costs nothing.
    ///
    /// `multianewarray_2d` stays in the table -- the ABI is append-only -- and
    /// is no longer emitted; see the note on that field.
    ///
    /// Appended at the END of the struct (ABI v14) so all prior golden offsets
    /// stay stable.
    pub multianewarray_n: usize,
}
...
helper_fields! {
    ...
    // v13: the ZGC inline-TLAB announce helper. Optional: 0 keeps every
    // inline allocation on its `tlab_post_init` call.
    (zgc_note_tlab_object,           FieldKind::OptionalPtr),
    // v14: the N-dimensional `multianewarray` helper, which supersedes
    // `multianewarray_2d`. Required, not optional: the emitter has no
    // fallback for a site it has already admitted -- `multianewarray_2d`
    // cannot serve one, which is the whole reason this slot exists.
    (multianewarray_n,               FieldKind::RequiredPtr),
}
...
const _: () = assert!(
    JitRuntimeHelpers::NUM_FIELDS == 77,
...
mod tests {
    ...
    fn make_helpers() -> JitRuntimeHelpers {
        JitRuntimeHelpers {
            ...
            zgc_note_tlab_object: 0x1218,
            multianewarray_n: 0x1220,
        }
    }
    ...
    fn jit_runtime_helpers_struct_size_matches_field_count() {
        ...
        // revision -- 76 as of v13. v10 appended dev's three reference-store
        ...
        // mask; v13 appended the ZGC inline-TLAB announce helper, so every
        // earlier golden offset below stays where it was.
        // v14 appended the N-dimensional `multianewarray` helper.
        assert_eq!(JitRuntimeHelpers::NUM_FIELDS, 77);
    }
    ...
    fn jit_runtime_helpers_repr_c_golden_offsets() {
        ...
        let probes: [(usize, &'static str, usize); JitRuntimeHelpers::NUM_FIELDS] = [
            ...
            (
                ...
                std::mem::offset_of!(JitRuntimeHelpers, zgc_note_tlab_object),
            ),
            (
                76,
                "multianewarray_n",
                std::mem::offset_of!(JitRuntimeHelpers, multianewarray_n),
            ),
        ];
    ...
    fn jit_runtime_helpers_all_fields_classified() {
        ...
        // v14's `multianewarray_n` is a RequiredPtr: 43 -> 44.
        assert_eq!(req, 44, "required-pointer count drifted");
        // v13's `zgc_note_tlab_object` is an OptionalPtr (0 = not wired).
        assert_eq!(opt, 18, "optional-pointer count drifted");
    ...
    fn jit_runtime_helpers_all_required_null_reports_every_name() {
        ...
        let required: Vec<&'static str> = h
            ...
            .map(|e| e.name)
            .collect();
        assert_eq!(required.len(), 44, "expected 44 required pointers");
    ...
    fn zero_field_by_name(h: &mut JitRuntimeHelpers, name: &str) {
        match name {
            ...
            "multianewarray_n" => h.multianewarray_n = 0,
