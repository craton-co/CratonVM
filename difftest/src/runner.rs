...
impl Mode {
    ...
    ///
    /// `--jdk-only` and `--compatible` are mutually exclusive by construction:
    /// no mode emits both.
    ///
    /// # Why the historical modes stopped returning `&[]`
    ///
    /// They used to, on the rule "keep producing the exact argv the committed
    /// gate baseline was captured with". On 2026-09-20 the launcher default
    /// flipped from `compatible` to `jdk-only`, and that rule inverted: an
    /// empty argv no longer reproduces the run the baseline recorded, it
    /// selects the *other* policy. Every `jit-on`, `nojit`, `no-intrinsics`,
    /// `moving-gc` and `low-jit-threshold` row in `difftest/ledger.json` and
    /// `path-gate-known.json` was captured under `compatible`, and
    /// `jdk_profile()` below still labels them `PROFILE_COMPATIBLE` — so a
    /// silent flip would have kept the label while changing the measurement,
    /// which is the one failure mode a frozen baseline exists to prevent.
    ///
    /// So the frozen thing is the *policy*, and it is now stated rather than
    /// inherited. `--compatible` on these modes produces exactly the VM
    /// behaviour their baselines were captured with.
    pub fn cli_args(self) -> &'static [&'static str] {
        match self {
            Mode::JdkOnlyJit | Mode::JdkOnlyNoJit => &["--jdk-only"],
            // `--real-jdk` rather than `--compatible`: it is the deprecated
            // alias for the same policy, and keeping it here is what makes
            // these two modes' committed census artefacts comparable to the
            // ones captured before the rename. The launcher prints a
            // deprecation note on stderr, which `Normalizer`'s
            // `strip_vm_diagnostics` drops like every other `[cratonvm]` line.
            ...
            Mode::InterpDecoded => &["--compatible", "--noverify"],
            _ => &["--compatible"],
        }
    }
...
mod tests {
    ...
    #[test]
    fn legacy_modes_run_the_compatible_policy_and_no_census() {
        // The whole point of the new modes being *new*: adding a policy axis
        // must not change what the historical five MEASURE. A dump leaking in
        // here silently re-measures the CI baseline.
        //
        // Until 2026-09-20 "measures the same thing" and "passes no launcher
        // flag" were the same assertion, and this test made the second one.
        // The launcher default flipped that day, so they came apart: an empty
        // argv now selects `jdk-only`. The invariant that actually matters is
        // the policy, so that is what is asserted — `--compatible` and nothing
        // else, which reproduces the VM behaviour these baselines hold.
        for mode in LEGACY_MODES {
            assert_eq!(
                mode.cli_args(),
                &["--compatible"],
                "{} must state the compatible policy and pass nothing else",
                mode.label()
            );
            assert!(
    ...
    #[test]
    fn every_mode_states_its_policy_and_never_both() {
        // Two properties, and the second one is why the first exists.
        //
        // (1) No mode passes both policies: the launcher refuses that argv
        //     outright, and before it did, the verdict would have been decided
        //     by argument order.
        // (2) EVERY mode passes one. Since 2026-09-20 an empty argv means
        //     `jdk-only`, so a mode that stated nothing would silently run
        //     strict while `jdk_profile()` kept labelling it `compatible` —
        //     every historical ledger row relabelled without being remeasured.
        for &m in Mode::all() {
            let args = m.cli_args();
            let strict = args.contains(&"--jdk-only");
            let compatible = args.contains(&"--compatible") || args.contains(&"--real-jdk");
            assert!(
                strict ^ compatible,
                "{} must state exactly one policy, got {args:?}",
                m.label()
            );
            assert_eq!(
                strict,
                m.is_jdk_only(),
                "{} disagrees with is_jdk_only()",
                m.label()
            );
            assert_eq!(
                m.jdk_profile(),
                if strict { "jdk-only" } else { "compatible" },
                "{} ledger profile disagrees with its argv",
                m.label()
            );
        }
    }
    ...
    fn execution_path_modes_set_the_flags_their_docs_name() {
        ...
        // Only `interp-decoded` adds a launcher flag of its own; the rest are
        // env-only, so they cannot perturb argv-sensitive behaviour. They all
        // carry `--compatible` since 2026-09-20 — that is not a knob of
        // theirs, it is the policy the launcher stopped defaulting to, and it
        // is identical across every mode here, so it still cannot make two of
        // them differ.
        for m in Mode::execution_paths() {
            let extra: Vec<&str> = m
                ...
                .collect();
            let expected: &[&str] = if *m == Mode::InterpDecoded {
                &["--noverify"]
            } else {
                &[]
            };
            assert_eq!(extra, expected, "{}", m.label());
        }
    }
