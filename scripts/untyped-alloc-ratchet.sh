...
scan() {
  ...
  echo "$files" | tr '\n' '\0' | xargs -0 awk '
    ...
    FNR == 1 { intest = 0; pend = 0; fn = "-" }
    {
      ...
      # A line comment QUOTING the pattern is not a call site. `git grep` over
      # source TEXT cannot tell the two apart, and the sites most likely to be
      # quoted in a comment are exactly the ones someone just finished
      # migrating — H16-1 §4b hit this for real: a replacement comment that
      # repeated the deleted call verbatim (to say what it used to be) counted
      # as a fifth site. Block comments (`/* ... */`) still slip through this
      # check; that is a known, written-down limit (H16-1 N6), not silently
      # assumed away.
      tl = $0
      sub(/^[ \t]*/, "", tl)
      if (substr(tl, 1, 2) == "//") next
