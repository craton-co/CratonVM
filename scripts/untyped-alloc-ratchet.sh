...
#
# LIMIT 7, and it is structural: THIS GATE COUNTS DIRECT SPELLINGS, NOT REACH.
# `alloc_ref_array` (native-collections/src/lib.rs) is one counted site and has
# **159 callers**. Routing one of those callers to a typed allocation removes a
# real fabrication and moves this number by ZERO. Measured 2026-08-21: typing the
# no-arg `HashMap()` table left `arrays: 151` unchanged.
#
# So the array column is a floor of a floor. Following helpers needs real call
# -graph analysis, not grep, and this file should not pretend otherwise. When a
# fix routes AROUND a helper rather than deleting a sentinel, say so in the
# commit -- the gate cannot.
#
