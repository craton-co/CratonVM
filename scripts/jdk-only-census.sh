...
# docs/jdk-only-audit.md §3 and §6 (the reproducible audit).
#
# ---------------------------------------------------------------------------
...
# under the name docs/jdk-only-audit.md §3.2 uses for it; `registry-no-stubs.json`
# is kept as an alias because CI's zero-stub gate reads that path.
#
# ---------------------------------------------------------------------------
# Derived artifacts (docs/jdk-only-audit.md §6)
# ---------------------------------------------------------------------------
#
...
# --- derived blocker artifacts (docs/jdk-only-audit.md §6) -----------------
...
case "$BLOCKERS" in
    ...
    generate|check|update)
        ...
        if PY="$(find_python)"; then :; else
            ...
            echo "       required by docs/jdk-only-audit.md §6 were NOT generated." >&2
