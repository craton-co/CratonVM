...
#
# WHAT IT COVERS, AND WHY THAT GREW ON 2026-09-21
#
# The `--lib` suites of the collector crates, PLUS a named list of cratonvm-gc
# INTEGRATION binaries (`INTEGRATION_TARGETS` below).
#
# The integration half is new, and it is here because the gate missed a real
# flake for want of it. `gc/tests/g1_w5b_sampled_audit.rs` was failing ELEVEN
# runs in twenty-five on a loaded 8-core host and nothing reported it: it is a
# `tests/*.rs` binary, this gate repeated only `--lib`, and a single `cargo
# test` run passes it most of the time on an idle machine.
#
# The shape is the same one the header above describes, one level out. Those
# binaries read PROCESS-GLOBAL censuses (`block_offset_process_census` and
# friends) whose values depend on the heap layout the parallel evacuator
# produced — so "run it once and it passed" says as little about them as it
# said about the narrow-oop tests. Repetition is again the only gate that sees
# it, and a binary that reads a process-global counter is exactly the
# population to repeat.
#
# TO ADD ONE: put the `tests/<name>.rs` stem in `INTEGRATION_TARGETS`. The rule
# of thumb is "does it assert on something no single test in it wrote?" — a
# process-global census, an env-latched flag's effect, a counter another test
# in the same binary could move.
#
# Usage:
...
#   FLAKE_INTEGRATION=0 scripts/gc-flake-gate.sh   # lib suites only
#
...
# Set to 0 to skip the integration half (e.g. when driving this at a single
# crate's `--lib` suite from a bisect).
FLAKE_INTEGRATION="${FLAKE_INTEGRATION:-1}"

# `cratonvm-gc` integration binaries that assert on process-global state, and
# are therefore in the population this gate exists for. See the header.
#
# All five read the block-offset census, whose value is a function of the heap
# layout the parallel evacuator produced. Four of them pin
# `CRATONVM_G1_WORKERS=1` to make that layout reproducible and the fifth
# (`g1_w4a_block_offsets_serial`) pins `CRATONVM_G1_PARALLEL_EVAC=0`, which is
# why that one never flaked. This list is what would notice if a future edit
# removed a pin: without it, `g1_w5b_sampled_audit` fails about two runs in
# five under load.
INTEGRATION_TARGETS=(
  "g1_w5b_sampled_audit"
  "g1_w5b_producer_rule_is_enforced"
  "g1_w4a_block_offsets_parallel"
  "g1_w4a_block_offsets_same_card"
  "g1_w4a_block_offsets_serial"
)
...
if [ "$FLAKE_INTEGRATION" != "0" ]; then
  for target in "${INTEGRATION_TARGETS[@]}"; do
    echo "=== flake gate: cratonvm-gc --test $target x $FLAKE_RUNS"

    build_out="$(cargo test -p cratonvm-gc --test "$target" --no-run 2>&1)" || {
      echo "$build_out"
      echo "FAIL: could not build the $target test binary"
      exit 1
    }
    # An integration target prints `Executable tests/<name>.rs (<path>)`, which
    # is a different line from the `Executable unittests ...` the lib half
    # matches — hence the second extractor rather than a shared one.
    bin="$(printf '%s\n' "$build_out" \
      | sed -n "s|.*Executable tests/$target[.]rs (\(.*\))\$|\1|p" | tail -1)"
    if [ -z "$bin" ] || [ ! -x "$bin" ]; then
      echo "$build_out"
      echo "FAIL: could not locate the $target test binary in the cargo output"
      exit 1
    fi
    ...
    for i in $(seq 1 "$FLAKE_RUNS"); do
      ...
      if [ "$rc" -ne 0 ]; then
        ...
        if [ "$printed" -lt "$MAX_LOGS_PRINTED" ]; then
          printed=$((printed + 1))
          echo "--- $target run $i FAILED (rc=$rc)"
          grep -E "^test .* FAILED|panicked at|^test result" "$log_dir/run.log" \
            | head -20
        fi
      fi
    ...
    if [ "$fails" -gt 0 ]; then
      echo "FLAKE GATE: $target failed $fails/$FLAKE_RUNS runs"
      overall_rc=1
    else
      echo "flake gate: $target clean over $FLAKE_RUNS runs"
    fi
  done
fi
