@echo off
REM Release build for the default-moving-young-gen worktree. Output -> cvmove.exe (unique).
cd /d "%~dp0"
...
if exist "target\release\cratonvm.exe" copy /Y "target\release\cratonvm.exe" "cvmove.exe"
