@echo off
REM Release build for the compact-ref-fields work. Output -> cvmcref.exe (unique).
cd /d "%~dp0"
...
if exist "target\release\cratonvm.exe" copy /Y "target\release\cratonvm.exe" "cvmcref.exe"
