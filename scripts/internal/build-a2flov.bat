@echo off
REM A2 free-list-overlap-coalesce fix build. Unique output -> cvma2flov.exe.
cd /d "%~dp0"
...
if exist "target\release\cratonvm.exe" copy /Y "target\release\cratonvm.exe" "cvma2flov.exe"
