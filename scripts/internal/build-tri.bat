@echo off
REM Release build for the known-issues triage worktree. Output copied to a unique name (cvtri.exe).
cd /d "%~dp0"
...
if exist "target\release\cratonvm.exe" copy /Y "target\release\cratonvm.exe" "cvtri.exe"
