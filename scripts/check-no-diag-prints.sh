...
#
# These markers (`[DIAG-*]`, `[TRACE-*]`, `[SB-TRACE]`) were used during
# bring-up to correlate Rust-side behaviour with failing Java workloads.
# They MUST NOT appear in landed code — they pollute stderr, race with
# structured tracing, and mask real errors. Structured diagnostics
...
if command -v rg >/dev/null 2>&1; then
    search() {
        rg --no-heading --line-number \
            --type rust \
            -e '(println|eprintln)!\s*\(\s*"\[(DIAG-|TRACE-|SB-TRACE)' \
            . 2>/dev/null || true
    }
else
    search() {
        grep -R --line-number --include='*.rs' -E \
            '(println|eprintln)!\s*\(\s*"\[(DIAG-|TRACE-|SB-TRACE)' \
            . 2>/dev/null | \
            grep -v '^\./target' | \
            grep -v '^\./scripts/' || true
    }
