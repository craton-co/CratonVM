...
param(
    ...
    # See the -WorkingDirectory note below: the fixture needs a `./target`.
    [string]$WorkDir = 'C:\craton\CratonVM'
)
...
foreach ($cls in $Classes) {
    ...
    # Pin the working directory. Hibernate's `GradleParallelTestingResolver`
    # writes a worker-id `<pid>.sequence` into `./target/`, so a caller whose
    # CWD happens to lack that directory gets an immediate
    # FileNotFoundException instead of a run — and the failure looks like a VM
    # bug. Every measurement in this investigation was taken from
    # C:\craton\CratonVM; keep them comparable by not inheriting the caller's.
