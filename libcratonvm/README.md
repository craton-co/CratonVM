...
`-D<key>=<value>`, plus the launcher's own `--compatible` (deprecated alias
`--real-jdk`) / `--synthetic-jdk` / `--jdk-only` (see
[Compatibility mode](#compatibility-mode)). Unsupported JNI
...
orthogonal to the JDK mode (`--compatible` / `--synthetic-jdk`), which selects
**which class library boots**; both surfaces set the two independently.

| C constant | Value | Meaning |
|---|---|---|
| `CRATONVM_COMPATIBILITY_COMPATIBLE` | `0` | Bridges, intrinsics **and** compatibility shims. The default on every entry point **of this library** — deliberately not on the `cratonvm` launcher, which has defaulted to jdk-only since 2026-09-20. These constants are ABI: a compiled host does not re-read this header, so a library upgrade must not start refusing work it never asked to have refused. |
...
JDK-only enforcement is at stage 1 of 4 of its rollout, so expect failures on
programs that run fine under `CRATONVM_COMPATIBILITY_COMPATIBLE` — which
remains this library's default even though the `cratonvm` launcher flipped to
JDK-only on 2026-09-20. See
