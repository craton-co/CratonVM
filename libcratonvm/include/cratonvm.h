 ...
 * (--compatible / --synthetic-jdk), which selects which class library boots.
 *
 * COMPATIBLE permits bridges, intrinsics AND compatibility shims. JDK_ONLY
 * makes real JDK class bytes authoritative: no class is fabricated without
 * real bytes and no synthetic-stub native is registered or invoked; it
 * requires a real JDK runtime image.
 *
 * COMPATIBLE IS STILL THIS LIBRARY'S DEFAULT — you reach it by doing nothing.
 * That is worth stating because on 2026-09-20 the `cratonvm` LAUNCHER flipped
 * its own default to jdk-only (opt out there with --compatible). This library
 * deliberately did not follow: these constants are ABI, a compiled host does
 * not re-read this header, and a library upgrade must not start refusing work
 * the host never asked to have refused. Ask for JDK_ONLY explicitly, the same
 * way you always did.
 *
 ...
 *
 * The mirror image holds for the compatible side: the option string
 * "--compatible" (and its deprecated alias "--real-jdk") states COMPATIBLE,
 * agrees with CRATONVM_COMPATIBILITY_COMPATIBLE, and contradicts
 * CRATONVM_COMPATIBILITY_JDK_ONLY. "--real-jdk" used to name only the class
 * library and so was silent about policy; it now means "--compatible" on both
 * the launcher and here, so that one argv cannot mean two policies depending
 * on which entry point consumed it. */
