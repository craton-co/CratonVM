...
// `docs/internal/gaps/gengc-mark-reference-queue-overflow-loses-entries-20260920.md`
...
impl ReferenceProcessor {
    ...
    pub fn process_references_with_finalizer_trace(
        ...
        // the referent's. See
        // `docs/internal/gaps/gengc-mark-dead-softref-object-still-roots-weak-closure-20260920.md`.
        //
    ...
    pub fn remove_collected_reference_objects(&mut self, is_live: &dyn Fn(usize) -> bool) {
        self.soft_refs.retain(|e| is_live(e.reference_obj));
        self.weak_refs.retain(|e| is_live(e.reference_obj));
        self.phantom_refs.retain(|e| is_live(e.reference_obj));
        self.rebuild_soft_position_indices();
    ...
    /// Addresses of Weak/Soft/Phantom `Reference` OBJECTS that this
    /// processor has discovered but not yet cleared — i.e. still in the
    /// JVMS §12.6.1 window between `discover_reference` and the clear+
    /// enqueue decision a future GC cycle will make for them.
    ///
    /// **Why this must be a GC root.** Nothing else keeps a `Reference`
    /// object itself alive once its last Java-visible use has passed: a
    /// caller commonly does `WeakReference<T> r = new WeakReference<>(x, q);`
    /// and then never reads the local `r` again, relying entirely on `q` to
    /// hand it back later. A precise (non-conservative) root scan correctly
    /// drops a local once its value is provably unused by any later
    /// bytecode — which is exactly what happens to `r` here. Without this
    /// method's caller treating these addresses as roots, the `Reference`
    /// object can be judged unmarked (dead) by the SAME cycle that is
    /// deciding whether to clear it, and the enqueue-into-`ReferenceQueue`
    /// splice in `process_references_after_gc` then declines it forever
    /// (`is_stale_young`'s "young, not in map" guard, correctly refusing to
    /// write through what its OWN inputs say is a dead address) — a
    /// `WeakReference` is cleared but never delivered, and `cleared` being
    /// permanently sticky means no later cycle ever retries it.
    ///
    /// Measured: `System.gc()` called twice in a row (no allocation in
    /// between) reliably drops the reference this way on every backend
    /// (Generational, G1, ZGC) prior to this fix — a single `System.gc()`
    /// always worked, because the local was still within its precise
    /// liveness window for that one call. This is the mechanism behind
    /// netty's `PooledByteBufAllocatorTest.testThreadCacheDestroyedByThreadCleaner`
    /// (`while (numThreadLocalCaches() > 0) { System.gc(); parkNanos(100ms); }`)
    /// never terminating: the second and every later `System.gc()` in that
    /// loop can never deliver the `Cleaner`-registered `WeakReference`.
    ///
    /// Once `cleared` is true the entry no longer needs this: a
    ...
    /// correctly still reachable through whatever rooted the referent.
    /// Including cleared entries here would be harmless (a `Reference`
    /// whose referent is already null costs nothing to over-root) but is
    /// avoided to keep this list — and the marking work it causes — exactly
    /// as large as the set that actually needs the guarantee.
...
mod tests {
    ...
    // WeakReference discovered but not yet cleared must be reported so the
    // caller can root it, since nothing else keeps a `Reference` object
    // alive past the caller's last read of its own local variable. Once
    // cleared, it no longer needs synthetic rooting -- see
    // `pending_reference_object_addresses`'s doc comment.
    #[test]
