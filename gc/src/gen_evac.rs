...
//! it has not established which two configurations it compared. The second
//! lever that would actually split the question — force the parallel arm
//! BUFFERLESS (`plab_bytes = 0`) while leaving everything else parallel — is
//! filed rather than built, because selecting it needs a `GcFlags` field and
//! `types/src/flags.rs` is outside this lane. See
//! `docs/internal/gaps/gengc-oldgen-par-evac-is-not-behaviour-neutral-20260920.md`.
...
impl<'a> ParEvac<'a> {
    ...
    fn bump_shared(&self, size: usize) -> Option<usize> {
        ...
        // gengc-round1 2026-09-20: `checked_add`. The cursor holds a raw
        // address and `fetch_add` wraps silently, so a plain `addr + size`
        // that wrapped would compare BELOW `to_region_end` and this would
        // return a span outside the arena for a worker to memcpy into. The
        // wrap needs a wild `size` to reach, which `evacuate`'s footprint
        // guards make unlikely rather than impossible — and the check is a
        // predicted-not-taken branch on a path that already has an atomic RMW.
        let fits = matches!(addr.checked_add(size), Some(end) if end <= self.to_region_end);
        if !fits {
...
/// owned by the caller.
unsafe fn write_filler(addr: usize, size: usize) {
    debug_assert!(
        addr % 8 == 0 && size % 8 == 0 && size > 0,
        "filler span must be an 8-aligned, non-zero multiple of 8 (addr={addr:#x} size={size})",
    );
