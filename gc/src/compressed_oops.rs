...
///
/// A SECOND call is a no-op that reports the geometry already in force. "Exactly
/// once" is a contract nothing enforced until 2026-09-20 — see the guard at the
/// top of the body.
pub fn enable_for_live_heap() -> Result<(u64, u8), String> {
    // "EXACTLY ONCE" IS NOW ENFORCED, and this is the single most dangerous
    // thing in this module to leave on trust.
    //
    // `cratonvm_types::narrow_oop::enable` has no idempotence guard of its
    // own: it stores BASE/SHIFT/LIMIT and sets ENABLED, unconditionally. A
    // second call with a different base therefore re-publishes the geometry
    // MID-PROCESS, and every reference already written under the old base
    // decodes to a different address from then on — silent, total heap
    // corruption with no fault at the moment it happens. It is the same
    // process-global-geometry failure class `scripts/gc-flake-gate.sh` exists
    // for, where five unit tests flipping `narrow_oop::enable` re-typed every
    // reference array in the test process and read for months as a race in
    // G1's parallel evacuator.
    //
    // `ACTIVE` is the one-shot latch, and it is set on the success path below,
    // so reaching here with it populated means a previous call already
    // published a geometry. Answer with THAT geometry rather than an error: the
    // caller's contract is "narrow oops are on at (base, shift)", which is true,
    // and an `Err` would make `vm_init` log narrow oops as refused while they
    // are in fact live — a worse lie than the duplicate call.
    //
    // RESIDUAL, stated rather than hidden: this closes a REPEATED call, not a
    // concurrent one. Two threads entering together could both pass this test
    // and both reach `narrow_oop::enable` below. Closing that would need the
    // one-shot latch to sit BEFORE the enable, and it cannot: `ACTIVE` is also
    // what `active()` answers from, so setting it ahead of a `narrow_oop::enable`
    // that then refused would leave `active()` describing a geometry nothing is
    // using. The caller is VM init on the primordial thread, before any other
    // thread exists, which is the premise this rests on — and unlike "exactly
    // once", that premise is one `vm_init` can be read to check.
    if let Some(active) = ACTIVE.get() {
        return Ok((active.base(), active.shift()));
    }
    ...
    // Arms the one-shot guard at the top of this function, so a later call
    // answers from here instead of re-publishing the process-global geometry.
