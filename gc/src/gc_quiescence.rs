...
///
/// # A reset-ordering hazard this counter has, filed not fixed
///
...
/// `docs/internal/gaps/gengc-plumbing-conservative-scan-reset-ordering-20260920.md`.
///
...
//
// Process-global in production; thread-local under `cfg(test)`, which is this
// module's stated convention for every counter (see the `TEST_JIT_ACTIVE_DEPTH`
// and `MOVING_YOUNG_COVERAGE_INCOMPLETE` comments) and which this one did not
// follow. It matters more here than for most: the gc unit tests run in parallel
// threads of one process, `begin_moving_young_coverage_cycle()` STORES ZERO
// into it, and `gen_heap`/`zgc` tests branch on `> 0` / `== 0`. One test
// opening a coverage cycle could therefore flip another test's collector
// decision from moving to non-moving, or back, with nothing failing
// deterministically.
#[cfg(not(test))]
...
#[cfg(test)]
thread_local! {
    static CONSERVATIVE_JIT_SCANS: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}
...
#[cfg(not(test))]
#[inline]
fn conservative_jit_scans_reset() {
...
#[inline]
fn conservative_jit_scans_get() -> usize {
    CONSERVATIVE_JIT_SCANS.with(std::cell::Cell::get)
}

#[cfg(test)]
#[inline]
fn conservative_jit_scans_bump() {
    CONSERVATIVE_JIT_SCANS.with(|c| c.set(c.get().saturating_add(1)));
}

#[cfg(test)]
#[inline]
fn conservative_jit_scans_reset() {
    CONSERVATIVE_JIT_SCANS.with(|c| c.set(0));
}
...
pub fn pinned_jit_roots_snapshot() -> Vec<usize> {
    ...
    out.extend(recover(xt_cycle_pin_set()).iter().copied());
    out
}
...
/// Record one frozen peer's register word into [`PEER_REG_CAPTURE`]. A no-op
/// unless `CRATONVM_DBG_PEER_REG_PAIRING` armed the pairing.
...
mod tests {
    ...
    /// CHARACTERIZATION TEST — this pins behaviour that is **wrong**, on
    /// purpose, so that fixing it is a deliberate act rather than a silent one.
    ///
    /// `CONSERVATIVE_JIT_SCANS` is cleared in exactly one place,
    /// [`begin_moving_young_coverage_cycle`], which the GC initiator runs
    /// during root gathering — i.e. after cooperatively parking peers have
    /// already deposited into it. Every sibling per-pause ledger is also
    /// cleared by [`reset_peer_proven_jit_depth`], which the barrier's
    /// `request_stw` runs before any peer can park, and `gc_barrier.rs`'s own
    /// comment says why that point is the load-bearing one. This counter is not
    /// in that reset, so a peer's "I looked" is discarded.
    ///
    /// What saves it today is a coincidence of ordering rather than a
    /// guarantee: the initiator's own publication runs after the clear. The
    /// hole is `vm/src/memory/roots.rs:1697`, where `g1_only_jit_pins()` on a
    /// non-G1 heap skips the initiator publication entirely — then the count
    /// reads zero while peers genuinely scanned and genuinely hold conservative
    /// roots, and `unrewritable_conservative_jit_roots` goes false, licensing a
    /// moving cycle behind unrewritable peer words.
    ///
    /// **If you are here because this test failed, you have probably fixed the
    /// bug.** Delete it, and read
    /// `docs/internal/gaps/gengc-plumbing-conservative-scan-reset-ordering-20260920.md`
    /// first — in particular that the counter is also ZGC's
    /// (`zgc.rs:5504` refuses on `== 0`), so the fix changes that backend too.
    #[test]
    fn conservative_jit_scans_are_erased_by_the_coverage_cycle_open() {
        ...
        // The initiator opens its coverage cycle — and the peer's deposit is
        // gone. This assertion is the bug.
        begin_moving_young_coverage_cycle();
        assert_eq!(
            conservative_jit_scans(),
            0,
            "TODAY the peer's deposit is discarded; see this test's doc comment \
