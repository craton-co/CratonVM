...
pub struct Tlab {
    ...
    /// The true (exclusive) end of the chunk this buffer owns. Equal to
    /// `end` for every buffer except a ZGC VM-TLAB chunk carved with LAZY
    /// ZEROING (`CRATONVM_ZGC_LAZY_TLAB_ZERO`, round 9 wave 11, `zgc11`):
    /// there `end` is a SOFT end, the edge of the part that has been zeroed
    /// so far, and `[end, chunk_end)` is owned by this buffer but may still
    /// hold stale bytes. [`Self::alloc_initialized`] zeroes the next step and
    /// moves `end` up when a bump misses it ([`Self::extend_zeroed`]), so the
    /// contract every consumer relies on -- `[cursor, end)` reads as zero --
    /// is unchanged, and so is everything the JIT reads (`end` at offset 8).
    ///
    /// Everything that speaks for the whole OWNED span uses this field
    /// instead: [`Self::reserved_tail`] (the skip region the STW protocol
    /// publishes), the retire's tail hand-back and the tail filler, and
    /// [`Self::remaining`]. Cold: appended after every JIT-read field.
    chunk_end: *mut u8,
    /// `[lo, hi)` inside `[end, chunk_end)` that needs no zeroing because the
    /// arena proved it was never written (its pristine window, see
    /// `Arena::pristine_lo`). `(0, 0)` when there is none. Only a lazily
    /// zeroed buffer carries one.
    lazy_skip: (usize, usize),
}

/// How far past the missed bump [`Tlab::extend_zeroed`] zeroes at once.
///
/// Small enough that the freshly zeroed lines are still in L1/L2 when the
/// allocations that follow write their headers and fields -- which is the
/// whole point of zeroing lazily: the chunk `memset` was a separate pass over
/// memory that the allocations then pulled back into the cache (round 9
/// wave 9-11, `perf-zgc-tlab-chunk-zeroing-is-the-last-allocation-cost`).
/// Large enough that the slow-path call it costs the compiled bump is paid
/// once per ~1 000 small objects.
pub const TLAB_LAZY_ZERO_STEP: usize = 32 * 1024;

/// Process-wide tallies of the lazy zeroing: `(extensions, bytes they
/// covered -- zeroed, or skipped as pristine --, owned tail bytes handed back
/// never zeroed)`. Diagnostics only
/// (`--verbose:gc`'s `zgc-tlab-zero` line); relaxed, one `fetch_add` per
/// extension (every [`TLAB_LAZY_ZERO_STEP`] bytes) or retire.
static LAZY_ZERO_EXTENSIONS: AtomicU64 = AtomicU64::new(0);
static LAZY_ZERO_BYTES: AtomicU64 = AtomicU64::new(0);
static LAZY_ZERO_TAIL_UNZEROED: AtomicU64 = AtomicU64::new(0);

/// `(extensions, bytes_zeroed, tail_bytes_never_zeroed)`; see
/// [`LAZY_ZERO_EXTENSIONS`].
pub fn tlab_lazy_zero_stats() -> (u64, u64, u64) {
    (
        LAZY_ZERO_EXTENSIONS.load(Ordering::Relaxed),
        LAZY_ZERO_BYTES.load(Ordering::Relaxed),
        LAZY_ZERO_TAIL_UNZEROED.load(Ordering::Relaxed),
    )
}

/// Zero `[lo, hi)` except the part inside `[skip_lo, skip_hi)`.
///
/// # Safety
/// `[lo, hi)` must be valid, writable memory owned by the caller.
unsafe fn zero_outside(lo: usize, hi: usize, skip: (usize, usize)) {
    let (skip_lo, skip_hi) = if skip.0 < skip.1 { skip } else { (0, 0) };
    let below_hi = hi.min(skip_lo);
    if lo < below_hi {
        // SAFETY: `[lo, below_hi)` is inside `[lo, hi)` (caller's contract).
        unsafe { std::ptr::write_bytes(lo as *mut u8, 0, below_hi - lo) };
    }
    let above_lo = lo.max(skip_hi);
    if above_lo < hi {
        // SAFETY: `[above_lo, hi)` is inside `[lo, hi)` (caller's contract).
        unsafe { std::ptr::write_bytes(above_lo as *mut u8, 0, hi - above_lo) };
    }
}
...
/// `Some(pristine skip range)` when `[lo, hi)` is exactly the chunk the ZGC
/// VM-TLAB refill on THIS thread just carved WITHOUT zeroing it
/// (`CRATONVM_ZGC_LAZY_TLAB_ZERO`); the record is consumed. `None` for every
/// other buffer, which is then zeroed memory exactly as [`Tlab::new`]'s
/// contract has always said.
#[cfg(feature = "zgc")]
fn lazy_zero_for_chunk(lo: usize, hi: usize) -> Option<(usize, usize)> {
    crate::zgc::take_lazy_zero_chunk(lo, hi)
}

/// Without the ZGC backend no chunk is ever handed out unzeroed.
#[cfg(not(feature = "zgc"))]
fn lazy_zero_for_chunk(_lo: usize, _hi: usize) -> Option<(usize, usize)> {
    None
}
...
impl Tlab {
    ...
    pub fn reserved_tail(&self) -> Option<(usize, usize)> {
        ...
        // The whole OWNED tail, not just its zeroed part: under lazy zeroing
        // (`chunk_end`) `[end, chunk_end)` is this buffer's too, and a sweep
        // or slide that did not withhold it would hand it to someone else
        // while `extend_zeroed` is about to bump into it.
        let e = self.chunk_end as usize;
        if c == 0 || e == 0 || c >= e {
            return None;
        }
...
impl Tlab {
    ...
    ///
    /// # The one exception to "zeroed" (round 9 wave 11, `zgc11`)
    ///
    /// Under `CRATONVM_ZGC_LAZY_TLAB_ZERO` the ZGC arm of
    /// `VmHeap::refill_tlab` hands out its chunk UNZEROED and records it for
    /// this thread; when `[ptr, ptr + size)` is exactly that chunk, this
    /// constructor consumes the record and starts with nothing zeroed (`end ==
    /// ptr`, `chunk_end == ptr + size`). The first bump zeroes the first
    /// [`TLAB_LAZY_ZERO_STEP`]; see [`Self::extend_zeroed`]. No other buffer
    /// can match that record, so for every other caller this is exactly the
    /// constructor it always was.
    pub unsafe fn new(ptr: *mut u8, size: usize) -> Self {
        Self::new_inner(ptr, size, true)
    }

    /// [`Self::new`]; `vm_buffer == false` (a heap staging buffer) never arms
    /// the inline announce and never adopts a lazily zeroed chunk.
    unsafe fn new_inner(ptr: *mut u8, size: usize, vm_buffer: bool) -> Self {
        ...
        let (zgc_owned_epoch, zgc_announce) = if vm_buffer {
            zgc_inline_announce_for_chunk(ptr as usize, aligned_end_addr)
        } else {
            (0, 0)
        };
        // Lazily zeroed? Then nothing is zeroed yet: the soft `end` starts at
        // `ptr` and `extend_zeroed` moves it. The inline announce armed above
        // names the WHOLE chunk (the owned record is `[ptr, chunk_end)`); the
        // JIT tests each object's bitmap word against `[start, end)`, a subset
        // of it, so the soft end only ever sends an edge word to the helper.
        let lazy = if vm_buffer {
            lazy_zero_for_chunk(ptr as usize, aligned_end_addr)
        } else {
            None
        };
        let (end, lazy_skip) = match lazy {
            Some(skip) => (ptr, skip),
            None => (aligned_end_addr as *mut u8, (0, 0)),
        };
        Self {
            start: ptr,
            cursor: ptr,
            ...
            process_published: 0,
            accounting: TlabAccounting::JavaThread,
            chunk_end: aligned_end_addr as *mut u8,
            lazy_skip,
        }
    }
    ...
    pub unsafe fn new_heap_staging(ptr: *mut u8, size: usize) -> Self {
        Self {
            ...
            zgc_owned_epoch: 0,
            zgc_announce: 0,
            // ...and never a lazily zeroed chunk: `new_inner(.., false)` does
            // not consult (or consume) this thread's VM-TLAB lazy record.
            ..Self::new_inner(ptr, size, false)
        }
    }
        ...
        F: FnOnce(*mut u8),
    {
        ...
        if new_cursor > self.end as usize && !self.extend_zeroed(new_cursor) {
            return None;
        }
    ...
    /// Lazy zeroing (round 9 wave 11, `zgc11`): make `[end, need_end)` --
    /// and up to [`TLAB_LAZY_ZERO_STEP`] beyond `end` -- zeroed and part of
    /// the soft `[cursor, end)`, when the chunk this buffer owns reaches that
    /// far. `false` (and nothing changes) when it does not, which is the
    /// ordinary "this TLAB is full" answer; for every buffer that is not
    /// lazily zeroed `chunk_end == end`, so that is the only answer.
    ///
    /// Only the owning thread calls this (from [`Self::alloc_initialized`]),
    /// and it moves only `end`. A peer that reads this buffer while the
    /// owner is suspended reads `cursor` and `chunk_end`
    /// ([`Self::reserved_tail`]), neither of which changes here, so a
    /// suspension in the middle of the `memset` publishes the same tail as
    /// one before or after it.
    #[cold]
    #[inline(never)]
    fn extend_zeroed(&mut self, need_end: usize) -> bool {
        let end = self.end as usize;
        let hard = self.chunk_end as usize;
        if end == 0 || need_end > hard {
            return false;
        }
        if need_end <= end {
            return true;
        }
        // 8-aligned: `hard` is (the constructor rounds it down), and the
        // tail filler and `reserved_tail` want an aligned soft end too.
        let want = need_end.max(end.saturating_add(TLAB_LAZY_ZERO_STEP));
        let target = (want.saturating_add(7) & !7usize).min(hard);
        // SAFETY: `[end, target)` lies inside `[start, chunk_end)`, the chunk
        // this buffer owns exclusively until retire (the constructor's
        // contract); nothing has been handed out above `end`, because every
        // bump stays below it.
        unsafe { zero_outside(end, target, self.lazy_skip) };
        self.end = target as *mut u8;
        LAZY_ZERO_EXTENSIONS.fetch_add(1, Ordering::Relaxed);
        LAZY_ZERO_BYTES.fetch_add((target - end) as u64, Ordering::Relaxed);
        true
    }

    /// Returns the remaining bytes available in this TLAB: the whole owned
    /// tail, including a lazily zeroed buffer's not-yet-zeroed part (every
    /// byte of it can still be handed out -- [`Self::extend_zeroed`]).
    pub fn remaining(&self) -> usize {
        (self.chunk_end as usize).saturating_sub(self.cursor as usize)
    }

    /// `(end, chunk_end)` as addresses: the zeroed (soft) end and the owned
    /// end. Equal unless this buffer is lazily zeroed and has not reached its
    /// chunk's end yet. Diagnostics and tests.
    pub fn lazy_zero_state(&self) -> (usize, usize) {
        (self.end as usize, self.chunk_end as usize)
    }

    /// Owned bytes above the soft `end` that were never zeroed, if this is a
    /// lazily zeroed buffer; counted as they leave at retire.
    fn note_unzeroed_tail_leaving(&self) {
        let (e, h) = (self.end as usize, self.chunk_end as usize);
        if e != 0 && h > e {
            LAZY_ZERO_TAIL_UNZEROED.fetch_add((h - e) as u64, Ordering::Relaxed);
        }
    }
    ...
    pub fn retire(&mut self) {
        ...
        // (`reserved_tail` is the whole owned tail, up to `chunk_end`.)
        self.note_unzeroed_tail_leaving();
        let reclaimed = match self.reserved_tail() {
            Some((tail_start, tail_end)) => reclaim_tail_via_sinks(tail_start, tail_end),
            None => false,
        };
        if reclaimed {
            self.end = self.chunk_end;
    ...
    pub unsafe fn install_tail_filler(&mut self, class_id: cratonvm_types::ClassId) {
        ...
        // A lazily zeroed buffer (`chunk_end`) owns up to `chunk_end`, and the
        // filler has to cover all of it; only `[cursor, zeroed_end)` is known
        // to read as zero, which is all the tripwires below may test. (For
        // every other buffer `chunk_end == end` and this changes nothing.)
        let zeroed_end = self.end as usize;
        if !self.chunk_end.is_null() {
            self.end = self.chunk_end;
        }
        ...
        // Only inside the zeroed part: past `zeroed_end` a lazily zeroed
        // chunk still holds whatever the arena block held before (the same
        // stale bytes the chunk `memset` used to clear), which is not an
        // occupant.
        let mut occupant = if aligned + 4 <= zeroed_end {
            unsafe { std::ptr::read_unaligned(aligned as *const u32) }
        } else {
            0
        };
        ...
        if occupant == 0 && cratonvm_types::flags().gc.dbg_deadref_store {
            // SAFETY: `[aligned, zeroed_end)` is inside this TLAB's own tail,
            // mapped and 8-aligned at both ends.
            let words = zeroed_end.min(end_addr).saturating_sub(aligned) / 8;
            for i in 0..words {
...
mod tests {
    ...
    /// Lazy zeroing (round 9 wave 11, `zgc11`), without a heap: a buffer
    /// whose soft `end` starts at `start` zeroes one step ahead of the bump
    /// that misses it, never the pristine range, never past the step, and
    /// publishes / hands back the whole owned chunk throughout.
    #[test]
    fn a_lazily_zeroed_buffer_zeroes_one_step_ahead_and_skips_the_pristine_range() {
        const K: usize = 1024;
        let (_owner, base, usable) = aligned_buffer(128 * K);
        let (lo, hi) = (base as usize, base as usize + usable);
        // Stale bytes everywhere, and a marked "pristine" range the arena
        // would have proved reads zero: it must never be written.
        unsafe { std::ptr::write_bytes(base, 0xAB, usable) };
        let (s_lo, s_hi) = (lo + 40 * K, lo + 48 * K);
        unsafe { std::ptr::write_bytes(s_lo as *mut u8, 0xCD, s_hi - s_lo) };
        fn bytes(a: usize, b: usize) -> &'static [u8] {
            // SAFETY: every range asked for lies inside `_owner`'s buffer,
            // which outlives each use.
            unsafe { std::slice::from_raw_parts(a as *const u8, b - a) }
        }
        ...
        assert_eq!(tlab.remaining(), usable);
        assert_eq!(tlab.reserved_tail(), Some((lo, hi)));

        assert_eq!(tlab.alloc(8, 8), Some(base));
        let step = TLAB_LAZY_ZERO_STEP;
        assert_eq!(tlab.lazy_zero_state(), (lo + step, hi));
        assert!(bytes(lo, lo + step).iter().all(|b| *b == 0));
        assert!(
            bytes(lo + step, s_lo).iter().all(|b| *b == 0xAB),
            "zeroed past the step"
        );

        // Inside the zeroed part: no extension.
        assert!(tlab.alloc(16 * K, 8).is_some());
        assert_eq!(tlab.lazy_zero_state(), (lo + step, hi));
        // A bump past the soft end zeroes the next step -- around the range.
        assert!(tlab.alloc(24 * K, 8).is_some());
        assert_eq!(tlab.lazy_zero_state(), (lo + 2 * step, hi));
        assert!(bytes(lo + step, s_lo).iter().all(|b| *b == 0));
        assert!(
            bytes(s_lo, s_hi).iter().all(|b| *b == 0xCD),
            "the pristine range was written"
        );
        assert!(bytes(s_hi, lo + 2 * step).iter().all(|b| *b == 0));
        assert!(bytes(lo + 2 * step, hi).iter().all(|b| *b == 0xAB));

        // One object reaching past a whole step: zeroed exactly as far as it
        // needs.
        let cursor = lo + 8 + 40 * K;
        assert_eq!(tlab.reserved_tail(), Some((cursor, hi)));
        assert!(tlab.alloc(72 * K, 8).is_some());
        let top = cursor + 72 * K;
        assert!(top > lo + 3 * step && top < hi);
        assert_eq!(tlab.lazy_zero_state(), (top, hi));
        assert!(bytes(lo + 2 * step, top).iter().all(|b| *b == 0));
        assert!(bytes(top, hi).iter().all(|b| *b == 0xAB));
        // Past the owned chunk: refused, nothing moves.
        assert!(tlab.alloc(hi - lo, 8).is_none());
        assert_eq!(tlab.lazy_zero_state(), (top, hi));

        let tail = tlab.retire_taking_tail();
        assert_eq!(tail, Some((top, hi)), "the whole owned tail goes back");
        assert!(tlab.is_retired() && tlab.reserved_tail().is_none());
        assert_eq!(tlab.lazy_zero_state(), (0, 0));
    }

    /// The tail filler of a lazily zeroed buffer covers the whole owned chunk,
    /// not just its zeroed part, and leaves the buffer fully consumed.
    #[test]
    fn a_lazily_zeroed_buffer_fills_to_its_chunk_end() {
        use crate::heap::HEADER_SIZE;
        let (_owner, base, usable) = aligned_buffer(96 * 1024);
        let (lo, hi) = (base as usize, base as usize + usable);
        unsafe { std::ptr::write_bytes(base, 0xAB, usable) };
        let mut tlab = Tlab {
            end: base,
            ..unsafe { Tlab::new_heap_staging(base, usable) }
        };
        assert!(tlab.alloc(64, 8).is_some());
        assert!(tlab.lazy_zero_state().0 < hi);
        let tail = unsafe { tlab.retire_with_filler_taking_tail(TLAB_FILLER_CLASS_ID) };
        assert_eq!(tail, Some((lo + 64, hi)));
        // The filler's body is zeroed all the way to the chunk's end.
        let body = unsafe {
            std::slice::from_raw_parts(
                (lo + 64 + HEADER_SIZE) as *const u8,
                hi - lo - 64 - HEADER_SIZE,
            )
        };
        assert!(
            body.iter().all(|b| *b == 0),
            "stale bytes left under the filler"
        );
        assert!(tlab.is_retired());
    }
