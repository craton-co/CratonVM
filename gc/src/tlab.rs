...
/// `gengc-alloc-tlab-sizer-is-blind-to-waste-20260920.md` step 2: a shrink
...
/// is what ends its life, and `TLAB_MAX_ALLOC` is 32 KiB against a 256 KiB
/// baseline — so a DRAIN can never reach half, and every hit here is an
/// early retire (a park, a blocking native, a thread exit, a forced GC).
...
impl Tlab {
    ...
    pub fn retire(&mut self) {
        ...
        let reclaimed = match self.reserved_tail() {
            Some((tail_start, tail_end)) => {
                let taken = reclaim_tail_via_sinks(tail_start, tail_end);
                if taken {
    ...
    fn finish_retire(&mut self, consumed: u64) {
        ...
        if !self.start.is_null() && self.accounting == TlabAccounting::JavaThread {
            ...
            // compare per retire, i.e. one per ~256 KiB allocated.
            if retire_is_wasteful(carved, consumed) {
...
mod tests {
    ...
    /// concurrently, so only monotone growth is a stable claim (the same
    /// reason `Tlab::process_published_bytes` exists for the allocation
    /// total).
    #[test]
    ...
    /// `gengc-alloc-tlab-sizer-is-blind-to-waste-20260920.md` proposes to size
    ...
    fn the_census_tells_an_early_retire_apart_from_a_drained_one() {
        ...
        // way (`>=`) for the reason in the doc comment above.
        let before = tlab_waste_stats();
        let mut buf = vec![0u8; 4096];
        let raw = buf.as_mut_ptr();
        let aligned = ((raw as usize + 7) & !7) as *mut u8;
    ...
    /// something.
    ///
    /// `gengc-alloc-tlab-instruments-are-write-only-20260920.md`'s verification
