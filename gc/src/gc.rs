...
//!
//! # Read this first: this module is LIVE; the semi-space collector in it is not selectable
//!
//! Recorded 2026-09-20 (generational-GC round 2, lane `plumbing2`). The file
//! header above describes the Cheney collector, which is what a reader
//! reasonably concludes the module is *for* — and that collector cannot be
//! selected by any `-XX:+Use…` flag, because there is no
//! [`crate::vm_heap::GcBackend`] variant and no `VmHeap` arm for
//! [`crate::heap::Heap`]. See that type's module header for the full statement.
//!
//! What IS live here, and is used across the workspace:
//!
//! * [`GcResult`] — the forwarding/pointer map **every** backend returns from a
//!   collection, including `GenerationalHeap`, `G1Collector` and ZGC;
//! * [`GcStats`] — the per-cycle counters those backends fill in;
//! * the JVMTI hook installers ([`install_gc_start_hook`],
//!   [`install_gc_finish_hook`], [`install_class_info_hook`],
//!   [`resolve_class_info`]), all re-exported from `lib.rs`.
//!
//! The only vestigial thing in this file is its own test fixture's use of
//! `Heap`. Do not delete the module.
//! `docs/internal/gaps/gengc-plumbing-vestigial-semispace-heap-20260920.md`
//! carries the reasoning and the retirement options.
//!
