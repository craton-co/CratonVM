...
impl GenerationalHeap {
    ...
    fn collect_garbage_inner(
        ...
        while young_cursor < young_used {
            ...
            // Skip a GAP-filler sentinel (Bug-D, 2026-06-12): its on-disk
            // layout is NOT a real ObjectHeader (offset 4 holds the raw gap
            // length, not header fields), so it must never reach
            // gen_object_total_size — see the established pattern at the
            // non-moving sweep's identical walk below.
            if header.class_id.as_u32() == crate::tlab::GAP_FILLER_CLASS_ID.as_u32() {
                // SAFETY: offset 4 lies within the >=8-byte gap.
                let gap = unsafe { std::ptr::read((obj_ptr as *const u8).add(4) as *const u32) } as usize;
                if (8..HEADER_SIZE).contains(&gap) && gap & 7 == 0 && young_cursor + gap <= young_used {
                    young_cursor += gap;
                    continue;
                }
                tracing::warn!(young_cursor, young_used, "GC: young object-start walk found an implausible GAP-filler sentinel");
                break;
            }
