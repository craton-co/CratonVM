...
impl ZgcRealHeap {
    ...
    pub(crate) fn carve_tlab_chunk(&self, want: usize, need: usize) -> Option<(*mut u8, usize)> {
        self.carve_tlab_chunk_inner(want, need, true)
            .map(|(ptr, size, _)| (ptr, size))
    }

    /// [`Self::carve_tlab_chunk`] WITHOUT the zeroing: the chunk comes back
    /// holding whatever the arena block held, together with the absolute
    /// `[lo, hi)` part of it that the arena proved was never written (reads
    /// as zero; `(0, 0)` when none). For the VM-TLAB lazy zeroing only
    /// (`CRATONVM_ZGC_LAZY_TLAB_ZERO`, round 9 wave 11, `zgc11`), whose
    /// buffer zeroes the chunk step by step just ahead of its bumps
    /// (`Tlab::extend_zeroed`). Blackening and the young set are done
    /// exactly as for a zeroed chunk.
    pub(crate) fn carve_tlab_chunk_unzeroed(
        &self,
        want: usize,
        need: usize,
    ) -> Option<(*mut u8, usize, (usize, usize))> {
        self.carve_tlab_chunk_inner(want, need, false)
    }

    fn carve_tlab_chunk_inner(
        &self,
        want: usize,
        need: usize,
        zero: bool,
        ...
        let pristine_abs = if skip_lo < skip_hi {
            (ptr as usize + skip_lo, ptr as usize + skip_hi)
        } else {
            (0, 0)
        };
        if !zero {
            // Lazy zeroing: the buffer zeroes it, skipping `pristine_abs`.
            // Counted here like the eager skip: those bytes are never zeroed.
            if skip_lo < skip_hi {
                self.counters
                    .tlab_pristine_bytes_skipped
                    .fetch_add(skip_hi - skip_lo, Ordering::Relaxed);
            }
        } else if skip_lo < skip_hi {
