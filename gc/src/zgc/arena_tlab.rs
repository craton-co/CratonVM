...
pub struct ZArenaTlabRegistry {
    ...
    /// Threads holding a **VM-thread** chunk (`zgc::vm_tlab::refill_tlab`),
    /// counted since the last [`Self::reset_vm_tlab_slots`].
    ///
    /// # Why this has to exist, and why it is not in `slots`
    ///
    /// [`Self::chunk_bytes_now`] divides [`ZGC_TLAB_RESERVATION_SHARE`] of the
    /// arena by "the buffers actually registered", because a chunk is *claimed*
    /// memory that no collection can reclaim while its owner lives — the whole
    /// derivation is on that constant, and it was written to close a real
    /// failure: Tomcat's `TestNonBlockingAPI` with ~4 000 threads × 512 KiB =
    /// the entire 2 GB heap, 1.99 GB on the free list in pieces and a 2 MB
    /// `char[]` with nowhere to go.
    ///
    /// `slots` counts only this module's own [`ZArenaTlab`] cells. Since round
    /// 9 wave 5 the VM thread's `Tlab` is **on by default**
    /// (`vm_tlab::ZGC_VM_TLAB_DEFAULT_ON`) and is the buffer nearly
    /// every Java allocation actually bumps into — and it carves from the same
    /// `carve_tlab_chunk` source, through `chunk_bytes_now`, while being
    /// invisible to `slots`. A thread that allocates only through the VM path
    /// never calls [`Self::with_attached`], so the divisor stayed at 1 and each
    /// thread was sized as though it were the only one. The bound the constant
    /// establishes was therefore off by the thread count on the path that
    /// matters, which is the original defect re-opened one layer up.
    ///
    /// A VM buffer cannot be put in `slots`: it is a bare [`Tlab`] the VM owns
    /// inside `JvmThread`, with no cell for this registry to hold and no handle
    /// it could retire. What the sizing needs is not the buffer but the *count*,
    /// so that is all this is.
    ///
    /// # Why a count with an epoch rather than a live registration
    ///
    /// There is no reliable "this thread's VM buffer is gone" event — a thread
    /// may exit without retiring, and `TlabTailSink::reclaim_tlab_tail` fires on
    /// every ordinary retire as well as the last one, so it cannot decrement. A
    /// monotone count would therefore ratchet: one burst of 4 000 threads would
    /// shrink every later chunk for the life of the process.
    ///
    /// Instead the count is *rebuilt*. [`Self::reset_vm_tlab_slots`] zeroes it
    /// and bumps [`Self::vm_tlab_slot_epoch`] at every collection (from
    /// `retire_all_tlabs`, which already prunes `slots` there), and each thread
    /// re-counts itself on its next refill. Dead threads drop out within one
    /// cycle; live ones re-appear within one chunk's worth of allocation. The
    /// transient under-count immediately after a collection sizes *one* chunk
    /// per thread generously, which is bounded by exactly the quantity the
    /// budget is expressed in.
    pub(crate) vm_tlab_slots: AtomicUsize,
    /// Bumped by [`Self::reset_vm_tlab_slots`]; a thread re-counts itself once
    /// per epoch. Starts at 1 so a thread's zero-initialised record never
    /// matches.
    pub(crate) vm_tlab_slot_epoch: AtomicU64,
    /// **Arena bytes currently inside a live chunk.** The quantity
    /// [`ZGC_TLAB_RESERVATION_SHARE`] is a budget *for*, measured in the units
    /// the budget is expressed in.
    ///
    /// # The design error this closes
    ///
    /// [`Self::vm_tlab_slots`] above, and the `live_slots` divisor beside it,
    /// bound the reservation by a **thread count**. The budget is a **byte**
    /// budget (`capacity / 16`), and a count is only the same thing if every
    /// claimant refills at the same instant — which is exactly what does not
    /// happen:
    ///
    /// 1. Each thread sizes against the count *at its own refill*. Thread 1
    ///    refills when the divisor is 1 and takes the ceiling; thread 200
    ///    refills when it is 200 and takes `budget / 200`. The **sum** of what
    ///    is outstanding is therefore not bounded by the budget; it only
    ///    converges towards it as threads re-refill.
    /// 2. The count is rebuilt per collection, so every cycle re-opens the
    ///    window: straight after [`Self::reset_vm_tlab_slots`] the divisor is
    ///    1 again and the first refill of each of `N` threads is sized at the
    ///    ceiling. `N x ceiling` outstanding is the unbounded shape, merely
    ///    bounded in *time* rather than in size.
    /// 3. Nothing measured the outstanding reservation at all. `allocated`
    ///    cannot answer it — `alloc_tlab` charges per object and `refill_tlab`
    ///    charges the whole chunk and credits the tail back, so the two halves
    ///    of one subsystem measure opposite things.
    ///
    /// This field is (3), and once it exists it replaces (1) and (2): see
    /// [`Self::chunk_bytes_now`].
    ///
    /// # Maintenance — one increment site, two release sites
    ///
    /// * **`+= size`** in `ZgcRealHeap::carve_tlab_chunk_inner`, the ONE chunk
    ///   source for both buffer kinds on this backend.
    /// * **`-= chunk_size`** in `ZgcRealHeap::tlab_retire_locked` (this
    ///   module's own cells, which know their extent as `ZArenaTlab::chunk`).
    /// * **`-= chunk_size`** in `zgc::vm_tlab`, from the per-thread record the
    ///   VM side keeps (`VM_TLAB_RESERVED`), released both at
    ///   `TlabTailSink::reclaim_tlab_tail` and at that thread's *next* refill
    ///   — the second is what makes the figure self-correcting rather than
    ///   monotone, because a thread that consumed a chunk whole never reaches
    ///   the tail sink.
    ///
    /// Maintained **unconditionally**, whatever
    /// [`Self::reserved_bytes_sizing`] says, so the number is available to be
    /// measured before it is acted on.
    ///
    /// # What it cannot account for, stated rather than hidden
    ///
    /// A thread that carves a chunk and then dies without retiring leaves its
    /// chunk counted forever. That is not a bookkeeping leak — those bytes
    /// genuinely are inside a chunk and handed to no object, and no collection
    /// returns them until a slide reclaims the span — but it does mean the
    /// figure ratchets on abnormal thread exit. The consequence is bounded and
    /// in the safe direction: [`Self::chunk_bytes_now`] clamps up to
    /// `min_tlab_size()`, so the worst case is small chunks, not
    /// a heap full of reservations. `vm/src/threading/thread_registry.rs`
    /// retires a thread's buffer at exit, so the ordinary path does return.
    ///
    /// Relaxed throughout: this is a sizing input, not a correctness signal.
    /// Nothing dereferences it and an off-by-one chunk in either direction is
    /// absorbed by the clamp.
    pub(crate) reserved_bytes: AtomicUsize,
    /// **High-water mark of [`Self::reserved_bytes`]** — the largest total
    /// ever outstanding in chunks at one instant.
    ///
    /// # Why the peak and not the current value
    ///
    /// [`Self::reserved_bytes`] read at teardown is close to zero on a healthy
    /// run: every thread has retired. The quantity the two sizing rules
    /// actually disagree about is the **sum outstanding at the worst moment**
    /// — `ZGC_TLAB_RESERVATION_SHARE` is a budget, and the count-based rule
    /// bounds a per-thread chunk size instead of a total, so it can exceed
    /// that budget by a factor of the thread count for one chunk's worth of
    /// allocation per thread, and again after every collection
    /// (`gap-c-tlab-reservation-is-bounded-by-a-thread-count-not-by-bytes.md`,
    /// residual points 1 and 2). A current value cannot see that; a peak is
    /// exactly it.
    ///
    /// So this is **the number that decides the default**. Read it against
    /// `capacity / ZGC_TLAB_RESERVATION_SHARE`: on the count arm a peak well
    /// above the budget is the unbounded shape the constant exists to
    /// prevent, and on the byte arm the peak is bounded by construction
    /// (`budget + threads past the knee x min_tlab_size()`). If the two arms
    /// produce the same peak on a workload, that workload does not reach the
    /// regime and says nothing either way — which is a reading, not a
    /// failure, and is why it is a counter rather than an assertion.
    ///
    /// Maintained **unconditionally**, on both arms, for the same reason
    /// [`Self::reserved_bytes`] is: a number that only exists on the arm being
    /// argued for cannot be used to argue for it.
    ///
    /// Monotone: it never decreases, so it survives the retire that empties
    /// `reserved_bytes` and is still readable at shutdown. Relaxed, and
    /// advanced with a `fetch_max`, so two threads charging concurrently
    /// cannot lose the larger of the two.
    pub(crate) reserved_bytes_peak: AtomicUsize,
    /// `CRATONVM_ZGC_TLAB_RESERVED_BYTES` — size chunks against
    /// [`Self::reserved_bytes`] instead of against the claimant count.
    ///
    /// **Default OFF**, and that is a deliberate staging choice rather than
    /// timidity. Two reasons:
    ///
    /// * The count-based bound landed default-on in wave 1 and closed the
    ///   acute regression (`CRATONVM_ZGC_VM_TLAB_SHARE`). Replacing a sizing
    ///   rule that is already load-bearing needs a measured A/B on a built
    ///   binary, and this session cannot build.
    /// * The two rules **disagree about what a collection means**, and the
    ///   disagreement is the point. `gc/tests/zgc_c_vm_tlab_reservation_share.rs`
    ///   asserts that `retire_all_tlabs` lets the chunk size recover after a
    ///   burst of threads that carved and exited without retiring. Under a byte
    ///   budget the honest answer is that it must **not** recover: those bytes
    ///   were never returned. Whoever flips this default has to update that
    ///   test, and they should — but it is a decision with a name on it, not a
    ///   silent consequence of this change.
    ///
    /// Read once per registry in [`Self::for_capacity`], and overridable for
    /// tests through `ZgcRealHeap::set_tlab_reserved_bytes_sizing`, so the
    /// switch is not a latched process global.
    ///
    /// # THE MEASUREMENT THAT DECIDES THE DEFAULT (2026-09-21, wave 4)
    ///
    /// The paragraphs above say the default is off because nobody has
    /// measured the flip. That is an honest reason exactly once; a second
    /// round of it is how a flag becomes permanent. So: here is the run, the
    /// counter, and the recommendation, and the flip itself is one line.
    ///
    /// **The run.** Both arms of the same binary, interleaved, on the
    /// thread-explosion workload this bound exists for — Tomcat
    /// `TestNonBlockingAPI` at `-Xmx2g`, which is the repro
    /// `ZGC_TLAB_RESERVATION_SHARE` was introduced for (~4 000 threads x a
    /// flat 512 KiB chunk = the whole heap; `gc/src/zgc.rs:11349-11371`).
    /// `CRATONVM_ZGC_TLAB_RESERVED_BYTES=1` against unset, `--verbose:gc`.
    ///
    /// **The counter, in priority order.**
    ///
    /// 1. `ZgcRealHeap::tlab_reserved_bytes_peak()` against
    ///    `tlab_reservation_budget_bytes()` — the sum outstanding at the worst
    ///    instant against the budget it is supposed to obey. This is the
    ///    quantity the two rules disagree about and the only one that can
    ///    separate them; see [`Self::reserved_bytes_peak`]. A run where both
    ///    arms report the same peak did not reach the regime and decides
    ///    nothing.
    /// 2. `[GC] zgc-frag:`'s `worst_largest_free_permille` — the
    ///    fragmentation shape the whole bound is about. Lower on the byte arm
    ///    is the win; unchanged means the count rule was already holding on
    ///    this workload.
    /// 3. Pass/fail of the two OOM repros that currently need a flag or a
    ///    bigger heap: `org.h2.test.store.TestMVStoreTool` at `-Xmx1g` and
    ///    `...DefaultCatalogAndSchemaTest` at `-Xmx1500m`.
    /// 4. Throughput must merely not regress: `CratonBench bintrees` and
    ///    `ZgcTlabStress 8 200` (ten rounds, not two — that workload is
    ///    bimodal and a two-round reading of it has already reversed once in
    ///    this tree).
    ///
    /// **The recommendation, stated before the numbers are in: flip it on.**
    /// The byte budget is the correct design and the count rule is not merely
    /// approximate, it bounds the wrong quantity — a per-thread chunk size
    /// rather than a total — and the shape of its failure is a fragmentation
    /// OOM this VM has hit repeatedly. What the measurement is for is the
    /// *cost*, not the correctness: the byte rule decays chunks toward
    /// `min_tlab_size()` once the budget is spent, and if that shows up as a
    /// throughput regression on (4) then the answer is to raise
    /// `ZGC_TLAB_RESERVATION_SHARE`'s budget, not to keep sizing against a
    /// thread count.
    ///
    /// **What flipping obliges, and it is still true.**
    /// `gc/tests/zgc_c_vm_tlab_reservation_share.rs` asserts that
    /// `retire_all_tlabs` lets the chunk size recover after a burst of threads
    /// that carved and exited without retiring. Under a byte budget the honest
    /// answer is that it must **not** recover: those bytes were never
    /// returned. That test has to be updated by whoever flips this, and the
    /// update is the point rather than a casualty of it.
    ///
    /// **The flip is one line**, here: `runtime_flag_on` becomes
    /// `cratonvm_types::flags::runtime_flag_default_on` in
    /// [`Self::for_capacity`], and the flag's `off_word` in
    /// `types/src/flag_groups.rs` becomes `Some("0")` in the same change —
    /// **both, or the inventory publishes the switch as the opposite of what
    /// it is**, which `flag_groups.rs:1616` records happening once already and
    /// which recurred on `zgc-page-evac` inside this round.
    pub(crate) reserved_bytes_sizing: AtomicBool,
...
impl ZArenaTlabRegistry {
    ...
    ///
    /// **Two rules live here, and they bound different quantities.** The
    /// default divides the budget by a *claimant count*; the one behind
    /// `CRATONVM_ZGC_TLAB_RESERVED_BYTES` subtracts the *bytes* already
    /// claimed. Both sections below are current; which one runs is
    /// [`Self::reserved_bytes_sizing`].
    ///
    /// # The claimant count (default)
    ///
    ...
    ///
    /// The divisor is **every** buffer claiming a chunk from this arena: this
    /// module's registered cells plus the VM threads' own buffers
    /// ([`Self::vm_tlab_slots`], and see that field for why the VM side is
    /// counted separately). Counting only the cells sized each VM thread as
    /// though it were alone, which is the failure
    /// [`ZGC_TLAB_RESERVATION_SHARE`] exists to prevent.
    ///
    /// For ordinary thread counts this changes nothing at all, and that is by
    /// construction rather than by luck: the quotient is clamped up to the
    /// `initial_chunk` ceiling, and at `-Xmx2g` that ceiling
    /// ([`ZGC_TLAB_MAX_CHUNK`], 512 KiB) is not reached until roughly 256
    /// buffers. Doubling a divisor of 8 to 16 leaves the answer pinned at the
    /// ceiling; it only starts to bite in the thread-explosion regime, which is
    /// the regime it is for.
    ///
    /// **Its residual is that the quantity bounded is not the quantity
    /// measured** — a count enforces a per-thread chunk size, not a total, and
    /// the total is what the budget is written in. See
    /// [`Self::reserved_bytes`] for the three ways that comes apart.
    ///
    /// # The byte budget (`CRATONVM_ZGC_TLAB_RESERVED_BYTES`, default off)
    ///
    /// With [`Self::reserved_bytes_sizing`] on, the divisor disappears
    /// entirely and the answer is `budget - reserved` — what is left of the
    /// budget after the chunks already outstanding. No thread count, no epoch,
    /// and no burst window at a collection, because [`Self::reserved_bytes`]
    /// does not reset at one: a chunk still held is still claimed, and one
    /// that was retired has already been subtracted by the retire that
    /// returned it.
    ///
    /// The series is self-limiting rather than divided: while
    /// `reserved <= budget - ceiling` every claimant gets the full ceiling, and
    /// past that the answer decays to `min_tlab_size()`. The **sum** of
    /// outstanding chunks is then `budget + (threads past the knee) x
    /// min_tlab_size()`, which is the bound the constant's own documentation is
    /// written about and which the count-based rule never actually enforced.
    pub(crate) fn chunk_bytes_now(&self, capacity: usize) -> usize {
        self.chunk_bytes_now_excluding(capacity, 0)
    }

    /// [`Self::chunk_bytes_now`], discounting `own_outstanding` bytes that the
    /// caller is **about to give back**.
    ///
    /// A buffer sizing its replacement chunk is still holding its current one,
    /// and `reserved_bytes` correctly counts it — but that chunk is retired a
    /// few lines later, so leaving it in the divisor would make a thread
    /// shrink its own successor with its own predecessor, once per refill,
    /// forever. `ZgcRealHeap::tlab_refill` passes its `ZArenaTlab::chunk`
    /// extent here; `zgc::vm_tlab::refill_tlab` has already released its
    /// record by the time it asks, so it passes nothing.
    ///
    /// Inert on the claimant-count arm, which has no byte to discount.
    pub(crate) fn chunk_bytes_now_excluding(
        &self,
        capacity: usize,
        own_outstanding: usize,
    ) -> usize {
        ...
        let want = if self.reserved_bytes_sizing.load(Ordering::Relaxed) {
            let reserved = self
                .reserved_bytes
                .load(Ordering::Relaxed)
                .saturating_sub(own_outstanding);
            budget.saturating_sub(reserved)
        } else {
            let live = self
                .live_slots
                .load(Ordering::Relaxed)
                .saturating_add(self.vm_tlab_slots.load(Ordering::Relaxed))
                .max(1);
            budget / live
        };
        let want = want.clamp(crate::tlab::min_tlab_size(), ceiling);
        want & !(ZGC_TLAB_ALIGN - 1)
    }

    /// Charge `bytes` to [`Self::reserved_bytes`]. Called once per carved
    /// chunk, from the single chunk source.
    #[inline]
    pub(crate) fn note_chunk_reserved(&self, bytes: usize) {
        let now = self
            .reserved_bytes
            .fetch_add(bytes, Ordering::Relaxed)
            .saturating_add(bytes);
        // `fetch_max` rather than load-compare-store: two threads charging at
        // once would otherwise each read the pre-charge value and the larger
        // of the two peaks would be lost -- on a thread-explosion workload,
        // which is the only regime this counter exists to describe.
        self.reserved_bytes_peak.fetch_max(now, Ordering::Relaxed);
    }

    /// Give `bytes` back to [`Self::reserved_bytes`]. Called once per chunk
    /// that stops being a chunk — a retire, a tail reclaim, or the same
    /// thread's next refill.
    ///
    /// Saturating rather than wrapping: a double release would otherwise wrap
    /// to `usize::MAX` and pin every later chunk at `min_tlab_size()` for the
    /// life of the process, which is a far worse failure than the double
    /// release itself. The callers are structured so it cannot happen (each
    /// release takes the extent out of the record that owns it, and the take
    /// is what authorises the release), but the arithmetic does not rely on
    /// that being audited correctly forever.
    #[inline]
    pub(crate) fn release_chunk_reserved(&self, bytes: usize) {
        if bytes == 0 {
            return;
        }
        let _ = self
    ...
    /// Count the calling thread as holding a VM-thread chunk, once per epoch.
    /// Called from `zgc::vm_tlab::refill_tlab`; `already` is the epoch this
    /// thread last counted itself in.
    ///
    /// Returns the epoch the thread is now counted in, which the caller stores
    /// back into its thread-local record.
    ///
    /// Relaxed throughout, and that is the whole ordering argument: this is a
    /// *sizing hint*, not a correctness signal. An over-count sizes a chunk
    /// small, an under-count sizes it large, and both are bounded by the same
    /// budget the divisor divides. Nothing dereferences it.
    #[inline]
    pub(crate) fn note_vm_tlab_slot(&self, already: u64) -> u64 {
        let epoch = self.vm_tlab_slot_epoch.load(Ordering::Relaxed);
        if already != epoch {
            self.vm_tlab_slots.fetch_add(1, Ordering::Relaxed);
        }
        epoch
    }

    /// Drop every thread's VM-buffer self-count and open a new epoch.
    ///
    /// Called from `retire_all_tlabs`, i.e. once per collection, alongside the
    /// prune that does the same job for [`Self::slots`]. A thread that is still
    /// alive re-counts itself on its next refill; one that has exited simply
    /// never does. See [`Self::vm_tlab_slots`] for why rebuilding beats
    /// decrementing.
    pub(crate) fn reset_vm_tlab_slots(&self) {
        self.vm_tlab_slots.store(0, Ordering::Relaxed);
        self.vm_tlab_slot_epoch.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub(crate) fn for_capacity(capacity: usize) -> Self {
        ...
        Self {
            ...
            vm_tlab_slots: AtomicUsize::new(0),
            // 1, not 0: a thread's zero-initialised record must never read as
            // "already counted in this epoch".
            vm_tlab_slot_epoch: AtomicU64::new(1),
            reserved_bytes: AtomicUsize::new(0),
            reserved_bytes_peak: AtomicUsize::new(0),
            // Read ONCE, here, and stored per registry rather than consulted
            // per refill: `flags::runtime_var_os`'s own census note is that an
            // uncached flag read on a repeated path *is* the bug. Per-registry
            // rather than in a `OnceLock` so two heaps in one process (which
            // the gc unit tests routinely create) can differ, and so a test can
            // override it without touching the environment.
            reserved_bytes_sizing: AtomicBool::new(cratonvm_types::flags::runtime_flag_on(
                "CRATONVM_ZGC_TLAB_RESERVED_BYTES",
            )),
        }
    }
    ...
    /// This thread's buffer, creating and registering it on first use, and
    /// running `f` against it.
    ///
    /// **No `Arc` refcount traffic on the cache-hit path.** The straight
    /// `attach() -> Arc<Mutex<ZArenaTlab>>` this replaced cloned the cached
    /// `Arc` on every call and dropped it at the end of the caller's scope —
    /// two atomic RMWs per allocation to hand back a pointer the calling
    /// thread's own `ZGC_TLAB_HANDLES` entry already keeps alive for the rest
    /// of the thread's life. Running `f` *inside* the `RefCell` borrow instead
    /// borrows the `Mutex<ZArenaTlab>` straight out of that entry: no clone,
    /// no drop, nothing but the plain load a thread-local read already is.
    /// The `Mutex` itself, and everyone who locks it, is unchanged — it is
    /// still what lets [`Self::cells`] (in turn `retire_all_tlabs`, called
    /// from both a safepoint and a concurrent, non-stopping walk) reach a
    /// buffer another thread is actively bumping.
    ///
    /// Callers must not, directly or through `f`, call `with_attached`,
    /// `attach` or push into `ZGC_TLAB_HANDLES` again for the SAME registry
    /// id while `f` is running: the fast path holds that entry's `RefCell`
    /// borrowed for `f`'s whole call, and a reentrant `borrow_mut` (the slow
    /// path's registration push) would panic. `f`'s only current bodies —
    /// `ZgcRealHeap::alloc_tlab`'s tail — lock a different `Mutex`
    /// (`ZArenaTlab`'s own cell) and touch `arena`/the object registry, never
    /// this registry, so this holds today; an auditor adding a second caller
    /// must recheck it.
    ///
    ...
    pub(crate) fn with_attached<R>(&self, f: impl FnOnce(&Mutex<ZArenaTlab>) -> R) -> R {
        let mut f = Some(f);
        let hit = ZGC_TLAB_HANDLES
            .try_with(|c| {
                let handles = c.borrow();
                handles
                    .iter()
                    .find(|(id, _)| *id == self.id)
                    .map(|(_, cell)| {
                        // SAFETY-FREE: `cell` borrows out of `handles`, which is
                        // still borrowed here, so this reference does not outlive
                        // it. `f` runs to completion inside that borrow.
                        let f = f.take().expect("with_attached: f consumed twice");
                        f(cell)
                    })
            })
            .ok()
...
impl ZgcRealHeap {
    ...
    /// **Arena bytes currently inside a live TLAB chunk**, across both buffer
    /// kinds — this module's registered cells and the VM threads' own buffers.
    ///
    /// The number nothing in this subsystem could answer before: `allocated`
    /// charges per object on one path and per chunk on the other, so neither
    /// half can say how much of the arena is claimed by a chunk and handed to
    /// no object. This is the quantity `ZGC_TLAB_RESERVATION_SHARE` is a
    /// budget for, and comparing it against `capacity / 16` is the whole test
    /// of whether that budget is being honoured.
    ///
    /// Maintained whether or not the byte-budget sizing is switched on, so it
    /// can be measured before it is acted on. See
    /// [`ZArenaTlabRegistry::reserved_bytes`].
    pub fn tlab_reserved_bytes(&self) -> usize {
        self.tlabs.reserved_bytes.load(Ordering::Relaxed)
    }

    /// **The largest total ever outstanding in chunks at one instant**, for
    /// this heap.
    ///
    /// This is the number that settles whether
    /// `CRATONVM_ZGC_TLAB_RESERVED_BYTES` should become the default, and it is
    /// the only one of the three that can: [`Self::tlab_reserved_bytes`] read
    /// at shutdown is ~0 because every thread has retired, and
    /// [`Self::tlab_reservation_budget_bytes`] is a constant. Read as a ratio
    /// against the budget:
    ///
    /// * `peak / budget` far above 1 on the **count** arm is the failure the
    ///   budget exists to prevent, present on the shipping default;
    /// * the same ratio near 1 on the **byte** arm is the byte budget doing
    ///   the job the constant is written about;
    /// * the two arms agreeing means the workload never reaches the regime,
    ///   and that run decides nothing.
    ///
    /// Monotone and maintained on both arms — see
    /// [`ZArenaTlabRegistry::reserved_bytes_peak`].
    pub fn tlab_reserved_bytes_peak(&self) -> usize {
        self.tlabs.reserved_bytes_peak.load(Ordering::Relaxed)
    }

    /// The byte budget `ZGC_TLAB_RESERVATION_SHARE` names, for this heap:
    /// `capacity / ZGC_TLAB_RESERVATION_SHARE`. Read beside
    /// [`Self::tlab_reserved_bytes`]; on its own it says nothing.
    pub fn tlab_reservation_budget_bytes(&self) -> usize {
        self.arena_end.saturating_sub(self.arena_base) / ZGC_TLAB_RESERVATION_SHARE
    }

    /// Is chunk sizing bounded by [`Self::tlab_reserved_bytes`] (the byte
    /// budget) rather than by a claimant count?
    ///
    /// `CRATONVM_ZGC_TLAB_RESERVED_BYTES`, default off — see
    /// [`ZArenaTlabRegistry::reserved_bytes_sizing`] for why the default is
    /// off and what flipping it obliges its author to do.
    pub fn tlab_reserved_bytes_sizing(&self) -> bool {
        self.tlabs.reserved_bytes_sizing.load(Ordering::Relaxed)
    }

    /// Test and A/B override of [`Self::tlab_reserved_bytes_sizing`].
    ///
    /// Takes effect at the next refill; chunks already outstanding are not
    /// resized (nothing can resize a chunk a thread is bumping into).
    pub fn set_tlab_reserved_bytes_sizing(&self, on: bool) {
        self.tlabs
            .reserved_bytes_sizing
    ...
    pub fn retire_all_tlabs(&self) -> ZArenaTlabRetireSummary {
        ...
        // The same job for the VM threads' own buffers, which have no cell here
        // to prune: drop the count and open a new epoch, so each live thread
        // re-counts itself on its next refill and a thread that has exited
        // simply stops being counted. See `ZArenaTlabRegistry::vm_tlab_slots`.
        self.tlabs.reset_vm_tlab_slots();
    ...
    pub(crate) fn tlab_retire_locked(&self, tlab: &mut ZArenaTlab) -> usize {
        ...
        // The chunk stops being a chunk here, so the reservation it charged at
        // `carve_tlab_chunk_inner` comes back. The WHOLE extent, not the tail:
        // the bytes below the cursor now belong to objects the registry knows
        // about, and the bytes above it are going to the free list two dozen
        // lines below. Neither is "inside a live chunk and handed to no
        // object" any more, which is what `reserved_bytes` counts.
        //
        // `chunk.take()` above is what authorises this: the release happens
        // exactly once per carve because a second retire finds no chunk and
        // returns before reaching this line -- the same idempotence the rest
        // of this function rests on.
        self.tlabs
            .release_chunk_reserved(chunk_end.saturating_sub(chunk_start));
    ...
    pub(crate) fn tlab_refill(&self, tlab: &mut ZArenaTlab, need: usize) -> Option<()> {
        // `need` is a REQUEST size; what the buffer will consume for it is the
        // aligned footprint. Stating the bound in the units the retry actually
        // spends is what makes this function's "the caller's retry is guaranteed
        // to fit" contract true for an arbitrary `need` rather than only for the
        // one caller whose `max_tlab_alloc` screen keeps it a long way below
        // `want`. Without it a `need` within seven bytes of the chunk size would
        // pass the gate, carve a fresh chunk, and then fail the retry anyway —
        // paying for a refill and taking `alloc_raw` regardless.
        let need = zgc_tlab_footprint(need);
        // Sized against what is claimed RIGHT NOW, not once at construction —
        // see `ZGC_TLAB_RESERVATION_SHARE`. A request too big for the current
        // chunk takes `alloc_raw`, which is where it went before TLABs existed.
        //
        // `_excluding` this buffer's own chunk: it is retired four lines below,
        // so counting it here would have a thread shrink its own successor with
        // its own predecessor on every refill. `None`/no chunk discounts zero.
        // (Retiring *before* the sizing would be the obvious alternative and is
        // wrong: the `want == 0 || need > want` refusal below leaves the buffer
        // holding a chunk it can still serve smaller requests from, and a
        // retire above it would throw that away to answer a question about a
        // chunk it is not going to get.)
        let own_chunk = tlab
            .chunk
            .map_or(0, |(start, end)| end.saturating_sub(start));
        let want = self
            .tlabs
            .chunk_bytes_now_excluding(self.arena_end.saturating_sub(self.arena_base), own_chunk);
        if want == 0 || need > want {
            return None;
        }
    ...
    pub(crate) fn carve_tlab_chunk(&self, want: usize, need: usize) -> Option<(*mut u8, usize)> {
        ...
        let carved = {
            ...
            let got = sized.or_else(|| arena.alloc(want, ZGC_TLAB_ALIGN).map(|p| (p, want)));
            // Which part of the chunk has never been written since the arena
            // was created, asked while the arena lock still pins the answer
            // to THIS hand-out. `None` unless the heap opted in.
            got.map(|(p, size)| (p, size, arena.take_last_hand_out_pristine(p)))
        };
        let (ptr, want, pristine) = carved?;
        ...
        //
        // SKIP THE BYTES THAT WERE NEVER WRITTEN (round 9 wave 10, `zgc10`).
        // The chunk memset was ~20 % of `CratonBench bintrees` -- a run that
        // never collects and carves every chunk from memory the OS has never
        // handed out, i.e. memory that already reads as zero. The arena knows
        // exactly which part of this hand-out is such memory (its pristine
        // window: never handed out, never a relocation or evacuation
        // destination -- see `Arena::pristine_lo`), and only the rest is
        // zeroed. Every consumer's "the chunk is zeroed" contract still holds
        // byte for byte; the zeroing of fresh pages is simply left to the OS
        // that already did it.
        let (skip_lo, skip_hi) = match pristine {
            Some((lo, hi)) => {
        ...
        if skip_lo < skip_hi {
            // SAFETY: `arena.alloc` guarantees `want` valid bytes at `ptr`, and
            // the span is exclusively ours until retire; both pieces lie
            // inside it (`skip_lo <= skip_hi <= want`).
            unsafe {
                std::ptr::write_bytes(ptr, 0, skip_lo);
                std::ptr::write_bytes(ptr.add(skip_hi), 0, want - skip_hi);
            }
            self.counters
                .tlab_pristine_bytes_skipped
                .fetch_add(skip_hi - skip_lo, Ordering::Relaxed);
        } else {
            // SAFETY: `arena.alloc` guarantees `want` valid bytes at `ptr`, and
            // the span is exclusively ours until retire.
        ...
        // THE ONE PLACE A CHUNK COMES INTO EXISTENCE on this backend, for both
        // buffer kinds -- this module's `ZArenaTlab` cells and the VM thread's
        // bare `Tlab` (`zgc::vm_tlab`). Charging the reservation here is what
        // makes `ZArenaTlabRegistry::reserved_bytes` complete by construction
        // rather than by an audit of the callers. Released by
        // `tlab_retire_locked` (cells) and by `vm_tlab`'s per-thread record
        // (the VM side); see the field.
        self.tlabs.note_chunk_reserved(want);
        Some((ptr, want))
    }
    ...
    pub(crate) fn alloc_tlab(&self, size: usize, init: &dyn Fn(*mut u8)) -> Option<*mut u8> {
        ...
        self.tlabs.with_attached(|cell| {
            ...
            // Registered INSIDE the cell lock, and before the pointer is
            // returned to anyone: `is_object_address` is a mutator-path
            // oracle on this heap, not just a GC one, so the window in which
            // a live object is absent from the registry has to be as narrow
            // as `alloc_raw`'s.
            // NOT the place to allocate black. `allocate_black_if_marking`
            // must run AFTER the header write -- `try_alloc_object`
            // `ptr::write`s the whole `ObjectHeader` over this address, and
            // on the header arm that clobbers the flags byte, so a bit set
            // here would be silently erased. The four callers that own the
            // header write are where it happens; the per-chunk blackening
            // below is what makes it cheap there.
            // The header BEFORE the registry insert, for the reason spelled
            // out at the twin call in `ZgcRealHeap::alloc_raw`: registration
            // is this heap's publication -- it is what `is_object_address`
            // answers from and what `collect_garbage`'s snapshot enumerates
            // -- so an address that is registered but not yet described is
            // one the sweep can size at `HEADER_SIZE` and free. The chunk is
            // zeroed by `tlab_refill`, so the window's reader sees an empty
            // object rather than garbage, which bounds the damage without
            // removing it.
            init(addr as *mut u8);
