...
//!
//! This is the *marking half* of the collector. It began as an unwired engine
//! beside `ZgcRealHeap`'s single-threaded `while let Some(addr) = work.pop()`
//! loop, and that is **no longer what it is** — the header said so until
//! 2026-09-20, three landings after it stopped being true, which is the exact
//! failure mode this subsystem's docs keep warning about.
//!
//! ## Where this engine is actually wired today
//!
//! All three through `impl ZMarkContext for ZgcRealHeap` (`gc/src/zgc.rs`):
//!
//! * **Parallel stop-the-world mark — DEFAULT ON.** `ZgcRealHeap::mark_parallel_stw`
//!   builds a [`ZMarkCoordinator`] inside the collection pause and runs
//!   [`mark_to_completion`](ZMarkCoordinator::mark_to_completion) against it.
//!   `CRATONVM_ZGC_PARMARK=0` is the kill switch and restores the serial loop,
//!   which is still there and is still the fallback for every refusal.
//! * **Concurrent mark — OPT-IN** (`CRATONVM_ZGC_CONC_START`).
//!   `ZgcRealHeap::start_concurrent_mark` opens a cycle at the mark-start
//!   safepoint, keeps the pool in `conc_pool`, and
//!   `ZgcRealHeap::finish_concurrent_mark` drives the mark-end handshake at the
//!   next collection. Mutators feed it through an SATB **pre-write** barrier,
//!   not through a load barrier — see the section below, which is the single
//!   most important thing to know before reading the termination protocol.
//! * **The `zgc_concurrent` driver** ([`crate::zgc_concurrent`]) drives the same
//!   pool on its own thread for the stop-the-world parallel path as well.
//!
//! The serial loop in `collect_garbage` remains, and remains the thing every
//! fail-closed path falls back to. What is gone is the claim that it is the only
//! thing that runs.
//!
...
//!
//! `mark_active` itself **is** set in production now (it was not when the
//! sentence below was first written): `ZgcRealHeap::start_concurrent_mark` arms
//! it whenever `CRATONVM_ZGC_CONC_START` opens a cycle. So the SATB slow path
//! is live code on an opt-in configuration, and the `ZMarkIngress` it feeds is
//! the heap's own instance rather than this pool's — `finish_concurrent_mark`
//! replays it through [`ZMarkCoordinator::push_roots`], which is what restores
//! the "everything published as mark work is already marked" invariant that
//! [`ZMarkHandle::mark_live`] maintains on this pool's own ingress.
//!
...
/// How often, in drain-loop steps, the drain loop tests the safepoint yield
/// flag. See [`ZMarkPauseControl`] for the latency this implies.
///
/// A "step" is one object for an ordinary object and one *chunk* for an object
/// whose context implements [`ZMarkContext::visit_refs_chunked`], which is what
/// makes the bound hold for a huge reference array as well.
pub const Z_MARK_YIELD_CHECK_INTERVAL: usize = 64;

/// Strong out-edges a worker asks [`ZMarkContext::visit_refs_chunked`] for in
/// one call, i.e. the granularity at which a huge reference array becomes
/// interruptible.
///
/// 4,096 slots is roughly 32 KiB of reference payload: large enough that the
/// per-chunk bookkeeping (one `Option` write, one loop head, one pause test) is
/// far under a percent of the work, and small enough that a chunk's marking
/// stays in the low tens of microseconds — so `64 * cost(chunk)` is a real
/// bound rather than a restatement of the array length.
///
/// Advisory: the default [`ZMarkContext::visit_refs_chunked`] ignores it and
/// scans the whole object, so on a context that has not opted in this constant
/// has no effect whatsoever.
pub const Z_MARK_REF_CHUNK: usize = 4096;

/// `CRATONVM_ZGC_MARK_REF_CHUNK`: override [`Z_MARK_REF_CHUNK`].
///
/// Latched on first read rather than re-read per drain, because this sits one
/// level above the hottest loop in the collector and an environment lookup
/// there would cost more than the chunking saves. `0` and unparseable values
/// fall back to the default; there is no "disable" value, because disabling is
/// what *not overriding* [`ZMarkContext::visit_refs_chunked`] already does.
pub fn ref_chunk_budget() -> usize {
    static LATCH: std::sync::OnceLock<usize> = std::sync::OnceLock::new();
    *LATCH.get_or_init(|| {
        cratonvm_types::flags::runtime_var_os("CRATONVM_ZGC_MARK_REF_CHUNK")
            .and_then(|raw| raw.to_string_lossy().trim().parse::<usize>().ok())
            .filter(|n| *n > 0)
            .unwrap_or(Z_MARK_REF_CHUNK)
    })
}
...
pub trait ZMarkContext: Send + Sync {
    ...
    /// [`visit_refs`](Self::visit_refs), **resumably**: visit at most `budget`
    /// of this object's strong out-edges starting at `cursor`, and return the
    /// cursor to resume from, or `None` when the object is finished.
    ///
    /// # Why this exists (2026-09-21)
    ///
    /// *Closes
    /// `docs/internal/zgc-round-20260920/gap-a-huge-reference-arrays-bound-the-yield-latency.md`.*
    ///
    /// [`ZMarkPauseControl`] documents the worst-case time-to-yield as
    /// `64 x cost(visit_refs)`, and then names its own tail: a `visit_refs` on a
    /// million-element `Object[]` is **one indivisible call**, so it alone can
    /// exceed any budget. That is not a throughput problem. Every mark-end
    /// safepoint in `ZgcConcurrentMarkController::drive_to_mark_end` goes
    /// through [`ZMarkPauseControl::wait_until_quiescent`], which spins and
    /// never gives up, **with the VM's mutators already stopped** — so the array
    /// scan lands directly inside the stop-the-world pause that concurrent
    /// marking exists to shorten, and `ZPhaseClock` attributes it to "the mark"
    /// with no further detail. A `HashMap` table, a `String` intern table and an
    /// `ArrayList` backing array are all this shape.
    ///
    /// OpenJDK's answer is `ZMark::follow_array`, which splits an object array
    /// into fixed-size pieces and pushes each as its own unit of mark work. This
    /// is the same split without widening the queue: the engine's stacks,
    /// stripes and ingress stay `u64` object bases — doubling the element width
    /// of the hottest data structure in the collector to carry a cursor that is
    /// zero for every object but a handful is the wrong trade — and the cursor
    /// lives in **one** in-progress slot per worker instead.
    ///
    /// # The default is `visit_refs`, so no implementor changes
    ///
    /// The provided body calls [`visit_refs`](Self::visit_refs) once and returns
    /// `None`, i.e. "this object was finished in one call". A context that has
    /// no large objects, or that has simply not been taught to chunk, is
    /// therefore byte-for-byte unaffected — including in which order its edges
    /// are reported. Chunking is opt-in **per implementor**, by override.
    ///
    /// # Contract for an override
    ///
    /// * `cursor` is opaque to the engine. It is whatever the previous call
    ///   returned, and `0` on the first call for an object.
    /// * Every edge must be reported exactly once across the whole sequence of
    ///   calls, and the same `skip_for(addr)` referent hole must be honoured on
    ///   every chunk — a referent that is skipped on chunk 0 and reported on
    ///   chunk 1 is a `WeakReference` that never clears.
    /// * `budget` is advisory; returning `Some` after fewer edges is legal,
    ///   returning `None` early is **not** — it drops the rest of the object.
    /// * A returned cursor must be **strictly greater** than the one passed in.
    ///   A cursor that stands still is an infinite loop in the drain, which
    ///   presents as a GC hang rather than as a wrong answer; a `debug_assert`
    ///   in [`ZMarkWorker::drain`] catches it in debug builds.
    /// * The object's mark bit is already set (everything queued as mark work
    ///   has been claimed), so a partially-scanned object is never re-claimed by
    ///   another worker. The obligation to finish it belongs to exactly one
    ///   worker and is held in that worker's in-progress slot.
    ///
    /// # Why a partial scan is safe against a mutator
    ///
    /// The ingress is an SATB **pre-write** barrier: a mutator that overwrites
    /// a slot in the not-yet-scanned tail of a half-scanned array publishes the
    /// *overwritten* value before the store lands, so the value this worker will
    /// never see is already on its way to [`ZMarkIngress`]. A mutator that
    /// stores a *new* reference into that tail either stores an object that is
    /// already marked, or one allocated during the cycle, which is allocated
    /// black. Neither case needs the scan to be atomic. Without the concurrent
    /// path the question does not arise at all, because no mutator is running.
    fn visit_refs_chunked(
        ...
        // `cursor == 0` rather than unconditional: the default never returns
        // `Some`, so the engine can never ask it to resume — but if some future
        // caller does, visiting the whole object a second time would double
        // every edge rather than finish it.
        if cursor == 0 {
    ...
    /// [`is_in_heap`](Self::is_in_heap) and [`try_mark`](Self::try_mark) as one
    /// call, for the per-EDGE path.
    ///
    /// # Why this exists (2026-09-20)
    ///
    /// [`ZMarkWorker::drain`]'s child callback is the hottest code in the
    /// collector: it runs once per reference slot of every live object, and it
    /// made **two** virtual calls through this trait for each one — `is_in_heap`
    /// to reject a wild pointer, then `try_mark` to claim the object. Both are
    /// indirect calls (the engine holds `&dyn ZMarkContext`), so neither can
    /// inline, and on `ZgcRealHeap` the first is a registry probe and the second
    /// is a header CAS on a line the first did not touch.
    ///
    /// Fusing them halves the dispatch and, more usefully, gives an implementor
    /// **one place** to do the two questions together — `ZgcRealHeap` can locate
    /// the object once and answer both from that, instead of being asked twice
    /// about the same address with nothing carried between the calls.
    ///
    /// # The default is the exact composition, so overriding is optional
    ///
    /// The provided body is `is_in_heap` then `try_mark`, in that order, with
    /// the same short-circuit. An implementor that does not override it gets
    /// byte-for-byte the behaviour the split calls had, which is why this could
    /// be added without touching any implementor.
    ///
    /// **An override must keep the order.** `try_mark` dereferences; asking it
    /// about an address `is_in_heap` would have refused is a wild-pointer
    /// dereference, which is the whole reason the gate is there.
    fn claim_child(&self, addr: u64) -> ZChildClaim {
        if !self.is_in_heap(addr) {
            return ZChildClaim::OffHeap;
        }
        if self.try_mark(addr) {
            ZChildClaim::Claimed
        } else {
            ZChildClaim::AlreadyMarked
        }
    }
...
/// What [`ZMarkContext::claim_child`] decided about one child pointer.
///
/// Three outcomes rather than a `bool`, because the caller does three different
/// things with them and two of them are indistinguishable in a `bool`:
/// `OffHeap` is a *refusal* and is counted as
/// [`ZMarkStats::off_heap_children`], while `AlreadyMarked` is the ordinary,
/// overwhelmingly common answer that makes a cyclic graph terminate. Collapsing
/// them would make a wild-pointer storm invisible.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ZChildClaim {
    /// [`ZMarkContext::is_in_heap`] refused the address. Nothing was marked and
    /// nothing may be dereferenced; the engine counts it and moves on.
    OffHeap,
    /// In the heap, and some other worker (or a mutator) already owns the
    /// obligation to scan it.
    AlreadyMarked,
    /// In the heap, newly marked by this call: the caller now owns the
    /// obligation to scan it and **must** push it onto a work stack.
    Claimed,
}
...
pub struct ZMarkStats {
    ...
    /// Times [`ZMarkContext::visit_refs_chunked`] returned `Some`, i.e. the
    /// number of times an object was left half-scanned and resumed.
    ///
    /// **Zero is the expected value and is not a failure.** The default
    /// `visit_refs_chunked` never returns `Some`, so a context that has not
    /// opted into chunking reports zero forever; a context that has opted in
    /// reports zero on a workload with no large reference arrays. What a
    /// nonzero value says is "the yield-latency tail
    /// `gap-a-huge-reference-arrays-bound-the-yield-latency.md` describes is
    /// real on this workload, and it is now interruptible". Reading it against
    /// [`objects_scanned`](Self::objects_scanned) gives the per-cycle cost of
    /// the split: one extra loop head and one `Option` write per count.
    pub ref_chunks: AtomicU64,
}
...
pub struct ZMarkStatsSnapshot {
    ...
    #[allow(missing_docs)]
    pub late_marks: u64,
    /// See [`ZMarkStats::ref_chunks`]. Zero means "no object needed splitting",
    /// which on a context that has not overridden
    /// [`ZMarkContext::visit_refs_chunked`] is the only possible answer.
    pub ref_chunks: u64,
}

impl ZMarkStats {
    /// Zero every PER-CYCLE counter: what a POOL REUSED ACROSS CYCLES must do
    /// before each one, because its readers ask "how much did THIS cycle do?".
    ///
    ...
    ///
    /// # `mark_end_restarts` and `yields` used to be left behind, and the doc
    /// claimed otherwise
    ///
    /// *Corrected 2026-09-20.* This method said "zero every counter" and then
    /// listed sixteen of the nineteen. The three omissions were not annotated
    /// as deliberate anywhere, and two of them are read per cycle:
    /// `mark_end_restarts` is the number the mark-end warning calls "marking is
    /// losing the race with the application", and a cumulative value makes that
    /// judgement about the *process* while printing it against one collection.
    /// `yields` is the same shape. Both are reset here now.
    ///
    /// # `late_marks` is deliberately NOT reset, and that is the one exception
    ///
    /// It counts calls that marked an object **with no cycle in flight** — i.e.
    /// calls that necessarily arrive *between* `end_cycle` and the next
    /// `begin_cycle_with`. Resetting it at the head of the next cycle would
    /// erase exactly the window it measures, so a persistent pool would report
    /// `late_marks == 0` forever no matter how badly the barrier's gate was
    /// wired. It is a wiring-defect counter about the process, not a workload
    /// counter about a cycle; see [`ZMarkStats::late_marks`].
    pub fn reset(&self) {
        for counter in [
    ...
    pub fn snapshot(&self) -> ZMarkStatsSnapshot {
        ZMarkStatsSnapshot {
            ...
            ref_chunks: self.ref_chunks.load(Ordering::Relaxed),
        }
    }
}
...
impl ZMarkStripeSet {
    ...
    /// [`publish`](Self::publish) from a borrowed slice, copying rather than
    /// moving.
    ///
    /// # Why this exists (2026-09-20)
    ///
    /// [`ZMarkShared::publish_distributed`] used to spread its batch over the
    /// stripes by building one `Vec<u64>` **per stripe** and `append`ing each —
    /// so a publish of `k` addresses allocated `stripe_count()` vectors, between
    /// 8 and 128 of them, whatever `k` was. That is paid on a *mutator* thread:
    /// `ZgcRealHeap::hand_satb_batch_to_the_marker` routes every SATB batch
    /// through `ZMarkCoordinator::push_roots`, which is this path. Allocating a
    /// hundred vectors to hand over a dozen addresses is not a rounding error on
    /// a store-heavy workload, and it happens on the thread that is trying to run
    /// the application.
    ///
    /// Taking a slice removes the intermediate entirely: the caller keeps one
    /// buffer and hands out sub-slices of it. The stripe still grows, but it is
    /// a `Vec` that lives for the cycle and reaches its steady-state capacity
    /// after the first few publishes.
    pub fn publish_slice(&self, key: usize, work: &[u64]) -> usize {
        if work.is_empty() {
            return 0;
        }
        let idx = self.index_for(key);
        let mut stripe = self.stripes[idx].lock();
        stripe.extend_from_slice(work);
        work.len()
    }
...
impl ZMarkTerminator {
    ...
    pub(crate) fn note_work_published(&self) {
        let mut g = self.state.lock();
...
///
/// ```text
///   64 x cost(ZMarkContext::visit_refs_chunked)
/// ```
///
/// which for ordinary objects (a handful of slots) is a few microseconds.
///
/// **The known tail was a huge reference array** — `visit_refs` on a
/// million-element `Object[]` is one indivisible call, so it alone could exceed
/// the budget, and since every mark-end safepoint waits here with the mutators
/// *already stopped*, that time was added straight to a stop-the-world pause.
/// OpenJDK splits large object arrays into fixed-size pieces
/// (`ZMark::follow_array`); [`ZMarkContext::visit_refs_chunked`] is the same
/// split, and [`ZMarkWorker::drain`] tests this flag between chunks.
///
/// **The bound is only as good as the context, and that is deliberate.** The
/// default `visit_refs_chunked` is one `visit_refs` call, so a context that has
/// not been taught to chunk still has the old tail — the engine handles opaque
/// `u64` bases and does not know what an array is. Chunking is a property an
/// implementor opts into; what this module now guarantees is that opting in
/// requires no change here and costs the common path nothing.
///
...
impl ZMarkShared {
    ...
    /// Is this pool shutting down — has [`ZMarkCoordinator::shutdown`] or its
    /// `Drop` asked the workers to leave?
    ///
    /// # Why this is public (2026-09-20)
    ///
    /// Because the *driver* in `crate::zgc_concurrent` has a stop flag of its
    /// own and, until this existed, could not see this one. The two are
    /// genuinely different flags — a caller may abandon a cycle without tearing
    /// the pool down, and may tear the pool down without abandoning a cycle —
    /// but a driver parked in `await_fixed_point` on a pool whose workers have
    /// already left is waiting for a fixed point nobody will ever declare
    /// (`ZMarkTerminator::worker_idle` returns `Terminated` on the stop flag
    /// **without** setting `terminated`, precisely so a stop does not look like
    /// a converged mark). That is an unbounded wait with a 5 ms poll, which is
    /// this tree's classic "GC hang presenting as an unexplained freeze".
    ///
    /// No production path reaches it today — the driver holds its own
    /// `Arc<ZMarkCoordinator>`, so the pool cannot be dropped underneath it —
    /// but "unreachable by construction elsewhere" is not a property this module
    /// wants to rest a liveness argument on.
    #[inline]
    pub fn is_stopping(&self) -> bool {
        self.should_stop.load(Ordering::Acquire)
    }

    /// Publish a batch spread evenly across the stripes, then notify.
    ///
    /// Spread rather than "all into one stripe" so the pool starts with every
    /// worker able to find work in its *own* stripe — the first steal then only
    /// happens once the graph shape has actually made the load uneven, instead
    /// of immediately and always.
    ///
    /// # Contiguous chunks, not round-robin (changed 2026-09-20)
    ///
    /// This used to deal the batch out one address at a time (`i % n_stripes`)
    /// into `n_stripes` freshly-allocated vectors. Two things were wrong with
    /// that and neither is visible in the mark set:
    ///
    /// * **It allocated `stripe_count()` vectors per call** — 8 to 128 of them —
    ///   regardless of how many addresses there were, and this call is on the
    ///   *mutator* path (`ZgcRealHeap::hand_satb_batch_to_the_marker` hands every
    ///   SATB batch to `push_roots`, which lands here). See
    ///   [`ZMarkStripeSet::publish_slice`].
    /// * **It touched every stripe even for a two-address batch**, so a small
    ///   publish took `n_stripes` uncontended-at-best mutexes, each of which a
    ///   worker's steal sweep may be sitting on.
    ///
    /// Chunking gives the same property the round-robin was chosen for — each
    /// participating stripe gets an equal share, so no worker starts empty — and
    /// it gives one more: a stripe's entries are now *contiguous in discovery
    /// order*, which is the order the DFS locality argument on
    /// [`ZMarkLocalStack`] is already built around. Only `min(k, n_stripes)`
    /// stripes are touched, so a batch smaller than the stripe count no longer
    /// pays for the stripes it has nothing to put in.
    ///
    /// The mark SET is identical either way — this decides which worker reaches
    /// an object first, never whether it is reached.
    fn publish_distributed(&self, addrs: Vec<u64>) {
        if addrs.is_empty() {
            return;
        }
        let n_stripes = self.stripes.stripe_count();
        // `div_ceil` over the number of stripes we will actually use, so the
        // chunk count never exceeds `n_stripes` and the last chunk is the short
        // one rather than an extra stripe's worth.
        let groups = n_stripes.min(addrs.len());
        let per_group = addrs.len().div_ceil(groups);
        let mut published = 0usize;
        for (i, chunk) in addrs.chunks(per_group).enumerate() {
            published += self.stripes.publish_slice(i, chunk);
        }
        if published > 0 {
...
impl ZMarkHandle {
    ...
    ///
    /// *(A `TODO(zgc_concurrent)` stood here asking that module to name
    /// [`new_buffer`](Self::new_buffer) in its `flush_mutator_buffers` contract.
    /// It has said so since before this pass — see that module's numbered step 2
    /// and [`ZgcMarkSafepoint`](crate::zgc_concurrent::ZgcMarkSafepoint)'s
    /// `# Contract` — so the TODO was stale and is removed. A TODO that has
    /// already been done is worse than none: the next reader re-derives the work
    /// before discovering it is not needed.)*
...
pub struct ZMarkWorker {
    ...
    rng: u64,
    last_cycle: u64,
    /// The one partially-scanned object this worker owes the cycle, as
    /// `(base, cursor)`. See [`ZMarkContext::visit_refs_chunked`].
    ///
    /// **Exactly one per worker, never one per queue entry.** The work stacks,
    /// stripes and ingress all stay `u64` bases; a cursor that is zero for every
    /// object but a handful does not belong in the hottest data structure in the
    /// collector. The consequence is that a partial scan is *not* stealable —
    /// the worker that started an array finishes it — which is the right trade
    /// while the case is rare, and is why the chunk budget is 4,096 slots rather
    /// than a handful.
    ///
    /// **It is work, and it counts as such everywhere.** A worker holding one is
    /// not idle ([`Self::has_work`]), must not offer termination, and must not
    /// carry it into another cycle.
    in_progress: Option<(u64, usize)>,
}
...
impl ZMarkWorker {
    ...
    /// Does this worker still owe the cycle anything?
    ///
    /// A non-empty local stack **or** a half-scanned object. The second half is
    /// the one that is easy to forget: an object whose mark bit is set but whose
    /// out-edges are half traced is invisible to every other worker (nobody can
    /// re-claim it) and invisible to the stripes (it is not in one), so a
    /// termination handshake offered while holding it would close the cycle with
    /// live edges untraced — and the sweep would then free their targets.
    #[inline]
    pub fn has_work(&self) -> bool {
        !self.local.is_empty() || self.in_progress.is_some()
    }

    /// Give up a half-scanned object by putting its **base** back on the local
    /// stack, so an ordinary publish can hand it to anyone.
    ///
    /// The cursor is discarded, so the object is re-scanned from the start.
    /// That is wasteful and it is correct: `visit_refs_chunked` reports each
    /// edge again, every one of those children is already marked, and
    /// `claim_child` answers `AlreadyMarked` for all of them. Losing the cursor
    /// costs one re-scan; losing the object costs a use-after-free.
    ///
    /// Used only where the worker is about to stop being the owner of it — a
    /// cycle boundary or a pool shutdown — never for a safepoint pause, where
    /// the worker parks and resumes with the cursor intact.
    #[inline]
    fn spill_in_progress(&mut self) {
        if let Some((addr, _cursor)) = self.in_progress.take() {
    ...
    pub fn run(&mut self) {
        ...
        loop {
            ...
            // A half-scanned object belongs to the cycle it was claimed in, and
            // its cursor means nothing in a new one (the mark bits have been
            // cleared and the graph has moved on). Turn it back into a plain
            // base FIRST, so the publish below cannot leave it stranded in a
            // field nobody else can see.
            self.spill_in_progress();
    ...
    fn run_cycle(&mut self, shared: &Arc<ZMarkShared>, my_cycle: u64) {
        loop {
            ...
            // Outstanding work after a zero-scan drain means the drain bailed
            // for a pause. Go round the loop so the pause branch above handles
            // it; do NOT fall through to the handshake, which would offer
            // termination while still holding work.
            //
            // `has_work`, not `!local.is_empty()`: a drain that stopped in the
            // middle of a huge array finishes with an EMPTY local stack and a
            // half-scanned object in `in_progress`, and that combination used to
            // be indistinguishable from "done". See `ZMarkWorker::has_work`.
            if scanned > 0 || self.has_work() {
                continue;
            }
    ...
    /// Scan up to `budget` objects from the local stack. Returns how many were
    /// **finished**; `0` means "local stack empty", "yielded", or "spent the
    /// whole budget inside one huge object and has it in `in_progress`".
    ///
    /// # Two budgets, because an object is no longer a unit of work
    ///
    /// *Changed 2026-09-21 with [`ZMarkContext::visit_refs_chunked`].*
    ///
    /// `scanned` counts objects **completed**, and is what the caller and
    /// [`ZMarkStats::objects_scanned`] mean. `steps` counts loop iterations —
    /// one per ordinary object, one per *chunk* of a chunked one — and is what
    /// the yield check and the loop bound run off.
    ///
    /// Using `scanned` for the yield check (as this did) is the bug the whole
    /// change exists to fix, and it is worse than it looks: mid-way through a
    /// million-element array `scanned` is frozen, so `scanned % 64 == 0` is
    /// decided once and for all by whatever value it froze at. Frozen at a
    /// multiple of 64 the check fires every iteration; frozen anywhere else it
    /// **never fires again for the whole object**. Counting steps makes the
    /// cadence independent of what the object turned out to be.
    ///
    /// Bounding the loop by `steps` as well keeps a single pathological object
    /// from holding the drain past its budget: the worker returns with `0`
    /// finished and its cursor parked, and `run_cycle` — which now asks
    /// [`has_work`](Self::has_work) rather than looking at the local stack —
    ...
    pub fn drain(&mut self, budget: usize) -> usize {
        ...
        let mut chunks_here: u64 = 0;
        while scanned < budget && steps < budget {
            if steps % Z_MARK_YIELD_CHECK_INTERVAL == 0 && pause.pause_requested() {
                break;
            }
            steps += 1;
            // RESUME BEFORE POPPING. A half-scanned object is an obligation this
            // worker already owns and nobody else can take; taking a fresh one
            // in front of it would let the in-progress slot be overwritten and
            // the object's remaining edges dropped.
            let (addr, cursor) = match self.in_progress.take() {
                Some(resume) => resume,
                None => match self.local.pop() {
                    Some(a) => (a, 0usize),
                    None => break,
                },
            };
            ...
            let next = {
                ...
                // The default body of `visit_refs_chunked` is one `visit_refs`
                // returning `None`, so for every context that has not opted into
                // chunking this is the identical call it always was, with the
                // identical edge order. See that method.
                ctx.visit_refs_chunked(addr, cursor, chunk, &mut |child: u64| {
                    if child == 0 {
                        return;
                    }
                    // ONE virtual call per edge, not two. `claim_child`'s
                    // default body is exactly `is_in_heap` then `try_mark`, so
                    // this is the same decision it always was; what changed is
                    // that an implementor can now answer both halves from one
                    // lookup. See `ZMarkContext::claim_child`.
                    match ctx.claim_child(child) {
                        // Raw reference-slot bytes can be anything at all when
                        // a mutator is mid-store; refuse rather than
                        // dereference. `ZgcRealHeap::collect_garbage` keeps the
                        // same counter under the name `wild_skipped`.
                        ZChildClaim::OffHeap => *off_heap += 1,
                        ZChildClaim::AlreadyMarked => {}
                        ZChildClaim::Claimed => {
                            *marked += 1;
                            local.push(child);
            ...
            match next {
                // More of this object to come. Park the cursor; the loop head
                // re-tests the pause flag before the next chunk, which is the
                // whole point of the split.
                Some(resume_at) => {
                    // A cursor that does not advance is an infinite loop: this
                    // drain would spend its whole step budget on one object and
                    // `run_cycle` would come straight back in, forever, with the
                    // pool never terminating. That presents as a GC hang, which
                    // is the failure mode this tree diagnoses most often and
                    // least easily.
                    debug_assert!(
                        resume_at > cursor,
                        "ZMarkContext::visit_refs_chunked returned a cursor that did \
                         not advance ({cursor} -> {resume_at}) for object {addr:#x}"
                    );
                    self.in_progress = Some((addr, resume_at));
                    chunks_here += 1;
                }
                None => scanned += 1,
            }
        ...
        if chunks_here > 0 {
            stats.ref_chunks.fetch_add(chunks_here, Ordering::Relaxed);
        }
        pause.leave_drain();
        scanned
    }
    ...
    pub fn acquire_work(&mut self) -> bool {
        ...
        //
        // `1..n_stripes`, not `1..=n_stripes`: the stripe count is a power of
        // two and `index_for` masks, so `offset == n_stripes` lands back on
        // `own` and is discarded by the guard below. The inclusive form ran one
        // extra iteration to throw it away, on the path a worker takes
        // immediately before offering itself for termination.
...
pub struct ZMarkCycleReport {
    ...
    /// `true` if the loop left because the POOL was told to stop, rather than
    /// because the mark converged.
    ///
    /// # Why this is a separate field and not folded into `budget_exhausted`
    ///
    /// *Added 2026-09-20.* [`ZMarkCoordinator::mark_to_completion`] has always
    /// had a `should_stop` escape at the head of its loop, and it reported the
    /// resulting report with `budget_exhausted == false` — i.e. **indis­tinguishable
    /// from success**. Its one production caller
    /// (`ZgcRealHeap::finish_concurrent_mark`) branches on exactly that field to
    /// decide whether the mark set may be swept against, so a stop landing
    /// mid-loop would have certified a partial mark set. That is a
    /// use-after-free, and the fact that it is currently unreachable (the caller
    /// holds the only `Arc`, so nothing can drop the pool underneath it) is a
    /// property of the caller rather than of this function.
    ///
    /// Two fields rather than one because they mean different things to an
    /// operator: an exhausted budget says *the mutators are out-running the
    /// marker*, and a stop says *somebody tore the pool down*. Only the
    /// consequence is shared — neither mark set may be swept against.
    pub stopped_early: bool,
    /// Counters at the end of the run.
    pub stats: ZMarkStatsSnapshot,
}

impl ZMarkCycleReport {
    /// May a sweep act on the mark bits this run produced?
    ///
    /// `true` only when the loop converged on its own. The two failure
    /// flags are separate because they have different operator meanings (see
    /// [`Self::stopped_early`]), but a caller deciding whether to trust the
    /// mark set wants one question, not two — and a caller that asks only one
    /// of them is the defect this accessor exists to make unnecessary.
    #[inline]
    pub fn mark_set_complete(&self) -> bool {
        !self.budget_exhausted && !self.stopped_early
    }
}
...
impl ZMarkCoordinator {
    ...
    pub fn push_roots(&self, roots: &[u64]) -> usize {
        let batch = self.claim_unmarked(roots);
    ...
    /// Gate, claim and collect: the loop [`push_roots`](Self::push_roots),
    /// [`dbg_push_roots_to_stripe`](Self::dbg_push_roots_to_stripe) and
    /// [`process_non_strong_refs`](Self::process_non_strong_refs) all need.
    ///
    /// Returns the subset of `addrs` this call **newly marked** — i.e. exactly
    /// the addresses whose scan obligation the caller now owns and must publish
    /// somewhere. Null and out-of-heap addresses are dropped silently, matching
    /// `ZgcRealHeap::collect_garbage`'s `registered.contains(&addr)` gate.
    ///
    /// *Factored 2026-09-20.* The three callers carried three textually
    /// identical copies of it. That is the shape in which one copy quietly stops
    /// matching the others — and the thing they share is not a convenience, it
    /// is the invariant that **everything published as mark work has already had
    /// its mark bit set**, which is what stops the same object being scanned
    /// twice and what makes the ingress's "already marked" contract true.
    fn claim_unmarked(&self, addrs: &[u64]) -> Vec<u64> {
        let ctx_arc = self.shared.context_arc();
        let ctx = &*ctx_arc;
        let mut batch: Vec<u64> = Vec::with_capacity(addrs.len());
        for &addr in addrs {
            if addr == 0 {
                continue;
            }
            if ctx.claim_child(addr) == ZChildClaim::Claimed {
                batch.push(addr);
            }
        }
        batch
    ...
    /// Hand a batch of addresses discovered by a **mutator** to the markers,
    /// through the ingress rather than through the work stripes.
    ///
    /// Returns how many were newly marked and queued.
    ///
    /// # Why this is not [`push_roots`](Self::push_roots) (added 2026-09-20)
    ///
    /// It does the same claiming — gate on `is_in_heap`, take the object with
    /// the atomic `try_mark`, drop whatever some other worker already owns — so
    /// the "everything queued as mark work is already marked" invariant holds
    /// identically. What differs is **where the survivors go**, and that is the
    /// whole point.
    ///
    /// [`ZMarkIngress`]'s own docs state the rule this method exists to honour:
    ///
    /// > If mutators published straight into [`ZMarkStripeSet`], every
    /// > load-barrier slow path would contend with the workers' steal path on
    /// > the same mutexes, and the steal path is the one that runs when the pool
    /// > is *starved* — i.e. exactly when added latency hurts most.
    ///
    /// `push_roots` publishes straight into the stripes, which is right for the
    /// root set (pushed once, at a safepoint, with no worker running) and wrong
    /// for a mutator batch. `ZgcRealHeap::hand_satb_batch_to_the_marker` calls
    /// `push_roots` today — from an arbitrary application thread, every
    /// `Z_SATB_HANDOFF_PER_BUCKET` reference stores, while the pool is tracing —
    /// so the SATB hand-off takes up to `stripe_count()` stripe mutexes per
    /// batch on the thread that is trying to run the application. This method is
    /// one uncontended bucket mutex instead.
    ///
    /// # What the caller gives up, and why it is nothing
    ///
    /// Work in the ingress is picked up by [`ZMarkWorker::acquire_work`]'s
    /// **last** resort rather than its first, so a busy pool drains it later
    /// than it would drain a stripe. That is the intended prioritisation — the
    /// workers have local work — and it costs no completeness: the termination
    /// probe asks [`ZMarkIngress::has_work`] under the same lock as the stripes,
    /// so a fixed point can never be declared over a non-empty ingress, and
    /// [`try_end_mark`](Self::try_end_mark) folds the ingress into the stripes at
    /// the mark-end safepoint.
    pub fn push_mutator_batch(&self, slot: usize, addrs: &[u64]) -> usize {
        let mut batch = self.claim_unmarked(addrs);
        if batch.is_empty() {
            return 0;
        }
        let n = self.shared.ingress.push_batch(slot, &mut batch);
        self.shared
            .stats
            .ingress_pushes
            .fetch_add(n as u64, Ordering::Relaxed);
        // The ingress lock is released inside `push_batch`; taking the
        // terminator's lock is legal here and nowhere earlier. See the module
        // header's locking discipline — the inverse order is an ABBA deadlock.
        self.shared.terminator.note_work_published();
        n
    }
    ...
    pub fn dbg_push_roots_to_stripe(&self, stripe_key: usize, roots: &[u64]) -> usize {
        let mut batch = self.claim_unmarked(roots);
        let marked = batch.len();
        self.shared
            .stats
            .roots_marked
    ...
    pub fn mark_to_completion(&self, max_restarts: usize) -> ZMarkCycleReport {
        ...
        loop {
            ...
            if self.shared.should_stop.load(Ordering::Acquire) {
                // NOT a success. Until 2026-09-20 this `break` fell into the
                // same report a converged loop produces, so a pool torn down
                // mid-mark reported a complete mark set to a caller that then
                // swept against it. See `ZMarkCycleReport::stopped_early`.
                stopped_early = true;
                tracing::warn!(
                    target: "zgc",
                    passes,
                    restarts,
                    "ZGC mark: mark_to_completion left because the pool was stopped; the \
                     mark set is INCOMPLETE and must not be swept against"
                );
                break;
            }
    ...
    pub fn process_non_strong_refs(&self, hook: &dyn ZNonStrongRefHook) -> usize {
        ...
        let batch = self.claim_unmarked(&keep);
...
pub struct TestMarkContext {
    ...
    /// Opt into [`ZMarkContext::visit_refs_chunked`]. `None` — the default —
    /// keeps the inherited body, i.e. one `visit_refs` per object, which is
    /// what every existing test asserts against. `Some(n)` reports at most `n`
    /// children per call regardless of the engine's budget, so a test can force
    /// a split without building a 100,000-child node.
...
impl TestMarkContext {
    ...
    /// Report at most `limit` children per
    /// [`ZMarkContext::visit_refs_chunked`] call, so a node with more than
    /// `limit` children is scanned in several resumable pieces.
    ///
    /// `limit` is this context's own cap, applied *in addition to* the budget
    /// the engine passes, so a test can force a split with a ten-child node
    /// instead of a 100,000-child one. Without this call the context keeps the
    /// inherited `visit_refs_chunked`, which never splits — which is what makes
    /// every other test in this module an assertion about the unchunked path.
    pub fn with_chunked_visits(mut self, limit: usize) -> Self {
        assert!(limit > 0, "a zero chunk limit would never finish an object");
...
impl ZMarkContext for TestMarkContext {
    ...
    /// Resumable form. Inherited (one `visit_refs`, `None`) unless
    /// [`with_chunked_visits`](Self::with_chunked_visits) opted in.
    fn visit_refs_chunked(
        &self,
        addr: u64,
        cursor: usize,
        ...
        // The visit hook fires ONCE per object, on the first chunk, so a test
        // that counts visits counts objects either way.
        if cursor == 0 {
            if let Some(hook) = &self.visit_hook {
                hook(addr);
            }
...
mod tests {
    ...
    // -- chunked reference scanning -----------------------------------------

    /// The default [`ZMarkContext::visit_refs_chunked`] must be exactly one
    /// [`ZMarkContext::visit_refs`] that finishes the object.
    ///
    /// This is the property that makes the method addable: every existing
    /// implementor inherits it, so nothing changes for a context that has not
    /// opted in. Asserted directly rather than inferred from a mark set,
    /// because "the mark set is the same" would also hold for a default that
    /// visited the object twice.
    #[test]
    fn the_inherited_chunked_visit_is_one_unsplit_visit_refs() {
        /// A context that overrides **only** the required methods, so
        /// `visit_refs_chunked` really is the trait's provided body and not a
        /// copy of it. `TestMarkContext` overrides the method (to offer
        /// `with_chunked_visits`) and therefore cannot answer this question.
        struct BareContext;
        impl ZMarkContext for BareContext {
            fn good_mask(&self) -> u64 {
                Z_MARKED0
            }
            fn try_mark(&self, _addr: u64) -> bool {
            ...
            fn visit_refs(&self, _addr: u64, f: &mut dyn FnMut(u64)) {
                for child in [2u64, 3, 4] {
                    f(child);
                }
            }
            fn is_in_heap(&self, _addr: u64) -> bool {
        ...
        let next = ctx.visit_refs_chunked(1, 0, 1, &mut |c| seen.push(c));
        assert_eq!(
            next, None,
            "the inherited body must report the object FINISHED, whatever budget \
             it is given — a `Some` here would park a cursor no override exists \
             to consume"
        );
        assert_eq!(sorted(seen), vec![2, 3, 4], "and must report every edge");

        // And it must not double-report if it is ever asked to resume.
        let mut again = Vec::new();
        assert_eq!(
            ctx.visit_refs_chunked(1, 7, 1, &mut |c| again.push(c)),
            None
        );
        assert!(
            again.is_empty(),
            "a nonzero cursor into the inherited body must be a no-op, not a \
             second full visit"
        );
    }

    /// Splitting an object changes nothing about what is reached.
    ///
    /// Two identical graphs, one context chunking at 7 children per call and
    /// one not, marked by real worker pools. The mark sets must be equal — that
    /// is the whole correctness claim of
    /// `gap-a-huge-reference-arrays-bound-the-yield-latency.md`'s fix: every
    /// edge is reported exactly once **across** the chunks, so an object that is
    /// half-scanned when a worker leaves is finished by the same worker and
    /// never dropped.
    ///
    /// The counters are the second half: the chunked arm must actually have
    /// split (or the equality is vacuous), and the plain arm must not have
    /// (or the inherited default has stopped being the default).
    #[test]
    fn chunked_and_unchunked_scans_produce_the_same_mark_set() {
        // One 200-child hub, which is the shape the gap page is about, scaled
        // down to a chunk limit rather than to a real array length.
        let children: Vec<u64> = (2..=201u64).collect();
        let g = graph(&[(1, children.as_slice())]);

        let plain = Arc::new(TestMarkContext::new(g.clone()));
        let plain_pool = ZMarkCoordinator::new(plain.clone(), 2);
        plain_pool.begin_cycle();
        assert_eq!(plain_pool.push_roots(&[1]), 1);
        let plain_report = plain_pool.mark_to_completion(RESTART_BUDGET);
        plain_pool.end_cycle();

        let chunky = Arc::new(TestMarkContext::new(g).with_chunked_visits(7));
        let chunky_pool = ZMarkCoordinator::new(chunky.clone(), 2);
        chunky_pool.begin_cycle();
        assert_eq!(chunky_pool.push_roots(&[1]), 1);
        let chunky_report = chunky_pool.mark_to_completion(RESTART_BUDGET);
        chunky_pool.end_cycle();

        assert!(!plain_report.budget_exhausted && !chunky_report.budget_exhausted);
        assert_eq!(
            chunky.marked_sorted(),
            plain.marked_sorted(),
            "chunking must not change what is reachable"
        );
        assert_eq!(chunky.marked_count(), 201, "the hub plus every child");

        assert_eq!(
            plain_report.stats.ref_chunks, 0,
            "the inherited visit never parks a cursor"
        );
        // 200 children at 7 per call is 29 calls, the last of which finishes
        // the object and returns `None`.
        assert_eq!(
            chunky_report.stats.ref_chunks, 28,
            "one continuation per call that did NOT finish the hub"
        );
        assert_eq!(
            chunky_report.stats.objects_scanned, 201,
            "`objects_scanned` counts objects FINISHED, not chunks — a drain \
             that conflated the two would make every huge array look like \
             hundreds of objects"
        );

        plain_pool.shutdown();
        chunky_pool.shutdown();
    }

    /// A safepoint taken while an object is half scanned must lose nothing.
    ///
    /// The in-progress cursor lives in a worker-private field that no stripe
    /// and no other worker can see, so a drain that dropped it on the way out
    /// to a pause would leave an object **marked** with its remaining
    /// out-edges untraced — the exact shape that closes a mark set over a live
    /// subgraph and then sweeps it. `run_cycle` therefore asks
    /// [`ZMarkWorker::has_work`] rather than looking at the local stack, and
    /// this test is what holds that in place.
    ///
    /// Like `workers_yield_for_a_safepoint_and_resume_without_losing_work`, it
    /// asserts on the mark **set** and never on timing: a run in which no
    /// safepoint happened to land mid-array degrades coverage instead of
    /// failing.
    #[test]
    fn a_safepoint_while_an_object_is_half_scanned_loses_no_edges() {
        let children: Vec<u64> = (2..=5001u64).collect();
        let g = graph(&[(1, children.as_slice())]);
        let ctx = Arc::new(TestMarkContext::new(g).with_chunked_visits(4));
        let pool = ZMarkCoordinator::new(ctx.clone(), 3);
        ...
        // Pause and resume repeatedly while the hub is being chunked through.
        // Each guard's construction is itself the assertion that every worker
        // reached a yield point — which, before chunking, a worker inside this
        // hub could not do until the whole 5000-slot scan had finished.
        for _ in 0..16 {
        ...
        let mut expected: Vec<u64> = vec![1];
        expected.extend(2..=5001u64);
        assert_eq!(
            ctx.marked_sorted(),
            expected,
            "a safepoint dropped the half-scanned hub's remaining edges"
        );
        assert!(
            pool.stats().snapshot().ref_chunks > 0,
            "the fixture must actually have split the hub, or this asserts nothing"
        );
        pool.shutdown();
    }
