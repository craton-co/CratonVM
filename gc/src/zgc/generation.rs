...
//!
//!   That rule now has an enforcement rather than only a sentence:
//!   `ZGenerationScope::note_if_colored` tests the single bit whose whole
//!   purpose is to answer "colored word or address?"
//!   (`vaddr::Z_COLORED_TAG`) on the *reject* path, counts every hit, logs
//!   once per scope and offers [`ZGenerationScope::assert_no_colored_roots`]
//!   to the driver. The one constant imported from `vaddr` is the tag; no
//!   offset mask, no metadata bit and no good mask crosses into this file, and
//!   nothing here converts anything. See that method for why the unenforced
//!   version of this rule is a platform-dependent use-after-free.
//!
//! # Scoping
//!
...
pub struct ZGenerationScope {
    ...
    rejected: AtomicU64,
    /// Of those, how many were **colored words** rather than addresses.
    ///
    /// See [`Self::colored_rejected`] for what this is and why a silent zero
    /// is the only acceptable value.
    colored_rejected: AtomicU64,
    /// Has the warning for [`Self::colored_rejected`] already been emitted for
    /// this scope? One line per scope, not one per edge: the failure is
    /// systematic — if one word is colored they all are — and a per-edge log
    /// on a cycle with millions of edges is a second outage.
    colored_warned: AtomicBool,
}
...
impl ZGenerationScope {
    ...
    pub fn from_pages(
        ...
        ZGenerationScope {
            ...
            colored_rejected: AtomicU64::new(0),
            colored_warned: AtomicBool::new(false),
        }
    }
    ...
    pub fn page_of(&self, addr: usize) -> Option<ZScopeEntry> {
        ...
        if hit.is_some() {
            ...
            // Nothing on the admit path; one test on the reject path.
            self.note_if_colored(addr);
        }
        hit
    }

    /// **The seam-4 tripwire.** A rejected address that carries
    /// [`crate::zgc::vaddr::Z_COLORED_TAG`] is not an out-of-generation pointer — it is a colored
    /// word that nobody converted, and every edge in the batch is about to be
    /// filtered away in silence.
    ///
    /// # Why this exists rather than a comment
    ///
    /// `adapters.rs`'s seam-4 header lists three decisions that must arrive
    /// with the edge-iteration capability and calls the third *"the
    /// highest-risk unassigned responsibility at this seam"*: **whether the
    /// yielded word is colored.** A reference slot in a real ZGC heap holds a
    /// `vaddr::ZColoredWord` — a 42-bit offset from the heap base plus four
    /// metadata bits plus [`crate::zgc::vaddr::Z_COLORED_TAG`] at bit 63 — while
    /// [`ZGenerationMarker::mark_from_roots`] documents its roots as uncolored
    /// machine addresses. Somebody has to strip the colour and add the base.
    ///
    /// Skipping that conversion **does not fail**. An offset lands below the
    /// heap base, `Self::find` answers `None`, and the edge is *filtered*.
    /// The remembered set then reads as empty, which is the exact shape of the
    /// use-after-free the module exists to prevent, and the only trace is a
    /// statistics counter reading `rejected == stores_seen`.
    ///
    /// **And the failure is platform-dependent.** On the Windows dev host the
    /// arena base is small, so an unconverted offset can still land inside the
    /// heap and the bug hides; on Linux the base is around `0x7f…` and every
    /// edge is dropped. A silent, platform-dependent edge filter is the kind
    /// of thing that ships, so this converts it into a counted, logged,
    /// debug-fatal event.
    ///
    /// # Why testing one bit is not "knowing the encoding"
    ///
    /// The module header says a colored word must never be handed to anything
    /// here, and it means it: this file decodes nothing, converts nothing, and
    /// does not import the offset mask, the metadata bits or the good mask.
    /// The single import is the **tag** — the bit whose entire purpose is to
    /// let a holder of a `u64` answer "is this a colored word or an address?"
    /// without a domain. Importing the constant rather than writing `1 << 63`
    /// is deliberate: the two must never drift, and a local copy is exactly
    /// how they would.
    ///
    /// No user-space machine address has bit 63 set on any platform this VM
    /// targets, so there is no false positive to trade against.
    #[inline]
    fn note_if_colored(&self, addr: usize) {
        let colored = (addr as u64) & crate::zgc::vaddr::Z_COLORED_TAG != 0;
        if !colored {
            return;
        }
        let seen = self.colored_rejected.fetch_add(1, Ordering::Relaxed) + 1;
        if !self.colored_warned.swap(true, Ordering::Relaxed) {
            tracing::error!(
                target: "cratonvm::gc::guard",
                addr = format_args!("{addr:#x}"),
                colored_rejected = seen,
                generation = ?self.generation,
                cycle = self.cycle,
                "zgc: a colored word reached the generation scope as a root. The \
                 colour/offset conversion at seam 4 (adapters.rs, decision 3) did not \
                 run, so EVERY cross-generation edge this cycle is being filtered \
                 silently and the remembered set will read as empty. On a large heap \
                 base this frees live objects.",
            );
        }
    }

    /// Rejected addresses that were colored words — see `Self::note_if_colored`.
    ///
    /// **Any non-zero value is a defect**, and specifically the one seam 4's
    /// decision 3 warns about. Zero is not proof the conversion ran (a heap
    /// that stores plain machine pointers, which is every heap in this tree
    /// today, cannot produce a colored word in the first place); non-zero is
    /// proof it did not.
    pub fn colored_rejected(&self) -> u64 {
        self.colored_rejected.load(Ordering::Relaxed)
    }

    /// **Fail the cycle if the colour conversion did not run.** Panics when
    /// [`Self::colored_rejected`] is non-zero.
    ///
    /// The counter and the log line make the failure visible; this makes it
    /// *stop the run*, which is the right trade for a fault whose alternative
    /// outcome is freeing live objects on one platform and appearing healthy
    /// on another. Call it once, at the end of a cycle, from the driver that
    /// owns the seam — not from the marker's inner loop, where a panic would
    /// unwind through a pause.
    ///
    /// Left to the driver rather than asserted inside [`Self::page_of`] on
    /// purpose: a scope is also built by tests and by diagnostics that
    /// deliberately offer junk addresses, and only the driver knows whether
    /// this cycle's roots were supposed to have been converted.
    pub fn assert_no_colored_roots(&self) {
        let seen = self.colored_rejected();
        assert!(
            seen == 0,
            "zgc: {seen} colored words were offered to a generation scope as roots. \
             The seam-4 colour/offset conversion (adapters.rs, decision 3) did not run, \
             so every cross-generation edge this cycle was filtered silently. Refusing \
             to continue -- on a heap base above 2^42 this frees live objects and the \
             only symptom is an empty remembered set.",
        );
    }
...
impl ZGenerationalPolicy {
    ...
    pub(crate) fn note_minor_complete(&self, used_after: usize, capacity: usize) {
    ...
    /// both floors move.
    pub(crate) fn note_major_complete(
        &self,
...
impl ZYoungGeneration {
    ...
    /// `(used bytes, live bytes, page count)` from **one** walk under **one**
    /// lock acquisition.
    ///
    /// # Why this exists rather than three getters
    ///
    /// [`ZGenerationalHeap::should_collect`] calls
    /// [`ZGenerationalHeap::stats`], and `stats` used to call [`Self::used`],
    /// [`Self::live_bytes`] and [`Self::page_count`] in turn — three separate
    /// `parking_lot::Mutex` acquisitions and two full O(pages) walks of the map,
    /// per generation, per call. `should_collect` is the *allocation poll*: the
    /// question asked after an allocation to decide whether to collect. Paying
    /// six lock acquisitions and four map walks to answer it puts an
    /// O(heap pages) term on the allocation path, which is precisely the shape
    /// of cost a generational split exists to remove.
    ///
    /// The three figures are also not independent — they are three reductions
    /// of the same map — so taking them apart could report a page count from
    /// one instant and a byte total from another. One walk is both cheaper and
    /// more coherent.
    ///
    /// The single-fact getters are kept: they read better at a call site that
    /// wants one number, and they now delegate here.
    pub fn usage(&self) -> (usize, usize, usize) {
        let map = self.pages.lock();
        let mut used: usize = 0;
        let mut live: usize = 0;
        for page in map.values() {
            used = used.saturating_add(page.used());
            live = live.saturating_add(page.live_bytes());
        }
        (used, live, map.len())
    }
...
impl ZOldGeneration {
    ...
    /// `(used bytes, live bytes, page count)` from one walk under one lock.
    /// The old-side twin of [`ZYoungGeneration::usage`]; see that method for
    /// why the allocation poll cannot afford three of them.
    pub fn usage(&self) -> (usize, usize, usize) {
        let state = self.state.lock();
        let mut used: usize = 0;
        let mut live: usize = 0;
...
pub struct ZMinorCycleReport {
    ...
    /// Remembered-set entries the view actually **yielded** this cycle,
    /// counted before the scope filter.
    ///
    /// Not [`ZRememberedSetView::entry_count`], which is an approximate table
    /// size and is `0` by default — the two answer different questions, and
    /// mixing them (this field used to be `max()` of the pair) reported a
    /// number that was neither, biased high.
    /// `roots_scanned - (caller roots admitted)` is how many of these survived
    /// the filter; the rest were stale cards.
    ...
    /// **Page ids this cycle moved young → old**, in promotion order.
    ///
    /// # Why the ids and not just the count (2026-09-20)
    ///
    /// Promotion creates an obligation the report previously made it impossible
    /// to discharge. The moment a page becomes old, every reference its objects
    /// hold into the young generation turns from a young→young edge — which the
    /// store barrier correctly filtered and which the young trace found on its
    /// own — into an old→young edge, which **only the remembered set can
    /// find**. `remembered.rs` states this as a hard rule on
    /// `ZStoreBarrier::on_promote` ("*Why this call is mandatory, not an
    /// optimisation*"): skip it and the next young cycle frees every young
    /// object reachable solely through a just-promoted parent.
    ///
    /// This module cannot make that call itself — it holds no barrier, no
    /// object layout and no remembered set, all deliberately (see the module
    /// header's "What this module does NOT do"). What it can do is *name the
    /// pages*, so the driver that owns all three can:
    ///
    /// 1. `ZRememberedSetTable::register_old_page_with_base_in(ns, id, base,
    ///    size)` for each — before step 2, or every field coarsens;
    /// 2. walk each page's objects and `ZStoreBarrier::on_promote` them.
    ///
    /// **`_in`, and pass the namespace.** The base-less
    /// `register_old_page_with_base` hard-codes
    /// `ZPageIdNamespace::Allocator`, and a driver whose ids are its own dense
    /// grid index — which `ZgcRealHeap`'s are — would trip the table's
    /// namespace-conflict `debug_assert!` on the first promoted page. The ids
    /// in this field are whatever numbering the caller's pages carry; only the
    /// caller knows which that is.
    ///
    /// Without this field the count was the only signal, and a count is not
    /// something you can iterate. That is why the obligation had no caller
    /// anywhere in the tree.
    pub promoted_page_ids: Vec<u64>,
    /// **Page ids this cycle freed**, i.e. returned to [`ZPageAllocator`].
    ///
    /// The other half of the same obligation, and the one the page-id lifecycle
    /// makes non-obvious: `ZPageAllocator` caches a freed Small/Medium page and
    /// hands it back **carrying its original id**, so a remembered set left
    /// registered under that id outlives the objects it describes and is handed
    /// straight back to the next page to wear the number. The driver calls
    /// `ZRememberedSetTable::remove` for each id here.
    ///
    /// A young page normally has no remembered set at all (young pages are
    /// absent from the table by construction), so this is empty-handed work on
    /// a minor cycle — but a major cycle's [`ZMajorCycleReport::old_freed_page_ids`]
    /// is the same list for old pages, where it is not.
    pub freed_page_ids: Vec<u64>,
}
...
pub struct ZMajorCycleReport {
    ...
    /// **Old page ids this cycle freed.** The driver must
    /// `ZRememberedSetTable::remove` each one: the page's remembered set
    /// describes objects that no longer exist, and the allocator may hand the
    /// same id back to a different page. See [`ZMinorCycleReport::freed_page_ids`].
    pub old_freed_page_ids: Vec<u64>,
}

// ---------------------------------------------------------------------------
// The nursery survivor census — one counter that can refute the young space
// ---------------------------------------------------------------------------

/// The nursery's address geometry for one cycle, in the shipping heap's own
/// terms.
///
/// [`ZgcRealHeap`](crate::zgc::ZgcRealHeap)'s nursery is `[floor, end)` inside
/// one arena, and its logical page grid is `Z_LOGICAL_PAGE_BYTES` starting at
/// the arena base — so a page index is `(addr - base) / page_bytes` and the
/// nursery covers a *suffix* of that grid. Passed as a struct rather than four
/// arguments because getting `base` and `floor` the wrong way round produces a
/// plausible-looking number rather than a failure.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ZNurseryGeometry {
    /// First byte of the address space the page grid is indexed from.
    pub base: usize,
    /// Logical page size. `Z_LOGICAL_PAGE_BYTES` (2 MiB) on the shipping heap.
    pub page_bytes: usize,
    /// First nursery byte — `gen_young_floor`.
    pub floor: usize,
    /// One past the last nursery byte — the arena's low cursor.
    pub end: usize,
}

impl ZNurseryGeometry {
    /// **Does this describe a nursery at all?**
    ///
    /// The struct's own doc says it exists because *"getting `base` and `floor`
    /// the wrong way round produces a plausible-looking number rather than a
    /// failure"* — and then [`Self::page_count`] used to do exactly that, by
    /// clamping `floor` up to `base` and carrying on. A driver that has not set
    /// its young floor yet passes `floor == 0`; with `base` at the arena, the
    /// clamp silently redefines the nursery as **the whole arena**, and every
    /// long-lived old object above the base is then counted as a nursery
    /// survivor. That is the census reading high — toward `Refuted` — for a
    /// reason that has nothing to do with the workload, which is precisely the
    /// "wrong in both directions at once" failure this tree has already paid
    /// for once with an allocation counter.
    ///
    /// So a `floor` below `base` is now **malformed, not clamped**:
    /// `page_count` answers `0`, [`ZNurserySurvivorCensus::finish_cycle`]
    /// counts the cycle as rejected rather than as evidence, and the operator
    /// sees a non-zero `geometry_rejected` instead of a number.
    ///
    /// This asks about the **frame**, not the extent: `floor == base` (the
    /// nursery is the whole grid, deliberately), `end == floor` (the nursery is
    /// empty) and even `end < floor` (the low cursor retracted below the floor
    /// after a compaction) are all well-formed descriptions of a nursery that
    /// happens to be empty, and [`Self::page_count`] answers `0` for them
    /// without anybody having made a mistake. Only a `floor` the grid cannot
    /// index, or a zero page size, is a caller error.
    pub fn is_well_formed(&self) -> bool {
        self.page_bytes != 0 && self.floor >= self.base
    }

    /// Logical pages the nursery spans, `0` when the nursery is empty **or the
    /// geometry is malformed** ([`Self::is_well_formed`]).
    pub fn page_count(&self) -> usize {
        if !self.is_well_formed() || self.end <= self.floor {
            return 0;
        }
        let first = (self.floor - self.base) / self.page_bytes;
        let last = (self.end - 1 - self.base) / self.page_bytes;
        last + 1 - first
    }
}

/// What the census has seen so far.
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
pub struct ZNurserySurvivorReport {
    /// Young cycles that were **evidence** — a well-formed, non-empty nursery.
    /// This is the denominator [`ZNurserySurvivorCensus::MIN_CYCLES`] gates on.
    pub cycles: u64,
    /// **Young cycles offered to the census at all**, evidence or not.
    ///
    /// # Why `cycles == 0` is not one answer but three (2026-09-20, wave 3)
    ///
    /// [`ZNurserySurvivorCensus::finish_cycle`] deliberately does not count a
    /// cycle whose nursery was empty — counting it would drag the percentage
    /// toward `Supported`, and an instrument must not drift on its own. The
    /// side effect is that `cycles == 0` collapses three completely different
    /// situations into one reading:
    ///
    /// 1. nobody wired the census up (the call site does not exist);
    /// 2. it is wired and ran N times, and the nursery was empty every time;
    /// 3. it is wired and ran N times, and the geometry was malformed every
    ///    time (see [`Self::geometry_rejected`]).
    ///
    /// The handoff that wires this says *"a claim built on a run where
    /// `survivor_cycles` reads `0` is a claim about nothing"* — true, but an
    /// operator who cannot tell (1) from (2) cannot tell a broken instrument
    /// from a real observation about a heap whose young floor never moves, and
    /// that is the difference between "fix the wiring" and "the nursery is
    /// inert, which is itself the finding". `cycles_observed` separates them:
    /// `0` means not wired; non-zero with `cycles == 0` means wired and
    /// inert.
    pub cycles_observed: u64,
    /// Of [`Self::cycles_observed`], how many were refused because the
    /// geometry was malformed — see [`ZNurseryGeometry::is_well_formed`].
    ///
    /// **Any non-zero value is a wiring defect, not a measurement.** It means
    /// `floor` was below `base` (typically an unset young floor), which before
    /// wave 3 was silently clamped and turned the whole arena into "the
    /// nursery".
    pub geometry_rejected: u64,
    /// Logical nursery pages examined, summed over cycles.
    pub pages: u64,
    /// Of those, how many held **at least one** survivor.
    pub pages_with_survivor: u64,
    /// Survivor bytes credited to nursery pages, summed over cycles.
    pub survivor_bytes: u64,
    /// `survivor_page_percent` of the **first** evidence cycle, `0` when there
    /// has been none. See [`Self::ratchet_margin`].
    pub first_cycle_percent: u64,
    /// ...and of the most recent one.
    pub last_cycle_percent: u64,
    /// The smallest per-cycle percentage seen.
    pub min_cycle_percent: u64,
    /// The largest per-cycle percentage seen.
    pub max_cycle_percent: u64,
}

impl ZNurserySurvivorReport {
    /// Has anything called the census? `false` is a statement about the
    /// *wiring*, not about the heap — see [`Self::cycles_observed`].
    pub fn is_wired(&self) -> bool {
        self.cycles_observed > 0
    }

    /// Percentage of examined nursery pages that held a survivor, `0` when
    /// nothing has been examined. Integer, because a tenth of a percent has no
    /// bearing on the decision this number exists to make.
    pub fn survivor_page_percent(&self) -> u64 {
        if self.pages == 0 {
            return 0;
        }
        self.pages_with_survivor.saturating_mul(100) / self.pages
    }

    /// **How far the per-cycle percentage climbed** — `last - first`, in
    /// points, and `0` when there is fewer than one evidence cycle or when it
    /// did not climb.
    ///
    /// # What this is for, and it is the admissibility test for the whole
    /// measurement
    ///
    /// [`Self::survivor_page_percent`] sums `pages` and `pages_with_survivor`
    /// over cycles. That aggregate is only a fair answer to
    /// `proposal-d-real-young-space.md`'s question if each cycle is an
    /// independent observation of a *fresh* nursery. On this heap it is not —
    /// see "THE CARRY-OVER RATCHET" on [`ZNurserySurvivorCensus`]. The
    /// nursery floor moves only at a whole-heap collection, and promotion does
    /// not move an object out of the nursery's address range, so a page that
    /// held a survivor in cycle *N* still holds it in cycle *N+1*, and the
    /// per-page bit only ever goes from 0 to 1 within one major epoch. Later
    /// cycles also examine more pages, so they dominate the sum.
    ///
    /// The consequence is that the aggregate is biased **high** — toward
    /// `Refuted` — by an amount that has nothing to do with the workload's
    /// survival rate, and the size of that bias is exactly what this reports:
    ///
    /// * a **small** margin means the first cycle already read what the last
    ///   one read, the ratchet is not driving the number, and the aggregate
    ///   verdict stands on its own;
    /// * a **large** margin means the reading climbed as the nursery filled
    ///   with objects it will never release, and a `Refuted` taken from the
    ///   aggregate is a claim about the instrument rather than about the heap.
    ///
    /// Read [`Self::first_cycle_percent`] as the closest thing this census has
    /// to a marginal reading: it is the one cycle whose nursery contains only
    /// what was allocated since the floor last moved.
    pub fn ratchet_margin(&self) -> u64 {
        self.last_cycle_percent
            .saturating_sub(self.first_cycle_percent)
    }

    /// The verdict, by the rule stated on [`ZNurserySurvivorCensus`].
    ///
    /// **Not adjusted for the carry-over ratchet, deliberately.** The
    /// thresholds were fixed before the runs and this file's own argument is
    /// that they must not be chosen after the numbers are in; moving them now
    /// would be the same offence in the opposite direction. Read
    /// [`Self::ratchet_margin`] beside the verdict instead — it says whether
    /// the verdict is admissible, and it is a separate number precisely so
    /// that it cannot be confused with a re-scored one.
    pub fn verdict(&self) -> ZYoungSpaceVerdict {
        if self.cycles < ZNurserySurvivorCensus::MIN_CYCLES || self.pages == 0 {
            return ZYoungSpaceVerdict::NotEnoughData;
        }
        let pct = self.survivor_page_percent();
        if pct >= ZNurserySurvivorCensus::REFUTED_AT_PERCENT {
            ZYoungSpaceVerdict::Refuted
        } else if pct <= ZNurserySurvivorCensus::SUPPORTED_BELOW_PERCENT {
            ZYoungSpaceVerdict::Supported
        } else {
            ZYoungSpaceVerdict::Inconclusive
        }
    }
}

/// What the census says about building a page-backed young generation.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ZYoungSpaceVerdict {
    /// Too few young cycles, or no nursery pages, to say anything.
    NotEnoughData,
    /// Nearly every nursery page held a survivor. **Do not build the young
    /// space**: page-granular reclamation cannot release a page that holds
    /// one, so there is nothing for it to reclaim.
    Refuted,
    /// Most nursery pages held no survivor, so most of them could have been
    /// released without being visited. The programme is worth its cost.
    Supported,
    /// Between the two thresholds. Stage 2 would have to be measured rather
    /// than predicted.
    Inconclusive,
}

/// **Per nursery page, did it hold a survivor?** — the one counter that can
/// refute `proposal-d-real-young-space.md` before any of it is built.
///
/// # Why a counter is the first move and not the first line of code
///
/// The proposal's own abandonment table names the risk it would actually bet
/// on:
///
/// > `pages_released` is near zero — nearly every nursery page holds a
/// > survivor → the workload's survival rate defeats page-granular
/// > reclamation. **Abandon.**
///
/// and then says the decisive thing: *that is measurable before any of this is
/// built.* A page-backed young generation's entire economic argument is that a
/// page holding no survivor is released **without any of its objects being
/// visited**. If nearly every 2 MiB nursery page holds at least one survivor,
/// there are no such pages, the young pause stays proportional to what was
/// allocated rather than to what survived, and the programme is refuted for
/// the cost of this struct.
///
/// Note what is *not* being measured: not the survival **rate** (a nursery can
/// be 1% live and still have a survivor on every page — that is the bad case,
/// and a byte-ratio would hide it), and not the survivors themselves, which
/// the young cycle already counts. The question is strictly one bit per page.
///
/// # The decision rule, stated before the run
///
/// Written here rather than in a report, in the spirit of
/// [`crate::zgc::census`]'s own rule, so the thresholds cannot be chosen after
/// the numbers are in:
///
/// | `survivor_page_percent` over [`Self::MIN_CYCLES`] young cycles | verdict |
/// |---|---|
/// | ≥ [`Self::REFUTED_AT_PERCENT`] (90) | **Refuted.** Close the proposal. At 90% of pages holding a survivor, releasing the other 10% saves a tenth of a sweep that is 59% of the pause — under 6%, against a second allocator, two TLAB paths and a relocation dependency. |
/// | ≤ [`Self::SUPPORTED_BELOW_PERCENT`] (60) | **Supported.** 40% of nursery pages could have been dropped whole. Proceed to stage 1. |
/// | between | **Inconclusive.** Stage 2 is the measurement; do not argue it. |
///
/// **The number to read is `survivor_page_percent`, and the number that
/// refutes the direction is 90 or above.**
///
/// # Cost
///
/// One bit per 2 MiB of nursery per young cycle, plus one shift and one `or`
/// per surviving object — over a live set the cycle is already enumerating,
/// inside a pause that already exists. It cannot perturb what it measures,
/// which is the same property [`crate::zgc::census`]'s `HeapWalk` strategy is
/// chosen for.
///
/// # Reading it: three numbers, not one (2026-09-20, wave 3)
///
/// The verdict rests on `survivor_page_percent`, but a percentage is only
/// admissible once you know the instrument engaged. Read in this order:
///
/// 1. [`ZNurserySurvivorReport::cycles_observed`] — did anything call this at
///    all? `0` is a statement about the wiring, never about the heap.
/// 2. [`ZNurserySurvivorReport::geometry_rejected`] — any non-zero value means
///    a caller handed over a frame that does not describe a nursery, and the
///    cycles behind it are not evidence. See
///    [`ZNurseryGeometry::is_well_formed`].
/// 3. [`ZNurserySurvivorReport::cycles`] — of the cycles that were offered, how
///    many were evidence. `cycles == 0` with `cycles_observed > 0` and
///    `geometry_rejected == 0` is the honest "the nursery never filled".
///
/// Those first two exist because this instrument's failure mode is the one
/// this tree has already recorded — *the allocation counter was wrong in both
/// directions at once* — and because a single `0` that means "not wired",
/// "inert" and "misconfigured" at the same time will be read as whichever the
/// reader already believes.
///
/// # ⚠ THE CARRY-OVER RATCHET — read before quoting this number (2026-09-21, wave 4)
///
/// This census was read on 2026-09-21 and its reading **refuted the round's
/// largest proposed programme** (`survivor_page_pct` 98/100/98 against a
/// `>= 90` threshold fixed in advance;
/// `docs/internal/zgc-round-20260920/measurement-nursery-survivor-census-20260921.md`).
/// A refutation is load-bearing in a way a confirmation is not, so the
/// instrument was re-audited against the heap it measures. It has a bias, it
/// is in the direction of the conclusion it produced, and it is structural
/// rather than a bug:
///
/// 1. **The nursery floor moves only at a whole-heap collection.**
///    `ZgcRealHeap::gen_young_floor` is stored at the end of a *major* cycle
///    (`gc/src/zgc.rs`, "AND THIS IS WHERE THE NURSERY BOUNDARY MOVES"), and a
///    young cycle sweeps `[floor, cursor)`. So "the nursery" is *everything
///    allocated since the last major*, not since the last young cycle.
/// 2. **Promotion does not move an object out of it.** On this backend
///    promotion is a label on a header, not a copy into another space, so a
///    promoted — i.e. genuinely old — object still sits inside `[floor, end)`
///    and is still counted by [`ZNurseryCycleObserver::note_survivor`]. The
///    generational probe that produced the published numbers recorded 2.05M
///    promotions.
/// 3. **Therefore the per-page bit is monotone within a major epoch.** A page
///    that held a survivor in cycle *N* holds the same object in cycle *N+1*;
///    nothing evacuated it. And because [`Self::finish_cycle`] sums `pages`
///    and `pages_with_survivor` across cycles, the later cycles — which
///    examine the most pages — dominate the aggregate.
///
/// The aggregate percentage is therefore biased **toward `Refuted`** by an
/// amount that is a property of this nursery rather than of the workload. That
/// is the same artefact wave 3 caught coming through a different door: a
/// malformed floor clamped up to the arena base would have counted every
/// long-lived old object as a nursery survivor. That door was shut
/// ([`ZNurseryGeometry::is_well_formed`]). This one is a floor that is stale
/// *by design* between majors, and `geometry_rejected == 0` says nothing about
/// it.
///
/// **It does not follow that the refutation is wrong.** It follows that the
/// aggregate alone cannot distinguish two hypotheses:
///
/// * *the workload's survival rate defeats page-granular reclamation* — which
///   is what `proposal-d` would be refuted by; from
/// * *this nursery never releases a survivor, so the census ratchets* — which
///   is a statement about the instrument.
///
/// [`ZNurserySurvivorReport::ratchet_margin`] separates them, and
/// [`ZNurserySurvivorReport::first_cycle_percent`] is the closest thing here
/// to a marginal reading: the first evidence cycle after a major is the one
/// whose nursery holds only what was allocated since the floor moved. A run
/// whose first cycle already reads ~98% is a run where the ratchet is not
/// doing the work and the refutation stands on its own. A run that climbs from
/// 40 to 98 is a run that refuted nothing.
///
/// **The published measurement was taken before these fields existed**, so it
/// does not carry them. Re-running it is cheap — same probe, same flags — and
/// until it is re-run, the correct standing of `proposal-d` is *refuted on an
/// aggregate whose admissibility has not been checked*, not *refuted*.
///
/// What would remove the bias rather than measure it: hand `note_survivor` the
/// object's age, and count only objects young enough not to have been
/// promoted. That is a signature change with a `zgc.rs` call site, so it is
/// filed rather than done —
/// `docs/internal/zgc-round-20260920/handoff-s-nursery-census-per-cycle.md`.
///
/// # Wiring
///
/// `gc/src/zgc.rs`'s young cycle drives it (PHASE G, beside the young-cycle
/// counters), applying
/// `docs/internal/zgc-round-20260920/handoff-j-nursery-survivor-census.md`.
#[derive(Debug, Default)]
pub struct ZNurserySurvivorCensus {
    cycles: AtomicU64,
    /// Every `finish_cycle`, evidence or not — see
    /// [`ZNurserySurvivorReport::cycles_observed`].
    cycles_observed: AtomicU64,
    /// Of those, refused for a malformed geometry.
    geometry_rejected: AtomicU64,
    /// One warning per census, not one per cycle: a malformed geometry is a
    /// wiring constant, so it is wrong on every cycle or none.
    geometry_warned: AtomicBool,
    pages: AtomicU64,
    pages_with_survivor: AtomicU64,
    survivor_bytes: AtomicU64,
    /// Per-cycle percentages — see [`ZNurserySurvivorReport::ratchet_margin`].
    /// All four are `0` until the first evidence cycle; `cycles` is what says
    /// whether that `0` is a reading.
    cycle_pct_first: AtomicU64,
    cycle_pct_last: AtomicU64,
    cycle_pct_min: AtomicU64,
    cycle_pct_max: AtomicU64,
}

impl ZNurserySurvivorCensus {
    /// Young cycles required before the verdict is anything but
    /// [`ZYoungSpaceVerdict::NotEnoughData`]. Three, because the first young
    /// cycle after a whole-heap collection sees a nursery that has only just
    /// started filling and its page count is not representative.
    pub const MIN_CYCLES: u64 = 3;
    /// At or above this percentage of nursery pages holding a survivor, the
    /// young-space programme is refuted.
    pub const REFUTED_AT_PERCENT: u64 = 90;
    /// At or below this percentage, it is supported.
    pub const SUPPORTED_BELOW_PERCENT: u64 = 60;

    /// A census that has seen nothing.
    pub fn new() -> ZNurserySurvivorCensus {
        ZNurserySurvivorCensus::default()
    }

    /// Record one young cycle.
    ///
    /// `survivors` yields `(address, bytes)` for every object that was still
    /// live when the young sweep finished — the live set the cycle already
    /// holds. Addresses outside `[geometry.floor, geometry.end)` are ignored
    /// rather than rejected: an old object below the floor is not a nursery
    /// survivor and is not this instrument's business.
    ///
    /// Returns the cumulative report, so a caller can log it without a second
    /// call.
    pub fn observe_cycle<I: IntoIterator<Item = (usize, usize)>>(
        &self,
        geometry: ZNurseryGeometry,
        survivors: I,
    ) -> ZNurserySurvivorReport {
        let mut observer = ZNurseryCycleObserver::new(geometry);
        for (addr, bytes) in survivors {
            observer.note_survivor(addr, bytes);
        }
        self.finish_cycle(observer)
    }

    /// Start a cycle that will report its survivors one at a time.
    ///
    /// The form a sweep loop wants: no `Vec` of survivors is built, and the
    /// per-object cost is a compare, a shift and an `or` into a stack-resident
    /// bitmap. Pair with [`Self::finish_cycle`].
    pub fn begin_cycle(&self, geometry: ZNurseryGeometry) -> ZNurseryCycleObserver {
        ZNurseryCycleObserver::new(geometry)
    }

    /// Fold one cycle's observations in and return the cumulative report.
    pub fn finish_cycle(&self, observer: ZNurseryCycleObserver) -> ZNurserySurvivorReport {
        // Counted FIRST and unconditionally: this is the engagement counter,
        // and its whole job is to be non-zero the moment anything calls the
        // census, so that `cycles == 0` can be read as "wired and inert"
        // rather than "not wired". See
        // `ZNurserySurvivorReport::cycles_observed`.
        self.cycles_observed.fetch_add(1, Ordering::Relaxed);
        if !observer.geometry.is_well_formed() {
            // Not evidence, and not the same non-evidence as an empty nursery:
            // this is a caller passing a geometry that does not describe a
            // nursery (typically `floor` below `base`, i.e. an unset young
            // floor). Before wave 3 it was silently clamped, which redefined
            // the nursery as the whole arena and counted every old object as a
            // nursery survivor.
            self.geometry_rejected.fetch_add(1, Ordering::Relaxed);
            if !self.geometry_warned.swap(true, Ordering::Relaxed) {
                tracing::error!(
                    target: "cratonvm::gc::guard",
                    base = format_args!("{:#x}", observer.geometry.base),
                    floor = format_args!("{:#x}", observer.geometry.floor),
                    end = format_args!("{:#x}", observer.geometry.end),
                    page_bytes = observer.geometry.page_bytes,
                    "zgc nursery census: malformed geometry -- the young floor is below \
                     the grid base (typically an unset floor), or the page size is \
                     zero. The cycle is NOT counted as evidence: clamping the floor up \
                     to the base would redefine the nursery as the whole arena and \
                     count every long-lived old object as a nursery survivor, which \
                     reads as 'Refuted' for a reason that has nothing to do with the \
                     workload.",
                );
            }
            return self.report();
        ...
        if observer.pages == 0 {
            // A cycle with an empty nursery is not evidence in either
            // direction, so it is not counted as a cycle either. Counting it
            // would drag `survivor_page_percent` toward zero — i.e. toward
            // "Supported" — which is the direction an instrument must never
            // drift on its own. It IS counted in `cycles_observed` above, so
            // the difference between this and "nobody called us" is visible.
            return self.report();
        }
        let with_survivor: u32 = observer.seen.iter().map(|w| w.count_ones()).sum();
        let prior_cycles = self.cycles.fetch_add(1, Ordering::Relaxed);
        self.pages
            .fetch_add(observer.pages as u64, Ordering::Relaxed);
        self.pages_with_survivor
            .fetch_add(u64::from(with_survivor), Ordering::Relaxed);
        self.survivor_bytes
            .fetch_add(observer.survivor_bytes, Ordering::Relaxed);

        // THIS CYCLE'S OWN PERCENTAGE, kept separately from the sum.
        //
        // The sum is what the pre-registered rule scores, and it is biased
        // high by the carry-over ratchet (see the type docs). These four are
        // what say by how much. They are recorded here rather than derived
        // later because a cycle's own denominator is gone the moment it is
        // added to the running total.
        let pct = u64::from(with_survivor)
            .saturating_mul(100)
            .checked_div(observer.pages as u64)
            .unwrap_or(0);
        if prior_cycles == 0 {
            // `0` is also the "no evidence yet" value of these fields, so the
            // first cycle must *store* rather than `fetch_min` — otherwise a
            // first reading of 98 would be minned against the initial 0 and
            // the series would report a cycle that never happened.
            self.cycle_pct_first.store(pct, Ordering::Relaxed);
            self.cycle_pct_min.store(pct, Ordering::Relaxed);
        } else {
            self.cycle_pct_min.fetch_min(pct, Ordering::Relaxed);
        }
        self.cycle_pct_last.store(pct, Ordering::Relaxed);
        self.cycle_pct_max.fetch_max(pct, Ordering::Relaxed);
        self.report()
    }

    /// Everything seen so far.
    pub fn report(&self) -> ZNurserySurvivorReport {
        ZNurserySurvivorReport {
            cycles: self.cycles.load(Ordering::Relaxed),
            cycles_observed: self.cycles_observed.load(Ordering::Relaxed),
            geometry_rejected: self.geometry_rejected.load(Ordering::Relaxed),
            pages: self.pages.load(Ordering::Relaxed),
            pages_with_survivor: self.pages_with_survivor.load(Ordering::Relaxed),
            survivor_bytes: self.survivor_bytes.load(Ordering::Relaxed),
            first_cycle_percent: self.cycle_pct_first.load(Ordering::Relaxed),
            last_cycle_percent: self.cycle_pct_last.load(Ordering::Relaxed),
            min_cycle_percent: self.cycle_pct_min.load(Ordering::Relaxed),
            max_cycle_percent: self.cycle_pct_max.load(Ordering::Relaxed),
        }
    }
}

/// One young cycle in progress — see [`ZNurserySurvivorCensus::begin_cycle`].
///
/// Owned by the cycle, not shared: no atomics, no lock, and the bitmap is one
/// bit per 2 MiB of nursery (a 2 GB nursery is sixteen `u64`s). Hand it back
/// to [`ZNurserySurvivorCensus::finish_cycle`] when the sweep is done.
#[derive(Debug)]
pub struct ZNurseryCycleObserver {
    /// The nursery this cycle is observing.
    geometry: ZNurseryGeometry,
    /// Grid index of the first nursery page, so the bitmap is indexed from
    /// the floor rather than from the arena base.
    first_page: usize,
    /// Nursery pages this cycle covers.
    pages: usize,
    /// One bit per nursery page: "held at least one survivor".
    seen: Vec<u64>,
    /// Survivor bytes credited to nursery pages.
    survivor_bytes: u64,
}

impl ZNurseryCycleObserver {
    fn new(geometry: ZNurseryGeometry) -> ZNurseryCycleObserver {
        let pages = geometry.page_count();
        let first_page = if pages == 0 {
            0
        } else {
            (geometry.floor.max(geometry.base) - geometry.base) / geometry.page_bytes
        };
        ZNurseryCycleObserver {
            first_page,
            pages,
            seen: vec![0u64; pages.div_ceil(64)],
            survivor_bytes: 0,
            geometry,
        }
    }

    /// Record that `addr` (a `bytes`-long object) was still live when the
    /// young sweep finished.
    ///
    /// Addresses outside `[floor, end)` are **ignored, not rejected**: an old
    /// object below the floor is not a nursery survivor and is not this
    /// instrument's business. Silently dropping them is what keeps the
    /// percentage honest on a heap whose floor has stopped moving, which is
    /// the documented inert state of the current nursery and exactly the shape
    /// where counting them would bias the answer toward `Refuted`.
    #[inline]
    pub fn note_survivor(&mut self, addr: usize, bytes: usize) {
        if self.pages == 0
            || addr < self.geometry.floor
            || addr >= self.geometry.end
            || addr < self.geometry.base
        {
            return;
        }
        let page = (addr - self.geometry.base) / self.geometry.page_bytes;
        ...
        self.survivor_bytes = self.survivor_bytes.saturating_add(bytes as u64);
    }
...
impl ZGenerationalHeap {
    ...
    /// Snapshot both generations. O(pages), with **one** lock acquisition and
    /// **one** map walk per generation — see [`ZYoungGeneration::usage`] for
    /// why that matters to a method the allocation poll calls through
    /// [`Self::should_collect`].
    pub fn stats(&self) -> ZGenerationalStats {
        let (young_used, young_live, young_pages) = self.young.usage();
        let (old_used, old_live, old_pages) = self.old.usage();
        ZGenerationalStats {
            young_used,
    ...
    ///
    /// # ⚠ The caller's obligation when this promotes anything
    ///
    /// A promoted page's objects hold references that were young→young a moment
    /// ago and are old→young now. The store barrier filtered every one of them
    /// — correctly, while the parent was young — so after promotion the
    /// **remembered set is the only possible record of them**, and it does not
    /// have them yet. Nothing in this module can fix that: it owns no barrier,
    /// no object layout and no remembered set, all by design.
    ///
    /// So [`ZMinorCycleReport::promoted_page_ids`] names the pages, and the
    /// driver must, before mutators resume:
    ///
    /// 1. register each promoted page on the remembered-set table
    ///    (`ZRememberedSetTable::register_old_page_with_base_in`, passing the
    ///    namespace its own page ids are in — see
    ///    [`ZMinorCycleReport::promoted_page_ids`]), then
    /// 2. walk its objects and call `ZStoreBarrier::on_promote` for each, which
    ///    records every field still pointing into young.
    ///
    /// Skip it and the **next** young cycle frees every young object reachable
    /// solely through a just-promoted parent — silently, with no error and no
    /// coarsening. `remembered.rs` states the same rule from its side, on
    /// `ZStoreBarrier::on_promote`.
    ///
    /// [`ZMinorCycleReport::freed_page_ids`] is the mirror obligation:
    /// `ZRememberedSetTable::remove` each, because the allocator recycles a
    /// freed Small/Medium page under its original id.
    ///
    ...
    pub fn collect_young(
        ...
        // What the view ACTUALLY yielded this cycle, not `max()` of that and
        // the view's own `entry_count()`. Those are two different quantities —
        // `entry_count` is an approximate *table* size (its default impl is
        // `0`, and a bitmap-backed impl would answer with every bit set across
        // every old page, including ones this cycle's iteration never reached)
        // — so maxing them reported a figure that is neither, and reported it
        // high. A remembered-set entry count that reads high is exactly the
        // direction that would make a future "generational is working" reading
        // look better than the run was.
        out.remembered_entries = remembered_entries;
    ...
    pub fn collect_major(
        ...
        ZMajorCycleReport {
            ...
            old_freed_page_ids: old_out.old_freed_page_ids,
        }
    }
    ...
    fn sweep_young(
        ...
        // Invalidate again, AFTER the pages have left the map.
        //
        // The cycle already invalidated at its start, and under a strict
        // safepoint that is sufficient: no allocation happens between the two
        // points, so the cache is still empty here. It is done again anyway
        // because the window the first call leaves open is the nastiest failure
        // this type has. A page cached as `hot_small` and then freed or promoted
        // by this sweep keeps its id; the allocator can hand that same id
        // straight back out (`page.rs`'s free cache recycles Small/Medium pages
        // under their original id), and a stale cache hit would then SUPPRESS
        // the re-registration of a page that is no longer in the young map. The
        // result is a young page invisible to the next cycle's scope — so its
        // objects are never marked and are swept while live, with no assertion
        // anywhere.
        //
        // Two relaxed stores, at the end of a collection, to remove the
        // dependence of a soundness property on the caller's safepoint
        // discipline. Cheap at any price.
        self.young.invalidate_hot_pages();

        // Publish WHICH pages moved, not just how many. Both lists carry an
        // obligation the driver has to discharge against the remembered set —
        // see the field docs on `ZMinorCycleReport::promoted_page_ids` for why
        // a count was not enough and why that left the obligation with no
        // caller in the tree.
        out.freed_page_ids = dead_ids;
        out.promoted_page_ids = promote_ids;
        out
    }
    ...
    fn sweep_old(&self, old_pages: &[Arc<ZPageReal>], report: &ZMarkReport) -> OldSweepOutcome {
        ...
        out.old_freed_page_ids = dead_ids;
        out
    }
}
...
mod tests {
    ...
    // -- seam 4, decision 3: the colour tripwire ----------------------------

    /// A colored word offered as a root is COUNTED, not silently filtered.
    ///
    /// This is the fault `adapters.rs`'s seam-4 header calls the highest-risk
    /// unassigned responsibility at the remembered/generation seam: nobody
    /// strips the colour and adds the heap base, `admits` answers `false` for
    /// a bare 42-bit offset, and every cross-generation edge disappears with
    /// no error anywhere. The counter is what turns "the remembered set is
    /// empty" into "the conversion did not run".
    #[test]
    fn a_colored_word_offered_as_a_root_is_counted_not_silently_filtered() {
        let alloc = allocator();
        let page = alloc
            .alloc_page(ZPageSizeClass::Small, OBJ)
            .expect("a small page");
        let addr = page.alloc(OBJ, 8).expect("bump");
        let outside = alloc.end() + 0x1000;
        let scope = ZGenerationScope::from_pages(ZGeneration::Young, 1, &[page], false);

        // An ordinary in-scope address: admitted, and not colored.
        assert!(scope.admits(addr));
        assert_eq!(scope.colored_rejected(), 0);

        // An ordinary OUT-of-scope address: rejected, and still not colored.
        // The tripwire shares the reject path with every genuine
        // cross-generation pointer and must not fire on one.
        assert!(!scope.admits(outside));
        assert_eq!(scope.rejected(), 1);
        assert_eq!(scope.colored_rejected(), 0);

        // The same shape as a COLORED WORD: a 42-bit offset plus the tag.
        // This is what a real ZGC reference slot holds and what the adapter
        // hands over when decision 3 is skipped.
        let colored = crate::zgc::vaddr::color(0x1234, crate::zgc::vaddr::ZColor::Remapped);
        assert!(crate::zgc::vaddr::is_colored_word(colored));
        assert!(!scope.admits(colored as usize));
        assert_eq!(
            scope.colored_rejected(),
            1,
            "a colored word must be distinguishable from an out-of-generation \
             pointer; without that the only symptom of a skipped conversion is an \
             empty remembered set, which reads exactly like a heap with no \
             cross-generation edges",
        );
    }

    /// The driver-side refusal: a cycle that saw a colored root must not be
    /// allowed to report success.
    #[test]
    #[should_panic(expected = "colored words were offered")]
    fn assert_no_colored_roots_fails_the_cycle() {
        let alloc = allocator();
        let page = alloc
            .alloc_page(ZPageSizeClass::Small, OBJ)
            .expect("a small page");
        let scope = ZGenerationScope::from_pages(ZGeneration::Young, 1, &[page], false);
        let colored = crate::zgc::vaddr::color(0x40, crate::zgc::vaddr::ZColor::Marked0);
        assert!(!scope.admits(colored as usize));
        scope.assert_no_colored_roots();
    }

    /// ...and a clean cycle passes it, so the assertion is not vacuous.
    #[test]
    fn assert_no_colored_roots_passes_a_clean_cycle() {
        let alloc = allocator();
        let page = alloc
            .alloc_page(ZPageSizeClass::Small, OBJ)
            .expect("a small page");
    ...
    fn nursery(floor_pages: usize, span_pages: usize) -> ZNurseryGeometry {
        ZNurseryGeometry {
            base: 0x1000_0000,
            page_bytes: 2 * MIB,
            floor: 0x1000_0000 + floor_pages * 2 * MIB,
            end: 0x1000_0000 + (floor_pages + span_pages) * 2 * MIB,
        }
    }

    /// The refuting shape: a survivor on every nursery page. This is the
    /// observation that closes `proposal-d-real-young-space.md` for the cost
    /// of one counter — a page-backed young generation can only release a page
    /// that holds none, and here there are none such.
    #[test]
    fn a_survivor_on_every_page_refutes_the_young_space() {
        let g = nursery(4, 10);
        let census = ZNurserySurvivorCensus::new();
        for _ in 0..ZNurserySurvivorCensus::MIN_CYCLES {
            let survivors: Vec<(usize, usize)> =
                (0..10).map(|i| (g.floor + i * 2 * MIB + 64, 48)).collect();
            census.observe_cycle(g, survivors);
        }
        let report = census.report();
        assert_eq!(report.pages, 30);
        assert_eq!(report.pages_with_survivor, 30);
        assert_eq!(report.survivor_page_percent(), 100);
        assert_eq!(report.verdict(), ZYoungSpaceVerdict::Refuted);
    }

    /// The supporting shape: survivors clustered on a minority of pages, so
    /// the rest could have been released without being visited.
    #[test]
    fn survivors_clustered_on_a_few_pages_support_the_young_space() {
        let g = nursery(0, 10);
        let census = ZNurserySurvivorCensus::new();
        for _ in 0..ZNurserySurvivorCensus::MIN_CYCLES {
            // Three pages of ten hold every survivor.
            let survivors: Vec<(usize, usize)> = (0..30)
                .map(|i| (g.floor + (i % 3) * 2 * MIB + 8 * i, 32))
                .collect();
            census.observe_cycle(g, survivors);
        }
        let report = census.report();
        assert_eq!(report.pages_with_survivor, 9);
        assert_eq!(report.survivor_page_percent(), 30);
        assert_eq!(report.verdict(), ZYoungSpaceVerdict::Supported);
    }

    /// Objects below the floor are OLD, not nursery survivors, and must not be
    /// counted — counting them would bias the census toward `Refuted` on
    /// exactly the heaps where the floor has stopped moving, which is the
    /// documented inert state of the current nursery.
    #[test]
    fn objects_below_the_floor_are_not_nursery_survivors() {
        let g = nursery(4, 4);
        let census = ZNurserySurvivorCensus::new();
        let below: Vec<(usize, usize)> = (0..4).map(|i| (g.base + i * 2 * MIB, 64)).collect();
        for _ in 0..ZNurserySurvivorCensus::MIN_CYCLES {
            census.observe_cycle(g, below.clone());
        }
        let report = census.report();
        assert_eq!(report.cycles, ZNurserySurvivorCensus::MIN_CYCLES);
        assert_eq!(report.pages, 12);
        assert_eq!(report.pages_with_survivor, 0);
        assert_eq!(report.survivor_bytes, 0);
        assert_eq!(report.verdict(), ZYoungSpaceVerdict::Supported);
    }

    /// An empty nursery is not evidence, and is not counted as a cycle:
    /// counting it would drag the percentage toward zero, i.e. toward
    /// `Supported`, which is the one direction an instrument must never drift
    /// on its own.
    #[test]
    fn an_empty_nursery_is_not_evidence() {
        let census = ZNurserySurvivorCensus::new();
        ...
        for _ in 0..10 {
            census.observe_cycle(empty, Vec::new());
        }
        let report = census.report();
        assert_eq!(report.cycles, 0);
        assert_eq!(report.pages, 0);
        assert_eq!(report.verdict(), ZYoungSpaceVerdict::NotEnoughData);
    }

    /// Below `MIN_CYCLES` the census refuses to answer, however clean the
    /// numbers look: the first young cycle after a whole-heap collection sees
    /// a nursery that has only just started filling.
    #[test]
    fn one_cycle_is_never_enough_to_decide() {
        let g = nursery(0, 8);
        let census = ZNurserySurvivorCensus::new();
        census.observe_cycle(g, [(g.floor + 16, 24)]);
        assert_eq!(census.report().verdict(), ZYoungSpaceVerdict::NotEnoughData);
    }
}
