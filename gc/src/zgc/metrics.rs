...
//!    a Generational run.
//!
//! # Adoption status (2026-09-20) — read this before trusting a reading
//!
//! From this module's first commit until 2026-09-20 **none of it ran.** Its
//! only reference anywhere in the tree was a `TODO` comment in `zgc.rs`; there
//! was no call. 1,377 production lines describing a running instrument, built
//! on the rule stated below that *the instrument goes in first*, that had never
//! taken a sample — while the collector grew a second, hand-rolled per-phase
//! timing system in `eprintln!`s. See
//! `docs/internal/zgc-round-20260920/gap-e-metrics-and-adapters-are-unreachable.md`.
//!
//! What is true now:
//!
//! * **Recording is live and unconditional.**
//!   [`super::ZgcRealHeap::record_metrics_for_cycle`] feeds
//!   [`ZgcMetrics::record_phase`] and [`ZgcMetrics::record_cycle`] on every
//!   collection, from the laps `collect_garbage`'s own `ZPhaseClock` already
//!   takes. The phase→lap mapping is documented at that function, including
//!   which five [`ZgcPhase`]s stay at `count=0` **because this collector has no
//!   such phase** — that is the "a zero is a finding" reading below actually
//!   being available, not a wiring gap.
//! * **Reporting is opt-in and additive.** `CRATONVM_ZGC_METRICS=1` prints
//!   [`ZgcMetrics::format_cycle_line`] per cycle and [`ZgcMetrics::format_summary`]
//!   at teardown. It does **not** replace `[GC] zgc-real:` / `[GC] zgc-pause:`,
//!   which every harness and archived comparison in this tree is keyed on.
//!   Retiring those in favour of this rendering is a wave with a compatibility
//!   window; it is no longer blocked on this module being unreachable.
//! * **One recording gap remains.** [`ZgcStallGuard`] /
//!   [`ZgcMetrics::scoped_allocation_stall`] has no caller, so the
//!   allocation-stall columns read `0`. On this collector that is nearly true —
//!   a mutator that cannot allocate performs the collection rather than waiting
//!   on one — but it is unmeasured, not measured-as-zero. Do not quote it.
//!
...
//!
//! ⚠ **That escape hatch is closed as of 2026-09-20.** The reasoning above was
//! "no collected aggregation can be misaligned because nothing calls it", and
//! the same sentence was still true a year later — which was the defect, not
//! the licence. The module is fed now and `CRATONVM_ZGC_METRICS` makes its
//! output reachable, so a run can have emitted rows that someone kept. Treat
//! [`ZgcMetrics::tsv_header`] as **append-only from here**: a new column goes
//! on the end, and a phase inserted in cycle order changes `format_summary`'s
//! table only.
//!
...
pub struct ZgcMetrics {
    ...
    /// **The distribution of the WHOLE-CYCLE pause**, one sample per
    /// [`Self::record_cycle`], with the same bounded ring and nearest-rank
    /// percentiles every phase gets.
    ///
    /// # Why this is not derivable from the per-phase stats
    ///
    /// `phase_stats(ConcurrentRelocate).p99_ns` is the p99 of the *slide*, and
    /// the sum of the per-phase p99s is not the p99 of the sum: the 99th-worst
    /// mark and the 99th-worst slide need not be the same cycle. A pause
    /// target is a statement about the total, so the total has to be sampled as
    /// a total. Nothing did, and `last_cycle_stw_ns` beside it is a gauge — it
    /// answers only for the most recent cycle.
    ///
    /// # What it is for
    ///
    /// `gap-e-relocate-runs-outside-the-measured-pause.md`'s first flip
    /// criterion asks for `pause_with_relocate_us` **p99** on two probes. That
    /// figure existed nowhere: `zgc-relocate` prints a per-cycle value and a
    /// percentile over it needs the whole log post-processed. This is that
    /// figure, computed in the process.
    ///
    /// It is also **independent of
    /// `CRATONVM_ZGC_PAUSE_TARGET_INCLUDES_RELOCATE`**, which is what makes it
    /// usable as an A/B discriminator rather than a restatement of the arm.
    /// The samples come from the phase guards, which bracket every phase
    /// including [`ZgcPhase::ConcurrentRelocate`], not from the `gc_started`
    /// clock the flag moves. So both arms report the pause the mutators
    /// actually took, and the only thing the flag changes is what the
    /// controller was *told*.
    ///
    /// Charged as STW or concurrent per [`Self::phase_was_stw`]; with today's
    /// `set_phases_run_concurrently(false)` that is every phase, i.e. the whole
    /// collection.
    cycle_stw: PhaseCounters,
}
...
impl ZgcMetrics {
    ...
    /// `(cycles_sampled, p50_ns, p99_ns, max_ns)` for the **whole-cycle**
    /// pause — the figure a pause target is actually a statement about.
    ///
    /// `cycles_sampled` is capped at [`RECENT_SAMPLE_CAP`] for the two
    /// percentiles; `max_ns` is over the whole run. A `0` count means no cycle
    /// has completed, and every figure then reads `0` — which is "not
    /// observed", not "no pause".
    ///
    /// **Read this, not the sum of the per-phase p99s**, and see the
    /// [`Self::cycle_stw`] field for why the two are different quantities. This
    /// is the reading
    /// `docs/internal/zgc-round-20260920/gap-e-relocate-runs-outside-the-measured-pause.md`
    /// asks for, and it is the same in both arms of
    /// `CRATONVM_ZGC_PAUSE_TARGET_INCLUDES_RELOCATE` — deliberately, so an A/B
    /// over it compares pauses rather than restating which clock was read.
    pub fn cycle_pause_distribution(&self) -> (u64, u64, u64, u64) {
        let mut samples = self.cycle_stw.recent.lock().snapshot();
        samples.sort_unstable();
        (
            samples.len() as u64,
            nearest_rank(&samples, 50),
            nearest_rank(&samples, 99),
            self.cycle_stw.max_ns.load(Ordering::Relaxed),
        )
    }
    ...
    pub fn record_cycle(
        ...
        // ...and into the distribution, which is the same number sampled
        // rather than overwritten. See the `cycle_stw` field.
        self.cycle_stw.record(cycle_stw);
    ...
    // ALLOWLISTED in `scripts/baselines/orphan-instruments-allowlist.txt`, and
    // reviewed rather than inherited: this recorder IS fed, by `ZgcStallGuard`'s
    // `Drop` below, and is `pub` only because `gc/tests/zgc_module_integration.rs`
    // drives it. It carries no `orphan-gate: read-by` annotation because the
    // only same-file function that calls it is `drop`, and the annotation's
    // cross-file half is a textual search for `drop(` -- which matches
    // essentially every file in the workspace. The annotation would be
    // verified, and verified against nothing.
