...
impl VmHeap {
    ...
    pub fn new_with_heap_sizing(
        ...
        match backend {
            GcBackend::Generational => {
                let heap = GenerationalHeap::with_capacity(total_bytes);
                ...
                //
                // The OLD generation is not commanded and does not need to be:
                // it is a raw `Vec<u8>` (`OldGen::new`), wholly committed at
                // construction, and this constructor gives it half the heap. So
                // the startup commit is `young_committed + old_capacity`, which
                // is at least `-Xms` for every `-Xms <= -Xmx` — the arithmetic
                // is on `commit_initial_heap`. `Committed(n)` is therefore the
                // honest answer even where the young pair alone could not
                // absorb `n`.
    ...
    ///
    /// # Why this is a helper, and why only `get_field` calls it today
    ///
    ...
    ///
    /// The *mutating* accessors — `set_field`, `set_field_volatile`,
    /// `set_field_as`, `set_field_volatile_as`, `set_array_element` — and
    /// `get_array_element` do **not** fire it, and a write through a stale
    /// receiver is strictly worse than a read through one: it corrupts the live,
    /// unrelated object that now occupies the address, whose owner has no idea a
    /// field was overwritten, and the damage surfaces arbitrarily later in a
    /// third place with no backtrace. Extending the instrument there is the
    /// obvious repair and is deliberately still open: both probes early-return
    /// on `vacated_frames_enabled()` (a relaxed load of a resolved `AtomicU8`
    /// plus a not-taken branch), but `set_field` is on **every reference store
    /// in the VM**, and "almost certainly noise" is the reasoning that put the
    /// unconditional `fetch_add` on the card barrier which `gc_metrics.rs`'s
    /// header now exists to explain gating. It wants one A/B run behind it.
    ///
    /// This helper exists so that when the measurement is taken the change is
    /// one line per accessor and the six sites cannot drift apart. See
    /// `docs/internal/gaps/gengc-plumbing-stale-receiver-check-only-on-reads-20260920.md`.
    #[inline]
    fn check_receiver(&self, obj: ObjectRef, site: &'static str, receiver_site: &'static str) {
        ...
        crate::gc_quiescence::report_vacated_receiver(obj.as_ptr() as usize, site);
        // The re-issue-proof half of the same question — see
        // `gc_quiescence::stale_use_verdict` for why the exact ledger above
        // cannot answer it.
        crate::gc_quiescence::check_stale_use(obj.as_ptr() as usize, receiver_site);
    }
    ...
    ///   (`committed_heap_bytes`), each an arena CAPACITY.
    /// * G1 — the committed prefix of its reservation (`committed_len`), which
    ///   grows on demand a region at a time and therefore moves with `-Xms`.
    /// * ZGC — the arena envelope captured at construction, fixed for the
    ///   collector's lifetime.
    ///
    /// # This is not what the process is holding
    ///
    /// The Generational and ZGC arms report address space, not backing. Both
    /// collectors sit on `HeapStore`, which RESERVES and commits in 2 MiB
    /// granules, so a fresh `-Xmx8g` process on either holds megabytes while
    /// this reports 8 GiB. That is the right answer for `totalMemory()` — it is
    /// what those heaps may use without further growth, and it does not move
    /// under a collection — and the wrong one for "what did `-Xms` do".
    /// [`Self::os_committed_bytes`] is the second question, deliberately under
    /// a different name.
    ...
    ///
    /// [`Self::committed_bytes`] answers `Runtime.totalMemory()` and is a sum
    /// of capacities; this one is the measurement, and the two differ by
    /// whatever a reserving store has not committed yet.
    ///
    /// # Why it exists
    ///
    /// `docs/internal/gc/heap-xms-xmx-mean-different-things-per-backend-20260920-RETIRED-20260921.md`
    ...
    /// without a number that moves when it works, and `committed_bytes` is not
    /// that number on two of three backends.
    ///
    ...
    pub fn enable_gc_logging(&self) {
        match self {
            ...
            // `g1.rs:18073`. Gap:
            // `docs/internal/gaps/gengc-plumbing-verbose-gc-does-nothing-20260920.md`.
    ...
    pub fn print_gc_summary(&self) {
        ...
        if let VmHeap::Generational(_) = self {
            ...
            //
            // `gengc-alloc-tlab-instruments-are-write-only-20260920.md`:
            ...
            //
            // In the `Generational` arm rather than unconditionally, so a G1
            // or ZGC summary is unchanged byte for byte — the census is
            // process-wide and every backend's TLAB retires through the same
            // file, so the other two backends' share of these numbers is
            // reported nowhere. See this round's review for that residue.
            ...
            // `gengc-alloc-evac-pool-width-frozen-20260920.md`: "`EvacPool::
...
///
/// `gengc-alloc-tlab-instruments-are-write-only-20260920.md` filed these two
...
fn render_tlab_guard_line(line: &str, urgent: bool) -> String {
    ...
    format!(
        "{line} — TRIPWIRE FIRED: a TLAB tail was filled over a word that \
         ...
         docs/internal/gaps/gengc-alloc-tlab-instruments-are-write-only-20260920.md"
    )
}
