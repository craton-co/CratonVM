            ...
            {
                {
                    ...
                    );
                    {
                        ...
                        // up to the arena's first high-water mark. The `lazy_*`
                        // figures are process-wide and zero unless
                        // `CRATONVM_ZGC_LAZY_TLAB_ZERO` is on.
                        let (_, refill_bytes, _, _) = h.vm_tlab_engagement();
                        let (ext, covered, unzeroed) = crate::tlab::tlab_lazy_zero_stats();
                        eprintln!(
                            "[GC] zgc-tlab-zero: pristine_bytes_skipped={} tlab_zero_refill_bytes={refill_bytes} lazy={} lazy_extensions={ext} lazy_bytes_covered={covered} lazy_tail_bytes_never_zeroed={unzeroed}",
                            h.tlab_pristine_bytes_skipped(),
                            h.vm_tlab_lazy_zero_enabled(),
                        );
                    }
