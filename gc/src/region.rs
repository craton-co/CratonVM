...
///
/// # W3-A — why this is `repr(u8)` with fixed discriminants
///
/// [`crate::g1::G1Region`] stores its classification in an `AtomicU8`, because
/// an evacuation worker retypes a freshly-claimed region while its peers are
/// reading the type of that same region through a raw base pointer (see the
/// `region_type` accessor pair on `G1Region` for the full argument). The
/// encoding therefore has to exist, and it has to live in exactly one place so
/// that no second, divergent copy can be written. `repr(u8)` plus the explicit
/// discriminants below is that one place: the values are part of the atomic's
/// contract, so they are spelled out rather than left to declaration order, and
/// changing one is a deliberate act rather than a side effect of inserting a
/// variant.
///
/// The discriminants are never persisted and never cross a process boundary —
/// they exist only for the duration of one heap — so the only invariant that
/// matters is that [`RegionType::from_u8`] round-trips every value
/// [`RegionType::to_u8`] can produce.
...
pub enum RegionType {
    ...
    HumongousContinuation = 4,
    /// Available for allocation.
    Free = 5,
}

/// W3-A — how many times [`RegionType::from_u8`] was handed a byte that is not
/// a discriminant and returned the fail-safe [`RegionType::Old`] instead.
///
/// Expected to be ZERO, and structurally unreachable: the only writer of the
/// byte is `G1Region::set_region_type`, which takes a `RegionType` and calls
/// `to_u8`, and an `AtomicU8` cannot tear. It is a counter rather than a
/// `debug_assert!` because every reader of this value sits inside a GC pause on
/// an evacuation worker, and an assertion there aborts the VM from a
/// `g1-evac-N` thread — a strictly worse bug than the one it would report (see
/// `PARALLEL_EVAC_UNALIGNED_CANDIDATE`'s note in `g1.rs` for the same argument
/// applied to a `make_forwarded` assert that really did abort).
///
/// Printed by `collector_decision_report`'s `[GC] g1 mark-residue:` line, which
/// is what makes "a counter, so it can be read" true rather than aspirational.
pub static REGION_TYPE_DECODE_FAILSAFE: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);

/// The value of [`REGION_TYPE_DECODE_FAILSAFE`].
pub fn region_type_decode_failsafes() -> usize {
    REGION_TYPE_DECODE_FAILSAFE.load(std::sync::atomic::Ordering::Relaxed)
}

impl RegionType {
    /// Every variant, in discriminant order.
    ///
    /// Kept beside [`Self::from_u8`] because the two have to agree, and used by
    /// the round-trip test below that checks they do.
    pub const ALL: [RegionType; 6] = [
        RegionType::Eden,
        RegionType::Survivor,
        RegionType::Old,
    ...
    /// This classification's stored byte.
    #[inline]
    pub const fn to_u8(self) -> u8 {
        self as u8
    }

    /// The classification a stored byte names.
    ///
    /// # What the fail-safe arm costs, measured
    ///
    /// The `fetch_add` is a side effect in the wildcard arm, and a side effect
    /// is what stops LLVM folding the decode into the caller's comparison. This
    /// is not a guess; it is the emitted x86-64 for
    /// `r.region_type() == RegionType::Free` with and without it:
    ///
    /// ```text
    ///   with the counter          without it              plain field (pre-W3-A)
    ///   movzbl 8(%rcx), %eax      movzbl 8(%rcx), %eax     cmpb $5, 8(%rcx)
    ///   cmpb   $6, %al            cmpb   $5, %al           sete %al
    ///   jb     .LBB0_2            sete   %al
    ///   cmpb   $5, %al
    ///   sete   %al
    /// ```
    ///
    /// So: **two instructions and one statically-predictable, never-taken
    /// branch per read**, on top of the one extra `movzbl` that the atomic
    /// itself costs (an atomic load cannot be folded into a `cmp`'s memory
    /// operand the way a plain field read can). That is the entire throughput
    /// price of W3-A on the read side, and it is paid at
    /// `classify_candidate_header_view`, which runs once per CSet-bound
    /// reference on the default young path.
    ///
    /// It is kept anyway, for two reasons. The counter has a READER —
    /// `collector_decision_report`'s `[GC] g1 mark-residue:` line — so the
    /// "prefer a counter to an assertion" argument is actually honoured rather
    /// than asserted. And five instructions against two is not a measurable
    /// difference on a screen that already does a shift, a bounds check, three
    /// tag validations and two header loads; the difference that WOULD have
    /// been measurable — an `Acquire` load instead of a plain one — turns out to
    /// be no difference at all on x86-64, where the two lower to the same
    /// `movzbl`. See `docs/internal/g1-2026-09-20/w3a-region-metadata-ub.md`.
    ///
    /// The COMPILE-time mistake this arm would otherwise be standing in for —
    /// a variant added to [`RegionType`] without an arm here — is caught by
    /// [`Self::every_variant_decodes`] instead, which is strictly better than
    /// catching it in production.
    ///
    /// # Why the fail-safe direction is `Old`
    ///
    /// `Free` is the dangerous answer: every screen in `g1.rs` treats a Free
    /// region as holding nothing, so a spurious `Free` makes
    /// `classify_candidate_header_view` return `HeaderVerdict::RegionFree` and
    /// the evacuator DROPS a live reference — the collector then frees the
    /// referent out from under a holder that still points at it. `Old` takes the
    /// `_` arm of that same match, which falls through to the base/cursor
    /// containment test and accepts the address if it is really there. A false
    /// accept is only the pre-screen behaviour; a false reject is a
    /// use-after-free. Same direction the rest of that file already chooses (see
    /// `candidate_header_is_plausible`'s note on `kept_unresolved_*`).
    #[inline]
    pub fn from_u8(raw: u8) -> Self {
        match raw {
            0 => RegionType::Eden,
            1 => RegionType::Survivor,
            2 => RegionType::Old,
            3 => RegionType::HumongousStart,
            4 => RegionType::HumongousContinuation,
            5 => RegionType::Free,
            _ => {
                REGION_TYPE_DECODE_FAILSAFE.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                RegionType::Old
            }
        }
    }

    /// COMPILE-TIME TRIPWIRE for [`Self::from_u8`] and [`Self::ALL`].
    ///
    /// This match has no wildcard, so adding a variant to [`RegionType`] stops
    /// this file compiling (E0004, non-exhaustive patterns) until whoever added
    /// it comes here and reads the paragraph above. That is the check the
    /// discarded runtime counter was standing in for, moved to the only place
    /// it can be made total.
    ///
    /// It is never called on a hot path — `region_type_round_trip` below is its
    /// only caller — and it always returns `true`; the RETURN VALUE is not the
    /// point, the exhaustiveness of the match is.
    #[inline]
    pub const fn every_variant_decodes(self) -> bool {
        match self {
            RegionType::Eden
            | RegionType::Survivor
            | RegionType::Old
            | RegionType::HumongousStart
            | RegionType::HumongousContinuation
            | RegionType::Free => true,
        }
    }
}

#[cfg(test)]
mod region_type_codec_tests {
    use super::RegionType;

    /// W3-A — the one invariant `G1Region`'s `AtomicU8` representation rests on:
    /// every variant `to_u8` can produce, `from_u8` returns.
    ///
    /// A variant added without a `from_u8` arm fails to compile at
    /// `every_variant_decodes`; a variant given the WRONG arm (a copy-paste of
    /// its neighbour's discriminant) compiles fine and is what this catches.
    #[test]
    fn region_type_round_trip() {
        assert_eq!(
            RegionType::ALL.len(),
            6,
            "a variant was added to RegionType without being added to ALL, so              the round-trip below no longer covers the whole enum"
        );
        for t in RegionType::ALL {
            assert!(t.every_variant_decodes());
            assert_eq!(
                RegionType::from_u8(t.to_u8()),
                t,
                "{t:?} does not survive the byte encoding G1Region stores it in"
            );
        }
        // Distinct discriminants — two variants sharing a byte would round-trip
        // individually and still make the collector unable to tell them apart.
        for (i, a) in RegionType::ALL.iter().enumerate() {
            for b in RegionType::ALL.iter().skip(i + 1) {
                assert_ne!(a.to_u8(), b.to_u8(), "{a:?} and {b:?} share a byte");
            }
        }
        // Everything outside the discriminants decodes to the fail-safe.
        for raw in 6u8..=255 {
            assert_eq!(RegionType::from_u8(raw), RegionType::Old);
        }
    }
}
// ---------------------------------------------------------------------------
...
impl RememberedSet {
    ...
    pub fn retain_sources<F: FnMut(usize) -> bool>(&self, mut keep: F) {
        let mut guard = self.sources.lock();
        guard.retain(|&s, _| keep(s));
        Self::shrink_if_slack(&mut guard);
    }
    ...
    pub fn retain_sources_in_generation<F: FnMut(usize, u64) -> bool>(&self, mut keep: F) {
        let mut guard = self.sources.lock();
        guard.retain(|&s, gen| keep(s, *gen));
        Self::shrink_if_slack(&mut guard);
    }

    /// LANE A — [`Self::retain_sources_in_generation`] and
    /// [`Self::sources_with_generations`] fused into ONE lock acquisition,
    /// returning the surviving source indices.
    ///
    /// # Why the fusion, and not just calling the two
    ///
    /// The scan side's shape is "snapshot every entry, decide which are dead,
    /// act on the live ones" and its criterion is the same one `cleanup`
    /// prunes with. Expressing that as `sources_with_generations()` followed by
    /// a later `retain_sources_in_generation` costs two acquisitions of this
    /// rset's mutex, two passes over the map, and — the part that actually
    /// shows up — an owned `Vec<(usize, u64)>` allocated per collection-set
    /// region per pause, of which only the `usize` half is ever used.
    ///
    /// The returned `Vec` is deliberately still owned: the caller
    /// (`G1Collector::live_rset_sources`) consults the REGION TABLE inside
    /// `keep`, and holding this mutex across the caller's own subsequent work
    /// would widen a lock the post-write barrier contends on.
    ///
    /// `keep` is called with `(source_region_index, recorded_generation)` and
    /// returns false for an entry to delete. A `false` here is a permanent
    /// judgement — see `G1Collector::lane_a_rset_prune_on_scan` for why the
    /// staleness test that drives it cannot un-fire.
    ///
    /// # WAVE 2 — this does not WRITE unless there is something to delete
    ///
    /// The wave-1 measurement that pushed `CRATONVM_G1_RSET_PRUNE_ON_SCAN` back
    /// to opt-in read as "the prune takes the write side of a lock the scan
    /// path only reads". There is no read side: `sources` is a plain
    /// `parking_lot::Mutex` and `sources_with_generations` takes it exclusively
    /// too. What the prune arm genuinely cost over the read arm was the
    /// WRITING — a `HashMap::retain` that dirties the table's control bytes and
    /// a `shrink_if_slack` that can reallocate and rehash — and it paid that on
    /// every scanned source whether or not a single entry was dead.
    ///
    /// The shape below removes exactly that. One pass over the map decides
    /// every entry and separates the dead ones; when none is dead — the
    /// overwhelmingly common case, because staleness is monotone and a source
    /// goes stale at most once in its life — the function returns having
    /// touched nothing but the two `Vec`s, and `dead` has not allocated at all
    /// (`Vec::new` does not). When some are dead they are removed by key, which
    /// is O(dead) rather than O(capacity).
    ///
    /// That makes the prune arm STRICTLY CHEAPER than the non-prune arm rather
    /// than strictly dearer: the non-prune arm calls
    /// [`Self::sources_with_generations`], which allocates a
    /// `Vec<(usize, u64)>` — twice the bytes, for a generation half the caller
    /// discards on the next line.
    ///
    /// "Gate the prune on a stale-fraction threshold" is what this is, with the
    /// threshold at its only defensible value. Any positive threshold means
    /// counting the stale entries first, and counting them is the whole pass;
    /// once the pass has run, deleting `n` entries costs `n` removals and
    /// declining to delete them costs the same pass again next pause, forever.
    /// So the threshold is "more than zero", and it is free to evaluate.
    pub fn retain_and_collect_sources<F: FnMut(usize, u64) -> bool>(
        &self,
        mut keep: F,
    ) -> Vec<usize> {
        let mut guard = self.sources.lock();
        let mut live = Vec::with_capacity(guard.len());
        // Not `with_capacity`: an empty `Vec` does not allocate, and "no entry
        // is stale" is the case this function is shaped around.
        let mut dead: Vec<usize> = Vec::new();
        for (&source, &generation) in guard.iter() {
            if keep(source, generation) {
                live.push(source);
            } else {
                dead.push(source);
            }
        ...
        if dead.is_empty() {
            // Nothing to collect, so nothing is written. The map's control
            // bytes stay Shared in every core's cache and `shrink_if_slack` —
            // which reallocates and rehashes when it fires — is not consulted.
            return live;
        }
        for source in dead {
            guard.remove(&source);
    ...
    /// Hand back the table an emptied-out remembered set is still holding.
    ///
    /// `HashMap::retain` never shrinks, so an Old region that accumulated
    /// thousands of sources over a mark cycle and then had nearly all of them
    /// pruned keeps the whole allocation for the life of the region — which,
    /// for an Old region, is until it is collected. That is the same "the total
    /// across the heap is what is unbounded" cost the coarsening cap exists to
    /// bound, arriving by a different door.
    ///
    /// The threshold is deliberately slack (a quarter full, and only above a
    /// floor where the allocation is worth reclaiming at all): `shrink_to_fit`
    /// reallocates and rehashes, and a set oscillating around the boundary
    /// would pay that on every pause.
    fn shrink_if_slack(guard: &mut FxHashMap<usize, u64>) {
        const SHRINK_FLOOR: usize = 64;
        if guard.capacity() >= SHRINK_FLOOR && guard.len() * 4 <= guard.capacity() {
            guard.shrink_to_fit();
        }
    }

    /// Is `source_region` named by this remembered set? O(1), and without the
    /// `Vec` [`Self::sources_with_generations`] allocates.
    ///
    /// A COARSENED rset answers `true` for every source, because that is what
    /// coarsening means — see the field docs. A caller that needs to tell
    /// "named precisely" from "named by coarsening" must ask
    /// [`Self::is_coarsened`] itself.
    pub fn names_source(&self, source_region: usize) -> bool {
        if self.is_coarsened() {
            return true;
        }
        self.sources.lock().contains_key(&source_region)
    }
