...
/// Keep the heap object at `addr` ALIVE across collections. Refcounted: each
/// `pin` must be balanced by exactly one [`unpin`]. A zero address is ignored
/// (null handle / non-heap pointer).
///
/// This does **not** hold the object in place, despite the name. That claim was
/// in this comment and it contradicted both the module contract ("It does NOT
/// need to keep the object **in place**") and [`update_after_gc`], which exists
/// precisely to re-key the table after a pinned object moved — a function with
/// no reason to exist if a pin were a no-relocation pin. The data-movement half
/// of the JNI hazard is closed by handing native code a detached copy, not by
/// immobility; [`is_pinned`] is available to a collector that wants to avoid
/// relocating pinned objects as an optimisation, and nothing depends on it.
