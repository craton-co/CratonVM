...
/// construction. See
/// `docs/internal/gaps/gengc-alloc-numa-hint-cannot-vary-20260920.md`.
///
...
///
/// The right call is still `sched_getcpu()`, and this is not it: it is a
/// `/proc` read and it costs orders of magnitude more. It is reachable without
/// adding `libc` to `gc/`, which has no dependencies by design, and
/// [`NumaTopology::current_thread_node`] is what keeps it off the allocation
/// path. The gap page's options (2) and (3) -- take the dependency, or move the
/// probe up into a crate that already has it -- are both still the better
/// answer and both still open.
#[cfg(target_os = "linux")]
