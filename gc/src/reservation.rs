...
impl HeapStore {
    ...
    /// Give the PHYSICAL pages under the whole granules of
    /// `[offset, offset + len)` back to the OS while leaving them mapped,
    /// committed and READABLE (round 9 wave 9, `zgc9`).
    ///
    /// The sibling of [`Self::decommit_range`] for a heap that has published
    /// its whole span to the JIT as safe to load from raw
    /// (`gen_heap::publish_jit_read_bounds`). A decommitted granule FAULTS on
    /// touch, so every decommit forces that bound to be withdrawn for good; a
    /// reset granule reads back as zero and leaves it valid. Same inward
    /// rounding, the same "the caller must prove nothing live is here"
    /// obligation, and on Linux and Windows the same "a re-used granule reads
    /// as zero" contract.
    ///
    /// **Call only while no mutator can touch the range** (a collector
    /// safepoint): the Windows arm releases and re-commits, and a load between
    /// the two would fault.
    ///
    /// Returns `(reset, decommitted)`. `decommitted` is non-zero only when the
    /// Windows arm released a run and the OS then refused to commit it again.
    /// That run is accounted exactly as [`Self::decommit_range`] accounts one
    /// (bits cleared, ring entry recorded), and the caller must treat those
    /// bytes as a real hole. `(0, 0)` on the owned arm.
    pub fn reset_range(&mut self, offset: usize, len: usize, site: &'static str) -> (usize, usize) {
        match self {
            HeapStore::Owned(_) => (0, 0),
            HeapStore::Reserved(r) => r.reset_range(offset, len, site),
        }
    }
}
...
impl Reservation {
    ...
    /// See [`HeapStore::reset_range`]. Rounds inward exactly as
    /// [`Self::decommit_range`] does, and touches only COMMITTED runs: an
    /// uncommitted granule has no physical page to give back, and the Windows
    /// arm's `MEM_DECOMMIT` + `MEM_COMMIT` must not commit one that was never
    /// committed.
    fn reset_range(&mut self, offset: usize, len: usize, site: &'static str) -> (usize, usize) {
        if len == 0 || offset >= self.reserved_len {
            return (0, 0);
        }
        let end = offset.saturating_add(len).min(self.reserved_len);
        ...
        let mut decommitted = 0usize;
        let mut run_start: Option<usize> = None;
        for g in first..last {
            match (self.is_committed(g), run_start) {
                (true, None) => run_start = Some(g),
                (false, Some(s)) => {
                    let (r, d) = self.reset_granules(s, g - s, site);
                    reset += r;
                    decommitted += d;
                    run_start = None;
                }
        ...
        if let Some(s) = run_start {
            let (r, d) = self.reset_granules(s, last - s, site);
            reset += r;
            decommitted += d;
        }
        (reset, decommitted)
    ...
    /// One committed run of [`Self::reset_range`].
    fn reset_granules(&mut self, first: usize, count: usize, site: &'static str) -> (usize, usize) {
        if count == 0 {
            return (0, 0);
        }
        let bytes = count * GRANULE;
        // SAFETY: bounded by the caller, as `commit_granules`; the caller
        // passes only runs whose every granule is committed.
        let outcome = unsafe { platform_reset(self.base.add(first * GRANULE), bytes) };
        match outcome {
            PlatformReset::Reset => (bytes, 0),
            // Refused before anything changed: the pages stay committed and
            // keep their bytes, which is exactly what not asking leaves.
            PlatformReset::Unchanged => (0, 0),
            // Released and NOT committed again (the Windows arm, at the commit
            // limit). The run is a real hole now: account it as
            // `decommit_granules` does, so the bitmap never claims a page the
            // OS has taken away, and the ring can name it in a crash report.
            PlatformReset::Unmapped => {
                record_decommit(self.base as usize + first * GRANULE, bytes, site);
                for g in first..first + count {
                    if self.is_committed(g) {
                        self.set_committed(g, false);
...
/// What [`platform_reset`] did to a committed run. (`Unmapped` is only ever
/// produced by the Windows arm.)
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[cfg_attr(not(target_os = "windows"), allow(dead_code))]
enum PlatformReset {
    /// The physical pages went back to the OS; the run is still mapped
    /// read-write and reads as zero.
    Reset,
    /// Nothing changed: the OS refused before touching the run.
    Unchanged,
    /// The run was released and could not be committed again. It now faults on
    /// touch, exactly like a decommitted run.
    Unmapped,
}
...
/// Release the physical pages of a COMMITTED run and commit it again, so it
/// stays readable and reads as zero (round 9 wave 9, `zgc9`).
///
/// Not `MEM_RESET`: that keeps the old bytes until the OS happens to trim the
/// page, and the bump path hands out committed memory on the promise that it
/// reads as zero. Decommit + commit gives demand-zero pages, costs the commit
/// charge nothing net, and leaves a window in which the run faults -- which is
/// why [`HeapStore::reset_range`] may only be called at a safepoint.
#[cfg(target_os = "windows")]
unsafe fn platform_reset(addr: *mut u8, len: usize) -> PlatformReset {
    if VirtualFree(addr, len, MEM_DECOMMIT) == 0 {
        return PlatformReset::Unchanged;
    }
    if VirtualAlloc(addr, len, MEM_COMMIT, PAGE_READWRITE).is_null() {
        return PlatformReset::Unmapped;
    }
    PlatformReset::Reset
}
...
/// The advise alone: the pages go back to the OS and the protection stays
/// `PROT_READ | PROT_WRITE`, so the run is still readable (a fresh zero page on
/// Linux) and nothing has to be committed again. See
/// [`HeapStore::reset_range`].
#[cfg(unix)]
unsafe fn platform_reset(addr: *mut u8, len: usize) -> PlatformReset {
    if madvise(addr, len, MADV_DONTNEED) == 0 {
        PlatformReset::Reset
    } else {
        PlatformReset::Unchanged
    }
}
...
#[cfg(not(any(unix, target_os = "windows")))]
unsafe fn platform_reset(_addr: *mut u8, _len: usize) -> PlatformReset {
    PlatformReset::Unchanged
}
...
mod tests {
    ...
    /// **A reset granule gives its pages back and stays READABLE** (round 9
    /// wave 9, `zgc9`; `HeapStore::reset_range`).
    ///
    /// The property the JIT's published read bound needs: after the give-back
    /// the granule must still be committed in the bitmap, loadable without a
    /// fault, and read as zero -- the same contract a re-committed granule
    /// has -- while the partial granules at either edge keep their bytes.
    #[test]
    fn a_reset_granule_stays_committed_readable_and_reads_as_zero() {
        ...
        unsafe {
            *res.base.add(GRANULE + 8) = 0x11; // partial leading granule
            *res.base.add(2 * GRANULE + 99) = 0xCD; // wholly inside the range
            *res.base.add(4 * GRANULE + 7) = 0xEF; // wholly inside the range
            *res.base.add(5 * GRANULE + 64) = 0x22; // partial trailing granule
        }
        // Mid-granule at both ends: only granules 2..5 are wholly inside.
        let (reset, decommitted) = res.reset_range(GRANULE + 16, 4 * GRANULE, "test");
        if reset == 0 && decommitted == 0 {
            return; // platform declined; nothing to assert
        }
        if decommitted != 0 {
            // The Windows arm could not commit the run again (commit limit).
            // Then it must be accounted as a hole, never as committed.
            assert!(!res.is_committed(2) || !res.is_committed(4));
            return;
        }
        assert_eq!(reset, 3 * GRANULE);
        assert_eq!(res.committed_granules(), 8, "a reset must not uncommit");
        for g in 0..8 {
            assert!(res.is_committed(g), "granule {g} must still be committed");
        }
        // SAFETY: every granule is still committed read-write.
        unsafe {
            assert_eq!(
                *res.base.add(2 * GRANULE + 99),
                0,
                "a reset granule reads as zero"
            );
            assert_eq!(
                *res.base.add(4 * GRANULE + 7),
                0,
                "a reset granule reads as zero"
            );
            assert_eq!(
                *res.base.add(GRANULE + 8),
                0x11,
                "the partial leading granule survives"
            );
            assert_eq!(
                *res.base.add(5 * GRANULE + 64),
                0x22,
                "the partial trailing granule survives"
            );
            // And it is writable without a commit.
            *res.base.add(3 * GRANULE + 5) = 0x5A;
            assert_eq!(*res.base.add(3 * GRANULE + 5), 0x5A);
        }
    }

    /// **A reset skips uncommitted granules** rather than committing them.
    #[test]
    fn a_reset_never_commits_what_was_not_committed() {
        let Some(mut res) = Reservation::reserve(4 * GRANULE) else {
            return;
        };
        assert!(res.commit_range(0, GRANULE));
        let (reset, decommitted) = res.reset_range(0, 4 * GRANULE, "test");
        assert_eq!(decommitted, 0);
        assert!(
            reset <= GRANULE,
            "only the one committed granule can be reset"
        );
        assert_eq!(res.committed_granules(), 1);
        assert!(!res.is_committed(1) && !res.is_committed(3));
    }
