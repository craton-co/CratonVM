...
impl OldGen {
        ...
        for bucket_idx in start_bucket..NUM_BUCKETS {
            ...
            for i in 0..self.buckets[bucket_idx].len() {
                ...
                if total_needed <= block.size {
                    ...
                    // PERF (gengc-round1, 2026-09-20): stop REFINING the
                    // best fit after a bounded number of candidates. The
                    // module header claims this scan is "amortised O(1)"
                    // because "bucket sizes are bounded within a factor of
                    // 2" — but that bounds each block's SIZE, not the
                    // bucket's LENGTH. A workload that frees a hundred
                    // thousand same-class objects without compacting (which
                    // is exactly what `old_gen_gc(compact = false)` leaves
                    // behind under conservative JIT roots) puts all of them
                    // in one bucket, and every subsequent allocation in that
                    // class walks all of them unless it hits an exact fit —
                    // reinstating the O(N) cost the segregated free list was
                    // built to remove.
                    //
                    // The cap is on FITTING candidates only, so it can never
                    // change whether a request succeeds: if the bucket holds
                    // no fit at all the scan is still exhaustive and the
                    // search still escalates, which is what keeps the round-5
                    // "cannot spuriously OOM" property intact. All it gives
                    // up is a marginally tighter fit among blocks whose sizes
                    // already differ by less than 2x.
                    fits_seen += 1;
    ...
    pub fn compact_with_drop_flags(
        ...
        if compacted_end < self.data.len() {
            ...
            if zero_end > compacted_end {
                // SAFETY: `[compacted_end, zero_end)` is inside `self.data`
                // and, after Phase 3's slide, holds no live object.
                unsafe {
                    std::ptr::write_bytes(base.add(compacted_end), 0, zero_end - compacted_end)
                };
            }
    ...
    /// `docs/internal/gaps/gengc-oldgen-fragmentation-has-no-trigger-20260920.md`
...
mod tests {
    ...
    fn a_lone_fit_behind_a_long_run_of_too_small_blocks_is_still_found() {
        ...
        // All of these land in the same bucket (`[256, 512)`), which is also
        // the bucket a 304-byte request starts its search in.
        const TOO_SMALL: usize = 256;
        const FITS: usize = 400;
        assert_eq!(bucket_for(TOO_SMALL), bucket_for(FITS));

        let mut small = Vec::new();
        for _ in 0..(BEST_FIT_PROBE_LIMIT * 2) {
    ...
    /// what `gengc-oldgen-fragmentation-has-no-trigger-20260920.md` asks for:
