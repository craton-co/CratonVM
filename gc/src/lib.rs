...
//!   (`-XX:+UseGenerationalGC`); see also `g1::G1Collector` and the ZGC backend,
//!   and [`vm_heap::VmHeap`], which is the enum the VM actually dispatches
//!   through
//! - [`Heap`] -- semi-space heap for Java objects and arrays. **Not a
//!   selectable backend**: no [`vm_heap::GcBackend`] variant and no
//!   [`vm_heap::VmHeap`] arm names it, so no `-XX:+Use…` flag can reach it. It
//!   is a small, complete reference implementation of
//!   [`collector::GarbageCollector`], used by that trait's doctest examples and
//!   by several test fixtures. This bullet led the list until 2026-09-20, which
//!   told a reader arriving at the crate that the primary offering was a
//!   collector they cannot select; the type's own module header
//!   ([`mod@heap`]) now carries the full statement, and the retirement options
//!   are in
//!   `docs/internal/gaps/gengc-plumbing-vestigial-semispace-heap-20260920.md`.
//!   Kept `pub` deliberately: this is a library crate with embedders
//!   (`cratonvm-embed`, `libcratonvm`), so removing the type would be a
//!   breaking change even though nothing in-tree selects it.
...
///
/// Nearly every `CRATONVM_*` flag this crate reads is a field on
...
///
/// **Two exceptions survive the migration**, and this paragraph exists because
/// the sentence above used to claim there were none — which made a reader
/// conclude that `rg` over `GcFlags` was a complete inventory of this crate's
/// flag reads. It is not. `CRATONVM_DBG_DM` (`reference.rs`) and
/// `CRATONVM_GC_CARD_METRICS` (`gc_metrics.rs`) still keep their own
/// `OnceLock` over `runtime_var_os`. Both are diagnostic gates rather than
/// behaviour switches, which is why they were left behind; moving them onto
/// `GcFlags` is the change that would let this doc drop the exception.
#[inline]
...
// `ReferenceQueue` was here until 2026-09-21. The Rust type is retired — see
// the note above `ReferenceProcessor` in `reference.rs`. The queue a Java
// program observes is the heap object, filled from
// `ReferenceProcessingResult::to_enqueue` and drained by
// `native-builtins/src/reference.rs`; nothing was ever re-exported for it.
pub use reference::{
