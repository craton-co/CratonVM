...
fn a_relocation_set_built_from_real_pages_round_trips_through_the_forwarding_registry() {
    ...
    // from `live_bytes / avg_object_bytes` (here 1024/16 = 64 objects -> 128
    // slots, since ZFWD_ASSUMED_AVG_OBJECT_BYTES became the SMALLEST possible
    // object rather than a guessed mean on 2026-09-20), and this test is about
    // the seam, not about probe behaviour at a 1.0 load factor — which
    // `forwarding.rs`'s own suite already covers.
...
///
/// # It fired once, as a real defect. FIXED 2026-09-20 — this test passes.
///
/// This block used to be headed "KNOWN FAILURE", and said that every `cargo
/// test` run in debug was expected to panic here. **That is no longer true, and
/// a stale "expected to fail" note is worse than no note at all**: it teaches
/// the next reader to wave past a panic from the one assertion standing between
/// a minor cycle and an undetected trace into the old generation.
///
/// The history, because the assertion is easier to read with it.
/// `ZGenerationalHeap::collect_young` closed by comparing the old generation's
/// live bytes against their pre-cycle value alone, while `sweep_young`
/// legitimately promotes by calling `ZOldGeneration::adopt_page(page, live)` on
/// a page whose `live_bytes` it has just set non-zero — and
/// `ZOldGeneration::live_bytes()` is `Σ page.live_bytes()` over the old map. So
/// any minor cycle that promoted anything raised the total and tripped the
/// assert, i.e. every debug build the moment the promotion policy did its job.
///
/// The invariant the assert means to state is "a minor cycle does not *mark* or
/// *sweep* old"; promotion is a legitimate, deliberate write to old. It now
/// compares against the *accounted* figure — `old_live_before +
/// out.bytes_promoted` — so old may change by promotion and by nothing else.
/// See `gc/src/zgc/generation.rs`, the `debug_assert_eq!` at the end of
/// `collect_young`, which carries the same history on the assertion itself.
#[test]
