...
use cratonvm_gc::reference::{CleanerThread, FinalizerThread, ReferenceProcessor, ReferenceType};
...
// -- WP1.10.F: RETIRED 2026-09-21 ------------------------------------------
//
// `reference_queue_enqueue_poll_fifo` and `reference_queue_evicts_oldest_on_overflow`
// went with `gc::reference::ReferenceQueue`, the production-dead Rust type
// they constructed. The retirement note above `ReferenceProcessor` in
// `gc/src/reference.rs` carries the argument; the short version is that no
// Java-visible reference ever passed through that type — the queue a program
// observes is the heap object, spliced by the VM from
// `ReferenceProcessingResult::to_enqueue` and drained by
// `native-builtins/src/reference.rs` — so neither the FIFO order nor the
// evict-the-oldest overflow policy these two pinned was ever observable.
//
// WP1.10's other letters are untouched: they cover `ReferenceProcessor`,
