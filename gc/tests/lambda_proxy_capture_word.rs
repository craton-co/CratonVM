...
/// Serialises every test in this file against `narrow_oop`'s PROCESS-GLOBAL
/// flag.
///
/// `compressed_oops_do_not_narrow_a_legacy_capture_cell` turns compressed oops
/// on for a window and off again. Every other test here builds a `VmHeap`, and
/// `ZgcRealHeap::with_capacity` refuses that combination with a
/// `debug_assert!` -- a 32-bit reference slot cannot carry the four
/// colored-pointer metadata bits plus the escape tag. So a neighbour that
/// happens to construct a ZGC heap inside that window dies on an assertion
/// about a configuration it never asked for, roughly one run in four.
///
/// The RACE predates the assertion: these tests have always shared the flag.
/// The assertion, added 2026-09-20, is only what made it audible -- which is
/// the argument for the assertion, not against it.
///
/// `std::sync::Mutex` rather than `parking_lot`'s, matching
/// `published_bounds_isolation.rs`: a panicking test poisons it and its
/// neighbours fail loudly instead of running against half-torn global state.
static NARROW_OOP_LOCK: Mutex<()> = Mutex::new(());
...
fn a_lambda_proxy_is_legacy_laid_out_on_every_backend() {
    let _serialise = NARROW_OOP_LOCK.lock().unwrap();
