...
pub trait NativeClassAccess {
    ...
    /// The array class `name` as the user-defined loader `loader_id` sees it
    /// (JVMS 5.3.3), synthesising it on demand -- the SAME `ClassId` an `ldc` /
    /// `anewarray` of that descriptor resolves to from a class that loader
    /// defined. Two loaders that each define their own `p/X` get two different
    /// `[Lp/X;` classes. `None` when the context has no loader-faithful
    /// answer (mock contexts), so the caller keeps its flat fallback.
    fn array_class_id_for_loader(&mut self, name: &str, loader_id: u32) -> Option<ClassId> {
        let _ = (name, loader_id);
        None
    }
    ...
    /// docs/jdk-only/E4-R11-CLASS-PATH-MODULE-BOOT-LAYER-FIX-20260813.md
    ...
    /// The `ModuleMainClass` attribute's class in INTERNAL (slash) form, if
    /// the module declares one. Backs `ModuleDescriptor.mainClass()`.
...
/// A receiver **validated once**, for a read-only region a safepoint cannot
/// cross.
///
/// # The cost this exists to remove
///
/// Every `NativeHeapAccess` read accessor is, at minimum, a
/// `is_object_address` membership probe and a forwarding barrier; for a field
/// it is also a layout lookup, a `check_field_index` and a descriptor
/// coercion. MEASURED on the 2026-09-20 `dev` build, `ChmProbe get`
/// (5 M `ConcurrentHashMap<Integer,Integer>.get`, `perf record -F 299` self
/// time): `is_object_address` **15.3 %** of the whole run across its two
/// layers, the layout lookups ~10 %, `check_field_index` 2.8 %,
/// `coerce_field_value_for_slot` 2.9 %, `load_and_forward_inner` 3.0 %. One
/// `get` makes roughly ELEVEN such accesses, and the probe is a random word of
/// a bitmap sized one bit per eight arena bytes — a cache miss, not an
/// instruction count. See
/// `retired/perf-a-collection-native-pays-the-checked-heap-api-per-field-20260920-RETIRED-20260921.md`.
///
/// Removing accesses one at a time does NOT work and was measured twice: wave
/// 12 removed three of the fifteen and both binaries came back flat inside a
/// ±5–9 % control arm. The lever is the cost of one access, which is why this
/// is a REGION and not another fused accessor.
///
/// # Why this is sound, and how the claim is CHECKED rather than asserted
///
/// A cursor borrows the context immutably (`&self`). Every `NativeContext`
/// method that can allocate, call Java, pin, or otherwise reach a safepoint
/// takes `&mut self` — so while a cursor is alive the borrow checker refuses
/// all of them. The "no allocation, no Java, no pins" premise that natives like
/// `chm_wrapper_chain_walk` already state in prose is, inside a region, a
/// compile error rather than a comment. Nothing can move, so the forwarding
/// barrier and the membership probe are re-answering a question the region's
/// entry already answered.
///
/// [`NativeHeapAccess::reached`] extends that to references READ through a
/// cursor: a reference slot of a live object holds a live object, and no
/// safepoint has intervened, so it needs no probe of its own. This is the arm
/// that removes most of the eleven.
///
/// # What a cursor is NOT
///
/// It is not a pin and not a root. It must not be stored, returned, or held
/// across anything that could collect — and it cannot be, because its lifetime
/// is the immutable borrow. An index passed to a cursor read is still the
/// caller's obligation exactly as it is for [`NativeHeapAccess::get_field`];
/// the implementation still bounds-checks it.
///
/// # The default is the checked path
///
/// [`Self::trusted`] is `0` for any implementor that does not override
/// [`NativeHeapAccess::read_cursor`] — mocks, test contexts, any heap with no
/// cheaper answer — and every cursor accessor then delegates to the ordinary
/// checked accessor. So a native written against this API is correct on every
/// implementor and merely faster on the one that overrides it.
#[derive(Clone, Copy, Debug)]
pub struct ReadCursor<'a> {
    /// The validated, forwarded reference. Always safe to hand to the checked
    /// accessors.
    obj: ObjectRef,
    /// Implementation-private token. `0` means "no trusted state — use the
    /// checked path"; the VM's heap puts the object's header address here.
    trusted: usize,
    /// The region: an immutable borrow of the context, which is what forbids
    /// every `&mut self` (i.e. every safepoint-reaching) method while this
    /// lives.
    region: std::marker::PhantomData<&'a ()>,
}

impl<'a> ReadCursor<'a> {
    /// A cursor with no trusted state: every read through it takes the
    /// ordinary checked path. The constructor an implementor uses when it has
    /// nothing cheaper, and the one the default [`NativeHeapAccess::read_cursor`]
    /// returns.
    #[inline]
    pub fn checked(obj: ObjectRef) -> Self {
        Self {
            obj,
            trusted: 0,
            region: std::marker::PhantomData,
        }
    }

    /// A cursor carrying an implementation-private `trusted` token.
    ///
    /// # Safety contract (not `unsafe`, because misuse cannot be UB *here*)
    ///
    /// Only the implementor that reads `trusted` may construct one, and only
    /// after validating `obj`. A token minted by one heap and read by another
    /// is a bug in that pairing; the token is opaque and carries no meaning
    /// across implementors.
    #[inline]
    pub fn trusted(obj: ObjectRef, trusted: usize) -> Self {
        Self {
            obj,
            trusted,
            region: std::marker::PhantomData,
    ...
    /// The validated reference. Safe to pass to any checked accessor.
    #[inline]
    pub fn object(&self) -> ObjectRef {
        self.obj
    }

    /// The implementation-private token, or `0`.
    #[inline]
...
pub trait NativeHeapAccess: NativeInvokeAccess {
    ...
    /// The WRITE twin of [`get_fields_typed`](Self::get_fields_typed): several
    /// slots of ONE object, with the descriptor supplied per slot.
    ///
    /// Each element of `slots` is `(slot index, descriptor byte)` and the value
    /// at the matching position of `values` is stored there; the shorter of the
    /// two bounds the batch. The descriptor must be the one this class actually
    /// declares for that slot -- read it out of the same class metadata the
    /// index came from -- because it selects the coercion the store performs.
    ///
    /// MEASURED, and the reason this exists: `SHA256Digest.processBlock`
    /// (`native-builtins/src/phases_late/bouncycastle.rs`) writes nine fields
    /// per 64-byte block. On the single-slot door each of those pays its own
    /// canonicalisation, corrupt-cell watch and descriptor resolution, and a
    /// block measured **5 893 ns** against HotSpot's 334 -- with the whole
    /// compression function already in Rust and the sixteen `processWord`
    /// decodes costing only 384 ns of it. The crossing, not the kernel and not
    /// the bytecode around it, was the wall.
    ///
    /// The default implementation loops over [`set_field`](Self::set_field), so
    /// an implementor with no cheaper route needs to do nothing.
    fn set_fields_typed(&self, obj: ObjectRef, slots: &[(usize, u8)], values: &[Value]) {
        for (slot, value) in slots.iter().zip(values.iter()) {
            let _ = slot.1;
            self.set_field(obj, slot.0, *value);
        }
    }

    /// Enter a read-only region on `obj`, validating it ONCE — see
    /// [`ReadCursor`] for the measurement that motivates it and for why the
    /// no-safepoint claim is a borrow-checker obligation rather than a comment.
    ///
    /// `None` when `obj` is not a live object on this heap, which is the same
    /// answer the checked accessors reach by returning defaults — a caller that
    /// gets `None` must fall back or bail, never dereference.
    ///
    /// The default implementation validates nothing extra and hands back a
    /// [`ReadCursor::checked`], so every accessor below is exactly today's
    /// behaviour for an implementor that does not override this.
    fn read_cursor(&self, obj: ObjectRef) -> Option<ReadCursor<'_>> {
        Some(ReadCursor::checked(obj))
    }

    /// A reference read THROUGH a cursor, promoted to a cursor of its own.
    ///
    /// This is the arm that removes most of a native's membership probes: a
    /// reference slot of a live object holds a live object, and inside a region
    /// no safepoint has intervened since the region's entry was validated, so
    /// the probe is re-answering a settled question.
    ///
    /// `value` MUST have been read through `parent` (one of the cursor
    /// accessors on this trait). Passing a reference from anywhere else — an
    /// argument, a side table, a previous call — defeats the premise, because
    /// only a read through the region carries its no-safepoint guarantee.
    ///
    /// `None` for a null, a non-reference `Value`, or a reference this
    /// implementation declines to trust.
    ///
    /// The default implementation validates through [`Self::read_cursor`], so
    /// it is exactly as safe as the checked path.
    fn reached(&self, parent: &ReadCursor<'_>, value: Value) -> Option<ReadCursor<'_>> {
        let _ = parent;
        match value {
            Value::Object(Some(obj)) => self.read_cursor(obj),
            _ => None,
        }
    }

    /// [`Self::get_field_typed`] through a cursor.
    ///
    /// Default: the checked accessor on the cursor's object.
    fn cursor_field_typed(&self, cursor: &ReadCursor<'_>, index: usize, descriptor: u8) -> Value {
        self.get_field_typed(cursor.object(), index, descriptor)
    }

    /// [`Self::get_fields_typed`] through a cursor.
    fn cursor_fields_typed(
        &self,
        cursor: &ReadCursor<'_>,
        slots: &[(usize, u8)],
        out: &mut [Value],
    ) {
        self.get_fields_typed(cursor.object(), slots, out);
    }

    /// [`Self::get_field_volatile`] through a cursor.
    ///
    /// Kept distinct from [`Self::cursor_field_typed`] because the ORDERING is
    /// not the region's to elide: a region proves nothing can move, not that a
    /// concurrent writer cannot publish. Only the membership probe and the
    /// forwarding barrier are skipped.
    fn cursor_field_volatile(&self, cursor: &ReadCursor<'_>, index: usize) -> Value {
        self.get_field_volatile(cursor.object(), index)
    }

    // --- read-and-promote, fused ---------------------------------------
    //
    // A reference read through a region is almost always read IN ORDER TO
    // walk it, so the natural spelling is `reached(&c, cursor_field(&c, i))`
    // — and that is two dynamic calls through `&dyn NativeContext` where the
    // checked accessor it replaces was one. MEASURED: the un-fused spelling
    // roughly doubled the walk's dynamic-call count and gave back most of
    // what the region saved — `ChmProbe get` was 13 % faster with the region
    // ON than OFF in the same binary, and FLAT against the binary without
    // the region at all. The three below are the same work in one call, and
    // they are the spelling a native should use.

    /// [`Self::cursor_field_typed`] of a REFERENCE slot, promoted to a cursor.
    ///
    /// `None` for null, a non-reference value, or a reference this
    /// implementation declines to trust.
    fn cursor_field_reference(
        &self,
        cursor: &ReadCursor<'_>,
        index: usize,
    ) -> Option<ReadCursor<'_>> {
        let value = self.cursor_field_typed(cursor, index, b'L');
        self.reached(cursor, value)
    }

    /// [`Self::cursor_field_volatile`] of a REFERENCE slot, promoted to a
    /// cursor.
    fn cursor_field_volatile_reference(
        &self,
        cursor: &ReadCursor<'_>,
        index: usize,
    ) -> Option<ReadCursor<'_>> {
        let value = self.cursor_field_volatile(cursor, index);
        self.reached(cursor, value)
    }

    /// [`Self::cursor_array_element`] of a REFERENCE array, promoted to a
    /// cursor.
    fn cursor_array_element_reference(
        &self,
        cursor: &ReadCursor<'_>,
        index: usize,
    ) -> Option<ReadCursor<'_>> {
        let value = self.cursor_array_element(cursor, index);
        self.reached(cursor, value)
    }

    /// [`Self::array_shape`] through a cursor.
    fn cursor_array_shape(&self, cursor: &ReadCursor<'_>) -> Option<(ArrayElementType, usize)> {
        self.array_shape(cursor.object())
    }

    /// [`Self::get_array_element`] through a cursor.
    fn cursor_array_element(&self, cursor: &ReadCursor<'_>, index: usize) -> Value {
        self.get_array_element(cursor.object(), index)
    }

    /// [`Self::class_id_of_object`] through a cursor — free once the region has
    /// the header.
    fn cursor_class_id(&self, cursor: &ReadCursor<'_>) -> crate::ClassId {
        self.class_id_of_object(cursor.object())
    }
    ...
    /// [`get_field_by_name`](Self::get_field_by_name) for several fields of ONE
    /// object at once; `out[i]` receives the value of `names[i]` (the shorter
    /// of the two bounds the batch, and a name that does not resolve reads as
    /// `Value::Object(None)`, exactly as the single-field read does).
    ///
    /// A native that reads a dozen fields of the same receiver per call pays
    /// the reference canonicalisation and the class lookup once instead of once
    /// per field. The default delegates to the single-field read, so an
    /// implementor with no cheaper path needs to do nothing.
    ///
    /// The names are `'static` on purpose: a string literal's address and
    /// length identify its content for the life of the process, which lets an
    /// implementation memoize `(class, name) -> slot` on the address alone,
    /// without hashing or comparing the characters.
    fn get_fields_by_name(&self, obj: ObjectRef, names: &[&'static str], out: &mut [Value]) {
        for (name, dst) in names.iter().zip(out.iter_mut()) {
            *dst = self.get_field_by_name(obj, name);
        }
    }

    /// [`set_field_by_name`](Self::set_field_by_name) for several fields of ONE
    /// object at once, applied in order. A name that does not resolve is
    /// dropped exactly as the single-field write drops it.
    ///
    /// The store half of [`get_fields_by_name`](Self::get_fields_by_name); the
    /// default delegates to the single-field write.
    fn set_fields_by_name(&self, obj: ObjectRef, fields: &[(&'static str, Value)]) {
        for (name, value) in fields {
            self.set_field_by_name(obj, name, *value);
        }
    }
    ...
    /// Resolve the slot declared by `java/lang/Throwable` for `field_name`.
    ///
    /// The default deliberately keeps this a normal by-name resolution. A VM
    /// whose Throwable layout is stable may override it with a **per-VM**
    /// cache; native code must never retain a slot index in a process global,
    /// because another VM in the same process can have a different bootstrap
    /// class layout.
    fn throwable_field_index(&self, field_name: &str) -> Option<usize> {
        self.resolve_field_index("java/lang/Throwable", field_name)
    }
...
pub trait NativeExceptionAccess: NativeHeapAccess {
    ...
    fn capture_throwable_stack_trace(
        &mut self,
        throwable: ObjectRef,
    ) -> std::sync::Arc<[StackTraceEntry]> {
        self.capture_stack_trace(self.identity_hash_code(throwable)).into()
    }

    /// Return the VM-local cached `StackTraceElement` class id when available.
    /// A default `None` keeps lightweight mock contexts source-compatible.
    fn cached_stack_trace_element_class_id(&self) -> Option<ClassId> {
        None
    }

    /// Record a resolved `StackTraceElement` id in VM-local state. The default
    /// is intentionally a no-op for contexts that do not own a VM.
    fn cache_stack_trace_element_class_id(&self, _class_id: ClassId) {}

    /// Return `Throwable.SUPPRESSED_SENTINEL` through VM-local cached metadata
    /// when available. The object itself is never cached here: it is movable
    /// heap state and must continue to be read from the static field.
    fn throwable_suppressed_sentinel(&self) -> Option<Value> {
        None
    }
...
pub trait NativeSystemAccess: NativeThreadAccess {
    ...
    /// [`class_id_of_object`](NativeClassAccess::class_id_of_object) and
    /// [`fast_unbox_primitive_wrapper`](Self::fast_unbox_primitive_wrapper) for
    /// a caller that wants BOTH answers about the same object.
    ///
    /// # why
    ///
    /// Because the second already reads the first. Every checked heap access
    /// in this API opens with an `is_object_address` membership probe and a
    /// forwarding barrier, and a caller that asks the two questions separately
    /// pays that pair twice to learn one class id. The map key comparisons do
    /// exactly that -- `chm_wrapper_chain_walk` asks for a node key's class to
    /// screen it against the lookup key's, then unboxes it, and
    /// `chm_wrapper_key_hash` + `chm_get_wrapper_key_fast` do the same to the
    /// lookup key itself.
    ///
    /// MEASURED (2026-09-20 dev build, `ChmProbe get`, 5 M
    /// `ConcurrentHashMap<Integer,Integer>.get`): 11.77 % of samples in
    /// `ZgcRealHeap::is_object_address` and 3.54 % in the `VmHeap` layer above
    /// it, spread over roughly fifteen checked accesses per lookup. This is the
    /// same "validate the receiver ONCE per call" direction
    /// `perf-collection-natives-cost-a-generic-native-dispatch-per-call`'s
    /// proposed fix 2 asks for, applied where two questions share one walk.
    ///
    /// The default implementation is the two calls, in that order, so an
    /// implementor with no fused path needs to do nothing and the answer is
    /// identical either way.
    /// `NativeClassAccess` is a supertrait of this one (through
    /// `NativeHeapAccess: NativeInvokeAccess: NativeClassAccess`), so this
    /// needs no `where` clause and stays dispatchable through
    /// `&dyn NativeContext` -- which is how every collection native holds its
    /// context.
    fn class_id_and_unboxed_primitive(
        &self,
        obj: ObjectRef,
    ) -> (crate::ClassId, Option<Option<Value>>) {
        (
            self.class_id_of_object(obj),
            self.fast_unbox_primitive_wrapper(obj),
        )
    }
    ...
    /// True iff the `CRATONVM_REAL` differential switch prefers real
    /// bytecode over a `SyntheticStub` native for `class_name` — what
    /// `vm::runtime::env_cache::real_bytecode_selector().prefers_real(..)`
    /// answers. `native-builtins` cannot depend on `vm` (the dependency runs
    /// the other way), so this is the `NativeContext` seam that lets a
    /// native ask the question without a circular crate edge.
    ///
    /// Exists because `java/util/Properties`'s native surface is retired
    /// (`RETIRED_SHADOW_PHASE3_TRIPLES`) on the precondition that
    /// `System.getProperties()`'s singleton carries a populated real `map`
    /// field before real bytecode can read it — true under `--jdk-only`
    /// (`native_system_get_properties_jdk_only`), but `CRATONVM_REAL=all`
    /// reaches the identical real-bytecode path in plain `Compatible` mode,
    /// where the singleton's `map` is still permanently null. See
    /// `jdk-only/the-system-properties-real-map-is-null-and-it-blocks-the-chm-retirement-20260909-RETIRED-20260921.md`.
    /// Default `false` (mock contexts have no differential switch).
    fn prefers_real_bytecode(&self, _class_name: &str) -> bool {
        false
    }

    /// True iff strict `--jdk-only` policy is in force for this VM
    /// (`CompatibilityMode::JdkOnly`).
    ///
    /// The seam for a native whose Rust-side REPRESENTATION of a JDK object is
    /// only readable by the natives that wrote it. Under `--jdk-only` real
    /// bytecode is authoritative and reads the object's real fields directly,
    /// so a representation that lives only in a side table is invisible to it.
    /// Default `false` (mock contexts are compatible-mode).
    fn is_jdk_only(&self) -> bool {
        false
    }
    ...
    /// The line separator to PUT ON THE WIRE -- the
    /// `java.lang.System.lineSeparator` static, which `initPhase1` captures
    /// once from the `line.separator` property.
    ///
    /// **Not `get_system_property("line.separator")`.** The JDK reads that
    /// property exactly once, at boot; `PrintStream.println`,
    /// `BufferedWriter.newLine`, `Properties.store` and `Files.write` all go
    /// through `System.lineSeparator()`, which is a plain field read. A later
    /// `System.setProperty("line.separator", ...)` -- or a write straight into
    /// the `System.props` object -- therefore changes nothing on HotSpot, and
    /// used to re-terminate every subsequent line on this VM.
    ///
    /// MEASURED on `apps/probes/SysPropsVmConsumers.java` against HotSpot
    /// 25.0.4+7: the property write was visible to `System.lineSeparator()`
    /// (`changed` against HotSpot's `unchanged`) AND to `println`, and under
    /// `--jdk-only` the two disagreed with each other -- an OBJECT write moved
    /// `println` while the accessor kept answering the old value, so the
    /// restore could put neither back and the probe's own output came out
    /// joined by the test separator. Two consumers of one property, two
    /// authorities, which is the split the retired
    /// `the-system-props-static-field-is-null` write-up's section 12
    /// predicted -- and its prescription (make the Rust store a cache of the
    /// `Properties` object) does not reach it, because the JDK reads neither.
    ///
    /// The live store is the fallback for the window before `initPhase1`
    /// step 3 runs -- which is where a `-Dline.separator=` override is already
    /// sitting, so that override is still honoured -- and the compile-time
    /// platform default the fallback for an image whose `java/lang/System`
    /// declares no such field (synthetic-JDK mode). It must be `\r\n` on
    /// Windows; several hand-rolled copies of this fallback carried `\n`.
    fn captured_line_separator(&self) -> String {
        if let Some(cid) = self.class_id_by_name("java/lang/System") {
            if let Some(idx) = self.static_field_index_by_name(cid, "lineSeparator") {
                if let Value::Object(Some(s)) = self.get_static_field(cid, idx) {
                    if let Some(text) = self.read_string(s) {
                        if !text.is_empty() {
                            return text;
                        }
                    }
        ...
        self.get_system_property("line.separator")
            .filter(|s| !s.is_empty())
...
pub struct StackTraceEntry {
    ...
    /// Exact descriptor of the captured method when it was available without
    /// a class-store lookup. Together with `method_name` this identifies an
    /// overload exactly for deferred line resolution.
    pub method_descriptor: Option<Arc<str>>,
    ...
    /// wrong method body. The descriptor is preferred when present; this index
    /// preserves an exact identity for legacy entries that predate it.
    ///
...
/// refresh; see
/// `docs/jdk-only/G48-1-the-input-side-hook-and-a-gate-that-could-go-quiet-20260817.md`.
...
pub struct NativeMethodRegistry {
    ...
    /// `java/io/StringReader`, `java/io/InputStreamReader`, `java/util/EnumSet`,
    /// `LinkedBlockingDeque`, and `ScheduledThreadPoolExecutor`.
    /// `StringJoiner` is registered by
...
impl NativeMethodRegistry {
    ...
    fn register_inner(
        ...
        // Real-JDK mode: drop synthetic `java/io/InputStreamReader` natives —
        // the same defect as `StringReader` above, one class over, found
        // 2026-09-17 when it took out the whole hibernate-reactive suite
        // (188/188 classes crashed before a single test ran).
        //
        // The synthetic surface parks the wrapped `InputStream` in slot 0 and
        // reads it back from there. On a real JDK 25 `InputStreamReader` slot 0
        // is `Reader.lock` — so the `<init>` shim clobbers the monitor object
        // and, decisively, never creates the real `sd` (`sun.nio.cs.StreamDecoder`)
        // that the genuine bytecode builds via `StreamDecoder.forInputStreamReader`.
        // The shim registers `read()I` but NOT `read([CII)I`, so the bulk read —
        // which is the one `BufferedReader.fill` calls, i.e. every `readLine()`
        // in the image, including `ServiceLoader`'s own provider-file parse —
        // keeps running real bytecode and dereferences that null `sd`:
        // `NullPointerException: Cannot invoke "sun.nio.cs.StreamDecoder.read(char[], int, int)"
        // because "this.sd" is null`.
        //
        // The registrar in `native-builtins/src/servlet.rs` now carries the
        // site-level `drops_real_layout_synthetic()` guard its `native-io` twin
        // grew on 2026-08-12 (W7-50). This entry is the mechanism-level half:
        // the cfg is a COMPILE-time question and this is the LOAD-time one, so
        // a future registrar that repeats the mistake is refused here rather
        // than shipping the same outage again.
        if self.drop_real_layout_synthetic && class_name == "java/io/InputStreamReader" {
            return;
        }
        ...
        // Real-JDK mode: drop the synthetic `java/io/BufferedReader.read`/
        // `read([CII)I` and the synthetic `java/io/BufferedWriter` six-method
        // surface, both registered by
        // `native-builtins/src/phases_late/nio_file.rs`. Found 2026-09-17
        // alongside `InputStreamReader` above — same family, same missing
        // guard, both classes seen carrying real bytecode in the probe run
        // that found this.
        //
        // `BufferedReader.read()I`/`read([CII)I`: when the object was not
        // built by the (now-retired) `Files.newBufferedReader` side-table
        // path, these delegate straight to the wrapped `Reader` at slot 0,
        // BYPASSING `cb`/`nChars`/`nextChar` and the mark()/reset()
        // bookkeeping that real `BufferedReader.fill` does. MEASURED: H2's
        // RUNSCRIPT/CSV/INIT BOM-skip does `mark(1); read(); reset()`, and the
        // native `read()` consumed a character the real `reset()` then
        // rewound past — dropping the first character of every script
        // (`"create table"` parsed as `"reate table"`). This is not the
        // InputStreamReader defect's failure mode (that one clobbers a field);
        // this one runs on the RIGHT field (slot 0 is genuinely the wrapped
        // Reader on a real `BufferedReader` in this VM's layout) but skips
        // real semantics the field exists to support.
        //
        // `BufferedWriter`'s six (`write(String,II)`, `write(I)`,
        // `write([CII)`, `newLine`, `flush`, `close`): the delegating arm
        // (reached whenever slot 0 holds a real `Writer` rather than a
        // synthetic fd) forwards straight to that `Writer`, never touching
        // `cb`/`nextChar`. MEASURED (`probes/L4StreamTailSweep.java`, row
        // `writer buffered`): `new BufferedWriter(sw, 4).write("ab")` reached
        // the delegate immediately, where HotSpot holds it in the buffer.
        // Real `BufferedWriter`/`BufferedReader` bytecode is self-contained
        // once the wrapped stream is real (which it is by default — the
        // fd-backed synthetic paths behind these are already retired from
        // shipping builds), so drop the synthetic surface and let it run.
        if self.drop_real_layout_synthetic
            && matches!(
                class_name,
        ...
        // Real-JDK mode: drop the synthetic `java/nio/CharBuffer.wrap`
        // factories (`wrap([C)` and `wrap(CharSequence)`,
        // `native-builtins/src/phases_late/charset_buffers.rs`,
        // `register_p62_char_buffer`).
        //
        // Same shape as the `java/io/StringWriter` entry just above: the
        // registration already sits behind `#[cfg(feature = "synthetic-jdk")]`
        // with a comment saying these are "DELIBERATELY not registered in
        // real-JDK mode", but a Cargo feature only decides what is COMPILED —
        // a feature-enabled binary RUN in real-JDK mode still registered both
        // over the real `java.nio.CharBuffer.wrap` bytecode. This is the
        // runtime half of that same intent, now paired with the matching
        // site-level `drops_real_layout_synthetic()` guard added there.
        //
        // `wrap(CharSequence)` is the concrete defect, not merely a
        // theoretical one: on HotSpot `CharBuffer.wrap(csq)` returns a
        // `StringCharBuffer` that HOLDS the `CharSequence` reference directly
        // — `isReadOnly() == true`, `hasArray() == false`, and a later
        // mutation of a mutable `csq` (e.g. a `StringBuilder`) is visible
        // through the buffer. The synthetic native instead COPIES the
        // sequence's chars into a fresh `char[]` and hands back a mutable
        // `HeapCharBuffer` (`isReadOnly() == false`, `hasArray() == true`) —
        // silently wrong rather than a crash: `put()` succeeds where HotSpot
        // throws `ReadOnlyBufferException`, and the buffer is frozen at
        // wrap-time instead of tracking the live sequence.
        //
        // It also feeds the CHARSET family below directly: Tomcat's
        // `MessageBytes.toBytes` is `encoder.encode(CharBuffer.wrap(charChunk))`,
        // so a wrongly-mutable, wrongly-copied `CharBuffer` would have flowed
        // straight into the real `CharsetEncoder` that entry restores. The
        // real `CharBuffer.wrap` bytecode is self-contained (it just builds a
        // real `HeapCharBuffer`/`StringCharBuffer` via the 3-arg overloads,
        // which are not intercepted anywhere in this file), so dropping the
        // synthetic surface is the whole fix.
        if self.drop_real_layout_synthetic
            && class_name == "java/nio/CharBuffer"
            && method_name == "wrap"
        {
            return;
        }
        ...
        // Real-JDK mode: drop the synthetic `java/io/CharArrayReader` natives
        // (found 2026-09-17,
        // synthetic-jdk-feature-shadows-real-classes-in-real-jdk-mode-20260917.md,
        // same family as `StringReader`/`InputStreamReader` above). The
        // native-io registrar's own RDR-MIGRATION comment already documents
        // the mismatch: the synthetic layout is `buf`/`pos`/`count` at slots
        // 0/1/2, but the real JDK 25 `CharArrayReader` inherits `Reader.lock`
        // at slot 0 — so `<init>` clobbers the monitor object with the
        // caller's `char[]` instead of setting it up, and only `read()I` was
        // ever implemented (not the bulk `read([CII)I` a real
        // `BufferedReader.fill` calls). The registrar's site-level guard was
        // cfg-only until this pass; this is the mechanism-level half. The
        // real `CharArrayReader` bytecode is self-contained (no missing
        // native primitive), so drop the synthetic surface and let it run,
        // same as `StringReader`/`InputStreamReader` above.
        if self.drop_real_layout_synthetic && class_name == "java/io/CharArrayReader" {
            return;
        }
        // Real-JDK mode: drop the synthetic `java/io/CharArrayWriter` natives.
        // Same 2026-09-17 sweep as `CharArrayReader` above. The native-io
        // registrar's own comment already names the concrete failure: the
        // synthetic 2-field carrier (`buf`/`count` at slots 0/1) collides
        // with the real JDK 25 layout, which is inherited `Writer.lock`
        // (slot 0) + `CharArrayWriter.buf` (slot 1) + `count` (slot 2) — so
        // the synthetic `<init>` never sets the inherited `lock`, and real
        // `write(String)` NPEs on `synchronized (lock)`. This is what turned
        // every JSP into an empty (HTTP 200, 0-byte body) servlet via
        // Jasper's `JspReader`. The real `CharArrayWriter` bytecode is
        // self-contained (its constructor chains through `Writer()`, which
        // sets `lock = this`), so drop the synthetic surface and let it run.
        if self.drop_real_layout_synthetic && class_name == "java/io/CharArrayWriter" {
            return;
        }
        // Real-JDK mode: drop the synthetic `java/io/LineNumberReader`
        // natives. Same 2026-09-17 sweep. `LineNumberReader` extends
        // `BufferedReader`, and the synthetic 4-field layout
        // (`in`/`lineNumber`/`pos`/`content` at slots 0-3) treats slot 0 as
        // the wrapped Reader, but on a real object in this Reader chain slot
        // 0 is inherited `Reader.lock` — so the synthetic `<init>` clobbers
        // the monitor instead of recording the wrapped Reader, and the real
        // `BufferedReader`/`Reader` bytecode built on top of that never sees
        // a delegate. The real `LineNumberReader` bytecode builds on real
        // `BufferedReader`, which is itself real by the time this runs, so
        // drop the synthetic surface and let it run.
        if self.drop_real_layout_synthetic && class_name == "java/io/LineNumberReader" {
            return;
        }
        // Real-JDK mode: drop the synthetic base-class `java/io/Reader.read`
        // natives (both `()I` and `(Ljava/nio/CharBuffer;)I` — scoped by
        // method name, not the full class, because `Reader.close()` is a
        // separate, universally-registered entry that must stay). Same
        // 2026-09-17 sweep. `native_sr_read` (backing `read()I`) looks up
        // `this` in a per-instance side table that only `StringReader.<init>`
        // populates; registered on the BASE class it intercepts every real
        // Reader subclass that inherits the concrete `read()I` body instead
        // of overriding it (FileReader, CharArrayReader, BufferedReader,
        // ...), and on any of those the lookup misses and throws "Stream
        // closed" instead of running the real, correct, concrete bytecode.
        // `read(CharBuffer)` is not itself unsafe (it mirrors the real
        // default via `invoke_virtual`), but it is registered in the same
        // guarded block as `read()I` in native-io, so it is dropped alongside
        // it here too — real `Reader.read(CharBuffer)` produces the identical
        // result, so there is no behavior to lose.
        if self.drop_real_layout_synthetic
        ...
        // Real-JDK mode: drop the synthetic base-class `java/io/Writer.write`
        // native (scoped by method name for the same reason as `Reader.read`
        // above — other `Writer`-family entries are unaffected). Same
        // 2026-09-17 sweep. `native_sw_write_int` (backing `write(I)V`) reads
        // and writes `SW_FIELD_BUF`/`SW_FIELD_COUNT` — `StringWriter`'s own
        // `char[]`/`count` slots. Registered on the BASE class it intercepts
        // every real Writer subclass that inherits the concrete `write(I)V`
        // body (BufferedWriter, OutputStreamWriter, PrintWriter, FileWriter,
        // CharArrayWriter, ...), where those slots are a different field of a
        // different type — `set_array_element`ing into whatever object sits
        // there corrupts it instead of writing the character. The real
        // `Writer.write(I)V` bytecode delegates to the subclass's
        // `write([CII)V`, so it runs correctly without a native.
        if self.drop_real_layout_synthetic
        ...
        // Real-JDK mode: drop the synthetic `java/nio/channels/FileLock`
        // instance natives (`<init>`/`position`/`size`/`isShared`/`isValid`/
        // `release`/`close` — distinct from `FileChannel.open` above, which
        // is the ABSTRACT `FileChannel` factory, not this class). Same
        // 2026-09-17 sweep; the native-io registrar's own header comment
        // already names the concrete failure: cratonvm's native-override
        // priority makes these SHADOW the concrete `sun/nio/ch/FileLockImpl`
        // methods, so `<init>` runs on the real `FileLockImpl` and writes its
        // in-process token into slot 5 — out of bounds on the real 5-field
        // layout the class comment measured via `javap` — and `release` only
        // clears that in-process token, never reaching the real
        // `FileChannelImpl.release` -> `FileLockTable.remove` path. A closed
        // lock then lingers in the JDK's per-file `FileLockTable`, and the
        // next `tryLock` on the same file throws `OverlappingFileLockException`
        // (H2 `SingleFileStore` reconnect / close+reopen). Real-JDK mode must
        // use the genuine `FileLockImpl` bytecode together with the
        // lock0/release0/FileKey natives that keep the `FileLockTable`
        // consistent, so drop this synthetic surface entirely.
        if self.drop_real_layout_synthetic && class_name == "java/nio/channels/FileLock" {
            return;
        }
        // Real-JDK mode: drop the synthetic `FileChannel.lock`/`tryLock`
        // NO-ARG natives (scoped by method name — the other `FileChannel`
        // entries, `open` above and `map`/`force`/`truncate`/`unmap0` etc.,
        // are unaffected). Same 2026-09-17 sweep, paired with the
        // `FileLock` entry above: these build a SYNTHETIC `FileLock` object
        // via the same synthetic FileLock natives that entry drops, so
        // leaving them registered here would hand back an object whose
        // instance natives no longer exist. Real-JDK mode uses the real,
        // final `FileChannel.lock()`/`tryLock()`, which delegate to the 3-arg
        // `FileChannelImpl` path and a genuine `FileLockImpl`.
        if self.drop_real_layout_synthetic
            && class_name == "java/nio/channels/FileChannel"
            && matches!(method_name, "lock" | "tryLock")
        {
            return;
        }
        ...
        // Real-JDK mode: drop the synthetic `java/lang/System`
        // `initPhase1`/`initPhase2`/`initPhase3` overrides
        // (`native-builtins/src/lib.rs`, `register_essential_natives_with_shims`).
        // The site's own comment already documents the intent — "only
        // override in synthetic-jdk mode. In real-JDK mode the actual JDK
        // bytecode runs, supported by the T14 ... natives" — but the
        // `#[cfg(feature = "synthetic-jdk")]` guarding it answers a
        // compile-time question, not a runtime one. `native_system_init_phase1`
        // unconditionally writes `System.out`/`err` static fields, stamps a
        // Charset onto them, and force-initializes the `System` class — real
        // VM-bootstrap side effects that the real `initPhase1` bytecode,
        // backed by the T14 `SystemProps$Raw`/`FileDescriptor`/
        // `Win32ErrorMode` natives named in that comment, already performs.
        // A `--features synthetic-jdk` binary run against a real JDK
        // registered this over the real method, so it ran INSTEAD of, not
        // alongside, the real bootstrap the T14 natives were built to
        // support. This entry is the mechanism-level half; the site-level
        // half is the matching `drops_real_layout_synthetic()` guard added
        // there.
        if self.drop_real_layout_synthetic
            && class_name == "java/lang/System"
            && matches!(method_name, "initPhase1" | "initPhase2" | "initPhase3")
        {
            return;
        }
        // Real-JDK mode: drop the synthetic JMX MXBean type-mapping overlay —
        // `ConvertingMethod.from`, `MXBeanMappingFactory`/
        // `DefaultMXBeanMappingFactory` `.mappingForType`/`.makeMapping`, and
        // `MXBeanMapping.toOpenValue`/`.fromOpenValue` (`native-builtins/src/
        // jmx_openmbean.rs`, `register_jmx_openmbean_natives_with`'s
        // `synthetic_mapping` arm).
        //
        // That file's own header already measured (2026-08-11, against JDK 25)
        // that leaving the REAL `com.sun.jmx.mbeanserver` bytecode alone is
        // strictly better than this overlay: with the primary fix (the
        // unconditional `MBeanIntrospector.getMethods` Object-method filter,
        // NOT part of this list) removing the `Class` -> `AnnotatedType[]`
        // self-reference at the source, the real `DefaultMXBeanMappingFactory`
        // recursion terminates and every platform-MXBean attribute matches
        // HotSpot exactly — `MemoryUsage` becomes a real `CompositeDataSupport`
        // instead of the overlay's raw `java.lang.management.MemoryUsage`
        // typed as `SimpleType.STRING`, `SystemProperties` becomes a real
        // `TabularDataSupport`, and a genuinely self-referential MXBean type is
        // rejected with HotSpot's own `NotCompliantMBeanException` instead of
        // being silently accepted as a String. That measurement is why
        // `real_mxbean_mapping_enabled()` already makes this overlay OFF by
        // default on a real-JDK run of the default build.
        //
        // The gate at that call site used to be `cfg!(feature =
        // "synthetic-jdk")` alone, which asks what was COMPILED rather than
        // which CLASS LIBRARY is LOADED: a `--features synthetic-jdk` binary
        // run against a real JDK satisfied the cfg and installed the overlay
        // over real, loadable bytecode anyway. The site now also reads
        // `drops_real_layout_synthetic()`; this entry is the mechanism-level
        // half, same relationship `InputStreamReader`/`StringWriter` have to
        // their own site-level guards above.
        //
        // Also confirmed NOT self-inflicted by a missing native: none of
        // `ConvertingMethod`/`MXBeanMapping`/`MXBeanMappingFactory`/
        // `DefaultMXBeanMappingFactory` is in `classloading::class_manager`'s
        // synthetic-field-count fallback table for any reason other than "the
        // real JDK class file can't be loaded" (see that table's own
        // System$1/System$2 and FileSystemException entries) — the fallback
        // is a last resort, not evidence the real class needs one.
        if self.drop_real_layout_synthetic
            && matches!(
                class_name,
        ...
        //
        // RETIRED 2026-09-17 (lane-concurrency-forkjoin-scheduler-20260917.md
        // item 2): the 3-arg constructor and getCorePoolSize()I used to be
        // kept here as real-layout Bridges for Spring's
        // ThreadPoolTaskScheduler anonymous subclass. A keep arm here cannot
        // coexist with a `RETIRED_SHADOW_L5_TRIPLES` entry for the same two
        // triples (the `Bridge -> SyntheticStub` re-tag in
        // `NativeMethodRegistry::register` runs before this predicate is ever
        // consulted, so a table entry drops the native in real-JDK mode
        // regardless of what this arm says) -- see that table's own comment.
        // Both triples are now in the table instead; this whole class drops
        // unconditionally like its neighbours above. Measured before landing:
        // `apps/probes/L5StpeKeepArm.java` (7/7, both modes, unchanged) and
        // the real-JDK Spring `scheduling.concurrent.*` cluster this arm's
        // own comment named (10 classes, 200 test methods, 189 passed/0
        // failed/11 pre-existing skips, byte-identical against the tree with
        // the keep arm still in place) -- see the FIXED write-up.
        if self.drop_real_layout_synthetic
...
mod tests {
    ...
    fn real_layout_mode_drops_enumset_native_surface() {
        ...
        // `java/io/InputStreamReader`: the 2026-09-17 addition. Both the
        // `<init>` that parks a stream in `Reader.lock`'s slot and the partial
        // `read()I` surface that leaves the bulk `read([CII)I` on real bytecode
        // must be refused — registering EITHER on a real layout is what left
        // `sd` null and NPE'd every `readLine()` in the image.
        real_layout.register(
        ...
        assert!(real_layout
            ...
            .is_none());
        real_layout.register("java/io/InputStreamReader", "read", "()I", dummy_native_2);
        assert!(real_layout
            .find("java/io/InputStreamReader", "read", "()I")
            .is_none());

        // ... and the same triples DO survive on a registry that is not the
        // real-JDK arm, so the assertions above are the filter talking and not
        // a registration that silently never happens.
        let mut synthetic = NativeMethodRegistry::new();
        synthetic.set_category(NativeKind::SyntheticStub);
        synthetic.register(
            "java/io/InputStreamReader",
            "<init>",
            "(Ljava/io/InputStream;Ljava/nio/charset/Charset;)V",
        ...
        // `keep_real_scheduled_executor_bridge` used to keep exactly this
        // triple alive under `Bridge` here -- the one real-layout survivor
        // this test relied on as its control. It is retired now (both
        // triples it kept are in `RETIRED_SHADOW_L5_TRIPLES` instead,
        // lane-concurrency-forkjoin-scheduler-20260917.md item 2), so `Bridge`
        // no longer saves it: this class drops unconditionally in real-layout
        // mode, same as `LinkedBlockingDeque` above.
  ...
  ")
    ...
    fn stack_trace_entry_clone_and_debug() {
        let entry = StackTraceEntry {
            ...
            method_descriptor: Some(Arc::from("()I")),
    ...
    fn stack_trace_entry_native_method() {
        let entry = StackTraceEntry {
            ...
            method_descriptor: Some(Arc::from("(Ljava/lang/Object;ILjava/lang/Object;II)V")),
            source_file: None,
