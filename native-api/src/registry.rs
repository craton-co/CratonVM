...
pub trait NativeInvokeAccess: NativeClassAccess {
    ...
    ///
    /// Restored 2026-07-27: the `refactor(native): split capabilities and
    /// select app packs` merge on `dev` dropped this trait method while
    /// leaving its only caller (`native-builtins/src/lang_class.rs`'s
    /// `native_method_invoke`) in place, so `dev` did not compile.
    ///
