...
const RETIRED_SHADOW_PREFIXES: &[&str] = &[
    ...
    // (`docs/jdk-only/the-ffm-carrier-is-the-vms-own-allocation-shape-20260829.md`),
    ...
    // is in
    // `docs/jdk-only/the-builtin-classloader-could-not-link-and-getname-was-never-tagged-20260910.md`;
...
/// `docs/jdk-only/the-system-properties-real-map-is-null-and-it-blocks-the-chm-retirement-20260909.md`.
///
...
/// # What was measured
///
/// `docs/jdk-only/heap-bytebuffer-and-chm-run-interpreted-under-jdk-only-20260918.md`. One release
...
/// (`docs/jdk-only/the-ffm-carrier-is-the-vms-own-allocation-shape-20260829.md`: the segment, arena
...
/// `docs/jdk-only/W7-22-shadow-retirement-logging-and-time.md`,
...
/// (`docs/known-issues/jdk-only-lanes/lane-4-io-nio-foreign.md` §9 Wave 9).
///
...
/// delegates to the three-argument form (retired earlier).
///
/// NOT in the table: `File.deleteOnExit()`. Its bytecode registers
/// `DeleteOnExitHook` through `JavaLangAccess.registerShutdownHook`, and this
/// VM does not run the JDK's `java.lang.Shutdown` hook slots at exit
/// (`shutdown hooks: ran=0` in the run summary). Measured with
/// `apps/probes/L4FileTempExit.java`: with the row retired, HotSpot leaves the
/// probe's temp directory empty and this VM leaves the file, the directory and
/// its child behind. The native keeps its own exit list; retiring it needs the
/// VM to drive `Shutdown`'s slots, which is not a `sun/nio` change.
...
/// Lane 4 wave 17: 65 rows over 20 classes -- the buffer, `CoderResult`,
/// `ByteOrder`, `FileDescriptor$1`, `NativeThreadSet`, `FileChannelImpl` and
/// stream-constructor rows the funnel still counted after waves 1-16.
///
...
mod tests {
    ...
    fn the_typed_buffer_families_beside_charbuffer_are_not_retired() {
        ...
        for c in [
            ...
            for (m, d) in [
                ...
                assert!(
                    wave17_measured || !triple_is_retired_shadow(c, m, d),
    ...
    fn wave_seventeen_files_and_provider_rows() {
        ...
        assert_eq!(RETIRED_SHADOW_L4_FILES2_TRIPLES.len(), 57);
        ...
        assert_eq!(n("sun/nio/fs/WindowsFileSystemProvider"), 9);
    ...
    #[test]
    fn wave_seventeen_buffers_are_sixty_five_rows_over_twenty_classes() {
        assert_eq!(RETIRED_SHADOW_L4_BUFFERS2_TRIPLES.len(), 65);
    ...
    fn wave_seventeen_file_temp_is_one_row_and_delete_on_exit_stays() {
        assert_eq!(RETIRED_SHADOW_L4_FILETMP_TRIPLES.len(), 1);
        for (c, m, d) in RETIRED_SHADOW_L4_FILETMP_TRIPLES {
            assert!(triple_is_retired_shadow(c, m, d), "{c}.{m}{d}");
        }
        assert!(!triple_is_retired_shadow("java/io/File", "deleteOnExit", "()V"));
    }

    /// Wave 16's 23 rows: every one is in the table, none is a duplicate of an
    /// earlier lane-4 table's row, and the three rows the table's doc comment
    /// names as held or blocked stay out.
    #[test]
    fn wave_sixteen_winfs_is_twentythree_rows() {
        assert_eq!(RETIRED_SHADOW_L4_WINFS_TRIPLES.len(), 23);
