...
pub fn triple_is_retired_shadow(class_name: &str, method_name: &str, descriptor: &str) -> bool {
    ...
    RETIRED_SHADOW_TRIPLES.binary_search(&key).is_ok()
        ...
        || RETIRED_SHADOW_LT_TRIPLES.binary_search(&key).is_ok()
}
...
mod tests {
    ...
    #[test]
    fn the_lane_t_table_is_disjoint_from_the_earlier_three() {
    ...
    /// `java/lang/` is NOT a prefix, and this is the lane-T half of
    /// [`a_prefix_alone_retires_nothing`]. 50 class names were added to
    /// [`RETIRED_SHADOW_PREFIXES`] rather than three package prefixes, and the
    /// difference is only observable through a name the tables do not carry.
    #[test]
    fn the_lane_t_prefixes_are_class_names_not_packages() {
        // Sibling `java/lang/` classes with registered natives, none of them
        // throwables, none of them admitted.
        for c in [
            "java/lang/String",
            ...
            assert!(
                !RETIRED_SHADOW_PREFIXES.iter().any(|p| c.starts_with(p))
                    || c.starts_with("java/util/")
                    || c.starts_with("java/text/"),
                "{c} is admitted by a lane-T prefix that should be a class name"
            );
        }
    }
