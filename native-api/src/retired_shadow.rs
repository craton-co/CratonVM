...
const RETIRED_SHADOW_PREFIXES: &[&str] = &[
    ...
    //
    // L0's prefixes are absent for a different reason: its table is EMPTY
    // (two corpus rounds withdrew all 54), so `java/lang/Class` and
    // `java/lang/Module` would cover nothing at all.
];
...
pub(crate) const RETIRED_SHADOW_TABLES: &[&[(&str, &str, &str)]] = &[
    ...
    // THIRD AND FOURTH, found by this merge on 2026-09-11: `origin/dev`'s tip
    // shipped this const listing SEVEN tables while its own predicate consulted
    // NINE. Lane 1's wave 3 (`L1_HM`, java/util/HashMap's 98) and wave 4
    // (`L1_JT`, jar and text, 41) were both consulted and both unlisted, so
    // neither had ever been asked the real-JDK keep-arm question. Nothing on
    // `dev` was red about it in a way anyone acted on, which is the point: the
    // omission is invisible to every per-table test and to the corpus.
    //
    // Four occurrences over three lanes in two days. It is structural, and the
    // note lane 7 left above says why: a table is DEFINED in one place,
    // CONSULTED in a second and LISTED in a third, and nothing textual
    // connects the three. The durable repair is to make the predicate iterate
    // this const so the second and third places become one; that is a
    // single-lane change to a file six lanes are editing daily, so it is
    // recorded here as the fix rather than taken mid-merge.
