...
/// armed together, so it is in the dial's blind spot described above. Eighteen
/// retired triples are invoked by that vector at all; the file-handle group
/// among them -- `FileOutputStream` (whose `close()` is the flush that commits
/// the keystore the test then reads back), `FileCleanable`, `FileDescriptor`,
/// and the abstract-receiver `java/nio/channels/FileChannel` -- is out as a
/// GROUP, un-attributed per triple. Naming the blocker honestly: the instrument
/// that would attribute it is a runtime exclusion switch on this table, which
/// would make a per-triple bisection cost one run instead of one build.
///
...
/// its absence is what cost this wave two of its three builds.
///
