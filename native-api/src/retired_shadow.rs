...
//! W7-56-infercaller-strict-RETIRED-20260917.md
//!
...
//! correct only via the eager stamp. W7-56-infercaller-strict-RETIRED-20260917.md
//!
...
//! docs/internal/jdk-only/P2-COLLECTIONS-SHADOWS-20260812-RETIRED-20260917.md. **Eight are
...
static RETIRED_SHADOW_TRIPLES: &[(&str, &str, &str)] = &[
    ...
    // docs/internal/jdk-only/P2-COLLECTIONS-SHADOWS-20260812-RETIRED-20260917.md.
    //
...
const RETIRED_SHADOW_PREFIXES: &[&str] = &[
    ...
    // (`docs/internal/jdk-only/the-ffm-carrier-is-the-vms-own-allocation-shape-20260829.md`),
    ...
    // is in
    // `docs/internal/jdk-only/the-builtin-classloader-could-not-link-and-getname-was-never-tagged-20260910.md`;
    ...
    // 2026-09-12, L1 wave 9. NARROW again, for the third time and the same
    // reason: `RETIRED_SHADOW_L1_DATE_TRIPLES` retires nine rows on
    // `sun/util/calendar/ZoneInfo`, and the locale-provider half of
    // `sun/util/` is still measured NOT retirable, so the package prefix is
    // still the wrong instrument.
    //
    // This string SUBSUMES wave 5's `sun/util/calendar/ZoneInfoFile` above --
    // the screen is `starts_with`, and `ZoneInfoFile` starts with `ZoneInfo`.
    // Wave 5's line stays because deleting it would take its note with it and
    // leave `the_zone_info_file_prefix_retires_only_the_two_measured_rows`
    // guarding a string no longer in the file. Two entries, one of them
    // redundant, is the cheaper of the two mistakes available here.
    "sun/util/calendar/ZoneInfo",
    // 2026-09-19, the `--jdk-only` throughput wave. NARROW, for the reason the
    // `sun/util/` ones above are: `jdk/internal/util/` is a large tree
    // (`ArraysSupport`, `Preconditions`, the `random` and `xml` packages) whose
    // other members are `Intrinsic`s and bridges nobody has measured, and
    // `RETIRED_SHADOW_JOTP_TRIPLES` retires exactly three rows under it. The
    // guard is `the_jotp_wave_is_the_three_int_precondition_checks`.
...
static RETIRED_SHADOW_L3_TRIPLES: &[(&str, &str, &str)] = &[
    ...
    // `Method.getAnnotatedReturnType()` was retired here on an A/B probe that
    // never put a TYPE_USE annotation on a return type. UN-RETIRED 2026-09-18:
    // the real body reads `getTypeAnnotationBytes0()` (null in this VM: there is
    // no constant-pool accessor to decode `RuntimeVisibleTypeAnnotations`
    // against), so under `--jdk-only` every JSpecify `@Nullable`/`@NonNull` on a
    // return type read as unspecified (`NullnessTests`, five methods). The
    // native is registered `Intrinsic` in `native-builtins/src/lib.rs`.
    (
...
/// `jdk-only/the-system-properties-real-map-is-null-and-it-blocks-the-chm-retirement-20260909-RETIRED-20260921.md`.
///
...
static RETIRED_SHADOW_L5_TRIPLES: &[(&str, &str, &str)] = &[
    ...
    // and `getCorePoolSize()I` were REMOVED on 2026-09-10, before this table
    // ever landed, because `registry.rs`'s `keep_real_scheduled_executor_bridge`
    // kept them in REAL-JDK mode for Spring's `ThreadPoolTaskScheduler`
    // anonymous subclass, and a retirement table cannot coexist with a keep
    // arm on the same two triples (see this module's header, "a retirement is
    // mode-blind and a keep arm is not"). RETIRED 2026-09-17
    // (lane-concurrency-forkjoin-scheduler-20260917.md item 2): the keep arm
    // is deleted in the SAME change as these two rows landing, per that
    // page's own §4 — the two do not coexist. The real-JDK Spring
    // `scheduling.concurrent.*` cluster this arm was written to protect
    // (`ThreadPoolTaskSchedulerTests` and friends, 200 test methods) is
    // byte-identical with the arm gone; see the FIXED write-up.
...
/// `docs/internal/jdk-only-lanes/lane-1-util-text-time-RETIRED-20260912.md`.
///
/// # Why these six and not the other ten
///
/// `docs/.../lane-1-util-text-time-RETIRED-20260912.md` §3 states the rule this wave is an
...
/// Lane 1 wave 7, 2026-09-12: `java/util/Hashtable`, all 79, and what could
/// not go before was not the state model.
///
/// # What the page said, and what the binary says
///
/// Section 1 held this family as "state is a Rust side table, so retiring
/// hands real bytecode an empty map", and section 10 item 4 had it at `+10`
/// armed with the ten rows all VIEWS answering EMPTY. Both readings are
/// refuted by one run. On `cratonvm-l1w7-base-20260912`, `--jdk-only`,
/// UNARMED, read through `--add-opens java.base/java.util=ALL-UNNAMED`
/// (`apps/probes/L1MapStateDiag`, HotSpot 25.0.4+7 as the oracle):
///
/// ```text
///   Hashtable.table    [Ljava.util.Hashtable$Entry;  len=11 nonNull=3
///                      nodes java.util.Hashtable$Entry     SAME as HotSpot
///   Hashtable.count    3                                   SAME
///   Hashtable.modCount 3    threshold 8                    SAME
/// ```
///
/// The state IS real -- `H23-2`'s node-class move landed it -- so the armed
/// arm was measuring a DECODE, not a store. Every one of those ten rows was
/// `toArray()` over a view the image's own bytecode minted, and
/// `hs_backing_map` answered `None` for it: the source map is in `this$0`,
/// and this VM's own carriers keep it in a trailing slot the real class does
/// not have. `native-collections`' `hs_backing_map` and `hs_view_elements`
/// now read it by name, and the arm moves with them:
///
/// ```text
///   java/util/Hashtable armed, one class per process, whole probe tree
///     control  cratonvm-l1w7-base-20260912   7 probes worse, 30 rows
///     trial    cratonvm-l1w7-t3-20260912     0 probes worse
///       L1MapViewToArrayProbe (168 rows)     30 -> 0
///       L1EntrySetRouteProbe                 10 -> 0
/// ```
///
/// # Why the family retires WHOLE
///
/// `Hashtable.entrySet()` hands back a `Collections$SynchronizedSet` around
/// the inner carrier, so the accessor and the carrier are one unit: retire the
/// accessor alone and a real inner view meets natives that cannot read it;
/// retire the carrier alone and a VM-minted view meets real bytecode. The 79
/// are `Hashtable` (22), its three view classes (56) and `$Entry` (1).
///
/// # `Properties` is not in the blast radius, and that is measured
///
/// `Properties extends Hashtable`, and JDK 9 moved its storage into
/// `Properties.map`; it overrides the whole `Map` surface, so a dispatch door
/// asking about the DECLARING class reaches `Properties` and never
/// `Hashtable`. `PropertiesCloneProbe`, `SysPropsRealMapProbe` and the
/// `F.props.*` rows of `apps/probes/L1MapViewToArrayProbe` read identically on
/// the control and the trial.
static RETIRED_SHADOW_L1_HT_TRIPLES: &[(&str, &str, &str)] = &[
    ...
    ("java/util/Hashtable$EntrySet", "hashCode", "()I"),
    ("java/util/Hashtable$EntrySet", "isEmpty", "()Z"),
    (
        "java/util/Hashtable$EntrySet",
        "iterator",
        "()Ljava/util/Iterator;",
    ...
    ("java/util/Hashtable$KeySet", "add", "(Ljava/lang/Object;)Z"),
    (
    ...
    ("java/util/Hashtable$KeySet", "hashCode", "()I"),
    ("java/util/Hashtable$KeySet", "isEmpty", "()Z"),
    (
        "java/util/Hashtable$KeySet",
        "iterator",
        "()Ljava/util/Iterator;",
    ...
    ("java/util/Hashtable$ValueCollection", "isEmpty", "()Z"),
    (
        "java/util/Hashtable$ValueCollection",
        "iterator",
        "()Ljava/util/Iterator;",
...
///     its own page (fixed 2026-09-17):
///     `core-reflection-has-no-member-filter-20260911.md`.
...
/// `--jdk-only` throughput wave, 2026-09-19 -- the three `Preconditions` bounds checks, 3 rows.
///
/// # What was measured
///
/// `docs/internal/jdk-only/heap-bytebuffer-and-chm-run-interpreted-under-jdk-only-20260918.md`. One release
/// binary, one probe (`tools/probes/NativeCrossingPerf.java`), only the mode differing:
///
/// ```text
///                                   default      --jdk-only    --jdk-only, this table armed
///   Objects.checkIndex                 4.5 ns        278 ns        7.7 ns
///   heap ByteBuffer.get(int)          20.7 ns        341 ns       27.3 ns
/// ```
///
/// `Preconditions.checkIndex` is the whole of the first row. Under `--jdk-only` the JIT's thin
/// direct helper for it is refused (`direct_native_helper` admits a helper only for a reviewed
/// `Intrinsic`, and this is a `Bridge`), so a compiled caller pays the generic dispatch and the
/// registered native RUNS in front of real bytecode: the census carries the same triple as both
/// `[bytecode-won]` and `[native-won]`, depending on the door. Arming the dispatch-time dial
/// (`CRATONVM_ENFORCE_NATIVE_SHADOW=jdk/internal/util/Preconditions`) does NOT measure this
/// retirement -- it made `Objects.checkIndex` 3x SLOWER (485 -> 1576 ns), because a method with a
/// registered native is never compiled (`native_skip`), so the yielded bytecode ran interpreted.
/// Only a refused registration takes the native out of the JIT's sight.
///
/// # Why each row is safe to retire
///
/// The three `int` overloads that take a `BiFunction`. The real bodies are `if (index < 0 || index >=
/// length) throw outOfBounds(oobef, ...); return index;` and are HotSpot intrinsic candidates. The
/// native's reason to exist -- honouring the exception formatter -- is `Preconditions.outOfBounds`'s
/// own job once the real class is the one running (`native-builtins/src/preconditions.rs`). The
/// non-JDK `(II)I` / `(III)I` spellings registered beside them have NO bytecode to yield to and are
/// deliberately not here.
///
/// # What this wave REFUSED, measured -- `Buffer.session()` / `checkSession()`
///
/// The first cut of this wave also retired those two on the ten buffer classes that register them
/// (`CharBuffer` took the same pair in wave 4), on the argument that the real `session()` is
/// `segment != null ? segment.sessionImpl() : null` and `segment` is `null` for a buffer that is not
/// a `MemorySegment` view. **That was wrong, and the corpus said so in seconds.** On the same
/// release binary, only this pair's rows differing:
///
/// ```text
///                                    session/checkSession retired    kept as shims
///   PcapWriteHandlerTest                     8 ok / 17 failed          25 ok / 0 failed
///   AdaptiveBigEndianDirectByteBufTest     339 ok / 76 failed         415 ok / 0 failed
///   every failure:  NoSuchMethodError: 'void jdk.internal.foreign.ArenaImpl.checkValidStateRaw()'
/// ```
///
/// A buffer whose `segment` is NOT null -- a direct buffer over a `MemorySegment`, which netty's
/// allocators make -- gets its real `sessionImpl()` run against this VM's own arena carrier
/// (`docs/internal/jdk-only/the-ffm-carrier-is-the-vms-own-allocation-shape-20260829.md`: the segment, arena
/// and session carriers are laid out deliberately unlike the JDK's, so their real bodies must never
/// run), and the invoke resolves against a class that never declares the method. The constant-`null`
/// shim is not a shortcut over correct bytecode there; it is what stops that bytecode running.
/// `CharBuffer` is not a counter-example: nothing in the corpus makes a segment-backed `CharBuffer`.
///
/// The cost of keeping them is small: a compiled `session()` site binds the thin `buffer_session`
/// helper regardless of policy, so the pair was never what made the accessors slow.
/// `the_jotp_wave_refuses_the_buffer_session_pair` pins the twenty rows out, by name.
///
/// Compatible mode is untouched: a `SyntheticStub` registers and dispatches normally there.
static RETIRED_SHADOW_JOTP_TRIPLES: &[(&str, &str, &str)] = &[
    (
        "jdk/internal/util/Preconditions",
        "checkFromIndexSize",
...
/// `--jdk-only` throughput wave, part two, 2026-09-19 -- the sixteen `Unsafe.*Unaligned`
/// accessors, which `RETIRED_SHADOW_L5S_TRIPLES`'s doc and
/// `the_l5s_wave_refuses_the_three_families_that_cannot_be_retired` recorded as "permanently
/// blocked, not pending". They are not, and the reason that record gives does not reach them.
///
/// # What the record said, and what it was about
///
/// "Every `*Unaligned` row does byte-offset arithmetic on the offset it is handed, and this VM
/// answers `objectFieldOffset` with a SLOT INDEX, so the JDK's arithmetic lands nowhere." That is
/// true of an offset taken from `objectFieldOffset` -- and no `*Unaligned` accessor is ever handed
/// one: they exist for byte-addressed memory, i.e. arrays and off-heap addresses. Both of those ARE
/// byte-offset models here, and the model is already implemented by the natives the real bytecode
/// composes:
///
/// ```text
///   getIntUnaligned(o, off)      real:  (off & 3) == 0  ->  getInt(o, off)
///                                       (off & 1) == 0  ->  makeInt(getShort(o, off), getShort(o, off + 2))
///                                       otherwise       ->  makeInt(getByte(o, off) .. getByte(o, off + 3))
///   getIntUnaligned(o, off, be)  real:  convEndian(be, getIntUnaligned(o, off))
/// ```
///
/// and `getInt`/`getShort`/`getLong(Object,long)` are `ACC_NATIVE` -- which §1.5 says can never be
/// retired -- bound to `native_unsafe_get_int_mb` and its siblings in
/// `native-builtins/src/unsafe_natives_ext.rs`. Those are the SAME handlers the sixteen shadows
/// here are registered to (the source says so: "the `*_mb` handlers do exactly that"). The shadows
/// therefore add nothing the composition does not already reach through byte-addressed natives; they
/// only stand in front of it.
///
/// # What they cost
///
/// A shadowed method is never compiled, and the census carries these as both `[bytecode-won]` and
/// `[native-won]`. Priced in isolation on a release binary
/// (`tools/probes/UnsafeCrossingPerf.java`), array target, one call:
///
/// ```text
///                                       default     --jdk-only
///   Unsafe.getInt(arr, off)  [ACC_NATIVE]  267 ns       268 ns
///   Unsafe.getIntUnaligned(arr, off, be)   277 ns       849 ns
///   Unsafe.putIntUnaligned(arr, off, v, be) 302 ns      875 ns
/// ```
///
/// -- the shadowed 3-arg method under `--jdk-only` costs three times the native it stands in front
/// of. They sit under every heap `ByteBuffer.get{Short,Char,Int,Long}` / `put*`, through the
/// already-retired `ScopedMemoryAccess` layer.
///
/// # The instrument that decides it
///
/// `tools/probes/UnsafeUnalignedMatrixProbe.java`, which this record's predecessor wrote for
/// exactly this family: every width and both byte orders over `byte[]`-backed heap AND direct
/// buffers at every alignment within a word, then reads that SPAN several elements of
/// `char[]`/`int[]`/`long[]` (the shape `ArraysSupport.vectorizedMismatch` produces, and the one a
/// truncating read gets wrong), then `Arrays.mismatch`. Diffed against HotSpot's transcript, under
/// `--jdk-only` with this table armed.
///
/// The sub-word CAS/atomic rows and the four that report the VM's own numbering
/// (`objectFieldOffset`, `staticFieldOffset`, `staticFieldBase`, `arrayIndexScale`) stay exactly
/// as `the_l5s_wave_refuses_the_three_families_that_cannot_be_retired` left them: their offsets
/// really are slot indices, and the reasoning for them is unchanged.
///
/// A separate table, not rows in `RETIRED_SHADOW_JOTP_TRIPLES`, so `CRATONVM_UNRETIRE_NATIVE_SHADOW`
/// can disarm one half of the throughput wave without the other.
static RETIRED_SHADOW_JOTP_UNALIGNED_TRIPLES: &[(&str, &str, &str)] = &[
    (
        "jdk/internal/misc/Unsafe",
        "getCharUnaligned",
        "(Ljava/lang/Object;J)C",
...
/// because it was forgotten: it was blocked on a concrete `Path`/
/// `FileSystemProvider`/`FileSystem` carrier until 2026-09-17, when the last
/// of those landed; see
/// `the_provider_family_retirement_is_four_of_nine_bucket_ab_rows` for the
/// wave that closed the blockers and what stayed excluded regardless.
///
...
/// `docs/internal/jdk-only/W7-22-shadow-retirement-logging-and-time.md`,
...
/// Lane 4 wave 7, the `java/io/PrintStream` wave's `java/nio/file/Path` cousin:
/// nine rows (a tenth candidate the census named is measured unsafe -- see
/// below) that `636ef84ed` ("fix(nio): stamp java.nio.file.Path receivers as
/// the real platform class") made retirable for the first time.
///
/// # Why now and not earlier
///
/// `java.nio.file.Path` is an interface, so every native-built `Path` this VM
/// ever handed out had to be a `new` on SOME class. Until `636ef84ed`,
/// `p57_alloc_path`/`p57_alloc_path_raw` in
/// `native-builtins/src/phases_late/nio_file.rs` minted that receiver under the
/// literal interface name `java/nio/file/Path` -- illegal per JVMS §6.5 and,
/// more to the point for this table, a receiver whose runtime class was never
/// going to run real bytecode for anything, retired or not: an
/// `InstantiationError`-shaped object has no image method to fall through to.
/// `636ef84ed` mints through `concrete_receiver::alloc_concrete` against
/// `path_layout::impl_class()` instead (`sun/nio/fs/WindowsPath` on this
/// platform, `sun/nio/fs/UnixPath` on Unix) and mirrors every
/// `register_phase57_nio_file` registration made under the interface name onto
/// that concrete class, which is what first gives these rows a real
/// receiver to land on.
///
/// # Why these nine (and one excluded tenth) and not the other ~21 `java/nio/file/Path` rows
///
/// `javap -p java.nio.file.Path` on this machine's JDK 25 (Microsoft build
/// 25.0.3.9) draws the line directly: these ten (nine retired, one excluded
/// below) are the DEFAULT or STATIC methods the interface declares WITH a
/// body --
///
/// ```text
///   public static java.nio.file.Path of(java.lang.String, java.lang.String...);
///   public static java.nio.file.Path of(java.net.URI);
///   public default boolean startsWith(java.lang.String);
///   public default boolean endsWith(java.lang.String);
///   public default java.nio.file.Path resolve(java.lang.String);
///   public default java.nio.file.Path resolveSibling(java.nio.file.Path);
///   public default java.nio.file.Path resolveSibling(java.lang.String);
///   public default java.io.File toFile();
///   public default java.nio.file.WatchKey register(java.nio.file.WatchService, java.nio.file.WatchEvent$Kind<?>...) throws java.io.IOException;
///   public default java.util.Iterator<java.nio.file.Path> iterator();
/// ```
///
/// -- while the other ~21 (`compareTo`, `equals`, `getFileName`,
/// `getFileSystem`, `getName`, `getNameCount`, `getParent`, `getRoot`,
/// `hashCode`, `isAbsolute`, `normalize`, `register` with the 3-arg
/// `Modifier...` descriptor, `relativize`, `resolve(Path)`, `startsWith(Path)`,
/// `subpath`, `toAbsolutePath`, `toRealPath`, `toString`, `toUri`,
/// `endsWith(Path)`) are `public abstract` on `Path` itself -- no `Code`, so
/// retiring their `("java/nio/file/Path", ...)` registration would make the
/// fallback an unimplemented interface method instead of a working body. A
/// second `javap -p sun.nio.fs.WindowsPath` confirms the receiver side of the
/// same fact: `WindowsPath` overrides every one of those 21 (directly or via a
/// covariant-return bridge) and overrides NONE of these ten, so a `WindowsPath`
/// receiver's `resolve(String)`, `toFile()`, `iterator()`, etc. were always
/// going to resolve to `Path`'s own default body regardless of which concrete
/// class the receiver wears -- retiring the interface-keyed row is retiring
/// the ONLY declaration that exists for these ten, on any receiver. That this
/// column is necessary but not SUFFICIENT is exactly what the excluded tenth
/// row below found: having a body is not the same as the body working against
/// what this VM's allocator actually wrote.
///
/// The census this wave is built from ran `apps/probes/L4FilesSweep.java`
/// under `--jdk-only --explain-jdk-only --dump-native-registry <file>` against
/// this same real JDK 25, filtered to `class == "java/nio/file/Path"`,
/// `owns_slot == true`, `kind == "bridge"` and
/// `image_declaring_method.has_code == true`; it named exactly these ten
/// candidate rows, independently agreeing with the two `javap` reads above --
/// nine of the ten are in the table; see below for the tenth.
///
/// # Where the nine are registered
///
/// All nine are registered in `register_phase57_nio_file` itself and so fall
/// inside that function's `__path_rows_before` .. `mirror_class_registrations`
/// window -- see that function's own comment for how the mirror onto the
/// concrete class works.
///
/// # The tenth row, `register(WatchService, Kind...)`, is excluded on a MEASURED NPE
///
/// The census named a tenth candidate,
/// `("java/nio/file/Path", "register", "(Ljava/nio/file/WatchService;[Ljava/nio/file/WatchEvent$Kind;)Ljava/nio/file/WatchKey;")`
/// -- registered by `register_watch_service` in `native-io/src/lib.rs`, a
/// different function in a different crate, outside `register_phase57_nio_file`'s
/// mirror window, and `javap -p java.nio.file.Path` confirms it is a default
/// method with real `Code` exactly like the nine that are here. It looked safe
/// on the same reasoning as the other nine. **It is not**, and only a probe
/// run caught it: `L4PathRetireProbe.java`'s `dir.register(ws,
/// ENTRY_CREATE)`, with this row added to the table and the binary rebuilt,
/// throws `NullPointerException: Cannot invoke
/// "sun.nio.fs.WindowsWatchService$Poller.register(...)" because
/// "this.poller" is null` -- HotSpot returns a valid `WatchKey`. `Path`'s real
/// default `register` body reads `((WindowsWatchService)
/// watcher).poller`, and the `WatchService` this VM hands back from
/// `FileSystem.newWatchService()` is never wired up as a real
/// `WindowsWatchService` (its `poller` field is never populated) the same way
/// `path_layout` populates `Path`'s `string`/`bytes` slots but never `fs` --
/// a SIBLING gap in a different carrier, not the same bug, but the same
/// shape: a receiver stamped with the right CLASS whose declared fields the
/// allocator never filled in. Left out until `WatchService`'s own carrier gets
/// the same treatment `path_layout.rs` gave `Path`.
///
/// # `NativeKind` re-checked by hand, not only by the census
///
/// All nine sit under `r.set_category(NativeKind::Bridge)` with no
/// `set_category(...Intrinsic)` between it and their registration --
/// `register_phase57_nio_file` sets `Bridge` once at its top and does not
/// change category before line 7974's mirror call. Unlike `PrintStream.charset()`
/// (`RETIRED_SHADOW_L4_PRINTSTREAM_TRIPLES`'s own note), nothing here is
/// `Intrinsic`, so no row in this table is inert.
///
/// # Verification
///
/// `L4FilesSweep` (395 rows) against a real JDK 25 oracle, this table armed
/// under `--jdk-only`: same four pre-existing divergences as the unpatched
/// tree, no new ones, exit 0. `L4PathRetireProbe.java`, purpose-built for
/// these nine rows plus the excluded tenth, matches HotSpot exactly on all
/// nine and is what caught the tenth.
///
/// Measured on **Windows** against **JDK 25** (Microsoft build 25.0.3.9).
static RETIRED_SHADOW_L4_PATH_TRIPLES: &[(&str, &str, &str)] = &[
    ("java/nio/file/Path", "endsWith", "(Ljava/lang/String;)Z"),
    ("java/nio/file/Path", "iterator", "()Ljava/util/Iterator;"),
    (
        "java/nio/file/Path",
        "of",
...
/// Lane 4 wave 9, `java/nio/file/spi/FileSystemProvider`: four of the nine
/// rows `the_provider_family_is_retired_by_no_table` used to block, all at
/// once, retired together.
///
/// # The blockers that closed
///
/// `p57_alloc_provider` minted every provider this VM builds as the LITERAL
/// abstract `FileSystemProvider` class until 2026-09-16, so dispatch on it
/// (`H11-1`) never had a concrete class to retarget onto.
/// `FILE_SYSTEM_PROVIDER_IMPLS` records what the concrete mint cost:
/// `L4FilesSweep` went from 0 to 172 differing lines the first time it was
/// tried, because
/// `sun/nio/fs/UnixPath.toUnixPath`'s `instanceof UnixPath` fails against this
/// VM's interface-stamped `Path`. That is the companion fix `java/nio/file/Path`
/// needed first (2026-09-16, "stamp java.nio.file.Path receivers as the real
/// platform class"); this wave is the provider half that fix unblocked,
/// mirrored onto each `FILE_SYSTEM_PROVIDER_IMPLS` class the same way
/// `RETIRED_SHADOW_L4_PRINTSTREAM_TRIPLES` needs no per-platform duplicate
/// rows -- the mirror copies the `Bridge` -> `SyntheticStub` re-tag from the
/// one row keyed on the abstract name onto every concrete provider class
/// `register_phase57_nio_file`'s own `for target in FILE_SYSTEM_PROVIDER_IMPLS`
/// loop reaches.
///
/// A SECOND blocker closed the same week, found by the SAME attempt that hit
/// this table TWICE: `path_layout.rs` resolved `PathSlots::fs` but
/// `p57_write_path_fields` never wrote it, so a native-built `Path`'s `fs`
/// field was always null (2026-09-16/17 fix: `p57_write_path_fields` now
/// resolves `fs` from the Path's own encoded text). `WindowsFileSystemProvider`'s
/// real overrides of `createLink`/`createSymbolicLink`/`newFileChannel`/
/// `readSymbolicLink` all call `Path.getFileSystem()` internally, so the
/// FIRST attempt at this table made `L4FilesSweep` crash outright with an
/// uncaught `NullPointerException`.
///
/// A THIRD blocker closed immediately after: once `fs` was no longer null,
/// `WindowsPath.getAbsolutePath()`'s real body started reading
/// `getFileSystem().defaultDirectory()`/`.defaultRoot()` -- package-private,
/// non-native methods declared only on the CONCRETE `sun.nio.fs.WindowsFileSystem`,
/// never on the abstract interface `p57_alloc_default_filesystem` stamped
/// every default FileSystem with. `NoSuchMethodError`, the SECOND attempt at
/// this table. Fixed by minting the default FileSystem singleton concrete too
/// (`cratonvm_native_api::filesystem_layout`, 2026-09-17) -- Windows-verified
/// only, same caveat as every carrier fix in this family.
///
/// All three are the SAME species of defect, one carrier layer apart each
/// time: a receiver stamped with an ABSTRACT/INTERFACE name has no real
/// method table for real bytecode to dispatch into, and each layer was
/// invisible until the layer above it started handing out receivers real
/// bytecode could actually reach.
///
/// # The census named nine; javap on the concrete class only clears four
///
/// The nine are `owns_slot`, `Bridge`-kind, bucket-A/B rows whose
/// `FileSystemProvider`-declared body `javap -c` shows as non-abstract (real
/// `Code`, not `throw new UnsupportedOperationException()` as the class's
/// only statement) -- `createLink`, `createSymbolicLink`,
/// `installedProviders`, `newAsynchronousFileChannel`, `newFileChannel`,
/// `newFileSystem(Path,Map)`, `newInputStream`, `newOutputStream`,
/// `readSymbolicLink`. That column answers "does the ABSTRACT class have a
/// body", not "is the receiver's real answer correct" -- the receiver is now
/// a CONCRETE class, so what matters is whether
/// `sun.nio.fs.WindowsFileSystemProvider` (the one real image on this
/// machine; Windows/x86_64, JDK 25 --
/// `sun.nio.fs.Unix*`/`Linux*`/`Mac*`/`Bsd*`/`Solaris*FileSystemProvider` are
/// NOT installed here and were not checked) overrides the method with real
/// work, or inherits the abstract default and what that default actually
/// does once a receiver reaches it. `javap -p` on the concrete class clears
/// only four:
///
/// ```text
///   method                       overridden on WindowsFileSystemProvider?
///   createLink                   yes
///   createSymbolicLink           yes
///   newFileChannel                yes
///   readSymbolicLink             yes
///   newAsynchronousFileChannel   yes (but see "held out anyway" below)
///   newFileSystem(Path,Map)      NO -- inherits the abstract default
///   newInputStream               NO -- inherits the abstract default
///   newOutputStream               NO -- inherits the abstract default
///   installedProviders           n/a -- `static`, never overridden by anything
/// ```
///
/// # The four retired here, and why "overridden" is enough
///
/// `createLink`, `createSymbolicLink`, `newFileChannel` and `readSymbolicLink`
/// are all registered directly in `register_phase57_nio_file`'s own `fsp`
/// block (so the mirror loop above reaches them), `WindowsFileSystemProvider`
/// overrides every one with real `Code`, and each one's own native
/// (`p57_create_hard_link`, `p57_create_symbolic_link`, the fd_table-backed
/// channel open, `p57_read_symbolic_link`) does the same real OS-level
/// operation the real override does -- no synthetic-only behaviour that
/// falling through to bytecode would lose. `newFileChannel`'s own
/// registration comment says as much already: "Real JDK delegates to
/// WindowsFileSystemProvider.newFileChannel which overrides this abstract
/// method. We provide a synthetic FileChannel ... so the existing
/// j.n.c.FileChannel native methods can drive it" -- a workaround for the
/// exact carrier bugs this wave's own doc closed.
///
/// # The three excluded on `javap -c` evidence, not a guess
///
/// `newFileSystem(Path,Map)`, `newInputStream` and `newOutputStream` all
/// inherit `FileSystemProvider`'s own default body rather than an override,
/// and `javap -c java.nio.file.spi.FileSystemProvider` disassembles what that
/// default actually does -- decisive rather than javadoc-quoting:
///
/// * `newFileSystem(Path,Map)` -- three bytecodes, `new
///   UnsupportedOperationException; dup; invokespecial <init>; athrow`,
///   unconditionally. `register_phase57_nio_file`'s own registration
///   comment on this exact triple says so before this wave existed: "abstract
///   by default and the base method throws UnsupportedOperationException,"
///   and the native it registers instead mounts a real jar-backed
///   `FileSystem` so smallrye's `ClassPathUtils.processAsJarPath` (Quarkus
///   boot) does not die on it. Retiring swaps a working mount for the
///   unconditional throw its own neighbouring comment already named.
/// * `newOutputStream` -- inherits the default, which does
///   `this.newByteChannel(path, optsWithWRITE, NO_ATTRS)` (an `invokevirtual`
///   on the RECEIVER, not a call to this table's own `newFileChannel` row) and
///   wraps the result via `Channels.newOutputStream`. The registration this
///   table does NOT retire says why that matters: "The fix: register
///   `newOutputStream` directly so we never fall through to the JDK
///   default... hand back a real `java.io.FileOutputStream` instance with
///   the fd wired onto its `FileDescriptor` -- that way every downstream
///   `write`/`flush`/`close` native (already registered against
///   `java/io/FileOutputStream`) just works without any new shim type," fixing
///   a measured Felix `BundleException: Unable to cache bundle` on every
///   `installBundle`. `Channels.newOutputStream`'s anonymous wrapper is not a
///   `FileOutputStream`, so any downstream cast/`instanceof`/`getFD()` this
///   VM has wired specifically to that class would stop matching.
/// * `newInputStream` -- inherits the default, which calls the STATIC
///   `Files.newByteChannel(path, opts)` (resolved through the path's own
///   provider, not `this`) and wraps it the same way. This table's own
///   `fsp_new_input_stream` additionally serves VFS/jar entries with no file
///   descriptor at all (`vfs_read`, an in-memory `ByteArrayInputStream`),
///   runs `capability_gate::open_read_gated` so a denial surfaces as
///   `SecurityException` rather than an I/O condition, and constructs a real
///   `java.io.FileInputStream` with `closeLock` backfilled (missing it NPEs
///   `close()` under try-with-resources, per `fis_backfill_constructor_fields`
///   in `native-io`) -- none of which the generic `Channels`-wrapped
///   `SeekableByteChannel` path is known to reproduce.
/// * `installedProviders` -- `static`, so there is no receiver to retarget
///   onto; the JVMS resolves an `invokestatic` to the literal declaring
///   class regardless of what `p57_alloc_provider` mints. Its real body does
///   REAL `ServiceLoader` discovery via `loadInstalledProviders()` (`javap
///   -c`: `getstatic installedProviders; ifnonnull; ...; invokestatic
///   FileSystems.getDefault; invokevirtual .provider; ...; invokestatic
///   loadInstalledProviders`) -- and this VM ships no `META-INF/services`
///   entries for a `jar:`/`jrt:` provider, so that discovery would answer a
///   one-element list. This table's own native answers three
///   (`file`/`jar`/`jrt`, `file` at element 0 to match
///   `FileSystems.getDefault().provider()` identity, per the class's own
///   javadoc contract HotSpot honours) specifically so
///   `FileSystems.getFileSystem(URI.create("jrt:/"))` resolves for the
///   in-process compiler reading the runtime image (HIB-CV-27). Retiring
///   trades a synthetic list this VM depends on for a real but incomplete
///   one.
///
/// # `newAsynchronousFileChannel` is held out on a structural gap, not a behavioural one
///
/// `WindowsFileSystemProvider` DOES override it with real `Code`, and the
/// native itself (`native-io/src/lib.rs`, `native_afc_provider_open`) reads as
/// exactly this wave's story one carrier early: its own comment says Craton
/// was "executing the abstract provider default, which throws
/// UnsupportedOperationException, instead of the Unix provider implementation"
/// before this row's synthetic AFC backend existed. What is different about
/// this one row is WHERE it is registered: every other row in this table is
/// inside `register_phase57_nio_file`'s own `fsp` block, so the `for target in
/// FILE_SYSTEM_PROVIDER_IMPLS { mirror_class_registrations(...) }` loop a few
/// lines above copies its `Bridge`/`SyntheticStub` kind onto every concrete
/// provider class. `native_afc_provider_open` is registered in
/// `native-io/src/lib.rs`'s `register_async_file_channel`, a completely
/// separate registrar (called from `register_phase92_io_completeness`, not
/// `register_phase57_nio_file`) -- confirmed by reading it, not assumed: that
/// function DOES run its own `mirror_class_registrations` loop (`for target
/// in AFC_IMPLS.iter().chain([AFC_ABSTRACT_IMPL])`), but the source class it
/// mirrors is `afc` (`java/nio/channels/AsynchronousFileChannel`), not `fsp`,
/// so a row registered under `java/nio/file/spi/FileSystemProvider` in that
/// same function is never selected by either mirror loop in the codebase. No
/// mirrored copy of this one triple exists on
/// `sun/nio/fs/WindowsFileSystemProvider` regardless of what this table says.
/// Whether a retired-but-unmirrored abstract-class row is even consulted for
/// a concrete receiver depends on registry-lookup semantics (an exact match
/// at the C25-retargeted concrete class, versus a hierarchy walk back up to
/// the abstract declaration) this file's own comments do not settle and this
/// wave did not verify against a build. Held out rather than asserted; the
/// fix, if wanted, is either mirroring this triple too or moving its
/// registration into the `fsp` block above.
///
/// Measured on **Windows/x86_64 against JDK 25** -- the only real-JDK image on
/// this machine. `sun.nio.fs.Unix/Linux/Mac/Bsd/SolarisFileSystemProvider`
/// were not directly `javap`'d; `FILE_SYSTEM_PROVIDER_IMPLS` picks whichever
/// one the running image actually has, and this table retires the row keyed
/// on the abstract class name, which the mirror then copies onto whichever
/// concrete class wins on THAT platform -- so an unverified platform inherits
/// this wave's verdict rather than getting its own.
static RETIRED_SHADOW_L4_FILESYSTEMPROVIDER_TRIPLES: &[(&str, &str, &str)] = &[
    (
...
/// Lane 4 wave 8, 2026-09-18. `java/io/` was admitted by wave 1; no new
/// prefix.
///
/// # A whole-class retirement, not a per-method one
///
/// `apps/probes/L4CensusTail.java` was written (an earlier session, never
/// tabled) to reach exactly two families this lane had never measured:
/// `java/io/LineNumberInputStream` (8 registrations) and
/// `java/io/StringBufferInputStream` (5). Both are minted by
/// `try_alloc_concurrent_synthetic` (`native-builtins/src/deprecated_util.rs`)
/// and their accessors read/write FOUR (LNIS) or THREE (SBIS) fixed slot
/// indices -- `native_lnis_init`'s own doc comment: `field 0 = wrapped
/// InputStream, 1 = line number, 2 = saved line number, 3 = pushback byte`
/// -- a private convention that does not correspond to either class's real
/// declared layout (`javap -p`: LNIS is `pushBack, lineNumber,
/// markLineNumber, markPushBack`, four fields with no slot for the inherited
/// `FilterInputStream.in`; SBIS is `buffer, pos, count`, three). This is the
/// lane's five-times-repeated carrier shape (`File`'s `prefixLength`,
/// `AbstractLayout.name`, `AbstractGroupLayout.elements`,
/// `CharBuffer.toString(int,int)`'s convention, `FileTime`'s unit) IF any row
/// is retired while a sibling stays native: real bytecode reading a field
/// this VM never wrote at that offset, or a native reading a field real
/// bytecode never populated the way the native's own convention assumes.
///
/// Both tables here are their WHOLE native surface, `<init>` included --
/// mirroring wave 6's `PrintStream`/`PrintWriter` ("retiring the
/// constructors hands construction back to the JDK and all four fields
/// arrive"). With `<init>` retired too, no native ever mints or touches
/// either object again: the real constructor's own `putfield`s populate the
/// real declared fields at the real offsets the JVM resolves for them, and
/// every accessor -- also retired -- reads them back the same way. There is
/// no partially-native state for the carrier question to apply to.
///
/// # Measured
///
/// `L4CensusTail`, 126 rows, oracle JDK 25 on windows/x86_64. The LNIS/SBIS
/// section is rows 1-38 of 129 in the probe's own numbering. Retired,
/// `--jdk-only`: **zero rows differ from the oracle** -- confirmed by running
/// the exact same rows the pre-fix binary had already produced correctly in
/// COMPATIBLE mode (this table's own precondition: unblocked only once
/// `native_vm_save_properties` let `System.initPhase1()`'s real bytecode
/// complete under `--jdk-only` at all, see the commit this table landed
/// with). `compatible` mode: unaffected, `SyntheticStub` is
/// `allowed_in(Compatible)`, so the native still wins there.
static RETIRED_SHADOW_L4_DEPRECATED_STREAMS_TRIPLES: &[(&str, &str, &str)] = &[
    (
...
/// Lane 4 wave 8, 2026-09-18, `java/nio/file/SimpleFileVisitor`'s four erased
/// `FileVisitor` bridge rows. `java/nio/` was admitted by wave 1.
///
/// `SimpleFileVisitor` (`register_p66_file_visitor`,
/// `native-builtins/src/phases_late/nio_file.rs`) declares NO instance
/// fields (`javap -p`) -- none of this wave's carrier question applies here;
/// the class is pure dispatch. The native bodies replicate documented
/// javadoc behaviour (`preVisitDirectory`/`visitFile` answer `CONTINUE`,
/// `visitFileFailed` rethrows, `postVisitDirectory` rethrows iff its
/// exception argument is non-null) that the real class's own bytecode
/// already implements identically -- `register_p66_file_visitor`'s own
/// comment records the one defect this surface had (an argument-index bug
/// that threw the wrong value; fixed before this table).
///
/// Measured with the same `L4CensusTail` run as the table above (its rows
/// 88-95: `bridge`/`direct` `visitFile`/`preVisitDirectory`/
/// `postVisitDirectory`, both the null- and exception-carrying calls,
/// reached through a reference typed as the raw `FileVisitor` interface AND
/// through `SimpleFileVisitor<Path>` directly). Zero rows differ from the
/// JDK 25 oracle in either shape.
static RETIRED_SHADOW_L4_FILEVISITOR_TRIPLES: &[(&str, &str, &str)] = &[
    (
...
/// Lane 4 wave 8, 2026-09-18, the `ByteBufferAsCharBuffer{B,L,RB,RL}` half of
/// wave 4's `toString(int,int)` fix that wave 4 itself never tabled.
/// `java/nio/` was admitted by wave 1.
///
/// `cb_to_string_range` (`native-builtins/src/phases_late/charset_buffers.rs`)
/// is registered on `java/nio/CharBuffer` itself AND, separately, on seven
/// concrete subclasses -- the comment at its own registration loop: "the
/// `ByteBufferAs*CharBuffer` family and `HeapCharBuffer` are what
/// `ByteBuffer.asCharBuffer()` returns in real-JDK mode... registering
/// directly on each ensures dispatch hits us regardless of how vtable
/// resolution handles abstract-in-base + native-on-base." Wave 4
/// (`RETIRED_SHADOW_L4_CHARBUFFER_TRIPLES`) retired the base-class row only;
/// the four `ByteBufferAsCharBuffer*` subclass rows -- the ones a real
/// `ByteBuffer.asCharBuffer()` actually returns -- were never tabled, so the
/// SAME defect §9.15 fixed in the shared function (`toString()` taking its
/// window from an absolute buffer position rather than the position-relative
/// one every real body except `StringCharBuffer`'s uses) was fixed but never
/// asked to prove itself on the classes that matter. `javap -p`: all four
/// declare `public java.lang.String toString(int, int)` with real `Code`.
///
/// `HeapCharBuffer`/`HeapCharBufferR`/`StringCharBuffer` are the other three
/// classes under the same registration loop, added here rather than left for
/// later: `L4CensusTail`'s own `HeapCharBuffer toString`/`HeapCharBufferR
/// toString`/`StringCharBuffer toString`/`StringCharBuffer sliced` rows
/// (measured alongside the four above, same run, same zero-diff result)
/// already exercise `toString()`'s delegation into exactly these three
/// classes' `toString(int,int)` -- `CharBuffer.allocate(6)` is a
/// `HeapCharBuffer`, `.asReadOnlyBuffer()` of it a `HeapCharBufferR`,
/// `CharBuffer.wrap(String)` a `StringCharBuffer`. `javap -p`: all three
/// declare `toString(int, int)` with real `Code` (`HeapCharBufferR`
/// overrides it itself, not inherited from `HeapCharBuffer`).
///
/// Measured with the same `L4CensusTail` run as the four rows above: its
/// `view BE/LE toString`, `... RO toString` and `... sliced toString` rows
/// (11-15 depending on endianness) construct exactly a
/// `ByteBufferAsCharBufferB` or `ByteBufferAsCharBufferL` via
/// `ByteBuffer.allocate(8).order(order).asCharBuffer()` and reach
/// `toString(int,int)` through `toString()`'s one-line delegation
/// (`toString(position(), limit())`, §9.15); `HeapCharBuffer toString`,
/// `HeapCharBufferR toString`, `StringCharBuffer toString` and
/// `StringCharBuffer sliced` do the same for the three rows below. Zero rows
/// differ from the JDK 25 oracle, for any of the seven. `RB`/`RL` (the
/// read-only `ByteBufferAsCharBuffer` views) are reached by the same rows'
/// `.asReadOnlyBuffer()` half and are not independently exercised by this
/// probe beyond that -- included here on the class's own `javap -p` evidence
/// (identical declared method, same shared native, same real `Code`) rather
/// than a separate probe row, and named here rather than left silent.
static RETIRED_SHADOW_L4_CHARBUFFER_VIEWS_TRIPLES: &[(&str, &str, &str)] = &[
    (
        "java/nio/ByteBufferAsCharBufferB",
        "toString",
...
/// Lane 4 wave 9, 2026-09-18, one row: `java/nio/file/Paths.get(String,
/// String...)`. `java/nio/` was admitted by wave 1.
///
/// Found by the funnel step run for the first time in this lane's history
/// against a WORKING `--jdk-only` (86 invocations across
/// `L4FilesSweep`/`L4Reach`/`TailFamilySweep`). The native
/// (`native-builtins/src/phases_late/nio_file.rs`) is pure string-joining
/// into `p57_alloc_path_checked` -- no filesystem call, no
/// `sun/nio/fs/WindowsNativeDispatcher` reach, unlike the wave this same
/// funnel run also surfaced and did NOT land (see the "ATTEMPTED, not yet
/// accepted" note this section replaces in git history: `Files`'s 32 rows
/// cascade into `WindowsNativeDispatcher.GetFileAttributesEx0`, a Win32
/// native this VM has never implemented -- measured via `L4FilesSweep`
/// crashing on its first call, `UnsatisfiedLinkError`, not a soft diff).
/// Real bytecode's `Paths.get` delegates to `FileSystems.getDefault()
/// .getPath(...)`, which reaches the ALREADY-NATIVE, already-invoked
/// (47 times in the same probe run) `WindowsFileSystem.getPath` -- this row
/// does not retire anything downstream of it, only the thin static wrapper.
static RETIRED_SHADOW_L4_PATHS_TRIPLES: &[(&str, &str, &str)] = &[(
    "java/nio/file/Paths",
    "get",
    "(Ljava/lang/String;[Ljava/lang/String;)Ljava/nio/file/Path;",
)];

/// Lane 4 wave 10/13/14/15, 21 rows: `sun/nio/fs/WindowsPath`'s
/// `Path`-interface methods, named by wave 9's funnel step
/// (`docs/internal/retired/lane-4-io-nio-foreign-RETIRED-20260920.md` §9 Wave 9).
///
/// Started as all 19 candidate rows. `getFileName`/`getParent` were
/// excluded through wave 14 and retired by wave 15:
///
/// - `getFileName`/`getParent` both scan `this.path` directly for a
///   literal `'\\'` via `String.lastIndexOf(92)`, completely independent
///   of the `offsets` cache (confirmed via `javap`: exactly 2
///   `lastIndexOf`/`indexOf` call sites in the whole class, one in each).
///   This VM used to store `path` with `/`, so that scan failed for any
///   multi-component path and no auxiliary field could fix it. Wave 15
///   changed what the slot HOLDS to the JDK's own `\` form and the string
///   the real `WindowsPathParser` normalises to
///   (`cratonvm_native_api::path_layout::stored_form`), with every native
///   READER converting back to the VM-internal `/` spelling
///   (`canonical_form`) -- which also repaired the mixed `C:/U/x\q` the
///   bytecode `resolve` built next to a parsed `C:/U/x/q`. The 2 rows
///   (3 descriptors: `getParent` has a covariant `WindowsPath` bridge) then
///   matched HotSpot on a 27-shape probe (`L4PathShapes`).
///
/// `toRealPath` (wave 14, both the real
/// `([Ljava/nio/file/LinkOption;)Lsun/nio/fs/WindowsPath;` and the bridge
/// `([Ljava/nio/file/LinkOption;)Ljava/nio/file/Path;`) was blocked the
/// same way through wave 13 -- `WindowsLinkSupport.getRealPath` →
/// `WindowsNativeDispatcher.GetFullPathName` → `GetFullPathName0`, a
/// Win32 native this VM had never implemented -- and broke `L4TrustPath`
/// (`UnsatisfiedLinkError`, not a silent wrong answer -- loading a
/// `KeyStore` reaches `Security$SecPropLoader.loadFromPath` →
/// `WindowsPath.toRealPath`) when tried anyway. Wave 14 implemented
/// `GetFullPathName0` (same FFI pattern as wave 10's
/// `GetFileAttributesEx0`/`CreateFile0`) and retried: `javap -c` on
/// `WindowsLinkSupport.getRealPath`/`resolveAllLinks` shows the
/// non-symlink path needs only `GetFullPathName0` plus
/// `WindowsFileAttributes.get` (→ `GetFileAttributesEx0`, wave 10) --
/// both now implemented. The symlink-follow branch
/// (`WindowsLinkSupport.readLinkImpl`) additionally needs
/// `DeviceIoControlGetReparsePoint`, still unimplemented; a 10-row
/// oracle-diff probe (`L4ToRealPath`, no symlinks: absolute/dotted/
/// dotdot/uppercase-cased/directory/missing-file/`NOFOLLOW_LINKS`
/// shapes) matched HotSpot exactly in both `--jdk-only` and default
/// mode, so this table retires `toRealPath` for the covered,
/// non-symlink case -- a real symlink would still throw
/// `UnsatisfiedLinkError` naming the missing native, same as any other
/// row in this lane blocked on a missing `WindowsNativeDispatcher`
/// primitive.
///
/// The other 18 (`compareTo`/`endsWith`/`equals`/`getFileSystem`/
/// `getName`/`getNameCount`/`getRoot`/`hashCode`/`isAbsolute`/
/// `normalize`/`relativize`/`resolve`/`startsWith`/`subpath`/
/// `toAbsolutePath`/`toRealPath`(x2)/`toUri`) are pure path-string
/// manipulation with no direct `\\`-scan -- `javap -c` confirms no other
/// `lastIndexOf`/`indexOf` call site in the class. `toAbsolutePath()` is
/// pure bytecode (string concatenation against
/// `WindowsFileSystem.defaultDirectory()`).
///
/// Landed after three real bugs, found and fixed across as many attempts
/// (§9.36 and its two follow-ups): (1) `getFileName()` [before it was
/// excluded] NPE'd on a `null this.root` -- fixed via a safe `root`/`type`
/// fallback in `p57_write_path_fields`. (2) `getName(0)` on a
/// multi-component path returned the whole string -- real
/// `WindowsPath.initOffsets()` (a null-guarded lazy cache) scans for
/// `'\\'`, which never appeared in the `/`-stored `path` of the time; fixed
/// by pre-populating the real `offsets` field (wave 15 later moved the
/// storage itself to `\`). (3) `"a//b"` (a legitimate path with a redundant
/// separator) came back `nameCount=3` instead of `2` -- real `path` is
/// always pre-normalized (single separators only) by construction, so a
/// literal translation of `initOffsets()`'s algorithm never needed to
/// handle a double separator; this VM's OWN allocator does not collapse
/// one, so the offsets computation was fixed to skip empty spans between
/// consecutive separators. All three fixes are general and landed
/// independent of this table.
static RETIRED_SHADOW_L4_WINDOWSPATH_TRIPLES: &[(&str, &str, &str)] = &[
    (
    ...
    ("sun/nio/fs/WindowsPath", "equals", "(Ljava/lang/Object;)Z"),
    (
        "sun/nio/fs/WindowsPath",
        "getFileName",
        "()Ljava/nio/file/Path;",
    ...
    ("sun/nio/fs/WindowsPath", "getNameCount", "()I"),
    (
        "sun/nio/fs/WindowsPath",
        "getParent",
        "()Ljava/nio/file/Path;",
    ...
    ("sun/nio/fs/WindowsPath", "hashCode", "()I"),
    ("sun/nio/fs/WindowsPath", "isAbsolute", "()Z"),
    (
        "sun/nio/fs/WindowsPath",
        "normalize",
        "()Ljava/nio/file/Path;",
    ...
    (
        "sun/nio/fs/WindowsPath",
        "toRealPath",
        "([Ljava/nio/file/LinkOption;)Lsun/nio/fs/WindowsPath;",
    ),
    ("sun/nio/fs/WindowsPath", "toUri", "()Ljava/net/URI;"),
];

/// Lane 4 wave 16, 23 rows: the Windows `sun/nio/fs` filesystem classes
/// -- `WindowsFileSystem` (9 of 10), `WindowsFileSystemProvider` (9 of 10),
/// `WindowsDirectoryStream` (2), `WindowsFileStore` (2) and
/// `WindowsPath.toString`.
///
/// Wave 9's funnel named these (`WindowsFileSystem` 10, `WindowsFileSystemProvider`
/// 11) and §9.34 named what stood between them and retirement: the
/// `WindowsNativeDispatcher` natives this VM's `sun/nio/fs/` support never
/// covered. Waves 10, 14, 15 and 16 implemented the ones these rows reach
/// (`CreateFile0`, `GetFileAttributesEx0`, `FindFirstFile0/1`,
/// `FindNextFile0`, `GetFileInformationByHandle0`, the volume and drive
/// queries, `SetFileTime0`, `SetEndOfFile`, `DeviceIoControl*`,
/// `GetFinalPathNameByHandle`, ...).
///
/// NOT in the table, and why (each is a named blocker, not a deferral):
///
/// - `WindowsFileSystem.newWatchService` and everything under it: the real
///   `WindowsWatchService` drives a `ReadDirectoryChangesW` overlapped-I/O
///   loop over a completion port (`CreateIoCompletionPort`,
///   `GetQueuedCompletionStatus0`, `PostQueuedCompletionStatus`,
///   `CancelIo`, `GetOverlappedResult`, `CreateEvent`) and a poller thread.
///   That is an async-I/O runtime, not a filesystem primitive.
/// - `WindowsFileSystemProvider.checkAccess`: for `WRITE`/`EXECUTE` the real
///   body asks the access-check machinery (`WindowsSecurity`: thread/process
///   token duplication, `GetFileSecurity0`, `AccessCheck`), the security
///   family this VM has none of.
/// - `WindowsFileAttributes.fileKey`: held on purpose since the 2026-08-19
///   census (`the_held_windows_attribute_triple_is_not_retired`). The real
///   body is `return null;`; the native answers a `(volume, index)` key that
///   lets `FileTreeWalker.wouldLoop` see a symlink cycle. HotSpot-identical
///   and strictly worse behaviour, so it stays a native by decision.
static RETIRED_SHADOW_L4_WINFS_TRIPLES: &[(&str, &str, &str)] = &[
    // `WindowsDirectoryStream.close`/`.iterator` held OUT (2026-09-22): see
    // the note after this table's closing bracket
    // (`newByteChannel`/`newDirectoryStream`/`readAttributes`'s doc) — the
    // fourth and fifth blockers found bisecting the SAME `javac` module-path
    // scan. `newDirectoryStream` mints its `*DirectoryStream` via
    // `alloc_concrete` directly rather than running the real constructor, so
    // `closed`/`closeLock` are never set and real `iterator()`/`close()`
    // NPE/throw the instant they run against it.
    ...
    // getScheme() held OUT (2026-09-22): see `newFileSystem`'s doc in the
    // wave-17 `FILES2` table below -- the two rows are the read half and the
    // dispatch half of the SAME carrier-sharing defect (every logical
    // provider is this one concrete class; a caller iterating
    // `installedProviders()` by scheme must see each one's OWN scheme, not
    // real bytecode's single hard-coded "file" answer).
    (
        "sun/nio/fs/WindowsFileSystemProvider",
        "isHidden",
        "(Ljava/nio/file/Path;)Z",
    ...
    ("sun/nio/fs/WindowsPath", "toString", "()Ljava/lang/String;"),
];
// `newByteChannel`, `newDirectoryStream` and `readAttributes(Class)` held OUT
// (2026-09-22, same wave, same defect as `getScheme`/`newFileSystem` above):
// each has its own jarfs/jrtfs-aware body on `fsp` (`newByteChannel` reads a
// mounted archive's `vfs_read` snapshot; `newDirectoryStream` lists via
// `jarfs_list_dir`/`jrtfs_list_dir_classified`; `readAttributes(Class)` is
// exactly what `Files.isDirectory` reaches through `readAttributesIfExists`
// while walking a listed directory). MEASURED bisecting `TestCompilerTests`
// with `CRATONVM_UNRETIRE_NATIVE_SHADOW`, one row at a time: each was its own
// blocker, in this order, before `javac`'s in-process module-path scan
// (`Locations$SystemModulesLocationHandler.initSystemModules`) got past the
// runtime-image listing. `WindowsDirectoryStream.close`/`.iterator` (wave 16's
// OWN table, retired above `RETIRED_SHADOW_L4_WINFS_TRIPLES`) are the fourth
// and fifth: my `newDirectoryStream` mints the concrete `*DirectoryStream`
// directly rather than running its real `<init>`, so `closed`/`closeLock`
// are never set and real `iterator()`/`close()` NPE/throw
// `IllegalStateException: Directory stream is closed` the instant they run
// against it.

/// Lane 4 wave 17: the eleven Java wrappers of `java.io.WinNTFileSystem`.
///
/// The JDK 25 class declares each operation as a Java method
/// (`list(File)`, `getLength(File)`, ...) that calls the JNI entry point of
/// the same name suffixed `0`. This VM shadowed the wrapper AND registered the
/// `*0` native (kind `Bridge`, `nio_file.rs` `fs_reg` / the `WinNTFileSystem`
/// loop); the wrapper is what the funnel counted. Retiring it hands the
/// wrapper to bytecode, which then calls the `*0` native that already exists.
/// The table names `WinNTFileSystem` only, so `UnixFileSystem` (Linux image)
/// is untouched and stays a separate measurement on a Linux host.
static RETIRED_SHADOW_L4_WINNTFS_TRIPLES: &[(&str, &str, &str)] = &[
    ...
    ("java/io/WinNTFileSystem", "getSpace", "(Ljava/io/File;I)J"),
    (
        "java/io/WinNTFileSystem",
        "list",
...
/// Lane 4 wave 19: the rows the wave-18 ledger (§9.37) could not dispose of --
/// the ones with no named blocker, measured on linux/x86_64 and retired here.
///
/// # What was left, and why these are the ones
///
/// Wave 18 walked the whole prefix set with `apps/probes/l4ledger.py` and
/// joined `apps/probes/l4blockers.json` onto it. 618 open bucket-A/B rows
/// matched a named blocker; 115 matched none, and "no probe in this tree
/// invokes it" is explicitly NOT one (§9.1). This table is what was left after
/// the 52 of those 115 that a blocker DOES cover were given one:
///
/// * the five typed buffers' `array`/`capacity`/`flip`/`hasRemaining`/`limit`/
///   `position`/`remaining` (35). `java/nio/Buffer`'s own `position`, `limit`
///   and `capacity` were retired by wave 17 and measured then; these are the
///   same three plus their siblings, on the five subclasses that inherit the
///   bodies.
/// * `ByteBufferAsCharBuffer{B,L}`'s five accessors (10), which `L4CharBufferSweep`
///   and `L4TypedBufferSweep` read back over both byte orders.
/// * `DirectByteBuffer.{getDouble,getFloat,putDouble,putFloat}` (4) -- the four
///   widths wave 17's `DirectByteBuffer` group left out.
/// * `ByteBuffer.toString`, `ByteOrder.equals`, `CharBuffer.charAt`,
///   `StringCharBuffer.toString` (4).
/// * `java.nio.file.Files`' remaining seven, which the wave-18 Unix dispatcher
///   natives (`stat0`/`lstat0`/`unlink0`/...) made reachable as bytecode for
///   the first time on this platform.
/// * `ObjectInputStream.resolveProxyClass` and
///   `ObjectStreamClass$RecordSupport.deserializationCtr` (2). These two had
///   **no caller anywhere in the probe tree**; `apps/probes/L4Serial.java` was
///   written for them (a record round-trip and a serialized dynamic proxy,
///   both read back) and reaches them 2 and 1 times.
///
/// # Not in the table
///
/// `session()`/`checkSession()` on all ten buffer classes stay out: the JOTP
/// wave refuses them on `dev` and the measurement is netty's, not this repo's.
/// So do `Bits$1`'s four pool-MXBean accessors and `Buffer$2`'s
/// `reserveMemory`/`unreserveMemory` -- those carry the VM's own direct-memory
/// accounting, which is the `Bits.reserveMemory` finding of wave 17 one class
/// along.
static RETIRED_SHADOW_L4_W19_TRIPLES: &[(&str, &str, &str)] = &[
    (
    ...
    ("java/nio/ByteBufferAsCharBufferB", "charAt", "(I)C"),
    ("java/nio/ByteBufferAsCharBufferB", "get", "()C"),
    ("java/nio/ByteBufferAsCharBufferB", "get", "(I)C"),
    ("java/nio/ByteBufferAsCharBufferB", "hasArray", "()Z"),
    (
        "java/nio/ByteBufferAsCharBufferB",
        "toString",
        "()Ljava/lang/String;",
    ),
    ("java/nio/ByteBufferAsCharBufferL", "charAt", "(I)C"),
    ("java/nio/ByteBufferAsCharBufferL", "get", "()C"),
    ("java/nio/ByteBufferAsCharBufferL", "get", "(I)C"),
    ("java/nio/ByteBufferAsCharBufferL", "hasArray", "()Z"),
    (
        "java/nio/ByteBufferAsCharBufferL",
        "toString",
        "()Ljava/lang/String;",
    ...
    ("java/nio/DirectByteBuffer", "getDouble", "(I)D"),
    ...
    ("java/nio/IntBuffer", "flip", "()Ljava/nio/Buffer;"),
    ...
    ("java/nio/LongBuffer", "flip", "()Ljava/nio/Buffer;"),
...
/// Lane 4 wave 18: the eleven Java wrappers of `java.io.UnixFileSystem`, the
/// Linux twin of wave 17's `WinNTFileSystem` table.
///
/// `RETIRED_SHADOW_L4_WINNTFS_TRIPLES`' doc comment ends "the table names
/// `WinNTFileSystem` only, so `UnixFileSystem` (Linux image) is untouched and
/// stays a separate measurement on a Linux host". This is that measurement,
/// taken 2026-09-20 on linux/x86_64 against Temurin 25.0.4+7 -- the first time
/// this lane has run on the platform whose rows these are.
///
/// The shape is identical to the Windows one: JDK 25's `UnixFileSystem`
/// declares each operation as a Java method that calls the JNI entry point of
/// the same name suffixed `0`, and this VM shadows BOTH. Measured in the
/// lane-4 registry census on Linux, every `*0` the eleven wrappers need is
/// registered and answering (`getBooleanAttributes0` 73 invocations,
/// `delete0` 19, `canonicalize0` 12, `rename0` 4) -- so retiring the wrapper
/// hands it to bytecode that calls a native which already exists.
///
/// NOT in the table, and each for a measured reason:
///
/// * `<init>()V` -- the carrier is VM-minted and the constructor is what
///   writes it. Wave 17 kept `WinNTFileSystem.<init>` native for the same
///   reason and `unconstructed_carrier_gate` is the guard that says so.
/// * the `*0` JNI natives -- bucket D (`ACC_NATIVE` in the image). §1.5
///   *requires* them to stay `Bridge`; they are the surface the retired
///   wrappers call.
/// * `getNameMax0`, `delete0(File,boolean)`, `getDriveDirectory`,
///   `getFinalPath0`, `listRoots0` -- registered under descriptors JDK 25's
///   `UnixFileSystem` does not declare (bucket F), so a table row would match
///   nothing.
static RETIRED_SHADOW_L4_UNIXFS_TRIPLES: &[(&str, &str, &str)] = &[
    (
...
/// Lane 4 wave 17: `java.io.File.createTempFile(String,String)`, which
/// delegates to the three-argument form (retired earlier) -- and, since wave
/// 18, `File.deleteOnExit()`.
///
/// # `deleteOnExit` was blocked on a VM gap, and wave 18 closed the gap
///
/// Wave 17 measured the row as correct and un-retirable: its bytecode registers
/// `DeleteOnExitHook` through `JavaLangAccess.registerShutdownHook`, which ends
/// in `java.lang.Shutdown.add(2, true, hook)`, and this VM ran none of
/// `Shutdown`'s hook SLOTS at exit (`shutdown hooks: ran=0`). With the row
/// retired, HotSpot left `L4FileTempExit`'s temp directory empty and this VM
/// left the file, the directory and its child on disk.
///
/// `lang_system::run_jdk_shutdown_slots` now walks that array on every exit
/// path, so the JDK's own hook runs where HotSpot runs it -- after the
/// application hooks, which ARE slot 1. Re-measured 2026-09-20 on
/// linux/x86_64: `L4FileTempExit`'s directory is empty after the run on both
/// VMs, and `L4ShutdownSlots` reports the same non-null `slot[2]` on both.
/// The row is in the table because the blocker is gone, not because the
/// verdict changed.
static RETIRED_SHADOW_L4_FILETMP_TRIPLES: &[(&str, &str, &str)] = &[
    (
        "java/io/File",
        "createTempFile",
...
/// Lane 4 wave 17: the `java.nio.file` static surface and the Windows provider
/// rows behind it -- `Files` (all 32 registered rows), `FileVisitResult`'s two
/// enum synthetics, `FileSystemProvider.newInputStream`/`newOutputStream`, and
/// the `WindowsFileSystemProvider` / `WindowsFileStore` /
/// `WindowsFileSystem.getFileStores` / `WindowsDirectoryStream.spliterator`
/// rows that had never been invoked (so the funnel's `invocations > 0` filter
/// never showed them) and that the retired `Files` surface now reaches.
///
/// # Why the four provider mutators are in the same table as `Files`
///
/// `WindowsFileSystemProvider.{copy,createDirectory,delete,move}` were
/// registered by `concrete_receiver.rs` as natives that DELEGATE to the
/// matching `Files` native (`nio_file.rs`, "one body per operation"). With the
/// `Files` rows retired, `Files.delete` is bytecode that calls
/// `provider.delete`, which is that native, which calls `Files.delete`:
/// unbounded native recursion (`EXCEPTION_STACK_OVERFLOW` at the first
/// `Files.createDirectory` of the run). Found by retiring each `Files` method
/// alone: exactly `copy`, `createDirectory`, `delete` and `move` overflowed.
/// Retiring the provider halves with them is what turns the loop into the JDK
/// body, `WindowsNativeDispatcher.CreateDirectory0`/`DeleteFile0`/
/// `RemoveDirectory0`/`MoveFileEx0`/`CopyFileEx0` -- the wave-15 natives that
/// nothing had reached until now.
///
/// NOT in the table, each with its named blocker:
///
/// - `FileSystems.getDefault`, `FileSystems.newFileSystem` (both overloads),
///   `WindowsFileSystemProvider.getFileSystem(URI)`: `getDefault` is the
///   identity anchor for the native `Path`/`FileSystem` carrier. Retiring
///   `getFileSystem(URI)` alone measured a second, distinct default
///   filesystem (`FileSystems.getFileSystem(file:///) == getDefault()` turns
///   `false`); the `newFileSystem` rows measured a regression
///   (`ProviderNotFoundException` for a non-archive became "no throw")
///   because they lean on `installedProviders`, below.
/// - `FileSystemProvider.installedProviders` (see the wave-9 table): the
///   native answers `file`/`jar`/`jrt`; the real body's `ServiceLoader`
///   discovery would answer one.
/// - `WindowsFileSystemProvider.checkAccess` and `newWatchService` /
///   `WindowsPath.register`: the security-token and IOCP families
///   (wave 16's table names them).
/// - `WindowsFileSystemProvider.newAsynchronousFileChannel`: IOCP again.
/// - `WindowsFileSystemProvider.newFileSystem(Path,Map)`: the class inherits
///   the abstract default, which throws; the native answers an archive
///   filesystem. `newFileSystem(URI,Map)` and `getScheme()` (wave 16's table)
///   are the same species, missed in the original wave and held out
///   2026-09-22: every logical provider (`file`/`jar`/`jrt`) this VM mints
///   shares this one concrete class, so retiring either row hands EVERY
///   provider instance real bytecode's single hard-coded "file"-provider
///   answer, and an in-process `javac` can never mount a classpath jar or the
///   runtime image (see the row's own comment).
static RETIRED_SHADOW_L4_FILES2_TRIPLES: &[(&str, &str, &str)] = &[
    (
    ...
    // newFileSystem(URI, Map) held OUT (2026-09-22, Spring in-process javac):
    // `p57_alloc_provider` mints EVERY logical provider (file/jar/jrt) as
    // this SAME concrete platform class, distinguished only by a "scheme"
    // field this native reads (see its own registration, `java/nio/file/spi/
    // FileSystemProvider` in `nio_file.rs`, category Bridge). Retiring the
    // mirrored copy on `WindowsFileSystemProvider` hands the call to REAL
    // bytecode, which has no idea about the scheme field and always behaves
    // as the FILE-scheme provider -- so a real `javac` classpath jar (or the
    // runtime image) can never be mounted: `Locations.<*>` gets
    // `ProviderNotFoundException`/`UnsupportedOperationException` from every
    // provider it tries, reported as "no file system provider is available
    // to handle this file". MEASURED: `TestCompilerTests`/
    // `CandidateComponentsIndexerTests` (`org.springframework.core.test.tools
    // .TestCompiler` / `org.springframework.context.index.test.TestCompiler`,
    // both backed by `ToolProvider.getSystemJavaCompiler()`) fail every
    // method under `--jdk-only` with this error; `CRATONVM_UNRETIRE_NATIVE_
    // SHADOW=sun/nio/fs/WindowsFileSystemProvider.newFileSystem` alone does
    // not clear it (see `getScheme`, held out the same day in the wave-16
    // table below, for the companion half of the SAME defect).
    (
        "sun/nio/fs/WindowsFileSystemProvider",
        "newInputStream",
        "(Ljava/nio/file/Path;[Ljava/nio/file/OpenOption;)Ljava/io/InputStream;",
    ...
    // readAttributes(Path,String,LinkOption[]) held OUT (2026-09-22): the
    // name-keyed sibling of the Class-keyed overload above/in the wave-16
    // table -- same jarfs/jrtfs-aware `fsp` body (`read_named_attributes`),
    // same carrier-sharing defect.
    (
        "sun/nio/fs/WindowsFileSystemProvider",
        "setAttribute",
...
/// Lane 4 wave 17: originally 65 rows over 20 classes -- the buffer,
/// `CoderResult`, `ByteOrder`, `FileDescriptor$1`, `NativeThreadSet`,
/// `FileChannelImpl` and stream-constructor rows the funnel still counted
/// after waves 1-16. Now 63 rows over 19 classes: `sun/nio/ch/Util`'s two rows
/// were removed 2026-09-22 (see below the table's closing bracket).
///
/// Every row is an `owns_slot`, `Bridge`, image-`Code` row that a lane-4 probe
/// invoked (`invocations > 0`), so each hands a method the JDK has real
/// bytecode for to that bytecode. `L4ByteBufferSweep`, `L4TypedBufferSweep`,
/// `L4CharBufferSweep`, `L4StreamTailSweep`, `L4PrintStreamSweep`,
/// `L4W6PrintCarrier`, `L4FileSweep` and `L4Reach` all stay at zero strict diff
/// lines against HotSpot with the whole table retired (debug binary).
///
/// `Buffer.session()` / `checkSession()` were in this table's first cut and are
/// OUT: `RETIRED_SHADOW_JOTP_TRIPLES`'s doc comment measured, on real netty
/// tests, that a segment-backed buffer's real `session()` runs against this
/// VM's own arena carrier (`NoSuchMethodError` on
/// `ArenaImpl.checkValidStateRaw`), and `the_jotp_wave_refuses_the_buffer_
/// session_pair` pins them out. The regression suite and every lane-4 probe
/// were green with them retired, which is the point: none of them builds a
/// segment-backed buffer. The rows that remain (`Buffer$2.acquireSession` /
/// `releaseSession`, the `DirectByteBuffer` accessors) reach the session only
/// through `session()`, which stays the native shim; that is an argument from
/// the bytecode, NOT a measurement, and the two netty tests above are the
/// verification still owed.
///
/// Not in the table, on purpose: `java/io/ByteArrayInputStream`'s three
/// funnel rows (wave 5 carved them out: they carry an OBSERVATION),
/// `java/nio/file/FileSystems`'s three, `FileSystemProvider.installedProviders`,
/// `WindowsFileAttributes.fileKey` and the two Windows watch/access rows (each
/// named in the table above this one).
static RETIRED_SHADOW_L4_BUFFERS2_TRIPLES: &[(&str, &str, &str)] = &[
    (
...
// `sun/nio/ch/Util.getTemporaryDirectBuffer(I)Ljava/nio/ByteBuffer;` and
// `.offerFirstTemporaryDirectBuffer(Ljava/nio/ByteBuffer;)V` were REMOVED from
// this table 2026-09-22 (H2 census): this wave's own doc comment above says the
// retirement rests on single-shot lane-4 probes (`invocations > 0`, zero strict
// diff), which cannot exercise the bug that makes these two natives necessary —
// `sun/nio/ch/Util$BufferCache` is a real `ThreadLocal`, but several CratonVM
// VM-worker threads can observe the SAME cache instance, and its unsynchronised
// `count`/`start`/ring-array bytecode then hands out a `count` that claims a
// null array slot. The caller's next `buf.capacity()` NPEs with "buf is null"
// (or, depending on timing, a stale/reused buffer throws
// `BufferUnderflowException`) — reproduced on 8 H2 classes
// (`TestReadOnly`/`TestCluster`/`TestUrlJavaObjectSerializer`/
// `TestAutoReconnect`/`TestJakartaServlet`/`TestPageStoreCoverage`/
// `TestPgServer`/`TestServlet`) once `--jdk-only` became the default
// (2026-09-20) sent these two back to the retired real bytecode. Re-registered
// as `NativeKind::Intrinsic` in `native-io/src/direct_buffer.rs` — the
// reviewed-correct exception, not a `Bridge` this table's gate can downgrade —
// because this native returns exactly what the real bytecode would, just
// without the race.
...
/// L1 wave 9 -- the `LinkedHashMap` family and what was left of `HashMap`,
/// 168 triples, on a carrier that now carries its own state.
///
/// # The one blocker two families had been waiting on since wave 3
///
/// Wave 7 measured this family on THREE trial binaries and refused it, and the
/// refusal was not about the store: `lhm_set` mirrors `head`, `tail`, `size`
/// and `table` into the real fields, `lhm_alloc_node` binds a real
/// `java/util/LinkedHashMap$Entry`, and on the wave-7 control every one of
/// them reads back identical to HotSpot 25.0.4+7. What refused it was the
/// WRITE direction, in two forms that looked like a dilemma:
///
/// ```text
...
///   the 12 iterator      no crash, 6 probes worse: every mutation through a
///   rows withdrawn       SequencedMap default is LOST
/// ```
///
/// Both horns are the same object. This crate mints its map iterators under
/// real class names and drives them from five snapshot fields appended past
/// the declared ones (`key_itr_base`); the class's OWN fields -- `this$0`,
/// `next`, `current`, `expectedModCount`, `index` -- were left at their
/// allocation defaults. That is invisible while the natives registered on
/// those classes are the only reader, and a retirement is precisely what ends
/// that. Retire the iterators and a real body reads a null `this$0`; keep
/// them and a real `SequencedMap` default writes through a VM cursor into a
/// model the real map no longer reads.
///
/// `seed_real_map_itr_fields` (`native-collections/src/lib.rs`) writes those
/// fields at the mint site, out of the JDK's own constructors read off
/// `javap -p -c` on all three images. The dilemma was an artefact of a carrier
/// that had a real class name and none of its state.
///
/// # Why this is not lane T's registrar after all
///
/// §10 item 3 of the lane page has said since wave 5 that these rows need a
/// `HashSet`-family iterator carrier, and that the carrier has to come with
/// `register_hashset_natives` -- whose `SET_CLASSES` spans `java/util/` and
/// `java/util/concurrent/`, making the registrar lane T's whole. That is
/// still true of the REGISTRAR and it turned out not to be true of the
/// BLOCKER. Nothing here retires a `java/util/HashSet` or
/// `java/util/LinkedHashSet` row, and `SET_CLASSES` is untouched: the fix is
/// at the site that MINTS the carrier, which serves the set family and the map
/// family alike and is reached from both. A blocker attributed to a registrar
/// because the registrar produced the symptom.
///
/// # Scope
///
/// 168 rows: every `owns_slot && kind == "bridge"` registration under
/// `java/util/HashMap` and `java/util/LinkedHashMap` and their view, iterator
/// and node classes that is bucket A or B **on all three supported images**.
///
/// Nothing is subtracted for waves 3 and 7, and that is not an oversight: a
/// retirement re-tags the registration `SyntheticStub`, so a row an earlier
/// wave took is no longer `kind == "bridge"` and the census does not offer it.
/// The first cut of this wave subtracted 14 rows anyway, having matched them
/// against the whole of `retired_shadow.rs` including the HOLD TESTS' own
/// arrays -- lists of triples those tests exist to assert are NOT retired.
/// Three of the 14 were live and `cargo test` said so within the minute.
///
/// Eleven rows stay out because
/// jdk-17.0.20.1+1 declares none of them -- the whole JDK 21 `SequencedMap`
/// surface (`putFirst`, `putLast`, `pollFirstEntry`, `pollLastEntry`,
/// `reversed`, `sequencedKeySet`, `sequencedValues`, `sequencedEntrySet`, and
/// `reversed` on the three view carriers).
static RETIRED_SHADOW_L1_LHM_TRIPLES: &[(&str, &str, &str)] = &[
    ...
    ("java/util/HashMap$EntrySet", "add", "(Ljava/lang/Object;)Z"),
    (
...
/// L1 wave 9 -- `java/util/Date`, 19 triples.
///
/// # `sun/util/calendar/ZoneInfo`'s nine rows were WITHDRAWN 2026-09-17
///
/// This table originally retired 28 rows: the 19 `java/util/Date` rows below,
/// plus all nine `Bridge`-category `sun/util/calendar/ZoneInfo` triples
/// (`getDSTSavings`, `useDaylightTime`, `observesDaylightTime`,
/// `inDaylightTime`, the six-arg `getOffset`, the one-arg `getOffset`,
/// `getOffsets`, `getOffsetsByWall`, `getRawOffset`). MEASURED on a fresh
/// `--jdk-only` build off `origin/dev`: with those nine retired, `register()`
/// refuses ALL of them under `--jdk-only` (a `SyntheticStub` is refused
/// without being inserted, so the class does not appear in
/// `--dump-native-registry` at all -- `grep -c 'sun/util/calendar'
/// reg.json` reads 0), and real JDK bytecode runs every DST accessor against
/// `alloc_synth_timezone`'s fabricated `ZoneInfo`, whose `transitions` array
/// is null and always has been. That is the EXACT defect
/// `G17-1-the-dst-family-and-the-fixture-that-compared-two-empty-strings-20260817.md`
/// and `G28-1-the-dst-rule-layer-rebuilt-20260817.md` fixed with the five
/// natives in `native-builtins/src/tzdb.rs::register_zoneinfo_dst_natives`,
/// reintroduced whole: `RSimpleDateFormatZone` throws
/// `getTimeZone("America/New_York").getOffset(JUL) must be -14400000 (DST)`
/// and `RSimpleTimeZoneRaw` reads `checks=393 fails=93`, both 100% repro,
/// both alone, on a low-loaded host -- not the `RMapGcStress` flake shape.
///
/// This table's own doc, below, already named the risk and reasoned past it:
/// `LocaleDateTzShadowSweep` armed ONE class at a time read
/// `sun/util/calendar/ZoneInfo` at **+4 WORSE**, and wave 9 retired it anyway
/// on a DIFFERENT, broader probe's **+0** reading (`L1LocaleProviderWorkload`,
/// 147 rows over multiple prefixes at once). §7's own words for that shape:
/// "a dial declines at the dispatch door while a retirement deletes the
/// registration" -- and "Convert a dial result into a table entry only via a
/// registry dump **from a run of the very probe whose improvement you are
/// citing**" (`docs/contributing/jdk-only-lane-operations.md` §"And the dial
/// is not the retirement"). `L1LocaleProviderWorkload`'s 411 engagements on
/// `sun/util/calendar/` were never the DST family's own contract edges --
/// `RSimpleTimeZoneRaw` is -- and the per-class **+4 WORSE** reading was the
/// one that turned out to be right.
///
/// # Two readings of the same family, and neither of them a verdict
///
/// `LocaleDateTzShadowSweep` armed one class at a time read
/// `sun/util/calendar/ZoneInfo` at **+4 WORSE**, and that is the reading §1 of
/// the lane page held this family on. Wave 6 then built
/// `apps/probes/L1LocaleProviderWorkload` -- 147 rows driving every public API
/// whose real-JDK implementation goes through these classes -- and read the
/// same prefixes at **+0**, with the engagement counter saying the probe had
/// actually asked:
///
/// ```text
///   java/util/Date          +0   reached=14
...
/// ```
///
/// Both are DIAL readings, and a dial declines at the dispatch door while a
/// retirement deletes the registration. That is why §12 of the lane page has
/// carried these as "candidates with no trial binary yet" for two waves: the
/// only instrument that can answer is a second binary, and this table is it.
/// **The `ZoneInfo` half of that binary is now measured, above, and the
/// answer is NO.**
///
/// # `java/util/TimeZone` is NOT here, and was censused with the rest
///
/// Ten of its rows are bucket A or B on all three images and it would have
/// been the natural third class. It stays out because it is the one member of
/// this family with a measurement AGAINST it that a dial artefact does not
/// explain: `the_l1_families_measured_load_bearing_are_still_held` records
/// `java/util/TimeZone.getID()Ljava/lang/String;` as measured load-bearing by
/// L1 wave 1, and `LocaleDateTzShadowSweep` read the class at **+2 WORSE**.
/// Wave 6's `+0` at reached=196 is evidence the other way, and two readings
/// that disagree are a reason to spend a binary on it alone rather than to
/// fold it into a table with rows that do not need one.
///
/// # Scope
///
/// 19 rows: every `owns_slot && kind == "bridge"` registration under
/// `java/util/Date` that is bucket A or B on all three supported images --
/// jdk-17.0.20.1+1, jdk-21.0.12+8 and jdk-25.0.4+7, censused separately.
/// `sun/util/calendar/ZoneInfoFile` contributes nothing: wave 5 took its two
/// rows (`RETIRED_SHADOW_L1_ZI_TRIPLES`) and no other row of that class is a
/// `Bridge` this crate owns.
static RETIRED_SHADOW_L1_DATE_TRIPLES: &[(&str, &str, &str)] = &[
    ...
    // The nine `sun/util/calendar/ZoneInfo` rows that were here (getDSTSavings,
    // both getOffset overloads, getOffsets, getOffsetsByWall, getRawOffset,
    // inDaylightTime, observesDaylightTime, useDaylightTime) were WITHDRAWN
    // 2026-09-17: see this table's own doc comment above for the measurement.
];

/// `java/lang/StringBuilder` + `java/lang/AbstractStringBuilder`, 2026-09-21 --
/// 118 of the 124 `Bridge` registrations `--dump-native-registry` reports for
/// the two classes, closing `H14-2`/`H14-3`'s last unretired "free" family
/// (`H14-3` N2, priced at zero corpus cost the same day `H22-2` found it
/// FATAL) and the open item the `StringBuilder` JIT-intrinsic lane
/// (`docs/internal/jdk-only/lane-stringbuilder-jit-intrinsic-RETIRED-20260917.md`)
/// explicitly left for "a future lane": landing the rows into this file.
///
/// # The list is machine-generated, not hand-picked
///
/// Every triple below has `owns_slot: true` and a `real_declaring_method` that
/// is `loaded`, `declared` on THAT exact class, `has_code` and NOT
/// `acc_native` -- i.e. real JDK 25 bytecode this class itself owns, not one
/// it merely inherits. The filter excludes exactly six of the 124 rows, and
/// each exclusion matches a `lang_string.rs` comment written before this table
/// existed:
///
/// * `AbstractStringBuilder.toString()` -- `public abstract`, no `Code`
///   attribute on any supported image (`register_string_builder_natives`'s own
///   "DO NOT RETIRE" comment, `H14-1` §4/`H25-2`'s two-row "no implementation"
///   bucket).
/// * `StringBuilder.append(AbstractStringBuilder)`, `.appendNull()`,
///   `.getCoder()`, `.getValue()`, `.repeat(CI)` -- registered on
///   `StringBuilder` by the shared `register_string_builder_natives(registry,
///   class)` helper, but NOT declared by `StringBuilder` itself (only by
///   `AbstractStringBuilder`, whose own copies of the same four methods ARE in
///   this table). Retiring the `StringBuilder`-keyed registration would be
///   retiring a triple with no receiver of its own; dispatch already resolves
///   these through the real `AbstractStringBuilder` inheritance chain once the
///   `AbstractStringBuilder` rows below stop shadowing it.
///
/// # `H22-2`'s FATAL result does not reproduce today -- MEASURED, not argued
///
/// `H22-2` (2026-08-21) armed exactly these two classes and got `rc=1`,
/// `ArrayStoreException: can not copy byte[] into char[]`, on the theory that
/// CratonVM's `char[]`/`count` builder cannot run JDK 25's compact-string
/// `AbstractStringBuilder` bytecode. `lang_string.rs` still carries several
/// comments repeating that warning verbatim for `codePointAt`/`codePointBefore`/
/// `codePointCount`/`appendCodePoint`, `getValue`/`getCoder`/`getChars`, the
/// four scalar `insert` overloads and the two `repeat` overloads named there --
/// every one of which IS in this table. Before trusting either the six-week-old
/// crash or the five-week-old comments over the other, this table was preceded
/// by a fresh differential probe (`SbLayoutRisk.java`, this session, on a clean
/// build of this exact commit) exercising precisely those flagged methods --
/// `codePointAt`, `getChars`, `insert(int,boolean)`, `insert(int,double)`,
/// `repeat(char,int)`, `repeat(CharSequence,int)` -- run three ways:
///
/// ```text
///   HotSpot 25.0.4+7                                byte-identical
///   CratonVM, unarmed                                byte-identical
///   CratonVM, --jdk-only + CRATONVM_ENFORCE_NATIVE_SHADOW=
///     java/lang/StringBuilder,java/lang/AbstractStringBuilder   byte-identical
///       [DIAL_DOOR_CENSUS] armed=true reached=102 yielded=102 leaked=0
/// ```
///
/// Zero leaked natives, zero divergence, on the arm that is strictly the same
/// shape as this table (per-triple re-tag to `SyntheticStub`, refused under
/// `--jdk-only`). The crash `H22-2` found does not reproduce against the
/// current object layout; the `lang_string.rs` warnings are stale documentation
/// of a defect that was fixed elsewhere in the five weeks since, and nobody
/// removed the comments. Two later lanes had already measured the same
/// direction more broadly and less precisely: lane 2
/// (`docs/internal/jdk-only/lane-2-lang-values-RETIRED-20260911.md`, dial-armed,
/// 40/0 corpus) and the JIT-intrinsic lane (2026-09-17, full `SUITE=all`
/// 136/0 and `SUITE=core` 95/0 with the same two classes armed) -- both
/// correctly flagged as "a clean arm means the corpus did not ask" rather than
/// proof, which is why this table's own probe targets the exact methods their
/// suites do not happen to exercise.
///
/// docs/known-issues/jdk-only/H14-3-five-retirements-are-free-and-properties-is-worse-than-hashmap-20260821.md
static RETIRED_SHADOW_STRINGBUILDER_TRIPLES: &[(&str, &str, &str)] = &[
    ("java/lang/AbstractStringBuilder", "<init>", "()V"),
    ("java/lang/AbstractStringBuilder", "<init>", "(I)V"),
    ("java/lang/AbstractStringBuilder", "<init>", "(Ljava/lang/CharSequence;)V"),
    ("java/lang/AbstractStringBuilder", "<init>", "(Ljava/lang/String;)V"),
    ("java/lang/AbstractStringBuilder", "append", "(C)Ljava/lang/AbstractStringBuilder;"),
    ("java/lang/AbstractStringBuilder", "append", "(C)Ljava/lang/Appendable;"),
    ...
    ("java/lang/AbstractStringBuilder", "append", "([C)Ljava/lang/AbstractStringBuilder;"),
    ("java/lang/AbstractStringBuilder", "append", "([CII)Ljava/lang/AbstractStringBuilder;"),
    (
        "java/lang/AbstractStringBuilder",
        "appendCodePoint",
    ...
    ("java/lang/AbstractStringBuilder", "getCoder", "()B"),
    ("java/lang/AbstractStringBuilder", "getValue", "()[B"),
    ("java/lang/AbstractStringBuilder", "indexOf", "(Ljava/lang/String;)I"),
    ("java/lang/AbstractStringBuilder", "indexOf", "(Ljava/lang/String;I)I"),
    ("java/lang/AbstractStringBuilder", "insert", "(IC)Ljava/lang/AbstractStringBuilder;"),
    ...
    ("java/lang/AbstractStringBuilder", "repeat", "(CI)Ljava/lang/AbstractStringBuilder;"),
    ("java/lang/AbstractStringBuilder", "repeat", "(II)Ljava/lang/AbstractStringBuilder;"),
    (
        "java/lang/AbstractStringBuilder",
        "repeat",
    ...
    ("java/lang/AbstractStringBuilder", "reverse", "()Ljava/lang/AbstractStringBuilder;"),
...
pub(crate) const RETIRED_SHADOW_TABLES: &[&[(&str, &str, &str)]] = &[
    ...
    // L1 wave 9. Listed in the same commit as the tables, which is the whole
    // of what the five notes above are about: every one of them is a lane that
    // wrote a table on a branch this const did not see.
    RETIRED_SHADOW_L1_LHM_TRIPLES,
    RETIRED_SHADOW_L1_DATE_TRIPLES,
    ...
    // Lane 4 wave 7, the Path half of the Path/FileSystemProvider retirement
    // wave. No new prefix: `java/nio/` was admitted by wave 1.
    RETIRED_SHADOW_L4_PATH_TRIPLES,
    // Lane 4 wave 9, 2026-09-17. No new prefix: `java/nio/` was admitted by
    // wave 1 and already covers `java/nio/file/spi/FileSystemProvider`. Four
    // of the nine rows the carrier fix, the `Path.fs`-field fix and the
    // `FileSystem` carrier fix together unblocked; see the table's own doc
    // comment for why the other five stayed out.
    RETIRED_SHADOW_L4_FILESYSTEMPROVIDER_TRIPLES,
    // Lane 4 wave 8, 2026-09-18. Same merge-drift risk the notes above
    // describe, so landed with this const in the same commit.
    RETIRED_SHADOW_L4_DEPRECATED_STREAMS_TRIPLES,
    RETIRED_SHADOW_L4_FILEVISITOR_TRIPLES,
    RETIRED_SHADOW_L4_CHARBUFFER_VIEWS_TRIPLES,
    RETIRED_SHADOW_L4_PATHS_TRIPLES,
    // Lane 4 wave 10/13, 2026-09-18. 16 of the 19 original candidates,
    // landed after three real bugs found and fixed across three attempts;
    // getFileName/getParent/toRealPath excluded (see the table's own doc:
    // the first two do a direct '\\'-scan no field pre-population can fix
    // on this VM's '/'-stored path, and toRealPath reaches a Win32 native
    // -- GetFullPathName0 -- this VM has not implemented yet).
    RETIRED_SHADOW_L4_WINDOWSPATH_TRIPLES,
    // 2026-09-19, the `--jdk-only` throughput wave. Registered here in the same
    // commit as the table, for the reason every note above gives.
    RETIRED_SHADOW_JOTP_TRIPLES,
    // Part two of the same wave. A separate table so the two halves can be disarmed
    // independently by `CRATONVM_UNRETIRE_NATIVE_SHADOW`.
    RETIRED_SHADOW_JOTP_UNALIGNED_TRIPLES,
    // Lane 4 wave 16, 2026-09-19. The Windows `sun/nio/fs` filesystem classes;
    // landed with this const in the same commit for the merge-drift reason the
    // notes above give.
    RETIRED_SHADOW_L4_WINFS_TRIPLES,
    // Lane 4 wave 17, 2026-09-19. The eleven `WinNTFileSystem` wrappers.
    RETIRED_SHADOW_L4_WINNTFS_TRIPLES,
    // Wave 18's Linux twin of the line above, measured on linux/x86_64.
    RETIRED_SHADOW_L4_UNIXFS_TRIPLES,
    // Wave 19's 63 rows, the ledger's remainder.
    RETIRED_SHADOW_L4_W19_TRIPLES,
    RETIRED_SHADOW_L4_FILETMP_TRIPLES,
    RETIRED_SHADOW_L4_FILES2_TRIPLES,
    RETIRED_SHADOW_L4_BUFFERS2_TRIPLES,
    // 2026-09-21, closing `H14-3` N2. Landed with this const in the same
    // commit, per every note above.
    RETIRED_SHADOW_STRINGBUILDER_TRIPLES,
];
...
mod tests {
    ...
    fn the_charbuffer_wave_is_seventeen_bridges_without_the_intrinsic() {
        for (c, m, d) in RETIRED_SHADOW_L4_CHARBUFFER_TRIPLES {
            ...
            assert_ne!(
                *m, "charAt",
                "charAt(I)C is not part of WAVE 4's table; wave 19 retires it from its \
                 own, after fixing the kind that made a row here inert"
            );
        }
        assert_eq!(RETIRED_SHADOW_L4_CHARBUFFER_TRIPLES.len(), 17);
        // WAVE 19 MOVED THIS, and the reason moves with it. The carve-out was
        // never about `charAt`'s body: it was about its KIND. The row was
        // registered `Intrinsic` -- inherited, `kind_stated: false`, from
        // `register_charset_natives`' `set_category(Intrinsic)` scope, whose
        // census tag is about Charset encode/decode -- and `register` re-tags a
        // retired triple only when the effective category is `Bridge`, so a
        // table row was inert by construction. Wave 19 states the kind at the
        // registration site (`native-builtins/src/lib.rs`, the `__bbacb_prev`
        // scope) as the `Bridge` it always was, and the row is in
        // `RETIRED_SHADOW_L4_W19_TRIPLES`. MEASURED on linux/x86_64: the census
        // reports the triple absent under `--jdk-only` and `bridge` under
        // `CRATONVM_UNRETIRE_NATIVE_SHADOW=all`, and `L4CharBufferSweep` /
        // `L4TypedBufferSweep` are 0-diff against HotSpot either way.
        assert!(
            triple_is_retired_shadow("java/nio/CharBuffer", "charAt", "(I)C"),
            "wave 19 retires charAt from its own table"
        );
        assert!(
            !RETIRED_SHADOW_L4_CHARBUFFER_TRIPLES
                .contains(&("java/nio/CharBuffer", "charAt", "(I)C")),
            "and not from THIS one, whose seventeen are wave 4's measurement"
        );
    }

    /// The `--jdk-only` throughput wave's first table is the three `int` `Preconditions` checks that
    /// take a `BiFunction` -- nothing else.
    ///
    /// The rows are asserted by DESCRIPTOR because the class also carries the non-JDK `(II)I` /
    /// `(III)I` spellings, which have no bytecode to yield to: retiring one would trade a shadow for
    /// an `UnsatisfiedLinkError`.
    #[test]
    fn the_jotp_wave_is_the_three_int_precondition_checks() {
        assert_eq!(RETIRED_SHADOW_JOTP_TRIPLES.len(), 3);
        for (c, m, d) in RETIRED_SHADOW_JOTP_TRIPLES {
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is in the JOTP table and the predicate cannot see it"
            );
            assert_eq!(*c, "jdk/internal/util/Preconditions");
            assert!(
                matches!(*m, "checkIndex" | "checkFromToIndex" | "checkFromIndexSize")
                    && d.ends_with("Ljava/util/function/BiFunction;)I"),
                "{c}.{m}{d} is outside this wave's measured scope"
            );
        }
        // The non-JDK shapes have no bytecode; the `long` overloads are a different wave.
        for (m, d) in [
            ("checkIndex", "(II)I"),
            ...
            assert!(
                !triple_is_retired_shadow("jdk/internal/util/Preconditions", m, d),
                "Preconditions.{m}{d} has no bytecode to yield to and must stay out"
            );
        }
        // The prefix alone admits the class to the search; it must not retire the rest of it.
        assert!(!triple_is_retired_shadow(
            "jdk/internal/util/ArraysSupport",
            "mismatch",
            "([BI[BII)I"
        ));
        // `HeapByteBuffer.getInt(I)I` etc. are NOT in this wave: the accessors themselves stay.
        assert!(!triple_is_retired_shadow(
            "java/nio/HeapByteBuffer",
            "getInt",
            "(I)I"
    ...
    /// `session()` / `checkSession()` on the ten buffer classes that register the shim were
    /// retired by this wave's first cut and are REFUSED: measured, on one release binary,
    /// `PcapWriteHandlerTest` 8 ok / 17 failed and `AdaptiveBigEndianDirectByteBufTest` 339 ok / 76
    /// failed with them retired, 25 / 0 and 415 / 0 with them kept, every failure a
    /// `NoSuchMethodError` on `ArenaImpl.checkValidStateRaw()`. See
    /// `RETIRED_SHADOW_JOTP_TRIPLES`'s doc comment for why: a segment-backed buffer's real
    /// `session()` runs against this VM's own arena carrier.
    ///
    /// `CharBuffer` is the one class of the family that IS retired (wave 4), and is excluded here
    /// on purpose: it took the pair on a measurement of its own and nothing builds a segment-backed
    /// `CharBuffer`. Guarded by name rather than by count so a later wave that widens by class
    /// fails HERE and not in a netty run.
    #[test]
    fn the_jotp_wave_refuses_the_buffer_session_pair() {
        for c in [
            "java/nio/Buffer",
            "java/nio/ByteBuffer",
            "java/nio/DirectByteBuffer",
            ...
            assert!(
                ...
                "{c}.session() is retired: a segment-backed buffer's real session() runs \
                 against the VM's own arena carrier (NoSuchMethodError on \
                 ArenaImpl.checkValidStateRaw)"
            );
            assert!(
                !triple_is_retired_shadow(c, "checkSession", "()V"),
                "{c}.checkSession() is retired: it calls session()"
            );
        }
        assert!(
            triple_is_retired_shadow(
            ...
            "CharBuffer.session() is wave 4's own retirement"
        );
    }

    /// Part two of the `--jdk-only` throughput wave is the sixteen `Unsafe.*Unaligned`
    /// accessors -- the `get`/`put` of `char`/`short`/`int`/`long`, in the 2-arg and the
    /// `bigEndian` 3-arg spelling -- and nothing else on `Unsafe`.
    ///
    /// The neighbours it must NOT take are asserted by name, because they are the ones the
    /// original "slot index" argument was about and it is still true of them: an `Unsafe` offset
    /// that came from `objectFieldOffset` is a slot index, so the sub-word atomics (which mask
    /// within an enclosing word at a computed byte offset) and the four numbering methods stay.
    #[test]
    fn the_jotp_unaligned_wave_is_the_sixteen_array_and_address_accessors() {
        assert_eq!(RETIRED_SHADOW_JOTP_UNALIGNED_TRIPLES.len(), 16);
        for (c, m, d) in RETIRED_SHADOW_JOTP_UNALIGNED_TRIPLES {
            assert_eq!(*c, "jdk/internal/misc/Unsafe");
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is in the JOTP unaligned table and the predicate cannot see it"
            );
            assert!(
                m.ends_with("Unaligned")
                    && (m.starts_with("get") || m.starts_with("put"))
                    && ["Char", "Short", "Int", "Long"]
                        .iter()
                        .any(|w| m.contains(w)),
                "{m} is not one of the sixteen"
            );
        }
        for m in [
            ...
            for (c, tm, _) in RETIRED_SHADOW_JOTP_UNALIGNED_TRIPLES {
                assert_ne!(
                    (*c, *tm),
                    ("jdk/internal/misc/Unsafe", m),
                    "jdk/internal/misc/Unsafe.{m} keeps its slot-index reason and must not be here"
                );
            }
        }
    }
    ...
    fn the_typed_buffer_families_beside_charbuffer_are_not_retired() {
        // Wave 17 (`RETIRED_SHADOW_L4_BUFFERS2_TRIPLES`) measured `java/nio/Buffer`
        // and retires exactly `position`, `limit`, `capacity` and the
        // five-argument `<init>`; `remaining`, `hasArray` and both `toString`
        // forms stay out, and so does everything on the five typed-buffer
        // classes that is not one of that table's `clear`/`get`/`put` rows.
        for c in [
            "java/nio/Buffer",
            "java/nio/IntBuffer",
            "java/nio/LongBuffer",
            ...
            for (m, d) in [
                ...
                let wave17_measured =
                    c == "java/nio/Buffer" && matches!(m, "position" | "limit" | "capacity" | "<init>");
                // WAVE 19 (2026-09-20) MEASURED THE FIVE, on linux/x86_64.
                // The guard's premise -- "none of them has been through a
                // probe" -- stopped being true, so the allowance moves with
                // the measurement rather than the assertion being deleted.
                // `L4TypedBufferSweep` is 409 rows per family over heap,
                // direct and VIEW receivers in both byte orders, reading every
                // value back, and it is 0-diff against HotSpot with
                // `position`, `limit`, `capacity`, `remaining`,
                // `hasRemaining`, `flip` and `array` retired on all five. What
                // stays out is what stays out for a reason: both `toString`
                // forms (the VM's carrier prints its own shape), `hasArray`,
                // and everything on `java/nio/Buffer` itself beyond wave 17's
                // four -- that class's `<init>` is the constructor EVERY buffer
                // runs, which is the paragraph above and still holds.
                let wave19_measured = c != "java/nio/Buffer"
                    && matches!(m, "position" | "limit" | "capacity" | "remaining");
                assert!(
                    wave17_measured
                        || wave19_measured
                        || !triple_is_retired_shadow(c, m, d),
                ...
                if wave19_measured {
                    assert!(
                        triple_is_retired_shadow(c, m, d),
                        "{c}.{m}{d} is wave 19's and the predicate cannot see it"
                    );
                }
            }
        }

        // `HeapCharBuffer`/`HeapCharBufferR`/`StringCharBuffer` are the same
        // shape, EXCEPT `toString(II)` -- wave 8's own
        // `RETIRED_SHADOW_L4_CHARBUFFER_VIEWS_TRIPLES` retires exactly that
        // one triple per class, measured, and this guard would otherwise fire
        // on its own wave's landing.
        for c in [
            ...
            for (m, d) in [
                ...
                // WAVE 19 measured ONE of these, and only on
                // `StringCharBuffer`: its no-argument `toString()`, which the
                // JDK implements as `toString(0, length())` over the wrapped
                // `CharSequence`. `L4CharBufferSweep` builds one with
                // `CharBuffer.wrap(CharSequence)` and reads it back
                // (`charAt`, `subSequence`, both `toString` forms, the
                // position/limit walk); 0-diff against HotSpot on
                // linux/x86_64. The two HEAP spellings are NOT measured and
                // stay out -- their `toString()` reads `hb`/`offset` on a
                // carrier this VM mints.
                let wave19_measured = c == "java/nio/StringCharBuffer"
                    && (m, d) == ("toString", "()Ljava/lang/String;");
                assert!(
                    wave19_measured || !triple_is_retired_shadow(c, m, d),
                    "{c}.{m}{d} is retired, and no wave has measured {c}"
                );
            }
            assert!(
                triple_is_retired_shadow(c, "toString", "(II)Ljava/lang/String;"),
                "{c}.toString(II) is wave 8's own retirement and the predicate cannot see it"
            );
        }
    }
    ...
    /// Wave 7's Path table: nine rows, all `java/nio/file/Path`, all the
    /// DEFAULT/STATIC methods `javap -p java.nio.file.Path` shows with `Code`
    /// -- and the twenty-odd abstract ones this wave deliberately left alone,
    /// asserted absent by name rather than by omission, PLUS the tenth
    /// candidate (`register(WatchService, Kind...)`) the table's own doc
    /// comment excludes on a measured `NullPointerException` from
    /// `WatchService`'s own unrelated carrier gap.
    #[test]
    fn wave_seven_is_nine_path_rows_and_the_abstract_ones_stay_out() {
        assert_eq!(RETIRED_SHADOW_L4_PATH_TRIPLES.len(), 9);
        for (c, m, d) in RETIRED_SHADOW_L4_PATH_TRIPLES {
            assert_eq!(
            ...
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is in the wave-7 Path table and the predicate cannot see it"
            );
        }

        // The twenty-odd `public abstract` methods `javap -p java.nio.file.Path`
        // shows with no `Code` -- a row for any of these replaces a working
        // native with an unimplemented interface method. `startsWith`/
        // `resolve`/`endsWith`/`resolveSibling` each have BOTH a default
        // (`String`) and an abstract (`Path`) overload with the same name; only
        // the `Path;)Z`/`Path;)Ljava/nio/file/Path;`-descriptor halves are
        // listed here, precisely because that is the pair a copy-paste would
        // collapse.
        for (m, d) in [
            ...
            ("endsWith", "(Ljava/nio/file/Path;)Z"),
            ("normalize", "()Ljava/nio/file/Path;"),
            ("resolve", "(Ljava/nio/file/Path;)Ljava/nio/file/Path;"),
            (
                "register",
                "(Ljava/nio/file/WatchService;[Ljava/nio/file/WatchEvent$Kind;\
                 [Ljava/nio/file/WatchEvent$Modifier;)Ljava/nio/file/WatchKey;",
            ),
            ("relativize", "(Ljava/nio/file/Path;)Ljava/nio/file/Path;"),
            // Not abstract -- a default method, same as the nine that ARE
            // retired -- but excluded anyway. See the table's own "excluded
            // on a MEASURED NPE" section: `WatchService.poller` is never
            // populated, so real bytecode NPEs where the native worked.
            (
                "register",
            ...
            assert!(
                !triple_is_retired_shadow("java/nio/file/Path", m, d),
                "java/nio/file/Path.{m}{d} is `public abstract` in java.nio.file.Path \
                 (javap -p, JDK 25) -- retiring it replaces a working native with an \
    ...
    /// The FOURTH backed-out family's blockers closed by 2026-09-17, and this
    /// wave is four of its nine rows, not the whole family and not zero.
    ///
    /// `the_provider_family_is_retired_by_no_table` used to assert the
    /// opposite of this -- that no table named
    /// `java/nio/file/spi/FileSystemProvider` at all -- because
    /// `p57_alloc_provider` stamped every provider this VM built with the
    /// ABSTRACT class on purpose, and a call against that receiver reached a
    /// declaration with no body. Three carrier fixes closed that in turn:
    /// Path (`636ef84ed`), FileSystemProvider (`24f103568`), and finally
    /// `FileSystem` itself (2026-09-17, `cratonvm_native_api::filesystem_layout`)
    /// -- the receiver is now `sun.nio.fs.WindowsFileSystemProvider` (this
    /// machine) or its per-platform sibling, and `javap -p` on that concrete
    /// class says which of the census's nine candidate rows it actually
    /// overrides. See `RETIRED_SHADOW_L4_FILESYSTEMPROVIDER_TRIPLES`'s own
    /// doc comment for the full nine-row table, the three blockers in the
    /// order they were found, and why five of the nine rows are excluded on
    /// `javap -c` evidence rather than cleared by "the abstract class has a
    /// body".
    #[test]
    fn the_provider_family_retirement_is_four_of_nine_bucket_ab_rows() {
        assert_eq!(RETIRED_SHADOW_L4_FILESYSTEMPROVIDER_TRIPLES.len(), 4);
        for (c, m, d) in RETIRED_SHADOW_L4_FILESYSTEMPROVIDER_TRIPLES {
            assert_eq!(
            ...
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is in the wave-9 FileSystemProvider table and the predicate cannot see it"
            );
        }

        // The five held out, by name, so an absence reads as a decision and
        // not an oversight the sorted/unique/reachability guards cannot see
        // (those guards are per-table and say nothing about what is missing).
        for (c, m, d) in [
            (
                "java/nio/file/spi/FileSystemProvider",
                "newFileSystem",
            ...
            // `newInputStream` / `newOutputStream` were held here until wave
            // 17, which retired them (the wave-17 table's doc comment, and
            // `wave_seventeen_files_and_provider_rows`).
            (
                "java/nio/file/spi/FileSystemProvider",
                "installedProviders",
                "()Ljava/util/List;",
            ...
            assert!(
                !triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is held out by this wave's own doc comment and must stay that way \
                 until the reason named there is closed"
            );
        }
    }

    /// Wave 8's deprecated-streams table: 13 rows over two classes, each
    /// class's WHOLE native surface (`<init>` through every accessor) taken
    /// together -- the carrier-defect precaution this table's own doc
    /// comment gives.
    #[test]
    fn wave_eight_deprecated_streams_is_whole_class_each() {
        assert_eq!(RETIRED_SHADOW_L4_DEPRECATED_STREAMS_TRIPLES.len(), 13);
        let mut lnis = 0;
        let mut sbis = 0;
        for (c, m, d) in RETIRED_SHADOW_L4_DEPRECATED_STREAMS_TRIPLES {
            assert!(
                *c == "java/io/LineNumberInputStream" || *c == "java/io/StringBufferInputStream",
                "{c}.{m}{d} is not one of this table's two classes"
            );
            if *c == "java/io/LineNumberInputStream" {
                lnis += 1;
            } else {
                sbis += 1;
            }
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is in the wave-8 deprecated-streams table and the predicate cannot see it"
            );
        }
        assert_eq!(lnis, 8, "LineNumberInputStream's registered surface is 8 rows");
        assert_eq!(sbis, 5, "StringBufferInputStream's registered surface is 5 rows");

        // The constructor for each class is IN the table -- the whole point
        // of retiring the class together rather than per-accessor.
        assert!(triple_is_retired_shadow(
            "java/io/LineNumberInputStream",
            "<init>",
            "(Ljava/io/InputStream;)V"
    ...
    /// Wave 8's `SimpleFileVisitor` table: four rows, the class's whole erased
    /// bridge surface, and none of the carrier question -- the class declares
    /// no instance fields.
    #[test]
    fn wave_eight_simple_file_visitor_is_four_bridge_rows() {
        assert_eq!(RETIRED_SHADOW_L4_FILEVISITOR_TRIPLES.len(), 4);
        for (c, m, d) in RETIRED_SHADOW_L4_FILEVISITOR_TRIPLES {
            assert_eq!(
                *c, "java/nio/file/SimpleFileVisitor",
                "{c}.{m}{d} is not this table's class"
            );
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is in the wave-8 SimpleFileVisitor table and the predicate cannot see it"
            );
        }
    }

    /// Wave 8's `ByteBufferAsCharBuffer` views table: four rows, all
    /// `toString(int,int)`, the concrete subclasses `ByteBuffer.asCharBuffer()`
    /// actually returns -- wave 4 retired only the base-class row.
    #[test]
    fn wave_eight_charbuffer_views_is_seven_tostring_ii_rows() {
        assert_eq!(RETIRED_SHADOW_L4_CHARBUFFER_VIEWS_TRIPLES.len(), 7);
        for (c, m, d) in RETIRED_SHADOW_L4_CHARBUFFER_VIEWS_TRIPLES {
            assert_eq!(*m, "toString");
            assert_eq!(*d, "(II)Ljava/lang/String;");
            assert!(
                c.starts_with("java/nio/ByteBufferAsCharBuffer")
                    || *c == "java/nio/HeapCharBuffer"
                    || *c == "java/nio/HeapCharBufferR"
                    || *c == "java/nio/StringCharBuffer",
                "{c}.{m}{d} is not one of this table's seven classes"
            );
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is in the wave-8 CharBuffer-views table and the predicate cannot see it"
            );
        }
    }

    /// Wave 9's single row: `Paths.get`, pure string-joining into the same
    /// Path allocator every other producer already uses.
    #[test]
    fn wave_nine_paths_get_is_one_row() {
        assert_eq!(RETIRED_SHADOW_L4_PATHS_TRIPLES.len(), 1);
        assert!(triple_is_retired_shadow(
            "java/nio/file/Paths",
            "get",
            "(Ljava/lang/String;[Ljava/lang/String;)Ljava/nio/file/Path;"
    ...
    /// Wave 10/13/14/15's 21 rows: `WindowsPath`'s `Path`-interface methods,
    /// landed after four real bugs found and fixed across four attempts.
    /// `getFileName`/`getParent` were excluded through wave 14 because both
    /// scan `this.path` directly for a literal `'\\'`, which this VM's
    /// `/`-stored `path` could never contain; wave 15 changed the STORAGE
    /// form to the JDK's own (`path_layout::stored_form`), which is what
    /// let them retire. `toRealPath` was blocked through wave 13
    /// (`WindowsNativeDispatcher.GetFullPathName0`, a Win32 native this
    /// VM had not implemented) and wave 14 implemented it, retiring both
    /// `toRealPath` overloads for the non-symlink case.
    #[test]
    fn wave_fifteen_windowspath_is_twentyone_rows() {
        assert_eq!(RETIRED_SHADOW_L4_WINDOWSPATH_TRIPLES.len(), 21);
        for (c, _, _) in RETIRED_SHADOW_L4_WINDOWSPATH_TRIPLES {
            assert_eq!(*c, "sun/nio/fs/WindowsPath");
        }
        for (m, d) in [
            ("compareTo", "(Ljava/nio/file/Path;)I"),
            ("endsWith", "(Ljava/nio/file/Path;)Z"),
            ...
            assert!(
                triple_is_retired_shadow("sun/nio/fs/WindowsPath", m, d),
                "sun/nio/fs/WindowsPath.{m}{d} is in the wave-15 WindowsPath table and the predicate cannot see it"
            );
        }
    }

    #[test]
    fn wave_seventeen_winntfs_is_eleven_rows() {
        assert_eq!(RETIRED_SHADOW_L4_WINNTFS_TRIPLES.len(), 11);
        for (c, m, d) in RETIRED_SHADOW_L4_WINNTFS_TRIPLES {
            assert_eq!(*c, "java/io/WinNTFileSystem");
            assert!(triple_is_retired_shadow(c, m, d), "{c}.{m}{d}");
            assert!(
                !m.ends_with('0'),
                "{m}: the *0 JNI natives are the surface the retired wrappers call and must stay"
            );
        }
        assert!(!triple_is_retired_shadow(
            "java/io/WinNTFileSystem",
        ...
        // Wave 17 asserted the Unix twin was NOT retired, because it was a
        // Windows-only measurement and said so. Wave 18 took that measurement
        // on a Linux host (`RETIRED_SHADOW_L4_UNIXFS_TRIPLES`), so the
        // assertion moves rather than being deleted: the row is retired, and
        // the `*0` native it calls still is not.
        assert!(triple_is_retired_shadow(
            "java/io/UnixFileSystem",
            "list",
            "(Ljava/io/File;)[Ljava/lang/String;"
    ...
    /// Wave 19: the ledger's remainder. Sorted, unique, every row retired, and
    /// the rows a named blocker covers stay OUT -- the `session`/`checkSession`
    /// pair the JOTP wave refuses most of all, since this table touches nine of
    /// the ten classes that carry it.
    #[test]
    fn wave_nineteen_is_the_ledger_remainder() {
        assert_eq!(RETIRED_SHADOW_L4_W19_TRIPLES.len(), 61);
        for (c, m, d) in RETIRED_SHADOW_L4_W19_TRIPLES {
            assert!(triple_is_retired_shadow(c, m, d), "{c}.{m}{d}");
            assert!(
                *m != "session" && *m != "checkSession",
                "{c}.{m}: the JOTP wave refuses the buffer session pair"
            );
        }
        for c in [
            "java/nio/ByteBuffer",
            ...
            assert!(!triple_is_retired_shadow(c, "checkSession", "()V"));
        }
        // WITHDRAWN BY ITS OWN MEASUREMENT, and the withdrawal is the point of
        // the arm: `Files.readSymbolicLink` was in the first cut of this table
        // and `L4W5Sweep` came back with one new diff line --
        // `sym.readTarget = p-src.txt` on HotSpot against an EMPTY string here.
        // The retired body hands the JDK's own `UnixFileSystemProvider` the
        // `readlink0` bytes, and the `UnixPath` it builds from them is the
        // `byte[]`-backed carrier this VM does not populate
        // (`unixpath-carrier`). MEASURED 2026-09-20 with
        // `CRATONVM_UNRETIRE_NATIVE_SHADOW=java/nio/file/Files.readSymbolicLink`:
        // the line comes back.
        assert!(!triple_is_retired_shadow(
            "java/nio/file/Files",
            "readSymbolicLink",
        ...
        // The VM's own direct-memory accounting, one class along from wave
        // 17's `Bits.reserveMemory` finding.
        for (c, m, d) in [
            ("java/nio/Bits$1", "getCount", "()J"),
            ("java/nio/Bits$1", "getMemoryUsed", "()J"),
            ("java/nio/Buffer$2", "reserveMemory", "(JJ)V"),
            ("java/nio/Buffer$2", "unreserveMemory", "(JJ)V"),
        ] {
            assert!(!triple_is_retired_shadow(c, m, d), "{c}.{m}{d}");
        }
    }

    /// Wave 18: the `java.io.UnixFileSystem` wrappers, the Linux twin of the
    /// test above. Same three invariants: eleven rows, all on that class, and
    /// no `*0` JNI native among them.
    #[test]
    fn wave_eighteen_unixfs_is_eleven_rows() {
        assert_eq!(RETIRED_SHADOW_L4_UNIXFS_TRIPLES.len(), 11);
        for (c, m, d) in RETIRED_SHADOW_L4_UNIXFS_TRIPLES {
            assert_eq!(*c, "java/io/UnixFileSystem");
            assert!(triple_is_retired_shadow(c, m, d), "{c}.{m}{d}");
            assert!(
                !m.ends_with('0'),
                "{m}: the *0 JNI natives are the surface the retired wrappers call and must stay"
        ...
        // The constructor writes the VM-minted carrier's fields; wave 17 kept
        // the Windows one native for the same reason.
        assert!(!triple_is_retired_shadow(
            "java/io/UnixFileSystem",
            "<init>",
            "()V"
    ...
    #[test]
    fn wave_seventeen_files_and_provider_rows() {
        let n = |c: &str| {
            RETIRED_SHADOW_L4_FILES2_TRIPLES
                .iter()
                .filter(|(cc, _, _)| *cc == c)
        ...
        // 57 -> 55 (2026-09-22): `newFileSystem(URI, Map)` and
        // `readAttributes(Path,String,LinkOption[])` held out — see the
        // table's own doc comments on those rows.
        assert_eq!(RETIRED_SHADOW_L4_FILES2_TRIPLES.len(), 55);
        assert_eq!(n("java/nio/file/Files"), 32);
        assert_eq!(n("java/nio/file/FileVisitResult"), 2);
        assert_eq!(n("java/nio/file/spi/FileSystemProvider"), 2);
        assert_eq!(n("sun/nio/fs/WindowsFileStore"), 10);
        assert_eq!(n("sun/nio/fs/WindowsFileSystemProvider"), 7);
        // The four provider mutators travel with `Files`: alone, `Files`
        // recurses through their delegating natives (see the table's doc).
        for m in ["copy", "createDirectory", "delete", "move"] {
            assert!(
                RETIRED_SHADOW_L4_FILES2_TRIPLES
                    .iter()
                    .any(|(c, mm, _)| *c == "sun/nio/fs/WindowsFileSystemProvider" && *mm == m),
                "{m}: retiring `Files` without it is an infinite native recursion"
            );
        }
        for (c, m, d) in [
            (
            ...
            assert!(
                !triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} has a named blocker (see the table's doc comment)"
            );
        }
    }

    #[test]
    fn wave_seventeen_buffers_are_sixty_three_rows_over_nineteen_classes() {
        // Was 65 rows / 20 classes at wave 17. `sun/nio/ch/Util`'s two rows
        // (`getTemporaryDirectBuffer`, `offerFirstTemporaryDirectBuffer`) were
        // removed 2026-09-22 — see the table's doc comment just above this
        // slice's definition for why (H2 concurrency bug the wave's
        // single-shot probes could not have caught).
        assert_eq!(RETIRED_SHADOW_L4_BUFFERS2_TRIPLES.len(), 63);
        let classes: std::collections::BTreeSet<&str> = RETIRED_SHADOW_L4_BUFFERS2_TRIPLES
            .iter()
            .map(|(c, _, _)| *c)
            .collect();
        assert_eq!(classes.len(), 19);
        // The funnel-listed rows a wave-5 verdict keeps native stay out.
        for (c, m, d) in [
            ("java/io/ByteArrayInputStream", "available", "()I"),
            ("java/io/ByteArrayInputStream", "reset", "()V"),
            ("java/io/ByteArrayInputStream", "skip", "(J)J"),
        ] {
            assert!(!RETIRED_SHADOW_L4_BUFFERS2_TRIPLES.contains(&(c, m, d)));
        }
    }

    #[test]
    fn wave_seventeen_file_temp_is_one_row_and_delete_on_exit_stays() {
        assert_eq!(RETIRED_SHADOW_L4_FILETMP_TRIPLES.len(), 2);
        for (c, m, d) in RETIRED_SHADOW_L4_FILETMP_TRIPLES {
            assert!(triple_is_retired_shadow(c, m, d), "{c}.{m}{d}");
        }
        // Wave 17 held this row out and named the blocker: the VM ran none of
        // `java.lang.Shutdown`'s hook slots, so the retired body's
        // `DeleteOnExitHook` registration went nowhere.
        // `lang_system::run_jdk_shutdown_slots` closed that in wave 18 and the
        // row is in the table above; the assertion moves with the measurement.
        assert!(triple_is_retired_shadow(
            "java/io/File",
            "deleteOnExit",
            "()V"
    ...
    /// Wave 16's rows (17 as of 2026-09-22; `getScheme`, `newByteChannel`,
    /// `newDirectoryStream`, `readAttributes(Class)` and the two
    /// `WindowsDirectoryStream` rows held out that day — see the table's own
    /// doc comments): every one is in the table, none is a duplicate of an
    /// earlier lane-4 table's row, and the rows the table's doc comment names
    /// as held or blocked stay out.
    #[test]
    fn wave_sixteen_winfs_is_seventeen_rows() {
        assert_eq!(RETIRED_SHADOW_L4_WINFS_TRIPLES.len(), 17);
        for (c, m, d) in RETIRED_SHADOW_L4_WINFS_TRIPLES {
            assert!(c.starts_with("sun/nio/fs/Windows"), "{c}");
            assert!(
                !RETIRED_SHADOW_L4_WINDOWSPATH_TRIPLES.contains(&(*c, *m, *d)),
                "{c}.{m}{d} is already in the wave-15 WindowsPath table"
            );
        }
        for (c, m, d) in [
            (
            ...
            ("sun/nio/fs/WindowsDirectoryStream", "close", "()V"),
            (
                "sun/nio/fs/WindowsDirectoryStream",
                "iterator",
                "()Ljava/util/Iterator;",
    ...
    fn the_zone_info_file_prefix_retires_only_the_two_measured_rows() {
        ...
        // NARROWED AGAIN 2026-09-12, L1 wave 9, and by a MEASUREMENT rather
        // than an argument, which is the only way this list has ever been
        // allowed to shrink. `java/util/Date` came off it: wave 6's `+0` at
        // reached=14 was a candidate and not a verdict, exactly as the note
        // above says, and wave 9 spent the trial binary that turns one into
        // the other. `RETIRED_SHADOW_L1_DATE_TRIPLES` retires its 19 rows and
        // nine more on `sun/util/calendar/ZoneInfo`.
        //
        // The three that remain are the ones with a measurement AGAINST them
        // or none at all, and they are the reason this guard is not simply
        // deleted: `LocaleResources` +12 on 254 engagements and
        // `CalendarDataUtility` +12 on 108 are measured NO, and
        // `sun/util/resources/Bundles` is the one class a workload built to
        // reach it still read at `reached == 0`.
        for c in [
    ...
    fn the_l1_hashmap_table_is_the_hashmap_family_and_only_it() {
        ...
        // AMENDED AGAIN 2026-09-12, L1 wave 9, and this time the assertion is
        // INVERTED rather than narrowed. Wave 7's refusal was correct about
        // what it measured and wrong about what the blocker was: it read
        // `NullPointerException: "this.this$0" is null` off
        // `LinkedHashMap$LinkedHashIterator.nextNode` and attributed it to
        // lane T's `register_hashset_natives`, because that registrar is what
        // MINTS the carrier for a `LinkedHashSet`. The carrier was the
        // problem and the registrar was not: `alloc_key_itr` allocates the
        // real class and this crate wrote only its own five appended fields,
        // leaving `this$0`, `next` and `expectedModCount` at zero.
        // `seed_real_map_itr_fields` writes them, and nothing in
        // `SET_CLASSES` moved. See `RETIRED_SHADOW_L1_LHM_TRIPLES`.
        for (c, m, d) in [
            (
                "java/util/LinkedHashMap",
                "put",
            ...
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is NOT retired. Wave 9 retired the whole \
                 `LinkedHashMap` family after seeding the iterator carrier's \
                 own declared fields; if this row came back out, say which \
                 measurement took it out."
            );
        }
        for (c, m, d) in [
            (
    ...
    fn wave_seven_refused_the_linked_hash_map_iterator_half() {
        // AMENDED 2026-09-12, L1 wave 9 -- INVERTED, not narrowed. Wave 7's
        // refusal was a correct measurement of a carrier with real class
        // identity and none of its own state: `alloc_key_itr` allocated
        // `LinkedHashMap$LinkedKeyIterator` etc. under their real names but
        // this crate wrote only its own five appended fields, leaving
        // `this$0`/`next`/`expectedModCount` at allocation defaults. Once a
        // retirement made real bytecode drive that object, `nextNode`
        // dereferenced a null `this$0` -- not because the family couldn't be
        // retired, but because the carrier was half-built.
        // `seed_real_map_itr_fields` (native-collections/src/lib.rs) writes
        // those fields at the mint site out of the JDK's own constructors,
        // and `SET_CLASSES` (lane T's registrar, the thing wave 7 named as
        // the blocker) is untouched.
        // `pollFirstEntry` dropped from this list: it is a JDK-21+
        // `SequencedMap` DEFAULT method (always bytecode, no native
        // registration), and jdk-17.0.20.1+1 does not even declare it -- the
        // census's own "REFUSED-SOMEWHERE" list has it. It was never a row a
        // retirement table could hold.
        for (c, m, d) in [
            (
                "java/util/LinkedHashMap",
                "put",
    ...
    fn wave_three_refused_the_iterators_the_views_and_lhm_s_inherited_eight() {
        for (c, m, d) in [
            ...
            // AMENDED 2026-09-12, L1 wave 9. Wave 3's refusal and wave 7's
            // re-refusal were both measured, and both were measurements of a
            // MIXED retirement: a family whose iterators and views stayed
            // native while some of its methods went to bytecode. That is the
            // state neither store survives -- `lhm_set`'s mirror is one-way,
            // so a retired inherited mutator writes where the surviving
            // natives never read. Wave 9 retires the family WHOLE, which
            // removes the reader that made the mirror's direction matter, and
            // seeds the iterator carrier so a real body can drive it.
            //
            // So these rows are retired now and this loop asserts it. The
            // sentence it replaces stays worth having: "wave 3 measured that
            // retiring it makes the probe tree WORSE" was true of retiring it
            // ALONE, and a subset of a family is not a small version of the
            // family.
            assert!(
                triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is not retired. Wave 9 took the HashMap and \
                 LinkedHashMap families whole; a row coming back out needs \
                 the measurement that took it."
            );
        }
    }

    /// L1 wave 9 retired the two map families and the `Date`/`ZoneInfo` pair,
    /// and this records the SHAPE rather than only the counts.
    ///
    /// The map table has to be the whole family or none of it, which is the
    /// one thing three waves of measurement agree on: wave 3 refused a subset,
    /// wave 7 refused two different subsets, and each refusal was a MIXED
    /// retirement rather than a verdict on the rows. So the assertions below
    /// name a member of every part -- map, view carrier, iterator carrier and
    /// node class -- and the eleven rows that stay out for the one reason that
    /// is not a measurement at all: jdk-17.0.20.1+1 does not declare them.
    #[test]
    fn wave_nine_retired_the_map_families_whole() {
        assert_eq!(
            RETIRED_SHADOW_L1_LHM_TRIPLES.len(),
            168,
            "the wave-9 map table changed size; re-take the three-image census"
        );
        assert_eq!(
            RETIRED_SHADOW_L1_DATE_TRIPLES.len(),
            19,
            "the wave-9 Date table changed size; re-take the census. (Was 28 \
             including nine `sun/util/calendar/ZoneInfo` rows, withdrawn \
             2026-09-17 -- see the table's doc comment.)"
        );
        for (c, m, d) in [
            (
                "java/util/LinkedHashMap",
            ...
            assert!(
                triple_is_retired_shadow(c, m, d),
                "wave 9 should have retired {c}.{m}{d}"
            );
        }
        // 2026-09-17 withdrawal: the nine `sun/util/calendar/ZoneInfo` rows
        // are NOT retired any more. Retiring them silently reintroduced
        // G17-1/G28-1's DST defect -- real JDK bytecode running against
        // `alloc_synth_timezone`'s fabricated `ZoneInfo` (null `transitions`)
        // -- MEASURED 100% reproducible on a fresh --jdk-only build:
        // `RSimpleDateFormatZone` throws `getOffset(JUL) must be -14400000
        // (DST)` and `RSimpleTimeZoneRaw` reads `checks=393 fails=93`. See
        // this table's own doc comment.
        for (c, m, d) in [
            ("sun/util/calendar/ZoneInfo", "getDSTSavings", "()I"),
            ("sun/util/calendar/ZoneInfo", "getOffset", "(IIIIII)I"),
            ("sun/util/calendar/ZoneInfo", "getOffset", "(J)I"),
            ("sun/util/calendar/ZoneInfo", "getOffsets", "(J[I)I"),
            ("sun/util/calendar/ZoneInfo", "getOffsetsByWall", "(J[I)I"),
            ("sun/util/calendar/ZoneInfo", "getRawOffset", "()I"),
            (
                "sun/util/calendar/ZoneInfo",
                "inDaylightTime",
                "(Ljava/util/Date;)Z",
            ...
            assert!(
                !triple_is_retired_shadow(c, m, d),
                "{c}.{m}{d} is retired again -- this reintroduces G17-1/G28-1's \
                 DST defect under --jdk-only. See the withdrawal note on \
                 RETIRED_SHADOW_L1_DATE_TRIPLES's doc comment."
            );
        }
        // The JDK 21 `SequencedMap` surface. Not a judgement about the rows:
        // jdk-17.0.20.1+1 declares none of them, and "declared by no supported
        // image" is a claim about THREE images.
        for (c, m, d) in [
            (
                "java/util/LinkedHashMap",
                "putFirst",
            ...
            assert!(
                !RETIRED_SHADOW_L1_LHM_TRIPLES.contains(&(c, m, d)),
                "{c}.{m}{d} is not declared by jdk-17.0.20.1+1 and must stay \
                 out of the wave-9 table"
            );
        }
        // `java/util/TimeZone` was censused with the Date family -- ten rows,
        // bucket A or B on all three images -- and deliberately left out. It
        // is the one member with a measurement against it that a dial artefact
        // does not explain, and `the_l1_families_measured_load_bearing_are_
        // still_held` is where that measurement lives.
        assert!(
            !triple_is_retired_shadow("java/util/TimeZone", "getID", "()Ljava/lang/String;"),
            "java/util/TimeZone stays out of wave 9: measured load-bearing by \
             wave 1 and +2 WORSE on LocaleDateTzShadowSweep. It wants its own \
             trial binary, not a seat in this table."
        );
    }
    ...
    fn the_l2_table_holds_only_what_lane_2_measured() {
        ...
        // the dial cannot promote. These rows need the two-binary score.
        //
        // `StringBuilder` — the third original blocker — is no longer one of
        // them. It stayed here through the JIT-intrinsic lane (2026-09-17,
        // full `SUITE=all`/`SUITE=core` clean) and lane 2's own text kept
        // calling that "not a retirement score" because both were single-binary
        // dial arms. 2026-09-21 supplied the two-binary score this comment
        // asked for: `RETIRED_SHADOW_STRINGBUILDER_TRIPLES` (118 rows,
        // including these exact two) landed, a binary was built WITH the
        // table, and the same differential probe that targets the methods
        // `lang_string.rs`'s stale comments still warn about
        // (`codePointAt`/`getChars`/`insert`/`repeat`) came back byte-identical
        // to HotSpot 25.0.4+7 on that binary — see the new table's own doc
        // comment for the numbers. Moved out of this "not retired" list into
        // `the_stringbuilder_table_is_reachable_and_sized` below.
        for (c, m, d) in [
    ...
    /// The StringBuilder table: sorted, unique, reachable through the real
    /// predicate (not asked directly), and sized to what
    /// `RETIRED_SHADOW_STRINGBUILDER_TRIPLES`'s own doc comment claims — 118
    /// rows, none of them the six excluded receivers.
    #[test]
    fn the_stringbuilder_table_is_reachable_and_sized() {
        for w in RETIRED_SHADOW_STRINGBUILDER_TRIPLES.windows(2) {
            assert!(
                w[0] < w[1],
        ...
        assert_eq!(
            RETIRED_SHADOW_STRINGBUILDER_TRIPLES.len(),
            118,
            "the table's own doc comment claims 118 of 124 registered triples; \
             a row added or removed here needs a fresh --dump-native-registry \
             pass, not a hand edit."
        );
        for (c, m, d) in RETIRED_SHADOW_STRINGBUILDER_TRIPLES {
            assert!(
                triple_is_retired_shadow(c, m, d),
                "unreachable entry: {c}.{m}{d}"
            );
        }
        // The six excluded rows: one truly abstract (no real body to yield
        // to), five registered on `StringBuilder` for a method it does not
        // itself declare (only `AbstractStringBuilder` does, and that class's
        // own copy IS in the table above).
        for (c, m, d) in [
            ...
            ("java/lang/StringBuilder", "appendNull", "()Ljava/lang/AbstractStringBuilder;"),
            ("java/lang/StringBuilder", "getCoder", "()B"),
            ("java/lang/StringBuilder", "getValue", "()[B"),
            ("java/lang/StringBuilder", "repeat", "(CI)Ljava/lang/AbstractStringBuilder;"),
        ] {
            assert!(
                !RETIRED_SHADOW_STRINGBUILDER_TRIPLES.binary_search(&(c, m, d)).is_ok(),
                "{c}.{m}{d} should not be in the StringBuilder table: see the \
                 table's own doc comment for why each of the six is excluded."
            );
        }
    }
    ...
    ///
    /// `docs/contributing/jdk-only-lane-operations.md` §2b (until 2026-09-12,
    /// `docs/internal/jdk-only/lane-0-integration-and-gates-RETIRED-20260916.md` §4)
    ...
    ///     **SUPERSEDED for `*Unaligned` on 2026-09-19** -- see
    ///     `RETIRED_SHADOW_JOTP_UNALIGNED_TRIPLES`. Those accessors are handed array and
    ///     off-heap offsets, which are byte offsets here. The sub-word atomics are unchanged;
    ...
    /// cross-cutting and has its own page (fixed 2026-09-17),
    /// `core-reflection-has-no-member-filter-20260911.md`.
    ...
    fn the_l5_prefixes_retire_nothing_on_their_own() {
        for (c, m, d) in [
            ...
            // The unaligned family was declined here as "a slot index has no bytes to
            // address". It was RETIRED on 2026-09-19 by `RETIRED_SHADOW_JOTP_UNALIGNED_TRIPLES`,
            // whose doc comment says why the reason did not reach it: those accessors are handed
            // array and off-heap offsets, never `objectFieldOffset`'s slot index. The rows were
            // removed from this list rather than the list being left to fail, and
            // `the_jotp_unaligned_wave_is_the_sixteen_array_and_address_accessors` is the
            // replacement guard.
        ] {
            assert!(
                !triple_is_retired_shadow(c, m, d),
    ...
    fn the_phase2_wave_retires_only_the_triple_it_measured() {
        ...
        // `open` was NOT retired by that wave, and the doc comment above records
        // why: it was inert, because nothing in the probe or the corpus reached
        // it (`invocations: 0`). Wave 17 retires it in
        // `RETIRED_SHADOW_L4_BUFFERS2_TRIPLES`, after retiring `Files` and
        // `WindowsFileSystemProvider.newByteChannel` made the real
        // `WindowsChannelFactory` call it (`invocations: 6` in the probe census),
        // so the premise that made it inert is gone.
        assert!(triple_is_retired_shadow(
            "sun/nio/ch/FileChannelImpl",
            "open",
...
Ljava/nio/channels/FileChannel;"
    ...
    fn a_prefix_alone_retires_nothing() {
        ...
        // (Wave 16 retired `WindowsPath.toString`, which used to be this arm's
        // example of a method the table does not hold.)
        assert!(!triple_is_retired_shadow(
            "sun/nio/fs/WindowsPath",
            "notAMethod",
            "()V"
    ...
    fn the_l3_table_is_sorted_and_unique() {
        // 24 from wave 1 + 9 from wave 3 (2026-09-12, the default-value
        // deferrals wave 1 pinned) - 1 un-retired 2026-09-18
        // (`Method.getAnnotatedReturnType`, see the row's comment). Asserted for
        // the reason L0's sibling asserts 19: the lane page states a count, and a
        // count in prose and a count in a table drift apart silently. §8e carries
        // this one.
        assert_eq!(
            RETIRED_SHADOW_L3_TRIPLES.len(),
            32,
            "wave 1 landed 24 and wave 3 nine more, minus the 2026-09-18 un-retirement; lane 3's 8d/8e carry the count"
        );
    ...
    fn the_l3_held_triples_are_not_retired() {
        for (c, m, d) in [
            ...
            //
            // Wave 1 deferred TWELVE triples here and pinned all twelve. Wave 3
            // (2026-09-11) discharged NINE of them and they are deleted from
            // this list rather than annotated -- the reverse direction of the
            // ratchet, on `harness-uncounted.txt`'s model: a row that has been
            // measured is an error to keep pinning, so clearing the entry is
            // what makes the run green.
            //
            // The nine cleared, and what discharged them: each has a row whose
            // ORACLE is not the default and on which this VM agrees, read off
            // wave 2's own arms in the lane page's 8b. `true` is not what a
            // blanket `()Z` yield returns, so the deferral reason -- "agrees
            // only where agreement is indistinguishable from the default" --
            // is answered. Three of the nine needed 8b's own verdict corrected
            // first: it had rejected them on `any attributed row still has the
            // default as its oracle`, where the rule is `ALL of them do`.
            //
            // The THREE that stay pinned, each for a different reason:
            //
            //   Field.setBoolean        reads back through Field.get, which is
            //                           HELD -- a disagreement could be the
            //                           reader's fault, not this row's.
            //   Field.trySetAccessible  a real BAD, not a default-bound
            //                           unknown: answers `true` where HotSpot
            //                           answers `false` for a JDK-internal
            //                           field. Priced in 8b; the remedy is a
            //                           check this VM does not perform, which
            //                           is the opposite of yielding.
            //   Constructor.setAccessible
            //                           differs only in the throwing FRAME and
            //                           its reader (Field.canAccess) is HELD.
            (
                "java/lang/reflect/Field",
                "setBoolean",
    ...
    fn the_held_collection_families_are_not_retired() {
        for (c, m, d) in [
            ...
            // 2026-09-12, L1 wave 7: `java/util/Hashtable` came OFF this
            // list, all 79 triples, and so did `java/util/LinkedHashMap`'s
            // 102. The `needs-VM-support` label was true when it was written
            // and is not now. Both families' state is REAL -- measured
            // unarmed through `--add-opens` against HotSpot 25.0.4+7:
            // `Hashtable.table` is a `[Ljava.util.Hashtable$Entry;` of real
            // `Hashtable$Entry` nodes with `count` written, and
            // `LinkedHashMap`'s `table`/`head`/`tail`/`size` are the real
            // fields. What was missing was a DECODE: `hs_backing_map`
            // answered `None` for a view the image's own bytecode minted,
            // because the source map is in `this$0` and this VM's carriers
            // keep it in a trailing slot the real class does not have. See
            // `RETIRED_SHADOW_L1_HT_TRIPLES`.
    ...
    fn the_phase3_wave_is_exactly_two_class_families() {
        ...
        // AMENDED 2026-09-12, L1 wave 7. `java/util/Hashtable.get` IS retired
        // now -- by `RETIRED_SHADOW_L1_HT_TRIPLES`, on its own trial binary --
        // so the question this assertion asks has to be the one it meant:
        // Hashtable must not ride in on PHASE 3's table, whose measurement was
        // the CHM/`Properties` union and said nothing about it.
        assert!(
            !RETIRED_SHADOW_PHASE3_TRIPLES.contains(&(
                "java/util/Hashtable",
                "get",
                "(Ljava/lang/Object;)Ljava/lang/Object;"
            )),
            "Hashtable rode in on the Phase 3 table. Wave 7 retired the family \
             on its own measurement; this table's is the CHM/Properties union."
        );
    }
    ...
    fn the_l1_families_measured_load_bearing_are_still_held() {
        for (c, m, d) in [
            ...
            // trial binaries -- see
            // `java/util/LinkedHashMap.put` came OFF this list 2026-09-12,
            // L1 wave 9: the whole family is retired now
            // (`RETIRED_SHADOW_L1_LHM_TRIPLES`), including the iterator half
            // `wave_seven_refused_the_linked_hash_map_iterator_half` used to
            // name as the reason `put` had to stay held. See that test.
            //
            ...
            // `java/util/HashMap$KeySet.iterator` came OFF this list
            // 2026-09-12, L1 wave 9, same table and same reason as
            // `LinkedHashMap.put` above.
            //
            // "4 probes worse" -- OFF this list 2026-09-12, L1 wave 7, with
            // the LinkedHashMap row above. Seven probes and 30 rows moved on
            // the wave-7 control, every one of them a `toArray()` over a view
            // the image's own bytecode minted, and every one of them 0 on the
            // trial. See `RETIRED_SHADOW_L1_HT_TRIPLES`.
            // 5 probes worse.
