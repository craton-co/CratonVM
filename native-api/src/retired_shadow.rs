...
pub(crate) const RETIRED_SHADOW_TABLES: &[&[(&str, &str, &str)]] = &[
    ...
    // Lane 1's waves 3 and 4, added here by lane 6's merge and NOT by lane 1 —
    // the third and fourth occurrence of this in two days. Both are consulted
    // by `triple_is_retired_shadow` and neither was listed, so
    // `the_tables_const_lists_every_table_the_predicate_consults` was RED on
    // `dev` when this branch merged it. The guard is doing its job; what it
    // keeps proving is that the job should not exist. A predicate arm and a
    // const row are the same decision written in two places, and the next
    // wave will forget one of them too unless the two are made one.
