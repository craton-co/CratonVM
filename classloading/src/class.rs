...
mod tests {
    ...
    /// Nothing in the type system stops a caller from writing
    /// `class.interfaces = ...` through a `get_mut` and leaving the
    /// `implementors` index stale — `Class::interfaces` is `pub`, and six
    /// production sites did exactly that until the CHA lane routed them through
    /// [`ClassStore::set_interfaces`].
    ///
    /// A stale index is SILENT. It does not crash and it does not produce a
    /// wrong answer at the point of the mistake: `unique_implementor` simply
    /// stops seeing a subtype, so class-hierarchy analysis quietly binds
    /// nothing for that interface and every measurement says the feature does
    /// not pay. That is exactly the shape of defect a source scan is for, and
    /// it is why the scan exists rather than a doc comment asking nicely.
    ///
    /// Scoped to this crate's own sources. `interfaces:` in a struct literal is
    /// construction, not mutation — `add` mirrors those — and `spec.interfaces`
    /// in `proxy_gen.rs` is a class *specification* that has not been stored
    /// yet, so neither is an offender.
    #[test]
    fn no_raw_write_to_a_stored_class_interfaces_bypasses_set_interfaces() {
        // Scan PRODUCTION code only: everything before each file's own
        // `#[cfg(test)]` module. Not a nicety — the first version of this test
        // failed on its own scanner line, because `t.contains(".interfaces =")`
        // contains `.interfaces =`. `flag_declaration_guard.rs` carries the
        // same warning about a scan that matches itself; this is that hazard in
        // a new shape, and splitting at the module boundary removes the whole
        // class of it rather than special-casing one line.
        const SOURCES: &[(&str, &str, &str)] = &[
            (
                "classloading/src/class.rs",
                include_str!("class.rs"),
                // A production item near the end of the scanned region, so an
                // empty or truncated scan cannot pass vacuously.
                "pub fn set_interfaces",
            ),
            (
                "classloading/src/class_manager.rs",
                include_str!("class_manager.rs"),
                "pub fn set_interfaces",
            ),
        ];
        let mut offenders: Vec<String> = Vec::new();
        for (name, src, witness) in SOURCES {
            let production = src
                .split(
                    "
#[cfg(test)]
mod tests {",
                ...
                .unwrap_or(src);
            assert!(
                production.contains(witness),
                "{name}: the scanned region no longer contains `{witness}` — the                  test-module boundary moved and this scan would pass vacuously"
            );
            for (n, line) in production.lines().enumerate() {
                ...
                // A field ASSIGNMENT: `.interfaces =` / `.interfaces=`. Never
                // `interfaces: ` (a struct literal, which `add` mirrors) and
                // never `.interfaces.` or `.interfaces;` (reads).
                if !(t.contains(".interfaces =") || t.contains(".interfaces=")) {
                    continue;
                }
                // The one legitimate writer is `set_interfaces` itself, which
                // updates the index in the same breath.
                if *name == "classloading/src/class.rs" && t.contains("class.interfaces =") {
                    continue;
                }
        ...
        assert!(
            offenders.is_empty(),
            "a raw write to a stored `Class::interfaces` bypasses              `ClassStore::set_interfaces`, so the `implementors` index goes              stale and class-hierarchy analysis silently stops seeing that              subtype:
  {}",
            offenders.join("
  ")
        );
