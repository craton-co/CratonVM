...
//! # v3 deferred work
//!
//! - **Static `Method[]` slots (defer-3).** Today the helper native
//!   `Proxy$Dispatch.invokeProxy` synthesizes a minimal
//!   `java.lang.reflect.Method` object on every invocation, populating
//!   only `name` / `signature` / `modifiers`. JDK ProxyGenerator emits a
//!   `static Method m_<i>` field per declared method, populated in
//!   `<clinit>` via `Class.forName(iface).getMethod(name, paramTypes)`,
//!   then GETSTATICs the slot in each method body and pushes that real
//!   `Method` to the helper. To land this:
//!   1. Extend `ProxyClassSpec` with `param_class_names: Vec<String>`
//!      per `ProxyMethod` (already implicit in the descriptor — derive
//!      via `descriptor_param_slots`).
//!   2. Emit a `fields[]` entry per method (`m_<i>` of type
//!      `Ljava/lang/reflect/Method;`).
//!   3. Emit a `<clinit>` that, for each method, loads the iface class
//!      mirror (LDC class CP entry), pushes the param-types Class[] via
//!      ANEWARRAY + AASTORE per param, INVOKEVIRTUALs
//!      `Class.getMethod(String, Class[])`, and PUTSTATICs the result.
//!   4. Replace each method body's `LDC name ; LDC desc` with
//!      `GETSTATIC m_<i>` and change the dispatch helper's signature to
//!      `(Object, Method, Object[]) Object`. Also update
//!      `register_reflect_proxy_natives` and `native_proxy_dispatch_invoke`
//!      to the new contract.
//!   Estimated ~150 LOC emitter delta + ~30 LOC native-side delta.
//!
//! - **Real StackMapTable (defer-4).** The bodies emitted today have
//!   no branches — they're a single linear basic block — so there are
//!   no merge points where a stackmap frame would be required.
//!   `same_frame` would suffice if any branch existed. Adding stackmaps
//!   would let us drop the `skip_verification: true` option and fully
//!   verify generated bytecode; not strictly necessary for correctness
//!   (the emitter is hand-controlled and trusted) but a defense in
//!   depth. To land: track abstract type state per local + stack across
//!   the body, emit one `same_frame` (or `same_locals_1_stack_item`)
//!   entry per basic-block boundary as `Code.attributes[StackMapTable]`.
//!
//! - **`UndeclaredThrowableException` wrapping (defer-6).** When
//!   `InvocationHandler.invoke` throws a checked exception not declared
//!   by the proxied method, JDK semantics wrap it in
//!   `java.lang.reflect.UndeclaredThrowableException`. Today
//!   `native_proxy_dispatch_invoke` and `proxy_invoke_handler_shared`
//!   propagate the exception verbatim. To land: at each
//!   `InvocationHandler.invoke` return-point, inspect the exception
//!   class against the method's `exceptionTypes` (which would need
//!   threading through from defer-3's static `Method[]`) and wrap
//!   non-`RuntimeException` / non-`Error` mismatches.
//!
