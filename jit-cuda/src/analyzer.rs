...
pub enum Reason {
    ...
    /// The method's return type is neither void nor a primitive
    /// scalar: a reference type, or — since 2026-09-21 — a primitive
    /// ARRAY.
    ///
    /// AUDIT 2026-09-21: the array half was found while closing
    /// [`Self::Allocation`], and is the same defect with the
    /// annotation removed. `ParamKind::from_field` maps `[I` to
    /// `I32Array` for a return type exactly as it does for a
    /// parameter, and `areturn` (0xB0) sits inside `classify`'s
    /// permitted `0xAC..=0xB1` band, so
    ///
    /// ```java
    /// static int[] doubleInPlace(int[] a) {
    ///     for (int i = 0; i < a.length; i++) { a[i] = a[i] * 2; }
    ///     return a;                       // no allocation at all
    /// }
    /// ```
    ///
    /// analyzed `Eligible` and then died in the emitter with "opcode
    /// 0xb0 not implemented in lowering". Unlike the `newarray` case
    /// this needs no `@GpuKernel` annotation and no hint — plain
    /// `Strict` admitted it — so any method with an array return type
    /// that happened to fit every other rule paid an analyze→lower
    /// round-trip and a blacklisting.
    ///
    /// Lowering it is the same cross-crate round `Allocation`
    /// describes, and stops at the same place: `build_param_list`
    /// emits `ret_ptr` + `ret_len` for an array return kind, and the
    /// VM marshaller pushes a return slot only for the four scalar
    /// `ParamKind`s. Returning a PARAMETER rather than a fresh array
    /// would need less than returning an allocation — the buffer is
    /// already on the device and already written back — but it still
    /// needs an `areturn` lowering and a `SerializedResult` that can
    /// name an existing argument. Neither exists.
    ///
    /// The out-parameter form is unaffected and is what every GPU
    /// fixture uses: take the output array as a parameter, return
    /// `void`. That shape needs none of the above, because the caller
    /// already holds the array.
    UnsupportedReturnType,
    /// `new` (0xBB), `newarray` (0xBC), `anewarray` (0xBD) or
    /// `multianewarray` (0xC5) — the kernel body allocates.
    ///
    /// AUDIT 2026-09-21: unconditional again. `newarray` of a
    /// primitive with a parameter-sourced size used to be admitted
    /// under [`AdmissionHint::AllowAllocation`], on the strength of a
    /// `prev_op` is-an-`iload` approximation, and the emitter has
    /// never had a dispatch arm for 0xBC — nor for the `areturn`
    /// (0xB0) that necessarily follows it, since the allocated array
    /// is the method's return value. Every `@GpuKernel(admit =
    /// ALLOW_ALLOCATION)` method was therefore analyzed-Eligible and
    /// then refused by `lowering::emit::Emitter::emit_op` with
    /// "opcode 0xbc not implemented in lowering", wasting an
    /// analyze→lower round-trip and blacklisting the method — the
    /// exact failure this module's contract with the emitter exists
    /// to prevent, and the `ldc` (C31) and `invokestatic` (intrinsic
    /// table) arms above were both written to close.
    ///
    /// Lowering it is not an emitter-local change. The allocated
    /// array is the kernel's *output buffer*, so the host has to
    /// allocate a device buffer sized from a runtime argument, copy it
    /// back, and surface it as a new Java array. `KernelSignature`
    /// and `lowering::build_param_list` already express half of that
    /// (an array `return_kind` emits `ret_ptr` + `ret_len` params),
    /// but the VM marshaller does not: `dispatch_method_from_native`
    /// pushes a return slot only for the four scalar `ParamKind`s and
    /// its array arms are explicitly "not yet wired", and
    /// `SerializedResult::PrimitiveArray*` is never populated on this
    /// path (see that enum's doc comment). Until those land, the only
    /// honest verdict is a reject, which sends the method to the CPU —
    /// same reasoning as [`Self::UnsupportedGridShape`] and
    /// [`Self::UnsupportedLaunchGeometry`].
    ///
    /// The `ALLOW_ALLOCATION` hint is still parsed and still sets
    /// [`AdmissionFlags::allocation`](crate::annotations::AdmissionFlags::allocation);
    /// nothing reads that bit today. Accepting the annotation keeps
    /// already-annotated sources compiling — see `docs/gpu/annotations.md`.
    Allocation,
...
/// - `AdmissionHint::AllowAllocation`  — **accepted but inert since
///   2026-09-21.** It used to admit `newarray` of a primitive
///   component whose size came from a method parameter. The emitter
///   has no lowering for `newarray` (or for the `areturn` of the
///   array it produces) and the VM marshaller cannot surface an array
///   return, so admitting it here only bought an analyze→lower
///   round-trip and a blacklisting. Allocation now rejects with
///   `Reason::Allocation` under every hint; see that variant's doc
///   comment for what lowering it would actually take.
...
fn analyze_with_annotations_and_pool_impl(
    ...
    // AUDIT 2026-09-21: an ARRAY return kind is a legal `ParamKind` —
    // `from_field` maps `[I` to `I32Array` just as it does for a
    // parameter — but it is not a shape anything downstream can carry.
    // See `Reason::UnsupportedReturnType`. This needs its own check
    // because `from_field` is the wrong place for it: the same mapping
    // is correct for parameters and wrong for returns, and the set of
    // array element types the pipeline admits is pinned against that
    // function by the VM's `analyzer_and_marshaller_admit_the_same_arrays`.
    if return_kind.is_array() {
        return OffloadVerdict::Rejected(Reason::UnsupportedReturnType);
    }
...
///
/// AUDIT 2026-09-21: this used to take a `prev_op: Option<u8>` — the
/// previous opcode in the linear bytecode stream — consulted by one
/// arm only, the `AllowAllocation` loosening, which approximated
/// "the `newarray` size comes from a method parameter" by checking
/// that the preceding instruction was an `iload`. That arm is gone
/// (see [`Reason::Allocation`]), and with it the parameter.
fn classify(op: u8, hint: AdmissionFlags) -> OpClass {
    match op {
        ...
        // Allocation: `new` (0xBB), `newarray` (0xBC), `anewarray`
        // (0xBD) and `multianewarray` (0xC5) all reject, and reject
        // unconditionally — `AdmissionHint::AllowAllocation` does NOT
        // loosen this band.
        //
        // AUDIT 2026-09-21: primitive `newarray` (0xBC) used to be
        // admitted when `hint.allocation` was set and the size came
        // from an `iload`. `lowering/emit.rs` has no arm for 0xBC (nor
        // for the `areturn` that follows it), so every such method was
        // admitted here and then refused one layer later — the precise
        // analyzer/emitter disagreement this file's contract forbids,
        // and the same shape as the `ldc` (C31) and `invokestatic`
        // arms above. Unlike those two, closing it in the emitter is
        // not possible alone: the allocated array is the kernel's
        // output buffer and the VM marshaller has no wiring for an
        // array return. See [`Reason::Allocation`] for the full
        // analysis and `docs/gpu/annotations.md` for what the hint
        // means to a user today.
...
mod tests {
    ...
    fn compare_opcodes_are_admitted_by_analyzer() {
        for op in 0x94..=0x98 {
            match classify(op, AdmissionFlags::STRICT) {
                OpClass::Ok => {}
                OpClass::Reject(r) => {
    ...
    /// AUDIT 2026-09-21: this used `RejectAllocation.build(I)[I`,
    /// which returns `[I` and is therefore now rejected on its return
    /// type before the body is scanned at all — it could no longer
    /// pin the allocation band. `RejectScratchAllocation.sumSquares`
    /// allocates the same `iload_0; newarray int` shape and returns a
    /// scalar, so allocation is the only thing left to reject it.
    /// The ordering itself is pinned by
    /// [`array_return_rejected_before_the_body_is_scanned`].
    #[test]
    fn reject_allocation() {
        let method = load_method("RejectScratchAllocation", "sumSquares", "(I)I");
        assert_eq!(
            analyze(&method),
    ...
    /// AUDIT 2026-09-21: a primitive ARRAY return kind is rejected, and
    /// rejected from the descriptor — before the bytecode scan, next to
    /// the parameter-kind check it belongs with.
    ///
    /// `RejectArrayReturn.doubleInPlace` is the shape that made this
    /// necessary: it allocates NOTHING (the output buffer is the
    /// parameter, handed straight back), so no hint and no annotation
    /// is involved, `Strict` admitted it, and `lowering/emit.rs` then
    /// refused the `areturn` with "opcode 0xb0 not implemented in
    /// lowering". See [`Reason::UnsupportedReturnType`].
    ///
    /// `RejectAllocation.build` pins the ORDER: it trips both rules —
    /// `newarray` in the body and `[I` in the descriptor — and the
    /// descriptor wins, because the return type is the more
    /// fundamental blocker and rewriting to avoid the allocation
    /// alone would not make it offloadable.
    #[test]
    fn array_return_rejected_before_the_body_is_scanned() {
        let no_allocation = load_method("RejectArrayReturn", "doubleInPlace", "([I)[I");
        assert_eq!(
            analyze(&no_allocation),
            OffloadVerdict::Rejected(Reason::UnsupportedReturnType),
            "an array return needs no allocation to be unlowerable"
        );

        let also_allocates = load_method("RejectAllocation", "build", "(I)[I");
        assert_eq!(
            analyze(&also_allocates),
            OffloadVerdict::Rejected(Reason::UnsupportedReturnType),
            "the descriptor check runs before the body scan"
        );
    }
    ...
    /// AUDIT 2026-09-21: `AllowAllocation` must NOT admit `newarray`.
    ///
    /// This test is the inverse of the one it replaces
    /// (`admit_allocation_loosens_primitive_newarray`), which pinned
    /// the loosening that produced the analyzer/emitter disagreement:
    /// `RejectScratchAllocation.sumSquares(I)I` opens with `iload_0;
    /// newarray int`, the canonical §2.4 pattern, and the emitter has
    /// no arm for 0xBC. Both hints must now reach the same verdict, so
    /// the method never gets lowered and never gets blacklisted. See
    /// [`Reason::Allocation`].
    #[test]
    fn allow_allocation_hint_does_not_admit_newarray() {
        let method = load_method("RejectScratchAllocation", "sumSquares", "(I)I");

        for hint in [AdmissionHint::Strict, AdmissionHint::AllowAllocation] {
            assert_eq!(
                analyze_with_annotations(&method, &annotate(hint)),
                OffloadVerdict::Rejected(Reason::Allocation),
                "{hint:?} must reject newarray as Allocation"
            );
        }
    }

    /// The `ALLOW_ALLOCATION` annotation surface still parses and
    /// still reaches the analyzer — it is *accepted and inert*, not
    /// removed, so already-annotated sources keep compiling. Pins the
    /// bit as set-but-unread: if a future round wires the lowering it
    /// has a flag to read, and until then nothing silently depends on
    /// it. See `docs/gpu/annotations.md`.
    #[test]
    fn allow_allocation_hint_is_still_parsed_into_flags() {
        let flags = AdmissionFlags::from(AdmissionHint::AllowAllocation);
        assert!(flags.allocation, "the hint must still set its bit");
        assert!(
            !flags.unguarded_integer_division && !flags.approximate_float_remainder,
            "AllowAllocation must not set any other loosening bit"
        );
    }
