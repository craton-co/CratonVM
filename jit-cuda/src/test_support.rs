...
pub fn load_method(class_name: &str, method_name: &str, descriptor: &str) -> ClassFileMethod {
    ...
    // The reader keeps method attributes lazy: `Code` arrives as a `Raw`
    // attribute that `ClassFileMethod::code()` reports as absent until
    // it is decoded. Force-decode here so the analyzer sees real
    // bytecode instead of failing early with `Rejected(NoCode)`.
    rustjvm_reader::attribute::force_decode_all(&mut method.attributes, &class.constant_pool)
        .unwrap_or_else(|e| {
            panic!(
                "failed to decode attributes of {method_name}{descriptor} in {}: {e:?}",
                path.display()
            )
        });
    method
