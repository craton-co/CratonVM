...
- **`--compatible` mode must stay byte-for-byte unchanged.** Strict mode is a
  separate policy, never inferred from a Cargo feature or an environment
  variable. Most of the dangerous mistakes recorded in `docs/jdk-only/` were
  `compatible`-mode behaviour changes made while intending to fix strict mode.
  Note the flags moved on 2026-09-20: `compatible` is no longer *the default*,
  it is what `--compatible` (deprecated alias `--real-jdk`) selects, and the
  `cratonvm` launcher now defaults to `jdk-only`. The invariant is unchanged —
  it was never about which mode was default, only that the permissive one must
  not drift. Every embedding entry point (`VmConfig::default`,
  `VmConfig::with_host_jdk_default`, `libcratonvm`, `cratonvm-embed`) still
  defaults to `Compatible`, deliberately; do not "align" them with the
  launcher.
