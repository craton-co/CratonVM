...
def image_verdict(image_declared, image_has_code, image_acc_native,
                   image_inherited_from):
    """`H14-1` Sec4's retirement verbs, from the four IMAGE columns already in
    each record -- so a future lane reads the verdict instead of re-deriving
    it from the raw booleans, one CSV column at a time, the way `H25-2` N1/N2
    had to.

    * declared is None    -> no registry entry matched this triple at all.
    * declared is False   -> DEAD (`H25-1`): no supported image declares the
      method anywhere in the receiver's hierarchy: a native standing in front
      of it can never be dispatched.
    * declared, inherited -> RELOCATE (`H14-1` Sec4's 156-row bucket): the
      registration is on the wrong class, not `H25-2` Sec3's exception to that
      rule (a deliberate override of an inherited method, e.g. `Package.equals`)
      -- this function cannot see intent, only image shape.
    * declared, has_code   -> RETIRE: real bytecode exists on the receiver
      itself; `H14-1` Sec4's 1244-row free-retirement bucket.
    * declared, acc_native -> BRIDGE: a legitimate Sec1.5 bridge to a real
      `ACC_NATIVE` method.
    * declared, neither    -> NO_IMPL (`H25-2`'s 1405-row population):
      abstract/interface, no `Code`, not `ACC_NATIVE` -- retiring this native
      removes the only implementation there is, UNLESS some concrete
      subclass supplies the method, which no static image scan can answer.
    """
    if image_declared is None:
        return "no-registry-entry"
    if not image_declared:
        return "dead"
    if image_inherited_from:
        return "relocate"
    if image_has_code:
        return "retire"
    if image_acc_native:
...
def main(argv):
    ...
    for key in wanted:
        ...
        rec["verdict"] = image_verdict(
            rec["image_declared"], rec["image_has_code"],
            rec["image_acc_native"], rec["image_inherited_from"],
        )
        recs.append(rec)
        if not site:
    ...
    table("by RETIREMENT VERDICT (dead/relocate/retire/bridge/no_impl)",
          lambda r: r["verdict"], 10)
    ...
    if args.csv:
        cols = ["class", "method", "descriptor", "outcome", "native_kind",
                ...
                "image_inherited_from", "vectors", "verdict"]
