...
STATUS 2026-09-21 (H14-1 N3): `--fn-indent` now defaults to 0 (true top-level
`fn`s only), matching `shadow-triage.py`. The old unconditional `<= 4` admitted
a `fn` nested INSIDE another function's body as if it were a registrar in its
own right -- `native-builtins/lib.rs::url_path_or_file_field` and
`::module_pkg_arg` are local helpers indented four columns inside
`register_essential_natives_with_shims`, and every registration between one
such helper and the next was misattributed to it, inflating it to the top
bucket. Pass `--fn-indent=4` to recover the old (artefact-prone) join for
diffing.
...
class FnIndex:
    ...
    def _load(self, path):
        ...
        for n, line in enumerate(lines, 1):
            ...
            if m and (len(line) - len(line.lstrip())) <= self.max_indent:
...
def main(argv):
    ...
    for a in argv:
        if a.startswith("--by="):
            ...
            root = a.split("=", 1)[1]
        elif a.startswith("--fn-indent="):
            fn_indent = int(a.split("=", 1)[1])
