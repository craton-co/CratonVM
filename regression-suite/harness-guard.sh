...
class_cv_args() {
  case "$1" in
    ...
    # RJitMapTierDiffNoJit is RJitMapTierDiff's own probe (`.run`, called
    # directly — see that file) re-run with the JIT disabled entirely, so
    # "Run it BOTH ways" (its header) is a scheduled control and not a manual
    # step. H10-1 N2.
    RJitMapTierDiffNoJit) printf '%s' "--nojit" ;;
