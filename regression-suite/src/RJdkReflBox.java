 ...
 *
 * <p>All 110 expectations come from {@code scratchpad/f19/ReflBoxOracle.java} run on Microsoft
 * OpenJDK 25.0.3+9 (build 25.0.3+9-LTS), except the seven {@code mhcoll.char}/{@code mhcoll.bool}/
 * {@code mhvar.char}/{@code mhcollc}/{@code mhcolll} rows, which come from
 * {@code scratchpad/f29/CollBox.java}/{@code CollBox2.java} §G and were re-measured in place by
 * lane F39 by running this whole class on the same oracle. The transcripts are §2 of lane F19's
 * record (retired to {@code docs/internal/retired/} once its findings landed and were measured)
 * and §1/§2 of
 * {@code docs/known-issues/jdk-only/F29-1-the-wrapper-class-comes-from-the-call-site-not-the-methodtype-20260813.md}.
 * {@code mhcoll.char}/{@code mhcoll.bool}/{@code mhvar.char} were added when NOMINATION 2 landed
 * (the call-site descriptor now reaches {@code MH_KIND_COLLECT}); before that they were the
 * measured-red rows F29-1 NOMINATION 4b named and held back.
...
public class RJdkReflBox {
    ...
    //   * an `Object[]` component settled nothing until NOMINATION 2 landed
    //     (F29-1 §2.3): `entry_call_site` now carries the caller's static
    //     parameter type into `mh_dispatch`'s `MH_KIND_COLLECT` arm, so a
    //     `char`/`boolean` argument the array component cannot distinguish
    //     from an `int` is boxed by what the call site actually named.
    //     `mhcoll.char`, `mhcoll.bool` and `mhvar.char` are the rows that
    //     prove it — ALSO canonical on HotSpot, measured — and this family's
    //     denominator is 11.
    ...
    static void mhcollect() throws Throwable {
        ...
        // The WRAPPER CLASS from a plain `Object[]` collector — settled by the
        // CALL SITE, not by the component or the runtime bit pattern (`CA` and
        // `I7`'s low bits can coincide; the class must not). F29-1 §1/§2.3,
        // NOMINATION 2.
