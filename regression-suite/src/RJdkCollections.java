...
public class RJdkCollections {
     ...
     * docs/jdk-only/W7-65-stream-reuse-throws.md -- two of them are
    ...
    /**
     * `getClass().getName()` on the iterator a few carriers hand out --
     * `apps/probes/ItrCarrierCensus`, measured against HotSpot 25.
     *
     * `Vector`/`Stack` share this VM's `ArrayList$Itr` carrier at the object
     * level (both extend `AbstractList` and carry a real {@code modCount}, so
     * the fail-fast behaviour is already right) but HotSpot hands out its own
     * {@code Vector$Itr} instead -- a display-only gap, closed in
     * {@code getclass_display_class_id} without touching dispatch.
     *
     * `List.of`/`copyOf` and `Collections.unmodifiableList` share one
     * synthetic {@code UnmodifiableListItr} carrier for both {@code
     * iterator()} and {@code listIterator()}, but HotSpot answers with THREE
     * different classes across that one shape: {@code
     * ImmutableCollections$ListItr} for the immutable factories (either
     * method), {@code Collections$UnmodifiableCollection$1} for {@code
     * unmodifiableList(...).iterator()}, and {@code
     * Collections$UnmodifiableList$1} for {@code
     * unmodifiableList(...).listIterator()}.
     */
    static void iteratorCarrierNames() {
        check(new ArrayList<>(List.of("a")).iterator().getClass().getName()
                        .equals("java.util.ArrayList$Itr"),
                "an ordinary ArrayList must keep its own iterator class name");

        java.util.Vector<String> v = new java.util.Vector<>();
        v.add("a");
        check(v.iterator().getClass().getName().equals("java.util.Vector$Itr"),
                "Vector.iterator() must report java.util.Vector$Itr");
        java.util.Stack<String> st = new java.util.Stack<>();
        st.push("a");
        check(st.iterator().getClass().getName().equals("java.util.Vector$Itr"),
                "Stack.iterator() inherits Vector's iterator class name");

        List<String> imm = List.of("a", "b", "c");
        check(imm.iterator().getClass().getName()
                        .equals("java.util.ImmutableCollections$ListItr"),
                "List.of(...).iterator() must report ImmutableCollections$ListItr");
        check(imm.listIterator().getClass().getName()
                        .equals("java.util.ImmutableCollections$ListItr"),
                "List.of(...).listIterator() must report ImmutableCollections$ListItr");

        List<String> unmod = Collections.unmodifiableList(new ArrayList<>(imm));
        check(unmod.iterator().getClass().getName()
                        .equals("java.util.Collections$UnmodifiableCollection$1"),
                "unmodifiableList(...).iterator() must report "
                        + "Collections$UnmodifiableCollection$1");
        check(unmod.listIterator().getClass().getName()
                        .equals("java.util.Collections$UnmodifiableList$1"),
                "unmodifiableList(...).listIterator() must report "
                        + "Collections$UnmodifiableList$1");
        System.out.println("CK RJdkCollections iteratorCarrierNames=ok");
    }
    ...
    // Every refusal below is a BARE NullPointerException -- getMessage() must
    // be null, not a helpful-NPE and not "". Paired throughout with the case
    // that must NOT throw, because the dangerous direction of this axis is
    // over-refusal (a null Comparator, an already-overriding immutable
    // receiver, or the wrong argument index on an overloaded native).
    static void checkBareNpe(String label, Runnable r) {
        checks++;
        try {
            r.run();
            throw new AssertionError(label + " [must throw]");
        } catch (NullPointerException e) {
            check(e.getMessage() == null, label + " [bare, getMessage()==null]");
        }
    }
    ...
    static void checkThrows(String label, Class<? extends Throwable> want, Runnable r) {
        ...
        try {
            ...
            check(want.isInstance(t), label + " [want " + want.getSimpleName() + ", got " + t.getClass().getName() + "]");
        }
    }
    ...
    static void nullFunctionAxis() {
        ...
        checkBareNpe("HashMap.computeIfAbsent", () -> new HashMap<String, String>().computeIfAbsent("k", null));
        checkBareNpe("HashMap.compute", () -> new HashMap<String, String>().compute("k", null));
        checkBareNpe("HashMap.computeIfPresent/abs", () -> new HashMap<String, String>().computeIfPresent("zz", null));
        checkBareNpe("TreeMap.forEach(null)", () -> new TreeMap<>().forEach(null));
        checkBareNpe("TreeMap.merge(k,v,null)", () -> new TreeMap<String, String>().merge("k", "v", null));
        checkBareNpe("LinkedHashMap.forEach(null)", () -> new LinkedHashMap<>().forEach(null));
        checkBareNpe("CHM.forEach(null)", () -> new java.util.concurrent.ConcurrentHashMap<>().forEach(null));
        checkBareNpe("CHM.replaceAll(null)", () -> new java.util.concurrent.ConcurrentHashMap<>().replaceAll(null));
        checkBareNpe("Properties.computeIfAbsent", () -> new java.util.Properties().computeIfAbsent("k", null));
        ...
        checkBareNpe("Vector.removeIf(null)", () -> new java.util.Vector<>().removeIf(null));
        checkBareNpe("LinkedList.removeIf(null)", () -> new java.util.LinkedList<>().removeIf(null));
        checkBareNpe("HashSet.forEach(null)", () -> new HashSet<>().forEach(null));
        checkBareNpe("TreeSet.forEach(null)", () -> new java.util.TreeSet<>().forEach(null));
        checkBareNpe("ArrayDeque.forEach(null)", () -> new java.util.ArrayDeque<>().forEach(null));
        ...
        l1.sort(null);
        check(l1.toString().equals("[a, b]"), "ArrayList.sort(null) sorts");
        List<String> l2 = new java.util.Vector<>(List.of("b", "a"));
        l2.sort(null);
        check(l2.toString().equals("[a, b]"), "Vector.sort(null) sorts");
        List<String> l3 = new java.util.LinkedList<>(List.of("b", "a"));
        l3.sort(null);
        check(l3.toString().equals("[a, b]"), "LinkedList.sort(null) sorts");
        List<String> l4 = new ArrayList<>(List.of("b", "a"));
        Collections.sort(l4, null);
        check(l4.toString().equals("[a, b]"), "Collections.sort(l,null) sorts");

        // Trap 2: an immutable receiver refuses FIRST, with a different type.
        checkThrows("List.of().removeIf(null) is UOE", UnsupportedOperationException.class,
                () -> List.of("a").removeIf(null));
        checkThrows("List.of().sort(null) is UOE", UnsupportedOperationException.class,
                () -> List.of("a").sort(null));
        checkThrows("unmodMap.replaceAll(null) is UOE", UnsupportedOperationException.class,
        ...
        checkThrows("unmodList.forEach(null) is NPE", NullPointerException.class,
        ...
        checkBareNpe("CHM.forEach(long,null)",
                () -> new java.util.concurrent.ConcurrentHashMap<String, String>().forEach(1L, null));
        // ...and the same overload with a real action must still run.
        java.util.concurrent.ConcurrentHashMap<String, String> chm =
                new java.util.concurrent.ConcurrentHashMap<>(Map.of("a", "1"));
        int[] seen = { 0 };
        chm.forEach(1L, (k, v) -> seen[0]++);
        check(seen[0] == 1, "CHM.forEach(long,action) still runs");
    }
