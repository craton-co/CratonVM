...
/**
 * JDK-only corpus: G9-1's Unicode version-skew vector.
 *
 * Covers the version skew above the BMP between Rust's Unicode tables and
 * JDK 25 (56 code points where Rust has a case mapping and the JDK has none),
 * the Final_Sigma rule the character-wise case-mapping arm used to drop when
 * a skewed code point was present in the same string, and the three
 * record-component hashCode/equals rules ({@code float}/{@code double} NaN
 * canonicalisation, {@code boolean} using {@code Boolean.hashCode} rather
 * than the raw {@code int}).
 *
 * Every expectation below is HotSpot 25.0.3+9, captured 2026-08-16
 * (docs/known-issues/jdk-only/G9-1-...). Values are printed as UTF-16 code
 * units, never as rendered characters, and every label is ASCII: a non-ASCII
 * label goes through the platform console encoding differently on the two
 * VMs and fails the differential with every assertion passing.
 */
public class RJdkG9Skew {
    ...
    static void sigma() {
        ...
        // No skewed code point: the bulk path.
    ...
    public static void main(String[] a) {
        ...
        charfam();
        if (fails > 0) {
            throw new AssertionError("RJdkG9Skew: " + fails + "/" + checks + " checks failed");
        }
        System.out.println("PASS RJdkG9Skew (" + checks + " checks)");
    }
}
