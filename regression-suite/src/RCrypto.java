...
public class RCrypto {
    ...
    public static void main(String[] a) throws Exception {
        ...
        // W7-39. Blowfish, RC4 (ARCFOUR) and HmacSHA224 were plain
        // NoSuchAlgorithmException before this record's fix routed the first
        // two through the real SunJCE SPI and added HmacSHA224 via
        // hmac::Hmac<sha2::Sha224>. Known-answer, not round-trip — measured on
        // Temurin 25.0.3+9 (W7-39 sections "Blowfish and RC4", "HmacSHA224"),
        // so a cipher that runs the wrong algorithm under the right name (the
        // AES-under-ChaCha20 shape this campaign has already found once) fails
        // here instead of agreeing with itself.
        byte[] w39Key = unhex("303132333435363738393a3b3c3d3e3f");
        byte[] w39Pt = "sixteen byte msg".getBytes("UTF-8");
        Cipher bf = Cipher.getInstance("Blowfish");
        bf.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(w39Key, "Blowfish"));
        String bfCt = hex(bf.doFinal(w39Pt));
        check(bfCt.equals("33b63e40d662746425f71a69f8cffcdadadae7ffa8950336"),
                "Blowfish/ECB/PKCS5Padding (default) known-answer mismatch: " + bfCt);
        Cipher rc4 = Cipher.getInstance("RC4");
        rc4.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(w39Key, "RC4"));
        String rc4Ct = hex(rc4.doFinal(w39Pt));
        check(rc4Ct.equals("27ca482b161e3ab93f812659b904df95"),
                "RC4 known-answer mismatch: " + rc4Ct);
        byte[] hmacKey = new byte[32];
        Arrays.fill(hmacKey, (byte) 0x0b);
        Mac hmac224 = Mac.getInstance("HmacSHA224");
        hmac224.init(new SecretKeySpec(hmacKey, "HmacSHA224"));
        String hmac224Val = hex(hmac224.doFinal("hi".getBytes("UTF-8")));
        check(hmac224Val.equals("86d73d8ce8749e5b49d61d6211e3a5493d4623984d9ca51aac19cc51"),
                "HmacSHA224 known-answer mismatch: " + hmac224Val);
        System.out.println("CK RCrypto w39=" + bfCt + "," + rc4Ct + "," + hmac224Val);

        // keygen.Blowfish — the row W7-39 itself flagged as "least able to
        // stand behind": correctness depends on a real JDK KeyGeneratorSpi
        // subclass loading and running inside CratonVM, which no other row in
        // this file exercises. Not a known-answer (key generation is random);
        // the three properties W7-39 names — right length, not all-zero, two
        // draws differ — are exactly what a KeyGenerator wired to nothing
        // (returning a constant or an all-zero buffer) would fail.
        KeyGenerator bfKg = KeyGenerator.getInstance("Blowfish");
        byte[] bfKey1 = bfKg.generateKey().getEncoded();
        byte[] bfKey2 = bfKg.generateKey().getEncoded();
        check(bfKey1.length == 16,
                "KeyGenerator.getInstance(\"Blowfish\") default is 128-bit, got "
                        + bfKey1.length * 8);
        check(!Arrays.equals(bfKey1, new byte[16]), "a generated Blowfish key must not be all zeros");
        check(!Arrays.equals(bfKey1, bfKey2), "two Blowfish keygen draws must differ");
        System.out.println("CK RCrypto keygenBlowfish=" + bfKey1.length + ",allZero="
                + Arrays.equals(bfKey1, new byte[16]) + ",twoDrawsDiffer="
                + !Arrays.equals(bfKey1, bfKey2));
