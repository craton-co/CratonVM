 ...
 *
 * <p>See W8-C3-1-intrinsic-census-round-2-RETIRED-20260921.md for the
...
public class RJdkIntrinsics2 {
    ...
    static void bounds() {
        ...
        // 121 -> 126: F37-1 NOM N5. §3.1's headline finding —
        // `ByteBuffer.allocate(8).asReadOnlyBuffer().put(0,(byte)1)` SUCCEEDED
        // where it must throw — had no fixture row anywhere in this suite: the
        // buffer rows above cover array()/arrayOffset()/hasArray()/isReadOnly()
        // and the contagion table, and none of them ever calls put(). Three
        // ReadOnlyBufferException rows (plain ByteBuffer, a typed heap family,
        // and an array-less ByteBufferAs<T>Buffer view — the three receiver
        // shapes F37-1 §3.2 measured) plus two writable negative controls, so
        // a future registrar reordering that un-shadows a guardless body (the
        // exact species F37-1 §3.1 found: `native-io` shadows `servlet.rs`'s
        // correctly-guarded body in every arm today) is caught here.
        step("bounds", "ByteBuffer.allocate(8).asReadOnlyBuffer().put(0,(byte)1)");
        t = null;
        try {
            ByteBuffer.allocate(OPAQUE_I[3] + 5).asReadOnlyBuffer().put(0, (byte) 1);
        } catch (Throwable x) {
            t = x;
        }
        check("java.nio.ReadOnlyBufferException".equals(nameOf(t)),
                "a read-only ByteBuffer.put(int,byte) must throw ReadOnlyBufferException,"
                        + " not silently write into the source's backing array; got "
                        + nameOf(t));
        step("bounds", "IntBuffer.allocate(4).asReadOnlyBuffer().put(0,x)");
        t = null;
        try {
            IntBuffer.allocate(OPAQUE_I[6] + 1).asReadOnlyBuffer().put(0, OPAQUE_I[3]);
        } catch (Throwable x) {
            t = x;
        }
        check("java.nio.ReadOnlyBufferException".equals(nameOf(t)),
                "the same guard on a TYPED heap family's put(int,int); got " + nameOf(t));
        step("bounds", "ByteBuffer.allocate(16).asIntBuffer().asReadOnlyBuffer().put(0,x)");
        t = null;
        try {
            ByteBuffer.allocate(OPAQUE_I[11]).asIntBuffer().asReadOnlyBuffer().put(0, OPAQUE_I[3]);
        } catch (Throwable x) {
            t = x;
        }
        check("java.nio.ReadOnlyBufferException".equals(nameOf(t)),
                "and on an ARRAY-LESS ByteBufferAsIntBufferRB view — ROBE, not UOE: the"
                        + " array-less-first ordering is arrayOffset()'s contract, not put()'s"
                        + " (F37-1 §3.2); got " + nameOf(t));
        // Negative controls: a WRITABLE receiver of each shape must still
        // accept the write, so the rows above are testing the read-only guard
        // and not a blanket refusal.
        ByteBuffer wRoCtl = ByteBuffer.allocate(OPAQUE_I[3] + 5);
        wRoCtl.put(0, (byte) 9);
        check(wRoCtl.get(0) == 9, "control: a writable ByteBuffer.put(int,byte) still writes");
        IntBuffer wIbCtl = IntBuffer.allocate(OPAQUE_I[6] + 1);
        wIbCtl.put(0, OPAQUE_I[3]);
        check(wIbCtl.get(0) == OPAQUE_I[3],
                "control: a writable IntBuffer.put(int,int) still writes");
        sectionEnd("bounds", 126);
    }
