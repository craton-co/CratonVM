 ...
 * HISTORY.
 *
 * Until 2026-09-21, {@code ofArray} over {@code int[]}, {@code long[]},
 * {@code float[]} and {@code double[]} did NOT alias on CratonVM: those
 * four allocated an off-heap native mirror and copied the array into
 * it, so those carriers could be handed to downcalls (a moving Java
 * array cannot be). A bulk {@code copy} INTO one of those mirrors left
 * the Java array untouched, silently — this file deliberately did not
 * assert content for that case, and exercised {@code int[]} only
 * through the layout overload below, which names the destination array
 * directly rather than wrapping it in a segment. {@code ofArray} now
 * mints the SAME aliasing carrier for all seven primitive array types
 * (see {@code native-builtins/src/panama.rs}'s registration site), so
 * {@link #nativeToAliasingHeapInt} below closes that gap directly.
 */
public class RSegmentBulkCopy {
    ...
    /**
     * The gap the class comment names: {@code ofArray(int[])} now aliases,
     * so a bulk copy INTO one must land in the Java array itself, both
     * directions. MEASURED on HotSpot 25.0.3+9-LTS ({@code FfmRepro3.java}):
     * {@code copy(src, 0, ofArray(int[4]), 0, 16)} fills the array, and
     * {@code copy(ofArray(int[]{7,8,9,10}), 0, nativeDst, 0, 16)} reads back
     * out of {@code nativeDst} as {@code [7, 8, 9, 10]}.
     */
    static void nativeToAliasingHeapInt(Arena arena) {
        MemorySegment src = arena.allocate(16, 8);
        for (int i = 0; i < 4; i++) {
            src.set(ValueLayout.JAVA_INT, (long) i * 4, (i + 1) * 111);
        }
        int[] dst = new int[4];
        MemorySegment.copy(src, 0L, MemorySegment.ofArray(dst), 0L, 16L);
        checkEquals(111, dst[0], "native -> aliasing heap int[] copied element 0");
        checkEquals(222, dst[1], "native -> aliasing heap int[] copied element 1");
        checkEquals(333, dst[2], "native -> aliasing heap int[] copied element 2");
        checkEquals(444, dst[3], "native -> aliasing heap int[] copied element 3");

        int[] host = {7, 8, 9, 10};
        MemorySegment nativeDst = arena.allocate(16, 8);
        MemorySegment.copy(MemorySegment.ofArray(host), 0L, nativeDst, 0L, 16L);
        for (int i = 0; i < 4; i++) {
            checkEquals(
                    host[i],
                    nativeDst.get(ValueLayout.JAVA_INT, (long) i * 4),
                    "aliasing heap int[] -> native copied element " + i);
        }
    }
