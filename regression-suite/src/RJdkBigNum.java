...
/**
 * JDK-only corpus: G10-1's bignum surface vector.
 *
 * Every observable is printed on its own {@code CK RJdkBigNum <key>=<value>}
 * line (the suite's harness contract, {@code harness-guard.sh}) and diffed
 * byte-for-byte against HotSpot rather than asserted against a hard-coded
 * golden value (docs/known-issues/jdk-only/G10-1-... section 12):
 * {@code BigDecimal.multiply}'s product-scale overflow guard,
 * {@code doubleValue}/{@code floatValue} at the scale extremes (must not
 * render an argument-driven number of digits), {@code setScale} across every
 * {@code RoundingMode} at the scales that overflow when negated, the rounding
 * table, {@code toString}/{@code toPlainString}/{@code toEngineeringString},
 * {@code BigInteger.equals}/{@code compareTo} against null and wrongly-typed
 * arguments, shifts at and beyond {@code Integer.MIN_VALUE}, and the
 * {@code mod}/{@code remainder}/{@code divide}/{@code modPow}/{@code
 * modInverse} refusal messages.
 *
 * Deliberately excludes {@code setScale(1.5, mode, 715827883)}: HotSpot
 * spends 60-90s on that row before {@code OutOfMemoryError} (the row is
 * *correct*, not a bug, and belongs nowhere in a fast regression gate).
 */
public class RJdkBigNum {
    ...
    static void guarded(String label, java.util.function.Supplier<Object> s) {
        ...
        System.out.println("CK RJdkBigNum " + label + "=" + out);
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("CK RJdkBigNum checks=" + checks);
        System.out.println("PASS RJdkBigNum (" + checks + " checks)");
    }
