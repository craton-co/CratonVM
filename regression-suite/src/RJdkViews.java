 ...
 * docs/jdk-only/W7-1-treemap-views-and-iterator-remove-contract.md,
...
public class RJdkViews {
    ...
    /**
     * docs/known-issues/jdk-only/W7-36-differential-view-families.md Part 1 R3
     * and Part 3, permanently gated here per that record's own section 6.3 --
     * these had no scheduled witness before, only the untracked
     * ShadowDifferentialProbe.
     */
    static void w736ViewResiduals() {
        // Part 3: Collections.unmodifiableList is idempotent by identity.
        List<Integer> base1 = new ArrayList<>(List.of(1, 2, 3));
        List<Integer> u = java.util.Collections.unmodifiableList(base1);
        List<Integer> u2 = java.util.Collections.unmodifiableList(u);
        check(u == u2, "unmodifiableList(unmodifiableList(l)) must return the same object");
        List<Integer> immutable = List.of(1, 2);
        List<Integer> wrapped = java.util.Collections.unmodifiableList(immutable);
        check(wrapped != immutable,
                "unmodifiableList(List.of(...)) must still wrap -- the JDK wraps ImmutableCollections$ListN too");

        // Part 3: a nested subList is a view of the ROOT list, not a snapshot
        // of the enclosing one, and a structural mutation through it must
        // walk back up every enclosing view while leaving a SIBLING view
        // over the same base still comodification-checked.
        List<String> base2 = new ArrayList<>(Arrays.asList("a", "b", "c", "d", "e"));
        List<String> sub = base2.subList(1, 4);
        List<String> sibling = base2.subList(0, 5);
        List<String> nested = sub.subList(0, 2);
        nested.clear();
        check(base2.equals(Arrays.asList("a", "d", "e")),
                "a nested subList.clear() must write through to the base list");
        check(sub.equals(java.util.Collections.singletonList("d")),
                "and to the enclosing view");
        boolean sawCme = false;
        try {
            sibling.get(0);
        } catch (ConcurrentModificationException expected) {
            sawCme = true;
        ...
        check(sawCme,
                "a SIBLING view over the same base must still raise ConcurrentModificationException");

        // Part 3: TreeSet.descendingSet() must write through to the backing
        // set, not just answer correctly from its own snapshot.
        TreeSet<Integer> ts = new TreeSet<>(Arrays.asList(9, 2, 1));
        NavigableSet<Integer> ds = ts.descendingSet();
        Integer polled = ds.pollFirst();
        check(polled != null && polled == 9, "descendingSet().pollFirst() removes the backing set's maximum");
        check(!ts.contains(9), "and the removal must write through to the backing TreeSet");
        check(ts.size() == 2, "so the backing set shrinks too");

        // Part 1 R3: ConcurrentHashMap's bulk-operation family walks the real
        // `table` field through a Traverser; a natively-backed CHM keeps its
        // entries in a side layout that never populates it, so these used to
        // answer null for every input rather than folding/searching.
        java.util.concurrent.ConcurrentHashMap<String, Integer> chm =
                new java.util.concurrent.ConcurrentHashMap<>();
        chm.put("a", 6);
        Integer reduced = chm.reduceValues(1L, v -> v, Integer::sum);
        check(reduced != null && reduced == 6,
                "ConcurrentHashMap.reduceValues must fold values via its BulkTask, not answer null");
        String found = chm.searchKeys(1L, k -> "a".equals(k) ? "found" : null);
        check("found".equals(found),
                "ConcurrentHashMap.searchKeys must short-circuit on the matching key, not answer null");
    }
