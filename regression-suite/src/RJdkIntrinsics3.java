 ...
 * W8-C3-1-intrinsic-census-round-2-RETIRED-20260921.md and in this lane's record.
 *
...
public class RJdkIntrinsics3 {
    ...
    static void boxid() {
        ...
        // Float and Double cache NOTHING. This is the only pair of rows that fails if a VM
        // makes the boxing family "consistent" -- and both values are still .equals-equal, so
        // an equality-shaped row cannot stand in for these. See F1-1 NOMINATION 2.
    ...
    // G43-1's own shape: `childValue` composes the parent's value rather than
    // copying it verbatim, so the child's answer differs from the parent's.
    static class CvITL extends InheritableThreadLocal<String> {
        @Override
        protected String childValue(String parentValue) {
            return "cv(" + parentValue + ")";
        }
    }
    ...
    static void tlocal() throws Exception {
        ...
        // -- G43-1: childValue is applied per snapshot, and composes ------------
        // ("cv(cv(root))" across two generations), is called for an explicit
        // null but not after a remove(), and reaches a grandchild whose parent
        // never read the inherited entry (the lazy-drain transitivity bug).
        CvITL cv = new CvITL();
        cv.set("cvparent");
        final String[] cvChild = new String[1];
        Thread cvT = new Thread(() -> cvChild[0] = cv.get());
        cvT.start();
        cvT.join();
        ckS("tlocal:childValue is applied to the child's copy", cvChild[0], "cv(cvparent)");
        ckS("tlocal:childValue does not touch the parent's own value", cv.get(), "cvparent");

        CvITL cvGen = new CvITL();
        cvGen.set("root");
        final String[] cvGen2 = new String[1];
        Thread cvGen1Thread = new Thread(() -> {
            Thread grandchild = new Thread(() -> cvGen2[0] = cvGen.get());
            grandchild.start();
            try {
                grandchild.join();
        ...
        cvGen1Thread.start();
        cvGen1Thread.join();
        ckS("tlocal:childValue composes once per generation", cvGen2[0], "cv(cv(root))");

        CvITL cvNull = new CvITL();
        cvNull.set(null);
        final String[] cvOfNull = new String[1];
        Thread cvNullT = new Thread(() -> cvOfNull[0] = cvNull.get());
        cvNullT.start();
        cvNullT.join();
        ckS("tlocal:childValue runs for an explicitly stored null", cvOfNull[0], "cv(null)");

        CvITL cvRemoved = new CvITL();
        cvRemoved.set("x");
        cvRemoved.remove();
        final String[] cvOfRemoved = new String[1];
        Thread cvRemovedT = new Thread(() -> cvOfRemoved[0] = cvRemoved.get());
        cvRemovedT.start();
        cvRemovedT.join();
        ckB("tlocal:childValue does not run after remove()", cvOfRemoved[0] == null, true);

        InheritableThreadLocal<String> gp = new InheritableThreadLocal<>();
        gp.set("gp-init");
        final String[] gcNoRead = new String[1];
        Thread childNoRead = new Thread(() -> {
            // Deliberately do not read `gp` before constructing the grandchild --
            // this is what isolates the lazy-drain transitivity bug from plain
            // inheritance (the read-first control right below agrees on both VMs
            // whether or not the bug is fixed).
            Thread grandchild = new Thread(() -> gcNoRead[0] = gp.get());
            grandchild.start();
            try {
                grandchild.join();
            } catch (InterruptedException ignored) {
        ...
        childNoRead.start();
        childNoRead.join();
        ckS("tlocal:grandchild inherits even when the child never read it", gcNoRead[0], "gp-init");

        InheritableThreadLocal<String> gp2 = new InheritableThreadLocal<>();
        gp2.set("gp-init");
        final String[] gcRead = new String[1];
        Thread childRead = new Thread(() -> {
            gp2.get(); // drains the pending inherited entry before the grandchild is built
            Thread grandchild = new Thread(() -> gcRead[0] = gp2.get());
            grandchild.start();
            try {
                grandchild.join();
            } catch (InterruptedException ignored) {
        ...
        childRead.start();
        childRead.join();
        ckS("tlocal:grandchild inherits when the child read first (control)", gcRead[0], "gp-init");

        sectionEnd("tlocal", 33);
    }
