...
public class RJdkJmx {
    ...
    static void objectNames() throws Exception {
        ...
        // H6-1 N5 / section 7.2: `getCanonicalName()`, `getKeyPropertyListString()`
        // and `toString()` are THREE DIFFERENT strings on a name whose source
        // order is not already sorted -- canonical SORTS keys, the other two
        // preserve SOURCE order -- and until this vector existed, only the
        // canonical accessor was asserted anywhere in the corpus, so H6-1's
        // own `getSerializedNameString` fix (source order, not canonical) had
        // no vector able to see it move.
        check(a.toString().equals("cratonvm.test:type=Counter,name=alpha"),
                "toString preserves source order: " + a.toString());
        check(a.getKeyPropertyListString().equals("type=Counter,name=alpha"),
                "getKeyPropertyListString preserves source order: " + a.getKeyPropertyListString());
        check(!a.toString().equals(a.getCanonicalName()),
                "source order and canonical order must differ for this fixture");

        // Serialization round-trip: `getSerializedNameString()` backs
        // `writeObject`, and a name with unsorted keys is exactly the shape
        // that distinguishes it from the canonical accessor.
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        try (ObjectOutputStream oos = new ObjectOutputStream(bos)) {
            oos.writeObject(a);
        }
        ObjectName aRoundTripped;
        try (ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(bos.toByteArray()))) {
            aRoundTripped = (ObjectName) ois.readObject();
        }
        check(aRoundTripped.equals(a), "ObjectName survives a serialization round-trip");
        check(aRoundTripped.toString().equals(a.toString()),
                "round-tripped ObjectName keeps source-order toString: " + aRoundTripped.toString());
        check(aRoundTripped.getCanonicalName().equals(a.getCanonicalName()),
                "round-tripped ObjectName keeps its canonical name");
