...
public class RJdkNio {
    ...
    static void randomAccessAndMapping(Path dir) throws Exception {
        ...
        try (FileChannel ch = FileChannel.open(m, StandardOpenOption.CREATE,
            ...
            // truncate(-1): the message is the bare literal HotSpot's
            // `FileChannel.truncate(long)` javadoc quotes -- it does not echo
            // the value. Measured against HotSpot 25 (`apps/probes/L4Diag`):
            // this VM used to say "Negative size: -1".
            boolean negThrew = false;
            try {
                ch.truncate(-1L);
            } catch (IllegalArgumentException expected) {
                negThrew = true;
                check("Negative size".equals(expected.getMessage()),
                        "truncate(-1) message must match HotSpot exactly: " + expected.getMessage());
            }
            check(negThrew, "a negative truncate size must throw IllegalArgumentException");
            check(ch.size() == 256, "a refused truncate must not change the file size");
