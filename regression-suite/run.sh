...
#
# RJitMapTierDiffNoJit landed 2026-09-21 (H10-1 N2): the SAME probe
# (`RJitMapTierDiff.run`, called directly — see its own file) scheduled a
# second time under `--nojit` via `class_cv_args`, so "run it BOTH ways" (the
# vector's own header) is actually scheduled rather than left as a manual
# step. It is a CONTROL, not a second copy of the defect check: every `moved=`
# is expected to read -1 here exactly as it does in the JIT-on run's COLD
# column, because staying interpreted for all iterations cannot produce a
# tier split by construction. A divergence here would mean the defect is not
# tier-dependent at all.
#
# RHashSetPresentMarker landed 2026-09-21 (H9-1 N4): pins real
# `HashSet.PRESENT` identity (`map.remove(o) == PRESENT`) across every VM
# producer that builds a HashSet-family object, so the identity-marker defect
# `H9-1` fixed in `11798a8a2` cannot regress silently. Before that commit two
# of this file's seven rows were measured red on the pristine binary
# (`newHashSetFromList`, `collectorsToSet`) -- `remove()` deleted the element
# and reported `false`.
#
# RJitSelfRecCatch landed 2026-09-22: a self-recursive method whose own body
# catches what its recursive call throws IMPLICITLY. It replaces
# `regression-suite/probes/R10SelfRecCatch.java`, and the reason it is a
# scheduled vector rather than a probe is the reason the probe was not enough:
# the probe warmed three shapes in a hot loop and then PRINTED four values from
# `main`, which this VM never compiles, so all four columns of its page's
# closing measurement read the interpreter while a wrong answer was live in two
# JIT arms. Every assertion here reads a value produced inside `drive`.
#
# Falsified before landing, which is the whole point. On pristine `dev`
# 7281ce4b6 it is RED 5/5 in the default arm AND 5/5 under
# CRATONVM_C2_ACCEPT=always, green 5/5 under --nojit and C2_ACCEPT=never
# (the interpreted arms are the oracle); on the fixed binary it is green 5/5 in
# all four. A representative failure:
#
#   AssertionError: recNpe: 9937/10000 calls returned 44, want 43
#     (44 = the handler ran in the throwing activation, with its locals)
#
# `explicit` is the built-in control — an explicit `throw` never reaches the
# deopt-servicing door and was green on both binaries. MEASURED against HotSpot
# 25.0.4 as the oracle: rc=0, PASS RJitSelfRecCatch (4 checks), ~1.0 s a run.
#
# RJitSelfRecUncovered landed 2026-09-22: the same family as RJitSelfRecCatch,
# one variable different — the self-recursive method has BOTH a covered and an
# uncovered self-call site, and the vector enters through the UNCOVERED one.
# `recDeep(4, 4)` must throw; on pristine `dev` 5a6d5073b it returned 1004 on
# 19,930 of 20,000 calls (19,937 under `always`) in all three JIT arms and 0
# under --nojit, RED 3/3 there and GREEN 5/5 per arm on the fix, because the
# drain downgraded an "outside every protected range" verdict to the pc-unknown
# search whenever the stamp named a self-call site, and that search matches a
# TYPED handler on exception class alone, ignoring its range.
#
# It is the deterministic successor to
# `docs/internal/retired/r10-selfrec-deep-handler-leaks-once-per-million-20260922-RETIRED-20260922.md`,
# whose own measurement entered through the COVERED branch (`recDeep(11, 4)`)
# and therefore saw the same defect at ~1 per 1.2 million calls — the rate at
# which the compiled/interpreted boundary happens to land on an activation whose
# stamp is the uncovered site. That page said a 10,000-call vector "would fire
# roughly once every 120 suite runs, which is a flaky vector rather than a
# guard"; this one fires on essentially every call, which is what changing the
# entry point bought. Both of its controls (`recDeep(11,4)` and a covered-only
# self-recursive method) were green on the pristine binary and are carried as
# controls rather than as evidence.
#
...
CORE_CLASSES="RCollections RStrings RNumbers RSerial RCrypto RExceptions RReflect ROptionalClassForName RPrivateLambdaOwner RLambdaDefaultOverload RJitGc RJitStringLayout RJitStackTraceLines RJitTreeSubMapIter RJitOsrStringDispatch RJitArrayTypecheck RJitArraycopyRefDeopt RJitMultiArrayClass RJitMultiArrayDims RJitMapTierDiff RJitMapTierDiffNoJit RHashSetPresentMarker RArrayStoreTiers RArrayStoreInterfaces RArrayStoreLibrary RArraysMismatch RExecutorShutdown RBlockingQueue RChmKeySetView RChannelInterrupt RChannelToString RSocketChannelInterrupt RAtomicArray RDirectBufferElem RMapResizeGc RMapGcStress RForNameGcStress ROverlaySystemGcStress RFileTimes RNioNoFollow RSyncMethodJit RJitSyncBlockScalar RFieldSiteCache RMethodSiteCache RDataInputFastPull RCanAccessRules RChaCha20Cipher RLockedIdentityHash RCanAccessReceiver RForeignLayoutCollections RForeignLayoutJdkInterfaces RLoaderChurnDefine RClassUnloadSweep RClassUnloadSweepGen RPriorityQueueGc RTreeRangeGc RJdkViews RJdkFormatLocale RJdkFormatSurface RJdkFfmSegment RJdkStrictMath RJdkByteOrder RJdkIntrinsics RJdkIntrinsics2 RShutdownHooks RSimpleTimeZoneRaw RImmutableFactoryTypes RJdkStringCodePoints RFsSingleton RJdkOptionalShape RSimpleDateFormatZone RJdkIntrinsics3 RJdkBridge1 RPropertiesClone RPermissionInit RSslNullSession RSslLiveSession RSslEndpointIdentification RVarHandleAccess RJdkVarHandleNullCoord RJdkVarHandleModeSupport RStringBuilderContent RUnsafeArrayBase RLangPackages RBuiltinLoaderPackages RLoaderIdentity RSegmentBulkCopy RStreamToListCopy RFileChannelFastIo RSocketFastIo RAnnotationProxyGate RJdkProcessStreams RNetIfaceScope RBigIntMontgomery RJitVarHandleRefRead REncodingFidelity RBufferPoolCount RJitLambdaNpeSupersede RThrowableFillFrames RLoaderExceptionShape RJitUnrollImplicitNpe RJitSelfRecCatch RJitSelfRecUncovered RMulticastSocketClose RMulticastSocketLoopback"
...
# RJitMultiArrayDims landed 2026-09-22 with the N-dimensional multianewarray
# lowering. It is a CORE vector for two separate reasons, and only the second is
# a red-to-green flip. (1) ARITY: `jit_scan` refused every `multianewarray` with
# `ndims != 2`, and a scan refusal is PERMANENT and whole-method, so one
# `new byte[3][4][4]` made `RomulusEngine.skinny_128_384_plus_enc` — 62.6% of
# RomulusTest's samples, 138x HotSpot — uncompilable at every door. Those rows
# were never WRONG, only never compiled, so their evidence is the `moved=` column
# and the wall in docs/jdk-only/, not the answers. (2) NEGATIVE DIMENSIONS: the
# interpreter checked them in POP order, i.e. innermost first, so
# `new byte[-1][4][-2]` threw -2 where HotSpot and this VM's own JIT helper both
# say -1, and `new byte[2][0][-1]` did not throw at all. Those rows ARE a flip,
# and they were red on BOTH tiers. MEASURED against HotSpot 25.0.3+9.
CORE_CLASSES="$PRUNED"
...
# ---- the policy this harness measures, stated rather than inherited -------
#
# On 2026-09-20 the launcher default flipped from `compatible` to `jdk-only`.
# Every number in this file, in expect/, in jdk-only-coverage.txt and in the
# HANDOFF documents it cites was measured with a bare invocation meaning
# COMPATIBLE. If the harness kept passing no policy flag, all of them would
# silently start describing a different mode -- the CORE list in particular,
# whose own header (see the RJdkViews note above) says it asserts
# `--real-jdk` COMPATIBILITY behaviour and is deliberately NOT the strict
# corpus.
#
# So a run that names no policy is given `--compatible` explicitly. That is
# the same VM behaviour a bare run had before the flip, and now the command
# line says which mode produced the number instead of depending on which
# binary ran it.
#
# A run that names a policy of its own gets nothing added: `--jdk-only` is
# the strict corpus arm, and `--compatible` / `--real-jdk` / `--synthetic-jdk`
# already select compatible (`--synthetic-jdk` actually CONFLICTS with
# `--compatible`, so appending it would turn that arm into a launch error).
POLICY_ARGS="--compatible"
case " ${CRATONVM_ARGS:-} " in
  *" --jdk-only "*|*" --compatible "*|*" --real-jdk "*|*" --synthetic-jdk "*) POLICY_ARGS="" ;;
esac
...
run_pass() {
  ...
  for c in $CLASSES; do
    ...
    cvout=$(CRATONVM_DISABLE_DEFAULT_WATCHDOG=1 timeout "$TIMEOUT" "$CV" --java-home "$JDK" ${POLICY_ARGS:-} ${CRATONVM_ARGS:-} $cvextra $extra "${cvreport[@]}" -cp "$BUILD$cpx" "$c" 2>&1)
    cvrc=$?
    cvkey=$(printf '%s\n' "$cvout" | extract)
