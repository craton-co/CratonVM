...
ensure_wrapper() {
  ...
  if [ -n "$CORPUS_RUNNER_SRC" ]; then
    # CANDIDATE LIST, like CORPUS_ROOT_CANDIDATES: the first entry that exists
    # wins. All six corpora declared ONE hard-coded /data/... path, which is
    # the Linux benchmark host's layout and exists on no other machine, so on
    # this Windows box every corpus died here with "declares CORPUS_RUNNER_SRC
    # ... which does not exist" -- a fixture precondition failure that reads
    # like a corpus defect. The error below keeps the WHOLE list, because a
    # one-path message cannot say what else was tried.
    local src="" cand
    for cand in $CORPUS_RUNNER_SRC; do
      if [ -f "$cand" ]; then src="$cand"; break; fi
    done
    [ -n "$src" ] || die "corpus '$CORPUS' declares CORPUS_RUNNER_SRC='$CORPUS_RUNNER_SRC' and none of those paths exist on this host"
    local cls="$BUILD/$CORPUS_RUNNER_CLASS.class"
    if [ ! -f "$cls" ] || [ "$src" -nt "$cls" ]; then
      log "compiling $(basename "$src") -> $BUILD"
      "$JAVAC" -d "$BUILD" -cp "$runner_cp" "$src" \
        || die "javac failed on $src -- the JUnit platform launcher is probably not on this corpus's composed classpath"
    fi
  fi
}
...
#                    (main-kind only -- see the junit note inside the rc=124 arm)
...
classify_arm() {
  local rc="$1" logf="$2" cap="${3:-0}" end_epoch="${4:-0}" kind="${5:-${CORPUS_KIND:-main}}"
  ...
  if [ "$rc" -eq 124 ]; then
    ...
    elif [ "$silent" -ge "$thresh" ] && [ "$kind" = junit ]; then
      # A `junit`-kind workload runs under `SbRunner`, which registers a
      # `SummaryGeneratingListener` plus a silent collector and prints NOTHING
      # between `CORPUS-START` and its single closing `SBRUNNER_RESULT` line.
      # Silence is therefore what a junit arm does for its whole run, whatever
      # it is doing, and a killed junit arm is "silent at the wall" BY
      # CONSTRUCTION. STALLED's documented claim -- "Hung, not slow" -- cannot
      # be earned by that evidence, so it must not be made here.
      #
      # MEASURED 2026-09-22: `org.bouncycastle.pqc.crypto.test.AllTests`
      # scored CV-TIMEOUT-STALLED at a 2400 s cap, "2375s with NO output",
      # which reads as a hang. It is not one -- run class by class, all 36 of
      # that suite's classes complete with `fail=0 err=0`; the suite is simply
      # larger than the cap (HotSpot itself needs 888 s for it). The verdict is
      # still CV-BROKEN either way; what was wrong was the stated reason.
...
adjudicate() {
  ...
  if [ "$cvstate" != RAN ]; then
    ...
    case "$cvstate" in
      ...
      TIMEOUT-UNKNOWN)
        if [ "${CORPUS_KIND:-main}" = junit ]; then
          ADJ_NOTE="killed at the ${cap}s cap. This is a junit-kind workload: SbRunner prints nothing between CORPUS-START and its closing SBRUNNER_RESULT, so the ${cvsilent}s of silence is what EVERY junit arm shows and is NOT evidence of a stall. Hung vs slower-than-one-cap is UNDETERMINED -- re-run with --timeout $((cap*3)), or run the suite's classes individually, before asserting either."
        else
...
CORPUS-END X completed=true"
  ...
  sc_assert "classify TIMEOUT-STALLED (silent at the wall)" TIMEOUT-STALLED \
    "$(classify_arm 124 "$T/tmo.log" 600 "$((now+600))" main)"
  # Same rc, same log, same silence -- but a junit workload is silent for its
  # whole run by construction, so the STALLED claim is not available to it.
  sc_assert "classify junit silence is NOT a stall" TIMEOUT-UNKNOWN \
    "$(classify_arm 124 "$T/tmo.log" 600 "$((now+600))" junit)"
  # Recent output is still informative, so BUSY stays available to a junit
  # arm: silence is the one thing it cannot use as evidence, not output.
  sc_assert "classify junit keeps BUSY when it WAS writing" TIMEOUT-BUSY \
    "$(classify_arm 124 "$T/tmo.log" 600 "$now" junit)"
