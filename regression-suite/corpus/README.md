...
| `TIMEOUT-STALLED` | killed at the wall having printed **nothing** for a long time. Hung, not slow. **`main`-kind only** — see below. |
...
A `junit`-kind workload can never earn `TIMEOUT-STALLED`, and does not get it.
Those run under `SbRunner`, which registers a `SummaryGeneratingListener` and a
silent collector and prints **nothing** between `CORPUS-START` and its single
closing `SBRUNNER_RESULT` line. Silence is therefore what a junit arm does for
its whole run whatever it is doing, so "silent at the wall" is true of every
killed junit arm by construction and says nothing about whether it was stuck.
Such a row is classified `TIMEOUT-UNKNOWN`, whose note names the reason. It is
still `CV-BROKEN`; only the stated cause changes.

MEASURED 2026-09-22, which is why this rule exists:
`org.bouncycastle.pqc.crypto.test.AllTests` scored `CV-TIMEOUT-STALLED` at a
2400 s cap — "2375s with NO output" — which reads as a hang. It is not one. Run
class by class, every class of that suite completes with `fail=0 err=0`; the
suite is simply bigger than the cap, and HotSpot itself needs 888 s for it.
...
`docs/internal/jdk-only/C8-CORPUS-HARNESS-DEFECTS-20260812.md` §2.1 and the
...
`docs/internal/jdk-only/C8-CORPUS-HARNESS-DEFECTS-20260812.md`.
