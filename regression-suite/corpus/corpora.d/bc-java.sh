...
# The Linux benchmark host's path first, then this Windows dev box's. The list
# used to name the Linux path TWICE and no other, so on Windows `resolve_root`
# found nothing and the corpus died with "no BUILT root on this host" -- even
# though the tree is checked out and built at the second path below, and even
# though `corpus_classpath` already defaults `M2_REPO` to a Windows location.
# (Adding a host-specific candidate is this file format's own convention; see
# `h2.sh`, which carries `C:/craton/h2root` for the same reason.)
CORPUS_ROOT_CANDIDATES="/data/cratonvm/apps/bc-java C:/craton/cratonvm/apps/bc-java"

CORPUS_RUNNER_CLASS="SbRunner"
# Candidate list, first existing wins -- same reason as
# CORPUS_ROOT_CANDIDATES above: the Linux benchmark host, then this box.
CORPUS_RUNNER_SRC="/data/cratonvm/apps/spring-boot/sb-runner/SbRunner.java C:/craton/cratonvm/apps/spring-boot/sb-runner/SbRunner.java"
