...
record_reach() {  # record_reach <phase> <requests> <admitted> <bodies> <c1> <c2> <osr>
    ...
    #
    #    OSR is NOT subtracted. It was, from 2026-08-03, because a C2-tier
    #    OSR compile used to land in `c2=` as well as `osr=`. Since
    #    2026-09-16 the tier manager counts an OSR compile only in `osr=`
    #    (`jit/src/tiered.rs`, `CompilationTier::C2 if !c.osr`), and since
    #    2026-09-18 `osr=` counts published OSR artifacts rather than queued
    #    requests -- so `c1 + c2` is already the non-OSR count, and
    #    subtracting `osr` again under-counted it.
    _c1=${5:--}; _c2=${6:--}; _osr=${7:--}
    [ "$_c1" = "-" ] && _c1=0
    [ "$_c2" = "-" ] && _c2=0
    [ "$_osr" = "-" ] && _osr=0
