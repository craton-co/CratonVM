...
#
# OSR compiles do not pass the admission chain (`compile_osr_artifact` calls
# the backend directly), which is why `osr>0` with `requests=0` is normal for a
# phase that is one long loop inside one method. They are not in `c2=` either:
# until 2026-09-16 a C2-tier OSR compile was counted in both `c2=` and `osr=`,
# and this check subtracted `osr` to undo that (measured on the Azure bench host
# 2026-08-03, `arithmetic` read `c1=0 c2=1 osr=1` with zero admission lines).
# The tier manager now counts it only in `osr=`, so the comparison is
# `c1 + c2 > requests`.
#
...
# `c2=` excludes OSR compiles since 2026-09-16 (tiered.rs), so c1 + c2 is
# already the non-OSR count; WOSR is kept for the report only.
NONOSR=$(( WC1 + WC2 ))
