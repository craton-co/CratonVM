...
> **The harness pins its own policy.** The launcher default flipped to
> `jdk-only` on 2026-09-20, so `run.sh` now passes `--compatible` explicitly
> whenever `CRATONVM_ARGS` names no policy of its own. Without that, every
> number in this file, in `jdk-only-coverage.txt` and in the HANDOFF documents
> they cite — all measured when a bare invocation meant COMPATIBLE — would
> silently start describing a different mode. The CORE list in particular
> asserts compatibility behaviour on purpose (see the `RJdkViews` note in
> `run.sh`). A `CRATONVM_ARGS` that names `--jdk-only`, `--compatible`,
> `--real-jdk` or `--synthetic-jdk` gets nothing added.
...
- **`JDKONLY_CLASSES`** is scheduled only under `SUITE=jdk-only`/`all`,
  ...
  *expected* to fail where `--compatible` passes. A vector written to expose a
